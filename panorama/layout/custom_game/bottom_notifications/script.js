...
"use strict";
/*!*****************************************!*\
  !*** ./bottom_notifications/script.tsx ***!
  \*****************************************/
__webpack_require__.r(__webpack_exports__);
...
const RATE_LIMIT_MAX_NOTIFICATIONS = 6;
const SAFE_ASSET_NAME = /^[a-z0-9_]{1,96}$/;
const SAFE_HERO_NAME = /^npc_dota_hero_[a-z0-9_]{1,64}$/;
...
function sanitizeText(value) {
    ...
    const text = withoutControlCharacters.charAt(0) === "#"
        ? $.Localize(withoutControlCharacters)
        : withoutControlCharacters;
    return text.slice(0, MAX_TEXT_LENGTH);
}
function sanitizeIcon(payload) {
    if (typeof payload.hero === "string" && SAFE_HERO_NAME.test(payload.hero)) {
        return { type: "hero", name: payload.hero };
    }
    if (typeof payload.ability === "string" && SAFE_ASSET_NAME.test(payload.ability)) {
        return { type: "ability", name: payload.ability };
    }
    if (typeof payload.item === "string" && SAFE_ASSET_NAME.test(payload.item)) {
        return { type: "item", name: payload.item };
    }
    return undefined;
}
function sanitizeNotification(payload) {
    ...
    const icon = sanitizeIcon(payload);
...
function canShowNotification() {
    const now = Game.Time();
    recentNotificationTimes = recentNotificationTimes.filter((time) => now - time < RATE_LIMIT_WINDOW_SECONDS);
    if (recentNotificationTimes.length >= RATE_LIMIT_MAX_NOTIFICATIONS) {
        return false;
    }
    recentNotificationTimes.push(now);
    return true;
}
function NotificationIcon({ icon }) {
    ...
    switch (icon.type) {
        case "hero":
            return react__WEBPACK_IMPORTED_MODULE_1__.createElement(DOTAHeroImage, { className: "SafeBottomNotificationIcon", heroname: icon.name, heroimagestyle: "icon" });
        case "ability":
            return react__WEBPACK_IMPORTED_MODULE_1__.createElement(DOTAAbilityImage, { className: "SafeBottomNotificationIcon", abilityname: icon.name });
        case "item":
            return react__WEBPACK_IMPORTED_MODULE_1__.createElement(DOTAItemImage, { className: "SafeBottomNotificationIcon", itemname: icon.name });
    }
}
function BottomNotifications() {
    const [notifications, setNotifications] = (0,react__WEBPACK_IMPORTED_MODULE_1__.useState)([]);
    const addNotification = (0,react__WEBPACK_IMPORTED_MODULE_1__.useCallback)((payload) => {
        const notification = sanitizeNotification(payload);
        if (notification === undefined || !canShowNotification()) {
            return;
        }
        setNotifications((current) => [...current, notification].slice(-MAX_VISIBLE_NOTIFICATIONS));
        $.Schedule(DISPLAY_SECONDS, () => {
            setNotifications((current) => current.filter((entry) => entry.id !== notification.id));
        });
    }, []);
    const removeNotifications = (0,react__WEBPACK_IMPORTED_MODULE_1__.useCallback)((payload) => {
        const rawCount = typeof (payload === null || payload === void 0 ? void 0 : payload.count) === "number" && isFinite(payload.count) ? payload.count : 1;
        const count = Math.max(1, Math.min(MAX_VISIBLE_NOTIFICATIONS, Math.floor(rawCount)));
        setNotifications((current) => current.slice(0, Math.max(0, current.length - count)));
    }, []);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("top_notification", addNotification, [addNotification]);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("bottom_notification", addNotification, [addNotification]);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("top_remove_notification", removeNotifications, [removeNotifications]);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("bottom_remove_notification", removeNotifications, [removeNotifications]);
    return (react__WEBPACK_IMPORTED_MODULE_1__.createElement(Panel, { className: "SafeBottomNotifications" }, notifications.map((notification) => (react__WEBPACK_IMPORTED_MODULE_1__.createElement(Panel, { key: notification.id, className: "SafeBottomNotification" },
        react__WEBPACK_IMPORTED_MODULE_1__.createElement(NotificationIcon, { icon: notification.icon }),
        notification.text !== "" && (react__WEBPACK_IMPORTED_MODULE_1__.createElement(Label, { className: "SafeBottomNotificationText", html: false, text: notification.text })))))));
}
(0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.render)(react__WEBPACK_IMPORTED_MODULE_1__.createElement(BottomNotifications, null), $.GetContextPanel());
