...
function PingSkills(){
    ...
    // [A72] Свой герой или чужой — знает только ОТПРАВИТЕЛЬ (у получателя свой LPID),
    // поэтому шлём готовый флаг, а сам текст получатель соберёт на своём языке.
    let bOwn = Entities.GetPlayerOwnerID(unit) == LPID ? 1 : 0
    let msg = cfg.BuildSkillsPing ? cfg.BuildSkillsPing(unit, bOwn) : ""
    cfg.SendPingTextToChat(msg, undefined, { kind: "skills", unit: unit, a: bOwn })
}
