...
var SelfPVP = false;
var bPass = false;
var bFlip = Game.IsHUDFlipped();
function PVPPingAbility(Hero, index, iPlayerID) {
    let alert_msg = "";
    if (Entities.IsEnemy(Hero)) {
        alert_msg = $.Localize("#PvP_playerinfo_alert_ability_enemy").replace("[s:hero]", $.Localize("#" + Entities.GetUnitName(Hero)));
        alert_msg = alert_msg.replace("[s:ability]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
    }
    else {
        if (iPlayerID == Players.GetLocalPlayer()) {
            alert_msg = $.Localize("#PvP_playerinfo_alert_ability_self").replace("[s:ability]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
        }
        else {
            alert_msg = $.Localize("#PvP_playerinfo_alert_ability_ally").replace("[s:hero]", $.Localize("#" + Entities.GetUnitName(Hero)));
            alert_msg = alert_msg.replace("[s:ability]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
        }
    }
    alert_msg = alert_msg.replace(/<font color='#\w+'>/g, "");
    alert_msg = alert_msg.replace(/<\/font>/g, "");
    Game.ServerCmd("say " + alert_msg);
}
function PVPPingItem(Hero, index, iPlayerID) {
    if (GameUI.IsAltDown() && (index != -1)) {
        let alert_msg = "";
        if (Entities.IsEnemy(Hero)) {
            alert_msg = $.Localize("#PvP_playerinfo_alert_item_enemy").replace("[s:hero]", $.Localize("#" + Entities.GetUnitName(Hero)));
            alert_msg = alert_msg.replace("[s:item]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
        }
        else {
            if (iPlayerID == Players.GetLocalPlayer()) {
                alert_msg = $.Localize("#PvP_playerinfo_alert_item_self").replace("[s:item]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
            }
            else {
                alert_msg = $.Localize("#PvP_playerinfo_alert_item_ally").replace("[s:hero]", $.Localize("#" + Entities.GetUnitName(Hero)));
                alert_msg = alert_msg.replace("[s:item]", $.Localize("#DOTA_Tooltip_ability_" + Abilities.GetAbilityName(index)));
            }
        }
        alert_msg = alert_msg.replace(/<font color='#\w+'>/g, "");
...
function PVP() {
    ...
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("ShowPvpBet", (event) => {
        if (event != undefined) {
            if (event.dataList) {
                let List = {};
                let bWillPVP = false;
                for (const iTeamNumber in event.dataList) {
                    const element = event.dataList[iTeamNumber];
                    if (List[iTeamNumber] == undefined) {
                        List[iTeamNumber] = [];
                    }
                    if (Number(iTeamNumber) == Players.GetTeam(Players.GetLocalPlayer())) {
                        bWillPVP = true;
                    }
                    for (const j in element) {
                        const iPlayerInfo = element[j];
                        if (iPlayerInfo != undefined) {
                            List[iTeamNumber].push(iPlayerInfo);
                        }
                    }
                }
                SelfPVP = bWillPVP;
    ...
    if (Record) {
        let History = Record === null || Record === void 0 ? void 0 : Record.bet_history;
        BetTotalReward = (_b = Record === null || Record === void 0 ? void 0 : Record.total_bet_reward) !== null && _b !== void 0 ? _b : 0;
        if (History) {
...
function PVPTeam({ teamID, teamIndex, teamInfo, BetTime, Show, ShowBet, BetSummary, SetShowBet, }) {
    ...
    function OnBetNumberChange(bet_number) {
        const player_gold = Players.GetGold(Players.GetLocalPlayer());
        if (bet_number > player_gold * 0.5) {
            if (refBetTextEntry.current) {
        ...
        if (bet_number < 0) {
            if (refBetTextEntry.current) {
                refBetTextEntry.current.text = String(0);
            }
        }
    }
            ...
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "TeamMain" },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "TeamLeft" },
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "TeamHeroList" }, teamInfo.map((info, i) => {
                        ...
                        let Hero = Players.GetPlayerHeroEntityIndex(iPlayerID);
                        if (Hero) {
                            let MaxHP = Entities.GetMaxHealth(Hero);
                            let MaxMana = Entities.GetMaxMana(Hero);
                            let HP = Entities.GetHealth(Hero);
                            let Mana = Entities.GetMana(Hero);
                            let AbilityList = [];
                            let ItemList = [];
                            let NeutralItem = Entities.GetItemInSlot(Hero, 16);
                            let SmokeItem = Entities.GetItemInSlot(Hero, 15);
                            ...
                            for (let index = 0; index < Entities.GetAbilityCount(Hero); index++) {
                                ...
                                if (ability) {
                                    if (Abilities.GetAbilityType(ability) == 2)
                                        continue;
                                    if (!Abilities.IsHidden(ability) && ability != -1) {
                                    ...
                                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "Party_info", visible: bPass },
                                        ...
                                        (() => {
                                            ...
                                            if (party_map != undefined) {
                                                let party_id = party_map[iPlayerID];
                                                if (party_id) {
                                                    for (const key in party_map) {
                                                        if (party_map[key] == party_id && Number(key) != iPlayerID) {
                                                            let partyPlayerInfo = Game.GetPlayerInfo(Number(key));
                                                            if (partyPlayerInfo) {
                                                                party_list.push({ playername: partyPlayerInfo.player_name, heroname: partyPlayerInfo.player_selected_hero });
                                                            }
                                                        }
                                                    }
                                                }
                    ...
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "BetList", visibility: SelfPVP ? "collapse" : "visible" }, BetSummary ?
                        Object.keys(BetSummary).map((v, i) => {
                            ...
                            const iPlayerID = (_a = Info === null || Info === void 0 ? void 0 : Info.nPlayerId) !== null && _a !== void 0 ? _a : -1;
