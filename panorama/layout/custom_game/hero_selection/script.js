...
for (let index = 0; index < Players.GetMaxPlayers(); index++) {
    if (Game.GetPlayerInfo(index) != undefined) {
        player_count = player_count + 1;
    }
}
const BAN_SLOT_LIMIT = 36;
const PHASE_ID = {
    NONE: 0,
    BAN: 1,
    SELECT: 2,
...
const BonusStatsLocMap = new Map([
    ["speed", ["DOTA_DerivedStatsHelp_Tooltip_Movespeed", "DOTA_DerivedStatsHelp_Tooltip_Movespeed_Desc"]],
    ["attack_speed2", ["DOTA_DerivedStatsHelp_Tooltip_BaseAttackTime", "DOTA_DerivedStatsHelp_Tooltip_BaseAttackTime_Desc"]],
    ["projectile_speed", ["DOTA_HeroStats_ProjectileSpeed_Name", "DOTA_HeroStats_ProjectileSpeed_Desc"]],
    ["attack_range", ["DOTA_DerivedStatsHelp_Tooltip_AttackRange", "DOTA_DerivedStatsHelp_Tooltip_AttackRange_Desc"]],
]);
function GetHeroNameByAbility(ability_name) {
    let HeroAbility = GameUI.CustomUIConfig().HeroAbilityKv;
    for (const hero_name in HeroAbility) {
        if (ability_name.match(hero_name) != null) {
            return hero_name;
        }
    }
}
function BanPhase() {
    ...
    let AttributeType = ["STRENGTH", "AGILITY", "INTELLECT", "ALL"];
    for (const HeroName in HeroList) {
        const hero_info = Heroes[HeroName];
        if (hero_info != undefined) {
            let AttributePrimary = hero_info.AttributePrimary.replace("DOTA_ATTRIBUTE_", "");
            HeroGroups[AttributePrimary].push(HeroName);
        }
    }
    let CurrentHeroInfo = {};
    if (CurrentHero != "") {
        CurrentHeroInfo.HeroID = Heroes[CurrentHero].HeroID;
        CurrentHeroInfo.AttributePrimary = Heroes[CurrentHero].AttributePrimary.replace("DOTA_ATTRIBUTE_", "").toLowerCase();
        CurrentHeroInfo.AttributeBaseStrength = Heroes[CurrentHero].AttributeBaseStrength;
        CurrentHeroInfo.AttributeStrengthGain = Heroes[CurrentHero].AttributeStrengthGain;
        CurrentHeroInfo.AttributeBaseIntellect = Heroes[CurrentHero].AttributeBaseIntelligence;
        CurrentHeroInfo.AttributeIntellectGain = Heroes[CurrentHero].AttributeIntelligenceGain;
        CurrentHeroInfo.AttributeBaseAgility = Heroes[CurrentHero].AttributeBaseAgility;
        CurrentHeroInfo.AttributeAgilityGain = Heroes[CurrentHero].AttributeAgilityGain;
        CurrentHeroInfo.AttackCapabilities = Heroes[CurrentHero].AttackCapabilities;
        CurrentHeroInfo.speed = Heroes[CurrentHero].MovementSpeed;
        CurrentHeroInfo.attack_range = Heroes[CurrentHero].AttackRange;
        CurrentHeroInfo.projectile_speed = CurrentHeroInfo.AttackCapabilities == "DOTA_UNIT_CAP_MELEE_ATTACK" ? "" : ((_a = Heroes[CurrentHero].ProjectileSpeed) !== null && _a !== void 0 ? _a : "");
        CurrentHeroInfo.attack_speed2 = Heroes[CurrentHero].AttackRate;
    }
    let ban_abilities = [];
    if (ban_abilities_obj != undefined) {
        for (const i in ban_abilities_obj) {
            ban_abilities.push(ban_abilities_obj[i]);
        }
    }
    let ban_heroes = [];
    if (ban_heroes_obj != undefined) {
        for (const i in ban_heroes_obj) {
            ban_heroes.push(ban_heroes_obj[i]);
        }
    }
    let CurrentHeroBanned = ban_heroes.indexOf(CurrentHero) != -1;
    let BanHeroTimes = 0;
    let BanAbilityTimes = 0;
    let PlayerBanHero = [];
    let PlayerBanAbility = [];
    if (banTimes) {
        if (banTimes.banHero) {
            for (const i in banTimes.banHero) {
                let banpos = banTimes.banHero[i];
                if (banpos == "") {
                    BanHeroTimes++;
                }
                PlayerBanHero.push(banpos);
            }
        }
        if (banTimes.banAbility) {
            for (const i in banTimes.banAbility) {
                let banpos = banTimes.banAbility[i];
                if (banpos == "") {
                    BanAbilityTimes++;
                }
                PlayerBanAbility.push(banpos);
            }
        }
    }
    let RecentBan_abilities = [];
    let RecentBan_heroes = [];
    if ((RecentBan_abilities_table === null || RecentBan_abilities_table === void 0 ? void 0 : RecentBan_abilities_table[String(Players.GetLocalPlayer())]) != undefined) {
        let player_ban_ability = RecentBan_abilities_table === null || RecentBan_abilities_table === void 0 ? void 0 : RecentBan_abilities_table[String(Players.GetLocalPlayer())];
        for (const i in player_ban_ability) {
            RecentBan_abilities.push(player_ban_ability[i].ability_name);
        }
    }
    if ((RecentBan_heroes_table === null || RecentBan_heroes_table === void 0 ? void 0 : RecentBan_heroes_table[String(Players.GetLocalPlayer())]) != undefined) {
        let player_ban_hero = RecentBan_heroes_table === null || RecentBan_heroes_table === void 0 ? void 0 : RecentBan_heroes_table[String(Players.GetLocalPlayer())];
        for (const i in player_ban_hero) {
            RecentBan_heroes.push(player_ban_hero[i].hero_name);
        }
    }
    let select_done = 1;
    let RerollCount = 0;
    let allherolist = [];
    if (HeroSelectInfo != undefined) {
        let page = (_b = HeroSelectInfo.page) !== null && _b !== void 0 ? _b : 1;
        let herolist = (_c = HeroSelectInfo.herolist[page]) !== null && _c !== void 0 ? _c : [];
        select_done = (_d = HeroSelectInfo.select_done) !== null && _d !== void 0 ? _d : 0;
        RerollCount = (_e = HeroSelectInfo.reroll_count) !== null && _e !== void 0 ? _e : 0;
        if (select_done == 0) {
            for (const index in herolist) {
                const heroID = herolist[index];
                allherolist.push(heroID);
            }
        }
    }
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("UpdatePhaseTime", (data) => {
        ...
        if (time >= 0) {
            ...
            let sec = time - min * 60;
            if (data.phase == PHASE_ID.BAN || data.phase == PHASE_ID.SELECT) {
                SetPhaseTime({ min: min, sec: sec, phase: data.phase });
            }
        }
    }, []);
    let TeamPlayerList = [];
    ...
    for (const TeamID of AllTeamIDs) {
        ...
        for (const Playerid of TeamPlayers) {
            let thisTeam = TeamPlayerList.find((value) => { return value.TeamID == TeamID; });
            if (thisTeam != undefined) {
                thisTeam.TeamPlayers.push(Playerid);
            }
            else {
                TeamPlayerList.push({ TeamID: TeamID, TeamPlayers: [Playerid] });
            }
        }
    }
    ...
    return react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "BanPhase", hittest: true, onactivate: () => { } },
        ...
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "Main", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Show: PhaseTime.sec >= 0 || PhaseTime.min >= 0 }) },
            ...
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "SelectPhaseMain", visibility: PhaseTime.phase == PHASE_ID.SELECT ? "visible" : "collapse" }, allherolist.map((v, i) => {
                ...
                heroInfo.AttributePrimary = Heroes[heroName].AttributePrimary.replace("DOTA_ATTRIBUTE_", "").toLowerCase();
                heroInfo.AttributeBaseStrength = Heroes[heroName].AttributeBaseStrength;
                heroInfo.AttributeStrengthGain = Heroes[heroName].AttributeStrengthGain;
                heroInfo.AttributeBaseIntellect = Heroes[heroName].AttributeBaseIntelligence;
                heroInfo.AttributeIntellectGain = Heroes[heroName].AttributeIntelligenceGain;
                heroInfo.AttributeBaseAgility = Heroes[heroName].AttributeBaseAgility;
                heroInfo.AttributeAgilityGain = Heroes[heroName].AttributeAgilityGain;
                heroInfo.AttackCapabilities = Heroes[heroName].AttackCapabilities;
                heroInfo.speed = Heroes[heroName].MovementSpeed;
                heroInfo.attack_range = Heroes[heroName].AttackRange;
            ...
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "BanPhaseMain", visibility: PhaseTime.phase == PHASE_ID.BAN ? "visible" : "collapse" },
                ...
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(HeroInfoCard, Object.assign({}, {
                    ...
                    ban_abilities: ban_abilities,
                    CurrentHeroBanned: CurrentHeroBanned,
                    BanHeroTimes: BanHeroTimes,
                    BanAbilityTimes: BanAbilityTimes,
                    CurrentSelectAbility: CurrentSelectAbility,
                    SetFunction: (sAbility) => {
                        SetCurrentSelectAbility(sAbility);
                    },
                    phase: PhaseTime.phase,
                }))),
...
function HeroInfoCard(_a) {
    ...
    let CurrentSelectAbility = (_e = data.CurrentSelectAbility) !== null && _e !== void 0 ? _e : "";
    let BanHeroTimes = (_f = data.BanHeroTimes) !== null && _f !== void 0 ? _f : 0;
    let BanAbilityTimes = (_g = data.BanAbilityTimes) !== null && _g !== void 0 ? _g : 0;
    let HeroAbility = GameUI.CustomUIConfig().HeroAbilityKv;
    let HeroInnateAbility = GameUI.CustomUIConfig().InnateAbilitiesKv;
    let ban_abilities = (_h = data.ban_abilities) !== null && _h !== void 0 ? _h : [];
    let SetFunction = (_j = data.SetFunction) !== null && _j !== void 0 ? _j : (() => { });
    let phase = (_k = data.phase) !== null && _k !== void 0 ? _k : PHASE_ID.NONE;
    let PlayerHeroID = Players.GetSelectedHeroID(Players.GetLocalPlayer());
    let selectDone = PlayerHeroID != 0;
    let InnateAbility = HeroInnateAbility[heroName];
    let bFreeSelect = false;
    if (Game.IsInToolsMode() || Game.GetConvarInt("sv_cheats") == 1) {
        bFreeSelect = true;
    }
    else if (player_count == 1) {
        if ((passData === null || passData === void 0 ? void 0 : passData[String(Players.GetLocalPlayer())]) && (passData === null || passData === void 0 ? void 0 : passData[String(Players.GetLocalPlayer())].level) == 1) {
            bFreeSelect = true;
        }
    }
...
function StrategyPhase() {
    ...
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useGameEvent)("UpdatePhaseTime", (data) => {
        ...
        if (time >= 0) {
            let min = Number(Math.floor(time / 60));
            let sec = time - min * 60;
            if (data.phase == PHASE_ID.STRATEGY) {
                if (min > 0 || sec > 0) {
                    if (!Show) {
                        SetShow(true);
                    }
    ...
    function GetSkinList() {
        ...
        if (SkinData != undefined) {
            if (SkinData[String(Players.GetLocalPlayer())] != undefined) {
                for (const id in SkinData[String(Players.GetLocalPlayer())]) {
                    const SkinInfo = SkinData[String(Players.GetLocalPlayer())][id];
                    if (SkinInfo.tag == heroName) {
        ...
        if (PhaseTime.phase == PHASE_ID.STRATEGY && (PhaseTime.min > 0 || PhaseTime.sec > 0)) {
            if (SkinList != undefined) {
                ...
                if (SkinInfo != undefined) {
                    if (SkinInfo.is_have == 1 && SkinInfo.is_equip == 0) {
                        if (SkinInfo.id != undefined) {
                            if (SkinInfo.id == "") {
                                GameEvents.SendCustomEventToServer("UnEquipItemNew", {
                                    id: SkinInfo.id,
                                    tag: heroName,
                                    item_type: "skin",
                                });
                                ...
                                GameEvents.SendCustomEventToServer("EquipItemNew", {
                                    id: SkinInfo.id,
                                    tag: heroName,
                                    item_type: "skin",
                                });
    ...
    let TitleText = $.Localize("#" + heroName);
    if (SkinList[CurrentSkin].id != "") {
                    ...
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "HeroSkin" },
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "HeroSkinList" },
                            ...
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "List" }, SkinList.map((SkinInfo, index) => {
                                ...
                                if (Selected) {
                                    PositionClassName = "Selected";
                                }
                                else if (CurrentSkin < index) {
                                    PositionClassName = "LeftofSelected";
                                }
                                else {
...
(function () {
    if (PreGame != null) {
        ...
        if (MainContents) {
            MainContents.style.zIndex = 20;
        }
        var StrategyScreen = PreGame.FindChildTraverse("StrategyScreen");
        if (StrategyScreen != null) {
            for (let p of StrategyScreen.Children()) {
                if (!p.BHasClass("SelectedHeroFacets")) {
                    p.visible = false;
                }
            }
        }
        var BottomPanelsContainer = PreGame.FindChildTraverse("BottomPanelsContainer");
        if (BottomPanelsContainer != null) {
            BottomPanelsContainer.style.opacity = "0";
        }
        var BacktoHeroGrid = PreGame.FindChildTraverse("BacktoHeroGrid");
        if (BacktoHeroGrid != null) {
            BacktoHeroGrid.style.width = "0px";
        }
    }
    if (PreGame != null) {
