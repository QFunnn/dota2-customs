...
let pRightNotificationContianer = $("#RightNotificationContianer");
let pMonsterNotificationContianer = $("#MonsterNotificationContianer");
let pPvPLoseNotificationContianer = $("#PvPLoseNotificationContianer");
function ReplacePlayerIcon(panel, playerID) {
    $.Schedule(0, function () {
        var _a;
        let heroIconPanels = ((_a = panel.FindChildTraverse("NotificationLabel")) === null || _a === void 0 ? void 0 : _a.FindChildrenWithClassTraverse("CombatEventHeroPlayer_" + playerID)) || [];
...
function ReplaceHeroIcon(panel, playerID) {
    $.Schedule(0, function () {
        ...
        for (let index = 0; index < heroIconPanels.length; index++) {
            let pHeroIconPanel = heroIconPanels[index];
            let tPlayerInfo = Game.GetPlayerInfo(playerID);
            let hero = Entities.GetUnitName(Players.GetPlayerHeroEntityIndex(playerID));
            pHeroIconPanel.RemoveClass("CombatEventHeroPlayer_" + playerID);
            pHeroIconPanel.BLoadLayoutSnippet("CombatEventHeroIcon");
            if (tPlayerInfo) {
                let pPlayerAvatarImage = pHeroIconPanel.FindChildTraverse("PlayerHeroImage");
                if (pPlayerAvatarImage) {
                    pPlayerAvatarImage.heroname = hero;
                }
            }
        }
    });
...
function ReplaceItemIcon(panel, itemName) {
    $.Schedule(0, function () {
        var _a;
        let itemIconPanels = ((_a = panel.FindChildTraverse("NotificationLabel")) === null || _a === void 0 ? void 0 : _a.FindChildrenWithClassTraverse("CombatEventItem_" + itemName)) || [];
        for (let index = 0; index < itemIconPanels.length; index++) {
            let pItemIconPanel = itemIconPanels[index];
            pItemIconPanel.RemoveClass("CombatEventItem_" + itemName);
            pItemIconPanel.AddClass("CombatEventItemIcon");
            (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.render)(react__WEBPACK_IMPORTED_MODULE_1__.createElement(DOTAItemImage, { id: "ItemImage", itemname: itemName }), pItemIconPanel);
        }
    });
}
function CreateNotification(tParams, parent, id) {
    ...
    if (typeof (tParams.player_id) == "number" && Players.IsValidPlayerID(tParams.player_id)) {
        let iHeroEntIndex = Players.GetPlayerHeroEntityIndex(tParams.player_id);
        ...
        panel.SetDialogVariable("player_name", "<panel class='CombatEventHeroIcon CombatEventHeroPlayer_" + tParams.player_id + "' />" + "<font color='#" + sPlayerColor + "'>" + sPlayerName + "</font>");
    ...
    if (typeof (tParams.player_id2) == "number") {
        let iHeroEntIndex = Players.GetPlayerHeroEntityIndex(tParams.player_id2);
    ...
    for (let key in tParams) {
        ...
        if (key.indexOf("b_int") != -1) {
            ...
            panel.SetDialogVariable(key, $.Localize("#" + value) == "#" + value ? value : $.Localize("#" + value));
            if (key.indexOf("itemname") != -1) {
                panel.SetDialogVariable(key, "<panel class='CombatEventItemIcon CombatEventItem_" + value + "' />" + $.Localize("#DOTA_Tooltip_Ability_" + value));
    ...
    if (pNotificationLabel) {
        ...
        pNotificationLabel.text = $.Localize("#" + tParams.message, panel) == "#" + tParams.message ? tParams.message : $.Localize("#" + tParams.message, panel);
    }
    return panel;
}
let UpperNotificationSeq = new RunSequentialActions();
...
let combat_events;
let CombatNotificationTime = 10.0;
...
function OnNotificationMonster(tParams) {
    let panel = $.CreatePanel("Panel", pMonsterNotificationContianer, "");
    CreateNotification(tParams, panel, "");
    panel.AddClass("Combat");
    let iconPanel = $.CreatePanel("Image", panel, "");
    iconPanel.AddClass("CombatIcon");
...
function OnNotificationPvPLose(tParams) {
    let panel = $.CreatePanel("Panel", pPvPLoseNotificationContianer, "");
    CreateNotification(tParams, panel, "");
    panel.AddClass("Combat");
    let iconPanel = $.CreatePanel("Image", panel, "");
    iconPanel.AddClass("CombatIcon");
...
function RightNotification(tParams) {
    tParams.teamnumber = undefined;
    let panel = $.CreatePanel("Panel", pRightNotificationContianer, "");
    CreateNotification(tParams, panel, "");
    pRightNotificationContianer.QueueToast(panel);
}
function update() {
    ...
    if (tParams.message != undefined && tParams.message != null) {
        for (let key in tParams) {
            ...
            if (key.indexOf("b_int_") != -1) {
                ...
                Label.SetDialogVariable(key, $.Localize("#" + value) == "#" + value ? value : $.Localize("#" + value));
            }
        }
        sMessage = $.Localize("#" + tParams.message, Label) == "#" + tParams.message ? tParams.message : $.Localize("#" + tParams.message, Label);
    }
    Label.SetDialogVariable("message", sMessage);
    let sText = $.Localize("#DOTA_ChatMessage_HudAll", Label);
    if (tParams.team_only == 1) {
...
function OnNotificationCloseHelpTip(tParams) {
    if (tParams.sPanelID && tParams.sPanelID != "") {
        let panel = $("#HelpTipContainer").FindChildTraverse(tParams.sPanelID);
        if (panel) {
            let iScheduleID = LoadData(panel, "iScheduleID");
            if (iScheduleID) {
...
(function () {
    var _a, _b, _c, _d;
    let HUD = (_c = (_b = (_a = $.GetContextPanel()) === null || _a === void 0 ? void 0 : _a.GetParent()) === null || _b === void 0 ? void 0 : _b.GetParent()) === null || _c === void 0 ? void 0 : _c.GetParent();
    if (HUD != undefined && HUD != null) {
        combat_events = HUD.FindChildTraverse("combat_events");
        ChatLinesPanel = ((_d = HUD.FindChildTraverse("HudChat")) === null || _d === void 0 ? void 0 : _d.FindChildTraverse("ChatLinesPanel")) || null;
    }
    update();
