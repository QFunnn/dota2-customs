...
/* harmony export */ });
/* unused harmony exports createPortal, useGameEvent, useNetTableValues, useRegisterForUnhandledEvent */
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "SeasonRuleset": () => (/* binding */ SeasonRuleset)
/* harmony export */ });
/**
 * 可由实体 `GetKeyValue` 覆盖的怪物字段。
 * 掉落池数组由掉落管理器直接读取完整行，避免把数组伪装成原生 KV 标量。
 */
const RULESET_MONSTER_KEY_VALUE_FIELDS = {
...
class SeasonRuleset {
    constructor(snapshot) {
        var _a;
        this.activeRulesetId = snapshot.rulesetId;
        this.activeTables = snapshot.tables;
        this.activeLocalizationTokens = (_a = snapshot.localizationTokens) !== null && _a !== void 0 ? _a : {};
        this.activeLocalizationTokensByLowerCase = {};
        for (const [baseToken, resolvedToken] of Object.entries(this.activeLocalizationTokens)) {
            this.activeLocalizationTokensByLowerCase[baseToken.toLowerCase()] = resolvedToken;
        }
    }
    GetActiveRulesetId() {
        return this.activeRulesetId;
    }
    GetTable(tableName) {
        return this.activeTables[tableName];
    }
    /** 仅查询装备表 `ak_items_equip`。 */
    GetEquipmentConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_items_equip')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 仅查询普通物品表，不与药剂、图纸或其他物品域合并。 */
    GetCommonItemConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_items')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 仅查询药剂表，不与普通物品或装备表合并。 */
    GetPotionConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_items_potion')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 仅查询图纸表；返回行的 `item` 指向实际产物。 */
    GetFormulaConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_items_formula')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 仅查询变更物品表，避免与同名普通物品产生隐式合并。 */
    GetItemChangeConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_items_change')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 读取当前赛季完整职业阶级表。 */
    GetProfessionTable() {
        return this.GetTable('ak_profession');
    }
    /** 按职业阶级行 ID 读取当前赛季职业配置。 */
    GetProfessionConfig(rowId) {
        var _a;
        return (_a = this.GetProfessionTable()) === null || _a === void 0 ? void 0 : _a[rowId];
    }
    /** 读取当前赛季完整标签数值修正规则表。 */
    GetTagModifierRulesTable() {
        return this.GetTable('ak_tag_modifier_rules');
    }
    /** 按规则 ID 读取当前赛季标签数值修正规则。 */
    GetTagModifierRuleConfig(ruleId) {
        var _a;
        return (_a = this.GetTagModifierRulesTable()) === null || _a === void 0 ? void 0 : _a[ruleId];
    }
    /** 读取当前赛季完整天赋树表。 */
    GetTalentTable() {
        return this.GetTable('ak_talent');
    }
    /** 按天赋 ID 读取当前赛季完整配置。 */
    GetTalentConfig(talentId) {
        var _a;
        return (_a = this.GetTalentTable()) === null || _a === void 0 ? void 0 : _a[talentId];
    }
    /** 读取当前赛季完整房间难度模式表。 */
    GetRoomDifficultyModeTable() {
        return this.GetTable('ak_room_difficulty_modes');
    }
    /** 按模式 ID 读取当前赛季的房间难度配置。 */
    GetRoomDifficultyModeConfig(modeId) {
        var _a;
        return (_a = this.GetRoomDifficultyModeTable()) === null || _a === void 0 ? void 0 : _a[modeId];
    }
    /** 读取当前赛季完整怪物实体表。 */
    GetMonsterTable() {
        return this.GetTable('ak_monster');
    }
    /** 按单位名读取当前赛季怪物完整行。 */
    GetMonsterConfig(unitName) {
        var _a;
        return (_a = this.GetMonsterTable()) === null || _a === void 0 ? void 0 : _a[unitName];
    }
    /**
     * 仅返回允许替代实体 KV 的怪物标量字段。
     * 原生 NPC 壳字段和数组型掉落字段必须由专用流程处理，不能通过此入口隐式替换。
     */
    ResolveMonsterKeyValue(unitName, key) {
        var _a;
        if (!RULESET_MONSTER_KEY_VALUE_FIELDS[key]) {
            return undefined;
        }
        const value = (_a = this.GetMonsterConfig(unitName)) === null || _a === void 0 ? void 0 : _a[key];
        return typeof value === 'string' || typeof value === 'number' ? value : undefined;
    }
    /**
     * 查询可制作产物行。
     *
     * 这里保持制作系统既有的合并顺序：宝石、装备、药剂、普通物品依次具有更高优先级。
     * 图纸只是产物关系入口，变更物品也不是制作产物，因此两者不参与此查询。
     */
    GetCraftItemConfig(itemName) {
        var _a, _b, _c;
        return ((_c = (_b = (_a = this.GetGemConfig(itemName)) !== null && _a !== void 0 ? _a : this.GetEquipmentConfig(itemName)) !== null && _b !== void 0 ? _b : this.GetPotionConfig(itemName)) !== null && _c !== void 0 ? _c : this.GetCommonItemConfig(itemName));
    }
    /**
     * 查询原生物品实体实际对应的完整行。
     *
     * 同名物品只选择一张完整表，不能把不同 CSV 的字段拼接在一起。顺序与
     * npc_items_custom.txt 的 #base 覆盖结果一致：后声明的表优先，保证脚本
     * 路由与原生 KV 壳对同名 ID 的归属一致。
     */
    GetItemConfig(itemName) {
        var _a, _b, _c, _d, _e;
        return ((_e = (_d = (_c = (_b = (_a = this.GetGemConfig(itemName)) !== null && _a !== void 0 ? _a : this.GetItemChangeConfig(itemName)) !== null && _b !== void 0 ? _b : this.GetFormulaConfig(itemName)) !== null && _c !== void 0 ? _c : this.GetEquipmentConfig(itemName)) !== null && _d !== void 0 ? _d : this.GetPotionConfig(itemName)) !== null && _e !== void 0 ? _e : this.GetCommonItemConfig(itemName));
    }
    GetAbilityConfig(abilityName) {
        var _a, _b, _c;
        return ((_b = (_a = this.GetTable('ak_abilities')) === null || _a === void 0 ? void 0 : _a[abilityName]) !== null && _b !== void 0 ? _b : (_c = this.GetTable('ak_monster_abilities')) === null || _c === void 0 ? void 0 : _c[abilityName]);
    }
    GetGemConfig(itemName) {
        var _a;
        return (_a = this.GetTable('ak_gems')) === null || _a === void 0 ? void 0 : _a[itemName];
    }
    /** 读取技能完整行的顶层标量字段；对象和数组仍由专用 resolver 处理。 */
    ResolveAbilityTopLevelValue(abilityName, key) {
        var _a;
        const value = (_a = this.GetAbilityConfig(abilityName)) === null || _a === void 0 ? void 0 : _a[key];
        return typeof value === 'string' || typeof value === 'number' ? value : undefined;
    }
    ResolveItemAbilityValues(itemName) {
        const itemConfig = this.GetItemConfig(itemName);
        if (!itemConfig) {
            return { managed: false, values: {} };
        }
        return { managed: true, values: itemConfig.AbilityValues || {} };
    }
    ResolveAbilityValues(abilityName) {
        const abilityConfig = this.GetAbilityConfig(abilityName);
        if (!abilityConfig) {
            return { managed: false, values: {} };
        }
        return { managed: true, values: abilityConfig.AbilityValues || {} };
    }
    /** 按当前赛季解析本地化 token；未覆盖时保持基础 token。 */
    ResolveLocalizationToken(baseToken) {
        var _a;
        const normalizedBaseToken = baseToken.startsWith('#') ? baseToken.slice(1) : baseToken;
        // 原生本地化 token 不区分大小写，兼容历史调用中的 DOTA/dota 写法。
        const resolvedToken = (_a = this.activeLocalizationTokens[normalizedBaseToken]) !== null && _a !== void 0 ? _a : this.activeLocalizationTokensByLowerCase[normalizedBaseToken.toLowerCase()];
        return typeof resolvedToken === 'string' && resolvedToken.length > 0 ? resolvedToken : normalizedBaseToken;
    }
}
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "localizeToken": () => (/* binding */ localizeToken)
/* harmony export */ });
/* unused harmony exports resolveRulesetLocalizationToken, localizeRulesetToken, tryLocalizeRulesetToken, localizeTemplateText */
/* harmony import */ var _ruleset_data__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./ruleset_data */ "./utils/ruleset_data.ts");

function localizeToken(token, params) {
    ...
    for (const name in params) {
        ...
        const replacement = typeof value === 'string' && value.startsWith('#') ? $.Localize(value) : `${value}`;
        text = text.split(`{${name}}`).join(replacement);
    }
    return text;
}
/** 根据当前对局的 Ruleset 映射选择实际本地化 token；未就绪或未覆盖时保留基础 token。 */
function resolveRulesetLocalizationToken(token) {
    var _a, _b;
    const hasPrefix = token.startsWith('#');
    const baseToken = hasPrefix ? token.slice(1) : token;
    const resolvedToken = (_b = (_a = (0,_ruleset_data__WEBPACK_IMPORTED_MODULE_0__.tryGetFrontendSeasonRuleset)()) === null || _a === void 0 ? void 0 : _a.ResolveLocalizationToken(baseToken)) !== null && _b !== void 0 ? _b : baseToken;
    return hasPrefix ? `#${resolvedToken}` : resolvedToken;
}
/** 读取当前赛季的本地化；映射只负责选 token，不按返回值切换到其他赛季。 */
function localizeRulesetToken(token, params) {
    return localizeToken(resolveRulesetLocalizationToken(token), params);
}
/** 有翻译时返回文案；赛季 token 缺失时不回退到基础 token。 */
function tryLocalizeRulesetToken(token, params) {
    const resolvedToken = resolveRulesetLocalizationToken(token);
    const localized = localizeToken(resolvedToken, params);
    const expectedToken = resolvedToken.startsWith('#') ? resolvedToken : `#${resolvedToken}`;
    return localized === expectedToken || localized.length === 0 ? undefined : localized;
}
function localizeTemplateText(text, fallback = '') {
    if (!text || text.length <= 0)
        return fallback;
    if (text.startsWith('#'))
        return $.Localize(text);
    return text.replace(/\$\{(#[^}]+)\}/g, (_match, token) => $.Localize(token));
}


/***/ }),

/***/ "./utils/ruleset_data.ts":
/*!*******************************!*\
  !*** ./utils/ruleset_data.ts ***!
  \*******************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "tryGetFrontendSeasonRuleset": () => (/* binding */ tryGetFrontendSeasonRuleset)
/* harmony export */ });
/* unused harmony exports initializeFrontendSeasonRuleset, disposeFrontendSeasonRuleset, getFrontendSeasonRulesetState, subscribeFrontendSeasonRuleset */
/* harmony import */ var _shared_season_ruleset__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../shared/season_ruleset */ "./shared/season_ruleset.ts");

const CUSTOM_GAME_SETUP_STATE = 2;
const RULESET_ID_PATTERN = /^[a-zA-Z0-9][a-zA-Z0-9_-]*$/;
...
function createStore() {
    return {
        version: 1,
        started: false,
        state: { status: 'waiting_state_2' },
        listeners: [],
    };
}
function getExistingStore() {
    const store = getCustomUIConfig().__AK_FRONTEND_SEASON_RULESET_STORE__;
    return (store === null || store === void 0 ? void 0 : store.version) === 1 ? store : undefined;
}
function getStore() {
    const config = getCustomUIConfig();
    const existing = config.__AK_FRONTEND_SEASON_RULESET_STORE__;
    if ((existing === null || existing === void 0 ? void 0 : existing.version) === 1) {
        return existing;
    }
    const store = createStore();
    config.__AK_FRONTEND_SEASON_RULESET_STORE__ = store;
    return store;
}
...
function getCurrentGameState() {
    try {
        const state = Number(Game.GetState());
...
function getCurrentMapName() {
    var _a;
    try {
        const rawMapName = (_a = Game.GetMapInfo()) === null || _a === void 0 ? void 0 : _a.map_name;
        if (typeof rawMapName !== 'string' || rawMapName.length === 0) {
            return undefined;
        }
        return rawMapName.replace('maps/', '').replace('.vpk', '');
    }
    catch (_error) {
        return undefined;
    }
}
function parseRawLocalization(rawLocalization) {
    if (!isRecord(rawLocalization)) {
        return undefined;
    }
    if (!isContentHash(rawLocalization.contentHash)) {
        return undefined;
    }
    if (!isRecord(rawLocalization.tokens)) {
        return undefined;
    }
    const tokens = {};
    for (const [baseToken, resolvedToken] of Object.entries(rawLocalization.tokens)) {
        if (baseToken.length === 0 ||
            baseToken.startsWith('#') ||
            typeof resolvedToken !== 'string' ||
            resolvedToken.length === 0 ||
            resolvedToken.startsWith('#')) {
            return undefined;
        }
        tokens[baseToken] = resolvedToken;
    }
    return {
        contentHash: rawLocalization.contentHash,
        tokens,
    };
}
function parseRawManifest(rawManifest) {
    if (!isRecord(rawManifest) || rawManifest.version !== 2 || !Array.isArray(rawManifest.seasons)) {
        return undefined;
    }
    const seasons = [];
    for (const rawSeason of rawManifest.seasons) {
        if (!isRecord(rawSeason) || typeof rawSeason.id !== 'string' || !RULESET_ID_PATTERN.test(rawSeason.id)) {
            return undefined;
        }
        if (!Array.isArray(rawSeason.maps) || rawSeason.maps.some(mapName => typeof mapName !== 'string' || mapName.length === 0)) {
            return undefined;
        }
        if (!isRecord(rawSeason.tables)) {
            return undefined;
        }
        const localization = parseRawLocalization(rawSeason.localization);
        if (!localization) {
            return undefined;
        }
        const tables = {};
        for (const [tableName, rawTable] of Object.entries(rawSeason.tables)) {
            if (!RULESET_ID_PATTERN.test(tableName) || !isRecord(rawTable)) {
                return undefined;
            }
            if (!isContentHash(rawTable.contentHash)) {
                return undefined;
            }
            tables[tableName] = { contentHash: rawTable.contentHash };
        }
        seasons.push({ id: rawSeason.id, maps: rawSeason.maps, tables, localization });
    }
    return { version: 2, seasons };
}
function getRulesetRevision(season) {
    const tableNames = Object.keys(season.tables).sort();
    const revisionParts = tableNames.map(tableName => `${tableName}:${season.tables[tableName].contentHash}`);
    return [...revisionParts, `localization:${season.localization.contentHash}`].join('|');
}
function tryBuildFrontendSeasonRuleset(store) {
    if (store.state.status === 'ready') {
        return true;
    }
    const gameState = getCurrentGameState();
    if (gameState === undefined || gameState < CUSTOM_GAME_SETUP_STATE) {
        setState(store, { status: 'waiting_state_2' });
        return false;
    }
    setState(store, { status: 'building' });
    const rawData = getCustomUIConfig().__AK_FRONTEND_SEASON_RULESET_RAW__;
    const mapName = getCurrentMapName();
    if (!rawData || rawData.manifest === undefined || rawData.tables === undefined || !mapName) {
        setState(store, { status: 'waiting_data' });
        return false;
    }
    const manifest = parseRawManifest(rawData.manifest);
    if (!manifest) {
        setState(store, { status: 'failed', reason: 'manifest_invalid' });
        return false;
    }
    const rawTablesByRuleset = rawData.tables;
    if (!isRecord(rawTablesByRuleset)) {
        setState(store, { status: 'failed', reason: 'ruleset_tables_invalid' });
        return false;
    }
    const matchedSeasons = manifest.seasons.filter(season => season.maps.includes(mapName));
    if (matchedSeasons.length !== 1) {
        setState(store, { status: 'failed', reason: `map_not_mapped:${mapName}` });
        return false;
    }
    const season = matchedSeasons[0];
    const tableNames = Object.keys(season.tables).sort();
    if (tableNames.length === 0 || !tableNames.includes('ak_items_equip')) {
        setState(store, { status: 'failed', reason: `tables_invalid:${season.id}` });
        return false;
    }
    const rawTables = rawTablesByRuleset[season.id];
    if (rawTables === undefined) {
        setState(store, { status: 'waiting_data' });
        return false;
    }
    if (!isRecord(rawTables)) {
        setState(store, { status: 'failed', reason: `ruleset_tables_invalid:${season.id}` });
        return false;
    }
    const tables = {};
    for (const tableName of tableNames) {
        ...
        if (!isRecord(table)) {
            setState(store, { status: 'failed', reason: `table_invalid:${season.id}:${tableName}` });
            return false;
        }
        tables[tableName] = table;
    }
    const ruleset = new _shared_season_ruleset__WEBPACK_IMPORTED_MODULE_0__.SeasonRuleset({ rulesetId: season.id, tables, localizationTokens: season.localization.tokens });
    setState(store, {
        status: 'ready',
        rulesetId: season.id,
        revision: getRulesetRevision(season),
        ruleset,
    });
    return true;
}
/** 在 HUD 根脚本启动时建立状态 2 构筑门槛。 */
function initializeFrontendSeasonRuleset() {
    const store = getStore();
    if (store.started) {
        tryBuildFrontendSeasonRuleset(store);
        return;
    }
    store.started = true;
    store.rawDataNotifier = () => {
        if (getCustomUIConfig().__AK_FRONTEND_SEASON_RULESET_STORE__ === store) {
            tryBuildFrontendSeasonRuleset(store);
        }
    };
    getCustomUIConfig().__AK_FRONTEND_SEASON_RULESET_RAW_READY__ = store.rawDataNotifier;
    store.gameStateListener = GameEvents.Subscribe('dota_game_state_change', event => {
        if (Number(event.new_state) >= CUSTOM_GAME_SETUP_STATE) {
            tryBuildFrontendSeasonRuleset(store);
        }
    });
    store.gameNewMapListener = GameEvents.Subscribe('game_newmap', () => {
        tryBuildFrontendSeasonRuleset(store);
    });
    tryBuildFrontendSeasonRuleset(store);
}
/** HUD 热重载时清理根脚本注册的生命周期监听。 */
function disposeFrontendSeasonRuleset() {
    const config = getCustomUIConfig();
    const store = config.__AK_FRONTEND_SEASON_RULESET_STORE__;
    if (!store || store.version !== 1) {
        return;
    }
    if (store.gameStateListener !== undefined) {
        GameEvents.Unsubscribe(store.gameStateListener);
    }
    if (store.gameNewMapListener !== undefined) {
        GameEvents.Unsubscribe(store.gameNewMapListener);
    }
    if (config.__AK_FRONTEND_SEASON_RULESET_RAW_READY__ === store.rawDataNotifier) {
        delete config.__AK_FRONTEND_SEASON_RULESET_RAW_READY__;
    }
    store.started = false;
    store.gameStateListener = undefined;
    store.gameNewMapListener = undefined;
    store.rawDataNotifier = undefined;
}
/** 返回当前对局已构筑的完整 Ruleset；未就绪时返回 undefined。 */
function tryGetFrontendSeasonRuleset() {
    var _a;
    const state = (_a = getExistingStore()) === null || _a === void 0 ? void 0 : _a.state;
    return (state === null || state === void 0 ? void 0 : state.status) === 'ready' ? state.ruleset : undefined;
}
/** 返回当前 Ruleset 生命周期状态，供需要延迟构筑派生数据的业务读取。 */
function getFrontendSeasonRulesetState() {
    var _a, _b;
    return (_b = (_a = getExistingStore()) === null || _a === void 0 ? void 0 : _a.state) !== null && _b !== void 0 ? _b : { status: 'waiting_state_2' };
}
/** 订阅当前 Ruleset 的构筑结果；业务仍自行消费完整数据表。 */
function subscribeFrontendSeasonRuleset(listener) {
    const store = getStore();
...
/* harmony import */ var _utils_localization__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @/utils/localization */ "./utils/localization.ts");
...
function getReason(config, loadingState) {
    var _a, _b, _c;
    const shutdownReason = (_a = config.shutdown) === null || _a === void 0 ? void 0 : _a.reason;
    if (shutdownReason === 'version' || shutdownReason === 'maintenance')
        return shutdownReason;
    if (isEnabled((_b = config.versionRestriction) === null || _b === void 0 ? void 0 : _b.active))
        return 'version';
    if (isEnabled((_c = config.maintenance) === null || _c === void 0 ? void 0 : _c.active) || loadingState.mode === 'maintenance')
        return 'maintenance';
    return 'none';
}
function getDefaultTitle(reason) {
    if (reason === 'version')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_游戏版本过低');
    if (reason === 'maintenance')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_系统正在维护');
    return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_本局已结束');
}
function getDefaultMessage(reason) {
    if (reason === 'version')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_当前游戏版本过低请更新后重新进入');
    if (reason === 'maintenance')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_服务器维护已开始请稍后再试');
    return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_本局已结束请返回主界面');
}
function getReasonLabel(reason) {
    if (reason === 'version')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_版本限制');
    if (reason === 'maintenance')
        return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_维护关停');
    return (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_对局结束');
}
function EndScreenMaintenancePanel() {
    var _a, _b, _c, _d, _e, _f, _g;
    const localPlayerId = Players.GetLocalPlayer();
    const maintenanceConfig = ((0,react_panorama_x__WEBPACK_IMPORTED_MODULE_2__.useNetTableKey)('custom_value', 'maintenance_config') || {});
    const loadingState = ((0,react_panorama_x__WEBPACK_IMPORTED_MODULE_2__.useNetTableKey)('custom_value', `loading_state_${localPlayerId}`) || {});
    const reason = (0,react__WEBPACK_IMPORTED_MODULE_1__.useMemo)(() => getReason(maintenanceConfig, loadingState), [loadingState, maintenanceConfig]);
    const isVersionRestricted = reason === 'version' || isEnabled((_a = maintenanceConfig.versionRestriction) === null || _a === void 0 ? void 0 : _a.active);
    const title = ((_b = maintenanceConfig.shutdown) === null || _b === void 0 ? void 0 : _b.title) || loadingState.title || getDefaultTitle(reason);
    const message = ((_c = maintenanceConfig.shutdown) === null || _c === void 0 ? void 0 : _c.message) || loadingState.message || ((_d = maintenanceConfig.maintenance) === null || _d === void 0 ? void 0 : _d.message) || getDefaultMessage(reason);
    const currentVersion = ((_e = maintenanceConfig.versionRestriction) === null || _e === void 0 ? void 0 : _e.currentVersionString) || `${((_f = maintenanceConfig.versionRestriction) === null || _f === void 0 ? void 0 : _f.currentBuildVersion) || '-'}`;
    const minimumVersion = `${((_g = maintenanceConfig.versionRestriction) === null || _g === void 0 ? void 0 : _g.minimumBuildVersion) || '-'}`;
    const rootClassName = `end_screen_panel ${reason === 'version' ? 'is_version' : reason === 'maintenance' ? 'is_maintenance' : 'is_unknown'}`;
    return ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: rootClassName, children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Panel, { className: "end_screen_back_glow", hittest: false }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Panel, { className: "end_screen_grid_texture", hittest: false }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(DOTAParticleScenePanel, { className: "end_screen_particle", hittest: false, particleName: "particles/econ/items/drow/drow_arcana/drow_arcana_loadout_snow_v2.vpcf", particleonly: true, startActive: true, cameraOrigin: "250 20 0", lookAt: "0 20 0", fov: 60, squarePixels: true }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_content", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_logo", text: "AGE OF WEAPONS" }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_card", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_header", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Panel, { className: "end_screen_status_badge", children: (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { text: getReasonLabel(reason) }) }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_title", text: title }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_message", text: message })] }), isVersionRestricted ? ((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_version_box", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_version_row", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_version_label", text: (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_当前版本') }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_version_value", text: currentVersion })] }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(Panel, { className: "end_screen_version_row", children: [(0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_version_label", text: (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_最低版本') }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_version_value", text: minimumVersion })] })] })) : null, (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { className: "end_screen_notice", text: (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_维护结束后不会继续写入本局存档') }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Button, { className: "end_screen_exit_button ButtonPrimary Blue", onactivate: () => {
                                    $.DispatchEvent('DOTAHUDShowDashboard');
                                }, children: (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { text: (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_退出游戏') }) })] }), (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Panel, { className: "end_screen_footer", children: (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(Label, { text: (0,_utils_localization__WEBPACK_IMPORTED_MODULE_3__.localizeToken)('end_screen_请返回主界面后根据提示重新进入游戏') }) })] })] }));
}
(0,react_panorama_x__WEBPACK_IMPORTED_MODULE_2__.render)((0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx)(EndScreenMaintenancePanel, {}), $('#end_screen_app'));
