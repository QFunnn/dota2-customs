...
"use strict";
/*!********************************************!*\
  !*** ./tooltips/reward_tooltip/script.tsx ***!
  \********************************************/
__webpack_require__.r(__webpack_exports__);
...
function rewardItemID(reward) {
    var _a, _b;
    return Number((_b = (_a = reward.item_id) !== null && _a !== void 0 ? _a : reward.itemid) !== null && _b !== void 0 ? _b : 0);
}
function rewardAmount(reward) {
    var _a, _b;
    return Number((_b = (_a = reward.amounts) !== null && _a !== void 0 ? _a : reward.amount) !== null && _b !== void 0 ? _b : 0);
}
function rewardImage(itemID) {
    return `file://{images}/custom_game/store_items/${itemID}.png`;
}
function rewardName(itemID) {
    const token = `#item_${itemID}`;
    const localized = $.Localize(token);
    if (localized !== token) {
        return localized;
    }
    const plainToken = `#${itemID}`;
    const plainLocalized = $.Localize(plainToken);
    return plainLocalized !== plainToken ? plainLocalized : `${itemID}`;
}
function TooltipContents({ rewardList }) {
    return (react__WEBPACK_IMPORTED_MODULE_1__.createElement(Panel, { id: "RewardTooltip" },
        react__WEBPACK_IMPORTED_MODULE_1__.createElement(Panel, { id: "Content" }, rewardList.map((reward, index) => {
            const itemID = rewardItemID(reward);
            return (react__WEBPACK_IMPORTED_MODULE_1__.createElement(Panel, { className: "Reward", key: `${itemID}_${index}` },
                react__WEBPACK_IMPORTED_MODULE_1__.createElement(Image, { className: "ProductImage", src: rewardImage(itemID) }),
                react__WEBPACK_IMPORTED_MODULE_1__.createElement(Label, { className: "ProductCount", text: `x${rewardAmount(reward)}` }),
                react__WEBPACK_IMPORTED_MODULE_1__.createElement(Label, { className: "ProductName", text: rewardName(itemID) })));
        }))));
}
function setupTooltip() {
    const rewardList = parseRewardList(tooltipPanel.GetAttributeString("reward_list", "[]"));
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.render)(react__WEBPACK_IMPORTED_MODULE_1__.createElement(TooltipContents, { rewardList: rewardList }), tooltipPanel);
}
(() => {
    tooltipPanel.SetPanelEvent("ontooltiploaded", setupTooltip);
    tooltipPanel.style.minHeight = "50px";
})();
