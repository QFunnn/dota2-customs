...
function UpdateAbilitySize(){
	$.Schedule(0.05, _UpdateAbilitySize)
}

function _UpdateAbilitySize(){
	if(!abilities_panel){return}

	overlay_base.AddClass("SixAbilities")
	overlay_base.AddClass("FiveAbilities")
	
	let MaxChilds = _GetVisibleChilds(abilities_panel.Children()).length
	...
	$.Each(abilities_panel.Children(), function(child, index){
		...
		let ButtonSize = child.FindChildTraverse("ButtonSize")
		let ButtonWell = child.FindChildTraverse("ButtonWell")
		let LevelUpLight = child.FindChildTraverse("LevelUpLight")
		let AbilityLevelContainer = child.FindChildTraverse("AbilityLevelContainer")
		let LevelUpBurstFXContainer = child.FindChildTraverse("LevelUpBurstFXContainer")
		if(!ButtonSize || !ButtonWell || !LevelUpLight || !AbilityLevelContainer || !LevelUpBurstFXContainer){return}

		if(MaxChilds >= 10){
			child.style.width = "48px"
			child.style.marginLeft = "-1px"
			child.style.marginRight = "-1px"

			ButtonSize.style.width = "48px"
			ButtonSize.style.height = "48px"

			ButtonWell.style.width = "44px"
			ButtonWell.style.height = "44px"
			ButtonWell.style.marginBottom = "16px"

			LevelUpLight.style.height = "62px"

			AbilityLevelContainer.style.marginBottom = "28px"

			for (let i = 0; i < AbilityLevelContainer.GetChildCount(); i++) {
				let Level = AbilityLevelContainer.GetChild(i)
				if(Level){
					Level.style.width = "6px"
					Level.style.margin = "3px 1.5px"
				}
			}

			LevelUpBurstFXContainer.style.paddingLeft = "3px"
			LevelUpBurstFXContainer.style.paddingRight = "1px"
		}else{
			child.style.width = "58px"

			child.style.marginLeft = "0px"
			child.style.marginRight = "0px"

			ButtonWell.style.width = "54px"
			ButtonWell.style.height = "54px"
			ButtonWell.style.marginBottom = "8px"

			LevelUpLight.style.height = "82px"

			AbilityLevelContainer.style.marginBottom = "20px"

			for (let i = 0; i < AbilityLevelContainer.GetChildCount(); i++) {
				let Level = AbilityLevelContainer.GetChild(i)
				if(Level){
					Level.style.width = "8px"
					Level.style.margin = "3px 2px"
				}
			}

			LevelUpBurstFXContainer.style.paddingLeft = "0px"
			LevelUpBurstFXContainer.style.paddingRight = "0px"
		}
	})
}
...
function _GetAbilityButton(ability_panel) {
	...
	return ability_panel.GetChild(0).GetChild(1).GetChild(3).GetChild(2).GetChild(0);
}
...
(function () {
	...
	GameUI.CustomUIConfig().ToggleAbilitiesReorder = ToggleAbilitiesReorder

	GameUI.CustomUIConfig().MakeDescVisible = function(bVisible){
		$.GetContextPanel().SetHasClass("PanelsMoveActive", bVisible)
	}
	...
	GameEvents.Subscribe( "dota_ability_changed", UpdateAbilitySize );
	GameEvents.Subscribe('dota_player_update_hero_selection', UpdateAbilitySize);
	GameEvents.Subscribe('dota_player_update_selected_unit', UpdateAbilitySize);
	GameEvents.Subscribe('dota_player_update_query_unit', UpdateAbilitySize);
	GameEvents.Subscribe('dota_player_update_assigned_hero', UpdateAbilitySize);
