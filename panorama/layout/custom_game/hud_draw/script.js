...
function FindItemTypeAndName(item_id) {
        ...
        if (list != undefined) {
            for (const i in list) {
                ...
                if (String(econ.id) == String(item_id) || String(econ.item_id) == String(item_id)) {
                    return {
                        item_type: item_type,
                        item_name: econ.name || econ.item_name || "",
                        is_collectible: econ.is_collectible == 1 || econ.is_collectible === true || econ.is_collectible === "true"
                    };
                }
            }
        }
    ...
    if (skinList != undefined) {
        ...
        if (skin != undefined) {
            return {
                item_type: "skin",
                item_name: skin.name || skin.item_name || "",
                is_collectible: skin.is_collectible == 1 || skin.is_collectible === true || skin.is_collectible === "true"
            };
        }
    }
    ...
    if (skinList != undefined) {
        ...
        if (skin != undefined) {
            return {
                item_type: "skin",
                item_name: skin.name || skin.item_name || "",
                rarity: Number(skin.rarity || 1),
                is_collectible: skin.is_collectible == 1 || skin.is_collectible === true || skin.is_collectible === "true"
            };
        }
    }
    return { item_type: "", item_name: "", rarity: 1 };
}
function PlayerHasItem(item_id, item_type) {
    var _a, _b, _c;
...
function GetPayTypeBalance(pay_type, player_wallet, player_boxes) {
    var _a, _b;
    const payTypeStr = String(pay_type);
    if (typeof pay_type === "number" || !isNaN(Number(pay_type))) {
        return (_a = player_boxes[payTypeStr]) !== null && _a !== void 0 ? _a : 0;
    }
    else {
        return (_b = player_wallet[payTypeStr]) !== null && _b !== void 0 ? _b : 0;
    }
}
function GetPayTypeIcon(pay_type) {
    const payTypeStr = String(pay_type);
    if (payTypeStr === "starlight") {
        return "file://{images}/custom_game/icon/shard.png";
    }
    else if (payTypeStr === "moonstone") {
        return "file://{images}/custom_game/icon/coin.png";
    }
    else if (payTypeStr === "token1" || payTypeStr === "token2") {
        return `file://{images}/custom_game/store/${payTypeStr}.png`;
    }
    else {
        return `file://{resources}/images/custom_game/store_items/${payTypeStr}.png`;
    }
}
...
class HudDraw extends react__WEBPACK_IMPORTED_MODULE_2__.Component {
    constructor() {
        ...
        this.netTableIDList = [];
        this.handleDrawResult = (res, token_type) => {
            this.setState({ inRequest: false });
            if (res.code == 0) {
                let data = res.data;
                if (data != undefined && data.add_items && data.add_items.common) {
                    let resultList = [];
                    for (const k in data.add_items.common) {
                        const info = data.add_items.common[k];
                        if (String(info.item_id) != String(token_type) && info.amounts > 0) {
                            const cosmetic_id = info.origin_item_id || info.item_id;
                            const details = FindItemTypeAndName(cosmetic_id);
                            resultList.push({
                                item_id: cosmetic_id,
                                amount: 1,
                                ...
                                whether_decomposition: info.origin_item_id != undefined && info.origin_item_id != null,
                                decomposition: info.origin_item_id != undefined && info.origin_item_id != null ? {
                                    token_type: String(info.item_id),
                                    number: info.amounts,
                                } : undefined,
                                is_collection: !!details.is_collectible,
                            });
                        }
                    }
    ...
    getTreasureList(treasures) {
        let list = [];
        for (const treasureIndex in treasures) {
            let data = treasures[treasureIndex];
            if (data == undefined)
                continue;
            data = Object.assign({}, data);
            let DropList = [];
            for (const i in data.drop_list) {
                let DropItem = data.drop_list[i];
                DropList.push(DropItem);
            }
            DropList.sort((a, b) => {
                return b.rarity - a.rarity;
            });
            data.drop_list = DropList;
            list.push(data);
        }
        return list;
    }
    getOldRedeemList(redeems) {
        let list = [];
        if (redeems) {
            for (const index in redeems) {
                const data = redeems[index];
                if (data) {
                    list.push(data);
                }
            }
    ...
    componentDidMount() {
        ...
        this.netTableIDList.push(CustomNetTables.SubscribeNetTableListener("service", (tableName, key, value) => {
            ...
            if (key == "player_wallet") {
                this.setState({
                    player_wallet: {
                        ...
                        token1: (_f = (_e = value === null || value === void 0 ? void 0 : value[Players.GetLocalPlayer()]) === null || _e === void 0 ? void 0 : _e.token1) !== null && _f !== void 0 ? _f : 0,
                        token2: (_h = (_g = value === null || value === void 0 ? void 0 : value[Players.GetLocalPlayer()]) === null || _g === void 0 ? void 0 : _g.token2) !== null && _h !== void 0 ? _h : 0,
                    }
                });
            }
            ...
            if (key == "player_boxes") {
                this.setState({
                    player_boxes: (_j = value === null || value === void 0 ? void 0 : value[Players.GetLocalPlayer()]) !== null && _j !== void 0 ? _j : {}
                });
            }
            if (key == "product_list") {
                this.setState({
                    product_list: this.getProductList(value)
                });
            }
            if (key == "redeem_list") {
                this.setState({
                    redeem_list: this.getOldRedeemList(value === null || value === void 0 ? void 0 : value[Players.GetLocalPlayer()])
                });
            }
        }));
    }
    ...
    render() {
        ...
        const { show, tabIndex, player_wallet, player_boxes, poolIndex, inRequest, treasure_list, pool_list, redeem_list, product_list, rareItemIndex, token_index } = this.state;
        const combinedList = [...(pool_list || []), ...(treasure_list || [])];
        let CurrentTreasureData = combinedList[poolIndex - 1];
        let token_type_list = [];
        let cost_1 = 99999999;
        let cost_10 = 99999999;
        let token_type = "moonstone";
        let DropList = [];
        const isCollectionPool = CurrentTreasureData && ("pool" in CurrentTreasureData);
        const bid = CurrentTreasureData && CurrentTreasureData.bid;
        if (CurrentTreasureData) {
            if (isCollectionPool && bid) {
                token_type = String(bid);
                token_type_list = [token_type];
                cost_1 = Number(CurrentTreasureData.one_shot_token_cost || 1);
                cost_10 = Number(CurrentTreasureData.ten_shot_token_cost || 10);
            }
            else {
                const token_type_str = String(CurrentTreasureData.token_type || "token1");
                token_type_list = token_type_str.split(",");
                token_type = token_type_list[token_index] || "token1";
                const cost_10_str = String(CurrentTreasureData.ten_shot_token_cost || "10");
                const cost_1_str = String(CurrentTreasureData.one_shot_token_cost || "1");
                let cost_10_list = cost_10_str.split(",");
                let cost_1_list = cost_1_str.split(",");
                cost_1 = Number(cost_1_list[token_index] || 1);
                cost_10 = Number(cost_10_list[token_index] || 10);
            }
            if (CurrentTreasureData.drop_list) {
                DropList = CurrentTreasureData.drop_list;
            }
            else if (CurrentTreasureData.drop_content) {
                const itemId = DROP_CONTENT_TO_ITEM_ID[CurrentTreasureData.drop_content] || 17000027;
                const details = FindItemTypeAndName(itemId);
                DropList = [{
                        item_id: itemId,
                        is_guarantee: true,
                        item_name: details.item_name || CurrentTreasureData.treasure_name || CurrentTreasureData.name || "",
                        rarity: 5,
                        weight: 100,
                        item_type: details.item_type || "skin"
                    }];
            }
            else {
                DropList = [];
        ...
        const current_balance = (isCollectionPool && bid) ? ((_a = player_boxes[token_type]) !== null && _a !== void 0 ? _a : 0) : ((_b = player_wallet[token_type]) !== null && _b !== void 0 ? _b : 0);
        const token_icon_src = (isCollectionPool && bid)
                                ...
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "DrawButtons", flowChildren: "right", horizontalAlign: "center" },
                                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { flowChildren: "down", className: "DrawButtonContainer" },
                                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Button, { className: "DrawBtnBlue", enabled: (!inRequest) && current_balance >= cost_1, onactivate: () => {
                                                ...
                                                const endTime = isCollectionPool ? rawEndTime * 1000 : rawEndTime;
                    ...
                    tabIndex == 1 &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "RedeemPage", width: "100%", height: "100%", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Show: tabIndex == 1 }), flowChildren: "down" },
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_EOMDesign_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "RedeemList" }, isCollectionPool ? ((product_list !== null && product_list !== void 0 ? product_list : []).map((redeemInfo, i) => {
                                ...
                                if (!firstItem)
                                    return null;
                                const item_id = firstItem.item_id;
                                const details = FindItemTypeRarityAndName(item_id);
                                const item_name = details.item_name;
                                const rarity = details.rarity;
                                const item_type = details.item_type;
                                const whether_have = (item_type !== "token") && PlayerHasItem(item_id, item_type);
                                const payType = redeemInfo.pay_type;
                                                ...
                                                (0,_utils_utils__WEBPACK_IMPORTED_MODULE_6__.Send2ServerCallback)("product_buy", {
                                                    ...
                                                    if (res.code == 0) {
                                                        (0,_utils_utils__WEBPACK_IMPORTED_MODULE_6__.ShowCustomPopup)("OpenBoxResult", {
                                                            ...
                                                            result: [{
                                                                    item_id: item_id,
                                                                    amount: firstItem.amounts || 1,
                                                                    rarity: rarity,
                                                                    item_name: item_name,
                                                                    item_type: item_type,
                                                                    whether_decomposition: false,
                                                                    is_collection: !!details.is_collectible,
                                                                }]
                                                        });
                                                    }
                                ...
                                if (!iteminfo)
                                    return null;
                                let item_id = iteminfo.item_id;
                                let item_name = iteminfo.item_name;
