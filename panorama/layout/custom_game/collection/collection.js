...
function InitContent() {
	...
	Object.entries(TABS).forEach(([tab_name, b_create], index) => {
		...
		if (index == 1) activate_content(true);
...
GameUI.Collection.OpenSubPanel = (name) => {
	if (name == "C_PayCurrency" && CustomNetTables.GetTableValue("game_state", "player_stats") == undefined) return;
...
function CheckProPlayer() {
	const stats = CustomNetTables.GetTableValue("game_state", "player_stats");
	const games = (stats?.[LOCAL_PLAYER_ID]?.wins || 0) + (stats?.[LOCAL_PLAYER_ID]?.loses || 0);
