...
function setInterval(callback) {
  ...
  var _run = function run() {
    intervals.set(intervalId, $.Schedule(timeout, _run));
...
var windowRoot=function(){var panel=$.GetContextPanel();while(panel){if(panel.BHasClass('WindowRoot'))return panel;panel=panel.GetParent();}}();var temporaryPanelHost=(_a=windowRoot.FindChildTraverse('__react_panorama_temporary_host__'))!==null&&_a!==void 0?_a:$.CreatePanel('Panel',windowRoot,'__react_panorama_temporary_host__');temporaryPanelHost.RemoveAndDeleteChildren();temporaryPanelHost.visible=false;var temporaryScenePanelHost=(_b=windowRoot.FindChildTraverse('__react_panorama_temporary_scene_host__'))!==null&&_b!==void 0?_b:$.CreatePanel('Panel',windowRoot,'__react_panorama_temporary_scene_host__');temporaryScenePanelHost.RemoveAndDeleteChildren();temporaryScenePanelHost.visible=false;GameUI.CustomUIConfig().temporaryScheduleHandle=-1;var _checkFunc=function checkFunc(){for(var i=0;i<temporaryScenePanelHost.GetChildCount();i+=1){var child=temporaryScenePanelHost.GetChild(i);if(child!==null){if(child.BHasClass('SceneLoaded')){child.SetParent(temporaryPanelHost);}}}temporaryPanelHost.RemoveAndDeleteChildren();GameUI.CustomUIConfig().temporaryScheduleHandle=$.Schedule(1,_checkFunc);};if(GameUI.CustomUIConfig().temporaryScheduleHandle!==-1){try{$.CancelScheduled(GameUI.CustomUIConfig().temporaryScheduleHandle);}catch(_c){}GameUI.CustomUIConfig().temporaryScheduleHandle=-1;}_checkFunc();for(var _i2=0,_arr=['Panel','Label','Image','DOTAAbilityImage','DOTAItemImage','DOTAHeroImage','DOTALeagueImage','EconItemImage','AnimatedImageStrip','DOTAEmoticon','Movie','DOTAHeroMovie','DOTAScenePanel','DOTAParticleScenePanel','DOTAEconItem','ProgressBar','CircularProgressBar','ProgressBarWithMiddle','DOTAUserName','DOTAUserRichPresence','DOTAAvatarImage','Countdown','Button','TextButton','ToggleButton','RadioButton','TextEntry','NumberEntry','Slider','SlottedSlider','DropDown','ContextMenuScript','Carousel','CarouselNav','DOTAHUDOverlayMap','DOTAMinimap','HTML','TabButton','TabContents','CustomLayoutPanel','GenericPanel'];_i2<_arr.length;_i2++){var panelName=_arr[_i2];globalThis[panelName]=panelName;}function createCommonjsModule(fn,basedir,module){return module={path:basedir,exports:{},require:function require(path,base){return commonjsRequire(path,base===undefined||base===null?module.path:base);}},fn(module,module.exports),module.exports;}function commonjsRequire(){throw new Error('Dynamic requires are not currently supported by @rollup/plugin-commonjs');}/*
object-assign
(c) Sindre Sorhus
@license MIT
...
var _callback=null;var _timeoutID=null;var _flushCallback2=function _flushCallback(){if(_callback!==null){try{var currentTime=exports.unstable_now();var hasRemainingTime=true;_callback(hasRemainingTime,currentTime);_callback=null;}catch(e){setTimeout(_flushCallback2,0);throw e;}}};var initialTime=Date.now();exports.unstable_now=function(){return Date.now()-initialTime;};_requestHostCallback=function requestHostCallback(cb){if(_callback!==null){// Protect against re-entrancy.
setTimeout(_requestHostCallback,0,cb);}else{_callback=cb;setTimeout(_flushCallback2,0);}};requestHostTimeout=function requestHostTimeout(cb,ms){_timeoutID=setTimeout(cb,ms);};cancelHostTimeout=function cancelHostTimeout(){clearTimeout(_timeoutID);};shouldYieldToHost=function shouldYieldToHost(){return false;};requestPaint=exports.unstable_forceFrameRate=function(){};}else{// Capture local references to native APIs, in case a polyfill overrides them.
...
var _appendAllChildrenToContainer=function appendAllChildrenToContainer(containerChildSet,workInProgress,needsVisibilityToggle,isHidden){// We only have the top Fiber that was created but we need recurse down its
// children to find all the terminal nodes.
var node=workInProgress.child;while(node!==null){// eslint-disable-next-line no-labels
if(node.tag===HostComponent){var instance=node.stateNode;if(needsVisibilityToggle&&isHidden){// This child is inside a timed out tree. Hide it.
...
var newIsHidden=node.memoizedState!==null;if(newIsHidden){var primaryChildParent=node.child;if(primaryChildParent!==null){if(primaryChildParent.child!==null){primaryChildParent.child["return"]=primaryChildParent;_appendAllChildrenToContainer(containerChildSet,primaryChildParent,true,newIsHidden);}var fallbackChildParent=primaryChildParent.sibling;if(fallbackChildParent!==null){fallbackChildParent["return"]=node;node=fallbackChildParent;continue;}}}}if(node.child!==null){// Continue traversing like normal
node.child["return"]=node;node=node.child;continue;}}else if(node.child!==null){node.child["return"]=node;node=node.child;continue;}// $FlowFixMe This is correct but Flow is confused by the labeled break.
...
_appendAllChildrenToContainer(newChildSet,workInProgress,false,false);portalOrRoot.pendingChildren=newChildSet;// Schedule an update on the container to swap out the container.
...
shouldDeprioritizeSubtree:undefined,setTimeout:setTimeout,clearTimeout:clearTimeout,noTimeout:-1,now:Date.now,isPrimaryRenderer:true,supportsMutation:true,supportsPersistence:false,supportsHydration:false,shouldSetTextContent:function shouldSetTextContent(){return false;},createInstance:function createInstance(type,newProps){var oncreated=newProps.oncreated;if(oncreated){delete newProps.oncreated;}var _splitInitialProps=splitInitialProps(type,newProps),initialProps=_splitInitialProps.initialProps,otherProps=_splitInitialProps.otherProps;if(type==='GenericPanel')type=newProps.type;// Create it on the context panel instead of rootContainerInstance to
// preserve style context for elements rendered outside of the main tree
var panel=initialProps!=null&&_typeof(initialProps)=="object"?// Create it on the context panel instead of rootContainerInstance to
// preserve style context for elements rendered outside of the main tree
$.CreatePanel(type,$.GetContextPanel(),newProps.id||'',initialProps):$.CreatePanel(type,$.GetContextPanel(),newProps.id||'');if(panelBaseNames.has(type)){fixPanelBase(panel);}for(var propName in otherProps){updateProperty(type,panel,propName,undefined,otherProps[propName]);}if(oncreated){oncreated(panel);}return panel;},createTextInstance:function createTextInstance(){throw new Error('react-panorama does not support text nodes. Use <Label /> element instead.');},appendInitialChild:appendChild,finalizeInitialChildren:function finalizeInitialChildren(){return false;},appendChild:appendChild,appendChildToContainer:appendChild,insertBefore:insertBefore,insertInContainerBefore:insertBefore,removeChild:removeChild,removeChildFromContainer:removeChild,// https://github.com/facebook/react/pull/8607
                    ...
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "ControlPanelTitle" },
                        ...
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_IconButton_EOM_IconButton__WEBPACK_IMPORTED_MODULE_13__.EOM_IconButton, { className: "CategoryHeaderIcon", tooltip: "切换布局", verticalAlign: "center", width: "20px", icon: react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Icons_EOM_Arrow__WEBPACK_IMPORTED_MODULE_6__["default"], { type: "Popout" }) },
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_DropDown_EOM_DropDown__WEBPACK_IMPORTED_MODULE_12__.EOM_DropDown, { id: "ToggleSize", onChange: (index, item) => { this.setState({ direction: item.id }); SaveConfig({ direction: item.id }); } },
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "左侧", id: "left" }),
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "上方", id: "top" }),
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "右侧", id: "right" }))),
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_IconButton_EOM_IconButton__WEBPACK_IMPORTED_MODULE_13__.EOM_IconButton, { className: "CategoryHeaderIcon", tooltip: "重载数据", verticalAlign: "center", width: "20px", icon: react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Icons_EOM_Refresh__WEBPACK_IMPORTED_MODULE_9__["default"], null), onactivate: self => GameEvents.SendEventClientSide("custom_refresh_order", { name: "RefreshContainer" }) }),
    ...
    render() {
        ...
        return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, Object.assign({}, this.props, this.getBaseProp()),
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CategoryHeader" },
                ...
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_Switch_EOM_Switch__WEBPACK_IMPORTED_MODULE_14__["default"], { horizontalAlign: "right", checkedChildren: "1\u5217", unCheckedChildren: "2\u5217", checked: col == 1 ? true : false, onChange: (checked) => { SaveConfig({ ["ctg_col_" + this.props.title]: checked ? 1 : 2 }); this.setState({ col: checked ? 1 : 2 }); } })),
...
function DemoTextEntry({ eventName, text, defaultValue = "" }) {
            ...
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "SelectionPicker" },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "SelectionPickerHeader", onactivate: () => { }, onmouseover: self => this.dragStart(self), onmouseout: self => this.dragable = false },
                    ...
                    this.props.hasFilter != false &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "SelectionSearch", className: "SearchBox" },
                            this.state.hasToggleList &&
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_DropDown_EOM_DropDown__WEBPACK_IMPORTED_MODULE_12__.EOM_DropDown, { placeholder: "\u7B5B\u9009", onChange: (index, item) => {
                                    ...
                                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { id: "EOM_DropDown_Clear", text: "X \u6E05\u9664\u7B5B\u9009" }),
                    ...
                    this.props.hasRawMode != false &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { className: "CodeModeLabel", tooltip: "查看内部编码", tooltipPosition: "top", height: "28px", verticalAlign: "center" },
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(TextButton, { style: { fontSize: "20px", width: "27px", height: "27px", marginTop: "2px" }, text: this.state.rawMode ? "汉" : "Aa", onactivate: () => this.toggleRawMode() })),
                    this.props.hasToggleSize != false &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_IconButton_EOM_IconButton__WEBPACK_IMPORTED_MODULE_13__.EOM_IconButton, { width: "30px", tooltip: "切换窗口大小", tooltipPosition: "top", icon: react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Icons_EOM_Arrow__WEBPACK_IMPORTED_MODULE_6__["default"], { type: "Expand" }) },
                    ...
                    this.props.hasLock != false &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_IconButton_EOM_IconButton__WEBPACK_IMPORTED_MODULE_13__.EOM_IconButton, { width: "26px", tooltip: "锁定窗口", tooltipPosition: "top", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("LockIconButton", { Unlock: !this.state.lock }), icon: react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Icons_EOM_Lock__WEBPACK_IMPORTED_MODULE_8__["default"], { type: "Small" }), onactivate: () => this.toggleLock() }),
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_IconButton_EOM_IconButton__WEBPACK_IMPORTED_MODULE_13__.EOM_IconButton, { width: "28px", tooltip: "关闭窗口", tooltipPosition: "top", icon: react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Icons_EOM_XClose__WEBPACK_IMPORTED_MODULE_10__["default"], { type: "Default" }), onactivate: () => ToggleSelection(eventName) })),
...
function EOM_DebugTool_Setting() {
    ...
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(SelectContainer, { eventName: "EOM_DebugTool_Setting", title: "调试工具设置", width: "500px", height: "700px", hasRawMode: false, hasToggleSize: false, hasFilter: false },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { id: "EOM_DebugTool_Setting", className: "EOM_DebugTool_Setting", flowChildren: "down", width: "100%", height: "100%" },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { flowChildren: "down", width: "100%", marginTop: "12px", onload: self => OnLoad(self) },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "\u9762\u677F\u70ED\u952E", className: "SectionHeader" }),
                ...
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { width: "100%" },
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_Button_EOM_Button__WEBPACK_IMPORTED_MODULE_11__["default"], { margin: "4px 8px", className: "AddNewKeyBind", type: "Text", text: "+ \u6309\u952E\u7ED1\u5B9A", onactivate: self => addKeyBind(self) }),
                    !deleteMode &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_Button_EOM_Button__WEBPACK_IMPORTED_MODULE_11__["default"], { margin: "4px 8px", className: "AddNewKeyBind", horizontalAlign: "right", type: "Text", text: "- \u5220\u9664\u7ED1\u5B9A", onactivate: self => setDeleteMode(true) }),
                    deleteMode &&
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_Button_EOM_Button__WEBPACK_IMPORTED_MODULE_11__["default"], { margin: "4px 8px", className: "AddNewKeyBind", horizontalAlign: "right", type: "Text", text: "\u53D6\u6D88", onactivate: self => setDeleteMode(false) }))),
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Inputs_EOM_Button_EOM_Button__WEBPACK_IMPORTED_MODULE_11__["default"], { verticalAlign: "bottom", type: "Outline", color: "Blue", text: "\u6E05\u7A7A\u8BBE\u7F6E", onactivate: self => { FireEvent("ChangeToolSettingKeyBind", "_clear_all_config"); setCustomKeyBindCount(0); } }))));
}
function EOM_DebugTool_IconPicker() {
    ...
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(SelectContainer, { eventName: "EOM_DebugTool_IconPicker", title: "\u56FE\u6807\u9009\u62E9", hasRawMode: false, toggleList: {
            All: "全部",
            Eom: "EOM",
            Tui7: "Tui7",
            Tui3: "Tui3",
...
class EOM_UnitInfo extends _EOM_BaseComponent__WEBPACK_IMPORTED_MODULE_5__["default"] {
    ...
    render() {
        return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(SelectContainer, { eventName: "EOM_UnitInfo", title: "单位信息面板", width: "480px", height: "620px", hasRawMode: false, hasToggleSize: false, hasFilter: false },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(_Container_EOM_Panel_EOM_Panel__WEBPACK_IMPORTED_MODULE_3__["default"], { flowChildren: "down" },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "单位名：" + `(${this.state.unitIndex})` + Entities.GetUnitName(this.state.unitIndex) }),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "位置：" + this.getPosition() }),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "朝向：" + this.getForward() }),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "生命：" + Entities.GetHealth(this.state.unitIndex) + "/" + Entities.GetMaxHealth(this.state.unitIndex) }),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "魔法：" + Entities.GetMana(this.state.unitIndex) + "/" + Entities.GetMaxMana(this.state.unitIndex) }),
...
function EOM_KeyBinder({ type = KeyBinderType.Normal, text = "", initDropIndex, tooltip, initKey = "", callback, checkFunc, }) {
    ...
    function OnClear(self) {
        ...
        if (pSelf) {
            ...
            setKeyName("");
            const bIsControlDown = GameUI.IsControlDown();
            const bIsShiftDown = GameUI.IsShiftDown();
            OnCancel(pSelf);
            callback("", false, dropIndex, bIsControlDown, bIsShiftDown);
        }
    }
...
function DebugTool() {
    ...
    return (react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__["default"], { containerElement: react__WEBPACK_IMPORTED_MODULE_1__.createElement(react__WEBPACK_IMPORTED_MODULE_1__.Fragment, null,
        ...
        react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.EOM_DebugTool_Category, { title: $.Localize("#HUD_Category_AbilitiesAndItems") },
            react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.DemoSelectionButton, { eventName: "AddNeutralItemButtonPressed", text: $.Localize("#HUD_AddNeutralItem") }),
        ...
        react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.EOM_DebugTool_Category, { title: $.Localize("#HUD_Category_Unit") },
            ...
            react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.DemoButton, { eventName: "ControlUnitButtonPressed", text: $.Localize("#HUD_SwitchControl") }),
            ...
            react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.DemoTextEntry, { eventName: "SetHeroLevel", text: $.Localize("#HUD_LevelUpto") }),
            react__WEBPACK_IMPORTED_MODULE_1__.createElement(_EOMDesign_Other_EOM_DebugTool_EOM_DebugTool__WEBPACK_IMPORTED_MODULE_2__.DemoTextEntry, { eventName: "DebugModifyGold", text: $.Localize("#HUD_GetGold") }),
