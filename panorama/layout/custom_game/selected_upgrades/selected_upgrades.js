...
function LocalizeUpgrade(ability_name, upgrade_name, b_generic) {
	let loc_upgrade = CheckLocalizeArray(
		[
			`${upgrade_name}_upgrade`,
			`DOTA_Tooltip_ability_${ability_name}_${upgrade_name}`,
			`upgrade_DOTA_Tooltip_ability_${ability_name}_${upgrade_name}`,
			b_generic ? `DOTA_Tooltip_demo_generic_orb_${upgrade_name}` : undefined,
		],
		"",
	);

	if (b_generic) loc_upgrade = loc_upgrade.replace(/<b>.*<\/b>/, "");
