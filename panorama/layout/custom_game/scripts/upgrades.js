...
function _SetFavorites(event) {
	// $.Msg("_SetFavorites");
	// JSON.print(event);

	favorites_upgrades = event.current_favorites;

	GameUI.SelectedUpgrades.UpdateFavoriteBuilds(event.builds || {});
}

function InitUpgradesRequest() {
	const portrait_unit = Players.GetLocalPlayerPortraitUnit(LOCAL_PLAYER_ID);
	if (portrait_unit == -1) return void $.Schedule(0, InitUpgradesRequest);
