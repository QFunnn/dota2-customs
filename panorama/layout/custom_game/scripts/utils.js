...
Game.GetTeamScore = function (team_id) {
	const team_info = Game.GetTeamDetails(team_id);
	if (!team_info) return 0;

	return team_info.team_score || 0;
};
...
function RandomChildrenSort(root) {
	let c_count = root.Children().length;

	for (const child of root.Children().sort(() => {
		return Math.floor(Math.random() * c_count--) ? 1 : -1;
	})) {
		root.MoveChildBefore(child, root.GetChild(0));
	}
}
...
function GenerateSublines(container, max_childs_per_line, margin_b_lines = 0, margin_b_childs = 0) {
	...
	return {
		...
		style: function (_counter = counter) {
			const style = [
				margin_b_childs > 0 && _counter > 1 ? `margin-left: ${margin_b_childs}px` : undefined,
		...
		force_restyle: function (filter_func) {
			for (const line of container.Children()) {
				let force_line_counter = 0;
				for (const child of line.Children()) {
					if (filter_func && !filter_func(child)) continue;

					force_line_counter++;

					child.ClearPropertyFromCode("margin");

					const style = this.style(force_line_counter);

					for (const [k, v] of Object.entries(CSSLineToObjectCamelCase(style))) child.style[k] = v;
				}
			}
		},
	};
...
const basic_prefix = $.Localize("perk_basic_modifier_prefix");
function GetPerkLocName(perk_name, tier) {
	let spec_loc_token = `${perk_name}_t${tier - 1}`;

	if ($.CanLocalize(spec_loc_token)) {
		return $.Localize(spec_loc_token);
	} else {
		let prefix = $.Localize(`perk_prefix_t${tier - 1}`);
		let loc_name = $.Localize(`DOTA_Tooltip_${perk_name}`).replace(basic_prefix, "");

		if (prefix != "") {
			let add_prefix = $.Localize(`${perk_name}_add_prefix`);
			prefix += add_prefix == `${perk_name}_add_prefix` ? "" : add_prefix;
		}

		return `${prefix} ${loc_name}`;
	}
}

function StyleObjectToCSSLine(style) {
