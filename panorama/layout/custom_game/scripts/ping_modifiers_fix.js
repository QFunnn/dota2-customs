...
const MODIFIERS_EXCEPTIONS = ["modifier_oracle_prognosticate"];
...
function RegisterModifiersPanels() {
	const register_children = (type, is_debuff) => {
		...
		for (let x = modifiers_info[type].counter; x < max_length; x++) {
			...
			const mod_button = mod_panel.GetChild(0);
			mod_button.SetPanelEvent("onactivate", () => {
				...
				if (MODIFIERS_EXCEPTIONS.includes(modifier_name)) {
					$.DispatchEvent("DOTAHUDBuffClicked", mod_button);
					return;
				}
