...
/**
 * Skippable Action to animate integer dialog variable over `animation_duration` seconds
 * @param {any} panel
 ...
 */
function SkippableAnimateDialogVariableIntAction(panel, variable_name, start_value, end_value, animation_duration) {
	SkippableLerpAction.call(this, animation_duration);
	this.panel = panel;
	this.variable_name = variable_name;
	this.start_value = start_value;
	this.end_value = end_value;
}
SkippableAnimateDialogVariableIntAction.prototype = new SkippableLerpAction();
SkippableAnimateDialogVariableIntAction.prototype.apply_progress = function (progress) {
 ...
 */
function SkippableAnimateProgressBarAction(progress_bar_panel, start_value, end_value, animation_duration) {
	SkippableLerpAction.call(this, animation_duration);
	this.progress_bar_panel = progress_bar_panel;
	this.start_value = start_value;
	this.end_value = end_value;
}
SkippableAnimateProgressBarAction.prototype = new SkippableLerpAction();
SkippableAnimateProgressBarAction.prototype.apply_progress = function (progress) {
	this.progress_bar_panel.value = Lerp(progress, this.start_value, this.end_value);
};

/**
 * Play skippable sound effect script for a given `duration`
 * @param {String} sound_name
 * @param {Number} duration - sound effect duration in seconds
 */
function SkippablePlaySoundForDurationAction(sound_name, duration) {
	this.sound_name = sound_name;
	this.duration = duration;
}
SkippablePlaySoundForDurationAction.prototype = new BaseAction();

SkippablePlaySoundForDurationAction.prototype.start = function () {
	this.sound_event_id = PlayUISoundEvent(this.sound_name);

	this.waitAction = new SkippableWaitAction(this.duration);
	this.waitAction.start();
};
SkippablePlaySoundForDurationAction.prototype.update = function () {
	return this.waitAction.update();
};
SkippablePlaySoundForDurationAction.prototype.finish = function () {
	StopUISoundEvent(this.sound_event_id);
