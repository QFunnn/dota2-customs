...
/**
 * @template Key,Value
 * @typedef {(firstValue: Value, secondValue: Value, firstKey: Key, secondKey: Key) => number} Comparator
 */
...
class Collection extends Map {
	 ...
	 *
	 * @param {Key} key - The key to get if it exists, or set otherwise
	 ...
	 *
	 * @param {Key[]} keys - The keys of the elements to check for
	 ...
	 *
	 * @param {(value: Value, key: Key, collection: this) => unknown} fn - The function to test with (should return a boolean)
	 * @param {unknown} thisArg - Value to use as `this` when executing the function
	 ...
	 *
	 * @template NewValue
	 * @param {(value: Value, key: Key, collection: this) => NewValue} fn - Function that produces an element of the new array, taking three arguments
	 * @param {unknown} thisArg - Value to use as `this` when executing the function
	 * @returns {NewValue[]}
	 * @example
	 * ```ts
	 ...
	 *
	 * @param {(value: Value, key: Key, collection: this) => boolean} fn - The function to test with (should return a boolean)
	 * @param {unknown} thisArg - Value to use as `this` when executing the function
	 * @example
	 * ```ts
	 ...
	 * @param {(accumulator: AccumulatorType, value: Value, key: Key, collection: this) => AccumulatorType} fn - Function used to reduce, taking four arguments; `accumulator`, `currentValue`, `currentKey` and `collection`
	 * @param {AccumulatorType} initialValue - Starting value for the accumulator
	 * @example
	 * ```ts
	 ...
	 * @example
	 * ```ts
	 * collection.count((member) => member.isActive);
	 * ```
	 */
	count(fn, thisArg) {
		if (typeof fn !== "function")
	 ...
	 *
	 * @param {Comparator<Key,Value>} compareFunction - Specifies a function that defines the sort order.
