...
function getItemImage(item_id) {
    if (item_id === 1200002) {
        return "file://{images}/custom_game/icon/coin.png";
    }
    const itemStr = item_id.toString();
    if (itemStr == "17000028")
        return `file://{images}/custom_game/store_items/${item_id}.png`;
    if (itemStr.startsWith("17") || itemStr.startsWith("15") || itemStr.startsWith("80")) {
        return `file://{images}/custom_game/items/item_${item_id}.png`;
    }
    return `file://{images}/custom_game/store_items/${item_id}.png`;
}
function HudLink() {
    ...
    const titleImage = (() => {
        if (language == "schinese")
            return `file://{images}/custom_game/new_link/t_cn.png`;
        ...
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "LinkRightPanel" },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "LinkExchangeList" },
                ...
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "ExchangeList" }, rewardTiersArray.map((tier, i) => {
                    ...
                    const received = tier.status === 2;
                    const finished = tier.status === 1;
                    const point = tier.target_num;
                    const amount = reward_item.amounts;
                    const rarity = i + 1;
