...
var ALERT_PREV_IDS = null
var NEW_ITEMS_ALERT_WAITING = false
var NEW_ITEMS_ALERT_DELAY = 4
CustomNetTables.SubscribeNetTableListener("sub_data", function(table_name, key, data)
{
    ...
    {
        for (let k in data.new_items)
        {
            new_key = new_key + "_" + data.new_items[k]
            ids[Number(data.new_items[k])] = true
        }
    }
    if (new_key == LAST_NEW_ITEMS_KEY) { return }
    LAST_NEW_ITEMS_KEY = new_key
    let added = []
    if (ALERT_PREV_IDS != null)
    {
        for (let id in ids)
        {
            if (!ALERT_PREV_IDS[id]) { added.push(id) }
        }
    }
    ALERT_PREV_IDS = ids
...
(function()
{
    let sub_init = CustomNetTables.GetTableValue("sub_data", Players.GetLocalPlayer())
    let init_ids = {}
    let init_key = ""
    if (sub_init && sub_init.new_items)
    {
        for (let k in sub_init.new_items)
        {
            init_key = init_key + "_" + sub_init.new_items[k]
            init_ids[Number(sub_init.new_items[k])] = true
        }
    }
    ALERT_PREV_IDS = init_ids
    LAST_NEW_ITEMS_KEY = init_key
    let cfg = GameUI.CustomUIConfig()
    //cfg.pending_new_items_alert = [21]

    if (cfg.pending_new_items_alert && cfg.pending_new_items_alert.length > 0)
    {
        ScheduleNewItemsAlert()
    }
})()

function GetChestItemInfoById(item_id)
{
    let id = String(item_id)
    let pools = ["poor_chests", "rich_chests"]
    for (let p of pools)
    {
        let pool = CustomNetTables.GetTableValue("shop_items", p)
        for (let k in pool)
        {
            let chest_items = pool[k] ? pool[k].chest_items : null
            for (let i in chest_items)
            {
                if (chest_items[i] && String(chest_items[i].item_id) == id) { return chest_items[i] }
            }
        }
    }
    return null
...
function GetAnyItemIconById(item_id)
{
    let id = String(item_id)

    let chest_item = GetChestItemInfoById(id)
    if (chest_item) { return chest_item.item_icon }

    for (let hero in SAVE_DATA_SETS_ITEMS)
    {
        let items = SAVE_DATA_SETS_ITEMS[hero]
        if (items && items[id] && items[id]["icon"])
        {
            return 's2r://panorama/images/' + items[id]["icon"] + '.png'
        }
    }

    let lists = ["pets", "tips", "high_five"]
    for (let l of lists)
    {
        let list = CustomNetTables.GetTableValue("shop_items", l)
        for (let k in list)
        {
            if (list[k] && String(list[k][1]) == id) { return list[k][5] }
        }
    }

    let effects = CustomNetTables.GetTableValue("shop_items", "effects")
    for (let effect_type in effects)
    {
        for (let k in effects[effect_type])
        {
            let e = effects[effect_type][k]
            if (e && String(e[1]) == id) { return e[5] }
        }
    }

    let heroes_list = CustomNetTables.GetTableValue("custom_pick", "hero_list")
    for (let hero in heroes_list)
    {
        if (SAVE_DATA_SETS_ITEMS[hero]) { continue }
        let items = CustomNetTables.GetTableValue("heroes_items_info", hero)
        if (items && items[id] && items[id]["icon"])
        {
            return 's2r://panorama/images/' + items[id]["icon"] + '.png'
        }
...
GameEvents.Subscribe_custom('new_items_alert', OnNewItemsAlertEvent)
function OnNewItemsAlertEvent(data)
{
    if (!data || !data.items) { return }
    let cfg = GameUI.CustomUIConfig()
    if (!cfg.pending_new_items_alert) { cfg.pending_new_items_alert = [] }
    for (let k in data.items)
    {
        let item_id = Number(data.items[k])
        if (!isNaN(item_id)) { cfg.pending_new_items_alert.push(item_id) }
    }
    if (cfg.pending_new_items_alert.length == 0) { return }
    ScheduleNewItemsAlert()
}

function GetSoundInfoById(item_id)
{
    let id = String(item_id)
    let sounds = ALL_TABLE_CUSTOM_SOUNDS
    if (!sounds) { return null }
    let categories = ["general_ru", "general_eng", "general_other"]
    for (let c of categories)
    {
        for (let k in sounds[c])
        {
            let sound = sounds[c][k]
            if (sound && String(sound[1]) == id)
            {
                return { item_name: sound[2], item_icon: "file://{images}/custom_game/shop/icons/chat_wheel_item.png", is_sound: 1, drop_type: "category_chat_wheel" }
            }
        }
    }
    return null
...
var EFFECT_SECTION_BY_TYPE =
{
    emblems: "effects_emblems",
    effect_attack: "attack",
    effect_regeneration: "regeneration",
    effect_teleportation: "effects_scroll",
    effect_blink: "effects_blink",
    effect_eul: "effects_eul",
    effect_force_staff: "effects_pike",
    effect_phase_boots: "effects_phase",
    effect_radiance: "effects_radiance",
    effect_mekansm: "effects_greaves",
    effect_shivas: "effects_shiva",
    effect_mjollnir: "effects_mjollnir",
    effect_dagon: "dagon",
    effect_hex: "hex",
}

function GetAlertItemInfoById(item_id)
{
    let info = GetChestItemInfoById(item_id)
    if (info) { return info }
    info = GetSoundInfoById(item_id)
    if (info) { return info }
    let id = String(item_id)

    let cosmetic_lists = [["tips", "category_tip"], ["high_five", "category_high_five"], ["pets", "category_pet"]]
    for (let entry of cosmetic_lists)
    {
        let list = CustomNetTables.GetTableValue("shop_items", entry[0])
        for (let k in list)
        {
            if (list[k] && String(list[k][1]) == id)
            {
                return { item_icon: list[k][5], drop_type: entry[1] }
            }
        }
    }
    ...
    for (let effect_type in effects)
    {
        for (let k in effects[effect_type])
        {
            let effect_item = effects[effect_type][k]
            if (effect_item && String(effect_item[1]) == id)
            {
                let section = EFFECT_SECTION_BY_TYPE[effect_type]
                return { item_icon: effect_item[5], drop_type: section ? ("item_" + section) : "" }
            }
        }
    }
    ...
    for (let hero in SAVE_DATA_SETS_ITEMS)
    {
        ...
        if (items && items[id] && items[id]["icon"])
        {
            return { item_icon: 's2r://panorama/images/' + items[id]["icon"] + '.png', drop_type: hero }
        }
    }
    let heroes_list = CustomNetTables.GetTableValue("custom_pick", "hero_list")
    for (let hero in heroes_list)
...
function ScheduleNewItemsAlert()
{
    if (NEW_ITEMS_ALERT_WAITING) { return }
    if (!$.GetContextPanel().FindChildTraverse("new_items_alert")) { return }
    NEW_ITEMS_ALERT_WAITING = true
    CheckNewItemsAlertState()
}

function CheckNewItemsAlertState()
{
    if (Game.GetState() >= DOTA_GameState.DOTA_GAMERULES_STATE_PRE_GAME)
    {
        if (NEW_ITEMS_ALERT_DELAY > 0)
        {
            $.Schedule(NEW_ITEMS_ALERT_DELAY, FireNewItemsAlert)
        }
        else
        {
            FireNewItemsAlert()
    ...
    $.Schedule(1, CheckNewItemsAlertState)
}

function FireNewItemsAlert()
{
    NEW_ITEMS_ALERT_WAITING = false
    let main = $.GetContextPanel().FindChildTraverse("new_items_alert")
    if (!main)
    {
        return
    }
    let cfg = GameUI.CustomUIConfig()
    let ids = cfg.pending_new_items_alert || []
    cfg.pending_new_items_alert = []
    if (ids.length > 0)
    {
        ShowNewItemsAlert(ids)
    }
}

function ShowNewItemsAlert(ids)
{
    let main = $.GetContextPanel().FindChildTraverse("new_items_alert")
    let icons = $.GetContextPanel().FindChildTraverse("new_items_alert_icons")
    if (!main || !icons) { return }
    ...
    for (let k in ids)
    {
        ...
        {
            entry = entry == "shards" ? { is_dup: 1, shards: 150 } : { id: entry, is_dup: 0, shards: 0 }
        }
        ...
        let icon_url = entry.is_dup == 1 ? 'file://{images}/econ/tools/battle_points_ti11_levels_5.png' : (item_info ? item_info.item_icon : GetAnyItemIconById(entry.id))
        if (!icon_url) { continue }

        let drop_type = item_info && item_info.drop_type != null ? item_info.drop_type : ""

        let container = $.CreatePanel("Panel", icons, "")
        container.AddClass("new_items_alert_container")

        let icon_panel = $.CreatePanel("Panel", container, "")
        icon_panel.AddClass("new_items_alert_icon")
        icon_panel.style.backgroundImage = 'url("' + icon_url + '")'

        let is_square = drop_type == "category_tip"
        icon_panel.SetHasClass("new_items_alert_icon_square", is_square)
        icon_panel.style.backgroundSize = is_square ? "85% 85%" : "100%"
        icon_panel.style.backgroundRepeat = "no-repeat"
        icon_panel.style["background-position"] = "50%"

        if (entry.is_dup == 1)
        {
            drop_type = ""

            let shards_panel = $.CreatePanel("Label", container, "")
            shards_panel.AddClass("new_items_alert_shards")
            shards_panel.text = String(entry.shards > 0 ? entry.shards : 150)
        }
        else if (item_info && item_info.is_sound == 1)
        {
            let phrase_name = $.CreatePanel("Label", container, "")
            phrase_name.AddClass("new_items_alert_phrase_name")
            phrase_name.text = $.Localize("#" + item_info.item_name)
        }
        else if (drop_type.startsWith("item_"))
        {
            let name = drop_type.replace(/^item_/, "");

            let item_icon = $.CreatePanel("Panel", container, "")
            item_icon.AddClass("new_items_alert_icon_item")

            item_icon.style.backgroundImage = 'url( "file://{images}/custom_game/shop/effects/section/' + name + '.png" );'
        ...
        if (drop_type.startsWith("npc_"))
        {
            ...
            let hero_icon = $.CreatePanel("Panel", container, "")
            hero_icon.AddClass("new_items_alert_icon_hero")
        ...
        if (drop_type == "category_tip")
        {
            let tip_icon = $.CreatePanel("Panel", container, "")
            tip_icon.AddClass("new_items_alert_icon_tip")

            tip_icon.style.backgroundImage = 'url( "file://{images}/custom_game/tips/tip_0.png" );'
            tip_icon.style.backgroundSize = '150%';
    ...
    $.Schedule(10, function()
    {
        ...
        main.AddClass("new_items_alert_hidden")
...
function GetItemIdFromBlockId(panel_id)
{
    ...
    if (panel_id.indexOf("item_id_") != 0) { return null }
    let item_id = Number(panel_id.substring("item_id_".length))
    return isNaN(item_id) ? null : item_id
}

function MarkItemSeen(item_id)
{
    if (item_id == null) { return }
    GameEvents.SendCustomGameEventToServer_custom("mark_item_seen", { item_id: item_id })
}
...
function SetBlockNewBadge(block, is_new, extra_class)
{
    ...
    let hover_catcher = block.FindChildTraverse("block_new_hover")
    if (is_new)
    {
        ...
        let item_id = GetItemIdFromBlockId(block.id)
        if (item_id != null && !hover_catcher)
        {
            hover_catcher = $.CreatePanel("Panel", block, "block_new_hover")
            hover_catcher.AddClass("BlockItemNewHover")
            hover_catcher.hittest = true
            hover_catcher.SetPanelEvent("onmouseover", function()
            {
                hover_catcher.DeleteAsync(0)
                SetBlockNewBadge(block, false)
                MarkItemSeen(item_id)
            })
        }
    }
    else
    {
        ...
        if (hover_catcher) { hover_catcher.DeleteAsync(0) }
    }
}
...
function RefreshNewItemBadges(new_items_source)
{
    ...
    for (let iid in all_ids)
    {
        ...
        if (block) { SetBlockNewBadge(block, new_ids[iid] === true && HasItemInventory(iid)) }
    }
    ...
    if (current_shop_hero_choose && SETS_PRIORITY[current_shop_hero_choose])
    {
        for (let si = 0; si < SETS_PRIORITY[current_shop_hero_choose].length; si++)
        {
            ...
            {
                if (new_ids[Number(set_info[0][sid])] === true && HasItemInventory(set_info[0][sid])) { has_new = true; break }
            }
