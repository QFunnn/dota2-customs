...
function CreateSoundInWheel(panel, sound)
{
    ...
    if (HasItemInventory(sound[0]) && typeof IsItemNewForPlayer == "function" && IsItemNewForPlayer(sound[0]))
    {
        var SoundNewBadge = $.CreatePanel("Label", SoundSelect, "sound_new_badge");
        SoundNewBadge.AddClass("BlockItem_new_badge");
        SoundNewBadge.AddClass("SoundNewBadge");
        SoundNewBadge.text = $.Localize("#chest_new_item")

        SoundSelect.SetPanelEvent("onmouseover", function()
        {
            SoundSelect.ClearPanelEvent("onmouseover")
            let sound_new_badge = SoundSelect.FindChildTraverse("sound_new_badge")
            if (sound_new_badge) { sound_new_badge.DeleteAsync(0) }
            if (typeof MarkItemSeen == "function") { MarkItemSeen(sound[0]) }
        })
    }
...
function SetEventChatWheel(panel, sound_name, id, active)
{
	...
	panel.SetPanelEvent("onactivate", function()
    {
		...
		let sound_new_badge = panel.FindChildTraverse("sound_new_badge")
		if (sound_new_badge) { sound_new_badge.DeleteAsync(0) }
	})
}
