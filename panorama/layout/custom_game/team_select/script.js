...
function TeamSelect() {
    ...
    let RankData = {};
    for (const i in player_rank_data) {
        const data = player_rank_data[i];
        if (data != undefined) {
            RankData[data.uid] = data;
        }
    }
    function GetTeams() {
        let TeamPlayerList = [];
        ...
        for (const TeamID of AllTeamIDs) {
            ...
            const MaxPlayerCount = Team.team_max_players;
            TeamPlayerList.push({ TeamID: TeamID, TeamPlayers: [] });
            ...
            if (thisTeam != undefined) {
                ...
                if (thisTeam.TeamPlayers.length < MaxPlayerCount) {
                    for (let i = 0; i < MaxPlayerCount - thisTeam.TeamPlayers.length; i++) {
                        thisTeam.TeamPlayers.push(-1);
                    }
                }
            }
        }
