...
//--------------------------------------------------------------------------------------------------
// Статус загрузки данных игроков с веб-сервера (loading / error / ready).
// Пока данные грузятся - показываем "Загрузка", при ошибке подключения - "Ошибка подключения
// к серверу". Сервер сам не даёт стартовать игре, пока состояние не станет "ready".
//--------------------------------------------------------------------------------------------------
var g_SetupStatusReceived = false;

function UpdateSetupLoadStatus( state )
{
	var statusPanel = $( "#UpUpLoadStatus" );
	var statusLabel = $( "#UpUpLoadStatusLabel" );
	if ( statusPanel === null || statusLabel === null )
		return;

	statusPanel.SetHasClass( "loading", state === "loading" );
	statusPanel.SetHasClass( "error", state === "error" );
	statusPanel.SetHasClass( "ready", state === "ready" );

	if ( state === "error" )
		statusLabel.text = $.Localize( "#upup_setup_error" );
	else
		statusLabel.text = $.Localize( "#upup_setup_loading" );
}

function OnSetupLoadStatus( data )
{
	g_SetupStatusReceived = true;
	UpdateSetupLoadStatus( data && data.state ? data.state : "loading" );
}

function RequestSetupLoadStatus( attemptsLeft )
{
	// Запрашиваем текущий статус у сервера. Повторяем, пока не получим первый ответ
	// (на случай, если запрос ушёл раньше, чем сервер зарегистрировал слушатель).
...
(function()
{
	...
	// Статус загрузки данных с веб-сервера: подписываемся и запрашиваем текущее состояние.
	UpdateSetupLoadStatus( "loading" );
