...
function UpdatePickButton(buttons) {
	...
	if (Players.GetSelectedHeroID(Players.GetLocalPlayer()) != 0) {
	...
	const nt = CustomNetTables.GetTableValue("game_state", "ban_state_endtime");
	const current_time = Game.GetGameTime();
	const start_time = nt ? nt.time : current_time;
	const display_time = start_time - current_time + wait_time[level];
...
function PickRandomHero() {
	...
	if (random_pressed || Players.GetSelectedHeroID(Players.GetLocalPlayer()) != 0) return;
	random_pressed = true;
...
function UpdateBanStage() {
	const dota_pre_game_hud = FindDotaHudElement("PreGame");
	if (!dota_pre_game_hud) return void $.Schedule(0.5, UpdateBanStage);

	const hero_inspect = FindDotaHudElement("HeroInspect");
	if (!hero_inspect) return void $.Schedule(0.5, UpdateBanStage);

	const hero_movie = FindDotaHudElement("HeroMovie");
	const _UpdateHeroInspect = () => {
	...
	$.RegisterEventHandler("PanelStyleChanged", hero_inspect, _UpdateHeroInspect);
}
