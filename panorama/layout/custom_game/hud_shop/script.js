...
const SHOP_RECIPE_NET_TABLE = "common";
const SHOP_RECIPE_MANIFEST_KEY = "shop_recipe_manifest";
...
const AGHANIMS_BLESSING_RECIPE_ID = 270;
const AGHANIMS_BLESSING_RECIPE_COST = 1600;
const MAX_QUICK_BUY_GOALS = 12;
const MAX_VISIBLE_QUICK_BUY_ITEMS = 8;
const INVENTORY_LAST_SLOT = 14;
const SHOP_DRAG_ITEM_ATTRIBUTE = "custom_shop_drag_item";
const PHYSICAL_SHOP_TYPES = [
...
const COMBINE_TREE_WIDTH = 394;
const COMBINE_ITEM_WIDTH = 40;
const COMBINE_ITEM_HEIGHT = 29;
const COMBINE_LEVEL_GAP = 31;
const COMBINE_TREE_PADDING = 3;
const TAB_DEFINITIONS = [
    { id: "basic", title: "#DOTA_Shop_Category_Basics" },
    { id: "upgrade", title: "#DOTA_Shop_Category_Upgrades" },
    { id: "side", title: "#DOTA_SHOP_NAME_SIDE" },
    { id: "pregame", title: "#DOTA_Shop_Category_Shop" },
];
let nativeShopPassthrough = false;
let customQuickBuyPanel = null;
let purchaseFirstQuickBuyItem = null;
let customShopDragActive = false;
let customShopDragOverQuickBuy = false;
let customShopDragSourcePanel = null;
function hideCustomQuickBuy() {
    if (!customQuickBuyPanel) {
        return;
    }
    customQuickBuyPanel.style.visibility = "collapse";
    customQuickBuyPanel.style.opacity = "0";
    customQuickBuyPanel.hittest = false;
    customQuickBuyPanel.hittestchildren = false;
}
function showCustomQuickBuy() {
    if (!customQuickBuyPanel) {
        return;
    }
    customQuickBuyPanel.SetHasClass("PotentialDropTarget", customShopDragActive);
    customQuickBuyPanel.SetHasClass("DragOver", customShopDragActive && customShopDragOverQuickBuy);
    if (customShopDragActive) {
        customQuickBuyPanel.style.visibility = "visible";
        customQuickBuyPanel.style.opacity = "1";
        customQuickBuyPanel.hittest = true;
        customQuickBuyPanel.hittestchildren = false;
        return;
    }
    const hasGoals = !customQuickBuyPanel.BHasClass("Empty");
    customQuickBuyPanel.style.visibility = "visible";
    customQuickBuyPanel.style.opacity = hasGoals ? "1" : "0.3";
    customQuickBuyPanel.hittest = hasGoals;
    customQuickBuyPanel.hittestchildren = hasGoals;
}
function setCustomShopDragState(active, overQuickBuy = false) {
    customShopDragActive = active;
    customShopDragOverQuickBuy = active && overQuickBuy;
    showCustomQuickBuy();
}
function setShopDragItem(panel, itemName) {
    if (panel) {
        panel.SetAttributeString(SHOP_DRAG_ITEM_ATTRIBUTE, itemName);
    }
}
function getShopDragItem(panel) {
    const itemName = (panel === null || panel === void 0 ? void 0 : panel.GetAttributeString(SHOP_DRAG_ITEM_ATTRIBUTE, "")) || "";
    return /^item_[A-Za-z0-9_]+$/.test(itemName) ? itemName : "";
}
function getHudRoot() {
...
function getNativeQuickBuy() {
    var _a, _b;
    const lowerHud = (_a = getHudRoot()) === null || _a === void 0 ? void 0 : _a.FindChildTraverse("lower_hud");
    const shopLauncher = lowerHud === null || lowerHud === void 0 ? void 0 : lowerHud.FindChildTraverse("shop_launcher_block");
    return (shopLauncher === null || shopLauncher === void 0 ? void 0 : shopLauncher.FindChildTraverse("quickbuy")) || ((_b = getHudRoot()) === null || _b === void 0 ? void 0 : _b.FindChildTraverse("quickbuy")) || null;
}
function suppressNativeQuickBuy() {
    const nativeQuickBuy = getNativeQuickBuy();
    const nativeQuickBuyRows = (nativeQuickBuy === null || nativeQuickBuy === void 0 ? void 0 : nativeQuickBuy.FindChildTraverse("QuickBuyRows")) || null;
    if (!nativeQuickBuy || !nativeQuickBuyRows) {
        hideCustomQuickBuy();
        customQuickBuyPanel === null || customQuickBuyPanel === void 0 ? void 0 : customQuickBuyPanel.SetHasClass("Attached", false);
        return;
    }
    if (nativeQuickBuy.GetAttributeInt("custom_shop_children_restored", 0) === 0) {
        for (let childIndex = 0; childIndex < nativeQuickBuy.GetChildCount(); childIndex++) {
            const child = nativeQuickBuy.GetChild(childIndex);
            if (!child || child.id === "CustomQuickBuy") {
                continue;
            }
            const inlineOpacity = child.style.opacity;
            if (inlineOpacity !== null && Number(inlineOpacity) === 0 && !child.hittest && !child.hittestchildren) {
                child.style.opacity = null;
                child.hittest = true;
                child.hittestchildren = true;
            }
        }
        nativeQuickBuy.SetAttributeInt("custom_shop_children_restored", 1);
    }
    nativeQuickBuy.style.overflow = "noclip";
    nativeQuickBuyRows.style.visibility = "collapse";
    nativeQuickBuyRows.style.opacity = "0";
    nativeQuickBuyRows.hittest = false;
    nativeQuickBuyRows.hittestchildren = false;
    const nativeOverflowIndicator = nativeQuickBuy.FindChildTraverse("OverflowIndicator");
    if (nativeOverflowIndicator) {
        nativeOverflowIndicator.style.visibility = "collapse";
        nativeOverflowIndicator.hittest = false;
    }
    if (customQuickBuyPanel) {
        if (customQuickBuyPanel.GetParent() !== nativeQuickBuy) {
            customQuickBuyPanel.SetParent(nativeQuickBuy);
        }
        customQuickBuyPanel.SetHasClass("Attached", true);
        showCustomQuickBuy();
    }
}
function suppressNativeShop() {
    const nativeShop = getNativeShop();
    if (!nativeShop) {
        return null;
    }
    const main = nativeShop.FindChildTraverse("Main");
    if (main) {
        main.style.visibility = "visible";
        main.style.opacity = nativeShopPassthrough ? "1" : "0";
        main.hittest = nativeShopPassthrough;
        main.hittestchildren = nativeShopPassthrough;
    }
    if (nativeShopPassthrough) {
        const neutralCategory = nativeShop.FindChildTraverse("GridNeutralsCategory");
        if (neutralCategory) {
            neutralCategory.style.overflow = "squish scroll";
            const tiers = neutralCategory.FindChildrenWithClassTraverse("NeutralItemsTier");
            for (let index = 0; index < tiers.length; index++) {
                tiers[index].style.minWidth = "310px";
            }
        }
        const hiddenNeutralControls = [
            "GridBasicsTab",
            "GridNewShopTab",
            "GridUpgradesTab",
        ...
        for (let index = 0; index < hiddenNeutralControls.length; index++) {
            const panel = nativeShop.FindChildTraverse(hiddenNeutralControls[index]);
            if (panel) {
    ...
    const guideButtonIds = ["GuidesButton", "GuidesButtonV2"];
    for (let index = 0; index < guideButtonIds.length; index++) {
        const panel = nativeShop.FindChildTraverse(guideButtonIds[index]);
        if (panel) {
            panel.style.visibility = "collapse";
            panel.hittest = false;
            panel.hittestchildren = false;
...
function getCommonNetTableValue(key) {
    return CustomNetTables.GetTableValue(SHOP_RECIPE_NET_TABLE, key);
}
function decodeRequirements(encoded) {
    const result = [];
    if (!encoded) {
        return result;
    }
    ...
    for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
        const match = /^(item_[A-Za-z0-9_]+)(?:\*(\d+))?$/.exec(tokens[tokenIndex]);
        if (!match) {
            continue;
        }
...
function readShopRecipeIndex() {
    var _a;
    const byResult = {};
    const byComponent = {};
    const manifest = getCommonNetTableValue(SHOP_RECIPE_MANIFEST_KEY);
    const chunkCount = Math.max(0, Number(manifest === null || manifest === void 0 ? void 0 : manifest.c) || 0);
    for (let chunkIndex = 1; chunkIndex <= chunkCount; chunkIndex++) {
        const chunk = (_a = getCommonNetTableValue(`${SHOP_RECIPE_CHUNK_PREFIX}${chunkIndex}`)) === null || _a === void 0 ? void 0 : _a.d;
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            if ((fields.length !== 4 && fields.length !== 5) || !/^item_[A-Za-z0-9_]+$/.test(fields[0])) {
                continue;
            }
            byResult[fields[0]] = {
                resultName: fields[0],
                recipeName: fields[1],
                cost: Math.max(0, Number(fields[2]) || 0),
                requirements: decodeRequirements(fields[3]),
                upgradeLevel: Math.max(0, Number(fields[4]) || 0),
            };
        }
    }
    const blessingRecipe = byResult[AGHANIMS_BLESSING_ITEM];
    if (blessingRecipe) {
        blessingRecipe.recipeName = blessingRecipe.recipeName || AGHANIMS_BLESSING_RECIPE;
        blessingRecipe.cost = blessingRecipe.cost > 0 ? blessingRecipe.cost : AGHANIMS_BLESSING_RECIPE_COST;
        blessingRecipe.requirements = [AGHANIMS_SCEPTER_ITEM];
        blessingRecipe.upgradeLevel = Math.max(2, blessingRecipe.upgradeLevel);
    }
    else {
        byResult[AGHANIMS_BLESSING_ITEM] = {
            resultName: AGHANIMS_BLESSING_ITEM,
            recipeName: AGHANIMS_BLESSING_RECIPE,
            cost: AGHANIMS_BLESSING_RECIPE_COST,
            requirements: [AGHANIMS_SCEPTER_ITEM],
            upgradeLevel: 2,
        };
    }
    const resultNames = Object.keys(byResult);
    for (let resultIndex = 0; resultIndex < resultNames.length; resultIndex++) {
        const recipe = byResult[resultNames[resultIndex]];
        const seenComponents = {};
        for (let componentIndex = 0; componentIndex < recipe.requirements.length; componentIndex++) {
            const componentName = recipe.requirements[componentIndex];
            if (seenComponents[componentName]) {
                continue;
            }
            seenComponents[componentName] = true;
            byComponent[componentName] = byComponent[componentName] || [];
            byComponent[componentName].push(recipe.resultName);
        }
    }
    return { byResult, byComponent };
}
function readShopItemCosts() {
    var _a;
    const itemCosts = {};
    const manifest = getCommonNetTableValue(SHOP_ITEM_COST_MANIFEST_KEY);
    const chunkCount = Math.max(0, Number(manifest === null || manifest === void 0 ? void 0 : manifest.c) || 0);
    for (let chunkIndex = 1; chunkIndex <= chunkCount; chunkIndex++) {
        const chunk = (_a = getCommonNetTableValue(`${SHOP_ITEM_COST_CHUNK_PREFIX}${chunkIndex}`)) === null || _a === void 0 ? void 0 : _a.d;
        if (typeof chunk !== "string" || chunk.length === 0) {
            continue;
        }
        const lines = chunk.split("\n");
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            const fields = lines[lineIndex].split("#");
            if (fields.length !== 2 || !/^item_[A-Za-z0-9_]+$/.test(fields[0])) {
                continue;
            }
            itemCosts[fields[0]] = Math.max(0, Number(fields[1]) || 0);
        }
    }
    return itemCosts;
}
...
function includeAghanimsBlessingRecipe(profile) {
    ...
    for (let categoryIndex = 0; categoryIndex < categoryIds.length; categoryIndex++) {
        ...
        const items = profile[categoryId] || [];
        if (items.indexOf(AGHANIMS_BLESSING_RECIPE) >= 0) {
            return profile;
        }
        if (!targetCategoryId && items.indexOf(AGHANIMS_SCEPTER_ITEM) >= 0) {
    ...
    const items = profile[targetCategoryId] || [];
    const anchorIndex = items.indexOf(AGHANIMS_SCEPTER_ITEM);
    const nextItems = items.slice();
    nextItems.splice(anchorIndex + 1, 0, AGHANIMS_BLESSING_RECIPE);
    return Object.assign(Object.assign({}, profile), { [targetCategoryId]: nextItems });
}
...
function isLoneDruidSpiritBear(entityIndex) {
    ...
    return playerId >= 0
        && entityIndex >= 0
        && Entities.IsControllableByPlayer(entityIndex, playerId)
        && /^npc_dota_lone_druid_bear\d*$/.test(Entities.GetUnitName(entityIndex));
}
function getShopPurchaseTargetEntityIndex() {
    const playerId = Players.GetLocalPlayer();
    const heroEntityIndex = Players.GetPlayerHeroEntityIndex(playerId);
    if (playerId < 0 || heroEntityIndex < 0) {
        return heroEntityIndex;
    }
    const portraitUnit = Players.GetLocalPlayerPortraitUnit();
    return isLoneDruidSpiritBear(portraitUnit) ? portraitUnit : heroEntityIndex;
}
function getOwnedItemCounts(heroEntityIndex) {
    const counts = {};
    if (heroEntityIndex < 0) {
        return counts;
    }
    for (let slot = 0; slot <= INVENTORY_LAST_SLOT; slot++) {
        const itemEntityIndex = Entities.GetItemInSlot(heroEntityIndex, slot);
        if (itemEntityIndex < 0) {
            continue;
        }
        const itemName = Abilities.GetAbilityName(itemEntityIndex);
        if (itemName) {
...
function getOwnedItemCount(itemName) {
    return getOwnedItemCounts(getShopPurchaseTargetEntityIndex())[itemName] || 0;
}
function getLocalPlayerGold() {
    const playerId = Players.GetLocalPlayer();
    return playerId >= 0 ? Math.max(0, Number(Players.GetGold(playerId)) || 0) : 0;
}
function canAffordShopItem(itemName, itemCosts, currentGold) {
    return itemCosts[itemName] !== undefined && currentGold >= itemCosts[itemName];
}
function getQuickBuyItemCost(itemName, itemCosts) {
    const syncedCost = itemCosts[itemName];
    return syncedCost === undefined
        ? Math.max(0, Number(GetItemCost(itemName)) || 0)
        : syncedCost;
}
function buildQuickBuyPlan(goals, recipeIndex, ownedCounts, itemCosts) {
    const availableCounts = Object.assign({}, ownedCounts);
    const entries = [];
    let entryIndex = 0;
    const planItem = (itemName, useOwnedItem, visiting, depth) => {
        if (useOwnedItem && (availableCounts[itemName] || 0) > 0) {
            availableCounts[itemName]--;
            return;
        }
        const recipe = recipeIndex.byResult[itemName];
        if (!recipe || visiting[itemName] || depth > 16) {
            entries.push({
                key: `${itemName}_${entryIndex++}`,
                itemName,
                cost: getQuickBuyItemCost(itemName, itemCosts),
            });
            return;
        }
        const nextVisiting = Object.assign(Object.assign({}, visiting), { [itemName]: true });
        for (let requirementIndex = 0; requirementIndex < recipe.requirements.length; requirementIndex++) {
            planItem(recipe.requirements[requirementIndex], true, nextVisiting, depth + 1);
        }
        if (recipe.cost > 0) {
            if ((availableCounts[recipe.recipeName] || 0) > 0) {
                availableCounts[recipe.recipeName]--;
            }
            else {
                entries.push({
                    key: `${recipe.recipeName}_${entryIndex++}`,
    ...
    for (let goalIndex = 0; goalIndex < goals.length; goalIndex++) {
        planItem(goals[goalIndex].itemName, false, {}, 0);
    }
    return entries;
}
function isUnitNearPhysicalShop(unitEntityIndex) {
    if (unitEntityIndex < 0) {
        return false;
    }
    for (let shopIndex = 0; shopIndex < PHYSICAL_SHOP_TYPES.length; shopIndex++) {
        if (Entities.IsInRangeOfShop(unitEntityIndex, PHYSICAL_SHOP_TYPES[shopIndex], true)) {
            return true;
        }
    }
    return false;
}
function getShopItemSlotId(categoryId, itemIndex) {
    return `CustomShopItemSlot_${categoryId}_${itemIndex}`;
}
function showShopItemTooltip(panel, itemName) {
    const purchaseTargetEntityIndex = getShopPurchaseTargetEntityIndex();
    $.DispatchEvent("DOTAShowAbilityShopItemTooltip", panel, itemName, "", purchaseTargetEntityIndex, -1);
}
function purchaseShopItem(itemName) {
    const heroEntityIndex = Players.GetPlayerHeroEntityIndex(Players.GetLocalPlayer());
    const purchaseTargetEntityIndex = getShopPurchaseTargetEntityIndex();
    if (heroEntityIndex < 0 || purchaseTargetEntityIndex < 0) {
        return;
    }
    GameEvents.SendCustomGameEventToServer(SHOP_PURCHASE_EVENT, {
        item_name: itemName,
        hero_entindex: heroEntityIndex,
        purchase_target_entindex: purchaseTargetEntityIndex,
    });
}
function selectShopItem(itemName, onSelect, onQuickBuy) {
    onSelect(itemName);
    if (!GameUI.IsShiftDown()) {
        return;
    }
    onQuickBuy(itemName, GameUI.IsControlDown());
}
function activateNativeShopControl(controlId) {
    var _a;
    const control = (_a = getHudRoot()) === null || _a === void 0 ? void 0 : _a.FindChildTraverse(controlId);
    if (control) {
        $.DispatchEvent("Activated", control, "mouse");
    }
}
function showTextTooltip(panel, token) {
    $.DispatchEvent("DOTAShowTextTooltip", panel, $.Localize(token));
}
function getTabForShopType(shopType) {
    if (shopType === DOTA_SHOP_TYPE.DOTA_SHOP_NEUTRALS) {
        return "neutral";
    }
    if (shopType === DOTA_SHOP_TYPE.DOTA_SHOP_SIDE || shopType === DOTA_SHOP_TYPE.DOTA_SHOP_SIDE2) {
        return "side";
    }
    if (shopType === DOTA_SHOP_TYPE.DOTA_SHOP_SECRET || shopType === DOTA_SHOP_TYPE.DOTA_SHOP_SECRET2) {
        return "secret";
    }
    return null;
}
function buildRecipeTree(selectedItem, recipeIndex, itemOrder) {
    if (!selectedItem) {
        return { nodes: [], edges: [], width: COMBINE_TREE_WIDTH, height: 0 };
    }
    const upgrades = [];
    const seenUpgrades = { [selectedItem]: true };
    const collectUpgradeChain = (componentName, depth) => {
        if (depth > 16) {
            return;
        }
        const results = recipeIndex.byComponent[componentName] || [];
        for (let resultIndex = 0; resultIndex < results.length; resultIndex++) {
            const resultName = results[resultIndex];
            const resultRecipe = recipeIndex.byResult[resultName];
            if (!resultRecipe || resultRecipe.upgradeLevel <= 0 || seenUpgrades[resultName]) {
                continue;
            }
            seenUpgrades[resultName] = true;
            upgrades.push(resultName);
            collectUpgradeChain(resultName, depth + 1);
        }
    };
    collectUpgradeChain(selectedItem, 0);
    const directUpgrades = recipeIndex.byComponent[selectedItem] || [];
    for (let upgradeIndex = 0; upgradeIndex < directUpgrades.length; upgradeIndex++) {
        const itemName = directUpgrades[upgradeIndex];
        if (itemOrder[itemName] === undefined || seenUpgrades[itemName]) {
            continue;
        }
        seenUpgrades[itemName] = true;
        upgrades.push(itemName);
    }
    upgrades.sort((left, right) => {
        var _a, _b;
        const leftLevel = ((_a = recipeIndex.byResult[left]) === null || _a === void 0 ? void 0 : _a.upgradeLevel) || 0;
        const rightLevel = ((_b = recipeIndex.byResult[right]) === null || _b === void 0 ? void 0 : _b.upgradeLevel) || 0;
        if (leftLevel > 0 && rightLevel > 0 && leftLevel !== rightLevel) {
            return leftLevel - rightLevel;
        }
        const leftOrder = itemOrder[left] === undefined ? 2147483647 : itemOrder[left];
        const rightOrder = itemOrder[right] === undefined ? 2147483647 : itemOrder[right];
        const orderDelta = leftOrder - rightOrder;
        return orderDelta || getLocalizedItemName(left).localeCompare(getLocalizedItemName(right));
    });
    const recipe = recipeIndex.byResult[selectedItem];
    const components = recipe ? recipe.requirements.slice() : [];
    if (recipe && recipe.cost > 0) {
        components.push(recipe.recipeName);
    }
    const width = COMBINE_TREE_WIDTH;
    const getRowX = (nodeIndex, nodeCount) => {
        if (nodeCount <= 1) {
            return Math.round((width - COMBINE_ITEM_WIDTH) * 0.5);
        }
        const availableWidth = width - COMBINE_TREE_PADDING * 2 - COMBINE_ITEM_WIDTH;
        return COMBINE_TREE_PADDING + Math.round(availableWidth * nodeIndex / (nodeCount - 1));
    };
    const nodes = [];
    let rowIndex = 0;
    const topNodes = [];
    if (upgrades.length > 0) {
        for (let index = 0; index < upgrades.length; index++) {
            const node = {
                key: `upgrade_${upgrades[index]}_${index}`,
                itemName: upgrades[index],
                x: getRowX(index, upgrades.length),
                y: COMBINE_TREE_PADDING + rowIndex * COMBINE_LEVEL_GAP,
                current: false,
            };
            topNodes.push(node);
            nodes.push(node);
    ...
    if (components.length > 0) {
        for (let index = 0; index < components.length; index++) {
            const node = {
                key: `component_${components[index]}_${index}`,
                itemName: components[index],
                x: getRowX(index, components.length),
                y: COMBINE_TREE_PADDING + rowIndex * COMBINE_LEVEL_GAP,
                current: false,
            };
    ...
    const edgePairs = [];
    for (let index = 0; index < topNodes.length; index++) {
        edgePairs.push([topNodes[index], currentNode]);
    }
    for (let index = 0; index < bottomNodes.length; index++) {
        edgePairs.push([currentNode, bottomNodes[index]]);
    }
    const edges = edgePairs.map(([startNode, endNode], edgeIndex) => {
        const startX = startNode.x + COMBINE_ITEM_WIDTH * 0.5;
        const startY = startNode.y + COMBINE_ITEM_HEIGHT * 0.5;
        const endX = endNode.x + COMBINE_ITEM_WIDTH * 0.5;
        const endY = endNode.y + COMBINE_ITEM_HEIGHT * 0.5;
        const deltaX = endX - startX;
        const deltaY = endY - startY;
        return {
            key: `${startNode.key}_${endNode.key}_${edgeIndex}`,
            x: Math.round(startX),
            y: Math.round(startY - 1),
            width: Math.round(Math.sqrt(deltaX * deltaX + deltaY * deltaY)),
            angle: Math.round(Math.atan2(deltaY, deltaX) * 1800 / Math.PI) / 10,
        };
    });
    return {
        nodes,
        edges,
        width,
        height: COMBINE_TREE_PADDING * 2 + COMBINE_ITEM_HEIGHT + Math.max(0, rowIndex - 1) * COMBINE_LEVEL_GAP,
    };
}
function ShopStockBadge({ stock }) {
    if (!stock) {
        return null;
    }
    return react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { className: "CustomShopStockBadge", text: `${Math.max(0, stock.c || 0)}`, hittest: false });
}
function ShopAffordableBorder() {
    return react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopAffordableBorder", hittest: false });
}
function ShopItemImage({ itemName, className, style }) {
    const dagonMatch = /^item_dagon(?:_([2-5]))?$/.exec(itemName);
    if (dagonMatch) {
        const levelSuffix = dagonMatch[1] ? `_${dagonMatch[1]}` : "";
        return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Image, { className: className, src: `s2r://panorama/images/items/dagon${levelSuffix}_png.vtex`, scaling: "stretch", style: style, hittest: false }));
    }
    return react__WEBPACK_IMPORTED_MODULE_2__.createElement(DOTAItemImage, { className: className, itemname: itemName, style: style, hittest: false });
}
function ShopItem({ itemName, className, style, stock, affordable, onSelect, onQuickBuy }) {
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { ref: (panel) => setShopDragItem(panel, itemName), className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopDetailItem", className, { Affordable: affordable, OutOfStock: stock && stock.c <= 0 }), style: style, draggable: true, hittest: true, hittestchildren: false, onactivate: () => selectShopItem(itemName, onSelect, onQuickBuy), oncontextmenu: () => {
            onSelect(itemName);
            purchaseShopItem(itemName);
        }, onmouseover: (panel) => showShopItemTooltip(panel, itemName), onmouseout: (panel) => $.DispatchEvent("DOTAHideAbilityTooltip", panel) },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItemImage, { className: "CustomShopDetailItemImage", itemName: itemName }),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopAffordableBorder, null),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopStockBadge, { stock: stock })));
}
function ShopItemSlot({ categoryId, itemIndex, itemName, visible, stock, affordable, onSelect, onQuickBuy }) {
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: getShopItemSlotId(categoryId, itemIndex), className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopItemSlot", { Visible: visible }) },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { ref: (panel) => setShopDragItem(panel, itemName), className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopItem", { Affordable: affordable, OutOfStock: stock && stock.c <= 0 }), draggable: true, hittest: true, hittestchildren: false, onactivate: () => selectShopItem(itemName, onSelect, onQuickBuy), oncontextmenu: () => {
                onSelect(itemName);
                purchaseShopItem(itemName);
            }, onmouseover: (panel) => showShopItemTooltip(panel, itemName), onmouseout: (panel) => $.DispatchEvent("DOTAHideAbilityTooltip", panel) },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItemImage, { className: "CustomShopItemImage", itemName: itemName }),
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopAffordableBorder, null),
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopStockBadge, { stock: stock }))));
}
function RecipeTree({ tree, stockItems, itemCosts, currentGold, onSelect, onQuickBuy }) {
    if (tree.nodes.length === 0) {
        return null;
    }
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopCombineTree", style: { width: `${tree.width}px`, height: `${tree.height}px` } },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopCombineConnectors", hittest: false }, tree.edges.map((edge) => (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { key: edge.key, className: "CustomShopCombineConnector", style: {
                x: `${edge.x}px`,
                y: `${edge.y}px`,
                width: `${edge.width}px`,
                transform: `rotateZ(${edge.angle}deg)`,
            }, hittest: false })))),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopCombineItems" }, tree.nodes.map((node) => (react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItem, { key: node.key, itemName: node.itemName, className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopCombineItem", { CurrentItem: node.current }), style: { x: `${node.x}px`, y: `${node.y}px` }, stock: stockItems[node.itemName], affordable: canAffordShopItem(node.itemName, itemCosts, currentGold), onSelect: onSelect, onQuickBuy: onQuickBuy }))))));
}
function ShopCategoryRow({ category, items, placements, stockItems, itemCosts, currentGold, onSelect, onQuickBuy }) {
    const hasVisibleItems = items.some((itemName, itemIndex) => placements[itemName] === getShopItemSlotId(category.id, itemIndex));
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopCategoryRow", { Visible: hasVisibleItems }) },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopCategoryHeading" },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: $.Localize(category.title) })),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopCategoryItems" }, items.map((itemName, itemIndex) => (react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItemSlot, { key: `${itemName}_${itemIndex}`, categoryId: category.id, itemIndex: itemIndex, itemName: itemName, visible: placements[itemName] === getShopItemSlotId(category.id, itemIndex), stock: stockItems[itemName], affordable: canAffordShopItem(itemName, itemCosts, currentGold), onSelect: onSelect, onQuickBuy: onQuickBuy }))))));
}
function CustomQuickBuy({ goals, recipeIndex, stockItems, itemCosts, currentGold, onGoalsChange, onClear }) {
    const [tick, setTick] = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(0);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        ...
        const update = () => {
            ...
            scheduleId = $.Schedule(0.25, update);
        };
        scheduleId = $.Schedule(0.25, update);
        return () => {
            stopped = true;
    ...
    const purchaseTargetEntityIndex = getShopPurchaseTargetEntityIndex();
    const ownedCounts = (0,react__WEBPACK_IMPORTED_MODULE_2__.useMemo)(() => getOwnedItemCounts(purchaseTargetEntityIndex), [purchaseTargetEntityIndex, tick]);
    const entries = (0,react__WEBPACK_IMPORTED_MODULE_2__.useMemo)(() => buildQuickBuyPlan(goals, recipeIndex, ownedCounts, itemCosts), [goals, itemCosts, ownedCounts, recipeIndex]);
    const visibleEntries = entries.slice(0, MAX_VISIBLE_QUICK_BUY_ITEMS);
    const overflowCount = Math.max(0, entries.length - visibleEntries.length);
    const nearShop = isUnitNearPhysicalShop(purchaseTargetEntityIndex);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        const scheduleId = $.Schedule(0, suppressNativeQuickBuy);
        return () => $.CancelScheduled(scheduleId);
    }, [entries.length]);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        const firstEntry = entries[0];
        purchaseFirstQuickBuyItem = firstEntry
            ? () => purchaseShopItem(firstEntry.itemName)
            : null;
        return () => {
            purchaseFirstQuickBuyItem = null;
        };
    }, [entries]);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        const remainingGoals = goals.filter((goal) => (ownedCounts[goal.itemName] || 0) <= goal.ownedCount);
        if (remainingGoals.length !== goals.length) {
            onGoalsChange(remainingGoals);
        }
    }, [goals, onGoalsChange, ownedCounts]);
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomQuickBuy", ref: (panel) => {
            customQuickBuyPanel = panel;
            suppressNativeQuickBuy();
        }, className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Empty: goals.length === 0, OutOfRange: goals.length > 0 && !nearShop }), hittest: goals.length > 0, hittestchildren: goals.length > 0 },
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomQuickBuyDropTarget", hittest: false, hittestchildren: false }),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomQuickBuyItems" }, visibleEntries.map((entry) => {
            const stock = stockItems[entry.itemName];
            return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { key: entry.key, className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomQuickBuyItem", {
                    Affordable: canAffordShopItem(entry.itemName, itemCosts, currentGold),
                    OutOfStock: stock && stock.c <= 0,
                }), onactivate: () => purchaseShopItem(entry.itemName), oncontextmenu: () => purchaseShopItem(entry.itemName), onmouseover: (panel) => showShopItemTooltip(panel, entry.itemName), onmouseout: (panel) => $.DispatchEvent("DOTAHideAbilityTooltip", panel) },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItemImage, { className: "CustomQuickBuyItemImage", itemName: entry.itemName, style: { opacity: "1", visibility: "visible" } }),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopAffordableBorder, null),
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopStockBadge, { stock: stock })));
        })),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Button, { id: "CustomQuickBuyClear", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Disabled: goals.length === 0 }), enabled: goals.length > 0, onactivate: onClear, onmouseover: (panel) => showTextTooltip(panel, "#custom_shop_quick_buy_clear"), onmouseout: (panel) => $.DispatchEvent("DOTAHideTextTooltip", panel) },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Image, { className: "CustomQuickBuyClearIcon", src: "s2r://panorama/images/control_icons/24px/x_close.vsvg", scaling: "stretch-to-fit-preserve-aspect", hittest: false })),
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomQuickBuyOverflow", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Hidden: overflowCount === 0 }), hittest: false, hittestchildren: false },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: "+" + overflowCount }))));
}
function CustomShop() {
    var _a;
    ...
    const itemOrder = (0,react__WEBPACK_IMPORTED_MODULE_2__.useMemo)(() => {
        ...
        for (let categoryIndex = 0; categoryIndex < categories.length; categoryIndex++) {
            const items = profile[categories[categoryIndex].id] || [];
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                if (result[items[itemIndex]] === undefined) {
                    result[items[itemIndex]] = order++;
                }
            }
        }
        return result;
    }, [categories, profile]);
    const defaultSelectedItem = (0,react__WEBPACK_IMPORTED_MODULE_2__.useMemo)(() => {
        for (let categoryIndex = 0; categoryIndex < categories.length; categoryIndex++) {
            const items = profile[categories[categoryIndex].id] || [];
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                if (itemIds[items[itemIndex]]) {
                    return items[itemIndex];
                }
            }
        }
        return "";
    }, [categories, itemIds, profile]);
    const [open, setOpen] = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(Game.IsShopOpen());
    ...
    const [quickBuyGoals, setQuickBuyGoals] = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(() => {
        ...
        return storedGoals
            .filter((goal) => goal && typeof goal.itemName === "string" && /^item_[A-Za-z0-9_]+$/.test(goal.itemName))
            .slice(0, MAX_QUICK_BUY_GOALS)
            .map((goal) => ({
            itemName: goal.itemName,
            ownedCount: Math.max(0, Number(goal.ownedCount) || 0),
        }));
    });
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        ...
        const updateGold = () => {
            ...
            const nextGold = getLocalPlayerGold();
            setCurrentGold((gold) => gold === nextGold ? gold : nextGold);
    ...
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        GameUI.CustomUIConfig().CustomShopQuickBuyGoals = quickBuyGoals;
    }, [quickBuyGoals]);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(() => {
        let stopped = false;
        let scheduleId;
        const maintainHud = () => {
            ...
            suppressNativeShop();
            suppressNativeQuickBuy();
            scheduleId = $.Schedule(0.5, maintainHud);
        };
        maintainHud();
        return () => {
            stopped = true;
    ...
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DOTAHUDShopOpened", (shopType) => {
        const showNativeNeutral = shopType === DOTA_SHOP_TYPE.DOTA_SHOP_NEUTRALS;
        nativeShopPassthrough = showNativeNeutral;
        setNativeNeutralOpen(showNativeNeutral);
        suppressNativeShop();
        $.Schedule(0, suppressNativeShop);
        setOpen(true);
        const matchingTab = getTabForShopType(shopType);
        if (matchingTab && availableTabs.some((tab) => tab.id === matchingTab)) {
            setSearch("");
    ...
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DOTAHUDShopClosed", () => {
        ...
        nativeShopPassthrough = false;
        if (resetToDefaultTab) {
            setActiveTab(((_a = availableTabs[0]) === null || _a === void 0 ? void 0 : _a.id) || "basic");
        }
    ...
    const itemPlacements = (0,react__WEBPACK_IMPORTED_MODULE_2__.useMemo)(() => {
        const result = {};
        if (!searchText && activeTab === "neutral") {
            return result;
        }
        for (let categoryIndex = 0; categoryIndex < categories.length; categoryIndex++) {
            const category = categories[categoryIndex];
            const categoryMatchesTab = category.tab === activeTab || (activeTab === "basic" && category.tab === "secret");
            if (!searchText && !categoryMatchesTab) {
                continue;
            }
            const categoryItems = profile[category.id] || [];
            for (let itemIndex = 0; itemIndex < categoryItems.length; itemIndex++) {
                const itemName = categoryItems[itemIndex];
                if (!itemIds[itemName] || result[itemName]) {
                    continue;
                }
                if (searchText) {
                    const localizedName = getLocalizedItemName(itemName).toLocaleLowerCase();
                    if (!itemName.toLocaleLowerCase().includes(searchText) && !localizedName.includes(searchText)) {
                        continue;
                    }
                }
                result[itemName] = getShopItemSlotId(category.id, itemIndex);
            }
        }
        return result;
    ...
    const updateQuickBuy = (0,react__WEBPACK_IMPORTED_MODULE_2__.useCallback)((itemName, allowDuplicate) => {
        setQuickBuyGoals((currentGoals) => {
            ...
            if (currentGoals.length >= MAX_QUICK_BUY_GOALS) {
                return currentGoals;
            }
            let ownedCount = getOwnedItemCount(itemName);
            for (let goalIndex = 0; goalIndex < currentGoals.length; goalIndex++) {
                const goal = currentGoals[goalIndex];
                if (goal.itemName === itemName) {
                    ownedCount = Math.max(ownedCount, goal.ownedCount + 1);
                }
            }
            return currentGoals.concat({
                itemName,
                ownedCount,
            });
        });
    }, []);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DragStart", (sourcePanel, dragSettings) => {
        const itemName = getShopDragItem(sourcePanel);
        if (!itemName) {
            return;
        }
        customShopDragSourcePanel === null || customShopDragSourcePanel === void 0 ? void 0 : customShopDragSourcePanel.RemoveClass("Dragging");
        customShopDragSourcePanel = sourcePanel;
        sourcePanel.AddClass("Dragging");
        const dragPreview = $.CreatePanel("Panel", sourcePanel, "");
        dragPreview.AddClass("CustomShopDragPreview");
        dragPreview.SetAttributeString(SHOP_DRAG_ITEM_ATTRIBUTE, itemName);
        dragPreview.hittest = false;
        dragPreview.hittestchildren = false;
        dragSettings.displayPanel = dragPreview;
        dragSettings.offsetX = 0;
        dragSettings.offsetY = 0;
        (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.render)(react__WEBPACK_IMPORTED_MODULE_2__.createElement(ShopItemImage, { className: "CustomShopDragPreviewImage", itemName: itemName }), dragPreview);
        setCustomShopDragState(true);
        $.DispatchEvent("DOTAHideAbilityTooltip", sourcePanel);
    }, []);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DragEnter", (targetPanel, draggedPanel) => {
        if (targetPanel !== customQuickBuyPanel || !getShopDragItem(draggedPanel)) {
            return false;
        }
        setCustomShopDragState(true, true);
        return true;
    }, []);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DragLeave", (targetPanel, draggedPanel) => {
        if (targetPanel !== customQuickBuyPanel || !getShopDragItem(draggedPanel)) {
            return false;
        }
        setCustomShopDragState(true, false);
        return true;
    }, []);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DragDrop", (targetPanel, draggedPanel) => {
        const itemName = getShopDragItem(draggedPanel);
        if (targetPanel !== customQuickBuyPanel || !itemName) {
            return false;
        }
        updateQuickBuy(itemName, false);
        setCustomShopDragState(true, false);
        return true;
    }, [updateQuickBuy]);
    (0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.useRegisterForUnhandledEvent)("DragEnd", (sourcePanel, draggedPanel) => {
        if (!getShopDragItem(sourcePanel) && !getShopDragItem(draggedPanel)) {
            return;
        }
        sourcePanel.RemoveClass("Dragging");
        if (customShopDragSourcePanel && customShopDragSourcePanel !== sourcePanel) {
            customShopDragSourcePanel.RemoveClass("Dragging");
        }
        customShopDragSourcePanel = null;
        setCustomShopDragState(false);
        if (draggedPanel !== sourcePanel && draggedPanel.BHasClass("CustomShopDragPreview")) {
    ...
    const selectTab = (tab) => {
        ...
        setActiveTab(tab);
        if (tab === "neutral") {
            activateNativeShopControl("MainShopButton");
            activateNativeShopControl("GridNeutralsTab");
            nativeShopPassthrough = true;
            setNativeNeutralOpen(true);
        }
        else {
            nativeShopPassthrough = false;
            setNativeNeutralOpen(false);
    ...
    return (react__WEBPACK_IMPORTED_MODULE_2__.createElement(react__WEBPACK_IMPORTED_MODULE_2__.Fragment, null,
        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomItemShop", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({
                Open: open,
                Flip: Game.IsHUDFlipped(),
                NativeShopPassthrough: nativeNeutralOpen,
            }), visible: open && !nativeNeutralOpen, hittest: open && !nativeNeutralOpen, hittestchildren: open && !nativeNeutralOpen },
            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopFrame" },
                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopMainArea" },
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopHeader" },
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopTopBar" },
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopSearchBox" },
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { className: "CustomShopSearchIcon" }),
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(TextEntry, { id: "CustomShopSearchEntry", text: search, placeholder: "#DOTA_Search", ontextentrychange: (panel) => setSearch(panel.text) }),
                                react__WEBPACK_IMPORTED_MODULE_2__.createElement(Button, { id: "CustomShopClearSearch", className: classnames__WEBPACK_IMPORTED_MODULE_1___default()({ Visible: search.length > 0 }), onactivate: () => setSearch("") },
                                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(Image, { src: "s2r://panorama/images/control_icons/24px/x_close.vsvg" })))),
                        react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopTabs" }, availableTabs.map((tab) => (react__WEBPACK_IMPORTED_MODULE_2__.createElement(Button, { key: tab.id, className: classnames__WEBPACK_IMPORTED_MODULE_1___default()("CustomShopTab", { Active: activeTab === tab.id && !searchText }), onactivate: () => selectTab(tab.id) },
                            react__WEBPACK_IMPORTED_MODULE_2__.createElement(Label, { text: $.Localize(tab.title) })))))),
                    react__WEBPACK_IMPORTED_MODULE_2__.createElement(Panel, { id: "CustomShopBody" },
...
(0,_demon673_react_panorama__WEBPACK_IMPORTED_MODULE_0__.render)(react__WEBPACK_IMPORTED_MODULE_2__.createElement(CustomShop, null), customShopRoot);
const quickBuyKey = Game.GetKeybindForCommand(DOTAKeybindCommand_t.DOTA_KEYBIND_PURCHASE_QUICKBUY);
if (quickBuyKey) {
    RegisterKeyBind(quickBuyKey, () => purchaseFirstQuickBuyItem === null || purchaseFirstQuickBuyItem === void 0 ? void 0 : purchaseFirstQuickBuyItem());
}
$.Schedule(0, () => {
    suppressNativeShop();
    suppressNativeQuickBuy();
