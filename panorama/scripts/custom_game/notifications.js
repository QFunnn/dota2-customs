...
// [Карта справа] Отражённый интерфейс Dota: панель «Информация» переезжает к левому
// краю. Позиции и направления — в CSS (селектор .HUDFlipped); порядок элементов
// внутри строк CSS не переставляет, поэтому делаем это здесь.
//
// Отдельно важен NotificationsMainMenu: в нём рядом с телом лежит панель настроек
// шириной 210px, и без разворота тело отъезжало от края ровно на её ширину — из-за
// этого «Информация» и открывалась непонятно где.
SetupHUDFlip(MAIN_PANEL, function(bFlipped){
    let Rows = [
        MAIN_PANEL.FindChildTraverse("NotificationButton"),
        MAIN_PANEL.FindChildTraverse("NotificationsMainMenu"),
        MAIN_PANEL.FindChildTraverse("MainHeader"),
    ]

    for(let Row of Rows){
        SetChildrenReversed(Row, bFlipped)
    }
})