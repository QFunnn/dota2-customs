        ...
        if(HeroIndex != -1){
            ...
            if(HealthProgressBar){
                ...
                HealthProgressBar.SetPanelEvent("onactivate", function(){
                    ...
                    if(!IsDotaAltPressed() || !cfg.SendPingTextToChat || !cfg.BuildDuelHpPing) return
                    // [A72] шлём снимок чисел + сущность: текст соберётся на языке получателя.
                    let hpText = `${Math.round(Health)} / ${Math.round(MaxHealth)}`
                    let regenText = `${PrefHP}${HealthRegen.toFixed(1)}`
                    cfg.SendPingTextToChat(cfg.BuildDuelHpPing(HeroIndex, hpText, regenText), undefined,
                        { kind: "duel_hp", unit: HeroIndex, a: hpText, b: regenText })
                })
            }
            ...
            if(ManaProgressBar){
                ...
                ManaProgressBar.SetPanelEvent("onactivate", function(){
                    let cfg = GameUI.CustomUIConfig()
                    if(!IsDotaAltPressed() || !cfg.SendPingTextToChat || !cfg.BuildDuelManaPing) return
                    let mpText = `${Math.round(Mana)} / ${Math.round(MaxMana)}`
                    let regenText = `${PrefMP}${ManaRegen.toFixed(1)}`
                    cfg.SendPingTextToChat(cfg.BuildDuelManaPing(HeroIndex, mpText, regenText), undefined,
                        { kind: "duel_mana", unit: HeroIndex, a: mpText, b: regenText })
                })
            }
            ...
            if(PlayerAghanimContainer){
                ...
                // [A72] строку собирает общий сборщик — получатель повторит её на своём языке.
                ...
                if(aghScepter){
                    ...
                    aghScepter.SetPanelEvent("onactivate", function(){
                        let cfg = GameUI.CustomUIConfig()
                        if(!IsDotaAltPressed() || !cfg.SendPingTextToChat || !cfg.BuildScepterStatusPing) return
                        let flag = bHasScepter ? 1 : 0
                        cfg.SendPingTextToChat(cfg.BuildScepterStatusPing(HeroIndex, flag), undefined,
                            { kind: "scepter_status", unit: HeroIndex, a: flag })
                    })
                }
                if(aghShard){
                    ...
                    aghShard.SetPanelEvent("onactivate", function(){
                        let cfg = GameUI.CustomUIConfig()
                        if(!IsDotaAltPressed() || !cfg.SendPingTextToChat || !cfg.BuildShardStatusPing) return
                        let flag = bHasShard ? 1 : 0
                        cfg.SendPingTextToChat(cfg.BuildShardStatusPing(HeroIndex, flag), undefined,
                            { kind: "shard_status", unit: HeroIndex, a: flag })
                    })
                }
            }
            ...
            itemPanelNeutral.GetChild(0).SetPanelEvent("onactivate", function(){
                ...
                if(ItemEntNeutral != undefined && ItemEntNeutral != -1){
                    ...
                    let cfg = GameUI.CustomUIConfig()
                    if(cfg.SendPingTextToChat && cfg.BuildNeutralNonePing){
                        cfg.SendPingTextToChat(cfg.BuildNeutralNonePing(HeroIndex), undefined,
                            { kind: "neutral_none", unit: HeroIndex })
                    }
                }
            })
            ...
            itemPanelSmokes.GetChild(0).SetPanelEvent("onactivate", function(){
                ...
                if(idx != undefined && idx != -1){
                    ...
                    // [A72] параметры для пересборки у получателя (его язык)
                    cfg.SendPingTextToChat(cfg.BuildSmokePingByCharges(HeroIndex, 0), undefined, { kind: "smoke", unit: HeroIndex, charges: 0 })
                }
            })
...
// ============================================================================
// [Карта справа] Отражённый интерфейс Dota: меню дуэли переезжает к правому краю.
//
// Внутри меню меняем местами РОВНО ДВЕ вещи — карточку героя и блок ставок:
// при отражении ставки должны оказаться слева, а герой справа (решение владельца
// 2026-08-04). Обе лежат в строке .DuelPlayer, выстроенной потоком, поэтому
// достаточно развернуть её детей.
//
// Внутрь самих блоков не лезем: содержимое карточки и ставок читается так же,
// как в обычном режиме. Реген здоровья и маны тоже остаётся справа.
// ============================================================================
function FlipDuelSides(Panel, bFlipped, Depth){
    if(!Panel || !Panel.IsValid() || Depth > 8){ return }

    for(let Child of Panel.Children()){
        if(Child.BHasClass("DuelPlayer")){
            SetChildrenReversed(Child, bFlipped)
        }

        FlipDuelSides(Child, bFlipped, Depth + 1)
    }
}

SetupHUDFlip($.GetContextPanel(), function(bFlipped){
    FlipDuelSides($.GetContextPanel(), bFlipped, 0)
})