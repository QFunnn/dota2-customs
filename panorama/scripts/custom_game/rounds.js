...
function PingRound(){
    ...
    if(!cfg.BuildRoundWarnPing){return}
    // [A72] шлём ключ строки раунда — получатель локализует его у себя.
    cfg.SendPingTextToChat(cfg.BuildRoundWarnPing(LastRoundName), undefined,
        { kind: "round", key: LastRoundName })
}
...
function UpdateRoundInfo(){
    ...
    LastRoundName = RoundName // [NP-24] запоминаем для пинга; [A72] храним КЛЮЧ, локализует получатель
...
// [Карта справа] Отражённый интерфейс Dota: блок текущего раунда переезжает к правому
// краю целиком. Внутреннюю раскладку НЕ трогаем — по решению владельца 2026-08-04:
// в этих строках элементы стоят не потоком, а выравниванием, и перестановка приводила
// к наложению («РАУНД 1» налезал на «ПОДГОТОВКА»). Переносим только сам блок.
SetupHUDFlip(MAIN_PANEL)