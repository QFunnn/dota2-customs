...
function PingStat(key, val, iconItem){
    ...
    if(!cfg || !cfg.SendPingTextToChat || !cfg.BuildStatPing) return
    // [A72] шлём ключ стата и снимок значения — текст соберёт получатель на своём языке.
    cfg.SendPingTextToChat(cfg.BuildStatPing(key, val, iconItem), undefined,
        { kind: "stat", key: key, a: val, item: iconItem })
}
