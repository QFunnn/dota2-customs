...
// [A72] Сборщики текста простых пингов. Вынесены отдельно, чтобы ОДИН и тот же код
// работал у отправителя (запасной вариант) и у получателя (там строка собирается
// заново — на его языке). Имя юнита берём из сущности: она реплицирована всем.
function _PingUnitName(u){
    if(u == undefined || u == -1){ return "" }
    let n = Entities.GetUnitName(u)
    return n ? $.Localize("#" + n) : ""
}
function BuildHpPing(u, hp, mhp){
    let pct = mhp > 0 ? Math.round(hp / mhp * 100) : 0
    return _PingUnitName(u) + ": <font color='#6cd16c'>HP " + hp + " / " + mhp + "</font> <font color='#bfe8bf'>(" + pct + "%)</font>"
}
function BuildManaPing(u, mp, mmp){
    let pct = mmp > 0 ? Math.round(mp / mmp * 100) : 0
    return _PingUnitName(u) + ": <font color='#4aa3ff'>MP " + mp + " / " + mmp + "</font> <font color='#a9d4ff'>(" + pct + "%)</font>"
}
function BuildHasItemPing(u, itemName, bHas){
    let status = bHas ? $.Localize("#PING_YES") : $.Localize("#PING_NO")
    return _PingUnitName(u) + ": " + ItemNameHtml(itemName) + " — " + status
}
function BuildNeutralNonePing(u){
    return $.Localize("#PING_NEUTRAL_NONE").replace("%s1", _PingUnitName(u))
}
function BuildLevelPing(u, level){
    return $.Localize("#PING_LEVEL").replace("%s1", _PingUnitName(u)).replace("%s2", level)
}
function BuildSkillsPing(u, bOwn){
    let icon = "<img class='PingSkillIcon' src='file://{images}/custom_game/talents2.png'/>"
    if(bOwn){ return icon + $.Localize("#PING_SHOW_MY_SKILLS") }
    return icon + $.Localize("#PING_SHOW_HERO_SKILLS").replace("%s1", _PingUnitName(u))
}
GameUI.CustomUIConfig().BuildSkillsPing = BuildSkillsPing

function BuildWantItemPing(u, itemName){
    let item = $.Localize("#DOTA_Tooltip_ability_" + itemName)
    return $.Localize("#PING_WANT_ITEM").replace("%s1", _PingUnitName(u)).replace("%s2", item)
}

// [A72] Пинги окна дуэли, панели статов и раунда собирались прямо на месте вызова
// (duel_info.js / top_stats.js / rounds.js) и уходили готовой строкой — получатель читал
// их на языке ОТПРАВИТЕЛЯ. Сборка вынесена сюда, чтобы RebuildPingMessage() повторил её
// на языке каждого получателя. Числа (hp/mana/реген/значение стата) — снимок отправителя,
// они не локализуются и передаются готовыми строками.
function BuildDuelHpPing(u, hpText, regenText){
    return $.Localize("#PING_HP").replace("%s1", _PingUnitName(u)).replace("%s2", hpText).replace("%s3", regenText)
}
function BuildDuelManaPing(u, mpText, regenText){
    return $.Localize("#PING_MANA").replace("%s1", _PingUnitName(u)).replace("%s2", mpText).replace("%s3", regenText)
}
function BuildScepterStatusPing(u, bHas){
    let status = $.Localize(bHas ? "#PING_YES" : "#PING_NO")
    return $.Localize("#PING_SCEPTER_STATUS").replace("%s1", _PingUnitName(u)).replace("%s2", status)
}
function BuildShardStatusPing(u, bHas){
    let status = $.Localize(bHas ? "#PING_YES" : "#PING_NO")
    return $.Localize("#PING_SHARD_STATUS").replace("%s1", _PingUnitName(u)).replace("%s2", status)
}
function BuildRoundWarnPing(roundKey){
    return $.Localize("#PING_ROUND_WARN").replace("%s1", "<font color='#ffcc55'>" + $.Localize(roundKey) + "</font>")
}
function BuildStatPing(statKey, val, iconItem){
    let icon = iconItem ? "<img class='PingItemIcon' src='file://{images}/items/"+iconItem+".png'/>" : ""
    return icon + $.Localize(statKey).replace("%s1", val)
}
GameUI.CustomUIConfig().BuildDuelHpPing = BuildDuelHpPing
GameUI.CustomUIConfig().BuildDuelManaPing = BuildDuelManaPing
GameUI.CustomUIConfig().BuildScepterStatusPing = BuildScepterStatusPing
GameUI.CustomUIConfig().BuildShardStatusPing = BuildShardStatusPing
GameUI.CustomUIConfig().BuildRoundWarnPing = BuildRoundWarnPing
GameUI.CustomUIConfig().BuildStatPing = BuildStatPing
GameUI.CustomUIConfig().BuildNeutralNonePing = BuildNeutralNonePing
...
function PingHP(){
    ...
    SendPingTextToChat(BuildHpPing(u, hp, mhp), undefined, { kind: "hp", unit: u, a: hp, b: mhp })
}
function PingMana(){
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let hn = $.Localize("#"+Entities.GetUnitName(u))
    let mp = Math.round(Entities.GetMana(u)), mmp = Math.round(Entities.GetMaxMana(u))
    SendPingTextToChat(BuildManaPing(u, mp, mmp), undefined, { kind: "mana", unit: u, a: mp, b: mmp })
}
function PingScepter(){
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let hn = $.Localize("#"+Entities.GetUnitName(u))
    let bHas = Entities.HasScepter(u) ? 1 : 0
    SendPingTextToChat(BuildHasItemPing(u, "item_ultimate_scepter", bHas), undefined, { kind: "has_item", unit: u, item: "item_ultimate_scepter", a: bHas })
}
function PingShard(){
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let hn = $.Localize("#"+Entities.GetUnitName(u))
    let bHas = HasModifier(u, "modifier_item_aghanims_shard") ? 1 : 0
    SendPingTextToChat(BuildHasItemPing(u, "item_aghanims_shard", bHas), undefined, { kind: "has_item", unit: u, item: "item_aghanims_shard", a: bHas })
}
function PingOwnNeutral(){
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let idx = Entities.GetItemInSlot(u, 16)
    if(idx != undefined && idx != -1){ PingAbilityToChat(idx, u) }
    else { SendPingTextToChat(BuildNeutralNonePing(u), undefined, { kind: "neutral_none", unit: u }) }
}
...
function PingLevel(){
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let hn = $.Localize("#"+Entities.GetUnitName(u))
    let lvl = Entities.GetLevel(u)
    SendPingTextToChat(BuildLevelPing(u, lvl), undefined, { kind: "level", unit: u, a: lvl })
}
function PingItemByName(itemname){
    if(!itemname || itemname == "") return
    let u = Players.GetLocalPlayerPortraitUnit(); if(u == undefined || u == -1) return
    let hn = $.Localize("#"+Entities.GetUnitName(u))
    SendPingTextToChat(BuildWantItemPing(u, itemname), undefined, { kind: "want_item", unit: u, item: itemname })
}
...
function ColorPingStatus(msg){
    ...
    // «на перезарядке»/«on cooldown» — оранжевым и ДО «готов», иначе «готов» внутри
    // соседнего слова могло бы перекрасить кусок статуса.
    msg = msg.replace(/(на\s+перезарядке|on\s+cooldown)/gi, "<font color='#e0a13e'>$1</font>")
...
function BuildAbilityPingMessage(abilityIndex, ent){
    ...
    if(isenemy){
        ...
        if(is_item){
            ...
            // Предмет на перезарядке раньше всё равно пинговался как «готов»: у способностей
            // проверка была, у предметов — нет. Своего ванильного ключа под item+cooldown у
            // Valve нет (есть только Ready/ReadyPassive/ReadyCharges), поэтому используем свой.
            if(!Abilities.IsCooldownReady( abilityIndex )){
                loc = "#PING_ITEM_COOLDOWN"
...
function RenderPingToAll(msg, hasIcon, rebuild){
    // [A72] msg — уже собранный текст на языке ОТПРАВИТЕЛЯ, он остаётся как запасной
    // вариант. Если передан rebuild-дескриптор, получатель соберёт строку сам и увидит
    // её на СВОЁМ языке (сущности реплицируются всем клиентам, так что данных хватает).
    let payload = { caller: LocalPID, msg: msg, has_icon: hasIcon ? 1 : 0, allow_self: 1, is_ping: 1 };
    if(rebuild && rebuild.kind){
        payload.rb_kind = rebuild.kind;
        if(rebuild.ability !== undefined){ payload.rb_ability = rebuild.ability }
        if(rebuild.ent !== undefined){ payload.rb_ent = rebuild.ent }
        if(rebuild.ability_name !== undefined){ payload.rb_ability_name = rebuild.ability_name }
        if(rebuild.unit_name !== undefined){ payload.rb_unit_name = rebuild.unit_name }
        if(rebuild.unit !== undefined){ payload.rb_unit = rebuild.unit }
        if(rebuild.charges !== undefined){ payload.rb_charges = rebuild.charges }
        if(rebuild.item !== undefined){ payload.rb_item = rebuild.item }
        if(rebuild.key !== undefined){ payload.rb_key = rebuild.key }
        if(rebuild.a !== undefined){ payload.rb_a = rebuild.a }
        if(rebuild.b !== undefined){ payload.rb_b = rebuild.b }
    }
    GameEvents.SendCustomGameEventToAllClients("chat_say_from_ally", payload);
}

// [A72] Пересборка текста пинга на стороне ПОЛУЧАТЕЛЯ — чтобы каждый читал на своём языке.
// Любая осечка (сущность не реплицирована, способность недоступна) → возвращаем null,
// и приёмник напечатает исходный msg отправителя. Тише деградировать, чем потерять пинг.
function RebuildPingMessage(event){
    try{
        if(event.rb_kind === "ability" && event.rb_ability !== undefined && event.rb_ent !== undefined){
            return BuildAbilityPingMessage(Number(event.rb_ability), Number(event.rb_ent));
        }
        if(event.rb_kind === "ability_by_name" && event.rb_ability_name){
            return BuildAbilityPingByName(event.rb_ability_name, event.rb_unit_name);
        }
        if(event.rb_kind === "smoke" && event.rb_unit !== undefined){
            return BuildSmokePingByCharges(Number(event.rb_unit), Number(event.rb_charges) || 0);
        }
        // [A72] Пинги без сущности: раунд и стат — только локализационный ключ + снимок значения.
        if(event.rb_kind === "round" && event.rb_key){
            return BuildRoundWarnPing(event.rb_key);
        }
        if(event.rb_kind === "stat" && event.rb_key){
            return BuildStatPing(event.rb_key, event.rb_a, event.rb_item);
        }
        // Простые пинги: параметры-снимки (числа/имя предмета) + сущность для имени героя.
        if(event.rb_unit !== undefined){
            let u = Number(event.rb_unit);
            if(event.rb_kind === "hp"){ return BuildHpPing(u, Number(event.rb_a), Number(event.rb_b)) }
            if(event.rb_kind === "mana"){ return BuildManaPing(u, Number(event.rb_a), Number(event.rb_b)) }
            if(event.rb_kind === "has_item"){ return BuildHasItemPing(u, event.rb_item, Number(event.rb_a) === 1) }
            if(event.rb_kind === "neutral_none"){ return BuildNeutralNonePing(u) }
            if(event.rb_kind === "level"){ return BuildLevelPing(u, Number(event.rb_a)) }
            if(event.rb_kind === "want_item"){ return BuildWantItemPing(u, event.rb_item) }
            if(event.rb_kind === "skills"){ return BuildSkillsPing(u, Number(event.rb_a) === 1) }
            if(event.rb_kind === "duel_hp"){ return BuildDuelHpPing(u, event.rb_a, event.rb_b) }
            if(event.rb_kind === "duel_mana"){ return BuildDuelManaPing(u, event.rb_a, event.rb_b) }
            if(event.rb_kind === "scepter_status"){ return BuildScepterStatusPing(u, Number(event.rb_a) === 1) }
            if(event.rb_kind === "shard_status"){ return BuildShardStatusPing(u, Number(event.rb_a) === 1) }
        }
    }catch(e){
        $.Msg("[A72] пересборка пинга не удалась, показываю оригинал: ", e);
    }
    return null;
}
...
function SendPingTextToChat(text, hasIcon, rebuild){
    ...
    RenderPingToAll(text, hasIcon, rebuild);   // [A72] rebuild -> получатель соберёт текст на своём языке
    return true;
}
...
function PingAbilityToChat(abilityIndex, ent){
    ...
    if(!msg){ return }
    // [A72] шлём и параметры: получатель пересоберёт строку локализованно.
    SendPingTextToChat(msg, undefined, { kind: "ability", ability: abilityIndex, ent: ent });
}
        ...
        if(panel.id == "inventory_neutral_slot"){
            panel.SetPanelEvent("onactivate", function(){
                ...
                if(idx != undefined && idx != -1){
                    ...
                    SendPingTextToChat(BuildNeutralNonePing(myUnit), undefined, { kind: "neutral_none", unit: myUnit })
                }
            })
            return
        }
        ...
        if(panel.id == "inventory_tpscroll_slot"){
            panel.SetPanelEvent("onactivate", function(){
                ...
                if(idx != undefined && idx != -1){
                    ...
                    SendPingTextToChat(BuildSmokePingByCharges(myUnit, 0), undefined, { kind: "smoke", unit: myUnit, charges: 0 })
                }
            })
            return
        }
            ...
            if(AbilityButton){
                ...
                if(!Abilities.IsItem( abilityIndex )){
                    ...
                    p.SetPanelEvent("oncontextmenu", function(){
                        // [MF-24] Alt+ПКМ по способности — закрепить индикатор дальности.
                        // Обработчик обязан жить ИМЕННО здесь: FixCast лежит поверх кнопки и
                        // перехватывает ПКМ, поэтому хук из range_indicator.js на саму панель
                        // до способностей не доходит (у предметов FixCast нет — там он работает).
                        if(IsDotaAltPressed()){
                            let cfg = GameUI.CustomUIConfig()
                            if(cfg && cfg.ToggleRangeIndicator){ cfg.ToggleRangeIndicator(abilityIndex) }
                            return
                        }
...
GameEvents.Subscribe("chat_say_from_ally", function(event){
    if(event.caller != LocalPID || event.allow_self == 1){
        // [A71] Мут категории «Текст» глушит и обычные сообщения, И alt-пинги.
        // Раньше пинги были исключением (`event.is_ping != 1`), из-за чего замученный
        // игрок всё равно спамил пингами: они идут кастомным событием, мимо мута Dota.
        // Свои пинги (allow_self) не трогаем — себя игрок не мутит.
        if(event.caller != LocalPID && _MuteCatLocal(event.caller, "text")){ return }
        // [A72] Пробуем собрать текст локально (свой язык), иначе печатаем присланный.
        let text = event.rb_kind ? RebuildPingMessage(event) : null
        SayLine(event.caller, text || event.msg, event.has_icon, event.is_ping)
    }
})