...
// Пришли ли НАСТОЯЩИЕ настройки игрока (profile_loaded=1 от Lua), а не стартовые дефолты.
// До этого момента UpdateSetting молчит, чтобы не затереть в БД сохранённое.
...
// Применяет сохранённые настройки к контролам меню.
// ВАЖНО: вызывается на КАЖДОЕ обновление setting_data, а не один раз.
// Почему: Lua публикует setting_data ДВАЖДЫ — сперва дефолты при регистрации игрока
// (zxc_server.lua, RegisterPlayer), и только потом реальные, когда придёт ответ
// get_player_profile. Раньше стоял одноразовый флаг settingInited, поэтому меню
// защёлкивало ДЕФОЛТЫ и больше не обновлялось — игроку казалось, что настройки не
// сохранились, он выставлял их заново, и UpdateSetting затирал в БД настоящие значения.
function InitSetting(){
   let settingData = GetPlayerTablesValue(`player_${Players.GetLocalPlayer()}`, "setting_data")
   if (settingData)
   {
    // Всё тело — под try: функция зовётся и во время парса, и на каждое обновление
    // setting_data. Раньше исключение отсюда обрывало ВЕСЬ скрипт настроек, и меню
    // оставалось пустым (ни биндов, ни дропдауна) — страхуемся, чтобы это не повторилось.
    try {
        settingInited = true
        // profile_loaded=1 ставит Lua только после ответа сервера. До этого в таблице
        // лежат дефолты — отправлять их обратно нельзя (затрут сохранённое в БД).
        SETTINGS_LOADED = String(settingData.profile_loaded) == "1"
        ...
        // Вкладку подсветки трогаем НЕ отсюда: её переменные (HL_inited и др.) объявлены
        // через let ниже по файлу, и обращение к ним из InitSetting падает с ReferenceError,
        // если функция вызвана во время парса. Перерисовка живёт в подписке в конце файла.
    } catch(e) {
        $.Msg("[Settings] InitSetting: ошибка применения настроек — ", e)
    }
   }
}
...
{
    // Пока профиль не приехал, в UI стоят дефолты — отправлять их нельзя: сервер
    // перезапишет ими сохранённые настройки игрока (ровно то, на что жаловались).
    if(!SETTINGS_LOADED){
        $.Msg("[Settings] UpdateSetting пропущен: профиль игрока ещё не загружен")
        return
    }
...
(function() {
    ...
    // Подписка на setting_data — В КОНЦЕ ФАЙЛА, не здесь. Причина: SubscribeAndFire
    // вызывает колбэк СРАЗУ, а этот блок выполняется во время парса, когда объявленные
    // ниже через let переменные ещё в TDZ — любое обращение к ним роняет весь скрипт.
...
function HL_Save(){
    // Тот же гард, что и в UpdateSetting: до прихода профиля в HL_settings лежат дефолты,
    // отправка затёрла бы сохранённые цвета игрока.
    if(!SETTINGS_LOADED){
        $.Msg("[Settings] HL_Save пропущен: профиль игрока ещё не загружен")
        return
    }
    GameEvents.SendCustomGameEventToServer("PlayerSettings", HL_settings)
}

// Единая точка реакции на setting_data (A67). Стоит ЗДЕСЬ, а не выше по файлу:
// SubscribeAndFire дёргает колбэк сразу, а все let-переменные подсветки объявлены выше
// этой строки — значит на момент вызова они уже инициализированы и TDZ не мешает.
//
// Зачем вообще: Lua публикует setting_data ДВАЖДЫ — сперва стартовые дефолты при
// регистрации игрока, потом реальные, когда придёт профиль с сервера. Раньше меню
// читало только первое (одноразовый флаг + подписка на чужие CustomNetTables) и
// показывало дефолты, а следующий клик отправлял их на сервер, затирая сохранённое.
SubscribeAndFirePlayerTableByKey(`player_${Players.GetLocalPlayer()}`, "setting_data", function(v){
    if(!v) return

    // Основная вкладка: чекбоксы, дропдаун, слайдеры.
    InitSetting()

    // Подсветка: значения синхронизируем всегда, UI перерисовываем только если
    // вкладку уже открывали (иначе панелей ещё нет — отрисуются при первом открытии).
    HL_LoadSettingsFromServer()
    if(HL_inited){
        for(let t of HL_TOGGLES){
            let btn = $(`#HL_Toggle_${t.id}`)
            if(btn){ btn.SetSelected(HL_settings[t.key] == 1) }
        }
        HL_RenderColorRow($("#HL_ColorRowsBasic"), HL_COLORS_BASIC)
        HL_RenderColorRow($("#HL_ColorRowsCategories"), HL_COLORS_CATEGORIES)
        HL_UpdateCategorySectionVisibility()
    }
})
