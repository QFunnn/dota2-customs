...
function GetRuneDetail(rune_id, callback, force = false) {
  CustomUIConfig.RuneDetailCache ??= {};
  const id = String(rune_id);
  ...
  return ServerRequest("get_rune_detail", {
    ...
    for (let [detailID, v] of Object.entries(data)) {
      CustomUIConfig.RuneDetailCache[detailID] = v;
      if (detailID === id) {
        result = v;
      }
    }
...
function ShowServerRuneTooltip(p, props) {
  GetRuneDetail(props.id1, () => {
    if (p.IsValid() && p.BHasHoverStyle()) {
      ShowCustomTooltip(p, "server_rune", {
        id1: props.id1
      });
    }
  }, props.force);
