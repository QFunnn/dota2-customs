...
(() => {
    ...
    function updateRoundInfoPanel() {
        ...
        const creepCountLabel = findPanel("RoundCreepCount");
        ...
        roundNameLabel.text = $.Localize(roundName);
        creepCountLabel.text = roundShowCreepCount && roundTotalCount > 0 ? `${roundCount}/${roundTotalCount}` : "";
        creepCountLabel.SetHasClass("Show", roundShowCreepCount && roundTotalCount > 0);
    ...
    function bindRoundInfo() {
        const applyQuestEvent = (event, resetProgress) => {
            if (event.name === "RoundPrepare") {
                ...
                if (event.creature_count != null) {
                    roundTotalCount = event.creature_count;
                }
                roundShowCreepCount = false;
                roundShowInfo = true;
            }
