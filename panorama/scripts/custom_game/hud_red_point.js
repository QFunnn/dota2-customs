...
const useActivityRedPointStore = () => {
  ...
  const [ultimateSkinRewards, setUltimateSkinRewards] = libs.createSignal();
  const [ultimateSkinConfigs, setUltimateSkinConfigs] = libs.createSignal();
  ...
  const [tiStoreItems, setTiStoreItems] = libs.createSignal([]);
  const [tiPurchasedProducts, setTiPurchasedProducts] = libs.createSignal();
  const [tiPlayerOrnaments, setTiPlayerOrnaments] = libs.createSignal();
  const [tiPlayerHeroes, setTiPlayerHeroes] = libs.createSignal();
  const redPoints = libs.createMemo(() => {
    ...
    const currentPlayerOrnaments = playerOrnaments();
    const currentUltimateSkinRewards = ultimateSkinRewards();
    const currentUltimateSkinConfigs = ultimateSkinConfigs();
    ...
    const yangeAwakeningSkinID = 5100074;
    const yangeAwakeRewardSkinID = 5100075;
    const yangeBalanceTokenID = 1100254;
    const yangeBalanceMaximum = 160;
    const yangeUltimateSkinID = Number(Object.values(currentUltimateSkinConfigs ?? {}).find(config => config.token_id == yangeBalanceTokenID)?.id ?? yangeAwakeRewardSkinID);
    const yangeRewardState = currentUltimateSkinRewards?.find(reward => reward.ultimate_skin_id == yangeAwakeRewardSkinID);
    const yangeIsAwakened = Object.values(currentPlayerOrnaments ?? {}).some(ornament => ornament.oid == yangeAwakeningSkinID);
    const yangeAwakeRewardClaimable = yangeIsAwakened && yangeRewardState?.awake_reward_received !== true;
    const yangeUltimateRewardClaimable = yangeIsAwakened && (currentPlayerTokens?.[yangeBalanceTokenID]?.num ?? 0) >= yangeBalanceMaximum && !Object.values(currentPlayerOrnaments ?? {}).some(ornament => ornament.oid == yangeUltimateSkinID) && yangeRewardState?.max_reward_received !== true;
    ...
    for (const activity of currentActivities) {
      ...
      const hasReward = loginRewardActivityIDs.has(activity.activity_id) || taskRewardActivityIDs.has(activity.activity_id) || starryTreasureRewardStage != undefined || activity.activity_id == 19011 && (yangeAwakeRewardClaimable || yangeUltimateRewardClaimable);
      if (hasReward) {
    ...
    if (availableTabSet.has("Activity_TIActivity")) {
      const tiPurchases = tiPurchasedProducts();
      const tiOrnaments = tiPlayerOrnaments();
      const tiHeroes = tiPlayerHeroes();
      if (tiPurchases != undefined && tiOrnaments != undefined && tiHeroes != undefined) {
        for (const item of tiStoreItems()) {
          if (item.status != 1 || item.real_price != 0) {
            continue;
          }
          if (item.limit_type >= 1 && tiPurchases[item.id] >= item.limit_count) {
            continue;
          }
          if (getCosmeticByStoreItem(item, tiOrnaments) || getHerobyStoreItem(item, tiHeroes)) {
            continue;
          }
          result.add(red_point_utils.createRedPointKey("activity", "Activity_TIActivity"));
          break;
        }
      }
    }
  ...
  libs.onMount(() => {
    ...
    const listeners = [useNetData("info_activity_data", setActivities), useNetData("login_activity_data", setLoginActivities, playerID), useNetData("info_activity_task", setActivityTasks), useNetData("activity_task_progresses", setTaskProgresses, playerID), useNetData("info_deep_sea", setDeepSeaRewards), useNetData("player_token", setPlayerTokens, playerID), useNetData("player_ornament", setPlayerOrnaments, playerID), useNetData("player_ultimate_skin_rewards", setUltimateSkinRewards, playerID), useNetData("info_ultimate_skin", setUltimateSkinConfigs), useNetData("info_shop_product_group_by_tag", data => {
      setTiStoreItems(data?.["ti15"] ?? []);
    }), useNetData("player_purchased_products", data => {
      setTiPurchasedProducts(data.purchased_products);
    }, playerID), useNetData("player_ornament", setTiPlayerOrnaments, playerID), useNetData("player_hero", setTiPlayerHeroes, playerID), useClientGlobalData("activity_tag_list", setActivityTags), useClientGlobalData("menu_bar_activity_tabs", setAvailableTabs)];
