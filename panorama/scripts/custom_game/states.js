...
// [A77] подписки экрана стадий уже созданы (см. DOTAStates)
let SETUP_SUBS_DONE = false;
let Subscribes = [];
...
function DOTAStates(){
	...
	if(Game.GameStateIs(DOTA_GameState.DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP)){
		// [A77] Сюда можно попасть двумя путями — по событию и по опросу состояния (см. конец
		// файла). Подписки создаём один раз, иначе колбэки задвоятся.
		if(SETUP_SUBS_DONE){ return }
		SETUP_SUBS_DONE = true
...
// [A77] Диагностический пульс экрана снят 2026-08-05 перед прод-заливкой.
// Рабочая часть фикса — опрос Game.GetState() ниже — на месте.

// ============================================================================
// [A77] СТРАХОВКА ОТ НЕДОСТАВЛЕННЫХ СОБЫТИЙ
//
// У игрока, зашедшего к нам из другой кастомки без перезапуска клиента, не
// доставляются игровые события — ни кастомные (из-за чего молчала библиотека
// PlayerTables), ни, судя по логам, движковые: подписка на current_state ниже в
// DOTAStates ни разу не сработала, потому что сам DOTAStates висит на
// game_rules_state_change и не был вызван. В итоге данные для пика уже приезжали
// зеркалом, а экран стадий их некому было показать.
//
// Поэтому переход в фазу настройки ловим ещё и опросом: Game.GetState() читается
// напрямую и от доставки событий не зависит. Повторный заход защищён флагом
// SETUP_SUBS_DONE, так что здоровым клиентам это ничего не ломает.
// ============================================================================
let A77_lastPolledState = -1

function A77_PollGameState(){
	let state = Game.GetState()

	if(state != A77_lastPolledState){
		A77_lastPolledState = state
		DOTAStates()
	}

	// после начала боя экран стадий уже не нужен
	if(!Game.GameStateIsAfter(DOTA_GameState.DOTA_GAMERULES_STATE_STRATEGY_TIME)){
		$.Schedule(0.5, A77_PollGameState)
	}
}
