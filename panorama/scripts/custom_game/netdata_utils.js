...
function createPlayerServiceNetTable(key, playerID, defaultValue) {
  const [data, setData] = libs.createSignal((() => {
    const cache = getServiceNetTable(key, playerID);
    if (cache) {
      return cache;
    }
    return defaultValue;
...
exports.createPlayerServiceNetTable = createPlayerServiceNetTable;
