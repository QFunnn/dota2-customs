...
let UISCALE_X = 1;
let UISCALE_Y = 1;
function refreshChatWheelPopupItems(value) {
    var _a;
    CHAT_WHEEL_POPUP_ITEMS = (_a = value !== null && value !== void 0 ? value : CustomNetTables.GetTableValue("chat_wheel", "list")) !== null && _a !== void 0 ? _a : {};
}
...
function CloseCallBody() {
    ...
    if (CurrentSelectedCallLine == 0)
        return;
    GameEvents.SendCustomGameEventToServer("chat_wheel_line_selected", { PlayerID: Players.GetLocalPlayer(), line_id: CurrentSelectedCallLine });
    CurrentSelectedCallLine = 0;
}
function OnSayLine(event) {
    if (Game.IsPlayerMuted(event.caller_player)) {
        // $.Msg(`ChatWheelDisabled = ${CHAT_WHEEL_MESSAGES_DISABLED}. GameIsPlayerMuted = ${Game.IsPlayerMuted(event.caller_player)}`)
        return;
    }
    ...
    let soundIcon = ``;
    if (itemInfo.Type == 2) {
        if (!CHAT_WHEEL_MESSAGES_DISABLED) {
...
$.Schedule(0.1, function () {
    ...
    CustomNetTables.SubscribeNetTableListener("chat_wheel", function (_, key, value) {
        var _a;
        if (key == "list") {
            refreshChatWheelPopupItems(value);
            const chatWheelPlayerTable = (_a = CustomNetTables.GetTableValue("chat_wheel", String(Players.GetLocalPlayer()))) !== null && _a !== void 0 ? _a : {};
            UpdatePopupChatWheel(chatWheelPlayerTable);
            return;
        }
