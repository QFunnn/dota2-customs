...
Object.entries(KeyValues.collection).forEach(([key, data], idx) => {
...
const Collection = props => {
  ...
  libs.createMemo(libs.on(() => props.type, () => {
    const maxRarity = Math.max(...Object.keys(collectionList()).map(rarity => Number(rarity)));
    SetSelectedID(collectionList()[maxRarity][0].id.toString());
  }));
