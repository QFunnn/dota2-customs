...
const Popup_PaymentOrder = props => {
  ...
  function PaymentWaySort(list, all = false) {
    return list.sort((a, b) => multiCompare(all ? (KeyValues.PaymentsOrderKV[b].region == "" ? 1 : 0) - (KeyValues.PaymentsOrderKV[a].region == "" ? 1 : 0) : (KeyValues.PaymentsOrderKV[a].region == "" ? 1 : 0) - (KeyValues.PaymentsOrderKV[b].region == "" ? 1 : 0), finiteNumber(Number(KeyValues.PaymentsOrderKV[b].order), 0) - finiteNumber(Number(KeyValues.PaymentsOrderKV[a].order), 0), a < b ? -1 : a > b ? 1 : 0));
  }
  {
    const BLACK_LIST = {
      russian: {
    ...
    Object.keys(KeyValues.PaymentsOrderKV).forEach(key => {
      ...
      if (flag && !BLACK_LIST[language$1]?.main.includes(key)) {
        mainPaymentList.push(key);
      }
      if (!BLACK_LIST[language$1]?.others.includes(key)) {
