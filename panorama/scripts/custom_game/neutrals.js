    ...
    if(bCraftEnabled){
        ...
        for (const _ in v.current_craft.enchants) {
            ...
            for (const ValueName in EnchantInfo.values) {
                ...
                // [A70/A53] Значений может быть МЕНЬШЕ, чем уровней: у малусов их обычно
                // одно на все уровни (Crude "intelligence_pct" = "-9", Titanic
                // "attack_speed" = "-9", при том что бонусы заданы как "9 12 15").
                // Раньше брали строго values[level-1] -> на 2-3 уровне выходил undefined,
                // и строка с малусом ПРОПАДАЛА из описания чары. Движок в такой ситуации
                // берёт последнее заданное значение — повторяем это поведение.
                let ValueIndex = Math.min(EnchantInfo.level - 1, values.length - 1)
                if(ValueIndex < 0){ ValueIndex = 0 }
                if(values && values[ValueIndex] !== undefined && values[ValueIndex] != 0){
                    let ResultValue = values[ValueIndex]
                ...
                if(inventory_neutral_slot){
                    ...
                    inventory_neutral_slot.SetPanelEvent("onactivate", function(){
                        ...
                        if(idx != undefined && idx != -1){
                            ...
                            // [A72] через общий сборщик — получатель соберёт текст на своём языке.
                            let cfg = GameUI.CustomUIConfig()
                            if(cfg.SendPingTextToChat && cfg.BuildNeutralNonePing){
                                cfg.SendPingTextToChat(cfg.BuildNeutralNonePing(myUnit), undefined,
                                    { kind: "neutral_none", unit: myUnit })
                            }
                        }
                    })
                }
