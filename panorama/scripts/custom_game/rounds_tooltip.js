            ...
            if(AbilitiesList){
                for (const _ in Info.abilities) {
                    ...
                    p.SetPanelEvent("onactivate", function(){
                        ...
                        cfg.SendPingTextToChat(cfg.BuildAbilityPingByName(AbilityName, CreepName), undefined, { kind: "ability_by_name", ability_name: AbilityName, unit_name: CreepName })
                    })
                }
            }
...
// [Карта справа] Окно «подробнее о раунде» открывается из блока текущего раунда,
// а тот при отражении стоит справа — значит и окно уходит туда же. Переносим только
// само окно, внутреннюю раскладку не трогаем (см. решение по блоку раундов).
SetupHUDFlip($.GetContextPanel())