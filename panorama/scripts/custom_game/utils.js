...
// ПОДДЕРЖКА ОТРАЖЁННОГО ИНТЕРФЕЙСА (у игрока карта СПРАВА)



// В настройках Dota есть отражение интерфейса: миникарта уезжает вправо, панель
// предметов — влево. Наши HUD-панели прижаты к краям экрана жёстко, поэтому при
// отражении часть из них оказывается не с той стороны и налезает на ванильные
// элементы. Панель, которая это учитывает, один раз зовёт SetupHUDFlip(root),
// а свои поправки пишет в CSS через селектор .HUDFlipped.
//
// Класс вешаем на КОРЕНЬ своей панели сами, а не полагаемся на то, что движок
// повесит его на родителя — так стили работают в любом случае.
// Game.IsHUDFlipped — метод именно Game, а не GameUI: проверено по client.dll,
// биндинг лежит в блоке CScriptBindingPR_Game.
// Настройку можно переключить прямо в матче, поэтому перечитываем её по таймеру,
// а не однократно на старте.
let HUD_FLIP_CHECK_INTERVAL = 1.0

// onChange (необязательный) вызывается на старте и при КАЖДОЙ смене режима. Туда
// выносим то, чего одним CSS не сделать, — например порядок детей внутри кнопки:
// при отражении надпись и стрелка должны меняться местами, а CSS порядок в потоке
// не переставляет.
function SetupHUDFlip(root, onChange){
    if(!root){ return }
    ...
    function Apply(){
        ...
        let bFlipped = Game.IsHUDFlipped()

        root.SetHasClass("HUDFlipped", bFlipped)

        if(bFlipped !== LastFlipped){
            LastFlipped = bFlipped

            if(onChange){
                try{ onChange(bFlipped) }
                catch(e){ $.Msg("[HUDFlip] ошибка обработчика: ", String(e)) }
            }
        }

        $.Schedule(HUD_FLIP_CHECK_INTERVAL, Apply)
    }

    Apply()
}

// Разворачивает порядок детей панели. Нужен для отражённого интерфейса: CSS порядок
// в потоке не меняет, а при переносе блока к другому краю надпись и иконка должны
// поменяться местами. Двигаем с конца перед первым — так получается точный реверс.
// Состояние помним на самой панели, чтобы повторный вызов не разворачивал обратно.
function SetChildrenReversed(panel, bReversed){
    if(!panel || !panel.IsValid()){ return }

    // Исходный порядок — как в разметке, то есть НЕ развёрнутый. Без этой строки
    // первый же вызов с bReversed = false сравнивался с undefined, не совпадал и
    // разворачивал панель в обычном режиме: у «Информации» из-за этого менялись
    // местами надпись со стрелкой, а тело уезжало на место панели настроек.
    if(panel._aa_reversed === undefined){ panel._aa_reversed = false }

    if(panel._aa_reversed === bReversed){ return }

    panel._aa_reversed = bReversed
    ...
    for(let i = Children.length - 1; i >= 1; i--){
        panel.MoveChildBefore(Children[i], First)
    }
}