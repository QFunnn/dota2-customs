...
// ============================================================================
// [A74] Чиним закреплённый предмет (sticky item) для ОБЫЧНОЙ доты.
//
// Проблема: в нашей лавке смоук кастомный — item_smoke_of_deceit_custom. Когда игрок
// закрепляет его, Dota запоминает ИМЯ предмета в клиентском конфиге (hud_sticky_item_name),
// и оно переживает выход из кастомки. В обычном матче такого предмета не существует:
// игра не понимает, что там лежит, закрепление не работает и не меняется через UI —
// вылечить можно только консольной командой (подтверждено владельцем 2026-08-03).
//
// Решение: сами выполняем эту команду у игрока и подставляем ванильный ТП —
// стандартный закреплённый предмет в обычной Dota.
// Канал рабочий и именно клиентский: addon_init.lua:77-81 выполняет SendToConsole
// под `if IsClient()`.
//
// КОГДА сбрасываем — два момента, оба уже «после игры» для конкретного игрока:
//   1) он ВЫБЫЛ (событие ShowPlayerLose) — покупать ему больше нечего, а из клиента
//      он чаще всего выходит сразу: SetSafeToLeave(true) в addon_game_mode.lua:67
//      разрешает уйти безнаказанно, поэтому до конца матча большинство не досиживает;
//   2) матч завершился (POST_GAME) — для победителей и тех, кто досмотрел.
// Учтено, что POST_GAME наступает НЕ мгновенно: мод ставит GAME_END_DELAY = 5 секунд
// перед SetGameWinner (players.lua:396-403), иначе сервер закрывается сразу.
//
// На входе в кастомку сбрасывать НЕЛЬЗЯ: игрок получил бы закреплённым ванильный
// ТП вместо нашего смоука и потерял бы быструю покупку смоука на всю игру.
// ЧТО ставим: свиток телепортации — это стандартный закреплённый предмет в обычной
// Dota (он же первым идёт в ванильных consumables). Ставим именно его, а не смоук:
// вернуть игроку то, что стояло ДО входа в кастомку, нельзя — API чтения конваров в
// панораме отсутствует (проверено по client.dll: GetConVarString/Int/Bool там нет),
// поэтому выбираем наименее произвольное значение — дефолт самой игры.
// ============================================================================
const STICKY_ITEM_FIX_VALUE = "item_tpscroll";
let _stickyItemFixed = false;

function FixStickyItemForVanillaDota(reason) {
    if (_stickyItemFixed) { return }
    _stickyItemFixed = true;
    GameEvents.SendEventClientSide("client_side_server_command", {
        command: "hud_sticky_item_name " + STICKY_ITEM_FIX_VALUE
    });
    $.Msg("[A74] sticky item переведён на ванильный ТП — для обычной доты (" + reason + ")");
}

GameEvents.Subscribe("game_rules_state_change", function () {
    if (Game.GetState() === DOTA_GameState.DOTA_GAMERULES_STATE_POST_GAME) {
        FixStickyItemForVanillaDota("конец матча");
    }
});

// Выбывание игрока — основной случай: он почти всегда выходит из клиента сразу,
// не дожидаясь POST_GAME. Событие приходит лично ему (players.lua:285).
GameEvents.Subscribe("ShowPlayerLose", function () {
    FixStickyItemForVanillaDota("игрок выбыл");
});
