...
function isLocalPlayerInDuel(players) {
...
function findSelfDuelOpponents(players) {
    const teams = players !== null && players !== void 0 ? players : dataList;
    const localPlayer = Players.GetLocalPlayer();
    let localTeamId;
    const opponents = [];
    ...
    for (const teamId in teams) {
        ...
        if (!team)
            continue;
        for (const info of team) {
            const playerId = info.iPlayerID;
            if (playerId !== undefined && playerId !== -1 && playerId !== localPlayer) {
...
function updateSelfDuelNoticeOpponent(players) {
    const opponents = findSelfDuelOpponents(players);
    const row = root.FindChildTraverse("SelfDuelNoticeOpponent");
    if (!row) {
        return;
    }
    row.SetHasClass("Hidden", opponents.length === 0);
    if (opponents.length === 0) {
        return;
    }
    const opponentLabel = row.FindChildTraverse("SelfDuelNoticeOpponentLabel");
    const opponentList = row.FindChildTraverse("SelfDuelNoticeOpponentList");
    if (opponentLabel) {
        opponentLabel.text = $.Localize("#SelfDuelNoticeOpponentLabel");
    }
    if (!opponentList) {
        return;
    }
    opponentList.RemoveAndDeleteChildren();
    for (const opponent of opponents) {
        const entry = $.CreatePanel("Panel", opponentList, "");
        entry.AddClass("SelfDuelNoticeOpponentEntry");
        const heroImage = $.CreatePanel("DOTAHeroImage", entry, "");
        heroImage.AddClass("SelfDuelNoticeOpponentHero");
        const hero = Players.GetPlayerHeroEntityIndex(opponent);
        heroImage.heroname = hero !== -1 ? Entities.GetUnitName(hero) : Players.GetPlayerSelectedHero(opponent);
        heroImage.heroimagestyle = "icon";
        const nameLabel = $.CreatePanel("Label", entry, "");
        nameLabel.AddClass("SelfDuelNoticeOpponentName");
        nameLabel.text = Players.GetPlayerName(opponent);
    }
}
function showSelfDuelNotification(players) {
    if (!showSelfDuelNoticeSetting) {
        return;
    }
    if (players && !selfPvp && !isLocalPlayerInDuel(players)) {
        return;
    }
    const notice = root.FindChildTraverse("SelfDuelNotice");
    if (!notice) {
        return;
    }
    updateSelfDuelNoticeOpponent(players);
    const token = ++selfDuelNoticeToken;
    notice.SetHasClass("Show", false);
    $.Schedule(0.03, () => {
        if (token !== selfDuelNoticeToken) {
            return;
        }
        Game.EmitSound("Creep.Sended");
        notice.SetHasClass("Show", true);
    });
    $.Schedule(4.2, () => {
        if (token !== selfDuelNoticeToken) {
            return;
...
let showSelfDuelNoticeSetting = true;
...
SubscribeAndFireNetTableByKey("player_settings", String(getSteamId32(Players.GetLocalPlayer())), (_, __, value) => {
    if (value) {
        ...
        showSelfDuelNoticeSetting = value.showSelfDuelNotice === undefined
            ? true
            : readNetBool(value.showSelfDuelNotice);
    }
});
function init() {
    ...
    GameEvents.Subscribe("ShowPvpBrief", (event) => {
        var _a;
        showSelfDuelNotification(dataList);
...
function updateBetButtonsText() {
    ...
    for (const button of betButtons) {
        ...
        const baseText = $.Localize("#PvP_bet_timer")
            .replace("([d:time]с)", "")
            .replace("([d:time]s)", "")
            .replace("[d:time]", "")
            .trim();
        const showTime = showBet && canBet && !selfPvp;
        textButton.text = showTime ? `${baseText} (${betTime}${$.Localize("#pvp_footer_seconds")})` : baseText;
    }
    const maxButtons = [];
