...
function OnPlayerSelectedTeam(nPlayerId, nTeamId, bSuccess) {
	...
	// $.Msg("OnPlayerSelectedTeam", playerInfo.player_id, nTeamId, nPlayerId);
...
// tournament_map 赛制选择
//--------------------------------------------------------------------------------------------------
function OnSelectTournamentMode(groupMode) {
	var playerInfo = Game.GetLocalPlayerInfo();
	if (Game.GetTeamSelectionLocked()) return;
	if (!playerInfo || !playerInfo.player_has_host_privileges) return;
	GameEvents.SendCustomGameEventToServer("set_tournament_mode", { group_mode: groupMode });
	// UpdateTournamentModeButtons(groupMode);
}
var _bGroupMode;
function UpdateTournamentModeButtons(groupMode) {
	$.Msg("UpdateTournamentModeButtons111", groupMode);
	if (_bGroupMode == undefined || _bGroupMode != groupMode) {
		if (Players.GetTeam(Players.GetLocalPlayer()) != DOTATeam_t.DOTA_TEAM_BADGUYS && Players.GetTeam(Players.GetLocalPlayer()) != DOTATeam_t.DOTA_TEAM_NOTEAM) {
			Game.PlayerJoinTeam(DOTATeam_t.DOTA_TEAM_NOTEAM);
		}
		$.Msg("UpdateTournamentModeButtons222", groupMode);
		_bGroupMode = groupMode;
		$.GetContextPanel().SetHasClass("GroupMode", groupMode == 1);
		var competitionBtn = $("#TournamentModeCompetitionBtn");
		var groupBtn = $("#TournamentModeGroupBtn");
		if (competitionBtn) competitionBtn.SetHasClass("selected", groupMode == 0);
		if (groupBtn) groupBtn.SetHasClass("selected", groupMode == 1);
	}
}

function InitTournamentModePanel() {
	var panel = $("#TournamentModeSelect");
	if (!panel) return;

	panel.visible = false;
	// panel.visible = !!_IsTournamentMap;

	// if (_IsTournamentMap) {
	// 	var playerInfo = Game.GetLocalPlayerInfo();
	// 	var isHost = playerInfo && playerInfo.player_has_host_privileges;
	// 	var competitionBtn = $("#TournamentModeCompetitionBtn");
	// 	var groupBtn = $("#TournamentModeGroupBtn");
	// 	if (competitionBtn) competitionBtn.enabled = !!isHost;
	// 	if (groupBtn) groupBtn.enabled = !!isHost;
	// 	// 订阅 net table 反映当前选择
	// 	CustomNetTables.SubscribeNetTableListener("common", (tableName, key, value) => {
	// 		if (key == "tournament_group_mode") {
	// 			UpdateTournamentModeButtons(value && value.enabled ? 1 : 0);
	// 		}
	// 	});
	// 	$.Msg("UpdateTournamentModeButtons000");
	// 	// 读取当前状态并初始化按钮
	// 	var currentMode = CustomNetTables.GetTableValue("common", "tournament_group_mode");
	// 	UpdateTournamentModeButtons(currentMode && currentMode.enabled ? 1 : 0);
	// }
}
...
(function () {
	...
	if (_IsTournamentMap) {
		...
		$.GetContextPanel().SetHasClass("Spectator", true);
	}
	...
	// 初始化 tournament_map 赛制选择面板
	InitTournamentModePanel();
	if (_IsGroupMode) {
		$.GetContextPanel().SetHasClass("GroupMode", true);
	}
