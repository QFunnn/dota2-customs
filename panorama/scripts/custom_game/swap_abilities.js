...
//удаление таких опасных способностей может вызвать вылет
var unremovableAbilities = {
    "monkey_king_wukongs_command": true,
};

//взаимоисключающие способности — чтобы игрок не выбрал их одновременно
...
//способности, взаимоисключающие по модели
var heroExclusion={}
...
function SelectAbility(ability_panel, player_id) {
	return () => {
        ...
        //проверяем конфликт со способностью оппонента
        //если выбрана способность союзника
		if (player_container_id!=own_player_id)
		{    
			//если своя способность конфликтует с какой-то способностью союзника
            if (selection_pair.own)
            {
                ...
                //проверяем взаимоисключение по модели
		...
		{    
            if (selection_pair.other && selection_pair.other_player_id)
            {
		        ...
		        //если способность союзника конфликтует со своей
...
function IsAbilityInnate(ability) {
...
function UpdatePlayerAbilities(player_panel, hero_entindex, player_id) {
	...
	for (var i = 0; i < 30; i++) {
		...
		let list = GameUI.CustomUIConfig().SubAbislityKv
		...
		// 	continue;
		// }
        // если это неудаляемая способность
...
(function () {
    ...
    //показываем страницу обмена способностями
