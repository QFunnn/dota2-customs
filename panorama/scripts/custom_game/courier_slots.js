...
// [Карта справа] Отражённый интерфейс Dota: бонусный тайник переезжает к левому краю.
// Позиции и направления — в CSS (селектор .HUDFlipped), а порядок надписи и стрелки
// внутри кнопки приходится менять руками: CSS переставить детей в потоке не умеет.
SetupHUDFlip($.GetContextPanel(), function(bFlipped){
    let Button = $("#BonusStashButton")
    ...
    for(let Child of Button.Children()){
        if(Child.BHasClass("BonusStashButtonLabel")){ Label = Child }
        if(Child.BHasClass("BonusStashButtonIcon")){ Icon = Child }
    }

    if(!Label || !Icon){ return }

    // у левого края стрелка идёт первой, у правого — как было, после надписи
    if(bFlipped){
        Button.MoveChildBefore(Icon, Label)
    }else{
        Button.MoveChildBefore(Label, Icon)
    }
})