...
const Treasure = () => {
  ...
  const receiveTreasure = () => {
    if (!enable) return;
    GameEvents.SendCustomEventToServer('select_treasure', {});
  };
...
const DataList = props => {
  const [local] = libs.splitProps(props, ["entIndex", "show", "type"]);
  const [battleData, setBattleData] = libs.createSignal([]);
  const dataTypes = () => Array.isArray(props.type) ? props.type : [props.type];
  const abilityNameList = () => {
    return battleData().slice().sort((a, b) => dataSum(b.damage) - dataSum(a.damage));
  };
  ...
  libs.onMount(() => {
    const timer = setInterval(() => {
      ...
      const combineRegen = dataTypes().includes("regen") && dataTypes().includes("regen_blocked");
      const combineAttack = dataTypes().includes("damage") && dataTypes().includes("attack_blocked");
      let maxData = 0;
      let allData = 0;
      for (const dataType of dataTypes()) {
        const records = battleRecord?.[dataType] ?? {};
        for (const abilityName in records) {
          const damage = records[abilityName];
          const value = Round(dataSum(damage));
          allData += value;
          if (combineRegen) {
            const counts = regenCounts.get(abilityName) ?? {
              normal: 0,
              blocked: 0
            };
            counts[dataType == "regen" ? "normal" : "blocked"] += damage.count ?? 0;
            regenCounts.set(abilityName, counts);
            const current = entriesByName.get(abilityName);
            if (!current || dataType == "regen") {
              entriesByName.set(abilityName, {
                abilityName,
                damage,
                type: dataType
              });
            }
          } else if (combineAttack) {
            const counts = attackCounts.get(abilityName) ?? {
              normal: 0,
              blocked: 0
            };
            counts[dataType == "damage" ? "normal" : "blocked"] += damage.count ?? 0;
            attackCounts.set(abilityName, counts);
            if (dataType == "damage") {
              entriesByName.set(abilityName, {
                abilityName,
                damage,
            ...
            } else if (!entriesByName.has(abilityName)) {
              entriesByName.set(abilityName, {
                abilityName,
                damage: {
                  count: 0
            ...
            maxData = Math.max(value, maxData);
          }
        }
      }
      if (combineRegen) {
        for (const [abilityName, entry] of entriesByName) {
          const counts = regenCounts.get(abilityName) ?? {
            normal: 0,
            blocked: 0
          };
          const totalCount = counts.normal + counts.blocked;
          entry.countLabel = counts.blocked > 0 ? `${counts.normal} / ${totalCount}` : `${counts.normal}`;
          entries.push(entry);
          maxData = Math.max(Round(dataSum(entry.damage)), maxData);
        }
      }
      if (combineAttack) {
        for (const [abilityName, entry] of entriesByName) {
...
const DataRow = props => {
  ...
  const sourceType = libs.createMemo(() => props.abilityName.split("@")[0]);
  const isRegenBlocked = libs.createMemo(() => props.type == "regen_blocked");
  ...
  const isAttack = libs.createMemo(() => ["Attack", "AbilityAttack", "SectAttack"].includes(sourceType()));
  const isSect = libs.createMemo(() => ["Sect", "SectAttack"].includes(sourceType()));
  const isAbilityUpgrade = libs.createMemo(() => sourceType() == "AbilityUpgrade");
  ...
  const getImageType = () => {
    if (isRegenBlocked()) {
      ...
      return "Talent";
    } else if (isAttack() && sourceType() == "Attack") {
      return "Attack";
    } else if (isSect()) {
      return "Sect";
  ...
  const damageList = () => Object.keys(props.damage).filter(damageType => damageType != "count");
  return libs.createComponent(EOM_Panel.EOM_Panel, {
    get className() {
      return libs.classNames("DataRow", {
        HealBlocked: isRegenBlocked()
      });
    },
    width: "100%",
    marginRight: "10px",
      ...
      return [libs.createComponent(libs.Dynamic, {
        get component() {
          return {
            ...
            RegenBlocked: () => libs.createComponent(GenericPanel.CImage, {
              className: "RecordImage",
              src: `file://{images}/custom_game/synergyicons/healer_psd.png`
            })
          }[imageType()];
        }
      }), libs.createComponent(GenericPanel.CLabel, {
        ...
        html: true,
        get text() {
          return `<font color='${abilityInfo().color}'>${$.Localize(tooltip())}</font> × ${props.countLabel ?? props.damage.count}`;
        }
      }), libs.createComponent(EOM_Panel.EOM_Panel, {
        ...
        get children() {
          return libs.createComponent(libs.For, {
            ...
            children: (damageType, index) => {
              ...
              const progressType = isRegenBlocked() ? "regenBlocked" : props.type == "regen" ? "regen" : "damageType" + damageType;
              return libs.createComponent(EOM_Icon.EOM_Icon, {
                get className() {
                  return libs.classNames("RecordProgress", progressType, {
                    Start: index() == 0,
                    End: index() == damageList().length - 1
                  });
                },
                get width() {
