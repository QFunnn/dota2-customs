...
(function () {
    ...
    /**
     * WindowBuilder
     * Универсальный помощник для сборки интерфейсных окон из 9-слайсов (4 угла, 4 стороны, фон).
     *
     * Пример использования:
     * const frame = WindowBuilder.create(parentPanel, { width: 640, height: 420 });
     * frame.content.AddClass("my-window-content");
     * $.CreatePanel("Label", frame.content, "").text = "Hello!";
     */
    ...
    function registerTheme(name, config) {
        themes[name] = {
            ...
            contentPadding: config.contentPadding ?? DEFAULT_THEME.contentPadding,
    ...
    function create(parent, themeName, size = {}) {
        ...
        const contentPadding = size.contentPadding ?? theme.contentPadding;
        ...
        buildSlices(overlay, theme, padding, width, height);
    ...
    function buildSlices(container, theme, padding, width, height) {
        ...
        bg.style.backgroundSize = `${tile}px ${tile}px`;
        bg.style.width = `${Math.max(0, width - padding * 2)}px`;
        bg.style.height = `${Math.max(0, height - padding * 2)}px`;
