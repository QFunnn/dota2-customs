...
LinkLuaModifier(
	"modifier_enraged_wildkin_hurricane_lua",
	"creature_ability/enraged_wildkin_hurricane_lua.lua",
...
function enraged_wildkin_hurricane_lua:OnSpellStart()
	...
	if
		IsValid(hCaster)
		and IsValid(hTarget)
		and not hTarget:TriggerSpellAbsorb(self)
	...
	then
		...
		hTarget:AddNewModifier(
			...
			self,
			"modifier_enraged_wildkin_hurricane_lua",
			{ x = vDir.x, y = vDir.y, z = vDir.z, duration = duration }
		)
	end
end
---------------------------------------------------------------------
