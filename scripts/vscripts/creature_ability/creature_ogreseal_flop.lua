...
creature_ogreseal_flop = class({}) ---@class creature_ogreseal_flop : CDOTA_Ability_Lua
...
function creature_ogreseal_flop:TryToDamage()
	...
	if #enemies > 0 then
		for _, enemy in pairs(enemies) do
			if enemy ~= nil and (not enemy:HasState(MODIFIER_STATE_DEBUFF_IMMUNE)) and (not enemy:IsInvulnerable()) then
				local DamageInfo =
				{
					victim = enemy,
				...
				if enemy:IsAlive() == false then
					...
					ParticleManager:SetParticleControlEnt(nFXIndex, 0, enemy, PATTACH_POINT_FOLLOW, "attach_hitloc",
						enemy:GetOrigin(), true)
					ParticleManager:SetParticleControl(nFXIndex, 1, enemy:GetOrigin())
					ParticleManager:SetParticleControlForward(nFXIndex, 1, -self:GetCaster():GetForwardVector())
					ParticleManager:SetParticleControlEnt(nFXIndex, 10, enemy, PATTACH_ABSORIGIN_FOLLOW, nil,
						enemy:GetOrigin(), true)
	...
	local nFXIndex = ParticleManager:CreateParticle("particles/test_particle/ogre_melee_smash.vpcf",
