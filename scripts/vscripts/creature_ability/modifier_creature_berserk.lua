...
function modifier_creature_berserk:OnIntervalThink()
	self:SetStackCount(math.abs(GameMode:GetMatch():GetCurrentRound():GetTimeLimit()))
...
function modifier_creature_berserk:GetModifierProcAttack_Feedback(params)
	if IsServer() then
		...
		if target ~= nil then
			...
			if debuff == nil then
				debuff =
					target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_creature_berserk_debuff", {
						duration = 12,
					})
				if debuff ~= nil then
