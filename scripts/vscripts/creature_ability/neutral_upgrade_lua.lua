...
function modifier_neutral_upgrade_lua:OnIntervalThink()
	if GameMode:GetMatch():GetCurrentRound():IsTimeExceeded() then
		local parent = self:GetParent()
		if not parent:HasModifier("modifier_creature_berserk") and not parent.by_chen_holy_persuasion then
...
function modifier_neutral_upgrade_lua:RecalculateStats()
	...
	self:SetStackCount(GameMode:GetMatch():GetCurrentRound().roundNumber or 1)
