...
function modifier_neutral_upgrade_lua:OnCreated(params)
	...
	if IsServer() then
		...
		if tKv.AttackRate then
			...
			self.AttackRate =
				math.max(self.min_base_attack_time_reduction, AttackRate - stack * self.base_attack_time_reduction)
		end
		--攻击力
		...
		if fDamageMax and fDamageMin then
			...
			elseif stack > 30 then
				flDamageMultiple = math.pow(1.165, 10)
					* math.pow(1.124, 10)
					* math.pow(1.113, 10)
