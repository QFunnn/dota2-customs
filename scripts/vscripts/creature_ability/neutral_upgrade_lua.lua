...
if modifier_neutral_upgrade_lua == nil then
	modifier_neutral_upgrade_lua = class({})
end
...
function modifier_neutral_upgrade_lua:OnCreated(params)
	...
	self.min_base_attack_time_reduction = self:GetAbilitySpecialValueFor("min_base_attack_time_reduction")
	self:StartIntervalThink(FrameTime())
	if IsServer() then
		--移除Roshan的Buff
		hParent:RemoveModifierByName("modifier_roshan_inherent_buffs")
		hParent:RemoveAbility("roshan_devotion")
		self:SetStackCount(GameMode.currentRound.nRoundNumber or 1)
		...
		if tKv.AttackRate then
			...
			self.AttackRate = math.max(self.min_base_attack_time_reduction, AttackRate - stack * self.base_attack_time_reduction)
		end
		--攻击力
		...
		if fDamageMax and fDamageMin then
			...
			elseif stack > 30 then
				flDamageMultiple = math.pow(1.165, 10) * math.pow(1.124, 10) * math.pow(1.113, 10) * math.pow(1.11, stack - 30)
			end
		...
		if fHealth then
			...
			elseif stack > 30 then
				fHPMultiple = math.pow(1.196, 10) * math.pow(1.145, 10) * math.pow(1.125, 10) * math.pow(1.12, stack - 30)
			end
...
function modifier_neutral_upgrade_lua:OnRefresh(params)
	local hParent = self:GetParent()
	if hParent:GetTeamNumber() ~= DOTA_TEAM_NEUTRALS then
		return
	end
...
function modifier_neutral_upgrade_lua:OnDestroy()
	if IsServer() then
	end
end
...
function modifier_neutral_upgrade_lua:OnIntervalThink()
	...
	local keys = {
		"base_attack_time_reduction",
		"attack_speed",
		"bonus_armor",
		"max_bonus_armor",
		"bonus_magical_resistance",
		"max_bonus_magical_resistance",
		"min_base_attack_time_reduction",
	}
	if IsValid(hAbility) and hAbility:GetLevel() >= 1 then
		for _, keyname in pairs(keys) do
			if self[keyname] == 0 then
				self[keyname] = self:GetAbilitySpecialValueFor(keyname)
				if self[keyname] == 0 then
					bKvDone = false
				end
			end
