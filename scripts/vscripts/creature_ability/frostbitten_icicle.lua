...
frostbitten_icicle = class({}) ---@class frostbitten_icicle : CDOTA_Ability_Lua
LinkLuaModifier("modifier_frostbitten_icicle", "creature_ability/modifier/modifier_frostbitten_icicle",
...
function frostbitten_icicle:OnSpellStart()
	if IsServer() then
		...
		local nFXIndex = ParticleManager:CreateParticle("particles/test_particle/dungeon_generic_blast_ovr_pre.vpcf",
		...
		Timers:CreateTimer(delay, function()
			EmitSoundOn("Hero_Tusk.IceShards", hCaster)
			local nFXIndex = ParticleManager:CreateParticle("particles/act_2/frostbitten_icicle.vpcf",
				PATTACH_WORLDORIGIN, nil)
			ParticleManager:SetParticleControl(nFXIndex, 0, vOrigin + Vector(0, 0, 40))
			ParticleManager:ReleaseParticleIndex(nFXIndex)

			local enemies = FindUnitsInRadius(hCaster:GetTeamNumber(), vOrigin, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
				DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false)
			for _, enemy in pairs(enemies) do
				if enemy ~= nil and enemy:HasState(MODIFIER_STATE_DEBUFF_IMMUNE) == false and enemy:IsInvulnerable() == false then
					local damageInfo = ---@type ApplyDamageOptions
					{
						victim = enemy,
