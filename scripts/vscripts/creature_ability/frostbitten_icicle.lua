...
LinkLuaModifier(
	"modifier_frostbitten_icicle",
	"creature_ability/modifier/modifier_frostbitten_icicle",
...
function frostbitten_icicle:OnSpellStart()
	if IsServer() then
		...
		Timers:CreateTimer(delay, function()
			...
			local nFXIndex =
				ParticleManager:CreateParticle("particles/act_2/frostbitten_icicle.vpcf", PATTACH_WORLDORIGIN, nil)
			...
			for _, enemy in pairs(enemies) do
				if
					enemy ~= nil
					and enemy:HasState(MODIFIER_STATE_DEBUFF_IMMUNE) == false
					and enemy:IsInvulnerable() == false
				then
