	...
	if IsServer() then
		if self.nHopCount == nil then
			...
			local nFXIndex = ParticleManager:CreateParticle(
				"particles/act_2/ogre_seal_warcry.vpcf",
...
function modifier_ogreseal_flop:UpdateVerticalMotion(me, dt)
	if IsServer() then
		-- Защита от бесконечного цикла: если уже завершили — ничего не делаем
		if self.bCompleted then
			return
		end
		...
		if bLanded == true then
			...
			if bDoneHopping then
				print("[OGRESEAL_FLOP] DONE HOPPING, setting bCompleted=true and Destroy()")
