...
harpy_storm_chain_lightning_lua = class({}) ---@class harpy_storm_chain_lightning_lua : CDOTA_Ability_Lua
...
function harpy_storm_chain_lightning_lua:OnSpellStart()
	if IsServer() then
		...
		if IsValid(self:GetCursorTarget()) then
...
function harpy_storm_chain_lightning_lua:Zap(source, target, damage)
	...
	local bounce_pfx = ParticleManager:CreateParticle("particles/items_fx/chain_lightning.vpcf", PATTACH_ABSORIGIN_FOLLOW,
		source)
	ParticleManager:SetParticleControlEnt(bounce_pfx, 0, target, PATTACH_POINT_FOLLOW, "attach_hitloc",
		target:GetAbsOrigin(), true)
	ParticleManager:SetParticleControlEnt(bounce_pfx, 1, source, PATTACH_POINT_FOLLOW, "attach_hitloc",
	...
	local nearby_enemies = FindUnitsInRadius(
		...
		source:GetAbsOrigin(),
		nil,
		self:GetSpecialValueFor("bounce_radius"),
