...
local h = {
	...
	sandTornado = { enabled = true, terrainTheme = "sand", chance = 30 },
}
...
function l.prototype.StartFlameColumnTraps(self)
	...
	self.flameColumnState.timerID = Timer:GameTimer(Q.startDelay, function()
...
function l.prototype.RefreshTrapEnemyDamage(self)
	...
	Game:EachPlayer(function(q, az)
		...
		if IsValid(aA) and aA:IsRealHero() then
			local aB = aA:GetAttackDamage()
			local aC = {
				target = nil,
				attacker = aA,
				damage = aB,
				damage_type = EOM_DAMAGE_TYPES.DAMAGE_TYPE_NONE,
				damage_flags = EOM_DAMAGE_FLAGS.TRAP,
				damage_category = DOTA_DAMAGE_CATEGORY_BARRIER,
			}
			local aD = GetOutgoingDamagePercent(nil, aA, aC)
			ay = math.max(ay, aB * (1 + aD * 0.01))
		end
	end)
...
function l.prototype.GetTrapDamage(self, P)
	...
	local aE = P:GetMaxHealth() * TRAP_DAMAGE_FACTOR
	local aF = GameRules:GetCustomGameDifficulty()
	local aG = DIFFICULTY_TRAP_DAMAGE_REDUCTION[aF] or 0
	if P:IsRealHero() and aG ~= 0 then
		aE = aE * (1 + aG / 100)
	end
	return aE
end
