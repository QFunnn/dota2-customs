...
function Spawn(self, entityKeyValues)
	...
	thisEntity:GameTimer(0.1, function()
...
function UpdateOnRemove(self) end
...
function AI_Boss_8.prototype.____constructor(self, hUnit, tData)
	...
	____root_tChildren_0[#____root_tChildren_0 + 1] = __TS__New(AI_BOSS_8_5, hUnit:FindAbilityByName("boss_8_5"))
	do
		local bt = __TS__New(BTC_Random)
		bt.tChildren = {}
		for ____, t in ipairs({
			{ "boss_8_1", AI_BOSS_8_1 },
			{ "boss_8_2", AI_BOSS_8_2 },
			{ "boss_8_3", AI_BOSS_8_3 },
			{ "boss_8_4", AI_BOSS_8_4 },
		}) do
		...
		local bt_checkInterval = __TS__New(BT_Precondition, function()
	...
	do
		local btAttack = __TS__New(BTA_AttackMove, function()
			local fDis
...
function AI_BOSS_8_1.prototype.____constructor(self, hAblt)
	...
	self.precondition = __TS__New(BT_Precondition, function()
...
function AI_BOSS_8_5.prototype.Evaluate(self)
	if BTA_CastAblity.prototype.Evaluate(self) > 0 then
		if
			self.typeResult == 1
			or self.hUnit:HasModifier("modifier_boss_8_5_ai") and self.hUnit:GetHealthPercent() <= 50
		then
			return 10000
		end
		return 0, "未达到50%血量"
