...
function bevavior()
	...
	if not thisEntity.init then
		...
		for _, tower in pairs(towers) do
			if
				IsValidEntity(tower)
				and (tower:GetUnitName() == "npc_towerradiant" or tower:GetUnitName() == "npc_towerdire")
			then
	...
	if control == false then
		if thisEntity.blast:IsFullyCastable() then
			if
				...
				and IsValidEntity(enemy_for_ability[1])
				and (thisEntity:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D()
					<= thisEntity.blast:GetCastRange(thisEntity:GetAbsOrigin(), thisEntity)
			then
		...
		if thisEntity.ice:IsFullyCastable() then
			if
				...
				and thisEntity:GetHealthPercent() <= thisEntity.ice:GetSpecialValueFor("health")
		...
		if thisEntity.ulti:IsFullyCastable() and flag then
			if
				IsValidEntity(enemy_for_ability[1])
				and (thisEntity:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D()
					<= thisEntity.ulti:GetCastRange(thisEntity:GetAbsOrigin(), thisEntity)
			then
				...
				for _, i in pairs(ice) do
					if
						IsValidEntity(i)
						and i:GetUnitName() == "npc_lich_ice_unit"
						and (i:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D()
							<= thisEntity.ulti:GetSpecialValueFor("radius")
					then
						distance = true
						break
					end
