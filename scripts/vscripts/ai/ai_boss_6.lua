...
function AI_Boss_6.prototype.____constructor(self, hUnit, tData)
	...
	____root_tChildren_0[#____root_tChildren_0 + 1] = __TS__New(AI_Boss_6_8, hUnit:FindAbilityByName("boss_6_8"))
	do
	...
	do
		local btMove = __TS__New(BTA_Move, function()
			local fDis
...
function AI_Boss_6_8.prototype.Evaluate(self)
	if BTA_CastAblity.prototype.Evaluate(self) > 0 then
		if
			self.typeResult == 1
			or self.hUnit:HasModifier("modifier_boss_6_8_ai") and self.hUnit:GetHealthPercent() <= 50
		then
			return 10000
		end
		return 0, "未达到50%血量"
