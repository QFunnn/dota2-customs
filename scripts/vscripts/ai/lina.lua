...
function bevavior()
	...
	if thisEntity.ulti:IsFullyCastable() then
		if
			enemy_for_ability[1] ~= nil
			and (thisEntity:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D() <= thisEntity.ulti:GetCastRange(
				thisEntity:GetAbsOrigin(),
				thisEntity
			)
	...
	if enemy ~= nil and (thisEntity:GetAbsOrigin() - thisEntity.tower_location):Length2D() > 1000 then
		thisEntity:SetForceAttackTarget(enemy)
		return 0.5
	end

	if ((thisEntity:GetAbsOrigin() - thisEntity.tower_location):Length2D() < 1000) or (enemy == nil) then
