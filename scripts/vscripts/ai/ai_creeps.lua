...
function NeutralThink()
	...
	for _, enemy in pairs(enemies) do
		if enemy and not enemy:IsNull() and IsValidEntity(enemy) and enemy:IsAlive() then
			...
			if
				enemy:GetTeamNumber() == DOTA_TEAM_GOODGUYS
				and not EXCLUDED_CAST_TARGETS[unitName]
				and not enemy:IsOther()
				and not enemy:IsCourier()
				and not enemy:IsInvulnerable()
				and not enemy:IsUnselectable()
			then
