...
function bevavior()
	...
	local enemy_for_ability = FindUnitsInRadius(
		...
		DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS + DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO,
		FIND_CLOSEST,
		false
	)
	...
	if control == false then
		if thisEntity.midnight:IsFullyCastable() then
			if IsValidEntity(enemy_for_ability[1]) then
				if
					...
					<= thisEntity.midnight:GetCastRange(thisEntity:GetAbsOrigin(), thisEntity)
				then
		...
		if thisEntity.malefice:IsFullyCastable() then
			if
				...
				and thisEntity:GetHealthPercent() <= thisEntity.malefice:GetSpecialValueFor("cast_health")
			then
				if
					(thisEntity:GetAbsOrigin() - enemy_for_ability[1]:GetAbsOrigin()):Length2D()
					<= thisEntity.malefice:GetCastRange(thisEntity:GetAbsOrigin(), thisEntity)
				then
		...
		if
			thisEntity.black_hole:IsFullyCastable()
