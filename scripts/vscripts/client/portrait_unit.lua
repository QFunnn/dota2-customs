...
function Spawn()
	...
	if not _G.PLAYER_PORTRAIT_UNIT then
		return
	end
	if _G.PLAYER_PORTRAIT_UNIT == -1 then
		return
	end
	...
	if unit_name == "npc_dota_hero_target_dummy" then
		return
	end
	if not _G.added_shop_heroes[unit_name] then
		return
	end
	local portrait_model = DEFAULT_MODELS_HEROES[unit_name]
	if not portrait_model then
		return
	end
	...
	if _G.AllItemsData[unit:GetPlayerOwnerID()] ~= nil then
		...
		for k, item_id in pairs(_G.AllItemsData[unit:GetPlayerOwnerID()]) do
			...
			if _G.HEROES_DATA_ITEMS_CLIENT[unit_name] and _G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)] then
				if
					#_G.AllItemsData[unit:GetPlayerOwnerID()] >= 7
					and string.find(_G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)].item_slot, "ambient")
				then
					is_ambient = true
				end
				...
				if
					unit_name == "npc_dota_hero_alchemist"
					and (
						_G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)].item_slot == "weapon"
						or _G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)].item_slot == "arms"
						or _G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)].item_slot == "offhand_weapon"
					)
				then
					is_ambient = true
				end
			...
			if not is_ambient then
				...
				if
					_G.HEROES_DATA_ITEMS_CLIENT[unit_name] and _G.HEROES_DATA_ITEMS_CLIENT[unit_name][tostring(item_id)]
				then
				...
				if _G.AllItemsStyles[unit:GetPlayerOwnerID()] ~= nil then
					if _G.AllItemsStyles[unit:GetPlayerOwnerID()][tostring(item_id)] ~= nil then
						unit_data["style_index" .. item_num] =
							_G.AllItemsStyles[unit:GetPlayerOwnerID()][tostring(item_id)]
					end
				end
				item_num = item_num + 1
			end
	...
	if light_new_data[unit_data.model] and light_new_data[unit_data.model].camera_offsetX then
		local result = tostring(100000 + light_new_data[unit_data.model].camera_offsetX)
			.. " "
			.. light_new_data[unit_data.model].camera_offsetY
			.. " "
			.. light_new_data[unit_data.model].camera_offsetZ
	...
	if portraits and portraits[unit_data.model] or (portraits_others and portraits_others[unit_data.model]) then
		...
		local new_vector_light = tostring((tonumber(vectors_light[1]) + 100000))
			.. " "
			.. vectors_light[2]
			.. " "
			.. vectors_light[3]
