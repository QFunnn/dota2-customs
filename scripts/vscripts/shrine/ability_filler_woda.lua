...
function modifier_ability_filler_woda:OnCreated()
	...
	self:GetParent().modifier_capture =
		Capture:AddNewModifier(self:GetParent(), nil, "modifier_ability_filler_woda_thinker", {})
...
function modifier_ability_filler_woda:OnIntervalThink() end
...
function modifier_ability_filler_woda_thinker:UpdateParticleForPlayer(player_id)
	...
	if player then
		self.pfx_list[player_id] = ParticleManager:CreateParticleForPlayer(
...
function modifier_ability_filler_woda_thinker:OnIntervalThink()
	...
	for _, unit in pairs(units) do
		if
			not unit:HasModifier("modifier_disconnect_player_no_damage")
			and not unit:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier")
			and not unit:HasModifier("modifier_fountain_invulnerability")
		then
	...
	if string.find(GetMapName(), "rating") then
		if teamcount > 1 then
			...
			if not self.Clown then
				self.Clown = ParticleManager:CreateParticle(
				...
				if self.CurrentTeam ~= nil then
					ParticleManager:SetParticleControl(
						...
						2,
						Vector(
							self.m_TeamColors[self.CurrentTeam][1],
							self.m_TeamColors[self.CurrentTeam][2],
							self.m_TeamColors[self.CurrentTeam][3]
						)
					)
				else
	...
	if self.CurrentTeam ~= nil and self.CaptureProgress >= 2 then
		local alyunits = FindUnitsInRadius(
			self.CurrentTeam,
			self:GetParent():GetAbsOrigin(),
			nil,
			350,
	...
	if self.CurrentTeam ~= nil and self.CurrentTeam ~= units[1]:GetTeamNumber() then
		self:UpdateClock(self.CaptureTeam)
		return
	end
	if self.CaptureTeam ~= nil and self.CaptureTeam ~= units[1]:GetTeamNumber() then
		self:UpdateClock(self.CaptureTeam)
		return
	end
...
function modifier_ability_filler_woda_thinker:StartCapturePoint(team)
	...
	if not self.CaptureProgressParticle then
		self.CaptureProgressParticle = ParticleManager:CreateParticle(
	...
	elseif team == nil then
		...
		ParticleManager:SetParticleControl(
			self.CaptureProgressParticle,
			3,
			Vector(self.m_TeamColors[team][1], self.m_TeamColors[team][2], self.m_TeamColors[team][3])
		)
	end
	if self.ReCaptureTime <= 0 then
		if self.CaptureProgress < 2 then
...
function modifier_ability_filler_woda_thinker:StartClock(team)
	...
	if not self.CaptureClockParticle then
		self.CaptureClockParticle = ParticleManager:CreateParticle(
