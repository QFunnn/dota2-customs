...
function modifier_woda_overthrow_fountain:OnIntervalThink()
	...
	self:GetParent():RemoveModifierByName("modifier_fountain_aura")
	if player_system.starting_overthrow == nil then
		return
	end
	local color = {
	...
	if self:GetParent().particle == nil then
		self:GetParent().particle =
			ParticleManager:CreateParticle("particles/shrine_teams.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
		ParticleManager:SetParticleControl(
			self:GetParent().particle,
			15,
			Vector(
				color[self:GetParent():GetTeamNumber()][1],
				color[self:GetParent():GetTeamNumber()][2],
				color[self:GetParent():GetTeamNumber()][3]
			)
		)
...
function modifier_woda_overthrow_fountain:IsAura()
	return true
end
function modifier_woda_overthrow_fountain:IsAuraActiveOnDeath()
	return false
end
