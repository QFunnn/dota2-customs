...
function sl_modifier_bless_10204.prototype.GetRedirectSpell(self, event)
	...
	local pick = self:_PickRedirectTarget()
	if not pick then
		return 0
	end
	if not AbilityForceCast:CheckAbilityCanCastOnTarget(ability, pick) then
		return 0
	end
	...
	if cd > 0 then
		parent:AddSLModifier(____exports.sl_modifier_bless_10204_cd, { caster = parent, duration = cd })
	end
	return 1
end
