...
function modifier_woda_handler_player:IsHidden()
	return true
end
function modifier_woda_handler_player:IsPurgable()
	return false
end
function modifier_woda_handler_player:IsPurgeException()
	return false
end
function modifier_woda_handler_player:RemoveOnDeath()
	return false
end
...
function modifier_woda_handler_player:OnIntervalThink()
	...
	if string.find(GetMapName(), "rating") then
		...
		if self.timer_leader_debuff >= 1 then
			...
			if not self:GetParent():IsIllusion() then
				...
				if player then
					if
						self:GetParent():HasModifier("modifier_wodaduel1")
						or self:GetParent():HasModifier("modifier_wodaduel2")
						or self:GetParent():HasModifier("modifier_wodaduel_duo")
					then
						CustomGameEventManager:Send_ServerToPlayer(
							player,
							"comeback_system_update",
							{ info = self.teams_damage, duel = 1 }
						)
					else
						CustomGameEventManager:Send_ServerToPlayer(
							player,
							"comeback_system_update",
							{ info = self.teams_damage, duel = 0 }
						)
					end
				end
			end
	...
	for _, mod in pairs(self.parent:FindAllModifiers()) do
		if
			mod
			and mod.GetModifierManaRegenPercentDecreaseCustom
			and mod:GetModifierManaRegenPercentDecreaseCustom() ~= nil
		then
...
function modifier_woda_handler_player:OnModifierAdded(params)
	...
	if params.added_buff:GetCaster() ~= self.parent then
		return
	end
	if not params.added_buff:IsDebuff() then
		return
	end
	if params.added_buff:GetDuration() <= 0 then
		return
	end
	local mod_name = params.added_buff:GetName()
	if mod_name == "modifier_cyclone" then
		return
	end
	if mod_name == "modifier_eul_cyclone" then
		return
	end
	if mod_name == "modifier_eul_cyclone_thinker" then
		return
	end
	if mod_name == "modifier_eul_wind_waker_thinker" then
		return
	end
	if mod_name == "modifier_wind_waker" then
		return
	end
	if mod_name == "modifier_lina_slow_burn_custom_debuff" then
		return
	end
	if mod_name == "modifier_kill" then
		return
	end
	local percentage = 0
	for _, mod in pairs(self.parent:FindAllModifiers()) do
...
function modifier_woda_handler_player:AddToHunt()
	...
	self.hunt_counter = 0
	if self.parent:HasModifier("modifier_wodahunt") then
		return
	end
	...
	CustomGameEventManager:Send_ServerToAllClients(
		...
		{ table_name = "huntplayers", data = hunt_table_new }
	)
...
function modifier_woda_handler_player:GetModifierTotalDamageOutgoing_Percentage(params)
	if self:GetParent():HasModifier("modifier_wodaduel1") then
		return
	end
	if self:GetParent():HasModifier("modifier_wodaduel2") then
		return
	end
	if self:GetParent():HasModifier("modifier_wodaduel_duo") then
		return
	end
...
function modifier_woda_handler_player:HuntOnDeath(params)
	if
		params.attacker:GetTeamNumber() == self:GetParent():GetTeamNumber()
		and params.unit:GetTeamNumber() ~= self:GetParent():GetTeamNumber()
	...
	then
		...
		if
			self.teams_damage[params.unit:GetPlayerOwnerID()] ~= nil
			and self.teams_damage[params.unit:GetPlayerOwnerID()] > 0
		then
			...
			local max_value = self.arena_max_damage[math.min(arena_system:GetCurrentArena(), 6)][1]
			self.teams_damage[params.unit:GetPlayerOwnerID()] =
				math.max(max_value, self.teams_damage[params.unit:GetPlayerOwnerID()] - 10)
		end
	end
	if
		params.unit:GetTeamNumber() == self:GetParent():GetTeamNumber()
		and params.attacker ~= nil
		and not params.attacker:IsNull()
		and not params.attacker:IsNeutralUnitType()
		and params.attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber()
		and params.unit:IsRealHero()
	then
		if self:GetParent():HasModifier("modifier_wodaduel1") then
			return
		...
		if
			self.teams_damage[params.attacker:GetPlayerOwnerID()] ~= nil
			and self.teams_damage[params.attacker:GetPlayerOwnerID()] < 0
		then
			...
			local max_value = self.arena_max_damage[math.min(arena_system:GetCurrentArena(), 6)][2]
			self.teams_damage[params.attacker:GetPlayerOwnerID()] =
				math.min(max_value, self.teams_damage[params.attacker:GetPlayerOwnerID()] + 10)
		end
	end
end
...
function modifier_woda_handler_player:LeaderOnDeath(params)
	...
	if params.attacker:GetTeamNumber() == self.parent:GetTeamNumber() and params.unit:IsRealHero() then
		...
		if self.parent:HasModifier("modifier_wodaduel1") then
			return
		end
		if self.parent:HasModifier("modifier_wodaduel_duo") then
			return
		end
		if self.parent:HasModifier("modifier_wodaduel2") then
			return
		end
		if params.reincarnate then
			return
		end
		if params.attacker:GetPlayerOwnerID() ~= self.parent:GetPlayerOwnerID() then
			return
		end
