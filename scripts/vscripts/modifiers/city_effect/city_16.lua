...
local a = "modifiers/city_effect/city_16"
...
h.modifier_city_16 = c()
local n = h.modifier_city_16
n.name = "modifier_city_16"
d(n, m)
...
function n.prototype.OnCreated(self, o)
	if IsServer() then
		PlayerData:eachPlayer(function(p, q)
			...
			if IsValid(r) then
				local s = r:AddNewModifier(r, nil, "modifier_city_16_buff", nil)
				if IsValid(s) then
...
h.modifier_city_16 = n
h.modifier_city_16_buff = c()
local v = h.modifier_city_16_buff
v.name = "modifier_city_16_buff"
d(v, j)
function v.prototype.EFunctionValues(self)
	return {
		[EOMModifierFunction.EOM_MODIFIER_PROPERTY_LOSE_PER_HP_GOLD_PERCENTAGE] = CityEffect:GetSpecialValueFor(
			"city_16",
			"gold_pct"
		),
	}
