...
function modifier_abilities_optimization_thinker:IsHidden()
	return false
end
function modifier_abilities_optimization_thinker:IsPurgable()
	return false
end
...
function modifier_abilities_optimization_thinker:OnAttackRecord(event)
	...
	if
		...
		and attacker:GetAttackTarget() == target
		and attacker._dont_stop_attack_flag == nil
	then
...
LinkLuaModifier(
	"modifier_windrunner_focus_fire_debuff",
	"modifiers/modifier_windrunner_focus_fire_debuff",
...
function modifier_abilities_optimization_thinker:OnAbilityExecuted(params)
	...
	elseif ability_name == "skeleton_king_vampiric_aura_custom" then
		...
		do
			if
				warlock_gl:GetUnitName() == "npc_dota_wraith_king_skeleton_warrior"
				and warlock_gl:GetOwner() == unit
			then
				warlock_gl:Destroy()
			end
		end
...
function modifier_abilities_optimization_thinker:OnAttackStart(params)
	...
	local attacker = params.attacker
	local target = params.target
	if attacker == nil or target == nil or attacker:IsNull() or target:IsNull() then
		return
	end
...
function modifier_abilities_optimization_thinker:OnDeath(params)
	...
	if attacker == nil or attacker:IsNull() or target == nil or target:IsNull() then
		return
	end
	...
	if
		attacker:HasModifier("modifier_windrunner_focusfire")
		and target:HasModifier("modifier_windrunner_focus_fire_debuff")
		and attacker:HasTalent("special_bonus_unique_windranger_2")
	then
