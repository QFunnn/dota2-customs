...
function modifier_minigames_unit:OnCreated(table)
	...
	local fx =
		ParticleManager:CreateParticle("particles/boss_lightning.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	...
	local TeamFx = ParticleManager:CreateParticleForTeam(
	...
	if WinBuffs ~= nil and WinBuffs < 100 then
		...
		local WinFx = ParticleManager:CreateParticle(
...
function modifier_minigames_unit:OnIntervalThink()
	...
	if GameRules:GetGameTime() >= self.SpecialInterval then
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"set_player_icon",
			{ entity = self:GetParent():entindex(), hero = self:GetCaster():entindex() }
		)
	end
