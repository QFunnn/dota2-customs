...
LinkLuaModifier(
	"modifier_haste_zone_thinker_buff",
...
function modifier_haste_zone_thinker:OnIntervalThink()
	...
	for _, hero in pairs(targets) do
		hero:AddNewModifier(
			...
			nil,
			"modifier_haste_zone_thinker_buff",
			{ duration = 15, thinker = self.parent:entindex() }
		)
	end
end

modifier_haste_zone_thinker_buff = class({})
function modifier_haste_zone_thinker_buff:IsHidden()
	return false
end
function modifier_haste_zone_thinker_buff:IsPurgable()
	return true
end
function modifier_haste_zone_thinker_buff:GetTexture()
	return "rune_haste"
end
...
function modifier_haste_zone_thinker_buff:DamageEvent_inc(params)
	...
	if self.thinker and (self.thinker:GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D() <= 400 then
		return
	end
