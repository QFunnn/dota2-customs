...
function modifier_event_proxy:RemoveOnDeath()
	return false
end
...
function modifier_event_proxy:OnTakeDamageKillCredit(event)
	...
	if not IsValidEntity(target) or not IsValidEntity(attacker) or not damage or not target:IsRealHero() then
		return
	end
	if attacker == target then
		return
	end -- self damage not calculated

	local attacker_player_id = attacker:GetPlayerOwnerID()
	if not IsValidPlayerID(attacker_player_id) then
		return
	end
