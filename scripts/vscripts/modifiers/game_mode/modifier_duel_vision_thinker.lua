...
LinkLuaModifier(
	"modifier_duel_vision_thinker_buff",
	"modifiers/game_mode/modifier_duel_vision_thinker",
	LUA_MODIFIER_MOTION_NONE
)

modifier_duel_vision_thinker = class({})

function modifier_duel_vision_thinker:IsHidden()
	return false
end
...
function modifier_duel_vision_thinker:OnIntervalThink()
	...
	for _, hero in pairs(targets) do
		hero:AddNewModifier(
			...
			nil,
			"modifier_duel_vision_thinker_buff",
			{ duration = 0.2, thinker = self:GetParent():entindex() }
		)
	end
end

modifier_duel_vision_thinker_buff = class({})
function modifier_duel_vision_thinker_buff:IsHidden()
	return true
end
function modifier_duel_vision_thinker_buff:IsPurgable()
	return false
end
