...
LinkLuaModifier(
	...
	"modifiers/game_mode/modifier_duel_logic",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_duel_field_active_hide_neutrals",
	"modifiers/game_mode/modifier_duel_logic",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_duel_field_active_spawn_blocker",
	"modifiers/game_mode/modifier_duel_logic",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_duel_field_thinker:CheckPosition(unit, is_allowed)
	...
	if unit:GetUnitName() == "npc_dota_donate_item_illusion" then
		return
	end
	if unit:IsInvulnerable() then
		return
	end
	if unit:HasModifier("modifier_primal_beast_pulverize_custom_debuff") then
		return
	end
	if not unit:IsAlive() then
		return
	end
	if unit:IsUnselectable() then
		return
	end
	...
	if is_allowed then
		if
			not self:CheckPos(unit:GetAbsOrigin()) or (GetGroundHeight(unit:GetAbsOrigin(), nil) >= self.height + 200)
		then
			change = true
		end
	else
...
function modifier_duel_field_thinker:OnIntervalThink()
	...
	if duel_mod then
		...
		for _, team in pairs(duel_mod.teams) do
			if
				not dota1x6.NIGHT_STALKER_TEAM
				or dota1x6.NIGHT_STALKER_TEAM == team
				or not duel_mod.teams_check[dota1x6.NIGHT_STALKER_TEAM]
			then
		...
		for team, tower in pairs(towers) do
			...
			if ids then
				...
				for _, id in pairs(ids) do
					if players[id] then
						local status = self:CheckPos(players[id]:GetAbsOrigin())
							and (players[id]:IsAlive() or players[id]:IsReincarnating())
						if status then
							has_players = true
						end
					end
		...
		if self.state == 1 then
			for team, timer in pairs(self.teams_indside) do
				if
					not dota1x6.NIGHT_STALKER_TEAM
					or dota1x6.NIGHT_STALKER_TEAM == team
					or not self.teams_indside[dota1x6.NIGHT_STALKER_TEAM]
				then
			...
			for _, unit in pairs(units) do
				if
					players[unit:GetId()]
					and not self.teams_indside[unit:GetTeamNumber()]
					and GetGroundHeight(unit:GetAbsOrigin(), nil) >= self.height
					and unit:GetTeamNumber() ~= DOTA_TEAM_CUSTOM_5
				then
...
function modifier_duel_field_thinker:UpdateParticle()
	...
	if not dota1x6.duel_arenas[self.index] or not dota1x6.duel_arenas[self.index].walls then
		return
	end

	if
		(self.state == 1 or self.parent:HasModifier("modifier_duel_field_active_thinker")) and not self.effect_active
	then
		self.effect_active = {}
...
function modifier_duel_field_thinker:IsAura()
	return self.state == 1
end
function modifier_duel_field_thinker:GetAuraDuration()
	return 0
end
function modifier_duel_field_thinker:GetAuraRadius()
	return self.radius
end
function modifier_duel_field_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_BOTH
end
function modifier_duel_field_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO
end
function modifier_duel_field_thinker:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
end
function modifier_duel_field_thinker:GetModifierAura()
	return "modifier_duel_field_hero_effect"
end
function modifier_duel_field_thinker:GetAuraEntityReject(unit)
	if not players[unit:GetId()] then
		return true
	end
	if not self.teams_indside[unit:GetTeamNumber()] then
		return true
	end
	if GetGroundHeight(unit:GetAbsOrigin(), nil) < self.height then
		return true
	end
	return false
end
...
function modifier_duel_field_hero_effect:NoDamage(unit)
	...
	if not self.aura_owner or not self.aura_mod then
		return 0
	end
	if not unit then
		return 0
	end
	if self.aura_mod.state ~= 1 then
		return 0
	end
	if not players[unit:GetId()] then
		return 0
	end
	if self.aura_mod.teams_indside[unit:GetTeamNumber()] then
		return 0
	end
...
function modifier_duel_field_active_thinker:OnCreated(params)
	...
	if not self.thinker_mod then
		return
	end
	if not duel_data[self.index] then
		return
	end
	if not duel_data[self.index].field then
		return
	end
...
function modifier_duel_field_active_thinker:DeathEvent(params)
	...
	if not self.heroes[unit] or self.heroes[unit] == 0 then
		return
	end
	self.heroes[unit] = 0
...
function modifier_duel_field_active_thinker:IsAura()
	return true
end
function modifier_duel_field_active_thinker:GetAuraDuration()
	return 2
end
function modifier_duel_field_active_thinker:GetAuraRadius()
	return self.thinker_mod.radius
end
function modifier_duel_field_active_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_BOTH
end
function modifier_duel_field_active_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BASIC
end
function modifier_duel_field_active_thinker:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
end
function modifier_duel_field_active_thinker:GetModifierAura()
	return "modifier_duel_field_active_hide_neutrals"
end
function modifier_duel_field_active_thinker:GetAuraEntityReject(unit)
	...
	if unit:HasModifier("modifier_duel_field_active_spawn_blocker") then
		return true
	end
	if not self.thinker_mod:CheckPos(unit:GetAbsOrigin()) then
		return true
	end
	return false
end
...
function modifier_duel_hero_thinker:IsHidden()
	return true
end
function modifier_duel_hero_thinker:IsPurgable()
	return false
end
function modifier_duel_hero_thinker:RemoveOnDeath()
	return false
end
...
function modifier_duel_hero_end:IsHidden()
	return true
end
function modifier_duel_hero_end:IsPurgable()
	return false
end
...
function modifier_duel_hero_teleport:IsHidden()
	return true
end
function modifier_duel_hero_teleport:IsPurgable()
	return false
end
