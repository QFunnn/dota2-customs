...
LinkLuaModifier("modifier_aegis_custom_particle", "modifiers/utils/modifier_aegis_custom", LUA_MODIFIER_MOTION_NONE)

modifier_aegis_custom = class({})

function modifier_aegis_custom:IsHidden()
	return false
end
function modifier_aegis_custom:IsPurgable()
	return false
end
function modifier_aegis_custom:IsPurgeException()
	return false
end
function modifier_aegis_custom:RemoveOnDeath()
	return false
end
...
function modifier_aegis_custom:ReincarnateTime()
	if IsServer() then
		if
			not self.parent:HasModifier("modifier_death")
			or self.parent:HasModifier("modifier_axe_culling_blade_custom_aegis")
		then
			local ability_reincarnation = self.parent:FindAbilityByName("skeleton_king_reincarnation_custom")
			if ability_reincarnation == nil or not ability_reincarnation:IsFullyCastable() then
				CreateModifierThinker(
					self.parent,
					nil,
					"modifier_aegis_custom_particle",
					{ duration = 5 },
...
function modifier_aegis_custom_particle:IsHidden()
	return false
end
function modifier_aegis_custom_particle:IsPurgable()
	return false
end
function modifier_aegis_custom_particle:IsPurgeException()
	return false
end
function modifier_aegis_custom_particle:RemoveOnDeath()
	return false
end
...
function modifier_aegis_custom_particle:OnCreated()
	...
	local ReincarnateParticle = ParticleManager:CreateParticle(
