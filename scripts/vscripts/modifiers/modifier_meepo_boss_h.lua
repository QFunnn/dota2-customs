...
function modifier_meepo_boss_h:IsHidden()
	return true
end
function modifier_meepo_boss_h:IsPurgable()
	return false
end

function modifier_meepo_boss_h:IsAura()
	return true
end
...
function modifier_meepo_boss_h_aura:IsHidden()
	return true
end
...
function modifier_meepo_boss_h_aura:OnIntervalThink()
	...
	if PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()) then
		CustomGameEventManager:Send_ServerToPlayer(
			PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()),
			"event_woda_boss_health_bar_update",
			{ health = health, max_health = max_health, unit_name = unit_name, hp = persentage }
		)
	end
	if self:GetParent():CanEntityBeSeenByMyTeam(self:GetCaster()) and self:GetCaster():IsAlive() then
		if PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()) then
			...
			CustomGameEventManager:Send_ServerToPlayer(
				...
				{ health = health, max_health = max_health, unit_name = unit_name }
			)
		end
	else
		if PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()) then
			CustomGameEventManager:Send_ServerToPlayer(
				PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()),
				"event_woda_boss_health_bar_end",
				{}
			)
		end
	end
...
function modifier_meepo_boss_h_aura:OnDestroy()
	...
	if PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()) then
		CustomGameEventManager:Send_ServerToPlayer(
			PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()),
			"event_woda_boss_health_bar_update",
			{ health = 0, max_health = max_health, unit_name = unit_name, hp = 0 }
		)
	end
	if PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()) then
		CustomGameEventManager:Send_ServerToPlayer(
