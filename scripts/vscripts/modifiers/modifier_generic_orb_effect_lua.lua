...
function modifier_generic_orb_effect_lua:ShouldLaunch( target )
	...
	if self.cast and self.ability:IsFullyCastable() and (not self:GetParent():IsSilenced() or (self.ability and self.ability:GetAbilityName() == "zuus_lightning_hands_custom")) then
		return true
	end
