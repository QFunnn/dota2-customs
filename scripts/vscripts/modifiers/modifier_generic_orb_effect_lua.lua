...
function modifier_generic_orb_effect_lua:OnRefresh(kv) end

function modifier_generic_orb_effect_lua:OnDestroy(kv) end
...
function modifier_generic_orb_effect_lua:ShouldLaunch(target)
	...
	if
		...
		and (
			not self:GetParent():IsSilenced()
			or (self.ability and self.ability:GetAbilityName() == "zuus_lightning_hands_custom")
		)
	then
		return true
	end
