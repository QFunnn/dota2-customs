...
LinkLuaModifier(
	"modifier_patrol_reward_2_necro_effect",
	"modifiers/patrol_rewards/modifier_patrol_reward_2_necro",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_patrol_reward_2_necro_creeps",
	"modifiers/patrol_rewards/modifier_patrol_reward_2_necro",
	LUA_MODIFIER_MOTION_NONE
)

modifier_patrol_reward_2_necro = class({})
function modifier_patrol_reward_2_necro:IsHidden()
	return true
end
function modifier_patrol_reward_2_necro:RemoveOnDeath()
	return false
end
function modifier_patrol_reward_2_necro:IsPurgable()
	return false
end
...
function modifier_patrol_reward_2_necro:OnCreated(table)
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"NecroAttack",
		{ delay = delay, player_id = self.parent:GetPlayerOwnerID(), attacker = self.parent:GetUnitName() }
	)
	...
	for team, tower in pairs(towers) do
		if team ~= self.parent:GetTeamNumber() then
			towers[team]:AddNewModifier(
				...
				self,
				"modifier_patrol_reward_2_necro_effect",
				{ duration = delay, is_caster = 0 }
			)
		end
	end

	if towers[self.parent:GetTeamNumber()] then
		towers[self.parent:GetTeamNumber()]:AddNewModifier(
			self.parent,
			self,
			"modifier_patrol_reward_2_necro_effect",
			{ duration = delay, is_caster = 1 }
		)
	end
...
function modifier_patrol_reward_2_necro_effect:IsHidden()
	return true
end
function modifier_patrol_reward_2_necro_effect:IsPurgable()
	return false
end
function modifier_patrol_reward_2_necro_effect:GetAttributes()
	...
	if self.ids then
		for _, id in pairs(self.ids) do
			CustomGameEventManager:Send_ServerToPlayer(
				...
				"NecroWave_think",
				{ time = self.timer, max = self.max_timer }
			)
		end
	end
end
...
function modifier_patrol_reward_2_necro_creeps:IsHidden()
	return true
end
function modifier_patrol_reward_2_necro_creeps:IsPurgable()
	return false
end
