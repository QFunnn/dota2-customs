...
function modifier_patrol_reward_1_vision:IsHidden()
	return false
end
function modifier_patrol_reward_1_vision:GetTexture()
	return self.hero_name
end
function modifier_patrol_reward_1_vision:RemoveOnDeath()
	return false
end
function modifier_patrol_reward_1_vision:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_patrol_reward_1_vision:IsPurgable()
	return false
end
...
function modifier_patrol_reward_1_vision:OnIntervalThink()
	...
	if not self.hero or self.hero:IsNull() or not self.hero:IsAlive() then
		return
	end
...
function modifier_patrol_reward_1_vision:DeathEvent(params)
	...
	if not self.hero or self.hero:IsNull() or self.hero:IsReincarnating() then
		return
	end
	if self.hero ~= params.unit then
		return
	end
