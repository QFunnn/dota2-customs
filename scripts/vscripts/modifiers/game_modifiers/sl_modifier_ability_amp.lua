...
function sl_modifier_ability_amp.prototype._CalculateAmpValueByType(
	...
	local ____amp_config_____type_0 = amp_config[____type]
	local all_level_values = ____amp_config_____type_0.all_level_values
	local level_values = ____amp_config_____type_0.level_values
	...
	if level_values and #level_values > 0 then
		...
		local ____level_values_index_1 = level_values[max_level + 1]
		if ____level_values_index_1 == nil then
			____level_values_index_1 = result
		end
		result = ____level_values_index_1
	end
...
function sl_modifier_ability_amp.prototype._FilterSpecialValueForAbilityAmp(self, ability, special_key)
	...
	if IsInToolsMode() then
		if self:_FilterSpecialRecord(ability) then
			local ____self__special_record_ability_name_2 = self._special_record[ability_name]
			if ____self__special_record_ability_name_2 == nil then
				____self__special_record_ability_name_2 = {}
			end
			local record = ____self__special_record_ability_name_2
...
function sl_modifier_ability_amp.prototype.RefreshSpecialAttr(self)
	...
	for key in pairs(self._attr_value) do
		...
		if attr_value ~= current_value then
			...
			local ____self__attr_cached_send_keys_3 = self._attr_cached_send_keys
			____self__attr_cached_send_keys_3[#____self__attr_cached_send_keys_3 + 1] = attr_name
			need_send = true
		end
	end
	if need_send then
...
function sl_modifier_ability_amp.prototype._GetTotalBonusValueForSpecialAttr(self, ability, special_key, original_value)
	...
	local ____table__special_attr_cached_table_ability_name_special_key_4 =
		self._special_attr_cached_table[ability_name]
	if ____table__special_attr_cached_table_ability_name_special_key_4 ~= nil then
		____table__special_attr_cached_table_ability_name_special_key_4 =
			____table__special_attr_cached_table_ability_name_special_key_4[special_key]
	end
	local cached_value = ____table__special_attr_cached_table_ability_name_special_key_4
	if cached_value then
		...
		for key in pairs(relation_attrs) do
			do
				...
				local ____self_6 = self._special_attr_calc_cb
				total_bonus = total_bonus + ____self_6[attr](____self_6, original_value, self._attr_value[attr])
			end
			::__continue70::
		end
		local ____self__special_attr_cached_table_7, ____ability_name_8 = self._special_attr_cached_table, ability_name
		if ____self__special_attr_cached_table_7[____ability_name_8] == nil then
			____self__special_attr_cached_table_7[____ability_name_8] = {}
		end
...
function sl_modifier_ability_amp.prototype.GetModifierOverrideAbilitySpecial(self, event)
	...
	local ability = ____event_9.ability
	local ability_special_value = ____event_9.ability_special_value
...
function sl_modifier_ability_amp.prototype.GetModifierOverrideAbilitySpecialValue(self, event)
	local ____event_10 = event
	local ability = ____event_10.ability
	local ability_special_level = ____event_10.ability_special_level
	local ability_special_value = ____event_10.ability_special_value
	local ability_name = ability:GetAbilityName()
	local ____table__total_cached_table_ability_name_ability_special_value_13 = self._total_cached_table[ability_name]
	if ____table__total_cached_table_ability_name_ability_special_value_13 ~= nil then
		____table__total_cached_table_ability_name_ability_special_value_13 =
			____table__total_cached_table_ability_name_ability_special_value_13[ability_special_value]
	end
	local ____table__total_cached_table_ability_name_ability_special_value_ability_special_level_11 =
		____table__total_cached_table_ability_name_ability_special_value_13
	if ____table__total_cached_table_ability_name_ability_special_value_ability_special_level_11 ~= nil then
		____table__total_cached_table_ability_name_ability_special_value_ability_special_level_11 =
			____table__total_cached_table_ability_name_ability_special_value_ability_special_level_11[ability_special_level]
	end
	local cached_value = ____table__total_cached_table_ability_name_ability_special_value_ability_special_level_11
	if cached_value then
		return cached_value
	else
		...
		local ____self__total_cached_table_15, ____ability_name_16 = self._total_cached_table, ability_name
		if ____self__total_cached_table_15[____ability_name_16] == nil then
			____self__total_cached_table_15[____ability_name_16] = {}
		end
		local ____self__total_cached_table_ability_name_17, ____ability_special_value_18 =
			self._total_cached_table[ability_name], ability_special_value
		if ____self__total_cached_table_ability_name_17[____ability_special_value_18] == nil then
			____self__total_cached_table_ability_name_17[____ability_special_value_18] = {}
		end
