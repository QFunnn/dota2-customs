...
function modifier_minigames_arena:AttackLandedModifier(event)
	...
	if
		parent
		and target == parent
		and not parent:HasModifier("modifier_minigames_arena_evasion")
		and not parent:HasModifier("modifier_item_custom_rune_shield")
		and not parent:HasModifier("modifier_minigames_warrior_refraction")
	then
