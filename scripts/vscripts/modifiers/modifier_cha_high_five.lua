...
function modifier_cha_high_five:IsHidden()
	return true
end
function modifier_cha_high_five:IsPurgable()
	return false
end
function modifier_cha_high_five:IsPurgeException()
	return false
end
function modifier_cha_high_five:OnCreated()
	if not IsServer() then
...
function modifier_cha_high_five:OnIntervalThink()
	...
	CreateModifierThinker(
		self:GetParent(),
		nil,
		"modifier_cha_high_five_thinker",
		{ duration = (vPoint - target:GetOrigin()):Length2D() / 700 },
		vPoint,
		self:GetParent():GetTeamNumber(),
		false
	)
