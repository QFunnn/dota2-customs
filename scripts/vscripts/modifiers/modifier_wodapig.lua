...
function modifier_wodapig:IsPurgable()
	return false
end

function modifier_wodapig:IsHidden()
	return true
end

function modifier_wodapig:RemoveOnDeath()
	return false
end
...
function modifier_wodapig:OnAttackLanded(keys)
	...
	if keys.target == self:GetParent() then
		if keys.attacker:GetUnitName() == "npc_dota_thinker" then
			return
		end
