...
function modifier_creeps_trap:IsHidden()
	return false
end
function modifier_creeps_trap:IsPurgable()
	return true
end
function modifier_creeps_trap:GetStatusEffectName()
	return "particles/status_fx/status_effect_techies_stasis.vpcf"
end
function modifier_creeps_trap:GetEffectName()
...
function modifier_creeps_trap:DamageEvent_inc(params)
	...
	if (self.parent:GetAbsOrigin() - params.attacker:GetAbsOrigin()):Length2D() >= 2000 then
		return
	end
...
function modifier_creeps_trap:OnIntervalThink()
	...
	for _, id in pairs(self.ids) do
		CustomGameEventManager:Send_ServerToPlayer(
			...
			"TrapAlert_think",
			{ time = self:GetRemainingTime(), max = Trap_Duration }
		)
	end
end
...
function modifier_creeps_movespeed:IsHidden()
	return true
end
function modifier_creeps_movespeed:IsPurgable()
	return false
end
