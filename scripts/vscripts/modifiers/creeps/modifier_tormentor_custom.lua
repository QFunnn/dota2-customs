...
function modifier_tormentor_custom:OnIntervalThink()
	...
	for team, tower in pairs(towers) do
		...
		for _, player in pairs(players) do
			if
				player:GetTeamNumber() == team
				and (player:IsAlive() or player:IsReincarnating())
				and (player:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.radius
			then
	...
	if team_count >= 2 then
		self.parent:AddNewModifier(
			self.parent,
			self.shield_ability,
			"modifier_tormentor_custom_damage_reduce",
			{ side = self.side }
		)
	...
	if
		self.shield <= self.max_shield * self.health_2 / 100
		and not self.parent:HasModifier("modifier_tormentor_custom_field")
		and not self.parent:HasModifier("modifier_tormentor_custom_field_cd")
	then
		self.parent:AddNewModifier(
			self.parent,
			self.shield_ability,
			"modifier_tormentor_custom_field",
			{ side = self.side, duration = self.field_duration }
		)
	end
	...
	if self.parent:HasModifier("modifier_tormentor_custom_field") then
		if not self.teams_inside then
			...
			for _, player in pairs(players) do
				if
					(player:IsAlive() or player:IsReincarnating())
					and (player:GetAbsOrigin() - self.origin):Length2D() <= self.radius
				then
		...
		for _, unit in pairs(units) do
			if
				unit:GetTeamNumber() ~= DOTA_TEAM_NEUTRALS
				and not self.teams_inside[unit:GetTeamNumber()]
				and not unit:HasModifier("modifier_tormentor_custom_field_knock_cd")
			then
	...
	if self.parent:GetAbsOrigin() == self.origin then
		return
	end
...
function modifier_tormentor_custom:DeathReward(attacker)
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"mini_alert_event",
		{
			hero_1 = attacker_table:GetUnitName(),
			hero_2 = low_player == -1 and low_player or low_player:GetUnitName(),
			event_type = "tormentor",
		}
	)
end
...
function modifier_tormentor_custom:GetModifierIncomingDamageConstant(params)
	...
	if
		((attacker:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() > self.radius or not attacker:IsAlive())
		or (self.teams_inside and not self.teams_inside[attacker:GetTeamNumber()])
	then
		...
		if self.invun_proc[invun_index] then
			...
			if self.shield - real_damage < min_shield then
				...
				self.parent:AddNewModifier(
					self.parent,
					self.shield_ability,
					"modifier_tormentor_custom_invun",
					{ side = self.side, duration = self.invun_duration }
				)
			end
		end
...
function modifier_tormentor_custom_field_cd:RemoveOnDeath()
	return false
end
function modifier_tormentor_custom_field_cd:IsDebuff()
	return true
end
