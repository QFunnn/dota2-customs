...
function modifier_waveupgrade:IsHidden()
	return true
end
function modifier_waveupgrade:IsPurgable()
	return false
end
...
function modifier_waveupgrade:NoDamage(attacker)
	...
	if self.no_damage == 0 then
		return 0
	end
	if not self.parent.host_team then
		return 0
	end
	if attacker:GetTeamNumber() == self.parent.host_team then
		return 0
	end
	return 1
end
...
function modifier_waveupgrade:OnCreated(table)
	...
	if self.exception[self.parent:GetUnitName()] then
		return
	end
