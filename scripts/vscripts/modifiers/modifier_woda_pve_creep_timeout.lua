...
LinkLuaModifier(
	"modifier_woda_pve_creep_timeout_armor_debuff",
...
function modifier_woda_pve_creep_timeout:OnIntervalThink()
	if IsServer() then
		...
		if
			self:GetParent():GetUnitName() == "npc_woda_pig_pve"
			or self:GetParent():GetUnitName() == "npc_woda_frog_pve"
		then
			...
			for _, enemy in pairs(enemies) do
				ApplyDamage({
					...
					victim = enemy,
					ability = nil,
					damage = enemy:GetMaxHealth() / 100 * self:GetStackCount(),
...
function modifier_woda_pve_creep_timeout_manacost:IsPurgable()
	return false
end
function modifier_woda_pve_creep_timeout_manacost:IsDebuff()
	return true
end
function modifier_woda_pve_creep_timeout_manacost:IsPurgeException()
	return false
end
function modifier_woda_pve_creep_timeout_manacost:RemoveOnDeath()
	return false
end
function modifier_woda_pve_creep_timeout_manacost:GetAttributes()
...
function modifier_woda_pve_creep_upgrade_wave:IsHidden()
	return true
end

function modifier_woda_pve_creep_upgrade_wave:IsPurgable()
	return false
end

function modifier_woda_pve_creep_upgrade_wave:IsPurgeException()
	return false
end
...
function modifier_woda_pve_creep_timeout_cooldown:IsPurgable()
	return false
end
function modifier_woda_pve_creep_timeout_cooldown:IsPurgeException()
	return false
end
function modifier_woda_pve_creep_timeout_cooldown:IsHidden()
	return true
end
function modifier_woda_pve_creep_timeout_cooldown:RemoveOnDeath()
	return false
end