...
function modifier_boss_low_health:IsPurgable()
	return false
end
function modifier_boss_low_health:IsHidden()
	return self:GetStackCount() == 1
end
function modifier_boss_low_health:IsPurgeException()
	return false
end
function modifier_boss_low_health:GetTexture()
...
function modifier_boss_low_health:OnTakeDamage(params)
	...
	if self:GetParent():HasModifier("modifier_boss_low_health_buff") then
		return
	end
	...
	self:GetParent()
		:AddNewModifier(self:GetParent(), nil, "modifier_boss_low_health_buff", { duration = self.invul_duration })
	self:GetParent():SetHealth(self.new_charge_health)
	self.charge_counter = math.max(self.charge_counter - 1, 1)
	local modifier_boss_shadow_fiend_passive_dark_cyclone =
		self:GetParent():FindModifierByName("modifier_boss_shadow_fiend_passive_dark_cyclone")
	if
		modifier_boss_shadow_fiend_passive_dark_cyclone
		and self.charge_counter <= 4
		and not modifier_boss_shadow_fiend_passive_dark_cyclone.activated
	then
...
function modifier_boss_low_health_buff:IsPurgable()
	return false
end
function modifier_boss_low_health_buff:IsPurgeException()
	return false
end
function modifier_boss_low_health_buff:GetTexture()
	return "item_aeon_disk"
end
