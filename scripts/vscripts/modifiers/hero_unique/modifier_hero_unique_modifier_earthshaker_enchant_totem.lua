...
function modifier_hero_unique_modifier_earthshaker_enchant_totem:IsHidden()
	return true
end
function modifier_hero_unique_modifier_earthshaker_enchant_totem:IsPurgable()
	return false
end
function modifier_hero_unique_modifier_earthshaker_enchant_totem:IsPurgeException()
	return false
end
function modifier_hero_unique_modifier_earthshaker_enchant_totem:RemoveOnDeath()
	return false
end
...
function modifier_hero_unique_modifier_earthshaker_enchant_totem:AttackLandedModifier(params)
	...
	if self.parent:HasModifier("modifier_earthshaker_enchant_totem") then
		return
	end
...
function modifier_hero_unique_modifier_earthshaker_enchant_totem:AttackLandedModifier(params)
	...
	if self.ability == nil or self.ability:IsNull() or self.ability:GetLevel() <= 0 then
		return
	end
	if params.attacker ~= self:GetParent() then
		return
	...
	if not self.parent:HasModifier("modifier_earthshaker_enchant_totem") then
		...
		if self:GetStackCount() >= 3 then
			Timers:CreateTimer(0, function()
				self:GetParent()
					:AddNewModifier(self:GetParent(), self.ability, "modifier_earthshaker_enchant_totem", {})
			end)
			self:SetStackCount(0)
		end
	end
	...
	if not self:GetParent():HasModifier("modifier_earthshaker_enchant_totem") then
		return
	end
