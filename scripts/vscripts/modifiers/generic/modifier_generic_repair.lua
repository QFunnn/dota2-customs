...
function modifier_generic_repair:IsHidden()
	return false
end
function modifier_generic_repair:IsPurgable()
	return false
end
function modifier_generic_repair:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_generic_repair:GetTexture()
...
function modifier_generic_repair:OnCreated(table)
	...
	if
		self.parent:GetUnitName() ~= "npc_towerradiant"
		and self.parent:GetUnitName() ~= "npc_towerdire"
		and table.heal_shrine
	then
		self.heal = table.heal_shrine
	end

	self.particle =
		ParticleManager:CreateParticle("particles/items5_fx/repair_kit.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_generic_repair:AddCustomTransmitterData()
	return {
		heal = self.heal,
	}
end
