...
LinkLuaModifier(
	"modifier_voice_module_damage_cd",
	"modifiers/main_mods/modifier_voice_module",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_voice_module_lasthit_cd",
	"modifiers/main_mods/modifier_voice_module",
	LUA_MODIFIER_MOTION_NONE
)

modifier_voice_module = class(mod_hidden)
function modifier_voice_module:RemoveOnDeath()
	return false
end
...
function modifier_voice_module:MakeSound(sound, player_only, timer, more_players, priority, global)
	...
	if self.sound ~= nil and not priority then
		return false
	end
	...
	if GameRules:GetDOTATime(false, false) - self.cd_time < self.current_cd and not priority then
		return false
	end
...
function modifier_voice_module:OrderEvent(params)
	...
	if GameRules:GetDOTATime(false, false) - self.cd_time < self.current_cd then
		return
	end
	...
	if
		ability
		and ability:GetName() ~= "invoker_sun_strike_custom"
		and GameRules:GetDOTATime(false, false) - self.cast_cd_time > self.cast_cd
		and (type == DOTA_UNIT_ORDER_CAST_POSITION or type == DOTA_UNIT_ORDER_CAST_TARGET)
		and ability:GetCastRange(self.parent:GetAbsOrigin(), nil)
	then
...
function modifier_voice_module:IsMkArcana()
	return self.parent:HasModifier("modifier_monkey_king_arcana_custom_v1")
		or self.parent:HasModifier("modifier_monkey_king_arcana_custom_v2")
		or self.parent:HasModifier("modifier_monkey_king_arcana_custom_v3")
		or self.parent:HasModifier("modifier_monkey_king_arcana_custom_v4")
end
...
function modifier_voice_module:DeathEvent(params)
	...
	if
		(self.parent == params.attacker or (params.attacker.owner and params.attacker.owner == self.parent))
...
function modifier_voice_module:DamageEvent_inc(params)
	...
	if not IsValid(params.attacker) or not params.attacker:IsHero() then
		return
	end
	if params.attacker:GetTeamNumber() == self.parent:GetTeamNumber() then
		return
	end
	if self.parent:HasModifier("modifier_voice_module_damage_cd") then
		return
	end
...
function modifier_voice_module:SpellEvent(params)
	...
	if self.unit_name == "npc_dota_hero_monkey_king" then
		if
			ability_name == "monkey_king_boundless_strike_custom"
			and not self.parent:HasTalent("modifier_monkey_king_boundless_7")
		then
			priority = true
...
function modifier_voice_module_lasthit_cd:IsHidden()
	return true
end
function modifier_voice_module_lasthit_cd:IsPurgable()
	return false
end
function modifier_voice_module_lasthit_cd:RemoveOnDeath()
	return false
end

modifier_voice_module_damage_cd = class({})
function modifier_voice_module_damage_cd:IsHidden()
	return true
end
function modifier_voice_module_damage_cd:IsPurgable()
	return false
end
function modifier_voice_module_damage_cd:RemoveOnDeath()
	return false
end
	...
	if self.unit_name == "npc_dota_hero_queenofpain" then
		if self.model_name == "models/items/queenofpain/queenofpain_arcana/queenofpain_arcana.vmdl" then
			if
				...
				and (target:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= 350
			then
				string_table = {
