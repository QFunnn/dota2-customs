...
function modifier_infinite_form_mana_drain:IsHidden()
	return true
end
function modifier_infinite_form_mana_drain:IsPurgable()
	return false
end
function modifier_infinite_form_mana_drain:IsPurgeException()
	return false
end
...
function modifier_infinite_form_mana_drain:OnIntervalThink()
	...
	local drain = self.parent:GetMaxMana() * 0.02 * 0.5
	if self.parent:GetMana() < drain then
		if ab.GetToggleState and ab:GetToggleState() then
			ab:ToggleAbility()
		end -- выключит форму (OnToggle off)
		self:Destroy()
		return
	end
	self.parent:SpendMana(drain, ab)
