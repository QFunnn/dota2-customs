...
modifier_creep_controll = class({
	...
	IsPermanent = function(self)
		return true
	end,
	GetAttributes = function(self)
		return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_IGNORE_DODGE + MODIFIER_ATTRIBUTE_PERMANENT
	end,

	IsBerserked = function(self)
	...
	OnIntervalThink = function(self)
		...
		if self:GetStackCount() > 60 then
			...
			if self.TeamID then
				...
				if active then
					teamPlayers = {}
					for pid, _ in pairs(active) do
						teamPlayers[pid] = true
					end
				end
			end
	...
	GetModifierMoveSpeedBonus_Constant = function(self)
		return CREEP_BONUS_MOVE_SPEED_PER_LATE_ROUND * self.UpgradeStacks
	end,
	...
	GetModifierModelScale = function(self)
		if self:GetStackCount() > 0 then
			if self.UnitName == "npc_dota_roshan" then
				return 25
			end
			if self.UnitName == "npc_dota_nian" then
				return 25
			end
	...
	GetModifierAttackSpeedBonus_Constant = function(self)
		return (CREEP_BONUS_ATTACK_SPEED_PER_LATE_ROUND * self.UpgradeStacks) + (self:GetStackCount() * 5)
	end,
	...
	GetModifierStatusResistanceStacking = function(self)
		...
		if self:GetStackCount() > 0 then
			...
			if self.LateResist == 0 then
				...
				if self.UnitName == "npc_dota_granite_golem" then
					BerserkBonus = 0
				end
			end
	...
	GetModifierSpellAmplify_Percentage = function(self)
		return 9 * self.SpellAmplifyStacks
	end,
	GetModifierIncomingDamage_Percentage = function(self)
		return -1 * self.ResistStacks
	end,
})
