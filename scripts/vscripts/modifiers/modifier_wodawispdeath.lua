...
function modifier_wodawispdeath:OnDeath(params)
	...
	if params.unit == self:GetParent() then
		...
		if self:GetParent():HasModifier("modifier_wodaduel_boss_pve") then
			return
		end
		if self:GetParent():HasModifier("modifier_wodaduel_boss_pve_end") then
			return
		end

		if GetMapName() == "arena" then
			if arena_system:GetAegisCount(self:GetParent()) <= 0 then
				return
			end
		end

		if player_system:IsLose(self:GetParent():GetPlayerID()) then
			return
		end
		...
		self.wisp:AddNewModifier(
			self.wisp,
			nil,
			"modifier_wodawispdeath_wisp",
			{ owner = self:GetParent():GetPlayerOwnerID() }
		)
...
function modifier_wodawispdeath:OnRespawn(params)
	...
	if GetMapName() == "arena" then
		...
		self:GetParent()
			:AddNewModifier(self:GetParent(), nil, "modifier_woda_pve_creep_timeout_cooldown", { duration = 5 })
	end
...
function modifier_wodawispdeath_wisp:OnCreated(params)
	...
	if GetMapName() == "rating_duo" or GetMapName() == "rating_duo_300" then
		...
		self.wisp_particle = ParticleManager:CreateParticle(
