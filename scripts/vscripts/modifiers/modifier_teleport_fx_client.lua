...
function modifier_teleport_fx_client:IsHidden()
	return true
end
function modifier_teleport_fx_client:IsPurgable()
	return false
end
function modifier_teleport_fx_client:IsPurgeException()
	return false
end
function modifier_teleport_fx_client:RemoveOnDeath()
	return false
end
function modifier_teleport_fx_client:OnCreated()
	...
	if player_system.PLAYERS_GLOBAL_INFORMATION[player_id] then
		if
			player_system.PLAYERS_GLOBAL_INFORMATION[player_id].teleport_id
			and tonumber(player_system.PLAYERS_GLOBAL_INFORMATION[player_id].teleport_id) ~= 0
		then
			particle =
				_G.EMBLEMS_LIST[tonumber(player_system.PLAYERS_GLOBAL_INFORMATION[player_id].teleport_id)]["particle_name"]
		end
	end
	...
	ParticleManager:SetParticleControl(
		...
		2,
		Vector(
			TEAMS_COLORS[self:GetParent():GetTeamNumber()][1] / 255,
			TEAMS_COLORS[self:GetParent():GetTeamNumber()][2] / 255,
			TEAMS_COLORS[self:GetParent():GetTeamNumber()][3] / 255
		)
	)
	...
	if particle == "particles/items2_fx/teleport_start.vpcf" then
		local pfx_origin = ParticleManager:CreateParticle(
			"particles/items2_fx/teleport_origin.vpcf",
