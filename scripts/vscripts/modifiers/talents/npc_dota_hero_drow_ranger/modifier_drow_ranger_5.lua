...
function modifier_drow_ranger_5:IsHidden()
	return true
end
function modifier_drow_ranger_5:IsPurgable()
	return false
end
function modifier_drow_ranger_5:IsPurgeException()
	return false
end
function modifier_drow_ranger_5:RemoveOnDeath()
	return false
end
	...
	if drow_ranger_marksmanship_custom then
		...
		for _, enemy in pairs(enemies) do
			if enemy ~= params.target then
				...
				local drow_ranger_frost_arrows_custom = self:GetParent()
					:FindAbilityByName("drow_ranger_frost_arrows_custom")

				local ultimate = false

				if drow_ranger_frost_arrows_custom then
					if
						drow_ranger_frost_arrows_custom:GetAutoCastState()
						and drow_ranger_frost_arrows_custom:IsFullyCastable()
					then
