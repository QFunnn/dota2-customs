...
function modifier_axe_9:IsHidden()
	return true
end
function modifier_axe_9:IsPurgable()
	return false
end
function modifier_axe_9:IsPurgeException()
	return false
end
function modifier_axe_9:RemoveOnDeath()
	return false
end
...
function modifier_axe_9:OnIntervalThink()
	...
	if self.distance_to_skill > self.bonus_distance[self:GetStackCount()] then
		self.distance_to_skill = 0
		local modifier_axe_counter_helix_custom = self:GetParent()
			:FindModifierByName("modifier_axe_counter_helix_custom")
