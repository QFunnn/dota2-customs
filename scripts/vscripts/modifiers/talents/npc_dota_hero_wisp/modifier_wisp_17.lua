...
function modifier_wisp_17:IsHidden()
	return true
end
function modifier_wisp_17:IsPurgable()
	return false
end
function modifier_wisp_17:IsPurgeException()
	return false
end
function modifier_wisp_17:RemoveOnDeath()
	return false
end
...
function modifier_wisp_17:OnIntervalThink()
	...
	if self:GetParent():GetMana() >= self:GetParent():GetMaxMana() then
		...
		self.current_shield =
			math.min(self.current_shield + (self:GetParent():GetManaRegen() * 0.5 * FrameTime()), self.max_shield)
	end
