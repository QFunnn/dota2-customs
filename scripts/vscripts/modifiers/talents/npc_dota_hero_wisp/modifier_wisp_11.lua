...
LinkLuaModifier(
	"modifier_wisp_11_buff",
	"modifiers/talents/npc_dota_hero_wisp/modifier_wisp_11",
	LUA_MODIFIER_MOTION_NONE
)

modifier_wisp_11 = class({})

function modifier_wisp_11:IsHidden()
	return true
end
function modifier_wisp_11:IsPurgable()
	return false
end
function modifier_wisp_11:IsPurgeException()
	return false
end
function modifier_wisp_11:RemoveOnDeath()
	return false
end
...
function modifier_wisp_11_buff:IsPurgable()
	return false
end
function modifier_wisp_11_buff:IsPurgeException()
	return false
end
function modifier_wisp_11_buff:IsHidden()
	return true
end
...
function modifier_wisp_11_buff:OnAttackLanded(params)
	...
	if RollPercentage(self.bonus[self:GetCaster():GetTalentLevel("modifier_wisp_11")]) then
		local units = FindUnitsInRadius(
			...
			nil,
			self:GetCaster():GetAoeBonus(self.radius),
		...
		local particle =
			ParticleManager:CreateParticle("particles/wisp_custom/attack_explosion.vpcf", PATTACH_WORLDORIGIN, nil)
