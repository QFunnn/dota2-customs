...
function modifier_visage_1:IsHidden()
	return true
end
function modifier_visage_1:IsPurgable()
	return false
end
function modifier_visage_1:IsPurgeException()
	return false
end
function modifier_visage_1:RemoveOnDeath()
	return false
end
...
function modifier_visage_1:OnIntervalThink()
	...
	if not self.particle then
		self.particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_visage/visage_cloak_ambient_damage.vpcf",
	...
	if self.damage_think >= 1 then
		...
		local visage_gravekeepers_cloak_custom = self:GetParent()
			:FindModifierByName("modifier_visage_gravekeepers_cloak_custom")
		...
		for _, enemy in pairs(enemies) do
			ApplyDamage({
				...
				ability = visage_gravekeepers_cloak_custom,
