...
function modifier_dragon_knight_2:RemoveOnDeath()
	return false
end
...
function modifier_dragon_knight_2:OnAttackLanded(params)
	...
	if self:GetParent():HasModifier("modifier_dragon_knight_2") then
		...
		if ability then
			local damage = self:GetParent():GetStrength()
				/ 100
				* ability.modifier_dragon_knight_2_str_multiple[self:GetParent()
					:GetTalentLevel("modifier_dragon_knight_2")]
			ApplyDamage({
