...
LinkLuaModifier(
	"modifier_phoenix_11_debuff",
	"modifiers/talents/npc_dota_hero_phoenix/modifier_phoenix_11",
	LUA_MODIFIER_MOTION_NONE
)

modifier_phoenix_11 = class({})

function modifier_phoenix_11:IsHidden()
	return true
end
function modifier_phoenix_11:IsPurgable()
	return false
end
function modifier_phoenix_11:IsPurgeException()
	return false
end
function modifier_phoenix_11:RemoveOnDeath()
	return false
end
...
function modifier_phoenix_11:OnModifierAdded(params)
	...
	if modifier:GetAbility():IsItem() then
		return
	end
...
function modifier_phoenix_11_debuff:OnIntervalThink()
	...
	for _, modifier in pairs(self:GetParent():FindAllModifiers()) do
		if
			modifier
			and modifier:GetCaster() == self:GetCaster()
			and modifier:GetAbility()
			and not modifier:GetAbility():IsItem()
		then
