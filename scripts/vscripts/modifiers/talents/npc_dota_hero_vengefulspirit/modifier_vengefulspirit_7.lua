...
function modifier_vengefulspirit_7:IsHidden()
	return true
end
function modifier_vengefulspirit_7:IsPurgable()
	return false
end
function modifier_vengefulspirit_7:IsPurgeException()
	return false
end
function modifier_vengefulspirit_7:RemoveOnDeath()
	return false
end
...
function modifier_vengefulspirit_7:OnTakeDamage(params)
	...
	if self.damage >= self:GetParent():GetMaxHealth() / 100 * 40 then
		...
		local ill = CreateIllusions(
			self:GetCaster(),
			self:GetCaster(),
			{ duration = 7, outgoing_damage = -50, incoming_damage = 100 },
			1,
			1,
			false,
			false
