...
LinkLuaModifier(
	"modifier_arc_warden_8_buff",
	"modifiers/talents/npc_dota_hero_arc_warden/modifier_arc_warden_8",
	LUA_MODIFIER_MOTION_NONE
)

modifier_arc_warden_8 = class({})

function modifier_arc_warden_8:IsHidden()
	return true
end
function modifier_arc_warden_8:IsPurgable()
	return false
end
function modifier_arc_warden_8:IsPurgeException()
	return false
end
function modifier_arc_warden_8:RemoveOnDeath()
	return false
end
...
function modifier_arc_warden_8:OnAbilityFullyCast(params)
	if IsServer() then
		...
		self:GetCaster():AddNewModifier(
			...
			nil,
			"modifier_arc_warden_8_buff",
			{ duration = self.bonus_duration[self:GetStackCount()] }
		)
	end
end

modifier_arc_warden_8_buff = class({})
function modifier_arc_warden_8_buff:IsPurgable()
	return false
end
function modifier_arc_warden_8_buff:IsPurgeException()
	return false
end
