...
function modifier_skills_bonuses:IsHidden()
	return true
end
function modifier_skills_bonuses:IsPurgable()
	return false
end
function modifier_skills_bonuses:IsPurgeException()
	return false
end
function modifier_skills_bonuses:RemoveOnDeath()
	return false
end
...
function modifier_skills_bonuses:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if params.damage_type == DAMAGE_TYPE_MAGICAL then
		...
		if
			...
			and not self:GetParent():HasModifier("modifier_magical_critical_strike_custom")
			and self:GetParent():IsAlive()
		then
			self:GetParent()
				:AddNewModifier(self:GetParent(), nil, "modifier_magical_critical_strike_custom", { duration = 0.6 })
...
function modifier_skills_bonuses:GetModifierBonusStats_Strength()
	...
	if
		self:GetValue("main_atributes") > 0
		and (self:GetParent():GetPrimaryAttribute() == 0 or self:GetParent():GetPrimaryAttribute() == 3)
	then
...
function modifier_skills_bonuses:GetModifierBonusStats_Agility()
	...
	if
		self:GetValue("main_atributes") > 0
		and (self:GetParent():GetPrimaryAttribute() == 1 or self:GetParent():GetPrimaryAttribute() == 3)
	then
		bonus = bonus + self:GetValue("main_atributes")
	end
...
function modifier_skills_bonuses:GetModifierBonusStats_Intellect()
	...
	if
		self:GetValue("main_atributes") > 0
		and (self:GetParent():GetPrimaryAttribute() == 2 or self:GetParent():GetPrimaryAttribute() == 3)
	then
		bonus = bonus + self:GetValue("main_atributes")
	end
...
function modifier_skills_bonuses:TakeDamageScriptModifier(params)
	...
	if
		...
		and not self:FlagExist(params.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION)
		and Lifesteal > 0
	then
