...
function modifier_woda_boss_bar_pve:IsHidden()
	return true
end
function modifier_woda_boss_bar_pve:IsPurgable()
	return false
end
...
function modifier_woda_boss_bar_pve:OnDestroy()
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_woda_boss_health_bar_update",
		{
			health = 0,
			max_health = self.fixed_max_health,
...
function modifier_woda_boss_bar_pve:OnIntervalThink()
	...
	if modifier_boss_low_health then
		local health = (modifier_boss_low_health.charge_counter * modifier_boss_low_health.new_charge_health)
			- (modifier_boss_low_health.new_charge_health - self:GetCaster():GetHealth())
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"event_woda_boss_health_bar_update",
			{
				...
				unit_name = self.unit_name,
				stages_count = max_charge_counter,
				hp_stages = hp_stages,
				multiplier = charge_counter,
			}
		)
		CustomGameEventManager:Send_ServerToAllClients(
			"event_woda_boss_health_bar_start",
			{ health = health, max_health = max_health, unit_name = self.unit_name, stages_count = max_charge_counter }
		)
	end
end
...
function modifier_woda_boss_bar_pve_aura:IsHidden()
	return true
end