...
modifier_windwalk = class({})

-- animation = ACT_DOTA_RUN

function modifier_windwalk:IsStunDebuff()
	return true
end

function modifier_windwalk:IsHidden()
	return true
end

function modifier_windwalk:IsPurgable()
	return false
end

function modifier_windwalk:RemoveOnDeath()
	return false
end

function modifier_windwalk:OnCreated(kv)
	if IsServer() then
		...
		self.vStartPosition =
			Entities:FindByName(nil, "center" .. ((self:GetParent().at_team_id or self:GetParent().team_id) - 6))
				:GetOrigin()
...
function modifier_windwalk:OnDestroy()
	if IsServer() then
...
-- function modifier_windwalk:DeclareFunctions()
--     local funcs = {
...
function modifier_windwalk:CheckState()
	local state = {
...
-- function modifier_windwalk:GetOverrideAnimation()
--     return animation or ACT_DOTA_RUN
-- end

function modifier_windwalk:UpdateHorizontalMotion(me, dt)
	if IsServer() then
		-- 判断是否到达了终点
