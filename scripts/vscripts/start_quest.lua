...
function start_quest:CheckTimer()
	...
	if start_quest.test_disable then
		return
	end
...
function start_quest:HeroPickEvent(id)
	...
	if not player or not subData then
		return
	end
	if subData.disable_quest == 1 then
		return
	end
	if HTTP.playersData[id].achivment_done_before and HTTP.playersData[id].achivment_done_before[2] then
		return
	end
...
function start_quest:BeginQuest(id, name)
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		"ShowQuestAlert",
		{ type = name, timer = quest_data.timer }
	)
...
function start_quest:SendCurrentQuests(data)
	...
	if not players[id] or players[id].disable_quest == 1 or players[id].completed_start_quest == 1 then
		return
	end
	...
	CustomGameEventManager:Send_ServerToPlayer(
		PlayerResource:GetPlayer(id),
		"UpdateCurrentQuests",
		{ quest_data = players[id].start_quest_table, max_quest = start_quest.max_quest }
	)
end
...
function start_quest:CheckQuest(data)
	...
	local id = data.id
	if not id or not players[id] then
		return
	end
	if players[id].disable_quest == 1 then
		return
	end
	if players[id].completed_start_quest == 1 then
		return
	end
...
function start_quest:CompleteQuest(id, quest_name, progress)
	...
	if not id or not players[id] or not players[id].start_quest_table then
		return
	end
				...
				Timers:CreateTimer(3, function()
					...
					elseif data.follow_message then
						CustomGameEventManager:Send_ServerToPlayer(
							PlayerResource:GetPlayer(id),
							"ShowQuestAlert",
							{ type = data.follow_message, timer = data.timer }
						)
					end
				end)
...
function start_quest:RegisterPing(name, player)
	...
	if name == "Quest_3" then
		...
		table.sort(spawner_table, function(x, y)
			return (y:GetAbsOrigin() - tower_origin):Length2D() > (x:GetAbsOrigin() - tower_origin):Length2D()
		end)
		local count = 0
	...
	if name == "Quest_7" then
		local radiant_creeps =
			Entities:FindAllByModel("models/creeps/lane_creeps/creep_radiant_melee/radiant_melee_mega.vmdl")
		...
		local dire_creeps =
			Entities:FindAllByModel("models/creeps/lane_creeps/creep_bad_melee/creep_bad_melee_mega.vmdl")
...
function start_quest:SendTip(id)
	...
	if not player or player.disable_tips == 1 then
		return
	end
	if player.completed_start_quest == 0 and player.disable_quest == 0 then
		return
	end
