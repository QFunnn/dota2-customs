...
function PoolSelectionHelper.prototype.IsSelecting(self, iPlayerID)
	return NetEventData:GetTableValue(self.sTable, self:selectKey(iPlayerID)) ~= nil
end
function PoolSelectionHelper.prototype.GetSelectList(self, iPlayerID)
	return NetEventData:GetTableValue(self.sTable, self:selectKey(iPlayerID)) or {}
end
function PoolSelectionHelper.prototype.ClearSelect(self, iPlayerID)
	NetEventData:SetTableValue(self.sTable, self:selectKey(iPlayerID), nil)
end
function PoolSelectionHelper.prototype.TryOpen(self, iPlayerID)
	...
	local iCount = NetEventData:GetTableValue(self.sTable, self:selectCountKey(iPlayerID))
...
function PoolSelectionHelper.prototype.GiveUpSelect(self, iPlayerID, addRefreshCount)
	...
	NetEventData:SetTableValue(self.sTable, self:selectCountKey(iPlayerID), iCount)
	self:ClearSelect(iPlayerID)
	return true
end
function PoolSelectionHelper.prototype.TryRefresh(self, iPlayerID, createSelection)
	local iCount = NetEventData:GetTableValue(self.sTable, self:freeRefreshKey(iPlayerID)) or 0
	if iCount <= 0 then
		return false
	end
	iCount = iCount - 1
	createSelection(_G, iPlayerID)
	NetEventData:SetTableValue(self.sTable, self:freeRefreshKey(iPlayerID), iCount)
	return true
end
function PoolSelectionHelper.prototype.IsValidChoice(self, iPlayerID, sName)
	return __TS__ArrayIncludes(self:GetSelectList(iPlayerID), sName)
end
