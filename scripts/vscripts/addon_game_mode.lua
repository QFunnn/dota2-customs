...
function GameMode:InitGameMode()
	...
	GameRules:GetGameModeEntity()
		:SetItemAddedToInventoryFilter(Dynamic_Wrap(item_filter_check, "ItemAddedToInventoryFilter"), item_filter_check)
	...
	CustomGameEventManager:RegisterListener("zxc_sec_probe", function(source, event)
		...
		_ENGINE_PRINT_RAW(
			string.format(
				"[SEC-EVT] PROBE claimed=%s real_from_client=%s src=%s",
				tostring(event.PlayerID),
				tostring(event.real),
				tostring(source)
			)
		)
...
function GameMode:OnGameRulesStateChange()
	...
	if nNewState == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
		...
		if ResetSpawnGroupTracking then
			ResetSpawnGroupTracking()
		end
		...
		PlayerTables:CreateTable(
			"globals",
			{ neutrals_rounds = GIVE_NEUTRALS_ROUNDS, bans_categories = SPEICAL_TIERS_TABLE },
			Players:GetAllPlayers(true)
		)
		...
		for PlayerID, PlayerInfo in pairs(HeroBuilder:GetAllPlayers()) do
			...
			print(
				string.format(
					...
					tostring(hasHero),
					tostring(source),
					tostring(PlayerInfo.selected_hero),
	...
	if nNewState == DOTA_GAMERULES_STATE_STRATEGY_TIME then
		for PlayerID, PlayerInfo in pairs(HeroBuilder:GetAllPlayers()) do
			...
			if hPlayer then
				...
				elseif
					PlayerInfo.selected_hero == ""
					or PlayerInfo.selected_hero == nil
					or PlayerInfo.selected_hero ~= heroName
				then
