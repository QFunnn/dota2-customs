...
function dota1x6:initiate_waves()
	...
	for i = 1, 2 do
		repeat
			random_factor = RandomInt(1, #boss)
...
function dota1x6:GetMkb(index, is_boss)
	...
	if not data or not data.mkb then
		return 0
	end
	return data.mkb
end
...
function dota1x6:SpawnNecro(team, caster_team, damage_out, damage_inc)
	...
	for i = 1, #necro_wave_info do
		local spawner = Entities:FindByName(nil, "spawner_team" .. number):GetAbsOrigin()
			+ RandomVector(RandomInt(-1, 1) + 125)
		local unit = dota1x6:CreateUnitCustom(
			necro_wave_info[i],
			spawner,
			true,
			nil,
			...
			function(unit)
				...
				if i == #necro_wave_info then
					for j = 1, #ally do
						ally[j].ally = ally
						ally[j]:AddNewModifier(
							ally[j],
							nil,
							"modifier_item_patrol_necro_creeps",
							{ caster_team = caster_team, damage_out = damage_out, damage_inc = damage_inc }
						)
					end
				end
			end
...
function dota1x6:spawn_wave(team, wave_number, boss, lownet, more_gold, trap_wave, show_gold)
	...
	if not array[wave_number] then
		return
	end

	local data = wave_types[array[wave_number]]

	if not data or not data.creeps then
		return
	end
...
function dota1x6:spawn_portal(team, necro_caster)
	...
	teleport_center.nWarningFX =
		ParticleManager:CreateParticle("particles/portals/portal_ground_spawn_endpoint.vpcf", PATTACH_WORLDORIGIN, nil)
