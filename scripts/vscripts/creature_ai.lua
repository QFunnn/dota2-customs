...
function CreepThinkBody()
	if not IsEntityReadyForAI() then
		return Jitter(1.0)
	end
...
function GetOrFindAttackTarget()
	if
		IsValid(thisEntity.attackTarget)
		and thisEntity.attackTarget:IsAlive()
		and not thisEntity.attackTarget:IsUnselectable()
	then
