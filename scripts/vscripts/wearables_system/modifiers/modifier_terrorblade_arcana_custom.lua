...
function modifier_terrorblade_arcana_custom:IsHidden()
	return true
end
function modifier_terrorblade_arcana_custom:IsPurgable()
	return false
end
function modifier_terrorblade_arcana_custom:IsPurgeException()
	return false
end
function modifier_terrorblade_arcana_custom:RemoveOnDeath()
	return false
end
...
function modifier_terrorblade_arcana_custom_death:IsHidden()
	return true
end
function modifier_terrorblade_arcana_custom_death:IsPurgable()
	return false
end
function modifier_terrorblade_arcana_custom_death:RemoveOnDeath()
	return false
end
...
function modifier_terrorblade_arcana_custom_death:OnRefresh(table)
	...
	if self.parent:IsAlive() or self.parent:IsReincarnating() then
		return
	end
	self.interval = 0.02
	...
	self.death_particle = ParticleManager:CreateParticle(
	...
	self.max_timer = 3
	if
		self.parent:GetUnitName() == "npc_dota_hero_nevermore"
		or self.parent:GetUnitName() == "npc_dota_hero_phantom_assassin"
	then
		self.max_timer = 1.5
	end
...
function modifier_terrorblade_arcana_custom_death:OnIntervalThink()
	...
	if self.timer >= self.max_timer and self.ended == false then
		...
		local particle = ParticleManager:CreateParticle(
			"particles/econ/items/terrorblade/terrorblade_horns_arcana/terrorblade_arcana_enemy_death_custom_pop.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
