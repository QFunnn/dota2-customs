...
ListenToGameEvent("event_change_ability_icon", function(event)
	...
	if unit then
		...
		if ability_name and ability_name ~= "" and ability_name ~= " " then
			wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_icon_replace[ability_name] = wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_icon_replace[ability_name]
				or {}
			wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_icon_replace[ability_name][asset] =
				modifier
		else
...
ListenToGameEvent("event_change_ability_pfx", function(event)
	...
	if unit then
		...
		if ability_name and ability_name ~= "" and ability_name ~= " " then
			wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_pfx_replace[ability_name] = wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_pfx_replace[ability_name]
				or {}
			wearables_system.ReplacementInfo[unit:GetUnitName()].ability_effects_pfx_replace[ability_name][asset] =
				modifier
		else
...
function wearables_system:GetAbilityIconReplacement(unit, icon_name, ability_handle, ability_name_new)
	...
	if ability_name then
		...
		if ability_effects_icon_replace and ability_effects_icon_replace[icon_name] then
			return ability_effects_icon_replace[icon_name]
		end
	end
	local ability_icon_replace = replacement_info.ability_icon_replace[icon_name]
	if ability_icon_replace then
		return ability_icon_replace
	end
	return icon_name
end
...
function wearables_system:GetParticleReplacementAbility(unit, particle_name, ability_handle, ability_name_new)
	...
	if ability_name then
		if
			replacement_info.ability_effects_pfx_replace[ability_name]
			and replacement_info.ability_effects_pfx_replace[ability_name][particle_name]
		then
