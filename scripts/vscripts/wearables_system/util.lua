...
function CDOTA_BaseNPC:RemoveAllDefaultItems()
	...
	for k, child in pairs(children) do
		if child and child:GetClassname() == "dota_item_wearable" then
			...
			if
				model_name ~= ""
				and wearables_system.items_data_slots[hero_name]
				and wearables_system.items_data_slots[hero_name][model_name]
			then
				wearables_system:SaveDefaultItemsData(
					hero_name,
					wearables_system.items_data_slots[hero_name][model_name],
					model_name,
					child:GetMaterialGroupHash()
				)
			end
			UTIL_Remove(child)
		end
...
function CDOTA_BaseNPC:GetItemStyle(item_id_original)
	if
		...
		and wearables_system.ITEMS_DATA[self:GetUnitName()]
		and wearables_system.ITEMS_DATA[self:GetUnitName()][item_id_original]
	then
...
function wearables_system:CheckingUniqueArcaneItems(hero)
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/pudge_cute/pudge_cute.vmdl"
	...
	elseif
		hero
		and (
			hero:GetModelName() == "models/items/pudge/arcana/pudge_arcana_base.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/juggernaut/juggernaut_arcana.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/items/axe/ti9_jungle_axe/axe_bare.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/items/ogre_magi/ogre_arcana/ogre_magi_arcana.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/shadow_fiend/shadow_fiend_arcana.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/crystal_maiden_persona/crystal_maiden_persona.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/phantom_assassin_persona/phantom_assassin_persona.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/antimage_female/antimage_female.vmdl"
			or hero:GetModelName() == "models/items/antimage_female/mh_antimage_kirin/antimage_female_kirin.vmdl"
			or hero:GetModelName() == "models/items/antimage_female/mh_antimage_kirin/antimage_female_kirin_rainbow.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/heroes/invoker_kid/invoker_kid.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/items/axe/axe_carnival/axe_carnival_base.vmdl"
	...
	if
		...
		and (
			hero:GetModelName()
				== "models/items/legion_commander/dark_carnival_legion_commander/dark_carnival_legion_commander_base.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/items/morphling/morphling_automaton/morphling_automaton.vmdl"
	...
	if
		hero
		and (
			hero:GetModelName() == "models/items/bristleback/bristlebot/bristlebot.vmdl"
...
function wearables_system:HasStartSelectionItem(player_id, hero_name, item_slot_type, item_id)
	if
		...
		and wearables_system.selection_data[player_id][hero_name][item_slot_type] ~= nil
	then
