...
--GetHeroItems("npc_dota_hero_pangolier", true)
	...
	for name, data in pairs(all_items) do
		if name == "items" then
			for item_id, item_data in pairs(data) do
				local is_effect = item_data["item_slot"]
					and item_data["item_slot"] == "ambient_effects"
					and item_data["visuals"]
					and type(item_data["visuals"] == "table")
				local is_item = not item_data["item_slot"] or item_data["item_slot"]:match("persona") == nil

				if
					item_data["prefab"]
						and item_data["used_by_heroes"]
						and item_data["model_player"]
						and item_data["prefab"] == "default_item"
						and is_item
					or is_effect
				then
									...
									if is_effect then
										...
										for effect_name, effect_data in pairs(item_data["visuals"]) do
											if effect_data["type"] and effect_data["type"] == "particle_create" then
												...
												local custom_effect_name = replace_table[effect_name]
														and replace_table[effect_name]
													or effect_name

												for effect_id, effect_data in
													pairs(all_items["attribute_controlled_attached_particles"])
												do
													if
														effect_data["system"]
														and effect_data["system"] == effect_name
														and effect_data["attach_type"]
														and effect_data["attach_entity"]
													then
														hero_table[hero_name]["effects"][custom_effect_name] = {}
														hero_table[hero_name]["effects"][custom_effect_name]["attach_type"] =
															effect_data["attach_type"]
														hero_table[hero_name]["effects"][custom_effect_name]["attach_entity"] =
															effect_data["attach_entity"]

														if effect_data["control_points"] then
															hero_table[hero_name]["effects"][custom_effect_name]["control_points"] =
																effect_data["control_points"]
														end
													end
												end
											end
