...
function Map:AddModifierToChecker(ModifierName, hParent, hCaster, hAbility)
	if not hCaster or not hAbility or not hParent or hCaster:IsNull() or hAbility:IsNull() or hParent:IsNull() then
		return
	end
...
function Map:AddAbilityToChecker(hAbility, hParent)
	if not hAbility or not hParent or hAbility:IsNull() or hParent:IsNull() then
		return
	end
...
function Map:RegisterArena(Name, Mins, Maxs, Center, GroundMins, GroundMaxs)
	...
	if CenterPosition == nil or type(CenterPosition) == "string" then
		return
	end
		...
		if Team ~= DOTA_TEAM_NEUTRALS and unit:GetUnitName() ~= "npc_dota_hero_target_dummy" then
			...
			if PlayerHero and PlayerHero:IsRealHero() then
				...
				if PlayerInfo then
					...
					if
						CurrentArenaName == "MINIGAMES"
						and PlayerHero ~= unit
						and string.match(unit:GetUnitName(), "minigames") == nil
					then
			...
			for ModifierName, ModifierInfo in pairs(Modifiers) do
				if ModifierInfo.caster and not ModifierInfo.caster:IsNull() then
					if not Unit:FindModifierByNameAndCaster(ModifierName, ModifierInfo.caster) then
						if
							ModifierInfo.ability
							and not ModifierInfo.ability:IsNull()
							and ModifierInfo.ability._bCustomDisabled == nil
						then
...
function Map:CreatePenguins()
	...
	if #Ents > 0 then
		for i = 1, 2 do
			...
			local Penguin =
				CreateUnitByName("npc_penguin_map", rEnt:GetAbsOrigin(), false, nil, nil, DOTA_TEAM_NEUTRALS)
...
function Map:CreateBabyRoshans()
	for RoshanName, Info in pairs(self.Roshans) do
		...
		if Ent then
			...
			local Roshan =
				CreateUnitByName("npc_baby_roshan_map", Ent:GetAbsOrigin(), false, nil, nil, DOTA_TEAM_NEUTRALS)
...
function Map:GetPositionArena(Position)
	...
	if Position.GetAbsOrigin then
		Position = Position:GetAbsOrigin()
	end
	...
	for ArenaName, ArenaInfo in pairs(self:GetAllArenas()) do
		if
			IsInCube(
				Position,
				Vector(ArenaInfo.mins.x, ArenaInfo.mins.y, 0),
				Vector(ArenaInfo.maxs.x, ArenaInfo.maxs.y, 0)
			)
		then
			Arena = ArenaName
			break
