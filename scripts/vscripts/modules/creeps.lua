...
function Creeps:SpawnUnitByName(
	sUnitName,
	vLocation,
	bClearSpace,
	hOwner,
...
function Creeps:DeleteUnitAsync(hUnit, bIsRoundSpawned)
	...
	if hUnit:IsNull() then
		table.insert(
			self.DeleteSchedule,
			{ unit = nil, time = GameRules:GetGameTime() + 10, spawngroup = SpawnGroup, unit_name = sUnitName }
		)
		return
	end
	...
	table.insert(
		self.DeleteSchedule,
		{ unit = hUnit, time = GameRules:GetGameTime() + 10, spawngroup = SpawnGroup, unit_name = sUnitName }
	)
end
...
function Creeps:DeleteDeathUnitsThink()
	if #self.DeleteSchedule > 0 then
		...
		self.DeleteSchedule = ArrayRemove(self.DeleteSchedule, function(t, i, j)
			local UnitInfo = t[i]
			if
				CurrentTime >= UnitInfo.time
				and UnitInfo.unit ~= nil
				and not UnitInfo.unit:IsNull()
				and not UnitInfo.unit:IsAlive()
			then
...
function Creeps:CreepsAIThink()
	if not Rounds:StateIs(GAME_STATES.IN_ACTION) or GameRules:IsGamePaused() then
		return 0.1
	end
...
function Creeps:FindMainArenaTarget(hUnit)
	...
	for _, Target in ipairs(PossibleTargets) do
		if
			...
			and Target:IsAlive()
			and Players:IsUnitCanAttackOrCastOnThis(hUnit, Target)
		then
...
function Creeps:TryCastAbilitites(hUnit, Target)
	-- Базовые проверки
	if hUnit.next_try_cast_ability ~= nil and hUnit.next_try_cast_ability > GameRules:GetGameTime() then
		return
	end
	if hUnit:IsSilenced() or hUnit:IsHexed() then
		return
	end
	...
	for i = 0, hUnit:GetAbilityCount() - 1 do
		...
		if hAbility and not hAbility:IsPassive() and hAbility:IsFullyCastable() then
			...
			if
				ContainsValue(Behavior, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET)
				and not ContainsValue(Behavior, DOTA_ABILITY_BEHAVIOR_ATTACK)
			then
				-- Вражеские способности
				if
					(
						ContainsValue(TargetTeam, DOTA_UNIT_TARGET_TEAM_ENEMY)
						or ContainsValue(TargetTeam, DOTA_UNIT_TARGET_TEAM_CUSTOM)
						or ContainsValue(TargetTeam, DOTA_UNIT_TARGET_TEAM_BOTH)
					) and (not Target:IsInvisibleForUnit(hUnit) or canCastOnInvisible)
				then
				...
				if
					ContainsValue(TargetTeam, DOTA_UNIT_TARGET_TEAM_FRIENDLY)
					or ContainsValue(TargetTeam, DOTA_UNIT_TARGET_TEAM_BOTH)
				then
			...
			elseif
				ContainsValue(Behavior, DOTA_ABILITY_BEHAVIOR_POINT)
				and (not Target:IsInvisibleForUnit(hUnit) or canCastOnInvisible)
			then
			...
			elseif
				ContainsValue(Behavior, DOTA_ABILITY_BEHAVIOR_NO_TARGET)
				and not ContainsValue(Behavior, DOTA_ABILITY_BEHAVIOR_AUTOCAST)
			then
...
function Creeps:FindNearbyEnemy(hUnit)
	...
	if CreepTeam == nil then
		...
		for _, enemy in ipairs(enemies) do
			if
				not enemy:IsInvisibleForUnit(hUnit)
				and not enemy:IsInvulnerable()
				and not enemy:IsAttackImmune()
				and Players:IsUnitCanAttackOrCastOnThis(hUnit, enemy)
			then
				nearbyTarget = enemy
				break
			end
		...
		for _, enemy in ipairs(self.CreepsCache[CreepTeam].enemies) do
			if
				...
				and enemy:IsAlive()
				and not enemy:IsInvisibleForUnit(hUnit)
				and not enemy:IsInvulnerable()
				and not enemy:IsAttackImmune()
				and Players:IsUnitCanAttackOrCastOnThis(hUnit, enemy)
			then
...
function Creeps:ThinkDefaultAI(hUnit)
	...
	if hUnit:HasModifier("modifier_creep_spawner_cha_ready") then
		return 0.1
	end
	...
	if hUnit:IsChanneling() or hUnit:GetCurrentActiveAbility() ~= nil then
		return 0.1
	end
	...
	elseif
		hCurrentTarget:IsAttackImmune()
		or hCurrentTarget:IsInvulnerable()
		or hCurrentTarget:IsInvisibleForUnit(hUnit)
		or (not hUnit:IsAttacking() and CalculateDistance(hUnit, hCurrentTarget) > 150)
		or hUnit:IsHexed()
		or hUnit:IsDisarmed()
		-- Специальная проверка для комбинации иммунитетов (физический + магический)
		or (hCurrentTarget:IsMagicImmune() and hCurrentTarget:IsAttackImmune())
	then
...
function Creeps:_SpawnUnitByName(SpawnInfo)
	...
	TryPrecacheUnitAsync(SpawnInfo.sUnitName, function(prec_Id)
		...
		CreateUnitByNameAsync(
			SpawnInfo.sUnitName,
			SpawnInfo.vLocation,
			SpawnInfo.bClearSpace,
			SpawnInfo.hOwner,
			SpawnInfo.hPlayerOwner,
			SpawnInfo.iTeam,
			function(hUnit)
