...
function Items:Init()
	...
	CustomGameEventManager:RegisterListener("items_move_to_custom_stash", function(_, event)
		self:OnPlayerWantMoveToCustomStash(event)
	end)
	CustomGameEventManager:RegisterListener("items_move_from_custom_stash", function(_, event)
		self:OnPlayerWantMoveFromCustomStash(event)
	end)
	CustomGameEventManager:RegisterListener("items_sell_from_custom_stash", function(_, event)
		self:OnPlayerWantSellFromCustomStash(event)
	end)
	CustomGameEventManager:RegisterListener("items_move_neutral_from_custom_stash", function(_, event)
		self:OnPlayerWantMoveNeutralFromCustomStash(event)
	end)
end
	...
	local MakeDotaToCustomSwap = function()
		...
		if DotaItem ~= nil then
			...
			if BestSlot == nil then
				...
				if
					DotaSlot == 16 and BestSlot_Item and not KeyValues:IsNeutralItem(BestSlot_Item:GetName())
					or (KeyValues:IsNeutralItem(BestSlot_Item:GetName()) and DotaSlot <= 5)
				then
		...
		elseif CustomItem ~= nil then
			...
			if CustomItem_Item then
				if
					(DotaSlot == 16 and not KeyValues:IsNeutralItem(CustomItem_Item:GetName()))
					or (KeyValues:IsNeutralItem(CustomItem_Item:GetName()) and DotaSlot <= 5)
				then
					self:Log("Item is not for this slot")
					return false
				end
...
function Items:_DOTASwapItems(Hero, slot1, slot2)
	...
	Hero:SetContextThink(DoUniqueString("Swap"), function(ent)
		...
		if Hero and not Hero:IsNull() and type(Slot1) == "number" and type(Slot2) == "number" then
			...
			if
				(Hero:GetItemInSlot(Slot1) and KeyValues:IsNeutralItem(Hero:GetItemInSlot(Slot1):GetName()))
				or (Hero:GetItemInSlot(Slot2) and KeyValues:IsNeutralItem(Hero:GetItemInSlot(Slot2):GetName()))
				or (type(slot1) == "table" and slot1.IsItem ~= nil and KeyValues:IsNeutralItem(slot1:GetName()))
				or (type(slot2) == "table" and slot2.IsItem ~= nil and KeyValues:IsNeutralItem(slot2:GetName()))
			then
...
function Items:UpdateItemBySlot(Unit, Item)
	if Item == nil or not Unit:HasAbility("techies_spoons_stash_custom") then
		return
	end
	...
	elseif
		Item._custom_deleted_intrinsic
		and (Item:GetItemSlot() < DOTA_STASH_SLOT_1 or Item:GetItemSlot() > DOTA_STASH_SLOT_6)
	then
		Item:SetItemState(1)
		...
		for SLOT_NAME, ItemInfo in pairs(self.Players[PlayerID].items) do
			if ItemInfo.item_name == Item:GetName() and ItemInfo.item ~= Item:entindex() then
				...
				if
					ItemH
					and (
						ItemH:GetCurrentCharges() < self:GetMaxStacks(ItemInfo.item_name)
						or self:GetMaxStacks(ItemInfo.item_name) <= 0
					)
				then
					return SLOT_NAME
				end
...
function Items:FixEnchant(PlayerID, Hero, Item)
	...
	if not KeyValues:IsNeutralItem(Item:GetName()) then
		return
	end
