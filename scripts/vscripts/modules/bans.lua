...
function Bans:Init()
	...
	CustomGameEventManager:RegisterListener("bans_ban", function(_, event)
		self:OnPlayerWantBan(event)
	end)
	CustomGameEventManager:RegisterListener("bans_random_bans", function(_, event)
		self:OnPlayerWantRandomBans(event)
	end)
	CustomGameEventManager:RegisterListener("bans_banned_showed", function(_, event)
		self:OnBansShowed(event)
	end)
...
function Bans:OnPlayerWantRandomBans(event)
	...
	if
		...
		or self.Players[PlayerID].used_random_bans
		or self.Players[PlayerID].banned_atleast_one_time
	then
		return
	end
	...
	for i = 1, self.Players[PlayerID].random_heroes do
		...
		if
			#PlayerFavoriteBans > 0
			and RollPseudoRandomPercentage(
				RANDOM_BANS_CHANCES["FAVORITES"],
		...
		elseif
			#SSSHeroes > 0
			and RollPseudoRandomPercentage(RANDOM_BANS_CHANCES["SSS"], DOTA_PSEUDO_RANDOM_CUSTOM_GAME_1, self.DummyUnit)
		then
		...
		elseif
			#AHeroes > 0
			and RollPseudoRandomPercentage(RANDOM_BANS_CHANCES["A"], DOTA_PSEUDO_RANDOM_CUSTOM_GAME_2, self.DummyUnit)
		then
	...
	for i = 1, self.Players[PlayerID].random_abilities do
		...
		elseif
			#SSSAbilities > 0
			and RollPseudoRandomPercentage(RANDOM_BANS_CHANCES["SSS"], DOTA_PSEUDO_RANDOM_CUSTOM_GAME_3, self.DummyUnit)
		then
		...
		elseif
			#AAbilities > 0
			and RollPseudoRandomPercentage(RANDOM_BANS_CHANCES["A"], DOTA_PSEUDO_RANDOM_CUSTOM_GAME_4, self.DummyUnit)
		then
...
function Bans:GetAllRandomBansGroupUnbanned(Category, GroupName)
	if RANDOM_BANS_GROUPS[Category] == nil or RANDOM_BANS_GROUPS[Category][GroupName] == nil then
		return {}
	end
	local AllUnbanned = {}
...
function Bans:OnPlayerWantBanHero(PlayerID, HeroName)
	if
		self.Players[PlayerID].heroes <= 0
		or self.Players[PlayerID].used_random_bans == true
		or table.contains(self.BanList.heroes, HeroName)
	then
		return
	end
...
function Bans:OnPlayerWantBanAbility(PlayerID, AbilityName)
	if
		self.Players[PlayerID].abilities <= 0
		or self.Players[PlayerID].used_random_bans == true
		or table.contains(self.BanList.abilities, AbilityName)
	then
		return
	end
