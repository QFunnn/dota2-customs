...
LinkLuaModifier(
	"modifier_arctic_burn_no_collision",
	"modifiers/modifier_arctic_burn_no_collision",
...
function Players:Init()
	...
	CustomGameEventManager:RegisterListener("players_player_want_pause_game", function(source, event)
		self:OnPlayerWantPauseGame(event)
	end)

	CustomGameEventManager:RegisterListener("players_player_want_tip_player", function(source, event)
		self:OnPlayerWantTipPlayer(event)
	end)
	...
	for _, Value in ipairs(Values) do
		...
		elseif Value ~= "random" then
			if Value == "loser_curse" then
				table.sort(TeamsStats, function(a, b)
...
function Players:TryMakeTeamLose(TeamID)
	if not self:IsActiveTeam(TeamID) then
		return
	end
...
function Players:MakeTeamLose(TeamID)
	DiscLog(
		"!!! MakeTeamLose(team=%d) CALLED, gameState=%d, gameTime=%.2f",
		TeamID,
		GameRules:State_Get(),
	...
	CustomNetTables:SetTableValue(
		"team_rank",
		tostring(TeamID),
		{ rank = CurrentPlace, defeat_round = Rounds:GetCurrentRound() }
	)

	for _, PlayerID in ipairs(self:GetTeamPlayers(TeamID, true)) do
		-- [A69] Запоминаем ДО смены состояния: игрок покинул игру, а не просто проиграл.
		-- Ниже state станет LOSER, и отличить ливера будет уже нельзя.
		local PlayerInfoA69 = self:GetPlayer(PlayerID)
		local nConnA69 = PlayerResource:GetConnectionState(PlayerID)
		local bLeftGameA69 = self:PlayerStateIs(PlayerID, PLAYER_STATE.LEAVED)
			or (PlayerInfoA69 ~= nil and PlayerInfoA69.was_disconnected == true)
			or nConnA69 == DOTA_CONNECTION_STATE_ABANDONED
			or nConnA69 == DOTA_CONNECTION_STATE_DISCONNECTED
		...
		-- ВАЖНО: CreatePlayerData читает героя (предметы/статы для итогов матча),
		-- поэтому удаление модельки — строго ПОСЛЕ него, ниже по циклу.
		...
		if CurrentPlace - 1 > 1 then
			...
			if hPlayer then
				CustomGameEventManager:Send_ServerToPlayer(
					hPlayer,
					"ShowPlayerLose",
					{ game_rank = CurrentPlace, valid_team = #self:GetAllActivatedTeams(true) }
				)
			end
		end
		...
		-- [A69] Модельку ПОКИНУВШЕГО игру убираем с карты: команда уже проиграла,
		-- возвращаться некуда, а брошенный герой стоял на базе и с ним можно было
		-- взаимодействовать (абуз). У обычных проигравших героя не трогаем — они
		-- остаются зрителями со своим героем.
		if bLeftGameA69 then
			local hLeftHero = PlayerResource:GetSelectedHeroEntity(PlayerID)
			if hLeftHero ~= nil and not hLeftHero:IsNull() then
				-- Сначала снимаем управление и прячем, потом удаляем сущность:
				-- голое UTIL_Remove на живом герое иногда оставляет висящие партиклы.
				hLeftHero:AddNoDraw()
				hLeftHero:Stop()
				if hLeftHero:IsAlive() then
					hLeftHero:ForceKill(false)
				end
				UTIL_Remove(hLeftHero)
				print(
					string.format(
						"[A69] Убрана моделька покинувшего игру: PID=%d, conn=%d",
						PlayerID,
						nConnA69 or -1
					)
				)
	...
	-- Осталось <=3 живых игроков: следующие дуэли сразу на аегисы, чтобы не затягивать игру —
	-- порог ROUND_WHEN_LOSE_AEGIS подменяется на текущую волну (см. Rounds:GetRoundWhenLoseAegis).
	if CurrentPlace <= 3 and Rounds.AegisRoundOverride == nil then
		Rounds.AegisRoundOverride = Rounds:GetCurrentRound()
		Rounds:UpdateDynamicSettings()
	end

	local VisionRevealer = CreateUnitByName("arena_vision_revelear", Vector(0, 0, 0), false, nil, nil, TeamID)
...
function Players:MakeTeamWin(TeamID)
	DiscLog(
		"!!! MakeTeamWin(team=%d) CALLED, gameState=%d, gameTime=%.2f",
		TeamID,
		GameRules:State_Get(),
		GameRules:GetGameTime()
	)
...
function Players:OnGameEnd(TeamID, bWin)
	DiscLog(
		"!!! OnGameEnd(team=%d, bWin=%s) CALLED, gameState=%d, gameTime=%.2f",
		TeamID,
		tostring(bWin),
		GameRules:State_Get(),
		GameRules:GetGameTime()
	...
	if self.Players[PlayerID] == nil then
		...
		if not bIsBot and PlayerResource:IsValidPlayerID(PlayerID) then
			...
			if OverrideBans then
				print(
					string.format(
						"[Players] Tournament ban override applied for SteamID=%d (PID=%d): heroes=%s abilities=%s random_heroes=%s random_abilities=%s",
						nSteamID,
						PlayerID,
						tostring(OverrideBans.heroes),
						tostring(OverrideBans.abilities),
						tostring(OverrideBans.random_heroes),
						tostring(OverrideBans.random_abilities)
					)
				)
...
-- Потолок зарядов паузы: у владельца батл-пасса он выше.
-- Пока профиль не доехал, БП ещё неизвестен и действует базовый потолок — это не мешает:
-- стартовые 2 заряда в него укладываются, а бонусный заряд выдаётся уже после загрузки
-- профиля (OnPlayerProfileLoaded), когда потолок для этого игрока становится больше.
function Players:GetMaxPauseCharges(PlayerID)
	if Server and Server.IsPlayerBattlePassSubscribed and Server:IsPlayerBattlePassSubscribed(PlayerID) then
		return PLAYER_MAX_PAUSES_BP
	end

	return PLAYER_MAX_PAUSES
end

function Players:ModifyPauseCharges(PlayerID, value)
	local PlayerInfo = self:GetPlayer(PlayerID)

	if not self:IsActivePlayer(PlayerID) then
		return
	end

	PlayerInfo.pause_count = math.min(math.max(PlayerInfo.pause_count + value, 0), self:GetMaxPauseCharges(PlayerID))
end
...
function Players:OnPlayerProfileLoaded(PlayerID, PlayerProfile)
	...
	if not bHasOverride and PlayerProfile and PlayerProfile.profile and PlayerProfile.profile.extra_bans then
		...
		if ExtraHeroes > 0 or ExtraAbilities > 0 then
			print(
				string.format(
					"[Players] Extra bans applied for SteamID=%d (PID=%d): +%d heroes, +%d abilities",
					nSteamID,
					PlayerID,
					ExtraHeroes,
					ExtraAbilities
...
function Players:OnPlayerConnected(event)
	...
	if
		...
		and PlayerResource:GetTeam(PlayerID) ~= 1
	then
...
function Players:LoadSpectator(PlayerID)
	if
		PlayerResource:GetTeam(PlayerID) ~= 1
		or PlayerResource:IsValidTeamPlayerID(PlayerID)
		or self.Spectators[PlayerID] ~= nil
	then
		return
	end
...
function Players:SubscribeSpectatorToGameData(PlayerID)
	if self.Spectators[PlayerID] == nil then
		return
	end
...
function Players:SubscibeSpectatorToPlayerGlobal(SpectatorID, PlayerID)
	if self.Spectators[SpectatorID] == nil then
		return
	end
...
function Players:OnPlayerDisconnected(event)
	...
	DiscLog(
		"DC@ENTRY: PID=%d, gameTime=%.2f, hPlayer=%s(null=%s), conn=%d",
		PlayerID,
		GameRules:GetGameTime(),
		tostring(hPlayerEntry),
	...
	if gameState < DOTA_GAMERULES_STATE_HERO_SELECTION then
		...
		DiscLog(
			"DC@FORCE_PICK: PID=%d, hPlayer=%s(null=%s), engineHero='%s', hbHero='%s', gameState=%d",
			PlayerID,
			tostring(hPlayerNow),
		...
		elseif engineHero and engineHero ~= "" then
			...
			DiscLog(
				"DC@FORCE_PICK(PID=%d): engine уже знает hero='%s', НЕ перевыбираем",
				PlayerID,
				engineHero
			)
		...
		if heroToSet then
			if hPlayerNow and not hPlayerNow:IsNull() then
				...
				local ok, err = pcall(function()
					hPlayerNow:SetSelectedHero(heroToSet)
				end)
				DiscLog(
					"DC@FORCE_PICK(PID=%d): SetSelectedHero(%s) -> ok=%s err=%s (reason=%s)",
					PlayerID,
					heroToSet,
					tostring(ok),
					tostring(err),
				...
				DiscLog(
					"DC@FORCE_PICK(PID=%d): heroToSet=%s, но hPlayer мёртв -- engine может не запомнить (reason=%s)",
					PlayerID,
					heroToSet,
					reason
				)
		...
		DiscLog(
			"OnPlayerDisconnected(PID=%d): gameState>=HERO_SELECTION (%d), engine forced pick пропускаем",
			PlayerID,
			gameState
		)
	end
		...
		for _, dt in ipairs({ 2, 5, 10, 20, 30, 45, 60, 75, 90 }) do
			Timers:CreateTimer(dt, function()
				...
				for _, tid in ipairs(self:GetAllActiveTeams(true) or {}) do
					...
					table.insert(
						teams,
						string.format("T%d:state=%s,active=%d", tid, tostring(ti and ti.state), activePlayers)
					)
				end
				local unsettled = {}
				for pid, info in pairs(HeroBuilder:GetAllPlayers() or {}) do
					if info.settled == false then
						table.insert(unsettled, tostring(pid))
					end
				end
				DiscLog(
					"SNAP T+%ds: gameState=%d, teams=[%s], unsettled=[%s], idealAbils=%d",
					dt,
					GameRules:State_Get(),
					table.concat(teams, " | "),
...
function Players:OnPlayerReconnected(event)
	...
	if not PlayerID or PlayerID < 0 then
		if event.userid then
			...
			if PlayerResource:IsValidPlayerID(user_pid) then
				PlayerID = user_pid
			end
		end
	end
	...
	if
		PlayerInfo
		and self:PlayerStateIs(PlayerID, PLAYER_STATE.LEAVED)
	...
	if gameState < DOTA_GAMERULES_STATE_HERO_SELECTION then
		DiscLog(
			"OnPlayerReconnected(PID=%d): gameState=%d < HERO_SELECTION -- игрок успел вернуться в стадию пика, UI должен отработать как обычно",
			PlayerID,
			gameState
		)
		return
...
function Players:OnNPCSpawned(event)
	...
	if IsRealHero(hUnit) and not bIsRespawn and hUnit.GetPlayerID then
		...
		if was_disconnected then
			...
			if hbPInfo and (not hbPInfo.selected_hero or hbPInfo.selected_hero == "") then
				...
				DiscLog(
					"OnNPCSpawned(PID=%d): sync HeroBuilder.selected_hero = '%s' (engine spawn winner)",
					PlayerID,
					hUnit:GetUnitName()
				)
			end
	...
	if hOwner and hOwner.HasModifier and hOwner:HasModifier("modifier_hero_refreshing") then
		...
		if
			hUnit
			and (
				hUnit:GetName() == "npc_dota_brewmaster_storm"
				or hUnit:GetName() == "npc_dota_brewmaster_fire"
				or hUnit:GetName() == "npc_dota_brewmaster_earth"
			)
		then
	...
	if
		hUnit
		and not hUnit:IsNull()
		and hUnit.GetUnitName
		and hUnit:GetUnitName()
		and not hUnit:IsIllusion()
		and not hUnit:IsTempestDouble()
		and not hUnit:HasModifier("modifier_arc_warden_tempest_double_lua")
	then
		if hUnit:IsSummoned() or SUMMON_UPGRADES_MULT[hUnit:GetUnitName()] then
			-- [A73] Саммон, появившийся во время пре-стана раунда, должен стоять вместе с
			-- владельцем. Раньше герой был обездвижен, а его саммоны на масс-арене уже
			-- бегали и атаковали. Наследование стана существовало только для юнитов
			-- мини-игр (ChangePlayerSelectedUnit) — обычные саммоны идут этим путём,
			-- поэтому вешаем здесь, СРАЗУ (до Timers ниже), иначе юнит успеет сдвинуться.
			local hPreStunOwner = GetRealUnit(hUnit)
			if hPreStunOwner and not hPreStunOwner:IsNull() and hPreStunOwner.FindModifierByName ~= nil then
				local hPreStun = hPreStunOwner:FindModifierByName("modifier_arenas_pre_stun")
				if hPreStun ~= nil then
					local flPreStunLeft = hPreStun:GetRemainingTime()
					if flPreStunLeft ~= nil and flPreStunLeft > 0 then
						hUnit:AddNewModifier(
							hPreStunOwner,
							nil,
							"modifier_arenas_pre_stun",
							{ duration = flPreStunLeft }
						)
					end
				end
			end
			...
			Timers:CreateTimer(flWaitTime, function()
				...
				if hOwner and hOwner.IsRealHero ~= nil and hOwner:IsRealHero() then
					if
						hOwner:HasModifier("modifier_item_summoner_crown")
						or hOwner:HasModifier("modifier_item_wraith_dominator")
					then
						...
						local hSourceItem = hOwner:FindItemInInventory("item_wraith_dominator")
							or hOwner:FindItemInInventory("item_summoner_crown_3")
							or hOwner:FindItemInInventory("item_summoner_crown_2")
							or hOwner:FindItemInInventory("item_summoner_crown_1")
						if hSourceItem and not hSourceItem:IsNull() then
							local hBuffAgi =
								hUnit:AddNewModifier(hOwner, hSourceItem, "modifier_item_summoner_crown_buff_agi", {})
							if (not hBuffAgi) or hBuffAgi:IsNull() then
								return nil
							end
							hBuffAgi:SetStackCount(hOwner:GetAgility() * flEfficiency)
							local hBuffInt =
								hUnit:AddNewModifier(hOwner, hSourceItem, "modifier_item_summoner_crown_buff_int", {})
												...
												nCurrentHealth
													* (
														...
														+ 0.01
															* hOwner:GetStrength()
															* flEfficiency
															* hSourceItem:GetSpecialValueFor("hp_bonus_per_str")
													)
											)
							...
							if hUnit:GetName() == "npc_dota_lone_druid_bear" then
								if
									hOwner
									and hOwner:IsRealHero()
									and hOwner.bUsedBearDarkMoon
									and not hUnit:HasModifier("modifier_item_dark_moon_shard")
								then
			...
			if player then
				if self.NextPauseChargeTime == 0 then
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"PauseNotification",
						{ message = "#dota_hud_error_no_pause_charges_no_time", time = "" }
					)
				else
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"PauseNotification",
						{
							message = "#dota_hud_error_no_pause_charges",
							time = math.floor(self.NextPauseChargeTime - CurrentTime),
						}
					)
				end
			end
		...
		if self.AllUnpauseTime > CurrentTime then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"PauseNotification",
					{ message = "#dota_hud_error_not_time_pause", time = math.floor(self.AllUnpauseTime - CurrentTime) }
				)
			end
			return
		end
		...
		elseif self.PauseOwner ~= PlayerID and self.UnpauseLastTime > CurrentTime then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					...
					"PauseNotification",
					{
						message = "#dota_hud_error_not_pause_owner",
						time = math.floor(self.UnpauseLastTime - CurrentTime),
					}
				)
			end
		end
...
function Players:OnPlayerWantTipPlayer(event)
	-- [A80/SEC] Кто чаевые ДАЁТ — только по движковому event.PlayerID: раньше бралось
	-- клиентское event.tips_player, то есть можно было разослать всем уведомление
	-- «игрок X поблагодарил Y» от чужого имени и попутно сжечь его кулдаун.
	-- Кому дают (tipped_player) — клиентское поле по смыслу, его и оставляем.
	local TipsPlayerID = event.PlayerID
	if TipsPlayerID == nil then
		TipsPlayerID = event.tips_player
	end
	local TippedPlayerID = event.tipped_player

	local TipsPlayerInfo = self:GetPlayer(TipsPlayerID)
	if not TipsPlayerInfo then
		return
	end
	...
	if CurrentTime >= TipsPlayerInfo.tip_cooldown then
		...
		CustomGameEventManager:Send_ServerToAllClients(
			...
			{ player_id_1 = TipsPlayerID, player_id_2 = TippedPlayerID }
		)
	else
		SendErrorToPlayer(TipsPlayerID, "#PLAYER_HUD_Error_Tip_Cooldown")
	end
end
			...
			callback = function()
				...
				for _, hUnit in ipairs(summonedCreature) do
					if
						...
						and hUnit.GetUnitName
						and hUnit:GetUnitName()
						and (hUnit:IsSummoned() or SUMMON_UPGRADES_MULT[hUnit:GetUnitName()])
						and not hUnit:IsIllusion()
						and not hUnit:IsTempestDouble()
						and not hUnit:HasModifier("modifier_arc_warden_tempest_double_lua")
					then
						if hUnit:IsAlive() then
							...
							if string.find(hUnit:GetUnitName(), "npc_dota_shadow_shaman_ward") == 1 then
								...
								if
									hOwner
									and hOwner:IsRealHero()
									and hOwner:HasAbility("shadow_shaman_mass_serpent_ward")
								then
...
function Players:RefreshBear(hBear)
	...
	if hOwner and hOwner:IsRealHero() then
		if
			hOwner:HasModifier("modifier_item_summoner_crown") or hOwner:HasModifier("modifier_item_wraith_dominator")
		then
...
function Players:GiveExtraCretures(PlayerID)
	...
	local PlayerInfo = self:GetPlayer(PlayerID)
	if PlayerInfo == nil or PlayerInfo.getted_extra_creeps == true or not self:IsActivePlayer(PlayerID) then
		return
	end

	if
		Rounds:GetCurrentRound() >= GetGameSetting("ROUND_WHEN_CAN_SUMMON_CREEPS")
		or #self:GetAllActivePlayers(true) <= PLAYERS_WHEN_CAN_SUMMON_CREEPS
	then
...
function Players:UpdatePlayersNetTableDamage()
	...
	for PlayerID, PlayerInfo in pairs(self:GetAllActivePlayers()) do
		...
		if #PlayerInfo.spell_damage_incoming > 0 then
			table.sort(PlayerInfo.spell_damage_incoming, function(x, y)
				return y.damage < x.damage
			end)
...
function Players:QueueCritBonus(attacker, target, crit_mult, name)
	...
	if not crit_mult or crit_mult <= 100 then
		return
	end
	if not attacker.GetAverageTrueAttackDamage then
		return
	end
...
function Players:OnDamageEvent(attacker, target, params)
	...
	if params.damage <= 0 or attacker == nil or target == nil or attacker == target then
		return
	end
		...
		for bonus_name, info in pairs(map) do
			...
			if split > 0 then
				...
				if attacker:IsRealHero() then
					self:UpdatePlayerDamage(
						attacker:GetPlayerOwnerID(),
						"spell_damage_outgoing",
						split,
						bonus_name,
						"ability",
						info.damage_type or DAMAGE_TYPE_PHYSICAL
					)
				end
...
function Players:UpdatePlayerDamage(PlayerID, nType, nDamage, sAbilityName, sAbilityType, nDamageType)
	...
	if not Server:IsPlayerBattlePassSubscribed(PlayerID) and not IsCheatsEnabled() then
		return
	end
	...
	if sAbilityName == nil or sAbilityType == nil or nDamageType == nil then
		return
	end
...
function Players:ModifyPlayerGold(PlayerID, gold, reliable, useTarget, useSound, target)
	local PlayerInfo = self:GetPlayer(PlayerID)
	if PlayerInfo == nil or not self:IsActivePlayer(PlayerID) then
		return -1
	end
	...
	if useTarget then
		...
		if RealPlayer and Target then
			...
			local fxCoins = ParticleManager:CreateParticleForPlayer(
			...
			local fxGold = ParticleManager:CreateParticleForPlayer(
				"particles/msg_fx/msg_goldbounty.vpcf",
				PATTACH_CUSTOMORIGIN,
				nil,
				RealPlayer
			)
			ParticleManager:SetParticleControl(
				fxGold,
				0,
				Target:GetAbsOrigin() + Vector(0, 0, Target:GetBaseHealthBarOffset())
			)
...
function Players:ModifyPlayerNetworth(PlayerID, value)
	if self.Players[PlayerID] == nil or not self:IsActivePlayer(PlayerID) then
		return
	end
...
function Players:UpdateLastDuelScore(WinnerTeam)
	...
	if RoundsLastDuelInfo == nil then
		return
	end
...
function Players:UpdateArena(PlayerID, ArenaName, Position, bNeedSetRespawnPoint, bNeedRefresh)
	...
	if PlayerInfo.arena == ArenaName then
		return
	end
...
function Players:UpdatePlayerHero(PlayerID, CustomCriteria, bNeedSetRespawnPoint, bNeedRefresh)
	...
	if self:IsValidHero(Hero) then
		...
		Hero:SetContextThink("HERO_UPDATING_PLAYER_" .. PlayerID, function()
			...
			if
				(
					self:PlayerStateIs(PlayerID, PLAYER_STATE.ABANDONED)
					or self:PlayerStateIs(PlayerID, PLAYER_STATE.LOSER)
				) and CustomCriteria == "KILL"
			then
					...
					Hero:SetContextThink("HERO_UPDATING_ARENA_PLAYER_" .. PlayerID, function()
						...
						if PlayerInfo.arena == "MASS_ARENA" then
							Hero:AddNewModifier(
								Hero,
								nil,
								"modifier_mass_arena_player",
								{ win_buffs = HeroBuilder:GetMinigamesWins(PlayerID) }
							)
						else
	...
	if bReplace then
		...
		if unit then
			...
			unit:AddNewModifier(
				Hero,
				nil,
				"modifier_minigames_unit",
				{ team = self:GetPlayerTeamNumber(PlayerID), win_buffs = HeroBuilder:GetMinigamesWins(PlayerID) }
			)
			...
			CustomNetTables:SetTableValue(
				"players",
				"player_" .. PlayerID .. "_select_unit",
				{ entity = unit:entindex() }
			)
...
function Players:AssignPlayerToTeam(PlayerID, TeamID)
	...
	if PlayerInfo == nil or TeamInfo == nil then
		return false
	end
	...
	if PlayerInfo.team ~= DOTA_TEAM_NOTEAM then
		return false
	end
...
function Players:GetUnitArena(Unit)
	if not self:IsValidHero(Unit) or not Unit:IsBaseNPC() then
		return
	end
...
function Players:IsUnitCanAttackOrCastOnThis(Unit, Target)
	if not self:IsValidHero(Unit) or Target == nil then
		return false
	end
...
function Players:IsActiveTeam(TeamID)
	...
	if TeamInfo.state ~= TEAM_STATE.IN_GAME then
		return false
	end
...
function Players:IsActivePlayer(PlayerID)
	...
	if PlayerInfo == nil or PlayerInfo.team == DOTA_TEAM_NOTEAM then
		return false
	end

	if PlayerInfo.state ~= PLAYER_STATE.IN_GAME and PlayerInfo.state ~= PLAYER_STATE.LEAVED then
		return false
	end

	if not self:IsActiveTeam(PlayerInfo.team) then
		return false
	end
