...
function ChatWheel:Init()
	...
	CustomGameEventManager:RegisterListener("chat_wheel_item_selected", function(source, event)
		self:OnPlayerSelectedItem(event)
	end)
	CustomGameEventManager:RegisterListener("chat_wheel_line_selected", function(source, event)
		self:OnPlayerSelectedLine(event)
	end)

	CustomGameEventManager:RegisterListener("chat_wheel_mute_player", function(source, event)
		self:OnPlayerWantMuteOtherPlayer(event)
	end)
end
...
function ChatWheel:OnPlayerSelectedItem(event)
	...
	if ItemInfo.free == false and not Server:PlayerHasItem(PlayerID, ItemName) then
		return
	end
	...
	if ChatWheelCooldown then
		if ChatWheelCooldown.free_charges > 0 then
			...
			CustomGameEventManager:Send_ServerToAllClients(
				"chat_wheel_say_line",
				{ caller_player = PlayerID, item_name = ItemName }
			)
			-- else
...
if not ChatWheel.bStarted then
	ChatWheel:Init()
end