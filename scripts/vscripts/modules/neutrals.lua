...
function NeutralItems:Init()
	...
	CustomGameEventManager:RegisterListener("neutrals_make_craft", function(_, event)
		self:OnPlayerCraftNeutralItem(event)
	end)
	CustomGameEventManager:RegisterListener("neutrals_player_want_swap", function(_, event)
		self:OnPlayerWantSwapNeutralItems(event)
	end)
	CustomGameEventManager:RegisterListener("neutrals_move_item_to_stash", function(_, event)
		self:OnPlayerWantMoveItemToStash(event)
	end)
	CustomGameEventManager:RegisterListener("neutrals_get_item_from_stash", function(_, event)
		self:OnPlayerWantGetItemFromStash(event)
	end)
end
...
function NeutralItems:IncreaseNeutralTier(PlayerID)
	if not self.Players[PlayerID] or self.Players[PlayerID].current_tier == 5 then
		return
	end
...
function NeutralItems:GiveNeutral(
	PlayerID,
	Tier,
	MaxItems,
	MaxEnchants,
	...
	if self.Players[PlayerID].info.current_craft == nil then
		local RandomItems = self:GetRandomItemsForPlayerByTier(
			PlayerID,
			NeutralTier,
			NeutralMaxItems,
			ExceptionsItems,
		...
		local RandomEnchants =
			self:GetRandomEnchantmentsForPlayerByTier(PlayerID, NeutralTier, NeutralMaxEnchants, ExceptionsEnchants)
...
function NeutralItems:OnPlayerCraftNeutralItem(event)
	...
	if
		...
		and EnchantName ~= ""
		and EnchantName ~= nil
		and self.Players[PlayerID].info.current_craft ~= nil
	then
			...
			if FreeSlot and Hero then
				...
				if Next ~= nil then
					...
					self:GiveNeutral(
						...
						Next.max_items,
						Next.max_enchants,
						Next.exception_items,
						Next.exception_enchants,
						Next.exclude_prev_tier
					)
				end
			else
				...
				if player then
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"SendHudError",
						{ message = "dota_hud_error_full_slots" }
					)
				end
...
function NeutralItems:OnPlayerWantSwapNeutralItems(event)
	...
	if
		DragItem
		and DropItem
		and DragItem:GetItemSlot() == DOTA_ITEM_NEUTRAL_ACTIVE_SLOT
		and Hero
		and DropItem:GetItemSlot() > 5
		and KeyValues:IsNeutralItem(DropItem:GetName())
	then
...
function NeutralItems:OnPlayerWantMoveItemToStash(event)
	...
	if self.Players[PlayerID].items[iItem] == nil or self.Players[PlayerID].items[iItem].place == "STASH" then
		return
	end
...
function NeutralItems:OnPlayerWantGetItemFromStash(event)
	...
	if iUnit == nil or iItem == nil then
		return
	end
	...
	if hUnit == nil or hItem == nil then
		return
	end

	if self.Players[PlayerID].items[iItem] == nil or self.Players[PlayerID].items[iItem].place == "INVENTORY" then
		return
	end

	if not hUnit:HasInventory() or not hUnit:IsRealHero() then
		return
	end
	...
	if hUnit:GetPlayerOwnerID() ~= PlayerID then
		return
	end
...
function NeutralItems:FixEnchant(PlayerID)
	...
	if ItemInActive then
		...
		elseif
			ItemInPassive ~= nil
			and NeedEnchant ~= nil
			and ItemInPassive:GetName() == NeedEnchant
			and ItemInPassive:GetLevel() ~= NeedLevel
		then
...
function NeutralItems:GetGettedItemsByTierSortedByTime(PlayerID, Tier)
	...
	table.sort(data, function(a, b)
		return a.craft_time > b.craft_time
	end)
...
function NeutralItems:GetFreeSlotForNeutralItem(Unit, PlayerID)
	...
	if Unit == nil or not Unit:HasInventory() then
		return nil
	end
...
if not NeutralItems.bStarted then
	NeutralItems:Init()
end