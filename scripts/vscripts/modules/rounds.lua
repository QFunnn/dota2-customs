...
function Rounds:GetRandomPVPPair(Teams)
	...
	if #cand == 0 then
		...
		for i = 1, n do
			for j = i + 1, n do
				cand[#cand + 1] = { Teams[i], Teams[j] }
			end
		end
	end

	local function duels(t)
		return self.DuelCount[t] or 0
	end
	-- Давность пары: сколько дуэлей назад ЭТА пара встречалась (ещё не встречались -> «бесконечно»).
	local function pairAge(a, b)
		return seq - (self.DuelPairLastSeq[MakeKey(a, b)] or -HUGE)
	end
