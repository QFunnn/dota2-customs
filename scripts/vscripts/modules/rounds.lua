...
function Rounds:Init()
	...
	-- A63(2): состояние дуэльного планировщика v4 (modules/duel_planner.lua).
	-- Очередь-план перебора, счётчики частоты/простоя/серий, сыгранные пары.
	-- Спека и эталон: vault -> Notes/Duel Matchmaking (сверено парным тестом лог-в-лог).
	self.DuelPlanState = DuelPlanner.NewState()
	...
	CustomGameEventManager:RegisterListener("ConfirmBet", function(_, event)
		self:OnPlayerConfirmBet(event)
	end)

	CustomGameEventManager:RegisterListener("duel_notification_closed", function(_, event)
		self:OnDuelNotificationClosed(event)
	end)
	CustomGameEventManager:RegisterListener("rounds_player_ready", function(_, event)
...
function Rounds:OnGameRulesStateChange()
	...
	if State == DOTA_GAMERULES_STATE_PRE_GAME then
		...
		CreateModifierThinker(
			nil,
			nil,
			"modifier_abilities_optimization_thinker",
			{},
			Vector(0, 0, 0),
			DOTA_TEAM_NEUTRALS,
			false
...
function Rounds:SendHintToPlayer(PlayerID, PlayerEnt, HintName, Delay)
	if not PlayerEnt or not PlayerEnt:IsAlive() then
		return
	end

	local PlayerServerInfo = Server:GetPlayerInfo(PlayerID)
	if
		not PlayerServerInfo
		or not PlayerServerInfo.settings
		or PlayerServerInfo.settings.settings_hints_enabled == 0
	then
		return
	end
...
function Rounds:PrepareRound(RoundNum, BonusPrepareTime)
	if self.CurrentState ~= GAME_STATES.NONE then
		return
	end
	...
	for PlayerID, PlayerInfo in pairs(AllActivePlayers) do
		...
		if table.contains(GIVE_NEUTRAL_BOOK_ROUNDS, RoundNum) then
			GameRules:SetItemStockCount(
				GameRules:GetItemStockCount(PlayerTeam, "item_reroll_neutral_book", PlayerID) + 1,
				PlayerTeam,
				"item_reroll_neutral_book",
				PlayerID
			)
		end
		...
		if table.contains(GIVE_SSS_RELEARN_BOOK_ROUNDS, RoundNum) then
			GameRules:SetItemStockCount(
				GameRules:GetItemStockCount(PlayerTeam, "item_relearn_book_sss", PlayerID) + 1,
				PlayerTeam,
				"item_relearn_book_sss",
				PlayerID
			)
		end
	...
	if RoundInfo.has_duel == true and #AllActiveTeams > 1 then
		...
		if NewPVPPair and NewPVPPair[1] ~= nil and NewPVPPair[2] ~= nil then
				...
				for _, PlayerInfo in ipairs(DuelPlayers) do
					for _, PlayerInfoEnemy in ipairs(DuelPlayers) do
						if PlayerInfo.TeamID ~= PlayerInfoEnemy.TeamID then
							local Duration =
								Server:GetPlayerSettingValue(PlayerInfo.PlayerID, "duel_notification_duration")
			...
			local RoundWhenLoseAegis = self:GetRoundWhenLoseAegis()
					...
					if Server:GetPlayerSettingValue(hPlyID, "hide_aegis_notification") ~= 1 then
						...
						if hPlyEnt then
							CustomGameEventManager:Send_ServerToPlayer(
								...
								"set_hint_dota_style",
								{
									text = "#hint_duel_notification_aegis",
									duration = ROUND_PVP_AEGIS_LOST_NOTIFICATION_DURATION - 1,
									image = "aegis",
								}
							)
						end
	...
	self.CurrentRoundInfo.controller =
		RoundController(RoundNum, RoundInfo, self.PVPPairsHistory.currentPair, PrepareTime)
		...
		if PlayerEnt and Server:IsPlayerNotificationsEnabled(PlayerID) then
			if RoundInfo.notification_text ~= nil then
				CustomGameEventManager:Send_ServerToPlayer(
					...
					"set_hint_dota_style",
					{
						text = RoundInfo.notification_text,
						duration = ROUND_HARD_ENEMY_NOTIFICATION_DURATION,
						image = "awarn",
					}
				)
			end
	...
	if RoundInfo.type == ROUND_TYPES.VOTING then
		...
		GameRules:GetGameModeEntity()
			:SetThink("RoundVoting", self, "GAME_ROUND_VOTING", GAME_ROUNDS_TIMINGS["ROUND_TYPE_SELECTING"])
	end
end
...
function Rounds:BeginRound()
	if self.CurrentState ~= GAME_STATES.PREPARING then
		return
	end
	...
	if GAME_ROUNDS_TIMINGS[Arena] then
		...
		if Arena == "MINIGAMES" then
			self.CurrentRoundInfo.max_duration = self.CurrentRoundInfo.max_duration
				+ GAME_ROUNDS_TIMINGS["MINIGAMES_DELAY"]
		end
	end
	...
	if Arena == "SELF" and self.CurrentRoundInfo.info.has_duel == true and self.PVPPairsHistory.currentPair ~= nil then
		local DuelArenaTeam =
			table.random({ self.PVPPairsHistory.currentPair.fTeam, self.PVPPairsHistory.currentPair.sTeam })
	...
	for PlayerID, PlayerInfo in pairs(Players:GetAllPlayers()) do
		...
		if not Rounds:IsFinishedTeam(PlayerInfo.team) then
			...
			if Players:IsActivePlayer(PlayerID) then
				...
				if Map:GetArenaInfo(SelfArena) == nil then
					self:EndRound()
					return
				end
				...
				if
					Arena == "SELF"
					and self:IsPlayerInPVPDuel(PlayerID)
					and table.count(AvailablePositionsDuel) > 0
					and DuelArena
				then
					SelfArena = DuelArena
					...
					local rKey = 1
					if
						PlayerInfo.team == self.PVPPairsHistory.currentPair.sTeam
						and table.count(AvailablePositionsDuel) > 1
					then
						rKey = 2
					end
				...
				if Delay > 0 then
					GameRules:GetGameModeEntity():SetThink(function()
						...
						if self.CurrentRoundInfo.controller then
							self.CurrentRoundInfo.controller:BeginTeamArena(
								PlayerInfo.team,
								self.CurrentRoundInfo.max_duration - Delay
							)
						end
					...
					if self.CurrentRoundInfo.controller then
						self.CurrentRoundInfo.controller:BeginTeamArena(
							PlayerInfo.team,
							self.CurrentRoundInfo.max_duration
						)
					end
				end
			end
	...
	if self.CurrentRoundInfo.info.type == ROUND_TYPES.VOTING and Arena == "MINIGAMES" then
		...
		GameRules:GetGameModeEntity()
			:SetThink("RoundVotingMinigames", self, "GAME_ROUND_VOTING_MINIGAMES", GAME_ROUNDS_TIMINGS["MINIGAMES"])
	end
end
			...
			for _, TeamID in ipairs(TeamsSnapshot) do
				if not self:IsFinishedTeam(TeamID) then
					local CurrentDuration =
						math.max(self.CurrentRoundInfo.current_duration - self:GetTeamStartDelay(TeamID), 0)
...
function Rounds:ForceKillStuckTeam(TeamID)
	...
	if ROUND_DEBUG_ENABLED then
		print(
			"[ROUND_DEBUG] FORCE KILL STUCK TEAM: TeamID="
				.. tostring(TeamID)
				.. " round="
				.. tostring(self.CurrentRound)
				.. " duration="
				.. tostring(self.CurrentRoundInfo.current_duration or "?")
				.. " (>"
				.. BERSERK_FORCE_KILL_DELAY
				.. "s on berserk)"
		)
	end
	...
	for _, PlayerID in ipairs(Players:GetTeamActivePlayers(TeamID, true)) do
		...
		if PlayerInfo and Players:IsValidHero(PlayerInfo.hero) and PlayerInfo.hero:IsAlive() then
			if ROUND_DEBUG_ENABLED then
				print("[ROUND_DEBUG] FORCE KILL: killing hero of PlayerID=" .. tostring(PlayerID))
			end
	...
	if bAnyAlive then
		...
		Timers:CreateTimer(GAME_PLAYER_LOSE_DELAY + 1.5, function()
			if not self:IsFinishedTeam(TeamID) then
				if ROUND_DEBUG_ENABLED then
					print(
						"[ROUND_DEBUG] FORCE KILL: team still in round after AFK delay, force EndTeam TeamID="
							.. tostring(TeamID)
					)
				end
				...
				if ROUND_DEBUG_ENABLED then
					print(
						"[ROUND_DEBUG] FORCE KILL: AFK-flow закрыл раунд штатно для TeamID="
							.. tostring(TeamID)
					)
				end
			end
		...
		if ROUND_DEBUG_ENABLED then
			print("[ROUND_DEBUG] FORCE KILL: no living heroes, EndTeam directly TeamID=" .. tostring(TeamID))
		end
		self:EndTeam(TeamID, true)
	end
end
...
function Rounds:RoundDurationEnded()
	if self.CurrentState ~= GAME_STATES.IN_ACTION or self.CurrentRoundInfo == nil then
		self:EndRound()
		return
	end
...
function Rounds:TeamFinishedMinigames(TeamID, Place)
	if self.CurrentRoundInfo == nil or self.CurrentRoundInfo.max_teams == nil then
		return
	end
...
function Rounds:EndRound(bStartNextRound)
	...
	if self.CurrentRoundInfo ~= nil and self.CurrentRoundInfo.controller ~= nil then
		...
		if
			bStartNextRound
			and self.TeamsDefDurations[self.CurrentRound]
			and table.count(self.TeamsDefDurations[self.CurrentRound]) > 0
			and self:IsQueueRound()
		then
			local List = table.keys(self.TeamsDefDurations[self.CurrentRound])

			table.sort(List, function(a, b)
				return self.TeamsDefDurations[self.CurrentRound][a] < self.TeamsDefDurations[self.CurrentRound][b]
			end)
	...
	if #TeamsCopy > 0 then
		...
		for _, Value in ipairs(Values) do
			...
			elseif Value ~= "random" then
				table.sort(TeamsCopy, function(a, b)
	...
	for _, TeamInfo in ipairs(teams) do
		for PlayerID, PlayerInfo in pairs(Players:GetTeamActivePlayers(TeamInfo.team)) do
			...
			if Players:IsValidHero(hHero) then
				...
				if
					PlayerServerInfo
					and PlayerServerInfo.settings
					and PlayerServerInfo.settings.settings_hints_enabled ~= 0
				then
					local Player = PlayerResource:GetPlayer(PlayerID)
					if Player then
...
function Rounds:GetRandomPVPPair(Teams)
	...
	-- A63(2): планировщик v4 — порядок перебора строится наперёд (modules/duel_planner.lua),
	-- жёсткие правила: уникальность пар в переборе; при 5+ живых никаких дуэлей подряд;
	-- серия 3 запрещена при 3+ живых; коридор ритма; при 2 живых правила отключены (добивка).
	-- Состояние коммитится при выборе пары: ливер на выбранной дуэли = победа сопернику,
	-- дуэль засчитана (модель владельца). Сверено парным тестом с JS-эталоном (0 расхождений).
	self.DuelPlanState = self.DuelPlanState or DuelPlanner.NewState()
	local stream = self.RandomStream
	local rng
	if stream and stream.RandomFloat then
		rng = function()
			return stream:RandomFloat(0, 1)
		end
	else
		rng = function()
			return math.random()
	...
	local pair = DuelPlanner.PickPair(self.DuelPlanState, Teams, rng)
	if not pair then
		return
	end
...
function Rounds:ShuffleAndGetRandomPVPPair()
	...
	table.sort(self.PVPPairs, function(a, b)
		return a.Joins < b.Joins
	end)
	...
	if LastPair == nil then
		...
		if PreLastPair ~= nil then
			for _, PairInfo in ipairs(self.PVPPairs) do
				...
				if
					not fTeamLastJoined
					and not sTeamLastJoined
					and not fTeamPreLastJoined
					and not sTeamPreLastJoined
				then
					ResultPairKey = _
					break
				end
...
function Rounds:EndPVPPair(LoserTeamID, bFast)
	if self.PVPPairsHistory.currentPair == nil or self:IsPvpPairEnded() then
		return
	end
	...
	do
		...
		-- alive = живых команд на момент дуэли: для честной пофазной сверки метрик подбора
		-- (число живых по last_wave на хвосте матча врёт — находка консилиума 2026-07-27)
		table.insert(
			self.DuelLog,
			{
				round = CurrentRound,
				...
				b = sSteam,
				winner = winnerSteam,
				alive = #Players:GetAllActiveTeams(true),
			}
		)
	end
				...
				hHero:SetContextThink(DoUniqueString("HeroPVPEndedDelay"), function()
					if Players:IsValidHero(hHero) then
						...
						if GameRules.xpTable[self.CurrentRound + 1] and GameRules.xpTable[self.CurrentRound] then
							local nExp = math.floor(
								(GameRules.xpTable[self.CurrentRound + 1] - GameRules.xpTable[self.CurrentRound]) * 0.7
							)
						...
						for _, modifier in pairs(hHero:FindAllModifiers()) do
							if
								modifier
								and modifier:GetCaster() ~= hHero
								and modifier:IsDebuff()
								and not ignores_mods[modifier:GetName()]
							then
								modifier:Destroy()
							end
						end
				...
				if BetInfo.team == WinnerTeamID then
					if BetInfo.ratio ~= nil then
						...
						if BetInfo.bet_type and BetInfo.bet_type == "free" then
							...
							local fx1 = ParticleManager:CreateParticle(
						...
						self:BetRecordInHistory(
							PlayerID,
							ResultGold - BetInfo.value,
							WinnerTeamID,
							LoserTeamID,
							BetInfo.team,
							fTeam,
...
function Rounds:OnPlayerConfirmBet(event)
	if self.CurrentRoundInfo == nil or self.CurrentRoundInfo.bets == nil or self.CurrentRoundInfo.bet_info == nil then
		return
	end
	...
	if
		PlayerID == nil
		or BetSum == nil
		or type(BetSum) ~= "number"
		or self.CurrentState ~= GAME_STATES.PREPARING
		or not Players:IsActivePlayer(PlayerID)
		or BetSum <= 0
		or self.CurrentRoundInfo.bets[PlayerID] ~= nil
	then
		return
	end
	...
	if
		not PlayerInfo
		or PlayerInfo.team == BetTeamID
		or self.CurrentRoundInfo.bet_info.fTeamID == PlayerInfo.team
		or self.CurrentRoundInfo.bet_info.sTeamID == PlayerInfo.team
	then
		return
	end
...
function Rounds:CalculateBets()
	if
		self.PVPPairsHistory.currentPair == nil
		or self.CurrentRoundInfo == nil
		or self.CurrentRoundInfo.bets == nil
		or self.CurrentState ~= GAME_STATES.PREPARING
	then
		return
	end
...
function Rounds:AddExtraCreep(PlayerID, UnitName)
	...
	if UnitCountInTeam >= 2 then
		CustomGameEventManager:Send_ServerToPlayer(
			...
			{ message = "#cha_data_error_most_have_creep" }
		)
		return false
	end
...
function Rounds:KillAllExtraCreepsByPlayerID(PlayerID)
	...
	if
		not Players:IsActivePlayer(PlayerID)
		and PlayerInfo
		and PlayerInfo.player_die_round ~= nil
		and self.CurrentRoundInfo
		and self.CurrentRoundInfo.controller
	then
...
function Rounds:IsThisExtraCreepCanBeLive(PlayerID, Round)
	...
	if
		not Players:IsActivePlayer(PlayerID)
		and PlayerInfo.player_die_round ~= nil
		and math.abs(PlayerInfo.player_die_round - Round) <= 3
	then
		return false
	end
...
function Rounds:OnPlayerReady(event)
	...
	if not Players:IsActivePlayer(PlayerID) then
		return
	end

	if self.PrepareReadyList[PlayerID] ~= nil and not IsCheatsEnabled() then
		return
	end
	...
	if AllReady then
		...
		if self.AlreadyAllPrepared == false then
			...
			if
				self.CurrentRoundInfo.info.type == ROUND_TYPES.VOTING
				and self.CurrentRoundInfo.game_mode_selected == false
			then
				GameRules:GetGameModeEntity():StopThink("GAME_ROUND_VOTING")
				Rounds:RoundVoting()
			end
	...
	elseif IsCheatsEnabled() then
		...
		GameRules:GetGameModeEntity():StopThink("GAME_ROUND_BEGIN_TIMER")
		GameRules:GetGameModeEntity()
			:SetThink("BeginRound", self, "GAME_ROUND_BEGIN_TIMER", self.CurrentRoundInfo.prepare_time)
	end
...
-- Волна, с которой проигравший дуэль теряет аегис: значение из настроек (деф. 70).
-- Когда живых игроков остаётся <=3, порог подменяется на волну, где это случилось
-- (Players:MakeTeamLose выставляет AegisRoundOverride) — дальше дуэли сразу на аегисы,
-- чтобы не затягивать игру. Берём минимум: если порог из настроек уже пройден, он и остаётся.
function Rounds:GetRoundWhenLoseAegis()
	local RoundWhenLoseAegis = GetGameSetting("ROUND_WHEN_LOSE_AEGIS")
	if self.AegisRoundOverride ~= nil and self.AegisRoundOverride < RoundWhenLoseAegis then
		RoundWhenLoseAegis = self.AegisRoundOverride
	end
	return RoundWhenLoseAegis
end
...
function Rounds:UpdateDynamicSettings()
	...
	-- A8 (реворк): аегисы теряются с волны из настроек (деф. 70) — штраф −5 за
	-- выбывших/недостающих игроков УБРАН; неполные лобби стартуют с той же волной.
	-- Исключение: когда живых осталось <=3, порог падает до текущей волны (GetRoundWhenLoseAegis).
	local RoundWhenLoseAegis = self:GetRoundWhenLoseAegis()
	...
	DynamicSettingsTable["magical_resist"] = GetGameSetting("CREEP_BONUS_MAGIC_RESISTANCE_PER_LATE_ROUND")
		* math.max(0, self:GetCurrentRound() - 50)
	DynamicSettingsTable["physical_resist"] = GetGameSetting("CREEP_BONUS_ARMOR_PER_LATE_ROUND")
		* math.max(0, self:GetCurrentRound() - 50)
...
function Rounds:IsPlayerInPVPDuel(PlayerID)
	if self.PVPPairsHistory.currentPair ~= nil then
		...
		if
			PlayerInfo
			and (
				PlayerInfo.team == self.PVPPairsHistory.currentPair.fTeam
				or PlayerInfo.team == self.PVPPairsHistory.currentPair.sTeam
			)
		then
			return true
		end
...
function Rounds:AddSpawnGroupToRound(spawngroup, unit_name, unit)
	if spawngroup == nil or spawngroup == 0 then
		return
	end
	...
	if existing then
		if existing.precache_unit_name == nil then
			existing.precache_unit_name = unit_name
		end
		if existing.unit == nil then
			existing.unit = unit
		end
		return
	end
...
function Rounds:GetRoundTeams()
	if self.CurrentRoundInfo == nil or self.CurrentRoundInfo.teams == nil then
		return {}
	end
...
function Rounds:IsFinishedTeam(TeamID)
	if self.CurrentRoundInfo == nil or self.CurrentRoundInfo.teams == nil or #self.CurrentRoundInfo.teams == 0 then
		return true
	end
...
function Rounds:RoundTypeIs(TYPE)
	if self.CurrentRoundInfo == nil or self.CurrentRoundInfo.info == nil or self.CurrentRoundInfo.info.type == nil then
		return false
	end
...
function Rounds:GetTopTeamsByAvgDuration(Teams)
	...
	table.sort(Top, function(a, b)
		return (averages[a] or 99999) < (averages[b] or 99999)
	end)
...
function Rounds:GetTeamDefDurationByRound(TeamID, RoundNum)
	if
		self.TeamsDefDurations ~= nil
		and self.TeamsDefDurations[RoundNum] ~= nil
		and self.TeamsDefDurations[RoundNum][TeamID] ~= nil
	then
...
function Rounds:GetTeamStartDelay(TeamID)
	if
		self.CurrentRoundInfo == nil
		or self.CurrentRoundInfo.teams_delay == nil
		or self.CurrentRoundInfo.teams_delay[TeamID] == nil
		or not self:IsQueueRound()
	then
		return 0
	end
...
function Rounds:IsQueueRound(RoundNum)
	...
	return GAME_QUEUE_ROUNDS_ENABLED
		and RoundNum >= GetGameSetting("ROUND_WHEN_QUEUE_START")
		and not self:IsBossRound(RoundNum)
end

if not Rounds.bStarted then
	Rounds:Init()
end