...
function Votes:Init()
	...
	CustomGameEventManager:RegisterListener("votes_player_voted", function(source, event)
		self:OnPlayerVoted(event)
	end)
...
function Votes:CreateVote(VoteName, Duration, Exceptions)
	if not VOTES_LIST[VoteName] then
		return
	end
...
function Votes:EndVote(VoteName, bNotShowResults, bUseRating)
	if not self.VotesProgress[VoteName] then
		return 0, {}, 0
	end
...
function Votes:SetupDefaultPlayerVotes(PlayerID)
	...
	for VoteName, VoteInfo in pairs(self.VotesProgress) do
		if
			VoteInfo.default_option ~= nil
			and VoteInfo.force_default_option == true
			and self.VotesProgress[VoteName].player_votes[PlayerID] == nil
		then
...
function Votes:SetupDefaultPlayersVoteTo(VoteName)
	...
	if self.VotesProgress[VoteName].default_option == nil or not self.VotesProgress[VoteName].force_default_option then
		return
	end

	if table.contains(self.VotesProgress[VoteName].exceptions, self.VotesProgress[VoteName].default_option) then
		return
	end
...
function Votes:OnPlayerVoted(event)
	...
	if not table.contains(VOTES_LIST[VoteName].options, Option) then
		return
	end

	if table.contains(self.VotesProgress[VoteName].exceptions, Option) then
		return
	end

	if
		self.VotesProgress[VoteName].player_votes[PlayerID]
		and self.VotesProgress[VoteName].player_votes[PlayerID].can_vote == false
	then
		return
	end
