...
function HeroBuilder:Init()
	...
	CustomGameEventManager:RegisterListener("builder_reroll_heroes", function(_, event)
		self:OnPlayerWantRerollHeroes(event)
	end)
	CustomGameEventManager:RegisterListener("builder_random_hero", function(_, event)
		self:OnPlayerWantRandomHero(event)
	end)
	CustomGameEventManager:RegisterListener("builder_select_hero", function(_, event)
		self:OnPlayerWantSelectHero(event)
	end)
	CustomGameEventManager:RegisterListener("builder_lock_hero", function(_, event)
		self:OnPlayerWantLockHero(event)
	end)
	CustomGameEventManager:RegisterListener("AbilitySelected", function(_, event)
		self:AbilitySelected(event)
	end)
	CustomGameEventManager:RegisterListener("builder_relearn_book_selected", function(_, event)
		self:RelearnBookAbilitySelected(event)
	end)
	CustomGameEventManager:RegisterListener("builder_omniscient_selected", function(_, event)
		self:OnPlayerOmniscientSelectedAbility(event)
	end)
	CustomGameEventManager:RegisterListener("builder_omniscient_closed", function(_, event)
		self:OnPlayerOmniscientClosed(event)
	end)
	CustomGameEventManager:RegisterListener("SwapAbility", function(_, event)
		self:SwapAbility(event)
	end)
	CustomGameEventManager:RegisterListener("RefreshHeroBindings", function(userID, event)
		self:OnRefreshHeroBindings(userID, event)
	end)
	CustomGameEventManager:RegisterListener("ReorderComplete", function(_, event)
		self:ReorderComplete(event)
	end)

	CustomGameEventManager:RegisterListener("builder_skill_selected", function(_, event)
		self:OnPlayerSelectedSkill(event)
	end)

	CustomGameEventManager:RegisterListener("ability_selection_ability_selected", function(_, event)
		self:OnPlayerAbilitySelected(event)
	end)
...
function HeroBuilder:UpdateMinigamesWins(PlayerID, value)
	...
	local UpdateWins = function()
		if self.Players[PlayerID].minigames_wins > 0 then
			local Modif = Hero:FindModifierByName("modifier_minigames_win_buff")
				or Hero:AddNewModifier(Hero, nil, "modifier_minigames_win_buff", {})
			if Modif then
	...
	for PlayerID, PlayerInfo in pairs(self.Players) do
		if self.Players[PlayerID].selected_hero == "" then
			...
			if engineHero and engineHero ~= "" then
				...
				print(
					string.format(
						"[HeroBuilder/Disc] SelectRandomHeroesForPlayers(PID=%d): sync HeroBuilder.selected_hero <- engineHero='%s'",
						PlayerID,
						engineHero
					)
				)
...
function HeroBuilder:IncrementBooks(PlayerID)
	...
	CustomNetTables:SetTableValue(
		"players",
		"player_" .. PlayerID .. "_collected_data",
		{ books = self.Players[PlayerID].books, pages = self.Players[PlayerID].pages }
	)
end
...
function HeroBuilder:IncrementAttributeBook(PlayerID, BookName)
	if self.Players[PlayerID] == nil or self.Players[PlayerID][BookName] == nil then
		return
	end
...
function HeroBuilder:OnPlayerWantSelectHero(event)
	...
	if HeroName == nil or self.Players[PlayerID] == nil or self.Players[PlayerID].selected_hero ~= "" then
		return
	end
...
function HeroBuilder:OnPlayerWantRandomHero(event)
	local PlayerID = event.PlayerID
	if self.Players[PlayerID] == nil or self.Players[PlayerID].selected_hero ~= "" then
		return
	end
...
function HeroBuilder:PreSelectHeroForDisconnected(PlayerID)
	...
	if self.Players[PlayerID].selected_hero ~= nil and self.Players[PlayerID].selected_hero ~= "" then
		DiscLog(
			"PreSelectHero(PID=%d): selected_hero уже '%s', skip",
			PlayerID,
			tostring(self.Players[PlayerID].selected_hero)
		)
		return
	end
...
function HeroBuilder:ForceCreateHeroForDisconnectedPlayer(PlayerID)
	...
	DiscLog(
		"ForceCreate(PID=%d): heroName=%s, hPlayer=%s, conn=%s",
		PlayerID,
		heroName,
		tostring(hPlayer),
		ConnName(PlayerID)
	...
	if hPlayer and not hPlayer:IsNull() then
		...
		DiscLog(
			"ForceCreate(PID=%d): hPlayer=nil (CDOTAPlayer entity уже уничтожен engine'ом) -- спавн отложен до реконнекта (engine спавнит сам) или MarkAbandoned через 60с",
			PlayerID
		)
	end
end
...
function HeroBuilder:HandleReconnectMissingHero(PlayerID)
	...
	DiscLog(
		"Reconnect(PID=%d): conn=%s, selected_hero=%s, gameState=%d",
		PlayerID,
		ConnName(PlayerID),
		tostring(heroName),
		GameRules:State_Get()
	...
	Timers:CreateTimer(1.0, function()
		...
		if stillNoHero then
			DiscLog(
				"Reconnect(PID=%d): через 1с hero всё ещё нет, повторный ForceCreate",
				PlayerID
			)
			self:ForceCreateHeroForDisconnectedPlayer(PlayerID)
		else
			DiscLog(
				"Reconnect(PID=%d): через 1с hero есть (%s) -- зову Reassign для control",
				PlayerID,
				src
			)
...
function HeroBuilder:HandleDisconnectedPicksAtFinish()
	...
	for PlayerID, hbInfo in pairs(self.Players) do
		...
		DiscLog(
			"FinishPicks(PID=%d): state=%s conn=%s hero=%s -> isDisc=%s hasHero=%s",
			PlayerID,
			tostring(stateName),
			ConnName(PlayerID),
			tostring(hbInfo.selected_hero),
			tostring(isDisc),
			tostring(hasHero)
		)
		...
		if isDisc and hasHero then
			...
			if hPlayer and not hPlayer:IsNull() then
				...
				DiscLog(
					"FinishPicks(PID=%d): SetSelectedHero(%s) -> ok=%s err=%s",
					PlayerID,
					hbInfo.selected_hero,
					tostring(ok),
					tostring(err)
				)
				...
				DiscLog(
					"FinishPicks(PID=%d): hPlayer мёртв -- engine уже знает selected hero, спавн пройдёт сам",
					PlayerID
				)
				skipped = skipped + 1
			end
...
function HeroBuilder:MarkPlayerAbandoned(PlayerID)
	...
	DiscLog(
		"MarkAbandoned(PID=%d): settled=true (hero оставлен живым, state остаётся LEAVED)",
		PlayerID
	)
...
function HeroBuilder:ReassignHeroControlOnReconnect(PlayerID)
	...
	if not hHero or hHero:IsNull() then
		DiscLog(
			"Reassign(PID=%d): hero нигде не найден -- нечего перехватывать",
			PlayerID
		)
		return
	end
	...
	DiscLog(
		"Reassign(PID=%d): full binding restored (hero=%s, source=%s) [SetPlayerID+Owner+AssignedHeroEntity, БЕЗ SetSelectedHero]",
		PlayerID,
		hHero:GetUnitName(),
		source
	)
...
function HeroBuilder:InitPlayerHero(hHero)
	...
	if not self.Players[PlayerID] then
		print(
			string.format(
				"[HeroBuilder/Disc] InitPlayerHero(PID=%d): нет HeroBuilder.Players entry, ВЫХОД",
				PlayerID
			)
		)
		return
	...
	print(string.format("[HeroBuilder/Disc] InitPlayerHero(PID=%d, hero=%s) START", PlayerID, hHero:GetUnitName()))
...
function HeroBuilder:AddAbilityFacetsAndInnate(hero, ability_name)
	...
	if new_ability and (not new_ability:IsNull()) then
		...
		if
			(
				not new_ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_PASSIVE)
				and not new_ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_HIDDEN)
				and new_ability:GetBehaviorInt() > 0
			) or valve_broken_abilities[ability_name]
		then
		...
		if hero and KeyValues:IsHeroInSSSTier(hero) then
			...
			if sss_count > 1 then
				...
				if not HeroInfo then
					break
				end -- защита от мусора, не должна срабатывать
...
function HeroBuilder:GenerateListRandomHeroes(PlayerID, LockedHero, bReserveUnique)
	...
	if bReserveUnique and not self.FreeUsedHeroes then
		self.FreeUsedHeroes = {}
	end
...
function HeroBuilder:OnPlayerWantRerollHeroes(event)
	...
	if
		not self.Players[PlayerID]
		or self.Players[PlayerID].selected_hero ~= ""
		or self.Players[PlayerID].reroll_count <= 0
	then
		return
	end
	...
	if self.Players[PlayerID].reroll_count_free <= 0 or LockedHero ~= nil then
		...
		if PlayerInfo.profile.coins < Price then
			return
		end
...
function HeroBuilder:SettleCurrentHero(PlayerID)
	if not self.Players[PlayerID] then
		print(
			string.format(
				"[HeroBuilder/Disc] SettleCurrentHero(PID=%d): нет HeroBuilder.Players entry, ВЫХОД",
				PlayerID
			)
		)
		return
	...
	print(string.format("[HeroBuilder/Disc] SettleCurrentHero(PID=%d) -- ставлю settled=true", PlayerID))
	...
	if not hHero or hHero:IsNull() then
		...
		if pInfo and pInfo.hero and not pInfo.hero:IsNull() then
			hHero = pInfo.hero
			DiscLog(
				"SettleCurrentHero(PID=%d): GetSelectedHeroEntity nil -> fallback на Players.Players[PID].hero",
				PlayerID
			)
		end
	end
	if not hHero or hHero:IsNull() then
		DiscLog(
			"SettleCurrentHero(PID=%d): hero не найден нигде, форсирую settled=true чтобы не блокировать AllSettled",
			PlayerID
		)
		self.Players[PlayerID].settled = true
	...
	if self.Players[PlayerID].selected_hero ~= actualName then
		DiscLog(
			"SettleCurrentHero(PID=%d): sync selected_hero '%s' -> '%s' (фактический unit)",
			PlayerID,
			tostring(self.Players[PlayerID].selected_hero),
			actualName
		)
...
function HeroBuilder:OnPlayerSettled(SettledPlayerID)
	...
	DiscLog("OnPlayerSettled@ENTRY: triggered_by=%s, gameTime=%.2f", tostring(SettledPlayerID), GameRules:GetGameTime())
	...
	print(
		string.format(
			"[HeroBuilder/Disc] OnPlayerSettled(triggered_by=%s): AllSettled=%s, unsettled=[%s], idealAbilitiesList_count=%d",
			tostring(SettledPlayerID),
			tostring(AllSettled),
	...
	if AllSettled and (self.IdealAbilitiesList == nil or #self.IdealAbilitiesList == 0) then
		...
		DiscLog(
			"OnPlayerSettled: AllSettled HIT! ability selection стартует через 0.5с (gameTime=%.2f)",
		...
		Timers:CreateTimer(0.5, function()
			for PlayerID, PlayerInfo in pairs(HeroBuilder:GetAllPlayers()) do
				...
				for i = 1, MaxCount do
					local type = PlayerInfo.SSS_ability_select_count > 0 and ABILITY_SELECTION_TYPE.BASIC_SSS
						or ABILITY_SELECTION_TYPE.BASIC
...
function HeroBuilder:ShowRandomSkillSelectionFree(PlayerID)
	...
	if
		CurrentRound == 5
		and self.Players[PlayerID].skills_points == 1
		and PlayerInfo
		and PlayerInfo.settings
		and PlayerInfo.settings.settings_hints_enabled ~= 0
	then
...
function HeroBuilder:ShowRandomSkillSelection(PlayerID)
	...
	if self.Players[PlayerID].skills_current_selecting ~= nil or self.Players[PlayerID].skills_points <= 0 then
		return
	end
	...
	local ExcludeSet = {}
	for _, s in ipairs(LastOffered) do
		ExcludeSet[s] = true
	end

	local Candidates = {}
	for _, s in ipairs(AvailableSkills) do
		if not ExcludeSet[s] then
	...
	PlayerTables:SetTableValue(
		"player_" .. PlayerID,
		"skills_selecting",
		self.Players[PlayerID].skills_current_selecting
	)
end
...
function HeroBuilder:UpdateSkillsNetTableInfo()
	...
	for _, PlayerID in ipairs(Players:GetAllPlayers(true)) do
		if self.Players[PlayerID] and self.Players[PlayerID].selected_hero_ent ~= nil then
			self.Players[PlayerID].selected_hero_ent:AddNewModifier(
				self.Players[PlayerID].selected_hero_ent,
				nil,
...
-- [A80] Приём выбора навыка. Раньше здесь не проверялось НИЧЕГО: клиент шлёт
-- builder_skill_selected на каждый клик (skills.js:56), а окно закрывается только после
-- ответа сервера — на большом пинге (через VPN) успевали десятки кликов, и все они
-- применялись. Отсюда 132 взятия одного навыка при положенных 9.
-- Проверки, которые раньше стояли ТОЛЬКО на показе набора (:1352), теперь дублируются
-- здесь, на приёме: сервер не верит клиенту ни в очках, ни в составе набора, ни в пределе.
function HeroBuilder:OnPlayerSelectedSkill(event)
	local PlayerID = event.PlayerID
	local SkillName = event.skill_name

	if SKILLS_LIST_TABLE[SkillName] == nil or not self.Players[PlayerID] then
		return
	end

	local PlayerData = self.Players[PlayerID]

	-- 1. Есть ли за что брать. Первая же проверка убивает клик-спам: после первого выбора
	--    очков 0, все следующие клики из той же пачки отваливаются здесь.
	if (PlayerData.skills_points or 0) <= 0 then
		return
	end

	-- 2. Набор реально показан игроку (после выбора он обнуляется на :1433).
	local Offered = PlayerData.skills_current_selecting
	if type(Offered) ~= "table" then
		return
	end

	-- 3. Навык именно из этого набора — иначе можно было бы прислать любое имя из таблицы.
	local bOffered = false
	for _, sName in pairs(Offered) do
		if sName == SkillName then
			bOffered = true
			break
		end
	end
	if not bOffered then
		return
	end

	-- 4. Предел взятий (constants/skills.lua: 4 у total_block/universal_evasion, 8 у прочих).
	--    Тот же расчёт, что при наборе пула на показе (:1358-1362).
	local SkillInfo = SKILLS_LIST_TABLE[SkillName]
	local MaxTakes = SkillInfo.max or 8
	if (PlayerData.skills_list[SkillName] or 0) >= MaxTakes then
		return
	end

	local Hero = PlayerData.selected_hero_ent
...
function HeroBuilder:ShowRandomAbilitiesSelection(PlayerID, Type, State, sDeletedAbility, sUsedItemName)
	...
	if Type == ABILITY_SELECTION_TYPE.FAST_RELEARN and State == 0 then
		...
		for _, AbilityName in pairs(AllPlayerAbilities) do
			...
			if
				(ABILITIES_SETTINGS[AbilityName] and ABILITIES_SETTINGS[AbilityName].bUnremovable)
				or (ability and ability.IsHiddenAbility and ability:IsHiddenAbility())
				or (ability and ability.rubick_spell)
				or stolen_abilities[AbilityName]
				or (ability == nil)
			then
	...
	if Type ~= ABILITY_SELECTION_TYPE.BASIC and Type ~= ABILITY_SELECTION_TYPE.BASIC_SSS and State == 0 then
		...
		if Type == ABILITY_SELECTION_TYPE.BASIC_SSS then
			...
			Abilities, sss_count = self:GenerateAbilitiesListForRandom(
				PlayerID,
				sDeletedAbility,
				Type == ABILITY_SELECTION_TYPE.RELEARN_RETURN,
				Type == ABILITY_SELECTION_TYPE.RELEARN_SSS
			)
		end
...
function HeroBuilder:ReturnUsedItemInSelection(PlayerID)
	if not self.Players[PlayerID] or not self:IsPlayerSelectAbilities(PlayerID) then
		return
	end

	local SelectionInfo = self.Players[PlayerID].abilities_selection_current
	if SelectionInfo.sUsedItemName == nil then
		return
	end
...
function HeroBuilder:OnPlayerAbilitySelected(event)
	...
	local SelectionInfo = self.Players[PlayerID].abilities_selection_current
	local bReplaceAbility = SelectionInfo.type ~= ABILITY_SELECTION_TYPE.BASIC
		and SelectionInfo.type ~= ABILITY_SELECTION_TYPE.BASIC_SSS
		and SelectionInfo.type_state == 0

	SelectionInfo.state = "HIDE"

	PlayerTables:SetTableValue("player_" .. PlayerID, "ability_selection", SelectionInfo)

	if (bReplaceAbility and SelectionInfo.replace_selected == true) or SelectionInfo.select_selected == true then
		return
	end
	...
	if bReplaceAbility then
		...
		if SelectionInfo.type == ABILITY_SELECTION_TYPE.DEV then
			...
			Timers:CreateTimer(0.25, function()
				self:ShowRandomAbilitiesSelection(
					PlayerID,
					SelectionInfo.type,
					1,
					AbilityName,
					SelectionInfo.sUsedItemName
				)
			end)
		end
		...
		if
			CurrentRound == 9
			and #self.Players[PlayerID].abilities_selection_schedule > 0
			and PlayerInfo
			and PlayerInfo.settings
			and PlayerInfo.settings.settings_hints_enabled ~= 0
		then
		...
		if
			table.contains(FlyAbilities, AbilityName)
			and PlayerInfo
			and PlayerInfo.settings
			and PlayerInfo.settings.settings_hints_enabled ~= 0
		then
		...
		if
			(
				sType == ABILITY_SELECTION_TYPE.RELEARN_SSS
				or sType == ABILITY_SELECTION_TYPE.RELEARN_RETURN
				or sType == ABILITY_SELECTION_TYPE.BASIC
			) and NotificationItem ~= nil
		then
		...
		if
			sType == ABILITY_SELECTION_TYPE.RELEARN_SSS
			or sType == ABILITY_SELECTION_TYPE.RELEARN_RETURN
			or sType == ABILITY_SELECTION_TYPE.RELEARN
		then
...
function HeroBuilder:ShowRandomAbilitySelectionFree(PlayerID, AbilityToDelete, bIsRelearnBook, bIsMainSSS, nParagonType)
	...
	if bIsMainSSS and self.Players[PlayerID].SSS_ability_select_count <= 0 then
		return false
	end
...
function HeroBuilder:ShowRandomAbilitySelection(PlayerID, AbilityToDelete, bIsRelearnBook, bIsMainSSS, nParagonType)
	if
		not self.Players[PlayerID]
		or (not bIsRelearnBook and not bIsMainSSS and not self:IsPlayerCanSelectAbilities(PlayerID))
		or not self.Players[PlayerID].selected_hero_ent
	then
		return
	end
	...
	if bIsMainSSS then
		if GetGameSetting("FIRST_ABILITY_IS_GENERAL") == 1 then
			...
			RandomAbilities =
				self:GenerateAbilitiesListForRandom(PlayerID, AbilityToDelete, bIsRelearnReturnBook, true, true)
		end
	else
		RandomAbilities =
			self:GenerateAbilitiesListForRandom(PlayerID, AbilityToDelete, bIsRelearnReturnBook, bIsRelearnSSSBook)
	end
	...
	for _, randomAbilityName in pairs(self.Players[PlayerID].random_abilities) do
		...
		if
			KeyValues.AbilitiesList[randomAbilityName] and KeyValues.AbilitiesList[randomAbilityName].LinkedAbilities
		then
	...
	if
		...
		or self.Players[PlayerID].selecting_ability_instance == nil
	then
		self.Players[PlayerID].selecting_ability_instance = "main"
	end

	if
		self.Players[PlayerID].selecting_ability_instance == "relearn_return"
		or self.Players[PlayerID].selecting_ability_instance == "relearn_sss"
		or self.Players[PlayerID].selecting_ability_instance == "relearn"
	then
		self:IncrementBooks(PlayerID)
	end
...
function HeroBuilder:AbilitySelected(event)
	...
	-- [A80/SEC] Отправитель берётся из event.PlayerID — это поле инжектит движок.
	-- Раньше здесь стоял event.player_id, который клиент присылает САМ
	-- (ability_select.js:467,486 шлют Players.GetLocalPlayer()), то есть достаточно было
	-- подменить число в событии, чтобы выбрать способность за другого игрока.
	-- Клиентское поле оставлено только как запасной вариант на случай, если движок
	-- почему-то не проставил своё (иначе окно выбора сломается у всех).
	local PlayerID = event.PlayerID
	if PlayerID == nil then
		PlayerID = event.player_id
	end

	if not self.Players[PlayerID] or not self:IsPlayerSelectingAbility(PlayerID) then
		return
	end
	...
	if
		AbilityName == nil
		and (Instance == "relearn" or Instance == "relearn_return" or Instance == "relearn_sss")
		and self.Players[PlayerID].selecting_ability_deleted_index == nil
	then
		local Hero = self.Players[PlayerID].selected_hero_ent
	...
	if not table.contains(self.Players[PlayerID].random_abilities, AbilityName) then
		return
	end
	...
	if
		CurrentRound == 9
		and self.Players[PlayerID].ability_select_count > 0
		and PlayerInfo
		and PlayerInfo.settings
		and PlayerInfo.settings.settings_hints_enabled ~= 0
	then
	...
	if
		(Instance == "relearn_sss" or Instance == "relearn_return" or Instance == "paragon" or Instance == "paragon_2")
...
function HeroBuilder:ShowEngineLinkedAbilities(hHero, abilitiesBefore, sAddedAbilityName)
	...
	for i = 0, hHero:GetAbilityCount() - 1 do
		...
		if hAbility and not hAbility:IsNull() then
			...
			if
				not abilitiesBefore[abilityName]
				and abilityName ~= sAddedAbilityName
...
function HeroBuilder:ShowOmniscientSelection(PlayerID)
	...
	if not self:IsPlayerSelectAbilities(PlayerID) then
		return
	end
...
function HeroBuilder:OnPlayerOmniscientSelectedAbility(event)
	...
	if not self.Players[PlayerID].omniscient_open then
		return
	end
	if not self:IsPlayerSelectAbilities(PlayerID) then
		return
...
function HeroBuilder:IsNonSpellAbility(hAbility)
	...
	if sName and string.find(sName, "special_bonus") then
		return true
	end
...
function HeroBuilder:FixAttackCapability()
	for PlayerID, PlayerInfo in pairs(self.Players) do
		if
			PlayerInfo.selected_hero_ent ~= nil
			and PlayerInfo.attack_capability_changed
			and not self:HasAttackCapabilityModifiers(PlayerInfo.selected_hero_ent)
		then
...
function HeroBuilder:ProcessScepterOwners()
	for PlayerID, PlayerInfo in pairs(self.Players) do
		if
			PlayerInfo.has_scepter
			and PlayerInfo.selected_hero_ent
			and not PlayerInfo.selected_hero_ent:HasScepter()
		then
...
function HeroBuilder:RegisterScepterOwner(hHero)
	if not hHero or not hHero:IsMainHero() or not hHero:IsRealHero() then
		return
	end
	local PlayedID = hHero:GetPlayerID()

	if not self.Players[PlayedID] then
		return
	end
			...
			for i, sAbilityName in ipairs(HEROES_SETTINGS[hHero:GetUnitName()].ScepterAbilities) do
				...
				if not hAbility then
					...
					if hScepterAbility and (not hScepterAbility:IsNull()) then
						...
						if i == 1 then
							Timers:CreateTimer(0.1, function()
								if not hScepterAbility or hScepterAbility:IsNull() then
									return
								end
...
function HeroBuilder:AddScepterShardAbility(nHeroIndex)
	...
	if
		...
		and hHero:GetUnitName()
		and HEROES_SETTINGS[hHero:GetUnitName()]
		and HEROES_SETTINGS[hHero:GetUnitName()].ShardAbilities
	then
		for i, sAbilityName in ipairs(HEROES_SETTINGS[hHero:GetUnitName()].ShardAbilities) do
			...
			if not hAbility then
				...
				if hScepterShardAbility then
					...
					if i == 1 then
						Timers:CreateTimer(0.1, function()
							if not hScepterShardAbility or hScepterShardAbility:IsNull() then
								return
							end
		...
		if hHero:HasAbility("sandking_epicenter") then
			if not hHero:HasModifier("modifier_sand_king_shard") then
				hHero:AddNewModifier(
					hHero,
					hHero:FindAbilityByName("sandking_epicenter"),
...
function HeroBuilder:RemoveScepterLinkAbilities(hHero, sRawAbilityName)
	if
		...
		and hHero:IsRealHero()
		and hHero:GetUnitName()
		and ABILITIES_SETTINGS[sRawAbilityName]
		and ABILITIES_SETTINGS[sRawAbilityName].ScepterAbilities
	then
		for i, sAbilityName in ipairs(ABILITIES_SETTINGS[sRawAbilityName].ScepterAbilities) do
			local hAbility = hHero:FindAbilityByName(sAbilityName)
			if hAbility then
				if
					HEROES_SETTINGS[hHero:GetUnitName()] == nil
					or (not table.contains(HEROES_SETTINGS[hHero:GetUnitName()].ScepterAbilities, sAbilityName))
				then
					hHero:RemoveAbilityWithRestructure(sAbilityName) -- Удаление способности
				end
			end
...
function HeroBuilder:RemoveShardLinkAbilities(hHero, sRawAbilityName)
	if
		...
		and hHero:GetUnitName()
		and ABILITIES_SETTINGS[sRawAbilityName]
		and ABILITIES_SETTINGS[sRawAbilityName].ShardAbilities
	then
		for i, sAbilityName in ipairs(ABILITIES_SETTINGS[sRawAbilityName].ShardAbilities) do
			...
			if hAbility then
				if
					HEROES_SETTINGS[hHero:GetUnitName()] == nil
					or (not table.contains(HEROES_SETTINGS[hHero:GetUnitName()].ShardAbilities, sAbilityName))
				then
...
function HeroBuilder:AddScepterLinkAbilities(hHero)
	if hHero and hHero:IsRealHero() and hHero:GetUnitName() and hHero:HasScepter() then
		for AbilityName, AbilityInfo in pairs(ABILITIES_SETTINGS) do
			...
			if
				hLoopAbility
				and not hLoopAbility:IsNull()
				and hLoopAbility.sRemovalTimer == nil
				and AbilityInfo.ScepterAbilities
			then
	...
	if hHero and hHero:IsRealHero() and hHero:GetUnitName() and hHero:HasShard() then
		for AbilityName, AbilityInfo in pairs(ABILITIES_SETTINGS) do
			...
			if
				...
				and not hLoopAbility:IsNull()
				and hLoopAbility.sRemovalTimer == nil
				and AbilityInfo.ShardAbilities
			then
						...
						if hLoopAbility.rubick_spell == nil then
							...
							if hShardAbility and (not hShardAbility:IsNull()) then
								if sAbilityName ~= "shadow_demon_demonic_cleanse" then
									...
									local shadow_demon_demonic_purge =
										hHero:FindAbilityByName("shadow_demon_demonic_purge")
								...
								if i == 1 then
									Timers:CreateTimer(0.1, function()
										if not hShardAbility or hShardAbility:IsNull() then
											return
										end
...
function HeroBuilder:GenerateAbilitiesListForRandom(
	PlayerID,
	delete_ability,
	bAddDeletedAbilityToPull,
	bMoreSSSSpells,
	bIsMainSSS
)
	if not self.Players[PlayerID] then
	...
	if bMoreSSSSpells then
		...
		for _, AbilityName in pairs(FreeSSSAbilities) do
			if
				not KeyValues:IsBannedAbility(AbilityName)
				and delete_ability ~= AbilityName
				and not table.contains(self.Players[PlayerID].abilities_list, AbilityName)
				and not Hero:HasAbility(AbilityName)
			then
...
function HeroBuilder:GenerateIdealAbilitiesForAllPlayers(bUseOnlySSS)
	...
	local Pool = bUseOnlySSS and Bans:GetAllRandomBansGroupUnbanned("ABILITIES", "SSS")
		or table.make_key_table(KeyValues:GetAllAbilities())
	...
	if AbilityInfo and AbilityInfo.LinkedAbilities then
		for sLinkedAbilityName, Level in pairs(AbilityInfo.LinkedAbilities) do
			...
			for _, sOtherAbility in ipairs(self.Players[PlayerID].abilities_list) do
				if
					(sOtherAbility ~= AbilityName)
					and KeyValues.AbilitiesList[sOtherAbility]
					and KeyValues.AbilitiesList[sOtherAbility].LinkedAbilities
					and KeyValues.AbilitiesList[sOtherAbility].LinkedAbilities[sLinkedAbilityName]
				then
					bStillUsing = true
				end
			end
...
if not HeroBuilder.bStarted then
	HeroBuilder:Init()
end