...
function RoundController:constructor(RoundNum, RoundInfo, CurrentPVPPair, PrepareTime)
	...
	if not _G.__stashfix_started then
		...
		ListenToGameEvent("entity_killed", function(event)
			...
			if not killed.IsRealHero or not killed:IsRealHero() then
				return
			end
			...
			Timers:CreateTimer(0.5, function()
				if killed:IsNull() then
					return
				end
				if killed:IsAlive() then
					Timers:CreateTimer(0.2, function()
						if not killed:IsNull() then
							CleanupBackpackMods(killed)
						end
					end)
					return
				end
				...
				if attempts > 120 then
					return
				end -- ~60с страховка, если не ожил
				return 0.5
			end)
		end, nil)
	end
...
function RoundController:PrepareRound()
	...
	for _, TeamID in ipairs(SpectatorsTeams) do
		...
		if not Players:IsActiveTeam(TeamID) then
			...
			if
				self.RoundInfo.type == ROUND_TYPES.BASIC
				and not Rounds:IsTeamInPVPDuel(TeamID)
				and self.RoundInfo.creeps ~= nil
			then
			...
			CustomNetTables:SetTableValue(
				"globals",
				"team_" .. TeamID .. "_round_info",
				{ last_time = Data.time, current_state = Data.state }
			)
		end
	end
	for TeamID, TeamInfo in pairs(Players:GetAllActiveTeams()) do
		...
		if self.RoundInfo.type == ROUND_TYPES.BASIC then
			...
			if Arena and not Rounds:IsTeamInPVPDuel(TeamID) and self.RoundInfo.creeps ~= nil then
				...
				self.TeamsData[TeamID].panorama_info.creeps_info =
					self:GetCreepsInfo(self.RoundInfo.creeps, ExtraCreepsList)
				...
				GameRules:GetGameModeEntity():SetThink(function()
					if Players:IsActiveTeam(TeamID) then
						...
						for UnitName, UnitInfo in pairs(self.RoundInfo.creeps) do
							if not self.TeamsData[TeamID].creeps_info[UnitName] then
								self.TeamsData[TeamID].creeps_info[UnitName] =
									{ next_spawn = GameRules:GetGameTime() + 0.4 + (i * 0.05), count = 1 }
							...
							elseif UnitInfo.count > self.TeamsData[TeamID].creeps_info[UnitName].count then
								local diff = self.TeamsData[TeamID].creeps_info[UnitName].next_spawn
									- GameRules:GetGameTime()
								if diff <= 0 then
									self.TeamsData[TeamID].creeps_info[UnitName].next_spawn = GameRules:GetGameTime()
										+ 0.4
										+ (i * 0.05)
									self.TeamsData[TeamID].creeps_info[UnitName].count = self.TeamsData[TeamID].creeps_info[UnitName].count
										+ 1
					...
					if
						self.TeamsData[TeamID].max_in_round > #self.TeamsData[TeamID].creeps_list
						and Players:IsActiveTeam(TeamID)
					then
						return 0.1
					else
						return -1
				...
				GameRules:GetGameModeEntity():SetThink(function()
					if #ExtraCreepsList > 0 and Players:IsActiveTeam(TeamID) then
						...
						if Rounds:IsThisExtraCreepCanBeLive(Unit.player_id, Unit.round) then
							...
							self.TeamsData[TeamID].panorama_info.extra_creeps = ArrayRemove(
								self.TeamsData[TeamID].panorama_info.extra_creeps,
								function(t, i, j)
									if
										t[i].name == Unit.name
										and t[i].player_id == Unit.player_id
										and t[i].round == Unit.round
									then
										return false
									end
									return true
							...
							PlayerTables:SetTableValue(
								"team_" .. TeamID,
								"round_info",
								self.TeamsData[TeamID].panorama_info
							)
						end
...
function RoundController:UpdateGlobalTeamTable(TeamID)
	if not self.TeamsData[TeamID] or not self.TeamsData[TeamID].panorama_info then
		return
	end
...
function RoundController:SpawnArenaVisionRevealer(TeamID)
	...
	if TeamInfo.vision_revealer_spawned then
		return
	end
	if TeamInfo.arena == nil or TeamInfo.arena == "" then
		return
	end
	if Rounds:IsTeamInPVPDuel(TeamID) then
		return
	...
	local VisionRevealer =
		CreateUnitByName("arena_vision_revelear", ArenaInfo.center, false, nil, nil, DOTA_TEAM_NEUTRALS)
...
function RoundController:PreCircle()
	...
	if self.FxPreCircle == nil and Settings then
		self.FxPreCircle = ParticleManager:CreateParticle(
		...
		ParticleManager:SetParticleControl(
			self.FxPreCircle,
			1,
			Vector(Settings.MAX_SIZE, Settings.MAX_SIZE, Settings.MAX_SIZE)
		)
	end
end
...
function RoundController:CircleThink()
	...
	if self.FxCircle == nil then
		self.FxCircle =
			ParticleManager:CreateParticle("particles/minigames_circle/minigames_circle.vpcf", PATTACH_WORLDORIGIN, nil)
		ParticleManager:SetParticleControl(self.FxCircle, 0, self.CircleArenaCenter)
		ParticleManager:SetParticleControl(
			self.FxCircle,
			1,
			Vector(Settings.MAX_SIZE, Settings.MIN_SIZE, -self.CircleSpeed)
		)
		...
		if self.FxCircleEnd == nil then
			self.FxCircleEnd = ParticleManager:CreateParticle(
				"particles/minigames_circle/minigames_pre_circle.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
			...
			ParticleManager:SetParticleControl(
				self.FxCircleEnd,
				1,
				Vector(Settings.MIN_SIZE, Settings.MIN_SIZE + 75, Settings.MIN_SIZE + 200)
			)
		end
	end
		...
		if TeamPlayerID ~= nil and PlayerInfo ~= nil and TeamInfo.extra_creeps_info then
			...
			if Players:IsValidHero(Hero) then
				for _, ExtraCreatureInfo in ipairs(TeamInfo.extra_creeps_info) do
					if
						not Rounds:IsThisExtraCreepCanBeLive(ExtraCreatureInfo.player_id, ExtraCreatureInfo.round)
						and ExtraCreatureInfo.unit
						and not ExtraCreatureInfo.unit:IsNull()
						and ExtraCreatureInfo.unit:IsAlive()
					then
...
function RoundController:TeamRoundOverTime(TeamID, bRoundTimeOver)
	...
	for PlayerID, PlayerInfo in pairs(Players:GetTeamActivePlayers(TeamID)) do
		...
		if
			Rounds:IsPlayerInPVPDuel(PlayerID)
			and not Rounds:IsFinishedTeam(PlayerInfo.team)
			and not Rounds:IsPvpPairEnded()
		then
			local Hero = PlayerInfo.hero
	...
	local GetUnitInfo = function(UnitName, Count, RoundNum)
		...
		if KV then
			...
			local Armor = (KV.ArmorPhysical or 0)
				+ (GetGameSetting("CREEP_BONUS_ARMOR_PER_LATE_ROUND") * math.max(0, RoundNum - 50))

			data.physical_resist = math.round((0.06 * Armor) / (1 + 0.06 * math.abs(Armor)) * 100)

			local baseResist = (KV.MagicalResistance or 0) * 0.01
			local roundBonus = (
				GetGameSetting("CREEP_BONUS_MAGIC_RESISTANCE_PER_LATE_ROUND") * math.max(0, RoundNum - 50)
			) * 0.01
...
function RoundController:GetEffectiveDamageRound(BaseHealth, RoundNum)
	if not BaseHealth or BaseHealth <= 0 then
		return RoundNum
	end

	local _, uncappedHealth = self:CalculateHealth(BaseHealth, RoundNum)
	if uncappedHealth < MAX_CREEP_HEALTH then
		return RoundNum
	end
...
function RoundController:CalculateDamageReduction(Health, RoundNum)
	if GAME_DAMAGE_REDUCTION_ENABLED == false then
		return 0
	end
	...
	return math.round(
		(
			1
			- (
				ReductionFromOverHealth
				/ (1 + ((CREEP_DAMAGE_REDUCTION_PER_LATE_ROUND * 0.01) * math.max(0, RoundNum - 50)))
			)
		) * 100
	)
end
...
function RoundController:FindAllOwnedUnits(player)
	...
	for _, v in ipairs(units) do
		if
			type(player) == "number" and ((v.GetPlayerID ~= nil and v:GetPlayerID() or v:GetPlayerOwnerID()) == pid)
			or v:GetPlayerOwner() == player
		then
...
function RoundController:OnEntityKilled(event)
	...
	if hKilledUnit == nil or hKilledUnit:IsNull() then
		return
	end
	...
	if self.RoundInfo.type == ROUND_TYPES.BASIC or self.RoundInfo.type == ROUND_TYPES.BOSS then
		if hKilledUnit and not hKilledUnit:IsHero() then
			...
			if bIsLastCreep then
				return
			end
			if hKilledUnit:IsNull() then
				return
			end
			...
			if TeamID ~= nil and Players:IsActiveTeam(TeamID) and self.TeamsData[TeamID] ~= nil then
				...
				if iKiller ~= nil then
					...
					if hKiller then
						...
						if
							RealKiller
							and RealKiller:IsRealHero()
							and Rounds:IsPlayerInPVPDuel(PlayerID)
							and Rounds:IsPlayerInPVPDuel(RealKiller:GetPlayerID())
						then
				...
				for TeamPlayerID, TeamPlayerInfo in pairs(Players:GetTeamActivePlayers(TeamID)) do
					...
					if ROUND_DEBUG_ENABLED then
						...
						sAfkLog = sAfkLog
							.. string.format(
								" PID=%d{valid=%s alive=%s reinc=%s lifes=%d}",
								TeamPlayerID,
								tostring(bValid),
								tostring(bAlive),
								tostring(bReinc),
				...
				if ROUND_DEBUG_ENABLED then
					print(
						string.format(
							"[ROUND_DEBUG] AFK_CHECK: TeamID=%d killedPID=%d round=%d AllTeamAFK=%s players={%s}",
							TeamID,
							PlayerID,
							self.RoundNum,
							tostring(AllTeamAFK),
				...
				if not Rounds:IsTeamInPVPDuel(TeamID) then
					...
					if nLifesKilled <= 0 then
						...
						if ROUND_DEBUG_ENABLED then
							print(
								string.format(
									"[ROUND_DEBUG] AEGIS_FALLBACK_SCHEDULED: TeamID=%d killedPID=%d lifes=0 delay=%.1fs",
									TeamID,
									PlayerID,
									fFallbackDelay
								)
						...
						GameRules:GetGameModeEntity():SetThink(function()
							...
							if not Players:IsActiveTeam(TeamID) then
								if ROUND_DEBUG_ENABLED then
									print(
										"[ROUND_DEBUG] AEGIS_FALLBACK_SKIP: TeamID="
											.. tostring(TeamID)
											.. " team уже не активна (видимо штатный flow отработал)"
									)
								end
								return
							end
							...
							if ROUND_DEBUG_ENABLED then
								print(
									string.format(
										"[ROUND_DEBUG] AEGIS_FALLBACK_CHECK: TeamID=%d anyAlive=%s players={%s}",
										TeamID,
										tostring(bAnyAlive),
										sCheckLog
									)
							...
							if not bAnyAlive then
								if ROUND_DEBUG_ENABLED then
									print(
										string.format(
											"[ROUND_DEBUG] AEGIS_FALLBACK_FIRE: TeamID=%d -- форсирую MakeTeamLose (штатный AFK-flow не сработал)",
											TeamID
										)
									)
								end
	...
	if self.RoundInfo.type == ROUND_TYPES.VOTING then
		if hKilledUnit and IsRealHero(hKilledUnit) then
			...
			if TeamID ~= nil and Players:IsActiveTeam(TeamID) then
				...
				for TeamPlayerID, TeamPlayerInfo in pairs(Players:GetTeamActivePlayers(TeamID)) do
					local Hero = TeamPlayerInfo.hero
					if
						Players:IsValidHero(Hero)
						and (Hero:IsAlive() or (not Hero:IsAlive() and Hero:IsReincarnating()))
					then
						AllTeamAFK = false
					end
				end
		...
		elseif hKilledUnit and not hKilledUnit:IsIllusion() and Players:IsSecondaryUnit(hKilledUnit) then
			...
			local allUnits = FindUnitsInRadius(
				hKilledUnit:GetTeamNumber(),
				hKilledUnit:GetAbsOrigin(),
				nil,
				FIND_UNITS_EVERYWHERE,
				DOTA_UNIT_TARGET_TEAM_FRIENDLY,
			...
			if RealHero and IsRealHero(RealHero) then
				...
				if TeamID ~= nil and Players:IsActiveTeam(TeamID) then
					...
					for TeamPlayerID, TeamPlayerInfo in pairs(Players:GetTeamActivePlayers(TeamID)) do
						...
						if
							Players:IsValidHero(Unit)
							and (Unit:IsAlive() or (not Unit:IsAlive() and Unit:IsReincarnating()))
						then
							AllTeamAFK = false
						else
...
function RoundController:SpawnCreep(UnitName, TeamID, Position, bIsExtra, PlayerID, Round)
	Creeps:SpawnUnitByName(UnitName, Position, true, nil, nil, DOTA_TEAM_NEUTRALS, function()
		...
		hUnit:SetPhysicalArmorBaseValue(
			hUnit:GetPhysicalArmorBaseValue()
				+ (GetGameSetting("CREEP_BONUS_ARMOR_PER_LATE_ROUND") * math.max(0, self.RoundNum - 50))
		)
		...
		hUnit:SetDeathXP(
			math.floor(
				(GameRules.xpTable[self.RoundNum + 1] - GameRules.xpTable[self.RoundNum])
					/ self.TeamsData[TeamID].max_in_round
			)
		)
...
function RoundController:TeamFinished(TeamID)
	...
	if _hasDuel and _dPair and TeamID ~= _dPair.fTeam and TeamID ~= _dPair.sTeam then
		...
		for _, dInfo in pairs(Players:GetTeamActivePlayers(_dPair.fTeam)) do
			if Players:IsValidHero(dInfo.hero) then
				fHero = dInfo.hero
				break
			end
		end
			...
			if #_watchers > 0 then
				Timers:CreateTimer(0.7, function()
					...
					Timers:CreateTimer(0.3, function()
						...
						if camDummy and not camDummy:IsNull() then
							UTIL_Remove(camDummy)
						end
					end)
				end)
			end
...
function RoundController:ClearTeamArena(TeamID, _Arena)
	if not self.TeamsData[TeamID] then
		return
	end
