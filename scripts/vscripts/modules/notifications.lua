...
function Notifications:ScheduleThink()
	...
	while true do
		...
		if NotificationInfo == nil then
			break
		end
		bHasData = true

		local KeyName = NotificationInfo.PlayerID == nil and "global_notification_"
			or "player_" .. NotificationInfo.PlayerID .. "_notification_"
...
function Notifications:AddNotification(nType, nRound, hValues, PlayerID)
	if not GAME_NOTIFICATIONS_ENABLED then
		return
	end
...
if not Notifications.bStarted then
	Notifications:Init()
end