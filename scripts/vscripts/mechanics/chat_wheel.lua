...
function ChatWheel:OnPlayerSelectedItem(event)
	...
	if Shop and Shop.IsShopAvailable and not Shop:IsShopAvailable(PlayerID) then
		return
	end
	...
	if LineID < 1 or LineID > 8 then
		logger:Log("Chat wheel line id is out of range")
		return
	end
	...
	if not canUseItem then
		logger:Log(
			string.format("Player %s tried to set locked chat wheel item '%s'", tostring(PlayerID), tostring(ItemName))
		)
		return
	end
	...
	CustomNetTables:SetTableValue("chat_wheel", tostring(PlayerID), self.Players[PlayerID].ChatWheel)
	if Shop and Shop.ScheduleInventorySave then
