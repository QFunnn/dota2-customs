...
local SUPPORTED_SLOTS = {
	...
	[ITEMS_DEFAULT_SLOTS.FX_ATTACK] = true,
}
...
function WereableSystem:Init()
	...
	self.RefreshTokens = self.RefreshTokens or {}
...
function WereableSystem:OnEquipItem(event)
	...
	self:RefreshPlayerSlot(playerId, slotName)
end
...
function WereableSystem:OnNpcSpawned(event)
	...
	self:RequestPlayerRefresh(playerId)
end

---@param event table
function WereableSystem:OnHeroFinishSpawn(event)
	local hero = EntIndexToHScript(event.heroindex or -1)
...
function WereableSystem:RequestPlayerRefresh(playerId)
	if playerId == nil or playerId < 0 then
		return
	end

	self.RefreshTokens = self.RefreshTokens or {}
	local refreshToken = (self.RefreshTokens[playerId] or 0) + 1
	self.RefreshTokens[playerId] = refreshToken

	local delays = { FrameTime(), 0.1, 0.5, 1.0, 2.0 }
	for _, delay in ipairs(delays) do
		Timers:CreateTimer(delay, function()
			if self.RefreshTokens[playerId] ~= refreshToken then
				return nil
			end

			self:RefreshPlayer(playerId)
			return nil
		end)
	end
end
...
---@param playerId integer
function WereableSystem:RefreshPlayer(playerId)
...
function WereableSystem:RefreshPlayerSlot(playerId, slotName)
	local equippedItem = Shop and Shop.GetEquippedItemInSlot and Shop:GetEquippedItemInSlot(playerId, slotName) or nil
	if equippedItem then
		self:ApplyItemBySlot(playerId, slotName, equippedItem)
	else
		self:RemoveItemBySlot(playerId, slotName)
	end
end
...
function WereableSystem:ApplyItemBySlot(playerId, slotName, itemName)
	if slotName == ITEMS_DEFAULT_SLOTS.FX_ATTACK then
		self:ApplyAttackEffectByItem(playerId, itemName)
		return
	end
...
---@param playerId integer
---@param slotName string
function WereableSystem:RemoveItemBySlot(playerId, slotName)
	if slotName == ITEMS_DEFAULT_SLOTS.FX_ATTACK then
		self:RemoveAttackEffect(playerId)
		return
	end
...
---@param playerId integer
function WereableSystem:RemoveAttackEffect(playerId)
	local hero = PlayerResource:GetSelectedHeroEntity(playerId)
	if not hero or hero:IsNull() then
		return
	end

	hero:RemoveModifierByName("modifier_cosmetic_attack_effect")
end

---@param playerId integer
---@param itemName string
function WereableSystem:ApplyAttackEffectByItem(playerId, itemName)
	local hero = PlayerResource:GetSelectedHeroEntity(playerId)
	if not hero or hero:IsNull() then
		return
	end
	...
	if hero:GetAttackCapability() ~= DOTA_UNIT_CAP_RANGED_ATTACK then
		return
	end
	...
	hero:AddNewModifier(hero, nil, "modifier_cosmetic_attack_effect", {
		projectile_name = particlePath,
	})
end
