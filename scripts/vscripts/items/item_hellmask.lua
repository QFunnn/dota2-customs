...
function item_hellmask:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_hellmask_active", { duration = duration_satanic })
end

function item_hellmask:GetIntrinsicModifierName()
	return "modifier_item_hellmask"
end

modifier_item_hellmask_active = class({})

function modifier_item_hellmask_active:IsPurgable()
	return false
end
...
function modifier_item_hellmask:IsPurgable()
	return false
end
function modifier_item_hellmask:IsHidden()
	return self.ability:GetSecondaryCharges() == 1
end
...
function modifier_item_hellmask:GetModifierDamageOutgoing_Percentage()
	if not self.parent:HasModifier("modifier_skill_hellcrown") then
		return
	end
...
function modifier_item_hellmask:GetModifierPreAttack_BonusDamage(params)
	if self.ability then
		local bonus_unique = self.ability:GetCurrentCharges() * self.bonus_damage_per_stack
		local total = self.bonus_damage + bonus_unique
		if _G.Players and _G.Players.QueueAttackBonus and params and params.attacker and params.target then
			_G.Players:QueueAttackBonus(params.attacker, params.target, total, "item_hellmask", DAMAGE_TYPE_PHYSICAL)
		end
		return total
	end
end
...
function modifier_item_hellmask:GetModifierPhysicalArmorBonus()
	if self.ability then
		...
		if self.ability:GetSecondaryCharges() == 1 then
			bonus_unique = 0
		end
...
function modifier_item_hellmask:TakeDamageScriptModifier(params)
	if IsServer() then
		...
		if params.attacker == self.parent then
			...
			if no_heal_target[params.unit:GetUnitName()] then
				return
			end
			if params.unit:IsWard() or params.unit:IsHeroWard() then
				return
			end
			...
			if params.unit:IsHero() or (not params.unit:IsHero() and params.unit:GetOwner() ~= nil) then
				lifesteal = lifesteal * 0.5
			end
			self.parent:Heal(params.damage * lifesteal, nil)
		end
	end
end
...
function modifier_item_hellmask_aura_buff:IsDebuff()
	return false
end
function modifier_item_hellmask_aura_buff:AllowIllusionDuplicate()
	return true
end
function modifier_item_hellmask_aura_buff:IsHidden()
	return true
end
function modifier_item_hellmask_aura_buff:IsPurgable()
	return false
end
...
function modifier_item_hellmask_buff:IsHidden()
	return true
end
function modifier_item_hellmask_buff:IsPurgable()
	return false
end
function modifier_item_hellmask_buff:IsDebuff()
	return false
end
...
function modifier_item_hellmask_aura_debuff:IsDebuff()
	return false
end
function modifier_item_hellmask_aura_debuff:AllowIllusionDuplicate()
	return true
end
function modifier_item_hellmask_aura_debuff:IsHidden()
	return true
end
function modifier_item_hellmask_aura_debuff:IsPurgable()
	return false
end
...
function modifier_item_hellmask_debuff:IsHidden()
	return true
end
function modifier_item_hellmask_debuff:IsPurgable()
	return false
end
function modifier_item_hellmask_debuff:IsDebuff()
	return true
end
