...
function modifier_item_aeon_disk_lua:OnCreated(keys)
	...
	if IsServer() and parent and parent:IsIllusion() then
		self:Destroy()
		return
	end
...
function modifier_item_aeon_disk_lua:OnRefresh(keys)
	...
	if (not self.parent) or not self.ability or self.parent:IsNull() or self.ability:IsNull() then
		return
	end
...
function modifier_item_aeon_disk_lua:GetModifierIncomingDamage_Percentage(keys)
	...
	if
		IsValidEntity(self.ability)
		and self.ability:IsCooldownReady()
		and parent:GetHealth() - keys.damage <= (health_threshold + 1)
		and self.ability:GetItemSlot() <= DOTA_ITEM_SLOT_6
	then
		if parent:HasModifier("modifier_nyx_assassin_spiked_carapace_custom") then
			local modifier_nyx_assassin_spiked_carapace_custom =
				parent:FindModifierByName("modifier_nyx_assassin_spiked_carapace_custom")
			if modifier_nyx_assassin_spiked_carapace_custom.carapaced_units[keys.attacker:entindex()] == nil then
				return
			end
		end
		if parent:HasModifier("modifier_oracle_false_promise_custom") then
			return
		end
		if self:GetParent():HasModifier("modifier_duel_curse") then
			return
		end
		...
		parent:AddNewModifier(
			parent,
			self.ability,
			"modifier_item_aeon_disk_lua_buff",
...
function modifier_item_aeon_disk_lua_buff:IsPurgable()
	return true
end
...
function modifier_item_aeon_disk_lua_buff:OnCreated(keys)
	...
	print(
		string.format(
			"[AEON_DIAG] BUFF_CREATED t=%.3f duration_arg=%s remaining=%.3f hp=%.1f/%.1f",
			self._aeonDiagStart,
			tostring(keys and keys.duration),
	...
	self.buff_pfx = ParticleManager:CreateParticle(
...
function modifier_item_aeon_disk_lua_buff:OnDestroy()
	if IsServer() then
		...
		print(
			string.format(
				"[AEON_DIAG] BUFF_DESTROYED elapsed=%.3f hp=%.1f alive=%s remaining=%.3f",
				elapsed,
				parent and parent:GetHealth() or -1,
				tostring(parent and parent:IsAlive()),
				self:GetRemainingTime() or -1
			)
		)
	end
	if self.buff_pfx then
		ParticleManager:ReleaseParticleIndex(self.buff_pfx)
	end
end
...
function modifier_item_aeon_disk_lua_buff:GetModifierIncomingDamage_Percentage(keys)
	if IsServer() then
		...
		print(
			string.format(
				"[AEON_DIAG] INCOMING dmg=%.1f flags=%s type=%s atk=%s inf=%s hp=%.1f",
				keys and keys.damage or -1,
				tostring(keys and keys.damage_flags),
...
function modifier_item_aeon_disk_lua_buff:GetAbsoluteNoDamagePhysical(keys)
	if IsServer() then
		print("[AEON_DIAG] ABS_NO_PHYS called dmg=" .. tostring(keys and keys.damage))
	end
	return 1
end
...
function modifier_item_aeon_disk_lua_buff:GetAbsoluteNoDamageMagical(keys)
	if IsServer() then
		print("[AEON_DIAG] ABS_NO_MAG called dmg=" .. tostring(keys and keys.damage))
	end
	return 1
end
...
function modifier_item_aeon_disk_lua_buff:GetAbsoluteNoDamagePure(keys)
	if IsServer() then
		print("[AEON_DIAG] ABS_NO_PURE called dmg=" .. tostring(keys and keys.damage))
	end
	return 1
end
