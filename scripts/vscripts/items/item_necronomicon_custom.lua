...
function item_necronomicon_custom:OnSpellStart()
	...
	local ranged_summon = CreateUnitByName(
		ranged_summon_name,
		ranged_loc,
		true,
		self:GetCaster(),
...
function item_necronomicon_melee_custom:OnSpellStart()
	...
	local melee_summon = CreateUnitByName(
		melee_summon_name,
		melee_loc,
		true,
		self:GetCaster(),
...
function modifier_invul_necronomicon:IsPurgable()
	return false
end
function modifier_invul_necronomicon:IsHidden()
	return true
end
function modifier_invul_necronomicon:IsPurgeException()
	return false
end
...
function modifier_invul_necronomicon:OnDestroy(params)
	...
	if self:GetParent():GetUnitName() ~= "npc_dota_necronomicon_warrior_3" then
		return
	end
	...
	for _, enemy in pairs(enemies) do
		ApplyDamage({
			...
			damage = self:GetAbility():GetSpecialValueFor("explosion"),
