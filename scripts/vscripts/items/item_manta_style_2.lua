...
function item_manta_style_2:OnSpellStart()
	...
	caster:AddNewModifier(
		caster,
		self,
		"modifier_item_manta_style_2_invulnerable",
...
modifier_item_manta_style_2_invulnerable = class({
	...
	OnDestroy = function(self)
		...
		local caster = self:GetCaster()
		local ability = self:GetAbility()
		if not IsServer() or not parent:IsAlive() or not ability then
			return
		end
		...
		}, self.IllusCount, Padding, true, true)
