...
LinkLuaModifier(
	...
	"items/item_revenants_brooch_custom",
...
function item_revenants_brooch_custom:GetHealthCost(level)
	if IsServer() then
		...
		if self:GetToggleState() then
			if not self:GetParent():HasModifier("modifier_huskar_blood_magic") then
				return
			end
			return self:GetSpecialValueFor("manacost_per_hit")
		end
...
function modifier_revenants_brooch_custom:IsPurgable()
	return false
end
function modifier_revenants_brooch_custom:IsPurgeException()
	return false
end
...
function modifier_revenants_brooch_custom:OnIntervalThink()
	...
	if self:GetAbility():GetToggleState() then
		if
			not self:GetParent():HasModifier("modifier_revenants_brooch_custom_counter")
			and not self:GetParent():IsIllusion()
		then
			self:GetParent()
				:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_revenants_brooch_custom_counter", {})
		end
	else
...
function modifier_revenants_brooch_custom_counter:GetModifierProjectileName()
	if self:GetParent():HasModifier("modifier_huskar_blood_magic") then
		if self:GetParent():GetHealth() < self:GetAbility():GetEffectiveHealthCost(-1) then
			return
		end
	else
		if self:GetParent():GetMana() < self:GetAbility():GetEffectiveManaCost(-1) then
			return
		end
	end
...
function modifier_revenants_brooch_custom_counter:GetModifierTotalDamageOutgoing_Percentage(event)
	...
	if
		event.inflictor
		and (
			event.inflictor:GetName() ~= "muerta_gunslinger_custom"
			and event.inflictor:GetName() ~= "windrunner_focusfire_whirlwind"
		)
	then
		return 0
	end
	...
	if event.target and string.find(event.target:GetUnitName(), "npc_dota_techies") then
		return
	end
	if not self:GetAbility() then
		return 0
	...
	if not event.process_procs then
		return
	end
	if parent:HasModifier("modifier_juggernaut_blade_fury_custom") then
		return
	end
	if self:GetParent():HasModifier("modifier_huskar_blood_magic") then
		if self:GetParent():GetHealth() < self:GetAbility():GetEffectiveHealthCost(-1) then
		...
		if not event.no_attack_cooldown and not self:GetCaster().riki_attack then
			if self:GetCaster():HasModifier("modifier_huskar_14") then
				...
				self:GetCaster()
					:SetHealth(math.max(self:GetCaster():GetHealth() - self:GetAbility():GetEffectiveHealthCost(-1), 1))
			end
		end
	else
		if self:GetParent():GetMana() < self:GetAbility():GetEffectiveManaCost(-1) then
	...
	local modifier_earthshaker_enchant_totem_custom = self:GetParent()
		:HasModifier("modifier_earthshaker_enchant_totem_custom")
	Timers:CreateTimer(FrameTime(), function()
		if modifier_earthshaker_enchant_totem_custom then
			return
		end
		ApplyDamage({
			attacker = parent,
			damage = event.original_damage * 0.8,
