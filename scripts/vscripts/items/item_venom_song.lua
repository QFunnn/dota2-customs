...
function modifier_item_venom_song:IsHidden()
	return true
end
function modifier_item_venom_song:IsPurgable()
	return false
end
function modifier_item_venom_song:IsPurgeException()
	return false
end
function modifier_item_venom_song:GetAttributes()
...
function modifier_item_venom_song:OnAttackLanded(params)
	if params.attacker == self:GetParent() then
		...
		params.target:AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_item_venom_song_debuff",
...
function modifier_item_venom_song_debuff:IsPurgable()
	return true
end
...
function modifier_item_venom_song_debuff:OnCreated()
	...
	self.damage = (self:GetCaster():GetAverageTrueAttackDamage(nil) / 100 * damage_phys)
		+ self:GetAbility():GetSpecialValueFor("base_damage")
