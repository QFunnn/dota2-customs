...
function item_bloodthorn_custom:OnSpellStart()
	...
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_bloodthorn_custom_active",
		{ duration = duration * (1 - target:GetStatusResistance()), silence_damage_percent = silence_damage_percent }
	)
end
...
function modifier_item_bloodthorn_custom_active:OnTakeDamage(params)
	...
	if params.inflictor == nil and params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK then
		local dmg = ApplyDamage({
			...
			damage = self:GetAbility():GetSpecialValueFor("proc_damage"),
			damage_type = DAMAGE_TYPE_MAGICAL,
			ability = self:GetAbility(),
		})
		if dmg > 0 then
