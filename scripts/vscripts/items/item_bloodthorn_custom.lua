...
LinkLuaModifier( "modifier_item_bloodthorn_custom", "items/item_bloodthorn_custom", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_item_bloodthorn_custom_active", "items/item_bloodthorn_custom", LUA_MODIFIER_MOTION_NONE )
...
function item_bloodthorn_custom:OnSpellStart()
	...
	target:AddNewModifier(self:GetCaster(), self, "modifier_item_bloodthorn_custom_active", {duration = duration * (1 - target:GetStatusResistance()), silence_damage_percent = silence_damage_percent})
end
...
function modifier_item_bloodthorn_custom:GetModifierConstantHealthRegen()
    if self:GetAbility() then
...
function modifier_item_bloodthorn_custom:OnAttackRecord(keys)
    if not IsServer() then return end
...
function modifier_item_bloodthorn_custom:OnAttackLanded(keys)
    if not IsServer() then return end
    if keys.attacker == self:GetParent() then
        if self:GetParent():IsIllusion() then return end
        if self.critProc then
        	local damage = self:GetAbility():GetSpecialValueFor("passive_proc_damage")
...
function modifier_item_bloodthorn_custom_active:OnTakeDamage(params)
    ...
    if params.inflictor == nil and params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK then
        local dmg = ApplyDamage({ victim = self:GetParent(), attacker = params.attacker, damage = self:GetAbility():GetSpecialValueFor("proc_damage"), damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility() })
        if dmg > 0 then
            ParticleManager:SetParticleControl(ParticleManager:CreateParticle("particles/items2_fx/orchid_pop.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent()), 1, Vector(dmg,dmg,dmg))
        end
    end
...
function modifier_item_bloodthorn_custom_active:OnDestroy()
    ...
    if self:GetRemainingTime() <= 0 then
        ...
        ParticleManager:SetParticleControl(ParticleManager:CreateParticle("particles/items2_fx/orchid_pop.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent()), 1, Vector(damage,damage,damage))
        if damage > 0 then
