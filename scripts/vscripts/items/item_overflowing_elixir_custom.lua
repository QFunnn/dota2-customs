...
LinkLuaModifier(
	...
	"items/item_overflowing_elixir_custom",
...
function item_overflowing_elixir_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_overflowing_elixir_custom", { duration = duration })
	self:GetCaster():EmitSound("Bottle.Drink")
end

modifier_item_overflowing_elixir_custom = class({})

function modifier_item_overflowing_elixir_custom:IsPurgable()
	return false
end
