...
function item_meteor_lens:OnSpellStart()
	...
	AddFOWViewer(
		...
		self:GetSpecialValueFor("land_time") + 0.5,
		false
	)
	...
	local ground_particle = ParticleManager:CreateParticleForTeam(
	...
	local meteor_hammer = ParticleManager:CreateParticle(
		...
		if not self:IsNull() then
			...
			for _, enemy in pairs(enemies) do
				...
				enemy:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_item_meteor_lens_burn",
...
function modifier_item_meteor_lens:IsHidden()
	return true
end
function modifier_item_meteor_lens:IsPurgable()
	return false
end
function modifier_item_meteor_lens:IsPurgeException()
	return false
end
function modifier_item_meteor_lens:RemoveOnDeath()
	return false
end
function modifier_item_meteor_lens:GetAttributes()
...
function modifier_item_meteor_lens:GetModifierSpellAmplify_Percentage()
	return self:GetAbility():GetSpecialValueFor("bonus_spell_damage")
end

function modifier_item_meteor_lens:GetModifierMPRegenAmplify_Percentage_Unique()
	return self:GetAbility():GetSpecialValueFor("bonus_mana_amp")
end

function modifier_item_meteor_lens:GetModifierSpellLifestealRegenAmplify_Percentage()
	return self:GetAbility():GetSpecialValueFor("bonus_spell_lifesteal_amp")
end
...
function modifier_item_meteor_lens_burn:OnIntervalThink()
	...
	local damage_in = ApplyDamage({
		attacker = self:GetCaster(),
		victim = self:GetParent(),
		ability = self:GetAbility(),
		damage = self:GetAbility():GetSpecialValueFor("burn_dps_units"),
		damage_type = DAMAGE_TYPE_MAGICAL,
	})
