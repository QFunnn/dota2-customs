...
function modifier_item_panic_button_custom:GetModifierIncomingDamage_Percentage(event)
	...
	if not parent:IsAlive() or parent:HasModifier("modifier_duel_curse") then
		return 0
	end
	...
	if
		...
		and attacker ~= parent
		and not parent:HasModifier("modifier_item_panic_button_custom_buff")
	...
	then
		...
		if
			ResultHealth <= HealthThreshold
			and bit:_and(damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS
		then
			parent:Purge(false, true, false, true, true)
			parent:AddNewModifier(
				parent,
				ability,
				"modifier_item_panic_button_custom_buff",
