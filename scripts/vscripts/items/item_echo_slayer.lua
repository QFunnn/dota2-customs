...
function modifier_item_echo_slayer:IsHidden()
	return true
end
function modifier_item_echo_slayer:IsPurgable()
	return false
end
function modifier_item_echo_slayer:IsPurgeException()
	return false
end
function modifier_item_echo_slayer:RemoveOnDeath()
	return false
end
function modifier_item_echo_slayer:GetAttributes()
...
function modifier_item_echo_slayer:StartSpeed(target, slow)
	...
	if self:GetParent():FindAllModifiersByName("modifier_item_echo_slayer")[1] ~= self then
		return
	end
	...
	if self:GetAbility() and not target:IsBuilding() and not target:IsMagicImmune() and slow then
		target:AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_item_echo_slayer_slow",
			{ duration = self:GetAbility():GetSpecialValueFor("slow_duration") * (1 - target:GetStatusResistance()) }
		)
	end
end
...
function modifier_item_echo_slayer:OnAttackFail(params)
	...
	if params.attacker:FindAllModifiersByName(self:GetName())[1] ~= self then
		return
	end
	...
	if
		self:GetParent():IsRangedAttacker() and not self:GetParent():HasModifier("modifier_vengefulspirit_retribution")
	then
		return
	end
...
function modifier_item_echo_slayer:OnAttackLanded(params)
	...
	if not self:GetParent():IsIllusion() then
		params.target:AddNewModifier(
			...
			{
				duration = (1 - params.target:GetStatusResistance()) * self:GetAbility()
					:GetSpecialValueFor("mage_slayer_duration"),
			}
		)
	end
	if params.attacker:FindAllModifiersByName(self:GetName())[1] ~= self then
...
function modifier_item_echo_slayer_speed:IsHidden()
	return true
end
function modifier_item_echo_slayer_speed:IsPurgable()
	return false
end
...
function modifier_item_echo_slayer_speed:GetModifierAttackSpeedBonus_Constant()
	if self:GetParent().jousting ~= nil then
		return
	end
	if self.is_attacked ~= nil then
		return
	end
	if IsClient() then
		return 0
...
function modifier_item_echo_slayer_speed:OnIntervalThink()
	if
		(
			self:GetParent():IsRangedAttacker()
			and not self:GetParent():HasModifier("modifier_vengefulspirit_retribution")
		...
		params.target:AddNewModifier(
			...
			"modifier_item_echo_slayer_slow",
			{
				duration = self:GetAbility():GetSpecialValueFor("slow_duration")
					* (1 - params.target:GetStatusResistance()),
			}
		)
	end
...
function modifier_item_echo_slayer_slow:IsHidden()
	return false
end
function modifier_item_echo_slayer_slow:IsPurgable()
	return true
end
