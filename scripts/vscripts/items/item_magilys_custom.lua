...
LinkLuaModifier(
	"modifier_item_gleipnir_magic_custom_debuff_rooted",
	"items/item_magilys_custom",
...
function modifier_item_magilys_custom:IsHidden()
	return true
end
function modifier_item_magilys_custom:IsPurgable()
	return false
end
function modifier_item_magilys_custom:IsPurgeException()
	return false
end
...
function modifier_item_magilys_custom:OnAbilityFullyCast(params)
	if params.unit == self.parent then
		...
		if self:GetAbility():GetName() ~= "item_gleipnir_magic_custom" then
			return
		end
		if not self:GetParent():HasModifier("modifier_item_kryptonite_another_cooldown") then
			...
			self:GetParent():AddNewModifier(
				self:GetParent(),
				self:GetAbility(),
				"modifier_item_kryptonite_another_cooldown",
				{ duration = 6 }
			)
		end
	end
...
function modifier_item_magilys_custom:FillacteryEffect(unit)
	...
	ApplyDamage({
		...
		damage = self:GetAbility():GetSpecialValueFor("bonus_spell_damage"),
	...
	local particle =
		ParticleManager:CreateParticle("particles/items_fx/phylactery.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_item_magilys_custom:TakeDamageScriptModifier(keys)
	...
	if self.ability:GetName() ~= "item_gleipnir_magic_custom" then
		return
	end
	...
	if bit:_and(keys.damage_flags, DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL) == DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL then
		return
	end
...
function modifier_item_magilys_custom:GetModifierTotalDamageOutgoing_Percentage(params)
	if params.damage_type == DAMAGE_TYPE_MAGICAL then
		...
		if self:FlagExist(params.damage_flags, DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS) then
			return
		end
		if self:FlagExist(params.damage_flags, DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION) then
			return
		end
		...
		if RollPercentage(chance) or self:GetAbility().IsCastAbility then
			...
			if self.parent:HasModifier("modifier_item_magilys_custom_cooldown") then
				if
					(
						params.target:HasModifier("modifier_item_gleipnir_magic_custom_debuff")
						and not params.target:IsHero()
					) or self:GetAbility().IsCastAbility
				then
					...
					SendOverheadEventMessage(
						...
						params.target,
						params.original_damage + (params.original_damage / 100 * (critical_damage - 100)),
						nil
					)
				...
				self.parent:AddNewModifier(
					self.parent,
					self:GetAbility(),
					"modifier_item_magilys_custom_cooldown",
					{ duration = self.cooldown_crit }
				)
				SendOverheadEventMessage(
					nil,
					OVERHEAD_ALERT_BONUS_SPELL_DAMAGE,
...
function modifier_item_magilys_custom_cooldown:IsHidden()
	return true
end
function modifier_item_magilys_custom_cooldown:IsPurgable()
	return false
end
function modifier_item_magilys_custom_cooldown:IsPurgeException()
	return false
end
...
function item_gleipnir_magic_custom:OnSpellStart()
	...
	for _, target in pairs(enemies) do
		local info = {
			...
			Ability = self,
			EffectName = "particles/items3_fx/gleipnir_projectile_custom.vpcf",
			iMoveSpeed = 1900,
...
function item_gleipnir_magic_custom:OnProjectileHit(target, vLocation)
	...
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_gleipnir_magic_custom_debuff",
...
function modifier_item_gleipnir_magic_custom_debuff:IsPurgable()
	return true
end
...
function modifier_item_gleipnir_magic_custom_debuff_rooted:IsPurgable()
	return true
end
...
function modifier_item_gleipnir_magic_custom_debuff_rooted:GetEffectName()
	return "particles/items3_fx/gleipnir__custom_root.vpcf"
end

modifier_item_kryptonite_another_cooldown = class({})
function modifier_item_kryptonite_another_cooldown:IsPurgable()
	return false
end
function modifier_item_kryptonite_another_cooldown:IsPurgeException()
	return false
end
function modifier_item_kryptonite_another_cooldown:RemoveOnDeath()
	return false
end
function modifier_item_kryptonite_another_cooldown:IsHidden()
	return true
end