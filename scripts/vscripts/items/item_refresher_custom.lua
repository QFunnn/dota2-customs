...
function item_refresher_custom:OnSpellStart()
	...
	local particle =
		ParticleManager:CreateParticle("particles/items2_fx/refresher.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	...
	local modifier_pangolier_gyroshell_custom = self:GetCaster()
		:FindModifierByName("modifier_pangolier_gyroshell_custom")
...
function modifier_item_refresher_custom:IsHidden()
	return true
end
function modifier_item_refresher_custom:IsPurgable()
	return false
end
function modifier_item_refresher_custom:GetAttributes()
