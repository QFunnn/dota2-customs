...
function item_diffusal_blade_2_custom:OnSpellStart()
	...
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_diffusal_blade_2_custom_debuff",
...
function modifier_item_diffusal_blade_2_custom:IsHidden()
	return true
end
function modifier_item_diffusal_blade_2_custom:IsPurgable()
	return false
end
function modifier_item_diffusal_blade_2_custom:IsPurgeException()
	return false
end
function modifier_item_diffusal_blade_2_custom:RemoveOnDeath()
	return false
end
function modifier_item_diffusal_blade_2_custom:GetAttributes()
...
function modifier_item_diffusal_blade_2_custom:GetModifierProcAttack_BonusDamage_Physical(keys)
	...
	if self:GetParent():FindAllModifiersByName("modifier_item_diffusal_blade_2_custom")[1] ~= self then
		return
	end
	if self:GetParent():HasItemInInventory("item_disperser") then
		return
	end
