...
function modifier_item_travel_boots_custom:GetModifierMoveSpeed_Limit()
	...
	if self:GetParent():HasModifier("modifier_spirit_breaker_charge_of_darkness_custom") then
		return
	end
	if self:GetParent():HasModifier("modifier_spirit_breaker_charge_of_darkness_custom_bonus") then
		return
	end
	if
		self:GetParent():HasModifier("modifier_abaddon_9")
		and self:GetParent():HasModifier("modifier_abaddon_jousting_charge")
	then
		return
	end
	if self:GetParent():HasModifier("modifier_roshan_bash_custom_active") then
		return
	end
	return new_max
end
...
function modifier_item_travel_boots_2_custom:IsPurgeException()
	return false
end
