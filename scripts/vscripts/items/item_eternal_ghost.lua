...
function item_eternal_ghost:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_eternal_ghost_active", { duration = duration })
end

modifier_item_eternal_ghost = class({})

function modifier_item_eternal_ghost:IsHidden()
	return true
end
function modifier_item_eternal_ghost:IsPurgable()
	return false
end
function modifier_item_eternal_ghost:IsPurgeException()
	return false
end
...
function modifier_item_eternal_ghost:GetModifierProperty_MagicalLifesteal(params)
	if self:GetParent():FindAllModifiersByName("modifier_item_eternal_ghost")[1] ~= self then
		return
	end
