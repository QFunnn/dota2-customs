...
function modifier_item_staff_of_raindrops:IsHidden()
	return true
end
function modifier_item_staff_of_raindrops:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT
end

function modifier_item_staff_of_raindrops:IsPurgable()
	return false
end
function modifier_item_staff_of_raindrops:IsPurgeException()
	return false
end
...
function modifier_item_staff_of_raindrops:GetModifierTotal_ConstantBlock(params)
	...
	if self:GetParent():FindAllModifiersByName("modifier_item_staff_of_raindrops")[1] ~= self then
		return
	end

	if params.damage < self:GetAbility():GetSpecialValueFor("damage_block_start") then
		return
	end
	...
	SendOverheadEventMessage(
		...
		self:GetParent(),
		self:GetAbility():GetSpecialValueFor("damage_block_magical"),
		nil
	)
