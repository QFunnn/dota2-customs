...
function modifier_item_edge_of_discord:IsPurgable()
	return false
end
function modifier_item_edge_of_discord:IsPurgeException()
	return false
end
...
function modifier_item_edge_of_discord:GetAttributes()
...
function modifier_item_edge_of_discord:GetModifierTotalDamageOutgoing_Percentage(params)
	if params.damage_category == DOTA_DAMAGE_CATEGORY_SPELL then
		if self:GetParent():FindAllModifiersByName("modifier_item_edge_of_discord")[1] ~= self then
			return
		end
		if self:GetParent().block_crit ~= nil then
			return
		end
		if RollPercentage(self:GetAbility():GetSpecialValueFor("chance")) then
			SendOverheadEventMessage(
				...
				params.original_damage
					+ (params.original_damage / 100 * (self:GetAbility():GetSpecialValueFor("spell_damage_crit") - 100)),
				nil
			)
