...
function modifier_item_health_radiance:IsHidden()
	return true
end
function modifier_item_health_radiance:IsPurgable()
	return false
end
function modifier_item_health_radiance:IsPurgeException()
	return false
end

function modifier_item_health_radiance:GetAttributes()
	...
	if self:GetParent():IsIllusion() then
		Timers:CreateTimer(FrameTime(), function()
			...
			if modifier_illusion then
				...
				if not caster:HasModifier("modifier_item_health_radiance_aura") then
...
function modifier_item_health_radiance_aura:IsAura()
	return true
end
function modifier_item_health_radiance_aura:IsHidden()
	return true
end
function modifier_item_health_radiance_aura:IsDebuff()
	return false
end
function modifier_item_health_radiance_aura:IsPurgable()
	return false
end
...
function modifier_item_health_radiance_burn:IsDebuff()
	return true
end
function modifier_item_health_radiance_burn:IsPurgable()
	return false
end
function modifier_item_health_radiance_burn:GetTexture()
	return "item_health_radiance"
end
...
function modifier_item_health_radiance_burn:OnCreated()
	...
	self.particle =
		ParticleManager:CreateParticle("particles/items2_fx/radiance.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_item_health_radiance_burn:OnIntervalThink()
	...
	local end_damage = (self:GetCaster():GetMaxHealth() / 100 * damage) + self.base_damage
	ApplyDamage({
...
function modifier_item_mana_radiance:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_item_mana_radiance:IsHidden()
	return true
end
function modifier_item_mana_radiance:IsPurgable()
	return false
end
function modifier_item_mana_radiance:IsPurgeException()
	return false
end
...
function modifier_item_mana_radiance:OnCreated(keys)
	...
	Timers:CreateTimer(FrameTime(), function()
		if self:GetParent():IsIllusion() then
			...
			if modifier_illusion then
				local caster = modifier_illusion:GetCaster()
				if not caster:HasModifier("modifier_item_mana_radiance_aura") then
...
function modifier_item_mana_radiance_aura:IsAura()
	return true
end
function modifier_item_mana_radiance_aura:IsHidden()
	return true
end
function modifier_item_mana_radiance_aura:IsDebuff()
	return false
end
function modifier_item_mana_radiance_aura:IsPurgable()
	return false
end
...
function modifier_item_mana_radiance_burn:IsDebuff()
	return true
end
function modifier_item_mana_radiance_burn:IsPurgable()
	return false
end
function modifier_item_mana_radiance_burn:GetTexture()
	return "item_mana_radiance"
end
...
function modifier_item_mana_radiance_burn:OnIntervalThink()
	...
	print(self:GetCaster():IsNull())
	local damage = self.damage
	local end_damage = (self:GetCaster():GetMaxMana() / 100 * damage) + self.base_damage
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		ability = self:GetAbility(),
...
function modifier_item_radiance_custom_aura:IsAura()
	return true
end
...
function modifier_item_radiance_custom_burn:IsDebuff()
	return true
end
function modifier_item_radiance_custom_burn:IsPurgable()
	return false
end
function modifier_item_radiance_custom_burn:GetTexture()
	return "item_radiance"
end
...
function modifier_item_radiance_custom_burn:OnCreated()
	...
	self.damage_per_evasion = self:GetAbility():GetSpecialValueFor("damage_per_evasion")
	self.base_damage = self:GetAbility():GetSpecialValueFor("base_damage")
	self.particle =
		ParticleManager:CreateParticle("particles/items2_fx/radiance.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	if self.particle then
...
function modifier_item_radiance_custom_burn:OnIntervalThink()
	...
	local damage_per_evasion = self.damage_per_evasion
	local evasion = self:GetCaster():GetEvasion()
	if self:GetCaster():HasModifier("modifier_windrunner_windrun_custom") then
		evasion = evasion / 2
	end
	local end_damage = (evasion * 100 * damage_per_evasion) + self.base_damage
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		ability = self:GetAbility(),
