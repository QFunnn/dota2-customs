...
function item_panzer_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_panzer_custom_active", { duration = duration })
end

modifier_item_panzer_custom = class({})
function modifier_item_panzer_custom:IsHidden()
	return true
end
function modifier_item_panzer_custom:IsPurgable()
	return false
end
function modifier_item_panzer_custom:IsPurgeException()
	return false
end
function modifier_item_panzer_custom:RemoveOnDeath()
	return false
end
...
function modifier_item_panzer_custom:AttackLandedModifier(params)
	...
	then
		...
		ApplyDamage({
			...
			damage = (params.original_damage / 100 * self:GetAbility():GetSpecialValueFor("passive_reflection_pct"))
				+ self:GetAbility():GetSpecialValueFor("passive_reflection_constant"),
...
function modifier_item_panzer_custom_active:OnDestroy()
	...
	self:GetParent():AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_item_panzer_custom_shield",
		{ duration = self:GetAbility():GetSpecialValueFor("phys_shield_duration") }
	)
end
...
function modifier_item_panzer_custom_active:TakeDamageScriptModifier(params)
	...
	if
		...
		and attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber()
	then
		if self:FlagExist(damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) then
			return
		end
		if self:FlagExist(damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) then
			return
		end
		EmitSoundOnClient("DOTA_Item.BladeMail.Damage", attacker:GetPlayerOwner())
		ApplyDamage({
			victim = attacker,
			damage = original_damage / 100 * self.return_damage,
