...
function item_halberd_aoe:OnSpellStart()
	...
	local Duration = target:IsRangedAttacker() and self:GetSpecialValueFor("disarm_range")
		or self:GetSpecialValueFor("disarm_melee")
	...
	for _, unit in pairs(All) do
		...
		if unit ~= target then
			...
			if not blocked then
				...
				local Duration = unit:IsRangedAttacker() and self:GetSpecialValueFor("disarm_range")
					or self:GetSpecialValueFor("disarm_melee")
...
function modifier_item_halberd_aoe:GetModifierPhysical_ConstantBlock()
	if
		self:GetAbility()
		and RollPseudoRandomPercentage(self.BlockChance, DOTA_PSEUDO_RANDOM_ITEM_VANGUARD, self:GetParent())
	then
