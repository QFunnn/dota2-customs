...
function modifier_item_mirror_shield_custom:IsPurgeException()
	return false
end
...
function modifier_item_mirror_shield_custom:OnIntervalThink()
	...
	if removed then
		...
		if not self.modifier_item_mirror_shield then
			self.modifier_item_mirror_shield =
				self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_item_mirror_shield", {})
		end
	end
end
