...
function modifier_item_ninja_gear_custom:IsPurgable()
	return false
end
function modifier_item_ninja_gear_custom:IsPurgeException()
	return false
end

function modifier_item_ninja_gear_custom:IsHidden()
	return true
end
...
function modifier_item_ninja_gear_custom:OnIntervalThink()
	...
	if health <= max then
		if not self:GetParent():HasModifier("modifier_item_ninja_gear_custom_buff") then
			self:GetParent()
				:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_item_ninja_gear_custom_buff", {})
		end
	end
end
...
function modifier_item_ninja_gear_custom_buff:IsPurgable()
	return false
end
