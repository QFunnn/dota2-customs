...
function item_ninja_gear_custom:OnSpellStart()
	...
	local particle =
		ParticleManager:CreateParticle("particles/items2_fx/smoke_of_deceit.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_ninja_gear_damage_penalty", { duration = duration })
end
...
function modifier_ninja_gear_custom_passive:IsHidden()
	return true
end
function modifier_ninja_gear_custom_passive:IsPurgable()
	return false
end
function modifier_ninja_gear_custom_passive:RemoveOnDeath()
	return false
end
...
function modifier_ninja_gear_custom:IsPurgable()
	return false
end
function modifier_ninja_gear_custom:GetPriority()
	return MODIFIER_PRIORITY_SUPER_ULTRA
end
function modifier_ninja_gear_custom:HeroEffectPriority()
	return MODIFIER_PRIORITY_SUPER_ULTRA
end
function modifier_ninja_gear_custom:GetTexture()
...
function modifier_ninja_gear_custom:OnCreated(args)
	...
	if self:GetParent():HasModifier("modifier_skill_hookah_master") then
		self:GetParent()
			:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_smoke_of_deceit_hookah_master_immunity", {})
	end
	self.radius = 1200
...
function modifier_ninja_gear_custom:OnIntervalThink()
	...
	local ParentPID = RealParent and RealParent.GetPlayerOwnerID and RealParent:GetPlayerOwnerID()
		or parent:GetPlayerOwnerID()
...
function modifier_ninja_gear_custom:GetModifierTotalDamageOutgoing_Percentage()
	if not self.parent:HasModifier("modifier_skill_hookah_master") then
		return
	end
	return 20
end
...
function modifier_ninja_gear_damage_penalty:IsPurgable()
	return false
end
function modifier_ninja_gear_damage_penalty:RemoveOnDeath()
	return false
end
function modifier_ninja_gear_damage_penalty:GetTexture()
	return "item_ninja_gear"
end
