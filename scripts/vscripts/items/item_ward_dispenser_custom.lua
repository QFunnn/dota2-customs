...
LinkLuaModifier(
	"modifier_item_ward_dispenser_custom_ward",
	"items/item_ward_dispenser_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_ward_dispenser_custom_sentry_ward",
	"items/item_ward_dispenser_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_ward_dispenser_custom_observer_ward",
	"items/item_ward_dispenser_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function item_ward_dispenser_custom:OnAbilityPhaseStart()
	...
	if
		...
		and Entities:FindByModelWithin(nil, "models/props_structures/radiant_statue001.vmdl", self.vTargetPosition, 600) ~= nil
...
function item_ward_dispenser_custom:OnSpellStart()
	...
	if self.type == "observer" then
		...
		hWard:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_kill",
			{ duration = self:GetSpecialValueFor("lifetime_observer") }
		)
	...
	elseif self.type == "sentry" then
		...
		hWard:AddNewModifier(
			...
			self,
			"modifier_kill",
			{ duration = self:GetSpecialValueFor("lifetime_sentry") }
		)
...
function modifier_item_ward_dispenser_custom_ward:IsPurgable()
	return false
end
function modifier_item_ward_dispenser_custom_ward:IsPurgeException()
	return false
end
...
function modifier_item_ward_dispenser_custom_ward:OnCreated()
	...
	if self:GetAbility() and self:GetAbility():entindex() then
		CustomNetTables:SetTableValue(
			"ward_type",
			tostring(self:GetAbility():entindex()),
			{ type = self:GetAbility().type }
		)
	end
...
function modifier_item_ward_dispenser_custom_observer_ward:IsHidden()
	return true
end
