...
LinkLuaModifier(
	"modifier_summon_flying_movement_aura",
	"modifiers/modifier_summon_flying_movement_aura",
...
function item_wraith_dominator:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_wraith_dominator_active", { duration = duration })
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_wraith_dominator_active_damage", { duration = duration })
end

modifier_item_wraith_dominator = class({})

function modifier_item_wraith_dominator:IsDebuff()
	return false
end
function modifier_item_wraith_dominator:IsHidden()
	return true
end
function modifier_item_wraith_dominator:IsPurgable()
	return false
end
function modifier_item_wraith_dominator:GetAttributes()
...
function modifier_item_wraith_dominator:OnCreated()
	if not IsServer() then
		return
	end
...
function modifier_item_wraith_dominator:OnIntervalThink()
	self:UpdateFlyingAura()
end

function modifier_item_wraith_dominator:UpdateFlyingAura()
	...
	if is_active and not parent:HasModifier("modifier_summon_flying_movement_aura") then
		parent:AddNewModifier(parent, ability, "modifier_summon_flying_movement_aura", {})
	elseif not is_active and not self:HasOtherFlyingAuraSource(parent) then
...
function modifier_item_wraith_dominator:HasOtherFlyingAuraSource(parent)
	for i = 0, parent:GetNumItemsInInventory() - 1 do
		local item = parent:GetItemInSlot(i)
		if item and item ~= self:GetAbility() and not item:IsInBackpack() then
			local name = item:GetAbilityName()
			if
				name == "item_summoner_crown_2"
				or name == "item_summoner_crown_3"
				or name == "item_wraith_dominator"
			then
				return true
			end
		end
...
function modifier_item_wraith_dominator:OnDestroy()
	if not IsServer() then
		return
	end
	local parent = self:GetParent()
...
function modifier_item_wraith_dominator:GetAuraEntityReject(target)
	-- A6: варды — не армейские призывы, дружественная аура их не баффает (по имени, IsWard() для них = false)
	if IsWardUnit(target) then
		return true
	end
...
function modifier_item_wraith_dominator_aura_buff:IsHidden()
	return false
end
function modifier_item_wraith_dominator_aura_buff:IsPurgable()
	return false
end
...
function modifier_item_wraith_dominator_aura_buff:AttackLandedModifier(params)
	if IsServer() then
		...
		if params.attacker == self:GetParent() then
			...
			if no_heal_target[params.target:GetUnitName()] then
				return
			end
...
function modifier_item_wraith_dominator_active:IsPurgable()
	return false
end
function modifier_item_wraith_dominator_active:IsHidden()
	return false
end
...
function modifier_item_wraith_dominator_active:OnIntervalThink()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/items5_fx/wraith_pact_pulses.vpcf",
	...
	for _, enemy in pairs(enemies) do
		if not enemy:HasModifier("modifier_oracle_false_promise_custom") then
			local fx = ParticleManager:CreateParticle(
				"particles/items5_fx/wraith_pact_pulses_target.vpcf",
...
function modifier_item_wraith_dominator_active:GetAuraEntityReject(target)
	if target:HasModifier("modifier_item_wraith_pact_custom_debuff") then
		return true
	else
		return false
	end
...
function modifier_item_wraith_dominator_active_debuff:IsHidden()
	return false
end
function modifier_item_wraith_dominator_active_debuff:IsPurgable()
	return false
end
...
function modifier_item_wraith_dominator_active_damage:IsHidden()
	return true
end
function modifier_item_wraith_dominator_active_damage:IsPurgable()
	return false
end
...
function modifier_item_wraith_dominator_active_damage:GetAuraEntityReject(target)
	-- A6: варды — не армейские призывы, дружественную damage-ауру не получают (по имени)
	if IsWardUnit(target) then
		return true
	end
	return false
...
function modifier_item_wraith_dominator_active_buff:IsHidden()
	return false
end
...
function modifier_item_wraith_dominator_active_buff:GetModifierDamageOutgoing_Percentage()
