...
function item_seer_stone_custom:OnSpellStart()
	...
	CreateModifierThinker(
		...
		{ duration = self:GetSpecialValueFor("duration"), radius = radius },
		self:GetCursorPosition(),
		self:GetCaster():GetTeamNumber(),
		false
	)
	...
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self:GetSpecialValueFor("duration") + 0.5, self:GetSpecialValueFor("radius"), 0)
	)
...
function modifier_item_seer_stone_custom:IsPurgable()
	return false
end
function modifier_item_seer_stone_custom:IsHidden()
	return true
end
