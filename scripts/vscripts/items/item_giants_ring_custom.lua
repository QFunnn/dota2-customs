...
function modifier_item_giants_ring_custom:IsPurgeException()
	return false
end
...
function modifier_item_giants_ring_custom:OnIntervalThink()
	...
	if self:GetParent():IsAlive() then
		...
		for _, enemy in pairs(enemies) do
			local damageTable = {
				...
				damage = self:GetParent():GetStrength() * self:GetAbility()
					:GetSpecialValueFor("pct_str_damage_per_second") * 0.5 / 100,
	...
	ParticleManager:SetParticleControl(
		effect_cast,
		1,
		Vector(self:GetAbility():GetSpecialValueFor("damage_radius"), 1, 1)
	)
