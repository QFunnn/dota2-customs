...
function item_bullwhip_custom:OnSpellStart()
	...
	local particle =
		ParticleManager:CreateParticle("particles/items4_fx/bull_whip.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	ParticleManager:SetParticleControl(
		particle,
		0,
		self:GetCaster():GetAbsOrigin() + self:GetCaster():GetForwardVector() * 1400
	)
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_bullwhip_custom_buff", { duration = duration })
...
function modifier_item_bullwhip_custom:IsHidden()
	return true
end

function modifier_item_bullwhip_custom:IsPurgable()
	return false
end
function modifier_item_bullwhip_custom:IsPurgeException()
	return false
end
...
function modifier_item_bullwhip_custom_buff:IsPurgable()
	return true
end
