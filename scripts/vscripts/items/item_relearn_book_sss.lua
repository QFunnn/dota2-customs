...
item_relearn_book_sss = class({})
function item_relearn_book_sss:OnChargeCountChanged(iCharges) end

function item_relearn_book_sss:OnSpellStart()
    ...
    if HeroBuilder:AddAbilitiesSelectionToSchedule(PlayerID, ABILITY_SELECTION_TYPE.RELEARN_SSS, self:GetName()) then
        EmitSoundOnClient("Item.TomeOfKnowledge", player)
        Util:RecordConsumableItem(PlayerID, "item_relearn_book_sss")
