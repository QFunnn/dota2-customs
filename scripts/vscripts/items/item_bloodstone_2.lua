...
function modifier_modifier_item_bloodstone_2_active:IsPurgable()
	return true
end
...
function modifier_item_bloodstone_2:OnDeathEvent(keys)
	if Rounds:RoundTypeIs(ROUND_TYPES.VOTING) and Rounds:StateIs(GAME_STATES.IN_ACTION) then
		return
	end
...
function modifier_item_bloodstone_2:TakeDamageScriptModifier(keys)
	...
	then
		...
		if
			...
			and (keys.damage_category == DOTA_DAMAGE_CATEGORY_SPELL or table_skills[keys.inflictor:GetAbilityName()])
			and keys.inflictor
		...
		then
			if keys.unit:IsIllusion() then
				...
				elseif keys.damage_type == DAMAGE_TYPE_MAGICAL and keys.unit.Script_GetMagicalArmorValue then
					keys.damage = keys.original_damage
						* (1 - GetReductionFromArmor(keys.unit:Script_GetMagicalArmorValue(false, nil)))
			...
			if keys.unit:IsCreep() then
				keys.attacker:Heal(
					math.max(keys.damage, 0) * self:GetAbility():GetSpecialValueFor("creep_lifesteal") * 0.01,
					keys.attacker
				)
			else
				keys.attacker:Heal(
					math.max(keys.damage, 0) * self:GetAbility():GetSpecialValueFor("hero_lifesteal") * 0.01,
					keys.attacker
				)
			end
		end
