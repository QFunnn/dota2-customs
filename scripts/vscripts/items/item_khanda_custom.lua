...
function modifier_item_khanda_custom:IsPurgable()
	return false
end
function modifier_item_khanda_custom:IsHidden()
	return true
end
function modifier_item_khanda_custom:IsPurgeException()
	return false
end
function modifier_item_khanda_custom:IsPurgable()
	return false
end
function modifier_item_khanda_custom:RemoveOnDeath()
	return false
end
...
function modifier_item_khanda_custom:OnTakeDamage(params)
	...
	if params.damage < self:GetAbility():GetSpecialValueFor("min_damage_to_activate") then
		return
	end
	...
	if self:GetParent():FindAllModifiersByName("modifier_item_khanda_custom")[1] ~= self then
		return
	end
	...
	local damage = self:GetAbility():GetSpecialValueFor("bonus_spell_damage")
		+ (
			...
			/ 100
			* self:GetAbility():GetSpecialValueFor("spell_crit_multiplier")
		)
	...
	params.unit:AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_item_khanda_custom_debuff",
		{ duration = self:GetAbility():GetSpecialValueFor("slow_duration") * (1 - params.unit:GetStatusResistance()) }
	)
...
function modifier_item_khanda_custom:GetModifierConstantHealthRegen()
	if self:GetAbility() then
...
function modifier_item_khanda_custom_debuff:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
	}
end

function modifier_item_khanda_custom_debuff:GetModifierMoveSpeedBonus_Percentage()
	return self:GetAbility():GetSpecialValueFor("slow")
end

function modifier_item_khanda_custom_debuff:CheckState()
	return {
