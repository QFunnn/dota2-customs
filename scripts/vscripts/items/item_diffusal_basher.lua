...
function item_diffusal_basher:OnSpellStart()
	...
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_diffusal_basher_debuff",
...
function modifier_item_diffusal_basher:IsHidden()
	return true
end
function modifier_item_diffusal_basher:IsPurgable()
	return false
end
function modifier_item_diffusal_basher:IsPurgeException()
	return false
end
function modifier_item_diffusal_basher:RemoveOnDeath()
	return false
end
...
function modifier_item_diffusal_basher:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_item_diffusal_basher:GetModifierProcAttack_BonusDamage_Physical(params)
	if self:GetParent():FindAllModifiersByName("modifier_item_diffusal_basher")[1] ~= self then
		return
	end
	if self:GetParent():GetUnitName() == "npc_dota_hero_faceless_void" then
		return
	end
	if self:GetParent():HasItemInInventory("item_basher") then
		return
	end
	if self:GetParent():HasItemInInventory("item_abyssal_blade") then
		return
	end
	...
	if
		...
		and not self:GetParent():HasModifier("modifier_item_diffusal_basher_cooldown")
		and not self:GetParent():IsIllusion()
	then
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_item_diffusal_basher_cooldown",
			{ duration = cooldown_bash }
		)
...
function modifier_item_diffusal_basher_cooldown:IsHidden()
	return true
end
function modifier_item_diffusal_basher_cooldown:IsPurgable()
	return false
end
function modifier_item_diffusal_basher_cooldown:RemoveOnDeath()
	return false
end