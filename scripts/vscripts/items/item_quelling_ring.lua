...
function modifier_item_quelling_ring:IsHidden()
	return true
end

function modifier_item_quelling_ring:IsPurgable()
	return false
end
function modifier_item_quelling_ring:IsPurgeException()
	return false
end

function modifier_item_quelling_ring:GetAttributes()
...
function modifier_item_quelling_ring:GetModifierProcAttack_BonusDamage_Physical(keys)
	...
	then
		if self:GetParent():FindAllModifiersByName("modifier_item_quelling_ring")[1] ~= self then
			return
		end
