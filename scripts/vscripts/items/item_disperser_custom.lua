...
function item_disperser_custom:GetAOERadius()
...
function item_disperser_custom:OnTarget(target)
	if not IsServer() then
		return
	end
	if target:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		...
		if not target:IsHero() then
			target:AddNewModifier(
				...
				self,
				"modifier_rooted",
				{ duration = self:GetSpecialValueFor("purge_root_duration") }
			)
		end
	end
end
...
function modifier_item_disperser_custom:IsHidden()
	return true
end
function modifier_item_disperser_custom:IsPurgable()
	return false
end
function modifier_item_disperser_custom:IsPurgeException()
	return false
end
function modifier_item_disperser_custom:RemoveOnDeath()
	return false
end
function modifier_item_disperser_custom:GetAttributes()
...
function modifier_item_disperser_custom:GetModifierPreAttack_BonusDamage(params)
...
function modifier_item_disperser_custom:AttackLandedModifier(params)
	if params.attacker == self.parent then
		...
		if self.parent:FindAllModifiersByName("modifier_item_disperser_custom")[1] ~= self then
			return
		end
		local enemies = FindUnitsInRadius(
