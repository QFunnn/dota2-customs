...
function item_force_field_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_force_field_custom_buff", { duration = duration })
	self:GetCaster():EmitSound("Item.ForceField.Cast")
end

modifier_item_force_field_custom = class({})

function modifier_item_force_field_custom:IsHidden()
	return true
end

function modifier_item_force_field_custom:IsPurgable()
	return false
end
function modifier_item_force_field_custom:IsPurgeException()
	return false
end
...
function modifier_item_force_field_custom_buff:IsPurgable()
	return false
end
...
function modifier_item_force_field_custom_buff:OnTakeDamage(params)
	...
	if
		bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS
