...
function item_eye_of_thor:OnSpellStart()
	...
	for _, enemy in pairs(heroes) do
		...
		ParticleManager:SetParticleControl(
			particle,
			0,
			Vector(enemy:GetAbsOrigin().x, enemy:GetAbsOrigin().y, enemy:GetAbsOrigin().z)
		)
		...
		enemy:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_item_eye_of_thor_chain",
			{ starting_unit_entindex = enemy:entindex() }
		)
	end
end
...
function modifier_item_eye_of_thor_buff:IsPurgable()
	return false
end
...
function modifier_item_eye_of_thor_buff:OnIntervalThink()
	...
	AddFOWViewer(
		...
		self:GetParent():GetDayTimeVisionRange(),
		0.1,
		false
	)
end
...
function modifier_item_eye_of_thor:IsHidden()
	return true
end
function modifier_item_eye_of_thor:IsPurgable()
	return false
end
function modifier_item_eye_of_thor:IsPurgeException()
	return false
end
function modifier_item_eye_of_thor:GetAttributes()
...
function modifier_item_eye_of_thor_chain:IsHidden()
	return true
end
function modifier_item_eye_of_thor_chain:IsPurgable()
	return false
end
function modifier_item_eye_of_thor_chain:RemoveOnDeath()
	return false
end
function modifier_item_eye_of_thor_chain:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
