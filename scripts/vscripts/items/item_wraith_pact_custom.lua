...
LinkLuaModifier(
	"modifier_item_wraith_pact_custom_debuff",
	"items/item_wraith_pact_custom.lua",
...
function item_wraith_pact_custom:OnSpellStart()
	...
	local hTotem =
		CreateUnitByName("npc_dota_item_wraith_pact_totem", point, true, caster, caster, caster:GetTeamNumber())
...
modifier_item_wraith_pact_custom = class({
	...
	GetModifierHealthBonus = function(self)
		return self.BonusHealth or 0
	end,
	GetModifierManaBonus = function(self)
		return self.BonusMana or 0
	end,
	...
	GetModifierAura = function(self)
		return "modifier_item_wraith_pact_custom_aura"
	end,
	...
	GetAuraEntityReject = function(self, target)
		if
			target:HasModifier("modifier_item_vladmir_aura")
			or target:HasModifier("modifier_item_wraith_dominator_aura_buff")
		then
			return true
		else
			return false
...
modifier_item_wraith_pact_custom_aura = class({
	...
	GetModifierConstantManaRegen = function(self)
		return self.BonusManaRegen or 0
	end,
	GetModifierBaseDamageOutgoing_Percentage = function(self)
		return self.BonusDamagePct or 0
	end,
	...
	AttackLandedModifier = function(self, event)
		...
		if event.attacker == self:GetParent() then
			...
			if no_heal_target[event.target:GetUnitName()] then
				return
			end
...
modifier_item_wraith_pact_custom_totem = class({
	...
	GetModifierAura = function(self)
		return "modifier_item_wraith_pact_custom_debuff"
	end,
	GetAuraDuration = function(self)
		return 0.5
	end,
...
modifier_item_wraith_pact_custom_debuff = class({
	...
	GetModifierTotalDamageOutgoing_Percentage = function(self)
		return -self.DamagePenalty or 0
	end,
