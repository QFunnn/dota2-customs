	...
	if not is_spent then
		if self and self:GetCaster() then
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_item_consecrated_wraps_speed_buff",
...
function modifier_item_evasion_cape_passive:IsHidden()
	return true
end
function modifier_item_evasion_cape_passive:IsPurgable()
	return false
end
function modifier_item_evasion_cape_passive:IsPurgeException()
	return false
end
function modifier_item_evasion_cape_passive:OnCreated()
	...
	self.modifier_item_consecrated_wraps =
		self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_item_consecrated_wraps", {})
end
