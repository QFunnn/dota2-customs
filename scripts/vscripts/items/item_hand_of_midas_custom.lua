...
LinkLuaModifier("modifier_item_hand_of_midas_custom", "items/item_hand_of_midas_custom", LUA_MODIFIER_MOTION_NONE)
...
function item_hand_of_midas_custom:OnSpellStart(new_target)
	...
	if self:CastFilterResultTarget(target) ~= UF_SUCCESS then
		return
	end
	local ability = self
	...
	local midas_particle =
		ParticleManager:CreateParticle("particles/items2_fx/hand_of_midas.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
...
function modifier_item_hand_of_midas_custom:RemoveOnDeath()
	return false
end
