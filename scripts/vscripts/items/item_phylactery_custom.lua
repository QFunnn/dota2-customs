...
function modifier_item_phylactery_custom:IsPurgable()
	return false
end
function modifier_item_phylactery_custom:IsHidden()
	return true
end
function modifier_item_phylactery_custom:IsPurgeException()
	return false
end
function modifier_item_phylactery_custom:IsPurgable()
	return false
...
function modifier_item_phylactery_custom:OnTakeDamage(params)
	...
	if self:GetParent():HasModifier("modifier_item_khanda_custom") then
		return
	end
	if self:GetParent():FindAllModifiersByName("modifier_item_phylactery_custom")[1] ~= self then
		return
	end

	if self:GetParent():HasModifier("modifier_item_ban_hammer") then
		return
	end
	...
	params.unit:AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_item_phylactery_custom_debuff",
...
function modifier_item_phylactery_custom:GetModifierConstantHealthRegen()
	if self:GetAbility() then
