...
LinkLuaModifier("modifier_item_expiriance_aura", "items/d_items/book_exp", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_expiriance_aura_cd", "items/d_items/book_exp", LUA_MODIFIER_MOTION_NONE)

modifier_item_expiriance_aura_cd = class({})
function modifier_item_expiriance_aura_cd:IsHidden()
	return true
end
function modifier_item_expiriance_aura_cd:IsDebuff()
	return false
end
function modifier_item_expiriance_aura_cd:IsPurgable()
	return false
end
function modifier_item_expiriance_aura_cd:RemoveOnDeath()
	return false
end
function modifier_item_expiriance_aura_cd:OnCreated(kv)
	if not IsServer() then
		return
	end
	self:StartIntervalThink(FrameTime())
end
function modifier_item_expiriance_aura_cd:OnIntervalThink()
	self:ForceRefresh()
...
function item_expiriance_aura:OnSpellStart()
	if IsServer() then
		...
		for _, Hero in pairs(Heroes) do
			Hero:AddNewModifier(self.caster, self, "modifier_item_expiriance_aura", { duration = self.duration })
		end
...
function modifier_item_expiriance_aura:OnIntervalThink()
	self:ForceRefresh()
	self:StartIntervalThink(-1)
end
