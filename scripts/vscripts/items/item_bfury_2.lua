...
function modifier_item_bfury_2:AllowIllusionDuplicate()
	return false
end
function modifier_item_bfury_2:IsPurgable()
	return false
end
function modifier_item_bfury_2:RemoveOnDeath()
	return false
end
function modifier_item_bfury_2:IsHidden()
	return true
end
function modifier_item_bfury_2:GetAttributes()
...
function modifier_item_bfury_2:GetModifierPreAttack_BonusDamage(keys)
	if _G.Players and _G.Players.QueueAttackBonus and keys and keys.attacker and keys.target then
		_G.Players:QueueAttackBonus(keys.attacker, keys.target, self.bonus_damage, "item_bfury_2", DAMAGE_TYPE_PHYSICAL)
	end
...
function modifier_item_bfury_2:GetModifierProcAttack_BonusDamage_Physical(keys)
	...
	if
		(self.parent:FindAllModifiersByName("modifier_item_bfury_2")[1] == self)
		and keys.target
...
function modifier_item_bfury_2:AttackLandedModifier(keys)
	...
	if keys.target then
		if keys.target.IsOther and keys.target:IsOther() then
			return
		end
		if
			keys.target.IsSummoned
			and keys.target:IsSummoned()
			and keys.target.GetBaseMoveSpeed
			and keys.target:GetBaseMoveSpeed() == 0
		then
			return
		end
	end
	...
	if keys.attacker:GetTeam() == keys.target:GetTeam() then
		return
	end
	...
	if
		keys.attacker:HasAbility("ursa_fury_swipes")
		and keys.target:HasModifier("modifier_ursa_fury_swipes_damage_increase")
	then
