...
LinkLuaModifier(
	"modifier_item_spear_of_mordiggian_cooldown",
	"items/item_spear_of_mordiggian",
...
function item_spear_of_mordiggian:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_item_spear_of_mordiggian_cooldown") then
		return
	end
	for i = 0, 5 do
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_spear_of_mordiggian_cooldown", { duration = 0.036 })
...
function modifier_item_spear_of_mordiggian_cooldown:IsHidden()
	return true
end
function modifier_item_spear_of_mordiggian_cooldown:IsPurgable()
	return false
end

modifier_item_spear_of_mordiggian = class({})

function modifier_item_spear_of_mordiggian:IsHidden()
	return true
end
function modifier_item_spear_of_mordiggian:IsPurgable()
	return false
end
function modifier_item_spear_of_mordiggian:IsPurgeException()
	return false
end
...
function modifier_item_spear_of_mordiggian_active:IsDebuff()
	return false
end
function modifier_item_spear_of_mordiggian_active:IsPurgable()
	return false
end
function modifier_item_spear_of_mordiggian_active:GetTexture()
	return "spear_of_mordiggian_1"
end
...
function modifier_item_spear_of_mordiggian_active:OnIntervalThink()
	local modifier_item_armlet_unholy_strength = self:GetParent()
		:FindModifierByName("modifier_item_armlet_unholy_strength")
	...
	if self.timer < 0.6 then
		...
		if not self:GetHealingDisable(self:GetParent()) then
			self:GetParent()
				:SetHealth(self:GetParent():GetHealth() + ((22 * self.strength_think) - modify_health_check))
		end
		return
	end
	self:StartIntervalThink(0.11)
	if self:GetParent():IsIllusion() then
		return
	end
	self:GetParent():ModifyHealth(
		math.max(1, self:GetParent():GetHealth() - self.unholy_health_drain),
		self:GetAbility(),
		false,
