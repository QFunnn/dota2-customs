...
function item_havoc_hammer_custom:OnSpellStart()
	...
	for _, unit in pairs(units) do
		...
		unit:AddNewModifier(
			...
			{ duration = 3 * (1 - unit:GetStatusResistance()) }
		)
	end
end
...
function modifier_item_havoc_hammer_custom:IsHidden()
	return true
end

function modifier_item_havoc_hammer_custom:IsPurgable()
	return false
end
function modifier_item_havoc_hammer_custom:IsPurgeException()
	return false
end
