...
function item_heaven_shield:OnSpellStart()
	...
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_heaven_shield_active",
...
function modifier_item_heaven_shield:IsHidden()
	return true
end
function modifier_item_heaven_shield:IsPurgable()
	return false
end
function modifier_item_heaven_shield:IsPurgeException()
	return false
end
...
function modifier_item_heaven_shield:GetModifierPhysical_ConstantBlock()
	if self:GetAbility() then
		if self:GetParent():FindAllModifiersByName("modifier_item_heaven_shield")[1] ~= self then
			return
		end
...
function modifier_item_heaven_shield:GetModifierHPRegenAmplify_Percentage()
	if
		self:GetCaster():HasModifier("modifier_item_sange")
		...
		or self:GetCaster():HasModifier("modifier_item_kaya_and_sange")
	then
		return
	end
	return self:GetAbility():GetSpecialValueFor("hp_regen_amp")
...
function modifier_item_heaven_shield_active:IsPurgable()
	return false
end
function modifier_item_heaven_shield_active:IsPurgeException()
	return false
end
