...
function item_dagon_custom:OnSpellStart()
	...
	if self:GetCursorTarget():IsMagicImmune() then
		return
	end
	...
	local dagon_pfx =
		ParticleManager:CreateParticle("particles/items_fx/dagon.vpcf", PATTACH_RENDERORIGIN_FOLLOW, self:GetCaster())
...
function item_dagon_5_custom:OnSpellStart()
	...
	self:GetCursorTarget():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_item_dagon_custom_break",
		{ duration = (1 - self:GetCursorTarget():GetStatusResistance()) * self:GetSpecialValueFor("duration") }
	)
	ApplyDamage({
		attacker = self:GetCaster(),
		victim = self:GetCursorTarget(),
...
function modifier_item_dagon_custom_break:IsHidden()
	return false
end
function modifier_item_dagon_custom_break:IsPurgable()
	return false
end
function modifier_item_dagon_custom_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_item_dagon_custom_break:GetEffectName()
