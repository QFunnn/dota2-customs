...
function extra_creature_add:IsRefreshable()
	return false
end
...
function extra_creature_add:CastFilterResult()
	...
	if
		...
		or #Players:GetAllActivePlayers(true) <= PLAYERS_WHEN_CAN_SUMMON_CREEPS
	then
		return UF_SUCCESS
	end
...
function extra_creature_add:OnSpellStart()
	...
	if caster:HasModifier("modifier_arc_warden_tempest_double_lua") then
		return
	end
