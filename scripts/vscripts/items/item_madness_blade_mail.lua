...
function item_madness_blade_mail:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_item_madness_blade_mail_buff", { duration = duration })
...
function modifier_item_madness_blade_mail:IsPurgable()
	return false
end
function modifier_item_madness_blade_mail:IsPurgeException()
	return false
end

function modifier_item_madness_blade_mail:GetAttributes()
...
function modifier_item_madness_blade_mail:OnAttackLanded(params)
	...
	if
		...
		and params.target == self:GetParent()
		and not params.attacker:IsMagicImmune()
	then
		...
		local damage_return = self:GetAbility():GetSpecialValueFor("damage_return_passive_perc")
				* params.original_damage
				/ 100
			+ self:GetAbility():GetSpecialValueFor("damage_return_passive")
		ApplyDamage({
...
function modifier_item_madness_blade_mail:GetModifierProperty_PhysicalLifesteal(params)
	return self:GetAbility():GetSpecialValueFor("bonus_lifesteal")
end
...
function modifier_item_madness_blade_mail_buff:GetModifierMoveSpeedBonus_Percentage()
	return self:GetAbility():GetSpecialValueFor("movespeed_active")
end

function modifier_item_madness_blade_mail_buff:GetModifierSlowResistance_Stacking()
	return self:GetAbility():GetSpecialValueFor("slow_resistance")
end
...
function modifier_item_madness_blade_mail_buff:OnTakeDamage(keys)
	...
	if
		keys.unit == self:GetParent()
		and not keys.attacker:IsBuilding()
		and keys.attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber()
		and bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS
		and bit.band(keys.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) ~= DOTA_DAMAGE_FLAG_REFLECTION
