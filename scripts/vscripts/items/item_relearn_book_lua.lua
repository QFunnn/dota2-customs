...
function item_relearn_book_lua:OnChargeCountChanged(iCharges) end
...
function item_relearn_book_lua:OnSpellStart()
    ...
    if HeroBuilder:AddAbilitiesSelectionToSchedule(PlayerID, ABILITY_SELECTION_TYPE.RELEARN, self:GetName()) then
        EmitSoundOnClient("Item.TomeOfKnowledge", player)
        Util:RecordConsumableItem(PlayerID, "item_relearn_book_lua")
