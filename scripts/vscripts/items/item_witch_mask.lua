...
function modifier_item_witch_mask:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_item_witch_mask:IsHidden()
	return true
end
function modifier_item_witch_mask:IsPurgable()
	return false
end
function modifier_item_witch_mask:IsPurgeException()
	return false
end
...
function modifier_item_witch_mask:OnTakeDamage(params)
	...
	then
		...
		if self:GetParent():FindAllModifiersByName("modifier_item_witch_mask")[1] ~= self then
			return
		end
	...
	if (self:GetParent():GetAbsOrigin() - params.unit:GetAbsOrigin()):Length2D() > 1200 then
		return
	end
	if self:GetParent():HasModifier("modifier_item_witch_mask_cooldown") then
		return
	end
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_item_witch_mask_cooldown",
		{ duration = self:GetAbility():GetSpecialValueFor("debuff_cooldown") }
	)
	params.unit:AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_item_witch_mask_debuff",
...
function modifier_item_witch_mask_cooldown:IsPurgable()
	return false
end
function modifier_item_witch_mask_cooldown:IsPurgeException()
	return false
end
function modifier_item_witch_mask_cooldown:RemoveOnDeath()
	return false
end
function modifier_item_witch_mask_cooldown:IsDebuff()
	return true
end
function modifier_item_witch_mask_cooldown:GetTexture()
	return "item_witch_mask"
end