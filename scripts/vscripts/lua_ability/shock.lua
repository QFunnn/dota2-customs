...
function Shock(event)
	...
	local cone_units = FindUnitsInRadius(
		DOTA_TEAM_BADGUYS,
		target:GetAbsOrigin(),
		nil,
		1000,
	...
	for _, unit in pairs(cone_units) do
		if targets_shocked < targets then
			if unit ~= target then
				if
					(unit.player == nil or unit.player == caster:GetOwner():GetPlayerID())
				...
				then
					...
					ParticleManager:SetParticleControl(
						...
						0,
						Vector(
							caster:GetAbsOrigin().x,
							caster:GetAbsOrigin().y,
							caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z
						)
					)
					ParticleManager:SetParticleControl(
						lightningBolt,
						1,
						Vector(origin.x, origin.y, origin.z + unit:GetBoundingMaxs().z)
					)
