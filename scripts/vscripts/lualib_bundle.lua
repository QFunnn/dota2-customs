...
local function __TS__ArrayReduce(self, callbackFn, ...)
	...
	for i = k + 1, len do
		accumulator = callbackFn(nil, accumulator, self[i], i - 1, self)
	end
	return accumulator
end
...
local function __TS__ArraySetLength(self, length)
	if length < 0 or length ~= length or length == math.huge or math.floor(length) ~= length then
		error("invalid array length: " .. tostring(length), 0)
	end
...
do
	...
	function __TS__Promise.prototype.____constructor(self, executor)
		...
		local success, ____error = ____pcall(executor, nil, function(____, v)
			return self:resolve(v)
		end, function(____, err)
	...
	function __TS__Promise.prototype.resolve(self, value)
		if isPromiseLike(value) then
			return value:addCallbacks(function(____, v)
				return self:resolve(v)
			end, function(____, err)
				return self:reject(err)
			end)
...
do
	...
	function __TS__AsyncAwaiter(generator)
		return __TS__New(__TS__Promise, function(____, resolve, reject)
			...
			local success, resultOrError = coresume(asyncCoroutine, function(____, v)
				resolved = true
...
do
	...
	local function descriptorIndex(self, key)
		return __TS__DescriptorGet(self, getmetatable(self), key)
	end
	local function descriptorNewIndex(self, key, value)
		return __TS__DescriptorSet(self, getmetatable(self), key, value)
	end
		...
		while i >= 0 do
			...
			if decorator ~= nil then
				...
				elseif desc == true then
					local value = rawget(target, key)
					local descriptor = __TS__ObjectGetOwnPropertyDescriptor(target, key)
						or { configurable = true, writable = true, value = value }
					local desc = decorator(nil, target, key, descriptor) or descriptor
					local isSimpleValue = desc.configurable == true
						and desc.writable == true
						and not desc.get
						and not desc.set
					if isSimpleValue then
						rawset(target, key, desc.value)
					else
						__TS__SetDescriptor(target, key, __TS__ObjectAssign({}, descriptor, desc))
					end
...
local function __TS__DecorateParam(paramIndex, decorator)
	return function(____, target, key)
		return decorator(nil, target, key, paramIndex)
	end
end
...
local function __TS__FunctionBind(fn, ...)
	...
	return function(____, ...)
		local args = { ... }
		__TS__ArrayUnshift(args, __TS__Unpack(boundArgs))
		...
		return function(...)
			...
			return {
				____coroutine = coroutine.create(function()
					return fn(__TS__Unpack(args, 1, argsLength))
				end),
	...
	function __TS__ParseInt(numberString, base)
		if base == nil then
			...
			if hexMatch ~= nil then
				base = 16
				numberString = (__TS__Match(hexMatch, "-")) and "-" .. __TS__StringSubstring(numberString, #hexMatch)
		...
		local allowedDigits = base <= 10 and __TS__StringSubstring(parseIntBasePattern, 0, base)
			or __TS__StringSubstring(parseIntBasePattern, 0, 10 + 2 * (base - 10))
...
local function __TS__NumberToFixed(self, fractionDigits)
	...
	return string.format(("%." .. tostring(f)) .. "f", self)
end
...
local function __TS__ObjectDefineProperty(target, key, desc)
	...
	if hasGetterOrSetter then
		if value ~= nil then
			error("Cannot redefine property: " .. tostring(key), 0)
		end
		descriptor = desc
	else
...
local function __TS__PromiseAll(iterable)
	...
	return __TS__New(__TS__Promise, function(____, resolve, reject)
		for index, promise in pairs(toResolve) do
			promise["then"](promise, function(____, data)
				results[index + 1] = data
				numToResolve = numToResolve - 1
				if numToResolve == 0 then
					resolve(nil, results)
				end
			end, function(____, reason)
				reject(nil, reason)
			end)
		end
	end)
...
local function __TS__PromiseAllSettled(iterable)
	...
	return __TS__New(__TS__Promise, function(____, resolve)
		for index, promise in pairs(toResolve) do
			promise["then"](promise, function(____, data)
...
local function __TS__PromiseRace(iterable)
	...
	return __TS__New(__TS__Promise, function(____, resolve, reject)
		for ____, promise in ipairs(pending) do
			promise["then"](promise, function(____, value)
				return resolve(nil, value)
			end, function(____, reason)
				return reject(nil, reason)
			end)
		end
	end)
end
	...
	if _G.__TS__originalTraceback == nil then
		...
		debug.traceback = function(thread, message, level)
			...
			local result = string.gsub(trace, "(%S+)%.lua:(%d+)", function(file, line)
				return replacer(nil, file .. ".lua", file .. ".ts", line)
			end)
			...
			result = string.gsub(result, '(%[string "[^"]+"%]):(%d+)', function(file, line)
				return stringReplacer(nil, file, line)
			end)
			return result
		end
	end
...
local function __TS__StringPadEnd(self, maxLength, fillString)
	...
	return self .. string.sub(fillString, 1, math.floor(maxLength))
end
...
do
	...
	function __TS__StringReplace(source, searchValue, replaceValue)
		...
		local replacement = type(replaceValue) == "string" and replaceValue
			or replaceValue(nil, searchValue, startPos - 1, source)
...
do
	...
	function __TS__StringReplaceAll(source, searchValue, replaceValue)
		if type(replaceValue) == "string" then
			local concat = table.concat(__TS__StringSplit(source, searchValue), replaceValue)
...
local function __TS__UsingAsync(self, cb, ...)
	...
	return __TS__AsyncAwaiter(function(____awaiter_resolve)
		local thrownError
		local ok, result = xpcall(function()
			return cb(nil, __TS__Unpack(args))
		end, function(err)
			thrownError = err
			return thrownError
		end)
