...
function upgrade:GetBroodScepter(player, type)
	local talent_table = {}
	if not ingame_talents["broodmother_spiders"] then
		return talent_table
	end
...
function upgrade:MainEpicAllowed(player, name)
	...
	for name, talent in pairs(ingame_talents[player:GetUnitName()]) do
		if
			talent["skill_number"] == skill
			and talent["rarity"] == "purple"
			and upgrade:GetMaxLevel(talent) == 1
			and not player:HasTalent(name)
		then
			return false
		end
	end
...
function upgrade:CheckProMod(player, data)
	...
	if not PRO_MOD_ALLOWED then
		return true
	end
	if not PRO_MOD_ALLOWED[player:GetUnitName()] then
		return true
	end
	if #PRO_MOD_ALLOWED[player:GetUnitName()] == 1 and HTTP.playersData[player:GetId()].firstOrangeTalent then
		return true
	end
...
function upgrade:find_legendary(player)
	if not players[player:GetId()] then
		return
	end
	if not ingame_talents[player:GetUnitName()] then
		return
	end
	...
	for name, data in pairs(ingame_talents[player:GetUnitName()]) do
		if data["rarity"] == "orange" then
			...
			new_data["name"] = name
			if
				not player:HasTalent(name)
				and upgrade:CheckProMod(player, data)
				and (not player.banned_talents or not player.banned_talents[name])
			then
	...
	table.sort(choise_table, function(x, y)
		return y["skill_number"] > x["skill_number"]
	end)
...
function upgrade:GetPatrol(name)
	...
	repeat
		r_2 = RandomInt(1, #possible_table)
	until r_2 ~= r_1
