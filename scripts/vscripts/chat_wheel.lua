...
_G.Sound_list = {
	...
	["general_ru"] = {
		...
		{ 75, "Voice.General_rus_75", 1500 },
		{ 76, "Voice.General_rus_76", 800 },
		{ 77, "Voice.General_rus_77", 800 },
		{ 78, "Voice.General_rus_78", 1500, 1 },
		{ 79, "Voice.General_rus_79", 1500, 1 },
		{ 80, "Voice.General_rus_80", 800, 1 },
		{ 81, "Voice.General_rus_81", 800, 1 },
		{ 82, "Voice.General_rus_82", 800, 1 },
		{ 83, "Voice.General_rus_83", 1500, 1 },
	},
...
function chat_wheel:SelectChatWheel(keys)
	...
	if shop and shop.UnmarkItemNew then
		shop:UnmarkItemNew(keys.PlayerID, keys.sound_name)
	end
end
	...
	for i = 1, 8 do
		if player_table then
			if player_table.chat_wheel then
				if
					player_table.chat_wheel[tostring(i)]
					and player_table.chat_wheel[tostring(i)] ~= 0
					and player_table.chat_wheel[tostring(i)] ~= "0"
				then
...
function chat_wheel:SelectHeroVO(keys)
	...
	if steam_id ~= "232290025" and steam_id ~= "177411785" then
		if
			not table_data
			or table_data.subscribed == 0
			or not table_data.heroes_data[hero_name]
			or table_data.heroes_data[hero_name].has_level == 0
			or not chat_wheel:HasHeroTier(tier_hero, sound_name, hero_name)
		then
	...
	if player.sound_use_one > 0 then
		...
		Timers:CreateTimer({
			...
			endTime = 1,
			callback = function()
				if player.sound_use_two <= 0 then
					return nil
				end
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"chat_dota_sound",
		{
			...
			sound_name = sound_string,
			sound_name_global = sound_name,
			tier = tier_hero,
