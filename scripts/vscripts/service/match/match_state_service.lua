...
local function GetDuelStats(playerId)
	local record = PvpRecordBook:Get(playerId)
	return {
		win = record.win,
		lose = record.lose,
	}
...
local function GetRelearnBookStats(playerId)
	local bookStats = GameMode:GetMatch():GetPlayerBookStats(playerId)
...
local function Save()
	...
	local matchState = {
		players = {},
		round = GameMode:GetMatch():GetCurrentRound().roundNumber,
...
--         HeroBuilderService:InitPlayerHeroDebug(newHero)
--     end)
-- end
...
--             HeroBuilderService:AddAbility(hero:GetPlayerID(), abilityInfo.name, abilityInfo.level, abilityInfo.cooldown, hero)
--         end
--     end
-- end
