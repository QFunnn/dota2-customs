...
local function copy_map_keys(source)
	local result = {}
	for ability_name, _ in pairs(source or {}) do
		result[ability_name] = 1
	end
	return result
end

local gameplay_kv = {
	no_spell_amp = copy_map_keys(GAMEPLAY_KV.NoSpellAmpAbilities),
	not_rearmable = copy_map_keys(GAMEPLAY_KV.NotRearmableAbilities),
	torture_pipe = copy_map_keys(GAMEPLAY_KV.DotAbilities),
	reflection = copy_map_keys(GAMEPLAY_KV.ReflectionAbilities),
}
...
function AbilityKVService:SendAbilitiesItemsKV(event)
	...
	if not player_id or not PlayerResource:IsValidPlayer(player_id) then
		return
	end

	local player = PlayerResource:GetPlayer(player_id)
	if not player or player:IsNull() or (player.kv_sended and not IsInToolsMode()) then
		return
	end

	CustomGameEventManager:Send_ServerToPlayer(player, "set_abilities_items_kv", {
		abilities_items_kv = abilities_items_kv,
		gameplay_kv = gameplay_kv,
	})
...
function AbilityKVService:FormatKV(kv)
	...
	if kv.AbilityValues then
		...
		kv.AbilityValues = table.filter(kv.AbilityValues, function(v, k)
			return kv.HasScepterUpgrade == 1
				or kv.HasShardUpgrade == 1
				or k:lower():find("scepter")
				or k:lower():find("shard")
				or contains_key(v, "shard")
				or contains_key(v, "scepter")
		end)
