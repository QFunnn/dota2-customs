...
function DestinyHero.InitGameplay(self, iPlayCount)
    EachPlayer(
        ...
        function(____, iPlayerID)
            local tUserDestinyHero = NetEventData:GetTableValue(
                "service",
                "UserDestinyHero_" .. tostring(iPlayerID)
            ) or ({})
            ...
            if table.count(tEffect.tAffix) > 0 then
                ...
                if IsValid(hHero) then
                    ...
                    if hBuff then
                        print((("[DestinyHero]: " .. tostring(iPlayerID)) .. " 添加命座英雄buff: ") .. json.encode(tEffect.tAffix))
                    end
                end
            end
            ...
            do
                ...
                while i <= iItemLevel do
                    local tEffect = Str2RewardMap(
                        _G,
                        tDestinyHeroCfg["effect_" .. tostring(i)]
                    )
...
DestinyHero = __TS__DecorateLegacy(
    {module(_G)},
    DestinyHero
)