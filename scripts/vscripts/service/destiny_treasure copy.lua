	...
	EachPlayer(_G, function(____, iPlayerID)
		...
		if table.count(tEffect.tAffix) > 0 then
			...
			if IsValid(hHero) then
				...
				if hBuff then
					print(
						(("[DestinyTreasure]: " .. tostring(iPlayerID)) .. " 添加命座宝物buff: ")
		...
		do
			...
			do
				...
				while i <= iItemLevel do
					local tEffect = Str2RewardMap(_G, tDestinyTreasureCfg["effect_" .. tostring(i)])
...
DestinyTreasure = __TS__DecorateLegacy({ module(_G) }, DestinyTreasure)