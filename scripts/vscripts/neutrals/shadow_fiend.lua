...
LinkLuaModifier(
	"modifier_boss_shadow_fiend_passive_shadowraze_thinker",
	"neutrals/shadow_fiend",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_boss_shadow_fiend_passive_shadowraze_debuff",
	"neutrals/shadow_fiend",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_boss_shadow_fiend_passive_shadowraze:IsHidden()
	return true
end
function modifier_boss_shadow_fiend_passive_shadowraze:IsPurgable()
	return false
end
function modifier_boss_shadow_fiend_passive_shadowraze:IsPurgeException()
	return false
end
...
function modifier_boss_shadow_fiend_passive_shadowraze_thinker:OnCreated()
	...
	ParticleManager:SetParticleControl(
		...
		1,
		Vector(
			self:GetAbility():GetSpecialValueFor("coil_radius"),
			0,
			-self:GetAbility():GetSpecialValueFor("coil_radius") + 65
		)
	)
...
function modifier_boss_shadow_fiend_passive_shadowraze_thinker:OnDestroy()
	...
	for _, unit in pairs(units) do
		...
		local modifier_boss_shadow_fiend_passive_shadowraze_debuff =
			unit:FindModifierByName("modifier_boss_shadow_fiend_passive_shadowraze_debuff")
		...
		ApplyDamage({
			...
			damage = unit:GetMaxHealth() / 100 * damage_percent,
	...
	local particle_raze_fx = ParticleManager:CreateParticle(
	...
	ParticleManager:SetParticleControl(
		particle_raze_fx,
		1,
		Vector(self:GetAbility():GetSpecialValueFor("coil_radius"), 1, 1)
	)
	ParticleManager:ReleaseParticleIndex(particle_raze_fx)
end

modifier_boss_shadow_fiend_passive_shadowraze_debuff = class({})
function modifier_boss_shadow_fiend_passive_shadowraze_debuff:IsPurgable()
	return false
end
...
LinkLuaModifier(
	"modifier_boss_shadow_fiend_passive_dark_cyclone_buff",
	"neutrals/shadow_fiend",
...
function modifier_boss_shadow_fiend_passive_dark_cyclone:IsHidden()
	return true
end
function modifier_boss_shadow_fiend_passive_dark_cyclone:IsPurgable()
	return false
end
function modifier_boss_shadow_fiend_passive_dark_cyclone:IsPurgeException()
	return false
end
...
function modifier_boss_shadow_fiend_passive_dark_cyclone:Start()
	...
	if self:GetParent():HasModifier("modifier_boss_shadow_fiend_passive_dark_cyclone_buff") then
		return
	end
...
function modifier_boss_shadow_fiend_passive_dark_cyclone_buff:IsHidden()
	return false
end
function modifier_boss_shadow_fiend_passive_dark_cyclone_buff:IsPurgable()
	return false
end
...
function modifier_boss_shadow_fiend_passive_dark_cyclone_buff:HorizontalMotion(unit, time)
	...
	elseif self:GetDuration() - self:GetElapsedTime() < 0.3 then
		self.step = self.step
			or (self.cyc_pos.z - self:GetParent():GetAbsOrigin().z)
				/ ((self:GetDuration() - self:GetElapsedTime()) / FrameTime())
...
LinkLuaModifier(
	"modifier_boss_shadow_fiend_passive_ultimate_explosion",
	"neutrals/shadow_fiend",
	LUA_MODIFIER_MOTION_NONE
)
...
function boss_shadow_fiend_passive_ultimate:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	if not target:HasModifier("modifier_nevermore_requiem_fear") then
		...
		target:FindModifierByName("modifier_nevermore_requiem_fear"):SetDuration(
			math.min(
				target:FindModifierByName("modifier_nevermore_requiem_fear"):GetRemainingTime() + slow_duration,
				max_duration
...
function modifier_boss_shadow_fiend_passive_ultimate:IsHidden()
	return true
end
function modifier_boss_shadow_fiend_passive_ultimate:IsPurgable()
	return false
end
function modifier_boss_shadow_fiend_passive_ultimate:IsPurgeException()
	return false
end
		...
		if self:GetAbility():IsFullyCastable() then
			...
			if self.target ~= nil then
				...
				self:GetParent():AddNewModifier(
					self:GetParent(),
					self:GetAbility(),
					"modifier_boss_shadow_fiend_passive_ultimate_cast",
					{ duration = 3.1 }
				)
...
function modifier_boss_shadow_fiend_passive_ultimate_cast:IsHidden()
	return true
end
function modifier_boss_shadow_fiend_passive_ultimate_cast:IsPurgable()
	return false
end
...
function modifier_boss_shadow_fiend_passive_ultimate_explosion:IsHidden()
	return true
end
function modifier_boss_shadow_fiend_passive_ultimate_explosion:IsPurgable()
	return false
end
function modifier_boss_shadow_fiend_passive_ultimate_explosion:OnCreated()
	...
	self.wings_particle = ParticleManager:CreateParticle(
...
function modifier_boss_shadow_fiend_passive_ultimate_explosion:UseRequeiem()
	...
	local particle_caster_souls_fx = ParticleManager:CreateParticle(
	...
	local particle_caster_ground_fx = ParticleManager:CreateParticle(
...
function modifier_boss_shadow_fiend_passive_ultimate_explosion:CreateRequiemSoulLine(
	caster,
	ability,
	line_end_position,
	death_cast
	...
	local particle_lines_fx = ParticleManager:CreateParticle(
	...
	ParticleManager:SetParticleControl(
		particle_lines_fx,
		0,
		caster:GetAbsOrigin() + (line_end_position - caster:GetAbsOrigin()):Normalized() * 105
	)
