		...
		Timers:CreateTimer(0.4, function()
			...
			for _, target in ipairs(targets) do
				target:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_woda_neutral_desecrate_debuff",
...
function modifier_woda_neutral_desecrate_debuff:IsHidden()
	return false
end
function modifier_woda_neutral_desecrate_debuff:IsPurgable()
	return true
end
function modifier_woda_neutral_desecrate_debuff:OnCreated()
	self.parent = self:GetParent()
	self.armor = self:GetAbility():GetSpecialValueFor("armor_reduction_pct")
		* self.parent:GetPhysicalArmorValue(false)
		/ 100
	if not IsServer() then
		return
	end
