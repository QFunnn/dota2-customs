...
LinkLuaModifier(
	"modifier_woda_neutral_chain_lightning",
	"neutrals/woda_neutral_chain_lightning",
...
function modifier_woda_neutral_chain_lightning:IsHidden()
	return true
end
function modifier_woda_neutral_chain_lightning:IsPurgable()
	return false
end
function modifier_woda_neutral_chain_lightning:RemoveOnDeath()
	return false
end
function modifier_woda_neutral_chain_lightning:GetAttributes()
	...
	if (self.unit_counter >= self.jump_count and self.jump_count > 0) or not self.zapped then
		...
		do
			if not self.units_affected[enemy] and enemy ~= self.current_unit and enemy ~= self.previous_unit then
				...
				self.zapped = true
				self.current_unit:EmitSound("n_creep_HarpyStorm.ChainLighting")
				self.damage = self.current_unit:GetMana()
					/ 100
					* self:GetAbility():GetSpecialValueFor("damage_from_mana")
				ApplyDamage({
					victim = self.current_unit,
					damage = self.damage,
					damage_type = self.damage_type,
