...
function modifier_boss_medusa_split:OnAttack(params)
	...
	for _, enemy in pairs(enemies) do
		if enemy ~= params.target then
			...
			count = count + 1
			if count >= self:GetAbility():GetSpecialValueFor("bonus_targets") then
				break
			end
		end
	end
...
function modifier_boss_medusa_aura:IsHidden()
	return true
end
function modifier_boss_medusa_aura:IsPurgable()
	return false
end
function modifier_boss_medusa_aura:IsPurgeException()
	return false
end
function modifier_boss_medusa_aura:IsAuraActiveOnDeath()
	return false
end
...
function modifier_boss_medusa_buff_visual:IsPurgable()
	return false
end
...
function modifier_boss_medusa_buff_visual:OnIntervalThink()
	...
	if self.stoned >= self:GetAbility():GetSpecialValueFor("delay") then
		self:GetParent():AddNewModifier(
			self:GetAuraOwner(),
			self:GetAbility(),
			"modifier_boss_medusa_debuff",
			{ duration = self:GetAbility():GetSpecialValueFor("duration") * (1 - self:GetParent():GetStatusResistance()) }
		)
	end
end
...
function modifier_boss_medusa_debuff:IsPurgable()
	return false
end
