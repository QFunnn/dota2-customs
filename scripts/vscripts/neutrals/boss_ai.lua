...
function CreepThink()
	...
	if
		thisEntity.hTarget
		and not thisEntity.hTarget:IsNull()
		and thisEntity.hTarget:IsAlive()
		and (not thisEntity.hTarget:IsUnselectable())
	then
		hTarget = thisEntity.hTarget
	else
		local vEnemies = {}

		vEnemies = FindUnitsInRadius(
			DOTA_TEAM_NEUTRALS,
...
function CastAbility(hTarget)
	for i = 1, thisEntity:GetAbilityCount() do
		...
		if
			...
			and not thisEntity:IsStunned()
			and not thisEntity:IsSilenced()
		then
			------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			if
				ContainsValue(hAbility:GetBehavior(), DOTA_ABILITY_BEHAVIOR_UNIT_TARGET)
				and not ContainsValue(hAbility:GetBehavior(), DOTA_ABILITY_BEHAVIOR_ATTACK)
			then
				if
					hTarget:IsPositionInRange(
						thisEntity:GetAbsOrigin(),
						hAbility:GetCastRange(thisEntity:GetAbsOrigin(), hTarget)
					)
				then
					if
						ContainsValue(hAbility:GetAbilityTargetTeam(), DOTA_UNIT_TARGET_TEAM_ENEMY)
						or ContainsValue(hAbility:GetAbilityTargetTeam(), DOTA_UNIT_TARGET_TEAM_CUSTOM)
					then
			...
			------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			if
				ContainsValue(hAbility:GetBehavior(), DOTA_ABILITY_BEHAVIOR_NO_TARGET)
				and not ContainsValue(hAbility:GetBehavior(), DOTA_ABILITY_BEHAVIOR_AUTOCAST)
			then
