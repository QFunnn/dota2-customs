...
LinkLuaModifier("modifier_woda_neutral_ogre_seal", "neutrals/woda_neutral_ogre_seal", LUA_MODIFIER_MOTION_BOTH)

woda_neutral_ogre_seal = class({})

function woda_neutral_ogre_seal:Precache(context)
	PrecacheResource("particle", "particles/items_fx/ogre_seal_totem_smash.vpcf", context)
	PrecacheResource("particle", "particles/items_fx/ogre_seal_totem.vpcf", context)
end

function woda_neutral_ogre_seal:OnAbilityPhaseInterrupted()
...
function woda_neutral_ogre_seal:OnSpellStart()
	if not IsServer() then
		return
	end
	...
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_woda_neutral_ogre_seal", kv)
end

function woda_neutral_ogre_seal:TryToDamage()
	if not IsServer() then
		return
	end
	...
	local nFXIndex = ParticleManager:CreateParticle(
		"particles/items_fx/ogre_seal_totem_smash.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetCaster()
	)
...
modifier_woda_neutral_ogre_seal = class({})
...
function modifier_woda_neutral_ogre_seal:IsStunDebuff()
	return true
end

function modifier_woda_neutral_ogre_seal:IsHidden()
	return true
end

function modifier_woda_neutral_ogre_seal:IsPurgable()
	return false
end

function modifier_woda_neutral_ogre_seal:OnCreated(kv)
	if IsServer() then
		if self.nHopCount == nil then
			self.nHopCount = 1
			self.flop_distances = { 120, 225, 200 }
		end
		...
		self.vLoc = Vector(kv.vLocX, kv.vLocY, kv.vLocZ)
			+ self:GetCaster():GetForwardVector() * self.flop_distances[self.nHopCount]
...
function modifier_woda_neutral_ogre_seal:OnDestroy()
	if IsServer() then
...
function modifier_woda_neutral_ogre_seal:CheckState()
	local state = {
...
function modifier_woda_neutral_ogre_seal:UpdateHorizontalMotion(me, dt)
	if IsServer() then
...
function modifier_woda_neutral_ogre_seal:GetEffectName()
	return "particles/items_fx/ogre_seal_totem.vpcf"
end

function modifier_woda_neutral_ogre_seal:UpdateVerticalMotion(me, dt)
	if IsServer() then
...
function modifier_woda_neutral_ogre_seal:OnHorizontalMotionInterrupted()
	if IsServer() then
		self.bHorizontalMotionInterrupted = true
	end
end

function modifier_woda_neutral_ogre_seal:OnVerticalMotionInterrupted()
	if IsServer() then
		self:Destroy()
	end
end

function modifier_woda_neutral_ogre_seal:GetOverrideAnimation(params)
