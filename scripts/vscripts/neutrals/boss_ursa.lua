...
function modifier_boss_ursa_enrage:IsHidden()
	return true
end
function modifier_boss_ursa_enrage:IsPurgable()
	return false
end
function modifier_boss_ursa_enrage:IsPurgeException()
	return false
end
...
function modifier_boss_ursa_enrage:StartEnrage()
	...
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_boss_ursa_enrage_active",
...
function modifier_boss_ursa_fury_swipes:GetModifierProcAttack_BonusDamage_Pure(params)
	...
	local stack = 0
	local modifier =
		params.target:FindModifierByNameAndCaster("modifier_boss_ursa_fury_swipes_debuff", self:GetCaster())
	...
	if modifier == nil then
		params.target:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_boss_ursa_fury_swipes_debuff",
...
function modifier_boss_ursa_jump:IsHidden()
	return true
end
function modifier_boss_ursa_jump:IsPurgable()
	return false
end
function modifier_boss_ursa_jump:IsPurgeException()
	return false
end
function modifier_boss_ursa_jump:RemoveOnDeath()
	return false
end
...
function modifier_boss_ursa_jump:StartJumpDelay()
	...
	Timers:CreateTimer(delay, function()
		...
		local knockback = parent:AddNewModifier(
			parent,
			ability,
			"modifier_boss_ursa_jump_active",
			{
				distance = distance,
				height = 100,
				duration = 0.25,
		...
		local callback = function()
			...
			local enemies_targets = FindUnitsInRadius(
			...
			for _, enemy in pairs(enemies_targets) do
				...
				enemy:AddNewModifier(
					parent,
					nil,
					"modifier_boss_ursa_jump_debuff",
...
function modifier_boss_ursa_jump_debuff:GetTexture()
	return "ursa_earthshock"
end

function modifier_boss_ursa_jump_debuff:IsPurgable()
	return false
end
