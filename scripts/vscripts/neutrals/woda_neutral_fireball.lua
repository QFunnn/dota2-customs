...
function woda_neutral_fireball:OnSpellStart()
	...
	Timers:CreateTimer(0, function()
		...
		CreateModifierThinker(
			self:GetCaster(),
			self,
			"modifier_woda_neutral_fireball_thinker",
			{ duration = duration },
			point,
...
function modifier_woda_neutral_fireball_thinker:IsHidden()
	return false
end
function modifier_woda_neutral_fireball_thinker:IsPurgable()
	return false
end
...
function modifier_woda_neutral_fireball_thinker:OnCreated(table)
	...
	self.particle =
		ParticleManager:CreateParticle("particles/neutral_fx/black_dragon_fireball.vpcf", PATTACH_WORLDORIGIN, nil)
