...
function HeroDemo:OnGameRulesStateChange()
	...
	print(
		string.format(
			"[HeroDemo/State] called, state=%d, IsCheatsEnabled=%s, AdminsLoaded=%s",
			GameRules:State_Get(),
	...
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_PRE_GAME then
		for i = 0, 19 do
			if PlayerResource:IsValidPlayerID(i) then
				...
				print(
					string.format(
						"[HeroDemo/State] PRE_GAME PID=%d admin=%s cheats=%s steamid=%s",
						i,
						tostring(admin),
						tostring(cheats),
						tostring(PlayerResource:GetSteamAccountID(i))
					)
				)
				...
				if admin or cheats then
					...
					print(
						string.format(
							"[HeroDemo/State] Init() called for PID=%d, self.init=%s",
							i,
							tostring(HeroDemo.init)
						)
					)
