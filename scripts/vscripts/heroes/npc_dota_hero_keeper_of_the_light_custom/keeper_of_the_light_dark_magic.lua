...
function keeper_of_the_light_dark_magic:OnSpellStart()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_keeper_of_the_light/keeper_dark_magic.vpcf",
	...
	if self:GetCaster():HasModifier("modifier_keeper_of_the_light_5") then
		local damage = self.modifier_keeper_of_the_light_5
			+ (
				self:GetCaster():GetStrength()
				/ 100
				* self.modifier_keeper_of_the_light_5_strength[self:GetCaster()
					:GetTalentLevel("modifier_keeper_of_the_light_5")]
			)
		ApplyDamage({
			victim = target,
