...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_keeper_of_the_light_custom/keeper_of_the_light_will_o_wisp_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_keeper_of_the_light_will_o_wisp_custom_aura",
	"heroes/npc_dota_hero_keeper_of_the_light_custom/keeper_of_the_light_will_o_wisp_custom",
...
function keeper_of_the_light_will_o_wisp_custom:GetCooldown(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_keeper_of_the_light_13") then
		bonus = self.modifier_keeper_of_the_light_13_cooldown[self:GetCaster()
			:GetTalentLevel("modifier_keeper_of_the_light_13")]
	end
...
function keeper_of_the_light_will_o_wisp_custom:OnSpellStart()
	...
	self.duration = self.off_duration_initial
		+ (self.on_duration * self.on_count)
		+ (self.off_duration * (self.on_count - 1))
	if not IsServer() then
		return
	end
	local npc_dota_ignis_fatuus =
		CreateUnitByName("npc_dota_ignis_fatuus", self.position, true, self.caster, self.caster, self.caster:GetTeam())
	if npc_dota_ignis_fatuus then
		npc_dota_ignis_fatuus:AddNewModifier(
			self.caster,
			self,
			"modifier_keeper_of_the_light_will_o_wisp_custom",
...
function modifier_keeper_of_the_light_will_o_wisp_custom:IsPurgable()
	return false
end
function modifier_keeper_of_the_light_will_o_wisp_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_keeper_of_the_light_13") then
		self.hit_count = self.hit_count
			+ self:GetAbility().modifier_keeper_of_the_light_13[self:GetCaster()
				:GetTalentLevel("modifier_keeper_of_the_light_13")]
	end
...
function modifier_keeper_of_the_light_will_o_wisp_custom:OnIntervalThink()
	...
	if
		not self.is_on and (self.pulses == 0 and self.timer >= self.off_duration_initial)
		or (self.pulses > 0 and self.timer >= self.off_duration)
	then
		self.is_on = true
	...
	elseif self.is_on then
		if self.timer >= self.on_duration then
			...
			for _, enemy in pairs(enemies) do
				if
					not enemy:HasModifier("modifier_keeper_of_the_light_will_o_wisp_custom_aura")
					and not self.units_pulse[enemy]
				then
					self.units_pulse[enemy] = true
					ApplyDamage({
						attacker = self:GetCaster(),
						victim = enemy,
						damage = self.wisp_damage,
					...
					if self:GetCaster():HasModifier("modifier_keeper_of_the_light_12") then
						for i = 1, self:GetAbility().modifier_keeper_of_the_light_12[self:GetCaster():GetTalentLevel(
							"modifier_keeper_of_the_light_12"
						)] do
...
function modifier_keeper_of_the_light_will_o_wisp_custom:OnRemoved()
	...
	for _, enemy in pairs(enemies) do
		local hypnotize_modifier =
			enemy:FindModifierByNameAndCaster("modifier_keeper_of_the_light_will_o_wisp_custom_aura", self.parent)
...
function modifier_keeper_of_the_light_will_o_wisp_custom_aura:IsPurgable()
	return false
end
