...
LinkLuaModifier(
	"modifier_keeper_of_the_light_light_illusion",
	"heroes/npc_dota_hero_keeper_of_the_light_custom/keeper_of_the_light_light_illusion",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_keeper_of_the_light_light_illusion_spirit",
	"heroes/npc_dota_hero_keeper_of_the_light_custom/keeper_of_the_light_light_illusion",
	LUA_MODIFIER_MOTION_NONE
)
...
function keeper_of_the_light_light_illusion:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_keeper_of_the_light_light_illusion", { duration = duration })
end

modifier_keeper_of_the_light_light_illusion = class({})
function modifier_keeper_of_the_light_light_illusion:IsPurgable()
	return false
end
function modifier_keeper_of_the_light_light_illusion:IsPurgeException()
	return false
end
function modifier_keeper_of_the_light_light_illusion:RemoveOnDeath()
	return false
end
function modifier_keeper_of_the_light_light_illusion:OnCreated()
	...
	if self.modifier_keeper_of_the_light_9 then
		self:GetCaster()
			:SwapAbilities(
				"keeper_of_the_light_light_illusion",
				"keeper_of_the_light_light_illusion_point",
				false,
				true
			)
	end
...
function modifier_keeper_of_the_light_light_illusion:CreateIllusion()
	...
	local illusions = CreateIllusions(
		self:GetParent(),
		self:GetParent(),
		{ outgoing_damage = -100, incoming_damage = 0, duration = duration },
		1,
		400,
		false,
		false
		...
		if self:GetCaster():HasModifier("modifier_keeper_of_the_light_dark_form") then
			...
			if keeper_of_the_light_dark_form then
				illusion:AddNewModifier(
					self:GetCaster(),
					keeper_of_the_light_dark_form,
					"modifier_keeper_of_the_light_dark_form",
					{}
				)
			end
		end
...
function modifier_keeper_of_the_light_light_illusion_spirit:IsHidden()
	return true
end
function modifier_keeper_of_the_light_light_illusion_spirit:IsPurgable()
	return false
end
function modifier_keeper_of_the_light_light_illusion_spirit:IsPurgeException()
	return false
end
...
function modifier_keeper_of_the_light_light_illusion_spirit:OnCreated()
	...
	self.attack_range = self:GetParent():Script_GetAttackRange() * -1
	modifier_keeper_of_the_light_light_illusion_spirit.WINTER2022_BUCKET_SOLDIER_AGGRO_RANGE = self:GetParent()
...
function modifier_keeper_of_the_light_light_illusion_spirit:GetEffectName()
	if self:GetParent():HasModifier("modifier_keeper_of_the_light_dark_form") then
		return
	end
...
function modifier_keeper_of_the_light_light_illusion_spirit:BotThink()
	...
	elseif self.nState == modifier_keeper_of_the_light_light_illusion_spirit.BUCKET_SOLDIER_STATE_ATTACKING then
		...
		if
			self.hAttackTarget ~= nil
			and self.hAttackTarget:IsNull() == false
			and self.hAttackTarget:IsRealHero() == false
		then
		...
		if
			self.hAttackTarget == nil
			or self.hAttackTarget:IsNull() == true
			or self.hAttackTarget:IsAlive() == false
		then
		...
		if
			self.hBucket:GetAggroTarget() ~= nil
			and not self.hBucket:GetAggroTarget():IsNull()
			and self.hBucket:GetAggroTarget():IsAlive()
			and not self.hBucket:GetAggroTarget():IsInvulnerable()
		then
...
function modifier_keeper_of_the_light_light_illusion_spirit:LeashToBucket()
	self.vLeashDestination = self.vInitialSpawnPos
		+ RandomVector(
			RandomInt(50, modifier_keeper_of_the_light_light_illusion_spirit.WINTER2022_BUCKET_SOLDIER_MAINTAIN_RANGE)
		)
	self.flMaxLeashTime = GameRules:GetGameTime()
		+ modifier_keeper_of_the_light_light_illusion_spirit.WINTER2022_BUCKET_SOLDIER_MAX_LEASH_TIME
end
...
function modifier_keeper_of_the_light_light_illusion_spirit:PatrolBucket()
	local vTargetPos = self.vInitialSpawnPos
		+ RandomVector(
			RandomInt(50, modifier_keeper_of_the_light_light_illusion_spirit.WINTER2022_BUCKET_SOLDIER_MAINTAIN_RANGE)
		)
...
function modifier_keeper_of_the_light_light_illusion_spirit:FindBestTarget()
	local fSearchRadius = modifier_keeper_of_the_light_light_illusion_spirit.WINTER2022_BUCKET_SOLDIER_AGGRO_RANGE
		+ (self.me:Script_GetAttackRange() / 2)

	local vSearchOrigin = self.hBucket:GetAbsOrigin()

	local Units = FindUnitsInRadius(
		self.me:GetTeamNumber(),
		vSearchOrigin,
		self.me,
		fSearchRadius,
