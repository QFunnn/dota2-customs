...
LinkLuaModifier(
	"modifier_keeper_of_the_light_illuminate_custom",
	"heroes/npc_dota_hero_keeper_of_the_light_custom/keeper_of_the_light_illuminate_custom",
...
function keeper_of_the_light_illuminate_custom:Precache(context)
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_keeper_of_the_light.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_keeper_of_the_light.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_keeper_of_the_light.vpcf",
		context
	)
end
...
function keeper_of_the_light_illuminate_custom:OnSpellStart()
	...
	local modifier_keeper_of_the_light_spirit_form_custom =
		self:GetCaster():FindModifierByName("modifier_keeper_of_the_light_spirit_form_custom")
	...
	if modifier_keeper_of_the_light_spirit_form_custom then
		local thinker = CreateModifierThinker(
			...
			self,
			"modifier_keeper_of_the_light_illuminate_custom",
			{ duration = max_channel_time, index = index },
		...
		local modifier_keeper_of_the_light_illuminate_custom =
			thinker:FindModifierByName("modifier_keeper_of_the_light_illuminate_custom")
		if modifier_keeper_of_the_light_illuminate_custom then
			modifier_keeper_of_the_light_illuminate_custom.direction = direction
			local pfx_kotl = ParticleManager:CreateParticle(
		...
		local modifier_keeper_of_the_light_illuminate_custom = self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_keeper_of_the_light_illuminate_custom",
			{ duration = max_channel_time, index = index }
...
function keeper_of_the_light_illuminate_custom:OnChannelFinish(bInterrupted)
	...
	local modifier_keeper_of_the_light_illuminate_custom =
		self:GetCaster():FindModifierByName("modifier_keeper_of_the_light_illuminate_custom")
	if modifier_keeper_of_the_light_illuminate_custom then
...
function modifier_keeper_of_the_light_illuminate_custom:IsHidden()
	return true
end
function modifier_keeper_of_the_light_illuminate_custom:IsPurgable()
	return false
end
function modifier_keeper_of_the_light_illuminate_custom:OnCreated(params)
	...
	local keeper_of_the_light_illuminate_end_custom = self:GetCaster()
		:FindAbilityByName("keeper_of_the_light_illuminate_end_custom")
	...
	local modifier_keeper_of_the_light_spirit_form_custom =
		self:GetCaster():FindModifierByName("modifier_keeper_of_the_light_spirit_form_custom")
	if modifier_keeper_of_the_light_spirit_form_custom then
		self.illuminate_heal =
			modifier_keeper_of_the_light_spirit_form_custom:GetAbility():GetSpecialValueFor("illuminate_heal")
		if self:GetCaster():HasModifier("modifier_keeper_of_the_light_19") then
			self.illuminate_heal = self.illuminate_heal
				+ self:GetAbility().modifier_keeper_of_the_light_19[self:GetCaster()
					:GetTalentLevel("modifier_keeper_of_the_light_19")]
		end
	end
	...
	if self:GetParent() == self:GetCaster() then
		local particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_keeper_of_the_light/kotl_illuminate_cast.vpcf",
	...
	self:GetCaster()
		:SwapAbilities(
			...
			"keeper_of_the_light_illuminate_end_custom",
			false,
			true
		)
...
function modifier_keeper_of_the_light_illuminate_custom:OnIntervalThink()
	...
	if GameRules:GetGameTime() - self.vision_time_count >= self.channel_vision_interval then
		...
		self.ability:CreateVisibilityNode(
			...
			self.channel_vision_radius,
			self.channel_vision_duration
		)
...
function modifier_keeper_of_the_light_illuminate_custom:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_keeper_of_the_light_15") then
		total_damage = total_damage
			+ (
				self:GetCaster():GetMaxMana()
				/ 100
				* self:GetAbility().modifier_keeper_of_the_light_15[self:GetCaster()
					:GetTalentLevel("modifier_keeper_of_the_light_15")]
			)
	end
	...
	CreateModifierThinker(
		...
		"modifier_keeper_of_the_light_illuminate_custom_thinker",
		{
			...
			illuminate_heal = self.illuminate_heal,
			direction_x = vVelocity.x,
			direction_y = vVelocity.y,
			damage = total_damage,
			index = self.index,
		},
...
function modifier_keeper_of_the_light_illuminate_custom_thinker:OnIntervalThink()
	...
	for _, target in pairs(targets) do
		...
		local target_angle = math.deg(
			math.atan2((target_pos.x - self.parent:GetAbsOrigin().x), target_pos.y - self.parent:GetAbsOrigin().y)
		)
...
function keeper_of_the_light_illuminate_end_custom:OnSpellStart()
	...
	local keeper_of_the_light_illuminate_custom = self:GetCaster()
		:FindAbilityByName("keeper_of_the_light_illuminate_custom")
	if
		keeper_of_the_light_illuminate_custom
		and keeper_of_the_light_illuminate_custom.thinker
		and not keeper_of_the_light_illuminate_custom.thinker:IsNull()
	then
		keeper_of_the_light_illuminate_custom.thinker:RemoveModifierByName(
			"modifier_keeper_of_the_light_illuminate_custom"
		)
	else
		local modifier_keeper_of_the_light_illuminate_custom =
	...
	local modifier_keeper_of_the_light_obscure = self:GetCaster()
		:FindModifierByName("modifier_keeper_of_the_light_obscure")
