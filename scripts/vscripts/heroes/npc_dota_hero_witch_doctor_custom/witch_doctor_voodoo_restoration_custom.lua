...
LinkLuaModifier(
	"modifier_witch_doctor_voodoo_restoration_custom_heal",
	"heroes/npc_dota_hero_witch_doctor_custom/witch_doctor_voodoo_restoration_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function witch_doctor_voodoo_restoration_custom:GetManaCost(level)
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_1") then
		bonus = self.BaseClass.GetManaCost(self, level)
			/ 100
			* self.modifier_witch_doctor_1_mana[self:GetCaster():GetTalentLevel("modifier_witch_doctor_1")]
	end
...
function modifier_witch_doctor_voodoo_restoration_custom:OnIntervalThink()
	...
	local bonus = 0
	if self:GetCaster():HasModifier("modifier_witch_doctor_2") then
		bonus = self:GetAbility().modifier_witch_doctor_2_radius[self:GetCaster()
			:GetTalentLevel("modifier_witch_doctor_2")]
	end
	if self.particle then
		ParticleManager:SetParticleControl(
			self.particle,
			1,
			Vector(self.radius + bonus, self.radius + bonus, self.radius + bonus)
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_1") then
		manacost = manacost
			- (
				manacost
				/ 100
				* self:GetAbility().modifier_witch_doctor_1_mana[self:GetCaster()
					:GetTalentLevel("modifier_witch_doctor_1")]
			)
	end
...
function modifier_witch_doctor_voodoo_restoration_custom:GetModifierMoveSpeedBonus_Percentage()
	if self:GetCaster():HasModifier("modifier_witch_doctor_3") then
		return self:GetAbility().modifier_witch_doctor_3_movespeed[self:GetCaster()
			:GetTalentLevel("modifier_witch_doctor_3")]
	end
	return 0
end
...
function modifier_witch_doctor_voodoo_restoration_custom_heal:IsDebuff()
	return self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:IsHidden()
	return false
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:IsPurgable()
	return false
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:IsPurgeException()
	return false
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:IsStunDebuff()
	return false
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:RemoveOnDeath()
	return true
end
function modifier_witch_doctor_voodoo_restoration_custom_heal:GetAttributes()
...
function modifier_witch_doctor_voodoo_restoration_custom_heal:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_4") then
		bonus = self:GetCaster():GetStrength()
			/ 100
			* self:GetAbility().modifier_witch_doctor_4_str[self:GetCaster():GetTalentLevel("modifier_witch_doctor_4")]
	end
	if self:GetCaster():HasModifier("modifier_witch_doctor_6") then
		bonus = bonus
			+ self:GetCaster():GetMaxHealth()
				/ 100
				* self:GetAbility().modifier_witch_doctor_6_max_health_bonus[self:GetCaster()
					:GetTalentLevel("modifier_witch_doctor_6")]
	end
