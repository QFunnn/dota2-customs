...
function witch_doctor_paralyzing_cask_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_21") then
		bounc_bonus = (self:GetCaster():GetMaxMana() / self.modifier_witch_doctor_21_mana)
			* self.modifier_witch_doctor_21_bonus
	end

	if self:GetCaster():HasModifier("modifier_witch_doctor_15") then
		bounc_bonus = bounc_bonus
			+ self.modifier_witch_doctor_15[self:GetCaster():GetTalentLevel("modifier_witch_doctor_15")]
	end
			...
			for _, unit in pairs(enemies) do
				if hTarget then
					if
						...
						and ((self["split_" .. ExtraData.index] >= 1) or #tJumpTargets == 0)
					then
			...
			if #tJumpTargets == 0 then
				...
				if self:GetCaster():HasModifier("modifier_witch_doctor_19") then
					if hTarget then
						...
						local damage = (
							(ExtraData.base_damage * count) + (self:GetSpecialValueFor("bounce_bonus_damage") * count)
						)
							/ 100
							* self.modifier_witch_doctor_19[self:GetCaster():GetTalentLevel("modifier_witch_doctor_19")]
				...
				if self:GetCaster():HasModifier("modifier_witch_doctor_20") then
					...
					if
						cooldown
							- self.modifier_witch_doctor_20[self:GetCaster():GetTalentLevel("modifier_witch_doctor_20")]
						<= 0
					then
						self:EndCooldown()
					else
						...
						self:StartCooldown(
							cooldown
								- self.modifier_witch_doctor_20[self:GetCaster()
									:GetTalentLevel("modifier_witch_doctor_20")]
						)
					end
				end
