...
LinkLuaModifier(
	"modifier_witch_doctor_death_ward_custom",
	"heroes/npc_dota_hero_witch_doctor_custom/witch_doctor_death_ward_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_witch_doctor_voodoo_restoration_custom",
	"heroes/npc_dota_hero_witch_doctor_custom/witch_doctor_voodoo_restoration_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_witch_doctor_death_ward_custom_kill",
	"heroes/npc_dota_hero_witch_doctor_custom/witch_doctor_death_ward_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_10") then
		if self:GetCaster():HasModifier("modifier_witch_doctor_11") then
			...
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_witch_doctor_death_ward_custom_switch",
				{
					duration = self.modifier_witch_doctor_10_duration[self:GetCaster()
						:GetTalentLevel("modifier_witch_doctor_10")],
				}
			)
		end
		return
...
function witch_doctor_death_ward_custom:CreateWard(vPosition)
	if self:GetCaster():HasModifier("modifier_witch_doctor_10") then
		...
		local death_ward = CreateUnitByName(
...
function modifier_witch_doctor_death_ward_custom:IsDebuff()
	return false
end
function modifier_witch_doctor_death_ward_custom:IsHidden()
	return true
end
function modifier_witch_doctor_death_ward_custom:IsPurgable()
	return false
end
function modifier_witch_doctor_death_ward_custom:IsPurgeException()
	return false
end
function modifier_witch_doctor_death_ward_custom:IsStunDebuff()
	return false
end
...
function modifier_witch_doctor_death_ward_custom:OnCreated()
	...
	self.wardParticle = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_5") then
		...
		if ability and ability:GetLevel() > 0 then
			self:GetParent()
				:AddNewModifier(self:GetCaster(), ability, "modifier_witch_doctor_voodoo_restoration_custom", {})
		end
	end
end
...
function modifier_witch_doctor_death_ward_custom:OnIntervalThink()
	...
	if self:GetParent():HasModifier("modifier_witch_doctor_death_ward_custom_kill") then
		return
	end
	...
	if self.attack_target ~= nil then
		if
			self.attack_target:IsAttackImmune()
			or self.attack_target:IsInvulnerable()
			or ((self:GetParent():GetAbsOrigin() - self.attack_target:GetAbsOrigin()):Length2D() > range)
			or (not self.attack_target:IsAlive())
			or (not self:GetParent():CanEntityBeSeenByMyTeam(self.attack_target))
			or self.attack_target:GetTeamNumber() == self:GetCaster():GetTeamNumber()
		then
	...
	if self.wardParticle and self.attack_target then
		ParticleManager:SetParticleControlForward(
			self.wardParticle,
			1,
			-(self:GetParent():GetAbsOrigin() - self.attack_target:GetAbsOrigin()):Normalized()
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_witch_doctor_9") then
		bounce = self:GetAbility().modifier_witch_doctor_9_bounce[self:GetCaster()
			:GetTalentLevel("modifier_witch_doctor_9")]
	end
...
function modifier_witch_doctor_death_ward_custom_switch:IsHidden()
	return true
end
function modifier_witch_doctor_death_ward_custom_switch:IsPurgable()
	return false
end
