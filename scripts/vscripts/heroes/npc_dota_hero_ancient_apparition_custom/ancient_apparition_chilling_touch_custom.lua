...
LinkLuaModifier(
	"modifier_ancient_apparition_chilling_touch_custom_debuff",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_chilling_touch_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_generic_orb_effect_lua_apparation",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_chilling_touch_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ancient_apparition_chilling_touch_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_ancient_apparition.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_ancient_apparition.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_ancient_apparition.vpcf",
		context
	)
end
...
function ancient_apparition_chilling_touch_custom:GetManaCost(level)
	if self:GetCaster():HasModifier("modifier_ancient_apparition_11") then
		return self.BaseClass.GetManaCost(self, level)
			- (
				self.BaseClass.GetManaCost(self, level)
				/ 100
				* self.modifier_ancient_apparition_11[self:GetCaster():GetTalentLevel("modifier_ancient_apparition_11")]
			)
	end
	return self.BaseClass.GetManaCost(self, level)
end
...
function ancient_apparition_chilling_touch_custom:ProcsMagicStick()
	return false
end
...
function ancient_apparition_chilling_touch_custom:OnOrbImpact(params)
	...
	if self:GetCaster():HasModifier("modifier_ancient_apparition_8") then
		local heroes = FindUnitsInRadius(
			...
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_ancient_apparition_8),
...
function ancient_apparition_chilling_touch_custom:DamageAttack(target)
	...
	if self:GetCaster():HasModifier("modifier_ancient_apparition_9") then
		damage = damage
			+ self.modifier_ancient_apparition_9[self:GetCaster():GetTalentLevel("modifier_ancient_apparition_9")]
	end
	ApplyDamage({
		victim = target,
		damage = damage,
...
function modifier_generic_orb_effect_lua_apparation:OnRefresh(kv) end

function modifier_generic_orb_effect_lua_apparation:OnDestroy(kv) end
