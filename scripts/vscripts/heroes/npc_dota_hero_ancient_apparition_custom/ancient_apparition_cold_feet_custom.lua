...
LinkLuaModifier(
	"modifier_ancient_apparition_cold_feet_custom",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_cold_feet_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ancient_apparition_cold_feet_custom_stunned",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_cold_feet_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ancient_apparition_cold_feet_custom_talent_boom",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_cold_feet_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ancient_apparition_cold_feet_custom:OnSpellStart()
	...
	if caster:HasModifier("modifier_ancient_apparition_15") then
		...
		local radius = self.modifier_ancient_apparition_15[self:GetCaster()
			:GetTalentLevel("modifier_ancient_apparition_15")]
...
function modifier_ancient_apparition_cold_feet_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_ancient_apparition_15") then
		self.damage = self.damage
			+ self:GetAbility().modifier_ancient_apparition_15_damage[self:GetCaster()
				:GetTalentLevel("modifier_ancient_apparition_15")]
	end
...
function modifier_ancient_apparition_cold_feet_custom:OnIntervalThink()
	if (self:GetParent():GetAbsOrigin() - self.original_position):Length2D() < self.break_distance then
		...
		if self.counter >= 0.5 then
			if (self.ticks / 2) < self.duration then
...
function modifier_ancient_apparition_cold_feet_custom_talent_boom:IsHidden()
	return true
end
function modifier_ancient_apparition_cold_feet_custom_talent_boom:IsPurgable()
	return false
end
function modifier_ancient_apparition_cold_feet_custom_talent_boom:OnDestroy()
	...
	if self:GetRemainingTime() <= 0 then
		...
		local damage = self:GetCaster():GetIntellect(false)
			/ 100
			* self:GetAbility().modifier_ancient_apparition_18[self:GetCaster()
				:GetTalentLevel("modifier_ancient_apparition_18")]
		ApplyDamage({
			attacker = self:GetCaster(),
			victim = self:GetParent(),
			ability = self:GetAbility(),
