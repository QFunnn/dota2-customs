...
LinkLuaModifier(
	"modifier_ancient_apparition_ice_blast_custom",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_ice_blast_custom",
...
LinkLuaModifier(
	"modifier_ancient_apparition_ice_blast_channel_point",
	"heroes/npc_dota_hero_ancient_apparition_custom/ancient_apparition_ice_blast_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ancient_apparition_ice_blast_custom:AttackEffect(target)
	...
	if not target:HasModifier("modifier_ancient_apparition_ice_blast_custom_talent_cooldown") then
		target:AddNewModifier(
			...
			self,
			"modifier_ancient_apparition_ice_blast_custom_talent_cooldown",
			{ duration = self.modifier_ancient_apparition_17_cooldown }
		)
		target:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_ancient_apparition_ice_blast_custom_debuff",
			{
				duration = self.modifier_ancient_apparition_17_duration * (1 - target:GetStatusResistance()),
				dot_damage = self:GetSpecialValueFor("dot_damage"),
				kill_pct = self:GetSpecialValueFor("kill_pct"),
			}
		)
	end
end
...
function ancient_apparition_ice_blast_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_ancient_apparition_21") then
		self.modifier_channel = self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_ancient_apparition_ice_blast_channel_point",
	...
	self.ice_blast_dummy = CreateModifierThinker(
		self:GetCaster(),
		self,
		"modifier_ancient_apparition_ice_blast_custom",
	...
	if self.release_ability then
		self:GetCaster():SwapAbilities(
			...
			"ancient_apparition_ice_blast_release_custom",
			false,
			true
		)
	end
...
function modifier_ancient_apparition_ice_blast_custom:IsPurgable()
	return false
end
...
function modifier_ancient_apparition_ice_blast_custom_debuff:IsDebuff()
	return true
end
function modifier_ancient_apparition_ice_blast_custom_debuff:IsPurgable()
	return false
end
...
function modifier_ancient_apparition_ice_blast_custom_debuff:OnIntervalThink()
	...
	local dmg = ApplyDamage(self.damage_table)
...
function modifier_ancient_apparition_ice_blast_custom_debuff:OnTakeDamageKillCredit(keys)
	if
		keys.target == self:GetParent()
		and (self:GetParent():GetHealth() / self:GetParent():GetMaxHealth()) * 100 <= self.kill_pct
		and keys.damage > 0
		and keys.original_damage > 0
	then
...
function ancient_apparition_ice_blast_release_custom:IsStealable()
	return false
end
function ancient_apparition_ice_blast_release_custom:ProcsMagicStick()
	return false
end
		...
		if self.ice_blast_ability.ice_blast_dummy and self.ice_blast_ability.initial_projectile then
			...
			local final_radius = math.min(
				self.ice_blast_ability:GetSpecialValueFor("radius_min")
					+ (
						(vector:Length2D() / self.ice_blast_ability:GetSpecialValueFor("speed"))
						* self.ice_blast_ability:GetSpecialValueFor("radius_grow")
					),
			...
			ParticleManager:SetParticleControl(
				marker_particle,
				0,
				self.ice_blast_ability.ice_blast_dummy:GetAbsOrigin()
			)
...
function ancient_apparition_ice_blast_release_custom:OnProjectileThink_ExtraData(location, data)
	if self.ice_blast_ability then
		AddFOWViewer(
			self:GetCaster():GetTeamNumber(),
			location,
			self.ice_blast_ability:GetSpecialValueFor("target_sight_radius"),
			3,
			false
		)
		local enemies = FindUnitsInRadius(
			...
			location,
			nil,
			self.ice_blast_ability:GetSpecialValueFor("path_radius"),
		...
		if self:GetCaster():HasModifier("modifier_ancient_apparition_19") then
			duration = duration
				+ self.ice_blast_ability.modifier_ancient_apparition_19[self:GetCaster():GetTalentLevel(
					"modifier_ancient_apparition_19"
				)]
		end
	...
	if not target and self.ice_blast_ability then
		...
		for _, enemy in pairs(enemies) do
			if enemy:IsInvulnerable() then
				...
				local ancient_apparition_cold_feet_custom = self:GetCaster()
					:FindAbilityByName("ancient_apparition_cold_feet_custom")
...
function modifier_ancient_apparition_ice_blast_custom_talent_cooldown:IsHidden()
	return true
end
function modifier_ancient_apparition_ice_blast_custom_talent_cooldown:IsPurgable()
	return false
end
function modifier_ancient_apparition_ice_blast_custom_talent_cooldown:RemoveOnDeath()
	return false
end

modifier_ancient_apparition_ice_blast_channel_point = class({})
function modifier_ancient_apparition_ice_blast_channel_point:IsPurgable()
	return false
end
function modifier_ancient_apparition_ice_blast_channel_point:IsPurgeException()
	return false
end
function modifier_ancient_apparition_ice_blast_channel_point:IsHidden()
	return true
end
function modifier_ancient_apparition_ice_blast_channel_point:OnCreated(params)
	...
	self.radius_tick = (
		self:GetAbility():GetSpecialValueFor("radius_max") - self:GetAbility():GetSpecialValueFor("radius_min")
	) / 10
	self.ice_blast_ability = self:GetAbility()
	self.marker_particle = ParticleManager:CreateParticleForTeam(
		"particles/units/heroes/hero_ancient_apparition/ancient_apparition_ice_blast_marker.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetCaster(),
		self:GetCaster():GetTeamNumber()
...
function modifier_ancient_apparition_ice_blast_channel_point:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_ancient_apparition_19") then
		duration = duration
			+ self.ice_blast_ability.modifier_ancient_apparition_19[self:GetCaster()
				:GetTalentLevel("modifier_ancient_apparition_19")]
	end
