...
LinkLuaModifier("modifier_spectre_suffering_specter", "heroes/hero_spectre/spectre_suffering_specter.lua",
	LUA_MODIFIER_MOTION_NONE)
--Abilities
if spectre_suffering_specter == nil then
	spectre_suffering_specter = class({}) ---@class spectre_suffering_specter : CDOTA_Ability_Lua
end
...
function spectre_suffering_specter:Fire(hTarget, bClearStacks)
	...
	if IsValid(hCaster) and IsValid(hTarget) then
		...
		"particles/units/heroes/hero_spectre/spectre_suffering_specter.vpcf", PATTACH_CUSTOMORIGIN, hCaster)
		ParticleManager:SetParticleControlEnt(iParticleID, 0, hCaster, PATTACH_POINT_FOLLOW, "attach_hitloc",
			Vector(0, 0, 0), true)
		...
		ParticleManager:SetParticleControlEnt(iParticleID, 11, hCaster, PATTACH_POINT_FOLLOW, "attach_hitloc",
...
function spectre_suffering_specter:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if IsValid(hCaster) and IsValid(hTarget) and IsValid(self) and not hTarget:TriggerSpellAbsorb(self) then
		local units = FindUnitsInRadius(hCaster:GetTeamNumber(), hTarget:GetAbsOrigin(), nil, radius,
...
if modifier_spectre_suffering_specter == nil then
	modifier_spectre_suffering_specter = class({}) ---@class CDOTA_Modifier_Lua
end

function modifier_spectre_suffering_specter:IsHidden() return false end

function modifier_spectre_suffering_specter:IsDebuff() return false end

function modifier_spectre_suffering_specter:IsPurgable() return false end

function modifier_spectre_suffering_specter:IsPurgeException() return false end
	...
	if IsValid(hParent) and hParent == params.attacker and fDamage > 0 then
		if IsValid(hInflictor) and hInflictor.GetAbilityName and type(hInflictor.GetAbilityName) == "function" then
			...
			if REFLECTION_ABILITIES[sInflictorName] ~= nil then
				local CurrentTime = GameRulesCustom:GetGameTime()
...
function modifier_spectre_suffering_specter:GetModifierHealthBonus()
	local baseHp = self:GetParent():GetStrength() * 22 + 120
	return baseHp * self.bonus_health_pct * 0.01
end
