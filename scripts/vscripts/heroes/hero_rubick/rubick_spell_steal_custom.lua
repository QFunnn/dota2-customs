...
LinkLuaModifier(
	"modifier_rubick_spell_steal_custom_buff",
	"heroes/hero_rubick/rubick_spell_steal_custom",
...
function rubick_spell_steal_custom:OnAbilityPhaseStart() end
...
function rubick_spell_steal_custom:ApplyStolenBonusModifiers()
	...
	if not self.currentSpell or self.currentSpell:IsNull() then
		return
	end
	local settings = ABILITIES_SETTINGS and ABILITIES_SETTINGS[self.currentSpell:GetAbilityName()]
	if not settings or not settings.BonusModifiers then
		return
	end
	local caster = self:GetCaster()
	for _, ModifierName in ipairs(settings.BonusModifiers) do
		...
		if not (ModifierName == "modifier_muerta_pierce_the_veil" and not caster:IsRealHero()) then
			...
			if mod then
				table.insert(self.currentSpellBonusModifiers, mod)
			end
		end
	end
end
...
function modifier_rubick_spell_steal_custom:AbilityHasImmediate(ability)
	if not DOTA_ABILITY_BEHAVIOR_IMMEDIATE then
		return false
	end
	local b = ability:GetBehavior()
	if type(b) ~= "number" then
		b = tonumber(tostring(b)) or 0
	end
...
function modifier_rubick_spell_steal_custom:TryRecordLastSpell(params)
	if not params.unit or not params.ability then
		return
	end
...
function modifier_rubick_spell_steal_custom:OnAbilityFullyCast(params)
	if IsServer() then
		if params.unit == self:GetParent() then
			if
				self:GetParent():HasTalent("special_bonus_unique_rubick_6")
				and params.ability
				and params.ability == self:GetAbility().currentSpell
			then
...
function modifier_rubick_spell_steal_custom:OnModifierAdded(params)
	...
	if params.added_buff:GetName() == "modifier_cyclone" then
		return
	end
	if params.added_buff:GetName() == "modifier_eul_cyclone" then
		return
	end
	if params.added_buff:GetName() == "modifier_eul_cyclone_thinker" then
		return
	end
	if params.added_buff:GetName() == "modifier_eul_wind_waker_thinker" then
		return
	end
	if params.added_buff:GetName() == "modifier_wind_waker" then
		return
	end
	if params.added_buff:GetAbility() ~= self:GetAbility().currentSpell then
		return
	end
	local new_duration = params.added_buff:GetDuration()
		+ (params.added_buff:GetDuration() / 100 * self:GetAbility():GetSpecialValueFor("stolen_debuff_amp"))
