...
LinkLuaModifier(
	"modifier_rubick_magic_duel_spell_amp",
	"heroes/hero_rubick/rubick_magic_duel.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_rubick_magic_duel_creep_cd",
	"heroes/hero_rubick/rubick_magic_duel.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
	...
	if IsValid(hCaster) and IsValid(hTarget) and hCaster:IsAlive() and hTarget:IsAlive() then
		...
		if not tDuelBuff then
			...
			local iParticleID = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_rubick/rubick_magic_duel_ring.vpcf",
				PATTACH_CUSTOMORIGIN,
				hCaster
			)
			ParticleManager:SetParticleControl(iParticleID, 0, vMidPos)
			-- ParticleManager:ReleaseParticleIndex(iParticleID)
			local tCasterBuff = hCaster:AddNewModifier(
				hCaster,
				self,
				"modifier_rubick_magic_duel",
				{ duration = duration * hTarget:GetStatusResistanceFactor(hCaster), target_index = hTarget:entindex() }
			)
...
function modifier_rubick_magic_duel:OnCreated(params)
	...
	if IsServer() then
		...
		if
			...
			or (
				not hParent:IsHero()
				and not hParent:IsIllusion()
				and not hCaster:HasModifier("modifier_rubick_magic_duel_creep_cd")
			)
		then
			self.bReward = true
		end
	...
	if IsServer() then
		...
		if bDeath then
			...
			if self.bReward then
				self.hDuelTarget:AddNewModifier(
					hCaster,
					hAbility,
					"modifier_rubick_magic_duel_spell_amp",
					{ stack = self.reward_spell_amp }
				)
...
function modifier_rubick_magic_duel:OnIntervalThink()
	...
	if
		not hParent:IsChanneling()
		and hParent:GetCurrentActiveAbility() == nil
		and IsValid(self.hDuelTarget)
		and self.hDuelTarget:IsAlive()
	then
		...
		for i = 0, hParent:GetAbilityCount() - 1 do
			...
			if
				...
				and hAbility:IsFullyCastable()
				and not HasBehavior(hAbility, DOTA_ABILITY_BEHAVIOR_CHANNELLED)
				and not hAbility:IsToggle()
				and not hAbility:IsPassive()
				and hAbility:GetAbilityName() ~= "rubick_magic_duel"
			then
		...
		if #tNormalAbilities > 0 then
			...
			if
				HasBehavior(hAbilitytoCast, DOTA_ABILITY_BEHAVIOR_POINT)
				or HasBehavior(hAbilitytoCast, DOTA_ABILITY_BEHAVIOR_OPTIONAL_POINT)
			then
				hParent:CastAbilityOnPosition(
					self.hDuelTarget:GetAbsOrigin() + RandomVector(RandomFloat(0, 50)),
					hAbilitytoCast,
					hParent:GetPlayerOwnerID()
				)
				bCastDone = true
			elseif HasBehavior(hAbilitytoCast, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) then
				...
				elseif iAbilityTargetTeam == DOTA_UNIT_TARGET_TEAM_ENEMY then
					...
					bCastDone = true
				elseif
					iAbilityTargetTeam == DOTA_UNIT_TARGET_TEAM_BOTH
					or iAbilityTargetTeam == DOTA_UNIT_TARGET_TEAM_CUSTOM
				then
					hParent:CastAbilityOnTarget(self.hDuelTarget, hAbilitytoCast, hParent:GetPlayerOwnerID())
					bCastDone = true
				end
		...
		if not bCastDone then
			...
			for i = 0, hParent:GetAbilityCount() - 1 do
				...
				if
					...
					and hAbility:IsCooldownReady()
					and hAbility:IsFullyCastable()
					and HasBehavior(hAbility, DOTA_ABILITY_BEHAVIOR_CHANNELLED)
					and not hAbility:IsToggle()
					and not hAbility:IsPassive()
					and hAbility:GetAbilityName() ~= "rubick_magic_duel"
				then
...
function modifier_rubick_magic_duel_spell_amp:OnCreated(params)
	if IsServer() then
		...
		local iParticleID = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_rubick/rubick_magic_duel_victory.vpcf",
