...
function Lift(keys)
	...
	ParticleManager:SetParticleControl(
		ability.lift_particle,
		0,
		Vector(target:GetAbsOrigin().x, target:GetAbsOrigin().y, target:GetAbsOrigin().z)
	)
	ParticleManager:SetParticleControl(
		ability.lift_particle,
		1,
...
function Drop(keys)
	...
	telekinesis.drop_particle =
		ParticleManager:CreateParticleForTeam(keys.particle, PATTACH_WORLDORIGIN, caster, caster:GetTeam())
