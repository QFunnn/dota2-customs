...
LinkLuaModifier(
	"modifier_windrunner_powershot_custom_magic_resistance",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_powershot_custom",
...
LinkLuaModifier(
	"modifier_windrunner_powershot_custom_slow",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_powershot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function windrunner_powershot_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_windrunner.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_windrunner.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_windrunner.vpcf",
		context
	)
end
...
function windrunner_powershot_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_windrunner_21") then
		self:GetCaster():AddNewModifier(
			...
			"modifier_windrunner_powershot_custom_magic_immune",
			{
				duration = self.modifier_windrunner_21_duration[self:GetCaster()
					:GetTalentLevel("modifier_windrunner_21")],
			}
		)
	end
end
...
function windrunner_powershot_custom:OnChannelFinish(bInterrupted)
	...
	local modifier_windrunner_tailwind_custom = self:GetCaster()
		:FindModifierByName("modifier_windrunner_tailwind_custom")
	...
	if self:GetCaster():HasModifier("modifier_windrunner_22") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_windrunner_22_bonus[self:GetCaster():GetTalentLevel("modifier_windrunner_22")]
			)
	end
...
function windrunner_powershot_custom:OnProjectileHitHandle(target, location, handle)
	...
	if self:GetCaster():HasModifier("modifier_windrunner_16") then
		target:AddNewModifier(
			...
			self,
			"modifier_windrunner_powershot_custom_magic_resistance",
			{ duration = self.modifier_windrunner_16_duration }
		)
	end
	...
	target:AddNewModifier(
		...
		self,
		"modifier_windrunner_powershot_custom_slow",
		{ duration = self:GetSpecialValueFor("slow_duration") * (1 - target:GetStatusResistance()), slow = slow }
	)
