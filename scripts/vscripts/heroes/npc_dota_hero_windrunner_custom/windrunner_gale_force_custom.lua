...
LinkLuaModifier(
	"modifier_windrunner_gale_force_custom",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_gale_force_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_windrunner_gale_force_custom_aura",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_gale_force_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_windrunner_gale_force_custom_debuff",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_gale_force_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function windrunner_gale_force_custom:OnVectorCastStart(vStartLocation, vDirection)
	...
	if self:GetCaster():HasModifier("modifier_windrunner_6") then
		duration = duration
			+ self.modifier_windrunner_6_bonus_duration[self:GetCaster():GetTalentLevel("modifier_windrunner_6")]
	end
	AddFOWViewer(self:GetCaster():GetTeamNumber(), vStartLocation, self:GetSpecialValueFor("radius"), duration, false)
	local mod = CreateModifierThinker(
		self:GetCaster(),
		self,
		"modifier_windrunner_gale_force_custom",
		{ duration = duration, x = vDirection.x, y = vDirection.y, z = vDirection.z },
		vStartLocation,
...
function modifier_windrunner_gale_force_custom:OnCreated(kv)
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_windrunner/windrunner_gale_force_owner.vpcf",
		PATTACH_WORLDORIGIN,
		nil
	)
	...
	for _, enemy in pairs(enemies) do
		if not enemy:IsCurrentlyHorizontalMotionControlled() then
			enemy:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_windrunner_gale_force_custom_debuff",
				{ duration = 0.1, x = self.direction.x, y = self.direction.y }
			)
		end
	end
end
...
function modifier_windrunner_gale_force_custom_aura:IsHidden()
	return true
end
...
function modifier_windrunner_gale_force_custom_aura:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_windrunner/windrunner_gale_force.vpcf",
...
function modifier_windrunner_gale_force_custom_aura:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_windrunner_4") then
		return
	end
	local damage = self:GetCaster():GetStrength()
		/ 100
		* self:GetAbility().modifier_windrunner_4_str_damage[self:GetCaster():GetTalentLevel("modifier_windrunner_4")]
	ApplyDamage({
...
function modifier_windrunner_gale_force_custom_debuff:IsHidden()
	return true
end
function modifier_windrunner_gale_force_custom_debuff:IsPurgable()
	return false
end
function modifier_windrunner_gale_force_custom_debuff:IsPurgeException()
	return false
end
...
function modifier_windrunner_gale_force_custom_debuff:OnDestroy()
	...
	then
		if
			not self:GetParent():HasModifier("modifier_slark_pounce_arc")
