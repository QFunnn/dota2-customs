...
LinkLuaModifier(
	"modifier_windrunner_tailwind_custom",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_tailwind_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_windrunner_tailwind_custom_buff",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_tailwind_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_windrunner_tailwind_custom:IsHidden()
	return true
end
function modifier_windrunner_tailwind_custom:IsPurgable()
	return false
end
function modifier_windrunner_tailwind_custom:IsPurgeException()
	return false
end
function modifier_windrunner_tailwind_custom:RemoveOnDeath()
	return false
end
...
function modifier_windrunner_tailwind_custom:OnAbilityExecuted(params)
	...
	if params.ability:GetName() == "windrunner_powershot_custom" then
		return
	end
	if params.ability:GetName() == "windrunner_ultra_powershot_custom" then
		return
	end
...
function modifier_windrunner_tailwind_custom:GetModifierTotal_ConstantBlock(kv)
	if IsServer() then
		...
		if
			self:GetCaster():HasModifier("modifier_windrunner_12")
			and self:GetCaster():HasModifier("modifier_windrunner_tailwind_custom_buff")
		then
			if
				kv.damage > 0
				and RollPercentage(
					self:GetAbility().modifier_windrunner_12[self:GetCaster():GetTalentLevel("modifier_windrunner_12")]
				)
			then
