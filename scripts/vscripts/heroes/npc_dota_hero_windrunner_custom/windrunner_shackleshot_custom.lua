...
LinkLuaModifier(
	"modifier_windrunner_shackleshot_custom_status_resistance",
	"heroes/npc_dota_hero_windrunner_custom/windrunner_shackleshot_custom",
...
function windrunner_shackleshot_custom:OnProjectileHit_ExtraData(target, location, ExtraData)
			...
			if math.abs(AngleDiff(target_angle, enemy_angle)) <= search_angle then
				...
				if self:GetCaster():HasModifier("modifier_windrunner_17") then
					target:AddNewModifier(
						...
						self,
						"modifier_windrunner_shackleshot_custom_status_resistance",
						{ duration = self.modifier_windrunner_17_duration }
					)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = fail_duration * (1 - target:GetStatusResistance()) }
	)
