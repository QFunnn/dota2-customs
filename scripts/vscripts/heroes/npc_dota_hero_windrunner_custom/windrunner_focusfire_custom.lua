...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_windrunner_custom/windrunner_focusfire_custom",
...
function modifier_windrunner_focusfire_custom:GetModifierAttackSpeedBonus_Constant()
	...
	if aggro and aggro ~= self.target then
		return
	end
	return self.bonus
end
...
function modifier_windrunner_focusfire_custom:OnIntervalThink()
	...
	if
		...
		or self:GetParent():IsDisarmed()
		or self:GetParent():IsStunned()
		or not self:GetParent():CanEntityBeSeenByMyTeam(self.target)
		or self:GetParent():IsChanneling()
	...
	if
		...
		and self:GetParent():AttackReady()
		and not self:GetParent():IsDisarmed()
		and not self:GetParent():IsStunned()
		and self:GetParent():CanEntityBeSeenByMyTeam(self.target)
		and not self:GetParent():IsChanneling()
