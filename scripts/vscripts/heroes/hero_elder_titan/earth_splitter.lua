...
LinkLuaModifier(
	...
	"heroes/hero_elder_titan/earth_splitter.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_elder_titan_earth_splitter",
	"heroes/hero_elder_titan/earth_splitter.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function ability_elder_titan_earth_splitter:OnSpellStart()
	...
	CreateModifierThinker(caster, self, "modifier_ability_elder_titan_earth_splitter_thinker", {
		duration = CrackDelay,
		start_x = casterPos.x,
		start_y = casterPos.y,
		end_x = EndPos.x,
...
modifier_ability_elder_titan_earth_splitter_thinker = class({
	...
	OnDestroy = function(self)
		...
		if not caster or not parent or self.StartPos == nil or self.EndPos == nil or not ability then
			return
		end
		...
		for _, enemy in pairs(enemies) do
			...
			if not Players:IsUnitCanAttackOrCastOnThis(caster, enemy) then
				goto continue
			end
			...
			if
				(
					enemy:GetUnitName() ~= "npc_dota_roshan"
					and enemy:GetUnitName() ~= "npc_dota_nian"
					and Rounds:GetCurrentRound() < 90
				)
				or enemy:IsHero()
				or (GetRealUnit(enemy) ~= nil and GetRealUnit(enemy):IsRealHero())
			then
				ApplyDamage({
					victim = enemy,
					attacker = caster,
			...
			if enemy:IsAlive() then
				...
				enemy:AddNewModifier(
					...
					ability,
					"modifier_ability_elder_titan_earth_splitter",
					{ duration = self.SlowDuration * (1 - enemy:GetStatusResistance()) }
				)
