...
LinkLuaModifier(
	"modifier_morphling_adaptive_strike_agi_custom_debuff",
	"heroes/npc_dota_hero_morphling_custom/morphling_adaptive_strike_agi_custom",
...
function morphling_adaptive_strike_agi_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_morphling.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_morphling.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_morphling.vpcf",
		context
	)
end
...
function morphling_adaptive_strike_agi_custom:OnProjectileHit(target, vLocation)
	...
	local damage_factor = GetLinearAttributeValue(
		damage_attribute,
		damage_opposing_attribute,
		damage_min,
		damage_max,
		self.attribute_ratio
	)
	local stun_duration =
		GetLinearAttributeValue(control_attribute, control_opposing_attribute, stun_min, stun_max, self.attribute_ratio)
	local knockback_distance = GetLinearAttributeValue(
		control_attribute,
		control_opposing_attribute,
		knockback_min,
		knockback_max,
		self.attribute_ratio
	)
	...
	ApplyDamage({
		...
		victim = target,
		ability = self,
		damage = damage_base + damage_attribute * damage_factor,
		damage_type = DAMAGE_TYPE_MAGICAL,
	})

	if str_check > agil_check then
		local nFXIndex = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_morphling/morphling_adaptive_strike_str.vpcf",
	...
	if self:GetCaster():HasModifier("modifier_morphling_6") then
		target:AddNewModifier(
			...
			self,
			"modifier_morphling_adaptive_strike_agi_custom_debuff",
			{ duration = self.modifier_morphling_6_duration }
		)
	end
	...
	local knockback = target:AddNewModifier(
		...
		"modifier_generic_knockback_lua",
		{
			duration = knockback_distance / 1000,
			height = 0,
...
function modifier_morphling_adaptive_strike_agi_custom_debuff:GetTexture()
	return "morphling_6"
end
