...
LinkLuaModifier(
	"modifier_morphling_waterraze_custom_debuff",
	"heroes/npc_dota_hero_morphling_custom/morphling_waterraze_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_waterraze_custom_mana_buff",
	"heroes/npc_dota_hero_morphling_custom/morphling_waterraze_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_waterraze_custom_mana_stack",
	"heroes/npc_dota_hero_morphling_custom/morphling_waterraze_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_waterraze_custom_talent_handler",
	"heroes/npc_dota_hero_morphling_custom/morphling_waterraze_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_waterraze_custom_talent_handler_cooldown",
	"heroes/npc_dota_hero_morphling_custom/morphling_waterraze_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function CastShadowRazeOnPoint(caster, ability, point, radius, passive)
	if caster:HasModifier("modifier_morphling_20") then
		caster:AddNewModifier(
			...
			ability,
			"modifier_morphling_waterraze_custom_mana_stack",
			{ duration = ability.modifier_morphling_20_duration }
		)
		caster:AddNewModifier(
			caster,
			ability,
...
function modifier_morphling_waterraze_custom_debuff:IsDebuff()
	return true
end

function modifier_morphling_waterraze_custom_debuff:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_morphling_19")
end
...
function modifier_morphling_waterraze_custom_mana_stack:IsHidden()
	return true
end
function modifier_morphling_waterraze_custom_mana_stack:IsPurgable()
	return false
end
function modifier_morphling_waterraze_custom_mana_stack:GetAttributes()
...
function modifier_morphling_waterraze_custom_mana_buff:IsPurgable()
	return false
end
...
function modifier_morphling_waterraze_custom_mana_buff:GetModifierExtraManaPercentage()
	return self:GetStackCount()
		* self:GetAbility().modifier_morphling_20[self:GetCaster():GetTalentLevel("modifier_morphling_20")]
end

function modifier_morphling_waterraze_custom_mana_buff:GetTexture()
	return "morphling_20"
end

modifier_morphling_waterraze_custom_talent_handler = class({})

function modifier_morphling_waterraze_custom_talent_handler:IsHidden()
	return true
end
function modifier_morphling_waterraze_custom_talent_handler:IsPurgable()
	return false
end
function modifier_morphling_waterraze_custom_talent_handler:RemoveOnDeath()
	return false
end
function modifier_morphling_waterraze_custom_talent_handler:IsPurgeException()
	return false
end
...
function modifier_morphling_waterraze_custom_talent_handler:OnAbilityFullyCast(params)
	...
	if not self:GetCaster():HasModifier("modifier_morphling_replicate_custom") then
		return
	end
	if params.ability:GetAbilityName() == "morphling_replicate_custom" then
		return
	end
	if params.ability:GetAbilityName() == "nevermore_frenzy_custom" then
		return
	end
	if params.ability:GetAbilityName() == "morphling_morph_replicate_custom" then
		return
	end
	if params.ability:GetAbilityName() == "invoker_quas_custom" then
		return
	end
	if params.ability:GetAbilityName() == "invoker_wex_custom" then
		return
	end
	if params.ability:GetAbilityName() == "invoker_exort_custom" then
		return
	end
	if not self:GetCaster():HasModifier("modifier_morphling_18") then
		return
	end
	if self:GetParent():HasModifier("modifier_morphling_waterraze_custom_talent_handler_cooldown") then
		return
	end
	local enemies = FindUnitsInRadius(
	...
	if morphling_waterraze1_custom and morphling_waterraze1_custom:GetLevel() > 0 then
		if #enemies > 0 then
			CastShadowRazeOnPoint(
				self:GetCaster(),
				morphling_waterraze1_custom,
				enemies[1]:GetAbsOrigin(),
				morphling_waterraze1_custom:GetSpecialValueFor("shadowraze_radius"),
				true
			)
			self:IncrementStackCount()
...
function modifier_morphling_waterraze_custom_talent_handler_cooldown:IsPurgable()
	return false
end
function modifier_morphling_waterraze_custom_talent_handler_cooldown:IsPurgeException()
	return false
end
function modifier_morphling_waterraze_custom_talent_handler_cooldown:RemoveOnDeath()
	return false
end
function modifier_morphling_waterraze_custom_talent_handler_cooldown:IsDebuff()
	return true
end