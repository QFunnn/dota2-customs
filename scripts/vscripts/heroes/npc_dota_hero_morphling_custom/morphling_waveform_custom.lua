...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_morphling_custom/morphling_waveform_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_morphling_waveform_custom_buff",
	"heroes/npc_dota_hero_morphling_custom/morphling_waveform_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_waveform_custom_reverse",
	"heroes/npc_dota_hero_morphling_custom/morphling_waveform_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if self:GetCaster():HasModifier("modifier_morphling_12") then
		manacost = manacost
			- (
				manacost
				/ 100
				* self.modifier_morphling_12_mana[self:GetCaster():GetTalentLevel("modifier_morphling_12")]
			)
	end
	return manacost
end
...
function morphling_waveform_custom:OnSpellStart(new_point)
	...
	if self:GetCaster():HasModifier("modifier_morphling_13") then
		self:GetCaster():AddNewModifier(
			...
			"modifier_morphling_waveform_custom_reverse",
			{
				duration = self.modifier_morphling_13 + (distance / speed),
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_morphling_waveform_custom",
		{ x = point.x, y = point.y, z = point.z, proj = proj }
	)
end
	...
	if target == nil then
		...
		if self:GetCaster():HasModifier("modifier_morphling_5") then
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_morphling_waveform_custom_buff",
				{ duration = self.modifier_morphling_5_duration }
			)
		end
	end
end
...
function modifier_morphling_waveform_custom:IsPurgable()
	return false
end
function modifier_morphling_waveform_custom:IsHidden()
	return true
end
function modifier_morphling_waveform_custom:IgnoreTenacity()
	return true
end
function modifier_morphling_waveform_custom:IsMotionController()
	return true
end
function modifier_morphling_waveform_custom:GetMotionControllerPriority()
...
function modifier_morphling_waveform_custom_buff:GetTexture()
	return "morphling_5"
end
...
function modifier_morphling_waveform_custom_reverse:IsHidden()
	return true
end
function modifier_morphling_waveform_custom_reverse:IsPurgable()
	return false
end
function modifier_morphling_waveform_custom_reverse:IsPurgeException()
	return false
end
function modifier_morphling_waveform_custom_reverse:RemoveOnDeath()
	return false
end
...
function morphling_waveform_reverse_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_morphling_waveform_custom") then
		return
	end
	local modifier_morphling_waveform_custom_reverse = self:GetCaster()
		:FindModifierByName("modifier_morphling_waveform_custom_reverse")
