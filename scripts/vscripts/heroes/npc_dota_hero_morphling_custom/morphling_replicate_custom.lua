...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_morphling_custom/morphling_replicate_custom",
...
LinkLuaModifier(
	"modifier_morphling_replicate_custom_remove",
	"heroes/npc_dota_hero_morphling_custom/morphling_replicate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function morphling_replicate_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_morphling_replicate_custom_manager",
		{ duration = duration, name = name, model = model, target = target:entindex() }
	)
...
function morphling_morph_replicate_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_morphling_replicate_custom") then
		...
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), morphling_replicate_custom, "modifier_morphling_replicate_custom", {})
	end
end

modifier_morphling_replicate_custom_manager = class({})
function modifier_morphling_replicate_custom_manager:IsPurgable()
	return false
end
function modifier_morphling_replicate_custom_manager:IsPurgeException()
	return false
end
function modifier_morphling_replicate_custom_manager:GetTexture()
	return self.name
end
			...
			if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				self.target:AddNewModifier(
					...
					"modifier_morphling_replicate_custom_stats_debuff",
					{
						...
						str = self.target:GetStrength() / 100 * self:GetAbility().modifier_morphling_14,
						agi = 0,
						int = 0,
					}
				)
			...
			if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				self.target:AddNewModifier(
					...
					"modifier_morphling_replicate_custom_stats_debuff",
					{
						duration = self:GetDuration(),
						str = 0,
						agi = self.target:GetAgility() / 100 * self:GetAbility().modifier_morphling_14,
						int = 0,
					}
				)
			end
			...
			if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				self.target:AddNewModifier(
					...
					"modifier_morphling_replicate_custom_stats_debuff",
					{
						...
						str = 0,
						agi = 0,
						int = self.target:GetIntellect(false) / 100 * self:GetAbility().modifier_morphling_14,
					}
				)
			end
			self:GetCaster():AddNewModifier(
		...
		if self.target:GetPrimaryAttribute() == 3 then
			if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				self.target:AddNewModifier(
					...
					{
						duration = self:GetDuration(),
						str = self.target:GetStrength() / 100 * self:GetAbility().modifier_morphling_14 / 2,
						agi = self.target:GetAgility() / 100 * self:GetAbility().modifier_morphling_14 / 2,
						int = self.target:GetIntellect(false) / 100 * self:GetAbility().modifier_morphling_14 / 2,
					}
				)
			end
			self:GetCaster():AddNewModifier(
...
function modifier_morphling_replicate_custom:IsHidden()
	return true
end
function modifier_morphling_replicate_custom:IsPurgeException()
	return false
end
function modifier_morphling_replicate_custom:IsPurgable()
	return false
end
...
function modifier_morphling_replicate_custom:OnCreated()
	...
	local morphling_adaptive_strike_agi_custom = self:GetCaster()
		:FindAbilityByName("morphling_adaptive_strike_agi_custom")
	if self:GetCaster():HasModifier("modifier_morphling_15") then
	...
	local modifier_morphling_replicate_custom_manager =
		self:GetCaster():FindModifierByName("modifier_morphling_replicate_custom_manager")
				...
				if old_spell then
					...
					if new_ability then
						...
						if original and not original:IsNull() then
							...
							if original_ab and original_ab:GetLevel() > 0 then
								...
								if new_ability:GetIntrinsicModifierName() ~= nil then
									local modifier_intrinsic =
										self:GetParent():FindModifierByName(new_ability:GetIntrinsicModifierName())
						...
						self:GetCaster():SwapAbilities(
							self:GetCaster():GetAbilityByIndex(id):GetAbilityName(),
					...
					if new_ability then
						...
						if
							new_ability:GetAbilityName() == "invoker_quas_custom"
							or new_ability:GetAbilityName() == "invoker_wex_custom"
							or new_ability:GetAbilityName() == "invoker_exort_custom"
						then
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		{ value = 1, mod = self:GetParent():HasModifier("modifier_morphling_15") }
	)
...
function modifier_morphling_replicate_custom:OnDestroy()
		...
		if ability_in_slot and ability_in_slot:GetAbilityName() ~= ability_original then
			if ability_original.additional_ability and not ability_original.additional_ability:IsNull() then
				if ability_original:GetAbilityIndex() ~= ability_original.main_slot then
					self:GetCaster():SwapAbilities(
						ability_original:GetAbilityName(),
						self:GetCaster():GetAbilityByIndex(ability_original.main_slot):GetAbilityName(),
						false,
						true
					)
				end
	...
	CustomGameEventManager:Send_ServerToPlayer(
		PlayerResource:GetPlayer(self:GetCaster():GetPlayerOwnerID()),
		"morph_stats_replicated",
		{ value = 0, mod = self:GetParent():HasModifier("modifier_morphling_15") }
	)
end
...
function modifier_morphling_replicate_custom_remove:IsPurgable()
	return false
end
...
function modifier_morphling_replicate_custom_remove:OnIntervalThink()
	if IsServer() then
		...
		for i = #caster.spell_steal_history, 1, -1 do
			...
			if
				hSpell:GetMaxAbilityCharges(-1) > 0
				and hSpell:GetCurrentAbilityCharges() < hSpell:GetMaxAbilityCharges(-1)
				and caster:HasModifier("modifier_morphling_replicate_custom_manager")
			then
...
function modifier_morphling_replicate_custom_stats_debuff:GetTexture()
	return "morphling_14"
end
function modifier_morphling_replicate_custom_stats_debuff:IsPurgable()
	return false
end
function modifier_morphling_replicate_custom_stats_debuff:IsPurgeException()
	return false
end
...
function modifier_morphling_replicate_custom_stats_buff:GetTexture()
	return "morphling_14"
end
function modifier_morphling_replicate_custom_stats_buff:IsPurgable()
	return false
end
function modifier_morphling_replicate_custom_stats_buff:IsPurgeException()
	return false
end
