...
LinkLuaModifier(
	"modifier_morphling_morph_agi_custom",
	"heroes/npc_dota_hero_morphling_custom/morphling_morph_agi_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_morphling_morph_agi_custom_interval",
	"heroes/npc_dota_hero_morphling_custom/morphling_morph_agi_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_morphling_morph_agi_custom:IsPurgable()
	return false
end
function modifier_morphling_morph_agi_custom:IsHidden()
	return true
end
...
function modifier_morphling_morph_agi_custom_interval:IsPurgable()
	return false
end
function modifier_morphling_morph_agi_custom_interval:IsHidden()
	return true
end
...
function modifier_morphling_morph_agi_custom_interval:OnIntervalThink()
	...
	if math.floor(self:GetCaster():GetBaseStrength()) > 1 then
		...
		if not self:GetCaster():HasModifier("modifier_morphling_1") then
			self:GetCaster()
				:SpendMana((self:GetAbility():GetSpecialValueFor("mana_cost") * self.interval), self:GetAbility())
		end
		...
		self:GetParent():ModifyHealth(
			self:GetParent():GetHealth() - (22 - modify_health_check),
			self,
			false,
