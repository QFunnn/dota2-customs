...
LinkLuaModifier(
	"modifier_necrolyte_reapers_scythe_lua",
	"heroes/hero_necrolyte/necrolyte_reapers_scythe_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_necrolyte_reapers_scythe_lua_stack",
	"heroes/hero_necrolyte/necrolyte_reapers_scythe_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if IsValid(hTarget) then
		if not hTarget:TriggerSpellAbsorb(self) then
			...
			ParticleManager:SetParticleControlForward(
				particleID,
				0,
				(hTarget:GetAbsOrigin() - hCaster:GetAbsOrigin()):Normalized()
			)
