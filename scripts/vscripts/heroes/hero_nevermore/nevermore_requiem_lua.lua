...
LinkLuaModifier(
	"modifier_nevermore_requiem_phase_buff_lua",
	"heroes/hero_nevermore/nevermore_requiem_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_requiem_souls_lua",
	"heroes/hero_nevermore/nevermore_requiem_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function nevermore_requiem_lua:OnOwnerDied()
	...
	if self.caster:IsReincarnating() then
		return
	end
...
function nevermore_requiem_lua:OnProjectileHitHandle(target, location, projectile_id)
	...
	if not IsValidEntity(target) then -- projectile reaches the end
		if is_scepter_cast and damage_dealt_sum > 0 then
			...
			self.caster:HealWithParams(
				damage_dealt_sum * 0.01 * heal_pct_scepter,
				self,
				false,
				true,
				self.caster,
	...
	if (not is_death_cast) and (not target:HasModifier("modifier_item_aeon_disk_lua_buff")) then
		...
		if fear_modifier then
			fear_debuff_time =
				math.min(fear_modifier:GetRemainingTime() + debuff_time_increment, self.slow_duration_max)
...
function modifier_nevermore_requiem_phase_buff_lua:IsHidden()
	return true
end

function modifier_nevermore_requiem_phase_buff_lua:IsPurgable()
	return false
end

function modifier_nevermore_requiem_phase_buff_lua:IsDebuff()
	return false
end
