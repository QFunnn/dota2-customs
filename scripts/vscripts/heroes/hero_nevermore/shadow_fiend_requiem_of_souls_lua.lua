...
LinkLuaModifier(
	...
	"heroes/hero_nevermore/modifier_shadow_fiend_requiem_of_souls_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_shadow_fiend_requiem_of_souls_lua_scepter",
	"heroes/hero_nevermore/modifier_shadow_fiend_requiem_of_souls_lua_scepter",
	LUA_MODIFIER_MOTION_NONE
)
...
function shadow_fiend_requiem_of_souls_lua:OnSpellStart()
	...
	if self:GetCaster():HasScepter() then
		local explodeDuration = self:GetSpecialValueFor("requiem_radius")
			/ self:GetSpecialValueFor("requiem_line_speed")
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_shadow_fiend_requiem_of_souls_lua_scepter", {
		...
		if pass then
			...
			if not hTarget:HasModifier("modifier_nevermore_requiem_fear") then
				hTarget:AddNewModifier(
					...
					{ duration = 0.9 * (1 - hTarget:GetStatusResistance()) }
				)
			else
				hTarget:FindModifierByName("modifier_nevermore_requiem_fear"):SetDuration(
					math.min(
						hTarget:FindModifierByName("modifier_nevermore_requiem_fear"):GetRemainingTime() + 0.9,
						2.7
...
function shadow_fiend_requiem_of_souls_lua:Implode(lines, modifier)
	...
	for i = 0, lines - 1 do
		...
		ParticleManager:SetParticleControl(
			particle_lines_fx,
			0,
			self:GetCaster():GetOrigin() + facing_vector * line_length
		)
		ParticleManager:SetParticleControl(particle_lines_fx, 1, -velocity)
		ParticleManager:SetParticleControl(particle_lines_fx, 2, Vector(0, line_length / line_speed, 0))
		ParticleManager:ReleaseParticleIndex(particle_lines_fx)
