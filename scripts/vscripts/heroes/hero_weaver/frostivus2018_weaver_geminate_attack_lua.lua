...
LinkLuaModifier(
	"modifier_frostivus2018_weaver_geminate_attack_lua",
	"heroes/hero_weaver/frostivus2018_weaver_geminate_attack_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_frostivus2018_weaver_geminate_attack_lua_delay",
	"heroes/hero_weaver/frostivus2018_weaver_geminate_attack_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
...
function modifier_frostivus2018_weaver_geminate_attack_lua:OnAttack(params)
	...
	if
		...
		and hTarget:IsAlive()
		and (not hAttacker:PassivesDisabled())
		and self:GetAbility():IsCooldownReady()
		and (not hAttacker:AttackFilter(params.record, ATTACK_STATE_NOT_PROCESSPROCS, ATTACK_STATE_NO_EXTENDATTACK))
	then
		for i = 1, self.tooltip_attack do
			hParent:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_frostivus2018_weaver_geminate_attack_lua_delay",
				{ delay = i, index = hTarget:entindex() }
			)
		end
...
if modifier_frostivus2018_weaver_geminate_attack_lua_delay == nil then
	modifier_frostivus2018_weaver_geminate_attack_lua_delay = class({}) ---@class CDOTA_Modifier_Lua
end
...
function modifier_frostivus2018_weaver_geminate_attack_lua_delay:OnIntervalThink()
	...
	if not self.delay_done then
		...
		local units = FindUnitsInRadius(
			...
			nil,
			hParent:Script_GetAttackRange() + self.bonus_range,
	...
	if
		self.main_target_count > 0
		and IsValid(self.hTarget)
		and self.hTarget:IsAlive()
		and (self.hTarget.IsAttackImmune ~= nil and type(self.hTarget.IsAttackImmune) == "function" and not self.hTarget:IsAttackImmune())
		and (not self.hTarget:IsInvulnerable())
	then
		hParent:Attack(
			self.hTarget,
			ATTACK_STATE_SKIPCOOLDOWN
				+ ATTACK_STATE_IGNOREINVIS
				+ ATTACK_STATE_NO_EXTENDATTACK
