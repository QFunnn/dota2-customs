...
LinkLuaModifier(
	...
	"heroes/hero_weaver/frostivus2018_weaver_geminate_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_frostivus2018_weaver_geminate_attack_custom_dmg",
	"heroes/hero_weaver/frostivus2018_weaver_geminate_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_frostivus2018_weaver_geminate_attack_custom:IsHidden()
	return true
end
function modifier_frostivus2018_weaver_geminate_attack_custom:IsPurgable()
	return false
end
...
function modifier_frostivus2018_weaver_geminate_attack_custom:AttackModifier(params)
	...
	local enemies = FindUnitsInRadius(
		...
		nil,
		self:GetParent():Script_GetAttackRange() + self:GetAbility():GetSpecialValueFor("bonus_range"),
	...
	for i, enemy in pairs(enemies) do
		if attacks >= self:GetAbility():GetSpecialValueFor("arrow_count") then
			break
		end
		attacks = attacks + 1
		Timers:CreateTimer(0.07 * i, function()
			...
			then
				local damage_modifier = self:GetCaster():AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_frostivus2018_weaver_geminate_attack_custom_dmg",
					{}
...
function modifier_frostivus2018_weaver_geminate_attack_custom_dmg:IsHidden()
	return true
end
function modifier_frostivus2018_weaver_geminate_attack_custom_dmg:IsPurgable()
	return false
end
