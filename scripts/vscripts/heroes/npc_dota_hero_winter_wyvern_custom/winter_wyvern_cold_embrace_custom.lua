...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_cold_embrace_custom",
...
function modifier_winter_wyvern_cold_embrace_custom:IsPurgable()
	return false
end
function modifier_winter_wyvern_cold_embrace_custom:IsPurgeException()
	return false
end
function modifier_winter_wyvern_cold_embrace_custom:IsHidden()
	return false
end
	...
	then
		if not params.unit:IsOther() and self:GetParent():HasModifier("modifier_winter_wyvern_5") then
			local damageTable = {
				...
				damage = params.original_damage / 100 * self:GetAbility().modifier_winter_wyvern_5[self:GetCaster()
					:GetTalentLevel("modifier_winter_wyvern_5")],
...
function modifier_winter_wyvern_cold_embrace_custom:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_winter_wyvern_17") then
		local winter_wyvern_splinter_blast_custom = self:GetCaster()
			:FindAbilityByName("winter_wyvern_splinter_blast_custom")
		if winter_wyvern_splinter_blast_custom then
			winter_wyvern_splinter_blast_custom:OnSpellStart(
				self:GetParent(),
				self:GetAbility().modifier_winter_wyvern_17[self:GetCaster():GetTalentLevel("modifier_winter_wyvern_17")]
			)
		end
	end
end