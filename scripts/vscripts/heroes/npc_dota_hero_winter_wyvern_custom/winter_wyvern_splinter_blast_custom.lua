...
LinkLuaModifier(
	"modifier_winter_wyvern_splinter_blast_custom_slow",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_splinter_blast_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_winter_wyvern_splinter_blast_custom_talent_debuff",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_splinter_blast_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function winter_wyvern_splinter_blast_custom:OnSpellStart(new_target, new_radius)
	...
	if self:GetCaster():HasModifier("modifier_winter_wyvern_16") then
		damage = damage
			+ (
				self:GetCaster():Script_GetMagicalArmorValue(nil)
				* self.modifier_winter_wyvern_16[self:GetCaster():GetTalentLevel("modifier_winter_wyvern_16")]
			)
	end
...
function winter_wyvern_splinter_blast_custom:CreateTrackingProjectile(params)
	...
	ParticleManager:SetParticleControl(
		particle,
		0,
		caster:GetAttachmentOrigin(params.iSourceAttachment) + arctic_flight_offset
	)
...
function winter_wyvern_splinter_blast_custom:OnTrackingProjectileHit(params)
	...
	for _, enemy in pairs(nearby_enemy_units) do
		if
			(enemy ~= params.target or self:GetCaster():HasModifier("modifier_winter_wyvern_15")) and enemy:IsAlive()
		then
			local extra_data = {
	...
	if target and target:IsAlive() then
		...
		if self:GetCaster():HasModifier("modifier_winter_wyvern_20") then
			target:AddNewModifier(
				...
				"modifier_stunned",
				{
					duration = self.modifier_winter_wyvern_20[self:GetCaster()
						:GetTalentLevel("modifier_winter_wyvern_20")] * (1 - target:GetStatusResistance()),
				}
			)
		end
		if self:GetCaster():HasModifier("modifier_winter_wyvern_18") then
			target:AddNewModifier(
				...
				self,
				"modifier_winter_wyvern_splinter_blast_custom_talent_debuff",
				{ duration = self.modifier_winter_wyvern_18_duration * (1 - target:GetStatusResistance()) }
			)
		end
	end
end
...
function modifier_winter_wyvern_splinter_blast_custom_slow:IsDebuff()
	return true
end
...
function modifier_winter_wyvern_splinter_blast_custom_talent_debuff:GetTexture()
	return "winter_wyvern_18"
end
...
function modifier_winter_wyvern_splinter_blast_custom_talent_debuff:GetModifierMagicalResistanceBonus()
	return self:GetStackCount()
		* self:GetAbility().modifier_winter_wyvern_18[self:GetCaster():GetTalentLevel("modifier_winter_wyvern_18")]
end