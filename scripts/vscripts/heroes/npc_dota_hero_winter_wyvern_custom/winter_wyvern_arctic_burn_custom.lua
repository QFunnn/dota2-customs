...
LinkLuaModifier(
	"modifier_winter_wyvern_arctic_burn_custom",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_arctic_burn_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_winter_wyvern_arctic_burn_custom_debuff",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_arctic_burn_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_winter_wyvern_arctic_burn_custom_handler",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_arctic_burn_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function winter_wyvern_arctic_burn_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_winter_wyvern.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_winter_wyvern.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_winter_wyvern.vpcf",
		context
	)
...
function winter_wyvern_arctic_burn_custom:OnToggle()
	...
	local modifier_winter_wyvern_arctic_burn_custom = self:GetCaster()
		:FindModifierByName("modifier_winter_wyvern_arctic_burn_custom")
	if toggle then
...
function winter_wyvern_arctic_burn_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_winter_wyvern_arctic_burn_custom", { duration = duration })
end

modifier_winter_wyvern_arctic_burn_custom = class({})
function modifier_winter_wyvern_arctic_burn_custom:IsPurgable()
	return false
end
...
function modifier_winter_wyvern_arctic_burn_custom:OnCreated()
	...
	if self:GetCaster() == self:GetParent() then
		local nFXIndexB = ParticleManager:CreateParticle(
	...
	if self:GetParent():IsRangedAttacker() then
		...
		self:GetParent()
			:SetRangedProjectileName("particles/units/heroes/hero_winter_wyvern/winter_wyvern_arctic_attack.vpcf")
	end
	if self:GetCaster():HasModifier("modifier_winter_wyvern_14") then
...
function modifier_winter_wyvern_arctic_burn_custom:OnDestroy()
	...
	GridNav:DestroyTreesAroundPoint(
		self:GetParent():GetOrigin(),
		self:GetAbility():GetSpecialValueFor("tree_destruction_radius"),
		false
	)
	if self:GetParent():IsRangedAttacker() then
...
function modifier_winter_wyvern_arctic_burn_custom:OnAttackLanded(params)
	...
	params.target:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_winter_wyvern_arctic_burn_custom_debuff",
		{ duration = damage_duration * (1 - params.target:GetStatusResistance()) }
	)
...
function modifier_winter_wyvern_arctic_burn_custom:OnAttack(params)
	...
	--    if self.target_indexs[params.target:entindex()] then return end
	--end
	local modifier_winter_wyvern_arctic_burn_custom_handler =
		self:GetParent():FindModifierByName("modifier_winter_wyvern_arctic_burn_custom_handler")
...
function modifier_winter_wyvern_arctic_burn_custom_debuff:OnIntervalThink()
	...
	if self.current_frame >= self.tick_rate then
		...
		if self:GetCaster():HasModifier("modifier_winter_wyvern_8") then
			damage = damage
				+ (
					self:GetAbility().modifier_winter_wyvern_8[self:GetCaster()
						:GetTalentLevel("modifier_winter_wyvern_8")] * self.tick_rate
				)
		end
		ApplyDamage({
	...
	if self:GetCaster():HasModifier("modifier_winter_wyvern_12") then
		AddFOWViewer(
			...
			self:GetAbility().modifier_winter_wyvern_12[self:GetCaster():GetTalentLevel("modifier_winter_wyvern_12")],
			0.1,
			false
		)
	end
...
function modifier_winter_wyvern_arctic_burn_custom_handler:IsHidden()
	return true
end
function modifier_winter_wyvern_arctic_burn_custom_handler:IsPurgable()
	return false
end
function modifier_winter_wyvern_arctic_burn_custom_handler:IsPurgeException()
	return false
end
function modifier_winter_wyvern_arctic_burn_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_winter_wyvern_arctic_burn_custom_handler:OnAttackLanded(params)
	...
	if self:GetParent():HasModifier("modifier_winter_wyvern_arctic_burn_custom") then
		return
	end
