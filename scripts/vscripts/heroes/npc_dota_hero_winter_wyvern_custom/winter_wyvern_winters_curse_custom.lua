...
LinkLuaModifier(
	"modifier_winter_wyvern_winters_curse_custom",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_winters_curse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_winter_wyvern_winters_curse_custom_aggro",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_winters_curse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_winter_wyvern_winters_curse_custom_attack_speed",
	"heroes/npc_dota_hero_winter_wyvern_custom/winter_wyvern_winters_curse_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function winter_wyvern_winters_curse_custom:CastFilterResultTarget(hTarget)
	if
		...
		and not self:GetCaster():HasModifier("modifier_winter_wyvern_7")
	then
...
function modifier_winter_wyvern_winters_curse_custom:IsPurgable()
	return false
end
function modifier_winter_wyvern_winters_curse_custom:IsPurgeException()
	return false
end
...
function modifier_winter_wyvern_winters_curse_custom:OnAttackStart(params)
	...
	if not self:GetCaster():HasModifier("modifier_winter_wyvern_3") then
		return
	end
...
function modifier_winter_wyvern_winters_curse_custom:GetDisableHealing()
	if
		self:GetCaster():HasModifier("modifier_winter_wyvern_7")
...
function modifier_winter_wyvern_winters_curse_custom:GetModifierAvoidDamage(params)
	if params.attacker then
		local is_wyvern = (params.attacker:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID())
			or (params.attacker:HasModifier("modifier_winter_wyvern_winters_curse_custom_aggro"))
		if is_wyvern then
			return 0
		end
	end
...
function modifier_winter_wyvern_winters_curse_custom:OnCreated()
	...
	local overhead_fx = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_winter_wyvern/wyvern_winters_curse_overhead.vpcf",
	...
	local ground_fx = ParticleManager:CreateParticle(
...
function modifier_winter_wyvern_winters_curse_custom_aggro:OnIntervalThink(kv)
	...
	if self:GetAuraOwner():IsNull() then
		return
	end
...
function modifier_winter_wyvern_winters_curse_custom_attack_speed:GetTexture()
	return "winter_wyvern_3"
end
