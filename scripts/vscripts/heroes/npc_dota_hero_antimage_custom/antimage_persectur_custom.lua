...
LinkLuaModifier(
	"modifier_antimage_persectur_custom",
	"heroes/npc_dota_hero_antimage_custom/antimage_persectur_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_persectur_custom_slow",
	"heroes/npc_dota_hero_antimage_custom/antimage_persectur_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_antimage_persectur_custom:IsHidden()
	return true
end
function modifier_antimage_persectur_custom:IsPurgable()
	return false
end
function modifier_antimage_persectur_custom:RemoveOnDeath()
	return false
end
...
function modifier_antimage_persectur_custom:OnAttackLanded(params)
	...
	if self:GetCaster():HasModifier("modifier_antimage_4") then
		mana_threshold = mana_threshold
			+ self:GetAbility().modifier_antimage_4[self:GetCaster():GetTalentLevel("modifier_antimage_4")]
	end
...
function modifier_antimage_persectur_custom_slow:IsPurgable()
	return true
end
function modifier_antimage_persectur_custom_slow:IsDebuff()
	return true
end
...
function modifier_antimage_persectur_custom_slow:GetModifierMoveSpeedBonus_Percentage()
	...
	if self:GetCaster():HasModifier("modifier_antimage_4") then
		min_slow = min_slow
			+ self:GetAbility().modifier_antimage_4_minmax[self:GetCaster():GetTalentLevel("modifier_antimage_4")]
		max_slow = max_slow
			+ self:GetAbility().modifier_antimage_4_minmax[self:GetCaster():GetTalentLevel("modifier_antimage_4")]
		mana_threshold = mana_threshold
			+ self:GetAbility().modifier_antimage_4[self:GetCaster():GetTalentLevel("modifier_antimage_4")]
