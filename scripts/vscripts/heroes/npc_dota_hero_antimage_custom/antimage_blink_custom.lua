...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_antimage_custom/antimage_blink_custom",
...
function antimage_blink_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_antimage.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_antimage.vpcf",
		context
	)
end
...
function antimage_blink_custom:OnSpellStart()
	...
	if caster:HasModifier("modifier_antimage_9") then
		blink_range = blink_range
			+ antimage_blink_custom.modifier_antimage_9[self:GetCaster():GetTalentLevel("modifier_antimage_9")]
	end
	...
	if self:GetCaster():HasModifier("modifier_antimage_15") then
		local heal = self:GetCaster():GetMana()
			/ 100
			* self.modifier_antimage_15[self:GetCaster():GetTalentLevel("modifier_antimage_15")]
	...
	if self:GetCaster():HasModifier("modifier_antimage_20") then
		...
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_antimage_blink_custom_magic_immune", { duration = 0.3 })
	end
	...
	if self:GetCaster():HasModifier("modifier_antimage_21") then
		local enemies = FindUnitsInRadius(
			...
			nil,
			self.modifier_antimage_21_radius,
		...
		for _, enemy in pairs(enemies) do
			...
			ApplyDamage({
				...
				damage = self:GetCaster():GetMana() / 100 * self.modifier_antimage_21_damage,
...
function modifier_antimage_blink_custom_magic_immune:IsPurgable()
	return false
end
