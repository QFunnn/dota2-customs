...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_antimage_custom/antimage_mana_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_mana_break_custom_slow",
	"heroes/npc_dota_hero_antimage_custom/antimage_mana_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_mana_break_custom_antimage_16",
	"heroes/npc_dota_hero_antimage_custom/antimage_mana_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_mana_break_custom_antimage_16_stack",
	"heroes/npc_dota_hero_antimage_custom/antimage_mana_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function antimage_mana_break_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_antimage_5") then
		...
		local enemies = FindUnitsInRadius(
			...
			nil,
			self.modifier_antimage_5_radius[self:GetCaster():GetTalentLevel("modifier_antimage_5")],
...
function modifier_antimage_mana_break_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if target:HasModifier("modifier_kez_shodo_sai_custom") then
		return
	end
	if self:GetParent():HasModifier("modifier_antimage_16") then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_antimage_2") then
		percent_damage_per_burn = percent_damage_per_burn
			+ self:GetAbility().modifier_antimage_2[self:GetCaster():GetTalentLevel("modifier_antimage_2")]
	end

	if self:GetCaster():HasModifier("modifier_antimage_3") then
		mana_per_hit = mana_per_hit
			+ self:GetAbility().modifier_antimage_3[self:GetCaster():GetTalentLevel("modifier_antimage_3")]
	end
...
function modifier_antimage_mana_break_custom_antimage_16:IsPurgable()
	return false
end
...
function modifier_antimage_mana_break_custom_antimage_16_stack:IsHidden()
	return true
end
function modifier_antimage_mana_break_custom_antimage_16_stack:IsPurgable()
	return false
end
function modifier_antimage_mana_break_custom_antimage_16_stack:GetAttributes()
