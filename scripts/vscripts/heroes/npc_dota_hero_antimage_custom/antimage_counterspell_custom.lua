...
LinkLuaModifier(
	"modifier_antimage_counterspell_custom",
	"heroes/npc_dota_hero_antimage_custom/antimage_counterspell_custom",
...
LinkLuaModifier(
	"modifier_antimage_counterspell_aura_9",
	"heroes/npc_dota_hero_antimage_custom/antimage_counterspell_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_counterspell_aura_9_debuff",
	"heroes/npc_dota_hero_antimage_custom/antimage_counterspell_custom",
	LUA_MODIFIER_MOTION_NONE
)

LinkLuaModifier(
	"modifier_antimage_counterspell_aura_10",
	"heroes/npc_dota_hero_antimage_custom/antimage_counterspell_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_antimage_counterspell_aura_10_debuff",
	"heroes/npc_dota_hero_antimage_custom/antimage_counterspell_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function antimage_counterspell_custom:OnSpellStart(target)
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_antimage_counterspell_custom_active", { duration = duration })
	...
	if self:GetCaster():HasModifier("modifier_antimage_8") then
		...
		local t = CreateIllusions(
			self:GetCaster(),
			self:GetCaster(),
			{
				duration = self.modifier_antimage_8_duration[self:GetCaster():GetTalentLevel("modifier_antimage_8")],
				outgoing_damage = damage_out,
				incoming_damage = damage_inc,
			},
			1,
			100,
			false,
		...
		if ill:HasModifier("modifier_antimage_mana_overload_custom_illusion") then
			...
			if self:GetCaster():HasModifier("modifier_antimage_8") then
				...
				local t = CreateIllusions(
					...
					self:GetCaster(),
					{
						duration = self.modifier_antimage_8_duration[self:GetCaster()
							:GetTalentLevel("modifier_antimage_8")],
						outgoing_damage = damage_out,
						incoming_damage = damage_inc,
					},
					1,
...
function modifier_antimage_counterspell_custom:IsPurgable()
	return false
end
		...
		if self:GetCaster():HasModifier("modifier_antimage_7") then
			if self:GetAbility():IsFullyCastable() then
				if
					...
					and not self:GetParent():HasModifier("modifier_wodarelax")
					and not self:GetParent():HasModifier("modifier_wodawisp")
				then
...
local function SpellReflect(parent, params)
	...
	if reflected_spell_name == "tidehunter_dead_in_the_water" then
		return
	end
...
function modifier_antimage_counterspell_custom_active:GetReflectSpell(params)
	...
	if self:GetCaster():HasModifier("modifier_antimage_11") then
		...
		local t = CreateIllusions(
			...
			self:GetCaster(),
			{
				duration = self:GetAbility().modifier_antimage_11_duration[self:GetCaster()
					:GetTalentLevel("modifier_antimage_11")],
				outgoing_damage = damage_out,
				incoming_damage = damage_inc,
			},
			1,
...
function modifier_antimage_counterspell_aura_9:IsHidden()
	return true
end
function modifier_antimage_counterspell_aura_9:RemoveOnDeath()
	return false
end
function modifier_antimage_counterspell_aura_9:IsPurgable()
	return false
end
...
function modifier_antimage_counterspell_aura_10:IsHidden()
	return true
end
function modifier_antimage_counterspell_aura_10:RemoveOnDeath()
	return false
end
function modifier_antimage_counterspell_aura_10:IsPurgable()
	return false
end
...
function modifier_antimage_counterspell_aura_9_debuff:IsPurgable()
	return false
end
