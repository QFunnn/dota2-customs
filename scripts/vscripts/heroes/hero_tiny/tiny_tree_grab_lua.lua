...
tiny_tree_grab_lua = class({}) ---@class tiny_tree_grab_lua : CDOTA_Ability_Lua
...
function tiny_tree_grab_lua:OnSpellStart(...)
	...
	local treeThrowAbility = caster:FindAbilityByName("tiny_tree_throw_lua")
	local treeGrabAbility = caster:FindAbilityByName("tiny_tree_grab_lua")

	if treeGrabAbility and treeThrowAbility then
		caster:SwapAbilities(treeGrabAbility, treeThrowAbility, false, true)
	end
	if treeThrowAbility then
		treeThrowAbility:SetHidden(false)
	end
