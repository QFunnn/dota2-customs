...
function modifier_tiny_grab_lua:IsPurgable()
	return false
end
function modifier_tiny_grab_lua:RemoveOnDeath()
	return true
end
...
function modifier_tiny_grab_lua:OnCreated(...)
	...
	self.splash_pct = ability:GetSpecialValueFor("splash_pct") / 100
...
function modifier_tiny_grab_lua:AllowIllusionDuplicate()
	return false
end
...
function modifier_tiny_grab_lua:DeclareFunctions(...)
	return {
		...
		MODIFIER_EVENT_ON_ATTACK_LANDED, -- GetModifierProcAttack_Feedback
...
function modifier_tiny_grab_lua:OnAttackLanded(params)
...
function modifier_tiny_grab_lua:OnRemoved()
	...
	local treeGrabAbility = parent:FindAbilityByName("tiny_tree_grab_lua")
	local treeThrowAbility = parent:FindAbilityByName("tiny_tree_throw_lua")
	if treeGrabAbility and treeGrabAbility then
		parent:SwapAbilities(treeGrabAbility, treeThrowAbility, true, false)
	end
	if treeGrabAbility then
		treeGrabAbility:UseResources(false, false, false, true)
	end
end