...
LinkLuaModifier(
	"modifier_faceless_void_time_lock_lua",
	"heroes/hero_faceless_void/faceless_void_time_lock_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_faceless_void_time_lock_lua_stun",
	"heroes/hero_faceless_void/faceless_void_time_lock_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
...
function faceless_void_time_lock_lua:Bash(hTarget, tAttackInfo, bTriggerbyWalk)
	...
	if IsValid(hTarget) and hTarget:IsAlive() then
		...
		hTarget:AddNewModifier(
			...
			self,
			"modifier_faceless_void_time_lock_lua_stun",
			{ duration = duration * hTarget:GetStatusResistanceFactor(hCaster) }
		)
		EmitSoundOn("Hero_FacelessVoid.TimeLockImpact", hTarget)
		if
			(
				hCaster.time_locking
				or bTriggerbyWalk
				or not hCaster:AttackFilter(
					tAttackInfo.record,
					ATTACK_STATE_NOT_PROCESSPROCS,
...
function modifier_faceless_void_time_lock_lua:OnAbilityExecuted(params)
	...
	if IsServer() then
		if
			...
			and hCaster:HasScepter()
			and hAbility:GetAbilityName() == "faceless_void_time_walk"
		then
			...
			local fWalkRange = hAbility:GetSpecialValueFor("range")
				+ hAbility:GetSpecialValueFor("shard_bonus_range")
				+ hCaster:GetCastRangeBonus()
...
function modifier_faceless_void_time_lock_lua:GetModifierProcAttack_Feedback(params)
	...
	then
		...
		if
			...
			and PRD(hParent, self.chance_pct, "faceless_void_time_lock_lua")
		then
