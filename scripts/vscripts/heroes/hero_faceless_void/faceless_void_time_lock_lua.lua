...
function faceless_void_time_lock_lua:Bash(target, attackInfo, isTriggerbyWalk)
	...
	if
		(
			caster.time_locking
			or isTriggerbyWalk
			or not caster:AttackFilter(attackInfo.record, ATTACK_STATE_NOT_PROCESSPROCS, ATTACK_STATE_NO_EXTENDATTACK)
...
function modifier_faceless_void_time_lock_lua:OnAbilityExecuted(event)
	...
	if
		...
		and ability:GetAbilityName() == "faceless_void_time_walk"
	then
		...
		local fWalkRange = ability:GetSpecialValueFor("range")
			+ ability:GetSpecialValueFor("shard_bonus_range")
...
function modifier_faceless_void_time_lock_lua:GetModifierProcAttack_Feedback(params)
	...
	then
		...
		if
			...
			and RollPseudoRandomPercentage(self.chance_pct, DOTA_PSEUDO_RANDOM_FACELESS_BASH, hParent)
		then
