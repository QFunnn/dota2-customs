...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_arcane_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_arcane_aura_custom_talent_debuff_use_ability",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_arcane_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function crystal_maiden_arcane_aura_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_crystal_maiden.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_crystal_maiden.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_crystal_maiden.vpcf",
		context
	)
end
...
function modifier_crystal_maiden_arcane_aura_custom:IsHidden()
	return true
end
function modifier_crystal_maiden_arcane_aura_custom:IsPurgable()
	return false
end
function modifier_crystal_maiden_arcane_aura_custom:IsPurgeException()
	return false
end
function modifier_crystal_maiden_arcane_aura_custom:RemoveOnDeath()
	return false
end
function modifier_crystal_maiden_arcane_aura_custom:IsAura()
	return not self:GetCaster():HasModifier("modifier_crystal_maiden_7")
end
...
function modifier_crystal_maiden_arcane_aura_custom:GetModifierMPRegenAmplify_Percentage()
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_17") then
		bonus = self:GetAbility().modifier_crystal_maiden_17[self:GetCaster()
			:GetTalentLevel("modifier_crystal_maiden_17")]
	end
...
function modifier_crystal_maiden_arcane_aura_custom_buff:GetModifierConstantManaRegen()
	...
	if distance <= self:GetAbility():GetSpecialValueFor("proximity_bonus_radius") then
		return (
			self:GetAbility():GetSpecialValueFor("base_mana_regen")
			* self:GetAbility():GetSpecialValueFor("proximity_bonus_factor")
		) * bonus
	end
...
function modifier_crystal_maiden_arcane_aura_custom_talent_debuff_use_ability:IsPurgable()
	return false
end
function modifier_crystal_maiden_arcane_aura_custom_talent_debuff_use_ability:IsDebuff()
	return true
end
