...
LinkLuaModifier(
	"modifier_crystal_maiden_glacial_guard_custom",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_glacial_guard_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_glacial_guard_custom_buff",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_glacial_guard_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_crystal_maiden_glacial_guard_custom:IsHidden()
	return true
end
function modifier_crystal_maiden_glacial_guard_custom:IsPurgable()
	return false
end
function modifier_crystal_maiden_glacial_guard_custom:IsPurgeException()
	return false
end
function modifier_crystal_maiden_glacial_guard_custom:RemoveOnDeath()
	return false
end
function modifier_crystal_maiden_glacial_guard_custom:IsPurgeException()
	return false
...
function modifier_crystal_maiden_glacial_guard_custom:OnSpentMana(params)
	...
	if self:GetParent():HasModifier("modifier_crystal_maiden_19") then
		mana_multiplier = mana_multiplier
			+ self:GetAbility().modifier_crystal_maiden_19[self:GetParent():GetTalentLevel("modifier_crystal_maiden_19")]
	end
	if params.cost > 0 and not params.ability:IsItem() then
		self:GetParent():AddNewModifier(
			...
			"modifier_crystal_maiden_glacial_guard_custom_buff",
			{
				duration = self:GetAbility():GetSpecialValueFor("barrier_duration"),
				barrier = params.cost / 100 * mana_multiplier,
			}
		)
	end
end
...
function modifier_crystal_maiden_glacial_guard_custom_buff:GetModifierIncomingDamageConstant(params)
	if not self:GetParent():HasModifier("modifier_crystal_maiden_19") then
		return
	end
