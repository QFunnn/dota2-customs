...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_crystal_nova_custom.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_crystal_nova_custom_frostbite_talent",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_crystal_nova_custom.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_crystal_nova_custom_debuff_regeneration",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_crystal_nova_custom.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function crystal_maiden_crystal_nova_custom:GetCastRange(location, target)
	if self:GetCaster():HasModifier("modifier_crystal_maiden_3") then
		return self:GetSpecialValueFor("radius")
			+ self.modifier_crystal_maiden_3_bonus_radius[self:GetCaster():GetTalentLevel("modifier_crystal_maiden_3")]
	end
...
function crystal_maiden_crystal_nova_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_4") then
		damage = damage
			+ (self:GetCaster():GetMaxHealth() / 100 * self.modifier_crystal_maiden_4_damage_from_max_health)
	end
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			self,
			"modifier_crystal_maiden_crystal_nova_custom",
			{ duration = debuffDuration * (1 - enemy:GetStatusResistance()) }
		)
		if self:GetCaster():HasModifier("modifier_crystal_maiden_2") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_crystal_maiden_crystal_nova_custom_frostbite_talent",
				{ duration = self.modifier_crystal_maiden_2_stack_duration }
			)
		end
		if self:GetCaster():HasModifier("modifier_crystal_maiden_5") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_crystal_maiden_crystal_nova_custom_debuff_regeneration",
				{ duration = self.modifier_crystal_maiden_5_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
	end
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_6") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_crystal_maiden_crystal_nova_custom_buff_regeneration",
			{ duration = self.modifier_crystal_maiden_6_duration }
		)
	end
...
function modifier_crystal_maiden_crystal_nova_custom_debuff_regeneration:IsPurgable()
	return true
end
...
function modifier_crystal_maiden_crystal_nova_custom_debuff_regeneration:GetModifierHPRegenAmplify_Percentage()
	return self:GetAbility().modifier_crystal_maiden_5_regeneration_reduce[self:GetCaster()
		:GetTalentLevel("modifier_crystal_maiden_5")]
end

modifier_crystal_maiden_crystal_nova_custom_buff_regeneration = class({})

function modifier_crystal_maiden_crystal_nova_custom_buff_regeneration:IsPurgable()
	return true
end
...
function modifier_crystal_maiden_crystal_nova_custom_buff_regeneration:GetModifierConstantHealthRegen()
	return self:GetAbility().modifier_crystal_maiden_6_bonus_regeneration_per_effect[self:GetCaster():GetTalentLevel(
		"modifier_crystal_maiden_6"
	)] * self:GetStackCount()
end

modifier_crystal_maiden_crystal_nova_custom_frostbite_talent = class({})

function modifier_crystal_maiden_crystal_nova_custom_frostbite_talent:IsPurgable()
	return false
end
function modifier_crystal_maiden_crystal_nova_custom_frostbite_talent:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/crystal_maiden_2stack.vpcf",
