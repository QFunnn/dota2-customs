...
LinkLuaModifier(
	"modifier_crystal_maiden_mass_frostbite",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_mass_frostbite",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_mass_frostbite_thinker",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_mass_frostbite",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function crystal_maiden_mass_frostbite:OnSpellStart()
	...
	local crystal_maiden_unit =
		CreateUnitByName("npc_dota_companion", origin, false, nil, nil, self:GetCaster():GetTeamNumber())
	if crystal_maiden_unit then
		crystal_maiden_unit:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_crystal_maiden_mass_frostbite",
			{ duration = clone_duration - FrameTime() }
		)
	...
	local knockback = self:GetCaster():AddNewModifier(
		...
		"modifier_generic_knockback_lua",
		{
			duration = hop_duration_custom,
			distance = hop_distance_custom,
			height = 0,
...
function modifier_crystal_maiden_mass_frostbite:IsHidden()
	return true
end

function modifier_crystal_maiden_mass_frostbite:IsPurgable()
	return false
end
...
function modifier_crystal_maiden_mass_frostbite_thinker:IsHidden()
	return true
end
function modifier_crystal_maiden_mass_frostbite_thinker:IsPurgable()
	return false
end
