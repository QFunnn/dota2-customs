...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_frostbite_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_crystal_maiden_frostbite_custom_passive",
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_frostbite_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function crystal_maiden_frostbite_custom:OnSpellStart(target)
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_10") then
		...
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_crystal_maiden_frostbite_custom_magic_immune",
			{ duration = self.modifier_crystal_maiden_10_duration }
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_11") then
		tick_damage = tick_damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_crystal_maiden_11_damage_agility[self:GetCaster():GetTalentLevel(
					"modifier_crystal_maiden_11"
				)]
			)
	end
	if self:GetCaster():HasModifier("modifier_crystal_maiden_15") and not self:GetParent():IsHero() then
		tick_damage = tick_damage
			+ (
				tick_damage
				/ 100
				* self:GetAbility().modifier_crystal_maiden_15[self:GetCaster()
					:GetTalentLevel("modifier_crystal_maiden_15")]
			)
	end
...
function modifier_crystal_maiden_frostbite_custom_passive:IsHidden()
	return true
end
function modifier_crystal_maiden_frostbite_custom_passive:IsPurgable()
	return false
end
...
function modifier_crystal_maiden_frostbite_custom_passive:OnTakeDamage(params)
	...
	if not self:GetParent():HasModifier("modifier_crystal_maiden_9") then
		return
	end
	if self:GetParent():HasModifier("modifier_crystal_maiden_frostbite_custom_cooldown") then
		return
	end
	...
	self:GetCaster():AddNewModifier(
		...
		"modifier_crystal_maiden_frostbite_custom_cooldown",
		{
			duration = self:GetAbility().modifier_crystal_maiden_9_cooldown[self:GetCaster():GetTalentLevel(
				"modifier_crystal_maiden_9"
			)],
		}
	)
end
...
function modifier_crystal_maiden_frostbite_custom_cooldown:IsPurgable()
	return false
end
function modifier_crystal_maiden_frostbite_custom_cooldown:IsDebuff()
	return true
end
...
function modifier_crystal_maiden_frostbite_custom_magic_immune:IsPurgable()
	return false
end
