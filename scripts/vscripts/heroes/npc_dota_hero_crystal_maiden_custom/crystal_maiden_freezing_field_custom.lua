...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_crystal_maiden_custom/crystal_maiden_freezing_field_custom",
...
function crystal_maiden_freezing_field_custom:OnToggle()
	...
	if not self:GetCaster():HasModifier("modifier_crystal_maiden_7") then
		return
	end
...
function modifier_crystal_maiden_freezing_field_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_crystal_maiden_7") then
		...
		if self.cooldown_damage >= self:GetAbility().modifier_crystal_maiden_7_damage_cooldown then
			local damage = self:GetCaster():GetMaxHealth()
				/ 100
				* self:GetAbility().modifier_crystal_maiden_7_self_damage
			ApplyDamage({
				attacker = self:GetCaster(),
