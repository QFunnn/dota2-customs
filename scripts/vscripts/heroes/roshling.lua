...
function modifier_gold_roshan_spell:IsHidden()
	return true
end
function modifier_gold_roshan_spell:IsPurgable()
	return false
end
function modifier_gold_roshan_spell:IsPurgeException()
	return false
end
...
function modifier_gold_roshan_bash:IsHidden()
	return true
end
function modifier_gold_roshan_bash:IsPurgable()
	return false
end
function modifier_gold_roshan_bash:IsPurgeException()
	return false
end
...
function modifier_gold_roshan_bash:AttackLandedModifier(params)
	...
	if RollPercentage(self:GetAbility():GetSpecialValueFor("chance")) and self:GetAbility():IsFullyCastable() then
		...
		params.target:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_bashed",
			{ duration = self:GetAbility():GetSpecialValueFor("duration") * (1 - params.target:GetStatusResistance()) }
		)
	end
end
...
function gold_roshan_clap:OnSpellStart()
	...
	local particle =
		ParticleManager:CreateParticle("particles/neutral_fx/roshan_slam.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
