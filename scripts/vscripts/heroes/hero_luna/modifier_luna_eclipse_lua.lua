...
function modifier_luna_eclipse_lua:OnRemoved() end

function modifier_luna_eclipse_lua:OnDestroy() end
			...
			if self.hits[unit] <= self.hit_count then
				...
				if self.aoe_radius > 0 then
					...
					for _, enemy in pairs(enemies) do
						if IsValid(enemy) and enemy:IsAlive() then
							enemy:AddNewModifier(
								...
								self:GetAbility(),
								"modifier_stunned",
								{ duration = self.stun_duration * enemy:GetStatusResistanceFactor(self:GetCaster()) }
							)
					...
					unit:AddNewModifier(
						...
						self:GetAbility(),
						"modifier_stunned",
						{ duration = self.stun_duration * unit:GetStatusResistanceFactor(self:GetCaster()) }
					)
					-- precache damage
