...
LinkLuaModifier(
	"modifier_witch_doctor_death_ward_lua",
	"heroes/hero_witch_doctor/witch_doctor_death_ward_lua.lua",
...
function witch_doctor_death_ward_lua:OnSpellStart()
	...
	self.hDeathWard =
		CreateUnitByName("npc_dota_witch_doctor_death_ward", vPosition, true, hCaster, hCaster, hCaster:GetTeamNumber())
...
function witch_doctor_death_ward_lua:Bounce(hDeathWard, hTarget, fDamage, tHashtable)
	...
	local hNextTarget = Util:GetBounceTarget(
		hTarget,
		hDeathWard:GetTeamNumber(),
...
function modifier_witch_doctor_death_ward_lua:OnIntervalThink()
	...
	if IsServer() then
		...
		if
			not IsValid(hForceAttackTarget)
			or not hForceAttackTarget:IsPositionInRange(vPosition, fRange)
			or UnitFilter(
				hForceAttackTarget,
				DOTA_UNIT_TARGET_TEAM_ENEMY,
				DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
			...
			or hForceAttackTarget:HasState(MODIFIER_STATE_DEBUFF_IMMUNE)
		then
...
function modifier_witch_doctor_death_ward_lua:OnOrder(params)
	...
	if params.unit == hParent then
		if params.order_type == DOTA_UNIT_ORDER_ATTACK_TARGET then
			...
			if
				IsValid(hTarget)
				and hTarget.IsPositionInRange
				and type(hTarget.IsPositionInRange) == "function"
				and hTarget:IsPositionInRange(vPosition, fRange)
				and UnitFilter(
						hTarget,
						DOTA_UNIT_TARGET_TEAM_ENEMY,
						DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
