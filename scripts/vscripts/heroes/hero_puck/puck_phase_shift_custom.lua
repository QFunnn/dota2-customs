...
LinkLuaModifier(
	...
	"heroes/hero_puck/puck_phase_shift_custom",
...
function modifier_puck_phase_shift_custom:IsPurgable()
	return false
end
...
function modifier_puck_phase_shift_custom:OnCreated(table)
	...
	local nStatusFX =
		ParticleManager:CreateParticle("particles/status_fx/status_effect_phase_shift.vpcf", PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControlEnt(
		nStatusFX,
		0,
		self:GetParent(),
	...
	if self:GetParent():HasShard() then
		local enemies = FindUnitsInRadius(
			...
			nil,
			self:GetParent():Script_GetAttackRange() + self:GetAbility():GetSpecialValueFor("shard_attack_range_bonus"),
