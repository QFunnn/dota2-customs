...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_razor_custom/razor_eye_of_the_storm_custom",
...
function razor_eye_of_the_storm_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_razor_18") then
		...
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_razor_eye_of_the_storm_custom", { duration = duration })
	end
end
...
function modifier_razor_eye_of_the_storm_custom_debuff:OnRefresh(kv)
	...
	if not self:GetCaster():HasModifier("modifier_razor_7") then
		if self:GetStackCount() >= max_debuffs then
			return
		end
	end
