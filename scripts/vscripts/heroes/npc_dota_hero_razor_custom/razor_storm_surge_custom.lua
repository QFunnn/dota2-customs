...
LinkLuaModifier(
	"modifier_razor_storm_surge_custom",
	"heroes/npc_dota_hero_razor_custom/razor_storm_surge_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_storm_surge_custom_slow",
	"heroes/npc_dota_hero_razor_custom/razor_storm_surge_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_storm_surge_custom_cooldown",
	"heroes/npc_dota_hero_razor_custom/razor_storm_surge_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function razor_storm_surge_custom:GetCooldown(iLevel)
	if
		self:GetCaster():HasModifier("modifier_razor_6")
		and self:GetCaster():HasModifier("modifier_razor_eye_of_the_storm_custom")
	then
...
function modifier_razor_storm_surge_custom:IsHidden()
	return true
end
function modifier_razor_storm_surge_custom:IsPurgable()
	return false
end
...
function modifier_razor_storm_surge_custom:HitEnemies(attacker, start_cooldown)
	...
	if start_cooldown then
		...
		if
			self.parent:HasModifier("modifier_razor_6")
			and self:GetCaster():HasModifier("modifier_razor_eye_of_the_storm_custom")
		then
	...
	if
		self:GetCaster():HasModifier("modifier_razor_eye_of_the_storm_custom")
		and self:GetCaster():HasModifier("modifier_razor_6")
	then
		local heroes = FindUnitsInRadius(
			self.parent:GetTeamNumber(),
			self.parent:GetOrigin(),
			nil,
			self:GetAbility().modifier_razor_6_radius,
			DOTA_UNIT_TARGET_TEAM_ENEMY,
	...
	if self:GetCaster():HasModifier("modifier_razor_5") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_razor_5[self:GetCaster():GetTalentLevel("modifier_razor_5")]
			)
	end
	local damage_table =
...
function modifier_razor_storm_surge_custom:OnAbilityFullyCast(params)
	...
	if self.parent:HasModifier("modifier_razor_storm_surge_custom_cooldown") then
		return
	end
...
function modifier_razor_storm_surge_custom:GetModifierIncomingDamage_Percentage(params)
	if not self:GetParent():HasModifier("modifier_razor_4") then
		return
	end
...
function modifier_razor_storm_surge_custom_slow:IsHidden()
	return false
end
function modifier_razor_storm_surge_custom_slow:IsPurgable()
	return true
end
...
function modifier_razor_storm_surge_custom_cooldown:IsHidden()
	return true
end
function modifier_razor_storm_surge_custom_cooldown:IsPurgable()
	return false
end
function modifier_razor_storm_surge_custom_cooldown:IsDebuff()
	return true
end