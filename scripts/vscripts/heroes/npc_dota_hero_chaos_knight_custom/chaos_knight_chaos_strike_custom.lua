...
LinkLuaModifier(
	"modifier_chaos_knight_chaos_strike_custom",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_chaos_strike_custom",
...
function modifier_chaos_knight_chaos_strike_custom:IsPurgeException()
	return false
end
function modifier_chaos_knight_chaos_strike_custom:RemoveOnDeath()
	return false
end
...
function modifier_chaos_knight_chaos_strike_custom:GetModifierPreAttack_CriticalStrike(params)
	if IsServer() and (not self:GetParent():PassivesDisabled()) then
		if self:GetParent():HasModifier("modifier_chaos_knight_19") then
			return
		end
		...
		if self:GetCaster():HasModifier("modifier_chaos_knight_6") then
			if self:GetCaster():GetHealthPercent() < self:GetAbility().modifier_chaos_knight_6_health then
				bonus_chance = self:GetAbility().modifier_chaos_knight_6[self:GetCaster()
					:GetTalentLevel("modifier_chaos_knight_6")]
			end
		end
		if RollPseudoRandomPercentage(self.chance + bonus_chance, DOTA_PSEUDO_RANDOM_CHAOS_CRIT, self:GetParent()) then
			...
			if self:GetCaster():HasModifier("modifier_chaos_knight_5") then
				bonus = self:GetAbility().modifier_chaos_knight_5[self:GetCaster()
					:GetTalentLevel("modifier_chaos_knight_5")]
			end
