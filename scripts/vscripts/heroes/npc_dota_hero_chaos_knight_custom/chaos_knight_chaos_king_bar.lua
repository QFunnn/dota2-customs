...
LinkLuaModifier(
	"modifier_chaos_knight_chaos_king_bar",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_chaos_king_bar",
...
function chaos_knight_chaos_king_bar:Precache(context)
	PrecacheResource(
		"particle",
		"particles/econ/items/lifestealer/lifestealer_immortal_backbone/chaos_immortal_backbone_rage.vpcf",
		context
	)
end
...
function chaos_knight_chaos_king_bar:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_chaos_knight_chaos_king_bar", { duration = duration })
	self:GetCaster():EmitSound("DOTA_Item.MinotaurHorn.Cast")
end

modifier_chaos_knight_chaos_king_bar = class({})
function modifier_chaos_knight_chaos_king_bar:IsPurgable()
	return false
end
function modifier_chaos_knight_chaos_king_bar:IsPurgeException()
	return false
end
