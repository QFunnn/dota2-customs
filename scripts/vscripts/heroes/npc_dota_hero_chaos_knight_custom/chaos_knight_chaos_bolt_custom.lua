...
function chaos_knight_chaos_bolt_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_chaos_knight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_chaos_knight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_chaos_knight.vpcf",
		context
	)
end
...
function chaos_knight_chaos_bolt_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_chaos_knight_18") then
		chaos_bolt_speed_mult = chaos_bolt_speed_mult
			+ (self.modifier_chaos_knight_18[self:GetCaster():GetTalentLevel("modifier_chaos_knight_18")] / 100)
	end
	local counter = 1
...
function chaos_knight_chaos_bolt_custom:OnProjectileHit_ExtraData(target, vLocation, kv)
	...
	if not is_spell_absorb then
		...
		if self:GetCaster():HasModifier("modifier_chaos_knight_16") then
			damage_min = damage_min
				+ (
					...
					/ 100
					* self.modifier_chaos_knight_16[self:GetCaster():GetTalentLevel("modifier_chaos_knight_16")]
				)
		...
		target:AddNewModifier(
			...
			self,
			"modifier_stunned",
			{ duration = stun_act * (1 - target:GetStatusResistance()) }
		)
		...
		if self:GetCaster():HasModifier("modifier_chaos_knight_9") and kv.main then
			...
			if chaos_knight_phantasm_custom and chaos_knight_phantasm_custom:GetLevel() > 0 then
				chaos_knight_phantasm_custom:CreateOneIllusion(
					target:GetAbsOrigin(),
					self.modifier_chaos_knight_9[self:GetCaster():GetTalentLevel("modifier_chaos_knight_9")],
					self.modifier_chaos_knight_9_damage
				)
			end
		end
	end
