...
LinkLuaModifier(
	"modifier_chaos_knight_phantasm_custom",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_phantasm_custom",
...
function chaos_knight_phantasm_custom:CreateOneIllusion(point, new_duration, new_outgoing, new_hero_copy)
	...
	if self:GetCaster():HasModifier("modifier_chaos_knight_11") then
		incoming_damage = incoming_damage
			+ self.modifier_chaos_knight_11[self:GetCaster():GetTalentLevel("modifier_chaos_knight_11")]
	end
	...
	local illusions = CreateIllusions(
		self:GetCaster(),
		hero_copy,
		{ duration = illusion_duration, outgoing_damage = outgoing_damage, incoming_damage = incoming_damage },
		1,
		150,
		false,
		false
...
function modifier_chaos_knight_phantasm_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_chaos_knight_11") then
		self.incoming_damage = self.incoming_damage
			+ self:GetAbility().modifier_chaos_knight_11[self:GetCaster():GetTalentLevel("modifier_chaos_knight_11")]
	end
...
function modifier_chaos_knight_phantasm_custom:OnDestroy(kv)
	...
	if self:GetCaster():HasModifier("modifier_chaos_knight_14") then
		local heroes = FindUnitsInRadius(
			...
			nil,
			self:GetAbility().modifier_chaos_knight_14,
