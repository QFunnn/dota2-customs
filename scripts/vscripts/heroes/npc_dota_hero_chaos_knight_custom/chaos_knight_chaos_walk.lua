...
LinkLuaModifier(
	"modifier_chaos_knight_chaos_walk",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_chaos_walk",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_chaos_knight_chaos_walk_thinker",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_chaos_walk",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_chaos_knight_chaos_walk_debuff",
	"heroes/npc_dota_hero_chaos_knight_custom/chaos_knight_chaos_walk",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_chaos_knight_chaos_walk:IsPurgable()
	return false
end
function modifier_chaos_knight_chaos_walk:IsPurgeException()
	return false
end
function modifier_chaos_knight_chaos_walk:RemoveOnDeath()
	return false
end
function modifier_chaos_knight_chaos_walk:IsHidden()
	return true
end
	...
	if self:GetParent():IgnoreWispAndInvisAndRelax(true, true, true) then
		if self.distance <= 0 or self.spawner_time <= 0 then
			...
			CreateModifierThinker(
				...
				self:GetAbility(),
				"modifier_chaos_knight_chaos_walk_thinker",
				{ duration = self.path_duration, x = self.pos.x, y = self.pos.y },
...
function modifier_chaos_knight_chaos_walk_thinker:PlayEffects(duration)
	local particle =
		ParticleManager:CreateParticle("particles/chaos_trail.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
