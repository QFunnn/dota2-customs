...
LinkLuaModifier(
	...
	"heroes/hero_naga_siren/naga_siren_rip_tide_lua",
...
LinkLuaModifier(
	"modifier_naga_siren_rip_tide_lua_effect",
	"heroes/hero_naga_siren/naga_siren_rip_tide_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function naga_siren_rip_tide_lua:IsHiddenWhenStolen()
	return false
end
function naga_siren_rip_tide_lua:IsRefreshable()
	return false
end
function naga_siren_rip_tide_lua:IsStealable()
	return false
end
function naga_siren_rip_tide_lua:IsNetherWardStealable()
	return false
end
...
function modifier_naga_siren_rip_tide_lua:OnAttackLanded(keys)
	...
	if IsServer() then
		if
			...
			and not hTarget:IsOther()
			and not hParent:PassivesDisabled()
		then
			if
				hAttacker:GetTeamNumber() == hParent:GetTeamNumber()
			...
			then
				if hAttacker:HasModifier("modifier_naga_siren_rip_tide_lua_effect") then
					...
					if self:GetStackCount() >= self.hits and GameRulesCustom:GetGameTime() > self.LastTime + 0.5 then
						local units = FindUnitsInRadius(
...
function modifier_naga_siren_rip_tide_lua_effect:IsDebuff()
	return false
end
function modifier_naga_siren_rip_tide_lua_effect:IsPurgable()
	return false
end
---------------------------------------------------------------------
--Modifiers
...
function modifier_naga_siren_rip_tide_lua_debuff:OnRemoved() end
function modifier_naga_siren_rip_tide_lua_debuff:OnDestroy() end
--------------------------------------------------------------------------------
-- Modifier Effects
