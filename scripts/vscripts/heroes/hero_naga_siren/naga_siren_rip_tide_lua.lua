...
LinkLuaModifier(
	"modifier_naga_siren_rip_tide_lua_count",
	"heroes/hero_naga_siren/naga_siren_rip_tide_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_naga_siren_rip_tide_lua:AttackLandedModifier(keys)
	...
	if hAttacker ~= self:GetParent() and self.hCaster ~= hAttacker then
		return
	end

	if hTarget:IsBuilding() or self.hCaster:PassivesDisabled() then
		return
	end
...
function modifier_naga_siren_rip_tide_lua_count:IsDebuff()
	return false
end
