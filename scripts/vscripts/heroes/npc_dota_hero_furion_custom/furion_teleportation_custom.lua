...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_furion_custom/furion_teleportation_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_teleportation_custom_shield",
	"heroes/npc_dota_hero_furion_custom/furion_teleportation_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function furion_teleportation_custom:GetCastPoint()
	if self:GetCaster():HasModifier("modifier_furion_13") then
		return self.BaseClass.GetCastPoint(self)
			- self.modifier_furion_13[self:GetCaster():GetTalentLevel("modifier_furion_13")]
	end
	return self.BaseClass.GetCastPoint(self)
end
...
function furion_teleportation_custom:OnAbilityPhaseStart()
	...
	if self:GetCaster():HasModifier("modifier_furion_13") then
		...
		if distance >= self.modifier_furion_13_max_range then
			point = GetGroundPosition(
				self:GetCaster():GetAbsOrigin() + direction * self.modifier_furion_13_max_range,
				self:GetCaster()
			)
		end
	end
	...
	self.nFXIndexEndTeam = ParticleManager:CreateParticleForTeam(
	...
	self.nFXIndexStart = ParticleManager:CreateParticle(
...
function furion_teleportation_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_furion_12") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_furion_teleportation_custom",
			{ duration = self.modifier_furion_12_duration }
		)
	end
...
function modifier_furion_teleportation_custom_shield:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_furion_8") then
		self.barrier = self.barrier
			+ (
				self:GetCaster():GetDisplayAttackSpeed()
				/ 100
				* self:GetAbility().modifier_furion_8[self:GetCaster():GetTalentLevel("modifier_furion_8")]
			)
	end
