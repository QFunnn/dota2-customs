...
LinkLuaModifier(
	"modifier_furion_sprout_custom_thinker_damage",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_sprout_custom_thinker_debuff",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_sprout_custom_magic_immune",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_sprout_custom_regen",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_sprout_custom_trees",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_sprout_custom_thinker_heal",
	"heroes/npc_dota_hero_furion_custom/furion_sprout_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if hTarget == nil or (hTarget ~= nil and (not hTarget:TriggerSpellAbsorb(self))) then
		...
		if self:GetCaster():HasModifier("modifier_furion_18") then
			...
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_furion_sprout_custom_magic_immune",
				{ duration = self.modifier_furion_18[self:GetCaster():GetTalentLevel("modifier_furion_18")] }
			)
		end
...
function modifier_furion_sprout_custom_thinker_damage:OnCreated()
	...
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self:GetAbility():GetSpecialValueFor("sprout_damage_radius"), 0, 0)
	)
...
function modifier_furion_sprout_custom_magic_immune:IsPurgable()
	return false
end
...
function modifier_furion_sprout_custom_trees:IsHidden()
...
function modifier_furion_sprout_custom_trees:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_furion_21") then
		return
	end
	local trees = GridNav:GetAllTreesAroundPoint(
		self:GetCaster():GetAbsOrigin(),
		self:GetAbility().modifier_furion_21_radius,
		false
	)
	self:SetStackCount(
		#trees * self:GetAbility().modifier_furion_21[self:GetCaster():GetTalentLevel("modifier_furion_21")]
	)
end
...
function modifier_furion_sprout_custom_thinker_heal:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_furion/furion_sprout_healing_aoe.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetParent()
	)
	ParticleManager:SetParticleControl(particle, 0, self:GetParent():GetAbsOrigin())
