...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_furion_custom/furion_force_of_nature_custom",
...
LinkLuaModifier(
	"modifier_furion_force_of_nature_custom_barrier",
	"heroes/npc_dota_hero_furion_custom/furion_force_of_nature_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function furion_force_of_nature_custom:OnSpellStart()
	...
	while nTreantsToSpawn > 0 do
		local hTreant = CreateUnitByName(
			"npc_dota_furion_treant_" .. self:GetLevel(),
			point,
			true,
			self:GetCaster(),
		...
		if hTreant ~= nil then
			...
			hTreant:AddNewModifier(
				self:GetCaster(),
				furion_spirit_of_the_forest_custom,
				"modifier_furion_spirit_of_the_forest_custom",
				{}
			)
		end
...
function modifier_furion_force_of_nature_custom:IsHidden()
	return true
end
function modifier_furion_force_of_nature_custom:IsPurgable()
	return false
end
...
function modifier_furion_force_of_nature_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_furion_2") then
		local new_health = self:GetParent():GetBaseMaxHealth()
			+ (
				self:GetCaster():GetMaxHealth()
				/ 100
				* self:GetAbility().modifier_furion_2[self:GetCaster():GetTalentLevel("modifier_furion_2")]
			)
...
function modifier_furion_force_of_nature_custom:GetModifierMoveSpeedBonus_Constant()
	if not self:GetCaster():HasModifier("modifier_furion_5") then
		return
	end
...
function modifier_furion_force_of_nature_custom:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_furion_3") then
		return
	end
	local units = FindUnitsInRadius(
		...
		nil,
		self:GetAbility().modifier_furion_3_radius,
	...
	for _, enemy in pairs(units) do
		local damage = self:GetParent():GetBaseMaxHealth()
			/ 100
			* self:GetAbility().modifier_furion_3[self:GetCaster():GetTalentLevel("modifier_furion_3")]
		ApplyDamage({
			victim = enemy,
...
function modifier_furion_force_of_nature_custom:OnDestroy(params)
	...
	if self:GetCaster():HasModifier("modifier_furion_4") then
		local units = FindUnitsInRadius(
			...
			self:GetParent():GetAbsOrigin(),
			nil,
			self:GetCaster():GetAoeBonus(self:GetAbility().modifier_furion_4_radius),
			DOTA_UNIT_TARGET_TEAM_ENEMY,
			DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
			DOTA_UNIT_TARGET_FLAG_NONE,
			FIND_ANY_ORDER,
		...
		for _, enemy in pairs(units) do
			local damage = self:GetParent():GetBaseMaxHealth()
				/ 100
				* self:GetAbility().modifier_furion_4_damage[self:GetCaster():GetTalentLevel("modifier_furion_4")]
			ApplyDamage({
				victim = enemy,
				attacker = self:GetCaster(),
				damage = damage,
		...
		local particle_explosion_fx = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_warlock/warlock_imp_explode.vpcf",
			PATTACH_WORLDORIGIN,
			self:GetParent()
		)
		ParticleManager:SetParticleControl(particle_explosion_fx, 0, self:GetParent():GetAbsOrigin())
		ParticleManager:SetParticleControl(
			particle_explosion_fx,
			3,
			Vector(
				self:GetCaster():GetAoeBonus(self:GetAbility().modifier_furion_4_radius),
				self:GetCaster():GetAoeBonus(self:GetAbility().modifier_furion_4_radius),
...
function modifier_furion_force_of_nature_custom_immortal:IsHidden()
...
function modifier_furion_force_of_nature_custom_immortal:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_furion_6") then
		return
	end
	local count = 0
	for _, unit in
		pairs(
			FindUnitsInRadius(
				...
				nil,
				self:GetAbility().modifier_furion_6,
	...
	do
		if
			unit
			and unit:GetOwner() == self:GetCaster()
			and (
				unit:GetUnitName() == "npc_dota_furion_treant_1"
				or unit:GetUnitName() == "npc_dota_furion_treant_2"
				or unit:GetUnitName() == "npc_dota_furion_treant_3"
				or unit:GetUnitName() == "npc_dota_furion_treant_4"
			)
		then
			count = count + 1
		end
...
function modifier_furion_force_of_nature_custom_barrier:OnCreated(kv)
	...
	self.barrier = self:GetAbility().modifier_furion_1[self:GetCaster():GetTalentLevel("modifier_furion_1")]
		/ 100
