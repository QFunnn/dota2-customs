...
LinkLuaModifier(
	"modifier_spirit_breaker_nether_strike_custom",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_nether_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_nether_strike_custom_magic_debuff",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_nether_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_break",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_nether_strike",
	LUA_MODIFIER_MOTION_NONE
)
...
function spirit_breaker_nether_strike_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_17") then
		local units = FindUnitsInRadius(
			...
			target:GetOrigin(),
			nil,
			self.modifier_spirit_breaker_17_radius,
		...
		for _, enemy in pairs(units) do
			enemy:AddNewModifier(
				...
				self,
				"modifier_spirit_breaker_nether_strike_custom_magic_debuff",
				{ duration = self.modifier_spirit_breaker_17_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
	end
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_6") then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_spirit_breaker_greater_bash_custom_break",
			{ duration = self.modifier_spirit_breaker_6 }
		)
		if self:GetCaster():HasModifier("modifier_spirit_breaker_17") then
			local units = FindUnitsInRadius(
				self:GetCaster():GetTeamNumber(),
...
function modifier_spirit_breaker_nether_strike_custom_magic_debuff:GetTexture()
	return "spirit_breaker_17"
end
function modifier_spirit_breaker_nether_strike_custom_magic_debuff:GetEffectName()
	return "particles/neutral_fx/icefire_bomb_debuff.vpcf"
end

modifier_spirit_breaker_greater_bash_custom_break = class({})
function modifier_spirit_breaker_greater_bash_custom_break:IsHidden()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_break:IsPurgable()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_spirit_breaker_greater_bash_custom_break:GetEffectName()
