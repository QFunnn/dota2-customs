...
LinkLuaModifier(
	"modifier_spirit_breaker_bulldoze_custom",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_bulldoze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_bulldoze_custom_health_regen",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_bulldoze",
	LUA_MODIFIER_MOTION_NONE
)
...
function spirit_breaker_bulldoze_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_spirit_breaker_bulldoze_custom", { duration = duration })
end

modifier_spirit_breaker_bulldoze_custom = class({})

function modifier_spirit_breaker_bulldoze_custom:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_spirit_breaker_7")
end
...
function modifier_spirit_breaker_bulldoze_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_5") then
		self.movespeed = self.movespeed
			+ self:GetAbility().modifier_spirit_breaker_5[self:GetCaster():GetTalentLevel("modifier_spirit_breaker_5")]
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_4") then
		self.barrier = self:GetAbility().modifier_spirit_breaker_4[self:GetCaster()
			:GetTalentLevel("modifier_spirit_breaker_4")]
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_2") then
		...
		local particle = ParticleManager:CreateParticle(
			"particles/spirit_breaker_buldoze_damage_aura.vpcf",
...
function modifier_spirit_breaker_bulldoze_custom:OnIntervalThink()
	...
	local enemies = FindUnitsInRadius(
		...
		nil,
		self:GetCaster():GetAoeBonus(self:GetAbility().modifier_spirit_breaker_2_radius),
	...
	local damage = self:GetParent():GetHealthRegen()
			/ 100
			* self:GetAbility().modifier_spirit_breaker_2[self:GetCaster():GetTalentLevel("modifier_spirit_breaker_2")]
		+ self:GetAbility().modifier_spirit_breaker_2_base
...
function modifier_spirit_breaker_bulldoze_custom_health_regen:OnCreated()
	...
	local bonus = self:GetCaster():GetStrength()
		/ 100
		* self:GetAbility().modifier_spirit_breaker_4[self:GetCaster():GetTalentLevel("modifier_spirit_breaker_4")]
...
function modifier_spirit_breaker_bulldoze_custom_health_regen:GetTexture()
	return "spirit_breaker_4"
end