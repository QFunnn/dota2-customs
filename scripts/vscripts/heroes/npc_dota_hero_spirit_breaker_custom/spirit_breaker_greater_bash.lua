...
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_knockback",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_vision",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_light",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_light_attackspeed",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_light_spell_amplify",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_stack_vision",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spirit_breaker_greater_bash_custom_scepter_cooldown",
	"heroes/npc_dota_hero_spirit_breaker_custom/spirit_breaker_greater_bash",
	LUA_MODIFIER_MOTION_NONE
)
...
function spirit_breaker_greater_bash_custom:OnSpellStart()
	...
	if
		...
		and not self.npc_dota_spirit_breaker_light:IsNull()
		and self.npc_dota_spirit_breaker_light:IsAlive()
	then
		UTIL_Remove(self.npc_dota_spirit_breaker_light)
		self.npc_dota_spirit_breaker_light = nil
	end
	self.npc_dota_spirit_breaker_light = CreateUnitByName(
		"npc_dota_spirit_breaker_light",
	...
	if self.npc_dota_spirit_breaker_light then
		self.npc_dota_spirit_breaker_light:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_spirit_breaker_greater_bash_custom_light",
			{ duration = 20 }
...
function modifier_spirit_breaker_greater_bash_custom:IsPurgable()
	return false
end
function modifier_spirit_breaker_greater_bash_custom:IsPurgeException()
	return false
end
function modifier_spirit_breaker_greater_bash_custom:RemoveOnDeath()
	return false
end
function modifier_spirit_breaker_greater_bash_custom:IsHidden()
	return true
end
...
function modifier_spirit_breaker_greater_bash_custom:GetModifierProcAttack_Feedback(params)
	...
	if params.target:IsWard() then
		return
	end
	if self:GetParent():IsIllusion() then
		return
	end
	if self:GetParent():HasModifier("modifier_item_diffusal_basher") then
		return
	end
	if self:GetCaster():HasModifier("modifier_spirit_breaker_13") then
		if self:GetCaster():HasModifier("modifier_spirit_breaker_greater_bash_custom_cooldown") then
			return
		end
	else
	...
	if bash_proc then
		if self:GetCaster():HasModifier("modifier_spirit_breaker_13") then
			self:GetCaster():AddNewModifier(
				...
				self:GetAbility(),
				"modifier_spirit_breaker_greater_bash_custom_cooldown",
				{ duration = 1.2 * self:GetCaster():GetCooldownReduction() }
			)
		else
...
function modifier_spirit_breaker_greater_bash_custom:AoeBash(target, point, bashed_units)
	...
	local units = FindUnitsInRadius(
		...
		point,
		nil,
		self:GetAbility().modifier_spirit_breaker_8_radius,
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		0,
		0,
...
function modifier_spirit_breaker_greater_bash_custom:BashTalentEight()
	if not self:GetCaster():HasModifier("modifier_spirit_breaker_8") then
		return
	end
	local modifier_spirit_breaker_greater_bash_custom_stack_vision = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_spirit_breaker_greater_bash_custom_stack_vision",
		{}
	...
	if
		modifier_spirit_breaker_greater_bash_custom_stack_vision
		and modifier_spirit_breaker_greater_bash_custom_stack_vision:GetStackCount()
			>= self:GetAbility().modifier_spirit_breaker_8_attacks[self:GetCaster()
				:GetTalentLevel("modifier_spirit_breaker_8")]
	then
		modifier_spirit_breaker_greater_bash_custom_stack_vision:SetStackCount(0)
		local vision_units = FindUnitsInRadius(
			self:GetCaster():GetTeamNumber(),
		...
		for _, vision_unit in pairs(vision_units) do
			if vision_unit and vision_unit:IsAlive() then
				vision_unit:AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_spirit_breaker_greater_bash_custom_vision",
					{
						duration = self:GetAbility().modifier_spirit_breaker_8_duration
							* (1 - vision_unit:GetStatusResistance()),
					}
				)
				break
			end
...
function modifier_spirit_breaker_greater_bash_custom:Bash(target, double, caster, new_direction, is_scepter)
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_9") then
		bash_duration_bonus = self:GetAbility().modifier_spirit_breaker_9_bash_duration[self:GetCaster():GetTalentLevel(
			"modifier_spirit_breaker_9"
		)]
	end
	...
	target:AddNewModifier(
		...
		self.ability,
		"modifier_stunned",
		{ duration = (self.duration + bash_duration_bonus) * (1 - target:GetStatusResistance()) }
	)
	local arc = target:AddNewModifier(
		...
		"modifier_spirit_breaker_greater_bash_custom_knockback",
		{
			dir_x = direction.x,
			dir_y = direction.y,
			duration = self.knockback_duration * (1 - target:GetStatusResistance()),
			distance = dist,
...
function modifier_spirit_breaker_greater_bash_custom_knockback:IsHidden()
	return true
end
...
function modifier_spirit_breaker_greater_bash_custom_knockback:OnRemoved() end
...
function modifier_spirit_breaker_greater_bash_custom_knockback:UpdateHorizontalMotion(me, dt)
	...
	if self:GetCaster():HasModifier("modifier_spirit_breaker_14") then
		...
		local modifier_spirit_breaker_greater_bash_custom =
			self:GetCaster():FindModifierByName("modifier_spirit_breaker_greater_bash_custom")
		if modifier_spirit_breaker_greater_bash_custom then
			...
			for _, enemy in pairs(enemies) do
				if
					not enemy:HasModifier("modifier_spirit_breaker_greater_bash_custom_knockback")
					and not enemy:HasModifier("modifier_spirit_breaker_greater_bash_custom_scepter_cooldown")
					and not self.ignore_heroes[enemy:entindex()]
					and enemy ~= self:GetParent()
				then
					local new_direction = enemy:GetAbsOrigin() - self:GetParent():GetAbsOrigin()
					new_direction.z = 0
					new_direction = new_direction:Normalized()
					self.ignore_heroes[enemy:entindex()] = true
					modifier_spirit_breaker_greater_bash_custom:Bash(
						enemy,
						nil,
						nil,
						new_direction,
					...
					enemy:AddNewModifier(
						...
						self:GetAbility(),
						"modifier_spirit_breaker_greater_bash_custom_scepter_cooldown",
						{ duration = self:GetAbility().cascading_bash_cooldown }
					)
				end
			end
		end
...
function modifier_spirit_breaker_greater_bash_custom_knockback:InitVerticalArc(
	height_start,
	height_max,
	height_end,
	duration
...
function modifier_spirit_breaker_greater_bash_custom_cooldown:IsHidden()
	return true
end
function modifier_spirit_breaker_greater_bash_custom_cooldown:IsPurgeException()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_cooldown:IsPurgable()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_cooldown:RemoveOnDeath()
	return false
end

modifier_spirit_breaker_greater_bash_custom_light = class({})
function modifier_spirit_breaker_greater_bash_custom_light:IsHidden()
	return true
end
function modifier_spirit_breaker_greater_bash_custom_light:IsPurgable()
	return false
end
...
function modifier_spirit_breaker_greater_bash_custom_light:OnAttackLanded(params)
	...
	params.attacker:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack",
		{ duration = self:GetAbility().modifier_spirit_breaker_13_duration }
	)
	params.attacker:AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
...
function modifier_spirit_breaker_greater_bash_custom_light_attackspeed:IsHidden()
	return true
end
function modifier_spirit_breaker_greater_bash_custom_light_attackspeed:IsPurgable()
	return false
end
...
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack:IsHidden()
	return true
end
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack:IsPurgable()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify:OnIntervalThink()
	...
	local stack = self:GetParent()
		:FindAllModifiersByName("modifier_spirit_breaker_greater_bash_custom_light_spell_amplify_stack")
	self:SetStackCount(
		#stack
			* self:GetAbility().modifier_spirit_breaker_13_spell[self:GetCaster()
				:GetTalentLevel("modifier_spirit_breaker_13")]
	)
end
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify:IsPurgable()
	return false
end
...
function modifier_spirit_breaker_greater_bash_custom_light_spell_amplify:GetTexture()
	return "spirit_breaker_13"
end

modifier_spirit_breaker_greater_bash_custom_stack_vision = class({})
function modifier_spirit_breaker_greater_bash_custom_stack_vision:IsPurgable()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_stack_vision:IsPurgeException()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_stack_vision:RemoveOnDeath()
	return false
end
function modifier_spirit_breaker_greater_bash_custom_stack_vision:GetTexture()
	return "spirit_breaker_8"
end
...
function modifier_spirit_breaker_greater_bash_custom_vision:IsHidden()
	return true
end
...
function modifier_spirit_breaker_greater_bash_custom_vision:OnIntervalThink()
	...
	if self:GetParent():IsNeutralUnitType() then
		...
		AddFOWViewer(
			self:GetCaster():GetTeamNumber(),
			self:GetParent():GetAbsOrigin(),
			self:GetParent():GetCurrentVisionRange(),
			0.1,
			true
		)
	end
...
function modifier_spirit_breaker_greater_bash_custom_scepter_cooldown:IsHidden()
	return true
end