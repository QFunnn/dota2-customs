...
LinkLuaModifier(
	"modifier_magnataur_empower_custom",
	"heroes/hero_magnus/magnataur_empower_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_magnataur_empower_custom_stack_counter",
	"heroes/hero_magnus/magnataur_empower_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_magnataur_empower_custom:GetFacetBonus()
	local stacks = self:GetParent()
		:GetModifierStackCount("modifier_magnataur_empower_custom_stack_counter", self:GetCaster())
	return (stacks or 0)
end
...
function modifier_magnataur_empower_custom:GetModifierProcAttack_Feedback(params)
	...
	if self:GetParent().bCanTriggerLock then
		return
	end

	local frostivus2018_clinkz_searing_arrows = self:GetParent()
		:FindAbilityByName("frostivus2018_clinkz_searing_arrows")
	...
	if self.SelfMultStackDur > 0 then
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_magnataur_empower_custom_stack_counter",
			{ duration = self.SelfMultStackDur }
		)
	end
	...
	if params.attacker:GetAttackCapability() == DOTA_UNIT_CAP_MELEE_ATTACK then
		...
		local blast_pfx = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_magnataur/magnataur_empower_cleave_effect.vpcf",
...
function modifier_magnataur_empower_custom_stack_counter:IsPurgable()
	return false
end
function modifier_magnataur_empower_custom_stack_counter:IsPurgeException()
	return false
end
