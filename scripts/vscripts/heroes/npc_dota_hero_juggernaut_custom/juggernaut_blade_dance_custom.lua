...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_blade_dance_custom",
...
function modifier_juggernaut_blade_dance_custom:RemoveOnDeath()
	return false
end
...
function modifier_juggernaut_blade_dance_custom:GetModifierPreAttack_CriticalStrike(params)
	...
	if self:GetCaster():HasModifier("modifier_juggernaut_12") then
		damage = damage
			+ self:GetAbility().modifier_juggernaut_12[self:GetCaster():GetTalentLevel("modifier_juggernaut_12")]
	end
...
function modifier_juggernaut_blade_dance_custom:GetModifierProcAttack_Feedback(params)
	...
	if not self.record or self.record ~= params.record then
		return
	end
