...
LinkLuaModifier(
	"modifier_juggernaut_omni_slash_custom",
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_omni_slash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_juggernaut_omni_slash_custom_effect",
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_omni_slash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function juggernaut_omni_slash_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_juggernaut_11") then
		self.duration = self.duration
			+ self.modifier_juggernaut_11[self:GetCaster():GetTalentLevel("modifier_juggernaut_11")]
	end
...
function juggernaut_omni_slash_custom:Omnislash(caster, target, duration)
	...
	local targets = FindUnitsInRadius(
		...
		DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NO_INVIS + DOTA_UNIT_TARGET_FLAG_INVULNERABLE,
		FIND_CLOSEST,
		false
	)
	...
	if first_target ~= nil and not first_target:IsUntargetableFrom(self.caster) then
		self.caster:AddNewModifier(
			...
			self,
			"modifier_juggernaut_omni_slash_custom",
			{ duration = duration, first_target = first_target:entindex(), scepter = false }
		)
	end
...
function modifier_juggernaut_omni_slash_custom:IsPurgable()
	return false
end
...
function modifier_juggernaut_omni_slash_custom:slash(first)
	...
	if #target >= 1 then
		for _, enemy in ipairs(target) do
			local can_hit = true
			if
				enemy:GetUnitName() == "npc_dota_techies_remote_mine"
				or enemy:GetUnitName() == "npc_teleport"
				or enemy:IsCourier()
			then
				can_hit = false
			end
			if enemy:HasModifier("modifier_morphling_waveform_custom") then
				can_hit = false
			end
			if can_hit == true and not enemy:IsUntargetableFrom(self:GetCaster()) then
				...
				if
					...
					and self:GetCaster():GetName() == "npc_dota_hero_juggernaut"
					and not self:GetParent():HasModifier("modifier_juggernaut_omnislash_legendary")
				then
				...
				if self.scepter == 1 then
					effect = "particles/juggernaut_trail_swift.vpcf"
				end
...
function modifier_juggernaut_omni_slash_custom:GetModifierPreAttack_BonusDamage()
	return self.damage
end
...
function juggernaut_swift_slash_custom:Omnislash(caster, target, duration)
	...
	if first_target ~= nil then
		self.caster:AddNewModifier(
			...
			self,
			"modifier_juggernaut_omni_slash_custom",
			{ duration = duration, first_target = first_target:entindex(), scepter = true }
		)
	end
