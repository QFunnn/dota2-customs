...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_blade_fury_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_juggernaut_blade_fury_custom_attack_damage",
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_blade_fury_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_juggernaut_blade_fury_custom_slow",
	"heroes/npc_dota_hero_juggernaut_custom/juggernaut_blade_fury_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function juggernaut_blade_fury_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_juggernaut.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_juggernaut.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_juggernaut.vpcf",
		context
	)
end
	...
	if self:GetCaster():HasModifier("modifier_juggernaut_18") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_juggernaut_18[self:GetCaster():GetTalentLevel("modifier_juggernaut_18")]
			)
	end
	...
	for _, enemy in ipairs(enemies) do
		if self:GetCaster():HasModifier("modifier_juggernaut_16") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_juggernaut_blade_fury_custom_slow",
				{ duration = self:GetInterval() + FrameTime() }
			)
		end
		ApplyDamage({
			victim = enemy,
...
function juggernaut_blade_fury_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_juggernaut_blade_fury_custom", { duration = duration })
	self:GetCaster():Purge(false, true, false, false, false)
end

modifier_juggernaut_blade_fury_custom = class({})

function modifier_juggernaut_blade_fury_custom:IsPurgable()
	return false
end
...
function modifier_juggernaut_blade_fury_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if
		self:GetParent():HasModifier("modifier_juggernaut_20")
		and self:GetParent():HasModifier("modifier_juggernaut_omni_slash_custom")
	then
		return
	end
	if self:GetParent():HasModifier("modifier_juggernaut_blade_fury_custom_attack_damage") then
		return
	end
	return -params.damage
end
...
function modifier_juggernaut_blade_fury_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_juggernaut_19") then
		radius = radius
			+ self:GetAbility().modifier_juggernaut_19[self:GetCaster():GetTalentLevel("modifier_juggernaut_19")]
	end
	if self:GetCaster():HasModifier("modifier_juggernaut_21") and self:GetParent():IsHero() then
		...
		if self.count >= 1 then
			local shard_targets = FindUnitsInRadius(
...
function modifier_juggernaut_blade_fury_custom_attack_damage:IsHidden()
	return true
end
function modifier_juggernaut_blade_fury_custom_attack_damage:IsPurgable()
	return false
end

modifier_juggernaut_blade_fury_custom_slow = class({})
function modifier_juggernaut_blade_fury_custom_slow:IsPurgable()
	return false
end
function modifier_juggernaut_blade_fury_custom_slow:IsHidden()
	return true
end
