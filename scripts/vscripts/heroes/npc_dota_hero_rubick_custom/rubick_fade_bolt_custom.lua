...
LinkLuaModifier(
	"modifier_rubick_fade_bolt_custom",
	"heroes/npc_dota_hero_rubick_custom/rubick_fade_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_rubick_fade_bolt_custom_debuff",
	"heroes/npc_dota_hero_rubick_custom/rubick_fade_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function rubick_fade_bolt_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_rubick.vpcf",
		context
	)
end
...
function rubick_fade_bolt_custom:UseFadeBolt(from, target, is_jump, jump_count, targets_table, start_cast, mini, flash)
	local modifier_rubick_fade_bolt_custom = target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_rubick_fade_bolt_custom",
		{ is_jump = is_jump, jump_count = jump_count, start_cast = start_cast, mini = mini, flash = flash }
	)
...
function modifier_rubick_fade_bolt_custom:IsHidden()
	return true
end
function modifier_rubick_fade_bolt_custom:IsDebuff()
	return true
end
function modifier_rubick_fade_bolt_custom:IsPurgable()
	return true
end
function modifier_rubick_fade_bolt_custom:IsPurgeException()
	return false
end
function modifier_rubick_fade_bolt_custom:RemoveOnDeath()
	return false
end
function modifier_rubick_fade_bolt_custom:GetAttributes()
...
function modifier_rubick_fade_bolt_custom:OnCreated(data)
	...
	if self:GetCaster():HasModifier("modifier_rubick_18") then
		self.duration = self.duration
			+ self:GetAbility().modifier_rubick_18[self:GetCaster():GetTalentLevel("modifier_rubick_18")]
	end
...
function modifier_rubick_fade_bolt_custom:Explosion()
	...
	local units = FindUnitsInRadius(
		...
		nil,
		self:GetAbility().modifier_rubick_1_radius,
	...
	for _, enemy in pairs(units) do
		ApplyDamage({
			victim = enemy,
			attacker = self:GetCaster(),
			damage = self:GetAbility().modifier_rubick_1_damage[self:GetCaster():GetTalentLevel("modifier_rubick_1")],
			damage_type = DAMAGE_TYPE_MAGICAL,
			ability = damage_ability,
		})
	end
	local particle = ParticleManager:CreateParticle(
		"particles/econ/items/rubick/rubick_arcana/rubick_arcana_hex_poof.vpcf",
...
function modifier_rubick_fade_bolt_custom_debuff:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_rubick_18")
end
