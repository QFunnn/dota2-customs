...
LinkLuaModifier(
	"modifier_rubick_telekinesis_custom",
	"heroes/npc_dota_hero_rubick_custom/rubick_telekinesis_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_rubick_telekinesis_custom_buff",
	"heroes/npc_dota_hero_rubick_custom/rubick_telekinesis_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function rubick_telekinesis_custom:OnSpellStart_target(target, shoud_swap)
	...
	if
		self:GetCaster():HasModifier("modifier_rubick_10")
	...
	then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_rubick_telekinesis_custom",
			{ duration = self.modifier_rubick_10_delay + self:GetSpecialValueFor("fall_duration") }
		)
	else
		local duration = (self:GetSpecialValueFor("lift_duration") + self:GetSpecialValueFor("fall_duration"))
	...
	if buff_duration > 0 then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_rubick_telekinesis_custom_buff",
			{ duration = buff_duration, target = target:entindex() }
		)
	end
end
...
function modifier_rubick_telekinesis_custom:IsHidden()
	return false
end
function modifier_rubick_telekinesis_custom:IsDebuff()
	return self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
end
function modifier_rubick_telekinesis_custom:IsStunDebuff()
	return false
end
function modifier_rubick_telekinesis_custom:RemoveOnDeath()
	return true
end
function modifier_rubick_telekinesis_custom:DestroyOnExpire()
	return true
end
...
function modifier_rubick_telekinesis_custom:OnCreated(kv)
	...
	if
		self:GetCaster():HasModifier("modifier_rubick_10")
		and self:GetCaster():GetTeamNumber() == self.parent:GetTeamNumber()
	then
...
function modifier_rubick_telekinesis_custom:UpdateLandPosition(new_pos)
	...
	self.lant_pfx = ParticleManager:CreateParticle(
	...
	for _, enemy in pairs(enemies) do
		if enemy ~= self.parent then
			enemy:AddNewModifier(
				...
				self.ability,
				"modifier_stunned",
				{ duration = self.ability:GetSpecialValueFor("stun_duration") * (1 - enemy:GetStatusResistance()) }
			)
		end
		if self:GetCaster():HasModifier("modifier_rubick_17") then
			local damage = self.ability.modifier_rubick_17
				+ (
					self:GetCaster():GetIntellect(false)
					/ 100
					* self.ability.modifier_rubick_17_intellect[self:GetCaster():GetTalentLevel("modifier_rubick_17")]
				)
			ApplyDamage({
				victim = enemy,
...
function modifier_rubick_telekinesis_custom:GetModifierIncomingDamage_Percentage()
	if not self:GetCaster():HasModifier("modifier_rubick_11") then
		return
	end
...
function modifier_rubick_telekinesis_custom_buff:IsHidden()
	return true
end
function modifier_rubick_telekinesis_custom_buff:IsPurgable()
	return false
end
function modifier_rubick_telekinesis_custom_buff:IsPurgeException()
	return false
end
function modifier_rubick_telekinesis_custom_buff:OnCreated(params)
	...
	local rubick_telekinesis_land_self_custom = self:GetCaster()
		:FindAbilityByName("rubick_telekinesis_land_self_custom")
	...
	if self.target == self:GetCaster() then
		if rubick_telekinesis_land_self_custom then
			...
			self:GetCaster()
				:SwapAbilities("rubick_telekinesis_custom", "rubick_telekinesis_land_self_custom", false, true)
...
function modifier_rubick_telekinesis_custom_buff:OnDestroy()
	...
	if self.back_ability then
		if self.target == self:GetCaster() then
			self:GetCaster()
				:SwapAbilities("rubick_telekinesis_land_self_custom", "rubick_telekinesis_custom", false, true)
		else
