...
LinkLuaModifier(
	"modifier_rubick_spell_steal_custom",
	"heroes/npc_dota_hero_rubick_custom/rubick_spell_steal_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_rubick_spell_steal_custom_scepter",
	"heroes/npc_dota_hero_rubick_custom/rubick_spell_steal_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_rubick_spell_steal_custom_hidden",
	"heroes/npc_dota_hero_rubick_custom/rubick_spell_steal_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function rubick_spell_steal_custom:OnProjectileHitHandle(target, location, proj_id)
	...
	if self:GetCaster():HasModifier("modifier_rubick_2") then
		return
	end
	if self.proj_data[proj_id] == nil then
		return
	end
...
function rubick_spell_steal_custom:CastRandomAbility()
	...
	if self:GetScepterEffect() then
		...
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_rubick_spell_steal_custom", { duration = steal_duration })
	else
		if self:GetCaster():HasModifier("modifier_rubick_spell_steal_custom_scepter") then
			self:GetCaster():RemoveModifierByName("modifier_rubick_spell_steal_custom_scepter")
		end
...
function rubick_spell_steal_custom:SetStolenSpell(original_ability)
	...
	if original_ability:IsNull() then
		return
	end
	local ability_name = original_ability:GetAbilityName()
	local modifier_rubick_spell_steal_custom_hidden = self:GetCaster()
		:FindModifierByName("modifier_rubick_spell_steal_custom_hidden")
	if modifier_rubick_spell_steal_custom_hidden == nil then
		return
	end
	...
	if self.currentSpell and not self.currentSpell:IsNull() then
		...
		if self.currentSpell:GetAbilityName() == "luna_eclipse_custom" then
			local luna_lucent_beam_custom =
				self.currentSpell.original_owner:FindAbilityByName("luna_lucent_beam_custom")
		...
		if self.currentSpell.additional_ability and not self.currentSpell.additional_ability:IsNull() then
			if self.currentSpell:GetAbilityIndex() ~= self.currentSpell.main_slot then
				self:GetCaster():SwapAbilities(
					self.currentSpell:GetAbilityName(),
					self:GetCaster():GetAbilityByIndex(self.currentSpell.main_slot):GetAbilityName(),
					false,
					true
				)
			end
	...
	if self.currentSpell_2 and not self.currentSpell_2:IsNull() then
		if self.currentSpell_2.additional_ability and not self.currentSpell_2.additional_ability:IsNull() then
			if self.currentSpell_2:GetAbilityIndex() ~= self.currentSpell_2.main_slot then
				self:GetCaster():SwapAbilities(
					self.currentSpell_2:GetAbilityName(),
					self:GetCaster():GetAbilityByIndex(self.currentSpell_2.main_slot):GetAbilityName(),
					false,
					true
				)
			end
...
function modifier_rubick_spell_steal_custom_scepter:IsHidden()
	return false
end
function modifier_rubick_spell_steal_custom_scepter:IsDebuff()
	return false
end
function modifier_rubick_spell_steal_custom_scepter:IsPurgable()
	return false
end
function modifier_rubick_spell_steal_custom_scepter:RemoveOnDeath()
	return false
end
...
function modifier_rubick_spell_steal_custom_hidden:IsHidden()
	return true
end
function modifier_rubick_spell_steal_custom_hidden:IsPurgable()
	return false
end
function modifier_rubick_spell_steal_custom_hidden:IsPurgeException()
	return false
end
function modifier_rubick_spell_steal_custom_hidden:RemoveOnDeath()
	return false
end
...
function modifier_rubick_spell_steal_custom_hidden:OnAbilityExecuted(params)
	...
	if useless_abilities[params.ability:GetAbilityName()] then
		return
	end
	if params.ability:GetAbilityIndex() > 5 then
		return
	end
...
function modifier_rubick_spell_steal_custom_hidden:UpdateAbility(ability, table_index)
	...
	if
		(ability:NumModifiersUsingAbility() <= 0 or self.ability_think[ability:GetAbilityName()] >= 20)
		and not ability:IsChanneling()
	then
	...
	for i = #abilities_list, 1, -1 do
		...
		if ability and not ability:IsNull() then
			...
			if self.swapable_abilities_list[ability:GetAbilityName()] then
				abilities_list[i] =
					ability:GetCaster():FindAbilityByName(self.swapable_abilities_list[ability:GetAbilityName()])
			end
