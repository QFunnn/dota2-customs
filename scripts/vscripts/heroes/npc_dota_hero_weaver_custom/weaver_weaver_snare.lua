...
LinkLuaModifier(
	"modifier_weaver_weaver_snare_thinker",
	"heroes/npc_dota_hero_weaver_custom/weaver_weaver_snare",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_weaver_snare_cooldown",
	"heroes/npc_dota_hero_weaver_custom/weaver_weaver_snare",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_weaver_snare_rooted",
	"heroes/npc_dota_hero_weaver_custom/weaver_weaver_snare",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_weaver_weaver_snare_thinker:OnCreated()
	...
	local particle =
		ParticleManager:CreateParticle("particles/weaver_snare_custom.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	...
	for _, unit in pairs(units) do
		if not unit:HasModifier("modifier_weaver_weaver_snare_cooldown") then
			unit:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_weaver_weaver_snare_cooldown",
				{ duration = self:GetAbility():GetSpecialValueFor("interval_cooldown") }
			)
			unit:AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_weaver_weaver_snare_rooted",
				{ duration = self:GetAbility():GetSpecialValueFor("root_duration") * (1 - unit:GetStatusResistance()) }
			)
			if self:GetCaster():HasModifier("modifier_weaver_6") then
				ApplyDamage({
					victim = unit,
					damage = self:GetCaster():GetStrength() / 100 * self:GetAbility().modifier_weaver_6[self
						:GetCaster()
						:GetTalentLevel("modifier_weaver_6")],
...
function modifier_weaver_weaver_snare_cooldown:IsHidden()
	return true
end
function modifier_weaver_weaver_snare_cooldown:IsPurgable()
	return false
end