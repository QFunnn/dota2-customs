...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_weaver_custom/weaver_shukuchi_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_shukuchi_custom_shield",
	"heroes/npc_dota_hero_weaver_custom/weaver_shukuchi_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_shukuchi_custom_damage_debuff",
	"heroes/npc_dota_hero_weaver_custom/weaver_shukuchi_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function weaver_shukuchi_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_weaver_16") then
		...
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_weaver_shukuchi_custom_shield",
			{ duration = self.modifier_weaver_16_duration }
		)
	end
end
...
function modifier_weaver_shukuchi_custom:IsPurgeException()
	return false
end
...
function modifier_weaver_shukuchi_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_weaver_15") then
		self.damage = self.damage
			+ (self:GetCaster():GetMaxMana() / 100 * self:GetAbility().modifier_weaver_15_max_mana[self:GetCaster()
				:GetTalentLevel("modifier_weaver_15")])
			+ self:GetAbility().modifier_weaver_15_base
	end
		...
		if not self.hit_targets[enemy] then
			...
			if self:GetCaster():HasModifier("modifier_weaver_18") then
				enemy:AddNewModifier(
					...
					self:GetAbility(),
					"modifier_weaver_shukuchi_custom_damage_debuff",
					{ duration = self:GetAbility().modifier_weaver_18_duration }
				)
			end
...
function modifier_weaver_shukuchi_custom:OnDestroy()
	...
	if not self:GetCaster():HasModifier("modifier_weaver_3") then
		return
	end
	local modifier_weaver_geminate_attack_custom = self:GetCaster()
		:FindModifierByName("modifier_weaver_geminate_attack_custom")
	if modifier_weaver_geminate_attack_custom == nil then
		return
	end
...
function modifier_weaver_shukuchi_custom:OnAttack(keys)
	if
		...
		and self:GetDuration() - self:GetRemainingTime() > 0.25
	then
		self:Destroy()
	end
end
...
function modifier_weaver_shukuchi_custom:GetModifierConstantHealthRegen()
	if not self:GetCaster():HasModifier("modifier_weaver_21") then
		return
	end
...
function modifier_weaver_shukuchi_custom:GetModifierIncomingDamage_Percentage()
	if not self:GetCaster():HasModifier("modifier_weaver_20") then
		return
	end
...
function modifier_weaver_shukuchi_custom_shield:IsPurgable()
	return false
end
function modifier_weaver_shukuchi_custom_shield:IsPurgeException()
	return false
end
...
function modifier_weaver_shukuchi_custom_damage_debuff:IsPurgable()
	return false
end
...
function modifier_weaver_shukuchi_custom_damage_debuff:OnRefresh()
	...
	if
		self:GetStackCount()
		< self:GetAbility().modifier_weaver_18_max[self:GetCaster():GetTalentLevel("modifier_weaver_18")]
	then
