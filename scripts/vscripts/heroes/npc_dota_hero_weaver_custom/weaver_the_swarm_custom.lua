...
LinkLuaModifier(
	"modifier_weaver_the_swarm_custom_unit",
	"heroes/npc_dota_hero_weaver_custom/weaver_the_swarm_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_the_swarm_custom_debuff",
	"heroes/npc_dota_hero_weaver_custom/weaver_the_swarm_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function weaver_the_swarm_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_weaver.vpcf",
		context
	)
end
...
function weaver_the_swarm_custom:OnSpellStart()
	...
	for beetles = 1, self:GetSpecialValueFor("count") do
		start_pos = self:GetCaster():GetAbsOrigin()
			+ RandomVector(RandomInt(0, self:GetSpecialValueFor("spawn_radius")))
		beetle_dummy = CreateModifierThinker(
			self:GetCaster(),
			self,
			nil,
			{},
...
function weaver_the_swarm_custom:OnProjectileHit_ExtraData(target, location, data)
	if
		target
		and not target:HasModifier("modifier_weaver_the_swarm_custom_debuff")
		and data.beetle_entindex
		and EntIndexToHScript(data.beetle_entindex)
		and not EntIndexToHScript(data.beetle_entindex):IsNull()
	then
		...
		if self:GetCaster():HasModifier("modifier_weaver_12") then
			destroy_attacks = destroy_attacks
				+ self.modifier_weaver_12[self:GetCaster():GetTalentLevel("modifier_weaver_12")]
		end
		if self:GetCaster():HasModifier("modifier_weaver_13") then
			armor_reduction = armor_reduction
				+ self.modifier_weaver_13[self:GetCaster():GetTalentLevel("modifier_weaver_13")]
		end
		...
		if
			data.beetle_entindex
			and EntIndexToHScript(data.beetle_entindex)
			and EntIndexToHScript(data.beetle_entindex).projectileID
		then
...
function modifier_weaver_the_swarm_custom_unit:OnIntervalThink()
	if self.target and not self.target:IsNull() then
		if
			(self.target:IsInvisible() and not self:GetParent():CanEntityBeSeenByMyTeam(self.target))
			or self:GetParent():HasModifier("modifier_imba_faceless_void_chronosphere_handler")
...
function modifier_weaver_the_swarm_custom_unit:OnDestroy()
	...
	if
		...
		and self.target:HasModifier("modifier_weaver_the_swarm_custom_debuff")
	then
...
function modifier_weaver_the_swarm_custom_debuff:OnIntervalThink()
	...
	if
		self:GetCaster():HasModifier("modifier_weaver_11")
		and not self:GetParent():IsAttackImmune()
		and not self:GetParent():IsInvulnerable()
	then
		self:GetParent():Script_ReduceMana(
			self.damage
				/ 100
				* self:GetAbility().modifier_weaver_11[self:GetCaster():GetTalentLevel("modifier_weaver_11")],
			self:GetAbility()
		)
	end
end
