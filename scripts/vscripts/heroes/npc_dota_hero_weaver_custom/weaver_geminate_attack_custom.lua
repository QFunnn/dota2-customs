...
LinkLuaModifier(
	"modifier_weaver_geminate_attack_custom",
	"heroes/npc_dota_hero_weaver_custom/weaver_geminate_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_geminate_attack_custom_debuff",
	"heroes/npc_dota_hero_weaver_custom/weaver_geminate_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if self.double_attack[params.record] ~= nil then
		if self:GetCaster():HasModifier("modifier_weaver_2") then
			params.target:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_weaver_geminate_attack_custom_debuff",
				{ duration = self:GetAbility().modifier_weaver_2_duration }
			)
		end
	...
	if self:GetAbility():IsFullyCastable() then
		...
		Timers:CreateTimer(delay, function()
			if
				...
				and params.target:IsAlive()
				and not params.target:IsAttackImmune()
				and not params.target:IsInvulnerable()
			then
		...
		if self:GetCaster():HasModifier("modifier_weaver_2") then
			local units = FindUnitsInRadius(
				...
				nil,
				self:GetAbility().modifier_weaver_2_radius,
