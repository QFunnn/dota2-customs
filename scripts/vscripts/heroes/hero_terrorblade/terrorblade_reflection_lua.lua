...
LinkLuaModifier(
	"modifier_terrorblade_reflection_creep_damage_lua",
	"heroes/hero_terrorblade/terrorblade_reflection_lua",
...
function terrorblade_reflection_lua:OnSpellStart()
	...
	for i, enemy in pairs(enemies) do
		if
			...
			and enemy:IsAlive()
			and Players:IsUnitCanAttackOrCastOnThis(caster, enemy)
			and self.affected_targets_counter < max_targets
		then
			...
			if enemy:IsRealHero() then
				...
				-- сейчас 25%). Иллюзия не должна бить как сам герой со всем билдом, но и
				...
				local hero_green_pct = self:GetSpecialValueFor("hero_green_damage_pct") or 25
				local bonus_damage = raw_bonus * hero_green_pct / 100
				if REFLECTION_DEBUG_ENABLED then
					print(
						"[Reflection DEBUG] HERO target:",
						enemy:GetUnitName(),
						"avg_true =",
						enemy_avg_true,
						"base_avg =",
						...
						bonus_damage,
						"refl_base_before =",
						reflection:GetBaseDamageMin(),
						"/",
						reflection:GetBaseDamageMax()
					)
				end
				if bonus_damage > 0 then
					...
					if REFLECTION_DEBUG_ENABLED then
						print(
							"[Reflection DEBUG] HERO refl_base_after =",
							reflection:GetBaseDamageMin(),
							"/",
							reflection:GetBaseDamageMax()
						)
...
function modifier_terrorblade_reflection_creep_damage_lua:OnCreated(keys)
	...
	if REFLECTION_DEBUG_ENABLED then
		print(
			"[Reflection DEBUG] CREEP OnCreated:",
			parent:GetUnitName(),
			"bat =",
			bat,
			"base_min/max =",
			parent:GetBaseDamageMin(),
			"/",
			parent:GetBaseDamageMax(),
			"avg_true =",
			parent:GetAverageTrueAttackDamage(nil)
		)
	end
end
...
function modifier_terrorblade_reflection_creep_damage_lua:OnIntervalThink()
	...
	-- Считаем зелёный (бонусный) урон с тем же коэффициентом что и для героев (hero_green_damage_pct из KV, по умолчанию 25%).
	...
	local hero_green_pct = ability:GetSpecialValueFor("hero_green_damage_pct") or 25
