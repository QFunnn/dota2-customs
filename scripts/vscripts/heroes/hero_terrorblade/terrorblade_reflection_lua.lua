...
LinkLuaModifier("modifier_terrorblade_reflection_lua_slow", "heroes/hero_terrorblade/terrorblade_reflection_lua", LUA_MODIFIER_MOTION_NONE)
...
function terrorblade_reflection_lua:OnSpellStart()
	...
	local illusion_duration = self:GetSpecialValueFor("illusion_duration")	-- +2s reflection duration
	...
	local enemies = FindUnitsInRadius(caster:GetTeam(), target_loc, nil, range, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS + DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO + DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_ANY_ORDER, false)
	for i, enemy in pairs(enemies) do
		if IsValid(enemy) and enemy:IsAlive() then
			...
			if enemy:IsRealHero() then
				...
				reflection:AddNewModifier(enemy, self, "modifier_illusion", { duration = illusion_duration, outgoing_damage = illusion_outgoing_damage, incoming_damage = 0 })
			...
			if modifier_apply_flag then
				...
				if IsValid(reflection) then
					enemy:AddNewModifier(caster, self, "modifier_terrorblade_reflection_lua_slow", { duration = illusion_duration, illusion_index = reflection:entindex() })
				else
					enemy:AddNewModifier(caster, self, "modifier_terrorblade_reflection_lua_slow", { duration = illusion_duration })
				end
				...
				ParticleManager:SetParticleControlEnt(particle_fx, 1, caster, PATTACH_POINT_FOLLOW, "attach_hitloc", enemy:GetAbsOrigin(), true)
				ParticleManager:SetParticleControlEnt(particle_fx, 0, enemy, PATTACH_POINT_FOLLOW, "attach_hitloc", enemy:GetAbsOrigin(), true)
...
function modifier_terrorblade_reflection_creep_damage_lua:OnIntervalThink()
	...
	if IsValid(hCaster) and IsValid(hParent) and IsValid(hAbility) then
		-- local fAverageDamage = hParent:GetAverageTrueAttackDamage(hParent)
