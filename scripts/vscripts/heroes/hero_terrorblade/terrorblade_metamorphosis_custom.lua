...
LinkLuaModifier(
	...
	"heroes/hero_terrorblade/terrorblade_metamorphosis_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_terrorblade_metamorphosis_transform_aura_applier",
	"heroes/hero_terrorblade/terrorblade_metamorphosis_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_infinite_form_mana_drain",
	"modifiers/modifier_infinite_form_mana_drain",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_terrorblade_metamorphosis:UseMeta(duration)
	...
	-- [MF-16] duration==nil → бесконечная (аганим-toggle) форма → навешиваем общий дрейн 3%/сек.
	if duration == nil then
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_infinite_form_mana_drain", {})
	end
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_custom_terrorblade_metamorphosis_transform",
		{ duration = transform_time, meta_duration = duration }
	)
	...
	for _, unit in
		pairs(
			FindUnitsInRadius(
				...
				nil,
				self:GetSpecialValueFor("metamorph_aura_tooltip"),
	...
	do
		if
			unit ~= self:GetCaster()
			and unit:IsIllusion()
			and unit:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID()
			and unit:GetName() == self:GetCaster():GetName()
		then
			unit:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_custom_terrorblade_metamorphosis_transform",
				{ duration = transform_time, meta_duration = nil }
			)
		end
	end
end
...
function modifier_custom_terrorblade_metamorphosis:OnCreated(table)
	...
	-- [#24] У меле-героев в форме нет базового ranged-снаряда → атаки-«тычки» невидимы.
	-- Явно задаём ванильный снаряд меты (как у DK Elder Dragon Form) — он рисуется на
	-- любом скелете. Одного GetModifierProjectileName для меле-моделей недостаточно.
	self:GetParent()
		:SetRangedProjectileName("particles/units/heroes/hero_terrorblade/terrorblade_metamorphosis_base_attack.vpcf")
...
function modifier_custom_terrorblade_metamorphosis:OnDestroy()
	...
	self:GetParent():SetRangedProjectileName(self.original_projectile or "")
	self:GetParent():RemoveModifierByName("modifier_custom_terrorblade_metamorphosis_transform_aura_applier")
end
...
-- [#24/форма] Меле-герои в форме переведены в ranged (SetAttackCapability), но у них
-- скорость снаряда 0 → ranged-атака не долетает (0 урона, нет снаряда). Даём скорость
-- снаряда, как у DK Elder Dragon Form. Для ranged-героев это просто небольшой бонус.
function modifier_custom_terrorblade_metamorphosis:GetModifierProjectileSpeedBonus()
	return 900
end

function modifier_custom_terrorblade_metamorphosis:GetModelScale()
	return 10
end
...
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:IsHidden()
	return true
end
...
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:IsAura()
	return true
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:IsAuraActiveOnDeath()
	return false
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraDuration()
	return 0.5
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraRadius()
	return self.metamorph_aura_tooltip
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraSearchType()
...
function modifier_custom_terrorblade_metamorphosis_transform_aura_applier:GetAuraEntityReject(hTarget)
	return hTarget == self:GetParent()
		or self:GetParent():IsIllusion()
		or not hTarget:IsIllusion()
		or hTarget:GetPlayerOwnerID() ~= self:GetCaster():GetPlayerOwnerID()
			and hTarget:GetName() ~= self:GetParent():GetName()
end
...
function modifier_custom_terrorblade_metamorphosis_transform_aura:OnCreated()
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_custom_terrorblade_metamorphosis_transform",
		{ duration = self:GetAbility():GetSpecialValueFor("transformation_time") }
	)
end
