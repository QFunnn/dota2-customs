...
LinkLuaModifier(
	"modifier_omniknight_hammer_of_purity_custom_debuff",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_hammer_of_purity_custom_debuff_armor",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_hammer_of_purity_custom_thinker",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_hammer_of_purity_custom_aura",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_generic_orb_effect_lua_omniknight",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_hammer_of_purity_custom_attack_heal",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_hammer_of_purity_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function omniknight_hammer_of_purity_custom:OnOrbImpact(params)
	...
	if self:GetCaster():HasModifier("modifier_omniknight_14") then
		local units = FindUnitsInRadius(
			...
			target:GetAbsOrigin(),
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_omniknight_14_radius),
	...
	if self:GetCaster():HasModifier("modifier_omniknight_13") then
		CreateModifierThinker(
			...
			"modifier_omniknight_hammer_of_purity_custom_thinker",
			{
				duration = self.modifier_omniknight_13_duration[self:GetCaster()
					:GetTalentLevel("modifier_omniknight_13")],
			},
			target:GetAbsOrigin(),
			self:GetCaster():GetTeamNumber(),
			false
...
function omniknight_hammer_of_purity_custom:ApplyDebuff(target)
	...
	if self:GetCaster():HasModifier("modifier_omniknight_11") then
		target:AddNewModifier(
			...
			ability,
			"modifier_omniknight_hammer_of_purity_custom_debuff_armor",
			{ duration = self.modifier_omniknight_11_duration * (1 - target:GetStatusResistance()) }
		)
	end
...
function modifier_omniknight_hammer_of_purity_custom_debuff_armor:GetModifierPhysicalArmorBonus()
	...
	if self:GetCaster():HasModifier("modifier_omniknight_11") then
		bonus =
			self:GetAbility().modifier_omniknight_11_armor[self:GetCaster():GetTalentLevel("modifier_omniknight_11")]
	end
	return bonus
end
...
function modifier_omniknight_hammer_of_purity_custom_thinker:IsHidden()
	return true
end
...
function modifier_omniknight_hammer_of_purity_custom_thinker:OnCreated()
	...
	local particle =
		ParticleManager:CreateParticle("particles/marci_field.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	ParticleManager:SetParticleControl(particle, 0, self:GetParent():GetAbsOrigin())
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self:GetCaster():GetAoeBonus(self:GetAbility().modifier_omniknight_13_radius), 0, 1)
	)
...
function modifier_omniknight_hammer_of_purity_custom_aura:IsPurgable()
	return false
end
...
function modifier_omniknight_hammer_of_purity_custom_aura:OnTakeDamage(params)
	...
	params.attacker:Heal(
		params.damage
			/ 100
			* self:GetAbility().modifier_omniknight_13_heal[self:GetCaster():GetTalentLevel("modifier_omniknight_13")],
		self:GetAbility()
	)
end
...
function modifier_generic_orb_effect_lua_omniknight:OnOrder(params)
	...
	if params.ability then
		if
			...
			or string.find(params.ability:GetName(), "omniknight_hammer_of_purity")
		then
			self.cast = true
			return
		end
...
function modifier_generic_orb_effect_lua_omniknight:GetModifierProjectileName()
	if not self.ability.GetProjectileName then
		return
	end
...
function modifier_generic_orb_effect_lua_omniknight:CheckState()
	if
		(self.ability:IsFullyCastable() and (self.ability:GetAutoCastState()) or self.cast)
		and self:GetCaster():HasModifier("modifier_omniknight_9")
	then
		return {
...
function modifier_omniknight_hammer_of_purity_custom_attack_heal:IsHidden()
	return true
end
function modifier_omniknight_hammer_of_purity_custom_attack_heal:IsPurgable()
	return false
end
function modifier_omniknight_hammer_of_purity_custom_attack_heal:IsPurgeException()
	return false
end
function modifier_omniknight_hammer_of_purity_custom_attack_heal:GetAttributes()
...
function modifier_omniknight_hammer_of_purity_custom_attack_heal:OnCreated(params)
	...
	self.tick_heal = (self.damage / 100 * self:GetAbility():GetSpecialValueFor("heal_pct"))
		/ self:GetAbility():GetSpecialValueFor("heal_duration")
