...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_omniknight_custom/omniknight_rain_of_purification",
...
function omniknight_rain_of_purification:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_omniknight_rain_of_purification", { duration = duration })
end

modifier_omniknight_rain_of_purification = class({})

function modifier_omniknight_rain_of_purification:IsHidden()
	return false
end
function modifier_omniknight_rain_of_purification:IsPurgable()
	return false
end
		...
		if caster:HasModifier("modifier_omniknight_4") then
			heal = heal
				+ (
					caster:GetStrength()
					/ 100
					* ability.modifier_omniknight_4_heal_increase_strength[caster:GetTalentLevel(
						"modifier_omniknight_4"
					)]
				)
		end
		heal = heal / 2
		...
		for _, enemy in pairs(enemies) do
			if enemy:GetTeamNumber() == caster:GetTeamNumber() then
				...
				if caster:HasModifier("modifier_omniknight_1") then
					local new_heal = heal
						* (ability.modifier_omniknight_1[caster:GetTalentLevel("modifier_omniknight_1")] / 100)
					enemy:AddNewModifier(
						self:GetCaster(),
						ability,
						"modifier_omniknight_purification_custom_repeat",
						{ duration = 3, new_heal = new_heal }
					)
				end
			else
				ApplyDamage({
