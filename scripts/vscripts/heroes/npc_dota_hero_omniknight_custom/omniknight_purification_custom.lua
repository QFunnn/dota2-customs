...
LinkLuaModifier(
	"modifier_omniknight_purification_custom_repeat",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_purification_custom",
...
function omniknight_purification_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_omniknight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_omniknight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_omniknight.vpcf",
		context
	)
end
...
function omniknight_purification_custom:GetAOERadius()
	...
	if self:GetCaster():HasModifier("modifier_omniknight_2") then
		bonus = bonus
			+ self.modifier_omniknight_2_bonus_radius[self:GetCaster():GetTalentLevel("modifier_omniknight_2")]
	end
	...
	if self:GetCaster():HasModifier("modifier_omniknight_4") then
		heal = heal
			+ (
				...
				/ 100
				* self.modifier_omniknight_4_heal_increase_strength[self:GetCaster()
					:GetTalentLevel("modifier_omniknight_4")]
			)
	end
	if multiplier then
...
function modifier_omniknight_purification_custom_repeat:IsPurgable()
	return false
end
function modifier_omniknight_purification_custom_repeat:IsPurgeException()
	return false
end
function modifier_omniknight_purification_custom_repeat:GetAttributes()
...
function modifier_omniknight_purification_custom_repeat:OnDestroy()
	...
	if self:GetParent():IsAlive() then
		self:GetAbility():OnSpellStart(
			self:GetParent(),
			self:GetAbility().modifier_omniknight_1[self:GetCaster():GetTalentLevel("modifier_omniknight_1")] / 100,
			self.new_heal
		)
	end
end