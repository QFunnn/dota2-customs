...
LinkLuaModifier(
	"modifier_omniknight_degen_aura_custom",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_degen_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_degen_aura_custom_debuff",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_degen_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_omniknight_degen_aura_custom_active",
	"heroes/npc_dota_hero_omniknight_custom/omniknight_degen_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function omniknight_degen_aura_custom:OnToggle()
	...
	if not self:GetCaster():HasModifier("modifier_omniknight_19") then
		return
	end
...
function modifier_omniknight_degen_aura_custom_active:IsPurgable()
	return false
end
...
function modifier_omniknight_degen_aura_custom:GetAuraRadius()
	local bonus = 0
	if self:GetCaster():HasModifier("modifier_omniknight_18") then
		bonus = self:GetAbility().modifier_omniknight_18_radius[self:GetCaster()
			:GetTalentLevel("modifier_omniknight_18")]
	end
...
function modifier_omniknight_degen_aura_custom_debuff:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_omniknight_17") then
		local damage = self:GetCaster():GetIntellect(false)
			/ 100
			* self:GetAbility().modifier_omniknight_17_damage[self:GetCaster():GetTalentLevel("modifier_omniknight_17")]
