...
LinkLuaModifier(
	"modifier_beastmaster_hawk_storm_gathering",
	"heroes/hero_beastmaster/beastmaster_hawk_storm_gathering.lua",
...
function modifier_beastmaster_hawk_storm_gathering:GetModifierProcAttack_Feedback(params)
	...
	if IsServer() then
		if IsValid(hParent) and IsValid(hTarget) then
			local iParticleID =
				ParticleManager:CreateParticle("particles/items_fx/chain_lightning.vpcf", PATTACH_CUSTOMORIGIN, nil)
			...
			if self:GetStackCount() >= self.max_stack then
				hTarget:AddNewModifier(
					...
					hAbility,
					"modifier_rooted",
					{ duration = self.root_duration * hTarget:GetStatusResistanceFactor(hParent) }
				)
