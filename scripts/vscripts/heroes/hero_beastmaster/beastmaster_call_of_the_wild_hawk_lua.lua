...
LinkLuaModifier(
	"modifier_beastmaster_call_of_the_wild_hawk_lua_summoned",
	"heroes/hero_beastmaster/beastmaster_call_of_the_wild_hawk_lua.lua",
				...
				function(unit)
					if IsValid(unit) then
						...
						table.insert(self.hawks, {
							hUnit = unit,
							fDietime = GameRules:GetGameTime() + hawk_duration,
						})
						...
						unit:AddNewModifier(
							...
							self,
							"modifier_beastmaster_call_of_the_wild_hawk_lua_summoned",
							{ duration = hawk_duration, x = vOffsetDirs[i].x, y = vOffsetDirs[i].y }
						)
