...
LinkLuaModifier(
	"modifier_jakiro_macropyre_custom",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_macropyre_custom",
...
LinkLuaModifier(
	"modifier_jakiro_macropyre_custom_thinker_aoe",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_macropyre_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_macropyre_custom_handler",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_macropyre_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_macropyre_custom_thinker_wall",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_macropyre_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function jakiro_macropyre_custom:CreateTargetAoeMacropyre(target)
	CreateModifierThinker(
		...
		self,
		"modifier_jakiro_macropyre_custom_thinker_aoe",
		{ duration = self.modifier_jakiro_10_duration },
		target:GetOrigin(),
		self:GetCaster():GetTeamNumber(),
		false
	)
...
function modifier_jakiro_macropyre_custom_thinker:CreateWalls(startpoint, endpoint, pos, dir)
	...
	local particle =
		ParticleManager:CreateParticle("particles/jakiro_custom/jakiro_fissure.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_jakiro_macropyre_custom_handler:IsHidden()
	return not self:GetCaster():HasModifier("modifier_jakiro_10")
end
function modifier_jakiro_macropyre_custom_handler:GetTexture()
	return "jakiro_10"
end
function modifier_jakiro_macropyre_custom_handler:IsPurgable()
	return false
end
function modifier_jakiro_macropyre_custom_handler:IsPurgeException()
	return false
end
function modifier_jakiro_macropyre_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_jakiro_macropyre_custom_handler:OnAttackLanded(params)
	...
	if not self:GetCaster():HasModifier("modifier_jakiro_10") then
		return
	end
	if self:GetCaster():HasModifier("modifier_jakiro_16") then
		return
	end
	self:IncrementStackCount()
	if
		self:GetStackCount()
		>= self:GetAbility().modifier_jakiro_10_attack_counter[self:GetCaster():GetTalentLevel("modifier_jakiro_10")]
	then
...
function modifier_jakiro_macropyre_custom_thinker_aoe:OnCreated(kv)
	...
	self.endpoint = self.startpoint + self.direction * self.range
	local effect_cast = ParticleManager:CreateParticle(
		"particles/jakiro_custom/jakiro_macropyre_aoe.vpcf",
		PATTACH_WORLDORIGIN,
		self.parent
	)
	ParticleManager:SetParticleControl(effect_cast, 0, self.startpoint)
