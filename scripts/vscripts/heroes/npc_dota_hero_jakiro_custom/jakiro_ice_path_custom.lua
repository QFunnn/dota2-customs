...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_ice_path_custom",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_ice_path_custom_magic_immune",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_ice_path_custom_handler",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_generic_knockback_lua", "modifiers/modifier_generic_knockback_lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier(
	"modifier_jakiro_ice_path_custom_thinker_target",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_ice_path_custom_heal_reduce",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_ice_path_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function jakiro_ice_path_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_jakiro_21") then
		...
		local knockback = self:GetCaster():AddNewModifier(
			...
			{
				duration = current_range / max_range,
		...
		if knockback then
			...
			local modifier_jakiro_ice_path_custom_magic_immune =
				self:GetCaster()
					:AddNewModifier(self:GetCaster(), self, "modifier_jakiro_ice_path_custom_magic_immune", {})
...
function modifier_jakiro_ice_path_custom_thinker:IsPurgable()
	return false
end
...
function modifier_jakiro_ice_path_custom_thinker:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_jakiro_19") then
		self.duration = self.duration
			+ self:GetAbility().modifier_jakiro_19[self:GetCaster():GetTalentLevel("modifier_jakiro_19")]
	end
...
function modifier_jakiro_ice_path_custom_magic_immune:IsPurgable()
	return false
end
function modifier_jakiro_ice_path_custom_magic_immune:GetTexture()
	return "jakiro_21"
end
...
function modifier_jakiro_ice_path_custom_handler:IsHidden()
	return not self:GetCaster():HasModifier("modifier_jakiro_13")
end
function modifier_jakiro_ice_path_custom_handler:GetTexture()
	return "jakiro_13"
end
function modifier_jakiro_ice_path_custom_handler:IsPurgable()
	return false
end
function modifier_jakiro_ice_path_custom_handler:IsPurgeException()
	return false
end
function modifier_jakiro_ice_path_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_jakiro_ice_path_custom_handler:OnAttackLanded(params)
	...
	if not self:GetCaster():HasModifier("modifier_jakiro_13") then
		return
	end
	if self:GetCaster():HasModifier("modifier_jakiro_2") then
		return
	end
	self:IncrementStackCount()
	if
		self:GetStackCount()
		>= self:GetAbility().modifier_jakiro_13_attack_counter[self:GetCaster():GetTalentLevel("modifier_jakiro_13")]
	then
...
function modifier_jakiro_ice_path_custom_thinker_target:IsPurgable()
	return false
end
...
function modifier_jakiro_ice_path_custom_thinker_target:PlayEffects1()
	local particle = ParticleManager:CreateParticle(
		"particles/jakiro_custom/jakiro_ice_path_aoe.vpcf",
		PATTACH_WORLDORIGIN,
		self.parent
	)
...
function modifier_jakiro_ice_path_custom_heal_reduce:GetTexture()
	return "jakiro_19"
end
