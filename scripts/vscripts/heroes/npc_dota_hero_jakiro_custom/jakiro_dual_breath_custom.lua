...
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_ice",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_fire",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_ice_rooted",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function jakiro_dual_breath_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_jakiro.vpcf",
		context
	)
end
...
function jakiro_dual_breath_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_jakiro_dual_breath_custom",
		{ duration = delay, x = point.x, y = point.y }
	)
end
...
function jakiro_dual_breath_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if self:GetCaster():HasModifier("modifier_jakiro_4") and data.fire then
		target:AddNewModifier(
			...
			{
				duration = 0.5,
				distance = self.modifier_jakiro_4,
				IsStun = false,
				direction_x = data.dir_x,
				direction_y = data.dir_y,
			}
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_jakiro_18") and not data.fire then
		target:AddNewModifier(
			...
			"modifier_jakiro_dual_breath_custom_ice_rooted",
			{
				duration = self.modifier_jakiro_18[self:GetCaster():GetTalentLevel("modifier_jakiro_18")]
...
function modifier_jakiro_dual_breath_custom:IsHidden()
	return true
end
function modifier_jakiro_dual_breath_custom:IsPurgable()
	return false
end
function modifier_jakiro_dual_breath_custom:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_jakiro_dual_breath_custom:RemoveOnDeath()
	return false
end
function modifier_jakiro_dual_breath_custom:OnCreated(kv)
	local distance = self:GetAbility():GetCastRange(self:GetParent():GetAbsOrigin(), self:GetParent())
...
function modifier_jakiro_dual_breath_custom_ice_rooted:GetTexture()
	return "jakiro_18"
end
