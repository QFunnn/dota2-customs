...
LinkLuaModifier(
	"modifier_generic_orb_effect_lua_jakiro_frost",
	"modifiers/modifier_generic_orb_effect_lua_jakiro_frost",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_liquid_ice_custom",
	"heroes/npc_dota_hero_jakiro_custom/jakiro_liquid_ice_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_jakiro_liquid_ice_custom:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_jakiro_11") then
		bonus_damage = bonus_damage
			+ self:GetAbility().modifier_jakiro_11[self:GetCaster():GetTalentLevel("modifier_jakiro_11")]
	end
	ApplyDamage({
