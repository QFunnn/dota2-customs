...
LinkLuaModifier(
	"modifier_luna_moon_glaive_custom",
	"heroes/npc_dota_hero_luna_custom/luna_moon_glaive_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_luna_moon_glaive_custom_slow",
	"heroes/npc_dota_hero_luna_custom/luna_moon_glaive_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function luna_moon_glaive_custom:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
		...
		if hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
			if self:GetCaster():HasModifier("modifier_luna_4") then
				hTarget:AddNewModifier(
					...
					self,
					"modifier_luna_moon_glaive_custom_slow",
					{ duration = self.modifier_luna_4_duration * (1 - hTarget:GetStatusResistance()) }
				)
			end
		end
	...
	if ExtraData.bounces < self.bounces and #enemies > 1 then
		...
		for _, enemy in pairs(enemies) do
			if
				enemy ~= hTarget
				and (not self.target_tracker[ExtraData.record][enemy:GetEntityIndex()] or all_enemies_bounced)
			then
				local glaive = {
					Target = enemy,
					Ability = self,
...
function modifier_luna_moon_glaive_custom:IsHidden()
	return true
end
function modifier_luna_moon_glaive_custom:IsPurgable()
	return false
end
function modifier_luna_moon_glaive_custom:IsPurgeException()
	return false
end
function modifier_luna_moon_glaive_custom:RemoveOnDeath()
	return false
end
...
function modifier_luna_moon_glaive_custom_slow:GetTexture()
	return "luna_4"
end
