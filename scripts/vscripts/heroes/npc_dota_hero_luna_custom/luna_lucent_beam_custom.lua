...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_luna_custom/luna_lucent_beam_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_luna_lucent_beam_custom_handler",
	"heroes/npc_dota_hero_luna_custom/luna_lucent_beam_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function luna_lucent_beam_custom:GetManaCost(level)
	if self:GetCaster():HasModifier("modifier_luna_2") then
		...
		return mana_cost
			- (mana_cost / 100 * self.modifier_luna_2_manacost[self:GetCaster():GetTalentLevel("modifier_luna_2")])
	end
...
function luna_lucent_beam_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_luna_15") then
		...
		local max_counter = self.modifier_luna_15_base
			+ math.floor(self:GetCaster():GetIntellect(false) / self.modifier_luna_15_intellect)
...
function luna_lucent_beam_custom:CastBeamTarget(target, is_ultimate, other_damage)
	...
	if self:GetCaster():HasModifier("modifier_luna_17") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_luna_17[self:GetCaster():GetTalentLevel("modifier_luna_17")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_luna_2") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_luna_lucent_beam_custom_damage_boost",
			{ duration = self.modifier_luna_2_duration }
		)
	end
end

modifier_luna_lucent_beam_custom_handler = class({})
function modifier_luna_lucent_beam_custom_handler:IsHidden()
	return true
end
function modifier_luna_lucent_beam_custom_handler:IsPurgable()
	return false
end
function modifier_luna_lucent_beam_custom_handler:IsPurgeException()
	return false
end
function modifier_luna_lucent_beam_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_luna_lucent_beam_custom_handler:OnTakeDamage(params)
	...
	if not self:GetParent():HasModifier("modifier_luna_18") then
		return
	end
	if self:GetParent():HasModifier("modifier_luna_lucent_beam_custom_cooldown") then
		return
	end
	...
	if params.attacker:HasModifier("modifier_wodarelax") then
		return
	end
	if params.attacker:HasModifier("modifier_wodawispdeath_wisp") then
		return
	end
	if params.attacker:HasModifier("modifier_wodawisp") then
		return
	end
	if params.attacker:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then
		return
	end
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_luna_lucent_beam_custom_cooldown",
		{ duration = self:GetAbility().modifier_luna_18[self:GetCaster():GetTalentLevel("modifier_luna_18")] }
	)
...
function modifier_luna_lucent_beam_custom_damage_boost:GetTexture()
	return "luna_2"
end
...
function modifier_luna_lucent_beam_custom_cooldown:GetTexture()
	return "luna_18"
end
function modifier_luna_lucent_beam_custom_cooldown:IsDebuff()
	return true
end
function modifier_luna_lucent_beam_custom_cooldown:IsPurgable()
	return false
end
function modifier_luna_lucent_beam_custom_cooldown:IsPurgeException()
	return false
end
function modifier_luna_lucent_beam_custom_cooldown:RemoveOnDeath()
	return false
end