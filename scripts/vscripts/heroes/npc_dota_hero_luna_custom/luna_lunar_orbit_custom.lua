...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_luna_custom/luna_lunar_orbit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_luna_lunar_orbit_custom_glaive",
	"heroes/npc_dota_hero_luna_custom/luna_lunar_orbit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_luna_lunar_orbit_custom_damage",
	"heroes/npc_dota_hero_luna_custom/luna_lunar_orbit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_luna_lunar_orbit_custom_blood",
	"heroes/npc_dota_hero_luna_custom/luna_lunar_orbit_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function luna_lunar_orbit_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_luna_lunar_orbit_custom",
		{ duration = self:GetSpecialValueFor("rotating_glaives_duration") }
	)
	if self:GetCaster():HasModifier("modifier_luna_19") then
		...
		self:GetCaster():AddNewModifier(
			...
			{ duration = 0.5, distance = distance, height = 100, direction_x = direction.x, direction_y = direction.y }
		)
...
function modifier_luna_lunar_orbit_custom:IsHidden()
	return false
end
function modifier_luna_lunar_orbit_custom:IsDebuff()
	return false
end
function modifier_luna_lunar_orbit_custom:IsPurgable()
	return false
end
function modifier_luna_lunar_orbit_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_luna_12") then
		self.count = self.count
			+ math.floor((self:GetCaster():GetStrength() / self:GetAbility().modifier_luna_12_count))
	end
	...
	for i = 1, self.count do
		...
		self.thinkers[i] = CreateModifierThinker(
			...
			self:GetAbility(),
			"modifier_luna_lunar_orbit_custom_glaive",
			{ angle = angle * i, owner = self:GetParent():entindex() },
			pos,
...
function modifier_luna_lunar_orbit_custom:OnIntervalThink()
	...
	if self:GetParent():HasModifier("modifier_luna_14") then
		if
			...
			or self:GetParent():HasModifier("modifier_wodawisp")
			or self:GetParent():HasModifier("modifier_smoke_of_deceit")
		then
			if self.thinkers then
				for i = 1, self.count do
	...
	for i = 1, self.count do
		if self.thinkers and self.thinkers[i] and not self.thinkers[i]:IsNull() then
			...
			for _, enemy in ipairs(enemies) do
				if
					not enemy:HasModifier("modifier_wodawisp")
					and not enemy:HasModifier("modifier_wodarelax")
					and enemy:IsAlive()
				then
					self.hits[i][enemy] = self.hits[i][enemy] or 0
					if self.hits[i][enemy] < 0.01 then
						self.hits[i][enemy] = self.grace_period
						local modifier_luna_lunar_orbit_custom_damage = self:GetCaster():AddNewModifier(
							self:GetCaster(),
							self:GetAbility(),
							"modifier_luna_lunar_orbit_custom_damage",
							{}
						...
						if self:GetCaster():HasModifier("modifier_luna_8") then
							ApplyDamage({
								...
								damage = self:GetAbility().modifier_luna_8[self:GetCaster()
									:GetTalentLevel("modifier_luna_8")],
						...
						if self:GetCaster():HasModifier("modifier_luna_13") then
							enemy:AddNewModifier(
								...
								self:GetAbility(),
								"modifier_luna_lunar_orbit_custom_blood",
								{ duration = self:GetAbility().modifier_luna_13_duration }
							)
						end
					end
				end
...
function modifier_luna_lunar_orbit_custom_damage:IsHidden()
	return true
end
function modifier_luna_lunar_orbit_custom_damage:IsPurgable()
	return false
end
function modifier_luna_lunar_orbit_custom_damage:IsPurgeException()
	return false
end
...
function modifier_luna_lunar_orbit_custom_damage:GetModifierDamageOutgoing_Percentage(params)
	if IsServer() then
		...
		local modifier_luna_lunar_orbit_custom_blood =
			params.target:FindModifierByName("modifier_luna_lunar_orbit_custom_blood")
		if modifier_luna_lunar_orbit_custom_blood then
			damage = damage
				+ modifier_luna_lunar_orbit_custom_blood:GetStackCount() * self:GetAbility().modifier_luna_13_dmg
		end
		return damage
	end
end
...
function modifier_luna_lunar_orbit_custom_blood:GetTexture()
	return "luna_13"
end
...
function modifier_luna_lunar_orbit_custom_blood:OnCreated()
	...
	if
		self:GetStackCount()
		>= self:GetAbility().modifier_luna_13_max[self:GetCaster():GetTalentLevel("modifier_luna_13")]
	then
		return
	end
