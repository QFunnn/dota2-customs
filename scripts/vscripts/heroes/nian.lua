...
function modifier_nian_bash:IsHidden()
	return true
end
function modifier_nian_bash:IsPurgable()
	return false
end
function modifier_nian_bash:IsPurgeException()
	return false
end
...
function modifier_nian_bash:AttackLandedModifier(params)
	...
	if RollPercentage(self:GetAbility():GetSpecialValueFor("chance")) then
		...
		local knockback = params.target:AddNewModifier(params.target, nil, "modifier_generic_knockback_lua", {
...
function nian_rush:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_creep_spawner_cha_ready") then
		self:EndCooldown()
		self:RefundManaCost()
		return
	end
...
function nian_rush:OnChargeFinish(interrupt, target)
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_nian_rush", { damage = damage_amp, duration = duration })
...
function modifier_nian_rush:OnCreated(kv)
	...
	local damage = self:GetAbility():GetSpecialValueFor("damage")
		+ (
			(math.floor(GameRules:GetDOTATime(false, false) / 60))
			* self:GetAbility():GetSpecialValueFor("damage_prirost")
		)
	...
	self.damageTable =
		{ attacker = self:GetParent(), damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility() }
end
...
function modifier_nian_rush:HitLogic()
	...
	for _, unit in pairs(units) do
		if not self.knockback_units[unit] then
			...
			if
				is_enemy
				or not (unit:IsCurrentlyHorizontalMotionControlled() or unit:IsCurrentlyVerticalMotionControlled())
			then
...
function modifier_nian_apocalypse_2:IsHidden()
	return true
end
function modifier_nian_apocalypse_2:IsPurgable()
	return false
end
...
function modifier_nian_apocalypse_bkb:OnIntervalThink()
	...
	CreateModifierThinker(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_nian_apocalypse_meteor",
		{},
		vPos,
...
function modifier_nian_apocalypse_bkb:IsPurgable()
	return false
end
function modifier_nian_apocalypse_bkb:IsHidden()
	return true
end
...
function modifier_nian_apocalypse_meteor:OnCreated(kv)
	...
	self.nPreviewFX =
		ParticleManager:CreateParticle("particles/neutral_fx/tower_mortar_marker.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_nian_apocalypse_meteor:OnIntervalThink()
	...
	if self.start_meteor == false then
		...
		local nFXIndex = ParticleManager:CreateParticle(
			"particles/gameplay/spring_meteor_crash/spring_meteor_crash.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
		...
		local nFXIndex2 = ParticleManager:CreateParticle(
			"particles/gameplay/spring_meteor_explosion/spring_meteor_explosion.vpcf",
		...
		for _, enemy in pairs(enemies) do
			if enemy ~= nil then
				...
				enemy:AddNewModifier(
					self.caster,
					self.ability,
					"modifier_nian_apocalypse_debuff",
