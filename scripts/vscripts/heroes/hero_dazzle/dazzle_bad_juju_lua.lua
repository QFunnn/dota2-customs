...
LinkLuaModifier(
	"modifier_dazzle_bad_juju_lua_debuff",
	"heroes/hero_dazzle/dazzle_bad_juju_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_bad_juju_lua_counter",
	"heroes/hero_dazzle/dazzle_bad_juju_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function dazzle_bad_juju_lua:OnSpellStart()
	...
	if IsValid(hCaster) then
		...
		hCaster:AddNewModifier(
			hCaster,
			self,
			"modifier_dazzle_bad_juju_lua",
			{ duration = mana_cost_increase_duration }
		)
...
function modifier_dazzle_bad_juju_lua:GetModifierPercentageHealthcostStacking(params)
	...
	if params.ability == hAbility then
		return -(
			math.pow(
					(100 + self.mana_cost_increase_pct + self.bonus_mana_cost_increase_pct) * 0.01,
					self:GetStackCount()
				)
				* 100
			- 100
...
function modifier_dazzle_bad_juju_lua_counter:OnIntervalThink()
	...
	for index = 0, hParent:GetAbilityCount() - 1 do
		...
		if
			IsValid(hAbility)
			and hAbility:IsRefreshable()
			and hAbility ~= self
			and hAbility:GetCooldown(-1) > 0
			and not hAbility:IsCooldownReady()
		then
