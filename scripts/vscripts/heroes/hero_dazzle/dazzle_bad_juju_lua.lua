...
LinkLuaModifier("modifier_dazzle_bad_juju_lua_counter", "heroes/hero_dazzle/dazzle_bad_juju_lua", LUA_MODIFIER_MOTION_NONE)

-- function dazzle_bad_juju_lua:GetIntrinsicModifierName()
-- 	return "modifier_dazzle_bad_juju_lua_counter"
-- end
function dazzle_bad_juju_lua:OnSpellStart()
	...
	if IsValid(hCaster) then
		for index = 0, hCaster:GetAbilityCount() - 1 do
			...
			if IsValid(hAbility) and hAbility ~= self and hAbility:IsRefreshable() then
		...
		-- 	local hItem = hCaster:GetItemInSlot(index)
		-- 	if IsValid(hItem) and hItem:IsRefreshable() then
		-- 		if not hItem:IsCooldownReady() then
		-- 			local cd_remain = hItem:GetCooldownTimeRemaining()
		-- 			hItem:EndCooldown()
		-- 			if cd_remain > cooldown_reduction_items then
		-- 				hItem:StartCooldown(cd_remain - cooldown_reduction_items)
		-- 			end
		-- 		end
		-- 	end
		-- end
		...
		local units = FindUnitsInRadius(hCaster:GetTeamNumber(), hCaster:GetAbsOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				unit:AddNewModifier(hCaster, self, "modifier_dazzle_bad_juju_lua_debuff", { duration = duration * unit:GetStatusResistanceFactor(hCaster) })
			end
		end
	end
end
...
function modifier_dazzle_bad_juju_lua:OnCreated(params)
	...
	if IsServer() then
		...
		local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_dazzle/dazzle_armor_friend.vpcf", PATTACH_ABSORIGIN_FOLLOW, hParent)
		ParticleManager:SetParticleControlEnt(iParticleID, 0, hParent, PATTACH_OVERHEAD_FOLLOW, "", Vector(0, 0, 0), true)
		ParticleManager:SetParticleControlEnt(iParticleID, 1, hParent, PATTACH_ABSORIGIN_FOLLOW, "", Vector(0, 0, 0), true)
...
---=======================================================
modifier_dazzle_bad_juju_lua_counter = class({})
	...
	for index = 0, hParent:GetAbilityCount() - 1 do
		...
		if IsValid(hAbility) and hAbility:IsRefreshable() and hAbility ~= self and hAbility:GetCooldown(-1) > 0 and not hAbility:IsCooldownReady() then
			if hAbility:IsSealingAbility() then
				iStack = iStack + math.max(10, (-10) * math.sqrt(hAbility:GetCooldown(-1) * hParent:GetCooldownReduction()) + 120)
				iStack_Cooldown = iStack_Cooldown + math.max(10, (-2) * hAbility:GetCooldown(-1) * hParent:GetCooldownReduction() + 150)
			end
		end
	end
	-- for index = DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_6 do
...
function modifier_dazzle_bad_juju_lua_debuff:OnCreated(params)
	...
	if IsServer() then
		...
		local hParent = self:GetParent()
		local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_dazzle/dazzle_armor_enemy.vpcf", PATTACH_ABSORIGIN_FOLLOW, hParent)
		ParticleManager:SetParticleControlEnt(iParticleID, 0, hParent, PATTACH_OVERHEAD_FOLLOW, "", Vector(0, 0, 0), true)
		ParticleManager:SetParticleControlEnt(iParticleID, 1, hParent, PATTACH_ABSORIGIN_FOLLOW, "", Vector(0, 0, 0), true)
		self:AddParticle(iParticleID, false, false, -1, false, false)
	end
