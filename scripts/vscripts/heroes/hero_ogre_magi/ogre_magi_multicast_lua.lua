...
function IsGlobalAbility(hAbility)
	if
		hAbility
		and (
			"invoker_sun_strike_lua" == hAbility:GetAbilityName()
			or "zuus_thundergods_wrath" == hAbility:GetAbilityName()
			or "furion_wrath_of_nature" == hAbility:GetAbilityName()
			or "zuus_cloud_custom" == hAbility:GetAbilityName()
			or "ancient_apparition_ice_blast" == hAbility:GetAbilityName()
			or "spectre_haunt" == hAbility:GetAbilityName()
		)
	then
		return true
	else
...
if IsServer() then
	function modifier_multicast_lua:OnAbilityExecutedCustom(keys)
		...
		if "item_hand_of_midas_custom" == castedAbility:GetAbilityName() then
			multiplier = multiplier * 0.5
		end
		...
		if
			RollPseudoRandomPercentage(
				math.floor(ability:GetSpecialValueFor("multicast_4_times") * multiplier),
				210,
				self:GetParent()
			)
		then
		...
		elseif
			RollPseudoRandomPercentage(
				math.floor(ability:GetSpecialValueFor("multicast_3_times") * multiplier),
				210,
				self:GetParent()
			)
		then
		...
		elseif
			RollPseudoRandomPercentage(
				math.floor(ability:GetSpecialValueFor("multicast_2_times") * multiplier),
				210,
				self:GetParent()
			)
		then
	...
	local MULTICAST_ABILITIES = {
		...
		item_book_of_shadows_custom = MULTICAST_TYPE_NONE,
		...
		ability_alchemist_unstable_concoction = MULTICAST_TYPE_NONE,
		ability_alchemist_unstable_concoction_throw = MULTICAST_TYPE_NONE,
		...
		ability_phoenix_supernova = MULTICAST_TYPE_NONE,
		...
		shadow_demon_disseminate_custom = MULTICAST_TYPE_NONE,

		zuus_lightning_bolt_custom = MULTICAST_TYPE_SAME,
		zuus_cloud_custom = MULTICAST_TYPE_SAME,
		kunkka_torrent_storm_custom = MULTICAST_TYPE_SAME,

		grimstroke_ink_creature = MULTICAST_TYPE_NONE,
		...
		item_tier1_token = MULTICAST_TYPE_NONE,
		item_tier2_token = MULTICAST_TYPE_NONE,
		item_tier3_token = MULTICAST_TYPE_NONE,
		item_tier4_token = MULTICAST_TYPE_NONE,
		item_tier5_token = MULTICAST_TYPE_NONE,
		faceless_void_time_zone = MULTICAST_TYPE_NONE,
		juggernaut_blade_fury = MULTICAST_TYPE_NONE,
		legion_commander_press_the_attack_custom = MULTICAST_TYPE_NONE,
		life_stealer_rage = MULTICAST_TYPE_NONE,
		chen_hand_of_god_custom = MULTICAST_TYPE_NONE,
	}

	function GetAbilityMulticastType(ability)
		local name = ability:GetAbilityName()
		if MULTICAST_ABILITIES[name] then
			return MULTICAST_ABILITIES[name]
		end
		if ability:IsToggle() then
			return MULTICAST_TYPE_NONE
		end
		if HasBehavior(DOTA_ABILITY_BEHAVIOR_PASSIVE, ability) then
			return MULTICAST_TYPE_NONE
		end
		return HasBehavior(DOTA_ABILITY_BEHAVIOR_UNIT_TARGET, ability) and MULTICAST_TYPE_DIFFERENT
			or MULTICAST_TYPE_SAME
	end
	...
	function PreformMulticast(caster, ability_cast, multicast, multicast_delay, target)
		...
		if multicast_type ~= MULTICAST_TYPE_NONE then
			local prt = ParticleManager:CreateParticle(
	...
	function CastMulticastedSpellInstantly(
		caster,
		ability,
		target,
		multicast_flag_data,
		...
		local castTarget = Tier1[math.random(#Tier1)]
			or Tier2[math.random(#Tier2)]
			or Tier3[math.random(#Tier3)]
			or Tier4[math.random(#Tier4)]
			or target
		...
		if multicasts >= 1 then
			Timers:CreateTimer(delay, function()
				...
				if multicast_type == MULTICAST_TYPE_SAME then
					...
					multicast_casted_data = CastMulticastedSpellInstantly(
						caster,
						ability,
						target,
						multicast_flag_data,
...
function CastAdditionalAbility(caster, ability, target, delay, channelData)
	...
	if channelTime > 0 then
		...
		dummy:SetBaseIntellect(caster:GetIntellect(false))
		for _, v in pairs(caster:FindAllModifiers()) do
			local buffName = v:GetName()
			local buffAbility = v:GetAbility()
			local dummyModifier = dummy:FindModifierByName(buffName)
				or dummy:AddNewModifier(dummy, buffAbility, buffName, nil)
			if dummyModifier then
	...
	if
		skill:GetAbilityName() == "nevermore_shadowraze1_custom"
		or skill:GetAbilityName() == "nevermore_shadowraze2_custom"
		or skill:GetAbilityName() == "nevermore_shadowraze3_custom"
		or skill:GetAbilityName() == "doom_bringer_devour_custom"
	then
	...
	elseif skill:GetAbilityName() == "item_hand_of_midas" then
		...
		if skill:GetAbilityName() == "ursa_enrage" then
	...
	if channelTime > 0 then
		if channelData.endTime then
			EndAdditionalAbilityChannel(
				caster,
				unit,
				skill,
				channelData.channelFailed,
