...
LinkLuaModifier(
	...
	"heroes/hero_ogre_magi/modifier_ogre_multicast_lua_bonus.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
local MULTICAST_ABILITIES = {
	...
	item_refresher_2_lua = MULTICAST_TYPE_NONE,
...
function modifier_ogre_magi_multicast_lua:GetModifierOverrideAbilitySpecialValue(event)
	...
	if not ability or not caster or not caster:IsHero() then
		return 0
	end ---@cast caster CDOTA_BaseNPC_Hero
...
function modifier_ogre_magi_multicast_lua:OnAbilityExecuted(event)
	...
	if
		not IsValid(target)
		and castedAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT)
		and castedAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_UNIT_TARGET)
	then
		target = castedAbility:GetCursorPosition()
	elseif
		castedAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT)
		and (not castedAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_UNIT_TARGET))
	then
		target = castedAbility:GetCursorPosition()
	end
...
function modifier_ogre_magi_multicast_lua:GetAbilityMulticastType(ability)
	...
	if ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_PASSIVE) then
		return MULTICAST_TYPE_NONE
	end
...
function modifier_ogre_magi_multicast_lua:CastMulticastedSpellInstantly(
	...
	local castTarget = table.random(Tier1)
		or table.random(Tier2)
		or table.random(Tier3)
		or table.random(Tier4)
		or target
...
function modifier_ogre_magi_multicast_lua:CastMulticastedSpell(
	...
	if multicasts >= 1 then
		Timers:CreateTimer(delay, function()
			...
			if multicast_type == MULTICAST_TYPE_SAME then
				selfRef:CastAdditionalAbility(caster, ability, target, delay * (prtNumber - 1), channelData)
			else
...
function modifier_ogre_magi_multicast_lua:CastAdditionalAbility(caster, ability, target, delay, channelData)
	...
	if not skill or skill:IsNull() then
		return
	end
	...
	if
		skill:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT) and (not skill:HasBehavior(DOTA_ABILITY_BEHAVIOR_UNIT_TARGET))
	then
	...
	if isChannelAbility or (FilterResult ~= nil and FilterResult.behavior == MULTICAST_BEHAVIOR_BONUS) then
		if (isChannelAbility and caster:IsChanneling()) or not isChannelAbility then
			if
				((not skill:IsItem()) and caster:HasAbility(skill:GetAbilityName()))
				or (skill:IsItem() and caster:HasItemInInventory(skill:GetAbilityName()))
			then
	...
	if skill:HasBehavior(DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) and not isCastDone then
		if
			...
			and (target:IsAlive() or skill:GetName() == "doom_bringer_devour" or skill:GetName() == "clinkz_death_pact")
		then
