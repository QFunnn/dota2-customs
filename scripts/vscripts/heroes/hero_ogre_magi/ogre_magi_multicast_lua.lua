...
LinkLuaModifier(
	...
	"heroes/hero_ogre_magi/ogre_magi_multicast_lua.lua",
...
function IsGlobalAbility(hAbility)
	if
		...
		and (
			...
			or "zuus_cloud" == hAbility:GetAbilityName()
...
if IsServer() then
	function modifier_multicast_lua:OnAbilityExecuted(keys)
		...
		if
			not IsValid(target)
			and HasBehavior(castedAbility, DOTA_ABILITY_BEHAVIOR_POINT)
			and HasBehavior(castedAbility, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET)
		then
			target = castedAbility:GetCursorPosition()
		end

		if
			HasBehavior(castedAbility, DOTA_ABILITY_BEHAVIOR_POINT)
			and (not HasBehavior(castedAbility, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET))
		then
			target = castedAbility:GetCursorPosition()
		end
	...
	function GetAbilityMulticastType(ability)
		...
		if HasBehavior(ability, DOTA_ABILITY_BEHAVIOR_PASSIVE) then
			return MULTICAST_TYPE_NONE
		end
		return HasBehavior(ability, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) and MULTICAST_TYPE_DIFFERENT
	...
	function GetMulticastFlags(caster, ability, multicast_type)
		...
		if multicast_type ~= MULTICAST_TYPE_SAME then
			...
			elseif
				abilityTargetType == DOTA_UNIT_TARGET_CREEP and HasBehavior(ability, DOTA_ABILITY_BEHAVIOR_POINT)
			then
...
function CastAdditionalAbility(caster, ability, target, delay, channelData)
	...
	if bChannelAbility or (type(FilterResult) ~= "boolean" and FilterResult.behavior == MULTICAST_BEHAVIOR_BONUS) then
		if (bChannelAbility and caster:IsChanneling()) or not bChannelAbility then
			if
				((not ability:IsItem()) and caster:HasAbility(ability:GetAbilityName()))
				or (ability:IsItem() and caster:HasItemInInventory(ability:GetAbilityName()))
			then
				CreateUnitByNameAsync(
					...
					function(illusion)
						...
						illusion:AddNewModifier(
							...
							caster:FindAbilityByName("ogre_magi_multicast_lua"),
							"modifier_ogre_multicast_lua_bonus",
							{ duration = illusion_duration, abilityindex = ability:entindex() }
						)
						...
						illusion:SetContextThink(DoUniqueString("ogre_magi_multicast_lua"), function()
							...
							if illusion_ability ~= nil then
								...
								if HasBehavior(ability, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) and IsValid(target) then
									illusion:SetForwardVector(
										(target:GetAbsOrigin() - caster:GetAbsOrigin()):Normalized()
									)
	...
	if HasBehavior(skill, DOTA_ABILITY_BEHAVIOR_UNIT_TARGET) and not bCastDone then
		if
			...
			and (
				target:IsAlive()
				or ability:GetName() == "doom_bringer_devour"
				or ability:GetName() == "clinkz_death_pact"
			)
		then
	...
	if
		target ~= nil
		and type(target) ~= "table"
		and HasBehavior(skill, DOTA_ABILITY_BEHAVIOR_POINT)
		and not bCastDone
	then
		if target then
			--保留好当前位置 使用完在马上复原
...
function modifier_ogre_multicast_lua_bonus:OnIntervalThink()
	...
	if not IsValid(hAbility) then
		self:Destroy()
	elseif
		HasBehavior(hAbility, DOTA_ABILITY_BEHAVIOR_CHANNELLED)
		and ((not hParent:IsChanneling()) or (not hCaster:IsChanneling()))
	then
		self:Destroy()
	end
end
