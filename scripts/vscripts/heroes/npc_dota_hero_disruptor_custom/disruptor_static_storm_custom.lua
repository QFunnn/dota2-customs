...
LinkLuaModifier(
	"modifier_disruptor_static_storm_custom",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_static_storm_custom",
	...
	if self:GetCaster():HasModifier("modifier_disruptor_19") then
		...
		if disruptor_glimpse_custom then
			local enemies = FindUnitsInRadius(
				...
				point,
				nil,
				self.modifier_disruptor_19[self:GetCaster():GetTalentLevel("modifier_disruptor_19")],
...
function modifier_disruptor_static_storm_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_disruptor_5") then
		duration = duration
			+ self:GetAbility().modifier_disruptor_5[self:GetCaster():GetTalentLevel("modifier_disruptor_5")]
	end
