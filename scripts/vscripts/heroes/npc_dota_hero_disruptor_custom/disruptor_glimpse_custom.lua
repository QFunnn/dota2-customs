...
LinkLuaModifier(
	"modifier_disruptor_glimpse_custom_thinker",
...
function disruptor_glimpse_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_disruptor.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_disruptor.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_disruptor.vpcf",
		context
	)
end
...
function disruptor_glimpse_custom:CastFilterResultTarget(hTarget)
	...
	if
		hTarget:GetTeamNumber() == self:GetCaster():GetTeamNumber()
		and not self:GetCaster():HasModifier("modifier_disruptor_17")
	then
	...
	if hUnit then
		...
		if self:GetCaster():HasModifier("modifier_disruptor_18") then
			max_damage = max_damage
				+ self:GetAbility().modifier_disruptor_18[self:GetCaster():GetTalentLevel("modifier_disruptor_18")]
		end
...
function modifier_disruptor_glimpse_custom_movement_check_aura:IsHidden()
	return true
end
function modifier_disruptor_glimpse_custom_movement_check_aura:IsPassive()
	return true
end
function modifier_disruptor_glimpse_custom_movement_check_aura:IsAura()
	return true
end
function modifier_disruptor_glimpse_custom_movement_check_aura:IsAuraActiveOnDeath()
	return true
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetAuraRadius()
	return FIND_UNITS_EVERYWHERE
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO
		+ DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
		+ DOTA_UNIT_TARGET_FLAG_INVULNERABLE
		+ DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_BOTH
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetModifierAura()
	return "modifier_disruptor_glimpse_custom"
end
function modifier_disruptor_glimpse_custom_movement_check_aura:GetAuraDuration()
	return 1
end
function modifier_disruptor_glimpse_custom_movement_check_aura:RemoveOnDeath()
	return false
end

modifier_disruptor_glimpse_custom = class({})
function modifier_disruptor_glimpse_custom:IsHidden()
	return true
end
function modifier_disruptor_glimpse_custom:IsPurgable()
	return true
end
