...
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom_talent_handler",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom_slow",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom_attackspeed_stack",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom_attackspeed_buff",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_disruptor_thunder_strike_custom_counter",
	"heroes/npc_dota_hero_disruptor_custom/disruptor_thunder_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_disruptor_thunder_strike_custom:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_disruptor_7")
end
...
function modifier_disruptor_thunder_strike_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_disruptor_1") then
		self.interval = self.interval
			+ self:GetAbility().modifier_disruptor_1[self:GetCaster():GetTalentLevel("modifier_disruptor_1")]
	end
	if self:GetCaster():HasModifier("modifier_disruptor_2") then
		self.count = self.count
			+ self:GetAbility().modifier_disruptor_2[self:GetCaster():GetTalentLevel("modifier_disruptor_2")]
	end
	...
	if self.interval_time >= self.interval then
		if self:GetCaster():HasModifier("modifier_disruptor_3") then
			self:GetCaster():AddNewModifier(
				...
				self:GetAbility(),
				"modifier_disruptor_thunder_strike_custom_attackspeed_stack",
				{ duration = self:GetAbility().modifier_disruptor_3_duration }
			)
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
		...
		if
			self:GetParent():HasModifier("modifier_disruptor_kinetic_field_custom")
			or self:GetParent():HasModifier("modifier_disruptor_kinetic_field_custom_big")
		then
			local enemies_circle = FindUnitsInRadius(
				self:GetCaster():GetTeamNumber(),
				self:GetParent():GetOrigin(),
				nil,
				-1,
			...
			for ___, enemy_circle in pairs(enemies_circle) do
				if
					enemy_circle:HasModifier("modifier_disruptor_kinetic_field_custom")
					or enemy_circle:HasModifier("modifier_disruptor_kinetic_field_custom_big")
				then
...
function modifier_disruptor_thunder_strike_custom_slow:IsHidden()
	return true
end
...
function modifier_disruptor_thunder_strike_custom_attackspeed_stack:IsHidden()
	return true
end
function modifier_disruptor_thunder_strike_custom_attackspeed_stack:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

modifier_disruptor_thunder_strike_custom_attackspeed_buff = class({})

function modifier_disruptor_thunder_strike_custom_attackspeed_buff:GetTexture()
	return "disruptor_3"
end
...
function modifier_disruptor_thunder_strike_custom_attackspeed_buff:GetModifierAttackSpeedBonus_Constant()
	return self:GetStackCount()
		* self:GetAbility().modifier_disruptor_3[self:GetCaster():GetTalentLevel("modifier_disruptor_3")]
end
...
function modifier_disruptor_thunder_strike_custom_counter:IsHidden()
	return true
end

function modifier_disruptor_thunder_strike_custom_counter:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_disruptor_thunder_strike_custom_counter:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_disruptor_thunder_strike_custom_counter_slow",
			{ duration = self.slow_duration }
		)
	end
	self:PlayEffects()
...
function modifier_disruptor_thunder_strike_custom_talent_handler:IsHidden()
	return true
end
function modifier_disruptor_thunder_strike_custom_talent_handler:IsPurgable()
	return false
end
function modifier_disruptor_thunder_strike_custom_talent_handler:IsPurgeException()
	return false
end
...
function modifier_disruptor_thunder_strike_custom_talent_handler:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_disruptor_4") then
		return
	end
	if
		RollPercentage(self:GetAbility().modifier_disruptor_4[self:GetCaster():GetTalentLevel("modifier_disruptor_4")])
	then
