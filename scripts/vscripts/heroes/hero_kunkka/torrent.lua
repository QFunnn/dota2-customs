...
function torrent_bubble_allies(keys)
	...
	local target = keys.target_points[1] or caster.target_position
	...
	for k, v in pairs(allHeroes) do
		if v:GetPlayerID() and v:GetTeam() == caster:GetTeam() then
			local fxIndex = ParticleManager:CreateParticleForPlayer(
				...
				PATTACH_ABSORIGIN,
				v,
				PlayerResource:GetPlayer(v:GetPlayerID())
			)
...
function torrent_emit_sound(keys)
	local dummy = CreateUnitByName(
		"npc_dummy_unit",
		keys.target_points[1] or keys.caster.target_position,
		false,
		keys.caster,
		keys.caster,
...
function torrent_vision(keys)
	...
	local target = keys.target_points[1] or keys.caster.target_position
