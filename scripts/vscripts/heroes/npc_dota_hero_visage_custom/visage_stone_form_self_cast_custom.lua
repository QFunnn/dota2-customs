...
LinkLuaModifier(
	"modifier_visage_stone_form_self_cast_custom",
	"heroes/npc_dota_hero_visage_custom/visage_stone_form_self_cast_custom",
...
function visage_stone_form_self_cast_custom:RefreshCustomCharges()
	if not self:GetCaster():HasModifier("modifier_visage_19") then
		return
	end
...
function visage_stone_form_self_cast_custom:GetBehavior()
	...
	local modifier_visage_stone_form_self_cast_custom =
		self:GetCaster():GetModifierStackCount("modifier_visage_stone_form_self_cast_custom", self:GetCaster())
	if modifier_visage_stone_form_self_cast_custom == 1 then
		return DOTA_ABILITY_BEHAVIOR_NOT_LEARNABLE
	...
	if visage_summon_familiars_custom and visage_summon_familiars_custom:GetLevel() > 0 then
		...
		if familiar then
			local visage_summon_familiars_stone_form_custom_familiar =
				familiar:FindAbilityByName("visage_summon_familiars_stone_form_custom")
	...
	if self:GetAltCastState() then
		...
		if visage_summon_familiars_custom then
			for _, familiar in pairs(visage_summon_familiars_custom.familiar_table) do
				...
				then
					local visage_summon_familiars_stone_form_custom =
						familiar:FindAbilityByName("visage_summon_familiars_stone_form_custom")
					if
						visage_summon_familiars_stone_form_custom
						and visage_summon_familiars_stone_form_custom:IsFullyCastable()
						...
						and not (self.flying and self.flying[familiar:entindex()])
						and (
							not nearest_familiar
							or (familiar:GetAbsOrigin() - point):Length2D()
								< (nearest_familiar:GetAbsOrigin() - point):Length2D()
						)
					then
		...
		if nearest_familiar then
			...
			Timers:CreateTimer(0.1, function()
				if
					nearest_familiar:IsNull()
					or not nearest_familiar:IsAlive()
					or GameRules:GetGameTime() >= timeout
				then
					self.flying[idx] = nil
					return
				end
				local visage_summon_familiars_stone_form_custom =
					nearest_familiar:FindAbilityByName("visage_summon_familiars_stone_form_custom")
				if
					not visage_summon_familiars_stone_form_custom
					or not visage_summon_familiars_stone_form_custom:IsFullyCastable()
					or nearest_familiar:HasModifier("modifier_visage_summon_familiars_stone_form_custom_root")
					or nearest_familiar:HasModifier("modifier_visage_summon_familiars_stone_form_custom_buff")
				then
					self.flying[idx] = nil
					return
				end
		...
		for _, familiar in pairs(visage_summon_familiars_custom.familiar_table) do
			...
			then
				...
				if
					...
					and (
						not nearest_familiar
						or (familiar:GetAbsOrigin() - self:GetCaster():GetAbsOrigin()):Length2D()
							< (nearest_familiar:GetAbsOrigin() - self:GetCaster():GetAbsOrigin()):Length2D()
					)
				then
					nearest_familiar = familiar
				end
...
function modifier_visage_stone_form_self_cast_custom:IsHidden()
	return true
end
function modifier_visage_stone_form_self_cast_custom:IsPurgable()
	return false
end
function modifier_visage_stone_form_self_cast_custom:IsPurgeException()
	return false
end
function modifier_visage_stone_form_self_cast_custom:RemoveOnDeath()
	return false
end
	...
	if visage_summon_familiars_custom and visage_summon_familiars_custom.familiar_table then
		for _, familiar in pairs(visage_summon_familiars_custom.familiar_table) do
			if
				...
				and not familiar:IsNull()
				and familiar:IsAlive()
				and familiar:GetPlayerOwnerID() == self:GetParent():GetPlayerOwnerID()
			then
				has_familiar = true
				...
				if stone_form then
					...
					if buff then
						...
						local root =
							familiar:FindModifierByName("modifier_visage_summon_familiars_stone_form_custom_root")
						if root then
							wait = math.max(
								wait,
								root:GetRemainingTime() + stone_form:GetSpecialValueFor("stone_duration")
							)
						end
					end
