...
LinkLuaModifier(
	"modifier_visage_silent_as_the_grave_custom",
	"heroes/npc_dota_hero_visage_custom/visage_silent_as_the_grave_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_silent_as_the_grave_custom_damage",
	"heroes/npc_dota_hero_visage_custom/visage_silent_as_the_grave_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function visage_silent_as_the_grave_custom:GetBonusDuration()
	...
	if self:GetCaster():HasModifier("modifier_visage_20") then
		bonus_duration = bonus_duration
			+ self.modifier_visage_20_duration[self:GetCaster():GetTalentLevel("modifier_visage_20")]
	end
...
function visage_silent_as_the_grave_custom:GetBonusDamage()
	...
	if self:GetCaster():HasModifier("modifier_visage_19") then
		bonus_damage = bonus_damage
			+ self.modifier_visage_19_damage[self:GetCaster():GetTalentLevel("modifier_visage_19")]
	end
	return bonus_damage
end
...
function visage_silent_as_the_grave_custom:OnSpellStart()
	...
	if
		visage_summon_familiars_custom
		and visage_summon_familiars_custom.familiar_table
		and #visage_summon_familiars_custom.familiar_table > 0
	then
...
function modifier_visage_silent_as_the_grave_custom:Trigger()
	...
	if self:GetCaster():HasModifier("modifier_visage_20") then
		...
		local modifier_visage_silent_as_the_grave_custom_damage =
			self:GetParent():FindModifierByName("modifier_visage_silent_as_the_grave_custom_damage")
