...
LinkLuaModifier(
	"modifier_visage_summon_familiars_custom",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_summon_familiars_self_cast_custom",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_summon_familiars_custom_armor",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_summon_familiars_custom_cooldown",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function visage_summon_familiars_custom:GetFamiliarHealth()
	...
	if self:GetCaster():HasModifier("modifier_visage_3") then
		familiar_health = familiar_health
			+ (
				self:GetCaster():GetStrength()
				/ 100
				* self.modifier_visage_3_str[self:GetCaster():GetTalentLevel("modifier_visage_3")]
			)
	end
	...
	if self:GetAltCastState() then
		if self.familiar_table then
			for _, familiar in pairs(self.familiar_table) do
				if
					familiar
					and not familiar:IsNull()
					and familiar:IsAlive()
					and familiar:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID()
				then
					local visage_summon_familiars_recall_custom =
						familiar:FindAbilityByName("visage_summon_familiars_recall_custom")
					if
						visage_summon_familiars_recall_custom
						and visage_summon_familiars_recall_custom:IsFullyCastable()
						and not familiar:HasModifier("modifier_visage_summon_familiars_stone_form_custom_root")
						and not familiar:HasModifier("modifier_visage_summon_familiars_stone_form_custom_buff")
					then
...
function visage_summon_familiars_custom:SpawnFamiliar(num, unit_count, new_point)
	...
	local spawn_location = self:GetCaster():GetAbsOrigin()
		+ (self:GetCaster():GetForwardVector() * 200)
		+ (self:GetCaster():GetRightVector() * side_offset)
	if new_point then
		spawn_location = new_point
	end
	local familiar = CreateUnitByName(
		"npc_dota_visage_familiar" .. math.min(self:GetLevel(), 3),
		spawn_location,
		true,
		nil,
		nil,
	...
	familiar:AddNewModifier(
		...
		self,
		"modifier_visage_summon_familiars_custom",
		{ count = num, unit_count = unit_count }
	)
...
function modifier_visage_summon_familiars_custom:IsHidden()
	return true
end
function modifier_visage_summon_familiars_custom:IsPurgable()
	return false
end
function modifier_visage_summon_familiars_custom:IsPurgeException()
	return false
end
...
function modifier_visage_summon_familiars_custom:OnIntervalThink()
	...
	local visage_gravekeepers_cloak_custom_familiar = self:GetParent()
		:FindAbilityByName("visage_gravekeepers_cloak_custom")
	...
	local visage_summon_familiars_stone_form_custom = self:GetCaster()
		:FindAbilityByName("visage_summon_familiars_stone_form_custom")
	local visage_summon_familiars_stone_form_custom_familiar =
		self:GetParent():FindAbilityByName("visage_summon_familiars_stone_form_custom")
	if visage_summon_familiars_stone_form_custom and visage_summon_familiars_stone_form_custom_familiar then
		visage_summon_familiars_stone_form_custom_familiar:SetLevel(
			visage_summon_familiars_stone_form_custom:GetLevel()
		)
	end
	if self:GetCaster():HasModifier("modifier_visage_5") then
		self:GetParent()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_visage_summon_familiars_custom_cooldown", {})
	end
...
function modifier_visage_summon_familiars_custom:GetModifierAttackRangeBonus()
	...
	if hero:HasModifier("modifier_visage_10") then
		bonus = bonus
			+ hero:GetAgility()
				/ 100
				* self:GetAbility().modifier_visage_10[hero:GetTalentLevel("modifier_visage_10")]
	end
	...
	if hero:HasModifier("modifier_visage_17") then
		bonus = bonus
			+ hero:GetIntellect(false)
				/ 100
				* self:GetAbility().modifier_visage_17[hero:GetTalentLevel("modifier_visage_17")]
	end
	return bonus
end
...
function modifier_visage_summon_familiars_custom:OnAttackLanded(params)
	...
	local hero = self:GetCaster()
	if not hero or hero:IsNull() or not hero:HasModifier("modifier_visage_13") then
		return
	end
	local target = params.target
	if not target or target:IsNull() or target == self:GetParent() then
		return
	end
...
function modifier_visage_summon_familiars_custom_armor:IsHidden()
	return false
end
function modifier_visage_summon_familiars_custom_armor:IsPurgable()
	return true
end
function modifier_visage_summon_familiars_custom_armor:GetTexture()
	return "visage_13"
end
...
function modifier_visage_summon_familiars_self_cast_custom:IsHidden()
	return true
end
function modifier_visage_summon_familiars_self_cast_custom:IsPurgable()
	return false
end
function modifier_visage_summon_familiars_self_cast_custom:IsPurgeException()
	return false
end
function modifier_visage_summon_familiars_self_cast_custom:RemoveOnDeath()
	return false
end
...
function modifier_visage_summon_familiars_custom_cooldown:IsHidden()
	return true
end
function modifier_visage_summon_familiars_custom_cooldown:IsPurgable()
	return false
end
function modifier_visage_summon_familiars_custom_cooldown:IsPurgeException()
	return false
end
function modifier_visage_summon_familiars_custom_cooldown:RemoveOnDeath()
	return false
end