...
LinkLuaModifier(
	"modifier_visage_summon_familiars_stone_form_custom",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_stone_form_custom",
...
LinkLuaModifier(
	"modifier_visage_summon_familiars_stone_form_custom_buff",
	"heroes/npc_dota_hero_visage_custom/visage_summon_familiars_stone_form_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function visage_summon_familiars_stone_form_custom:ProcsMagicStick()
	return false
end
...
function visage_summon_familiars_stone_form_custom:GetBehavior()
	if
		self:GetCaster():GetModifierStackCount("modifier_visage_summon_familiars_stone_form_custom", self:GetCaster())
		== 1
	then
...
function visage_summon_familiars_stone_form_custom:OnSpellStart(is_die)
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_visage_summon_familiars_stone_form_custom_root",
		{ duration = self:GetSpecialValueFor("stun_delay"), is_die = is_die }
	)
end

modifier_visage_summon_familiars_stone_form_custom_root = class({})
function modifier_visage_summon_familiars_stone_form_custom_root:IsPurgable()
	return false
end
function modifier_visage_summon_familiars_stone_form_custom_root:OnCreated(params)
	...
	local visage_summon_familiars_recall_custom = self:GetCaster()
		:FindAbilityByName("visage_summon_familiars_recall_custom")
...
function modifier_visage_summon_familiars_stone_form_custom_root:OnDestroy()
	...
	self:GetCaster():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_visage_summon_familiars_stone_form_custom_buff",
		{ duration = self:GetAbility():GetSpecialValueFor("stone_duration"), is_die = self.is_die }
	)
end
...
function modifier_visage_summon_familiars_stone_form_custom_buff:OnCreated(params)
	...
	if hero and not hero:IsNull() and hero:HasModifier("modifier_visage_6") then
		...
		self.stun_damage = self.stun_damage
			+ (self:GetCaster():GetMaxHealth() * self:GetAbility().modifier_visage_6_damage[level] / 100)
	end
	if hero and not hero:IsNull() and hero:HasModifier("modifier_visage_19") then
		...
		self.stun_damage = self.stun_damage
			+ (
				...
				* self:GetAbility().modifier_visage_19_damage[level]
				/ 100
			)
	end
	...
	local stone_form_particle = ParticleManager:CreateParticle(
...
function modifier_visage_summon_familiars_stone_form_custom_buff:OnIntervalThink()
	...
	self.stone_form_overhead_particle = ParticleManager:CreateParticleForTeam(
...
function modifier_visage_summon_familiars_stone_form_custom_buff:OnDestroy()
	...
	local stone_form_transform_particle = ParticleManager:CreateParticle(
...
function modifier_visage_summon_familiars_stone_form_custom:IsHidden()
	return true
end
function modifier_visage_summon_familiars_stone_form_custom:IsPurgable()
	return false
end
function modifier_visage_summon_familiars_stone_form_custom:IsPurgeException()
	return false
end
function modifier_visage_summon_familiars_stone_form_custom:RemoveOnDeath()
	return false
end
