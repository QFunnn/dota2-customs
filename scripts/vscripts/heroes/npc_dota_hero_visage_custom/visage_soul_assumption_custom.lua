...
LinkLuaModifier(
	"modifier_visage_soul_assumption_custom",
	"heroes/npc_dota_hero_visage_custom/visage_soul_assumption_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_soul_assumption_custom_stacks",
	"heroes/npc_dota_hero_visage_custom/visage_soul_assumption_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_soul_assumption_custom_counter",
	"heroes/npc_dota_hero_visage_custom/visage_soul_assumption_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_soul_assumption_custom_resist",
	"heroes/npc_dota_hero_visage_custom/visage_soul_assumption_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function visage_soul_assumption_custom:GetTargetCount()
	...
	if self:GetCaster():HasModifier("modifier_visage_15") then
		target_count = target_count
			+ self.modifier_visage_15_targets[self:GetCaster():GetTalentLevel("modifier_visage_15")]
	end
	return target_count
end
...
function visage_soul_assumption_custom:OnSpellStart()
	...
	local assumption_counter_modifier =
		self:GetCaster():FindModifierByNameAndCaster("modifier_visage_soul_assumption_custom_counter", self:GetCaster())
	local damage_bars = 0
	...
	if assumption_counter_modifier then
		...
		overflow_counter = math.max(
			assumption_counter_modifier:GetStackCount()
				- (self:GetSpecialValueFor("damage_limit") * self:GetStackLimit()),
			0
		)
		local assumption_stack_modifiers = self:GetCaster()
			:FindAllModifiersByName("modifier_visage_soul_assumption_custom_stacks")
	...
	if damage_bars > 0 then
		effect_name = "particles/units/heroes/hero_visage/visage_soul_assumption_bolt"
			.. math.min(damage_bars, 6)
			.. ".vpcf"
	end
	local projectile = {
		Target = nil,
	...
	if target_count > 1 and target then
		for _, target_type in pairs({ DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_BASIC }) do
			...
			local extra_enemies = FindUnitsInRadius(
				...
				target:GetAbsOrigin(),
				nil,
				self.modifier_visage_15_search,
...
function visage_soul_assumption_custom:OnProjectileHit_ExtraData(target, location, data)
	if target and not target:TriggerSpellAbsorb(self) then
		...
		if self:GetCaster():HasModifier("modifier_visage_18") and data.charges > 0 then
			local resist_reduction = data.charges
				* self.modifier_visage_18[self:GetCaster():GetTalentLevel("modifier_visage_18")]
			target:RemoveModifierByName("modifier_visage_soul_assumption_custom_resist")
			target:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_visage_soul_assumption_custom_resist",
				{ duration = self.modifier_visage_18_duration, resist_reduction = resist_reduction }
			)
		end
		ApplyDamage({
			victim = target,
			damage = soul_base_damage + (soul_charge_damage * data.charges),
...
function modifier_visage_soul_assumption_custom:IsHidden()
	return self:GetStackCount() <= 0
end
function modifier_visage_soul_assumption_custom:IsPurgable()
	return false
end
function modifier_visage_soul_assumption_custom:GetAttributes()
...
function modifier_visage_soul_assumption_custom:UpdateCharges()
	...
	local assumption_counter_modifier =
		self:GetParent():FindModifierByNameAndCaster("modifier_visage_soul_assumption_custom_counter", self:GetCaster())
...
function modifier_visage_soul_assumption_custom:DisableForTalent4()
	...
	local assumption_stack_modifiers = self:GetParent()
		:FindAllModifiersByName("modifier_visage_soul_assumption_custom_stacks")
	for _, mod in pairs(assumption_stack_modifiers) do
		mod:Destroy()
	end
...
function modifier_visage_soul_assumption_custom:OnTakeDamage(keys)
	...
	if
		(keys.unit:GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D()
			<= self:GetAbility():GetSpecialValueFor("radius")
		and (keys.attacker:IsControllableByAnyPlayer())
		...
		and keys.inflictor ~= self:GetAbility()
	then
		self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_visage_soul_assumption_custom_counter",
			{ duration = self:GetAbility():GetSpecialValueFor("stack_duration"), stacks = keys.original_damage }
		)
		self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
...
function modifier_visage_soul_assumption_custom_stacks:IsHidden()
	return true
end
function modifier_visage_soul_assumption_custom_stacks:IsPurgable()
	return false
end
function modifier_visage_soul_assumption_custom_stacks:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_visage_soul_assumption_custom_stacks:OnCreated(params)
	...
	local assumption_modifier = self:GetParent()
		:FindModifierByNameAndCaster("modifier_visage_soul_assumption_custom", self:GetCaster())
	local assumption_counter_modifier =
		self:GetParent():FindModifierByNameAndCaster("modifier_visage_soul_assumption_custom_counter", self:GetCaster())
...
function modifier_visage_soul_assumption_custom_counter:IsHidden()
	return true
end
function modifier_visage_soul_assumption_custom_counter:IsPurgable()
	return false
end

modifier_visage_soul_assumption_custom_resist = class({})
function modifier_visage_soul_assumption_custom_resist:IsHidden()
	return false
end
function modifier_visage_soul_assumption_custom_resist:IsDebuff()
	return true
end
function modifier_visage_soul_assumption_custom_resist:GetTexture()
	return "visage_18"
end
