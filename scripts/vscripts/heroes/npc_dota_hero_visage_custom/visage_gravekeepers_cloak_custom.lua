...
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_custom",
	"heroes/npc_dota_hero_visage_custom/visage_gravekeepers_cloak_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_custom_precast",
	"heroes/npc_dota_hero_visage_custom/visage_gravekeepers_cloak_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_custom_buff",
	"heroes/npc_dota_hero_visage_custom/visage_gravekeepers_cloak_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_custom_secondary",
	"heroes/npc_dota_hero_visage_custom/visage_gravekeepers_cloak_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_visage_gravekeepers_cloak_custom:IsAura()
	return true
end
function modifier_visage_gravekeepers_cloak_custom:IsAuraActiveOnDeath()
	return false
end
function modifier_visage_gravekeepers_cloak_custom:GetAuraRadius()
	return self:GetAbility():GetSpecialValueFor("radius")
end
function modifier_visage_gravekeepers_cloak_custom:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE
		+ DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD
		+ DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED
end
function modifier_visage_gravekeepers_cloak_custom:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_visage_gravekeepers_cloak_custom:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_visage_gravekeepers_cloak_custom:GetModifierAura()
	return "modifier_visage_gravekeepers_cloak_custom_secondary"
end
function modifier_visage_gravekeepers_cloak_custom:GetAuraEntityReject(hTarget)
	return self:GetCaster():PassivesDisabled()
		or not hTarget:GetOwner()
		or not hTarget:GetOwner() == self:GetCaster()
		or not string.find(hTarget:GetUnitName(), "npc_dota_visage_familiar")
end
...
function modifier_visage_gravekeepers_cloak_custom:OnCreated()
	...
	self.cloak_particle = ParticleManager:CreateParticle(
...
function modifier_visage_gravekeepers_cloak_custom:UpdateCloakParticle()
	if not IsServer() or not self.cloak_particle then
		return
	end
...
function modifier_visage_gravekeepers_cloak_custom:DoVisage2Pulse()
	...
	if parent:HasModifier("modifier_wodawisp") then
		return
	end
	if parent:HasModifier("modifier_wodarelax") then
		return
	end
	if parent:HasModifier("modifier_disconnect_player_no_damage") then
		return
	end
	...
	local particle =
		ParticleManager:CreateParticle("particles/visage_vacuum/visage_vacuum.vpcf", PATTACH_ABSORIGIN_FOLLOW, parent)
...
function modifier_visage_gravekeepers_cloak_custom:GetModifierTotal_ConstantBlock(keys)
	if
		...
		and keys.damage > self:GetAbility():GetSpecialValueFor("minimum_damage")
	...
	then
		...
		if not self:GetParent():PassivesDisabled() then
			local reduction = math.min(
				self:GetAbility():GetSpecialValueFor("damage_reduction") * (self:GetStackCount() + 1),
				self:GetAbility():GetSpecialValueFor("max_damage_reduction")
			)
...
function modifier_visage_gravekeepers_cloak_custom_secondary:UpdateCloakParticle()
	...
	if layers == self.last_layers then
		return
	end
...
function modifier_visage_gravekeepers_cloak_custom_secondary:GetModifierTotal_ConstantBlock(keys)
	local reduction = math.min(
		self:GetCaster():GetModifierStackCount("modifier_visage_gravekeepers_cloak_custom", self:GetCaster())
			* self.damage_reduction,
		self.max_damage_reduction
	)
	return keys.damage * reduction * 0.01
end

modifier_visage_gravekeepers_cloak_custom_buff = class({})
function modifier_visage_gravekeepers_cloak_custom_buff:IsPurgable()
	return false
end
function modifier_visage_gravekeepers_cloak_custom_buff:OnCreated()
	...
	local enemies = FindUnitsInRadius(
		...
		caster:GetAbsOrigin(),
		nil,
		self:GetParent():GetAoeBonus(self:GetAbility().modifier_visage_7_radius),
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		DOTA_UNIT_TARGET_FLAG_NONE,
		FIND_ANY_ORDER,
	...
	for _, enemy in pairs(enemies) do
		enemy:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_stunned",
			{ duration = self:GetAbility().modifier_visage_7_stun * (1 - enemy:GetStatusResistance()) }
		)
		ApplyDamage({
			victim = enemy,
			attacker = caster,
...
function modifier_visage_gravekeepers_cloak_custom_precast:IsPurgable()
	return false
end
function modifier_visage_gravekeepers_cloak_custom_precast:OnRemoved()
	if not IsServer() or not self:GetAbility() or self:GetRemainingTime() > 0 then
		return
	end
	...
	self:GetCaster():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_visage_gravekeepers_cloak_custom_buff",
		{ duration = self:GetAbility().modifier_visage_7_invuln }
	)
end
