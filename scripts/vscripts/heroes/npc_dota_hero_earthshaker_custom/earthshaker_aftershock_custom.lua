...
LinkLuaModifier(
	"modifier_earthshaker_aftershock_custom",
	"heroes/npc_dota_hero_earthshaker_custom/earthshaker_aftershock_custom",
...
function earthshaker_aftershock_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_earthshaker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_earthshaker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_earthshaker.vpcf",
		context
	)
end
...
function modifier_earthshaker_aftershock_custom:AfterShockApply(location)
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_17") then
		damage = damage
			+ self:GetAbility().modifier_earthshaker_17[self:GetCaster():GetTalentLevel("modifier_earthshaker_17")]
	end
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_10") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_earthshaker_10[self:GetCaster():GetTalentLevel("modifier_earthshaker_10")]
			)
	end

	if self:GetCaster():HasModifier("modifier_earthshaker_13") then
		radius = radius
			+ self:GetAbility().modifier_earthshaker_13[self:GetCaster():GetTalentLevel("modifier_earthshaker_13")]
	end
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_14") then
		...
		for k, v in pairs(Entities:FindAllByName("npc_dota_thinker")) do
			local modifier_earthshaker_fissure_custom_thinker =
				v:FindModifierByName("modifier_earthshaker_fissure_custom_thinker")
			if
				modifier_earthshaker_fissure_custom_thinker
				and modifier_earthshaker_fissure_custom_thinker:GetCaster() == self:GetCaster()
			then
				local enemies = FindUnitsInRadius(
					self:GetCaster():GetTeamNumber(),
					v:GetOrigin(),
...
function modifier_earthshaker_aftershock_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_11") then
		...
		if
			...
			> self:GetAbility().modifier_earthshaker_11_range[self:GetCaster():GetTalentLevel("modifier_earthshaker_11")]
		then
