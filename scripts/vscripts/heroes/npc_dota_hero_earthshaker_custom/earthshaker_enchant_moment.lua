...
LinkLuaModifier(
	"modifier_earthshaker_enchant_moment",
	"heroes/npc_dota_hero_earthshaker_custom/earthshaker_enchant_moment",
...
LinkLuaModifier(
	"modifier_earthshaker_enchant_moment_stack",
	"heroes/npc_dota_hero_earthshaker_custom/earthshaker_enchant_moment",
	LUA_MODIFIER_MOTION_NONE
)
...
function earthshaker_enchant_moment:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_7") and target ~= self:GetCaster() then
		...
		local arc = caster:AddNewModifier(caster, self, "modifier_earthshaker_enchant_moment_jump", {
			target_x = point.x,
			target_y = point.y,
			distance = distance,
			duration = 1,
		...
		arc:SetEndCallback(function()
			...
			for _, enemy in pairs(enemies) do
				ApplyDamage({
					...
					damage = (self:GetCaster():GetMana() / 100 * self:GetSpecialValueFor("damage_mana"))
						+ self:GetSpecialValueFor("base_damage"),
			...
			local nFXIndex = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_earthshaker/earthshaker_totem_leap_impact.vpcf",
				PATTACH_WORLDORIGIN,
				self:GetCaster()
			)
		...
		if self:GetCaster():HasModifier("modifier_earthshaker_19") then
			...
			self:GetCaster()
				:AddNewModifier(self:GetCaster(), self, "modifier_earthshaker_enchant_moment", { duration = duration })
		end
		self:GetCaster():EmitSound("Hero_EarthShaker.Totem")
		local aftershock = self:GetCaster():FindModifierByName("modifier_earthshaker_aftershock_custom")
		if aftershock then
...
function modifier_earthshaker_enchant_moment:GetModifierExtraManaPercentage()
	return self:GetAbility().modifier_earthshaker_19[self:GetCaster():GetTalentLevel("modifier_earthshaker_19")]
		* self:GetStackCount()
end
...
function modifier_earthshaker_enchant_moment_stack:IsHidden()
	return true
end
function modifier_earthshaker_enchant_moment_stack:GetAttributes()
