...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_earthshaker_custom/earthshaker_enchant_totem_custom",
		...
		if not self:GetCaster():IsCustomHasTethered() and not self:GetCaster():IsRooted() then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					...
					{ message = "#DOTA_ToolTip_Disabled_By_Root" }
				)
			end
			return false
		end
...
function earthshaker_enchant_totem_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_7") and target ~= self:GetCaster() then
		if not self:GetCaster():IsCustomHasTethered() and not self:GetCaster():IsRooted() then
			...
			local arc = caster:AddNewModifier(caster, self, "modifier_earthshaker_enchant_totem_custom_jump", {
				target_x = point.x,
				target_y = point.y,
				distance = distance,
				duration = 1,
...
function modifier_earthshaker_enchant_totem_custom:GetModifierBaseDamageOutgoing_Percentage()
	...
	if self:GetCaster():HasModifier("modifier_earthshaker_4") then
		bonus = self:GetAbility().modifier_earthshaker_4_bonus_dmg[self:GetCaster()
			:GetTalentLevel("modifier_earthshaker_4")] * self:GetParent():GetLevel()
	end
...
function modifier_earthshaker_enchant_totem_custom:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	local modifier_revenants_brooch_custom_counter = self:GetParent()
		:FindModifierByName("modifier_revenants_brooch_custom_counter")
	if
		modifier_revenants_brooch_custom_counter
		and modifier_revenants_brooch_custom_counter:GetOverrideAttackMagical() > 0
	then
		ApplyDamage({
