...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_undying_custom/undying_flesh_golem_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_undying_flesh_golem_custom_debuff",
	"heroes/npc_dota_hero_undying_custom/undying_flesh_golem_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function undying_flesh_golem_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_undying_flesh_golem_custom", { duration = duration })
end

modifier_undying_flesh_golem_custom = class({})
function modifier_undying_flesh_golem_custom:IsPurgable()
	return false
end
function modifier_undying_flesh_golem_custom:IsPurgeException()
	return false
end
function modifier_undying_flesh_golem_custom:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_undying_7")
end
...
function modifier_undying_flesh_golem_custom:OnAttackLanded(params)
	...
	if undying_tombstone_custom and undying_tombstone_custom:GetLevel() > 0 then
		undying_tombstone_custom:CreateZombieDecay(
			params.target:GetAbsOrigin() + RandomVector(zombie_spawn_padding),
			zombie_duration,
			true
		)
	end
