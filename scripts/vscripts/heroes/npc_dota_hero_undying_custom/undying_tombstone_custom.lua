...
LinkLuaModifier(
	"modifier_undying_tombstone_custom",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_undying_tombstone_custom_zombie",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zombie_ai_no_tombstone",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tombstone_movespeed_aura",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tombstone_damage_aura",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tombstone_movespeed",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tombstone_damage",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_undying_tombstone_custom_invul",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_undying_tombstone_body_bonuses_damage",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function undying_tombstone_custom:OnAbilityPhaseStart()
	...
	if
		self:GetCursorTarget() == nil
		and Entities:FindByModelWithin(nil, "models/props_structures/radiant_statue001.vmdl", self.vTargetPosition, 800) ~= nil
		and GetMapName() == "overthrow"
	then
		return false
	end
...
function undying_tombstone_custom:SpawnTombstone(point)
	...
	for k, v in pairs(Entities:FindAllInSphere(self:GetCaster():GetAbsOrigin(), 99999)) do
		if
			v:GetName() == "npc_dota_unit_undying_tombstone"
			and (
				v:FindModifierByName("modifier_undying_tombstone_custom")
				and v:FindModifierByName("modifier_undying_tombstone_custom"):GetCaster() == self:GetCaster()
			)
		then
			v:ForceKill(false)
		end
	...
	local nTombstoneFX = ParticleManager:CreateParticle(
...
function modifier_undying_tombstone_custom:IsHidden()
	return true
end
function modifier_undying_tombstone_custom:IsPurgable()
	return false
end
function modifier_undying_tombstone_custom:IsDebuff()
	return false
end
...
function modifier_undying_tombstone_custom:OnIntervalThink()
	...
	if
		self:GetCaster():HasModifier("modifier_undying_17")
		and not self:GetParent():HasModifier("modifier_tombstone_damage_aura")
	then
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_tombstone_damage_aura", {})
	end
		...
		for _, enemy in pairs(enemies) do
			if not enemy:IsCourier() and not IsUndyingZombie(enemy) then
				...
				FindClearSpaceForUnit(
					zombie,
					enemy:GetAbsOrigin() + RandomVector(enemy:GetHullRadius() + zombie:GetHullRadius()),
					true
				)
				ResolveNPCPositions(zombie:GetAbsOrigin(), enemy:GetHullRadius())
				zombie:SetAggroTarget(enemy)
				zombie:SetForceAttackTarget(enemy)
				local mod = zombie:AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_undying_tombstone_custom_zombie",
					{ enemy_entindex = enemy:entindex(), tombstone = self:GetParent():entindex() }
				)
				zombie:AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_undying_tombstone_custom_invul",
					{}
				)
				local undying_tombstone_zombie_deathstrike =
					zombie:FindAbilityByName("undying_tombstone_zombie_deathstrike")
				if undying_tombstone_zombie_deathstrike then
					undying_tombstone_zombie_deathstrike:SetLevel(1)
				end
				local neutral_spell_immunity = zombie:FindAbilityByName("neutral_spell_immunity")
...
function modifier_undying_tombstone_custom_zombie:IsPurgable()
	return false
end
function modifier_undying_tombstone_custom_zombie:IsHidden()
	return true
end
...
function modifier_undying_tombstone_custom_zombie:GetModifierProcAttack_BonusDamage_Physical(params)
	if self:GetCaster():HasModifier("modifier_undying_14") then
		...
		local damage = self:GetCaster():GetAverageTrueAttackDamage(nil)
			/ 100
			* self:GetAbility().modifier_undying_14[self:GetCaster():GetTalentLevel("modifier_undying_14")]
...
function modifier_undying_tombstone_custom_zombie:GetModifierAttackRangeBonus()
	if not self:GetCaster():HasModifier("modifier_undying_11") then
		return
	end
...
function modifier_zombie_ai_no_tombstone:IsHidden()
	return true
end
function modifier_zombie_ai_no_tombstone:IsPurgable()
	return false
end
function modifier_zombie_ai_no_tombstone:IsDebuff()
	return false
end
...
function modifier_zombie_ai_no_tombstone:OnIntervalThink()
	...
	if self:GetParent().target and self:GetParent():GetAggroTarget() == nil then
		ExecuteOrderFromTable({
			UnitIndex = self:GetParent():entindex(),
			OrderType = DOTA_UNIT_ORDER_ATTACK_MOVE,
			Position = self:GetParent().target:GetOrigin(),
		})
	end
...
function modifier_tombstone_movespeed_aura:IsAura()
	return true
end
function modifier_tombstone_movespeed_aura:IsAuraActiveOnDeath()
	return false
end
function modifier_tombstone_movespeed_aura:GetAuraDuration()
	return 0
end
...
function modifier_tombstone_damage_aura:IsAura()
	return true
end
function modifier_tombstone_damage_aura:IsAuraActiveOnDeath()
	return false
end
function modifier_tombstone_damage_aura:GetAuraDuration()
	return 0
end
...
function modifier_tombstone_damage:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/undying_debuff_tomb_mana.vpcf",
...
function modifier_tombstone_damage:OnIntervalThink()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/items2_fx/necronomicon_archer_manaburn_explosion.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetParent()
	)
	ParticleManager:SetParticleControlEnt(
	...
	local damage = self:GetCaster():GetManaRegen()
		/ 100
		* self:GetAbility().modifier_undying_17_damage[self:GetCaster():GetTalentLevel("modifier_undying_17")]
	ApplyDamage({
...
LinkLuaModifier(
	"modifier_undying_tombstone_body",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_undying_tombstone_body_bonuses",
	"heroes/npc_dota_hero_undying_custom/undying_tombstone_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_undying_tombstone_body:SpawnZombie()
	...
	local zombie = CreateUnitByName(
		"npc_dota_unit_undying_zombie_tombstone_body",
		self:GetParent():GetAbsOrigin(),
		true,
		self:GetCaster(),
		self:GetCaster(),
	...
	FindClearSpaceForUnit(
		zombie,
		self:GetParent():GetAbsOrigin() + RandomVector(self:GetParent():GetHullRadius() + zombie:GetHullRadius()),
		true
	)
	...
	local mod =
		zombie:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_undying_tombstone_body_bonuses", {})
...
function modifier_undying_tombstone_body_bonuses_damage:IsHidden()
	return true
end
function modifier_undying_tombstone_body_bonuses_damage:IsPurgable()
	return false
end
function modifier_undying_tombstone_body_bonuses_damage:IsPurgeException()
	return false
end
...
function modifier_undying_tombstone_body_bonuses:IsPurgable()
	return false
end
function modifier_undying_tombstone_body_bonuses:IsHidden()
	return true
end
function modifier_undying_tombstone_body_bonuses:IsPurgable()
	return false
...
function modifier_undying_tombstone_body_bonuses:GetModifierProcAttack_BonusDamage_Physical(params)
	if not self:GetCaster():HasModifier("modifier_undying_14") then
		return
	end
	if self:GetCaster():HasModifier("modifier_disconnect_player_no_damage") then
		return
	end
	local modifier_undying_tombstone_body_bonuses_damage =
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_undying_tombstone_body_bonuses_damage", {})
...
function modifier_undying_tombstone_body_bonuses:CheckState()
	return {
		...
		[MODIFIER_STATE_DISARMED] = not self:GetCaster():IsAlive()
			or self:GetCaster():HasModifier("modifier_wodawisp")
...
function modifier_undying_tombstone_custom_invul:IsPurgable()
	return false
end

function modifier_undying_tombstone_custom_invul:IsHidden()
	return true
end

function modifier_undying_tombstone_custom_invul:RemoveOnDeath()
	return false
end
