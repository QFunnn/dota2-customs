...
LinkLuaModifier(
	"modifier_night_stalker_day_walker",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_day_walker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_day_walker_vision",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_day_walker",
	LUA_MODIFIER_MOTION_NONE
)
...
function night_stalker_day_walker:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_night_stalker_day_walker", { duration = duration })

	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_night_stalker/nightstalker_ulti_day.vpcf",
	...
	if self:GetCaster():HasModifier("modifier_night_stalker_13") then
		local units = FindUnitsInRadius(
			...
			DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + MODIFIER_STATE_OUT_OF_GAME,
			FIND_CLOSEST,
			false
		)
		for _, unit in pairs(units) do
			if
				unit
				and (
					unit:GetUnitName() == "npc_woda_pig"
					or unit:GetUnitName() == "npc_woda_frog"
					or unit:HasModifier("modifier_wodacreepchampion")
					or unit:HasModifier("modifier_wodacreepchampionred")
				)
			then
				unit:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_night_stalker_day_walker_vision",
					{ duration = self.modifier_night_stalker_13_pigfrog_vision }
				)
				break
			end
		end
...
function modifier_night_stalker_day_walker:IsPurgable()
	return false
end
function modifier_night_stalker_day_walker:IsPurgeException()
	return false
end
...
function modifier_night_stalker_day_walker:OnIntervalThink()
	...
	self.current_shield =
		math.min(self.current_shield + (self:GetAbility().modifier_night_stalker_17_regen * 0.1), self.max_shield)
...
function modifier_night_stalker_day_walker:GetModifierProcAttack_BonusDamage_Physical(params)
	if params.target then
		...
		if self:GetParent():HasModifier("modifier_night_stalker_17") then
			return
		end
...
function modifier_night_stalker_day_walker_vision:IsHidden()
	return true
end
function modifier_night_stalker_day_walker_vision:IsPurgable()
	return false
end
function modifier_night_stalker_day_walker_vision:IsPurgeException()
	return false
end
