...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_crippling_fear_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_crippling_fear_custom_debuff",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_crippling_fear_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function night_stalker_crippling_fear_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_night_stalker_crippling_fear_custom", { duration = duration })
end
...
function modifier_night_stalker_crippling_fear_custom:IsPurgable()
	return false
end
function modifier_night_stalker_crippling_fear_custom:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_night_stalker_21")
end
...
function modifier_night_stalker_crippling_fear_custom:IsAura()
	if self:GetCaster():HasModifier("modifier_night_stalker_21") then
		...
		if not self:GetParent():IgnoreWispAndInvisAndRelax(true, true, true) then
			return
		end
	end
	return true
...
function modifier_night_stalker_crippling_fear_custom_debuff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_night_stalker_15") then
		damage = damage
			+ self:GetAbility().modifier_night_stalker_15[self:GetCaster():GetTalentLevel("modifier_night_stalker_15")]
	end
	if self:GetCaster():HasModifier("modifier_night_stalker_19") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_night_stalker_19[self:GetCaster()
					:GetTalentLevel("modifier_night_stalker_19")]
			)
	end
