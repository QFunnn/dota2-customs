...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_midnight_feast_custom",
...
function night_stalker_midnight_feast_custom:GetBehavior()
	if
		self:GetCaster():HasModifier("modifier_night_stalker_4")
		and self:GetCaster():GetModifierStackCount("modifier_night_stalker_midnight_feast_custom", self:GetCaster())
			>= 1
	then
		return DOTA_ABILITY_BEHAVIOR_UNIT_TARGET + DOTA_ABILITY_BEHAVIOR_POINT
	end
	if
		self:GetCaster():GetModifierStackCount("modifier_night_stalker_midnight_feast_custom", self:GetCaster()) >= 1
	then
...
function modifier_night_stalker_midnight_feast_custom:IsHidden()
	return true
end
function modifier_night_stalker_midnight_feast_custom:IsPurgable()
	return false
end
function modifier_night_stalker_midnight_feast_custom:IsPurgeException()
	return false
end
function modifier_night_stalker_midnight_feast_custom:RemoveOnDeath()
	return false
end
...
function modifier_night_stalker_midnight_feast_custom:OnIntervalThink()
	...
	if self:GetParent():HasModifier("modifier_night_stalker_10") then
		return
	end
	if
