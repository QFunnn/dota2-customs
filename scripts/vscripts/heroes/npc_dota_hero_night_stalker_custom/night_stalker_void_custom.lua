...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_void_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_void_custom_thinker",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_void_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_void_custom_thinker_debuff",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_void_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_void_custom_thinker_night",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_void_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_night_stalker_void_custom_thinker_night_buff",
	"heroes/npc_dota_hero_night_stalker_custom/night_stalker_void_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function night_stalker_void_custom:OnSpellStart()
	...
	if target == nil then
		local units = FindUnitsInRadius(
			...
			point,
			nil,
			self.modifier_night_stalker_5_radius,
	...
	if self:GetCaster():HasModifier("modifier_night_stalker_3") then
		CreateModifierThinker(
			...
			self,
			"modifier_night_stalker_void_custom_thinker",
			{ duration = self.modifier_night_stalker_3_duration },
			point,
			self:GetCaster():GetTeamNumber(),
			false
		)
...
function modifier_night_stalker_void_custom_thinker:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_night_stalker/night_stalker_void_zone.vpcf",
...
function modifier_night_stalker_void_custom_thinker_debuff:IsPurgable()
	return false
end
function modifier_night_stalker_void_custom_thinker_debuff:GetTexture()
	return "night_stalker_3"
end
...
function modifier_night_stalker_void_custom_thinker_debuff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_night_stalker_5") then
		if not self:GetAuraOwner():IsNull() then
			local modifier_night_stalker_void_custom_thinker =
				self:GetAuraOwner():FindModifierByName("modifier_night_stalker_void_custom_thinker")
			if modifier_night_stalker_void_custom_thinker then
				if modifier_night_stalker_void_custom_thinker.targets[self:GetParent():entindex()] == nil then
					self:GetParent():AddNewModifier(
						...
						"modifier_stunned",
						{
							duration = self:GetAbility().modifier_night_stalker_5_stun * (1 - self:GetParent()
								:GetStatusResistance()),
						}
					)
...
function modifier_night_stalker_void_custom_thinker_night_buff:IsPurgable()
	return false
end
function modifier_night_stalker_void_custom_thinker_night_buff:GetTexture()
	return "night_stalker_3"
end