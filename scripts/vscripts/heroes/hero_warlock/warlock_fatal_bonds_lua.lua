...
function modifier_fatal_bonds_debuff:IsDebuff()
	return true
end
function modifier_fatal_bonds_debuff:IsHidden()
	return false
end
function modifier_fatal_bonds_debuff:IsPurgable()
	return true
end
function modifier_fatal_bonds_debuff:GetAttributes()
...
function modifier_fatal_bonds_debuff:OnCreated(keys)
	if IsServer() then
		...
		if not keys.damage_share then
			keys.damage_share = 12
		end
		self.damage_share = keys.damage_share * 0.01
		self.share_targets = {}

		if keys.target_1 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_1))
		end
		if keys.target_2 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_2))
		end
		if keys.target_3 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_3))
		end
		if keys.target_4 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_4))
		end
		if keys.target_5 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_5))
		end
		if keys.target_6 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_6))
		end
		if keys.target_7 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_7))
		end
		if keys.target_8 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_8))
		end
		if keys.target_9 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_9))
		end
		if keys.target_10 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_10))
		end
		if keys.target_11 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_11))
		end
		if keys.target_12 then
			table.insert(self.share_targets, EntIndexToHScript(keys.target_12))
		end
	...
	if IsServer() then
		...
		for _, target in pairs(self.share_targets) do
			if
				(not target:IsNull())
				and target:IsAlive()
				and target:HasModifier("modifier_fatal_bonds_debuff")
				and parent ~= target
			then
				local bonds_pfx = ParticleManager:CreateParticle(
