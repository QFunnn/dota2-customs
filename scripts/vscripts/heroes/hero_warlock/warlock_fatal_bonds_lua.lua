...
LinkLuaModifier(
	"modifier_fatal_bonds_debuff_lua",
	"heroes/hero_warlock/warlock_fatal_bonds_lua",
...
function modifier_fatal_bonds_debuff_lua:IsDebuff()
	return true
end

function modifier_fatal_bonds_debuff_lua:IsHidden()
	return false
end

function modifier_fatal_bonds_debuff_lua:IsPurgable()
	return true
end

function modifier_fatal_bonds_debuff_lua:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_fatal_bonds_debuff_lua:ShouldUseOverheadOffset()
	return true
end
...
function modifier_fatal_bonds_debuff_lua:OnTooltip()
	return self.damage_share_pct
		or (self:GetAbility() and self:GetAbility():GetSpecialValueFor("damage_share_percentage"))
		or 0
end
...
function modifier_fatal_bonds_debuff_lua:OnTakeDamage(event)
	...
	if HasFlag(event.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) then
		return
	end
