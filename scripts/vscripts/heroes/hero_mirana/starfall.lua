...
function ability_mirana_starfall:CastWave()
	...
	Timers:CreateTimer(0.8, function()
		if not caster or caster:IsNull() or not self or self:IsNull() then
			return
		end
		...
		if SecondaryAll[1] then
			SecondaryAll[1]:AddNewModifier(
				caster,
				self,
				"modifier_ability_mirana_starfall_delay",
				{ duration = HitDelay, secondary = 1 }
			)
		end
	end)
...
modifier_ability_mirana_starfall = class({
	...
	OnIntervalThink = function(self)
		...
		if
			...
			and ability.CastWave
			and parent
			and self.nextTime <= GameRules:GetGameTime()
		then
...
modifier_ability_mirana_starfall_delay = class({
	...
	OnCreated = function(self, table)
		...
		local fxAttack = ParticleManager:CreateParticle(
	...
	OnDestroy = function(self)
		...
		if parent:IsAlive() and self.BlindDuration > 0 and self.bIsSecondary then
			parent:AddNewModifier(
				caster,
				ability,
				"modifier_ability_mirana_starfall_blind",
