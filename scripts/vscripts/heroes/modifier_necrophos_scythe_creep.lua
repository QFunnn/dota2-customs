...
LinkLuaModifier(
	"modifier_necrolyte_reapers_scythe_respawn_time_custom",
	"heroes/modifier_necrophos_scythe_creep",
	LUA_MODIFIER_MOTION_NONE
)

modifier_necrophos_scythe_creep = class({})

function modifier_necrophos_scythe_creep:IsPurgable()
	return false
end
function modifier_necrophos_scythe_creep:IsHidden()
	return true
end
...
function modifier_necrophos_scythe_creep:OnCreated()
	if IsServer() then
		...
		local stun_fx = ParticleManager:CreateParticle(
	...
	if IsServer() then
		...
		if not target:IsAlive() and self:GetAbility().bRoundDueled == false then
			...
			local modifier = self:GetCaster()
				:FindModifierByName("modifier_necrolyte_reapers_scythe_respawn_time_custom")
			if modifier then
				modifier:IncrementStackCount()
			else
				modifier = self:GetCaster():AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_necrolyte_reapers_scythe_respawn_time_custom",
					{}
...
function modifier_necrolyte_reapers_scythe_respawn_time_custom:IsPurgable()
	return false
end
function modifier_necrolyte_reapers_scythe_respawn_time_custom:RemoveOnDeath()
	return false
end
