...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_tinker_custom/tinker_march_of_the_machines_custom",
...
function modifier_tinker_march_of_the_machines_custom_thinker:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_tinker_2") then
		duration = duration
			+ self:GetAbility().modifier_tinker_2_duration[self:GetCaster():GetTalentLevel("modifier_tinker_2")]
	end
	...
	if self:GetCaster():HasModifier("modifier_tinker_3") then
		splash_damage = splash_damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_tinker_3[self:GetCaster():GetTalentLevel("modifier_tinker_3")]
			)
	end
