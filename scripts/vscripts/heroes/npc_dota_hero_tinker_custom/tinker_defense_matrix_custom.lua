...
LinkLuaModifier(
	"modifier_tinker_defense_matrix_custom",
	"heroes/npc_dota_hero_tinker_custom/tinker_defense_matrix_custom",
...
function modifier_tinker_defense_matrix_custom:IsHidden()
	return false
end
...
function modifier_tinker_defense_matrix_custom:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_tinker_17") then
		self.damage_absorb = self.damage_absorb
			+ (
				self:GetCaster():GetIntellect(false)
				/ 100
				* self:GetAbility().modifier_tinker_17[self:GetCaster():GetTalentLevel("modifier_tinker_17")]
			)
	end
