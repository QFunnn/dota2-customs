...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_tinker_custom/tinker_deploy_turrets_custom",
...
LinkLuaModifier(
	"modifier_tinker_deploy_turrets_custom_handler",
	"heroes/npc_dota_hero_tinker_custom/tinker_deploy_turrets_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_tinker_deploy_turrets_custom_handler:IsHidden()
	return true
end
function modifier_tinker_deploy_turrets_custom_handler:IsPurgable()
	return false
end
function modifier_tinker_deploy_turrets_custom_handler:IsPurgeException()
	return false
end
function modifier_tinker_deploy_turrets_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_tinker_deploy_turrets_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_tinker_8") then
		self.drop_damage = self.drop_damage
			+ self:GetAbility().modifier_tinker_8[self:GetCaster():GetTalentLevel("modifier_tinker_8")]
	end
	if self:GetCaster():HasModifier("modifier_tinker_10") then
		self.missile_range = self.missile_range
			+ self:GetAbility().modifier_tinker_10[self:GetCaster():GetTalentLevel("modifier_tinker_10")]
		self.missile_target_range = self.missile_target_range
			+ self:GetAbility().modifier_tinker_10[self:GetCaster():GetTalentLevel("modifier_tinker_10")]
	end
...
function modifier_tinker_deploy_turrets_custom:OnIntervalThink()
	...
	if not self.spawned_turrets then
		...
		if self:GetCaster():HasModifier("modifier_tinker_13") then
			turret_cycle_delay = turret_cycle_delay
				- self:GetAbility().modifier_tinker_13[self:GetCaster():GetTalentLevel("modifier_tinker_13")]
		end
		...
		for i = 1, self.max do
			...
			local turret = CreateUnitByName(
				"npc_dota_tinker_turret" .. self:GetAbility():GetLevel(),
				line_position,
				true,
				nil,
				nil,
			...
			if self:GetCaster():HasModifier("modifier_tinker_10") then
				...
				new_health = new_health
					+ self:GetAbility().modifier_tinker_10_hp[self:GetCaster():GetTalentLevel("modifier_tinker_10")]
			...
			if IsValidCustom(turret) and turret:IsAlive() then
				...
				elseif IsValidCustom(self.target) then
					...
					if self.turrets[turret] <= 0 then
						...
						self.info.vSpawnOrigin =
							turret:GetAttachmentOrigin(turret:ScriptLookupAttachment("attach_attack1"))
