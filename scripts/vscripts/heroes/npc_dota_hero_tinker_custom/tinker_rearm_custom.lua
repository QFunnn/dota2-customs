...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_tinker_custom/tinker_rearm_custom",
...
function tinker_rearm_custom:GetManaCost(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_tinker_12") then
		manacost = manacost
			- (manacost / 100 * self.modifier_tinker_12_mana[self:GetCaster():GetTalentLevel("modifier_tinker_12")])
	end
	return manacost
end
...
function modifier_tinker_rearm_custom:IsPurgeException()
	return false
end
