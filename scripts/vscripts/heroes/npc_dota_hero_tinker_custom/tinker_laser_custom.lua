...
LinkLuaModifier(
	"modifier_tinker_laser_custom",
	"heroes/npc_dota_hero_tinker_custom/tinker_laser_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tinker_laser_custom_debuff",
	"heroes/npc_dota_hero_tinker_custom/tinker_laser_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function tinker_laser_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_tinker.vpcf",
		context
	)
end
...
function tinker_laser_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_tinker_19") then
		local units = FindUnitsInRadius(
			...
			target:GetAbsOrigin(),
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_tinker_19),
...
function modifier_tinker_laser_custom_debuff:GetTexture()
	return "tinker_20"
end
function modifier_tinker_laser_custom_debuff:GetAttributes()
...
function modifier_tinker_laser_custom_debuff:OnCreated()
	self.health = self:GetParent():GetMaxHealth()
		/ 100
		* self:GetAbility().modifier_tinker_20[self:GetCaster():GetTalentLevel("modifier_tinker_20")]
end
