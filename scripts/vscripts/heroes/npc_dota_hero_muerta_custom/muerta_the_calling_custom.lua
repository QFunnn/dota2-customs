...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_muerta_custom/muerta_the_calling_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_the_calling_custom_thinker",
	"heroes/npc_dota_hero_muerta_custom/muerta_the_calling_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_muerta_the_calling_custom_thinker_buff",
	"heroes/npc_dota_hero_muerta_custom/muerta_the_calling_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_the_calling_custom_thinker_heal",
	"heroes/npc_dota_hero_muerta_custom/muerta_the_calling_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_the_calling_caster",
	"heroes/npc_dota_hero_muerta_custom/muerta_the_calling_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_muerta_the_calling_custom_thinker_buff:IsHidden()
	return true
end
function modifier_muerta_the_calling_custom_thinker_buff:IsPurgable()
	return false
end
...
function modifier_muerta_the_calling_custom_thinker_buff:OnCreated()
	...
	self.radius = self:GetAbility():GetSpecialValueFor("dead_zone_distance")
		+ (self:GetAbility():GetSpecialValueFor("hit_radius") * 2)
end
...
function modifier_muerta_the_calling_custom_thinker_heal:IsPurgable()
	return false
end
...
function modifier_muerta_the_calling_custom:IsHidden()
	return true
end
function modifier_muerta_the_calling_custom:IsPurgable()
	return false
end
...
function modifier_muerta_the_calling_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_muerta_5") then
		self.wisp_1 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[1], y_offset[1], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
		)
		self.wisp_2 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[2], y_offset[2], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
		)
		self.wisp_3 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[3], y_offset[3], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
		)
		self.wisp_4 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[5], y_offset[5], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
		)
		self.wisp_5 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[6], y_offset[6], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
		)
		self.wisp_6 = CreateUnitByName(
			"npc_dota_companion",
			self:GetParent():GetAbsOrigin() + Vector(x_offset[7], y_offset[7], 0.0),
			true,
			self:GetCaster(),
			self:GetCaster():GetOwner(),
			self:GetCaster():GetTeamNumber()
...
function modifier_muerta_the_calling_custom_thinker:IsHidden()
	return true
end
function modifier_muerta_the_calling_custom_thinker:IsPurgable()
	return false
end
function modifier_muerta_the_calling_custom_thinker:RemoveOnDeath()
	return false
end
...
function modifier_muerta_the_calling_custom_damage:IsHidden()
	return true
end
function modifier_muerta_the_calling_custom_damage:OnCreated()
	...
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		...
		if self:GetCaster():HasModifier("modifier_muerta_1") then
			damage = damage
				+ (
					...
					/ 100
					* self:GetAbility().modifier_muerta_1[self:GetCaster():GetTalentLevel("modifier_muerta_1")]
				)
		end
		...
		self:GetParent():AddNewModifier(
			...
			"modifier_silence",
			{
				duration = self:GetAbility():GetSpecialValueFor("silence_duration") * (1 - self:GetParent()
					:GetStatusResistance()),
			}
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_muerta_11") then
		if self:GetParent():GetTeamNumber() == self:GetCaster():GetTeamNumber() then
			self:GetParent():Heal(
				self:GetCaster():GetAverageTrueAttackDamage(nil) / 100 * self:GetAbility().modifier_muerta_11,
				self:GetAbility()
			)
		end
	end
...
function modifier_muerta_the_calling_custom_debuff:IsPurgable()
	return false
end
...
function modifier_muerta_the_calling_caster:IsHidden()
	return true
end
function modifier_muerta_the_calling_caster:IsPurgable()
	return false
end
function modifier_muerta_the_calling_caster:RemoveOnDeath()
	return false
end
...
function modifier_muerta_the_calling_caster:OnCreated()
	...
	self.radius = self:GetAbility():GetSpecialValueFor("dead_zone_distance")
		+ (self:GetAbility():GetSpecialValueFor("hit_radius") * 2)
	local particle =
		ParticleManager:CreateParticle("particles/muerta_buff.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	ParticleManager:SetParticleControl(particle, 0, self:GetParent():GetAbsOrigin())
	ParticleManager:SetParticleControl(particle, 1, Vector(self.radius, self.radius, self.radius))
	self:AddParticle(particle, false, false, -1, false, false)
	local r = self.radius
...
function modifier_muerta_the_calling_caster:OnIntervalThink()
	...
	local new_radius_checker = self:GetAbility():GetSpecialValueFor("dead_zone_distance")
		+ (self:GetAbility():GetSpecialValueFor("hit_radius") * 2)
	...
	if
		...
		or self:GetCaster():PassivesDisabled()
	then
		if self.wisp_1 then
