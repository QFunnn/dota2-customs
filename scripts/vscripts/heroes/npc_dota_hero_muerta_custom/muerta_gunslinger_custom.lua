...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_muerta_custom/muerta_gunslinger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_gunslinger_custom_slow",
	"heroes/npc_dota_hero_muerta_custom/muerta_gunslinger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_muerta_gunslinger_custom:IsHidden()
	return true
end
function modifier_muerta_gunslinger_custom:IsPurgable()
	return false
end
...
function modifier_muerta_gunslinger_custom:OnAttackLanded(params)
	...
	if self:GetCaster():HasModifier("modifier_muerta_10") then
		params.target:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_muerta_gunslinger_custom_slow",
			{ duration = self:GetAbility().modifier_muerta_10_duration }
		)
	end
	if self:GetCaster():HasModifier("modifier_muerta_12") then
		...
		if target:HasModifier("modifier_generic_arc_lua") then
			return
		end
		...
		params.target:AddNewModifier(
			...
			{
				duration = 0.1,
				distance = self:GetAbility().modifier_muerta_12,
				height = 10,
...
function modifier_muerta_gunslinger_custom:OnAttack(params)
	...
	local heroes = FindUnitsInRadius(
		...
		nil,
		self:GetCaster():Script_GetAttackRange() + self:GetAbility():GetSpecialValueFor("target_search_bonus_range"),
	...
	for i = #creeps, 1, -1 do
		if
			creeps[i] ~= nil
			and (
				creeps[i] == params.target
				or creeps[i]:GetUnitName() == "npc_dota_bounty_hunter_gold_bag"
				or creeps[i]:GetUnitName() == "npc_dota_spirit_breaker_light"
			)
		then
...
function modifier_muerta_gunslinger_custom_slow:GetTexture()
	return "muearta_10"
end
