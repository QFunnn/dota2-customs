...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_muerta_custom/muerta_dead_shot_custom",
...
function muerta_dead_shot_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_muerta.vpcf",
		context
	)
...
function modifier_muerta_dead_shot_custom_thinker:IsHidden()
	return true
end
function modifier_muerta_dead_shot_custom_thinker:IsPurgable()
	return false
end
	...
	if target and data.tracking == 1 then
		...
		if not target:IsMagicImmune() then
			target:AddNewModifier(
				...
				self,
				"modifier_muerta_dead_shot_custom_debuff",
				{ duration = self:GetSpecialValueFor("impact_slow_duration") * (1 - target:GetStatusResistance()) }
			)
		end
		...
		local range = self:GetEffectiveCastRange(location, nil)
			* self:GetSpecialValueFor("ricochet_distance_multiplier")
		...
		if self:GetCaster():HasModifier("modifier_muerta_15") then
			...
			local heroes = FindUnitsInRadius(
				...
				target:GetOrigin(),
				nil,
				self:GetCaster():GetAoeBonus(self.modifier_muerta_15_radius),
				DOTA_UNIT_TARGET_TEAM_ENEMY,
			...
			for _, hero in pairs(heroes) do
				ApplyDamage({
					...
					ability = self,
					damage = self.modifier_muerta_15[self:GetCaster():GetTalentLevel("modifier_muerta_15")],
	...
	if target and data.tracking == 0 and target:entindex() ~= data.source then
		...
		if self:GetCaster():HasModifier("modifier_muerta_15") then
			...
			local heroes = FindUnitsInRadius(
				...
				target:GetOrigin(),
				nil,
				self.modifier_muerta_15_radius,
				DOTA_UNIT_TARGET_TEAM_ENEMY,
				DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
				DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES,
				0,
		...
		if self:GetCaster():HasModifier("modifier_muerta_16") then
			target:AddNewModifier(
				...
				"modifier_muerta_dead_shot_custom_fear",
				{
					duration = self:GetSpecialValueFor("ricochet_fear_duration") * (1 - target:GetStatusResistance()),
