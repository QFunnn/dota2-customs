...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_muerta_custom/muerta_pierce_the_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_pierce_the_veil_custom_range",
	"heroes/npc_dota_hero_muerta_custom/muerta_pierce_the_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_pierce_the_veil_custom_ghost_attack",
	"heroes/npc_dota_hero_muerta_custom/muerta_pierce_the_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function muerta_pierce_the_veil_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_muerta_pierce_the_veil_custom", { duration = duration })
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_muerta_pierce_the_veil_buff", { duration = duration })
end
...
function modifier_muerta_pierce_the_veil_custom:IsHidden()
	return true
end
function modifier_muerta_pierce_the_veil_custom:IsPurgeException()
	return false
end
...
function modifier_muerta_pierce_the_veil_custom:GetModifierAttackRangeBonus()
	if not self:GetCaster():HasModifier("modifier_muerta_12") then
		return
	end
...
function modifier_muerta_pierce_the_veil_custom:GetMinHealth()
	if not self:GetCaster():HasModifier("modifier_muerta_21") then
		return
	end
	return 1
end
