...
LinkLuaModifier(
	...
	"heroes/hero_lich/lich_chain_frost_custom",
...
function lich_chain_frost_custom:OnProjectileHit_ExtraData(target, location, extradata)
	...
	if extradata.main_chain_frost == 1 then
		Timers:CreateTimer(0.25, function()
			...
			for i = #enemies, 1, -1 do
				if
					...
					and (
						...
						or enemies[i]:HasModifier("modifier_creep_spawner_cha_ready")
					)
				then
			...
			for i = #enemies, 1, -1 do
				if enemies[i] ~= nil and (enemies[i]:IsNull() or not enemies[i]:IsAlive()) then
					table.remove(enemies, i)
				end
			end
			...
			chain_frost_projectile.ExtraData = {
				bounces_left = extradata.bounces_left,
				current_projectile_speed = projectile_speed,
				main_chain_frost = true,
				mega_main_frost = extradata.mega_main_frost,
				counter = extradata.counter,
