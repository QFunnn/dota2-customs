...
modifier_ability_lich_frost_shield = class({
	...
	IsPurgable = function(self)
		return (self.Undispellable or 0) == 0
	end,
...
function modifier_ability_lich_frost_shield:OnDeath(event)
	if self.FacetBonusDurationPerKillHero == 0 then
		return
	end
	...
	if caster and unit and attacker then
		...
		if
			(bDiedFromFrostShield and inflictor:GetCaster() == caster)
			or (bDiedUnderFrostShield and Modif:GetCaster() == caster)
		then
			local BonusDuration = self.FacetBonusDurationPerKillCreep
			if
				unit:IsRealHero()
				and not unit:IsIllusion()
				and not unit:IsStrongIllusion()
				and not unit:IsTempestDouble()
