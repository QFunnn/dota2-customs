...
LinkLuaModifier(
	"modifier_medusa_split_shot_custom",
	"heroes/npc_dota_hero_medusa_custom/medusa_split_shot_custom",
...
function medusa_split_shot_custom:ResetToggleOnRespawn()
	return false
end
...
function modifier_medusa_split_shot_custom:IsPurgeException()
	return false
end
function modifier_medusa_split_shot_custom:RemoveOnDeath()
	return false
end
...
function modifier_medusa_split_shot_custom:OnCreated(kv)
	...
	self.modifier_medusa_split_shot =
		self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_medusa_split_shot", {})
end
...
function modifier_medusa_split_shot_custom:SplitShotModifier(target)
	...
	if self:GetCaster():HasModifier("modifier_medusa_4") then
		if
			RollPercentage(self:GetAbility().modifier_medusa_4[self:GetCaster():GetTalentLevel("modifier_medusa_4")])
		then
			for i = 1, self.count do
				...
				self.parent:PerformAttack(
					target,
					false,
					self:GetCaster():HasModifier("modifier_medusa_5"),
					true,
					false,
					true,
					false,
