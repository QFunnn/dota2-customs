...
LinkLuaModifier(
	"modifier_medusa_mana_shield_custom",
	"heroes/npc_dota_hero_medusa_custom/medusa_mana_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_medusa_mana_shield_custom_active",
	"heroes/npc_dota_hero_medusa_custom/medusa_mana_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function medusa_mana_shield_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_medusa.vpcf",
		context
	)
end
...
function medusa_mana_shield_custom:OnToggle()
	...
	if self:GetToggleState() then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_medusa_mana_shield_custom_active",
			{ duration = self.modifier_medusa_20_duration }
		)
		self:StartCooldown(3)
	else
		local modifier_medusa_mana_shield_custom_active =
			self:GetCaster():FindModifierByName("modifier_medusa_mana_shield_custom_active")
...
function medusa_mana_shield_custom:Spawn()
	...
	if self:GetCaster():HasModifier("medusa_mana_shield_custom") then
		return
	end
...
function modifier_medusa_mana_shield_custom:IsPurgeException()
	return false
end
function modifier_medusa_mana_shield_custom:RemoveOnDeath()
	return false
end
...
function modifier_medusa_mana_shield_custom_active:IsPurgable()
	return false
end
