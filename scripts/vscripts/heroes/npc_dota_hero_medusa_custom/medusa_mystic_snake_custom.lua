...
LinkLuaModifier(
	"modifier_medusa_mystic_snake_custom_debuff",
	"heroes/npc_dota_hero_medusa_custom/medusa_mystic_snake_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_medusa_stone_gaze_custom_petrified",
	"heroes/npc_dota_hero_medusa_custom/medusa_stone_gaze_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_medusa_mystic_snake_custom_handler",
	"heroes/npc_dota_hero_medusa_custom/medusa_mystic_snake_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function medusa_mystic_snake_custom:OnProjectileHit_ExtraData(target, location, ExtraData)
		...
		if self:GetCaster():HasModifier("modifier_medusa_14") then
			...
			if medusa_stone_gaze_custom and medusa_stone_gaze_custom:GetLevel() > 0 then
				target:AddNewModifier(
					self:GetCaster(),
					medusa_stone_gaze_custom,
					"modifier_medusa_stone_gaze_custom_petrified",
					{
						duration = self.modifier_medusa_14,
						physical_bonus = medusa_stone_gaze_custom:GetSpecialValueFor("bonus_physical_damage"),
	...
	data.projectile_info.iMoveSpeed = data.projectile_info.iMoveSpeed
		+ (data.projectile_info.iMoveSpeed / 100 * self:GetSpecialValueFor("snake_speed_scale"))
...
function medusa_mystic_snake_custom:Returned(data)
	...
	if self:GetCaster():HasModifier("modifier_medusa_10") then
		self:GetCaster():Heal(
			data.mana_stolen / 100 * self.modifier_medusa_10[self:GetCaster():GetTalentLevel("modifier_medusa_10")],
			self
		)
...
function modifier_medusa_mystic_snake_custom_handler:IsHidden()
	return true
end
function modifier_medusa_mystic_snake_custom_handler:IsPurgable()
	return false
end
function modifier_medusa_mystic_snake_custom_handler:IsPurgeException()
	return false
end
function modifier_medusa_mystic_snake_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_medusa_mystic_snake_custom_handler:OnAbilityFullyCast(params)
	...
	if self:GetCaster():HasModifier("modifier_medusa_mystic_snake_custom_handler_cooldown") then
		return
	end
	if not self:GetCaster():HasModifier("modifier_medusa_11") then
		return
	end
	if params.unit:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		return
	end
	...
	self:GetCaster():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_medusa_mystic_snake_custom_handler_cooldown",
		{ duration = self:GetAbility().modifier_medusa_11[self:GetCaster():GetTalentLevel("modifier_medusa_11")] }
	)
end

modifier_medusa_mystic_snake_custom_handler_cooldown = class({})
function modifier_medusa_mystic_snake_custom_handler_cooldown:IsPurgable()
	return false
end
function modifier_medusa_mystic_snake_custom_handler_cooldown:IsPurgeException()
	return false
end
function modifier_medusa_mystic_snake_custom_handler_cooldown:RemoveOnDeath()
	return false
end
function modifier_medusa_mystic_snake_custom_handler_cooldown:IsDebuff()
	return true
end
function modifier_medusa_mystic_snake_custom_handler_cooldown:GetTexture()
	return "medusa_11"
end