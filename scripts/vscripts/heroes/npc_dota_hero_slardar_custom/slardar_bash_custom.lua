...
LinkLuaModifier(
	"modifier_slardar_bash_custom",
	"heroes/npc_dota_hero_slardar_custom/slardar_bash_custom",
	...
	if self:GetCaster():HasModifier("modifier_slardar_20") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_slardar_20[self:GetCaster():GetTalentLevel("modifier_slardar_20")]
			)
	end
	...
	if self:GetStackCount() > attack_count then
		...
		if self:GetCaster():HasModifier("modifier_slardar_8") then
			local enemies = FindUnitsInRadius(
				...
				nil,
				self:GetAbility().modifier_slardar_8[self:GetCaster():GetTalentLevel("modifier_slardar_8")],
