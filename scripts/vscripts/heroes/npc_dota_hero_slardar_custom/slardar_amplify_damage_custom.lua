...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_slardar_custom/slardar_amplify_damage_custom",
...
function slardar_amplify_damage_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_slardar.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_slardar.vpcf",
		context
	)
end
...
function slardar_amplify_damage_custom:CustomActive(target, duration)
	...
	if
		modifier_slardar_amplify_damage_custom
		and modifier_slardar_amplify_damage_custom:GetRemainingTime()
...
function modifier_slardar_amplify_damage_custom:OnIntervalThink(kv)
	...
	if self.puddle_timer >= 0.5 then
		...
		local hThinkersNearby =
			Entities:FindAllByClassnameWithin("npc_dota_thinker", self:GetParent():GetAbsOrigin(), 50)
		...
		if thinker_near == nil then
			CreateModifierThinker(
				...
				self:GetAbility(),
				"modifier_slardar_slithereen_crush_custom_puddle",
				{ duration = puddle_duration, ultimate = 1 },
				self:GetParent():GetAbsOrigin(),
				self:GetCaster():GetTeamNumber(),
				false
			)
			...
			local modifier_slardar_slithereen_crush_custom_puddle =
				thinker_near:FindModifierByName("modifier_slardar_slithereen_crush_custom_puddle")
