...
function modifier_shredder_chakram_thinker_lua:OnCreated(kv)
	...
	self.disarm_modifier =
		self.caster:AddNewModifier(self.caster, self.ability, "modifier_shredder_chakram_disarm_lua", {})

	self.chakram_particle_main =
		ParticleManager:GetParticleReplacement("particles/shredder_chakram_upd.vpcf", self.caster)
	self.chakram_particle_stay = ParticleManager:GetParticleReplacement(
		"particles/units/heroes/hero_shredder/shredder_chakram_stay.vpcf",
		self.caster
	)
	self.chakram_particle_return = ParticleManager:GetParticleReplacement(
		"particles/units/heroes/hero_shredder/shredder_chakram_return.vpcf",
		self.caster
	)

	-- Create Particle
	self.effect_cast = ParticleManager:CreateParticle(self.chakram_particle_main, PATTACH_WORLDORIGIN, nil)
	ParticleManager:SetParticleControl(self.effect_cast, 0, self:GetParent():GetOrigin())
	ParticleManager:SetParticleControl(
		self.effect_cast,
		1,
		Vector(self:GetAbility():GetSpecialValueFor("radius"), 0, 0)
	)
	ParticleManager:SetParticleControl(self.effect_cast, 7, direction * self.speed)
...
function modifier_shredder_chakram_thinker_lua:OnDestroy()
	...
	if self.disarm_modifier and not self.disarm_modifier:IsNull() then
		self.disarm_modifier:Destroy()
	end

	if self.ability and not self.ability:IsNull() then
		self.caster:SwapAbilities(self.ability:GetAbilityName(), self.ability.return_ability_name, true, false)
	end
end
...
function modifier_shredder_chakram_thinker_lua:StayThink()
	...
	local cost = self.manacost * self.interval

	if (self.caster:GetOrigin() - origin):Length2D() > self.max_range or (not self.caster:IsAlive()) then
		self:Return()
		return
	end

	if self.caster:HasModifier("modifier_huskar_blood_magic") then
		if self.caster:GetHealth() <= cost then
			self:Return()
			return
		end
		self.caster:SetHealth(self.caster:GetHealth() - cost)
	else
		if self.caster:GetMana() < cost then
			self:Return()
			return
		end
		self.caster:SpendMana(cost, self.ability)
	end
...
function modifier_shredder_chakram_thinker_lua:Return()
	if self.state == _STATE_RETURNING then
		return
	end
	...
	self.effect_cast =
		ParticleManager:CreateParticle(self.chakram_particle_return, PATTACH_ABSORIGIN_FOLLOW, self.parent)
	ParticleManager:SetParticleControl(self.effect_cast, 0, self:GetParent():GetOrigin())
...
function modifier_shredder_chakram_thinker_lua:Damage(target_point, is_staying)
	...
	local function _damage(enemy)
		...
		enemy:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_shredder_chakram_slow_lua",
