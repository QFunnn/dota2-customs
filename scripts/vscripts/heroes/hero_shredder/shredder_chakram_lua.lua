...
shredder_chakram_lua = class({}) ---@class shredder_chakram_lua : CDOTA_Ability_Lua
shredder_chakram_lua_return = class({}) ---@class shredder_chakram_lua_return : CDOTA_Ability_Lua

LinkLuaModifier(
	"modifier_shredder_chakram_thinker_lua",
	"heroes/hero_shredder/modifier_shredder_chakram_thinker_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_shredder_chakram_slow_lua",
	"heroes/hero_shredder/modifier_shredder_chakram_slow_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_shredder_chakram_disarm_lua",
	"heroes/hero_shredder/modifier_shredder_chakram_disarm_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function shredder_chakram_lua:OnSpellStart()
	...
	if not self:IsHidden() then
		caster:SwapAbilities(self, return_ability, false, true)
	end
	caster:EmitSound("Hero_Shredder.Chakram.Cast")
end

function shredder_chakram_lua_return:OnSpellStart()
	if not self.thinker_modifier or type(self.thinker_modifier) ~= "table" then
		return
	end
...
function shredder_chakram_2_lua:GetBehavior()
	if IsServer() then
		...
		if not self.isInited and second_chakram > 0 then
			self:SetHidden(false)
			HeroBuilderService:SetAbilityToSlot(self:GetCaster(), self)
			self.isInited = true
		end
	end
...
function shredder_chakram_2_lua:Spawn()
	...
	if not main_ability or main_ability:IsNull() or main_ability:GetLevel() == 0 then
		return
	end
	...
	Timers:CreateTimer(0, function()
		if not self or self:IsNull() or not main_ability or main_ability:IsNull() then
			return
		end
