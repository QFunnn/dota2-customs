...
function modifier_kunkka_rum:OnDestroy()
	if IsServer() then
		...
		self:GetParent():AddNewModifier(
			...
			{ duration = ability:GetSpecialValueFor("duration"), stored_damage = self.damage_counter }
		)
		self.damage_counter = 0
...
function modifier_kunkka_rum:IsPurgeException()
	return false
end
...
function modifier_kunkka_rum_debuff:IsPurgeException()
	return false
end
