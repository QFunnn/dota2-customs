...
LinkLuaModifier(
	"modifier_kunkka_torrent_custom_damage",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_torrent_custom_debuff",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_torrent_custom_talent",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_kunkka_torrent_custom_puddle",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_torrent_custom_puddle_buff",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kunkka_torrent_custom:OnSpellStart(new_point)
	...
	local bubbles_pfx = ParticleManager:CreateParticleForTeam(
	...
	if self:GetCaster():HasModifier("modifier_kunkka_7") then
		local bonus_particle = ParticleManager:CreateParticleForTeam(
...
function kunkka_torrent_custom:CreateTorrent(origin)
	...
	if self:GetCaster():HasModifier("modifier_kunkka_7") then
		local torrent_fx = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_kunkka_7") then
		CreateModifierThinker(
			...
			self,
			"modifier_kunkka_torrent_custom_puddle",
			{ duration = self.modifier_kunkka_7_duration },
			origin,
			self:GetCaster():GetTeamNumber(),
			false
		)
	...
	Timers:CreateTimer(FrameTime(), function()
		...
		for _, enemy in pairs(enemies) do
			enemy:AddNewModifier(
				...
				ability,
				"modifier_kunkka_torrent_custom_damage",
				{ duration = math.min(dmg_duration, damage_tick_interval) }
			)
		end
		dmg_duration = dmg_duration - FrameTime()
		if dmg_duration <= 0 then
			return
		end
		return FrameTime()
	end)
...
function modifier_kunkka_torrent_custom_damage:IsHidden()
	return true
end
...
function modifier_kunkka_torrent_custom_damage:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_kunkka_2") then
		self.torrent_damage = self.torrent_damage
			+ (
				self:GetCaster():GetHealthRegen()
				/ 100
				* self:GetAbility().modifier_kunkka_2[self:GetCaster():GetTalentLevel("modifier_kunkka_2")]
			)
	end
...
function modifier_kunkka_torrent_custom_talent:IsHidden()
	return true
end
function modifier_kunkka_torrent_custom_talent:IsPurgable()
	return false
end
function modifier_kunkka_torrent_custom_talent:IsPurgeException()
	return false
end
function modifier_kunkka_torrent_custom_talent:DeclareFunctions()
	return {}
end
function modifier_kunkka_torrent_custom_talent:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_kunkka_1") then
		return
	end
	if self:GetParent():HasModifier("modifier_kunkka_torrent_custom_talent_cooldown") then
		return
	end
	if RollPercentage(self:GetAbility().modifier_kunkka_1_chance) then
		self:GetCaster():AddNewModifier(
			...
			"modifier_kunkka_torrent_custom_talent_cooldown",
			{
				duration = self:GetAbility().modifier_kunkka_1_cooldown[self:GetCaster()
					:GetTalentLevel("modifier_kunkka_1")],
			}
		)
...
function modifier_kunkka_torrent_custom_talent_cooldown:IsPurgable()
	return false
end
function modifier_kunkka_torrent_custom_talent_cooldown:IsPurgeException()
	return false
end
function modifier_kunkka_torrent_custom_talent_cooldown:IsDebuff()
	return true
end
function modifier_kunkka_torrent_custom_talent_cooldown:GetTexture()
	return "kunkka_1"
end

modifier_kunkka_torrent_custom_puddle = class({})
function modifier_kunkka_torrent_custom_puddle:IsHidden()
	return true
end
function modifier_kunkka_torrent_custom_puddle:IsPurgable()
	return false
end
...
function modifier_kunkka_torrent_custom_puddle_buff:IsPurgable()
	return false
end
function modifier_kunkka_torrent_custom_puddle_buff:IsPurgeException()
	return false
end
...
LinkLuaModifier(
	"modifier_kunkka_torrent_storm_custom",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_torrent_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_kunkka_torrent_storm_custom:IsHidden()
	return false
end
function modifier_kunkka_torrent_storm_custom:IsPurgable()
	return false
end
