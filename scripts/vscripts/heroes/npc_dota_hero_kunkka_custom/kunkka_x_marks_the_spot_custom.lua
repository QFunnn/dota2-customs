...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_kunkka_custom/kunkka_x_marks_the_spot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_x_marks_the_spot_custom_buff",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_x_marks_the_spot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_kunkka_x_marks_the_spot_custom_debuff:OnCreated(params)
	...
	self.x_pfx = ParticleManager:CreateParticle(
...
function modifier_kunkka_x_marks_the_spot_custom_debuff:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_kunkka_15") then
		return
	end
	...
	if self:GetParent():GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		...
		if distance <= 1500 and distance > 0 then
			heal = distance
				* self:GetAbility().modifier_kunkka_15[self:GetCaster():GetTalentLevel("modifier_kunkka_15")]
				/ 100
		end
...
function modifier_kunkka_x_marks_the_spot_custom_buff:RemoveOnDeath()
	return false
end
