...
LinkLuaModifier(
	"modifier_kunkka_tidebringer_custom_passive",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidebringer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_tidebringer_custom_particle",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidebringer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_tidebringer_custom_debuff_slow",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidebringer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_tidebringer_custom_buff_damage",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidebringer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_tidebringer_custom_buff_damage_stack",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidebringer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function kunkka_tidebringer_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_kunkka.vpcf",
		context
	)
end
...
function kunkka_tidebringer_custom:OnOrbImpact(params, mark)
	...
	if self:GetCaster():HasModifier("modifier_kunkka_4") then
		bonus = bonus
			+ (
				self:GetCaster():GetHealthRegen()
				/ 100
				* self.modifier_kunkka_4[self:GetCaster():GetTalentLevel("modifier_kunkka_4")]
			)
	end
	...
	local enemies_to_cleave = self:FindUnitsInCone(
		self:GetCaster():GetTeamNumber(),
		self:CalculateDirection(params.target, self:GetCaster()),
	...
	for _, target_cleave in pairs(enemies_to_cleave) do
		if self:GetCaster():HasModifier("modifier_kunkka_8") then
			target_cleave:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_kunkka_tidebringer_custom_debuff_slow",
				{ duration = self.modifier_kunkka_8_duration }
			)
		end
	end
	...
	if self:GetCaster():HasModifier("modifier_kunkka_11") then
		if #enemies_to_cleave > 0 then
			local mod = self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_kunkka_tidebringer_custom_buff_damage",
				{ duration = self.modifier_kunkka_11_duration[self:GetCaster():GetTalentLevel("modifier_kunkka_11")] }
			)
...
function kunkka_tidebringer_custom:FindUnitsInCone(
	teamNumber,
	vDirection,
	vPosition,
	startRadius,
...
function modifier_kunkka_tidebringer_custom_passive:OnIntervalThink()
	...
	if self:GetAbility():IsFullyCastable() then
		if not self:GetCaster():HasModifier("modifier_kunkka_tidebringer_custom_particle") then
			self:GetCaster()
				:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_kunkka_tidebringer_custom_particle", {})
		end
	else
...
function modifier_kunkka_tidebringer_custom_passive:GetModifierProcAttack_Feedback(params)
	if self:GetParent().mark then
		if self.ability.OnOrbImpact then
			self.ability:OnOrbImpact(params, true)
		end
		return
	end
		...
		if self:GetCaster():HasModifier("modifier_kunkka_4") then
			bonus = bonus
				+ (
					self:GetCaster():GetHealthRegen()
					/ 100
					* self:GetAbility().modifier_kunkka_4[self:GetCaster():GetTalentLevel("modifier_kunkka_4")]
				)
		end
		if self:GetCaster():HasModifier("modifier_kunkka_12") then
...
function modifier_kunkka_tidebringer_custom_particle:IsPurgable()
	return false
end
function modifier_kunkka_tidebringer_custom_particle:IsHidden()
	return true
end
function modifier_kunkka_tidebringer_custom_particle:RemoveOnDeath()
	return false
end
...
function modifier_kunkka_tidebringer_custom_particle:OnCreated()
	...
	local particle_glow_fx = ParticleManager:CreateParticle(
...
function modifier_kunkka_tidebringer_custom_buff_damage:GetTexture()
	return "kunkka_11"
end
...
function modifier_kunkka_tidebringer_custom_buff_damage_stack:IsHidden()
	return true
end
function modifier_kunkka_tidebringer_custom_buff_damage_stack:GetAttributes()
...
function modifier_kunkka_tidebringer_custom_buff_damage_stack:OnDestroy()
	...
	local modifier_kunkka_tidebringer_custom_buff_damage =
		self:GetCaster():FindModifierByName("modifier_kunkka_tidebringer_custom_buff_damage")
	if modifier_kunkka_tidebringer_custom_buff_damage then
		modifier_kunkka_tidebringer_custom_buff_damage:SetStackCount(
			modifier_kunkka_tidebringer_custom_buff_damage:GetStackCount() - self.count
		)
	end
end