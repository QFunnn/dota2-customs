...
LinkLuaModifier(
	"modifier_kunkka_admirals_rum_custom",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_admirals_rum_custom",
...
function kunkka_admirals_rum_custom:ApplyGhostShip(target)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_kunkka_admirals_rum_custom_buff",
		{ duration = buff_duration, multiplier = 2, ultimate = true }
	)
end

modifier_kunkka_admirals_rum_custom = class({})
function modifier_kunkka_admirals_rum_custom:IsHidden()
	return true
end
function modifier_kunkka_admirals_rum_custom:IsPurgable()
	return false
end
function modifier_kunkka_admirals_rum_custom:IsPurgeException()
	return false
end
function modifier_kunkka_admirals_rum_custom:RemoveOnDeath()
	return false
end
...
function modifier_kunkka_admirals_rum_custom:OnTakeDamage(params)
	...
	if self:GetParent():GetHealthPercent() > self.damage_threshold then
		return
	end
	if self:GetParent():HasModifier("modifier_kunkka_admirals_rum_custom_buff") then
		return
	end
...
function modifier_kunkka_admirals_rum_custom_buff:IsPurgeException()
	return false
end
function modifier_kunkka_admirals_rum_custom_buff:IsHidden()
	return false
end
function modifier_kunkka_admirals_rum_custom_buff:IsPurgable()
	return false
end
function modifier_kunkka_admirals_rum_custom_buff:IsDebuff()
	return false
end
...
function modifier_kunkka_admirals_rum_custom_buff:OnDestroy()
	...
	self:GetParent():AddNewModifier(
		...
		ability,
		"modifier_kunkka_admirals_rum_custom_debuff",
		{ duration = ability:GetSpecialValueFor("buff_duration"), stored_damage = self.damage_counter }
	)
	self.damage_counter = 0
end

modifier_kunkka_admirals_rum_custom_debuff = class({})
function modifier_kunkka_admirals_rum_custom_debuff:IsPurgable()
	return false
end
function modifier_kunkka_admirals_rum_custom_debuff:IsPurgeException()
	return false
end
function modifier_kunkka_admirals_rum_custom_debuff:GetAttributes()
