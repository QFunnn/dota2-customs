...
LinkLuaModifier(
	"modifier_kunkka_tidal_wave_custom",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_tidal_wave_custom",
		...
		if not target:IsDebuffImmune() then
			target:AddNewModifier(
				...
				"modifier_kunkka_tidal_wave_custom",
				{
					...
					height = 0,
					direction_x = ExtraData.x,
					direction_y = ExtraData.y,
				}
			)
		end
		...
		if self:GetCaster():HasModifier("modifier_kunkka_19") then
			damage = damage
				+ (
					...
					/ 100
					* self.modifier_kunkka_19[self:GetCaster():GetTalentLevel("modifier_kunkka_19")]
				)
		end
		ApplyDamage({
			victim = target,
