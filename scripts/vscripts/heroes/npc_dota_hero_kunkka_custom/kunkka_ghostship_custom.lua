...
LinkLuaModifier(
	"modifier_kunkka_ghostship_custom_damage_tick",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_ghostship_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kunkka_ghostship_custom_magic_immune",
	"heroes/npc_dota_hero_kunkka_custom/kunkka_ghostship_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kunkka_ghostship_custom:OnProjectileThink_ExtraData(vLocation, data)
	...
	if data.index and data.is_boat then
		...
		if not caster:HasModifier("modifier_kunkka_20") then
			return
		end
		...
		if projectile.delay and data.cannonade_interval < GameRules:GetGameTime() - projectile.delay then
			...
			for cannonRank = 1, cannonCount do
				for sideIndex = 1, 2 do
					...
					local cannonBallPosition = projPosition
						+ boatDirection * forwardOffset
						+ cannonDirection * cannonSide * cannonRadius
					local cannon_proj = {
						Ability = self,
						EffectName = pfx,
...
function kunkka_ghostship_custom:StartShip(
	parti,
	spawn_pos,
	start_distance,
	crash_distance,
		...
		if caster:GetTeam() == target:GetTeam() and ExtraData.is_boat then
			...
			if self:GetCaster():HasModifier("modifier_kunkka_13") then
				target:AddNewModifier(
					...
					self,
					"modifier_kunkka_ghostship_custom_magic_immune",
					{ duration = self.modifier_kunkka_13[self:GetCaster():GetTalentLevel("modifier_kunkka_13")] }
				)
			end
		end
		if ExtraData.is_cannon then
			if not self[ExtraData.index][target:entindex()] then
				...
				ApplyDamage({
					...
					ability = self,
					damage = self.modifier_kunkka_20 / 100 * damage,
...
function modifier_kunkka_ghostship_custom_damage_tick:IsHidden()
	return true
end
function modifier_kunkka_ghostship_custom_damage_tick:IsPurgable()
	return false
end
function modifier_kunkka_ghostship_custom_damage_tick:IsPurgeException()
	return false
end
