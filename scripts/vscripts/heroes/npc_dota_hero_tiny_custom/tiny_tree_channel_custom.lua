...
LinkLuaModifier(
	"modifier_tiny_tree_channel_custom",
	"heroes/npc_dota_hero_tiny_custom/tiny_tree_channel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tiny_tree_grab_custom_debuff",
		...
		if enemy ~= nil then
			...
			if tiny_toss_tree_custom and tiny_toss_tree_custom:GetLevel() > 0 then
				enemy:AddNewModifier(
					self:GetCaster(),
					tiny_toss_tree_custom,
					"modifier_tiny_tree_grab_custom_debuff",
					{
						duration = tiny_toss_tree_custom:GetSpecialValueFor("slow_duration")
...
function modifier_tiny_tree_channel_custom:IsHidden()
	return true
end
function modifier_tiny_tree_channel_custom:IsPurgable()
	return false
end
function modifier_tiny_tree_channel_custom:IsPurgeException()
	return false
end
...
function modifier_tiny_tree_channel_custom:OnCreated()
	...
	local target_particle = ParticleManager:CreateParticleForTeam(
		"particles/units/heroes/hero_tiny/tiny_tree_channel.vpcf",
		PATTACH_WORLDORIGIN,
		nil,
		self:GetCaster():GetTeamNumber()
