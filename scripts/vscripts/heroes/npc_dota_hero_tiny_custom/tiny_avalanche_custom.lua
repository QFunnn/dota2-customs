...
LinkLuaModifier(
	"modifier_tiny_avalanche_custom_handler",
	"heroes/npc_dota_hero_tiny_custom/tiny_avalanche_custom",
...
function tiny_avalanche_custom:OnProjectileHit_ExtraData(hTarget, vLocation, data)
	...
	if self:GetCaster():HasModifier("modifier_tiny_4") then
		avalanch_damage = avalanch_damage
			+ (
				self:GetCaster():GetMaxHealth()
				/ 100
				* self.modifier_tiny_4[self:GetCaster():GetTalentLevel("modifier_tiny_4")]
			)
	end
			...
			for _, enemy in pairs(enemies_tick) do
				...
				if enemy:IsAlive() and not heroes[enemy] then
					...
					enemy:AddNewModifier(
						...
						ability,
						"modifier_stunned",
						{ duration = data.tick_count * stun_duration }
					)
				end
			end
		end
...
function modifier_tiny_avalanche_custom_handler:IsHidden()
	return true
end
function modifier_tiny_avalanche_custom_handler:IsPurgable()
	return false
end
function modifier_tiny_avalanche_custom_handler:IsPurgeException()
	return false
end
function modifier_tiny_avalanche_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_tiny_avalanche_custom_handler:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_tiny_13") then
		return
	end
	...
	if self.distance >= self:GetAbility().modifier_tiny_13[self:GetCaster():GetTalentLevel("modifier_tiny_13")] then
		self.distance = math.max(
			0,
			self.distance - self:GetAbility().modifier_tiny_13[self:GetCaster():GetTalentLevel("modifier_tiny_13")]
		)
