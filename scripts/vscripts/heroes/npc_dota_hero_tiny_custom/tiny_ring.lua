...
function modifier_tiny_ring:IsHidden()
	return true
end
function modifier_tiny_ring:IsPurgable()
	return false
end
function modifier_tiny_ring:IsPurgeException()
	return false
end
function modifier_tiny_ring:RemoveOnDeath()
	return false
end
...
function modifier_tiny_ring:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_tiny_10") then
		AddFOWViewer(
			...
			self:GetAbility().modifier_tiny_10_vision[self:GetCaster():GetTalentLevel("modifier_tiny_10")],
			FrameTime(),
			false
		)
	end
	...
	if self.damage_think >= 0.5 then
		self.damage_think = 0
		local damage_agility = self:GetCaster():GetAgility()
			/ 100
			* self:GetAbility():GetSpecialValueFor("damage_agility")
		...
		if self:GetParent():IsAlive() then
			...
			local effect_cast = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_sandking/sandking_epicenter_tiny.vpcf",
