...
LinkLuaModifier(
	"modifier_tiny_tree_grab_custom",
	"heroes/npc_dota_hero_tiny_custom/tiny_tree_grab_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tiny_toss_tree_custom_damage",
	"heroes/npc_dota_hero_tiny_custom/tiny_tree_grab_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tiny_toss_custom",
	"heroes/npc_dota_hero_tiny_custom/tiny_toss_custom",
...
function modifier_tiny_tree_grab_custom:IsPurgable()
	return false
end
function modifier_tiny_tree_grab_custom:IsPurgeException()
	return false
end
...
function modifier_tiny_tree_grab_custom:SpawnItems()
	...
	self.tree =
		SpawnEntityFromTableSynchronous("prop_dynamic", { model = "models/heroes/tiny/tiny_tree/tiny_tree.vmdl" })
...
function modifier_tiny_tree_grab_custom:GetModifierProcAttack_Feedback(params)
	...
	if parent:GetTeam() == params.target:GetTeam() then
		return
	end
	...
	local enemies = FindUnitsInLine(
		...
		params.target:GetAbsOrigin() + direction * self.splash_range,
		nil,
		self.splash_width,
...
function tiny_toss_tree_custom:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	if extra_data and extra_data.index and self[extra_data.index] then
		return
	end
		...
		if self:GetCaster():HasModifier("modifier_tiny_19") then
			if #enemies > 0 then
				...
				if tiny_toss_custom and tiny_toss_custom:GetLevel() > 0 and not enemies[1]:IsDebuffImmune() then
					...
					enemies[1]:AddNewModifier(
						self:GetCaster(),
						tiny_toss_custom,
						"modifier_tiny_toss_custom",
						{ point_x = enemies[1]:GetAbsOrigin().x, point_y = enemies[1]:GetAbsOrigin().y, use = true }
					)
				end
			end
		end
			...
			if #enemies > 0 then
				...
				if tiny_toss_custom and tiny_toss_custom:GetLevel() > 0 and not target:IsDebuffImmune() then
					...
					target:AddNewModifier(
						...
						tiny_toss_custom,
						"modifier_tiny_toss_custom",
						{ point_x = target:GetAbsOrigin().x, point_y = target:GetAbsOrigin().y, use = true }
					)
				end
			end
		end
		local cleave_p = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_tiny/tiny_craggy_cleave.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
...
function modifier_tiny_toss_tree_custom_damage:IsHidden()
	return true
end
function modifier_tiny_toss_tree_custom_damage:IsPurgable()
	return false
end
function modifier_tiny_toss_tree_custom_damage:IsPurgeException()
	return false
end
function modifier_tiny_toss_tree_custom_damage:RemoveOnDeath()
	return false
end
...
function modifier_tiny_toss_tree_custom_damage:GetModifierAvoidDamage(params)
	if self:GetParent():GetUnitName() == "npc_dota_thinker" then
		return 1
	end
end
