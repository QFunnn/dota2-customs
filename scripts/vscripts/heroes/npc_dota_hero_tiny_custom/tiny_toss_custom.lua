...
LinkLuaModifier(
	"modifier_tiny_toss_custom_debuff",
	"heroes/npc_dota_hero_tiny_custom/tiny_toss_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tiny_toss_custom_magic_immune",
	"heroes/npc_dota_hero_tiny_custom/tiny_toss_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function tiny_toss_custom:FindEnemies()
	...
	for _, unit in pairs(units) do
		local filter1 = (unit ~= caster)
			and (not unit:IsAncient())
			and (not unit:FindModifierByName("modifier_tiny_toss_custom"))
...
function modifier_tiny_toss_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_tiny_15") then
		self.damage = self.damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_tiny_15[self:GetCaster():GetTalentLevel("modifier_tiny_15")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_tiny_16") then
		if self:GetParent() == self:GetCaster() then
			self:GetParent()
				:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_tiny_toss_custom_magic_immune", {})
		end
	end
	local height = 850
		...
		for _, enemy in pairs(enemies) do
			...
			if self:GetCaster():HasModifier("modifier_tiny_20") then
				enemy:AddNewModifier(
					...
					self:GetAbility(),
					"modifier_tiny_toss_custom_debuff",
					{ duration = self:GetAbility().modifier_tiny_20_duration }
				)
			end
		end
...
function modifier_tiny_toss_custom_magic_immune:IsPurgable()
	return false
end
function modifier_tiny_toss_custom_magic_immune:GetTexture()
	return "tiny_16"
end
