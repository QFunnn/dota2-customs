...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_savage_roar_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_savage_roar_custom_lone_druid_13",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_savage_roar_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_savage_roar_custom_lone_druid_15_thinker",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_savage_roar_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_savage_roar_custom_lone_druid_15",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_savage_roar_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lone_druid_savage_roar_custom:OnSpellStart(check)
	...
	if self:GetCaster():HasModifier("modifier_lone_druid_13") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_lone_druid_savage_roar_custom_lone_druid_13",
			{ duration = self.modifier_lone_druid_13_duration }
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_lone_druid_15") then
		for i = 0, 6 do
			...
			CreateModifierThinker(
				...
				self,
				"modifier_lone_druid_savage_roar_custom_lone_druid_15_thinker",
				{ duration = self.modifier_lone_druid_15_duration },
				point + offset,
				self:GetCaster():GetTeamNumber(),
				false
			)
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_lone_druid_14") then
			local damage = self:GetCaster():GetAgility()
				/ 100
				* self.modifier_lone_druid_14_damage[self:GetCaster():GetTalentLevel("modifier_lone_druid_14")]
			ApplyDamage({
				victim = enemy,
...
function modifier_lone_druid_savage_roar_custom:GetModifierMoveSpeedBonus_Percentage()
	if self:GetParent():HasModifier("modifier_lone_druid_savage_roar_custom_lone_druid_15") then
		return 0
	end
...
function modifier_lone_druid_savage_roar_custom_lone_druid_15:GetTexture()
	return "lone_druid_15"
end