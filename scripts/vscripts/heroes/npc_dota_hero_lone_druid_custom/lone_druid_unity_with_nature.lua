...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_unity_with_nature",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_unity_with_nature_bear",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_unity_with_nature",
	LUA_MODIFIER_MOTION_NONE
)
...
function lone_druid_unity_with_nature:OnSpellStart()
	...
	local target = self:GetCursorTarget()
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_lone_druid_unity_with_nature", { bear = target:entindex() })
end

modifier_lone_druid_unity_with_nature = class({})

function modifier_lone_druid_unity_with_nature:IsPurgable()
	return false
end
...
function modifier_lone_druid_unity_with_nature:OnCreated(kv)
	...
	local particle =
		ParticleManager:CreateParticle("particles/items3_fx/blink_swift_end.vpcf", PATTACH_WORLDORIGIN, nil)
	ParticleManager:SetParticleControl(particle, 0, self:GetParent():GetAbsOrigin())
end
