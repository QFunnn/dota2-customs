...
LinkLuaModifier(
	"modifier_lone_druid_moonlight_debuff",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_moonlight",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_moonlight_debuff_stack",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_moonlight",
	LUA_MODIFIER_MOTION_NONE
)
...
function lone_druid_moonlight:GetCooldown(level)
	local bonus = 1
	if
		self:GetCaster():HasModifier("modifier_lone_druid_true_form_custom")
		and self:GetCaster():HasModifier("modifier_lone_druid_19")
	then
		bonus = 2
	end
	...
	Timers:CreateTimer(0.2, function()
		...
		if self:GetCaster():HasModifier("modifier_lone_druid_18") then
			target:AddNewModifier(
				...
				"modifier_lone_druid_moonlight_debuff_stack",
				{
					duration = self.modifier_lone_druid_18_duration[self:GetCaster()
						:GetTalentLevel("modifier_lone_druid_18")],
				}
			)
			target:AddNewModifier(
				self:GetCaster(),
...
function modifier_lone_druid_moonlight_debuff:GetEffectName()
	return "particles/witch_blade_debuff_52.vpcf"
end

function modifier_lone_druid_moonlight_debuff:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

modifier_lone_druid_moonlight_debuff_stack = class({})
function modifier_lone_druid_moonlight_debuff_stack:IsHidden()
	return true
end
function modifier_lone_druid_moonlight_debuff_stack:GetAttributes()
