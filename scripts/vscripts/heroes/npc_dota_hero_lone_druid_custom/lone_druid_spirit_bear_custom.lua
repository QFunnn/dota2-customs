...
LinkLuaModifier(
	"modifier_lone_druid_spirit_bear_custom",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_spirit_bear_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lone_druid_spirit_bear_custom_disarmed",
	"heroes/npc_dota_hero_lone_druid_custom/lone_druid_spirit_bear_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lone_druid_spirit_bear_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_lone_druid.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_lone_druid.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_lone_druid.vpcf",
		context
	)
end
...
function lone_druid_spirit_bear_custom:OnSpellStart()
	...
	for _, unit in
		pairs(
			FindUnitsInRadius(
				...
				DOTA_UNIT_TARGET_TEAM_FRIENDLY,
				DOTA_UNIT_TARGET_ALL,
				DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED + DOTA_UNIT_TARGET_FLAG_INVULNERABLE,
				FIND_ANY_ORDER,
				false
			)
		)
	...
	for bears = 1, bear_count do
		...
		local end_particle = ParticleManager:CreateParticle(
...
function modifier_lone_druid_spirit_bear_custom:IsPurgeException()
	return false
end
function modifier_lone_druid_spirit_bear_custom:IsHidden()
	return false
end
function modifier_lone_druid_spirit_bear_custom:IsPurgable()
	return false
end
function modifier_lone_druid_spirit_bear_custom:RemoveOnDeath()
	return false
end
function modifier_lone_druid_spirit_bear_custom:IsPermanent()
	return true
end
...
function modifier_lone_druid_spirit_bear_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_lone_druid_2") then
		self.bonus_damage_talent =
			self:GetAbility().modifier_lone_druid_2_bonus_dmg[self:GetCaster():GetTalentLevel("modifier_lone_druid_2")]
		self.movespeed_talent =
			self:GetAbility().modifier_lone_druid_2_bonus_dmg[self:GetCaster():GetTalentLevel("modifier_lone_druid_2")]
	end

	if self:GetCaster():HasModifier("modifier_lone_druid_3") then
		self.armor_talent = self:GetAbility().modifier_lone_druid_3_armor[self:GetCaster()
			:GetTalentLevel("modifier_lone_druid_3")]
	end

	if self:GetCaster():HasModifier("modifier_lone_druid_5") then
		self.health_talent = self:GetCaster():GetStrength()
			* self:GetAbility().modifier_lone_druid_5_health[self:GetCaster():GetTalentLevel("modifier_lone_druid_5")]
	end

	if self:GetCaster():HasModifier("modifier_lone_druid_6") then
		self.bonus_damage_talent = self.bonus_damage_talent
			+ (
				self:GetCaster():GetStrength()
				* self:GetAbility().modifier_lone_druid_6_damage[self:GetCaster()
					:GetTalentLevel("modifier_lone_druid_6")]
			)
	end

	if self:GetCaster():HasModifier("modifier_lone_druid_7") then
		self.attack_speed_talent = self.attack_speed_talent
			+ (
				self:GetCaster():GetStrength()
				* self:GetAbility().modifier_lone_druid_7_attack_speed[self:GetCaster()
					:GetTalentLevel("modifier_lone_druid_7")]
			)
	end
...
function modifier_lone_druid_spirit_bear_custom:OnIntervalThink()
	...
	if distance < self:GetAbility():GetSpecialValueFor("distance_silence") then
		...
		self:GetParent()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_lone_druid_spirit_bear_custom_disarmed", {})
	end
...
function modifier_lone_druid_spirit_bear_custom:CheckState()
	if not self:GetCaster():HasModifier("modifier_lone_druid_3") then
		return
	end
	return {
...
function modifier_lone_druid_spirit_bear_custom:OnDeath(params)
	...
	if params.attacker and params.attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber() then
		...
		ApplyDamage({
			...
			damage = self:GetParent():GetMaxHealth() / 100 * backlash_damage,
...
function modifier_lone_druid_spirit_bear_custom_disarmed:IsHidden()
	return true
end
function modifier_lone_druid_spirit_bear_custom_disarmed:IsPurgable()
	return false
end
