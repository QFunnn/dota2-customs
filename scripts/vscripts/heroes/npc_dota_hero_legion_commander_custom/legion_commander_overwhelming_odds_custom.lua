...
LinkLuaModifier(
	"modifier_legion_commander_overwhelming_odds_custom_buff",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_overwhelming_odds_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_overwhelming_odds_custom_debuff",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_overwhelming_odds_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_overwhelming_odds_custom_barrier",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_overwhelming_odds_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function legion_commander_overwhelming_odds_custom:OnSpellStart(point_new, check)
	...
	if
		self:GetCaster():HasModifier("modifier_legion_commander_14")
		and not self:GetCaster():HasModifier("modifier_legion_commander_duel_custom")
	then
	...
	local enemies_creeps = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		point,
		nil,
		radius,
	...
	local enemies_heroes = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		point,
		nil,
		radius,
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_9") then
		...
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_legion_commander_9[self:GetCaster():GetTalentLevel("modifier_legion_commander_9")]
			)
	end
	...
	for _, enemy in ipairs(enemies) do
		...
		if enemy:IsIllusion() then
			ApplyDamage({
				victim = enemy,
				damage = full_damage + (enemy:GetMaxHealth() / 100 * illusion_dmg_pct),
				damage_type = damage_type,
				attacker = self:GetCaster(),
				ability = self,
			})
		else
			local end_damage_line = ApplyDamage({
				victim = enemy,
				damage = full_damage,
				damage_type = damage_type,
				attacker = self:GetCaster(),
	...
	local buff = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_legion_commander_overwhelming_odds_custom_buff",
		{ duration = duration }
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_13") then
		...
		if heroes_damage_to_barrier > 0 then
			local barrier = heroes_damage_to_barrier
				/ 100
				* self.modifier_legion_commander_13[self:GetCaster():GetTalentLevel("modifier_legion_commander_13")]
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_legion_commander_overwhelming_odds_custom_barrier",
				{ duration = shield_duration, barrier = barrier }
			)
		end
	end
...
function modifier_legion_commander_overwhelming_odds_custom_buff:IsHidden()
	return false
end
function modifier_legion_commander_overwhelming_odds_custom_buff:IsPurgable()
	return true
end
...
function modifier_legion_commander_overwhelming_odds_custom_buff:OnCreated(table)
	...
	self.poof = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_legion_commander/legion_commander_odds_buff.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetParent()
	)
