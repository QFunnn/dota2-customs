...
LinkLuaModifier(
	"modifier_legion_commander_moment_of_courage_custom",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_moment_of_courage_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_moment_of_courage_custom_speed",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_moment_of_courage_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_moment_of_courage_custom_heal",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_moment_of_courage_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_moment_of_courage_custom_heal_mark",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_moment_of_courage_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_moment_of_courage_custom_no_trigger",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_moment_of_courage_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_legion_commander_moment_of_courage_custom:IsPurgable()
	return false
end
...
function modifier_legion_commander_moment_of_courage_custom:GetModifierProperty_PhysicalLifesteal(params)
	...
	if params.target:HasModifier("modifier_legion_commander_moment_of_courage_custom_heal_mark") then
		...
		if self:GetCaster():HasModifier("modifier_legion_commander_3") then
			bonus = self:GetAbility().modifier_legion_commander_3[self:GetCaster()
				:GetTalentLevel("modifier_legion_commander_3")]
		end
...
function modifier_legion_commander_moment_of_courage_custom:OnAttackRecord(params)
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_5") then
		if params.attacker == self:GetParent() then
			...
			if self:GetParent():HasModifier("modifier_legion_commander_moment_of_courage_custom_no_trigger") then
				return
			end
			if self:GetParent():PassivesDisabled() then
				return
...
function modifier_legion_commander_moment_of_courage_custom_speed:IsHidden()
	return true
end
function modifier_legion_commander_moment_of_courage_custom_speed:IsPurgable()
	return false
end
...
function modifier_legion_commander_moment_of_courage_custom_speed:OnIntervalThink()
	...
	if not self:GetParent():GetAttackTarget() then
		return
	end
	...
	ParticleManager:SetParticleControlForward(
		particle,
		5,
		(target:GetOrigin() - self:GetParent():GetOrigin()):Normalized()
	)

	local no_trigger = self:GetParent():AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_legion_commander_moment_of_courage_custom_no_trigger",
		{}
	...
	local modifier_legion_commander_moment_of_courage_custom =
		self:GetParent():FindModifierByName("modifier_legion_commander_moment_of_courage_custom")
	...
	modifier_legion_commander_moment_of_courage_custom:SetStackCount(
		modifier_legion_commander_moment_of_courage_custom.trigger_attacks
	)
...
function modifier_legion_commander_moment_of_courage_custom_heal:IsHidden()
	return true
end
function modifier_legion_commander_moment_of_courage_custom_heal:IsPurgable()
	return false
end
...
function modifier_legion_commander_moment_of_courage_custom_heal_mark:IsHidden()
	return true
end
function modifier_legion_commander_moment_of_courage_custom_heal_mark:IsPurgable()
	return false
end
function modifier_legion_commander_moment_of_courage_custom_heal_mark:RemoveOnDeath()
	return false
end

modifier_legion_commander_moment_of_courage_custom_no_trigger = class({})
function modifier_legion_commander_moment_of_courage_custom_no_trigger:IsHidden()
	return true
end
function modifier_legion_commander_moment_of_courage_custom_no_trigger:IsPurgable()
	return false
end