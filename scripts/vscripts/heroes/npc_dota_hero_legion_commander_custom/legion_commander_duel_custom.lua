...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_duel_custom",
...
LinkLuaModifier(
	"modifier_legion_commander_duel_custom_creep",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_duel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_duel_custom_thinker_magic",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_duel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_duel_custom_thinker_magic_debuff",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_duel_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function legion_commander_duel_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_legion_commander.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_legion_commander.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_legion_commander.vpcf",
		context
	)
end
...
function legion_commander_duel_custom:StartDuel(target)
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_6") then
		duration = duration
			+ self.modifier_legion_commander_6[self:GetCaster():GetTalentLevel("modifier_legion_commander_6")]
	end
	...
	target:AddNewModifier(
		...
		self,
		"modifier_legion_commander_duel_custom",
		{ duration = duration, target = self.caster:entindex() }
	)
end
...
function legion_commander_duel_custom:WinArena()
	...
	if not mod then
		mod = self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_legion_commander_duel_custom_damage", {})
	end
...
function legion_commander_duel_custom:WinDuel(caster, winner, loser)
	...
	if winner == caster then
		local legion_commander_press_the_attack_custom =
			winner:FindAbilityByName("legion_commander_press_the_attack_custom")
		if legion_commander_press_the_attack_custom then
			legion_commander_press_the_attack_custom:OnSpellStartCustom()
		end
	end

	local duel_victory_particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_legion_commander/legion_commander_duel_victory.vpcf",
...
function legion_commander_duel_custom:TalentOds(check)
	local heroes = FindUnitsInRadius(
		...
		nil,
		self.modifier_legion_commander_radius,
	...
	if new_target ~= nil then
		local legion_commander_overwhelming_odds_custom =
			self:GetCaster():FindAbilityByName("legion_commander_overwhelming_odds_custom")
...
function modifier_legion_commander_duel_custom:IsHidden()
	return false
end
function modifier_legion_commander_duel_custom:IsPurgable()
	return false
end
function modifier_legion_commander_duel_custom:IsPurgeException()
	return false
end
function modifier_legion_commander_duel_custom:IsDebuff()
	return true
end
function modifier_legion_commander_duel_custom:GetAttributes()
...
function modifier_legion_commander_duel_custom:OnCreated(table)
	...
	local legion_commander_press_the_attack_custom = self:GetParent()
		:FindAbilityByName("legion_commander_press_the_attack_custom")
	if legion_commander_press_the_attack_custom then
		legion_commander_press_the_attack_custom:SetActivated(false)
	end
	local legion_commander_moment_of_courage_custom = self:GetParent()
		:FindAbilityByName("legion_commander_moment_of_courage_custom")
	...
	if self:GetCaster() == self:GetParent() then
		local center_point = self:GetCaster():GetAbsOrigin()
			+ ((self.target:GetAbsOrigin() - self:GetCaster():GetAbsOrigin()) / 2)
		...
		if self:GetCaster():HasModifier("modifier_legion_commander_21") then
			...
			self.magic_zone = CreateModifierThinker(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_legion_commander_duel_custom_thinker_magic",
				{},
...
function modifier_legion_commander_duel_custom:OnIntervalThink()
	...
	if
		(self:GetParent():GetAbsOrigin() - self.target:GetAbsOrigin()):Length2D()
			> self:GetAbility():GetSpecialValueFor("victory_range")
		or not self.target:HasModifier(self:GetName())
	then
		self:Destroy()
	end
end
...
function modifier_legion_commander_duel_custom_damage:IsHidden()
	return false
end
function modifier_legion_commander_duel_custom_damage:IsPurgable()
	return false
end
function modifier_legion_commander_duel_custom_damage:IsPurgeException()
	return false
end
function modifier_legion_commander_duel_custom_damage:RemoveOnDeath()
	return false
end
function modifier_legion_commander_duel_custom_damage:IsDebuff()
	return false
end
function modifier_legion_commander_duel_custom_damage:GetTexture()
	return "legion_commander_duel"
end
...
function modifier_legion_commander_duel_custom_damage:OnIntervalThink()
	...
	if self:GetParent():HasModifier("modifier_legion_commander_4") then
		bonus = self.duel_wins
			* self:GetAbility().modifier_legion_commander_4[self:GetCaster()
				:GetTalentLevel("modifier_legion_commander_4")]
	end
...
function modifier_legion_commander_duel_custom_damage:GetModifierSpellAmplify_Percentage()
	if self:GetParent():HasModifier("modifier_legion_commander_21") then
		return (self:GetStackCount() / self:GetAbility().modifier_legion_commander_21_per)
			* self:GetAbility().modifier_legion_commander_21_bonus
	end
end

modifier_legion_commander_duel_custom_creep = class({})
function modifier_legion_commander_duel_custom_creep:IsPurgable()
	return false
end
function modifier_legion_commander_duel_custom_creep:RemoveOnDeath()
	return false
end
function modifier_legion_commander_duel_custom_creep:IsPurgeException()
	return false
end
function modifier_legion_commander_duel_custom_creep:IsHidden()
	return true
end
...
function modifier_legion_commander_duel_custom_creep:OnDeath(params)
	...
	if not self:GetParent():HasModifier("modifier_legion_commander_2") then
		return
	end
	...
	if
		self:GetStackCount()
		>= self:GetAbility().modifier_legion_commander_2[self:GetCaster():GetTalentLevel("modifier_legion_commander_2")]
	then
		local modifier_legion_commander_duel_custom_damage =
			self:GetParent():FindModifierByName("modifier_legion_commander_duel_custom_damage")
		if modifier_legion_commander_duel_custom_damage then
			modifier_legion_commander_duel_custom_damage.damage = modifier_legion_commander_duel_custom_damage.damage
				+ 1
		else
			modifier_legion_commander_duel_custom_damage = self:GetParent():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_legion_commander_duel_custom_damage",
				{}
		...
		self:SetStackCount(
			math.max(
				0,
				self:GetStackCount()
					- self:GetAbility().modifier_legion_commander_2[self:GetCaster()
						:GetTalentLevel("modifier_legion_commander_2")]
			)
		)
	end
end
...
function modifier_legion_commander_duel_custom_thinker_magic:IsAura()
	return true
end
function modifier_legion_commander_duel_custom_thinker_magic:GetAuraRadius()
	return self:GetCaster():GetAoeBonus(self:GetAbility().modifier_legion_commander_21_radius)
end
function modifier_legion_commander_duel_custom_thinker_magic:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end
function modifier_legion_commander_duel_custom_thinker_magic:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_legion_commander_duel_custom_thinker_magic:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_legion_commander_duel_custom_thinker_magic:GetModifierAura()
	return "modifier_legion_commander_duel_custom_thinker_magic_debuff"
end
...
function modifier_legion_commander_duel_custom_thinker_magic_debuff:IsPurgable()
	return false
end
...
function modifier_legion_commander_duel_custom_thinker_magic_debuff:OnIntervalThink()
	...
	local damage = self:GetCaster():GetAverageTrueAttackDamage(nil)
		/ 100
		* self:GetAbility().modifier_legion_commander_21_damage
	ApplyDamage({
