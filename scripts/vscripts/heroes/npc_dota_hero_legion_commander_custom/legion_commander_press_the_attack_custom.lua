...
LinkLuaModifier(
	"modifier_legion_commander_press_the_attack_custom_buff",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_press_the_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_legion_commander_press_the_attack_custom_magic_immune",
	"heroes/npc_dota_hero_legion_commander_custom/legion_commander_press_the_attack_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function legion_commander_press_the_attack_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_18") then
		local units = FindUnitsInRadius(
			...
			point,
			nil,
			self.modifier_legion_commander_18_radius,
...
function legion_commander_press_the_attack_custom:OnTarget(target)
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_15") then
		duration = duration
			+ self.modifier_legion_commander_15[self:GetCaster():GetTalentLevel("modifier_legion_commander_15")]
	end
...
function modifier_legion_commander_press_the_attack_custom_buff:OnCreated(table)
	...
	if self:GetCaster():HasModifier("modifier_legion_commander_18") then
		self.move_speed = self.move_speed
			+ self:GetAbility().modifier_legion_commander_18[self:GetCaster()
				:GetTalentLevel("modifier_legion_commander_18")]
	end
	if not IsServer() then
		return
	end
...
function modifier_legion_commander_press_the_attack_custom_buff:GetModifierSpellAmplify_Percentage()
	if self:GetCaster():HasModifier("modifier_legion_commander_20") then
		return self:GetAbility().modifier_legion_commander_20[self:GetCaster()
			:GetTalentLevel("modifier_legion_commander_20")]
	end
end

modifier_legion_commander_press_the_attack_custom_magic_immune = class({})

function modifier_legion_commander_press_the_attack_custom_magic_immune:IsPurgable()
	return false
end
