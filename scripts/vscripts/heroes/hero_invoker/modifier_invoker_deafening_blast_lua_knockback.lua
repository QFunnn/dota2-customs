...
function modifier_invoker_deafening_blast_lua_knockback:OnCreated(kv)
    if IsServer() then
...
function modifier_invoker_deafening_blast_lua_knockback:OnHorizontalMotionInterrupted()
    self:Destroy()
end

function modifier_invoker_deafening_blast_lua_knockback:UpdateHorizontalMotion(me, dt)
    local new_pos = me:GetAbsOrigin() + self.direction * 3
