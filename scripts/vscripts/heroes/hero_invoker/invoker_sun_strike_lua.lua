...
LinkLuaModifier(
	...
	"heroes/hero_invoker/modifier_invoker_sun_strike_lua_thinker",
	...
	if self.bCataclysm then
		...
		if units and #units > 0 then
			...
			for _, unit in pairs(units) do
				...
				self:CastSunStrikeAtPoint(
					pos + RandomVector(RandomFloat(cataclysm_minimum_distance, cataclysm_maximum_distance))
				)
				self:CastSunStrikeAtPoint(
					pos + RandomVector(RandomFloat(cataclysm_minimum_distance, cataclysm_maximum_distance))
				)
