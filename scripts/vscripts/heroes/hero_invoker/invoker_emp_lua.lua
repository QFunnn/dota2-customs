...
function modifier_invoker_emp_lua_thinker:OnCreated(kv)
	...
	self.damage_per_mana_pct = self.has_shard and self:GetAbilitySpecialValueFor("damage_per_mana_pct_shard")
		or self:GetAbilitySpecialValueFor("damage_per_mana_pct")
	...
	if IsValid(hParent) then
		...
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				...
				local fDistance = math.min(
					self.drag_speed_shard * FrameTime(),
					(hParent:GetAbsOrigin() - unit:GetAbsOrigin()):Length2D()
				)
...
function modifier_invoker_emp_lua_drag:OnIntervalThink()
	...
	if IsValid(hParent) and IsValid(hThinker) and not hParent:HasState(MODIFIER_STATE_DEBUFF_IMMUNE) then
		...
		local fDistance =
			math.min(self.drag_speed_shard * FrameTime(), (hThinker:GetAbsOrigin() - hParent:GetAbsOrigin()):Length2D())
