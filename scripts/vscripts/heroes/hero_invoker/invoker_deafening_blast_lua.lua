...
LinkLuaModifier(
	"modifier_invoker_deafening_blast_lua_knockback",
	"heroes/hero_invoker/modifier_invoker_deafening_blast_lua_knockback",
...
function invoker_deafening_blast_lua:OnProjectileHit_ExtraData(hTarget, vLocation, tExtradata)
	...
	hTarget:AddNewModifier(
		...
		{ duration = self.disarm_duration * hTarget:GetStatusResistanceFactor(hCaster) }
	)
	hTarget:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_invoker_deafening_blast_lua_knockback",
		{
			duration = self.knockback_duration * hTarget:GetStatusResistanceFactor(hCaster),
			x = dir.x,
			y = dir.y,
			z = dir.z,
		}
