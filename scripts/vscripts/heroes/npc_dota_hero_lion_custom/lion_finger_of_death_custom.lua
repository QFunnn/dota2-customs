...
LinkLuaModifier(
	"modifier_lion_finger_of_death_custom",
	"heroes/npc_dota_hero_lion_custom/lion_finger_of_death_custom",
...
LinkLuaModifier(
	"modifier_lion_finger_of_death_custom_passive",
	"heroes/npc_dota_hero_lion_custom/lion_finger_of_death_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lion_finger_of_death_custom_zapret",
	"heroes/npc_dota_hero_lion_custom/lion_finger_of_death_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lion_finger_of_death_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_lion.vpcf",
		context
	)
end
...
function lion_finger_of_death_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_lion_17") then
		local targets = FindUnitsInRadius(
			...
			target:GetAbsOrigin(),
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_lion_17),
	...
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_lion_finger_punch",
		{ duration = self:GetSpecialValueFor("punch_duration") }
	)
end
...
function lion_finger_of_death_custom:DamageParticle(target)
	if not self:GetCaster():HasModifier("modifier_lion_14") then
		...
		local particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_lion/lion_spell_finger_of_death_woda.vpcf",
		...
		ParticleManager:SetParticleControl(
			iParticleID,
			6,
			vStart + vDirection * flDistance * 0.7 + RandomVector(RandomInt(flDistance * 0.2, flDistance * 0.25))
		)
		ParticleManager:SetParticleControl(
			iParticleID,
			10,
			vStart + vDirection * flDistance * 0.3 + RandomVector(RandomInt(flDistance * 0.2, flDistance * 0.25))
		)
...
function modifier_lion_finger_of_death_custom_passive:IsPurgable()
	return false
end
function modifier_lion_finger_of_death_custom_passive:RemoveOnDeath()
	return false
end
...
function modifier_lion_finger_of_death_custom_passive:GetModifierHealthBonus()
	if self:GetCaster():HasModifier("modifier_lion_3") then
		return self:GetStackCount()
			* self:GetAbility().modifier_lion_3[self:GetCaster():GetTalentLevel("modifier_lion_3")]
	end
	return 0
end
...
function modifier_lion_finger_of_death_custom_passive:GetModifierBonusStats_Agility()
	if self:GetCaster():HasModifier("modifier_lion_10") then
		return self:GetStackCount()
			* self:GetAbility().modifier_lion_10[self:GetCaster():GetTalentLevel("modifier_lion_10")]
	end
	return 0
end
...
function modifier_lion_finger_of_death_custom_buff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_lion_finger_of_death_custom_buff:IsHidden()
	return true
end
function modifier_lion_finger_of_death_custom_buff:IsPurgable()
	return false
end
...
function modifier_lion_finger_of_death_custom:OnCreated(kv)
	...
	if bonus_damage_modifier then
		bonus_damage = bonus_damage
			+ (self:GetAbility():GetSpecialValueFor("damage_per_kill") * bonus_damage_modifier:GetStackCount())
	end
...
function modifier_lion_finger_of_death_custom_zapret:IsHidden()
	return true
end
function modifier_lion_finger_of_death_custom_zapret:IsPurgable()
	return false
end
function modifier_lion_finger_of_death_custom_zapret:IsPurgeException()
	return false
end
function modifier_lion_finger_of_death_custom_zapret:RemoveOnDeath()
	return false
end