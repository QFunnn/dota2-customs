...
LinkLuaModifier(
	"modifier_lion_mana_drain_custom",
	"heroes/npc_dota_hero_lion_custom/lion_mana_drain_custom",
...
function lion_mana_drain_custom:OnSpellStart()
	...
	local modifier =
		target:AddNewModifier(self:GetCaster(), self, "modifier_lion_mana_drain_custom", { duration = duration })
	...
	if self:GetCaster():HasModifier("modifier_lion_1") then
		local heroes = FindUnitsInRadius(
			...
			nil,
			self:GetCastRange(self:GetCaster():GetAbsOrigin(), target),
		...
		if heroes_has then
			local modifier = heroes[1]:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_lion_mana_drain_custom",
				{ duration = duration }
		...
		elseif creep_has then
			local modifier = creeps[1]:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_lion_mana_drain_custom",
				{ duration = duration }
		...
		if modifier_lion_1 and modifier_lion_1:GetStackCount() > 1 then
			if heroes_has and heroes[2] ~= nil then
				local modifier = heroes[2]:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_lion_mana_drain_custom",
					{ duration = duration }
			...
			elseif creep_has and creeps[2] ~= nil then
				local modifier = creeps[2]:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_lion_mana_drain_custom",
					{ duration = duration }
...
function modifier_lion_mana_drain_custom:IsPurgable()
	return false
end
...
function modifier_lion_mana_drain_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_lion_1") then
		self.mana = self.mana
			+ self:GetAbility().modifier_lion_1_mana[self:GetCaster():GetTalentLevel("modifier_lion_1")]
	end
				...
				damage = damage
					+ (
						(
							...
							/ 100
							* self:GetAbility().modifier_lion_4[self:GetCaster():GetTalentLevel("modifier_lion_4")]
						) * self.interval
					)
			end
...
function modifier_lion_mana_drain_custom:PlayEffects()
	if not self:GetCaster():HasModifier("modifier_lion_2") then
		...
		local effect_cast = ParticleManager:CreateParticle(
			"particles/lion_mana_drain_damage.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			self:GetParent()
		)
		if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
