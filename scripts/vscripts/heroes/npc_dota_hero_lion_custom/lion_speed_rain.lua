...
LinkLuaModifier(
	"modifier_lion_speed_rain",
	"heroes/npc_dota_hero_lion_custom/lion_speed_rain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lion_speed_rain_debuff",
	"heroes/npc_dota_hero_lion_custom/lion_speed_rain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lion_speed_rain_buff",
	"heroes/npc_dota_hero_lion_custom/lion_speed_rain",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_lion_speed_rain:IsPurgable()
	return false
end
function modifier_lion_speed_rain:GetAttributes()
...
function modifier_lion_speed_rain:OnIntervalThink()
	...
	if self.modifier_lion_speed_rain_debuff then
		self.modifier_lion_speed_rain_debuff:SetStackCount(
			self.modifier_lion_speed_rain_debuff:GetStackCount() + self.speed_agility
		)
		self.modifier_lion_speed_rain_debuff:ForceRefresh()
	else
		self.modifier_lion_speed_rain_debuff = self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_lion_speed_rain_debuff",
	...
	if self.modifier_lion_speed_rain_buff then
		self.modifier_lion_speed_rain_buff:SetStackCount(
			self.modifier_lion_speed_rain_buff:GetStackCount() + self.speed_agility
		)
		self.modifier_lion_speed_rain_buff:ForceRefresh()
	else
		self.modifier_lion_speed_rain_buff = self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_lion_speed_rain_buff",
			{ duration = self:GetAbility():GetSpecialValueFor("buff_duration") }
...
function modifier_lion_speed_rain:PlayEffects()
	local effect_cast =
		ParticleManager:CreateParticle("particles/lion_green_static.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_lion_speed_rain_debuff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_lion_speed_rain_debuff:IsPurgable()
	return false
end

function modifier_lion_speed_rain_debuff:OnCreated() end
...
function modifier_lion_speed_rain_buff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_lion_speed_rain_buff:IsPurgable()
	return false
end

function modifier_lion_speed_rain_buff:OnCreated() end
