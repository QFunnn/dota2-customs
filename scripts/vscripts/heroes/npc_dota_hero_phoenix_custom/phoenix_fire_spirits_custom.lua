...
LinkLuaModifier(
	"modifier_phoenix_fire_spirits_custom_count",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_fire_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_fire_spirits_custom_debuff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_fire_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_phoenix_fire_spirits_custom_count:IsPurgable()
	return false
end
function modifier_phoenix_fire_spirits_custom_count:IsPurgeException()
	return false
end
function modifier_phoenix_fire_spirits_custom_count:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_phoenix_12")
end
...
function modifier_phoenix_fire_spirits_custom_count:OnIntervalThink()
	...
	if not self:GetParent():HasModifier("modifier_phoenix_12") then
		return
	end
	if self:GetStackCount() >= self.numSpirits then
		return
	end
	self.spirit_cooldown = self.spirit_cooldown + 1
	if
		self.spirit_cooldown
		>= self:GetAbility().modifier_phoenix_12[self:GetCaster():GetTalentLevel("modifier_phoenix_12")]
	then
		self.spirit_cooldown = 0
		...
		local phoenix_launch_fire_spirit_custom = self:GetParent()
			:FindAbilityByName("phoenix_launch_fire_spirit_custom")
		if phoenix_launch_fire_spirit_custom then
...
function phoenix_launch_fire_spirit_custom:OnSpellStart()
	...
	local modifier_phoenix_fire_spirits_custom_count =
		caster:FindModifierByName("modifier_phoenix_fire_spirits_custom_count")
	if not modifier_phoenix_fire_spirits_custom_count then
		return
	end
...
function phoenix_launch_fire_spirit_custom:OnProjectileHit_ExtraData(hTarget, vLocation, data)
	...
	local pfx_explosion = ParticleManager:CreateParticle(
...
function modifier_phoenix_fire_spirits_custom_debuff:GetTexture()
	return "phoenix_fire_spirits"
end
function modifier_phoenix_fire_spirits_custom_debuff:GetEffectName()
	return "particles/units/heroes/hero_phoenix/phoenix_fire_spirit_burn.vpcf"
end
function modifier_phoenix_fire_spirits_custom_debuff:GetEffectAttachType()
	...
	if self:GetCaster():HasModifier("modifier_phoenix_13") then
		self.damage_per_second = self.damage_per_second
			+ (
				...
				/ 100
				* phoenix_fire_spirits_custom.modifier_phoenix_13[self:GetCaster():GetTalentLevel("modifier_phoenix_13")]
			)
	end
