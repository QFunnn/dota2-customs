...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_phoenix_custom/phoenix_sun_ray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_sun_ray_custom_dummy_unit_thinker",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_sun_ray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_sun_ray_custom_dummy_buff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_sun_ray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_sun_ray_custom_buff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_sun_ray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_sun_ray_custom_debuff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_sun_ray_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function phoenix_sun_ray_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_phoenix.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_phoenix.vpcf",
		context
	)
end
...
function phoenix_sun_ray_custom:OnSpellStart()
	...
	local pathLength = self:GetCastRange(self:GetCaster():GetAbsOrigin(), self:GetCaster())
	...
	local modifier_phoenix_sun_ray_custom_caster_dummy = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_phoenix_sun_ray_custom_caster_dummy",
	...
	caster:SetContextThink(DoUniqueString("updateSunRay"), function()
		...
		if
			not modifier_phoenix_sun_ray_custom_caster_dummy or modifier_phoenix_sun_ray_custom_caster_dummy:IsNull()
		then
		...
		if
			caster.sun_ray_is_moving
			and not GameRules:IsGamePaused()
			and not self:GetCaster():HasModifier("modifier_phoenix_supernova_custom_buff")
		then
		...
		for i = 1, numVision do
			AddFOWViewer(
				caster:GetTeamNumber(),
				(casterOrigin + casterForward * (vision_radius * 2 * (i - 1))),
				vision_radius,
				FrameTime(),
				false
			)
...
function modifier_phoenix_sun_ray_custom_caster_dummy:IsPurgable()
	return false
end
function modifier_phoenix_sun_ray_custom_caster_dummy:IsPurgeException()
	return false
end
...
function modifier_phoenix_sun_ray_custom_caster_dummy:OnCreated()
	...
	self.pfx_sunray_flare = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_phoenix/phoenix_sunray_flare.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetCaster()
	)
	ParticleManager:SetParticleControlEnt(
		self.pfx_sunray_flare,
		9,
		self:GetCaster(),
		PATTACH_POINT_FOLLOW,
		"attach_mouth",
...
function modifier_phoenix_sun_ray_custom_dummy_unit_thinker:IsHidden()
	return true
end
function modifier_phoenix_sun_ray_custom_dummy_unit_thinker:IsPurgable()
	return false
end
function modifier_phoenix_sun_ray_custom_dummy_unit_thinker:IsPurgeException()
	return false
end
...
function modifier_phoenix_sun_ray_custom_dummy_buff:IsDebuff()
	return false
end
function modifier_phoenix_sun_ray_custom_dummy_buff:IsHidden()
	return true
end
function modifier_phoenix_sun_ray_custom_dummy_buff:IsPurgable()
	return false
end
function modifier_phoenix_sun_ray_custom_dummy_buff:IsPurgeException()
	return false
end
	...
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		if self:GetCaster():HasModifier("modifier_phoenix_6") then
			self:GetParent():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_phoenix_dying_light_custom_debuff",
				{ duration = self.tick_interval * 1.9 * (1 - self:GetParent():GetStatusResistance()) }
			)
		end
		self:GetParent():AddNewModifier(
			self:GetCaster(),
...
function modifier_phoenix_sun_ray_custom_debuff:IsHidden()
	return true
end
function modifier_phoenix_sun_ray_custom_debuff:IsPurgable()
	return false
end
function modifier_phoenix_sun_ray_custom_debuff:IsPurgeException()
	return false
end
function modifier_phoenix_sun_ray_custom_debuff:GetEffectName()
...
function modifier_phoenix_sun_ray_custom_debuff:OnIntervalThink()
	...
	local ability = self:GetAbility()
	local caster = self:GetCaster()
	if not caster:HasModifier("modifier_phoenix_sun_ray_custom_dummy_unit_thinker") then
		return
	end
	...
	local total_damage_perc = (taker_health * self.hp_perc_damage * self.tick_interval)
		+ ((taker_health * self.hp_perc_damage / tick_sum * self.tick_interval) * num_stack)
	local damageTable = {
		victim = taker,
		attacker = self:GetCaster(),
		damage = total_damage_base + total_damage_perc,
...
function modifier_phoenix_sun_ray_custom_buff:IsDebuff()
	return false
end
function modifier_phoenix_sun_ray_custom_buff:IsHidden()
	return true
end
function modifier_phoenix_sun_ray_custom_buff:IsPurgable()
	return false
end
function modifier_phoenix_sun_ray_custom_buff:IsPurgeException()
	return false
end
function modifier_phoenix_sun_ray_custom_buff:GetEffectName()
...
function modifier_phoenix_sun_ray_custom_buff:OnIntervalThink()
	...
	local total_damage_base = (base_dmg * self.tick_interval) + ((base_dmg / tick_sum * self.tick_interval) * num_stack)
	local total_damage_perc = (taker_health * self.hp_perc_heal * self.tick_interval)
		+ ((taker_health * self.hp_perc_heal / tick_sum * self.tick_interval) * num_stack)
