...
LinkLuaModifier(
	"modifier_phoenix_dying_light_custom",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_dying_light_custom",
...
function modifier_phoenix_dying_light_custom:IsHidden()
	return true
end
function modifier_phoenix_dying_light_custom:IsPurgable()
	return false
end
function modifier_phoenix_dying_light_custom:IsPurgeException()
	return false
end
function modifier_phoenix_dying_light_custom:RemoveOnDeath()
	return false
end
function modifier_phoenix_dying_light_custom:IsAura()
	return (self:GetParent():GetHealth() < self:GetParent():GetMaxHealth())
		or self:GetParent():HasModifier("modifier_phoenix_supernova_custom_buff")
end
...
function modifier_phoenix_dying_light_custom_debuff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_phoenix_1") then
		self.damage_pct = self.damage_pct
			+ self:GetAbility().modifier_phoenix_1[self:GetCaster():GetTalentLevel("modifier_phoenix_1")]
	end
	self:StartIntervalThink(1)
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_phoenix/phoenix_outburst_target.vpcf",
