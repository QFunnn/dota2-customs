...
LinkLuaModifier(
	"modifier_phoenix_frost_ray",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_frost_ray",
...
function modifier_phoenix_frost_ray:IsHidden()
	return true
end
function modifier_phoenix_frost_ray:IsPurgable()
	return false
end
function modifier_phoenix_frost_ray:IsPurgeException()
	return false
end
function modifier_phoenix_frost_ray:OnCreated()
	...
	self.length_distance = self:GetAbility():GetSpecialValueFor("length_distance")
	...
	local pfx = ParticleManager:CreateParticle(
		"particles/phoenix_custom/phoenix_sunray_creo_forge.vpcf",
	...
	local units = FindUnitsInLine(
		...
		self:GetCaster():GetAbsOrigin() + self:GetCaster():GetForwardVector() * 32,
		endcapPos,
		nil,
		self.width,
