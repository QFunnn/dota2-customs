...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_phoenix_custom/phoenix_icarus_dive_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_phoenix_icarus_dive_custom_extend_burn",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_icarus_dive_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_icarus_dive_custom_slow_debuff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_icarus_dive_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function phoenix_icarus_dive_custom:OnSpellStart()
	...
	local modifier_phoenix_icarus_dive_custom_dash_dummy = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_phoenix_icarus_dive_custom_dash_dummy",
	...
	self:GetCaster():SetContextThink(DoUniqueString("updateIcarusDive"), function()
		ParticleManager:SetParticleControl(
			pfx,
			0,
			self:GetCaster():GetAbsOrigin() + self:GetCaster():GetRightVector() * 32
		)
		...
		if
			not modifier_phoenix_icarus_dive_custom_dash_dummy
			or modifier_phoenix_icarus_dive_custom_dash_dummy:IsNull()
		then
			return
		end
		...
		if
			self:GetCaster():HasModifier("modifier_phoenix_sun_ray_custom_caster_dummy")
			and phoenix_sun_ray_custom
			and phoenix_sun_ray_custom.pfx
		then
		...
		for _, enemy in pairs(enemies) do
			enemy:AddNewModifier(
				...
				self,
				"modifier_phoenix_icarus_dive_custom_slow_debuff",
				{ duration = burn_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
...
function modifier_phoenix_icarus_dive_custom_dash_dummy:IsHidden()
	return true
end
function modifier_phoenix_icarus_dive_custom_dash_dummy:IsPurgable()
	return false
end
function modifier_phoenix_icarus_dive_custom_dash_dummy:IsPurgeException()
	return false
end
function modifier_phoenix_icarus_dive_custom_dash_dummy:GetEffectName()
	...
	if self:GetCaster():HasModifier("modifier_phoenix_13") then
		self.damage_per_second = self.damage_per_second
			+ (
				...
				/ 100
				* self:GetAbility().modifier_phoenix_13[self:GetCaster():GetTalentLevel("modifier_phoenix_13")]
			)
	end
...
function modifier_phoenix_icarus_dive_custom_slow_debuff:GetEffectName()
	return "particles/units/heroes/hero_phoenix/phoenix_icarus_dive_burn_debuff.vpcf"
end
function modifier_phoenix_icarus_dive_custom_slow_debuff:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end
function modifier_phoenix_icarus_dive_custom_slow_debuff:GetModifierMoveSpeedBonus_Percentage()
