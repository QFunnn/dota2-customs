...
function phoenix_ice_debuff:AddStack(target, count)
	if not self:GetCaster():HasModifier("modifier_phoenix_20") then
		return
	end
	if not self:IsTrained() then
		self:SetLevel(1)
	end
	local modifier_phoenix_ice_debuff =
		target:AddNewModifier(self:GetCaster(), self, "modifier_phoenix_ice_debuff", { duration = 5 })
...
function modifier_phoenix_ice_debuff:AddStackCounter(count)
	...
	if self:GetStackCount() >= 9 then
		local modifier_stunned = self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_stunned",
			{
				duration = self:GetAbility().stun_duration[self:GetCaster():GetTalentLevel("modifier_phoenix_20")]
