...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_phoenix_custom/phoenix_supernova_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_supernova_custom_egg_thinker",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_supernova_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_supernova_custom_debuff",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_supernova_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phoenix_supernova_custom_death",
	"heroes/npc_dota_hero_phoenix_custom/phoenix_supernova_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function phoenix_supernova_custom:OnSpellStart()
	...
	local modifier_phoenix_icarus_dive_custom_dash_dummy =
		self:GetCaster():FindModifierByName("modifier_phoenix_icarus_dive_custom_dash_dummy")
	if modifier_phoenix_icarus_dive_custom_dash_dummy then
		modifier_phoenix_icarus_dive_custom_dash_dummy:Destroy()
	end
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_phoenix_supernova_custom_buff", { duration = egg_duration })
...
function modifier_phoenix_supernova_custom_buff:IsPurgable()
	return false
end
function modifier_phoenix_supernova_custom_buff:IsPurgeException()
	return false
end
...
function modifier_phoenix_supernova_custom_buff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_phoenix_7") then
		for i = 0, 8 do
			...
			if
				ability
				and ability:GetName() ~= "phoenix_sun_ray_custom"
				and ability:GetName() ~= "phoenix_sun_ray_toggle_move_custom"
				and ability:GetName() ~= "phoenix_sun_ray_stop_custom"
			then
				ability:SetActivated(false)
			end
		end
	else
		local modifier_phoenix_sun_ray_custom_caster_dummy =
			self:GetCaster():FindModifierByName("modifier_phoenix_sun_ray_custom_caster_dummy")
...
function modifier_phoenix_supernova_custom_buff:GetAbsoluteNoDamagePhysical()
	return 1
end
function modifier_phoenix_supernova_custom_buff:GetAbsoluteNoDamageMagical()
	return 1
end
function modifier_phoenix_supernova_custom_buff:GetAbsoluteNoDamagePure()
	return 1
end
...
function modifier_phoenix_supernova_custom_egg_thinker:IsPurgable()
	return false
end
function modifier_phoenix_supernova_custom_egg_thinker:IsPurgeException()
	return false
end
function modifier_phoenix_supernova_custom_egg_thinker:IsAura()
	return true
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAuraRadius()
	return self:GetAbility():GetSpecialValueFor("aura_radius")
end
function modifier_phoenix_supernova_custom_egg_thinker:GetModifierAura()
	return "modifier_phoenix_supernova_custom_debuff"
end
...
function modifier_phoenix_supernova_custom_egg_thinker:GetOverrideAnimation()
	return ACT_DOTA_IDLE
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAbsoluteNoDamagePhysical()
	return 1
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAbsoluteNoDamageMagical()
	return 1
end
function modifier_phoenix_supernova_custom_egg_thinker:GetAbsoluteNoDamagePure()
	return 1
end
function modifier_phoenix_supernova_custom_egg_thinker:GetModifierHealthBarPips(data)
...
function modifier_phoenix_supernova_custom_egg_thinker:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_phoenix_5") then
		self.max_hero_attacks = self.max_hero_attacks
			+ self:GetAbility().modifier_phoenix_5[self:GetCaster():GetTalentLevel("modifier_phoenix_5")]
			+ (
				math.floor(self:GetCaster():GetMaxHealth() / self:GetAbility().modifier_phoenix_5_attack_max_health)
				* self:GetAbility().modifier_phoenix_5_attack
			)
	end
	if not IsServer() then
		return
...
function modifier_phoenix_supernova_custom_egg_thinker:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		ApplyDamage({
			...
			damage = self.damage_per_sec * self.tick_interval,
...
function modifier_phoenix_supernova_custom_egg_thinker:OnDeath(params)
	...
	if self:GetCaster():IsAlive() then
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), nil, "modifier_phoenix_supernova_custom_death", { duration = 1 })
...
function modifier_phoenix_supernova_custom_debuff:IsHidden()
	return false
end
function modifier_phoenix_supernova_custom_debuff:IsPurgable()
	return false
end
function modifier_phoenix_supernova_custom_debuff:GetHeroEffectName()
...
function modifier_phoenix_supernova_custom_debuff:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_phoenix/phoenix_supernova_radiance_streak_light.vpcf",
		PATTACH_POINT_FOLLOW,
		self:GetParent()
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_phoenix_supernova_custom_death:IsHidden()
	return true
end
function modifier_phoenix_supernova_custom_death:IsPurgable()
	return false
end
function modifier_phoenix_supernova_custom_death:IsPurgeException()
	return false
end
function modifier_phoenix_supernova_custom_death:RemoveOnDeath()
	return false
end