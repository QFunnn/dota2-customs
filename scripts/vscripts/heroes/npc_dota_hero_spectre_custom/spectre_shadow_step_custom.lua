...
LinkLuaModifier(
	"modifier_spectre_shadow_step_custom",
	"heroes/npc_dota_hero_spectre_custom/spectre_shadow_step_custom",
...
function spectre_shadow_step_custom:OnSpellStart()
	...
	local illusions = CreateIllusions(
		caster,
		caster,
		{ outgoing_damage = outgoing_damage, incoming_damage = incoming_damage, duration = -1 },
		1,
		150,
		false,
		false
	...
	for _, illusion in pairs(illusions) do
		...
		illusion:AddNewModifier(
			...
			self,
			"modifier_spectre_shadow_step_custom",
			{ duration = -1, target = target:entindex() }
		)
	end
end

modifier_spectre_shadow_step_custom = class({})
function modifier_spectre_shadow_step_custom:IsHidden()
	return true
end
function modifier_spectre_shadow_step_custom:IsPurgable()
	return false
end
