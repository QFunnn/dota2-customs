...
LinkLuaModifier(
	"modifier_spectre_haunt_custom",
	"heroes/npc_dota_hero_spectre_custom/spectre_haunt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spectre_reality_custom",
	"heroes/npc_dota_hero_spectre_custom/spectre_haunt_custom",
...
function spectre_haunt_custom:OnSpellStart()
	...
	for count = #heroes, 1, -1 do
		if
			heroes[count]
			and (
				heroes[count]:GetUnitName() == "npc_woda_wisp_death"
				or not heroes[count]:IsAlive()
				or heroes[count]:IsIllusion()
			)
		then
	...
	local illusions = CreateIllusions(
		caster,
		caster,
		{ outgoing_damage = outgoing, incoming_damage = incoming, duration = duration },
		#heroes,
		distance,
		false,
		true
...
function spectre_reality_custom:OnSpellStart()
	...
	for _, haunt in pairs(haunts) do
		if
			(
				haunt:HasModifier("modifier_spectre_haunt_custom")
				or haunt:HasModifier("modifier_spectre_shadow_step_custom")
			) and haunt:GetPlayerOwner() == caster:GetPlayerOwner()
		then
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_spectre_reality_custom",
		{ point_x = pos.x, point_y = pos.y, point_z = pos.z }
	)
...
function modifier_spectre_reality_custom:IsHidden()
	return true
end
function modifier_spectre_reality_custom:IsPurgable()
	return false
end
function modifier_spectre_reality_custom:IsPurgeException()
	return false
end
function modifier_spectre_reality_custom:OnCreated(params)
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_spectre/spectre_shadowstep_tracking.vpcf",
