...
LinkLuaModifier(
	"modifier_spectre_spectral_dagger_custom_dummy_proj",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_spectre_spectral_path",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_spectre_spectral_in_path",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_spectre_spectral_grace",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_spectre_spectral_path_by_heroes",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_spectre_spectral_grace_invis",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_spectre_spectral_dagger_custom_dummy_invis",
	"heroes/npc_dota_hero_spectre_custom/spectre_spectral_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function spectre_spectral_dagger_custom:OnSpellStart(new_target)
	...
	if target and (target:IsHero() or new_target) then
		...
		local pathFX = ParticleManager:CreateParticle(
	...
	if Target ~= nil and not Target:IsInvulnerable() then
		...
		if self:GetCaster():HasModifier("modifier_spectre_10") and data.tracking == 1 then
			...
			local illusions = CreateIllusions(
				self:GetCaster(),
				self:GetCaster(),
				{ outgoing_damage = outgoing_damage, incoming_damage = incoming_damage, duration = duration },
				1,
				150,
				false,
				true
...
function spectre_spectral_dagger_custom:OnProjectileThink_ExtraData(vLocation, data)
	...
	if data.last_path_pos and (vLocation - data.last_path_pos):Length2D() < self:GetSpecialValueFor("path_radius") then
		return
	end
	...
	CreateModifierThinker(
		...
		self,
		"modifier_ability_spectre_spectral_path",
		{ duration = self:GetSpecialValueFor("dagger_path_duration") },
		vLocation,
...
function modifier_spectre_spectral_dagger_custom_dummy_proj:GetAbsoluteNoDamageMagical()
	return 1
end
function modifier_spectre_spectral_dagger_custom_dummy_proj:GetAbsoluteNoDamagePure()
	return 1
end
function modifier_spectre_spectral_dagger_custom_dummy_proj:GetAbsoluteNoDamagePhysical()
	return 1
end
...
function modifier_ability_spectre_spectral_path:OnIntervalThink()
	...
	for _, hero in ipairs(all) do
		if
			hero == self:GetCaster()
			or (hero:IsIllusion() and hero:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID())
		then
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_ability_spectre_spectral_grace",
				{ duration = self.dagger_grace_period }
			)
			break
		end
	end
...
function modifier_ability_spectre_spectral_in_path:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_spectre_9") then
		self.bonus_movespeed = self.bonus_movespeed
			+ self:GetAbility().modifier_spectre_9[self:GetCaster():GetTalentLevel("modifier_spectre_9")]
	end
	if not IsServer() then
		return
	end
...
function modifier_ability_spectre_spectral_in_path:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_spectre_16") then
		local damage = self:GetAbility().modifier_spectre_16_base_dmg
			+ (
				self:GetCaster():GetMana()
				/ 100
				* self:GetAbility().modifier_spectre_16_perc_dmg[self:GetCaster():GetTalentLevel("modifier_spectre_16")]
			)
		ApplyDamage({
	...
	if self:GetCaster():HasModifier("modifier_spectre_21") then
		...
		if spectre_desolate_custom and spectre_desolate_custom:GetLevel() > 0 then
			...
			if self:GetParent():HasModifier("modifier_spectre_12") then
				damage = damage
					+ spectre_desolate_custom.modifier_spectre_12[self:GetCaster():GetTalentLevel("modifier_spectre_12")]
			end
			if self:GetCaster():HasModifier("modifier_spectre_6") then
				damage = damage * 2
			end
			if
				self:GetCaster():HasModifier("modifier_spectre_13")
				and RollPercentage(self:GetCaster():GetEvasion() * 100)
			then
				damage = damage * 2
			end
			ApplyDamage({
				...
				ability = spectre_desolate_custom,
			})
		end
	end
end
...
function modifier_ability_spectre_spectral_in_path:GetModifierPropertyRestorationAmplification()
	if
		self:GetCaster():HasModifier("modifier_spectre_20")
		and self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
	then
...
function modifier_ability_spectre_spectral_grace:OnCreated()
	...
	if self:GetParent():HasModifier("modifier_spectre_18") then
		local enemies = FindUnitsInRadius(
			...
			self:GetParent():GetAbsOrigin(),
			nil,
			self:GetAbility().modifier_spectre_18,
			DOTA_UNIT_TARGET_TEAM_ENEMY,
			DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
			DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
...
function modifier_ability_spectre_spectral_path_by_heroes:OnCreated()
	...
	self.pathFX = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_spectre/spectre_shadow_path.vpcf",
...
function modifier_ability_spectre_spectral_path_by_heroes:OnIntervalThink()
	...
	if self.last_path_pos and (pos - self.last_path_pos):Length2D() < self.path_radius then
		return
	end
	...
	CreateModifierThinker(
		...
		self:GetAbility(),
		"modifier_ability_spectre_spectral_path",
		{ duration = self.dagger_path_duration },
		pos,
		caster:GetTeamNumber(),
		false
	)
...
function modifier_ability_spectre_spectral_grace_invis:IsPurgable()
	return false
end

function modifier_ability_spectre_spectral_grace_invis:GetTexture()
	return "spectre_18"
end
...
function modifier_spectre_spectral_dagger_custom_dummy_invis:GetAbsoluteNoDamageMagical()
	return 1
end
function modifier_spectre_spectral_dagger_custom_dummy_invis:GetAbsoluteNoDamagePure()
	return 1
end
function modifier_spectre_spectral_dagger_custom_dummy_invis:GetAbsoluteNoDamagePhysical()
	return 1
end
