...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_spectre_custom/spectre_desolate_custom",
...
function spectre_desolate_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_spectre.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_spectre.vpcf",
		context
	)
end
...
function modifier_spectre_desolate_custom:OnAttackLanded(params)
	...
	if self:GetCaster():HasModifier("modifier_spectre_8") then
		local enemies_new = FindUnitsInRadius(
			self:GetParent():GetTeamNumber(),
			params.target:GetOrigin(),
			nil,
			self:GetAbility().modifier_spectre_8,
			DOTA_UNIT_TARGET_TEAM_ENEMY,
			DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
			0,
			0,
