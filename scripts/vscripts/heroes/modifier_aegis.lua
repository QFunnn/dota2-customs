...
function modifier_aegis:Reincarnate()
	...
	Timers:CreateTimer({
		endTime = AEGIS_REINCARNATE_DELAY,
		callback = function()
	...
	Timers:CreateTimer({
		endTime = AEGIS_REINCARNATE_DELAY + 0.3,
		callback = function()
			hCaster:AddNewModifier(
				hCaster,
				hAbility,
				"modifier_aegis_buff",
				{ duration = AEGIS_REINCARNATE_BUFF_DURATION }
			)
		end,
	})
...
function modifier_aegis:ReincarnateTime()
	...
	if Rounds:IsPlayerInPVPDuel(nPlayerID) then
		return
	end

	if not Players:IsActivePlayer(nPlayerID) then
		return
	end

	if Rounds:RoundTypeIs(ROUND_TYPES.VOTING) then
		return
	end
