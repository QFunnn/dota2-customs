...
function modifier_aegis:ReincarnateTime()
	...
	if
		...
		and GameMode:GetMatch():IsTeamAbandoned(teamId)
	then
		return nil
	end
	...
	if ReincarnationService:CheckReincarnationAbilityReady(self:GetParent()) then
		return nil
	end
...
function modifier_aegis:OnDeath(keys)
	...
	if
		ReincarnationService:IsReincarnationWork(self:GetParent())
		or self:GetParent().bJoiningPvp == true
	...
	Timers:CreateTimer(0.5, function()
		HeroRefreshService:RefreshAbilityAndItem(caster, {
	...
	if GameMode:GetMatch():GetCurrentRound().roundNumber >= 15 then
...
function modifier_aegis:CompensateDeath()
	...
	for _ = 1, 2 do
		local item =
			ExtenderStash:GiveItemToPlayerStashInventoryOrDrop(parent:GetPlayerOwnerID(), "item_relearn_book_lua")
		if item then
