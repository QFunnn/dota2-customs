...
LinkLuaModifier(
	"modifier_kez_talon_toss_custom",
	"heroes/npc_dota_hero_kez_custom/kez_talon_toss_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_talon_toss_custom_damage",
	"heroes/npc_dota_hero_kez_custom/kez_talon_toss_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_talon_toss_custom:OnProjectileHit(target, location)
	...
	local modifier_kez_talon_toss_custom_damage =
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_kez_talon_toss_custom_damage", {})
...
function modifier_kez_talon_toss_custom_damage:IsHidden()
	return true
end
function modifier_kez_talon_toss_custom_damage:IsPurgeException()
	return false
end
function modifier_kez_talon_toss_custom_damage:IsPurgable()
	return false
end
function modifier_kez_talon_toss_custom_damage:RemoveOnDeath()
	return false
end
...
function modifier_kez_talon_toss_custom_damage:GetModifierOverrideAttackDamage()
	...
	if self:GetCaster():HasModifier("modifier_kez_15") then
		damage = damage
			+ (
				self:GetCaster():GetIntellect(true)
				/ 100
				* self:GetAbility().modifier_kez_15[self:GetCaster():GetTalentLevel("modifier_kez_15")]
			)
	end
	return damage
end