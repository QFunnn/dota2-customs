...
LinkLuaModifier(
	"modifier_kez_grappling_claw_custom",
	"heroes/npc_dota_hero_kez_custom/kez_grappling_claw_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_kez_grappling_claw_custom_debuff",
	"heroes/npc_dota_hero_kez_custom/kez_grappling_claw_custom",
...
function kez_grappling_claw_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/skywrath_mage/skywrath_arcana/skywrath_arcana_rod_of_atos_debuff.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/items/skywrath_mage/skywrath_arcana/skywrath_arcana_rod_of_atos_impact.vpcf",
		context
	)
...
function kez_grappling_claw_custom:OnSpellStart()
	...
	local modifier_kez_grappling_claw_custom = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_kez_grappling_claw_custom",
		{ length = length, target = target:entindex() }
	)
...
function modifier_kez_grappling_claw_custom:IsHidden()
	return true
end
function modifier_kez_grappling_claw_custom:IsPurgeException()
	return false
end
function modifier_kez_grappling_claw_custom:IsPurgable()
	return false
end
function modifier_kez_grappling_claw_custom:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_kez_19") then
		debuff_duration = debuff_duration
			+ self:GetAbility().modifier_kez_19[self:GetCaster():GetTalentLevel("modifier_kez_19")]
	end
	self.modifier_kez_grappling_claw_custom_debuff = self.target:AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_kez_grappling_claw_custom_debuff",
		{ duration = debuff_duration * (1 - self.target:GetStatusResistance()) }
	)
...
function modifier_kez_grappling_claw_custom:OnDestroy()
	...
	if not self:GetCaster():HasModifier("modifier_kez_19") then
		if
			self.modifier_kez_grappling_claw_custom_debuff
			and not self.modifier_kez_grappling_claw_custom_debuff:IsNull()
		then
