...
LinkLuaModifier(
	"modifier_kez_kazurai_katana_custom",
	"heroes/npc_dota_hero_kez_custom/kez_kazurai_katana_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_kazurai_katana_custom_debuff",
	"heroes/npc_dota_hero_kez_custom/kez_kazurai_katana_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_kazurai_katana_custom_fx",
	"heroes/npc_dota_hero_kez_custom/kez_kazurai_katana_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_kazurai_katana_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(target, self, "modifier_kez_kazurai_katana_custom_fx", { duration = impale_duration })
	...
	if self:IsTargetBackFacing(target) and self:GetCaster():HasModifier("modifier_kez_7") then
		target:AddNewModifier(
			...
			self,
			"modifier_stunned",
			{ duration = impale_duration * (1 - target:GetStatusResistance()) }
		)
	end
	local modifier_kez_kazurai_katana_custom_debuff =
		target:FindModifierByName("modifier_kez_kazurai_katana_custom_debuff")
...
function modifier_kez_kazurai_katana_custom:IsHidden()
	return true
end
function modifier_kez_kazurai_katana_custom:IsPurgable()
	return false
end
function modifier_kez_kazurai_katana_custom:IsPurgeException()
	return false
end
function modifier_kez_kazurai_katana_custom:RemoveOnDeath()
	return false
end
...
function modifier_kez_kazurai_katana_custom:OnAttackLanded(params)
	...
	if not params.target:IsBaseNPC() then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_kez_5") then
		katana_bleed_attack_damage_pct = katana_bleed_attack_damage_pct
			+ self:GetAbility().modifier_kez_5[self:GetCaster():GetTalentLevel("modifier_kez_5")]
	end
	local modifier_kez_kazurai_katana_custom_debuff =
		target:FindModifierByNameAndCaster("modifier_kez_kazurai_katana_custom_debuff", self:GetCaster())
	if not modifier_kez_kazurai_katana_custom_debuff then
		modifier_kez_kazurai_katana_custom_debuff = target:AddNewModifier(
			self:GetParent(),
			self:GetAbility(),
			"modifier_kez_kazurai_katana_custom_debuff",
			{ duration = self.katana_bleed_duration }
		)
	end
	if modifier_kez_kazurai_katana_custom_debuff then
		local new_stacks =
			math.floor((params.original_damage / 100 * katana_bleed_attack_damage_pct) * self.katana_bleed_duration)
...
function modifier_kez_kazurai_katana_custom_debuff:GetAttributes()
...
function modifier_kez_kazurai_katana_custom_debuff:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_kez/kez_katana_bleed.vpcf",
...
function modifier_kez_kazurai_katana_custom_fx:IsHidden()
	return true
end
function modifier_kez_kazurai_katana_custom_fx:IsPurgable()
	return false
end
function modifier_kez_kazurai_katana_custom_fx:IsPurgeException()
	return false
end
