...
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom",
	"heroes/npc_dota_hero_kez_custom/kez_falcon_rush_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom_speed",
	"heroes/npc_dota_hero_kez_custom/kez_falcon_rush_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom_clone",
	"heroes/npc_dota_hero_kez_custom/kez_falcon_rush_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom_barrier",
	"heroes/npc_dota_hero_kez_custom/kez_falcon_rush_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom_damage",
	"heroes/npc_dota_hero_kez_custom/kez_falcon_rush_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_falcon_rush_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_kez_10") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_kez_falcon_rush_custom_barrier",
			{ duration = self.modifier_kez_10_duration }
		)
	end
	---------------------------------------------------------------------------------------------
	if self:GetCaster():HasModifier("modifier_kez_21") then
		return
	end
...
function modifier_kez_falcon_rush_custom:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_kez_9")
end
...
function modifier_kez_falcon_rush_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_kez_10") then
		self.rush_range = self.rush_range
			+ self:GetAbility().modifier_kez_10[self:GetCaster():GetTalentLevel("modifier_kez_10")]
	end
...
function modifier_kez_falcon_rush_custom:GetModifierPropertyPseudoRandomBonus(params)
	if not self:GetCaster().MissChanceActive then
		return
	end
...
function modifier_kez_falcon_rush_custom:OnIntervalThink()
	...
	if not self.bRushChecking then
		return
	end
	...
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_kez_falcon_rush_custom_speed", { duration = 5 })
...
function modifier_kez_falcon_rush_custom:OnAttackLanded(params)
	...
	local modifier_kez_falcon_rush_custom_speed = self:GetParent()
		:FindModifierByName("modifier_kez_falcon_rush_custom_speed")
...
function modifier_kez_falcon_rush_custom_speed:IsHidden()
	return true
end
function modifier_kez_falcon_rush_custom_speed:IsPurgable()
	return false
end
...
function modifier_kez_falcon_rush_custom_clone:IsHidden()
	return true
end
function modifier_kez_falcon_rush_custom_clone:IsPurgable()
	return false
end
function modifier_kez_falcon_rush_custom_clone:GetAttributes()
...
function modifier_kez_falcon_rush_custom_clone:OnIntervalThink()
	...
	local modifier_kez_falcon_rush_custom_damage =
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_kez_falcon_rush_custom_damage", {})
	if modifier_kez_falcon_rush_custom_damage then
		...
		self:GetCaster():PerformAttack(
			...
			false,
			false,
			self:GetCaster():GetChanceToEvasion(self:GetParent())
		)
...
function modifier_kez_falcon_rush_custom_barrier:GetTexture()
	return "kez_10"
end
...
function modifier_kez_falcon_rush_custom_damage:IsHidden()
	return true
end
function modifier_kez_falcon_rush_custom_damage:IsPurgable()
	return false
end
function modifier_kez_falcon_rush_custom_damage:IsPurgeException()
	return false
end
function modifier_kez_falcon_rush_custom_damage:RemoveOnDeath()
	return false
end
