...
LinkLuaModifier(
	"modifier_kez_shodo_sai_custom",
	"heroes/npc_dota_hero_kez_custom/kez_shodo_sai_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_shodo_sai_custom_handler",
	"heroes/npc_dota_hero_kez_custom/kez_shodo_sai_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_shodo_sai_custom_mark",
	"heroes/npc_dota_hero_kez_custom/kez_shodo_sai_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_shodo_sai_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_kez_shodo_sai_custom", { duration = parry_duration })
end
...
function kez_shodo_sai_custom:ParryEffect(target)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = parry_stun_duration * (1 - target:GetStatusResistance()) }
	)
...
function kez_shodo_sai_custom:AddTargetMark(target, parry, full_chance)
	...
	if RollPercentage(chance) then
		target:AddNewModifier(
			...
			self,
			"modifier_kez_shodo_sai_custom_mark",
			{ duration = vuln_duration * (1 - target:GetStatusResistance()), parry = parry }
		)
	end
end

modifier_kez_shodo_sai_custom = class({})
function modifier_kez_shodo_sai_custom:IsPurgable()
	return true
end
function modifier_kez_shodo_sai_custom:IsPurgeException()
	return true
end
...
function modifier_kez_shodo_sai_custom_handler:IsHidden()
	return true
end
function modifier_kez_shodo_sai_custom_handler:IsPurgable()
	return false
end
function modifier_kez_shodo_sai_custom_handler:IsPurgeException()
	return false
end
function modifier_kez_shodo_sai_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_kez_shodo_sai_custom_handler:OnTakeDamage(params)
	...
	if params.inflictor:GetAbilityName() == "kez_kazurai_katana_custom" then
		return
	end
...
function modifier_kez_shodo_sai_custom_handler:GetModifierPreAttack_CriticalStrike(params)
	...
	if
		params.target
		and not params.target:HasModifier("modifier_kez_shodo_sai_custom_mark")
		and RollPercentage(chance)
	then
...
function modifier_kez_shodo_sai_custom_mark:IsPurgable()
	return false
end
function modifier_kez_shodo_sai_custom_mark:IsPurgeException()
	return false
end
