...
LinkLuaModifier(
	"modifier_kez_ravens_veil_custom",
	"heroes/npc_dota_hero_kez_custom/kez_ravens_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_ravens_veil_custom_debuff",
	"heroes/npc_dota_hero_kez_custom/kez_ravens_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_generic_ring_lua", "modifiers/modifier_generic_ring_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_kez_falcon_rush_custom_magic_immune",
	"heroes/npc_dota_hero_kez_custom/kez_ravens_veil_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_ravens_veil_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_kez_13") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_kez_falcon_rush_custom_magic_immune",
			{ duration = self.modifier_kez_13[self:GetCaster():GetTalentLevel("modifier_kez_13")] }
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_kez_8") then
		...
		if kez_switch_weapons_custom then
			self:GetCaster():AddNewModifier(
				...
				{ duration = kez_switch_weapons_custom:GetSpecialValueFor("sai_swap_duration") }
			)
		end
	end
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_kez_ravens_veil_custom", { duration = buff_duration })
...
function modifier_kez_ravens_veil_custom:IsPurgable()
	return false
end
...
function modifier_kez_ravens_veil_custom:OnAbilityFullyCast(params)
	...
	if params.ability:GetAbilityName() == "kez_switch_weapons_custom" then
		return
	end
	self:Destroy()
end
...
function modifier_kez_ravens_veil_custom_debuff:IsPurgeException()
	return true
end
...
function modifier_kez_ravens_veil_custom_debuff:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_kez/kez_sai_ultimate_impact.vpcf",
...
function modifier_kez_falcon_rush_custom_magic_immune:IsPurgable()
	return false
end
function modifier_kez_falcon_rush_custom_magic_immune:GetTexture()
	return "kez_13"
end
