...
LinkLuaModifier(
	"modifier_kez_raptor_dance_custom",
	"heroes/npc_dota_hero_kez_custom/kez_raptor_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_raptor_dance_custom_cast",
	"heroes/npc_dota_hero_kez_custom/kez_raptor_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_raptor_dance_custom_buff",
	"heroes/npc_dota_hero_kez_custom/kez_raptor_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_raptor_dance_custom:OnAbilityPhaseStart()
	...
	self:GetCaster():AddNewModifier(
		...
		{ duration = self:GetSpecialValueFor("invuln_period") }
	)
	return true
end
...
function kez_raptor_dance_custom:CastFinish(bInterrupted)
	...
	local modifier_kez_raptor_dance_custom_cast = self:GetCaster()
		:FindModifierByName("modifier_kez_raptor_dance_custom_cast")
...
function kez_raptor_dance_custom:StrikeDamage(strikes)
	...
	for _, unit in pairs(units) do
		...
		if modifier_kez_shodo_sai_custom_mark then
			local critical_damage = modifier_kez_shodo_sai_custom_mark:GetModifierPreAttack_Target_CriticalStrike({
		...
		local particle_target = ParticleManager:CreateParticle(
...
function modifier_kez_raptor_dance_custom:IsPurgable()
	return false
end
function modifier_kez_raptor_dance_custom:IsPurgeException()
	return false
end
...
function modifier_kez_raptor_dance_custom_cast:IsHidden()
	return true
end
function modifier_kez_raptor_dance_custom_cast:IsPurgable()
	return false
end
function modifier_kez_raptor_dance_custom_cast:IsPurgeException()
	return false
end
...
function modifier_kez_raptor_dance_custom_buff:IsHidden()
	return true
end
function modifier_kez_raptor_dance_custom_buff:IsPurgable()
	return false
end
function modifier_kez_raptor_dance_custom_buff:IsPurgeException()
	return false
end
function modifier_kez_raptor_dance_custom_buff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_kez_6") then
		self.strikes = self.strikes
			+ self:GetAbility().modifier_kez_6[self:GetCaster():GetTalentLevel("modifier_kez_6")]
	end
	self.abilities = {
