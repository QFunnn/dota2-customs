...
LinkLuaModifier(
	"modifier_kez_echo_slash_custom_debuff",
	"heroes/npc_dota_hero_kez_custom/kez_echo_slash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_kez_echo_slash_custom_damage",
	"heroes/npc_dota_hero_kez_custom/kez_echo_slash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function kez_echo_slash_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_kez.vpcf",
		context
	)
end
...
function kez_echo_slash_custom:GetManaCost(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_kez_2") then
		manacost = manacost
			* (1 - (self.modifier_kez_2_manacost[self:GetCaster():GetTalentLevel("modifier_kez_2")] / 100))
	end
	return manacost
end
...
function kez_echo_slash_custom:PreCreateAttack(
	delay,
	katana_distance,
	travel_distance,
	start_point,
...
function kez_echo_slash_custom:CreateAttack(
	katana_distance,
	travel_distance,
	start_point,
	end_point,
	...
	for _, target in pairs(units) do
		target:AddNewModifier(
			...
			self,
			"modifier_kez_echo_slash_custom_debuff",
			{ duration = tag_slow_duration }
		)
		local modifier_kez_echo_slash_custom_damage =
			self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_kez_echo_slash_custom_damage", {})
		if modifier_kez_echo_slash_custom_damage then
			if self:GetCaster():HasModifier("modifier_kez_17") then
				local damage_attack = self:GetCaster():GetAverageTrueAttackDamage(nil)
					+ self:GetSpecialValueFor("echo_hero_damage")
				ApplyDamage({
					victim = target,
				...
				self:GetCaster():PerformAttack(
					...
					false,
					true,
					self:GetCaster():GetChanceToEvasion(target)
				)
				self:GetCaster().spell_attack = nil
				local modifier_kez_kazurai_katana_custom = self:GetCaster()
					:FindModifierByName("modifier_kez_kazurai_katana_custom")
...
function modifier_kez_echo_slash_custom_damage:IsHidden()
	return true
end
function modifier_kez_echo_slash_custom_damage:IsPurgable()
	return false
end
function modifier_kez_echo_slash_custom_damage:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_kez_3") then
		self.attack_factor = self.attack_factor
			+ self:GetAbility().modifier_kez_3[self:GetCaster():GetTalentLevel("modifier_kez_3")]
	end
end
