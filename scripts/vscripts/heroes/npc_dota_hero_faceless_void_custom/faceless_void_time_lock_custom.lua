...
LinkLuaModifier(
	"modifier_faceless_void_time_lock_custom_handle",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_lock_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_faceless_void_time_lock_custom_stun",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_lock_custom",
	LUA_MODIFIER_MOTION_NONE
)
			...
			if self:GetCaster():HasModifier("modifier_faceless_void_18") then
				damage = damage
					+ (
						...
						/ 100
						* self.modifier_faceless_void_18[self:GetCaster():GetTalentLevel("modifier_faceless_void_18")]
					)
			end
			ApplyDamage({
				victim = target,
...
function modifier_faceless_void_time_lock_custom_handle:IsPurgable()
	return false
end
function modifier_faceless_void_time_lock_custom_handle:IsDebuff()
	return false
end
function modifier_faceless_void_time_lock_custom_handle:IsHidden()
	return true
end
function modifier_faceless_void_time_lock_custom_handle:OnAttackLanded(params)
	if params.attacker == self:GetParent() then
		...
		if self:GetParent():GetTeamNumber() == params.target:GetTeamNumber() then
			return
		end
