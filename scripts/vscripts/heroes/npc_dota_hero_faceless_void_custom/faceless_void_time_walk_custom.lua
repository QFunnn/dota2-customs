...
LinkLuaModifier(
	"modifier_faceless_void_time_walk_custom",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_walk_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_faceless_void_time_walk_custom_counter",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_walk_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_faceless_void_time_walk_custom_silence",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_walk_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
...
function faceless_void_time_walk_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_faceless_void_time_walk_custom",
		{ duration = range / self:GetSpecialValueFor("speed") }
	)
	...
	Timers:CreateTimer(1.5, function()
		local faceless_void_time_walk_reverse_custom = self:GetCaster()
			:FindAbilityByName("faceless_void_time_walk_reverse_custom")
...
function modifier_faceless_void_time_walk_custom:IsPurgable()
	return false
end
function modifier_faceless_void_time_walk_custom:IsPurgeException()
	return false
end
...
function modifier_faceless_void_time_walk_custom:OnCreated(data)
	...
	if self:GetCaster():HasModifier("modifier_faceless_void_10") then
		self.range = self.range
			+ self:GetAbility().modifier_faceless_void_10_range[self:GetCaster()
				:GetTalentLevel("modifier_faceless_void_10")]
	end
	...
	if not self.reverse and not self:GetCaster():HasModifier("modifier_faceless_void_8") then
		self:GetCaster():HealWithParams(
			self:GetCaster().time_walk_damage_taken,
			self:GetAbility(),
			false,
			true,
			self:GetParent(),
		...
		local modifier_faceless_void_time_walk_custom_counter =
			self:GetCaster():FindModifierByName("modifier_faceless_void_time_walk_custom_counter")
	...
	if not self.reverse then
		if not self:GetCaster():HasModifier("modifier_faceless_void_8") then
			...
			local units = FindUnitsInRadius(
				...
				nil,
				self:GetAbility().modifier_faceless_void_8_radius,
			...
			for _, unit in pairs(units) do
				unit:AddNewModifier(
					...
					"modifier_faceless_void_time_walk_custom_silence",
					{
						duration = self:GetAbility().modifier_faceless_void_8_duration[self:GetCaster():GetTalentLevel(
							"modifier_faceless_void_8"
		...
		if self:GetCaster():HasModifier("modifier_faceless_void_21") then
			...
			if faceless_void_time_lock_custom and faceless_void_time_lock_custom:GetLevel() > 0 then
				local units = FindUnitsInRadius(
					...
					self:GetCaster():GetAbsOrigin(),
					nil,
					self:GetAbility().modifier_faceless_void_21,
					DOTA_UNIT_TARGET_TEAM_ENEMY,
					DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO,
					DOTA_UNIT_TARGET_FLAG_NONE,
					FIND_ANY_ORDER,
...
function modifier_faceless_void_time_walk_custom_counter:IsPurgable()
	return false
end
function modifier_faceless_void_time_walk_custom_counter:IsDebuff()
	return false
end
function modifier_faceless_void_time_walk_custom_counter:IsHidden()
	return true
end
...
function modifier_faceless_void_time_walk_custom_silence:IsDebuff()
	return true
end
function modifier_faceless_void_time_walk_custom_silence:IsPurgable()
	return false
end
function modifier_faceless_void_time_walk_custom_silence:GetTexture()
	return "faceless_void_8"
end
...
function faceless_void_time_walk_reverse_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_faceless_void_10") then
		range = range
			+ faceless_void_time_walk_custom.modifier_faceless_void_10_range[self:GetCaster():GetTalentLevel(
				"modifier_faceless_void_10"
			)]
	end
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), faceless_void_time_walk_custom, "modifier_faceless_void_time_walk_custom", {
	...
	ParticleManager:SetParticleControl(
		pcf,
		1,
		Vector(faceless_void_time_walk_custom:GetSpecialValueFor("radius"), 0, 0)
	)
	EmitSoundOn("Hero_FacelessVoid.TimeWalk", self:GetCaster())
	ProjectileManager:ProjectileDodge(self:GetCaster())
