...
LinkLuaModifier(
	"modifier_faceless_void_time_dilation_custom",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_dilation_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_faceless_void_time_dilation_custom_handle",
	"heroes/npc_dota_hero_faceless_void_custom/faceless_void_time_dilation_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function faceless_void_time_dilation_custom:OnSpellStart()
	...
	for _, unit in pairs(units) do
		...
		local hit_pfx_2 = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_faceless_void/faceless_void_dialatedebuf_d.vpcf",
			PATTACH_ABSORIGIN,
			unit
		)
...
function modifier_faceless_void_time_dilation_custom:GetAttributes()
...
function modifier_faceless_void_time_dilation_custom:OnCreated()
	...
	for i = 0, self:GetParent():GetAbilityCount() - 1 do
		...
		if
			ability
			and not self:GetParent():IsDebuffImmune()
			and ability:GetCooldownTimeRemaining() > 0
			and not ability:IsToggle()
			and not self.ignored_abilities[ability:GetAbilityName()]
		then
...
function modifier_faceless_void_time_dilation_custom:OnIntervalThink()
	...
	if self.damage_interval <= 0 then
		...
		if self:GetCaster():HasModifier("modifier_faceless_void_3") then
			base_damage = self:GetAbility().modifier_faceless_void_3[self:GetCaster()
				:GetTalentLevel("modifier_faceless_void_3")]
		end
		if self:GetCaster():HasModifier("modifier_faceless_void_2") then
			damage = damage
				+ (
					...
					/ 100
					* self:GetAbility().modifier_faceless_void_2[self:GetCaster()
						:GetTalentLevel("modifier_faceless_void_2")]
				)
		end
...
function modifier_faceless_void_time_dilation_custom_handle:IsHidden()
	return true
end
function modifier_faceless_void_time_dilation_custom_handle:IsPurgable()
	return false
end
function modifier_faceless_void_time_dilation_custom_handle:IsPurgeException()
	return false
end
function modifier_faceless_void_time_dilation_custom_handle:RemoveOnDeath()
	return false
end
...
function modifier_faceless_void_time_dilation_custom_handle:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_faceless_void_4") then
		return
	end
