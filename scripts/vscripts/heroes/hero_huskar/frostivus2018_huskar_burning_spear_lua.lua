...
LinkLuaModifier(
	"modifier_frostivus2018_huskar_burning_spear_lua",
	"heroes/hero_huskar/frostivus2018_huskar_burning_spear_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_frostivus2018_huskar_burning_spear_lua_debuff",
	"heroes/hero_huskar/frostivus2018_huskar_burning_spear_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
...
function modifier_frostivus2018_huskar_burning_spear_lua:GetModifierProcAttack_Feedback(params)
	...
	if
		...
		and hTarget:IsAlive()
		and self.tRecords[params.record] ~= nil
	then
		...
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				...
				unit:AddNewModifier(
					...
					hAbility,
					"modifier_frostivus2018_huskar_burning_spear_lua_debuff",
					{ duration = self.duration, delay = iCount * FrameTime() }
				)
			end
		end
	end
...
function modifier_frostivus2018_huskar_burning_spear_lua:OnAttackStart(params)
	...
	then
		if
			(hParent:GetCurrentActiveAbility() == hAbility or hAbility:GetAutoCastState())
			and not hParent:IsSilenced()
			and hAbility:CastFilterResultTarget(hTarget) == UF_SUCCESS
		then
			self.bActive = true
...
function modifier_frostivus2018_huskar_burning_spear_lua:OnAttack(params)
	if self.tRecords[params.record] ~= nil then
		...
		if IsValid(hParent) and hParent:IsAlive() and hAttacker == hParent then
			...
			hParent:ModifyHealth(
				hParent:GetHealth() - hp_lose,
				hAbility,
				false,
...
function modifier_frostivus2018_huskar_burning_spear_lua_debuff:OnIntervalThink()
	...
	--先打伤害
	if
		GameRules:GetGameTime() > self.LastBurnTime + self.tick_rate
		and IsValid(hCaster)
		and IsValid(hAbility)
		and hParent:IsAlive()
	then
