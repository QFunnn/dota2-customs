...
function modifier_frostivus2018_huskar_burning_spear_lua:GetModifierProcAttack_Feedback(params)
	...
	then ---@cast ability CDOTABaseAbility
...
function modifier_frostivus2018_huskar_burning_spear_lua:OnAttackStart(params)
	...
	if
		...
		and target:IsAlive()
		and attacker == parent
	then ---@cast ability frostivus2018_huskar_burning_spear_lua
		if
			(parent:GetCurrentActiveAbility() == ability or ability:GetAutoCastState())
			and not parent:IsSilenced()
			and ability:CastFilterResultTarget(target) == UF_SUCCESS
		then
			self.bActive = true
