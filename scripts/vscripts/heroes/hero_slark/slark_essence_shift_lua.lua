...
LinkLuaModifier(
	"modifier_slark_essence_shift_lua",
	"heroes/hero_slark/slark_essence_shift_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_essence_shift_lua_agi",
	"heroes/hero_slark/slark_essence_shift_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_essence_shift_lua_agi_permenant",
	"heroes/hero_slark/slark_essence_shift_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)

slark_essence_shift_lua = class({}) ---@class slark_essence_shift_lua : CDOTA_Ability_Lua
...
--------------------------------------------------------------------------------
modifier_slark_essence_shift_lua = class({}) ---@class modifier_slark_essence_shift_lua : CDOTA_Modifier_Lua
...
function modifier_slark_essence_shift_lua:OnDeath(keys)
	...
	if
		unit_killed
		and unit_killed:IsRealHero()
		and self.parent:GetTeamNumber() ~= unit_killed:GetTeamNumber()
		and (not unit_killed:IsIllusion())
		and (not unit_killed:IsTempestDouble())
		and (not unit_killed:HasModifier("modifier_arc_warden_tempest_double_lua"))
	then
		...
		if flDistance < 1000 then
			if self.parent:HasModifier("modifier_slark_essence_shift_lua_agi_permenant") then
				local nStack =
					self.parent:GetModifierStackCount("modifier_slark_essence_shift_lua_agi_permenant", self.parent)
				self.parent
					:FindModifierByName("modifier_slark_essence_shift_lua_agi_permenant")
					:SetStackCount(nStack + 1)
			else
				local hModifier = self.parent:AddNewModifier(
					self.parent,
					self.ability,
					"modifier_slark_essence_shift_lua_agi_permenant",
					{}
...
function modifier_slark_essence_shift_lua:GetModifierProcAttack_Feedback(params)
	...
	if
		not (
			IsValid(hParent)
			and IsValid(hTarget)
			and not hParent:PassivesDisabled()
			and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber()
			and not hParent:HasModifier("modifier_hero_refreshing")
		)
	then
		return
	end

	if hParent:IsIllusion() or hTarget:IsIllusion() then
		return
	end
	...
	local particle_cast =
		ParticleManager:GetParticleReplacement("particles/units/heroes/hero_slark/slark_essence_shift.vpcf", hParent)
	...
	local stacks = ability:GetSpecialValueFor("creep_agi_gain")

	if hTarget:IsRealHero() then
		stacks = ability:GetSpecialValueFor("agi_gain")
	end

	hParent:AddNewModifier(hParent, ability, "modifier_slark_essence_shift_lua_agi", {
...
----------------------------------------------------------
modifier_slark_essence_shift_lua_agi = class({}) ---@class modifier_slark_essence_shift_lua_agi : CDOTA_Modifier_Lua

function modifier_slark_essence_shift_lua_agi:GetTexture()
...
function modifier_slark_essence_shift_lua_agi:IsDebuff()
	return false
end

function modifier_slark_essence_shift_lua_agi:IsPurgable()
	return false
end

function modifier_slark_essence_shift_lua_agi:GetAttributes()
...
function modifier_slark_essence_shift_lua_agi:OnCreated(params)
	if IsServer() then
		...
		table.insert(
			self.tStack,
			{ fDieTime = GameRulesCustom:GetGameTime() + (params.duration or 0), iCount = iCount }
		)
...
---------------------------------------------------------------------------
modifier_slark_essence_shift_lua_agi_permenant = class({}) ---@class modifier_slark_essence_shift_lua_agi_permenant : CDOTA_Modifier_Lua

function modifier_slark_essence_shift_lua_agi_permenant:GetTexture()
	return "slark_essence_shift"
end
...
function modifier_slark_essence_shift_lua_agi_permenant:IsDebuff()
	return false
end

function modifier_slark_essence_shift_lua_agi_permenant:IsPurgable()
	return false
end

function modifier_slark_essence_shift_lua_agi_permenant:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT
end
