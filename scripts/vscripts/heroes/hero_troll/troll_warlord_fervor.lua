...
LinkLuaModifier(
	...
	"heroes/hero_troll/troll_warlord_fervor",
...
function modifier_troll_warlord_fervor_custom:AttackLandedModifier(params)
	...
	if self:GetCaster():HasShard() then
		local chance = self:GetAbility():GetSpecialValueFor("base_chance")
			+ (self:GetStackCount() * self:GetAbility():GetSpecialValueFor("extra_attack_chance_per_stack"))
		local nearby_enemies = FindUnitsInRadius(
			self:GetParent():GetTeamNumber(),
			self:GetParent():GetAbsOrigin(),
			nil,
			self:GetParent():Script_GetAttackRange() + self:GetAbility():GetSpecialValueFor("range_buffer"),
		...
		if #nearby_enemies > 0 then
			self:GetParent()
				:PerformAttack(
					nearby_enemies[RandomInt(1, #nearby_enemies)],
					true,
					true,
					true,
					false,
