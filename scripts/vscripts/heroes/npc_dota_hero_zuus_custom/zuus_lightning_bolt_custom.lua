...
LinkLuaModifier(
	"modifier_zuus_lightning_bolt_custom_true_sight",
	"heroes/npc_dota_hero_zuus_custom/zuus_lightning_bolt_custom",
	...
	if nimbus then
		...
		if self:GetCaster():HasModifier("modifier_zuus_17") then
			...
			ParticleManager:SetParticleControl(
				...
				1,
				Vector(
					self:GetCaster():GetAoeBonus(self.modifier_zuus_17),
					self:GetCaster():GetAoeBonus(self.modifier_zuus_17),
					self:GetCaster():GetAoeBonus(self.modifier_zuus_17)
				)
			)
	...
	if self:GetCaster():HasModifier("modifier_zuus_16") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_zuus_16[self:GetCaster():GetTalentLevel("modifier_zuus_16")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_zuus_17") and not nimbus then
		...
		for _, target_unit in pairs(units) do
			...
			target_unit:AddNewModifier(
				...
				self,
				"modifier_stunned",
				{ duration = stun_duration * (1 - target_unit:GetStatusResistance()) }
			)
			ApplyDamage({
				attacker = caster,
				ability = self,
...
function modifier_zuus_lightning_bolt_custom_true_sight:IsHidden()
	return true
end
function modifier_zuus_lightning_bolt_custom_true_sight:IsPurgable()
	return false
end
