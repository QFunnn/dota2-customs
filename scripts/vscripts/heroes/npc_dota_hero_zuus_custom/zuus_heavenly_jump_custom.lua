...
LinkLuaModifier(
	"modifier_zuus_heavenly_jump_custom_debuff",
	"heroes/npc_dota_hero_zuus_custom/zuus_heavenly_jump_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_heavenly_jump_custom_double_jump",
	"heroes/npc_dota_hero_zuus_custom/zuus_heavenly_jump_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_heavenly_jump_custom_buff_magic_immune",
	"heroes/npc_dota_hero_zuus_custom/zuus_heavenly_jump_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_heavenly_jump_custom_handler",
	"heroes/npc_dota_hero_zuus_custom/zuus_heavenly_jump_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function zuus_heavenly_jump_custom:GetIntrinsicModifierName()
	if self:GetAbilityName() == "zuus_heavenly_jump_custom_immune" then
		return
	end
...
function zuus_heavenly_jump_custom:StartAoeAttack()
	local units = FindUnitsInRadius(
		...
		nil,
		self:GetCaster():GetAoeBonus(self.modifier_zuus_2_radius),
	...
	for _, target in pairs(units) do
		...
		local thunder = ParticleManager:CreateParticle(
	...
	local aoe = ParticleManager:CreateParticle(
...
function zuus_heavenly_jump_custom:StartAoeStun()
	...
	local units = FindUnitsInRadius(
		...
		self:GetCaster():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self.modifier_zuus_3_radius),
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		0,
		FIND_CLOSEST,
	...
	for _, target in pairs(units) do
		target:AddNewModifier(
			...
			"modifier_stunned",
			{
				duration = self.modifier_zuus_3[self:GetCaster():GetTalentLevel("modifier_zuus_3")]
...
function zuus_heavenly_jump_custom:Jump(point)
	...
	if
		...
		and modifier_generic_arc_lua.zuus_jump
		and self:GetCaster():HasModifier("modifier_zuus_1")
	then
		...
		if arc then
			...
			arc:SetEndCallback(function()
				...
				local modifier_zuus_heavenly_jump_custom_buff_magic_immune =
					self:GetCaster():FindModifierByName("modifier_zuus_heavenly_jump_custom_buff_magic_immune")
		...
		is_double_jump = true
		local modifier_zuus_heavenly_jump_custom_double_jump =
			self:GetCaster():FindModifierByName("modifier_zuus_heavenly_jump_custom_double_jump")
	...
	local units = {}
	local units_heroes = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		self:GetCaster():GetAbsOrigin(),
		nil,
		range,
	...
	local units_creeps = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		self:GetCaster():GetAbsOrigin(),
		nil,
		range,
...
function modifier_zuus_heavenly_jump_custom_debuff:OnCreated()
	...
	local particle_slow = ParticleManager:CreateParticle(
...
function modifier_zuus_heavenly_jump_custom_double_jump:IsHidden()
	return true
end
function modifier_zuus_heavenly_jump_custom_double_jump:IsPurgable()
	return false
end
function modifier_zuus_heavenly_jump_custom_double_jump:IsPurgeException()
	return false
end
...
function modifier_zuus_heavenly_jump_custom_buff_magic_immune:IsPurgable()
	return false
end
function modifier_zuus_heavenly_jump_custom_buff_magic_immune:GetTexture()
	return "zuus_6"
end
...
function modifier_zuus_heavenly_jump_custom_handler:IsHidden()
	return true
end
function modifier_zuus_heavenly_jump_custom_handler:IsPurgable()
	return false
end
function modifier_zuus_heavenly_jump_custom_handler:IsPurgeException()
	return false
end
function modifier_zuus_heavenly_jump_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_zuus_heavenly_jump_custom_handler:OnDeath(params)
	...
	if self:GetCaster():HasModifier("modifier_zuus_4") then
		...
		if
			params.unit:HasModifier("modifier_wodacreepchampion")
			or params.unit:HasModifier("modifier_wodacreepchampionred")
		then
