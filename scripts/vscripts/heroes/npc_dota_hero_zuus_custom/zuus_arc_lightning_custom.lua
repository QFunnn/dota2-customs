...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_zuus_custom/zuus_arc_lightning_custom",
...
function zuus_arc_lightning_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_zuus.vpcf",
		context
	)
end
...
function zuus_arc_lightning_custom:StartArc(target, bounces, damage_reduce)
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_zuus_arc_lightning_custom",
		{ starting_unit_entindex = target:entindex(), bounces = bounces, damage_reduce = damage_reduce }
	)
end
...
function modifier_zuus_arc_lightning_custom:OnIntervalThink()
	...
	do
		if
			not self.units_affected[enemy]
			and enemy ~= self.current_unit
			and enemy:GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
		then
			local ligth_particle = "particles/units/heroes/hero_zuus/zuus_arc_lightning.vpcf"
			self.lightning_particle =
				ParticleManager:CreateParticle(ligth_particle, PATTACH_ABSORIGIN_FOLLOW, self.current_unit)
			ParticleManager:SetParticleControlEnt(
				self.lightning_particle,
				0,
				self.current_unit,
