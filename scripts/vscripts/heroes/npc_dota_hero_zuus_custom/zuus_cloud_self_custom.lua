...
LinkLuaModifier(
	"modifier_zuus_cloud_self_custom",
	"heroes/npc_dota_hero_zuus_custom/zuus_cloud_self_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_cloud_self_custom_handler",
	"heroes/npc_dota_hero_zuus_custom/zuus_cloud_self_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_zuus_cloud_self_custom_handler:IsPurgable()
	return false
end
function modifier_zuus_cloud_self_custom_handler:IsPurgeException()
	return false
end
function modifier_zuus_cloud_self_custom_handler:RemoveOnDeath()
	return false
end
function modifier_zuus_cloud_self_custom_handler:IsHidden()
	return true
end
function modifier_zuus_cloud_self_custom_handler:OnCreated()
	...
	self.zuus_nimbus_unit = CreateUnitByName(
		"npc_dota_zeus_cloud",
		Vector(self.target_point.x, self.target_point.y, 450),
		false,
		caster,
		nil,
		caster:GetTeam()
	...
	self.zuus_nimbus_unit:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_zuus_cloud_self_custom",
		{ cloud_bolt_interval = cloud_bolt_interval, cloud_radius = cloud_radius }
	)
end
...
function modifier_zuus_cloud_self_custom:IsHidden()
	return true
end
function modifier_zuus_cloud_self_custom:IsPurgable()
	return false
end
function modifier_zuus_cloud_self_custom:IsPurgeException()
	return false
end
...
function modifier_zuus_cloud_self_custom:OnCreated(keys)
	...
	ParticleManager:SetParticleControl(
		...
		1,
		Vector(self:GetCaster():GetAoeBonus(self.cloud_radius), 0, 0)
	)
...
function modifier_zuus_cloud_self_custom:OnIntervalThink()
	...
	if self.zuus_nimbus_particle then
		...
		ParticleManager:SetParticleControl(
			self.zuus_nimbus_particle,
			2,
			Vector(new_point.x, new_point.y, new_point.z + 450)
		)
	end
	...
	local nearby_enemy_units = FindUnitsInRadius(
		...
		self:GetParent():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self.cloud_radius),
	...
	if self.lightning_bolt and self.lightning_bolt:GetLevel() > 0 and self.counter >= self.cloud_bolt_interval then
		if #nearby_enemy_units > 0 then
			self.lightning_bolt:CastLightningBolt(
				self:GetCaster(),
				nearby_enemy_units[1],
				nearby_enemy_units[1]:GetAbsOrigin(),
				self:GetParent()
			)
			self.counter = 0
		end
	...
	local check_pigs = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		self:GetParent():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self.cloud_radius),
	...
	if #check_pigs > 0 then
		if
			check_pigs[1]:GetUnitName() == "npc_woda_pig"
			or check_pigs[1]:GetUnitName() == "npc_woda_frog"
			or check_pigs[1]:GetUnitName() == "npc_woda_pig_pve"
			or check_pigs[1]:GetUnitName() == "npc_woda_frog_pve"
			or check_pigs[1]:GetUnitName() == "npc_dota_weaver_swarm"
		then
