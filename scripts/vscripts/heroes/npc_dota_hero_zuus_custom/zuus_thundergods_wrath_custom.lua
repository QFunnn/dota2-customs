...
LinkLuaModifier(
	"modifier_zuus_thundergods_wrath_custom_fow",
	"heroes/npc_dota_hero_zuus_custom/zuus_thundergods_wrath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_thundergods_wrath_custom_buff_damage",
	"heroes/npc_dota_hero_zuus_custom/zuus_thundergods_wrath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_thundergods_wrath_custom_buff_magic_immune",
	"heroes/npc_dota_hero_zuus_custom/zuus_thundergods_wrath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_thundergods_wrath_custom_handler",
	"heroes/npc_dota_hero_zuus_custom/zuus_thundergods_wrath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_zuus_thundergods_wrath_custom_target_damage",
	"heroes/npc_dota_hero_zuus_custom/zuus_thundergods_wrath_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function zuus_thundergods_wrath_custom:OnAbilityPhaseStart()
	...
	self.thundergod_spell_cast =
		ParticleManager:CreateParticle(particle_start, PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
...
function zuus_thundergods_wrath_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_zuus_7") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_zuus_thundergods_wrath_custom_buff_damage",
			{ duration = self.modifier_zuus_7_duration }
		)
	end
	if self:GetCaster():HasModifier("modifier_zuus_13") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_zuus_thundergods_wrath_custom_buff_magic_immune",
			{ duration = self.modifier_zuus_13[self:GetCaster():GetTalentLevel("modifier_zuus_13")] }
		)
	end
	...
	local modifier_zuus_thundergods_wrath_custom_handler =
		self:GetCaster():FindModifierByName("modifier_zuus_thundergods_wrath_custom_handler")
	if
		modifier_zuus_thundergods_wrath_custom_handler
		and modifier_zuus_thundergods_wrath_custom_handler:GetStackCount() > 0
		and self:GetCaster():HasModifier("modifier_zuus_18")
	then
		damage = damage
			+ (
				self.modifier_zuus_18_damage[self:GetCaster():GetTalentLevel("modifier_zuus_18")]
				* modifier_zuus_thundergods_wrath_custom_handler:GetStackCount()
			)
	end
	local damage_table = {
	...
	for _, hero in pairs(units) do
		if hero and hero:IsAlive() and not hero:IsNull() then
			...
			if
				(not hero:IsMagicImmune())
				and not hero:IsInvulnerable()
				and not hero:IsOutOfGame()
				and (not hero:IsInvisible() or caster:CanEntityBeSeenByMyTeam(hero))
			then
...
function modifier_zuus_thundergods_wrath_custom_fow:IsHidden()
	return true
end
function modifier_zuus_thundergods_wrath_custom_fow:IsPurgable()
	return false
end
...
function modifier_zuus_thundergods_wrath_custom_fow:OnIntervalThink()
	...
	AddFOWViewer(
		...
		self:GetAbility():GetSpecialValueFor("sight_radius_day"),
		FrameTime() * 2,
		true
	)
end
...
function modifier_zuus_thundergods_wrath_custom_buff_damage:IsPurgable()
	return false
end
function modifier_zuus_thundergods_wrath_custom_buff_damage:GetTexture()
	return "zuus_7"
end
...
function modifier_zuus_thundergods_wrath_custom_buff_magic_immune:IsPurgable()
	return false
end
function modifier_zuus_thundergods_wrath_custom_buff_magic_immune:GetTexture()
	return "zuus_13"
end
...
function modifier_zuus_thundergods_wrath_custom_handler:IsHidden()
	return not self:GetCaster():HasModifier("modifier_zuus_18")
end
function modifier_zuus_thundergods_wrath_custom_handler:GetTexture()
	return "zuus_18"
end
function modifier_zuus_thundergods_wrath_custom_handler:IsPurgable()
	return false
end
function modifier_zuus_thundergods_wrath_custom_handler:IsPurgeException()
	return false
end
function modifier_zuus_thundergods_wrath_custom_handler:RemoveOnDeath()
	return false
end

modifier_zuus_thundergods_wrath_custom_target_damage = class({})
function modifier_zuus_thundergods_wrath_custom_target_damage:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_zuus_thundergods_wrath_custom_target_damage:IsHidden()
	return true
end
function modifier_zuus_thundergods_wrath_custom_target_damage:IsPurgable()
	return false
end
