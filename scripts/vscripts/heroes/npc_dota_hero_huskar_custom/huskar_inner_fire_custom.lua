...
LinkLuaModifier(
	"modifier_huskar_inner_fire_custom",
	"heroes/npc_dota_hero_huskar_custom/huskar_inner_fire_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_huskar_inner_fire_custom_debuff",
	"heroes/npc_dota_hero_huskar_custom/huskar_inner_fire_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_generic_knockback_lua", "modifiers/modifier_generic_knockback_lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier(
	"modifier_huskar_inner_fire_custom_handler",
	"heroes/npc_dota_hero_huskar_custom/huskar_inner_fire_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_huskar_inner_fire_custom_rooted",
	"heroes/npc_dota_hero_huskar_custom/huskar_inner_fire_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function huskar_inner_fire_custom:GetManaCost(level)
	if not self:GetCaster():HasModifier("modifier_huskar_15") then
		return
	end
...
function huskar_inner_fire_custom:UseInnerFire(point)
	...
	for _, unit in pairs(units) do
		...
		local callback = function()
			if self:GetCaster():HasModifier("modifier_huskar_11") then
				unit:AddNewModifier(
					...
					"modifier_huskar_inner_fire_custom_rooted",
					{
						duration = self.modifier_huskar_11[self:GetCaster():GetTalentLevel("modifier_huskar_11")]
		...
		if self:GetCaster():HasModifier("modifier_huskar_17") then
			local stack_count = self:GetCaster():GetMana()
				/ self.modifier_huskar_17[self:GetCaster():GetTalentLevel("modifier_huskar_17")]
		...
		unit:AddNewModifier(
			...
			self,
			"modifier_huskar_inner_fire_custom_debuff",
			{ duration = disarm_duration * (1 - unit:GetStatusResistance()) }
		)
	end
...
function modifier_huskar_inner_fire_custom_rooted:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/huskar_root_inner_fire.vpcf",
...
function modifier_huskar_inner_fire_custom_handler:IsHidden()
	return true
end
function modifier_huskar_inner_fire_custom_handler:IsPurgable()
	return false
end
function modifier_huskar_inner_fire_custom_handler:IsPurgeException()
	return false
end
function modifier_huskar_inner_fire_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_huskar_inner_fire_custom_handler:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_huskar_12") then
		self:SetStackCount(
			math.floor(self:GetCaster():GetDisplayAttackSpeed() / 100 * self:GetAbility().modifier_huskar_12)
		)
	end
end