...
LinkLuaModifier(
	...
	"modifiers/modifier_generic_orb_effect_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_huskar_burning_spear_custom_debuff",
	"heroes/npc_dota_hero_huskar_custom/huskar_burning_spear_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function huskar_burning_spear_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_huskar.vpcf",
		context
	)
end
...
function huskar_burning_spear_custom:GetHealthCost(level)
	...
	return (self:GetSpecialValueFor("health_cost") * self:GetCaster():GetMaxHealth() / 100)
		* (1 - self:GetCaster():Script_GetMagicalArmorValue(self))
end
...
function huskar_burning_spear_custom:OnOrbFire()
	if self:GetCaster():HasModifier("modifier_huskar_10") then
		return
	end
...
function modifier_huskar_burning_spear_custom_debuff:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_huskar_20")
end
...
function modifier_huskar_burning_spear_custom_debuff:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_huskar_20") then
		AddFOWViewer(
			...
			self:GetAbility().modifier_huskar_20,
			FrameTime(),
			true
		)
	end
	...
	if self.burn_tick >= 1 then
		local burn_damage = self:GetAbility():GetSpecialValueFor("burn_damage")
			+ (self:GetParent():GetMaxHealth() / 100 * self.burn_damage_max_pct)
		if self:GetCaster():HasModifier("modifier_huskar_16") then
			burn_damage = burn_damage
				+ self:GetAbility().modifier_huskar_16_damage[self:GetCaster():GetTalentLevel("modifier_huskar_16")]
		end
		ApplyDamage({
			...
			damage = burn_damage * self:GetStackCount(),
		})
		self.burn_tick = 0
	end
end
...
function modifier_huskar_burning_spear_custom_debuff:GetModifierMagicalResistanceBonus()
	if self:GetCaster():HasModifier("modifier_huskar_19") then
		return self:GetAbility().modifier_huskar_19[self:GetCaster():GetTalentLevel("modifier_huskar_19")]
			* self:GetStackCount()
	end
end
