...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_huskar_custom/huskar_life_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_huskar_life_break_custom_debuff",
	"heroes/npc_dota_hero_huskar_custom/huskar_life_break_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function huskar_life_break_custom:OnSpellStart()
	...
	local modifier_huskar_life_break_custom = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_huskar_life_break_custom",
		{ target = target:entindex(), duration = 10 }
	)
end

modifier_huskar_life_break_custom_debuff_immune = class({})
function modifier_huskar_life_break_custom_debuff_immune:IsHidden()
	return true
end
function modifier_huskar_life_break_custom_debuff_immune:IsPurgable()
	return false
end
...
function modifier_huskar_life_break_custom:IsHidden()
	return true
end
function modifier_huskar_life_break_custom:IsPurgable()
	return false
end
function modifier_huskar_life_break_custom:OnCreated(params)
	...
	if self:GetParent():HasModifier("modifier_huskar_6_buff") then
		self.health_damage = self.health_damage
			+ (self:GetParent():GetModifierStackCount("modifier_huskar_6_buff", self:GetParent()) / 100)
	end
...
function modifier_huskar_life_break_custom:UpdateHorizontalMotion(me, dt)
	...
	if
		(self.target and (self.target:GetAbsOrigin() - me:GetOrigin()):Length2D() <= 128)
		or (self.target and (self.target:GetAbsOrigin() - me:GetAbsOrigin()):Length2D() > self.break_range)
		or (not me:IsDebuffImmune() and me:IsStunned())
		or (self.target and not self.target:IsAlive())
	then
		self:Destroy()
		return
	end
		...
		if not self:GetCaster():HasModifier("modifier_huskar_14") then
			if self:GetCaster():HasModifier("modifier_huskar_15") then
				...
				ApplyDamage({
					...
					damage = self.health_cost_percent * self:GetParent():GetHealth(),
...
function modifier_huskar_life_break_custom:ImpactEffect(target)
	...
	if self:GetCaster():HasModifier("modifier_huskar_7") then
		target:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_huskar_life_break_custom_debuff_aggresive",
			{ duration = self:GetAbility().modifier_huskar_7 * (1 - target:GetStatusResistance()) }
		)
	end
end
...
function modifier_huskar_life_break_custom_debuff:IsHidden()
	return false
end
function modifier_huskar_life_break_custom_debuff:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_huskar_13")
end
...
function modifier_huskar_life_break_custom_debuff_aggresive:IsPurgable()
	return false
end
