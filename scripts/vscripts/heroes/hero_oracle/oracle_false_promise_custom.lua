...
LinkLuaModifier(
	...
	"heroes/hero_oracle/oracle_false_promise_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_oracle_false_promise_custom_handler",
	"heroes/hero_oracle/oracle_false_promise_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function oracle_false_promise_custom:ApplyFalsePromise(target)
	...
	self.false_promise_cast_particle = ParticleManager:CreateParticle(
	...
	self.false_promise_target_particle = ParticleManager:CreateParticle(
...
function modifier_oracle_false_promise_custom:DestroyOnExpire()
	return not self:GetParent():IsInvulnerable()
end
function modifier_oracle_false_promise_custom:GetPriority()
	return MODIFIER_PRIORITY_ULTRA
end
function modifier_oracle_false_promise_custom:IsPurgable()
	return false
end
...
function modifier_oracle_false_promise_custom:OnCreated()
	...
	self.flBaseAttackTime = GetUnitBaseAttackTime(self:GetParent())
		- self:GetAbility():GetSpecialValueFor("shard_bat_bonus")
...
function modifier_oracle_false_promise_custom:OnDestroy()
	...
	if self:GetParent():HasModifier("modifier_hero_refreshing") then
		return
	end
	if self:GetParent():HasModifier("modifier_duel_teleporting") then
		return
	end

	if self.damage_counter < self.heal_counter then
		self:GetParent():EmitSound("Hero_Oracle.FalsePromise.Healed")
		self.end_particle = ParticleManager:CreateParticle(
		...
		for _, instance in pairs(self.damage_instances) do
			if not instance.attacker or instance.attacker:IsNull() then
				instance.attacker = self:GetCaster() and not self:GetCaster():IsNull() and self:GetCaster()
					or self:GetParent()
			end
			if self.heal_counter > 0 then
				...
				local hp_before = self:GetParent():GetHealth()
				ApplyDamage(instance)
				local actual_dealt = hp_before - self:GetParent():GetHealth()
				-- Borrowed Time гарантированно защищает: если его баф активен, урон уже поглощён
				-- и превращён в лечение -> НЕ добиваем принудительно (иначе FORCE-ветка пробивала бы BT).
				if
					actual_dealt < instance.damage
					and self:GetParent():IsAlive()
					and not self:GetParent():HasModifier("modifier_abaddon_borrowed_time_custom_buff")
				then
					local missing = instance.damage - actual_dealt
					local new_hp = self:GetParent():GetHealth() - missing
					if new_hp <= 0 then
						self:GetParent():Kill(self:GetAbility(), instance.attacker)
					else
...
function modifier_oracle_false_promise_custom:OnTakeDamage(params)
	if params.unit == self:GetParent() then
		self:HealHero(params)
	end
end

function modifier_oracle_false_promise_custom:OnAbilityHealRecive(keys)
	if keys.unit == self:GetParent() and self:GetRemainingTime() >= 0 then
		self.heal_counter = self.heal_counter + (keys.gain * 2)
		ParticleManager:SetParticleControl(
			self.overhead_particle,
			1,
			Vector(self.damage_counter - self.heal_counter, 0, 0)
		)
		ParticleManager:SetParticleControl(
			self.overhead_particle,
			2,
			Vector(self.heal_counter - self.damage_counter, 0, 0)
		)
	end
end

-- function modifier_oracle_false_promise_custom:GetAbsoluteNoDamagePhysical(params)
--     self:HealHero(params)
-- 	return 1
-- end

-- function modifier_oracle_false_promise_custom:GetAbsoluteNoDamageMagical(params)
--     self:HealHero(params)
-- 	return 1
-- end

-- function modifier_oracle_false_promise_custom:GetAbsoluteNoDamagePure(params)
--     self:HealHero(params)
-- 	return 1
-- end

function modifier_oracle_false_promise_custom:HealHero(params)
	if not IsServer() then
		return
	end
...
function modifier_oracle_false_promise_custom:AttackModifier(keys)
	if
		self:GetCaster():HasScepter()
		and keys.attacker == self:GetParent()
		and not keys.no_attack_cooldown
		and self.invis_modifier
		and not self.invis_modifier:IsNull()
	then
		self.invis_modifier:Destroy()
		self:StartIntervalThink(self:GetAbility():GetSpecialValueFor("shard_fade_time"))
	end
...
function modifier_oracle_false_promise_custom_handler:IsHidden()
	return true
end
function modifier_oracle_false_promise_custom_handler:IsPurgable()
	return false
end
...
function modifier_oracle_false_promise_custom_handler:OnIntervalThink()
	...
	if
		self:GetParent():HasModifier("modifier_phoenix_sun")
		or self:GetParent():HasModifier("modifier_ability_phoenix_supernova_hiding")
	then
