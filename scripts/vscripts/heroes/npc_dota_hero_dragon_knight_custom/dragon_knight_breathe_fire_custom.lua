...
LinkLuaModifier(
	"modifier_dragon_knight_breathe_fire_custom",
	"heroes/npc_dota_hero_dragon_knight_custom/dragon_knight_breathe_fire_custom",
...
function dragon_knight_breathe_fire_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_dragon_knight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_dragon_knight.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_dragon_knight.vpcf",
		context
	)
end
...
function dragon_knight_breathe_fire_custom:GetCooldown(level)
	...
	if self:GetCaster():HasModifier("modifier_dragon_knight_15") then
		bonus = self.modifier_dragon_knight_15_cooldown_reduce[self:GetCaster()
...
function dragon_knight_breathe_fire_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_dragon_knight_9") then
		projectile_distance = projectile_distance
			+ self.modifier_dragon_knight_9_cast_range[self:GetCaster():GetTalentLevel("modifier_dragon_knight_9")]
	end
...
function dragon_knight_breathe_fire_custom:OnProjectileHit(target, location)
	...
	if self:GetCaster():HasModifier("modifier_dragon_knight_16") then
		damage = damage
			+ self.modifier_dragon_knight_16_bonus_damage[self:GetCaster():GetTalentLevel("modifier_dragon_knight_16")]
	end
...
function modifier_dragon_knight_breathe_fire_custom:GetModifierMoveSpeedBonus_Percentage()
	if not self:GetCaster():HasModifier("modifier_dragon_knight_10") then
		return 0
	end
