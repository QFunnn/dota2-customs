...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dragon_knight_custom/dragon_knight_dragon_tail_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dragon_knight_dragon_tail_custom_armor",
	"heroes/npc_dota_hero_dragon_knight_custom/dragon_knight_dragon_tail_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dragon_knight_dragon_tail_custom_armor_debuff",
	"heroes/npc_dota_hero_dragon_knight_custom/dragon_knight_dragon_tail_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function dragon_knight_dragon_tail_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_dragon_knight_5") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_dragon_knight_dragon_tail_custom_armor",
			{ duration = self.modifier_dragon_knight_5_duration }
		)
	end
	if self:GetCaster():HasModifier("modifier_dragon_knight_7") then
...
function dragon_knight_dragon_tail_custom:Hit(target, dragonform, aoe)
	...
	if self:GetCaster():HasModifier("modifier_dragon_knight_6") then
		target:AddNewModifier(
			...
			self,
			"modifier_dragon_knight_dragon_tail_custom_armor_debuff",
			{ duration = self.modifier_dragon_knight_6_duration * (1 - target:GetStatusResistance()) }
		)
	end
...
function modifier_dragon_knight_dragon_tail_custom_armor:GetModifierPhysicalArmorBonus()
	return self:GetAbility().modifier_dragon_knight_5_bonus_armor[self:GetCaster()
		:GetTalentLevel("modifier_dragon_knight_5")]
end
