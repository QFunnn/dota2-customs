...
LinkLuaModifier(
	"modifier_void_spirit_astral_step_custom",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_step_custom_handler",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_step_custom_thinker",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_step_custom_thinker_debuff",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_step_custom_10_buff",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function void_spirit_astral_step_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_void_spirit/astral_step/void_spirit_astral_step_thinker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
			...
			Timers:CreateTimer(FrameTime(), function()
				if self:GetCurrentAbilityCharges() < self:GetMaxAbilityCharges(self:GetLevel()) then
					...
					local particle = ParticleManager:CreateParticle(
						"particles/items2_fx/refresher_void_spirit.vpcf",
						PATTACH_ABSORIGIN_FOLLOW,
						self:GetCaster()
					)
...
function void_spirit_astral_step_custom:Strike(point)
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_12") then
		CreateModifierThinker(
			...
			"modifier_void_spirit_astral_step_custom_thinker",
			{
				point_start_x = origin.x,
				point_start_y = origin.y,
				point_start_z = origin.z,
				point_end_x = target.x,
				point_end_y = target.y,
				point_end_z = target.z,
				duration = self.modifier_void_spirit_12,
			},
			origin,
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_7") and void_spirit_aether_remnant_custom then
		...
		CreateModifierThinker(
			...
			origin - self:GetCaster():GetForwardVector() * 10,
			self:GetCaster():GetTeamNumber(),
			false
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_10") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_void_spirit_astral_step_custom_10_buff",
			{ duration = self.modifier_void_spirit_10_duration }
		)
	end
end
...
function modifier_void_spirit_astral_step_custom_thinker:OnIntervalThink()
	...
	local enemies = FindUnitsInLine(
		...
		self.target,
		nil,
		self:GetCaster():GetAoeBonus(self:GetAbility().modifier_void_spirit_12_width),
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES
	)
...
function modifier_void_spirit_astral_step_custom:IsHidden()
	return false
end
function modifier_void_spirit_astral_step_custom:IsDebuff()
	return true
end
function modifier_void_spirit_astral_step_custom:IsStunDebuff()
	return false
end
function modifier_void_spirit_astral_step_custom:IsPurgable()
	return true
end
function modifier_void_spirit_astral_step_custom:GetAttributes()
...
function modifier_void_spirit_astral_step_custom_handler:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_custom_handler:IsPurgable()
	return false
end
function modifier_void_spirit_astral_step_custom_handler:IsPurgeException()
	return false
end
function modifier_void_spirit_astral_step_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_void_spirit_astral_step_custom_handler:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_void_spirit_9") then
		return
	end
	if
		RollPercentage(
			self:GetAbility().modifier_void_spirit_9[self:GetCaster():GetTalentLevel("modifier_void_spirit_9")]
		)
	then
...
function modifier_void_spirit_astral_step_custom_10_buff:GetTexture()
	return "void_spirit_10"
end
...
function modifier_void_spirit_astral_step_custom_10_buff:OnIntervalThink()
	...
	AddFOWViewer(
		...
		self:GetAbility().modifier_void_spirit_10_radius,
		FrameTime(),
		false
	)
end
...
function modifier_void_spirit_astral_step_custom_thinker_debuff:IsPurgable()
	return false
end
function modifier_void_spirit_astral_step_custom_thinker_debuff:IsPurgeException()
	return false
end
...
function modifier_void_spirit_astral_step_custom_thinker_debuff:OnIntervalThink()
	...
	local damage = self:GetCaster():GetAgility()
		/ 100
		* self:GetAbility().modifier_void_spirit_12_damage[self:GetCaster():GetTalentLevel("modifier_void_spirit_12")]
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		damage = damage * 0.5,
