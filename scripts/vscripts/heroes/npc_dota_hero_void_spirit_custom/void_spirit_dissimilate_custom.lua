...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_dissimilate_custom",
...
function modifier_custom_void_dissimilate:OnCreated(kv)
	...
	if self:GetParent():HasModifier("modifier_void_spirit_17") then
		...
		self:GetParent():HealWithParams(
			self:GetAbility().modifier_void_spirit_17,
			self:GetAbility(),
			false,
			true,
			self:GetCaster(),
...
function modifier_custom_void_dissimilate:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_18") then
		damage = damage
			+ self:GetCaster():GetMana()
				/ 100
				* self:GetAbility().modifier_void_spirit_18[self:GetCaster():GetTalentLevel("modifier_void_spirit_18")]
	end
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_void_spirit_20") then
			local modifier_rooted = enemy:AddNewModifier(
				...
				"modifier_rooted",
				{
					duration = self:GetAbility().modifier_void_spirit_20[self:GetCaster()
	...
	if
		...
		and self:GetCaster():HasModifier("modifier_void_spirit_6")
		and void_spirit_aether_remnant_custom:GetLevel() > 0
		and self.selected ~= 1
	then
...
function modifier_custom_void_dissimilate:PlayEffects2(point, hit)
	...
	local particle_exit = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_void_spirit/dissimilate/void_spirit_dissimilate_exit.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetParent()
	)
