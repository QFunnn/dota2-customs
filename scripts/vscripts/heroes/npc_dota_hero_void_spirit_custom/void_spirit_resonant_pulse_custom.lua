...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_resonant_pulse_silence",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_void_spirit_resonant_pulse:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_void_spirit_8")
end
...
function modifier_void_spirit_resonant_pulse:CreateWave(caster, new_damage)
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_15") then
		self.damage = self.damage
			+ (
				(self:GetCaster():GetStrength() + self:GetCaster():GetAgility() + self:GetCaster():GetIntellect(false))
				/ 100
				* self:GetAbility().modifier_void_spirit_15[self:GetCaster():GetTalentLevel("modifier_void_spirit_15")]
			)
	end
	...
	pulse:SetCallback(function(enemy)
		...
		if caster:HasModifier("modifier_void_spirit_11") then
			enemy:AddNewModifier(
				...
				"modifier_void_spirit_resonant_pulse_silence",
				{
					duration = ability.modifier_void_spirit_11[caster:GetTalentLevel("modifier_void_spirit_11")]
						* (1 - enemy:GetStatusResistance()),
				}
			)
		end
		if current_buff and current_buff:IsNull() then
			current_buff = caster:AddNewModifier(
				caster,
				ability,
				"modifier_void_spirit_resonant_pulse",
				{ duration = duration, refresh = 1 }
			)
		end
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_8") then
		...
		self.max_shield = self.max_shield
			+ (
				...
				/ 100
				* self:GetAbility().modifier_void_spirit_8[self:GetCaster():GetTalentLevel("modifier_void_spirit_8")]
			)
	end
...
function modifier_void_spirit_resonant_pulse_silence:GetTexture()
	return "void_spirit_11"
end
