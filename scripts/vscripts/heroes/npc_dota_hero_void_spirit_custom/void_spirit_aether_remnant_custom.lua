...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_aether_remnant_custom",
...
LinkLuaModifier(
	"modifier_custom_void_remnant_buff",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_aether_remnant_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function void_spirit_aether_remnant_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_void_spirit/aether_remnant/void_spirit_aether_remnant_run_red.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_void_spirit/aether_remnant/void_spirit_aether_remnant_pre_red.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_void_spirit/aether_remnant/void_spirit_aether_remnant_watch_red.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_void_spirit.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_void_spirit.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_void_spirit.vpcf",
		context
	)
end
...
function void_spirit_aether_remnant_custom:OnVectorCastStart(vStartLocation, vDirection)
	...
	local thinker = CreateModifierThinker(
		...
		self,
		"modifier_custom_void_remnant_thinker",
		{ dir_x = vDirection.x, dir_y = vDirection.y, dir_z = vDirection.z, main = 1 },
		point,
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_3") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_custom_void_remnant_buff",
			{ duration = self.modifier_void_spirit_3 }
		)
	end
end
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_2") then
		self.damage = self.damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_void_spirit_2[self:GetCaster():GetTalentLevel("modifier_void_spirit_2")]
			)
	end
...
function modifier_custom_void_remnant_thinker:CheckoutPosition(start_point, direction)
	...
	AddFOWViewer(
		...
		start_point + direction * self.distance / 2,
		self.watch_vision,
		0.1,
		true
	)
	...
	local enemies_heroes = FindUnitsInLine(
		...
		if self.creeps_count >= 0.5 then
			...
			for _, creep in pairs(enemies_heroes) do
				local damageTable = {
					...
					damage = (self.damage * self:GetAbility().modifier_void_spirit_1 / 100) * 0.5,
...
function modifier_custom_void_remnant_thinker:PlayEffects4(target, illusion)
	...
	ParticleManager:SetParticleControl(
		effect_cast,
		2,
		Vector(target:GetAbsOrigin().x, target:GetAbsOrigin().y, target:GetAbsOrigin().z + 150)
	)
...
function modifier_custom_void_remnant_target:GetPriority()
	return 20
end
...
function modifier_custom_void_remnant_buff:GetTexture()
	return "void_spirit_3"
end
