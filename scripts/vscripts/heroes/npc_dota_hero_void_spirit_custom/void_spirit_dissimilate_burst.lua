...
LinkLuaModifier(
	"modifier_void_spirit_dissimilate_burst",
	"heroes/npc_dota_hero_void_spirit_custom/void_spirit_dissimilate_burst",
...
function void_spirit_dissimilate_burst:OnSpellStart()
	...
	if not void_spirit_dissimilate_custom then
		return
	end
...
function modifier_void_spirit_dissimilate_burst:OnCreated()
	...
	ParticleManager:SetParticleControl(
		effect_cast,
		0,
		GetGroundPosition(self:GetParent():GetAbsOrigin(), self:GetParent())
	)
	...
	EmitSoundOnLocationWithCaster(
		...
		"Hero_VoidSpirit.Dissimilate.Portals",
		self:GetCaster()
	)
end
...
function modifier_void_spirit_dissimilate_burst:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_void_spirit_18") then
		damage = damage
			+ self:GetCaster():GetMana()
				/ 100
				* self.void_spirit_dissimilate_custom.modifier_void_spirit_18[self:GetCaster()
					:GetTalentLevel("modifier_void_spirit_18")]
	end
	...
	for _, enemy in pairs(enemies) do
		...
		ApplyDamage({
			...
			damage = deal_damage,
			damage_type = self.void_spirit_dissimilate_custom:GetAbilityDamageType(),
			ability = self.void_spirit_dissimilate_custom,
		})
		if self:GetCaster():HasModifier("modifier_void_spirit_20") then
			local modifier_rooted = enemy:AddNewModifier(
				self:GetCaster(),
				self.void_spirit_dissimilate_custom,
				"modifier_rooted",
				{
					duration = self.void_spirit_dissimilate_custom.modifier_void_spirit_20[self:GetCaster()
						:GetTalentLevel("modifier_void_spirit_20")] * (1 - enemy:GetStatusResistance()),
				}
			)
			if modifier_rooted then
				local pfx = ParticleManager:CreateParticle(
					"particles/void_custom/oracle_fortune_purge_root_pnt.vpcf",
