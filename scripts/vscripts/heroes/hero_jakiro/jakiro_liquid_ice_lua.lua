...
function jakiro_liquid_ice_lua:OnSpellStart()
	...
	local info = {
		...
		flExpireTime = GameRulesCustom:GetGameTime() + 10,
...
function jakiro_liquid_ice_lua:OnProjectileHit(target, location)
	...
	--7.30: жидкий лёд добавляет оглушение
...
function modifier_liquid_ice_lua:OnCreated()
	...
	self.pct_health_damage = self:GetAbilitySpecialValueFor("pct_health_damage")
	if IsServer() then
		self:StartIntervalThink(1.0)
	end
end
function modifier_liquid_ice_lua:OnRefresh()
	self.base_damage = self:GetAbilitySpecialValueFor("base_damage")
	self.pct_health_damage = self:GetAbilitySpecialValueFor("pct_health_damage")
end
function modifier_liquid_ice_lua:OnIntervalThink()
	...
	if IsValid(hParent) then
		dmg = dmg + hParent:GetMaxHealth() * self.pct_health_damage * 0.01
		local damageTable = {
