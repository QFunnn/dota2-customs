...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_spirit_siphon_custom",
...
function death_prophet_spirit_siphon_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_death_prophet_spirit_siphon_counter", { duration = duration })
	...
	if self:GetCaster():HasModifier("modifier_death_prophet_8") then
		...
		local enemies = FindUnitsInRadius(
			...
			nil,
			self.modifier_death_prophet_8_radius,
			DOTA_UNIT_TARGET_TEAM_ENEMY,
			DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		...
		for _, enemy in pairs(enemies) do
			if enemy ~= target and not enemy:HasModifier("modifier_death_prophet_spirit_siphon_custom_debuff") then
				...
				self:GetCaster():AddNewModifier(
					...
					self,
					"modifier_death_prophet_spirit_siphon_custom",
					{ duration = duration, target = enemy:entindex() }
				)
				self:GetCaster():AddNewModifier(
					self:GetCaster(),
					self,
				...
				count = count + 1
				if
					count >= self.modifier_death_prophet_8[self:GetCaster():GetTalentLevel("modifier_death_prophet_8")]
				then
					break
				end
			end
...
function modifier_death_prophet_spirit_siphon_counter:IsPurgable()
	return false
end
...
function modifier_death_prophet_spirit_siphon_custom:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_death_prophet_spirit_siphon_custom:IsHidden()
	return true
end
function modifier_death_prophet_spirit_siphon_custom:IsPurgable()
	return false
end
...
function modifier_death_prophet_spirit_siphon_custom:OnIntervalThink()
	...
	if
		self.target
		and not self.target:IsNull()
		and (
			not self.target:IsAlive()
			or not self.target:HasModifier("modifier_death_prophet_spirit_siphon_custom_debuff")
		)
	then
		self:Destroy()
		return
	...
	local max_distance = self:GetAbility():GetCastRange(self:GetCaster():GetAbsOrigin(), self:GetCaster())
		+ self:GetCaster():GetCastRangeBonus()
		+ self:GetAbility():GetSpecialValueFor("siphon_buffer")
...
function modifier_death_prophet_spirit_siphon_custom_debuff:IsPurgable()
	return false
end
...
function modifier_death_prophet_spirit_siphon_custom_debuff:OnCreated()
	...
	if death_prophet_exorcism_custom and death_prophet_exorcism_custom:GetLevel() > 0 then
		if self:GetCaster():HasModifier("modifier_death_prophet_11") then
			death_prophet_exorcism_custom:CastGhostToTarget(
				self:GetParent(),
				death_prophet_exorcism_custom.modifier_death_prophet_13[self:GetCaster()
					:GetTalentLevel("modifier_death_prophet_11")]
			)
		end
	end
end
...
function modifier_death_prophet_spirit_siphon_custom_debuff:OnIntervalThink()
	...
	if
		self:GetCaster():HasModifier("modifier_death_prophet_18")
		and self.fear_active
	...
	then
		...
		if self.fear >= 3 then
			...
			self:GetParent():AddNewModifier(
				...
				{
					duration = self:GetAbility().modifier_death_prophet_18[self:GetCaster()
						:GetTalentLevel("modifier_death_prophet_18")] * (1 - self:GetParent():GetStatusResistance()),
				}
			)
		end
	end
