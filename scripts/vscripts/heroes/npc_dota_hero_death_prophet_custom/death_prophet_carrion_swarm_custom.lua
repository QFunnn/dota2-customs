...
LinkLuaModifier(
	"modifier_death_prophet_carrion_swarm_custom_back",
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_carrion_swarm_custom",
	LUA_MODIFIER_MOTION_NONE
)

death_prophet_carrion_swarm_custom = class({})

death_prophet_carrion_swarm_custom.modifier_death_prophet_16 = { -0.75, -1.5 }
death_prophet_carrion_swarm_custom.modifier_death_prophet_20 = { 4.5, 3 }
...
function death_prophet_carrion_swarm_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_death_prophet.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_death_prophet.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_death_prophet.vpcf",
		context
	)
end
		...
		if table.original == 1 then
			if self:GetCaster():HasModifier("modifier_death_prophet_20") then
				self:GetCaster():AddNewModifier(
					...
					"modifier_death_prophet_carrion_swarm_custom_back",
					{
						duration = self.modifier_death_prophet_20[self:GetCaster()
							:GetTalentLevel("modifier_death_prophet_20")],
						x = vLocation.x,
						y = vLocation.y,
					}
				)
...
function modifier_death_prophet_carrion_swarm_custom_back:IsPurgable()
	return false
end
function modifier_death_prophet_carrion_swarm_custom_back:IsHidden()
	return true
end
function modifier_death_prophet_carrion_swarm_custom_back:GetAttributes()
