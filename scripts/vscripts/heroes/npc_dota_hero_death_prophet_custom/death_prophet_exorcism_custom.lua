...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_exorcism_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_death_prophet_exorcism_custom_unit",
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_exorcism_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_death_prophet_exorcism_custom_damage_cast",
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_exorcism_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_death_prophet_exorcism_custom_unit_talent",
	"heroes/npc_dota_hero_death_prophet_custom/death_prophet_exorcism_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function death_prophet_exorcism_custom:OnSpellStart()
	...
	local modifier_death_prophet_exorcism_custom = self:GetCaster()
		:FindModifierByName("modifier_death_prophet_exorcism_custom")
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_death_prophet_exorcism_custom", { duration = duration })
end
...
function modifier_death_prophet_exorcism_custom:IsPurgable()
	return false
end
function modifier_death_prophet_exorcism_custom:DestroyOnExpire()
	return false
end
...
function modifier_death_prophet_exorcism_custom:OnCreated()
	...
	for i = 1, spirits do
		Timers:CreateTimer(i * ghost_spawn_rate, function()
			if not parent:HasModifier("modifier_death_prophet_exorcism_custom") then
				return
			end
			local unit = CreateUnitByName(
				"npc_dota_companion",
...
function modifier_death_prophet_exorcism_custom:OnIntervalThink()
	...
	if self.spirits_entindex and #self.spirits_entindex <= 0 then
		self:Destroy()
	end
...
function modifier_death_prophet_exorcism_custom_unit:IsHidden()
	return true
end
function modifier_death_prophet_exorcism_custom_unit:IsPurgable()
	return false
end
function modifier_death_prophet_exorcism_custom_unit:IsPurgeException()
	return false
end
...
function modifier_death_prophet_exorcism_custom_unit:OnCreated()
	...
	self:GetParent().pSpiritGlow = ParticleManager:CreateParticle(
	...
	self:GetParent().pSpiritModel = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_death_prophet_12") then
		min_damage = min_damage
			+ self:GetAbility().modifier_death_prophet_12[self:GetCaster():GetTalentLevel("modifier_death_prophet_12")]
	...
	unit:OnPhysicsFrame(function(unit)
		...
		local angle_difference =
			RotationDelta(VectorToAngles(unit:GetPhysicsVelocity():Normalized()), VectorToAngles(direction)).y
		...
		elseif unit.state == "target_acquired" then
			...
			if collision then
				if unit.current_target ~= nil then
					if
						not unit.current_target:IsAttackImmune()
						or self:GetCaster():HasModifier("modifier_death_prophet_21")
					then
						local damage_table = {}
						...
						if unit.current_target.InvulCount == 0 then
							...
							ParticleManager:SetParticleControlEnt(
								...
								"attach_hitloc",
								unit.current_target:GetAbsOrigin(),
								true
							)
						elseif unit.damage_done > 0 then
							local particle =
								ParticleManager:CreateParticle(particleDamage, PATTACH_ABSORIGIN, unit.current_target)
							ParticleManager:SetParticleControl(particle, 0, unit.current_target:GetAbsOrigin())
							ParticleManager:SetParticleControlEnt(
								particle,
								1,
...
function modifier_death_prophet_exorcism_custom_unit:OnDestroy()
	...
	if modifier_death_prophet_exorcism_custom then
		for count = #modifier_death_prophet_exorcism_custom.spirits_entindex, 1, -1 do
			if
				modifier_death_prophet_exorcism_custom.spirits_entindex[count]
				and modifier_death_prophet_exorcism_custom.spirits_entindex[count] == self.index
			then
...
function modifier_death_prophet_exorcism_custom_damage_cast:IsHidden()
	return true
end
function modifier_death_prophet_exorcism_custom_damage_cast:IsPurgable()
	return false
end
...
function modifier_death_prophet_exorcism_custom_damage_cast:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_death_prophet_1") then
		return
	end
	if self:GetParent():HasModifier("modifier_death_prophet_13") and params.inflictor == nil then
		self:GetAbility():CastGhostToTarget(
			params.unit,
			self:GetAbility().modifier_death_prophet_13[self:GetCaster():GetTalentLevel("modifier_death_prophet_13")]
		)
	end
end
...
function modifier_death_prophet_exorcism_custom_unit_talent:IsHidden()
	return true
end
function modifier_death_prophet_exorcism_custom_unit_talent:IsPurgable()
	return false
end
function modifier_death_prophet_exorcism_custom_unit_talent:IsPurgeException()
	return false
end
