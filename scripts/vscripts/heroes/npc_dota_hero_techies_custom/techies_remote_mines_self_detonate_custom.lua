...
function techies_remote_mines_self_detonate_custom:OnSpellStart(use_ability)
	...
	local modifier_techies_remote_mines_custom = self:GetCaster()
		:FindModifierByName("modifier_techies_remote_mines_custom")
	if modifier_techies_remote_mines_custom == nil then
		return
	end
	...
	if not use_ability then
		...
		for _, id in pairs(units) do
			...
			if
				unit
				and unit ~= self:GetCaster()
				and unit:HasAbility("techies_remote_mines_self_detonate_custom")
				and unit:GetPlayerOwnerID() == self:GetCaster():GetPlayerOwnerID()
			then
				local techies_remote_mines_self_detonate_custom =
					unit:FindAbilityByName("techies_remote_mines_self_detonate_custom")
	...
	Timers:CreateTimer(detonate_delay, function()
		...
		local damage = techies_remote_mines_custom:GetSpecialValueFor("damage")
			/ 100
			* techies_remote_mines_custom:GetCaster():GetDisplayAttackSpeed()
		...
		for _, enemy in pairs(enemies) do
			ApplyDamage({
				...
				ability = techies_remote_mines_custom,
			})
		end
		...
		if techies_remote_mines_custom:GetCaster():HasModifier("modifier_techies_10") then
			...
			local reduction_talent = techies_remote_mines_custom.modifier_techies_10[techies_remote_mines_custom
				:GetCaster()
				:GetTalentLevel("modifier_techies_10")]
