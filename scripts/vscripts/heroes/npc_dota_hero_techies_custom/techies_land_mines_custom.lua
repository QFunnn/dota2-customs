...
LinkLuaModifier(
	"modifier_techies_land_mines_custom",
	"heroes/npc_dota_hero_techies_custom/techies_land_mines_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_land_mines_custom_debuff",
	"heroes/npc_dota_hero_techies_custom/techies_land_mines_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function techies_land_mines_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_techies.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_techies.vpcf",
		context
	)
end
...
function modifier_techies_land_mines_custom:IsPurgable()
	return false
end
function modifier_techies_land_mines_custom:IsPurgeException()
	return false
end
function modifier_techies_land_mines_custom:IsHidden()
	return true
end
function modifier_techies_land_mines_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_techies_19") then
		self.activation_delay = self.activation_delay
			+ self:GetAbility().modifier_techies_19[self:GetCaster():GetTalentLevel("modifier_techies_19")]
	end
...
function modifier_techies_land_mines_custom:OnIntervalThink()
	...
	for _, hero in pairs(enemies) do
		self:GetParent()
			:AddNewModifier(hero, self:GetAbility(), "modifier_truesight", { duration = FrameTime() + FrameTime() })
	end
	if #enemies > 0 then
...
function modifier_techies_land_mines_custom:GetDamageMine(target)
	...
	if self:GetCaster():HasModifier("modifier_techies_17") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_techies_17[self:GetCaster():GetTalentLevel("modifier_techies_17")]
			)
	end
	...
	elseif length >= max_distance then
		...
		return damage
			* ((max_distance - length) / (max_distance - self.min_distance) * self.outer_damage + self.outer_damage)
	end
end
...
function modifier_techies_land_mines_custom_debuff:GetAttributes()
