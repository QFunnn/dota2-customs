...
LinkLuaModifier(
	"modifier_techies_suicide_custom",
	"heroes/npc_dota_hero_techies_custom/techies_suicide_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_techies_suicide_custom_magic_immune",
	"heroes/npc_dota_hero_techies_custom/techies_suicide_custom",
...
function techies_suicide_custom:GetCastPoint()
	if self:GetCaster():HasModifier("modifier_techies_7") then
		return self.BaseClass.GetCastPoint(self)
			- (self.modifier_techies_7_cast_point / 100 * self.BaseClass.GetCastPoint(self))
	end
	return self.BaseClass.GetCastPoint(self)
end
...
function techies_suicide_custom:OnAbilityPhaseStart()
	if IsServer() then
		if
			self:GetCaster():HasModifier("modifier_techies_6")
			and not self:GetCaster():HasModifier("modifier_techies_suicide_custom_magic_immune_cooldown")
		then
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_techies_suicide_custom_magic_immune",
				{ duration = self.modifier_techies_6 }
			)
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_techies_suicide_custom_magic_immune_cooldown",
				{ duration = self.modifier_techies_6_cooldown }
			)
		end
	end
	return true
...
function modifier_techies_suicide_custom:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_techies_3") then
		self.damage = self.damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_techies_3[self:GetCaster():GetTalentLevel("modifier_techies_3")]
			)
	end
	...
	for i, unit in ipairs(units) do
		...
		unit:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_stunned",
			{ duration = self.duration * (1 - unit:GetStatusResistance()) }
		)
	end
	ApplyDamage({
...
function modifier_techies_suicide_custom_magic_immune:IsPurgable()
	return false
end
...
function modifier_techies_suicide_custom_magic_immune_cooldown:IsPurgeException()
	return false
end
function modifier_techies_suicide_custom_magic_immune_cooldown:RemoveOnDeath()
	return false
end
function modifier_techies_suicide_custom_magic_immune_cooldown:IsDebuff()
	return true
end
function modifier_techies_suicide_custom_magic_immune_cooldown:IsPurgable()
	return false
end