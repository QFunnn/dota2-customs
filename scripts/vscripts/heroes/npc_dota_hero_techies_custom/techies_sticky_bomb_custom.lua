...
LinkLuaModifier(
	"modifier_techies_sticky_bomb_custom",
	"heroes/npc_dota_hero_techies_custom/techies_sticky_bomb_custom",
...
LinkLuaModifier(
	"modifier_techies_sticky_bomb_handler",
	"heroes/npc_dota_hero_techies_custom/techies_sticky_bomb_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function techies_sticky_bomb_custom:ThrowBomb(point)
	...
	local npc_dota_techies_remote_mine = CreateUnitByName(
	...
	if npc_dota_techies_remote_mine then
		...
		npc_dota_techies_remote_mine:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_techies_sticky_bomb_custom_motion",
...
function modifier_techies_sticky_bomb_custom_motion:OnDestroy()
	...
	if #units > 0 then
		...
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_techies_sticky_bomb_activated",
			{ duration = self.countdown, target = target:entindex() }
		)
	else
		self:GetParent():AddNewModifier(
			self:GetCaster(),
...
function modifier_techies_sticky_bomb_custom_motion:UpdateVerticalMotion(me, dt)
	...
	vNewPos.z = self.vStartPosition.z
		+ (
			-0.5 * TECHIES_ACCELERATION_Z * (self.flCurrentTimeVert * self.flCurrentTimeVert)
...
function modifier_techies_sticky_bomb_activated:OnCreated(params)
	...
	if self.target then
		...
		if not modifier_techies_sticky_bomb_slow then
			modifier_techies_sticky_bomb_slow =
				self.target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_techies_sticky_bomb_slow", {})
		end
...
function modifier_techies_sticky_bomb_activated:OnDestroy()
	...
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self.explosion_radius, self.explosion_radius, self.explosion_radius)
	)
...
function modifier_techies_sticky_bomb_handler:IsHidden()
	return true
end
function modifier_techies_sticky_bomb_handler:IsPurgeException()
	return false
end
function modifier_techies_sticky_bomb_handler:IsPurgable()
	return false
end
function modifier_techies_sticky_bomb_handler:RemoveOnDeath()
	return false
end
...
function modifier_techies_sticky_bomb_handler:OnTakeDamage(params)
	...
	if not self:GetParent():HasModifier("modifier_techies_16") then
		return
	end
	if not self:GetParent():IsAlive() then
		return
	end
	if self:GetParent():HasModifier("modifier_techies_sticky_bomb_handler_cooldown") then
		return
	end
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_techies_sticky_bomb_handler_cooldown",
		{ duration = self:GetAbility().modifier_techies_16[self:GetCaster():GetTalentLevel("modifier_techies_16")] }
	)
	self:GetAbility():ThrowBomb(params.attacker:GetAbsOrigin())
end

modifier_techies_sticky_bomb_handler_cooldown = class({})
function modifier_techies_sticky_bomb_handler_cooldown:IsDebuff()
	return true
end
function modifier_techies_sticky_bomb_handler_cooldown:IsPurgable()
	return false
end
function modifier_techies_sticky_bomb_handler_cooldown:IsPurgeException()
	return false
end
function modifier_techies_sticky_bomb_handler_cooldown:RemoveOnDeath()
	return false
end