...
LinkLuaModifier(
	"modifier_techies_reactive_tazer_custom",
	"heroes/npc_dota_hero_techies_custom/techies_reactive_tazer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_reactive_tazer_custom_debuff",
	"heroes/npc_dota_hero_techies_custom/techies_reactive_tazer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_reactive_tazer_custom_thinker",
	"heroes/npc_dota_hero_techies_custom/techies_reactive_tazer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_reactive_tazer_custom_debuff_thinker",
	"heroes/npc_dota_hero_techies_custom/techies_reactive_tazer_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_reactive_tazer_custom_regen_buff",
	"heroes/npc_dota_hero_techies_custom/techies_reactive_tazer_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_techies_reactive_tazer_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_techies_12") then
		self.bonus_ms = self.bonus_ms
			+ self:GetAbility().modifier_techies_12_speed[self:GetCaster():GetTalentLevel("modifier_techies_12")]
		self.disarm_duration = self.disarm_duration
			+ self:GetAbility().modifier_techies_12[self:GetCaster():GetTalentLevel("modifier_techies_12")]
	end
	...
	if techies_reactive_tazer_stop_custom then
		...
		self:GetCaster()
			:SwapAbilities("techies_reactive_tazer_custom", "techies_reactive_tazer_stop_custom", false, true)
	end
end
...
function modifier_techies_reactive_tazer_custom:OnAttackLanded(params)
	...
	params.attacker:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_techies_reactive_tazer_custom_debuff",
		{ duration = self.disarm_duration * (1 - params.attacker:GetStatusResistance()) }
	)
end
...
function modifier_techies_reactive_tazer_custom:OnDestroy()
	...
	for _, unit in pairs(units) do
		unit:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_techies_reactive_tazer_custom_debuff",
			{ duration = self.disarm_duration * (1 - unit:GetStatusResistance()) }
		)
	end
	local techies_reactive_tazer_stop_custom = self:GetCaster():FindAbilityByName("techies_reactive_tazer_stop_custom")
	if techies_reactive_tazer_stop_custom then
		self:GetCaster()
			:SwapAbilities("techies_reactive_tazer_stop_custom", "techies_reactive_tazer_custom", false, true)
	end
	...
	if self:GetCaster():HasModifier("modifier_techies_8") then
		CreateModifierThinker(
			...
			self:GetAbility(),
			"modifier_techies_reactive_tazer_custom_thinker",
			{ duration = self:GetAbility().modifier_techies_8_duration },
...
function modifier_techies_reactive_tazer_custom_debuff:GetAttributes()
...
function techies_reactive_tazer_stop_custom:OnSpellStart()
	...
	local modifier_techies_reactive_tazer_custom =
		techies_reactive_tazer_custom.target:FindModifierByName("modifier_techies_reactive_tazer_custom")
...
function modifier_techies_reactive_tazer_custom_thinker:IsHidden()
	return true
end
function modifier_techies_reactive_tazer_custom_thinker:IsPurgable()
	return false
end
function modifier_techies_reactive_tazer_custom_thinker:IsAura()
	return true
end
function modifier_techies_reactive_tazer_custom_thinker:GetModifierAura()
	return "modifier_techies_reactive_tazer_custom_debuff_thinker"
end
function modifier_techies_reactive_tazer_custom_thinker:GetAuraRadius()
	return self:GetAbility():GetSpecialValueFor("stun_radius")
end
function modifier_techies_reactive_tazer_custom_thinker:GetAuraDuration()
	return 0.1
end
function modifier_techies_reactive_tazer_custom_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_techies_reactive_tazer_custom_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_techies_reactive_tazer_custom_thinker:GetAuraSearchFlags()
	return 0
end
function modifier_techies_reactive_tazer_custom_thinker:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/techies_tazer_ground_explosion.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetParent()
	)
	ParticleManager:SetParticleControl(particle, 0, self:GetParent():GetAbsOrigin())
...
function modifier_techies_reactive_tazer_custom_debuff_thinker:OnIntervalThink()
	...
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		damage = self:GetAbility().modifier_techies_8[self:GetCaster():GetTalentLevel("modifier_techies_8")],
