...
LinkLuaModifier(
	"modifier_techies_mutually_assured_destruction_custom",
	"heroes/npc_dota_hero_techies_custom/techies_mutually_assured_destruction_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_remote_mines_custom",
	"heroes/npc_dota_hero_techies_custom/techies_mutually_assured_destruction_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_remote_mines_custom_invul",
	"heroes/npc_dota_hero_techies_custom/techies_mutually_assured_destruction_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_techies_remote_mines_custom_plant",
	"heroes/npc_dota_hero_techies_custom/techies_mutually_assured_destruction_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function techies_mutually_assured_destruction_custom:GetCastPoint()
	...
	if self:GetCaster():HasModifier("modifier_techies_10") then
		mult = mult
			- (self.modifier_techies_10_cast_point[self:GetCaster():GetTalentLevel("modifier_techies_10")] / 100)
	end
...
function techies_mutually_assured_destruction_custom:CreateMine(point, death)
	...
	local modifier_techies_remote_mines_custom =
		mine:AddNewModifier(self:GetCaster(), self, "modifier_techies_remote_mines_custom", { duration = duration })
	if death then
...
function modifier_techies_mutually_assured_destruction_custom:IsHidden()
	return true
end
function modifier_techies_mutually_assured_destruction_custom:IsPurgable()
	return false
end
function modifier_techies_mutually_assured_destruction_custom:IsPurgeException()
	return false
end
function modifier_techies_mutually_assured_destruction_custom:RemoveOnDeath()
	return false
end
...
function modifier_techies_mutually_assured_destruction_custom:GetModifierOverrideAbilitySpecial(data)
	if
		data.ability:GetAbilityName() == "techies_mutually_assured_destruction_custom"
		and data.ability_special_value == "current_mana_regen"
	then
		return 1
	end
	if
		data.ability:GetAbilityName() == "techies_mutually_assured_destruction_custom"
		and data.ability_special_value == "current_damage"
	then
		return 1
	end
end
...
function modifier_techies_remote_mines_custom:IsHidden()
	return true
end
function modifier_techies_remote_mines_custom:IsPurgable()
	return false
end
function modifier_techies_remote_mines_custom:IsPurgeException()
	return false
end
function modifier_techies_remote_mines_custom:RemoveOnDeath()
	return false
end
function modifier_techies_remote_mines_custom:OnCreated()
	...
	local particle_mine_fx = ParticleManager:CreateParticle(
...
function modifier_techies_remote_mines_custom:ActivatePlant(death)
	...
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_techies_remote_mines_custom_invul", {})
	if not death then
		local duration = self:GetAbility():GetSpecialValueFor("explosion_delay")
		if self:GetCaster():HasModifier("modifier_techies_11") then
			duration = duration
				+ self:GetAbility().modifier_techies_11[self:GetCaster():GetTalentLevel("modifier_techies_11")]
		end
...
function modifier_techies_remote_mines_custom:OnDestroy()
	...
	if self.activate_plant then
		local radius = self:GetAbility():GetSpecialValueFor("radius")
		local particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_techies/techies_mad_explode.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
		...
		for _, enemy in pairs(enemies) do
			ApplyDamage({
				...
				damage = self:GetAbility():GetAbilityDamageMine(),
				damage_type = DAMAGE_TYPE_MAGICAL,
				ability = self:GetAbility(),
			})
			local particle = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_techies/techies_mad_scorch.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				enemy
			)
			ParticleManager:ReleaseParticleIndex(particle)
...
function modifier_techies_remote_mines_custom:GetDisableHealing()
	return 1
end
function modifier_techies_remote_mines_custom:GetModifierIgnoreMovespeedLimit()
	return 1
end

modifier_techies_remote_mines_custom_invul = class({})
function modifier_techies_remote_mines_custom_invul:IsHidden()
	return true
end
function modifier_techies_remote_mines_custom_invul:IsPurgable()
	return false
end
...
function modifier_techies_remote_mines_custom_plant:IsHidden()
	return true
end
function modifier_techies_remote_mines_custom_plant:IsPurgable()
	return false
end
function modifier_techies_remote_mines_custom_plant:IsPurgeException()
	return false
end
function modifier_techies_remote_mines_custom_plant:OnCreated()
	...
	self:GetCaster()
		:SwapAbilities("techies_mutually_assured_destruction_custom", "techies_focused_detonate_custom", false, true)
...
function modifier_techies_remote_mines_custom_plant:OnDestroy()
	...
	self:GetCaster()
		:SwapAbilities("techies_focused_detonate_custom", "techies_mutually_assured_destruction_custom", false, true)
end
...
function techies_focused_detonate_custom:OnSpellStart()
	...
	local techies_mutually_assured_destruction_custom =
		self:GetCaster():FindAbilityByName("techies_mutually_assured_destruction_custom")
	if techies_mutually_assured_destruction_custom then
		techies_mutually_assured_destruction_custom.mine:RemoveModifierByName(
			"modifier_techies_remote_mines_custom_plant"
		)
		local modifier_techies_remote_mines_custom =
			techies_mutually_assured_destruction_custom.mine:FindModifierByName("modifier_techies_remote_mines_custom")
		if modifier_techies_remote_mines_custom then
