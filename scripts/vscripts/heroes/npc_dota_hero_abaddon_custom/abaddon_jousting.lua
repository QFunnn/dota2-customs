...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_abaddon_custom/abaddon_jousting",
...
LinkLuaModifier(
	"modifier_abaddon_jousting_slow",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_jousting",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_abaddon_jousting_ignore_damage",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_jousting",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_abaddon_jousting_handler",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_jousting",
	LUA_MODIFIER_MOTION_BOTH
)
...
function abaddon_jousting:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_primal_beast/primal_beast_onslaught_range_finder.vpcf",
		context
	)
...
function abaddon_jousting:GetCooldown(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_abaddon_10") then
		bonus = (
			self:GetCaster():GetModifierStackCount("modifier_abaddon_jousting_handler", self:GetCaster())
			/ self.modifier_abaddon_10_attackspeed
		) * self.modifier_abaddon_10_cooldown
	end
...
function abaddon_jousting:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_abaddon_jousting_cast",
		{ duration = duration, speed = self:GetCaster():GetIdealSpeed() }
	)
...
function modifier_abaddon_jousting_cast:OnCreated(kv)
	self.speed = self:GetAbility():GetSpecialValueFor("max_distance")
		/ self:GetAbility():GetSpecialValueFor("chargeup_time")
...
function modifier_abaddon_jousting_cast:OnRemoved()
	...
	if self:GetCaster():HasModifier("modifier_abaddon_13") then
		self:GetCaster():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_abaddon_jousting_ignore_damage",
			{ duration = self:GetAbility().modifier_abaddon_13_duration }
		)
	end
...
function modifier_abaddon_jousting_charge:OnCreated(kv)
	...
	local particle = ParticleManager:CreateParticle(
		"particles/yashio/yasuo_spell_first/yasuo_spell_first_buff.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetCaster()
	)
	...
	local particle2 =
		ParticleManager:CreateParticle("particles/abaddon_run.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	ParticleManager:SetParticleControlEnt(
		particle2,
		0,
		self:GetCaster(),
	...
	local feet = ParticleManager:CreateParticle(
...
function modifier_abaddon_jousting_charge:GetModifierIgnoreMovespeedLimit()
	...
	if not self:GetCaster():HasModifier("modifier_abaddon_9") then
		return
	end
	return 1
end
		...
		if self.targets[unit:entindex()] == nil then
			...
			if self:GetCaster():HasModifier("modifier_abaddon_12") then
				unit:AddNewModifier(
					...
					self:GetAbility(),
					"modifier_abaddon_jousting_slow",
					{ duration = self:GetAbility().modifier_abaddon_12_duration }
				)
			end
			ApplyDamage({
				victim = unit,
				attacker = self:GetParent(),
				damage = self:GetAbility():GetSpecialValueFor("damage_from_speed") / 100 * self:GetCaster()
					:GetIdealSpeed(),
...
function modifier_abaddon_jousting_slow:GetTexture()
	return "abaddon_12"
end
...
function modifier_abaddon_jousting_ignore_damage:GetTexture()
	return "abaddon_13"
end
function modifier_abaddon_jousting_ignore_damage:IsPurgable()
	return false
end
...
function modifier_abaddon_jousting_handler:IsHidden()
	return true
end
function modifier_abaddon_jousting_handler:IsPurgable()
	return false
end
function modifier_abaddon_jousting_handler:IsPurgeException()
	return false
end
function modifier_abaddon_jousting_handler:RemoveOnDeath()
	return false
end
