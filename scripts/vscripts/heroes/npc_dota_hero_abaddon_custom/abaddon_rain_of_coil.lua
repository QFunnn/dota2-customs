...
LinkLuaModifier(
	"modifier_abaddon_rain_of_coil",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_rain_of_coil",
...
function modifier_abaddon_rain_of_coil:IsPurgable()
	return false
end
...
function modifier_abaddon_rain_of_coil:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/abaddon/abaddon_rain_of_coil.vpcf",
		...
		if abaddon_death_coil_custom and abaddon_death_coil_custom:GetLevel() > 0 then
			...
			if self:GetCaster():HasModifier("modifier_abaddon_15") then
				...
				if self:GetCaster():HasModifier("modifier_abaddon_16") then
					radius = radius
						+ abaddon_death_coil_custom.modifier_abaddon_16[self:GetCaster()
							:GetTalentLevel("modifier_abaddon_16")]
				end
				local units = FindUnitsInRadius(
					self:GetCaster():GetTeamNumber(),
					target:GetAbsOrigin(),
