...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_abaddon_custom/abaddon_aphotic_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_abaddon_aphotic_shield_custom_handler",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_aphotic_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function abaddon_aphotic_shield_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_abaddon.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_abaddon.vpcf",
		context
	)
end
...
function modifier_abaddon_aphotic_shield_custom:IsPurgable()
	return true
end
...
function modifier_abaddon_aphotic_shield_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_abaddon_20") then
		self.barrier = self.barrier
			+ self:GetAbility().modifier_abaddon_20[self:GetCaster():GetTalentLevel("modifier_abaddon_20")]
	end
...
function modifier_abaddon_aphotic_shield_custom_handler:IsHidden()
	return true
end
function modifier_abaddon_aphotic_shield_custom_handler:IsPurgable()
	return false
end
function modifier_abaddon_aphotic_shield_custom_handler:IsPurgeException()
	return false
end
function modifier_abaddon_aphotic_shield_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_abaddon_aphotic_shield_custom_handler:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_abaddon_19") then
		return
	end
