...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_abaddon_custom/abaddon_borrowed_time_custom",
...
function abaddon_borrowed_time_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_abaddon_borrowed_time_custom_buff", { duration = duration })
	if self:GetCaster():HasModifier("modifier_abaddon_6") then
		...
		if abaddon_death_coil_custom and abaddon_death_coil_custom:GetLevel() > 0 then
			local units = FindUnitsInRadius(
				...
				nil,
				self.modifier_abaddon_6[self:GetCaster():GetTalentLevel("modifier_abaddon_6")],
...
function modifier_abaddon_borrowed_time_custom:IsHidden()
	return true
end
function modifier_abaddon_borrowed_time_custom:IsPurgeException()
	return false
end
function modifier_abaddon_borrowed_time_custom:IsPurgable()
	return false
end
function modifier_abaddon_borrowed_time_custom:RemoveOnDeath()
	return false
end
...
function modifier_abaddon_borrowed_time_custom:OnTakeDamage(params)
	...
	if
		(self:GetParent():GetHealth() <= self:GetAbility():GetSpecialValueFor("hp_threshold"))
...
function modifier_abaddon_borrowed_time_custom_buff:IsPurgable()
	return false
end
function modifier_abaddon_borrowed_time_custom_buff:IsPurgeException()
	return false
end
...
function modifier_abaddon_borrowed_time_custom_buff:HealHero(params)
	...
	local particle_heal = ParticleManager:CreateParticle(
...
function modifier_abaddon_borrowed_time_custom_buff:OnTakeDamage(params)
	...
	if not self:GetCaster():HasModifier("modifier_abaddon_5") then
		return
	end
	...
	if
		...
		and not params.attacker:IsBuilding()
	...
	then
		if not params.unit:IsOther() then
			...
			ApplyDamage({
				...
				damage = params.original_damage / 100 * self:GetAbility().modifier_abaddon_5[self:GetCaster()
					:GetTalentLevel("modifier_abaddon_5")],
