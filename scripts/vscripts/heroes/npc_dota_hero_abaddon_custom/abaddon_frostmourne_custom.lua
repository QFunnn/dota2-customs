...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_abaddon_custom/abaddon_frostmourne_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_abaddon_frostmourne_custom_debuff_stack",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_frostmourne_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_abaddon_frostmourne_custom_debuff",
	"heroes/npc_dota_hero_abaddon_custom/abaddon_frostmourne_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
abaddon_frostmourne_custom.modifier_abaddon_3 = { 20, 40, 60 }
...
function modifier_abaddon_frostmourne_custom:IsPurgable()
	return false
end
function modifier_abaddon_frostmourne_custom:IsPurgeException()
	return false
end
function modifier_abaddon_frostmourne_custom:IsHidden()
	return true
end
function modifier_abaddon_frostmourne_custom:RemoveOnDeath()
	return false
end
...
function modifier_abaddon_frostmourne_custom:OnAttackLanded(params)
	...
	if params.target:IsInvulnerable() then
		return
	end
	...
	local modifier_abaddon_frostmourne_custom_debuff_stack = params.target:AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_abaddon_frostmourne_custom_debuff_stack",
	...
	if
		modifier_abaddon_frostmourne_custom_debuff_stack
		and modifier_abaddon_frostmourne_custom_debuff_stack:GetStackCount() >= hit_count
	then
		...
		if self:GetCaster():HasModifier("modifier_abaddon_1") then
			local units = FindUnitsInRadius(
				...
				nil,
				self:GetAbility().modifier_abaddon_1,
...
function modifier_abaddon_frostmourne_custom_debuff:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_abaddon_3") then
		curse_dps = curse_dps
			+ self:GetAbility().modifier_abaddon_3[self:GetCaster():GetTalentLevel("modifier_abaddon_3")]
	end
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		damage = curse_dps * 0.5,
...
function modifier_abaddon_frostmourne_custom_debuff:OnAttackLanded(params)
	...
	params.attacker:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_abaddon_frostmourne_custom_buff",
		{ duration = self:GetAbility():GetSpecialValueFor("curse_duration"), target = self:GetParent():entindex() }
	)
end
