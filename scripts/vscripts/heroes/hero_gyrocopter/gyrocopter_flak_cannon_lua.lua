...
function modifier_gyrocopter_flak_cannon_lua_scepter:OnIntervalThink()
	...
	if IsValid(hParent) and hParent:IsAlive() and IsValid(hAbility) then
		...
		if hParent:HasScepter() and GameRulesCustom:GetGameTime() >= self.LastTime + self.fire_rate and not hParent:PassivesDisabled() and not hParent:IsOutOfGame() and not hParent:IsInvisible() then
			self.LastTime = GameRulesCustom:GetGameTime()
