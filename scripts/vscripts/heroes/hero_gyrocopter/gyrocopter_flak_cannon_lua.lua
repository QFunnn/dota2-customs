...
function gyrocopter_flak_cannon_lua:OnSpellStart(...)
	...
	local max_attacks = self:GetSpecialValueFor("max_attacks")
		+ hCaster:GetTalentValue("special_bonus_unique_gyrocopter_2")

	if hCaster and not hCaster:IsNull() then
		local hModifier =
			hCaster:AddNewModifier(hCaster, self, "modifier_gyrocopter_flak_cannon_lua", { duration = flDuration })
		if hModifier then
...
function modifier_gyrocopter_flak_cannon_lua:IsPurgeException()
	return false
end
...
function modifier_gyrocopter_flak_cannon_lua:AttackModifier(params)
	...
	for i, hEnemy in pairs(enemies) do
		Timers:CreateTimer(flDelay * i, function()
			if not hEnemy or hEnemy:IsNull() then
				return
			end
			-- A1: не бить невидимых без детекта (раньше Flak попадал в инвиз без вардов)
			if hEnemy:IsInvisible() and not hParent:CanEntityBeSeenByMyTeam(hEnemy) then
				return
			end
...
function modifier_gyrocopter_flak_cannon_lua_scepter:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		...
		if
			...
			and enemy:IsAlive()
			and (not enemy:IsInvisible() or hParent:CanEntityBeSeenByMyTeam(enemy))
		then
