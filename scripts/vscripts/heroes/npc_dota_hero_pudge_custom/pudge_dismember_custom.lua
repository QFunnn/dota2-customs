...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_pudge_custom/pudge_dismember_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_dismember_custom_pull",
	"heroes/npc_dota_hero_pudge_custom/pudge_dismember_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_pudge_dismember_custom_channel",
	"heroes/npc_dota_hero_pudge_custom/pudge_dismember_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_dismember_custom_break",
	"heroes/npc_dota_hero_pudge_custom/pudge_dismember_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_pudge_dismember_custom:IsDebuff()
	return true
end
...
function modifier_pudge_dismember_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_pudge_11") then
		self.agility_damage = true
		self.talent_particle = ParticleManager:CreateParticle(
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_pudge_dismember_custom_pull",
		{ duration = self:GetAbility():GetChannelTime() }
	)
end
...
function modifier_pudge_dismember_custom_pull:IsHidden()
	return true
end
...
function modifier_pudge_dismember_custom_pull:UpdateHorizontalMotion(me, dt)
	...
	if
		distance:Length2D() > self.pull_distance_limit and self.parent:HasModifier("modifier_pudge_dismember_custom")
	then
...
function modifier_pudge_dismember_custom_channel:IsHidden()
	return true
end
function modifier_pudge_dismember_custom_channel:IsPurgable()
	return false
end
function modifier_pudge_dismember_custom_channel:GetAttributes()
...
function modifier_pudge_dismember_custom_channel:OnAbilityExecuted(params)
	...
	if params.ability == self:GetAbility() then
		...
		self:GetCaster():SetModifierStackCount(
			"modifier_pudge_dismember_custom_channel",
			self:GetCaster(),
			duration * (1 - params.target:GetStatusResistance()) * 100
		)
	end
end
...
function modifier_pudge_dismember_custom_break:IsHidden()
	return true
end
function modifier_pudge_dismember_custom_break:IsPurgable()
	return false
end
