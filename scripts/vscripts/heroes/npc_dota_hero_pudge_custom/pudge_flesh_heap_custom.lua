...
LinkLuaModifier(
	"modifier_pudge_flesh_heap_custom_active",
	"heroes/npc_dota_hero_pudge_custom/pudge_flesh_heap_custom",
...
function modifier_pudge_flesh_heap_custom_active:IsPurgable()
	return false
end
...
function modifier_pudge_flesh_heap_custom_active:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_pudge_14") then
		self.talent = true
		local talent = ParticleManager:CreateParticle(
			"particles/pudge_fleshheap_talentstatus_effect.vpcf",
		...
		local talent = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_pudge/pudge_fleshheap_block_activation.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			self:GetCaster()
		)
		self:AddParticle(talent, false, true, 10, true, false)
...
function modifier_pudge_flesh_heap_custom_active:GetModifierAttackSpeedBonus_Constant()
	if not self:GetCaster():HasModifier("modifier_pudge_8") then
		return
	end
