...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_pudge_custom/pudge_rot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_rot_custom_slow",
	"heroes/npc_dota_hero_pudge_custom/pudge_rot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_rot_custom_dummy",
	"heroes/npc_dota_hero_pudge_custom/pudge_rot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_rot_custom_cooldown",
	"heroes/npc_dota_hero_pudge_custom/pudge_rot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function pudge_rot_custom:ResetToggleOnRespawn()
	return true
end
...
function modifier_pudge_rot_custom:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_pudge_2") then
		self.rot_radius = self.rot_radius
			+ self:GetAbility().modifier_pudge_2[self:GetCaster():GetTalentLevel("modifier_pudge_2")]
	end
	...
	if self:GetCaster():HasModifier("modifier_pudge_4") then
		self.rot_damage = self.rot_damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_pudge_4[self:GetCaster():GetTalentLevel("modifier_pudge_4")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_pudge_16") then
		self.rot_damage = self.rot_damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_pudge_16[self:GetCaster():GetTalentLevel("modifier_pudge_16")]
			)
	end
...
function modifier_pudge_rot_custom_slow:OnIntervalThink()
	...
	if self:GetAuraOwner() == nil or self:GetAuraOwner():IsNull() then
		return
	end
	if self:GetCaster():HasModifier("modifier_pudge_5") then
		if not self:GetParent():HasModifier("modifier_pudge_rot_custom") then
			...
			if original_mod then
				if
					original_mod:GetCaster() == self:GetAuraOwner()
					or self:GetAuraOwner():GetUnitName() == "npc_dota_companion"
				then
					self.time_rot = self.time_rot + FrameTime()
					if self.time_rot >= self:GetAbility().modifier_pudge_5_duration then
						self.time_rot = 0
						if self:GetCaster():HasModifier("modifier_pudge_rot_custom_cooldown") then
							return
						end
						self:GetParent():AddNewModifier(
							...
							"modifier_pudge_rot_custom",
							{
								duration = self:GetAbility().modifier_pudge_5[self:GetCaster()
									:GetTalentLevel("modifier_pudge_5")],
								talent = true,
								talent_incisisies = true,
							}
						)
	...
	if target ~= nil then
		...
		if pudge_rot_custom and pudge_rot_custom:GetLevel() > 0 then
			target:AddNewModifier(
				...
				pudge_rot_custom,
				"modifier_pudge_rot_custom",
				{ duration = self:GetSpecialValueFor("duration"), talent = true }
			)
		end
		target:AddNewModifier(
			self:GetCaster(),
...
function modifier_pudge_rot_custom_dummy:IsHidden()
	return true
end
function modifier_pudge_rot_custom_dummy:IsPurgable()
	return false
end
...
function modifier_pudge_rot_custom_cooldown:IsHidden()
	return true
end
function modifier_pudge_rot_custom_cooldown:IsPurgable()
	return false
end
function modifier_pudge_rot_custom_cooldown:RemoveOnDeath()
	return false
end
function modifier_pudge_rot_custom_cooldown:IsPurgeException()
	return false
end