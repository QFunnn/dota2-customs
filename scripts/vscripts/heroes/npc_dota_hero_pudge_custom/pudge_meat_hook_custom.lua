...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_meat_hook_custom_thinker",
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pudge_meat_hook_custom_fart_debuff",
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_pudge_meat_hook_custom_debuff",
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_pudge_hook_custom_dummy",
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_pudge_meat_hook_boss_abuse",
	"heroes/npc_dota_hero_pudge_custom/pudge_meat_hook_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
...
function pudge_meat_hook_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_pudge.vpcf",
		context
	)
end
...
function pudge_meat_hook_custom:UseHook(direction)
	...
	if self:GetCaster():HasModifier("modifier_pudge_19") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_pudge_meat_hook_custom_cooldown_talent",
			{ duration = (self.hook_distance / self.hook_speed) + 0.6 }
		)
	end
	...
	ParticleManager:SetParticleControlEnt(
		...
		self:GetCaster():GetOrigin() + self.vHookOffset,
		true
	)
...
function pudge_meat_hook_custom:OnProjectileHitHandle(target, position, projectileIndex)
	...
	if not self.hooks[projectileIndex].thinker or self.hooks[projectileIndex].thinker:IsNull() then
		return
	end

	if self.hooks[projectileIndex] == nil then
		return true
	end
	...
	if self.hooks[projectileIndex].bRetracting == false then
		...
		if target and target:GetUnitName() == "npc_dota_bounty_hunter_gold_bag" then
			return false
		end
		if target and target:GetUnitName() == "npc_dota_kunkka_mark" then
			return false
		end
		local bTargetPulled = false
		if target ~= nil then
			local modifier_pudge_meat_hook_custom_cooldown_talent =
				self:GetCaster():FindModifierByName("modifier_pudge_meat_hook_custom_cooldown_talent")
			...
			if
				target:GetUnitName() == "npc_dota_creature_barrel"
				or target:GetUnitName() == "small_barrel"
				or target:GetUnitName() == "small_barrel_side"
				or target:GetUnitName() == "big_barrel"
			then
				return false
			end
			...
			if
				target:GetUnitName() ~= "npc_dota_creature_barrel"
				and target:GetUnitName() ~= "small_barrel"
				and target:GetUnitName() ~= "small_barrel_side"
				and target:GetUnitName() ~= "big_barrel"
				and target:GetUnitName() ~= "boss_1_pve"
				and target:GetUnitName() ~= "boss_2_pve"
				and target:GetUnitName() ~= "boss_3_pve"
				and target:GetUnitName() ~= "boss_4_pve"
				and target:GetUnitName() ~= "boss_5_pve"
				and target:GetUnitName() ~= "boss_6_pve"
				and target:GetUnitName() ~= "boss_3"
				and target:GetUnitName() ~= "boss_5"
			then
				if
					not target:HasModifier("modifier_wodarelax")
					and not target:HasModifier("modifier_wodarelax_invul")
					and not target:HasModifier("modifier_stunned_duel_custom")
				then
					...
					if self:GetCaster():HasModifier("modifier_pudge_17") then
						damage = (
							(
								distance
								/ 100
								* self.modifier_pudge_17[self:GetCaster():GetTalentLevel("modifier_pudge_17")]
							) / flFollowthroughDuration
						) * FrameTime()
					end
					target:AddNewModifier(
						...
						self,
						"modifier_pudge_meat_hook_custom_debuff",
						{ duration = flFollowthroughDuration, damage = damage }
					)
				end
			else
			...
			if self:GetCaster():HasModifier("modifier_pudge_18") then
				...
				CreateModifierThinker(
					...
					self,
					"modifier_pudge_meat_hook_custom_thinker",
					{ duration = 5, x = direction.x, y = direction.y },
					target:GetAbsOrigin(),
					self:GetCaster():GetTeamNumber(),
					false
				)
			...
			if target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				...
				if not target:IsAlive() then
					self.hooks[projectileIndex].bDiedInHook = true
				end
		...
		if self.hooks[projectileIndex].hVictim == nil then
			local dummy =
				CreateUnitByName("npc_dota_companion", position, false, nil, nil, self:GetCaster():GetTeamNumber())
		...
		if bTargetPulled then
			ParticleManager:SetParticleControlEnt(
				self.hooks[projectileIndex].particleIndex,
				0,
				self:GetCaster(),
				PATTACH_ABSORIGIN,
				"attach_weapon_chain_rt",
				self.hooks[projectileIndex].start_position + self.vHookOffset,
				true
			)
		...
		if self.hooks[projectileIndex].hVictim ~= nil and not self.hooks[projectileIndex].hVictim:IsNull() then
			...
			local flPad = self:GetCaster():GetPaddedCollisionRadius()
				+ self.hooks[projectileIndex].hVictim:GetPaddedCollisionRadius()
			if vVictimPosCheck:Length2D() > flPad then
				local check_dir = (
					self.hooks[projectileIndex].start_position - self.hooks[projectileIndex].hVictim:GetAbsOrigin()
				):Normalized()
		...
		if
			not self.hooks[projectileIndex].hVictim:IsNull()
			and self.hooks[projectileIndex].hVictim:GetUnitName() == "npc_dota_companion"
		then
	...
	if self.hooks[projectileIndex] then
		...
		if self.hooks[projectileIndex].bRetracting then
			...
			if self.hooks[projectileIndex].hVictim then
				...
				if
					self:GetCaster():HasModifier("modifier_pudge_18")
					and self.hooks[projectileIndex].hVictim:GetUnitName() ~= "npc_dota_companion"
				then
				...
				if
					not self.hooks[projectileIndex].hVictim:HasModifier("modifier_wodarelax")
					and not self.hooks[projectileIndex].hVictim:HasModifier("modifier_wodarelax_invul")
					and not self.hooks[projectileIndex].hVictim:HasModifier("modifier_stunned_duel_custom")
				then
					self.hooks[projectileIndex].hVictim:SetOrigin(
						GetGroundPosition(position, self.hooks[projectileIndex].hVictim)
					)
...
function modifier_pudge_meat_hook_custom_thinker:IsHidden()
	return true
end
function modifier_pudge_meat_hook_custom_thinker:IsPurgable()
	return false
end
...
function modifier_pudge_meat_hook_custom_fart_debuff:IsHidden()
	return false
end
function modifier_pudge_meat_hook_custom_fart_debuff:IsPurgable()
	return false
end
...
function modifier_pudge_meat_hook_custom_fart_debuff:OnIntervalThink()
	...
	local damage = self:GetCaster():GetIntellect(false)
		/ 100
		* self:GetAbility().modifier_pudge_18[self:GetCaster():GetTalentLevel("modifier_pudge_18")]
	ApplyDamage({
...
function modifier_pudge_meat_hook_boss_abuse:IsHidden()
	return true
end
function modifier_pudge_meat_hook_boss_abuse:IsPurgable()
	return false
end
function modifier_pudge_meat_hook_boss_abuse:RemoveOnDeath()
	return false
end

modifier_pudge_hook_custom_dummy = class({})

function modifier_pudge_hook_custom_dummy:IsHidden()
	return true
end
function modifier_pudge_hook_custom_dummy:IsPurgable()
	return false
end
...
function pudge_chain_binding:OnSpellStart()
	...
	local hookshot_particle =
		ParticleManager:CreateParticle("particles/pudge_chain_hook.vpcf", PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControlEnt(
		hookshot_particle,
		0,
		self:GetCaster(),
	...
	self:GetCaster()
		:AddNewModifier(
			...
			self,
			"modifier_pudge_meat_hook_custom",
			{ duration = hookshot_duration + 0.1 }
		)
	...
	if hTarget == nil then
		if ExtraData.hookshot_particle then
			...
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_pudge_chain_binding",
				{ particle1 = ExtraData.hookshot_particle, x = vLocation.x, y = vLocation.y, z = vLocation.z }
			)
...
function modifier_pudge_chain_binding:IsHidden()
	return true
end
function modifier_pudge_chain_binding:IsPurgable()
	return true
end
function modifier_pudge_chain_binding:RemoveOnDeath()
	return true
end
function modifier_pudge_chain_binding:IsDebuff()
	return false
end
...
function modifier_pudge_chain_binding:IsAura()
	return true
end
...
function modifier_pudge_chain_binding_damage:IsPurgable()
	return false
end
function modifier_pudge_chain_binding_damage:IsHidden()
	return true
end
...
function modifier_pudge_meat_hook_custom_cooldown_talent:RemoveOnDeath()
	return false
end
function modifier_pudge_meat_hook_custom_cooldown_talent:IsPurgable()
	return false
end
function modifier_pudge_meat_hook_custom_cooldown_talent:IsHidden()
	return true
end
...
function modifier_pudge_meat_hook_custom_cooldown_talent:OnDestroy()
	...
	if self:GetStackCount() == 0 then
		...
		if meathook then
			...
			if
				cooldown - self:GetAbility().modifier_pudge_19[self:GetCaster():GetTalentLevel("modifier_pudge_19")]
				<= 0
			then
				meathook:EndCooldown()
			else
				meathook:EndCooldown()
				meathook:StartCooldown(
					cooldown - self:GetAbility().modifier_pudge_19[self:GetCaster():GetTalentLevel("modifier_pudge_19")]
				)
			end
		end
