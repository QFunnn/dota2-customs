...
LinkLuaModifier(
	"modifier_pudge_innate_graft_flesh_custom",
	"heroes/npc_dota_hero_pudge_custom/pudge_innate_graft_flesh_custom",
...
function modifier_pudge_innate_graft_flesh_custom:IsPurgable()
	return false
end
function modifier_pudge_innate_graft_flesh_custom:IsPurgeException()
	return false
end
function modifier_pudge_innate_graft_flesh_custom:RemoveOnDeath()
	return false
end
...
function modifier_pudge_innate_graft_flesh_custom:OnDeath(params)
	...
	if
		(
			...
			<= self:GetAbility():GetSpecialValueFor("flesh_heap_range")
		) or (params.attacker and params.attacker == self:GetParent())
	then
...
function modifier_pudge_innate_graft_flesh_custom:GetModifierBonusStats_Strength()
	if self:GetCaster():HasModifier("modifier_pudge_13") then
		return
	end
	if self:GetCaster():HasModifier("modifier_pudge_20") then
		return
	end
...
function modifier_pudge_innate_graft_flesh_custom:GetModifierBonusStats_Agility()
	if not self:GetCaster():HasModifier("modifier_pudge_13") then
		return
	end
	if self:GetCaster():HasModifier("modifier_pudge_20") then
		return
...
function modifier_pudge_innate_graft_flesh_custom:GetModifierBonusStats_Intellect()
	if not self:GetCaster():HasModifier("modifier_pudge_20") then
		return
	end
	return self:GetStackCount() * self:GetAbility():GetSpecialValueFor("flesh_heap_strength_buff_amount")
end