...
LinkLuaModifier(
	"modifier_wisp_tether_custom",
	"heroes/npc_dota_hero_wisp_custom/wisp_tether_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_tether_custom_ally",
	"heroes/npc_dota_hero_wisp_custom/wisp_tether_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_tether_custom_latch",
	"heroes/npc_dota_hero_wisp_custom/wisp_tether_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_wisp_tether_custom_slow",
	"heroes/npc_dota_hero_wisp_custom/wisp_tether_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_tether_custom_attack_custom",
	"heroes/npc_dota_hero_wisp_custom/wisp_tether_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function wisp_tether_custom:OnSpellStart()
	...
	if target == nil and self:GetCaster():HasModifier("modifier_wisp_8") then
		local illusions = CreateIllusions(
			...
			self:GetCaster(),
			{
				outgoing_damage = self.modifier_wisp_8_outgoing_damage - 100,
				incoming_damage = self.modifier_wisp_8_incoming_damage - 100,
				duration = self.modifier_wisp_8_duration[self:GetCaster():GetTalentLevel("modifier_wisp_8")],
			},
			1,
			150,
			false,
	...
	if distance >= latch_distance then
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_wisp_tether_custom_latch", { target = target:entindex() })
	end
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_wisp_tether_custom", { target = target:entindex() })
...
function wisp_tether_custom:OnProjectileHit(target, vLocation)
	...
	if target and not target:IsAttackImmune() then
		local modifier_wisp_tether_custom_attack_custom =
			self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_wisp_tether_custom_attack_custom", {})
...
function modifier_wisp_tether_custom_latch:IsHidden()
	return true
end
function modifier_wisp_tether_custom_latch:IsPurgable()
	return false
end
...
function modifier_wisp_tether_custom:IsHidden()
	return false
end
function modifier_wisp_tether_custom:IsPurgable()
	return false
end
function modifier_wisp_tether_custom:GetPriority()
...
function modifier_wisp_tether_custom:OnIntervalThink()
	...
	if self.update_timer > 1 then
		SendOverheadEventMessage(
			...
			OVERHEAD_ALERT_HEAL,
			self.target,
			self.total_gained_health * self.tether_heal_amp,
			nil
		)
		SendOverheadEventMessage(
			nil,
			...
			self.target,
			self.total_gained_mana * self.tether_mana_amp,
			nil
		)
		self.total_gained_mana = 0
		self.total_gained_health = 0
	...
	if self:GetParent():HasModifier("modifier_wisp_tether_custom_latch") then
		return
	end
	...
	if modifier_wisp_relocate_custom and modifier_wisp_relocate_custom:GetElapsedTime() <= 0.2 then
		return
	end
	if (self.target:GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D() <= self.radius then
		return
	end
	self:Destroy()
end
...
function modifier_wisp_tether_custom:OnHealReceived(params)
	...
	if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		if params.inflictor and string.find(params.inflictor:GetName(), "dagon") then
			return
		end
		self.damage = self.damage
			+ ((self:GetAbility().modifier_wisp_1_base_damage + (self:GetAbility().modifier_wisp_1_per_level * self
				:GetCaster()
				:GetLevel())) * 0.115)
			+ (params.gain / 100 * self:GetAbility().modifier_wisp_1)
	else
...
function modifier_wisp_tether_custom:OnManaGained(params)
	...
	if not self.target.GiveMana then
		return
	end
	if self.target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		return
...
function modifier_wisp_tether_custom_ally:IsHidden()
	return false
end
function modifier_wisp_tether_custom_ally:IsPurgable()
	return false
end
...
function modifier_wisp_tether_custom_ally:OnDestroy()
	...
	local modifier_wisp_overcharge_custom_debuff = self:GetParent()
		:FindModifierByName("modifier_wisp_overcharge_custom_debuff")
...
function modifier_wisp_tether_custom_ally:OnAttackLanded(params)
	...
	if not self:GetCaster():HasModifier("modifier_wisp_14") then
		return
	end
...
function modifier_wisp_tether_custom_attack_custom:IsPurgable()
	return false
end
function modifier_wisp_tether_custom_attack_custom:IsPurgeException()
	return false
end
function modifier_wisp_tether_custom_attack_custom:IsHidden()
	return true
end
