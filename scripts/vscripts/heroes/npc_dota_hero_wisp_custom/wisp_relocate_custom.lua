...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
...
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker_end",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_armor_debuff",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker_souls",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker_souls_debuff",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker_souls_cooldown",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_relocate_custom_thinker_souls_buff",
	"heroes/npc_dota_hero_wisp_custom/wisp_relocate_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function wisp_relocate_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_wisp_relocate_custom_cast_delay",
		{ duration = cast_delay, point_x = point.x, point_y = point.y, point_z = point.z }
	)
	AddFOWViewer(self:GetCaster():GetTeamNumber(), point, 200, cast_delay, false)
end

modifier_wisp_relocate_custom_cast_delay = class({})
function modifier_wisp_relocate_custom_cast_delay:IsPurgable()
	return false
end
function modifier_wisp_relocate_custom_cast_delay:IsPurgeException()
	return false
end
function modifier_wisp_relocate_custom_cast_delay:OnCreated(params)
	...
	local channel_pfx = ParticleManager:CreateParticle(
...
function modifier_wisp_relocate_custom_cast_delay:OnIntervalThink()
	...
	if
		...
		or self:GetCaster():IsHexed()
		or self:GetCaster():IsNightmared()
		or self:GetCaster():IsOutOfGame()
		or self:GetCaster():IsRooted()
	then
		self:Destroy()
	end
end
...
function modifier_wisp_relocate_custom_cast_delay:OnDestroy()
	...
	if self:GetRemainingTime() <= 0 then
		...
		if self:GetCaster():HasModifier("modifier_wisp_relocate_custom") then
			local caster_teleport_pfx = ParticleManager:CreateParticle(
			...
			local teleport_out_pfx = ParticleManager:CreateParticle(
			...
			if self:GetCaster():HasModifier("modifier_wisp_18") then
				CreateModifierThinker(
					...
					self:GetAbility(),
					"modifier_wisp_relocate_custom_thinker_souls",
					{ duration = self:GetAbility().modifier_wisp_18_duration },
					self.point,
					self:GetCaster():GetTeamNumber(),
					false
				)
...
function modifier_wisp_relocate_custom_thinker:OnCreated()
	...
	local endpoint_pfx = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_wisp/wisp_relocate_marker_endpoint.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetParent()
	)
...
function modifier_wisp_relocate_custom_thinker_end:OnCreated()
	...
	self.caster_origin_pfx = ParticleManager:CreateParticle(
...
function modifier_wisp_relocate_custom:IsDebuff()
	return false
end
function modifier_wisp_relocate_custom:IsHidden()
	return false
end
function modifier_wisp_relocate_custom:IsPurgable()
	return false
end
function modifier_wisp_relocate_custom:IsStunDebuff()
	return false
end
function modifier_wisp_relocate_custom:IsPurgeException()
	return false
end
function modifier_wisp_relocate_custom:OnCreated(params)
	...
	self.modifier_wisp_relocate_custom_thinker_end = CreateModifierThinker(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_wisp_relocate_custom_thinker_end",
		{ duration = self.return_time },
		self.return_point,
		self:GetCaster():GetTeamNumber(),
		false
	)
	...
	if modifier_wisp_tether_custom and modifier_wisp_tether_custom.target ~= nil and self:GetCaster():IsAlive() then
		...
		if is_allow or self:GetCaster():HasModifier("modifier_wisp_6") then
			...
			self.ally_teleport_pfx = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_wisp/wisp_relocate_teleport.vpcf",
				PATTACH_CUSTOMORIGIN,
				modifier_wisp_tether_custom.target
			)
			ParticleManager:SetParticleControlEnt(
				self.ally_teleport_pfx,
				0,
				modifier_wisp_tether_custom.target,
				PATTACH_POINT,
				"attach_hitloc",
				modifier_wisp_tether_custom.target:GetAbsOrigin(),
				true
			)
			modifier_wisp_tether_custom.target:Interrupt()
			if self:GetCaster():HasModifier("modifier_wisp_6") then
				modifier_wisp_tether_custom.target:AddNewModifier(
					self:GetCaster(),
					self:GetAbility(),
					"modifier_wisp_relocate_custom_armor_debuff",
					{
						duration = self:GetAbility().modifier_wisp_6_duration
							* (1 - modifier_wisp_tether_custom.target:GetStatusResistance()),
					}
				)
			end
		end
	...
	if not self:GetCaster():HasModifier("modifier_wisp_6") then
		self.relocate_timerPfx = ParticleManager:CreateParticle(
...
function modifier_wisp_relocate_custom:OnDestroy()
	...
	if
		self.modifier_wisp_relocate_custom_thinker_end and not self.modifier_wisp_relocate_custom_thinker_end:IsNull()
	then
...
function modifier_wisp_relocate_custom_armor_debuff:GetTexture()
	return "wisp_6"
end
...
function modifier_wisp_relocate_custom_thinker_souls:OnCreated()
	...
	local particle =
		ParticleManager:CreateParticle("particles/wisp_custom/shadow_thinker.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_wisp_relocate_custom_thinker_souls:OnIntervalThink()
	...
	local units = FindUnitsInRadius(
		...
		self:GetParent():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self:GetAbility().modifier_wisp_18_radius),
	...
	for _, unit in pairs(units) do
		if not unit:HasModifier("modifier_wisp_relocate_custom_thinker_souls_cooldown") then
			unit:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_wisp_relocate_custom_thinker_souls_cooldown",
				{ duration = self:GetAbility().modifier_wisp_18_cooldown }
			)
			unit:AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_wisp_relocate_custom_thinker_souls_debuff",
				{ duration = self:GetAbility().modifier_wisp_18_silence_duration * (1 - unit:GetStatusResistance()) }
			)
		end
	end
end
...
function modifier_wisp_relocate_custom_thinker_souls_debuff:GetTexture()
	return "wisp_18"
end
...
function modifier_wisp_relocate_custom_thinker_souls_cooldown:IsHidden()
	return true
end
function modifier_wisp_relocate_custom_thinker_souls_cooldown:IsPurgable()
	return false
end
function modifier_wisp_relocate_custom_thinker_souls_cooldown:IsPurgeException()
	return false
end

modifier_wisp_relocate_custom_thinker_souls_buff = class({})
function modifier_wisp_relocate_custom_thinker_souls_buff:GetTexture()
	return "wisp_19"
end
