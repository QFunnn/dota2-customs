...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_wisp_custom/wisp_overcharge_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_overcharge_custom_magic_immune",
	"heroes/npc_dota_hero_wisp_custom/wisp_overcharge_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_overcharge_custom_debuff",
	"heroes/npc_dota_hero_wisp_custom/wisp_overcharge_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function wisp_overcharge_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_wisp.vpcf",
		context
	)
end
...
function wisp_overcharge_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_wisp_13") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_wisp_overcharge_custom_magic_immune",
			{ duration = self.modifier_wisp_13[self:GetCaster():GetTalentLevel("modifier_wisp_13")] }
		)
		...
		if
			...
			and modifier_wisp_tether_custom.target
			and modifier_wisp_tether_custom.target:GetTeamNumber() == self:GetCaster():GetTeamNumber()
		then
...
function modifier_wisp_overcharge_custom:OnCreated()
	...
	self.overcharge_pfx = ParticleManager:CreateParticle(
...
function modifier_wisp_overcharge_custom_debuff:GetModifierHealthRegenPercentage()
	if not self:GetCaster():HasModifier("modifier_wisp_3") then
		return
	end
	local bonus = 0
	if self:GetCaster():HasModifier("modifier_wisp_5") then
	...
	return -(self.hp_regen + bonus)
		/ 100
		* self:GetAbility().modifier_wisp_3[self:GetCaster():GetTalentLevel("modifier_wisp_3")]
end
...
function modifier_wisp_overcharge_custom_magic_immune:GetTexture()
	return "wisp_13"
end

function modifier_wisp_overcharge_custom_magic_immune:IsPurgable()
	return false
end
