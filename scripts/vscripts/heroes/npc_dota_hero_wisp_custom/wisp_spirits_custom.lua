...
LinkLuaModifier(
	"modifier_wisp_spirits_custom",
	"heroes/npc_dota_hero_wisp_custom/wisp_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_spirits_custom_wisp",
	"heroes/npc_dota_hero_wisp_custom/wisp_spirits_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_wisp_spirits_custom_hero_hit",
	"heroes/npc_dota_hero_wisp_custom/wisp_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_spirits_custom_creep_hit",
	"heroes/npc_dota_hero_wisp_custom/wisp_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_spirits_custom_creep_slow",
	"heroes/npc_dota_hero_wisp_custom/wisp_spirits_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function wisp_spirits_custom:GetIntrinsicModifierName()
	...
	if self:GetCaster():HasModifier("modifier_wisp_10") then
		return
	end
...
function wisp_spirits_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_wisp_spirits_custom", { duration = spirit_duration })
end

modifier_wisp_spirits_custom = class({})
function modifier_wisp_spirits_custom:IsPurgable()
	return false
end
function modifier_wisp_spirits_custom:IsPurgeException()
	return false
end
function modifier_wisp_spirits_custom:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_wisp_21")
end
...
function modifier_wisp_spirits_custom:OnIntervalThink()
	...
	if self.spirits_num_spirits < idealNumSpiritsSpawned then
		local npc_dota_wisp_spirit = CreateUnitByName(
		...
		npc_dota_wisp_spirit:AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_wisp_spirits_custom_wisp",
			{}
	...
	if not self:GetCaster():HasModifier("modifier_wisp_21") then
		...
		for _, spirit in pairs(self.spirits_spiritsSpawned) do
			if spirit ~= nil and not spirit:IsNull() then
				local modifier_wisp_spirits_custom_hero_hit =
					spirit:FindModifierByName("modifier_wisp_spirits_custom_hero_hit")
...
function modifier_wisp_spirits_custom_wisp:OnCreated(params)
	...
	local modifier_wisp_spirits_custom = self:GetCaster():FindModifierByName("modifier_wisp_spirits_custom")
	if not modifier_wisp_spirits_custom then
		return
	end
...
function modifier_wisp_spirits_custom_wisp:OnIntervalThink()
	...
	if
		self:GetParent():HasModifier("modifier_wisp_spirits_custom_hero_hit")
...
function modifier_wisp_spirits_custom_wisp:Explosion(is_destroy)
	...
	if self:GetParent():HasModifier("modifier_wisp_spirits_custom_hero_hit") then
		return
	end
	...
	local enemies_hit = FindUnitsInRadius(
		self:GetCaster():GetTeam(),
	...
	if self:GetCaster():HasModifier("modifier_wisp_15") then
		creep_damage = creep_damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_wisp_15_creep[self:GetCaster():GetTalentLevel("modifier_wisp_15")]
			)
		hero_damage = hero_damage
			+ (
				self:GetCaster():GetMana()
				/ 100
				* self:GetAbility().modifier_wisp_15[self:GetCaster():GetTalentLevel("modifier_wisp_15")]
			)
	end
	local damage_table = {
		...
		if enemy:IsAlive() and not self:GetParent():IsNull() then
			...
			if self:GetParent():HasModifier("modifier_wisp_21") then
				enemy:AddNewModifier(
					...
					"modifier_wisp_spirits_custom_creep_slow",
					{
						duration = self:GetAbility().modifier_wisp_21_duration * (1 - self:GetParent()
							:GetStatusResistance()),
					}
				)
			end
	...
	if is_destroy then
		...
		self:GetParent()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_wisp_spirits_custom_hero_hit", {})
	end
end
		...
		if enemy:IsAlive() and not self:GetParent():IsNull() then
			...
			if enemy:IsConsideredHero() and not enemy:IsIllusion() then
				...
				enemy:AddNewModifier(
					...
					self:GetAbility(),
					"modifier_wisp_spirits_custom_creep_slow",
					{ duration = self:GetAbility().modifier_wisp_21_duration }
				)
			end
			if hit then
				ApplyDamage(damage_table)
...
function modifier_wisp_spirits_custom_creep_hit:IsHidden()
	return true
end

modifier_wisp_spirits_custom_hero_hit = class({})
function modifier_wisp_spirits_custom_hero_hit:IsHidden()
	return true
end
...
function modifier_wisp_spirits_custom_creep_slow:GetTexture()
	return "wisp_21"
end
