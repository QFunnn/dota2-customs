...
LinkLuaModifier(
	...
	"heroes/hero_vengefulspirit/vengefulspirit_magic_missile_lua.lua",
	...
	if IsValid(hTrueTarget) then
		if not hTrueTarget:TriggerSpellAbsorb(self) then
			...
			if IsValid(hTrueTarget) and hTrueTarget:IsAlive() then
				hTrueTarget:AddNewModifier(
					hCaster,
					self,
					"modifier_stunned",
					{ duration = magic_missile_stun * hTrueTarget:GetStatusResistanceFactor(hCaster) }
				)
			end
			--рикошет блокируется Linken's Sphere
...
function vengefulspirit_magic_missile_lua:Bounce(hCaster, hTarget, bFirebyAttack, tHashtable)
	...
	local hNextTarget = GetBounceTarget(
		hTarget,
...
function modifier_vengefulspirit_magic_missile_lua:OnAttack(params)
	...
	if IsServer() then
		if
			...
			and (not hParent:IsIllusion() or hParent:HasModifier("modifier_ogre_multicast_lua_bonus"))
		then
