...
LinkLuaModifier(
	"modifier_ability_undying_reincarnate_buff",
	"heroes/hero_undying/reincarnate",
...
function modifier_ability_undying_reincarnate:Reincarnate(event)
	...
	if
		...
		and target
		and target == parent
		and parent:GetHealth() == 1
		and not parent:HasModifier("modifier_dazzle_shallow_grave")
	then
		...
		if IsServer() then
			...
			local fx = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_undying/undying_tower_destruction_lv.vpcf",
			...
			fx = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_undying/undying_scepter_tomb_enter.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				parent
			)
			ParticleManager:SetParticleControl(fx, 1, parent:GetAbsOrigin())
