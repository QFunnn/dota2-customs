...
function modifier_undying_tombstone_lua_passive:IsHidden()
	return true
end
...
function modifier_undying_tombstone_lua_passive:OnIntervalThink()
	if IsServer() then
		--убираем мёртвого зомби из очереди
...
function modifier_undying_tombstone_lua_passive:OnAttackLanded(keys)
	...
	if
		self:GetParent():HasModifier("modifier_item_aghanims_shard")
		and self:GetParent():HasModifier("modifier_undying_flesh_golem")
	then
		if #self.hZombies < self.passive_max_zombie then
			local sZombieName = "npc_dota_creature_tombstone_zombie"
			--50% шанс призвать ползающего зомби
			...
			if hZombie ~= nil then
				...
				hZombie:AddNewModifier(
					self:GetParent(),
					self:GetAbility(),
					"modifier_kill",
					{ duration = self.zombie_duration }
				)
