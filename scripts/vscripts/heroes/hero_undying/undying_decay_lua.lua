...
LinkLuaModifier(
	"modifier_undying_decay_lua_debuff_counter",
	"heroes/hero_undying/undying_decay_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_undying_decay_lua_buff", "heroes/hero_undying/undying_decay_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_undying_decay_lua_buff_counter",
	"heroes/hero_undying/undying_decay_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function undying_decay_lua:OnSpellStart()
	...
	local decay_damage = self:GetSpecialValueFor("decay_damage")
		+ caster:GetTalentValue("special_bonus_unique_undying_8")
	local radius = self:GetSpecialValueFor("radius")
	local str_steal_hero = (has_scepter and self:GetSpecialValueFor("str_steal_hero_scepter"))
		or self:GetSpecialValueFor("str_steal_hero")
	local str_steal_creep = (has_scepter and self:GetSpecialValueFor("str_steal_creep_scepter"))
		or self:GetSpecialValueFor("str_steal_creep")
	...
	if duration_talent then
		decay_duration = decay_duration + duration_talent:GetSpecialValueFor("value")
	end

	-- Cast effects
	caster:EmitSound("Hero_Undying.Decay.Cast")
	caster:EmitSound("Hero_Undying.Decay.Target")

	local particle_cast =
		ParticleManager:GetParticleReplacement("particles/units/heroes/hero_undying/undying_decay.vpcf", caster)
	local particle_enemy = ParticleManager:GetParticleReplacement(
		...
		if not enemy:IsIllusion() then
			if enemy:IsRealHero() then
				local debuff = enemy:AddNewModifier(
					caster,
					self,
					"modifier_undying_decay_lua_debuff",
				...
				local buff = caster:AddNewModifier(
					caster,
					self,
					"modifier_undying_decay_lua_buff",
					{ duration = decay_duration }
				)
...
function modifier_undying_decay_lua_debuff:IsHidden()
	return false
end
function modifier_undying_decay_lua_debuff:IsDebuff()
	return true
end
function modifier_undying_decay_lua_debuff:IsPurgable()
	return false
end
...
function modifier_undying_decay_lua_buff:IsDebuff()
	return false
end
