...
LinkLuaModifier("modifier_undying_decay_lua_debuff_counter", "heroes/hero_undying/undying_decay_lua",
	LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_undying_decay_lua_buff", "heroes/hero_undying/undying_decay_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_undying_decay_lua_buff_counter", "heroes/hero_undying/undying_decay_lua",
	LUA_MODIFIER_MOTION_NONE)
...
function undying_decay_lua:OnSpellStart()
	...
	local particle_cast = ParticleManager:GetParticleReplacement(
		"particles/units/heroes/hero_undying/undying_decay.vpcf", caster)
	local particle_enemy = ParticleManager:GetParticleReplacement(
		"particles/units/heroes/hero_undying/undying_decay_strength_xfer.vpcf", caster)
	...
	local targets = FindUnitsInRadius(caster:GetTeam(), target_loc, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
	...
	for _, enemy in pairs(targets) do
		...
		if not enemy:IsIllusion() then
			if enemy:IsRealHero() then
				local debuff = enemy:AddNewModifier(caster, self, "modifier_undying_decay_lua_debuff",
				...
				local buff = caster:AddNewModifier(caster, self, "modifier_undying_decay_lua_buff",
					{ duration = decay_duration })
