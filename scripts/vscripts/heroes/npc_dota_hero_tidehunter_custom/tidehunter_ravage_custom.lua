...
function tidehunter_ravage_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_tidehunter_6") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_tidehunter_6[self:GetCaster():GetTalentLevel("modifier_tidehunter_6")]
			)
	end
	...
	ring:SetCallback(function(enemy)
		local knockback = enemy:AddNewModifier(
			...
			self,
			"modifier_generic_arc_lua_tidehunter",
			{ duration = knock_duration * (1 - enemy:GetStatusResistance()), height = height, isStun = true }
		)
		enemy:AddNewModifier(
			caster,
			self,
			"modifier_stunned",
			{ duration = (duration + knock_duration) * (1 - enemy:GetStatusResistance()) }
		)
...
function tidehunter_ravage_custom:CreateOneRavageAttack(target)
	...
	local knockback = target:AddNewModifier(
		...
		self,
		"modifier_generic_arc_lua_tidehunter",
		{ duration = 0.5 * (1 - target:GetStatusResistance()), height = 350, isStun = true }
	)
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_stunned",
		{ duration = (duration + 0.5) * (1 - target:GetStatusResistance()) }
	)
	if self:GetCaster():HasModifier("modifier_tidehunter_7") then
		duration = duration + self.modifier_tidehunter_7_stun
	end
...
function modifier_generic_arc_lua_tidehunter:OnRemoved() end
...
function modifier_generic_ring_lua:OnRemoved() end
