...
LinkLuaModifier(
	"modifier_tidehunter_gush_custom",
	"heroes/npc_dota_hero_tidehunter_custom/tidehunter_gush_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_generic_arc_lua_tidehunter",
	"heroes/npc_dota_hero_tidehunter_custom/tidehunter_ravage_custom",
...
function tidehunter_gush_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_tidehunter.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_tidehunter.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_tidehunter.vpcf",
		context
	)
end
...
function tidehunter_gush_custom:GetCastRange(vLocation, hTarget)
	...
	if self:GetCaster():HasModifier("modifier_tidehunter_21") then
		bonus = self:GetCaster():GetIntellect(false)
			* self.modifier_tidehunter_21[self:GetCaster():GetTalentLevel("modifier_tidehunter_21")]
	end
...
function tidehunter_gush_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if data.scepter == 0 and target:TriggerSpellAbsorb(self) then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_tidehunter_19") then
		local bonus_dmg = (
			(target:GetAbsOrigin() - self:GetCaster():GetAbsOrigin()):Length2D() / self.modifier_tidehunter_19_distance
		) * self.modifier_tidehunter_19_damage
...
function modifier_tidehunter_gush_custom:GetModifierPropertyRestorationAmplification()
	if not self:GetCaster():HasModifier("modifier_tidehunter_20") then
		return
	end
