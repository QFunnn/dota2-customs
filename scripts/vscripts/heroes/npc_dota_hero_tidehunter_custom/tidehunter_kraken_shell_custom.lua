...
LinkLuaModifier(
	"modifier_tidehunter_kraken_shell_custom",
	"heroes/npc_dota_hero_tidehunter_custom/tidehunter_kraken_shell_custom",
...
function modifier_tidehunter_kraken_shell_custom_active:OnCreated()
	...
	if self:GetParent():HasModifier("modifier_tidehunter_18") then
		self.active_move_speed_penalty_pct = self:GetAbility().modifier_tidehunter_18_movespeed[self:GetCaster()
			:GetTalentLevel("modifier_tidehunter_18")]
	end
	if not IsServer() then
		return
	end
...
function modifier_tidehunter_kraken_shell_custom_active:CheckState()
	if not self:GetParent():HasModifier("modifier_tidehunter_18") then
		return
	end
	return {
...
function modifier_tidehunter_kraken_shell_custom:IsHidden()
	return true
end
function modifier_tidehunter_kraken_shell_custom:IsDebuff()
	return false
end
function modifier_tidehunter_kraken_shell_custom:IsPurgable()
	return false
end
