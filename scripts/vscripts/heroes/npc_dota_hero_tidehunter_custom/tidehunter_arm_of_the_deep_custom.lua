...
function tidehunter_arm_of_the_deep_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_tidehunter/tidehunter_arm_of_the_deep_projectile.vpcf",
		context
	)
end
...
function tidehunter_arm_of_the_deep_custom:StartMiniRavage(direction)
	...
	if tidehunter_ravage_custom and tidehunter_ravage_custom:GetLevel() > 0 then
		local info = {
			...
			fDistance = (tidehunter_ravage_custom:GetSpecialValueFor("radius") / 100 * self:GetSpecialValueFor(
				"range_pct"
...
function tidehunter_arm_of_the_deep_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if tidehunter_ravage_custom then
		...
		target:AddNewModifier(
			...
			"modifier_stunned",
			{
				duration = (tidehunter_ravage_custom:GetSpecialValueFor("duration") / 100 * self:GetSpecialValueFor(
					"duration_pct"
				)) + 0.5,
			}
		)
		...
		if self:GetCaster():HasModifier("modifier_tidehunter_6") then
			damage = damage
				+ (
					...
					/ 100
					* tidehunter_ravage_custom.modifier_tidehunter_6[self:GetCaster()
						:GetTalentLevel("modifier_tidehunter_6")]
				)
		end
