...
LinkLuaModifier(
	...
	"heroes/hero_monkey_king/monkey_king_jingu_mastery_lua",
...
LinkLuaModifier(
	"modifier_monkey_king_jingu_mastery_lua_count_debuff",
	"heroes/hero_monkey_king/monkey_king_jingu_mastery_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_jingu_mastery_talent_buff",
	"heroes/hero_monkey_king/monkey_king_jingu_mastery_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_monkey_king_jingu_mastery_talent_buff:IsPurgable()
	return false
end
function modifier_monkey_king_jingu_mastery_talent_buff:IsPurgeException()
	return false
end

function monkey_king_jingu_mastery_lua:IsHiddenWhenStolen()
	return false
end
function monkey_king_jingu_mastery_lua:IsRefreshable()
	return false
end
function monkey_king_jingu_mastery_lua:IsStealable()
	return false
end
function monkey_king_jingu_mastery_lua:IsNetherWardStealable()
	return false
end
...
function modifier_monkey_king_jingu_mastery_lua:AttackLandedModifier(keys)
	...
	if attacker ~= self.parent and self.caster ~= attacker then
		return
	end

	if
		target:IsBuilding()
		or self.caster:PassivesDisabled()
		or not (self.caster:IsRealHero() or self.caster:IsTempestDouble())
		or self.caster:IsIllusion()
	then
		return
	end

	if target:IsIllusion() or target:IsOther() then
		return
	end

	-- if caster has already buff do nothing
	if self.caster:HasModifier("modifier_monkey_king_jingu_mastery_lua_buff") then
		return
	end

	-- prevent monkey ulti to get buffed
	if attacker:HasModifier("modifier_monkey_king_fur_army_soldier") then
		return
	end
	...
	if not jinguMasteryStack_debuff then
		jinguMasteryStack_debuff = target:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_monkey_king_jingu_mastery_lua_count_debuff",
		...
		if not jinguMasteryStack_debuff then
			return
		end -- error log tells me that newly created modifier is missing
	...
	if jinguMasteryStack_debuff:GetStackCount() + 1 >= self.required_hit then
		...
		local jinguBuffStack = self.caster:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_monkey_king_jingu_mastery_lua_buff",
			{ duration = self.max_duration }
		)
		if not jinguBuffStack then
			return
		end

		jinguBuffStack:SetStackCount(
			self.ability:GetSpecialValueFor("charges")
				+ self.parent:GetTalentValue("special_bonus_unique_monkey_king_11")
		)
		self:GetAbility().jingu_mastery_buff_modifier = jinguBuffStack

		local jingu_start_particle = ParticleManager:CreateParticle(
		...
		if not target.jingu_overhead_particle then
			target.jingu_overhead_particle = ParticleManager:CreateParticle(
		...
		ParticleManager:SetParticleControl(
			target.jingu_overhead_particle,
			1,
			Vector(0, jinguMasteryStack_debuff:GetStackCount(), 0)
		)
	end
...
function modifier_monkey_king_jingu_mastery_lua_count_debuff:IsPurgable()
	return false
end
function modifier_monkey_king_jingu_mastery_lua_count_debuff:IsPurgeException()
	return false
end
function modifier_monkey_king_jingu_mastery_lua_count_debuff:OnCreated() end

function modifier_monkey_king_jingu_mastery_lua_count_debuff:OnRefresh() end
...
function modifier_monkey_king_jingu_mastery_lua_buff:IsPurgable()
	return false
end
function modifier_monkey_king_jingu_mastery_lua_buff:IsPurgeException()
	return false
end
...
function modifier_monkey_king_jingu_mastery_lua_buff:GetModifierAttackSpeedBonus_Constant()
	if not self:GetCaster():HasTalent("special_bonus_unique_cha_monkey_king") then
		return
	end
	return 100
end
function modifier_monkey_king_jingu_mastery_lua_buff:OnCreated()
	...
	if caster.jingubuff_overhead_particle == nil then
		caster.jingubuff_overhead_particle = ParticleManager:CreateParticle(
	...
	if caster.jingubuff_weapon_glow_particle == nil then
		caster.jingubuff_weapon_glow_particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_monkey_king/monkey_king_tap_buff.vpcf",
			PATTACH_ROOTBONE_FOLLOW,
			caster
		)
		ParticleManager:SetParticleControlEnt(
			caster.jingubuff_weapon_glow_particle,
			0,
			caster,
			PATTACH_ROOTBONE_FOLLOW,
			"attach_hitloc",
...
function modifier_monkey_king_jingu_mastery_lua_buff:AttackLandedModifier(keys)
	...
	if not (attacker:IsRealHero() or attacker:IsTempestDouble()) or attacker:IsIllusion() then
		return
	end
