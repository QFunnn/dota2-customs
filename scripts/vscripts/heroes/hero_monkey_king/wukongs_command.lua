...
function MakeMonkeys(keys)
	...
	local i = 0
	ability.monkeyTimer = Timers:CreateTimer(function()
		i = i + 1
		--create monkey
		ability.monkeys["idle"][i] = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_monkey_king/monkey_king_furarmy_singlemonkey.vpcf",
			PATTACH_WORLDORIGIN,
			caster
		)
		ability.monkeys["attack"][i] = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_monkey_king/monkey_king_furarmy_singlemonkey_attack.vpcf",
			PATTACH_WORLDORIGIN,
			caster
		)
