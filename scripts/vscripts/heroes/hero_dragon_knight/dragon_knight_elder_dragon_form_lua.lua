...
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_lua_form",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_lua_green",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_lua_red",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_lua_blue",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_lua_scepter",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_dragon_knight_elder_dragon_form_lua_form:OnDestroy()
	if IsServer() then
		...
		if
			GetUnitKeyValuesByName(hParent:GetUnitName()).AttackCapabilities ~= "DOTA_UNIT_CAP_RANGED_ATTACK"
			and not hParent:HasModifier("modifier_terrorblade_metamorphosis")
		then
		...
		local iParticleID = ParticleManager:CreateParticle(
			ParticleManager:GetParticleReplacement(self.sParticlePath, self:GetCaster()),
...
function modifier_dragon_knight_elder_dragon_form_lua_form:GetModifierCastRangeBonusStacking(params)
	if
		IsValid(params.ability)
		and bit.band(tonumber(tostring(params.ability:GetBehavior())), DOTA_ABILITY_BEHAVIOR_ATTACK)
...
function modifier_dragon_knight_elder_dragon_form_lua_green:GetModifierProcAttack_Feedback(params)
	if not IsValid(params.target) or params.target:GetClassname() == "dota_item_drop" then
		return
	end
...
function modifier_dragon_knight_elder_dragon_form_lua_red:GetModifierProcAttack_Feedback(params)
	...
	for n, hTarget in pairs(tTargets) do
		if hTarget ~= params.target then
			...
			local modifier_dragon_knight_elder_dragon_form_lua_green =
				params.attacker:FindModifierByName("modifier_dragon_knight_elder_dragon_form_lua_green")
...
function modifier_dragon_knight_elder_dragon_form_lua_blue:OnDestroy() end
...
function modifier_dragon_knight_elder_dragon_form_lua_blue:GetModifierProcAttack_Feedback(params)
	...
	for n, hTarget in pairs(tTargets) do
		hTarget:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_dragon_knight_elder_dragon_form_lua_frost",
			{ duration = self.frost_duration * hTarget:GetStatusResistanceFactor(params.attacker) }
		)
	end
	-- end
end
...
function modifier_dragon_knight_elder_dragon_form_lua_frost:GetModifierMoveSpeedBonus_Percentage(params)
	if IsValid(self:GetCaster()) and self:GetCaster():IsIllusion() then
		return
	end
