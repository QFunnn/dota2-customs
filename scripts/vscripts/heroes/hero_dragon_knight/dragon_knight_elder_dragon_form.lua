...
LinkLuaModifier(
	"modifier_dragon_knight_elder_dragon_form_custom",
	"heroes/hero_dragon_knight/dragon_knight_elder_dragon_form",
...
function modifier_dragon_knight_elder_dragon_form_custom:OnCreated(kv)
	...
	if blood_innate and not blood_innate:IsNull() then
		...
		print(
			"[DK DRAGON FORM] innate blood: regen_bonus="
				.. self.blood_bonus_regen
				.. " armor_bonus="
				.. self.blood_bonus_armor
		)
	end
	self.magic_resist = 0
...
function modifier_dragon_knight_elder_dragon_form_custom:GetModifierProcAttack_Feedback(params)
	...
	if self.parent.anchor_attack_talent then
		print("СОРРИ ПАССИВКА ТАЙДА")
		return
	end
	if self.parent.bCanTriggerLock then
		print("СОРРИ ПАССИВКА ВОЙДА")
		return
	end
...
function modifier_dragon_knight_elder_dragon_form_custom:Splash(target, damage)
	...
	if self.parent:HasModifier("modifier_item_bfury_2") then
		return
	end
	if self.parent:HasModifier("modifier_item_bfury_3") then
		return
	end
	if self.parent:HasModifier("modifier_item_ranged_cleave") then
		return
	end
	if self.parent:HasModifier("modifier_item_ranged_cleave_2") then
		return
	end
	if self.parent:HasModifier("modifier_item_ranged_cleave_3") then
		return
	end
	...
	if
		self.parent:HasAbility("ursa_fury_swipes") and target:HasModifier("modifier_ursa_fury_swipes_damage_increase")
	then
