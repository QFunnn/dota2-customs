...
function modifier_axe_counter_helix_lua:OnDestroy() end
...
function modifier_axe_counter_helix_lua:OnAttackLanded(params)
	if IsServer() then
		...
		if params.attacker:IsOther() or params.attacker:IsBuilding() then
			return
		end
		...
		--если есть шард — шанс выше на 10%
		...
		for _, enemy in pairs(enemies) do
			...
			--если есть шард — добавляем debuff
			if shard_debuff_duration > 0 then
				if not enemy:HasModifier("modifier_axe_counter_helix_lua_debuff") then
					local hDebuff = enemy:AddNewModifier(
						self:GetParent(),
						self:GetAbility(),
						"modifier_axe_counter_helix_lua_debuff",
						{ duration = shard_debuff_duration * enemy:GetStatusResistanceFactor(self:GetParent()) }
					)
					if hDebuff then
					...
					if
						enemy:FindModifierByName("modifier_axe_counter_helix_lua_debuff"):GetStackCount()
						< shard_max_stacks
					then
						enemy:FindModifierByName("modifier_axe_counter_helix_lua_debuff"):IncrementStackCount()
					end
					enemy
						:FindModifierByName("modifier_axe_counter_helix_lua_debuff")
						:SetDuration(shard_debuff_duration, true)
				end
			end
		end
...
function modifier_axe_counter_helix_lua:GetModifierIncomingDamage_Percentage(params)
	...
	if IsServer() then
		if
			IsValid(hCaster)
			and params.target == hCaster
			and params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK
			and IsValid(params.attacker)
		then
			if params.attacker:FindModifierByNameAndCaster("modifier_axe_counter_helix_lua_debuff", hCaster) ~= nil then
				return -params.attacker
					:FindModifierByNameAndCaster("modifier_axe_counter_helix_lua_debuff", hCaster)
					:GetStackCount() * shard_damage_reduction
			end
		end
	end
end