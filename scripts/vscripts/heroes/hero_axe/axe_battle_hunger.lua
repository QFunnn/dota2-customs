...
LinkLuaModifier(
	...
	"heroes/hero_axe/axe_battle_hunger",
...
function axe_battle_hunger_custom:OnSpellStart(new_target)
	...
	local mod = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_axe_battle_hunger_custom_buff",
...
function modifier_axe_battle_hunger_custom_buff:GetModifierPhysicalArmorBonus()
	if self:GetCaster():HasScepter() then
		return (
			self:GetAbility():GetSpecialValueFor("scepter_armor_change")
			* self:GetCaster():GetModifierStackCount("modifier_axe_battle_hunger_custom_hero", self:GetCaster())
		)
			+ (
				(self:GetAbility():GetSpecialValueFor("scepter_armor_change") / 2)
				* self:GetCaster():GetModifierStackCount("modifier_axe_battle_hunger_custom_creep", self:GetCaster())
			)
	end
	return 0
end
...
function modifier_axe_battle_hunger_custom_buff:GetModifierMoveSpeedBonus_Percentage()
	return (
		self:GetAbility():GetSpecialValueFor("speed_bonus")
		* self:GetCaster():GetModifierStackCount("modifier_axe_battle_hunger_custom_hero", self:GetCaster())
	)
		+ (
			(self:GetAbility():GetSpecialValueFor("speed_bonus") / 2)
			* self:GetCaster():GetModifierStackCount("modifier_axe_battle_hunger_custom_creep", self:GetCaster())
		)
end
...
function modifier_axe_battle_hunger_custom_debuff:OnDestroy(kv)
	...
	if
		not self:GetCaster():HasModifier("modifier_axe_battle_hunger_custom_hero")
		and not self:GetCaster():HasModifier("modifier_axe_battle_hunger_custom_creep")
	then
...
function modifier_axe_battle_hunger_custom_creep:IsHidden()
	return true
end
function modifier_axe_battle_hunger_custom_creep:IsPurgable()
	return false
end
...
function modifier_axe_battle_hunger_custom_hero:IsHidden()
	return true
end
function modifier_axe_battle_hunger_custom_hero:IsPurgable()
	return false
end
