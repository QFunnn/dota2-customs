...
LinkLuaModifier(
	...
	"heroes/hero_axe/axe_culling_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_culling_blade_custom_armor",
	"heroes/hero_axe/axe_culling_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function axe_culling_blade_custom:OnSpellStart()
	...
	local start_damage = ApplyDamage({
		victim = target,
		attacker = caster,
		damage = damage,
	...
	if not target:IsAlive() then
		...
		if not target:IsHero() then
			if self.bRoundDueled == true then
				return
			end
...
function modifier_axe_culling_blade_custom_armor:IsHidden()
	return self:GetStackCount() == 0
end
function modifier_axe_culling_blade_custom_armor:IsPurgable()
	return false
end
function modifier_axe_culling_blade_custom_armor:IsPurgeException()
	return false
end
function modifier_axe_culling_blade_custom_armor:RemoveOnDeath()
	return false
end
