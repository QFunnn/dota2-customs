...
LinkLuaModifier(
	"modifier_anchor_smash_lua",
	"heroes/hero_tidehunter/modifier_anchor_smash_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_anchor_smash_reduction_lua",
	"heroes/hero_tidehunter/modifier_anchor_smash_reduction_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function tidehunter_anchor_smash_lua:OnAbilityUpgrade(ability)
	if ability:GetAbilityName() == "tidehunter_kraken_shell" then
		...
		if modifier then
			modifier:SetStackCount(self.kills or 0)
		end
	end
end
...
function tidehunter_anchor_smash_lua:OnSpellStart(...)
	...
	for i, enemy in pairs(enemies) do
		Timers:CreateTimer(delay * i, function()
			...
			if enemy:IsAttackImmune() or enemy:IsInvulnerable() then
				return
			end
