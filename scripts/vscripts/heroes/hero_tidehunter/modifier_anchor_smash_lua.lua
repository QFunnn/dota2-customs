...
function modifier_anchor_smash_lua:IsHidden()
	return self:GetStackCount() <= 0
end
function modifier_anchor_smash_lua:IsPurgable()
	return false
end
function modifier_anchor_smash_lua:RemoveOnDeath()
	return false
end
...
function modifier_anchor_smash_lua:GetModifierProcAttack_Feedback(keys)
	...
	if not self.is_smash_on_attack then
		return
	end
	...
	if not RollPercentage(self.smash_on_attack_chance) then
		return
	end
	if self.parent:HasModifier("modifier_anchor_smash_cooldown_lua") then
		return
	end
	self.parent:AddNewModifier(
		self.parent,
		self.ability,
		"modifier_anchor_smash_cooldown_lua",
		{ duration = self.smash_on_attack_cooldown }
	)
...
function modifier_anchor_smash_lua:GetModifierPreAttack_BonusDamage()
	if self.parent.anchor_attack then
...
LinkLuaModifier(
	"modifier_anchor_smash_cooldown_lua",
	"heroes/hero_tidehunter/modifier_anchor_smash_lua",
	LUA_MODIFIER_MOTION_NONE
)

function modifier_anchor_smash_cooldown_lua:IsHidden()
	return true
end
function modifier_anchor_smash_cooldown_lua:IsDebuff()
	return true
end
function modifier_anchor_smash_cooldown_lua:IsPurgable()
	return false
end
function modifier_anchor_smash_cooldown_lua:RemoveOnDeath()
	return true
end
function modifier_anchor_smash_cooldown_lua:GetAttributes()
