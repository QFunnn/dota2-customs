...
LinkLuaModifier(
	"modifier_life_stealer_feast_lua",
	"heroes/hero_life_stealer/life_stealer_feast_lua.lua",
		...
		if IsValid(hTarget) and hTarget:IsAlive() and hTarget.IsRoshan and not hTarget:IsRoshan() then
			...
			then
				...
				SendOverheadEventMessage(
					hParent:GetPlayerOwner(),
					OVERHEAD_ALERT_HEAL,
					hParent,
					hTarget:GetMaxHealth() * self.hp_leech_percent * 0.01,
					hParent:GetPlayerOwner()
				)
