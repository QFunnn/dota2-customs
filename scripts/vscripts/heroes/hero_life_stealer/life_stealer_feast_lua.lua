...
LinkLuaModifier("modifier_life_stealer_feast_lua", "heroes/hero_life_stealer/life_stealer_feast_lua.lua", LUA_MODIFIER_MOTION_NONE)
--Abilities
if life_stealer_feast_lua == nil then
	life_stealer_feast_lua = class({})
end
...
if modifier_life_stealer_feast_lua == nil then
	modifier_life_stealer_feast_lua = class({})
end
...
function modifier_life_stealer_feast_lua:OnDestroy()
	if IsServer() then
	end
end
	...
	if params.attacker == hParent and not hParent:PassivesDisabled() then
		if IsValid(hTarget) and hTarget:IsAlive() and hTarget.IsRoshan and not hTarget:IsRoshan() then
			if UnitFilter(hTarget, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, hParent:GetTeamNumber()) == UF_SUCCESS then
				local particleID = ParticleManager:CreateParticle("particles/generic_gameplay/generic_lifesteal.vpcf", PATTACH_ABSORIGIN_FOLLOW, hParent)
				ParticleManager:ReleaseParticleIndex(particleID)
				SendOverheadEventMessage(hParent:GetPlayerOwner(), OVERHEAD_ALERT_HEAL, hParent, hTarget:GetMaxHealth() * self.hp_leech_percent * 0.01, hParent:GetPlayerOwner())
				hParent:HealWithParams(hTarget:GetMaxHealth() * self.hp_leech_percent * 0.01, self:GetAbility(), true, true, hParent, false)
			end
		end
	end
end
