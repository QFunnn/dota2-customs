...
LinkLuaModifier(
	"modifier_nevermore_righteous_lord",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_righteous_lord",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_righteous_lord_debuff",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_righteous_lord",
	LUA_MODIFIER_MOTION_NONE
)
...
function nevermore_righteous_lord:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_doom_bringer/doom_bringer_doom_aura_for_nevermore.vpcf",
		context
	)
...
function modifier_nevermore_righteous_lord:OnCreated()
	...
	ParticleManager:SetParticleControl(
		self.particle,
		1,
		Vector(self.current_radius, self.current_radius, self.current_radius)
	)
...
function modifier_nevermore_righteous_lord:OnIntervalThink()
	...
	self.aura = false
	local damage_self = (
		(self:GetParent():GetMaxHealth() / 100 * self:GetAbility():GetSpecialValueFor("damage_self"))
		+ self:GetAbility():GetSpecialValueFor("base_damage_self")
	) * self:GetAbility():GetSpecialValueFor("interval")
	if not self:GetParent():IsMagicImmune() then
		damage_self = ApplyDamage({
...
function modifier_nevermore_righteous_lord_debuff:OnIntervalThink()
	...
	local damage = (
		(self:GetCaster():GetMaxHealth() / 100 * self:GetAbility():GetSpecialValueFor("damage_enemy_health"))
		+ self:GetAbility():GetSpecialValueFor("base_damage_enemy")
	) * self:GetAbility():GetSpecialValueFor("interval")
	...
	if self:GetCaster():HasModifier("modifier_nevermore_1") then
		if
			self:GetParent():GetUnitName() == "npc_woda_pig"
			or self:GetParent():GetUnitName() == "npc_woda_frog"
			or self:GetParent():GetUnitName() == "npc_woda_pig_pve"
			or self:GetParent():GetUnitName() == "npc_woda_frog_pve"
			or self:GetParent():GetUnitName() == "npc_dota_weaver_swarm"
		then
