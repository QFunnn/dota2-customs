...
LinkLuaModifier(
	"modifier_nevermore_requiem_custom_debuff",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_requiem_custom",
...
function nevermore_requiem_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_nevermore.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_nevermore.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_nevermore.vpcf",
		context
	)
end
...
function nevermore_requiem_custom:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	if not death_cast then
		if not target:HasModifier("modifier_nevermore_requiem_fear") then
			...
			target:FindModifierByName(modifier_debuff):SetDuration(
				math.min(target:FindModifierByName(modifier_debuff):GetRemainingTime() + slow_duration, max_duration)
					* (1 - target:GetStatusResistance()),
				true
			)
		end
...
function modifier_nevermore_requiem_custom_debuff:IsHidden()
	return false
end
function modifier_nevermore_requiem_custom_debuff:IsPurgable()
	return true
end
function modifier_nevermore_requiem_custom_debuff:IsDebuff()
	return true
end
