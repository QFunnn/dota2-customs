...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_nevermore_custom/nevermore_necromastery_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_necromastery_custom_bonus_souls",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_necromastery_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_necromastery_custom_9_debuff",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_necromastery_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_nevermore_necromastery_custom:DestroyOnExpire()
	return false
end
...
function modifier_nevermore_necromastery_custom:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_nevermore_8") then
		return
	end
	local cooldown = self:GetAbility().modifier_nevermore_8_cooldown[self:GetCaster()
		:GetTalentLevel("modifier_nevermore_8")]
...
function modifier_nevermore_necromastery_custom:OnDeath(params)
	...
	if params.attacker ~= self:GetParent() and params.unit == self:GetParent() and self:GetParent():IsRealHero() then
		...
		if
			not self:GetParent():HasModifier("modifier_wodaduel1")
			and not self:GetParent():HasModifier("modifier_wodaduel_duo")
			and not self:GetParent():HasModifier("modifier_wodaduel2")
			and not self:GetParent():HasModifier("modifier_nevermore_12")
		then
...
function modifier_nevermore_necromastery_custom:GetModifierIncomingDamage_Percentage()
	if not self:GetCaster():HasModifier("modifier_nevermore_19") then
		return 0
	end
end

function modifier_nevermore_necromastery_custom:GetModifierAttackSpeedBonus_Constant()
	if not self:GetCaster():HasModifier("modifier_nevermore_13") then
		return 0
	end
	return self:GetStackCount()
		* self:GetAbility().modifier_nevermore_13[self:GetCaster():GetTalentLevel("modifier_nevermore_13")]
end
...
function modifier_nevermore_necromastery_custom:OnAttack(params)
	...
	if
		self:GetParent():HasModifier("modifier_nevermore_9")
		and not self:GetParent():HasModifier("modifier_nevermore_necromastery_custom_9_debuff")
...
function modifier_nevermore_necromastery_custom:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_nevermore_14") then
		return
	end
	self:GetParent():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_nevermore_necromastery_custom_bonus_souls",
		{ duration = self:GetAbility().modifier_nevermore_14_duration }
	)
	self:AddNewSoul(1)
end
...
function modifier_nevermore_necromastery_custom:GetMaxSoulsCounter()
	...
	if self:GetCaster():HasModifier("modifier_nevermore_18") then
		maximum = maximum
			+ (
				(math.min(self:GetCaster():GetIntellect(false), 600) / self:GetAbility().modifier_nevermore_18_int)
				* self:GetAbility().modifier_nevermore_18_bonus
			)
	end
	if self:GetCaster():HasModifier("modifier_nevermore_10") then
		maximum = maximum
			+ self:GetAbility().modifier_nevermore_10_souls[self:GetCaster():GetTalentLevel("modifier_nevermore_10")]
	end
	if self:GetCaster():HasModifier("modifier_nevermore_14") then
		maximum = maximum
			+ (#self:GetParent():FindAllModifiersByName("modifier_nevermore_necromastery_custom_bonus_souls"))
	end
	...
	local modifier_nevermore_frenzy_custom_pre_buff = self:GetCaster()
		:FindModifierByName("modifier_nevermore_frenzy_custom_pre_buff")
...
function modifier_nevermore_necromastery_custom_bonus_souls:IsHidden()
	return true
end
function modifier_nevermore_necromastery_custom_bonus_souls:IsPurgable()
	return false
end
function modifier_nevermore_necromastery_custom_bonus_souls:RemoveOnDeath()
	return false
end
function modifier_nevermore_necromastery_custom_bonus_souls:GetAttributes()
...
function modifier_nevermore_necromastery_custom_9_debuff:IsPurgable()
	return false
end
function modifier_nevermore_necromastery_custom_9_debuff:IsDebuff()
	return true
end
function modifier_nevermore_necromastery_custom_9_debuff:IsPurgeException()
	return false
end
function modifier_nevermore_necromastery_custom_9_debuff:RemoveOnDeath()
	return false
end