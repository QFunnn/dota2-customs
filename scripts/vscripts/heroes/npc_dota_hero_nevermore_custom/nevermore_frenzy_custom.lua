...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_nevermore_custom/nevermore_frenzy_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_frenzy_custom_pre_buff",
	"heroes/npc_dota_hero_nevermore_custom/nevermore_frenzy_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_nevermore_frenzy_custom:IsPurgeException()
	return false
end
...
function modifier_nevermore_frenzy_custom:OnIntervalThink()
	...
	local modifier_nevermore_necromastery_custom = self:GetCaster()
		:FindModifierByName("modifier_nevermore_necromastery_custom")
	local heroes_tick = 0
	local enemies = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		self:GetParent():GetAbsOrigin(),
		nil,
		self.soul_collection_radius,
...
function modifier_nevermore_frenzy_custom:OnDestroy()
	...
	if not_alive_counter > 0 then
		local modifier_nevermore_frenzy_custom_pre_buff = self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_nevermore_frenzy_custom_pre_buff",
...
function modifier_nevermore_frenzy_custom_pre_buff:IsPurgable()
	return false
end
