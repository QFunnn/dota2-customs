...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_nevermore_custom/nevermore_shadowraze_custom",
...
function ApplyShadowRazeDamage(caster, ability, enemy)
	...
	if caster:HasModifier("modifier_nevermore_16") then
		stack_bonus_damage = stack_bonus_damage
			+ ability.modifier_nevermore_16[caster:GetTalentLevel("modifier_nevermore_16")]
	end
...
function modifier_nevermore_shadowraze_custom_debuff:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_nevermore_17")
end
...
function modifier_nevermore_shadowraze_custom_debuff:GetModifierMoveSpeedBonus_Percentage()
	if self:GetCaster():HasModifier("modifier_nevermore_16") then
		return self:GetAbility().modifier_nevermore_16_slow[self:GetCaster():GetTalentLevel("modifier_nevermore_16")]
			* self:GetStackCount()
	end
end
