...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_bristleback_make_spray",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_bristleback_custom_hp_bonus",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_bristleback_custom_hp_bonus_count",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_bristleback_shield_active",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_bristleback_bristleback_scepter",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_bristleback_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function bristleback_bristleback_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_bristleback.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_bristleback.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_bristleback.vpcf",
		context
	)
end
...
function bristleback_bristleback_custom:OnSpellStart()
	...
	if not self:GetCaster():HasModifier("modifier_bristleback_21") then
		return
	end
...
function bristleback_bristleback_custom:OnToggle()
	...
	if not self:GetCaster():HasModifier("modifier_bristleback_7") then
		return
	end
	local caster = self:GetCaster()
...
function modifier_bristleback_bristleback_shield_active:IsPurgable()
	return false
end
function modifier_bristleback_bristleback_shield_active:IsPurgeException()
	return false
end
...
function modifier_bristleback_bristleback_custom:GetModifierIncomingDamage_Percentage(keys)
	...
	if self:GetCaster():HasModifier("modifier_bristleback_5") then
		self.back_damage_reduction = self.back_damage_reduction
			+ self:GetAbility().modifier_bristleback_5[self:GetCaster():GetTalentLevel("modifier_bristleback_5")]
	...
	if self:GetCaster():HasModifier("modifier_bristleback_14") then
		...
		if
			(difference >= 149) and (difference <= 260)
			or self:GetParent():HasModifier("modifier_bristleback_bristleback_shield_active")
		then
		...
		if
			(difference <= (self.back_angle / 1))
			or (difference >= (360 - (self.back_angle / 1)))
			or self:GetParent():HasModifier("modifier_bristleback_bristleback_shield_active")
		then
			local particle = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_bristleback/bristleback_back_dmg.vpcf",
...
function modifier_bristleback_bristleback_custom:OnTakeDamage(keys)
	...
	if not bristleback_quill_spray_custom then
		return
	end
	if not bristleback_quill_spray_custom:IsTrained() then
		return
	end
	...
	local reverseEnemyAngle = math.deg(math.atan2(reverseEnemyVector.x, reverseEnemyVector.y))
	local difference = math.abs(forwardAngle - reverseEnemyAngle)
	local modifier_bristleback_warpath_custom = self:GetParent()
		:FindModifierByName("modifier_bristleback_warpath_custom")
	if self:GetCaster():HasModifier("modifier_bristleback_14") then
		local vector = self:GetCaster():GetOrigin() - self:GetParent():GetOrigin()
		local center_angle = VectorToAngles(vector).y
		local facing_angle = VectorToAngles(self:GetParent():GetForwardVector()).y
				...
				if self:GetStackCount() < self.quill_release_threshold then
					...
					if self:GetCaster():HasModifier("modifier_bristleback_4") then
						self:GetCaster():AddNewModifier(
							...
							"modifier_bristleback_bristleback_custom_hp_bonus",
							{
								duration = self:GetAbility().modifier_bristleback_4_duration[self:GetCaster()
									:GetTalentLevel("modifier_bristleback_4")],
							}
						)
						self:GetCaster():AddNewModifier(
							self:GetCaster(),
...
function modifier_bristleback_bristleback_make_spray:IsHidden()
	return true
end
function modifier_bristleback_bristleback_make_spray:IsPurgable()
	return false
end
...
function modifier_bristleback_bristleback_make_spray:Proc()
	...
	if self:GetCaster():HasModifier("modifier_bristleback_10") then
		local bristleback_viscous_nasal_goo_custom = self:GetCaster()
			:FindAbilityByName("bristleback_viscous_nasal_goo_custom")
		if bristleback_viscous_nasal_goo_custom and bristleback_viscous_nasal_goo_custom:GetLevel() > 0 then
			if self:GetCaster():HasModifier("modifier_bristleback_10") then
				local enemies = FindUnitsInRadius(
					...
					nil,
					self:GetAbility().modifier_bristleback_10[self:GetCaster():GetTalentLevel("modifier_bristleback_10")],
	...
	if not quill_spray_ability then
		return
	end
	if not quill_spray_ability:IsTrained() then
		return
	end
...
function modifier_bristleback_bristleback_custom_hp_bonus:IsPurgable()
	return false
end
function modifier_bristleback_bristleback_custom_hp_bonus:IsPurgeException()
	return false
end
function modifier_bristleback_bristleback_custom_hp_bonus:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_bristleback_bristleback_custom_hp_bonus:IsHidden()
	return true
end
...
function modifier_bristleback_bristleback_custom_hp_bonus_count:IsPurgable()
	return false
end
function modifier_bristleback_bristleback_custom_hp_bonus_count:IsPurgeException()
	return false
end
...
function modifier_bristleback_bristleback_custom_hp_bonus_count:OnIntervalThink()
	...
	local modifier_bristleback_bristleback_custom_hp_bonus =
		self:GetParent():FindAllModifiersByName("modifier_bristleback_bristleback_custom_hp_bonus")
	self:SetStackCount(#modifier_bristleback_bristleback_custom_hp_bonus)
end

modifier_bristleback_bristleback_cooldown = class({})
function modifier_bristleback_bristleback_cooldown:IsHidden()
	return true
end
function modifier_bristleback_bristleback_cooldown:IsPurgable()
	return false
end
function modifier_bristleback_bristleback_cooldown:IsPurgeException()
	return false
end
function modifier_bristleback_bristleback_cooldown:RemoveOnDeath()
	return false
end

modifier_bristleback_bristleback_scepter = class({})

function modifier_bristleback_bristleback_scepter:IsHidden()
	return true
end
function modifier_bristleback_bristleback_scepter:IsPurgable()
	return false
end
