...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_bristleback_custom/bristleback_viscous_nasal_goo_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_viscous_nasal_goo_custom_stack",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_viscous_nasal_goo_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_viscous_nasal_goo_custom_count",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_viscous_nasal_goo_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if enemy == nil and caster == nil then
		if self:GetCaster():HasModifier("modifier_bristleback_9") then
			...
			local enemies = FindUnitsInRadius(
				...
				nil,
				self:GetCaster()
					:GetAoeBonus(self.modifier_bristleback_9[self:GetCaster():GetTalentLevel("modifier_bristleback_9")]),
...
function bristleback_viscous_nasal_goo_custom:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	if hTarget == nil or not hTarget:IsAlive() or hTarget:IsMagicImmune() then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_bristleback_8") then
		...
		if self:GetCaster():HasModifier("modifier_bristleback_11") then
			modifier_bristleback_11 = self.modifier_bristleback_11[self:GetCaster()
				:GetTalentLevel("modifier_bristleback_11")]
		end
		local stack_max = self:GetSpecialValueFor("stack_limit") + modifier_bristleback_11
		local stack =
			hTarget:GetModifierStackCount("modifier_bristleback_viscous_nasal_goo_custom_count", self:GetCaster())
		local damage = self.modifier_bristleback_8[self:GetCaster():GetTalentLevel("modifier_bristleback_8")]
			+ (stack * self.modifier_bristleback_bonus[self:GetCaster():GetTalentLevel("modifier_bristleback_8")])
		ApplyDamage({
			victim = hTarget,
			damage = damage,
		...
		if stack < stack_max then
			hTarget:AddNewModifier(
				...
				self,
				"modifier_bristleback_viscous_nasal_goo_custom_stack",
				{ duration = self.modifier_bristleback_8_duration * (1 - hTarget:GetStatusResistance()) }
			)
			hTarget:AddNewModifier(
				self:GetCaster(),
				self,
...
function modifier_bristleback_viscous_nasal_goo_custom:OnRefresh()
	...
	local modifier_bristleback_11 = 0
	if self:GetCaster():HasModifier("modifier_bristleback_11") then
		modifier_bristleback_11 = self:GetAbility().modifier_bristleback_11[self:GetCaster()
			:GetTalentLevel("modifier_bristleback_11")]
	end
...
function modifier_bristleback_viscous_nasal_goo_custom:OnIntervalThink()
	...
	if
		self:GetCaster():HasModifier("modifier_bristleback_13")
		and self:GetStackCount() >= self:GetAbility().modifier_bristleback_13_need_stack
	then
		local damage = self:GetCaster():GetAgility()
			/ 100
			* self:GetAbility().modifier_bristleback_13[self:GetCaster():GetTalentLevel("modifier_bristleback_13")]
		ApplyDamage({
...
function modifier_bristleback_viscous_nasal_goo_custom:GetModifierMagicalResistanceBonus()
	if self:GetCaster():HasModifier("modifier_bristleback_17") then
		return self:GetStackCount()
			* self:GetAbility().modifier_bristleback_17[self:GetCaster():GetTalentLevel("modifier_bristleback_17")]
	end
	return 0
end

modifier_bristleback_viscous_nasal_goo_custom_count = class({})
function modifier_bristleback_viscous_nasal_goo_custom_count:IsHidden()
	return true
end
function modifier_bristleback_viscous_nasal_goo_custom_count:IsPurgable()
	return false
end
...
function modifier_bristleback_viscous_nasal_goo_custom_stack:IsHidden()
	return true
end
function modifier_bristleback_viscous_nasal_goo_custom_stack:IsPurgable()
	return false
end
function modifier_bristleback_viscous_nasal_goo_custom_stack:GetAttributes()
