...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_bristleback_custom/bristleback_warpath_custom",
...
LinkLuaModifier(
	"modifier_bristleback_warpath_custom_buff_count",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_warpath_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_bristleback_warpath_custom:IncStacks()
	...
	if mod:GetStackCount() < max_stacks then
		...
		for _, all_counts in
			ipairs(self:GetParent():FindAllModifiersByName("modifier_bristleback_warpath_custom_buff_count"))
		do
			all_counts:Destroy()
			mod:IncrementStackCount()
			self:GetParent():AddNewModifier(
...
function modifier_bristleback_warpath_custom_buff:IsHidden()
	return false
end
function modifier_bristleback_warpath_custom_buff:IsPurgable()
	return false
end
function modifier_bristleback_warpath_custom_buff:GetEffectName()
...
function modifier_bristleback_warpath_custom_buff:GetModifierBonusStats_Strength()
	if self:GetCaster():HasModifier("modifier_bristleback_2") then
		return self:GetAbility().modifier_bristleback_2[self:GetCaster():GetTalentLevel("modifier_bristleback_2")]
			* self:GetStackCount()
	end
	return 0
end
...
function modifier_bristleback_warpath_custom_buff:GetModifierSpellAmplify_Percentage()
	if self:GetCaster():HasModifier("modifier_bristleback_20") then
		return self:GetAbility().modifier_bristleback_20[self:GetCaster():GetTalentLevel("modifier_bristleback_20")]
			* self:GetStackCount()
	end
	return 0
end
...
function modifier_bristleback_warpath_custom_buff_count:IsHidden()
	return true
end
function modifier_bristleback_warpath_custom_buff_count:IsPurgable()
	return false
end
function modifier_bristleback_warpath_custom_buff_count:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_bristleback_warpath_custom_buff_count:GetEffectName()
	return "particles/units/heroes/hero_bristleback/bristleback_warpath_dust.vpcf"
end
