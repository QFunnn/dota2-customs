...
LinkLuaModifier(
	"modifier_bristleback_quill_spray_custom_thinker",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_quill_spray_custom",
...
LinkLuaModifier(
	"modifier_bristleback_quill_spray_custom_health_regen",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_quill_spray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_quill_spray_custom_health_regen_count",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_quill_spray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_quill_spray_custom_health_cost_increased",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_quill_spray_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bristleback_quill_spray_custom_handler",
	"heroes/npc_dota_hero_bristleback_custom/bristleback_quill_spray_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function bristleback_quill_spray_custom:GetHealthCost(level)
	...
	local stack =
		self:GetCaster()
			:GetModifierStackCount("modifier_bristleback_quill_spray_custom_health_cost_increased", self:GetCaster())
...
function bristleback_quill_spray_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_bristleback_3") then
		self:GetCaster():AddNewModifier(
			...
			"modifier_bristleback_quill_spray_custom_health_regen",
			{
				duration = self.modifier_bristleback_3_duration[self:GetCaster()
					:GetTalentLevel("modifier_bristleback_3")],
			}
		)
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
...
function modifier_bristleback_quill_spray_custom_health_cost_increased:IsPurgable()
	return false
end
function modifier_bristleback_quill_spray_custom_health_cost_increased:IsPurgeException()
	return false
end
...
function bristleback_quill_spray_custom:MakeSpray(location, is_cone, is_ability)
	...
	CreateModifierThinker(
		...
		self,
		"modifier_bristleback_quill_spray_custom_thinker",
		{ duration = self.duration, cone = cone, is_ability = is_ability },
		self.location,
	...
	for _, enemy in pairs(enemies) do
		...
		if not hit_already then
			...
			if self:GetCaster():HasModifier("modifier_bristleback_6") and self.is_ability then
				bonus = self:GetCaster():GetMaxHealth()
					/ 100
					* self:GetAbility().modifier_bristleback_6_enemy[self:GetCaster()
						:GetTalentLevel("modifier_bristleback_6")]
			end
			...
			if self:GetCaster():HasModifier("modifier_bristleback_18") then
				bonus_max = self:GetAbility().modifier_bristleback_18[self:GetCaster()
					:GetTalentLevel("modifier_bristleback_18")]
			end
			local end_damage = math.min(
				self.quill_base_damage + (self.quill_stack_damage * quill_spray_stacks),
				(self.max_damage + bonus_max)
			) + bonus
			if self.cone == 1 then
			...
			if
				...
				and (enemy.IsReincarnating and not enemy:IsReincarnating())
			then
...
function modifier_bristleback_quill_spray_custom_count:IsHidden()
	return true
end
function modifier_bristleback_quill_spray_custom_count:IsPurgable()
	return false
end
function modifier_bristleback_quill_spray_custom_count:GetAttributes()
...
function modifier_bristleback_quill_spray_custom_health_regen:IsPurgable()
	return false
end
function modifier_bristleback_quill_spray_custom_health_regen:IsPurgeException()
	return false
end
function modifier_bristleback_quill_spray_custom_health_regen:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_bristleback_quill_spray_custom_health_regen:IsHidden()
	return true
end
...
function modifier_bristleback_quill_spray_custom_health_regen_count:IsPurgable()
	return false
end
function modifier_bristleback_quill_spray_custom_health_regen_count:IsPurgeException()
	return false
end
...
function modifier_bristleback_quill_spray_custom_health_regen_count:OnIntervalThink()
	...
	local modifier_bristleback_quill_spray_custom_health_regen =
		self:GetParent():FindAllModifiersByName("modifier_bristleback_quill_spray_custom_health_regen")
...
function modifier_bristleback_quill_spray_custom_handler:IsHidden()
	return true
end
function modifier_bristleback_quill_spray_custom_handler:IsPurgable()
	return false
end
function modifier_bristleback_quill_spray_custom_handler:RemoveOnDeath()
	return false
end
function modifier_bristleback_quill_spray_custom_handler:IsPurgeException()
	return false
end
...
function modifier_bristleback_quill_spray_custom_handler:OnIntervalThink()
	...
	if self:GetAbility():IsFullyCastable() and self:GetAbility():GetAutoCastState() then
		if self:GetParent():HasModifier("modifier_woda_stunned") then
			return
		end
		...
		if self:GetParent():HasModifier("modifier_bristleback_8") then
			return
		end
		if self:GetParent():IsSilenced() or self:GetParent():IsStunned() then
			return
		end
