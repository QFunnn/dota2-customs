...
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_lua",
	"heroes/hero_visage/visage_gravekeepers_cloak_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_secondary_lua",
	"heroes/hero_visage/visage_gravekeepers_cloak_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_visage_gravekeepers_cloak_lua_shard_buff",
	"heroes/hero_visage/visage_gravekeepers_cloak_lua",
	LUA_MODIFIER_MOTION_NONE
)

visage_gravekeepers_cloak_lua = class({}) ---@class visage_gravekeepers_cloak_lua : CDOTA_Ability_Lua
		...
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				unit:AddNewModifier(caster, self, "modifier_stunned", {
					duration = stun_duration * unit:GetStatusResistanceFactor(caster),
				})
				ApplyDamage({
					victim = unit,
					attacker = caster,
...
function modifier_visage_gravekeepers_cloak_lua:IsHidden()
	return false
end

function modifier_visage_gravekeepers_cloak_lua:IsDebuff()
	return false
end

function modifier_visage_gravekeepers_cloak_lua:IsPurgable()
	return false
end

function modifier_visage_gravekeepers_cloak_lua:GetAttributes()
	return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end

function modifier_visage_gravekeepers_cloak_lua:IsAura()
	return true
end

function modifier_visage_gravekeepers_cloak_lua:GetAuraRadius()
	return self:GetAbility():GetSpecialValueFor("radius")
end

function modifier_visage_gravekeepers_cloak_lua:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end

function modifier_visage_gravekeepers_cloak_lua:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function modifier_visage_gravekeepers_cloak_lua:GetAuraSearchType()
	return DOTA_UNIT_TARGET_ALL
end

function modifier_visage_gravekeepers_cloak_lua:GetModifierAura()
	return "modifier_visage_gravekeepers_cloak_secondary_lua"
end
...
function modifier_visage_gravekeepers_cloak_lua:OnStackCountChanged(old_stack_count)
	...
	if new_stack_count and new_stack_count > 0 then
		local cloak_pfx = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_visage/visage_cloak_lyr4.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			parent
		)
		...
		if not self.barrier_pfx then
			self.barrier_pfx = ParticleManager:CreateParticle(
...
function modifier_visage_gravekeepers_cloak_lua:OnIntervalThink()
	if self.max_layers > self:GetStackCount() then
		for i = #self.tStack, 1, -1 do
			...
			if info ~= nil then
				if info.time <= GameRulesCustom:GetGameTime() then
...
function modifier_visage_gravekeepers_cloak_lua:GetModifierIncomingDamage_Percentage(params)
	...
	if IsValid(hTarget) and IsValid(hAttacker) and iStackCount > 0 then
		if
			fDamage > self.minimum_damage
			and bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS
		then
			local iLoseStack = 1
...
function modifier_visage_gravekeepers_cloak_lua:LoseStack(iStack)
	...
	for i = 1, iStack do
		...
		table.insert(self.tStack, { time = GameRulesCustom:GetGameTime() + self.recovery_time })
	end
end

modifier_visage_gravekeepers_cloak_secondary_lua = class({})

function modifier_visage_gravekeepers_cloak_secondary_lua:IsHidden()
	return false
end

function modifier_visage_gravekeepers_cloak_secondary_lua:IsDebuff()
	return false
end

function modifier_visage_gravekeepers_cloak_secondary_lua:IsPurgable()
	return false
end
...
function modifier_visage_gravekeepers_cloak_secondary_lua:GetModifierIncomingDamage_Percentage(keys)
	...
	if bit:_and(keys.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) == DOTA_DAMAGE_FLAG_HPLOSS then
		return
	end
	...
	if not aura or aura:IsNull() then
		return
	end
...
function modifier_visage_gravekeepers_cloak_lua_shard_buff:IsHidden()
	return false
end

function modifier_visage_gravekeepers_cloak_lua_shard_buff:IsDebuff()
	return false
end

function modifier_visage_gravekeepers_cloak_lua_shard_buff:IsPurgable()
	return false
end
...
function modifier_visage_gravekeepers_cloak_lua_shard_buff:OnCreated()
	self.health_regen_pct = self:GetAbility():GetSpecialValueFor("health_regen_pct")
		/ (self:GetAbility():GetSpecialValueFor("shard_duration") or 1)
end
