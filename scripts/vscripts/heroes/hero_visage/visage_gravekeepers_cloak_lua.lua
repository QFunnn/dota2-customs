...
LinkLuaModifier(
	"modifier_visage_summon_familiars_stone_form_buff_custom",
	"heroes/hero_visage/visage_gravekeepers_cloak_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_visage_gravekeepers_cloak_lua:OnPlayerLearnedAbility(keys)
	...
	if keys.PlayerID ~= caster:GetPlayerOwnerID() then
		return
	end
...
function modifier_visage_gravekeepers_cloak_lua:GetModifierIncomingDamage_Percentage(keys)
	...
	if keys.attacker == keys.target or keys.damage <= self.minimum_damage then
		return
	end
...
function modifier_visage_summon_familiars_stone_form_buff_custom:IsPurgable()
	return false
end
