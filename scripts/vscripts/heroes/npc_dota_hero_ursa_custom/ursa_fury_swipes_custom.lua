...
LinkLuaModifier(
	"modifier_ursa_fury_swipes_custom",
	"heroes/npc_dota_hero_ursa_custom/ursa_fury_swipes_custom",
...
function modifier_ursa_fury_swipes_custom:IsHidden()
	return true
end
function modifier_ursa_fury_swipes_custom:IsPurgable()
	return false
end
function modifier_ursa_fury_swipes_custom:IsPurgeException()
	return false
end
function modifier_ursa_fury_swipes_custom:RemoveOnDeath()
	return false
end
	...
	if modifier_ursa_fury_swipes_custom_debuff then
		if not self:GetParent():PassivesDisabled() then
			target:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_ursa_fury_swipes_custom_debuff",
				{ duration = self.bonus_reset_time * (1 - target:GetStatusResistance()) }
			)
		end
...
function modifier_ursa_fury_swipes_custom:GetModifierPreAttack_BonusDamage(params)
	...
	if target then
		...
		local modifier_ursa_fury_swipes_custom_debuff =
			target:FindModifierByName("modifier_ursa_fury_swipes_custom_debuff")
		if modifier_ursa_fury_swipes_custom_debuff then
			local stack_count = modifier_ursa_fury_swipes_custom_debuff:GetStackCount()
			return damage * stack_count
		end
...
function modifier_ursa_fury_swipes_custom_debuff:IsPurgable()
	return false
end
...
function modifier_ursa_fury_swipes_custom_debuff:GetModifierPhysicalArmorBonus()
	if not self:GetCaster():HasModifier("modifier_ursa_6") then
		return 0
	end
