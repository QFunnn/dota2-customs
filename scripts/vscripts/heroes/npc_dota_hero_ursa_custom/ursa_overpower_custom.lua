...
LinkLuaModifier(
	"modifier_ursa_overpower_custom",
	"heroes/npc_dota_hero_ursa_custom/ursa_overpower_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ursa_overpower_custom_cooldown",
	"heroes/npc_dota_hero_ursa_custom/ursa_overpower_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_ursa_overpower_custom:OnTakeDamage(params)
	...
	if not self:GetParent():HasModifier("modifier_ursa_16") then
		return
	end
	if params.inflictor and params.inflictor == self:GetAbility() then
		return
	end
	if self:GetParent():HasModifier("modifier_ursa_overpower_custom_cooldown") then
		return
	end
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_ursa_overpower_custom_cooldown",
		{ duration = self:GetAbility().modifier_ursa_16_cooldown }
	)
	local units = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		params.unit:GetAbsOrigin(),
		nil,
		self:GetCaster():GetCurrentVisionRange(),
...
function modifier_ursa_overpower_custom:TalentExplosion(target)
	...
	local particle =
		ParticleManager:CreateParticle("particles/ursa_custom_fx/ursa_ti10_earthshock.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	local damage = self:GetCaster():GetIntellect(false)
		/ 100
		* self:GetAbility().modifier_ursa_16[self:GetCaster():GetTalentLevel("modifier_ursa_16")]
...
function modifier_ursa_overpower_custom:GetModifierAttackSpeedBonus_Constant()
	if self:GetParent():HasModifier("modifier_ursa_16") then
		return
	end
...
function modifier_ursa_overpower_custom_cooldown:IsHidden()
	return true
end
function modifier_ursa_overpower_custom_cooldown:IsPurgeException()
	return false
end
function modifier_ursa_overpower_custom_cooldown:IsPurgable()
	return false
end
function modifier_ursa_overpower_custom_cooldown:RemoveOnDeath()
	return false
end