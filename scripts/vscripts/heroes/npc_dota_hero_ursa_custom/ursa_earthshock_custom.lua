...
LinkLuaModifier(
	"modifier_ursa_earthshock_custom_debuff",
	"heroes/npc_dota_hero_ursa_custom/ursa_earthshock_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ursa_earthshock_custom_handler",
	"heroes/npc_dota_hero_ursa_custom/ursa_earthshock_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ursa_earthshock_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_ursa.vpcf",
		context
	)
end
	...
	if IsClient() then
		if self:GetCaster():HasModifier("modifier_ursa_17") then
			return self:GetSpecialValueFor("hop_distance")
				+ self.modifier_ursa_17[self:GetCaster():GetTalentLevel("modifier_ursa_17")]
		end
	end
end
			...
			if ursa_fury_swipes_custom and ursa_fury_swipes_custom:GetLevel() > 0 then
				...
				for i = 1, self.modifier_ursa_11[self:GetCaster():GetTalentLevel("modifier_ursa_11")] do
					enemy:AddNewModifier(
						...
						{ duration = bonus_reset_time * (1 - enemy:GetStatusResistance()) }
					)
				end
			end
		end
...
function modifier_ursa_earthshock_custom_handler:IsHidden()
	return true
end
function modifier_ursa_earthshock_custom_handler:IsPurgable()
	return false
end
function modifier_ursa_earthshock_custom_handler:IsPurgeException()
	return false
end
function modifier_ursa_earthshock_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_ursa_earthshock_custom_handler:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_ursa_12") then
		return
	end
	...
	if self.distance >= self:GetAbility().modifier_ursa_12[self:GetCaster():GetTalentLevel("modifier_ursa_12")] then
		self.distance = math.max(
			0,
			self.distance - self:GetAbility().modifier_ursa_12[self:GetCaster():GetTalentLevel("modifier_ursa_12")]
		)
