...
LinkLuaModifier(
	"modifier_ursa_enrage_custom",
	"heroes/npc_dota_hero_ursa_custom/ursa_enrage_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ursa_enrage_custom_debuff",
	"heroes/npc_dota_hero_ursa_custom/ursa_enrage_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ursa_enrage_custom:OnSpellStart(new_duration, earth_shock)
	...
	if self:GetCaster():HasModifier("modifier_ursa_21") then
		local particle = ParticleManager:CreateParticle(
			"particles/ursa_custom_fx/ursa_scream.vpcf",
		...
		local enemies = FindUnitsInRadius(
			...
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_ursa_21_radius),
		...
		for _, enemy in pairs(enemies) do
			ApplyDamage({
				...
				damage = self.modifier_ursa_21_damage,
				ability = self,
			...
			enemy:AddNewModifier(
				...
				self,
				"modifier_ursa_enrage_custom_debuff",
				{ duration = self.modifier_ursa_21_duration }
			)
		end
	end
end
...
function modifier_ursa_enrage_custom:IsPurgable()
	return false
end
function modifier_ursa_enrage_custom:IsPurgeException()
	return false
end
...
function modifier_ursa_enrage_custom:OnIntervalThink()
	...
	local units = FindUnitsInRadius(
		...
		self:GetCaster():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self:GetAbility().modifier_ursa_1_radius),
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		0,
		FIND_ANY_ORDER,
	...
	local damage = (
		...
		/ 100
		* self:GetAbility().modifier_ursa_1[self:GetCaster():GetTalentLevel("modifier_ursa_1")]
	) * 0.5
...
function modifier_ursa_enrage_custom:GetModifierTotalDamageOutgoing_Percentage()
	if not self:GetCaster():HasModifier("modifier_ursa_7") then
		return
	end
