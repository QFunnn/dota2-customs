...
LinkLuaModifier(
	"modifier_riki_backstab_custom",
	"heroes/npc_dota_hero_riki_custom/riki_backstab_custom",
...
function modifier_riki_backstab_custom:IsPurgable()
	return false
end
function modifier_riki_backstab_custom:IsPurgeException()
	return false
end
function modifier_riki_backstab_custom:RemoveOnDeath()
	return false
end
...
function modifier_riki_backstab_custom:GetModifierPreAttack_BonusDamage(params)
	...
	if self:GetParent():HasModifier("modifier_riki_4") then
		return
	end
	...
	if
		(difference <= (110 / 1))
		or (difference >= (360 - (110 / 1)))
		or (difference <= 70)
		or (difference >= (360 - 70))
	then
...
function modifier_riki_backstab_custom:GetBonusDamage()
	...
	if self:GetCaster():HasModifier("modifier_riki_13") then
		damage_multiplier = damage_multiplier
			+ self:GetAbility().modifier_riki_13[self:GetCaster():GetTalentLevel("modifier_riki_13")]
	end
