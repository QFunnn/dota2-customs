...
LinkLuaModifier(
	"modifier_riki_poison_dart_custom_sleep",
	"heroes/npc_dota_hero_riki_custom/riki_poison_dart_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_poison_dart_custom_debuff",
	"heroes/npc_dota_hero_riki_custom/riki_poison_dart_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function riki_poison_dart_custom:OnSpellStart()
	...
	local particle =
		ParticleManager:CreateParticle("particles/riki_shard_cast.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
...
function modifier_riki_poison_dart_custom_sleep:OnDestroy()
	...
	self:GetParent():AddNewModifier(
		...
		"modifier_riki_poison_dart_custom_debuff",
		{
			duration = self:GetAbility():GetSpecialValueFor("debuff_duration") * (1 - self:GetParent()
				:GetStatusResistance()),
		}
	)
	ApplyDamage({
