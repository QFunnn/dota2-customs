...
LinkLuaModifier(
	"modifier_riki_blinkstrike_slow_custom",
	"heroes/npc_dota_hero_riki_custom/riki_blink_strike_custom",
...
function riki_blink_strike_custom:GetManaCost(level)
	...
	if self:GetCaster():HasModifier("modifier_riki_5") then
		bonus = self.BaseClass.GetManaCost(self, level)
			/ 100
			* self.modifier_riki_5[self:GetCaster():GetTalentLevel("modifier_riki_5")]
	end
...
function riki_blink_strike_custom:OnSpellStart()
	...
	if target then
		...
		local attacker_new = Vector(
			victim_position.x - 100 * math.cos(victim_angle_rad),
			victim_position.y - 100 * math.sin(victim_angle_rad),
			0
		)
		if self:GetCaster():HasModifier("modifier_riki_4") then
		...
		if target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
			...
			target:AddNewModifier(
				...
				self,
				"modifier_riki_blinkstrike_slow_custom",
				{ duration = self:GetSpecialValueFor("slow") * (1 - target:GetStatusResistance()) }
			)
		end
			...
			for _, unit in pairs(units) do
				if unit ~= target then
					...
					unit:AddNewModifier(
						...
						self,
						"modifier_riki_blinkstrike_slow_custom",
						{ duration = self:GetSpecialValueFor("slow") * (1 - unit:GetStatusResistance()) }
					)
				end
			end
		end
