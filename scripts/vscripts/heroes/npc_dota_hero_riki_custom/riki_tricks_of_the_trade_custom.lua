...
LinkLuaModifier(
	"modifier_riki_tricks_of_the_trade_custom",
	"heroes/npc_dota_hero_riki_custom/riki_tricks_of_the_trade_custom",
...
function modifier_riki_tricks_of_the_trade_custom:IsHidden()
	return true
end
function modifier_riki_tricks_of_the_trade_custom:IsPurgable()
	return false
end
function modifier_riki_tricks_of_the_trade_custom:IsPurgeException()
	return false
end
function modifier_riki_tricks_of_the_trade_custom:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_riki_12") then
		self.attack_count = self.attack_count
			+ (
				self:GetAbility().modifier_riki_12_base
				+ math.floor(
					(
						self:GetCaster():GetAgility()
						/ self:GetAbility().modifier_riki_12_agility[self:GetCaster():GetTalentLevel("modifier_riki_12")]
					)
				)
			)
	end
	...
	self.particle_radius =
		ParticleManager:CreateParticle("particles/units/heroes/hero_riki/riki_tricks.vpcf", PATTACH_WORLDORIGIN, nil)
