...
LinkLuaModifier(
	"modifier_riki_smoke_screen_custom",
	"heroes/npc_dota_hero_riki_custom/riki_smoke_screen_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_smoke_screen_custom_debuff",
	"heroes/npc_dota_hero_riki_custom/riki_smoke_screen_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_smoke_screen_custom_buff_aura",
	"heroes/npc_dota_hero_riki_custom/riki_smoke_screen_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_smoke_screen_custom_buff",
	"heroes/npc_dota_hero_riki_custom/riki_smoke_screen_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function riki_smoke_screen_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_riki.vpcf",
		context
	)
end
...
function modifier_riki_smoke_screen_custom:IsPurgable()
	return false
end
function modifier_riki_smoke_screen_custom:IsHidden()
	return true
end
function modifier_riki_smoke_screen_custom:IsAura()
	return true
end
...
function modifier_riki_smoke_screen_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_riki_17") then
		self.radius = self.radius
			+ self:GetAbility().modifier_riki_17_radius[self:GetCaster():GetTalentLevel("modifier_riki_17")]
	end
...
function modifier_riki_smoke_screen_custom_debuff:IsDebuff()
	return true
end
	...
	if self:GetCaster():HasModifier("modifier_riki_17") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_riki_17_damage[self:GetCaster():GetTalentLevel("modifier_riki_17")]
			)
	end
	ApplyDamage({
...
function modifier_riki_smoke_screen_custom_buff_aura:IsPurgable()
	return false
end
function modifier_riki_smoke_screen_custom_buff_aura:IsHidden()
	return true
end
function modifier_riki_smoke_screen_custom_buff_aura:IsAura()
	return true
end
...
function modifier_riki_smoke_screen_custom_buff:IsPurgable()
	return false
end
function modifier_riki_smoke_screen_custom_buff:IsDebuff()
	return false
end
...
function modifier_riki_smoke_screen_custom_buff:GetModifierTotal_ConstantBlock(kv)
	if IsServer() then
		...
		if
			kv.damage > 0
			and RollPercentage(self:GetAbility().modifier_riki_11[self:GetCaster():GetTalentLevel("modifier_riki_11")])
		then
