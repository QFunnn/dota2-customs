...
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_invis",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_buff",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_buff_cooldown",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_immortality_buff",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_cooldown_fx",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_pull",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_riki_permanent_invisibility_custom_illusion",
	"heroes/npc_dota_hero_riki_custom/riki_permanent_invisibility_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function riki_permanent_invisibility_custom:OnSpellStart()
	...
	local illusions = CreateIllusions(
		...
		self:GetCaster(),
		{
			duration = self.modifier_riki_14_duration,
			outgoing_damage = self.modifier_riki_14_outgoing_damage - 100,
			incoming_damage = self.modifier_riki_14_incoming_damage - 100,
		},
		1,
		100,
		false,
	...
	if illusions and illusions[1] then
		...
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_riki_permanent_invisibility_custom_pull",
			{ duration = self.modifier_riki_14_duration, illusion = illusions[1]:entindex() }
		)
...
function modifier_riki_permanent_invisibility_custom:IsHidden()
	return true
end
function modifier_riki_permanent_invisibility_custom:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom:IsPurgeException()
	return false
end
function modifier_riki_permanent_invisibility_custom:RemoveOnDeath()
	return false
end
function modifier_riki_permanent_invisibility_custom:OnCreated()
	...
	self:GetParent()
		:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_riki_permanent_invisibility_custom_invis", {})
end
...
function modifier_riki_permanent_invisibility_custom:GetMinHealth()
	...
	if not self:GetParent():HasModifier("modifier_riki_20") then
		return
	end
	if self:GetParent():HasModifier("modifier_riki_permanent_invisibility_custom_immortality_cooldown") then
		return
	end
	return 1
end
function modifier_riki_permanent_invisibility_custom:OnTakeDamage(params)
	...
	if self:GetParent():HasModifier("modifier_riki_permanent_invisibility_custom_immortality_buff") then
		return
	end
	if self:GetParent():HasModifier("modifier_riki_permanent_invisibility_custom_immortality_cooldown") then
		return
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_riki_permanent_invisibility_custom_immortality_buff",
		{ duration = self:GetAbility().modifier_riki_20_duration[self:GetCaster():GetTalentLevel("modifier_riki_20")] }
	)
	self:GetParent():AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_riki_permanent_invisibility_custom_immortality_cooldown",
		{ duration = self:GetAbility().modifier_riki_20_cooldown }
	)
...
function modifier_riki_permanent_invisibility_custom:OnIntervalThink()
	...
	if self:GetParent():HasModifier("modifier_riki_permanent_invisibility_custom_illusion") then
		return
	end
...
function modifier_riki_permanent_invisibility_custom:OnAttackLanded(params)
	...
	if self:GetParent():HasModifier("modifier_riki_permanent_invisibility_custom_pull") then
		return
	end
...
function modifier_riki_permanent_invisibility_custom_invis:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_invis:IsPurgeException()
	return false
end
function modifier_riki_permanent_invisibility_custom_invis:RemoveOnDeath()
	return false
end
...
function modifier_riki_permanent_invisibility_custom_invis:OnIntervalThink()
	...
	local modifier_riki_permanent_invisibility_custom =
		self:GetParent():FindModifierByName("modifier_riki_permanent_invisibility_custom")
	local enemies = FindUnitsInRadius(
		self:GetParent():GetTeamNumber(),
		self:GetParent():GetAbsOrigin(),
		nil,
	...
	if #enemies > 0 then
		...
		if modifier_riki_permanent_invisibility_custom then
			modifier_riki_permanent_invisibility_custom:StartIntervalThink(
				modifier_riki_permanent_invisibility_custom.fade_delay
			)
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_riki_permanent_invisibility_custom_cooldown_fx",
				{ duration = modifier_riki_permanent_invisibility_custom.fade_delay }
			)
...
function modifier_riki_permanent_invisibility_custom_invis:OnDestroy()
	...
	if
		self:GetCaster():HasModifier("modifier_riki_7")
		and not self:GetCaster():HasModifier("modifier_riki_permanent_invisibility_custom_buff_cooldown")
	then
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_riki_permanent_invisibility_custom_buff",
			{ duration = self:GetAbility().modifier_riki_7_duration }
		)
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_riki_permanent_invisibility_custom_buff_cooldown",
			{ duration = self:GetAbility().modifier_riki_7_cooldown }
		)
	end
end

modifier_riki_permanent_invisibility_custom_buff_cooldown = class({})
function modifier_riki_permanent_invisibility_custom_buff_cooldown:GetTexture()
	return "riki_7"
end
function modifier_riki_permanent_invisibility_custom_buff_cooldown:IsDebuff()
	return true
end
function modifier_riki_permanent_invisibility_custom_buff_cooldown:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_buff_cooldown:IsPurgeException()
	return false
end
function modifier_riki_permanent_invisibility_custom_buff_cooldown:IsHidden()
	return true
end

modifier_riki_permanent_invisibility_custom_buff = class({})
function modifier_riki_permanent_invisibility_custom_buff:GetTexture()
	return "riki_7"
end
...
function modifier_riki_permanent_invisibility_custom_buff:OnCreated()
	...
	local particle =
		ParticleManager:CreateParticle("particles/riki_effect_uninvis.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	ParticleManager:ReleaseParticleIndex(particle)
	self:StartIntervalThink(FrameTime())
end
...
function modifier_riki_permanent_invisibility_custom_buff:OnIntervalThink()
	...
	AddFOWViewer(
		self:GetParent():GetTeamNumber(),
		self:GetParent():GetAbsOrigin(),
		self:GetAbility().modifier_riki_7_radius,
		FrameTime(),
		false
	)
end
...
function modifier_riki_permanent_invisibility_custom_immortality_buff:GetTexture()
	return "riki_20"
end
function modifier_riki_permanent_invisibility_custom_immortality_buff:IsHidden()
	return false
end
function modifier_riki_permanent_invisibility_custom_immortality_buff:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_immortality_buff:GetEffectName()
	return "particles/helm_riki.vpcf"
end
...
function modifier_riki_permanent_invisibility_custom_immortality_buff:OnDestroy()
	...
	self:GetParent():Heal(
		self:GetParent():GetMaxHealth()
			/ 100
			* self:GetAbility().modifier_riki_20_heal[self:GetCaster():GetTalentLevel("modifier_riki_20")],
		self:GetAbility()
	)
end

modifier_riki_permanent_invisibility_custom_immortality_cooldown = class({})
function modifier_riki_permanent_invisibility_custom_immortality_cooldown:GetTexture()
	return "riki_20"
end
function modifier_riki_permanent_invisibility_custom_immortality_cooldown:IsHidden()
	return false
end
function modifier_riki_permanent_invisibility_custom_immortality_cooldown:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_immortality_cooldown:IsDebuff()
	return true
end
function modifier_riki_permanent_invisibility_custom_immortality_cooldown:RemoveOnDeath()
	return false
end

modifier_riki_permanent_invisibility_custom_cooldown_fx = class({})
function modifier_riki_permanent_invisibility_custom_cooldown_fx:GetTexture()
	return "riki_permanent_invisibility"
end
function modifier_riki_permanent_invisibility_custom_cooldown_fx:IsHidden()
	return false
end
function modifier_riki_permanent_invisibility_custom_cooldown_fx:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_cooldown_fx:IsDebuff()
	return true
end
function modifier_riki_permanent_invisibility_custom_cooldown_fx:RemoveOnDeath()
	return false
end

modifier_riki_permanent_invisibility_custom_illusion = class({})
function modifier_riki_permanent_invisibility_custom_illusion:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_illusion:IsHidden()
	return true
end
function modifier_riki_permanent_invisibility_custom_illusion:IsPurgeException()
	return false
end
...
function modifier_riki_permanent_invisibility_custom_illusion:GetDisableHealing()
	return 1
end
function modifier_riki_permanent_invisibility_custom_illusion:GetModifierForceMaxHealth()
	return self:GetAbility().modifier_riki_14_health
end

modifier_riki_permanent_invisibility_custom_pull = class({})
function modifier_riki_permanent_invisibility_custom_pull:IsPurgable()
	return false
end
function modifier_riki_permanent_invisibility_custom_pull:IsPurgeException()
	return false
end
...
function modifier_riki_permanent_invisibility_custom_pull:OnCreated(data)
	...
	local modifier_riki_permanent_invisibility_custom_cooldown_fx =
		self:GetParent():FindModifierByName("modifier_riki_permanent_invisibility_custom_cooldown_fx")
	...
	local particle =
		ParticleManager:CreateParticle("particles/riki_web_nothl.vpcf", PATTACH_POINT_FOLLOW, self.illusion)
