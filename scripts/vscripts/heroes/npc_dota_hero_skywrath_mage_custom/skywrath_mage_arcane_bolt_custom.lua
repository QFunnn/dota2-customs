...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_arcane_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_arcane_bolt_custom_magic_immune",
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_arcane_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack",
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_arcane_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
	...
	if caster:HasModifier("modifier_skywrath_mage_2") and new_target == nil then
		...
		for i = 1, extra_count do
			...
			for _, enemy in pairs(enemies) do
				if enemy and enemy:IsAlive() and enemy:IsHero() and not used_targets[enemy:entindex()] then
					extra_target = enemy
					break
				end
			end
			...
			if not extra_target then
				for _, enemy in pairs(enemies) do
					if enemy and enemy:IsAlive() and not used_targets[enemy:entindex()] then
						extra_target = enemy
						break
					end
				end
...
function skywrath_mage_arcane_bolt_custom:OnProjectileHit_ExtraData(target, location, extraData)
	...
	if self:GetCaster():HasModifier("modifier_skywrath_mage_6") then
		target:AddNewModifier(
			...
			self,
			"modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack",
			{ duration = self.modifier_skywrath_mage_6_duration }
		)
		target:AddNewModifier(
			self:GetCaster(),
			self,
	...
	target:AddNewModifier(
		...
		{ duration = self:GetSpecialValueFor("lifesteal_duration") }
	)
...
function modifier_skywrath_mage_arcane_bolt_custom:IsHidden()
	return not self:GetCaster():HasModifier("modifier_skywrath_mage_1")
end
function modifier_skywrath_mage_arcane_bolt_custom:IsPurgable()
	return false
end
function modifier_skywrath_mage_arcane_bolt_custom:IsPurgeException()
	return false
end
...
function modifier_skywrath_mage_arcane_bolt_custom:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_skywrath_mage_1") then
		return
	end
	self:IncrementStackCount()
	if
		self:GetStackCount()
		>= self:GetAbility().modifier_skywrath_mage_1_attacks[self:GetCaster()
			:GetTalentLevel("modifier_skywrath_mage_1")]
	then
...
function modifier_skywrath_mage_arcane_bolt_custom_magic_immune:GetTexture()
	return "skywrath_mage_6"
end
...
function modifier_skywrath_mage_arcane_bolt_custom_magic_immune:OnIntervalThink()
	...
	local modifiers = self:GetParent()
		:FindAllModifiersByName("modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack")
	self:SetStackCount(#modifiers)
end

modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack = class({})

function modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack:GetTexture()
	return "skywrath_mage_6"
end
function modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack:IsHidden()
	return true
end
function modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack:GetAttributes()
...
function modifier_skywrath_mage_arcane_bolt_custom_magic_immune_stack:GetModifierMagicalResistanceBonus()
	return self:GetAbility().modifier_skywrath_mage_6_magicimmune[self:GetCaster()
		:GetTalentLevel("modifier_skywrath_mage_6")]
end