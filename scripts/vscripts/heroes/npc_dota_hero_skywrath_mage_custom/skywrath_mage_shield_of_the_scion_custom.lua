...
LinkLuaModifier(
	"modifier_skywrath_mage_shield_of_the_scion_custom",
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_shield_of_the_scion_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_shield_of_the_scion_custom_buff",
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_shield_of_the_scion_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_skywrath_mage_shield_of_the_scion_custom:IsHidden()
	return self:GetStackCount() <= 0
end
function modifier_skywrath_mage_shield_of_the_scion_custom:IsPurgable()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom:IsPurgeException()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom:RemoveOnDeath()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom:DestroyOnExpire()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom:OnTakeDamage(params)
	...
	if params.inflictor:IsItem() then
		return
	end
	...
	if params.damage > 0 then
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_skywrath_mage_shield_of_the_scion_custom_buff",
			{ duration = barrier_duration, barrier = params.damage / 100 * damage_barrier }
		)
...
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:IsHidden()
	return true
end
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:IsPurgable()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:IsPurgeException()
	return false
end
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:GetAttributes()
...
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:OnCreated(params)
	...
	self.modifier_skywrath_mage_shield_of_the_scion_custom =
		self:GetCaster():FindModifierByName("modifier_skywrath_mage_shield_of_the_scion_custom")
...
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:GetModifierIncomingDamageConstant(params)
	if not self:GetParent():HasModifier("modifier_skywrath_mage_15") then
		return
	end
...
function modifier_skywrath_mage_shield_of_the_scion_custom_buff:GetModifierIncomingSpellDamageConstant(params)
	if self:GetParent():HasModifier("modifier_skywrath_mage_15") then
		return
	end
	return self:ShieldFunction(params)
end