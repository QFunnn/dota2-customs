...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_thinker_slow",
	"heroes/npc_dota_hero_skywrath_mage_custom/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skywrath_mage_mystic_flare_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_skywrath_mage_20") then
		duration = duration
			+ self.modifier_skywrath_mage_20_duration[self:GetCaster():GetTalentLevel("modifier_skywrath_mage_20")]
	end
	...
	if self:GetCaster():HasModifier("modifier_skywrath_mage_21") then
		...
		local end_point = self:GetCaster():GetAbsOrigin()
			+ forward_vector
				* (self:GetCastRange(self:GetCaster():GetAbsOrigin(), self:GetCaster()) + self:GetCaster()
					:GetCastRangeBonus())
		...
		for i = 0, count do
			...
			CreateModifierThinker(
				...
				self,
				"modifier_skywrath_mage_mystic_flare_custom_thinker",
				{ duration = duration, real_duration = self:GetSpecialValueFor("duration") },
				origin,
		...
		if
			not self:GetAutoCastState()
			and not self:GetCaster():IsRooted()
			and not self:GetCaster():IsCustomHasTethered()
		then
...
function modifier_skywrath_mage_mystic_flare_custom_thinker:IsAura()
	return self:GetCaster():HasModifier("modifier_skywrath_mage_19")
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:IsAuraActiveOnDeath()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetAuraDuration()
	return 0.1
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetAuraRadius()
	return self.radius
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_ALL
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:GetModifierAura()
	return "modifier_skywrath_mage_mystic_flare_custom_thinker_slow"
end

modifier_skywrath_mage_mystic_flare_custom_thinker_slow = class({})

function modifier_skywrath_mage_mystic_flare_custom_thinker_slow:GetTexture()
	return "skywrath_mage_19"
end
