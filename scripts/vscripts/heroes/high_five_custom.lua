...
modifier_high_five_custom_search = class({
	...
	GetEffectName = function(self)
		return "particles/econ/events/ti9/high_five/high_five_lvl1_overhead.vpcf"
	end,

	OnDestroy = function(self)
		if self.bSearch or IsClient() then
			return
		end
		if not self.parent.__high_five_lose or self.parent.__high_five_lose <= HighFiveLimitLose then
			if self.parent and not self.parent:IsNull() then
				local UnitNameFormatted = "<font color='#70EA72'>"
					.. PlayerResource:GetPlayerName(self.parent:GetPlayerID())
					.. "</font>"
	...
	OnIntervalThink = function(self)
		...
		for k, v in pairs(units) do
			...
			if modifier and v ~= self.parent then
				...
				if
					(not self.parent.__high_five_succes or self.parent.__high_five_succes <= HighFiveLimitSucces)
					and (
						not modifier.parent.__high_five_succes
						or modifier.parent.__high_five_succes <= HighFiveLimitSucces
					)
				then
					...
					local heroNameParent = "<font color='"
						.. self:GetRgbColor(self.parent)
						.. "'>"
						.. PlayerResource:GetPlayerName(playerID_parent)
						.. "</font>"
					local heroNameSearch = "<font color='"
						.. self:GetRgbColor(modifier.parent)
						.. "'>"
						.. PlayerResource:GetPlayerName(playerID_search)
						.. "</font>"
