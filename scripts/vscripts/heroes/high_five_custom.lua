...
function modifier_high_five_custom_custom:IsHidden()
	return true
end
function modifier_high_five_custom_custom:IsPurgable()
	return false
end
function modifier_high_five_custom_custom:IsPurgeException()
	return false
end
...
function modifier_high_five_custom_custom:OnIntervalThink()
	...
	CreateModifierThinker(
		self:GetParent(),
		nil,
		"modifier_high_five_custom_custom_thinker",
		{ duration = (vPoint - target:GetOrigin()):Length2D() / 700, particle = self.explosion_proj },
		vPoint,
		self:GetParent():GetTeamNumber(),
		false
	)
