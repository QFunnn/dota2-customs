...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_vampiric_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_custom_aura",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_vampiric_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_custom_ghost_buff",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_vampiric_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_custom_ghost_buff_death",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_vampiric_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_skeleton_king_vampiric_aura_custom_aura:IsHidden()
	return true
end
function modifier_skeleton_king_vampiric_aura_custom_aura:IsPurgable()
	return false
end
function modifier_skeleton_king_vampiric_aura_custom_aura:IsAura()
	return true
end
function modifier_skeleton_king_vampiric_aura_custom_aura:GetAuraRadius()
	return 1200
end
function modifier_skeleton_king_vampiric_aura_custom_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_skeleton_king_vampiric_aura_custom_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_skeleton_king_vampiric_aura_custom_aura:GetModifierAura()
	return "modifier_skeleton_king_vampiric_aura_custom"
end
function modifier_skeleton_king_vampiric_aura_custom_aura:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE
end

modifier_skeleton_king_vampiric_aura_custom = class({})
function modifier_skeleton_king_vampiric_aura_custom:IsHidden()
	return true
end
function modifier_skeleton_king_vampiric_aura_custom:IsPurgable()
	return false
end
...
function modifier_skeleton_king_vampiric_aura_custom:GetModifierProperty_PhysicalLifesteal(params)
	if self:GetParent():IsRealHero() and self:GetParent() ~= self:GetCaster() then
		return
	end
	if self:GetParent():HasModifier("modifier_skeleton_king_2") then
		return
	end
	if params.inflictor ~= nil or self:GetParent():HasModifier("modifier_skeleton_vampiric_aura_ghost") then
	else
		if self:GetParent():HasModifier("modifier_skeleton_vampiric_aura_ghost") then
			return
		end
...
function modifier_skeleton_king_vampiric_aura_custom:GetMinHealth()
	...
	if self:GetParent():HasModifier("modifier_skeleton_king_vampiric_aura_custom_ghost_buff") then
		return
	end
	if self:GetParent():HasModifier("modifier_skeleton_king_vampiric_aura_custom_ghost_buff_death") then
		return
	end
	local only_caster = not self:GetCaster():HasModifier("modifier_skeleton_king_14")
	if only_caster and self:GetParent() ~= self:GetCaster() then
		return
	end
	local skeleton_king_reincarnation_custom = self:GetParent():FindAbilityByName("skeleton_king_reincarnation_custom")
	if
		self:GetParent():HasModifier("modifier_skeleton_king_5")
		or (skeleton_king_reincarnation_custom and (skeleton_king_reincarnation_custom:GetLevel() <= 0 or not skeleton_king_reincarnation_custom:IsFullyCastable() or skeleton_king_reincarnation_custom:IsHidden()))
		or self:GetParent() ~= self:GetCaster()
	then
		return 1
	end
end
...
function modifier_skeleton_king_vampiric_aura_custom_ghost_buff:IsHidden()
	return false
end
function modifier_skeleton_king_vampiric_aura_custom_ghost_buff:IsPurgable()
	return false
end
...
function modifier_skeleton_king_vampiric_aura_custom_ghost_buff_death:IsHidden()
	return true
end
function modifier_skeleton_king_vampiric_aura_custom_ghost_buff_death:IsPurgeException()
	return false
end
function modifier_skeleton_king_vampiric_aura_custom_ghost_buff_death:IsPurgable()
	return false
end