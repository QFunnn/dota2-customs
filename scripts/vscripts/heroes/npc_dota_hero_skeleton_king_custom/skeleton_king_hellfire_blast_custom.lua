...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_hellfire_blast_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_hellfire_blast_skelet_buff",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_hellfire_blast_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skeleton_king_hellfire_blast_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_skeletonking/skeleton_king_weapon_blur_reverse.vpcf",
		context
	)
	PrecacheResource(
		"particle",
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/wraith_king/wraith_king_ti6_bracer/wraith_king_ti6_hellfireblast_warmup.vpcf",
		context
	)
	PrecacheResource(
		"particle",
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/wraith_king/wraith_king_ti6_bracer/wraith_king_ti6_hellfireblast_explosion.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_skeleton_king.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_skeleton_king.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_skeleton_king.vpcf",
		context
	)
end
...
function skeleton_king_hellfire_blast_custom:OnSpellStart(new_target)
	...
	local all_skeletons = FindUnitsInRadius(
	...
	for _, skelet in pairs(all_skeletons) do
		if skelet.skelet then
			skelet:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_skeleton_king_hellfire_blast_skelet_buff",
				{ duration = self:GetSpecialValueFor("blast_stun_duration") }
			)
		end
	end
end
...
function skeleton_king_hellfire_blast_custom:UseStun(target)
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_6") then
		stun_duration = stun_duration
			+ self.modifier_skeleton_king_6[self:GetCaster():GetTalentLevel("modifier_skeleton_king_6")]
	end
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_3") then
		slow_duration = slow_duration
			+ self.modifier_skeleton_king_3[self:GetCaster():GetTalentLevel("modifier_skeleton_king_3")]
	end
	...
	target:AddNewModifier(
		...
		self,
		"modifier_skeleton_king_hellfire_blast_custom_debuff",
		{ duration = stun_duration + slow_duration, delay = stun_duration }
	)
	if self:GetCaster():HasModifier("modifier_skeleton_king_1") then
		...
		for _, unit in pairs(units) do
			if unit ~= target then
				unit:AddNewModifier(
					...
					self,
					"modifier_skeleton_king_hellfire_blast_custom_debuff",
					{ duration = slow_duration, delay = 0 }
				)
			end
		end
	end
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_9") then
		local illusion = CreateIllusions(
			...
			self:GetCaster(),
			{
				duration = self.modifier_skeleton_king_9[self:GetCaster():GetTalentLevel("modifier_skeleton_king_9")],
				outgoing_damage = self.modifier_skeleton_king_9_out - 100,
				incoming_damage = self.modifier_skeleton_king_9_inc - 100,
			},
			1,
			0,
			false,
...
function modifier_skeleton_king_hellfire_blast_custom_debuff:IsPurgeException()
	return true
end
function modifier_skeleton_king_hellfire_blast_custom_debuff:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_1") then
		self.per_damage = self.per_damage
			+ (
				self:GetCaster():GetHealthRegen()
				/ 100
				* self:GetAbility().modifier_skeleton_king_1_health_regen[self:GetCaster()
					:GetTalentLevel("modifier_skeleton_king_1")]
			)
	end
	local particle_name_debuff = "particles/units/heroes/hero_skeletonking/skeletonking_hellfireblast_debuff.vpcf"
	if self:GetCaster():HasModifier("modifier_skeleton_king_1") then
		particle_name_debuff =
			"particles/econ/items/wraith_king/wraith_king_ti6_bracer/wraith_king_ti6_hellfireblast_debuff.vpcf"
	end
	local debuff_particle =
		ParticleManager:CreateParticle(particle_name_debuff, PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_3") then
		self.count = self.count
			+ self:GetAbility().modifier_skeleton_king_3[self:GetCaster():GetTalentLevel("modifier_skeleton_king_3")]
	end
