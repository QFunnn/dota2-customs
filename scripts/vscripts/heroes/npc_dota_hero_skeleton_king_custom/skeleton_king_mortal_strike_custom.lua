...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_mortal_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_mortal_strike_custom_debuff",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_mortal_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_skeleton_king_mortal_strike_custom:IsHidden()
	return true
end
function modifier_skeleton_king_mortal_strike_custom:IsPurgable()
	return false
end
...
function modifier_skeleton_king_mortal_strike_custom:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_13") then
		params.unit:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_skeleton_king_mortal_strike_custom_debuff",
			{ duration = 3, damage = params.damage }
		)
	end
end

modifier_skeleton_king_mortal_strike_custom_debuff = class({})
function modifier_skeleton_king_mortal_strike_custom_debuff:IsPurgable()
	return false
end
function modifier_skeleton_king_mortal_strike_custom_debuff:IsPurgeException()
	return false
end
function modifier_skeleton_king_mortal_strike_custom_debuff:OnCreated(params)
	...
	self.damage = params.damage
		/ 100
		* self:GetAbility().modifier_skeleton_king_13[self:GetCaster():GetTalentLevel("modifier_skeleton_king_13")]
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_skeletonking/wraith_king_curse_overhead.vpcf",
...
function modifier_skeleton_king_mortal_strike_custom_debuff:OnDestroy()
	...
	local modifier_skeleton_king_vampiric_aura_custom =
		self:GetCaster():FindModifierByName("modifier_skeleton_king_vampiric_aura_custom")
