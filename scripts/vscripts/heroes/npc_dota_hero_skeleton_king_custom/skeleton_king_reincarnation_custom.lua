...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_reincarnation_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom_slow_aura",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_reincarnation_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom_blink",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_reincarnation_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skeleton_king_reincarnation_custom:ReincarnationStart(params)
	...
	if reincarnate then
		...
		if
			skeleton_king_bone_guard_custom
			and skeleton_king_bone_guard_custom:GetLevel() > 0
			and not self:GetCaster():HasModifier("modifier_skeleton_king_8")
		then
			local units = FindUnitsInRadius(
				self:GetCaster():GetTeamNumber(),
				self:GetCaster():GetAbsOrigin(),
				nil,
				self:GetSpecialValueFor("slow_radius"),
			...
			if #units > 0 then
				for i = 1, shard_skeleton_count do
					skeleton_king_bone_guard_custom:CreateSkeleton(
						self:GetCaster():GetAbsOrigin(),
						nil,
						nil,
						true,
...
function modifier_skeleton_king_reincarnation_custom:RemoveOnDeath()
	return false
end
function modifier_skeleton_king_reincarnation_custom:IsHidden()
	return true
end
function modifier_skeleton_king_reincarnation_custom:IsPurgable()
	return false
end
...
function modifier_skeleton_king_reincarnation_custom:ReincarnateTime()
	if IsServer() then
		...
		if self:GetParent():HasModifier("modifier_skeleton_king_5") then
			return nil
		end
		if self:GetParent().IS_LOSE_GAME ~= nil then
			return
		end
...
function modifier_skeleton_king_reincarnation_custom_slow:IsHidden()
	return false
end
