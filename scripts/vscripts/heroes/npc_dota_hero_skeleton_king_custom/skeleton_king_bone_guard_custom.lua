...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_bone_guard_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_vampiric_aura_ghost",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_bone_guard_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skeleton_king_bone_guard_custom:CreateSkeleton(origin, target, duration_custom, reincarnation, ultimate)
	...
	local skelet =
		CreateUnitByName(unit_name, origin, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
...
function modifier_skeleton_vampiric_aura_ghost:IsPurgable()
	return false
end
function modifier_skeleton_vampiric_aura_ghost:IsPurgeException()
	return false
end
function modifier_skeleton_vampiric_aura_ghost:IsHidden()
	return true
end
function modifier_skeleton_vampiric_aura_ghost:RemoveOnDeath()
	return false
end
function modifier_skeleton_vampiric_aura_ghost:OnCreated()
	...
	health = health
		+ self:GetAbility().modifier_skeleton_king_15_health[self:GetCaster()
			:GetTalentLevel("modifier_skeleton_king_15")]
