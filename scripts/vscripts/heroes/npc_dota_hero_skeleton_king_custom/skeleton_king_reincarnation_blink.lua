...
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom_damage_buff",
	"heroes/npc_dota_hero_skeleton_king_custom/skeleton_king_reincarnation_blink",
...
function skeleton_king_reincarnation_blink:OnSpellStart()
	...
	local particle_start =
		ParticleManager:CreateParticle("particles/wraith_king_blink_effect.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	local skeleton_king_hellfire_blast_custom = self:GetCaster()
		:FindAbilityByName("skeleton_king_hellfire_blast_custom")
	...
	if self:GetCaster():HasModifier("modifier_skeleton_king_4") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_skeleton_king_reincarnation_custom_damage_buff",
			{ duration = self.modifier_skeleton_king_4_duration }
		)
	end
end
