...
LinkLuaModifier(
	"modifier_ability_marci_unleash_debuff_silence",
	"heroes/hero_marci/unleash.lua",
...
modifier_ability_marci_unleash = class({
	...
	OnAbilityfullCastCustom = function(self, event)
		if event.unit ~= self.parent or self.dispel == 0 then
			return
		end
	...
	OnDeathEvent = function(self, event)
		if event.attacker ~= self.parent or self.extend_duration == 0 then
			return
		end
...
modifier_ability_marci_unleash_fury = class({
	...
	GetModifierAttackSpeed_Limit = function(self)
		return 1
	end,
	GetModifierAttackSpeedBonus_Constant = function(self)
	...
	OnCreated = function(self)
		...
		self.animation =
			self.parent:AddNewModifier(self.parent, self.ability, "modifier_ability_marci_unleash_animation", {})
	...
	OnDestroy = function(self)
		...
		self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_ability_marci_unleash_recovery",
