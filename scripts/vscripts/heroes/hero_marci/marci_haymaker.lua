	...
	if IsServer() then
		...
		if IsValid(hParent) and iTargetIndex then
			...
			if IsValid(hTarget) then
				...
				local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_marci/marci_haymaker_charge.vpcf", PATTACH_CUSTOMORIGIN, hParent)
...
function modifier_marci_haymaker_delay:OnDestroy()
	if IsServer() then
		...
		if IsValid(hParent) and IsValid(hAbility) then
			local iParticleID2 = ParticleManager:CreateParticle("particles/units/heroes/hero_marci/marci_haymaker_bash.vpcf", PATTACH_CUSTOMORIGIN, hParent)
			...
			local iParticleID3 = ParticleManager:CreateParticle("particles/units/heroes/hero_marci/marci_haymaker_attackquick_swipes.vpcf", PATTACH_CUSTOMORIGIN, hParent)
