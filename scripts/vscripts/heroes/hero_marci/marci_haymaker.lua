...
function marci_haymaker:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if IsValid(hTarget) then
		...
		if IsPointInsideRectangle(hTarget:GetAbsOrigin(), lu, ru, ld, rd) then
			local vHitPos = hTarget:GetAbsOrigin()
			local vDirTarget = vHitPos - vStartPos
			vDirEnd.z = 0
			vDirTarget.z = 0
			local cos = GetDotProduct(vDirTarget, vDirEnd) / (vDirTarget:Length2D() * vDirEnd:Length2D())
			...
			if bBash then
				...
				if iStack > 0 then
					hParent:AddNewModifier(
						hParent,
						hAbility,
						"modifier_marci_haymaker_shield",
						{ duration = self.shield_duration, stack = iStack }
					)
					hParent:AddNewModifier(
						hParent,
						hAbility,
						"modifier_marci_haymaker_delay",
						{ duration = self.delay, target_index = hTarget:entindex(), stack = iStack }
					)
...
function modifier_marci_haymaker:GetModifierIncomingDamage_Percentage(params)
	...
	if IsValid(hAttacker) and IsValid(hParent) and IsValid(hAbility) and hParent ~= hAttacker and fDamage > 0 then
		hParent:AddNewModifier(
			hParent,
			hAbility,
			"modifier_marci_haymaker_damage",
			{ duration = self.store_duration, stack = math.floor(fDamage) }
		)
	end
end
---------------------------------------------------------------------
		...
		if IsValid(hParent) and iTargetIndex then
			...
			if IsValid(hTarget) then
				...
				local iParticleID = ParticleManager:CreateParticle(
					"particles/units/heroes/hero_marci/marci_haymaker_charge.vpcf",
	...
	if IsServer() then
		...
		if IsValid(hParent) and IsValid(hAbility) then
			local iParticleID2 = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_marci/marci_haymaker_bash.vpcf",
				PATTACH_CUSTOMORIGIN,
				hParent
			)
			...
			EmitSoundOn("Hero_ElderTitan.EchoStomp", hParent)

			local iParticleID3 = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_marci/marci_haymaker_attackquick_swipes.vpcf",
				PATTACH_CUSTOMORIGIN,
				hParent
			)
			ParticleManager:SetParticleControlTransformForward(
				iParticleID3,
				0,
				self.vStartPos + self.vDir * self.range * 0.5,
				self.vDir
			)
			...
			local info = {
				...
				fExpireTime = GameRulesCustom:GetGameTime() + 10,
				ExtraData = {
