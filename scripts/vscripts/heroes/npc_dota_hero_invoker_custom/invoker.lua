...
LinkLuaModifier(
	"modifier_invoker_quas_custom_passive",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invoker_quas_custom_damage_interval",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function invoker_quas_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_invoker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_invoker.vpcf",
		context
	)
end
...
function modifier_invoker_quas_custom_passive:IsHidden()
	return true
end
function modifier_invoker_quas_custom_passive:IsPurgable()
	return false
end
function modifier_invoker_quas_custom_passive:IsPurgeException()
	return false
end
...
function modifier_invoker_quas_custom_passive:OnAttackLanded(params)
	...
	if not self:GetCaster():HasModifier("modifier_invoker_15") then
		return
	end
	params.target:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_invoker_quas_custom_damage_interval",
		{ duration = self:GetAbility().modifier_invoker_15_duration + FrameTime() }
	)
end
...
function modifier_invoker_quas_custom_damage_interval:OnIntervalThink()
	...
	ApplyDamage({
		...
		damage = self:GetAbility().modifier_invoker_15_damage[self:GetCaster():GetTalentLevel("modifier_invoker_15")]
			* self:GetAbility():GetLevel(),
...
function modifier_invoker_quas_custom_damage_interval:OnTooltip()
	return self:GetAbility().modifier_invoker_15_damage[self:GetCaster():GetTalentLevel("modifier_invoker_15")]
		* self:GetAbility():GetLevel()
end

LinkLuaModifier("modifier_invoker_wex_custom", "heroes/npc_dota_hero_invoker_custom/invoker", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_invoker_wex_custom_passive",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_wex_custom_passive:IsHidden()
	return true
end
function modifier_invoker_wex_custom_passive:IsPurgable()
	return false
end
function modifier_invoker_wex_custom_passive:IsPurgeException()
	return false
end
...
function modifier_invoker_wex_custom_passive:GetModifierAttackSpeedBonus_Constant()
	if not self:GetParent():HasModifier("modifier_invoker_8") then
		return
	end
	return self:GetAbility().modifier_invoker_8_attack_speed[self:GetCaster():GetTalentLevel("modifier_invoker_8")]
		* self:GetAbility():GetLevel()
end
...
LinkLuaModifier(
	"modifier_invoker_exort_custom_passive",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_exort_custom_passive:IsHidden()
	return true
end
function modifier_invoker_exort_custom_passive:IsPurgable()
	return false
end
function modifier_invoker_exort_custom_passive:IsPurgeException()
	return false
end
...
function modifier_invoker_exort_custom_passive:GetModifierPreAttack_BonusDamage()
	if not self:GetParent():HasModifier("modifier_invoker_1") then
		return
	end
	return self:GetAbility().modifier_invoker_1_damage[self:GetCaster():GetTalentLevel("modifier_invoker_1")]
		* self:GetAbility():GetLevel()
end
...
function invoker_invoke_custom:OnHeroLevelUp()
	...
	if self:GetCaster():GetLevel() >= self:GetSpecialValueFor("first_ability_level") and not self.reach_points[1] then
		...
		print("first invoke")
	elseif
		self:GetCaster():GetLevel() >= self:GetSpecialValueFor("second_ability_level") and not self.reach_points[2]
	then
		self.reach_points[2] = true
		self:GetCaster():SetAbilityPoints(self:GetCaster():GetAbilityPoints() + 1)
		print("second invoke")
	elseif
		self:GetCaster():GetLevel() >= self:GetSpecialValueFor("third_ability_level") and not self.reach_points[3]
	then
...
function orb_manager:Add(modifier, particle)
	...
	modifier:GetCaster().invoked_orbs_particle[3] =
		ParticleManager:CreateParticle(particle, PATTACH_OVERHEAD_FOLLOW, modifier:GetCaster())
	ParticleManager:SetParticleControlEnt(
		modifier:GetCaster().invoked_orbs_particle[3],
		1,
		modifier:GetCaster(),
		PATTACH_POINT_FOLLOW,
		modifier:GetCaster().invoked_orbs_particle_attach[1],
		modifier:GetCaster():GetAbsOrigin(),
		false
	)
...
function modifier_invoker_alacrity_custom:GetModifierAttackRangeBonus()
	if not self:GetCaster():HasModifier("modifier_invoker_10") then
		return
	end
...
function modifier_invoker_emp_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_invoker_12") then
		self.mana_burned = self.mana_burned
			+ self:GetAbility().modifier_invoker_12_mana[self:GetCaster():GetTalentLevel("modifier_invoker_12")]
	end
...
function modifier_emp_pull_custom:IsHidden()
	return true
end
function modifier_emp_pull_custom:IsPurgable()
	return false
end
...
function modifier_emp_pull_custom:OnIntervalThink()
	...
	if
		not self:GetParent():IsCurrentlyHorizontalMotionControlled()
		and not self:GetParent():IsCurrentlyVerticalMotionControlled()
	then
...
function modifier_emp_pull_custom:OnDestroy()
	...
	if self:GetParent():IsCurrentlyVerticalMotionControlled() then
		return
	end
	FindClearSpaceForUnit(self:GetParent(), self:GetParent():GetAbsOrigin(), true)
end
...
LinkLuaModifier(
	"modifier_invoker_ghost_walk_custom_purge",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function invoker_ghost_walk_custom:OnSpellStart()
	...
	local duration = self:GetSpecialValueFor("duration")
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_invoker_ghost_walk_custom_purge", { duration = duration })
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_invoker_ghost_walk_custom", { duration = duration })
	local effect_cast = ParticleManager:CreateParticle(
...
function modifier_invoker_ghost_walk_custom_purge:IsHidden()
	return true
end
...
function modifier_invoker_ghost_walk_custom_debuff:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_invoker_20") then
		return
	end
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		damage = self:GetAbility().modifier_invoker_20 * 0.5,
		damage_type = DAMAGE_TYPE_MAGICAL,
		ability = self:GetAbility(),
	})
end
...
LinkLuaModifier(
	"modifier_invoker_cold_snap_custom",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invoker_cold_snap_custom_passive",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_cold_snap_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_invoker_17") then
		self.damage = self.damage
			+ self:GetAbility().modifier_invoker_17[self:GetCaster():GetTalentLevel("modifier_invoker_17")]
	end
...
function modifier_invoker_cold_snap_custom:OnTakeDamage(params)
	if IsServer() then
		...
		if params.damage < self.threshold then
			return
		end
...
function modifier_invoker_cold_snap_custom:Freeze()
	...
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", { duration = self.duration })
...
LinkLuaModifier(
	"modifier_invoker_forge_spirit_custom",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_1_spirit_melting_strike_custom_debuff",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invulnerable_forge_custom",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_forged_spirit_melting_strike_custom_debuff",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invulnerable_forge_custom:IsPurgable()
	return false
end
function modifier_invulnerable_forge_custom:IsHidden()
	return true
end
function modifier_invulnerable_forge_custom:IsPurgeException()
	return false
end
...
function invoker_chaos_meteor_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_invoker_5") then
		...
		if
			self:GetCaster():GetTalentLevel("modifier_invoker_5") == 1
			or self:GetCaster():GetTalentLevel("modifier_invoker_5") == 2
		then
			CreateModifierThinker(
				self:GetCaster(),
				self,
				"modifier_invoker_chaos_meteor_custom_thinker",
				{},
				RotatePosition(point, QAngle(0, -90, 0), point + direction * 200),
				self:GetCaster():GetTeamNumber(),
				false
			)
		end
...
function modifier_invoker_chaos_meteor_custom_burn:GetAttributes()
	...
	if IsServer() then
		if self:GetAbility() and (not self:GetAbility():IsNull()) then
			...
			if self:GetCaster():HasModifier("modifier_invoker_4") then
				damage = damage
					+ (
						self:GetCaster():GetStrength()
						/ 100
						* self:GetAbility().modifier_invoker_4[self:GetCaster():GetTalentLevel("modifier_invoker_4")]
					)
			end
			local delay = 1
			self.damageTable = {
...
LinkLuaModifier(
	"modifier_invoker_tornado_custom",
	"heroes/npc_dota_hero_invoker_custom/invoker",
...
LinkLuaModifier(
	"modifier_invoker_tornado_custom_twister_damage",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_BOTH
)
...
function modifier_invoker_tornado_custom_twister:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_invoker/invoker_twister.vpcf",
		PATTACH_WORLDORIGIN,
		nil
	)
...
function modifier_invoker_tornado_custom_twister_damage:OnIntervalThink()
	...
	ApplyDamage({
		victim = self:GetParent(),
		attacker = self:GetCaster(),
		damage = self:GetCaster():GetAgility() * 0.25,
		damage_type = DAMAGE_TYPE_MAGICAL,
		ability = self:GetAbility(),
	})
end
...
function modifier_invoker_tornado_custom:OnDestroy()
	...
	local damage = self:GetAbility():GetSpecialValueFor("base_damage")
		+ ability_manager:GetValueWex(self:GetAbility(), self:GetCaster(), "wex_damage")
...
LinkLuaModifier(
	...
	"modifiers/modifier_generic_knockback_lua.lua",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
...
function invoker_deafening_blast_custom:OnSpellStart()
	...
	if caster:HasModifier("modifier_invoker_11") then
		...
		for var = 1, 13, 1 do
			...
			projectile.vVelocity = RotatePosition(Vector(0, 0, 0), QAngle(0, i, 0), caster:GetForwardVector())
				* travel_speed
			i = i + 30
		end
	else
...
function invoker_deafening_blast_custom:OnProjectileHit_ExtraData(target, location, ExtraData)
	if target then
		...
		if self:GetCaster():HasModifier("modifier_invoker_19") then
			damage = damage
				+ self.modifier_invoker_19_damage_blast[self:GetCaster():GetTalentLevel("modifier_invoker_19")]
		end

		if self:GetCaster():HasModifier("modifier_invoker_11") then
			disarm_duration = disarm_duration
				+ self.modifier_invoker_11[self:GetCaster():GetTalentLevel("modifier_invoker_11")]
		end

		if self:GetCaster():HasModifier("modifier_invoker_19") then
			knockback_duration = knockback_duration
				+ self.modifier_invoker_19[self:GetCaster():GetTalentLevel("modifier_invoker_19")]
			knockback_distance = knockback_distance
				+ (self.modifier_invoker_19[self:GetCaster():GetTalentLevel("modifier_invoker_19")] * 160)
		end
...
function modifier_invoker_deafening_blast_custom:GetEffectName()
	return "particles/units/heroes/hero_invoker/invoker_deafening_blast_disarm_debuff.vpcf"
end
function modifier_invoker_deafening_blast_custom:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end
...
LinkLuaModifier(
	"modifier_invoker_ice_wall_custom_slow",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invoker_ice_wall_custom_aura",
	"heroes/npc_dota_hero_invoker_custom/invoker",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_ice_wall_custom:CreateWall(kv)
	...
	local ice_wall_particle_effect = ParticleManager:CreateParticle(
	...
	local ice_spikes_particle_effect = ParticleManager:CreateParticle(
...
function modifier_invoker_ice_wall_custom:OnCreated(kv)
	...
	if self.modifier_invoker_21 then
		...
		self.glacier_marker_particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_invoker/invoker_glacierwall_marker.vpcf",
			PATTACH_WORLDORIGIN,
			self.parent
		)
		...
		ParticleManager:SetParticleControl(
			self.glacier_marker_particle,
			1,
			Vector(wall_total_length, wall_width, self:GetDuration())
		)
	else
		self:CreateWall(kv)
	end
...
function modifier_invoker_ice_wall_custom:OnIntervalThink()
	...
	if
		self.modifier_invoker_21
		and not self.glacier_wall_created
		and self:GetElapsedTime() >= self.glacier_formation_delay
	then
		...
		if self:IsUnitInProximity(enemy:GetAbsOrigin()) then
			if self.count >= 3 and self.modifier_invoker_21 and not self.hit_targets[enemy] then
				enemy:AddNewModifier(
					...
					self.ability,
					"modifier_invoker_ice_wall_custom_root",
					{ duration = (1 - enemy:GetStatusResistance()) * 1.5 }
				)
				...
				local particle = ParticleManager:CreateParticle(
					"particles/units/heroes/hero_invoker/invoker_ice_wall_hero_shatter.vpcf",
					PATTACH_ABSORIGIN_FOLLOW,
					enemy
				)
...
function modifier_invoker_ice_wall_custom:CreateGlacierWall()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_invoker/invoker_glacierwall.vpcf",
		PATTACH_WORLDORIGIN,
		self.parent
	)
	...
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self.glacier_formation_speed, self.glacier_wall_width, self:GetDuration())
	)
	self:AddParticle(particle, false, false, -1, false, true)
end
...
function modifier_invoker_ice_wall_custom:IsUnitInProximity(target_position)
	if self.modifier_invoker_21 then
		...
		local formed_length = math.min(
			self.glacier_wall_half_length,
			self.glacier_formation_speed * (self:GetElapsedTime() - self.glacier_formation_delay)
		)
		local center = self.parent:GetAbsOrigin()
		return self:IsUnitNearLine(target_position, center, center + self.glacier_dir_1 * formed_length)
			or self:IsUnitNearLine(target_position, center, center + self.glacier_dir_2 * formed_length)
	end
...
function modifier_invoker_ice_wall_custom_aura:IsHidden()
	return true
end
function modifier_invoker_ice_wall_custom_aura:IsPurgable()
	return false
end
			...
			if enemy ~= nil and enemy:IsAlive() then
				...
				enemy:AddNewModifier(
					...
					"modifier_invoker_ice_wall_custom_slow",
					{
						...
						enemy_slow = self.ice_wall_slow * (1 - enemy:GetStatusResistance()),
					}
				)
			end
		end
...
function modifier_invoker_ice_wall_custom_slow:IsPassive()
	return false
end
function modifier_invoker_ice_wall_custom_slow:IsBuff()
	return false
end
function modifier_invoker_ice_wall_custom_slow:IsDebuff()
	return true
end
function modifier_invoker_ice_wall_custom_slow:IsPurgable()
	return false
end
function modifier_invoker_ice_wall_custom_slow:IsHidden()
	return false
end
function modifier_invoker_ice_wall_custom_slow:GetEffectName()
	return "particles/units/heroes/hero_invoker/invoker_ice_wall_debuff.vpcf"
end
function modifier_invoker_ice_wall_custom_slow:GetStatusEffectName()
	return "particles/status_fx/status_effect_frost.vpcf"
end
		...
		if self:GetCaster():HasModifier("modifier_invoker_18") then
			damage = damage
				+ (
					damage
					/ 100
					* self:GetAbility().modifier_invoker_18[self:GetCaster():GetTalentLevel("modifier_invoker_18")]
				)
		end
	end
