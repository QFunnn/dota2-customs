...
LinkLuaModifier(
	"modifier_bloodseeker_blood_mist_lua",
	"heroes/hero_bloodseeker/bloodseeker_blood_mist_lua.lua",
...
function modifier_bloodseeker_blood_mist_lua:OnIntervalThink()
	...
	if IsValid(hParent) and IsValid(hAbility) then
		...
		if self.LastTickTime + self.tick < GameRulesCustom:GetGameTime() then
			self.LastTickTime = GameRulesCustom:GetGameTime()
			ApplyDamage({
				attacker = hParent,
				victim = hParent,
			...
			for _, unit in pairs(units) do
				if IsValid(unit) and unit:IsAlive() then
					...
					local fDamage = self.damage
						* (100 + math.floor((fRadius - fDistance) / self.range_damage_threshold) * self.range_damage_pct)
						* 0.01
					ApplyDamage({
						attacker = hParent,
						victim = unit,
