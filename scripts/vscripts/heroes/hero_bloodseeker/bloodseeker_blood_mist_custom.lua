...
LinkLuaModifier(
	...
	"heroes/hero_bloodseeker/bloodseeker_blood_mist_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bloodseeker_blood_mist_custom_slow",
	"heroes/hero_bloodseeker/bloodseeker_blood_mist_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bloodseeker_blood_mist_custom_shield",
	"heroes/hero_bloodseeker/bloodseeker_blood_mist_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_bloodseeker_blood_mist_custom:IsHidden()
	return false
end
function modifier_bloodseeker_blood_mist_custom:IsPurgable()
	return false
end
function modifier_bloodseeker_blood_mist_custom:IsPurgeException()
	return false
end
...
function modifier_bloodseeker_blood_mist_custom:OnCreated()
	...
	local particle = ParticleManager:CreateParticle(
		"particles/bloodmist_custom_particle.vpcf",
	...
	if keys.unit == self:GetParent() then
		if self:GetParent():GetHealthPercent() >= 100 then
			if not self:GetParent():HasModifier("modifier_hero_refreshing") then
				...
				--    modifier_bloodseeker_blood_mist_custom_shield:SetStackCount( math.min(modifier_bloodseeker_blood_mist_custom_shield:GetStackCount() + health, self:GetCaster():GetMaxHealth()*0.5 ))
				--end
			end
		end
	end
...
function modifier_bloodseeker_blood_mist_custom_slow:IsPurgable()
	return true
end
...
function modifier_bloodseeker_blood_mist_custom_shield:IsHidden()
	return true
end
function modifier_bloodseeker_blood_mist_custom_shield:IsPurgable()
	return false
end
function modifier_bloodseeker_blood_mist_custom_shield:IsPurgeException()
	return false
end
...
function modifier_bloodseeker_blood_mist_custom_shield:OnIntervalThink()
	...
	local shield_minus = (shield / 100 * 2) * 0.1
