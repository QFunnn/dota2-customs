...
LinkLuaModifier(
	"modifier_marci_unleash_custom",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_unleash_custom_attacks",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_unleash_custom_attacks_refresh",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_unleash_custom_attacks_animation",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_marci_unleash_custom_str_buff",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_unleash_custom_str_debuff",
	"heroes/npc_dota_hero_marci_custom/marci_unleash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function marci_unleash_custom:GetIntrinsicModifierName()
	if self:GetCaster():HasModifier("modifier_marci_15") then
		return
	end
	if self:GetCaster():HasModifier("modifier_marci_14") then
	...
	for _, enemy in pairs(enemies) do
		...
		if not enemy:IsHero() and self:GetCaster():HasModifier("modifier_marci_5") then
			pulse_damage = pulse_damage
				+ self.modifier_marci_5_creep_damage[self:GetCaster():GetTalentLevel("modifier_marci_5")]
		end
		ApplyDamage({
			victim = enemy,
			...
			ability = ability_usefull,
		})
		enemy:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_marci_unleash_custom_debuff",
			{ duration = pulse_debuff_duration }
		)
		if self:GetCaster():HasModifier("modifier_marci_5") and enemy:IsRealHero() then
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_marci_unleash_custom_str_buff",
				{ duration = self.modifier_marci_5_duration[self:GetCaster():GetTalentLevel("modifier_marci_5")] }
			)
			enemy:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_marci_unleash_custom_str_debuff",
				{
					duration = self.modifier_marci_5_duration[self:GetCaster():GetTalentLevel("modifier_marci_5") * (1 - enemy:GetStatusResistance())],
				}
			)
		end
		if self:GetCaster():HasModifier("modifier_marci_7") then
			enemy:AddNewModifier(
				...
				{ duration = self.modifier_marci_7 }
			)
		end
	end
...
function modifier_marci_unleash_custom:IsHidden()
	return self:GetCaster():HasModifier("modifier_marci_14")
end
function modifier_marci_unleash_custom:IsPurgable()
	return false
end
function modifier_marci_unleash_custom:IsPurgeException()
	return false
end
function modifier_marci_unleash_custom:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_marci_14")
end
...
function modifier_marci_unleash_custom:OnIntervalThink()
	...
	if
		self:GetCaster():HasModifier("modifier_marci_14")
		and not self:GetParent():HasModifier("modifier_marci_unleash_custom_attacks")
	then
		self:GetParent()
			:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_marci_unleash_custom_attacks", {})
	end
end
...
function modifier_marci_unleash_custom:OnDestroy()
	...
	local recovery = self:GetParent()
		:FindModifierByNameAndCaster("modifier_marci_unleash_custom_attacks_refresh", self:GetParent())
	if recovery then
...
function modifier_marci_unleash_custom_attacks:IsPurgable()
	return false
end
function modifier_marci_unleash_custom_attacks:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_marci_14")
end
...
function modifier_marci_unleash_custom_attacks:OnCreated(kv)
	...
	self.success = 0
	self.modifier_marci_unleash_custom_attacks_animation =
		self:GetParent()
			:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_marci_unleash_custom_attacks_animation", {})
...
function modifier_marci_unleash_custom_attacks:OnDestroy()
	...
	local modifier_marci_unleash_custom =
		self:GetParent():FindModifierByNameAndCaster("modifier_marci_unleash_custom", self:GetParent())
	if not modifier_marci_unleash_custom then
		return
	end
	if self.forced then
		return
	...
	if self:GetParent():HasModifier("modifier_marci_14") then
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_marci_unleash_custom_attacks_refresh",
			{ duration = FrameTime(), success = self.success }
		)
	else
		self:GetParent():AddNewModifier(
			self:GetParent(),
			self:GetAbility(),
			"modifier_marci_unleash_custom_attacks_refresh",
			{ duration = self.time_between_flurries, success = self.success }
		)
	end
...
function modifier_marci_unleash_custom_attacks_animation:IsHidden()
	return true
end
function modifier_marci_unleash_custom_attacks_animation:IsDebuff()
	return false
end
function modifier_marci_unleash_custom_attacks_animation:IsPurgable()
	return false
end
...
function modifier_marci_unleash_custom_attacks_refresh:IsDebuff()
	return true
end
function modifier_marci_unleash_custom_attacks_refresh:IsPurgable()
	return false
end
...
function modifier_marci_unleash_custom_str_buff:GetTexture()
	return "marci_5"
end
...
function modifier_marci_unleash_custom_str_debuff:GetTexture()
	return "marci_5"
end
