...
LinkLuaModifier(
	"modifier_marci_guardian_custom",
	"heroes/npc_dota_hero_marci_custom/marci_guardian_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_guardian_custom_buff",
	"heroes/npc_dota_hero_marci_custom/marci_guardian_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_guardian_custom_timer",
	"heroes/npc_dota_hero_marci_custom/marci_guardian_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function marci_guardian_custom:OnSpellStart()
	...
	if
		self:GetCaster():HasModifier("modifier_marci_7")
		and self:GetCaster():HasModifier("modifier_marci_unleash_custom")
	then
	...
	if self:GetCaster():HasModifier("modifier_marci_4") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_marci_guardian_custom_buff",
			{ duration = self:GetSpecialValueFor("bodyguard_duration") }
		)
	...
	local modifier_marci_guardian_custom = self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_marci_guardian_custom_buff",
		{ duration = self:GetSpecialValueFor("bodyguard_duration") }
	...
	local modifier_marci_guardian_custom_buff = target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_marci_guardian_custom_buff",
		{ duration = self:GetSpecialValueFor("bodyguard_duration") }
...
function modifier_marci_guardian_custom:IsHidden()
	return true
end
function modifier_marci_guardian_custom:IsPurgable()
	return false
end
function modifier_marci_guardian_custom:IsPurgeException()
	return false
end
function modifier_marci_guardian_custom:RemoveOnDeath()
	return false
end
...
function modifier_marci_guardian_custom_buff:IsPurgable()
	return false
end
function modifier_marci_guardian_custom_buff:IsPurgeException()
	return false
end
...
function modifier_marci_guardian_custom_buff:GetModifierBaseDamageOutgoing_Percentage()
	...
	return self:GetAbility():GetSpecialValueFor("bonus_damage")
		/ 100
		* self:GetAbility():GetSpecialValueFor("max_partner_penalty")
end
...
function modifier_marci_guardian_custom_buff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_marci_4") then
		self.barrier = self.barrier
			+ self:GetAbility().modifier_marci_4_max_barrier[self:GetCaster():GetTalentLevel("modifier_marci_4")]
	end
...
function modifier_marci_guardian_custom_buff:GetModifierIncomingDamageConstant(params)
	...
	if params.damage >= self.current_shield then
		if self.target then
			local modifier_marci_guardian_custom_buff =
				self.target:FindModifierByName("modifier_marci_guardian_custom_buff")
			if modifier_marci_guardian_custom_buff then
...
function modifier_marci_guardian_custom_buff:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_marci_4") then
		shared_barrier_replenish_amount = shared_barrier_replenish_amount
			+ self:GetAbility().modifier_marci_4_heal_barrier[self:GetCaster():GetTalentLevel("modifier_marci_4")]
	end
	self.current_shield =
		math.min(self.current_shield + (params.damage / 100 * shared_barrier_replenish_amount), self.max_shield)
	if self.target then
		local modifier_marci_guardian_custom_buff =
			self.target:FindModifierByName("modifier_marci_guardian_custom_buff")
		if modifier_marci_guardian_custom_buff then
...
function modifier_marci_guardian_custom_timer:GetTexture()
	return "marci_4"
end