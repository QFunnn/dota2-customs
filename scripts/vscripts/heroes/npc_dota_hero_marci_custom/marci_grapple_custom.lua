...
LinkLuaModifier(
	"modifier_marci_grapple_custom_debuff",
	"heroes/npc_dota_hero_marci_custom/marci_grapple_custom",
	...
	if self:GetCaster():HasModifier("modifier_marci_20") then
		impact_damage = impact_damage
			+ (
				...
				/ 100
				* self.modifier_marci_20[self:GetCaster():GetTalentLevel("modifier_marci_20")]
			)
	end
	local targetpos = caster:GetOrigin() - caster:GetForwardVector() * throw_distance_behind

	if
		self:GetCaster():HasModifier("modifier_marci_16")
		and self:GetCaster():GetTeamNumber() == target:GetTeamNumber()
	then
		local find_radius = self.modifier_marci_16 + (self:GetCaster():GetMaxMana() / 100 * self.modifier_marci_16_mana)
		local find_enemies = FindUnitsInRadius(
			self:GetCaster():GetTeamNumber(),
	...
	arc:SetEndCallback(function()
		...
		for _, enemy in pairs(enemies) do
			...
			local effect_cast = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_marci/marci_dispose_aoe_damage.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
