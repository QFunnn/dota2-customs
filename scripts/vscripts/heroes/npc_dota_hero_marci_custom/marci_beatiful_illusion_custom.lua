...
LinkLuaModifier(
	"marci_beatiful_illusion_custom_handler",
	"heroes/npc_dota_hero_marci_custom/marci_beatiful_illusion_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"marci_beatiful_illusion_custom_handler_explosion",
	"heroes/npc_dota_hero_marci_custom/marci_beatiful_illusion_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function marci_beatiful_illusion_custom:OnSpellStart()
	...
	local illusions = CreateIllusions(
		self:GetCaster(),
		self:GetCaster(),
		{ outgoing_damage = damage_out, incoming_damage = damage_in, duration = duration },
		illusion_count,
		150,
		false,
		true
...
function marci_beatiful_illusion_custom_handler:IsHidden()
	return true
end
function marci_beatiful_illusion_custom_handler:IsPurgable()
	return false
end
function marci_beatiful_illusion_custom_handler:IsPurgeException()
	return false
end
function marci_beatiful_illusion_custom_handler:RemoveOnDeath()
	return false
end
function marci_beatiful_illusion_custom_handler:IsAura()
	return true
end
function marci_beatiful_illusion_custom_handler:GetModifierAura()
	return "marci_beatiful_illusion_custom_handler_explosion"
end
function marci_beatiful_illusion_custom_handler:GetAuraRadius()
	return -1
end
function marci_beatiful_illusion_custom_handler:GetAuraDuration()
	return 3
end
function marci_beatiful_illusion_custom_handler:IsAuraActiveOnDeath()
	return true
end
function marci_beatiful_illusion_custom_handler:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function marci_beatiful_illusion_custom_handler:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function marci_beatiful_illusion_custom_handler:GetAuraSearchFlags()
...
function marci_beatiful_illusion_custom_handler_explosion:IsHidden()
	return false
end
...
function marci_beatiful_illusion_custom_handler_explosion:OnDestroy(params)
	...
	if not self:GetCaster():HasModifier("modifier_marci_17") then
		return
	end
	local damage = self:GetCaster():GetMaxMana()
		/ 100
		* self:GetAbility().modifier_marci_17[self:GetCaster():GetTalentLevel("modifier_marci_17")]
	local units = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
		self:GetParent():GetAbsOrigin(),
		nil,
		self:GetAbility().modifier_marci_17_radius,
	...
	local particle =
		ParticleManager:CreateParticle("particles/marci_illusion_death_effect.vpcf", PATTACH_WORLDORIGIN, nil)
