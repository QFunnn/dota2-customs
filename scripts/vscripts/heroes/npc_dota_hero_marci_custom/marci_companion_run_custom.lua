...
LinkLuaModifier(
	"modifier_marci_companion_run_custom",
	"heroes/npc_dota_hero_marci_custom/marci_companion_run_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_marci_companion_run_custom_buff",
	"heroes/npc_dota_hero_marci_custom/marci_companion_run_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_marci_companion_run_custom_handler",
	"heroes/npc_dota_hero_marci_custom/marci_companion_run_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function marci_companion_run_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_marci.vpcf",
		context
	)
end
	...
	if self:GetCaster():HasModifier("modifier_marci_11") then
		percent = percent
			- (
				(
					self:GetCaster()
						:GetModifierStackCount("modifier_marci_companion_run_custom_handler", self:GetCaster())
					/ 100
					* self.modifier_marci_11[self:GetCaster():GetTalentLevel("modifier_marci_11")]
				) / 100
			)
	end
...
function marci_companion_run_custom:DealDamage()
	...
	local damageTable =
		{ attacker = caster, damage = self:GetDamage(), damage_type = DAMAGE_TYPE_MAGICAL, ability = self }
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			self,
			"modifier_stunned",
			{ duration = (1 - enemy:GetStatusResistance()) * debuff_duration }
		)
	end
	if
...
function marci_companion_run_custom:OnSpellStartOther()
	...
	self.modifier = caster:AddNewModifier(
		...
		self,
		"modifier_marci_companion_run_custom",
		{ proj = tostring(proj), target = target:entindex(), is_toggle = is_toggle }
	)
end
...
function modifier_marci_companion_run_custom_handler:IsHidden()
	return true
end
function modifier_marci_companion_run_custom_handler:IsPurgable()
	return false
end
function modifier_marci_companion_run_custom_handler:IsPurgeException()
	return false
end
function modifier_marci_companion_run_custom_handler:RemoveOnDeath()
	return false
end
