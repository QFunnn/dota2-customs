...
LinkLuaModifier(
	"modifier_aghanim_ray_caster",
	"heroes/npc_dota_hero_meepo_custom/aghanim_ray",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_shard_thinker",
	"heroes/npc_dota_hero_meepo_custom/aghanim_shard",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_ray_magic_immune",
	"heroes/npc_dota_hero_meepo_custom/aghanim_ray",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_ray_21_talent_buff",
	"heroes/npc_dota_hero_meepo_custom/aghanim_ray",
	LUA_MODIFIER_MOTION_NONE
)
...
function aghanim_ray:OnSpellStart()
	...
	if distance > (self:GetSpecialValueFor("max_distance") + self:GetCaster():GetCastRangeBonus()) then
		point = self:GetCaster():GetAbsOrigin()
			+ direction * (self:GetSpecialValueFor("max_distance") + self:GetCaster():GetCastRangeBonus())
	end
	...
	if self:GetCaster():HasModifier("modifier_meepo_16") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_aghanim_ray_magic_immune",
			{ duration = self.modifier_meepo_16[self:GetCaster():GetTalentLevel("modifier_meepo_16")] }
		)
	end
	if self:GetCaster():HasModifier("modifier_meepo_4") then
		point = self:GetCaster():GetAbsOrigin()
		...
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_aghanim_ray_caster",
			{ duration = self:GetSpecialValueFor("duration"), x = point.x, y = point.y, z = point.z }
		)
		return
	end
	self:GetCaster():AddNewModifier(
...
function modifier_aghanim_ray:IsPurgable()
	return false
end
...
function modifier_aghanim_ray:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_meepo_17") then
		ray_speed = ray_speed
			+ self:GetAbility().modifier_meepo_17[self:GetCaster():GetTalentLevel("modifier_meepo_17")]
	end
	...
	if self:GetCaster():HasModifier("modifier_meepo_19") then
		health_ever = health_ever
			+ self:GetAbility().modifier_meepo_19[self:GetCaster():GetTalentLevel("modifier_meepo_19")]
	end
	...
	if self.tick_damage >= 0.25 then
		...
		for _, enemy in pairs(enemies) do
			...
			if self:GetCaster():HasModifier("modifier_meepo_20") then
				damage = damage
					+ (
						self:GetCaster():GetIntellect(false)
						/ 100
						* self:GetAbility().modifier_meepo_20[self:GetCaster():GetTalentLevel("modifier_meepo_20")]
					)
			end
			if self:GetCaster():HasModifier("modifier_meepo_2") then
				damage = damage
					+ (
						self:GetCaster():GetMaxHealth()
						/ 100
						* self:GetAbility().modifier_meepo_2[self:GetCaster():GetTalentLevel("modifier_meepo_2")]
					)
			end
...
function modifier_aghanim_ray:OnOrder(params)
	...
	if params.unit == self:GetParent() then
		...
		if validMoveOrders[params.order_type] then
			...
			direction = direction:Normalized()
			if
				distance > (self:GetAbility():GetSpecialValueFor("max_distance") + self:GetCaster():GetCastRangeBonus())
			then
				vTargetPos = self:GetCaster():GetAbsOrigin()
					+ direction
						* (self:GetAbility():GetSpecialValueFor("max_distance") + self:GetCaster():GetCastRangeBonus())
			end
			if distance < self:GetAbility():GetSpecialValueFor("min_distance") then
				vTargetPos = self:GetCaster():GetAbsOrigin()
					+ direction * self:GetAbility():GetSpecialValueFor("min_distance")
			end
...
function modifier_aghanim_ray_magic_immune:IsPurgable()
	return false
end
function modifier_aghanim_ray_magic_immune:GetTexture()
	return "aghanim_16"
end
...
function modifier_aghanim_ray_caster:IsPurgable()
	return false
end
function modifier_aghanim_ray_caster:IsPurgeException()
	return false
end
function modifier_aghanim_ray_caster:RemoveOnDeath()
	return not self:GetCaster():HasModifier("modifier_meepo_6")
end
...
function modifier_aghanim_ray_caster:OnIntervalThink()
	...
	local heroes = FindUnitsInRadius(
		...
		self:GetCaster():GetAbsOrigin(),
		nil,
		self:GetAbility().modifier_meepo_4[self:GetCaster():GetTalentLevel("modifier_meepo_4")],
		DOTA_UNIT_TARGET_TEAM_ENEMY,
