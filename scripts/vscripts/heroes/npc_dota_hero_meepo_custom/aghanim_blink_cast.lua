...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_meepo_custom/aghanim_blink_cast",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_aghanim_blink_attack_debuff",
	"heroes/npc_dota_hero_meepo_custom/aghanim_blink_cast",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_blink_attack_buff",
	"heroes/npc_dota_hero_meepo_custom/aghanim_blink_cast",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function aghanim_blink_cast:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_aghanim_blink_attack_backawards") then
		local modifier_aghanim_blink_attack_backawards = self:GetCaster()
			:FindModifierByName("modifier_aghanim_blink_attack_backawards")
		if modifier_aghanim_blink_attack_backawards then
			...
			local knockback = self:GetCaster()
				:AddNewModifier(self:GetCaster(), self, "modifier_generic_knockback_lua", {
	...
	local knockback = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_generic_knockback_lua", {
		direction_x = direciton.x,
		direction_y = direciton.y,
		distance = distance_teleport,
		duration = 0.25,
	...
	local nFXIndex =
		ParticleManager:CreateParticle("particles/creatures/aghanim/aghanim_preimage.vpcf", PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControl(nFXIndex, 0, origin)
	ParticleManager:SetParticleControl(nFXIndex, 1, point)
	ParticleManager:SetParticleFoWProperties(nFXIndex, 0, 2, 64.0)
	ParticleManager:ReleaseParticleIndex(nFXIndex)
	...
	local callback = function(bInterrupted)
		...
		if self:GetCaster():HasModifier("modifier_meepo_10") then
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_aghanim_blink_attack_buff",
				{ duration = self.modifier_meepo_10_duration }
			)
		end
		if self:GetCaster():HasModifier("modifier_meepo_13") then
			self:GetCaster():AddNewModifier(
				...
				self,
				"modifier_aghanim_blink_attack_backawards",
				{ duration = self.modifier_meepo_13, x = origin.x, y = origin.y, z = origin.z }
			)
		end
	end
...
function modifier_aghanim_blink_attack_backawards:IsPurgable()
	return false
end
function modifier_aghanim_blink_attack_backawards:RemoveOnDeath()
	return false
end
...
function aghanim_blink_cast:StunTargets(point)
	...
	local nFXCastIndex = ParticleManager:CreateParticle(
	...
	if true then
		...
		for _, enemy in pairs(enemies) do
			if enemy ~= nil and enemy:IsInvulnerable() == false then
				...
				direciton.z = 0
				direciton = direciton:Normalized()
				local knockback = enemy:AddNewModifier(self:GetCaster(), self, "modifier_generic_knockback_lua", {
					direction_x = direciton.x,
					direction_y = direciton.y,
					height = 50,
					distance = dlina - 75,
...
function modifier_aghanim_blink_attack_buff:GetTexture()
	return "aghanim_10"
end
