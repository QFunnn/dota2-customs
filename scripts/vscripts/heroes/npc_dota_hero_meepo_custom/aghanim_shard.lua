...
LinkLuaModifier(
	"modifier_aghanim_shard_debuff",
	"heroes/npc_dota_hero_meepo_custom/aghanim_shard",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_aghanim_shard_debuff_break",
	"heroes/npc_dota_hero_meepo_custom/aghanim_shard",
	LUA_MODIFIER_MOTION_NONE
)
...
function aghanim_shard:ApplyEffect(target)
	...
	if self:GetCaster():HasModifier("modifier_meepo_5") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_meepo_5_damage_health[self:GetCaster():GetTalentLevel("modifier_meepo_5")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_meepo_5") then
		target:AddNewModifier(
			...
			"modifier_aghanim_shard_debuff_break",
			{
				duration = self.modifier_meepo_5[self:GetCaster():GetTalentLevel("modifier_meepo_5")]
...
function aghanim_shard:OnProjectileHit_ExtraData(target, vLocation, table)
	...
	if target == nil then
		if table.main == 1 then
			...
			local dummy =
				CreateUnitByName("npc_dota_companion", point, false, nil, nil, self:GetCaster():GetTeamNumber())
...
function modifier_aghanim_shard_debuff:OnCreated()
	...
	self.particle =
		ParticleManager:CreateParticle("particles/aghs_crystal_debuff.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
...
function modifier_aghanim_shard_thinker:IsHidden()
	return true
end
function modifier_aghanim_shard_thinker:IsPurgable()
	return false
end
function modifier_aghanim_shard_thinker:RemoveOnDeath()
	return false
end
...
function modifier_aghanim_shard_debuff_break:GetTexture()
	return "aghanim_5"
end
function modifier_aghanim_shard_debuff_break:IsHidden()
	return false
end
function modifier_aghanim_shard_debuff_break:IsPurgable()
	return false
end
function modifier_aghanim_shard_debuff_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_aghanim_shard_debuff_break:GetEffectName()
