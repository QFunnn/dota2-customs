...
function aghanim_blast:OnSpellStart()
	...
	local effect_cast =
		ParticleManager:CreateParticle("particles/aghanim_lesh_tormented.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
	...
	for _, creep in pairs(creeps) do
		creep:AddNewModifier(
			...
			self,
			"modifier_stunned",
			{ duration = stun_duration * (1 - creep:GetStatusResistance()) }
		)
		local aghanim_shard = self:GetCaster():FindAbilityByName("aghanim_shard")
		if aghanim_shard and aghanim_shard:GetLevel() > 0 then
