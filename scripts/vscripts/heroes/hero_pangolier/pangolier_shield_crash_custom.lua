...
LinkLuaModifier(
	"modifier_pangolier_shield_crash_custom",
	"heroes/hero_pangolier/pangolier_shield_crash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_shield_crash_custom_debuff",
	"heroes/hero_pangolier/pangolier_shield_crash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function pangolier_shield_crash_custom:GetCooldown(level)
	if
		self:GetCaster():HasModifier("modifier_pangolier_gyroshell")
		and self:GetCaster():HasTalent("special_bonus_unique_pangolier_2")
	then
	...
	arc:SetEndCallback(function()
		...
		if stack > 0 or stack_creep > 0 then
			caster:AddNewModifier(
				...
				self,
				"modifier_pangolier_shield_crash_custom",
				{ duration = buff_duration, stack = stack, stack_creep = stack_creep }
			)
		end
...
function modifier_pangolier_shield_crash_custom:IsPurgable()
	return false
end
