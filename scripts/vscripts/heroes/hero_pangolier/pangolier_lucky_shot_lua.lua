...
LinkLuaModifier(
	"modifier_pangolier_lucky_shot_lua",
	"heroes/hero_pangolier/pangolier_lucky_shot_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_lucky_shot_lua_debuff",
	"heroes/hero_pangolier/pangolier_lucky_shot_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
	...
	then
		if PRD(hParent, self.chance_pct, "pangolier_lucky_shot_lua") then
			...
			ParticleManager:SetParticleControlTransformForward(
				...
				(hTarget:GetAbsOrigin() - hParent:GetAbsOrigin()):Normalized()
			)
			...
			hTarget:AddNewModifier(
				...
				hAbility,
				"modifier_pangolier_lucky_shot_lua_debuff",
				{ duration = self.duration * hTarget:GetStatusResistanceFactor(hParent) }
			)
		end
	end
end
function modifier_pangolier_lucky_shot_lua:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if
		...
		and not hParent:IsIllusion()
		and hInflictor
		and iCategory == DOTA_DAMAGE_CATEGORY_SPELL
		and not hParent:PassivesDisabled()
		and IsValid(hTarget)
		and hTarget:IsAlive()
		and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber()
