...
LinkLuaModifier(
	"modifier_ability_alchemist_unstable_concoction",
	"heroes/hero_alchemist/unstable_concoction.lua",
...
function ability_alchemist_unstable_concoction_throw:GetAOERadius()
...
function ability_alchemist_unstable_concoction_throw:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	...
	if Damage == nil or Stun == nil or bIsLinear == nil then
		return
	end

	if bIsLinear == 1 and hTarget ~= nil then
		return
	end
	if bIsLinear == 0 and hTarget == nil then
		return
	end
