...
function modifier_magic_crit:IsHidden()
	return true
end
function modifier_magic_crit:IsDebuff()
	return false
end
function modifier_magic_crit:IsPurgable()
	return false
end
function modifier_magic_crit:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
function modifier_magic_crit:RemoveOnDeath()
	return false
end
...
function modifier_magic_crit:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if IsServer() then
		if
			...
			and IsValid(hTarget)
			and hTarget:IsAlive()
			and not hParent:HasModifier("modifier_item_aeon_disk_lua_buff")
		then
			local bShouldActive = true
			if hAbility ~= nil then
				local AbilityName = hAbility:GetAbilityName()
				if
					PERCENTAGE_ABILITIES and PERCENTAGE_ABILITIES[AbilityName]
					or (AbilityName == "enigma_black_hole" and hParent:HasScepter())
					or (
			...
			if bShouldActive then
				local MagicCritResult = GetCustomModifierProps(
					hParent,
					"MODIFIER_PROPERTY_MAGICAL_CRIT_DAMAGE",
					params,
					CUSTOM_MODIFIER_PROPS_CAL_TYPE_MAX
				)
				if MagicCritResult then
					...
					if value > 0 then
						...
						hTarget:AddNewModifier(
							hParent,
							hAbility,
							"modifier_magic_crit_msg",
							{ duration = FrameTime(), record = params.record }
						)
...
function modifier_magic_crit_msg:IsHidden()
	return true
end
function modifier_magic_crit_msg:IsDebuff()
	return true
end
function modifier_magic_crit_msg:IsPurgable()
	return false
end
function modifier_magic_crit_msg:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
function modifier_magic_crit_msg:RemoveOnDeath()
	return false
end
	...
	if IsServer() then
		for i = #self.records, 1, -1 do
			if self.records[i] == params.record then
				...
				local particleID =
					ParticleManager:CreateParticle("particles/msg_fx/msg_crit.vpcf", PATTACH_OVERHEAD_FOLLOW, hParent)
