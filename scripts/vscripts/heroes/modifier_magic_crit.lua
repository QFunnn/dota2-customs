...
function modifier_magic_crit:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if IsServer() then
		if
			...
			and IsValid(target)
			and target:IsAlive()
			and not parent:HasModifier("modifier_item_aeon_disk_lua_buff")
		then
			local shouldActive = true
			if ability ~= nil then
				local abilityName = ability:GetAbilityName()
				if
					GAMEPLAY_KV.NoSpellAmpAbilities and GAMEPLAY_KV.NoSpellAmpAbilities[abilityName]
					or (abilityName == "enigma_black_hole" and parent:HasScepter())
					or (
						abilityName == "witch_doctor_voodoo_restoration"
						and parent:HasTalent("special_bonus_unique_witch_doctor_2")
					)
				then
					shouldActive = false
				else
