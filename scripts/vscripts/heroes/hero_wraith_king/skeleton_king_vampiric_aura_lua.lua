...
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_lua",
	"heroes/hero_wraith_king/skeleton_king_vampiric_aura_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_lua_effect",
	"heroes/hero_wraith_king/skeleton_king_vampiric_aura_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_lua_summoned",
	"heroes/hero_wraith_king/skeleton_king_vampiric_aura_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skeleton_king_vampiric_aura_lua_reincarnate",
	"heroes/hero_wraith_king/skeleton_king_vampiric_aura_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function skeleton_king_vampiric_aura_lua:OnSpellStart()
	...
	if skeleton_duration > 0 then
		CreateUnitByNameAsync(
			"npc_dota_wraith_king_skeleton_fighter",
			hCaster:GetAbsOrigin() + RandomVector(150),
			true,
			hCaster,
			hCaster,
				...
				for i = #self.fighters, 1, -1 do
					if (not IsValid(self.fighters[i])) or (not self.fighters[i]:IsAlive()) then
						...
						if fighter_count > max_fighter_count then
							self.fighters[i]:RemoveModifierByName(
								"modifier_skeleton_king_vampiric_aura_lua_reincarnate"
							)
...
function modifier_skeleton_king_vampiric_aura_lua_effect:OnDamageCalculated(params)
	...
	if hParent == params.attacker then
		if params.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK then
			...
			hParent:HealWithParams(
				params.damage * self.vampiric_aura * 0.01,
				self:GetAbility(),
				true,
				true,
				hParent,
...
function modifier_skeleton_king_vampiric_aura_lua_summoned:GetUnitLifetimeFraction(params)
	return ((self:GetDieTime() - GameRulesCustom:GetGameTime()) / self:GetDuration())
end
...
function modifier_skeleton_king_vampiric_aura_lua_reincarnate:OnDestroy() end
