...
LinkLuaModifier("modifier_tusk_snowball_lua_ally", "heroes/hero_tusk/tusk_snowball_lua.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
...
-- function tusk_snowball_lua:GetCooldown(iLevel)
-- 	return self.BaseClass.GetCooldown(self, iLevel) - self:GetCaster():GetTalentValue("special_bonus_unique_tusk_6")
-- end
...
if tusk_launch_snowball_lua == nil then
    tusk_launch_snowball_lua = class({})
end
...
if modifier_tusk_snowball_lua == nil then
    modifier_tusk_snowball_lua = class({})
end
...
function modifier_tusk_snowball_lua:OnCreated(params)
    ...
    if IsServer() then
        ...
        if IsValid(self.hTarget) and self:ApplyHorizontalMotionController() then
            ...
            self.hTarget:AddNewModifier(hParent, hAbility, "modifier_tusk_snowball_lua_debuff", nil)

            hParent:AddNoDraw()
            hParent:EmitSound("Hero_Tusk.Snowball.Loop")

            if hAbility ~= nil and not hAbility:IsHidden() then
                local tusk_launch_snowball_lua = hParent:AddAbility("tusk_launch_snowball_lua")
                if tusk_launch_snowball_lua then
                    tusk_launch_snowball_lua:SetLevel(hAbility:GetLevel())
                    hParent:SwapAbilities(hAbility:GetName(), tusk_launch_snowball_lua:GetName(), false, true)
					tusk_launch_snowball_lua:SetHidden(false)
                end
            end

            self.iStartParticleID = ParticleManager:CreateParticle(ParticleManager:GetParticleReplacement("particles/units/heroes/hero_tusk/tusk_snowball_start.vpcf", hParent), PATTACH_CUSTOMORIGIN, nil)
            ParticleManager:SetParticleControl(self.iStartParticleID, 0, hParent:GetAbsOrigin())
            ParticleManager:SetParticleControlEnt(self.iStartParticleID, 1, self.hTarget, PATTACH_CUSTOMORIGIN_FOLLOW, nil, self.hTarget:GetAbsOrigin(), true)
            ParticleManager:SetParticleControl(self.iStartParticleID, 3, Vector(1, 1, self.snowball_radius))

            self.fWindupTime = GameRules:GetGameTime() + self.snowball_windup
            self:StartIntervalThink(0)
        else
            self:Destroy()
        end
    end
    -- AddModifierEvents(MODIFIER_EVENT_ON_ORDER, self)
end
...
function modifier_tusk_snowball_lua:OnDestroy()
    ...
    if IsServer() then
        ...
        if tusk_launch_snowball_lua then
            hParent:SwapAbilities(self:GetAbility():GetName(), tusk_launch_snowball_lua:GetName(), true, false)
...
function modifier_tusk_snowball_lua:Launch()
    ...
    self.iParticleID = ParticleManager:CreateParticle(ParticleManager:GetParticleReplacement("particles/units/heroes/hero_tusk/tusk_snowball.vpcf", hParent), PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(self.iParticleID, 0, hParent:GetAttachmentOrigin(DOTA_PROJECTILE_ATTACHMENT_NONE))
    if IsValid(self.hTarget) then
        ParticleManager:SetParticleControlEnt(self.iParticleID, 1, self.hTarget, PATTACH_CUSTOMORIGIN_FOLLOW, nil, self.hTarget:GetAbsOrigin(), true)
    else
    ...
    ParticleManager:SetParticleControl(self.iParticleID, 3, Vector(self.snowball_radius, self.snowball_radius, self.snowball_radius))

    local tusk_launch_snowball_lua = hParent:FindAbilityByName("tusk_launch_snowball_lua")
    if IsValid(tusk_launch_snowball_lua) then
		local hAbility = self:GetAbility()
        if IsValid(hAbility) then
            hParent:SwapAbilities(hAbility:GetName(), tusk_launch_snowball_lua:GetName(), true, false)
...
function modifier_tusk_snowball_lua:OnIntervalThink()
    if IsServer() then
        if GameRules:GetGameTime() >= self.fWindupTime then
            self:Launch()
        end
...
function modifier_tusk_snowball_lua:UpdateHorizontalMotion(me, dt)
    ...
    if IsServer() then
        ...
        local fDamage = self.snowball_damage-- + hParent:GetTalentValue("special_bonus_unique_tusk_2") + iHero*self.snowball_damage_bonus
        local tTargets = FindUnitsInLine(hParent:GetTeamNumber(), vStart, vEnd, nil, fRadius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, 0)
        for n, hTarget in pairs(tTargets) do
            if TableFindKey(self.tTargets, hTarget) == nil then
                hTarget:AddNewModifier(hParent, self:GetAbility(), "modifier_stunned", { duration = self.stun_duration * hTarget:GetStatusResistanceFactor(hParent) })
                ...
                EmitSoundOnLocationWithCaster(hTarget:GetAbsOrigin(), hTarget:IsHero() and "Hero_Tusk.Snowball.Stun" or "Hero_Tusk.Snowball.Stun.Small", hParent)
            end
        end
        self.tTargets = tTargets

        if fTime >= self.snowball_duration or hParent:IsPositionInRange(self.vPosition, fRadius) then
            EmitSoundOnLocationWithCaster(self.vPosition, "Hero_Tusk.Snowball.ProjectileHit", hParent)
...
function modifier_tusk_snowball_lua_ally:OnCreated(params)
    if IsServer() then
        ...
        if self:ApplyHorizontalMotionController() then
            local iParticleID = ParticleManager:CreateParticle(ParticleManager:GetParticleReplacement("particles/units/heroes/hero_tusk/tusk_snowball_load.vpcf", hCaster), PATTACH_CUSTOMORIGIN, nil)
...
function modifier_tusk_snowball_lua_debuff:GetEffectName()
    return ParticleManager:GetParticleReplacement("particles/units/heroes/hero_tusk/tusk_snowball_target.vpcf", self:GetCaster())
end
