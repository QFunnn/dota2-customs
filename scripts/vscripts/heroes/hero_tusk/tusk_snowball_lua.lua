...
LinkLuaModifier(
	"modifier_tusk_snowball_lua_ally",
	"heroes/hero_tusk/tusk_snowball_lua.lua",
...
function modifier_tusk_snowball_lua:Launch()
	...
	ParticleManager:SetParticleControl(
		self.iParticleID,
		0,
		hParent:GetAttachmentOrigin(DOTA_PROJECTILE_ATTACHMENT_NONE)
	)
	if IsValid(self.hTarget) then
		ParticleManager:SetParticleControlEnt(
			self.iParticleID,
		...
		for n, hTarget in pairs(tTargets) do
			if TableFindKey(self.tTargets, hTarget) == nil then
				...
				EmitSoundOnLocationWithCaster(
					...
					hTarget:IsHero() and "Hero_Tusk.Snowball.Stun" or "Hero_Tusk.Snowball.Stun.Small",
					hParent
				)
			end
		end
...
function modifier_tusk_snowball_lua:OnOrder(params)
	if
		...
		and (params.order_type == DOTA_UNIT_ORDER_MOVE_TO_TARGET or params.order_type == DOTA_UNIT_ORDER_ATTACK_TARGET)
	...
	if
		...
		and params.order_type == DOTA_UNIT_ORDER_MOVE_TO_POSITION
		and self:GetParent():IsPositionInRange(params.new_pos, self.snowball_windup_radius)
		and self.snowball_windup_radius
		and UnitFilter(
				params.unit,
				DOTA_UNIT_TARGET_TEAM_FRIENDLY,
