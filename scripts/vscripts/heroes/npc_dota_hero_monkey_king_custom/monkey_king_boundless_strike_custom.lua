...
LinkLuaModifier(
	"modifier_monkey_king_boundless_strike_custom_damage",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_boundless_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_boundless_strike_custom",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_boundless_strike_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function monkey_king_boundless_strike_custom:GetManaCost(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_12") then
		bonus = self.BaseClass.GetManaCost(self, iLevel)
			/ 100
			* self.modifier_monkey_king_12_mana_cost[self:GetCaster():GetTalentLevel("modifier_monkey_king_12")]
	end
...
function monkey_king_boundless_strike_custom:GetCastPoint()
	local bonus = 0
	if self:GetCaster():HasModifier("modifier_monkey_king_12") then
		bonus = self.BaseClass.GetCastPoint(self)
			/ 100
			* self.modifier_monkey_king_12_cast_point[self:GetCaster():GetTalentLevel("modifier_monkey_king_12")]
	end
...
function monkey_king_boundless_strike_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_monkey_king.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_monkey_king.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_monkey_king.vpcf",
		context
	)
...
function monkey_king_boundless_strike_custom:CastStrike(start_point, end_point)
	...
	vTargetPosition =
		GetGroundPosition(vStartPosition + vDirection:Normalized() * (strike_cast_range - strike_radius / 2), caster)
	...
	local modifier_monkey_king_boundless_strike_custom_damage =
		caster:AddNewModifier(caster, self, "modifier_monkey_king_boundless_strike_custom_damage", {})
	...
	for _, enemy in pairs(enemies) do
		...
		if
			self:GetCaster():HasModifier("modifier_monkey_king_20")
			and not self:GetCaster():HasModifier("modifier_monkey_king_2")
		then
	...
	local modifier_monkey_king_jingu_mastery_custom_buff =
		caster:FindModifierByName("modifier_monkey_king_jingu_mastery_custom_buff")
	if
		modifier_monkey_king_jingu_mastery_custom_buff
		and modifier_monkey_king_jingu_mastery_custom_buff:GetElapsedTime() > 0.1
	then
...
function modifier_monkey_king_boundless_strike_custom_damage:IsHidden()
	return true
end
function modifier_monkey_king_boundless_strike_custom_damage:IsPurgable()
	return false
end
...
function modifier_monkey_king_boundless_strike_custom_damage:OnCreated(table)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_9") then
		self.strike_crit_mult = self.strike_crit_mult
			+ self:GetAbility().modifier_monkey_king_9[self:GetCaster():GetTalentLevel("modifier_monkey_king_9")]
	end
end
...
function modifier_monkey_king_boundless_strike_custom:IsHidden()
	return true
end
function modifier_monkey_king_boundless_strike_custom:IsPurgable()
	return false
end
function modifier_monkey_king_boundless_strike_custom:IsPurgeException()
	return false
end
function modifier_monkey_king_boundless_strike_custom:RemoveOnDeath()
	return false
end
function modifier_monkey_king_boundless_strike_custom:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_monkey_king_10") then
		return
	end
	...
	if new_cooldown > 0 then
		...
		new_cooldown = new_cooldown
			- self:GetAbility().modifier_monkey_king_10[self:GetParent():GetTalentLevel("modifier_monkey_king_10")]
		if new_cooldown > 0 then
