...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_monkey_king_tree_dance_custom_jump",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_monkey_king_tree_dance_custom_cooldown",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_BOTH
)
...
function monkey_king_tree_dance_custom:OnAbilityPhaseStart()
	...
	local modifier_monkey_king_tree_dance_custom = self:GetCaster()
		:FindModifierByName("modifier_monkey_king_tree_dance_custom")
	if modifier_monkey_king_tree_dance_custom and modifier_monkey_king_tree_dance_custom.tree == target then
		...
		if player then
			CustomGameEventManager:Send_ServerToPlayer(
				...
				{ message = "#dota_hud_error_treejump_tree_target_same_tree" }
			)
		end
		return false
	end
...
function monkey_king_tree_dance_custom:OnSpellStart()
	...
	local modifier = caster:AddNewModifier(
		...
		"modifier_monkey_king_tree_dance_custom_jump",
		{
			...
			start_offset = perched_spot_height * perch,
...
function modifier_monkey_king_tree_dance_custom_cooldown:IsHidden()
	return true
end
function modifier_monkey_king_tree_dance_custom_cooldown:IsPurgable()
	return false
end
function modifier_monkey_king_tree_dance_custom_cooldown:RemoveOnDeath()
	return false
end
...
function modifier_monkey_king_tree_dance_custom_cooldown:OnTakeDamage(params)
	...
	if params.unit:HasModifier("modifier_monkey_king_tree_dance_custom") then
		return
	end
...
function modifier_monkey_king_tree_dance_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_19") then
		self.unperched_stunned_duration = self.unperched_stunned_duration
			+ self:GetAbility().modifier_monkey_king_19[self:GetCaster():GetTalentLevel("modifier_monkey_king_19")]
	end
	...
	then
		...
		local modifier = self.parent:AddNewModifier(
			...
			"modifier_monkey_king_tree_dance_custom_jump",
			{
				...
				speed = 350,
				height = 1,
				start_offset = self.perched_spot_height,
				fix_end = false,
				isForward = true,
			}
		)
...
function modifier_monkey_king_tree_dance_custom:OnIntervalThink()
	...
	local mod = self.parent:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_stunned",
		{ duration = self.unperched_stunned_duration }
	)
	self.unperched = true
	self:Destroy()
end
