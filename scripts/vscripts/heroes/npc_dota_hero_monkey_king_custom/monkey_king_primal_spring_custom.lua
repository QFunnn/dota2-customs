...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_primal_spring_custom",
...
function monkey_king_primal_spring_custom:CanBeCast()
	...
	if caster:HasModifier("modifier_monkey_king_17") and caster:HasModifier("modifier_monkey_king_mischief_custom") then
		return true
	end
	if caster:HasModifier("modifier_monkey_king_mischief_custom") then
		return false
	end
...
function monkey_king_primal_spring_custom:OnSpellStart()
	...
	if self.sub and not self.sub:IsNull() then
		if caster:HasModifier("modifier_monkey_king_2") then
			return
		end
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_15") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_monkey_king_15[self:GetCaster():GetTalentLevel("modifier_monkey_king_15")]
			)
	end
...
function monkey_king_primal_spring_custom:PlayEffects1()
	local caster = self:GetCaster()
	self.effect_cast1 = ParticleManager:CreateParticle(
...
function monkey_king_primal_spring_early_custom:OnSpellStart()
	if not self.main or self.main:IsNull() then
		return
	end
