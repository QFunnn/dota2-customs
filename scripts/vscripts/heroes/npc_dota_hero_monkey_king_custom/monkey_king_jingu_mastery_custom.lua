...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_jingu_mastery_custom",
...
LinkLuaModifier(
	"modifier_monkey_king_jingu_mastery_custom",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_jingu_mastery_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_monkey_king_jingu_mastery_custom:IsHidden()
	return true
end
function modifier_monkey_king_jingu_mastery_custom:IsPurgable()
	return false
end
function modifier_monkey_king_jingu_mastery_custom:IsPurgeException()
	return false
end
function modifier_monkey_king_jingu_mastery_custom:RemoveOnDeath()
	return false
end
...
function modifier_monkey_king_jingu_mastery_custom:OnAttackLanded(params)
	...
	local modifier_monkey_king_wukongs_command_custom_soldier_active =
		self.parent:FindModifierByName("modifier_monkey_king_wukongs_command_custom_soldier_active")
	if modifier_monkey_king_wukongs_command_custom_soldier_active then
		if modifier_monkey_king_wukongs_command_custom_soldier_active.ultimate == 1 then
			return
		end
	...
	local modifier_monkey_king_jingu_mastery_custom_buff =
		self.parent:FindModifierByName("modifier_monkey_king_jingu_mastery_custom_buff")
...
function modifier_monkey_king_jingu_mastery_custom:OnTakeDamage(params)
	...
	if
		(
			params.inflictor == nil
			or (params.inflictor and params.inflictor:GetName() == "windrunner_focusfire_whirlwind")
		)
...
function modifier_monkey_king_jingu_mastery_custom_hit:OnRefresh(table)
	...
	if self:GetStackCount() >= self.max_stack then
		self.caster:AddNewModifier(
			...
			self.ability,
			"modifier_monkey_king_jingu_mastery_custom_buff",
			{ duration = self.duration_buff, new_attacks = self.new_attacks }
		)
		self:Destroy()
	end
end
...
function modifier_monkey_king_jingu_mastery_custom_buff:IsHidden()
	return false
end
