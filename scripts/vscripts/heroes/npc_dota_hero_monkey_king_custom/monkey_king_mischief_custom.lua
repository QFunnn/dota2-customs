...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_mischief_custom",
...
LinkLuaModifier(
	"modifier_monkey_king_17_barrier",
	"modifiers/talents/npc_dota_hero_monkey_king/modifier_monkey_king_17",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_monkey_king_mischief_custom:IsPurgeException()
	return false
end
...
function modifier_monkey_king_mischief_custom:OnIntervalThink()
	...
	if
		...
		and not self:GetParent():HasModifier("modifier_monkey_king_transform_courier")
		and not self:GetParent():HasModifier("modifier_monkey_king_transform_runes")
	then
		self:Destroy()
	end
end
...
function modifier_monkey_king_mischief_custom:OnDestroy()
	...
	if
		self:GetCaster():HasModifier("modifier_monkey_king_17")
		and not self:GetCaster():HasModifier("modifier_monkey_king_tree_dance_custom")
	then
		local monkey_king_primal_spring_custom = self:GetCaster():FindAbilityByName("monkey_king_primal_spring_custom")
		if monkey_king_primal_spring_custom then
...
function modifier_monkey_king_transform_courier:IsPurgable()
	return false
end
function modifier_monkey_king_transform_courier:IsPurgeException()
	return false
end
function modifier_monkey_king_transform_courier:OnCreated()
	...
	local particle_fx = ParticleManager:CreateParticle(
...
function modifier_monkey_king_transform_courier:GetAbsoluteNoDamagePhysical()
	if self:GetElapsedTime() > self.invul_duration then
		return
	end
	return 1
end
...
function modifier_monkey_king_transform_runes:IsPurgable()
	return false
end
function modifier_monkey_king_transform_runes:IsPurgeException()
	return false
end
...
function modifier_monkey_king_transform_runes:OnDestroy()
	...
	local modifier_duration =
		self:GetAbility().modifier_monkey_king_13_duration[self:GetCaster():GetTalentLevel("modifier_monkey_king_13")]
	if modifier_name == "modifier_rune_illusion" then
		self:GetParent():Purge(false, true, false, false, false)
		CreateIllusions(
			self:GetParent(),
			self:GetParent(),
			{ duration = modifier_duration, outgoing_damage = -65, incoming_damage = 100 },
			2,
			75,
			true,
			true
		...
		if modifier_handle and modifier_handle:GetRemainingTime() > modifier_duration then
			return
		end
