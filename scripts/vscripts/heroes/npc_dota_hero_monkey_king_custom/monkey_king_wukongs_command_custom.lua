...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_wukongs_command_custom_thinker",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_monkey_king_wukongs_command_custom_effect",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_wukongs_command_custom_scepter",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_wukongs_command_custom_cooldown",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_monkey_king_wukongs_command_custom_nodraw",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_transfiguration_custom",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_wukongs_command_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function monkey_king_wukongs_command_custom:GetIntrinsicModifierName()
	if not self:GetCaster():IsRealHero() or self:GetCaster():IsTempestDouble() then
		return
	end
...
function monkey_king_wukongs_command_custom:OnSpellStart()
	...
	position = GetGroundPosition(
		caster:GetAbsOrigin() + vDirection:Normalized() * math.min(vDirection:Length2D(), cast_range),
		caster
	)
	...
	self.thinker = CreateModifierThinker(
		...
		self,
		"modifier_monkey_king_wukongs_command_custom_thinker",
		{ duration = duration + 0.3, radius = max_radius },
		position,
	...
	for i = 1, num_second_soldiers, 1 do
		Timers:CreateTimer((i - 1) * interval, function()
			if caster:HasModifier("modifier_monkey_king_wukongs_command_custom_buff") then
				...
				local vTargetPosition = GetGroundPosition(
					position
						+ second_radius * Rotation2D(Vector(0, 1, 0), math.rad((i - 0.25) * 360 / num_second_soldiers)),
					soldier
				)
				...
				soldier:AddNewModifier(
					...
					self,
					"modifier_monkey_king_wukongs_command_custom_soldier_active",
					{ position = position, radius = max_radius, target_position = vTargetPosition, ultimate = true }
				)
			end
		end)
	end
		...
		for i = 1, num_first_soldiers, 1 do
			Timers:CreateTimer((i - 1) * interval, function()
				if caster:HasModifier("modifier_monkey_king_wukongs_command_custom_buff") then
					...
					local vTargetPosition = GetGroundPosition(
						position
							+ first_radius * Rotation2D(Vector(0, 1, 0), math.rad((i - 0.5) * 360 / num_first_soldiers)),
						soldier
					)
					soldier:RemoveModifierByName("modifier_monkey_king_wukongs_command_custom_soldier_active")
					soldier:AddNewModifier(
...
function monkey_king_transfiguration_custom:OnSpellStart()
	...
	if near_soldier == nil then
		...
		if player then
			CustomGameEventManager:Send_ServerToPlayer(
				...
				{ message = "#dota_hud_error_no_target" }
			)
		end
		self:EndCooldown()
		self:RefundManaCost()
	...
	local monkey_target = monkey_king_wukongs_command_custom:SpawnMonkeyKingPointScepter(
		second_point,
		transfiguration_duration + 0.1,
		true,
		true
	)
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_monkey_king_transfiguration_custom",
		{ duration = transfiguration_duration }
	)
...
function monkey_king_wukongs_command_custom:CreateSoldier(count)
	...
	if #self.soldiers >= self.soldiers_max then
		return
	end
	local caster = self:GetCaster()
...
function monkey_king_wukongs_command_custom:SpawnMonkeyKingPointScepter(point, duration, teleport, no_particle)
	...
	local monkey_king_jingu_mastery_custom_caster = self:GetCaster()
		:FindAbilityByName("monkey_king_jingu_mastery_custom")
...
function monkey_king_wukongs_command_custom:CreateSoldiers()
	if self:GetCaster():HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then
		return
	end
	local count = 1
	if self.soldiers == nil then
...
function modifier_monkey_king_wukongs_command_custom_cooldown:IsPurgable()
	return false
end
function modifier_monkey_king_wukongs_command_custom_cooldown:RemoveOnDeath()
	return false
end
function modifier_monkey_king_wukongs_command_custom_cooldown:IsDebuff()
	return true
end

modifier_monkey_king_wukongs_command_custom_scepter = class({})
function modifier_monkey_king_wukongs_command_custom_scepter:IsHidden()
	return true
end
function modifier_monkey_king_wukongs_command_custom_scepter:IsPurgable()
	return false
end
...
function modifier_monkey_king_wukongs_command_custom_scepter:OnIntervalThink()
	...
	if not self.parent:HasModifier("modifier_monkey_king_3") then
		return
	end
	...
	if self.parent:HasModifier("modifier_monkey_king_tree_dance_custom") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_tree_dance_custom_jump") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_transform") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_transform_courier") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_transform_runes") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_mischief_custom") then
		return
	end
	if self.parent:HasModifier("modifier_monkey_king_wukongs_command_custom_cooldown") then
		return
	end
	if self.parent:HasModifier("modifier_wodawisp") then
		return
	end
	if self.parent:HasModifier("modifier_wodarelax") then
		return
	end
	local duration = self:GetAbility().modifier_monkey_king_3_duration[self:GetCaster()
		:GetTalentLevel("modifier_monkey_king_3")]
	duration = duration
		+ (
			math.floor(
				self:GetCaster():GetStrength() / self:GetAbility().modifier_monkey_king_3_bonus_duration_strength
			) * self:GetAbility().modifier_monkey_king_3_bonus_duration
		)
	self:GetAbility()
		:SpawnMonkeyKingPointScepter(self.parent:GetAbsOrigin() + RandomVector(RandomInt(100, 300)), duration)
	self.parent:AddNewModifier(
		self.parent,
		self.ability,
		"modifier_monkey_king_wukongs_command_custom_cooldown",
		{ duration = self:GetAbility().modifier_monkey_king_3_interval }
	)
end
...
function modifier_monkey_king_wukongs_command_custom_soldier_active:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_5") then
		self.attack_speed = self.attack_speed
			+ self:GetAbility().modifier_monkey_king_5[self:GetCaster():GetTalentLevel("modifier_monkey_king_5")]
	end
	self.move_speed = 700
	if not IsServer() then
		return
	...
	for i = 0, 17 do
		...
		if (i <= 5 or i >= 16) and item then
			...
			if
				new_item and (new_item:GetName() == "item_armlet" or new_item:GetName() == "item_spear_of_mordiggian")
			then
				if
					self.caster:HasModifier("modifier_item_armlet_unholy_strength")
					and new_item:GetName() == "item_armlet"
				then
					self.parent:AddNewModifier(self.parent, new_item, "modifier_item_armlet_unholy_strength", {})
				end
				if
					self.caster:HasModifier("modifier_item_spear_of_mordiggian_active")
					and new_item:GetName() == "item_spear_of_mordiggian"
				then
...
function modifier_monkey_king_wukongs_command_custom_soldier_active:CheckTarget()
	...
	if
		not self.attack_target
		or self.attack_target:IsNull()
		or not self.attack_target:IsAlive()
		or ((self.attack_target:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() > (35 + self.search_radius))
		or not self.attack_target:IsPositionInRange(self.position, self.radius)
	then
...
function modifier_monkey_king_wukongs_command_custom_inactive:IsHidden()
	return true
end
function modifier_monkey_king_wukongs_command_custom_inactive:IsPurgable()
	return false
end
...
function modifier_monkey_king_wukongs_command_custom_nodraw:IsHidden()
	return false
end
function modifier_monkey_king_wukongs_command_custom_nodraw:IsPurgable()
	return false
end
...
function modifier_monkey_king_wukongs_command_custom_thinker:OnCreated(params)
	...
	if not self.modifier_monkey_king_7 then
		self:GetCaster()
			:SwapAbilities("monkey_king_wukongs_command_custom", "monkey_king_transfiguration_custom", false, true)
		local monkey_king_transfiguration_custom = self:GetCaster()
			:FindAbilityByName("monkey_king_transfiguration_custom")
...
function modifier_monkey_king_wukongs_command_custom_thinker:OnDestroy()
	...
	if not self.modifier_monkey_king_7 then
		self:GetCaster()
			:SwapAbilities("monkey_king_transfiguration_custom", "monkey_king_wukongs_command_custom", false, true)
	end
...
function modifier_monkey_king_wukongs_command_custom_soldier:GetActivityTranslationModifiers()
	if self.parent:HasModifier("modifier_monkey_king_mischief_anim") then
		return
	end
...
function modifier_monkey_king_wukongs_command_custom_rapier:IsHidden()
	return true
end
function modifier_monkey_king_wukongs_command_custom_rapier:IsPurgable()
	return false
end
...
function modifier_monkey_king_transfiguration_custom:IsHidden()
	return false
end
function modifier_monkey_king_transfiguration_custom:IsPurgable()
	return false
end
...
function string.gsplit(str)
	...
	if string.len(str) ~= 0 then
		for i = 1, string.len(str) do
			...
			if
				(string.byte(new_str) >= 48 and string.byte(new_str) <= 57)
				or (string.byte(new_str) >= 65 and string.byte(new_str) <= 90)
				or (string.byte(new_str) >= 97 and string.byte(new_str) <= 122)
			then
