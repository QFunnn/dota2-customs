...
LinkLuaModifier(
	"modifier_monkey_king_jingu_magic_custom_hit",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_jingu_magic_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_jingu_magic_custom_buff",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_jingu_magic_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_jingu_magic_custom",
	"heroes/npc_dota_hero_monkey_king_custom/monkey_king_jingu_magic_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function monkey_king_jingu_magic_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_stack_magical.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_overhead_magical.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_start_magical.vpcf",
		context
	)
...
function modifier_monkey_king_jingu_magic_custom:IsHidden()
	return true
end
function modifier_monkey_king_jingu_magic_custom:IsPurgable()
	return false
end
function modifier_monkey_king_jingu_magic_custom:IsPurgeException()
	return false
end
function modifier_monkey_king_jingu_magic_custom:RemoveOnDeath()
	return false
end
...
function modifier_monkey_king_jingu_magic_custom:OnAbilityFullyCast(params)
	...
	if self.exceptions[params.ability:GetAbilityName()] then
		return
	end
	local modifier_monkey_king_jingu_magic_custom_buff =
		self.parent:FindModifierByName("modifier_monkey_king_jingu_magic_custom_buff")
	if not modifier_monkey_king_jingu_magic_custom_buff then
		self:GetParent()
			:AddNewModifier(self:GetParent(), self.ability, "modifier_monkey_king_jingu_magic_custom_hit", {})
	end
end
...
function modifier_monkey_king_jingu_magic_custom:GetModifierProperty_MagicalLifesteal(params)
	...
	if
		modifier_monkey_king_jingu_magic_custom_buff
		and modifier_monkey_king_jingu_magic_custom_buff:GetElapsedTime() > 0.1
	then
...
function modifier_monkey_king_jingu_magic_custom_hit:IsPurgable()
	return false
end
...
function modifier_monkey_king_jingu_magic_custom_buff:IsHidden()
	return false
end
...
function modifier_monkey_king_jingu_magic_custom_buff:OnCreated(params)
	...
	if self:GetCaster():HasModifier("modifier_monkey_king_2") then
		self.spell_amplify = self.spell_amplify
			+ self:GetAbility().modifier_monkey_king_2_damage[self:GetCaster():GetTalentLevel("modifier_monkey_king_2")]
		self.spell_lifesteal = self.spell_lifesteal
			+ self:GetAbility().modifier_monkey_king_2_lifesteal[self:GetCaster()
				:GetTalentLevel("modifier_monkey_king_2")]
	end
	if not IsServer() then
		return
	end
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_monkey_king/monkey_king_tap_buff_magical.vpcf",
		PATTACH_ABSORIGIN,
		self.parent
	)
...
function modifier_monkey_king_jingu_magic_custom_buff:OnIntervalThink()
	...
	if
		...
		or not self.caster:HasModifier(self.ability:GetIntrinsicModifierName())
	then
		self:Destroy()
	end
end
