...
function zuus_arc_lightning_lua:Jump(hCaster, hTarget, radius, jump_count, jump_delay, damage, damage_health_pct, damage_pct, tUnits)
	self:GameTimer(jump_delay, function()
		if IsValid(hCaster) and IsValid(hTarget) then
			...
			if IsValid(hNewTarget) then
				EmitSoundOnLocationWithCaster(hNewTarget:GetAbsOrigin(), "Hero_Zuus.ArcLightning.hTarget", hCaster)
				...
				if #tUnits < jump_count then
					self:Jump(hCaster, hNewTarget, radius, jump_count, jump_delay, damage, damage_health_pct, damage_pct, tUnits)
				end
			end
		end
	end)
...
function zuus_arc_lightning_lua:ArcLightning(hTarget, bFirebyAttack)
	...
	if bFirebyAttack and IsValid(hHand) then
...
function zuus_arc_lightning_lua:OnSpellStart()
	...
	if IsValid(hTarget) and not hTarget:TriggerSpellAbsorb(self) then
