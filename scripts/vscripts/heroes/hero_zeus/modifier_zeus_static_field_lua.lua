...
function modifier_zeus_static_field_lua:IsHidden()
	return true
end
function modifier_zeus_static_field_lua:IsPurgable()
	return false
end
function modifier_zeus_static_field_lua:AllowIllusionDuplicate()
	return false
end
...
function modifier_zeus_static_field_lua:SetStats()
	...
	self.damage_health_pct = self.ability:GetSpecialValueFor("damage_health_pct")
		+ self.parent:FindTalentValue("special_bonus_unique_zeus")
...
function modifier_zeus_static_field_lua:OnSpentMana(params)
	...
	if hAbility:IsPassive() then
		return
	end
	if hAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_PASSIVE) or hAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_TOGGLE) then
		return
	end
...
function modifier_zeus_static_field_lua:Shock(target, is_zeus_ability)
	if not is_zeus_ability and target and target:IsAlive() then
		target:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_zeus_static_field_shock_lua",
			{ duration = self.shock_duration }
		)
	end

	self.damage_table.victim = target
	self.damage_table.damage = target:GetHealth() * self.damage_health_pct
	if not is_zeus_ability then
		self.damage_table.damage = self.damage_table.damage * self.nonzeus_multiplier
	end
	...
	local particle_cast =
		ParticleManager:GetParticleReplacement("particles/units/heroes/hero_zuus/zuus_static_field.vpcf", self.parent)
