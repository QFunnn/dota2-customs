...
function modifier_hero_refreshing:OnDestroy(kv)
	if IsServer() then
		...
		do
			if
				unit:GetOwner() == self:GetCaster()
				and (
					unit:GetUnitName() == "npc_dota_lone_druid_bear1"
					or unit:GetUnitName() == "npc_dota_lone_druid_bear2"
					or unit:GetUnitName() == "npc_dota_lone_druid_bear3"
					or unit:GetUnitName() == "npc_dota_lone_druid_bear4"
				)
			then
				local modifier_heal = unit:FindModifierByName("modifier_hero_refreshing")
				if modifier_heal then
					modifier_heal:Destroy()
				end
			end
		end
	...
	if IsServer() then
		...
		if not IsCheatsEnabled() then
			for _, modifier in pairs(self:GetParent():FindAllModifiers()) do
				if
					modifier
					and modifier:GetCaster() ~= self:GetParent()
					and modifier:IsDebuff()
					and save_modifier[modifier:GetName()] == nil
				then
					local ability_name = ""
		...
		if self:GetParent():IsRealHero() then
			...
			do
				if
					unit:GetOwner() == self:GetCaster()
					and (unit:GetUnitName() == "npc_dota_lone_druid_bear1" or unit:GetUnitName() == "npc_dota_lone_druid_bear2" or unit:GetUnitName() == "npc_dota_lone_druid_bear3" or unit:GetUnitName() == "npc_dota_lone_druid_bear4")
					and not unit:HasModifier("modifier_hero_refreshing")
				then
			...
			if self:GetParent().tempest_double_hClone then
				self:GetParent().tempest_double_hClone
					:AddNewModifier(self:GetParent(), nil, "modifier_hero_refreshing", {})
			end
		end
		...
		if self:GetParent() and self:GetParent():IsRealHero() then
			for i = 0, 32 do
				...
				if
					hItem
					and not hItem:IsNull()
					and hItem.GetAbilityName
					and "item_bottle" == hItem:GetAbilityName()
				then
...
function modifier_hero_refreshing:GetModifierCastRangeBonusStacking(params)
	if params.ability then
		if params.ability.GetCastRange then
			local new = params.ability:GetCastRange(
				params.ability:GetCaster():GetAbsOrigin(),
				params.ability:GetCaster()
			) + self:GetParent():GetCastRangeBonus()
			if new > 0 then
				return (new * 0.5) * -1
			end
		end
