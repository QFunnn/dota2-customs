...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_pangolier_custom/pangolier_rollup_custom",
...
function pangolier_rollup_custom:GetBehavior()
	if
		...
		and self:GetCaster():HasModifier("modifier_pangolier_17")
	then
...
function modifier_pangolier_rollup_custom:IsPurgable()
	return false
end
function modifier_pangolier_rollup_custom:IsPurgeException()
	return false
end
...
function modifier_pangolier_rollup_custom:OnAttack(params)
	...
	if self:GetParent():HasModifier("modifier_pangolier_gyroshell_custom_shard_damage_cd") then
		return
	end
	self:GetParent():AddNewModifier(
		self:GetParent(),
		self:GetAbility(),
		"modifier_pangolier_gyroshell_custom_shard_damage_cd",
		{ duration = self:GetAbility():GetSpecialValueFor("damage_cd") }
	)
