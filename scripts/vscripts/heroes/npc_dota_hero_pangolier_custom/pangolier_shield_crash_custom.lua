...
LinkLuaModifier(
	"modifier_pangolier_shield_crash_custom_buff",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_shield_crash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_shield_crash_custom_magic_immune",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_shield_crash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_shield_crash_custom_double_jump",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_shield_crash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function pangolier_shield_crash_custom:GetCooldown(iLevel)
	...
	if
		self:GetCaster():HasModifier("modifier_pangolier_gyroshell_custom")
		or self:GetCaster():HasModifier("modifier_pangolier_rollup_custom")
	then
...
function pangolier_shield_crash_custom:OnSpellStart(new_ability, new_duration)
	...
	if self:GetCaster():HasModifier("modifier_pangolier_4") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_pangolier_shield_crash_custom_magic_immune",
			{ duration = self.modifier_pangolier_4[self:GetCaster():GetTalentLevel("modifier_pangolier_4")] }
		)
	end
	...
	if self:GetCaster():HasModifier("modifier_pangolier_16") then
		if not self:GetCaster():HasModifier("modifier_pangolier_shield_crash_custom_double_jump") then
			self:EndCooldown()
			self:GetCaster()
				:AddNewModifier(self:GetCaster(), self, "modifier_pangolier_shield_crash_custom_double_jump", {})
		else
	...
	if is_double_jumping then
		...
		if self:GetCaster():HasModifier("modifier_pangolier_16") then
			damage = damage
				+ (damage / 100 * self.modifier_pangolier_16[self:GetCaster():GetTalentLevel("modifier_pangolier_16")])
		end
	end
			...
			for _, move in pairs(moves) do
				self:GetCaster():AddNewModifier(
					...
					"modifier_pangolier_swashbuckle_custom_attacks",
					{
						...
						dir_y = move.y,
						duration = 3,
						new_strikes = self.modifier_pangolier_13[self:GetCaster()
							:GetTalentLevel("modifier_pangolier_13")],
					}
				)
			end
		end
	...
	arc:SetEndCallback(function(is_interrupt)
		...
		if not caster:HasModifier("modifier_pangolier_gyroshell_custom") and not new_ability then
			...
			caster:AddNewModifier(
				...
				{ duration = ulti:GetSpecialValueFor("jump_recover_time") }
			)
		end
		caster:FadeGesture(ACT_DOTA_RUN)
		if not is_interrupt then
			...
			local enemies_hero = FindUnitsInRadius(
				self:GetCaster():GetTeamNumber(),
				self:GetCaster():GetAbsOrigin(),
				nil,
				radius,
			...
			if #enemies_hero > 0 then
				caster:AddNewModifier(
					...
					self,
					"modifier_pangolier_shield_crash_custom_buff",
					{ duration = buff_duration, enemies = #enemies_hero, is_double_jumping = is_double_jumping }
				)
			end
			...
			if not is_interrupt then
				Timers:CreateTimer(FrameTime(), function()
					...
					self:GetCaster():AddNewModifier(
						...
						{ duration = new_duration, original = 1 }
					)
				end)
			end
			--caster:StartGesture(ACT_DOTA_RUN)
...
function modifier_pangolier_shield_crash_custom_buff:IsHidden()
	return false
end
...
function modifier_pangolier_shield_crash_custom_buff:OnCreated(table)
	...
	if table.is_double_jumping and table.is_double_jumping == 1 then
		self.hero_shield = self.hero_shield
			+ (
				self.hero_shield
				/ 100
				* self.ability.modifier_pangolier_16[self:GetCaster():GetTalentLevel("modifier_pangolier_16")]
			)
	end
	if not IsServer() then
		return
...
function modifier_pangolier_shield_crash_custom_magic_immune:IsPurgable()
	return false
end
function modifier_pangolier_shield_crash_custom_magic_immune:GetTexture()
	return "pangolier_4"
end
...
function modifier_pangolier_shield_crash_custom_double_jump:IsHidden()
	return true
end
function modifier_pangolier_shield_crash_custom_double_jump:IsPurgable()
	return false
end
function modifier_pangolier_shield_crash_custom_double_jump:IsPurgeException()
	return false
end
