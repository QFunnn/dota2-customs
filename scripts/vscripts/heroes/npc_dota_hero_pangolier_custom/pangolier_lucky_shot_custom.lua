...
LinkLuaModifier(
	"modifier_pangolier_lucky_shot_custom",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_lucky_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_lucky_shot_custom_debuff",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_lucky_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pangolier_lucky_shot_custom_buff",
	"heroes/npc_dota_hero_pangolier_custom/pangolier_lucky_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_pangolier_lucky_shot_custom:IsPurgable()
	return false
end
function modifier_pangolier_lucky_shot_custom:IsHidden()
	return true
end
function modifier_pangolier_lucky_shot_custom:IsPurgeException()
	return false
end
function modifier_pangolier_lucky_shot_custom:RemoveOnDeath()
	return false
end
...
function modifier_pangolier_lucky_shot_custom:OnAttackStart(params)
	...
	if not RollPseudoRandomPercentage(chance, 450, self.caster) then
		return
	end
	self.proc = true
...
function modifier_pangolier_lucky_shot_custom_debuff:IsHidden()
	return false
end
function modifier_pangolier_lucky_shot_custom_debuff:IsPurgable()
	return true
end
function modifier_pangolier_lucky_shot_custom_debuff:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_pangolier_6") then
		self.armor = self.armor
			+ self:GetAbility().modifier_pangolier_6_armor[self:GetCaster():GetTalentLevel("modifier_pangolier_6")]
		self.attack_slow = self.attack_slow
			+ self:GetAbility().modifier_pangolier_6_slow[self:GetCaster():GetTalentLevel("modifier_pangolier_6")]
	end
	if not IsServer() then
		return
	end
...
function modifier_pangolier_lucky_shot_custom_buff:IsPurgable()
	return false
end
function modifier_pangolier_lucky_shot_custom_buff:GetTexture()
	return "pangolier_2"
end
...
function modifier_pangolier_lucky_shot_custom_buff:OnAttackLanded(params)
	...
	local enemies_to_cleave = self:FindUnitsInCone(
		...
		self:GetAbility().modifier_pangolier_2_radius,
		self:GetAbility().modifier_pangolier_2_radius,
		self:GetAbility().modifier_pangolier_2_radius,
		nil,
	...
	for _, enemy in pairs(enemies_to_cleave) do
		local damage = params.original_damage
			/ 100
			* self:GetAbility().modifier_pangolier_2_cleave[self:GetCaster():GetTalentLevel("modifier_pangolier_2")]
		ApplyDamage({
			attacker = self:GetCaster(),
			victim = enemy,
			ability = self:GetAbility(),
...
function modifier_pangolier_lucky_shot_custom_buff:FindUnitsInCone(
	teamNumber,
	vDirection,
	vPosition,
	startRadius,
