...
function modifier_leshrac_diabolic_edict_lua:OnRefresh(kv) end

function modifier_leshrac_diabolic_edict_lua:OnRemoved() end
...
function modifier_leshrac_diabolic_edict_lua:PlayEffects(unit)
	...
	if unit then
		...
		ParticleManager:SetParticleControl(
			effect_cast,
			1,
			self.parent:GetOrigin() + RandomVector(RandomInt(0, self.radius))
		)
	end
