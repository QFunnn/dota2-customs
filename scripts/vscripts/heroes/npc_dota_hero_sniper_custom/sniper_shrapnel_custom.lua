...
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom",
	"heroes/npc_dota_hero_sniper_custom/sniper_shrapnel_custom",
...
function modifier_sniper_shrapnel_custom_thinker:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_sniper_18") then
		self.delay = self.delay
			+ self:GetAbility().modifier_sniper_18_delay[self:GetCaster():GetTalentLevel("modifier_sniper_18")]
	end
	...
	if self:GetCaster():HasModifier("modifier_sniper_18") then
		self.duration = self.duration
			+ self:GetAbility().modifier_sniper_18_cooldown[self:GetCaster():GetTalentLevel("modifier_sniper_18")]
	end
	self.interval = 1
	self.start = false
	if not IsServer() then
