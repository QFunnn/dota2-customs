...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_sniper_custom/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_handler",
	"heroes/npc_dota_hero_sniper_custom/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_stack_debuff",
	"heroes/npc_dota_hero_sniper_custom/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sniper_assassinate_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_sniper.vpcf",
		context
	)
end
...
function sniper_assassinate_custom:OnAbilityPhaseInterrupted()
	...
	self.target = nil
	if not modifier_sniper_assassinate_custom then
		return
	end
...
function sniper_assassinate_custom:OnProjectileHit_ExtraData(hTarget, vLocation, data)
	...
	local modifier_wisp_relocate_custom_cast_delay =
		target:FindModifierByName("modifier_wisp_relocate_custom_cast_delay")
	...
	local modifier_sniper_assassinate_custom_handler = self:GetCaster()
		:FindModifierByName("modifier_sniper_assassinate_custom_handler")
...
function modifier_sniper_assassinate_custom:GetAttributes()
...
function modifier_sniper_assassinate_custom_handler:IsHidden()
	return not self:GetCaster():HasModifier("modifier_sniper_11")
end
function modifier_sniper_assassinate_custom_handler:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_handler:IsPurgeException()
	return false
end
function modifier_sniper_assassinate_custom_handler:RemoveOnDeath()
	return false
end
function modifier_sniper_assassinate_custom_handler:GetTexture()
	return "sniper_11"
end
...
function modifier_sniper_assassinate_custom_handler:GetModifierAttackRangeBonus()
	...
	if self:GetCaster():HasModifier("modifier_sniper_11") then
		stacks = (
			self:GetAbility().modifier_sniper_11_attack_range[self:GetCaster():GetTalentLevel("modifier_sniper_11")]
			* self:GetStackCount()
		)
	end
	return br + stacks
...
function modifier_sniper_assassinate_custom_handler:GetModifierBonusStats_Agility()
	if self:GetCaster():HasModifier("modifier_sniper_11") then
		return self:GetAbility().modifier_sniper_11_agility[self:GetCaster():GetTalentLevel("modifier_sniper_11")]
			* self:GetStackCount()
	end
end
...
function modifier_sniper_assassinate_custom_stack_debuff:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_stack_debuff:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_stack_debuff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
