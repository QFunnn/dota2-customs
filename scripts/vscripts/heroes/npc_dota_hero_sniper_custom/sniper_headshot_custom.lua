...
LinkLuaModifier(
	"modifier_sniper_headshot_custom",
	"heroes/npc_dota_hero_sniper_custom/sniper_headshot_custom",
...
LinkLuaModifier(
	"modifier_sniper_headshot_custom_disarmed",
	"heroes/npc_dota_hero_sniper_custom/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_headshot_custom_active_attack",
	"heroes/npc_dota_hero_sniper_custom/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sniper_headshot_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_sniper_headshot_custom_disarmed",
		{ duration = self.modifier_sniper_1_disarm_duration * (1 - self:GetCaster():GetStatusResistance()) }
	)
...
function sniper_headshot_custom:OnProjectileHit_ExtraData(target, location, extraData)
	...
	if self:GetCaster():HasModifier("modifier_sniper_5") then
		attack_count = attack_count
			+ self.modifier_sniper_5_base
			+ (
				(
					self:GetCaster():GetStrength()
					/ self.modifier_sniper_5_str[caster:GetTalentLevel("modifier_sniper_5")]
				) * self.modifier_sniper_5_attack_bonus
			)
	end
	local modifier_sniper_headshot_custom_active_attack =
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_sniper_headshot_custom_active_attack", {})
	if modifier_sniper_headshot_custom_active_attack then
		...
		local particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_snapfire/hero_snapfire_shotgun_impact.vpcf",
...
function modifier_sniper_headshot_custom:IsPurgeException()
	return false
end
function modifier_sniper_headshot_custom:RemoveOnDeath()
	return false
end
...
function modifier_sniper_headshot_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	if self:GetCaster():HasModifier("modifier_sniper_17") then
		return
	end
	if not IsServer() then
		return
	...
	if self:GetCaster():HasModifier("modifier_sniper_2") then
		damage = damage
			+ (
				self:GetAbility().modifier_sniper_2[self:GetCaster():GetTalentLevel("modifier_sniper_2")]
				+ (self:GetCaster():GetStrength() * self:GetAbility().modifier_sniper_2_per_str)
			)
	end
...
function modifier_sniper_headshot_custom_active_attack:IsHidden()
	return true
end
function modifier_sniper_headshot_custom_active_attack:IsPurgable()
	return false
end
function modifier_sniper_headshot_custom_active_attack:IsPurgeException()
	return false
end
function modifier_sniper_headshot_custom_active_attack:RemoveOnDeath()
	return false
end