...
LinkLuaModifier(
	"modifier_kotl_chakra_burn",
	"heroes/hero_keeper_of_the_light/modifier_kotl_chakra_burn",
...
function kotl_chakra_burn:OnSpellStart()
	...
	local nFXIndex = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_keeper_of_the_light/keeper_cast02.vpcf",
	...
	for _, target in pairs(targets) do
		...
		target:AddNewModifier(
			...
			hAbility,
			"modifier_kotl_chakra_burn",
			{ duration = duration * target:GetStatusResistanceFactor(hCaster), stack = stack }
		)
	end
end