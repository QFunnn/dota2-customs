	...
	if hParent == params.unit then
		if earth_shock_duration > 0 and params.ability:GetAbilityName() == "ursa_earthshock" then
			...
			if not hParent:HasModifier("modifier_ursa_enrage_lua_buff") then
				bAdd = true
			elseif
				hParent:HasModifier("modifier_ursa_enrage_lua_buff")
				and hParent:FindModifierByName("modifier_ursa_enrage_lua_buff"):GetRemainingTime()
			...
			if bAdd then
				hParent:AddNewModifier(
					hParent,
					self:GetAbility(),
					"modifier_ursa_enrage_lua_buff",
					{ duration = earth_shock_duration }
				)
			end
		end
	end
