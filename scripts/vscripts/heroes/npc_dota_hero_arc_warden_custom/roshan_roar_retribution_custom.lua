...
LinkLuaModifier(
	"modifier_roshan_roar_retribution_custom",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roar_retribution_custom",
...
function roshan_roar_retribution_custom:OnSpellStart()
	...
	local particle_roar =
		ParticleManager:CreateParticle("particles/roshan_roar_mouths.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	...
	ParticleManager:SetParticleControl(
		particle_roar,
		1,
		self:GetCaster():GetAbsOrigin() + self:GetCaster():GetForwardVector() * 300
	)
...
function modifier_roshan_roar_retribution_custom:IsPurgable()
	return false
end
