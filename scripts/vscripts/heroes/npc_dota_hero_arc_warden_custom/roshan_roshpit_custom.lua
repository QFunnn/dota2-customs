...
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker_wall",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker_stunned",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_generic_knockback_lua", "modifiers/modifier_generic_knockback_lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker_stun_cooldown",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker_friendly",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_roshan_roshpit_custom_thinker_friendly_buff",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_roshpit_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_roshan_roshpit_custom_thinker:IsHidden()
	return true
end
...
function modifier_roshan_roshpit_custom_thinker:OnCreated(kv)
	...
	local particle = ParticleManager:CreateParticle(
		"particles/roshan_arena_roshpit.vpcf",
...
function modifier_roshan_roshpit_custom_thinker:OnIntervalThink()
	...
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_roshan_roshpit_custom_thinker_wall", {})
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_roshan_roshpit_custom_thinker_stunned", {})
	...
	if not self.owner then
		...
		if not self:GetParent():IsDebuffImmune() then
			...
			self:GetParent():AddNewModifier(
				...
				"modifier_roshan_roshpit_custom_thinker_stun_cooldown",
				{
					duration = self:GetAbility().modifier_arc_warden_11[self:GetCaster()
						:GetTalentLevel("modifier_arc_warden_11")],
				}
			)
...
function modifier_roshan_roshpit_custom_thinker_stunned:GetAuraEntityReject(unit)
	...
	if unit:IsCurrentlyVerticalMotionControlled() then
		return true
	end
...
function modifier_roshan_roshpit_custom_thinker_stun_cooldown:IsHidden()
	return true
end
function modifier_roshan_roshpit_custom_thinker_stun_cooldown:IsPurgable()
	return false
end
