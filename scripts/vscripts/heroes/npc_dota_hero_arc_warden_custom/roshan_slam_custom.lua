...
LinkLuaModifier(
	"modifier_roshan_slam_custom_debuff",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_slam_custom",
...
function roshan_slam_custom:GetCooldown(iLevel)
	local bonus = 0
	if
		self:GetCaster():HasModifier("modifier_arc_warden_17")
		and not self:GetCaster():HasModifier("modifier_arc_warden_17_debuff")
	then
	...
	if self:GetCaster():HasModifier("modifier_arc_warden_15") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_arc_warden_15[self:GetCaster():GetTalentLevel("modifier_arc_warden_15")]
			)
	end
