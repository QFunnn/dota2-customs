...
LinkLuaModifier(
	"modifier_roshan_bash_custom",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_bash_custom",
...
LinkLuaModifier(
	"modifier_roshan_bash_custom_active_cooldown",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_bash_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_roshan_bash_custom_cooldown",
	"heroes/npc_dota_hero_arc_warden_custom/roshan_bash_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function roshan_bash_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_arc_warden.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_arc_warden.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_arc_warden.vpcf",
		context
	)
end
...
function roshan_bash_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_roshan_bash_custom_active_cast",
		{ duration = 0.75, speed = self:GetCaster():GetIdealSpeed() }
	)
end
...
function roshan_bash_custom:OnChargeFinish(interrupt, target)
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_roshan_bash_custom_active",
		{ duration = self.modifier_arc_warden_3_duration }
	)
end
...
function modifier_roshan_bash_custom:IsHidden()
	return true
end
function modifier_roshan_bash_custom:IsPurgable()
	return false
end
function modifier_roshan_bash_custom:IsPurgeException()
	return false
end
function modifier_roshan_bash_custom:RemoveOnDeath()
	return false
end
...
function modifier_roshan_bash_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if
		self:GetParent():GetUnitName() ~= "npc_dota_hero_arc_warden"
		and self:GetParent():GetUnitName() ~= "npc_dota_hero_rubick"
	then
		return
	end
	if self:GetParent():HasModifier("modifier_item_abyssal_blade") then
		return
	end
	if self:GetParent():HasModifier("modifier_item_basher") then
		return
	end
	if self:GetParent():HasModifier("modifier_item_diffusal_basher") then
		return
	end
	if self:GetParent():HasItemInInventory("item_basher") then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_arc_warden_9") then
		chance = chance
			+ self:GetAbility().modifier_arc_warden_9[self:GetCaster():GetTalentLevel("modifier_arc_warden_9")]
	end
	if self:GetCaster():HasModifier("modifier_arc_warden_2") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_arc_warden_2[self:GetCaster():GetTalentLevel("modifier_arc_warden_2")]
			)
	end
	if
		RollPseudoRandomPercentage(chance, 322, self:GetParent())
		and not self:GetParent():HasModifier("modifier_roshan_bash_custom_cooldown")
	then
		...
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_roshan_bash_custom_cooldown",
			{ duration = 0.5 * self:GetCaster():GetCooldownReduction() }
		)
		return damage
	end
end
...
function modifier_roshan_bash_custom:OnAbilityExecuted(params)
	...
	if not self:GetCaster():HasModifier("modifier_arc_warden_12") then
		return
	end
	...
	if self.exceptions[params.ability:GetAbilityName()] ~= nil then
		return
	end
	...
	-- end
	if not RollPercentage(self:GetAbility().modifier_arc_warden_12_chance) then
		return
	end
	local heroes = FindUnitsInRadius(
		...
		nil,
		self:GetAbility().modifier_arc_warden_12,
	...
	if target ~= nil then
		...
		local stun_duration = self:GetAbility():GetSpecialValueFor("stun_duration")
			/ 100
			* self:GetAbility().modifier_arc_warden_12_duration_percent
...
function modifier_roshan_bash_custom_active:IsPurgable()
	return false
end
function modifier_roshan_bash_custom_active:IsPurgeException()
	return false
end
...
function modifier_roshan_bash_custom_active:OnIntervalThink()
	...
	local units = FindUnitsInRadius(
		...
		self:GetParent():GetAbsOrigin(),
		nil,
		self:GetCaster():GetAoeBonus(self:GetAbility().modifier_arc_warden_3_radius),
		DOTA_UNIT_TARGET_TEAM_ENEMY,
...
function modifier_roshan_bash_custom_active:GetModifierMoveSpeedBonus_Constant()
	...
	if self:GetCaster():HasModifier("modifier_arc_warden_4") then
		bonus = bonus
			+ self:GetAbility().modifier_arc_warden_4[self:GetCaster():GetTalentLevel("modifier_arc_warden_4")]
	end
	return bonus
end
...
function modifier_roshan_bash_custom_active_cooldown:IsPurgable()
	return false
end
function modifier_roshan_bash_custom_active_cooldown:IsPurgeException()
	return false
end
function modifier_roshan_bash_custom_active_cooldown:IsHidden()
	return true
end

modifier_roshan_bash_custom_cooldown = class({})
function modifier_roshan_bash_custom_cooldown:IsPurgable()
	return false
end
function modifier_roshan_bash_custom_cooldown:IsPurgeException()
	return false
end
function modifier_roshan_bash_custom_cooldown:IsHidden()
	return true
end

modifier_roshan_bash_custom_active_cast = class({})
function modifier_roshan_bash_custom_active_cast:IsPurgable()
	return false
end
...
function modifier_roshan_bash_custom_active_cast:OnIntervalThink()
	...
	if
		self:GetParent():IsRooted()
		or self:GetParent():IsStunned()
		or self:GetParent():IsSilenced()
		or self:GetParent():IsCurrentlyHorizontalMotionControlled()
		or self:GetParent():IsCurrentlyVerticalMotionControlled()
	then
