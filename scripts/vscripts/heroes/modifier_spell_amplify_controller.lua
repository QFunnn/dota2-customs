...
modifier_spell_amplify_controller = class({}) ---@class modifier_spell_amplify_controller : CDOTA_Modifier_Lua

function modifier_spell_amplify_controller:IsHidden()
	return true
end

function modifier_spell_amplify_controller:IsDebuff()
	return false
end

function modifier_spell_amplify_controller:IsPurgable()
	return false
end

function modifier_spell_amplify_controller:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end

function modifier_spell_amplify_controller:RemoveOnDeath()
	return false
end
...
function modifier_spell_amplify_controller:OnIntervalThink()
	...
	if self:IsNull() or not self:GetParent() then
		return
	end
...
function modifier_spell_amplify_controller:SetValues()
	if
		...
		and (not self:IsNull())
		and self:GetParent()
		and (not self:GetParent():IsNull())
		and self:GetParent().GetIntellect
		and self:GetParent().flSP
	then
...
if IsServer() then
	function modifier_spell_amplify_controller:GetModifierSpellAmplify_Percentage(params)
		...
		if IsServer() and inflictor and not inflictor:IsNull() then
			...
			--процентные способности не действуют
			if
				GAMEPLAY_KV.NoSpellAmpAbilities and GAMEPLAY_KV.NoSpellAmpAbilities[ability_name]
				or (ability_name == "enigma_black_hole" and parent:HasScepter())
			then
				return 0
			end

			--является ли цель героем
			...
			if target and not target:IsNull() then
				if
					...
					and string.find(target:GetUnitName(), "npc_dota_hero") == 1
				then
					bHeroTarget = true
				end
			end
			...
			if
				GAMEPLAY_KV.DotAbilities
				and GAMEPLAY_KV.DotAbilities[ability_name]
				and parent.GetIntellect
				and parent.flDotSP
				and not DamageFlagFilter(DOTA_DAMAGE_FLAG_NO_DOT, params.damage_flags)
			then
				if bHeroTarget then
					return 0.35
							* parent.flDotSP
							* (parent:GetIntellect(true) + parent:GetStrength() + parent:GetAgility())
						+ self.spell_amplify * 0.2
				else
					local test = parent.flDotSP
							* (parent:GetIntellect(true) + parent:GetStrength() + parent:GetAgility())
						+ self.spell_amplify
