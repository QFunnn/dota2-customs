...
if IsServer() then
	...
	function modifier_spell_amplify_controller:GetModifierSpellAmplify_Percentage(params)
		...
		if IsServer() and inflictor and not inflictor:IsNull() then
			...
			--百分比技能无效
			if
				PERCENTAGE_ABILITIES and PERCENTAGE_ABILITIES[ability_name]
			...
			if
				DOT_ABILITIES
				and DOT_ABILITIES[ability_name]
			...
			then
				if bHeroTarget then
					...
					return parent.flDotSP * (parent:GetIntellect(true) + parent:GetStrength() + parent:GetAgility())
						+ self.spell_amplify
				end
			end
