...
function modifier_spell_amplify_controller:SetValues()
	if
		...
		and self.parent.GetIntellect
		and self.parent.flSP
	then
...
if IsServer() then
	...
	function modifier_spell_amplify_controller:GetModifierSpellAmplify_Percentage(params)
		...
		if inflictor and not inflictor:IsNull() then
			...
			if
				percentage_abilities[ability_name]
				and (ability_name ~= "enigma_black_hole" or parent:HasScepter())
				and (
					ability_name ~= "witch_doctor_voodoo_restoration_custom"
					or parent:HasTalent("special_bonus_unique_witch_doctor_2")
				)
			then
				return 0
			end
			...
			if ability_name == "huskar_burning_spear" or ability_name == "huskar_burning_spear_custom" then
				...
				if parent.GetIntellect and parent.flDotSP then
					...
					if modifier_rubick_arcane_supremacy then
						bonus = parent:GetIntellect(false) * 0.25
					end
			...
			if dot_abilities[ability_name] and parent.GetIntellect and parent.flDotSP then
				...
				if bHeroTarget then
					amp = (
						0.35
							* parent.flDotSP
							* (parent:GetIntellect(false) + parent:GetStrength() + parent:GetAgility())
						+ self.spell_amplify * 0.2
					) + bonus_modifier_rubick_arcane_supremacy
				else
					amp = (
						parent.flDotSP * (parent:GetIntellect(false) + parent:GetStrength() + parent:GetAgility())
						+ self.spell_amplify
					) + bonus_modifier_rubick_arcane_supremacy
				end
