...
LinkLuaModifier(
	"modifier_oracle_fortunes_end_custom_purge",
	"heroes/npc_dota_hero_oracle_custom/oracle_fortunes_end_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_oracle_fortunes_end_custom_thinker",
	"heroes/npc_dota_hero_oracle_custom/oracle_fortunes_end_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_oracle_fortunes_end_custom_attack_speed_target",
	"heroes/npc_dota_hero_oracle_custom/oracle_fortunes_end_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_oracle_fortunes_end_custom_attack_speed_buff",
	"heroes/npc_dota_hero_oracle_custom/oracle_fortunes_end_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function oracle_fortunes_end_custom:OnSpellStart()
	...
	self.fortunes_particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_oracle/oracle_fortune_channel.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetCaster()
	)
	ParticleManager:SetParticleControlEnt(
		self.fortunes_particle,
		0,
		self:GetCaster(),
...
function oracle_fortunes_end_custom:ApplyFortunesEnd(target, charge_pct)
	...
	self.aoe_particle = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_oracle_11") then
		...
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_oracle_11[self:GetCaster():GetTalentLevel("modifier_oracle_11")]
			)
	end
	...
	do
		...
		enemy:AddNewModifier(
			...
			{
				duration = (math.max(
					self:GetSpecialValueFor("maximum_purge_duration") * math.min(charge_pct, 1),
					self:GetSpecialValueFor("minimum_purge_duration")
		...
		if self:GetCaster():HasModifier("modifier_oracle_8") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_oracle_fortunes_end_custom_attack_speed_target",
				{ duration = self.modifier_oracle_8_duration }
			)
		end
	end
end
...
function modifier_oracle_fortunes_end_custom_thinker:IsHidden()
	return true
end
function modifier_oracle_fortunes_end_custom_thinker:IsPurgable()
	return false
end
...
function modifier_oracle_fortunes_end_custom_attack_speed_target:GetTexture()
	return "oracle_8"
end
function modifier_oracle_fortunes_end_custom_attack_speed_target:IsPurgable()
	return false
end
function modifier_oracle_fortunes_end_custom_attack_speed_target:IsPurgeException()
	return false
end
...
function modifier_oracle_fortunes_end_custom_attack_speed_target:OnAttackStart(params)
	...
	params.attacker:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_oracle_fortunes_end_custom_attack_speed_buff",
		{ duration = self:GetAbility().modifier_oracle_8_duration }
	)
end
...
function modifier_oracle_fortunes_end_custom_attack_speed_buff:IsHidden()
	return true
end
function modifier_oracle_fortunes_end_custom_attack_speed_buff:IsPurgable()
	return false
end
