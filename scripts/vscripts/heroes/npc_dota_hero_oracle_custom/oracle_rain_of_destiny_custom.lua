...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_oracle_custom/oracle_rain_of_destiny_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_oracle_rain_of_destiny_custom_buff",
	"heroes/npc_dota_hero_oracle_custom/oracle_rain_of_destiny_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_oracle_rain_of_destiny_custom:IsPurgable()
	return false
end
function modifier_oracle_rain_of_destiny_custom:IsPurgeException()
	return false
end
...
function modifier_oracle_rain_of_destiny_custom_buff:OnIntervalThink()
	...
	local damage = self:GetAbility():GetSpecialValueFor("base")
		+ (self:GetCaster():GetManaRegen() / 100 * self:GetAbility():GetSpecialValueFor("percent"))
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		ApplyDamage({
