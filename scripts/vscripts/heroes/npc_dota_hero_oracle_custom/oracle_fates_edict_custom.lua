...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_oracle_custom/oracle_fates_edict_custom",
...
function modifier_oracle_fates_edict_custom:OnCreated()
	self.magic_damage_resistance_pct_tooltip = self:GetAbility()
		:GetSpecialValueFor("magic_damage_resistance_pct_tooltip")
	if not IsServer() then
		return
	end
	self:GetParent():EmitSound("Hero_Oracle.FatesEdict")
	self.disarm_particle = ParticleManager:CreateParticle(
