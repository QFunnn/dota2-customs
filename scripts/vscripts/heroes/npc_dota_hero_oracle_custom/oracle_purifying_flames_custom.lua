...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_oracle_custom/oracle_purifying_flames_custom",
...
function oracle_purifying_flames_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_oracle.vpcf",
		context
	)
end
...
function oracle_purifying_flames_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_oracle_15") then
		...
		local nearby_enemies = FindUnitsInRadius(
			...
			target:GetAbsOrigin(),
			nil,
			self.modifier_oracle_15_radius,
...
function oracle_purifying_flames_custom:CastFlames(target)
	...
	self.purifying_particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_oracle/oracle_purifyingflames_hit.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		target
	)
	ParticleManager:SetParticleControlEnt(
		self.purifying_particle,
		1,
		target,
	...
	self.purifying_cast_particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_oracle/oracle_purifyingflames_cast.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetCaster()
	)
	ParticleManager:SetParticleControlEnt(
		self.purifying_cast_particle,
		1,
		self:GetCaster(),
		PATTACH_POINT_FOLLOW,
		"attach_attack1",
	...
	if self:GetCaster():HasModifier("modifier_oracle_20") then
		damage = damage
			+ (
				self:GetCaster():GetManaRegen()
				/ 100
				* self.modifier_oracle_20[self:GetCaster():GetTalentLevel("modifier_oracle_20")]
			)
	end
	...
	if
		target:GetUnitName() == "npc_woda_pig"
		or target:GetUnitName() == "npc_woda_frog"
		or target:GetUnitName() == "npc_woda_pig_pve"
		or target:GetUnitName() == "npc_woda_frog_pve"
		or target:GetUnitName() == "npc_dota_weaver_swarm"
	then
		return
	end
...
function modifier_oracle_purifying_flames_custom:IsDebuff()
	return false
end
function modifier_oracle_purifying_flames_custom:GetAttributes()
