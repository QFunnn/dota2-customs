...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_oracle_custom/oracle_false_promise_custom",
...
function oracle_false_promise_custom:ApplyFalsePromise(target, talent)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_oracle_false_promise_custom",
		{ duration = self:GetSpecialValueFor("duration"), talent_21 = talent }
	)
end
...
function modifier_oracle_false_promise_custom:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_oracle_18") then
		if self.damage_counter > 0 then
			...
			local damage = self.damage_counter
				/ 100
				* self:GetAbility().modifier_oracle_18[self:GetCaster():GetTalentLevel("modifier_oracle_18")]
	...
	if self.talent_21 == 1 then
		...
		if self.damage_counter < self.heal_counter then
			...
			self:GetParent():Heal(
				math.max(heal, self:GetParent():GetMaxHealth() / 100 * self:GetAbility().modifier_oracle_21),
				self:GetAbility()
			)
		else
			self:GetParent()
				:Heal(self:GetParent():GetMaxHealth() / 100 * self:GetAbility().modifier_oracle_21, self:GetAbility())
		end
		return
	end
				...
				if self.heal_counter < instance.damage then
					...
					if self:GetCaster():HasModifier("modifier_oracle_5") then
						if instance.damage > 0 then
							instance.damage = instance.damage
								* (
									1
									- (
										self:GetAbility().modifier_oracle_5[self:GetCaster()
											:GetTalentLevel("modifier_oracle_5")] / 100
									)
								)
						end
					end
...
function modifier_oracle_false_promise_custom:ApplyDamageFunc(keys, attacker)
	if attacker and self:GetRemainingTime() >= 0 then
		self.attacked_particle = ParticleManager:CreateParticle(
		...
		if attacker == self:GetCaster() then
			flags = DOTA_DAMAGE_FLAG_REFLECTION
...
function modifier_oracle_false_promise_custom_handler:IsPurgeException()
	return false
end
function modifier_oracle_false_promise_custom_handler:IsPurgable()
	return false
end
function modifier_oracle_false_promise_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_oracle_false_promise_custom_handler:GetMinHealth()
	...
	if not self:GetParent():HasModifier("modifier_oracle_21") then
		return
	end
	return 1
end
