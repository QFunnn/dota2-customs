...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_oracle_custom/oracle_change_of_fate",
...
function modifier_oracle_change_of_fate:IsDebuff()
	return true
end
function modifier_oracle_change_of_fate:IsPurgeException()
	return false
end
function modifier_oracle_change_of_fate:IsPurgable()
	return false
end
...
function modifier_oracle_change_of_fate:GetAbsoluteNoDamagePhysical(params)
	if params.inflictor and params.inflictor:GetAbilityName() == "oracle_change_of_fate" then
		return
	end
	if params.damage <= 0 then
		return
	end
	if self:GetParent():HasModifier("modifier_abaddon_borrowed_time_custom_buff") then
		return
	end
...
function modifier_oracle_change_of_fate:OnHealReceived(params)
	if params.unit == self:GetParent() and self:GetRemainingTime() >= 0 then
		...
		ApplyDamage({
			...
			damage_flags = damage_flag + DOTA_DAMAGE_FLAG_REFLECTION + DOTA_DAMAGE_FLAG_HPLOSS,
		})
	end
end
