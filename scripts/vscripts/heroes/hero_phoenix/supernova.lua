...
LinkLuaModifier(
	"modifier_ability_phoenix_supernova_debuff",
	"heroes/hero_phoenix/supernova.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_phoenix_supernova_hiding",
	"heroes/hero_phoenix/supernova.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function ability_phoenix_supernova:OnSpellStart()
	...
	local hEgg =
		CreateUnitByName("npc_dota_phoenix_sun", position, false, caster, caster:GetOwner(), caster:GetTeamNumber())
...
modifier_ability_phoenix_supernova = class({
	...
	GetModifierAura = function(self)
		return "modifier_ability_phoenix_supernova_debuff"
	end,
			...
			if caster then
				...
				for _, enemy in pairs(enemies) do
					enemy:AddNewModifier(
						...
						self:GetAbility(),
						"modifier_stunned",
						{ duration = (self.StunDuration or 0) * (1 - enemy:GetStatusResistance()) }
					)
				end
			end
		end
...
function modifier_ability_phoenix_supernova:AttackLandedModifier(event)
	...
	if hEgg ~= event.target or not event.attacker or not event.attacker:IsRealHero() then
		return
	end
...
function modifier_ability_phoenix_supernova:OnDeathEvent(event)
	...
	if hEgg ~= event.unit or not event.attacker or not event.attacker:IsRealHero() then
		return
	end
...
function modifier_ability_phoenix_supernova_hiding:OnCreated()
	...
	for i = 0, caster:GetAbilityCount() - 1 do
		...
		if
			...
			and (not hasShard or (hasShard and hAbility:GetName() ~= "phoenix_sun_ray"))
		then
