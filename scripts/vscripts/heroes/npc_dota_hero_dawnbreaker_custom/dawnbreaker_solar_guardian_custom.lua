...
LinkLuaModifier(
	"modifier_dawnbreaker_solar_guardian_custom",
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_solar_guardian_custom",
...
function dawnbreaker_solar_guardian_custom:CastFilterResultLocation(vLoc)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_8") then
		max_offset_distance = max_offset_distance
			+ self.modifier_dawnbreaker_8[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_8")]
	end
	local allies = FindUnitsInRadius(
		caster:GetTeamNumber(),
		vLoc,
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_fire_wreath_custom") then
		...
		if player then
			CustomGameEventManager:Send_ServerToPlayer(
				...
				{ message = "#dota_hud_error_fire_wreath_in_progress" }
			)
		end
		return false
	end
...
function dawnbreaker_solar_guardian_custom:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_dawnbreaker_solar_guardian_custom",
		{ duration = channel + leaptime, x = point.x, y = point.y }
	)
	self.point = point
...
function dawnbreaker_solar_guardian_custom:OnChannelFinish(interrupted)
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_dawnbreaker_solar_guardian_custom_leap",
		{ duration = duration, x = self.point.x, y = self.point.y }
	)
end
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_10") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_dawnbreaker_10[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_10")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_11") then
		heal = heal
			+ (
				self:GetCaster():GetAgility()
				/ 100
				* self.modifier_dawnbreaker_11[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_11")]
			)
	end

	point = GetGroundPosition(point, self:GetCaster())
	local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian_damage.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetCaster()
	)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_10") then
		self.damage = self.damage
			+ (
				self:GetCaster():GetAgility()
				/ 100
				* self:GetAbility().modifier_dawnbreaker_10[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_10")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_11") then
		self.heal = self.heal
			+ (
				self:GetCaster():GetAgility()
				/ 100
				* self:GetAbility().modifier_dawnbreaker_11[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_11")]
			)
	end
...
function modifier_dawnbreaker_solar_guardian_custom:PlayEffects1()
	local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_dawnbreaker_solar_guardian_custom:PlayEffects4(target)
	local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian_healing_buff.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		target
	)
	ParticleManager:ReleaseParticleIndex(effect_cast)
...
function modifier_dawnbreaker_solar_guardian_custom_leap:OnCreated(kv)
	...
	local arc = self.parent:AddNewModifier(
		...
		{ duration = kv.duration, height = arc_height, isStun = false, isForward = true }
	)
...
function modifier_dawnbreaker_solar_guardian_custom_leap:OnDestroy()
	...
	local modifier_dawnbreaker_solar_guardian_custom =
		self.parent:FindModifierByName("modifier_dawnbreaker_solar_guardian_custom")
...
function modifier_dawnbreaker_solar_guardian_custom_leap:PlayEffects1()
	local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_dawnbreaker/dawnbreaker_solar_guardian_airtime_buff.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent
	)
	self:AddParticle(effect_cast, false, false, -1, false, false)
