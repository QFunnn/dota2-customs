...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_celestial_hammer_custom",
...
function dawnbreaker_celestial_hammer_custom:GetCastRange(vLocation, hTarget)
	if IsClient() then
		...
		if self:GetCaster():HasModifier("modifier_dawnbreaker_20") then
			cast_range_multiplier = cast_range_multiplier
				+ self.modifier_dawnbreaker_20[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_20")] / 100
		end
...
function dawnbreaker_celestial_hammer_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_20") then
		speed = speed
			+ (speed / 100 * self.modifier_dawnbreaker_20[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_20")])
	end
	local dawnbreaker_celestial_hammer_custom_immune = self:GetCaster()
		:FindAbilityByName("dawnbreaker_celestial_hammer_custom_immune")
...
function dawnbreaker_celestial_hammer_custom:HammerHit(target, location)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_19") then
		target:AddNewModifier(
			...
			"modifier_stunned",
			{
				duration = self.modifier_dawnbreaker_19[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_19")]
...
function modifier_dawnbreaker_celestial_hammer_custom:IsPurgeException()
	return false
end
...
function modifier_dawnbreaker_celestial_hammer_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_20") then
		self.speed = self.speed
			+ (
				self.speed
				/ 100
				* self:GetAbility().modifier_dawnbreaker_20[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_20")]
			)
	end
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_18") then
		self.duration = self.duration
			+ self:GetAbility().modifier_dawnbreaker_18[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_18")]
	end
	self.interval = 0.1
	if not IsServer() then
		return
...
function modifier_dawnbreaker_celestial_hammer_custom:OnIntervalThink()
	local thinker = CreateModifierThinker(
		...
		self.ability,
		"modifier_dawnbreaker_celestial_hammer_custom_trail",
		{ duration = self.duration, x = self.prev_pos.x, y = self.prev_pos.y },
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_16") then
		self.damage = self.damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_dawnbreaker_16[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_16")]
			)
	end
