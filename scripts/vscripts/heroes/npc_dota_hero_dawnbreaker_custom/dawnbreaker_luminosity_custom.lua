...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_luminosity_custom",
...
LinkLuaModifier(
	"modifier_dawnbreaker_luminosity_custom_buff_animate",
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_luminosity_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_dawnbreaker_luminosity_custom:IsHidden()
	return self:GetStackCount() < 1 or self:GetParent():HasModifier("modifier_dawnbreaker_luminosity_custom_buff")
end
function modifier_dawnbreaker_luminosity_custom:IsPurgable()
	return false
end
function modifier_dawnbreaker_luminosity_custom:IsPurgeException()
	return false
end
...
function modifier_dawnbreaker_luminosity_custom:OnAttackFinished(params)
	...
	if self:GetParent():HasModifier("modifier_dawnbreaker_luminosity_custom_buff") then
		self:GetParent()
			:AddNewModifier(self:GetParent(), self.ability, "modifier_dawnbreaker_luminosity_custom_buff_animate", {})
	end
end
...
function modifier_dawnbreaker_luminosity_custom:Increment()
	...
	if self:GetStackCount() >= self.count then
		local mod =
			self.parent:AddNewModifier(self.parent, self.ability, "modifier_dawnbreaker_luminosity_custom_buff", {})
		if self:GetParent():HasModifier("modifier_dawnbreaker_9") then
			local dawnbreaker_solar_guardian_custom = self:GetParent()
			...
			if dawnbreaker_solar_guardian_custom and dawnbreaker_solar_guardian_custom:GetLevel() > 0 then
				...
				if modifier_dawnbreaker_9_buff then
					local talent_counter = math.min(
						modifier_dawnbreaker_9_buff:GetStackCount() + 1,
						self:GetAbility().modifier_dawnbreaker_9[self:GetCaster()
							:GetTalentLevel("modifier_dawnbreaker_9")]
					)
...
function modifier_dawnbreaker_luminosity_custom_buff:OnDestroy()
	...
	local modifier_dawnbreaker_luminosity_custom =
		self.parent:FindModifierByName("modifier_dawnbreaker_luminosity_custom")
...
function modifier_dawnbreaker_luminosity_custom_buff:OnAttackLanded(params)
	...
	local modifier_dawnbreaker_luminosity_custom_buff_animate =
		self:GetParent():FindModifierByName("modifier_dawnbreaker_luminosity_custom_buff_animate")
	...
	if self:GetParent():HasModifier("modifier_dawnbreaker_9") then
		...
		if dawnbreaker_solar_guardian_custom and dawnbreaker_solar_guardian_custom:GetLevel() > 0 then
			if
				modifier_dawnbreaker_9_buff
				and modifier_dawnbreaker_9_buff:GetStackCount()
					>= self:GetAbility().modifier_dawnbreaker_9[self:GetCaster()
						:GetTalentLevel("modifier_dawnbreaker_9")]
			then
...
function modifier_dawnbreaker_luminosity_custom_buff_animate:IsHidden()
	return true
end
