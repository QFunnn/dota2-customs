...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_fire_wreath_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_dawnbreaker_fire_wreath_custom_slow",
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_fire_wreath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dawnbreaker_fire_wreath_custom_immune",
	"heroes/npc_dota_hero_dawnbreaker_custom/dawnbreaker_fire_wreath_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function dawnbreaker_fire_wreath_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_dawnbreaker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_dawnbreaker.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_dawnbreaker.vpcf",
		context
	)
end
...
function dawnbreaker_fire_wreath_custom:GetCooldown(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_4") then
		bonus = self.modifier_dawnbreaker_4_cooldown
			* (
				self:GetCaster():GetStrength()
				/ self.modifier_dawnbreaker_4_str[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_4")]
			)
	end
...
function dawnbreaker_fire_wreath_custom:GetManaCost(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_2") then
		default_manacost = default_manacost
			- (
				default_manacost
				/ 100
				* self.modifier_dawnbreaker_2[self:GetCaster():GetTalentLevel("modifier_dawnbreaker_2")]
			)
	end
...
function modifier_dawnbreaker_fire_wreath_custom:IsPurgable()
	return false
end
...
function modifier_dawnbreaker_fire_wreath_custom:OnCreated(kv)
	...
	if self.parent:HasModifier("modifier_dawnbreaker_1") then
		self.swipe_damage = self.swipe_damage
			+ self:GetAbility().modifier_dawnbreaker_1[self.parent:GetTalentLevel("modifier_dawnbreaker_1")]
	end
	...
	if self.parent:HasModifier("modifier_dawnbreaker_1") then
		self.smash_damage = self.smash_damage
			+ self:GetAbility().modifier_dawnbreaker_1[self.parent:GetTalentLevel("modifier_dawnbreaker_1")]
	end
	...
	if self.parent:HasModifier("modifier_dawnbreaker_3") then
		self.swipe_radius = self.swipe_radius
			+ self:GetAbility().modifier_dawnbreaker_3[self.parent:GetTalentLevel("modifier_dawnbreaker_3")]
		self.swipe_radius = self.swipe_radius + (self:GetAbility().modifier_dawnbreaker_3_kills * bonus_kills)
	end

	if self.parent:HasModifier("modifier_dawnbreaker_3") then
		self.smash_radius = self.smash_radius
			+ self:GetAbility().modifier_dawnbreaker_3[self.parent:GetTalentLevel("modifier_dawnbreaker_3")]
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_6") then
		self.modifier_dawnbreaker_fire_wreath_custom_immune = self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_dawnbreaker_fire_wreath_custom_immune",
			{}
...
function modifier_dawnbreaker_fire_wreath_custom:OnDestroy()
	...
	if
		self.modifier_dawnbreaker_fire_wreath_custom_immune
		and not self.modifier_dawnbreaker_fire_wreath_custom_immune:IsNull()
	then
	...
	for _, enemy in pairs(enemies) do
		...
		if not enemy:IsMagicImmune() then
			enemy:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_dawnbreaker_fire_wreath_custom_slow",
				{ duration = self.swipe_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
	end
...
function modifier_dawnbreaker_fire_wreath_custom:Smash()
	...
	for _, enemy in pairs(enemies) do
		...
		if not enemy:IsMagicImmune() then
			enemy:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_stunned",
				{ duration = self.smash_duration * (1 - enemy:GetStatusResistance()) }
			)
			enemy:AddNewModifier(
				self.parent,
				self:GetAbility(),
				...
				{
					duration = self.arc_duration * (1 - enemy:GetStatusResistance()),
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_7") then
		local dawnbreaker_solar_guardian_custom = self:GetCaster()
			:FindAbilityByName("dawnbreaker_solar_guardian_custom")
...
function modifier_dawnbreaker_fire_wreath_custom:UpdateHorizontalMotion(me, dt)
	...
	if self:GetCaster():HasModifier("modifier_dawnbreaker_6") then
		speed = math.max(
			self:GetCaster():GetIdealSpeed()
				- (self:GetCaster():GetIdealSpeed() / 100 * self:GetAbility().modifier_dawnbreaker_6_slow),
			self:GetAbility().modifier_dawnbreaker_6_min_speed
		)
	end
...
function modifier_dawnbreaker_fire_wreath_custom:OnOrder(params)
	...
	if not self.orders[params.order_type] then
		return
	end
	if not self:GetParent():HasModifier("modifier_dawnbreaker_6") then
		return
	end
...
function modifier_dawnbreaker_fire_wreath_custom_immune:IsPurgable()
	return false
end
