...
function tinker_rearm_datadriven:OnChannelFinish(bInterrupted)
	...
	for i = 0, caster:GetAbilityCount() - 1 do
		...
		if
			...
			and not (
				caster:HasModifier("modifier_hero_refreshing")
				and "razor_eye_of_the_storm" == hAbility:GetAbilityName()
			)
		then
			local flRatio = 1
	...
	local modifier_tinker_rearm_datadriven_debuff = self:GetCaster()
		:FindModifierByName("modifier_tinker_rearm_datadriven_debuff")
	if modifier_tinker_rearm_datadriven_debuff then
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_tinker_rearm_datadriven_debuff", { duration = 6 })
...
function modifier_tinker_rearm_datadriven_debuff:IsPurgable()
	return false
end
...
function modifier_tinker_rearm_datadriven:IsHidden()
	return true
end
function modifier_tinker_rearm_datadriven:IsPurgable()
	return false
end
function modifier_tinker_rearm_datadriven:IsPurgeException()
	return false
end
