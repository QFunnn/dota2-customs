...
function modifier_tinker_rearm_lua_counter:OnIntervalThink()
		...
		if IsValid(hAbility) and hAbility:IsRefreshable() and hAbility ~= self then
			if not hAbility:IsCooldownReady() then
				if hAbility:IsSealingAbility() then
					iBonusTime = iBonusTime
						+ hAbility:GetCooldownTimeRemaining() * self.sealing_bonus_rate
						+ self.sealing_bonus_base
				end
			else
				-- 充能型技能
				if hAbility:GetMaxAbilityCharges(-1) > 0 then
					if hAbility:IsSealingAbility() then
						iBonusTime = iBonusTime
							+ hAbility:GetAbilityChargeRestoreTime(-1) * self.sealing_bonus_rate
							+ self.sealing_bonus_base
					end
				end
			end
	...
	for index = DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_6 do
		...
		if
			IsValid(hItem)
			and hItem:IsRefreshable()
			and hItem:GetCooldown(-1) > 0
			and not hItem:IsCooldownReady()
			and not exempt_table[hItem:GetAbilityName()]
		then
			if hItem:IsSealingAbility() then
				iBonusTime = iBonusTime
					+ hItem:GetCooldownTimeRemaining() * self.sealing_bonus_rate
					+ self.sealing_bonus_base
			end
		end
	end
