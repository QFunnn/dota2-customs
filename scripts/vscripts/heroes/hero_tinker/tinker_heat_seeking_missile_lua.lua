...
function tinker_heat_seeking_missile_lua:OnProjectileHit_ExtraData(hTarget, location, extraData)
	...
	if IsValid(hCaster) and IsValid(hTarget) and hTarget:IsAlive() then
		...
		if stun_duration > 0 then
			hTarget:AddNewModifier(hCaster, self, "modifier_bashed", { duration = stun_duration * hTarget:GetStatusResistanceFactor(hCaster) })
		end

		-- effects
		local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_tinker/tinker_missle_explosion.vpcf", PATTACH_ABSORIGIN_FOLLOW, hTarget)
		ParticleManager:ReleaseParticleIndex(effect_cast)
