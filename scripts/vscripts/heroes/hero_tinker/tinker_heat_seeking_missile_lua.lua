...
tinker_heat_seeking_missile_lua = class({}) ---@class tinker_heat_seeking_missile_lua : CDOTA_Ability_Lua
...
function tinker_heat_seeking_missile_lua:OnSpellStart()
	...
	-- find enemies
	local enemies = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetOrigin(), nil, radius,
	...
	if #enemies < 1 then
		...
		local effect_cast = ParticleManager:CreateParticle("particles/units/heroes/hero_tinker/tinker_missile_dud.vpcf",
...
function tinker_heat_seeking_missile_lua:OnProjectileHit_ExtraData(hTarget, location, extraData)
	local hCaster = self:GetCaster()
	if IsValid(hCaster) and IsValid(hTarget) and hTarget:IsAlive() then
		-- Apply damage
		local damage = {
			victim = hTarget,
			attacker = hCaster,
		...
		if stun_duration > 0 then
			hTarget:AddNewModifier(hCaster, self, "modifier_bashed",
				{
					duration = stun_duration * hTarget:GetStatusResistanceFactor(hCaster)
				})
		end

		-- effects
		local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_tinker/tinker_missle_explosion.vpcf", PATTACH_ABSORIGIN_FOLLOW, hTarget)
		ParticleManager:ReleaseParticleIndex(effect_cast)
