...
LinkLuaModifier(
	"modifier_lina_laguna_blade_custom",
	"heroes/npc_dota_hero_lina_custom/lina_laguna_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lina_laguna_blade_custom_barrier",
	"heroes/npc_dota_hero_lina_custom/lina_laguna_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_lina_laguna_blade_custom_activate_time",
	"heroes/npc_dota_hero_lina_custom/lina_laguna_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lina_laguna_blade_custom:GetBehavior()
	if
		self:GetCaster():HasModifier("modifier_lina_7")
		and not self:GetCaster():HasModifier("modifier_lina_laguna_blade_custom_activate_time")
	then
...
function lina_laguna_blade_custom:OnSpellStart(new_target)
	...
	then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_lina_laguna_blade_custom_activate_time",
			{ duration = self.modifier_lina_7_duration }
		)
		self:EndCooldown()
	...
	if self:GetCaster():HasModifier("modifier_lina_11") then
		self:GetCaster():AddNewModifier(
			...
			self,
			"modifier_lina_laguna_blade_custom_buff",
			{ duration = self.modifier_lina_11_duration }
		)
	end
	local modifier_lina_laguna_blade_custom_activate_time =
		self:GetCaster():FindModifierByName("modifier_lina_laguna_blade_custom_activate_time")
...
function lina_laguna_blade_custom:LagunaToTarget(target, no_visual)
	...
	if modifier_lina_laguna_blade_custom_activate_time then
		brew_time =
			math.min(GameRules:GetGameTime() - modifier_lina_laguna_blade_custom_activate_time:GetCreationTime(), 5)
	end
	target:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_lina_laguna_blade_custom",
		{ duration = delay, brew_time = brew_time }
	)
end
...
function modifier_lina_laguna_blade_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_lina_18") then
		self.damage = self.damage
			+ self:GetAbility().modifier_lina_18[self:GetCaster():GetTalentLevel("modifier_lina_18")]
	end
	...
	if self:GetCaster():HasModifier("modifier_lina_4") then
		if end_damage > 0 then
			self:GetCaster():AddNewModifier(
				...
				self:GetAbility(),
				"modifier_lina_laguna_blade_custom_barrier",
				{ duration = self:GetAbility().modifier_lina_4_duration, barrier = end_damage }
			)
		end
	end
...
function modifier_lina_laguna_blade_custom_barrier:GetTexture()
	return "lina_4"
end
function modifier_lina_laguna_blade_custom_barrier:OnCreated(kv)
	...
	self.barrier = (kv.barrier or 0)
		/ 100
		* self:GetAbility().modifier_lina_4[self:GetCaster():GetTalentLevel("modifier_lina_4")]
	if not IsServer() then
		return
	end
...
function modifier_lina_laguna_blade_custom_buff:GetTexture()
	return "lina_11"
end
...
function modifier_lina_laguna_blade_custom_activate_time:GetTexture()
	return "lina_7"
end
function modifier_lina_laguna_blade_custom_activate_time:IsPurgable()
	return false
end
function modifier_lina_laguna_blade_custom_activate_time:IsPurgeException()
	return false
end
...
function modifier_lina_laguna_blade_custom_activate_time:PlayEffects2()
	...
	local mid = 1
	if self.tick_halfway then
		mid = 8
	end
	local effect_cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_lina/ultimate_timer.vpcf",
