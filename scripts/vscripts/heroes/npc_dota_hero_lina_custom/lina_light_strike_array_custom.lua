...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_lina_custom/lina_light_strike_array_custom",
...
function modifier_lina_light_strike_array_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_lina_16") then
		self.damage = self.damage
			+ self:GetAbility().modifier_lina_16[self:GetCaster():GetTalentLevel("modifier_lina_16")]
	end
...
function modifier_lina_light_strike_array_custom:OnDestroy()
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_stunned",
			{ duration = self.light_strike_array_stun_duration * (1 - enemy:GetStatusResistance()) }
		)
