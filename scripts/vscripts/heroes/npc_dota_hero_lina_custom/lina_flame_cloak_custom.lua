...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_lina_custom/lina_flame_cloak_custom",
...
function lina_flame_cloak_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_lina_flame_cloak_custom", { duration = flame_cloak_duration })
	if self:GetCaster():HasModifier("modifier_lina_13") then
		local health_restore = self:GetCaster():GetMaxHealth()
			/ 100
			* self.modifier_lina_13[self:GetCaster():GetTalentLevel("modifier_lina_13")]
		self:GetCaster():Purge(false, true, false, false, false)
		self:GetCaster():HealWithParams(health_restore, self, false, true, self:GetCaster(), false)
		local particle =
			ParticleManager:CreateParticle("particles/marci_heal.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
...
function modifier_lina_flame_cloak_custom:IsPurgable()
	return false
end
function modifier_lina_flame_cloak_custom:IsPurgeException()
	return false
end
