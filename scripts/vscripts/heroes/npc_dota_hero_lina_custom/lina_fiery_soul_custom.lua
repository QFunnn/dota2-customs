...
LinkLuaModifier(
	"modifier_lina_fiery_soul_custom",
	"heroes/npc_dota_hero_lina_custom/lina_fiery_soul_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lina_fiery_soul_custom_fired",
	"heroes/npc_dota_hero_lina_custom/lina_fiery_soul_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lina_fiery_soul_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_lina.vpcf",
		context
	)
...
function lina_fiery_soul_custom:OnSpellStart()
	...
	for _, unit in pairs(units) do
		...
		unit:AddNewModifier(
			...
			{ duration = 0.2, distance = knockback_distance, height = 0, direction_x = dir.x, direction_y = dir.y }
		)
	end
end
...
function modifier_lina_fiery_soul_custom:GetTexture()
	return "lina_fiery_soul"
end
...
function modifier_lina_fiery_soul_custom:AddStack()
	...
	local modifier_lina_laguna_blade_custom_buff = self:GetParent()
		:FindModifierByName("modifier_lina_laguna_blade_custom_buff")
	if modifier_lina_laguna_blade_custom_buff then
		fiery_soul_max_stacks = modifier_lina_laguna_blade_custom_buff:GetAbility().modifier_lina_11[self:GetCaster()
			:GetTalentLevel("modifier_lina_11")]
	end
...
function modifier_lina_fiery_soul_custom:OnTakeDamage(params)
	...
	if params.inflictor:IsItem() or params.inflictor:IsToggle() then
		return
	end
...
function modifier_lina_fiery_soul_custom_fired:GetTexture()
	return "lina_19"
end
