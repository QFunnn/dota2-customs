...
function lina_dragon_slave_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_lina_17") then
		points_back_spawn = self:GetCaster():GetAbsOrigin()
			- self:GetCaster():GetForwardVector()
				* self.modifier_lina_17_range[self:GetCaster():GetTalentLevel("modifier_lina_17")]
	end
...
function lina_dragon_slave_custom:CreateProjectile(direction, spawnpoint, particle)
	...
	if self:GetCaster():HasModifier("modifier_lina_17") then
		...
		projectile_distance = projectile_distance
			+ self.modifier_lina_17_distance[self:GetCaster():GetTalentLevel("modifier_lina_17")]
	end
	local info = {
