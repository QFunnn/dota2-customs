...
LinkLuaModifier(
	"modifier_lina_brand_of_fire_custom",
	"heroes/npc_dota_hero_lina_custom/lina_brand_of_fire_custom",
...
function modifier_lina_brand_of_fire_custom:IsHidden()
	return true
end
function modifier_lina_brand_of_fire_custom:IsPurgable()
	return false
end
function modifier_lina_brand_of_fire_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_lina_2") then
		self.radius = self.radius
			+ self:GetAbility().modifier_lina_2_radius[self:GetCaster():GetTalentLevel("modifier_lina_2")]
	end
	if not IsServer() then
		return
	end
	local particle =
		ParticleManager:CreateParticle("particles/lina_mark_ability.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	...
	if self:GetCaster():HasModifier("modifier_lina_3") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_lina_3[self:GetCaster():GetTalentLevel("modifier_lina_3")]
			)
	end
	local units = FindUnitsInRadius(
		self:GetCaster():GetTeamNumber(),
