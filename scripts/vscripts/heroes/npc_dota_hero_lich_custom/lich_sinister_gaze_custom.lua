...
LinkLuaModifier(
	"modifier_lich_sinister_gaze_custom",
	"heroes/npc_dota_hero_lich_custom/lich_sinister_gaze_custom",
	...
	if self:GetCaster():HasModifier("modifier_lich_1") then
		...
		for _, target_new in pairs(self.heroes) do
			...
			if target_new:IsHero() then
				...
				duration = (self:GetSpecialValueFor("channel_duration") + bonus)
					* self:GetSpecialValueFor("creep_duration_multiplier")
			end
			target_new:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_lich_sinister_gaze_custom_debuff",
				{ duration = duration * (1 - target_new:GetStatusResistance()) }
			)
			target_new:AddNewModifier(
				self:GetCaster(),
				nil,
		...
		self.target:AddNewModifier(
			...
			self,
			"modifier_lich_sinister_gaze_custom_debuff",
			{ duration = self:GetChannelTime() * (1 - self.target:GetStatusResistance()) }
		)
		self.target:AddNewModifier(
			self:GetCaster(),
			nil,
...
function modifier_lich_sinister_gaze_custom:IsHidden()
	return true
end
function modifier_lich_sinister_gaze_custom:IsPurgable()
	return false
end
function modifier_lich_sinister_gaze_custom:GetAttributes()
...
function modifier_lich_sinister_gaze_custom:OnAbilityExecuted(params)
	...
	if params.ability:GetCaster() ~= self:GetCaster() then
		return
	end
	...
	if params.target:IsHero() then
		...
		self:SetStackCount(
			(
				(self:GetAbility():GetSpecialValueFor("channel_duration") + bonus)
				* self:GetAbility():GetSpecialValueFor("creep_duration_multiplier")
			) * 100
		)
	end
end
