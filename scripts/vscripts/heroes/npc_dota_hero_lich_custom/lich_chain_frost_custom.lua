...
LinkLuaModifier(
	"modifier_lich_chain_frost_custom_slow",
	"heroes/npc_dota_hero_lich_custom/lich_chain_frost_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lich_chain_frost_custom_frostbound",
	"heroes/npc_dota_hero_lich_custom/lich_chain_frost_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function lich_chain_frost_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_lich.vpcf",
		context
	)
end
...
function lich_chain_frost_custom:CastFilterResultTarget(hTarget)
	if
		hTarget:IsHero()
		and hTarget:GetTeamNumber() == self:GetCaster():GetTeamNumber()
		and hTarget:GetUnitName() ~= "npc_dota_lich_ice_spire"
	then
	...
	if self:GetCaster():HasModifier("modifier_lich_5") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_lich_5[self:GetCaster():GetTalentLevel("modifier_lich_5")]
			)
	end
		...
		Timers:CreateTimer(0.20, function()
			...
			for i = #enemies, 1, -1 do
				if
					...
					and (
						target == enemies[i]
						or enemies[i]:GetName() == "npc_dota_unit_undying_zombie"
						or enemies[i]:GetUnitName() == "npc_dota_techies_remote_mine"
					)
				then
			...
			if #enemies <= 0 then
				...
				local modifier_lich_chain_frost_custom_frostbound = target:AddNewModifier(
					caster,
					self,
					"modifier_lich_chain_frost_custom_frostbound",
					{ duration = self:GetSpecialValueFor("frostbound_duration") + bonus_duration }
				)
			...
			local chain_frost_projectile = {
				...
				iVisionTeamNumber = caster:GetTeamNumber(),
				ExtraData = {
					bounces_left = extradata.bounces_left,
					current_projectile_speed = extradata.current_projectile_speed,
...
function modifier_lich_chain_frost_custom_slow:IsHidden()
	return false
end
function modifier_lich_chain_frost_custom_slow:IsPurgable()
	return true
end
function modifier_lich_chain_frost_custom_slow:IsDebuff()
	return true
end
...
function modifier_lich_chain_frost_custom_frostbound:IsPurgable()
	return false
end
function modifier_lich_chain_frost_custom_frostbound:IsPurgeException()
	return false
end
...
function modifier_lich_chain_frost_custom_frostbound:OnIntervalThink()
	...
	local enemies = FindUnitsInRadius(
		...
		nil,
		self:GetAbility():GetSpecialValueFor("jump_range"),
		DOTA_UNIT_TARGET_TEAM_BOTH,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		DOTA_UNIT_TARGET_FLAG_NO_INVIS + DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,
		FIND_ANY_ORDER,
		false
	)
	local spires = FindUnitsInRadius(
