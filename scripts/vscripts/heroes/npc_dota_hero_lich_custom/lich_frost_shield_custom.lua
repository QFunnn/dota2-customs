...
LinkLuaModifier(
	"modifier_lich_frost_shield_custom",
	"heroes/npc_dota_hero_lich_custom/lich_frost_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lich_frost_shield_custom_debuff",
	"heroes/npc_dota_hero_lich_custom/lich_frost_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lich_frost_shield_custom_scream",
	"heroes/npc_dota_hero_lich_custom/lich_frost_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_lich_frost_shield_custom_armor_debuff",
	"heroes/npc_dota_hero_lich_custom/lich_frost_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_lich_frost_shield_custom_armor_stack",
	"heroes/npc_dota_hero_lich_custom/lich_frost_shield_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_lich_frost_shield_custom:IsPurgable()
	return not self:GetParent():HasModifier("modifier_lich_8")
end
function modifier_lich_frost_shield_custom:IsPurgeException()
	return not self:GetParent():HasModifier("modifier_lich_8")
end
function modifier_lich_frost_shield_custom:RemoveOnDeath()
	return not self:GetParent():HasModifier("modifier_lich_8")
end
...
function modifier_lich_frost_shield_custom:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetParent():HasModifier("modifier_lich_9") then
			damage = damage
				+ (
					self:GetParent():GetAverageTrueAttackDamage(nil)
					/ 100
					* self:GetAbility().modifier_lich_9[self:GetCaster():GetTalentLevel("modifier_lich_9")]
				)
			ApplyDamage({
				victim = enemy,
			...
			if not enemy:HasModifier("modifier_lich_frost_shield_custom_scream_cooldown") then
				if not enemy:IsDebuffImmune() then
					enemy:AddNewModifier(
						...
						self:GetAbility(),
						"modifier_lich_frost_shield_custom_scream",
						{ duration = self:GetAbility().modifier_lich_14 }
					)
					enemy:AddNewModifier(
						self:GetCaster(),
						self:GetAbility(),
						"modifier_lich_frost_shield_custom_scream_cooldown",
						{ duration = self:GetAbility().modifier_lich_14_cooldown }
					)
				end
			end
		end
		if self:GetCaster():HasModifier("modifier_lich_12") then
			enemy:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_lich_frost_shield_custom_armor_stack",
				{ duration = self:GetAbility().modifier_lich_12_duration }
			)
			enemy:AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
	...
	if self:GetCaster():HasModifier("modifier_lich_8") then
		local find_bug = FindUnitsInRadius(
			self:GetParent():GetTeamNumber(),
			self:GetParent():GetAbsOrigin(),
			nil,
			self.radius,
...
function modifier_lich_frost_shield_custom_scream:GetTexture()
	return "lich_14"
end
...
function modifier_lich_frost_shield_custom_scream_cooldown:IsPurgable()
	return false
end
function modifier_lich_frost_shield_custom_scream_cooldown:IsPurgeException()
	return false
end
function modifier_lich_frost_shield_custom_scream_cooldown:RemoveOnDeath()
	return false
end
function modifier_lich_frost_shield_custom_scream_cooldown:IsHidden()
	return true
end

modifier_lich_frost_shield_custom_armor_stack = class({})
function modifier_lich_frost_shield_custom_armor_stack:IsHidden()
	return true
end
function modifier_lich_frost_shield_custom_armor_stack:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

modifier_lich_frost_shield_custom_armor_debuff = class({})
function modifier_lich_frost_shield_custom_armor_debuff:GetTexture()
	return "lich_12"
end
function modifier_lich_frost_shield_custom_armor_debuff:OnCreated()
	...
	if self:GetParent():IsHero() then
		self.particle = ParticleManager:CreateParticle(
			"particles/lich_armor_debuff.vpcf",
