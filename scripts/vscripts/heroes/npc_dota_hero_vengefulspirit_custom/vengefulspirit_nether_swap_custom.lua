...
LinkLuaModifier(
	"modifier_vengefulspirit_nether_swap_custom",
	"heroes/npc_dota_hero_vengefulspirit_custom/vengefulspirit_nether_swap_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_vengefulspirit_nether_swap_passive",
	"heroes/npc_dota_hero_vengefulspirit_custom/vengefulspirit_nether_swap_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function vengefulspirit_nether_swap_custom:SwapTarget(target)
	...
	local target_pfx = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_vengefulspirit_4") then
		local health = self:GetCaster():GetMaxHealth()
			/ 100
			* self.modifier_vengefulspirit_4[self:GetCaster():GetTalentLevel("modifier_vengefulspirit_4")]
...
function modifier_vengefulspirit_nether_swap_custom:OnCreated(table)
	...
	if self:GetCaster():HasModifier("modifier_vengefulspirit_11") then
		self.barrier = self.barrier
			+ self:GetAbility().modifier_vengefulspirit_11[self:GetCaster():GetTalentLevel("modifier_vengefulspirit_11")]
	end
...
function modifier_vengefulspirit_nether_swap_passive:IsHidden()
	return true
end
function modifier_vengefulspirit_nether_swap_passive:IsPurgable()
	return false
end
