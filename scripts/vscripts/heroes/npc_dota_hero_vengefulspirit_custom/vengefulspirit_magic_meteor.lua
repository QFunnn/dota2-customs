...
function vengefulspirit_magic_meteor:OnSpellStart()
	...
	local ground_particle = ParticleManager:CreateParticleForTeam(
		"particles/vengeful_spirit/meteor_cast.vpcf",
		PATTACH_WORLDORIGIN,
		self:GetCaster(),
		self:GetCaster():GetTeam()
	)
	...
	local meteor_hammer =
		ParticleManager:CreateParticle("particles/veng_meteor_spell.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
	...
	local vengefulspirit_magic_missile_custom = self:GetCaster()
		:FindAbilityByName("vengefulspirit_magic_missile_custom")
		...
		if self:GetCaster():HasModifier("modifier_vengefulspirit_18") then
			damage = damage
				+ (
					...
					/ 100
					* vengefulspirit_magic_missile_custom.modifier_vengefulspirit_18[self:GetCaster():GetTalentLevel(
						"modifier_vengefulspirit_18"
					)]
				)
		end
