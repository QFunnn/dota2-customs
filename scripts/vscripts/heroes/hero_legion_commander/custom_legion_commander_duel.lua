...
LinkLuaModifier(
	"modifier_duel_buff",
	"heroes/hero_legion_commander/custom_legion_commander_duel",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_duel_damage",
	"heroes/hero_legion_commander/custom_legion_commander_duel",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_legion_commander_duel:OnSpellStart()
	...
	target:AddNewModifier(
		...
		self,
		"modifier_duel_buff",
		{ duration = duration, target = self:GetCaster():entindex() }
	)
end
...
function modifier_duel_buff:IsPurgable()
	return false
end
function modifier_duel_buff:IsDebuff()
	return self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
end
function modifier_duel_buff:GetAttributes()
...
function modifier_duel_buff:OnCreated(table)
	...
	if self:GetCaster() == self:GetParent() then
		self:GetCaster().particle = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_legion_commander/legion_duel_ring.vpcf",
			PATTACH_ABSORIGIN,
			self:GetCaster()
		)
		self:GetCaster():EmitSound("Hero_LegionCommander.Duel")
		local center_point = self.target:GetAbsOrigin()
			+ ((self:GetCaster():GetAbsOrigin() - self.target:GetAbsOrigin()) / 1)
...
function modifier_duel_buff:GetModifierIncomingDamage_Percentage(params)
	...
	if params.attacker == self.target then
		return
	end
	return -50
end
...
function modifier_duel_damage:IsPurgable()
	return false
end
function modifier_duel_damage:RemoveOnDeath()
	return false
end
function modifier_duel_damage:GetTexture()
