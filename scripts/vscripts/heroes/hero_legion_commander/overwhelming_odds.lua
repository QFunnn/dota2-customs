...
LinkLuaModifier(
	...
	"heroes/hero_legion_commander/overwhelming_odds.lua",
...
LinkLuaModifier(
	"modifier_ability_legion_commander_overwhelming_odds_buff",
	"heroes/hero_legion_commander/overwhelming_odds.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_legion_commander_overwhelming_odds_shield",
	"heroes/hero_legion_commander/overwhelming_odds.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function ability_legion_commander_overwhelming_odds:OnSpellStart()
	...
	if ShieldDuration > 0 and Damage > 0 then
		caster:AddNewModifier(
			...
			self,
			"modifier_ability_legion_commander_overwhelming_odds_shield",
			{ duration = ShieldDuration, damage = Damage }
		)
	end
...
modifier_ability_legion_commander_overwhelming_odds = class({
	...
	AttackStartModifier = function(self, event)
		...
		if
			parent
			and target == parent
			and self.nextCooldown <= GameRules:GetGameTime()
			and ability
...
modifier_ability_legion_commander_overwhelming_odds_lifesteal = class({
	...
	TakeDamageScriptModifier = function(self, event)
		...
		if
			parent
			and attacker == parent
			and event.damage_category == DOTA_DAMAGE_CATEGORY_ATTACK
			and event.inflictor == nil
		then
