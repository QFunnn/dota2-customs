...
LinkLuaModifier(
	"modifier_bane_fiends_grip_lua_illusion",
	"heroes/hero_bane/bane_fiends_grip_lua.lua",
...
function bane_fiends_grip_lua:OnSpellStart()
	...
	if (not bTriggerSpellAbsorb) and IsValid(hTarget) and hTarget:IsAlive() then
		hTarget:AddNewModifier(
			hCaster,
			self,
			"modifier_bane_fiends_grip_lua",
			{ duration = self:GetChannelTime() * hTarget:GetStatusResistanceFactor(hCaster) }
		)
	...
	if IsValid(hTarget) then
		...
		if self.tTargets and type(self.tTargets) == "table" then
			for index, bIsTarget in pairs(self.tTargets) do
				...
				if
					IsValid(hLoopTarget)
					and hLoopTarget:IsAlive()
					and hLoopTarget:HasModifier("modifier_bane_fiends_grip_lua")
				then
...
function modifier_bane_fiends_grip_lua:OnIntervalThink()
	...
	if IsValid(hParent) and IsValid(hCaster) and hCaster:IsAlive() and hParent:IsAlive() then
		if self == buffs[1] then
			...
			hParent:Script_ReduceMana(
				hParent:GetMaxMana() * self.fiend_grip_mana_drain * 0.01 * self.fiend_grip_tick_interval,
				self:GetAbility()
			)
		end
	else
