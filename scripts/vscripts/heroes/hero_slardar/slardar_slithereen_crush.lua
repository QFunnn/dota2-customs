...
LinkLuaModifier(
	"slardar_slithereen_crush_custom_debuff",
	"heroes/hero_slardar/slardar_slithereen_crush",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"slardar_slithereen_crush_custom_puddle",
	"heroes/hero_slardar/slardar_slithereen_crush",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"slardar_slithereen_crush_custom_puddle_buff",
	"heroes/hero_slardar/slardar_slithereen_crush",
	LUA_MODIFIER_MOTION_NONE
)
...
function slardar_slithereen_crush_custom:OnSpellStart()
		...
		if self:GetCaster():HasShard() then
			...
			if corrosive_ability then
				enemy:AddNewModifier(
					...
					{ duration = self:GetSpecialValueFor("shard_amp_duration") * (1 - enemy:GetStatusResistance()) }
				)
			end
		end
	end
	...
	self:GetCaster():EmitSound("Hero_Slardar.Slithereen_Crush")
...
function slardar_slithereen_crush_custom_debuff:IsHidden()
	return true
end
...
function slardar_slithereen_crush_custom_puddle:IsHidden()
	return true
end
function slardar_slithereen_crush_custom_puddle:IsPurgable()
	return false
end
...
function slardar_slithereen_crush_custom_puddle_buff:IsHidden()
	return true
end
...
function slardar_slithereen_crush_custom_puddle_buff:OnCreated()
	if not IsServer() then
		return
	end
	...
	Parent:AddNewModifier(Parent, nil, "modifier_slardar_puddle", {})
end

function slardar_slithereen_crush_custom_puddle_buff:OnDestroy()
	if not IsServer() then
		return
	end
	...
	Parent:RemoveModifierByName("modifier_slardar_puddle")
end
