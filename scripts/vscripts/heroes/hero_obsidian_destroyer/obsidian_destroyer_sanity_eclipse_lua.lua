...
function obsidian_destroyer_sanity_eclipse_lua:OnSpellStart()
	...
	local particle_damage = ParticleManager:GetParticleReplacement(
	...
	local damage_mana_multiplier = self:GetSpecialValueFor("damage_multiplier") -- + caster:FindTalentValue("special_bonus_unique_outworld_devourer_4") 天赋加成由kv实现了
	...
	for _, enemy in pairs(enemies) do
		if
			(not enemy:IsInvulnerable() and not enemy:IsOutOfGame())
			or enemy:HasModifier("modifier_obsidian_destroyer_astral_imprisonment_prison")
		then
			...
			local eclipse_damage_particle =
				ParticleManager:CreateParticle(self.particle_damage, PATTACH_ABSORIGIN_FOLLOW, enemy)
