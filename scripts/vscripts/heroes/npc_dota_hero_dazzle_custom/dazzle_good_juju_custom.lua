...
LinkLuaModifier(
	"modifier_dazzle_good_juju_custom",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_good_juju_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_good_juju_custom_debuff",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_good_juju_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_good_juju_custom_handler",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_good_juju_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_dazzle_good_juju_custom_handler:IsHidden()
	return true
end
function modifier_dazzle_good_juju_custom_handler:IsPurgeException()
	return false
end
function modifier_dazzle_good_juju_custom_handler:IsPurgable()
	return false
end
function modifier_dazzle_good_juju_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_dazzle_good_juju_custom_handler:OnAbilityFullyCast(params)
	if IsServer() then
		...
		if self.exceptions[hAbility:GetAbilityName()] ~= nil then
			return
		end
...
function modifier_dazzle_good_juju_custom:IsHidden()
	return true
end
function modifier_dazzle_good_juju_custom:IsPurgable()
	return false
end
function modifier_dazzle_good_juju_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_dazzle_4") then
		self.radius = self.radius
			+ self:GetAbility().modifier_dazzle_4[self:GetCaster():GetTalentLevel("modifier_dazzle_4")]
	end
	...
	local particle =
		ParticleManager:CreateParticle("particles/dazzle_effect.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_dazzle_good_juju_custom_debuff:GetModifierPropertyRestorationAmplification()
	if not self:GetCaster():HasModifier("modifier_dazzle_6") then
		return
	end
...
function modifier_dazzle_good_juju_custom_debuff:OnIntervalThink()
	...
	local damage = self:GetAbility():GetSpecialValueFor("damage")
		+ (self:GetCaster():GetMaxHealth() / 100 * self:GetAbility():GetSpecialValueFor("damage_percent"))
	ApplyDamage({
