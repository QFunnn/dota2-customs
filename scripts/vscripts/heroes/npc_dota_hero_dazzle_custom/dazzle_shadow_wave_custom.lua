...
LinkLuaModifier(
	"modifier_dazzle_shadow_wave_custom_slow",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_shadow_wave_custom",
...
function dazzle_shadow_wave_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_dazzle.vpcf",
		context
	)
end
	...
	if self:GetCaster():HasModifier("modifier_dazzle_15") then
		self.damage = self.damage
			+ (
				...
				/ 100
				* self.modifier_dazzle_15[self:GetCaster():GetTalentLevel("modifier_dazzle_15")]
			)
	end
	self.damageTable = {
		attacker = caster,
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_dazzle_19") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_dazzle_shadow_wave_custom_slow",
				{ duration = self.modifier_dazzle_19_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
	end
	local jump = jumps - 1
	...
	if source ~= self:GetCaster() then
		...
		if self:GetCaster():HasModifier("modifier_dazzle_19") then
			source:AddNewModifier(
				...
				self,
				"modifier_dazzle_shadow_wave_custom_slow",
				{ duration = self.modifier_dazzle_19_duration * (1 - source:GetStatusResistance()) }
			)
		end
	end
	local dazzle_innate_weave_custom = self:GetCaster():FindAbilityByName("dazzle_innate_weave_custom")
		...
		if self:GetCaster():HasModifier("modifier_dazzle_15") then
			damage = damage
				+ (
					self:GetCaster():GetMaxMana()
					/ 100
					* dazzle_shadow_wave_custom.modifier_dazzle_15[self:GetCaster():GetTalentLevel("modifier_dazzle_15")]
				)
		end
			...
			if unit:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
				...
				if self:GetCaster():HasModifier("modifier_dazzle_19") then
					unit:AddNewModifier(
						...
						"modifier_dazzle_shadow_wave_custom_slow",
						{
							duration = dazzle_shadow_wave_custom.modifier_dazzle_19_duration
...
function modifier_dazzle_healing_step:IsPurgable()
	return false
end
function modifier_dazzle_healing_step:IsHidden()
	return self:GetStackCount() == 0
end
function modifier_dazzle_healing_step:IsPurgeException()
	return false
end
function modifier_dazzle_healing_step:RemoveOnDeath()
	return false
end