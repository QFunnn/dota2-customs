...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dazzle_custom/dazzle_shallow_grave_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_shallow_grave_custom_handler",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_shallow_grave_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_dazzle_shallow_grave_custom:IsPurgable()
	return false
end
...
function modifier_dazzle_shallow_grave_custom:OnCreated(kv)
	...
	self.modifier_dazzle_nothl_projection_soul_clone = self:GetCaster()
		:HasModifier("modifier_dazzle_nothl_projection_soul_clone")
...
function modifier_dazzle_shallow_grave_custom:OnDestroy()
	...
	if self.modifier_dazzle_nothl_projection_soul_clone then
		local shallow_grave_heal =
			self:GetCaster():FindAbilityByName("dazzle_nothl_projection"):GetSpecialValueFor("shallow_grave_heal")
		self:GetParent():Heal(shallow_grave_heal, self:GetCaster())
	end
end

modifier_dazzle_shallow_grave_custom_handler = class({})
function modifier_dazzle_shallow_grave_custom_handler:IsPurgable()
	return false
end
function modifier_dazzle_shallow_grave_custom_handler:IsHidden()
	return true
end
function modifier_dazzle_shallow_grave_custom_handler:IsPurgeException()
	return false
end
function modifier_dazzle_shallow_grave_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_dazzle_shallow_grave_custom_handler:GetMinHealth()
	if not self:GetParent():HasModifier("modifier_dazzle_11") then
		return
	end
	if self:GetParent():HasModifier("modifier_dazzle_1") then
		return
	end
	if self:GetParent():HasModifier("modifier_dazzle_16") then
		return
	end
	if self:GetParent():HasModifier("modifier_dazzle_nothl_projection_soul_clone") then
		return
	end
...
function modifier_dazzle_shallow_grave_custom_handler:OnTakeDamage(params)
	...
	if self:GetParent():GetHealthPercent() > self:GetAbility().modifier_dazzle_11 then
		return
	end
	if self:GetParent():PassivesDisabled() then
		return
	...
	self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_dazzle_shallow_grave", { duration = duration })
	local dazzle_innate_weave_custom = self:GetCaster():FindAbilityByName("dazzle_innate_weave_custom")
	if dazzle_innate_weave_custom and dazzle_innate_weave_custom:GetLevel() > 0 then
