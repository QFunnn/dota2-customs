...
LinkLuaModifier(
	"modifier_dazzle_poison_touch_custom",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_poison_touch_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_poison_touch_custom_handler",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_poison_touch_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_poison_touch_custom_handler_cooldown",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_poison_touch_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function dazzle_poison_touch_custom:OnSpellStart()
	...
	local enemies = self:FindUnitsInCone(
		...
		target:GetOrigin(),
		caster:GetOrigin(),
		caster:GetOrigin() + direction * distance,
		start_radius,
		end_radius,
		nil,
...
function dazzle_poison_touch_custom:OnProjectileHit(target, location)
	...
	if
		self:GetCaster():HasModifier("modifier_dazzle_nothl_projection_soul_clone")
		or self:GetCaster():HasModifier("modifier_dazzle_10")
	then
		...
		if
			not self:GetCaster():HasModifier("modifier_dazzle_nothl_projection_soul_clone")
			and self:GetCaster():HasModifier("modifier_dazzle_10")
		then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_hexxed",
			{ duration = duration_hex * (1 - target:GetStatusResistance()) }
		)
...
function dazzle_poison_touch_custom:FindUnitsInCone(
	nTeamNumber,
	vCenterPos,
	vStartPos,
	vEndPos,
	...
	if self:GetCaster():HasModifier("modifier_dazzle_9") then
		damage = damage
			+ (
				...
				/ 100
				* self:GetAbility().modifier_dazzle_9[self:GetCaster():GetTalentLevel("modifier_dazzle_9")]
			)
	end
...
function modifier_dazzle_poison_touch_custom:OnAttackLanded(params)
	...
	if
		params.attacker == self:GetCaster()
		or params.attacker:HasModifier("modifier_dazzle_nothl_projection_soul_clone")
	then
		is_valid = true
	end
...
function modifier_dazzle_poison_touch_custom_handler:IsPurgable()
	return false
end
function modifier_dazzle_poison_touch_custom_handler:IsHidden()
	return true
end
function modifier_dazzle_poison_touch_custom_handler:IsPurgeException()
	return false
end
function modifier_dazzle_poison_touch_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_dazzle_poison_touch_custom_handler:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_dazzle_8") then
		...
		if params.attacker:HasModifier("modifier_dazzle_poison_touch_custom_handler_cooldown") then
			return
		end
		if self:GetParent():HasModifier("modifier_dazzle_18") then
			return
		end
		if self:GetParent():HasModifier("modifier_dazzle_1") then
			return
		end
		...
		params.attacker:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_dazzle_poison_touch_custom_handler_cooldown",
			{ duration = self:GetAbility().modifier_dazzle_8_cooldown }
		)
...
function modifier_dazzle_poison_touch_custom_handler_cooldown:IsPurgable()
	return false
end
function modifier_dazzle_poison_touch_custom_handler_cooldown:IsHidden()
	return true
end
function modifier_dazzle_poison_touch_custom_handler_cooldown:IsPurgeException()
	return false
end
function modifier_dazzle_poison_touch_custom_handler_cooldown:RemoveOnDeath()
	return false
end