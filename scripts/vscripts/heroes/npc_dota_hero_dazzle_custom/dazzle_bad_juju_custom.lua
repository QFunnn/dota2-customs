...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_dazzle_custom/dazzle_bad_juju_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_dazzle_bad_juju_custom_cost",
	"heroes/npc_dota_hero_dazzle_custom/dazzle_bad_juju_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function dazzle_bad_juju_custom:OnSpellStart()
	...
	if modifier_dazzle_bad_juju_custom then
		modifier_dazzle_bad_juju_custom = self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_dazzle_bad_juju_custom",
			{ duration = self:GetSpecialValueFor("mana_cost_increase_duration") }
		)
		if modifier_dazzle_bad_juju_custom:GetStackCount() < self:GetSpecialValueFor("max_stacks") then
			...
			local modifier_dazzle_bad_juju_custom_cost = self:GetCaster()
				:FindModifierByName("modifier_dazzle_bad_juju_custom_cost")
...
function modifier_dazzle_bad_juju_custom:IsPurgable()
	return false
end
function modifier_dazzle_bad_juju_custom:IsPurgeException()
	return false
end
...
function modifier_dazzle_bad_juju_custom_cost:IsPurgable()
	return false
end
function modifier_dazzle_bad_juju_custom_cost:IsPurgeException()
	return false
end
function modifier_dazzle_bad_juju_custom_cost:IsHidden()
	return true
end