...
--------------------------------------------------------------------------------
function modifier_doom_bringer_devour_datadriven:IsPurgable()
	return false
end
function modifier_doom_bringer_devour_datadriven:IsDebuff()
	return true
end
function modifier_doom_bringer_devour_datadriven:IsHidden()
	return false
end
...
function modifier_doom_bringer_devour_datadriven:OnDestroy(params)
	if IsServer() then
		...
		if caster and caster:IsAlive() then
			...
			self:GetAbility():GetCaster():ModifyGoldFiltered(self.bonus_gold, true, 0)
		end
	end
end