...
function modifier_skywrath_mage_shard_lua:IsHidden()
	return true
end
function modifier_skywrath_mage_shard_lua:IsDebuff()
	return false
end
function modifier_skywrath_mage_shard_lua:IsPurgable()
	return false
end

function modifier_skywrath_mage_shard_lua:GetAttributes()
...
function modifier_skywrath_mage_shard_lua:OnAbilityFullyCast(event)
	...
	if cast_ability:IsToggle() then
		return
	end
	--без кулдауна не срабатывает
	if cast_ability:GetCooldownTimeRemaining() < 0.05 then
		return
	end

	if self.ability_exceptions and self.ability_exceptions[cast_ability:GetAbilityName()] then
		return
	end
...
function modifier_skywrath_mage_shard_bonus_counter_lua:GetTexture()
	return "skywrath_mage_arcane_bolt"
end
function modifier_skywrath_mage_shard_bonus_counter_lua:IsHidden()
	return self:GetStackCount() == 0
end
function modifier_skywrath_mage_shard_bonus_counter_lua:IsDebuff()
	return false
end
function modifier_skywrath_mage_shard_bonus_counter_lua:IsPurgable()
	return false
end
function modifier_skywrath_mage_shard_bonus_counter_lua:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
