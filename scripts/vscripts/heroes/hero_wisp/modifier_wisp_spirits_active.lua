...
function modifier_wisp_spirits_active:RemoveOnDeath()
	return true
end
function modifier_wisp_spirits_active:IsPurgable()
	return false
end
...
function modifier_wisp_spirits_active:AddSpirit()
	...
	if self.summoned_spirits_count >= self.spirits_count then
		...
		if IsValid(self.parent) and self.parent:HasScepter() then
...
function modifier_wisp_spirits_active:ProcessSpiritMovement(index, metadata, parent_origin)
	...
	local spirit_coord =
		Vector(parent_origin.x + self.current_range * angle_sin, parent_origin.y + self.current_range * angle_cos, 0)
...
function modifier_wisp_spirits_active:OnRemoved()
	...
	if self.parent then
		if self.ability and not self.ability:IsNull() then
			self.parent:SwapAbilities(self.ability:GetAbilityName(), "wisp_spirits_in_lua", true, false)
		end
...
function modifier_wisp_spirits_active:GetSpiritParticle(position)
	local particle =
		ParticleManager:CreateParticle("particles/units/heroes/hero_wisp/wisp_guardian.vpcf", PATTACH_CUSTOMORIGIN, nil)
...
function modifier_wisp_spirits_creep:IsHidden()
	return true
end
function modifier_wisp_spirits_creep:IsPurgable()
	return false
end