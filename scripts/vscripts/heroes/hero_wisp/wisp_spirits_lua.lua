...
LinkLuaModifier(
	"modifier_wisp_spirits_active",
	"heroes/hero_wisp/modifier_wisp_spirits_active",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wisp_spirits_creep",
	"heroes/hero_wisp/modifier_wisp_spirits_active",
	LUA_MODIFIER_MOTION_NONE
)
...
function wisp_spirits_lua:Spawn()
	...
	self.scepter_gained_listener = EventDriver:Listen("Hero:ScepterReceived", function(self, event)
		if event.hero ~= self:GetCaster() then
			return
		end
...
function wisp_spirits_lua:OnSpellStart()
	...
	if caster:HasModifier("modifier_wisp_spirits_active") then
		caster:RemoveModifierByName("modifier_wisp_spirits_active")
	end
	...
	caster:SwapAbilities(self:GetAbilityName(), "wisp_spirits_in_lua", false, true)
...
function wisp_spirits_in_lua:ResetToggleOnRespawn()
	return true
end