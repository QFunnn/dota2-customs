...
LinkLuaModifier(
	"modifier_nyx_assassin_mana_burn_custom",
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_mana_burn_custom",
...
function modifier_nyx_assassin_mana_burn_custom:IsHidden()
	return true
end
function modifier_nyx_assassin_mana_burn_custom:IsPurgable()
	return false
end
function modifier_nyx_assassin_mana_burn_custom:IsPurgeException()
	return false
end
function modifier_nyx_assassin_mana_burn_custom:RemoveOnDeath()
	return false
end
function modifier_nyx_assassin_mana_burn_custom:OnTakeDamage(params)
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_19") then
		mana_burn_pct = mana_burn_pct
			+ self:GetAbility().modifier_nyx_assassin_19[self:GetCaster():GetTalentLevel("modifier_nyx_assassin_19")]
	end
