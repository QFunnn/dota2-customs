...
LinkLuaModifier(
	"modifier_nyx_assassin_jolt_custom_debuff",
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_jolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nyx_assassin_jolt_custom",
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_jolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function nyx_assassin_jolt_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_12") then
		...
		local illusions = CreateIllusions(
			...
			self:GetCaster(),
			{
				duration = self.modifier_nyx_assassin_12_illsuion_duration[self:GetCaster()
					:GetTalentLevel("modifier_nyx_assassin_12")],
...
function nyx_assassin_jolt_custom:CastJolt(target)
	...
	local old_damage = 0
	local modifier_nyx_assassin_jolt_custom_debuff =
		target:FindModifierByName("modifier_nyx_assassin_jolt_custom_debuff")
...
function modifier_nyx_assassin_jolt_custom:IsHidden()
	return true
end
function modifier_nyx_assassin_jolt_custom:IsPurgable()
	return false
end
function modifier_nyx_assassin_jolt_custom:IsPurgeException()
	return false
end
function modifier_nyx_assassin_jolt_custom:RemoveOnDeath()
	return false
end
...
function modifier_nyx_assassin_jolt_custom:OnTakeDamage(params)
	...
	params.unit:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_nyx_assassin_jolt_custom_debuff",
		{ duration = self.damage_echo_duration, damage = params.damage }
	)
end

modifier_nyx_assassin_jolt_custom_debuff = class({})
function modifier_nyx_assassin_jolt_custom_debuff:IsHidden()
	return true
end
function modifier_nyx_assassin_jolt_custom_debuff:IsPurgable()
	return false
end
function modifier_nyx_assassin_jolt_custom_debuff:IsPurgeException()
	return false
end
function modifier_nyx_assassin_jolt_custom_debuff:RemoveOnDeath()
	return false
end
