...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_vendetta_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nyx_assassin_vendetta_custom_debuff",
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_vendetta_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nyx_assassin_vendetta_custom_invisible",
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_vendetta_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function nyx_assassin_vendetta_custom:OnSpellStart()
	...
	local modifier_nyx_assassin_burrow_custom = self:GetCaster()
		:FindModifierByName("modifier_nyx_assassin_burrow_custom")
	if modifier_nyx_assassin_burrow_custom then
		modifier_nyx_assassin_burrow_custom:Destroy()
	end
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_nyx_assassin_vendetta_custom", { duration = duration })
	if self:GetCaster():HasModifier("modifier_nyx_assassin_20") then
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_nyx_assassin_vendetta_custom_invisible",
			{ duration = self.modifier_nyx_assassin_20[self:GetCaster():GetTalentLevel("modifier_nyx_assassin_20")] }
		)
	end
end

modifier_nyx_assassin_vendetta_custom = class({})
function modifier_nyx_assassin_vendetta_custom:IsPurgable()
	return false
end
function modifier_nyx_assassin_vendetta_custom:IsPurgeException()
	return false
end
...
function modifier_nyx_assassin_vendetta_custom:OnAttackLanded(params)
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_17") then
		target:AddNewModifier(
			...
			self:GetAbility(),
			"modifier_nyx_assassin_vendetta_custom_debuff",
			{ duration = self:GetAbility().modifier_nyx_assassin_17_duration }
		)
	end
	target:AddNewModifier(
		self:GetCaster(),
		...
		{ duration = self:GetAbility():GetSpecialValueFor("break_duration") * (1 - target:GetStatusResistance()) }
	)
	local particle = ParticleManager:CreateParticle(
