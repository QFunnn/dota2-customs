...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_nyx_assassin_custom/nyx_assassin_spiked_carapace_custom",
...
function modifier_nyx_assassin_spiked_carapace_custom:IsPurgable()
	return false
end
...
function modifier_nyx_assassin_spiked_carapace_custom:OnCreated()
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_4") then
		self.damage_reflect_pct = self.damage_reflect_pct
			+ self:GetAbility().modifier_nyx_assassin_4[self:GetCaster():GetTalentLevel("modifier_nyx_assassin_4")]
	end
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_burrow_custom") then
		...
		if nyx_assassin_burrow_custom then
			local units = FindUnitsInRadius(
				...
				self:GetCaster():GetAbsOrigin(),
				nil,
				nyx_assassin_burrow_custom:GetSpecialValueFor("carrapase_radius"),
...
function modifier_nyx_assassin_spiked_carapace_custom:GetAbsoluteNoDamagePhysical(params)
	...
	if self:GetParent():HasModifier("modifier_nyx_assassin_6") then
		return
	end
...
function modifier_nyx_assassin_spiked_carapace_custom:ReflectDamage(target, damage, params)
	...
	if target:IsHero() and not self.carapaced_units[target:entindex()] then
		...
		if params ~= nil then
			ApplyDamage({
				victim = target,
				damage = params.original_damage / 100 * self.damage_reflect_pct,
...
function modifier_nyx_assassin_spiked_carapace_custom:GetModifierTotal_ConstantBlock(params)
	...
	if not self:GetParent():HasModifier("modifier_nyx_assassin_6") then
		return
	end
...
function modifier_nyx_assassin_spiked_carapace_custom:GetModifierAttackSpeedBonus_Constant()
	if not self:GetCaster():HasModifier("modifier_nyx_assassin_8") then
		return
	end
