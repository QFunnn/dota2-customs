...
function nyx_assassin_impale_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_nyx_assassin.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_nyx_assassin.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_nyx_assassin.vpcf",
		context
	)
end
		...
		if RollPercentage(self.modifier_nyx_assassin_5[self:GetCaster():GetTalentLevel("modifier_nyx_assassin_5")]) then
			Timers:CreateTimer(0.1, function()
				local particle = ParticleManager:CreateParticle(
					"particles/items2_fx/refresher_red.vpcf",
...
function nyx_assassin_impale_custom:OnProjectileHit(target, location)
	...
	if self:GetCaster():HasModifier("modifier_nyx_assassin_1") then
		impale_damage = impale_damage
			+ (
				self:GetCaster():GetStrength()
				/ 100
				* self.modifier_nyx_assassin_1[self:GetCaster():GetTalentLevel("modifier_nyx_assassin_1")]
			)
	end
	...
	if not target:IsDebuffImmune() then
		target:AddNewModifier(
			...
			{ duration = 0.5 * (1 - target:GetStatusResistance()), distance = 0, height = 250, IsStun = true }
		)
	end
