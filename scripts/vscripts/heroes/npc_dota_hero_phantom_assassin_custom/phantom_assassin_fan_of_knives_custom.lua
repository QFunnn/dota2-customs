...
LinkLuaModifier(
	"modifier_phantom_assassin_fan_of_knives_custom_thinker",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_fan_of_knives_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_fan_of_knives_custom",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_fan_of_knives_custom",
	LUA_MODIFIER_MOTION_NONE
)
		...
		if not hit_already and not enemy:IsMagicImmune() then
			local damage = self:GetAbility():GetSpecialValueFor("damage")
				+ (
					...
					/ 100
					* self:GetAbility():GetSpecialValueFor("damage_intellect")
				)
			ApplyDamage({
				victim = enemy,
...
function modifier_phantom_assassin_fan_of_knives_custom:IsHidden()
	return false
end
function modifier_phantom_assassin_fan_of_knives_custom:IsPurgable()
	return true
end
function modifier_phantom_assassin_fan_of_knives_custom:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_phantom_assassin_fan_of_knives_custom:GetEffectName()
