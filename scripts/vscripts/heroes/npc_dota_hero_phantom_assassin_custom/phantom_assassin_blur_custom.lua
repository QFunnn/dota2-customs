...
LinkLuaModifier(
	"modifier_phantom_assassin_blur_custom",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_blur_custom",
...
function phantom_assassin_blur_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_phantom_assassin.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_phantom_assassin.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_phantom_assassin.vpcf",
		context
	)
end
...
function phantom_assassin_blur_custom:GetBehavior()
	if self:GetCaster():HasModifier("modifier_phantom_assassin_20") then
		return DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_IMMEDIATE + DOTA_ABILITY_BEHAVIOR_NOT_LEARNABLE
	end
...
function phantom_assassin_blur_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_phantom_assassin_blur_custom_active", { duration = duration })
...
function modifier_phantom_assassin_blur_custom:IsHidden()
	return true
end
function modifier_phantom_assassin_blur_custom:IsPurgable()
	return false
end

modifier_phantom_assassin_blur_custom_active = class({})

function modifier_phantom_assassin_blur_custom_active:IsPurgable()
	return false
end
