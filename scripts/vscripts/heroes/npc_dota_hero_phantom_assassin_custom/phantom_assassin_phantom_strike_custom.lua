...
LinkLuaModifier(
	"modifier_custom_phantom_assassin_phantom_strike",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_phantom_strike_custom",
...
function phantom_assassin_phantom_strike_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_phantom_assassin_11") then
		duration = duration
			+ self.modifier_phantom_assassin_11[self:GetCaster():GetTalentLevel("modifier_phantom_assassin_11")]
	end
	local blinkPosition
	...
	if self:GetCaster():HasModifier("modifier_phantom_assassin_8") then
		for _, enemy in pairs(enemies) do
			local damage = self.modifier_phantom_assassin_8_base
				+ (
					self:GetCaster():GetAgility()
					/ 100
					* self.modifier_phantom_assassin_8_perc[self:GetCaster()
						:GetTalentLevel("modifier_phantom_assassin_8")]
				)
			ApplyDamage({
				victim = enemy,
...
function modifier_custom_phantom_assassin_phantom_strike:GetModifierProperty_PhysicalLifesteal(params)
	if not self:GetParent():HasModifier("modifier_phantom_assassin_11") then
		return
	end
	return self:GetAbility().modifier_phantom_assassin_11_lifesteal[self:GetCaster()
		:GetTalentLevel("modifier_phantom_assassin_11")]
end