...
LinkLuaModifier(
	"modifier_phantom_assassin_stifling_dagger_custom",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_stifling_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_stifling_dagger_custom_debuff",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_stifling_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_stifling_dagger_custom_talent",
	"heroes/npc_dota_hero_phantom_assassin_custom/phantom_assassin_stifling_dagger_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function phantom_assassin_stifling_dagger_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_phantom_assassin_15") then
		...
		for _, enemy in pairs(enemies) do
			...
			if
				secondary_knives_thrown
				>= self.modifier_phantom_assassin_15[self:GetCaster():GetTalentLevel("modifier_phantom_assassin_15")]
			then
				break
			end
		end
...
function modifier_phantom_assassin_stifling_dagger_custom_talent:IsHidden()
	return true
end
function modifier_phantom_assassin_stifling_dagger_custom_talent:IsPurgable()
	return false
end
function modifier_phantom_assassin_stifling_dagger_custom_talent:RemoveOnDeath()
	return false
end
...
function modifier_phantom_assassin_stifling_dagger_custom_talent:OnAbilityExecuted(params)
	...
	local enemies = FindUnitsInRadius(
		...
		self:GetCaster():GetAbsOrigin(),
		nil,
		self:GetAbility().modifier_phantom_assassin_21,
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,
		FIND_ANY_ORDER,
...
function phantom_assassin_stifling_dagger_custom:OnProjectileHit(target, location)
	...
	local modifier = self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_phantom_assassin_stifling_dagger_custom", {})
...
function modifier_phantom_assassin_stifling_dagger_custom:IsHidden()
	return true
end
function modifier_phantom_assassin_stifling_dagger_custom:IsPurgable()
	return false
end
function modifier_phantom_assassin_stifling_dagger_custom:RemoveOnDeath()
	return false
end
...
function modifier_phantom_assassin_stifling_dagger_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_phantom_assassin_9") then
		self.attack_factor = self.attack_factor
			+ self:GetAbility().modifier_phantom_assassin_9[self:GetCaster()
				:GetTalentLevel("modifier_phantom_assassin_9")]
	end
end
