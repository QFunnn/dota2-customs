...
function necrolyte_death_pulse_custom:GetManaCost(iLevel)
	...
	if self:GetCaster():HasModifier("modifier_necrolyte_9") then
		bonus = self.BaseClass.GetManaCost(self, iLevel)
			/ 100
			* self.modifier_necrolyte_9_manacost[self:GetCaster():GetTalentLevel("modifier_necrolyte_9")]
	end
...
function necrolyte_death_pulse_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_necrolyte_15") then
		area_of_effect = area_of_effect
			+ self.modifier_necrolyte_15_radius[self:GetCaster():GetTalentLevel("modifier_necrolyte_15")]
	end
	...
	if self:GetCaster():HasModifier("modifier_necrolyte_9") then
		damage = damage
			+ (
				...
				/ 100
				* self.modifier_necrolyte_9_damage[self:GetCaster():GetTalentLevel("modifier_necrolyte_9")]
			)
	end
	if self:GetCaster():HasModifier("modifier_necrolyte_20") then
