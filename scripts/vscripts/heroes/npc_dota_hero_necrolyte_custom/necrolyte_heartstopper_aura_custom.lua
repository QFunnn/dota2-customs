...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_necrolyte_custom/necrolyte_heartstopper_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_necrolyte_heartstopper_aura_custom_damage",
	"heroes/npc_dota_hero_necrolyte_custom/necrolyte_heartstopper_aura_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_necrolyte_heartstopper_aura_custom_damage:IsPurgable()
	return false
end
...
function modifier_necrolyte_heartstopper_aura_custom_damage:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_necrolyte_6") then
		aura_damage = aura_damage
			+ self:GetAbility().modifier_necrolyte_6[self:GetCaster():GetTalentLevel("modifier_necrolyte_6")]
	end
	local damage = (self:GetParent():GetMaxHealth() * aura_damage / 100)
	if self:GetCaster():HasModifier("modifier_necrolyte_1") then
		if
			not self:GetParent():IsHero()
			or self:GetCaster():GetStrength() >= self:GetAbility().modifier_necrolyte_1_strength_count_to_heroes
		then
			damage = damage
				+ (self:GetCaster():GetStrength() / 100 * self:GetAbility().modifier_necrolyte_1_creep_damage)
		end
	end
	if self:GetCaster():HasModifier("modifier_necrolyte_3") and self:GetAbility():GetToggleState() then
...
function modifier_necrolyte_heartstopper_aura_custom_damage:GetModifierPhysicalArmorBonus()
	if not self:GetCaster():HasModifier("modifier_necrolyte_13") then
		return
	end
	if self:GetParent() == self:GetCaster() then
		return
...
function modifier_necrolyte_heartstopper_aura_custom_damage:GetModifierMagicalResistanceBonus()
	if not self:GetCaster():HasModifier("modifier_necrolyte_18") then
		return
	end
	if self:GetParent() == self:GetCaster() then
		return
...
function modifier_necrolyte_heartstopper_aura_custom_damage:GetModifierPropertyRestorationAmplification()
	if not self:GetCaster():HasModifier("modifier_necrolyte_5") then
		return
	end
	if self:GetParent() == self:GetCaster() then
		return
