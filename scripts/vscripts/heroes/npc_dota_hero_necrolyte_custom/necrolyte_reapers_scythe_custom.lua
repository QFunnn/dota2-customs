...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_necrolyte_custom/necrolyte_reapers_scythe_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_necrolyte_reapers_scythe_custom_buff",
	"heroes/npc_dota_hero_necrolyte_custom/necrolyte_reapers_scythe_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function necrolyte_reapers_scythe_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_necrolyte.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_necrolyte.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_necrolyte.vpcf",
		context
	)
end
...
function modifier_necrolyte_reapers_scythe_custom:IsPurgable()
	return false
end
function modifier_necrolyte_reapers_scythe_custom:IsPurgeException()
	return false
end
function modifier_necrolyte_reapers_scythe_custom:GetAttributes()
...
function modifier_necrolyte_reapers_scythe_custom:OnRemoved()
	...
	if not self:GetParent():IsAlive() then
		local modifier_necrolyte_reapers_scythe_custom_buff =
			self:GetCaster():FindModifierByName("modifier_necrolyte_reapers_scythe_custom_buff")
	...
	if not self:GetParent():IsHero() then
		...
		if
			self:GetParent():GetUnitName() ~= "npc_dota_creature_barrel"
			and self:GetParent():GetUnitName() ~= "small_barrel"
			and self:GetParent():GetUnitName() ~= "small_barrel_side"
			and self:GetParent():GetUnitName() ~= "big_barrel"
			and self:GetParent():GetUnitName() ~= "boss_1_pve"
			and self:GetParent():GetUnitName() ~= "boss_2_pve"
			and self:GetParent():GetUnitName() ~= "boss_3_pve"
			and self:GetParent():GetUnitName() ~= "boss_4_pve"
			and self:GetParent():GetUnitName() ~= "boss_5_pve"
			and self:GetParent():GetUnitName() ~= "boss_6_pve"
			and self:GetParent():GetUnitName() ~= "boss_3"
			and self:GetParent():GetUnitName() ~= "boss_5"
		then
			self:GetParent():Kill(self:GetAbility(), self:GetCaster())
		end
	else
		self:GetParent()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", { duration = FrameTime() })
...
function modifier_necrolyte_reapers_scythe_custom_buff:IsPurgable()
	return false
end
function modifier_necrolyte_reapers_scythe_custom_buff:IsPurgeException()
	return false
end
function modifier_necrolyte_reapers_scythe_custom_buff:RemoveOnDeath()
	return false
end
function modifier_necrolyte_reapers_scythe_custom_buff:IsHidden()
