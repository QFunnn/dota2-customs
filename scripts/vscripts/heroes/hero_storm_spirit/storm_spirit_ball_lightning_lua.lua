...
storm_spirit_ball_lightning_lua = class({}) ---@class storm_spirit_ball_lightning_lua : CDOTA_Ability_Lua

LinkLuaModifier(
	"modifier_storm_spirit_ball_lightning_lua",
	"heroes/hero_storm_spirit/storm_spirit_ball_lightning_lua",
...
function storm_spirit_ball_lightning_lua:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		"modifier_storm_spirit_ball_lightning_lua", -- modifier name
		{
			x = point.x,
			y = point.y,
			duration = (point - caster:GetAbsOrigin()):Length2D()
				/ self:GetSpecialValueFor("ball_lightning_move_speed"),
		} -- kv
	)
end

--------------------------------------------------------------------------------
modifier_storm_spirit_ball_lightning_lua = class({}) ---@class modifier_storm_spirit_ball_lightning_lua : CDOTA_Modifier_Lua
...
function modifier_storm_spirit_ball_lightning_lua:OnRefresh(kv) end

function modifier_storm_spirit_ball_lightning_lua:OnRemoved() end
...
function modifier_storm_spirit_ball_lightning_lua:UpdateHorizontalMotion(me, dt)
	...
	if self.talent_remnant_interval > 0 then
		...
		if self.talent_travel >= self.talent_remnant_interval then
			...
			if remnant ~= nil then
				...
				logger:Log(remnant.OnSpellStart == nil)
	...
	if travel - self.travel_total < self.tick then
		return
	end
