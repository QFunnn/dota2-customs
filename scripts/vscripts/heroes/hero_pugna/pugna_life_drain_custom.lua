...
LinkLuaModifier(
	"modifier_pugna_life_drain_custom",
	"heroes/hero_pugna/pugna_life_drain_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pugna_life_drain_custom_debuff",
	"heroes/hero_pugna/pugna_life_drain_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_pugna_life_drain_custom_buff",
	"heroes/hero_pugna/pugna_life_drain_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_pugna_life_drain_custom:GetPriority()
	return MODIFIER_PRIORITY_HIGH
end
function modifier_pugna_life_drain_custom:IsHidden()
	return true
end
function modifier_pugna_life_drain_custom:IsPurgable()
	return false
end
...
function modifier_pugna_life_drain_custom:OnCreated()
	...
	if IsServer() then
		...
		if self.is_ally then
			self.particle_drain_fx = ParticleManager:CreateParticle(
		...
		Timers:CreateTimer(self.tick_rate, function()
			if mod and not mod:IsNull() then
				mod:StartIntervalThink(self.tick_rate)
			end
		end)
	else
...
function modifier_pugna_life_drain_custom:OnIntervalThink()
	if IsServer() then
		if
			...
			and not self.parent:IsStrongIllusion()
		then
			...
			if self:GetCaster():HasScepter() then
				...
				if modifier_friendly then
					self:GetCaster():AddNewModifier(
						...
						self:GetAbility(),
						"modifier_pugna_life_drain_custom_buff",
						{ duration = self:GetAbility():GetSpecialValueFor("spell_amp_drain_duration") }
					)
					modifier_friendly:SetStackCount(math.min(modifier_friendly:GetStackCount() + 2, 100))
				else
					local modifier_friendly = self:GetCaster():AddNewModifier(
						self:GetCaster(),
						self:GetAbility(),
						"modifier_pugna_life_drain_custom_buff",
						{ duration = self:GetAbility():GetSpecialValueFor("spell_amp_drain_duration") }
				...
				if modifier_enemy then
					...
					local modifier_enemy = self:GetParent():AddNewModifier(
						self:GetCaster(),
						self:GetAbility(),
						"modifier_pugna_life_drain_custom_debuff",
						{ duration = self:GetAbility():GetSpecialValueFor("spell_amp_drain_duration") }
