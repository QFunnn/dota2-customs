...
LinkLuaModifier(
	"modifier_slark_pounce_custom",
	"heroes/npc_dota_hero_slark_custom/slark_pounce_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_pounce_custom_debuff",
	"heroes/npc_dota_hero_slark_custom/slark_pounce_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_pounce_arc",
	"heroes/npc_dota_hero_slark_custom/slark_pounce_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_slark_pounce_leashed",
	"heroes/npc_dota_hero_slark_custom/slark_pounce_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_slark_pounce_custom_thinker",
	"heroes/npc_dota_hero_slark_custom/slark_pounce_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function slark_pounce_custom:StartShadowClones(point)
	if self:GetCaster():GetTalentLevel("modifier_slark_9") > 0 then
		...
		local unit_bonus = CreateUnitByName(
			"npc_dota_slark_visual",
			self:GetCaster():GetAbsOrigin(),
			false,
			nil,
...
function modifier_slark_pounce_custom_thinker:IsHidden()
	return true
end
function modifier_slark_pounce_custom_thinker:IsPurgable()
	return false
end
function modifier_slark_pounce_custom_thinker:RemoveOnDeath()
	return false
end
function modifier_slark_pounce_custom_thinker:IsPurgeException()
	return false
end
...
function modifier_slark_pounce_custom:IsPurgeException()
	return false
end
...
function modifier_slark_pounce_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_slark_13") then
		self.leash_duration = self.leash_duration
			+ self:GetAbility().modifier_slark_13[self:GetCaster():GetTalentLevel("modifier_slark_13")]
	end
	...
	self.arc = self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_slark_pounce_arc",
		{ speed = speed, duration = duration, distance = distance, height = height }
	)
...
function modifier_slark_pounce_custom:OnIntervalThink()
	...
	local enemies_damage = FindUnitsInRadius(
		...
		if self:GetCaster():HasModifier("modifier_slark_8") or self:GetCaster():HasModifier("modifier_slark_16") then
			if self:GetCaster().targets_attack[enemy_dmg:entindex()] == nil then
				...
				if self:GetCaster():HasModifier("modifier_slark_8") then
					local damage = self:GetAbility().modifier_slark_8
						+ (
							self:GetCaster():GetAgility()
							/ 100
							* self:GetAbility().modifier_slark_8_pct[self:GetCaster():GetTalentLevel("modifier_slark_8")]
						)
					ApplyDamage({
				...
				if self:GetCaster():HasModifier("modifier_slark_16") then
					local damage = self:GetAbility().modifier_slark_16
						+ (
							self:GetCaster():GetIntellect(false)
							/ 100
							* self:GetAbility().modifier_slark_16_pct[self:GetCaster()
								:GetTalentLevel("modifier_slark_16")]
						)
					ApplyDamage({
						attacker = self:GetCaster(),
						victim = enemy_dmg,
	...
	target:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_slark_pounce_custom_debuff",
		{ duration = self.leash_duration, radius = self.leash_radius, purgable = false }
	)
	target:EmitSound("Hero_Slark.Pounce.Impact")

	local essence_stacks = self:GetAbility():GetSpecialValueFor("essence_stacks")
	if self:GetCaster():HasModifier("modifier_slark_10") then
		essence_stacks = essence_stacks
			+ self:GetAbility().modifier_slark_10_stacks[self:GetCaster():GetTalentLevel("modifier_slark_10")]
	end
...
function modifier_slark_pounce_custom_debuff:OnCreated(kv)
	...
	self.leash = self:GetParent()
		:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_slark_pounce_leashed", kv)
...
function modifier_slark_pounce_arc:IsPurgeException()
	return false
end
