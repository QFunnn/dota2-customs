...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_slark_custom/slark_essence_shift_custom",
...
function modifier_slark_essence_shift_custom:IsHidden()
	return self:GetStackCount() == 0 or self:GetParent():HasModifier("modifier_slark_3")
end
...
function modifier_slark_essence_shift_custom:GetModifierProcAttack_Feedback(params)
	if IsServer() and (not self:GetParent():PassivesDisabled()) then
		...
		if not target:IsHero() and self:GetParent():HasModifier("modifier_slark_11") then
			if self:GetParent():HasModifier("modifier_slark_17") then
				return
			end
...
function modifier_slark_essence_shift_custom:AttackTargetCreep(target, pounce)
	...
	if self:GetCaster():HasModifier("modifier_slark_essence_shift_custom_buff") then
		local modifier_slark_essence_shift_custom_buff = self:GetCaster()
			:FindModifierByName("modifier_slark_essence_shift_custom_buff")
		if modifier_slark_essence_shift_custom_buff then
			modifier_slark_essence_shift_custom_buff:SetDuration(
				math.max(modifier_slark_essence_shift_custom_buff:GetRemainingTime(), duration),
				true
			)
		end
	else
	...
	if self:GetCaster():HasModifier("modifier_slark_3") then
		...
		if self:GetCaster():HasModifier("modifier_slark_essence_shift_custom_buff_strength") then
			local modifier_slark_essence_shift_custom_buff_strength =
				self:GetCaster():FindModifierByName("modifier_slark_essence_shift_custom_buff_strength")
			if modifier_slark_essence_shift_custom_buff_strength then
				modifier_slark_essence_shift_custom_buff_strength:SetDuration(
					math.max(modifier_slark_essence_shift_custom_buff_strength:GetRemainingTime(), duration),
					true
				)
			end
		else
...
function modifier_slark_essence_shift_custom:GetModifierBonusStats_Agility()
	if self:GetParent():HasModifier("modifier_slark_3") then
		return
	end
...
function modifier_slark_essence_shift_custom:GetModifierBonusStats_Strength()
	if not self:GetParent():HasModifier("modifier_slark_3") then
		return
	end
	return self:GetStackCount()
end
...
function modifier_slark_essence_shift_custom_debuff:OnDeath(params)
	...
	if
		(
			(self:GetCaster():GetAbsOrigin() - target:GetAbsOrigin()):Length2D()
			<= self:GetAbility():GetSpecialValueFor("steal_radius")
...
function modifier_slark_essence_shift_custom_stack_strength:IsPurgable()
	return false
end
function modifier_slark_essence_shift_custom_stack_strength:IsPurgeException()
	return false
end
function modifier_slark_essence_shift_custom_stack_strength:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_slark_essence_shift_custom_stack_strength:IsHidden()
	return true
end

modifier_slark_essence_shift_custom_buff_strength = class({})
function modifier_slark_essence_shift_custom_buff_strength:IsPurgable()
	return false
end
function modifier_slark_essence_shift_custom_buff_strength:IsPurgeException()
	return false
end
function modifier_slark_essence_shift_custom_buff_strength:GetTexture()
	return "slark_3"
end
...
function modifier_slark_essence_shift_custom_buff_strength:OnIntervalThink()
	...
	local modifier_slark_essence_shift_custom_stack_strength =
		self:GetParent():FindAllModifiersByName("modifier_slark_essence_shift_custom_stack_strength")
...
function modifier_slark_essence_shift_custom_buff_strength:GetModifierBonusStats_Strength()
	return self:GetStackCount()
		* self:GetAbility().modifier_slark_3[self:GetCaster():GetTalentLevel("modifier_slark_3")]
end