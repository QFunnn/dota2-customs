...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_slark_custom/slark_dark_pact_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_dark_pact_custom_talent_handler",
	"heroes/npc_dota_hero_slark_custom/slark_dark_pact_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_dark_pact_custom_buff",
	"heroes/npc_dota_hero_slark_custom/slark_dark_pact_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_slark_dark_pact_custom_stack_buff",
	"heroes/npc_dota_hero_slark_custom/slark_dark_pact_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function slark_dark_pact_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_slark.vpcf",
		context
	)
end
...
function modifier_slark_dark_pact_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_slark_5") then
		self.radius = self.radius
			+ self:GetAbility().modifier_slark_5[self:GetCaster():GetTalentLevel("modifier_slark_5")]
		local heal = self:GetCaster():GetMaxHealth()
			/ 100
			* self:GetAbility().modifier_slark_5_heal[self:GetCaster():GetTalentLevel("modifier_slark_5")]
	...
	if self:GetCaster():HasModifier("modifier_slark_6") then
		self.total_damage = self.total_damage
			+ self:GetAbility().modifier_slark_6
			+ (
				self:GetCaster():GetStrength()
				/ 100
				* self:GetAbility().modifier_slark_6_str_damage[self:GetCaster():GetTalentLevel("modifier_slark_6")]
			)
	end
		...
		if self:GetParent() == self:GetCaster() then
			if self:GetCaster():HasModifier("modifier_slark_2") then
				self:GetCaster():AddNewModifier(
					...
					"modifier_slark_dark_pact_custom_stack_buff",
					{
						duration = self:GetAbility().modifier_slark_2_duration[self:GetCaster()
							:GetTalentLevel("modifier_slark_2")],
					}
				)
				self:GetCaster():AddNewModifier(
					self:GetCaster(),
...
function modifier_slark_dark_pact_custom_stack_buff:IsHidden()
	return true
end
function modifier_slark_dark_pact_custom_stack_buff:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_slark_dark_pact_custom_buff:GetTexture()
	return "slark_2"
end
...
function modifier_slark_dark_pact_custom_talent_handler:IsHidden()
	return true
end
function modifier_slark_dark_pact_custom_talent_handler:IsPurgable()
	return false
end
function modifier_slark_dark_pact_custom_talent_handler:IsPurgeException()
	return false
end
...
function modifier_slark_dark_pact_custom_talent_handler:OnAbilityFullyCast(params)
	...
	if not self:GetCaster():HasModifier("modifier_slark_21") then
		return
	end
	if params.ability:GetAbilityName() == "item_moon_shard" then
		return
	end
