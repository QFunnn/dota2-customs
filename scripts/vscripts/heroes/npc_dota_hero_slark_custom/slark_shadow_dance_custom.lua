...
LinkLuaModifier( "modifier_slark_shadow_dance_custom", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_slark_shadow_dance_custom_passive", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_slark_shadow_dance_custom_attack", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_slark_shadow_dance_custom_pull", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_HORIZONTAL )
...
slark_shadow_dance_custom.modifier_slark_12 = {400,600}

function slark_shadow_dance_custom:OnSpellStart(new_duration)
	...
	if self:GetCaster():HasModifier("modifier_slark_12") then
		local enemies = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCaster():GetOrigin(), nil, self.modifier_slark_12[self:GetCaster():GetTalentLevel("modifier_slark_12")], DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, 0, false )
    	for _, target in pairs(enemies) do
    		target:AddNewModifier(self:GetCaster(), self, "modifier_slark_shadow_dance_custom_pull", {})
    	end
	end
end
...
function modifier_slark_shadow_dance_custom_passive:OnTakeDamage(params)
    ...
    if params.attacker:IsHero() then return end
    self:GetParent():AddNewModifier(self:GetParent(), nil, "modifier_slark_shadow_dance_custom_attack", {duration = 2})
end
...
function modifier_slark_shadow_dance_custom_passive:OnIntervalThink()
	local active = true
	local targets = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 1800, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, 0, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, false )
...
LinkLuaModifier( "modifier_slark_depth_shroud_custom_thinker", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_slark_shadow_dance_custom_shard", "heroes/npc_dota_hero_slark_custom/slark_shadow_dance_custom", LUA_MODIFIER_MOTION_NONE )
...
function modifier_slark_shadow_dance_custom_attack:IsHidden() return true end

modifier_slark_shadow_dance_custom_pull = class({})
function modifier_slark_shadow_dance_custom_pull:GetTexture() return "slark_12" end
function modifier_slark_shadow_dance_custom_pull:OnCreated(params)
	if not IsServer() then return end
    ...
    print("ddd", self:GetParent():GetUnitName())
...
function modifier_slark_shadow_dance_custom_pull:UpdateHorizontalMotion( me, dt )
	if not IsServer() then return end
	local distance = self:GetCaster():GetOrigin() - me:GetOrigin()

	if distance:Length2D() > 150 then
		me:SetOrigin( me:GetOrigin() + distance:Normalized() * 1200 * dt )
	else
		self:Destroy()
	end
end

function modifier_slark_shadow_dance_custom_pull:OnDestroy()
	if not IsServer() then return end
