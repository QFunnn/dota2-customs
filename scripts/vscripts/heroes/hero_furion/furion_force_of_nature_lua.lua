...
LinkLuaModifier("modifier_furion_force_of_nature_lua", "heroes/hero_furion/furion_force_of_nature_lua", LUA_MODIFIER_MOTION_NONE)
    ...
    for _, tree in pairs(trees) do
        if count < max_treants then
            if IsValid(tree) then
                CreateUnitByNameAsync(unit_name, tree:GetAbsOrigin(), true, hCaster, hCaster, hCaster:GetTeamNumber(), function(unit)
