...
LinkLuaModifier(
	...
	"heroes/hero_furion/furion_force_of_nature_custom",
...
function furion_force_of_nature_custom:OnSpellStart()
	...
	do
		if
			(
				warlock_gl:GetUnitName() == "npc_dota_furion_treant"
				or warlock_gl:GetUnitName() == "npc_dota_furion_treant_large"
			) and warlock_gl:GetOwner() == self:GetCaster()
		then
			warlock_gl:Destroy()
		end
	end
