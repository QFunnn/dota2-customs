...
LinkLuaModifier(
	"modifier_warlock_fatal_bonds_custom",
	"heroes/npc_dota_hero_warlock_custom/warlock_fatal_bonds_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_fatal_bonds_custom_handler",
	"heroes/npc_dota_hero_warlock_custom/warlock_fatal_bonds_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function warlock_fatal_bonds_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_warlock.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_warlock.vpcf",
		context
	)
end
...
function warlock_fatal_bonds_custom:OnSpellStart()
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_warlock_2") then
			duration = duration
				+ self.modifier_warlock_2_duration[self:GetCaster():GetTalentLevel("modifier_warlock_2")]
		end
...
function modifier_warlock_fatal_bonds_custom:IsHidden()
	return false
end
function modifier_warlock_fatal_bonds_custom:IsPurgable()
	return self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
end
function modifier_warlock_fatal_bonds_custom:IsDebuff()
	return true
end
function modifier_warlock_fatal_bonds_custom:GetAttributes()
...
function modifier_warlock_fatal_bonds_custom:OnCreated()
	...
	local particle_head = ParticleManager:CreateParticle(
...
function modifier_warlock_fatal_bonds_custom:OnTakeDamage(params)
	...
	if params.inflictor == self.ability then
		return
	end
	if self:GetParent() == self:GetCaster() then
		self.damage_counter[params.damage_type] = (self.damage_counter[params.damage_type] or 0)
...
function modifier_warlock_fatal_bonds_custom:SetDamage()
	...
	for _, unit in pairs(self:GetAbility().units_table) do
		if
			...
			and unit:GetTeamNumber() ~= self.caster:GetTeamNumber()
		then
			local effect = false
...
function modifier_warlock_fatal_bonds_custom_handler:IsHidden()
	return self:GetStackCount() == 0
end
function modifier_warlock_fatal_bonds_custom_handler:IsPurgable()
	return false
end
function modifier_warlock_fatal_bonds_custom_handler:IsPurgeException()
	return false
end
function modifier_warlock_fatal_bonds_custom_handler:RemoveOnDeath()
	return false
end
function modifier_warlock_fatal_bonds_custom_handler:GetTexture()
	return "warlock_7"
end
...
function modifier_warlock_fatal_bonds_custom_handler:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_warlock_7") then
		return
	end
