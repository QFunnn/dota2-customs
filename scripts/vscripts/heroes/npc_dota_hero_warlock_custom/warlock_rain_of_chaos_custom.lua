...
LinkLuaModifier(
	"modifier_warlock_rain_of_chaos_custom",
	"heroes/npc_dota_hero_warlock_custom/warlock_rain_of_chaos_custom",
...
function modifier_warlock_rain_of_chaos_custom:IsHidden()
	return true
end
function modifier_warlock_rain_of_chaos_custom:IsPurgable()
	return false
end
function modifier_warlock_rain_of_chaos_custom:IsPurgeException()
	return false
end
...
function modifier_warlock_rain_of_chaos_custom:GetModifierAttackSpeedBonus_Constant()
	if not self:GetCaster():HasModifier("modifier_warlock_21") then
		return
	end
...
function modifier_warlock_rain_of_chaos_custom:OnDestroy()
	...
	if warlock_eldritch_summoning_custom and self:GetCaster():HasModifier("modifier_warlock_14") then
		...
		local tooltip_imp_explode_radius =
			warlock_eldritch_summoning_custom:GetSpecialValueFor("tooltip_imp_explode_radius")
		local enemies = FindUnitsInRadius(
			self:GetCaster():GetTeamNumber(),
