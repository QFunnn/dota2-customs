...
LinkLuaModifier(
	"modifier_warlock_eldritch_summoning_custom",
	"heroes/npc_dota_hero_warlock_custom/warlock_eldritch_summoning_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_eldritch_summoning_custom_logic",
	"heroes/npc_dota_hero_warlock_custom/warlock_eldritch_summoning_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function warlock_eldritch_summoning_custom:SpawnImp(point)
	...
	golem:AddNewModifier(
		...
		self,
		"modifier_kill",
		{ duration = self:GetSpecialValueFor("minor_imp_duration") }
	)
	golem:AddNewModifier(
		self:GetCaster(),
		self,
...
function modifier_warlock_eldritch_summoning_custom:IsPurgable()
	return false
end
function modifier_warlock_eldritch_summoning_custom:IsPurgeException()
	return false
end
function modifier_warlock_eldritch_summoning_custom:IsHidden()
	return true
end
function modifier_warlock_eldritch_summoning_custom:RemoveOnDeath()
	return false
end
...
function modifier_warlock_eldritch_summoning_custom:OnDeath(params)
	...
	for _, mod in pairs(unit:FindAllModifiers()) do
		if
			mod
			and mod:GetAbility()
			and mod:GetAbility():GetCaster() == self:GetParent()
			and not mod:GetAbility():IsItem()
		then
			spawn_imp = true
			break
		end
...
function modifier_warlock_eldritch_summoning_custom_logic:IsPurgable()
	return false
end
function modifier_warlock_eldritch_summoning_custom_logic:IsPurgeException()
	return false
end
function modifier_warlock_eldritch_summoning_custom_logic:IsHidden()
	return true
end

function modifier_warlock_eldritch_summoning_custom_logic:CheckState()
	if not self:GetCaster():HasModifier("modifier_warlock_11") then
		return
	end
	return {
