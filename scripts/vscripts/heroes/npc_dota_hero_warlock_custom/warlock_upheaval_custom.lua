...
LinkLuaModifier(
	"modifier_warlock_upheaval_custom",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_upheaval_custom_debuff",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_upheaval_custom_thinker_debuff",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_upheaval_custom_handler",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)

LinkLuaModifier(
	"modifier_warlock_upheaval_custom_thinker_buffs",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_upheaval_custom_buff",
	"heroes/npc_dota_hero_warlock_custom/warlock_upheaval_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function warlock_upheaval_custom:OnSpellStart()
	...
	self.modifier_thinker = CreateModifierThinker(
		self:GetCaster(),
		self,
		"modifier_warlock_upheaval_custom",
		{ duration = duration },
	...
	if self:GetCaster():HasModifier("modifier_warlock_17") then
		self.modifier_thinker_friendly = CreateModifierThinker(
			self:GetCaster(),
			self,
			"modifier_warlock_upheaval_custom_thinker_buffs",
			{ duration = duration },
...
function modifier_warlock_upheaval_custom:IsPurgable()
	return false
end
function modifier_warlock_upheaval_custom:IsPurgeException()
	return false
end
function modifier_warlock_upheaval_custom:IsHidden()
	return false
end
function modifier_warlock_upheaval_custom:IsAura()
	return true
end
function modifier_warlock_upheaval_custom:IsAuraActiveOnDeath()
	return false
end
function modifier_warlock_upheaval_custom:GetAuraRadius()
	return self.radius
end
function modifier_warlock_upheaval_custom:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_NONE
end
function modifier_warlock_upheaval_custom:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_warlock_upheaval_custom:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_warlock_upheaval_custom:GetModifierAura()
	return "modifier_warlock_upheaval_custom_thinker_debuff"
end
function modifier_warlock_upheaval_custom:GetAuraDuration()
	return 0
end
function modifier_warlock_upheaval_custom:OnCreated(params)
	...
	if self:GetParent() == self:GetCaster() or params.is_move then
		local particle = ParticleManager:CreateParticle(
			"particles/warlock_custom_fx/warlock_upheaval.vpcf",
...
function modifier_warlock_upheaval_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_warlock_15") then
		...
		if self.modifier_warlock_15_timer >= self:GetAbility().modifier_warlock_15 then
			self.modifier_warlock_15_timer = 0
			local warlock_eldritch_summoning_custom = self:GetCaster()
				:FindAbilityByName("warlock_eldritch_summoning_custom")
...
function modifier_warlock_upheaval_custom_thinker_debuff:IsPurgable()
	return false
end
function modifier_warlock_upheaval_custom_thinker_debuff:IsPurgeException()
	return false
end
function modifier_warlock_upheaval_custom_thinker_debuff:IsHidden()
	return true
end
	...
	if self:GetCaster():HasModifier("modifier_warlock_18") then
		...
		self.max_damage = self.max_damage
			+ (
				regeneration_sum
				/ 100
				* self:GetAbility().modifier_warlock_18[self:GetCaster():GetTalentLevel("modifier_warlock_18")]
			)
	end
...
function modifier_warlock_upheaval_custom_handler:IsPurgable()
	return false
end
function modifier_warlock_upheaval_custom_handler:IsHidden()
	return not self:GetCaster():HasModifier("modifier_warlock_20")
end
function modifier_warlock_upheaval_custom_handler:IsPurgeException()
	return false
end
function modifier_warlock_upheaval_custom_handler:RemoveOnDeath()
	return false
end
function modifier_warlock_upheaval_custom_handler:GetTexture()
	return "warlock_20"
end
...
function modifier_warlock_upheaval_custom_handler:OnAbilityFullyCast(params)
	...
	if not self:GetParent():HasModifier("modifier_warlock_20") then
		return
	end
	...
	if self:GetStackCount() <= 0 then
		self:SetStackCount(
			self:GetAbility().modifier_warlock_20[self:GetCaster():GetTalentLevel("modifier_warlock_20")]
		)
		CreateModifierThinker(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_warlock_upheaval_custom",
			{ duration = self:GetAbility().modifier_warlock_20_duration },
...
function modifier_warlock_upheaval_custom_thinker_buffs:IsPurgable()
	return false
end
function modifier_warlock_upheaval_custom_thinker_buffs:IsPurgeException()
	return false
end
function modifier_warlock_upheaval_custom_thinker_buffs:IsHidden()
	return false
end
function modifier_warlock_upheaval_custom_thinker_buffs:IsAura()
	return true
end
function modifier_warlock_upheaval_custom_thinker_buffs:IsAuraActiveOnDeath()
	return false
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetAuraRadius()
	return self.radius
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_INVULNERABLE
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetModifierAura()
	return "modifier_warlock_upheaval_custom_buff"
end
function modifier_warlock_upheaval_custom_thinker_buffs:GetAuraDuration()
	return 0
end
