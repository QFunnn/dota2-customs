...
function chen_creep_blink:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_chen.vpcf",
		context
	)
end
...
function chen_creep_blink:OnSpellStart()
	...
	local particle_start =
		ParticleManager:CreateParticle("particles/creep_particles/creep_blink_start.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	local particle_end = ParticleManager:CreateParticle(
		"particles/creep_particles/creep_blink_end.vpcf",
		PATTACH_ABSORIGIN,
		self:GetCaster()
	)
