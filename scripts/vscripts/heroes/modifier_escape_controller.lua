...
function modifier_escape_controller:OnIntervalThink()
	...
	if HeroPlacementService:IsEscaping(hParent, hParent:GetAbsOrigin()) then
		...
		local suppose_pos = HeroPlacementService:GetRoomCenter(HeroPlacementService:GetSupposeRoom(hParent))
		...
		for i = 1, 2000, 50 do
			new_pos = dir * i + vPos
			if not HeroPlacementService:IsEscaping(hParent, new_pos) then
				FindClearSpaceForUnit(hParent, new_pos, true)
				return
			end
		end

		local suppose_room = HeroPlacementService:GetSupposeRoom(hParent)
		local suppose_pos = HeroPlacementService:GetRoomCenter(suppose_room)
