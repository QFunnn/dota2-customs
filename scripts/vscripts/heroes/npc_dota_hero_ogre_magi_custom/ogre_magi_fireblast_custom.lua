...
LinkLuaModifier(
	"modifier_ogre_magi_fireblast_custom",
	"heroes/npc_dota_hero_ogre_magi_custom/ogre_magi_fireblast_custom",
...
function ogre_magi_fireblast_custom:Cast(new_target, ignore_multicast)
	...
	if not new_target then
		...
		if modifier_ogre_magi_multicast_custom then
			modifier_ogre_magi_multicast_custom:OnAbilityFullyCast({
				ability = self,
				unit = caster,
				target = target,
				is_fast_multicast = true,
			})
		end
	end
end
...
function ogre_magi_fireblast_custom:Impact(target, ability)
	...
	if caster:HasModifier("modifier_ogre_magi_16") then
		damage = damage
			+ (
				...
				* self.modifier_ogre_magi_16[caster:GetTalentLevel("modifier_ogre_magi_16")]
				/ 100
			)
	end
	...
	if caster:HasModifier("modifier_ogre_magi_15") then
		local enemies = FindUnitsInRadius(
			...
			target:GetAbsOrigin(),
			nil,
			self.modifier_ogre_magi_15_radius,
...
function modifier_ogre_magi_fireblast_custom:IsHidden()
	return true
end
function modifier_ogre_magi_fireblast_custom:IsPurgable()
	return false
end
function modifier_ogre_magi_fireblast_custom:IsPurgeException()
	return false
end
function modifier_ogre_magi_fireblast_custom:RemoveOnDeath()
	return false
end
function modifier_ogre_magi_fireblast_custom:OnAttackLanded(params)
	...
	if not self:GetParent():HasModifier("modifier_ogre_magi_14") then
		return
	end
	if self:GetParent():HasModifier("modifier_ogre_magi_3") then
		return
	end
