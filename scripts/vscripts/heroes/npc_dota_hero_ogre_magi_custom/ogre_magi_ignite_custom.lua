...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_ogre_magi_custom/ogre_magi_ignite_custom",
...
function ogre_magi_ignite_custom:OnSpellStart(new_target)
	...
	local units = FindUnitsInRadius(
		...
		caster:GetAbsOrigin(),
		nil,
		self:GetSpecialValueFor("ignite_multicast_aoe"),
	...
	for _, enemy in pairs(units) do
		if enemy ~= target then
			...
			if extra_count >= extra_targets then
				break
			end
		end
	end
...
function ogre_magi_ignite_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if modifier_ogre_magi_ignite_custom then
		modifier_ogre_magi_ignite_custom:SetDuration(
			modifier_ogre_magi_ignite_custom:GetRemainingTime() + duration,
			true
		)
	else
...
function modifier_ogre_magi_ignite_custom:OnIntervalThink()
	...
	if self.caster:HasModifier("modifier_ogre_magi_7") then
		damage = damage
			+ self.ability.modifier_ogre_magi_7_damage_per_second
				* math.min(self:GetRemainingTime(), self.ability.modifier_ogre_magi_7_max_seconds)
	end
