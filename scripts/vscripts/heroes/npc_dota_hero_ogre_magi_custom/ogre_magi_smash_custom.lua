...
LinkLuaModifier(
	"modifier_ogre_magi_smash_custom_buff",
	"heroes/npc_dota_hero_ogre_magi_custom/ogre_magi_smash_custom",
...
function ogre_magi_smash_custom:OnProjectileHit(target, vLocation)
	...
	local damage = self:GetSpecialValueFor("damage")
		+ (self:GetCaster():GetStrength() / 100 * self:GetSpecialValueFor("damage_str"))
	ApplyDamage({
		victim = target,
...
function modifier_ogre_magi_smash_custom_buff:IsHidden()
	return false
end
	...
	if caster and not caster:IsNull() and caster:HasModifier("modifier_ogre_magi_4") then
		...
		if ogre_magi_ignite_custom then
			local particle = ParticleManager:CreateParticle(
				"particles/ogre_magi_spell/ogre_magi_spell.vpcf",
				PATTACH_WORLDORIGIN,
				caster
			)
			...
			local duration =
				self:GetAbility().modifier_ogre_magi_4_duration[caster:GetTalentLevel("modifier_ogre_magi_4")]
			local enemies = FindUnitsInRadius(
				caster:GetTeamNumber(),
				self:GetParent():GetAbsOrigin(),
				nil,
				self:GetParent():GetAoeBonus(self:GetAbility().modifier_ogre_magi_4_radius),
			...
			for _, enemy in pairs(enemies) do
				if not caster:HasModifier("modifier_ogre_magi_10") then
					...
					local modifier_ogre_magi_ignite_custom =
						enemy:FindModifierByName("modifier_ogre_magi_ignite_custom")
					if modifier_ogre_magi_ignite_custom then
						modifier_ogre_magi_ignite_custom:SetDuration(
							modifier_ogre_magi_ignite_custom:GetRemainingTime() + ignite_duration,
							true
						)
					else
