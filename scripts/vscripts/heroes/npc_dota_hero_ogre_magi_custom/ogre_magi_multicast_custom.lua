...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_ogre_magi_custom/ogre_magi_multicast_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ogre_magi_multicast_custom_use",
	"heroes/npc_dota_hero_ogre_magi_custom/ogre_magi_multicast_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function ogre_magi_multicast_custom:GetMulticastChance(times)
	...
	if caster:HasModifier("modifier_ogre_magi_10") then
		chance = chance
			+ caster:GetAgility() / self.modifier_ogre_magi_10[caster:GetTalentLevel("modifier_ogre_magi_10")]
	end
	if caster:HasModifier("modifier_ogre_magi_17") then
		chance = chance
			+ caster:GetIntellect(false)
				/ self.modifier_ogre_magi_17[caster:GetTalentLevel("modifier_ogre_magi_17")]
	end
	return chance
end
...
function modifier_ogre_magi_multicast_custom:IsPurgeException()
	return false
end
function modifier_ogre_magi_multicast_custom:RemoveOnDeath()
	return false
end
...
function modifier_ogre_magi_multicast_custom:OnIntervalThink()
	...
	if
		self.parent:HasModifier("modifier_ogre_magi_3")
		and self.parent:HasModifier("modifier_ogre_magi_10")
		and self.parent:HasModifier("modifier_ogre_magi_17")
	then
		if not self.head_particle then
			self.head_particle = ParticleManager:CreateParticle(
...
function modifier_ogre_magi_multicast_custom:OnAbilityFullyCast(params)
	...
	if self.item_exceptions[params.ability:GetName()] then
		return
	end
	if params.ability:GetName() == "" then
		return
	end
	if not params.is_fast_multicast then
		if params.ability:GetName() == "ogre_magi_fireblast_custom" then
			return
		end
		if params.ability:GetName() == "ogre_magi_unrefined_fireblast_custom" then
			return
		end
	end
	...
	if target and target:GetTeamNumber() == self.parent:GetTeamNumber() then
		if params.ability:GetName() == "item_cyclone" then
			return
		end
		if params.ability:GetName() == "item_wind_waker" then
			return
		end
	end
	local point = nil
	local triple = self:GetCaster():HasModifier("modifier_ogre_magi_3")
		and self:GetCaster():HasModifier("modifier_ogre_magi_10")
		and self:GetCaster():HasModifier("modifier_ogre_magi_17")
	...
	if point == nil then
		self.parent:AddNewModifier(
			...
			"modifier_ogre_magi_multicast_custom_use",
			{
				...
				multicast = multicast_multi,
				delay = delay,
				single = single,
				repeat_target = repeat_target and 1 or 0,
			}
		)
	else
		self.parent:AddNewModifier(
...
function modifier_ogre_magi_multicast_custom_use:OnCreated(kv)
	...
	if self.target ~= nil then
		...
		if
			self:GetCaster() == self.target
			and bit.band(self.ability:GetBehaviorInt(), DOTA_ABILITY_BEHAVIOR_NO_TARGET) ~= 0
		then
			self.no_target = 1
		end
			...
			for _, unit in pairs(units) do
				if not self.targets[unit] then
					...
					if filter then
						if
							#units > 1
							and self.ability:GetAbilityName() ~= "ogre_magi_ignite_custom"
							and not self.repeat_target
						then
