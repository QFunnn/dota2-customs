...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_drow_ranger_custom/drow_ranger_multishot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_drow_ranger_multishot_custom_arbalet_14",
	"heroes/npc_dota_hero_drow_ranger_custom/drow_ranger_multishot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function drow_ranger_multishot_custom:GetCooldown(level)
	if self:GetAbilityName() == "drow_ranger_multishot_2_custom" then
		return 0
	end
...
function drow_ranger_multishot_custom:OnSpellStart()
	...
	self.modifier = caster:AddNewModifier(
		...
		self,
		"modifier_drow_ranger_multishot_custom",
		{ duration = duration, x = point.x, y = point.y, z = point.z }
	)
end
...
function drow_ranger_multishot_custom:OnVectorCastStart(vStartLocation, vDirection)
	...
	if self:GetCaster():HasModifier("modifier_drow_ranger_14") then
		self.arbalet =
			CreateUnitByName("npc_dota_companion", vStartLocation, false, nil, nil, self:GetCaster():GetTeamNumber())
...
function drow_ranger_multishot_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if self.targets[target] == data.wave then
		return false
	end
		...
		if ability and ability:GetLevel() > 0 then
			...
			if self:GetCaster():HasModifier("modifier_drow_ranger_4") then
				target:AddNewModifier(
					...
					{ duration = ability.modifier_drow_ranger_3_duration }
				)
			end
		end
	end
...
function modifier_drow_ranger_multishot_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_drow_ranger_11") then
		wave_count = wave_count
			+ self:GetAbility().modifier_drow_ranger_11_wave_bonus[self:GetCaster()
				:GetTalentLevel("modifier_drow_ranger_11")]
	end
	...
	if self:GetCaster():HasModifier("modifier_drow_ranger_9") then
		local percent = self:GetAbility().modifier_drow_ranger_9[self:GetCaster()
			:GetTalentLevel("modifier_drow_ranger_9")]
...
function modifier_drow_ranger_multishot_custom:OnIntervalThink()
	...
	if self.current_arrows < self.arrows then
		...
		self.info.vSpawnOrigin = self:GetCaster()
			:GetAttachmentOrigin(self:GetCaster():ScriptLookupAttachment("attach_attack1"))
...
function modifier_drow_ranger_multishot_custom_arbalet_14:IsHidden()
	return true
end

function modifier_drow_ranger_multishot_custom_arbalet_14:IsPurgable()
	return false
end
