...
LinkLuaModifier(
	"modifier_drow_ranger_wave_of_silence_custom_buff",
	"heroes/npc_dota_hero_drow_ranger_custom/drow_ranger_wave_of_silence_custom",
	LUA_MODIFIER_MOTION_BOTH
)
...
function drow_ranger_wave_of_silence_custom:OnSpellStart()
	...
	local info = {
		...
		ExtraData = {
			x = self:GetCaster():GetOrigin().x,
			y = self:GetCaster():GetOrigin().y,
			z = self:GetCaster():GetOrigin().z,
			pointx = point.x,
			pointy = point.y,
			pointz = point.z,
		},
...
function drow_ranger_wave_of_silence_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if self.targets[target:entindex()] ~= nil then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_drow_ranger_17") then
		local damage = (
			...
			/ 100
			* self.modifier_drow_ranger_17_damage_frommana[self:GetCaster():GetTalentLevel("modifier_drow_ranger_17")]
		) + self.modifier_drow_ranger_17_base_damage
		ApplyDamage({
			victim = target,
	...
	target:AddNewModifier(
		...
		self,
		"modifier_drow_ranger_wave_of_silence_custom_knockback",
		{ duration = duration, distance = distance, direction_x = vec.x, direction_y = vec.y }
	)
	target:AddNewModifier(
		self:GetCaster(),
		self,
...
function modifier_drow_ranger_wave_of_silence_custom_knockback:UpdateHorizontalMotion(me, dt)
	...
	if parent:GetIdealSpeed() < 100 then
		return
	end
...
function modifier_drow_ranger_wave_of_silence_custom_buff:GetModifierMoveSpeedBonus_Percentage()
	...
	if self:GetCaster():HasModifier("modifier_drow_ranger_15") then
		bonus = self:GetAbility().modifier_drow_ranger_15_bonus_ms[self:GetCaster()
			:GetTalentLevel("modifier_drow_ranger_15")]
	end
