...
LinkLuaModifier(
	"modifier_arc_warden_tempest_double_lua",
	"heroes/hero_arc_warden/arc_warden_tempest_double_lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_arc_warden_tempest_double_lua_illusion",
	"heroes/hero_arc_warden/arc_warden_tempest_double_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function arc_warden_tempest_double_lua:InitDoubleAsync()
		...
		Timers:CreateTimer(0, function()
			...
			for i = 0, hCaster:GetAbilityCount() - 1 do
				...
				if hAbility ~= nil then
					...
					if
						not string.find(AbilityName, "special_bonus") and not table.contains(BanAbility, AbilityName)
					then
						local hNewAbility = unit:AddAbility(AbilityName)
						if IsValid(hNewAbility) then
							local ability_kv = hAbility:GetAbilityKeyValues()
							local grantedByScepter = type(ability_kv) == "table"
								and IsKvFlagEnabled(ability_kv.IsGrantedByScepter)
							local grantedByShard = type(ability_kv) == "table"
								and IsKvFlagEnabled(ability_kv.IsGrantedByShard)
			...
			for i = 0, unit:GetAbilityCount() - 1 do
				...
				if IsValid(hAbility) then ---@cast hAbility CDOTABaseAbility
					local AbilityName = hAbility:GetAbilityName()
					if AbilityPool:Contains(AbilityName) then
						...
						if linkedAbilities then
							for _, LinkAbilityName in pairs(linkedAbilities) do
								...
								if
									linkAbility
									and IsValid(linkAbility)
									and linkAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_HIDDEN)
								then
	...
	if IsUnitValid(unit) then
		local iParticleID = ParticleManager:CreateParticle(
			"particles/units/heroes/hero_arc_warden/arc_warden_tempest_cast_custom.vpcf",
	...
	for name, transfer_type in pairs(self.transferable_modifiers) do
		...
		if hCaster_modifier and not hCaster_modifier:IsNull() then
			...
			if not hClone_modifier then
				hClone_modifier = hClone:AddNewModifier(
					hCaster,
					hCaster_modifier:GetAbility(),
					name,
...
function modifier_arc_warden_tempest_double_lua:OnRefresh() end
...
function modifier_arc_warden_tempest_double_lua:OnDeath(params)
	...
	if killer_player_id ~= nil and killer_player_id >= 0 then
		...
		SendOverheadEventMessage(
			PlayerResource:GetPlayer(killer_player_id),
			OVERHEAD_ALERT_GOLD,
			PlayerResource:GetSelectedHeroEntity(killer_player_id),
			bounty,
			nil
		)
	end
...
function modifier_arc_warden_tempest_double_lua_illusion:OnRefresh() end
