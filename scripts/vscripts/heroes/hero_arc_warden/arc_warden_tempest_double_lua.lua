...
LinkLuaModifier(
	"modifier_tempest_double_min_health",
	"heroes/hero_arc_warden/modifier_tempest_double_min_health",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tempest_double_illusion",
	"heroes/hero_arc_warden/modifier_tempest_double_illusion",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tempest_double_hidden",
	"heroes/hero_arc_warden/modifier_tempest_double_hidden",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tempest_double_dying",
	"heroes/hero_arc_warden/modifier_tempest_double_dying",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tempest_double_talent",
	"heroes/hero_arc_warden/arc_warden_tempest_double_lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function arc_warden_tempest_double_lua:OnSpellStart()
	...
	if hCaster:IsTempestDouble() then
		return
	end
	local hClone = self:GetHerohClone()
	if not hClone or hClone:IsNull() then
		return
	end
	...
	for i = 0, hCaster:GetAbilityCount() - 1 do
		...
		if hOriginalHeroAbility and hOriginalHeroAbility.GetAbilityName then
			...
			if
				not string.find(sOriginalAbilityName, "special_bonus")
				and not Illusion.abilityException[sOriginalAbilityName]
			then
				...
				if not hClone:HasAbility(sOriginalAbilityName) then
					if not (sOriginalAbilityName == "tiny_tree_grab_lua" and hClone.bTreeTempGrab) then
						...
						if
							not hNewAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_PASSIVE)
							and not hNewAbility:HasBehavior(DOTA_ABILITY_BEHAVIOR_HIDDEN)
							and hNewAbility:GetBehaviorInt() > 0
						then
	...
	for i = 0, hCaster:GetAbilityCount() - 1 do
		...
		if hOriginalHeroAbility and hOriginalHeroAbility.GetAbilityName then
			...
			then
				if
					hClone:HasAbility(sOriginalAbilityName)
					and hClone:GetAbilityByIndex(i)
					and hClone:GetAbilityByIndex(i).GetAbilityName
				then
	...
	Timers:CreateTimer(0.25, function()
		...
		local modifier_lion_finger_of_death_custom_passive =
			self:GetCaster():FindModifierByName("modifier_lion_finger_of_death_custom_passive")
		if modifier_lion_finger_of_death_custom_passive then
			local modifier_lion_finger_of_death_custom_passive_clone =
				hClone:FindModifierByName("modifier_lion_finger_of_death_custom_passive")
			if modifier_lion_finger_of_death_custom_passive_clone then
				modifier_lion_finger_of_death_custom_passive_clone:SetStackCount(
					modifier_lion_finger_of_death_custom_passive:GetStackCount()
				)
			end
		end
		local modifier_lina_laguna_blade_custom_passive =
			self:GetCaster():FindModifierByName("modifier_lina_laguna_blade_custom_passive")
		if modifier_lina_laguna_blade_custom_passive then
			local modifier_lina_laguna_blade_custom_passive_clone =
				hClone:FindModifierByName("modifier_lina_laguna_blade_custom_passive")
			if modifier_lina_laguna_blade_custom_passive_clone then
				modifier_lina_laguna_blade_custom_passive_clone:SetStackCount(
					modifier_lina_laguna_blade_custom_passive:GetStackCount()
				)
			end
		end
	end)
	...
	for name, transfer_type in pairs(self.transferable_modifiers) do
		...
		if hCaster_modifier and not hCaster_modifier:IsNull() then
			...
			if not hClone_modifier then
				hClone_modifier =
					hClone:AddNewModifier(hClone, nil, name, { duration = hCaster_modifier:GetDuration() })
			end
...
function arc_warden_tempest_double_lua:GetHerohClone()
	...
	if not self.hClone then
		if hCaster.tempest_double_hClone then
			...
			hClone:RemoveModifierByName("modifier_duel_curse")
			hClone:RemoveModifierByName("modifier_duel_curse_cooldown")
			hClone.IsRealHero = function()
				return true
			end
			hClone.IsMainHero = function()
				return false
			end
			hClone.IsTempestDouble = function()
				return true
			end
	...
	Timers:CreateTimer(0.1, function()
		for index = 0, 5 do
			...
			if hCaster_item and not hCaster_item:IsNull() then
				local clonable = true
				if
					hCaster_item.GetName
					and hCaster_item:GetName()
					and hCaster_item:GetName() == "item_smoke_of_deceit_custom"
				then
					clonable = false
				end
				if clonable then
...
function modifier_tempest_double_talent:IsHidden()
	return true
end
function modifier_tempest_double_talent:IsPurgeException()
	return false
end
function modifier_tempest_double_talent:RemoveOnDeath()
	return false
end
function modifier_tempest_double_talent:IsPurgable()
	return false
end
