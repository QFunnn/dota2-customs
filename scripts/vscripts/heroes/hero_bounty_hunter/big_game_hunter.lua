...
LinkLuaModifier(
	...
	"heroes/hero_bounty_hunter/big_game_hunter",
...
function modifier_ability_bounty_hunter_big_game_hunter:OnDeathEvent(event)
	...
	local bCountDeath = (parent:HasScepter() and ability and ability:GetName() == "bounty_hunter_shuriken_toss")
		or self.RegisteredAttacks[event.record] == true
	...
	if
		...
		and not target:IsStrongIllusion()
		and not target:IsTempestDouble()
		and not target:IsClone()
	then
...
function modifier_ability_bounty_hunter_big_game_hunter:OnOrder(event)
	...
	if unit and unit == self:GetParent() then
		if
			...
			and ability
			and ability:GetName() == "bounty_hunter_jinada"
		then
