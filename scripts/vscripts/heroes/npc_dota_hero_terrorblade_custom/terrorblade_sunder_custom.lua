...
LinkLuaModifier(
	"modifier_terrorblade_sunder_custom_debuff",
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_sunder_custom",
...
function terrorblade_sunder_custom:OnSpellStart()
	...
	local new_health_caster = self:GetCaster():GetMaxHealth()
		* math.max(target_health_percent, self:GetSpecialValueFor("hit_point_minimum_pct"))
		* 0.01
	local new_health_target = target:GetMaxHealth()
		* math.max(caster_health_percent, self:GetSpecialValueFor("hit_point_minimum_pct"))
		* 0.01

	local new_mana_caster = self:GetCaster():GetMaxMana()
		* math.max(target_mana_percent, self:GetSpecialValueFor("hit_point_minimum_pct"))
		* 0.01
	local new_mana_target = target:GetMaxMana()
		* math.max(caster_mana_percent, self:GetSpecialValueFor("hit_point_minimum_pct"))
		* 0.01
	...
	if
		self:GetCaster():HasModifier("modifier_terrorblade_20")
...
function modifier_terrorblade_sunder_custom_debuff:IsPurgable()
	return false
end
function modifier_terrorblade_sunder_custom_debuff:IsPurgeException()
	return false
end
