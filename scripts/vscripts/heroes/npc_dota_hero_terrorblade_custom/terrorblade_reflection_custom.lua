...
LinkLuaModifier(
	"modifier_terrorblade_reflection_custom",
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_reflection_custom",
...
function terrorblade_reflection_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_10") then
		illusion_duration = illusion_duration
			+ self.modifier_terrorblade_10[self:GetCaster():GetTalentLevel("modifier_terrorblade_10")]
	end
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_14") then
		flag = DOTA_UNIT_TARGET_FLAG_NOT_CREEP_HERO
			+ DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS
			+ DOTA_UNIT_TARGET_FLAG_NO_INVIS
		...
		for _, enemy in pairs(enemies) do
			if not enemy:IsDebuffImmune() then
				...
				enemy:AddNewModifier(
					...
					self,
					"modifier_terrorblade_reflection_custom_debuff",
					{ duration = illusion_duration * (1 - enemy:GetStatusResistance()) }
				)
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_10") then
		if #units > 0 then
			if not units[1]:IsDebuffImmune() then
				...
				units[1]:AddNewModifier(
					self:GetCaster(),
					self,
					"modifier_terrorblade_reflection_custom_debuff",
					{ duration = illusion_duration * (1 - units[1]:GetStatusResistance()) }
				)
				local illusions = CreateIllusions(
					self:GetCaster(),
					self:GetCaster(),
				...
				for _, illusion in pairs(illusions) do
					...
					illusion:AddNewModifier(
						...
						{
							duration = illusion_duration * (1 - units[1]:GetStatusResistance()),
							enemy_entindex = units[1]:entindex(),
						}
					)
				end
			end
...
function modifier_terrorblade_reflection_custom:IsPurgable()
	return false
end
function modifier_terrorblade_reflection_custom:IsHidden()
	return true
end
function modifier_terrorblade_reflection_custom:GetStatusEffectName()
	return "particles/status_fx/status_effect_terrorblade_reflection.vpcf"
end
function modifier_terrorblade_reflection_custom:StatusEffectPriority()
	return 10000000
end
function modifier_terrorblade_reflection_custom:GetModifierModelScale()
	return 30
end
function modifier_terrorblade_reflection_custom:GetModelScale()
	return 2
end
...
function modifier_terrorblade_reflection_custom:OnIntervalThink()
	if
		...
		and self.target:HasModifier("modifier_terrorblade_reflection_custom_debuff")
	then
