...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_terror_wave_custom",
...
function terrorblade_terror_wave_custom:OnSpellStart(duration)
	...
	local pulse = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_generic_ring_lua_terrorblade", {
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_16") then
		...
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			terrorblade_demon_hunter,
			"modifier_terrorblade_demon_hunter",
			{ duration = scepter_meta_duration }
		)
	else
		if self:GetCaster():HasModifier("modifier_terrorblade_21") then
			return
		end
		self:GetCaster():AddNewModifier(
			self:GetCaster(),
			terrorblade_metamorphosis_custom,
			"modifier_custom_terrorblade_metamorphosis_transform",
			{ duration = delay, meta_duration = scepter_meta_duration }
		)
		for _, unit in
			pairs(
				FindUnitsInRadius(
					...
					nil,
					terrorblade_metamorphosis_custom:GetSpecialValueFor("metamorph_aura_tooltip"),
			...
			if (not self.IsCircle) or (target:GetOrigin() - self.origin):Length2D() > (radius - self.width) then
				...
				if not target:IsMagicImmune() then
					target:AddNewModifier(
						...
						{ duration = fear_duration * (1 - target:GetStatusResistance()) }
					)
				end
