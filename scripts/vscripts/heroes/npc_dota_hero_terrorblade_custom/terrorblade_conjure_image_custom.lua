...
LinkLuaModifier(
	"modifier_terrorblade_conjure_image_custom",
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_conjure_image_custom",
...
function terrorblade_conjure_image_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_8") then
		illusion_incoming_damage = illusion_incoming_damage
			- self.modifier_terrorblade_8[self:GetCaster():GetTalentLevel("modifier_terrorblade_8")]
	end

	if self:GetCaster():HasModifier("modifier_terrorblade_9") then
		illusion_duration = illusion_duration
			+ self.modifier_terrorblade_9[self:GetCaster():GetTalentLevel("modifier_terrorblade_9")]
	end

	if self:GetCaster():HasModifier("modifier_terrorblade_12") then
		illusion_outgoing_damage = illusion_outgoing_damage
			+ self.modifier_terrorblade_12[self:GetCaster():GetTalentLevel("modifier_terrorblade_12")]
	end
...
function modifier_terrorblade_conjure_image_custom:IsPurgable()
	return false
end
function modifier_terrorblade_conjure_image_custom:IsHidden()
	return true
end
function modifier_terrorblade_conjure_image_custom:GetStatusEffectName()
	return "particles/status_fx/status_effect_terrorblade_reflection.vpcf"
end
function modifier_terrorblade_conjure_image_custom:StatusEffectPriority()
	return 10000000
end
function modifier_terrorblade_conjure_image_custom:GetModifierModelScale()
	return 30
end
function modifier_terrorblade_conjure_image_custom:GetModelScale()
	return 2
end