...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_demon_hunter",
...
function terrorblade_demon_hunter:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_terrorblade_demon_hunter", { duration = duration })
end

modifier_terrorblade_demon_hunter = class({})

function modifier_terrorblade_demon_hunter:IsPurgable()
	return false
end
...
function modifier_terrorblade_demon_hunter:GetModifierProcAttack_BonusDamage_Magical(params)
	...
	if not self:GetParent():HasModifier("modifier_terrorblade_17") then
		return
	end
	...
	local steal_mana = params.damage
		/ 100
		* self:GetAbility().modifier_terrorblade_17[self:GetCaster():GetTalentLevel("modifier_terrorblade_17")]
