...
LinkLuaModifier(
	"modifier_terrorblade_demon_zeal_custom",
	"heroes/npc_dota_hero_terrorblade_custom/terrorblade_demon_zeal_custom",
...
function terrorblade_demon_zeal_custom:OnSpellStart(new_duration)
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_terrorblade_demon_zeal_custom", { duration = duration })
	self:GetCaster():EmitSound("Hero_Terrorblade.DemonZeal.Cast")
end

modifier_terrorblade_demon_zeal_custom = class({})
function modifier_terrorblade_demon_zeal_custom:IsPurgable()
	return false
end
function modifier_terrorblade_demon_zeal_custom:IsPurgeException()
	return false
end
function modifier_terrorblade_demon_zeal_custom:GetEffectName()
	return "particles/models/heroes/terrorblade/demon_zeal.vpcf"
end
...
function modifier_terrorblade_demon_zeal_custom:GetModifierConstantHealthRegen()
	...
	if self:GetCaster():HasModifier("modifier_terrorblade_4") then
		bonus = self:GetAbility().modifier_terrorblade_4_hp_regen[self:GetCaster()
			:GetTalentLevel("modifier_terrorblade_4")]
	end
