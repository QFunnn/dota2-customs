...
LinkLuaModifier(
	"modifier_venomancer_poison_nova_custom",
	"heroes/hero_venomancer/venomancer_poison_nova_custom",
...
function venomancer_poison_nova_custom:OnSpellStart()
	...
	local ring = self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_venomancer_poison_nova_custom_ring", {
			start_radius = 0,
	...
	ring:SetCallback(function(enemy)
		...
		if mod then
			print(
				"[DEBUG STATUS_RES] Poison Nova | requested: "
					.. duration
					.. " | actual: "
					.. mod:GetDuration()
					.. " | target SR: "
					.. enemy:GetStatusResistance()
			)
		end
...
function modifier_venomancer_poison_nova_custom:OnIntervalThink()
	...
	local damageTable = {
		...
		damage = (self.damage * self:GetParent():GetMaxHealth() / 100) + self:GetAbility()
			:GetSpecialValueFor("base_damage"),
...
function modifier_venomancer_poison_nova_custom_ring:OnRemoved() end
