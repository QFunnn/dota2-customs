...
function modifier_dark_seer_wall_of_replica_nb2017:OnCreated(kv)
	...
	if IsServer() then
		self.replica_damage_outgoing = self:GetAbility():GetSpecialValueFor("replica_damage_outgoing")
			+ self:GetCaster():GetTalentValue("special_bonus_unique_dark_seer_7")
				...
				if bTryCreate == true then
					...
					if flDistOffLine <= (enemy:GetPaddedCollisionRadius() + 50.0) then
						...
						if enemy:IsRealHero() then
							...
							replica = illusions[1]
							replica.IsRealHero = function()
								return false
							end
							replica.IsMainHero = function()
								return false
							end
						elseif not enemy:IsIllusion() then
							replica = CreateUnitByName(
								enemy:GetUnitName(),
								enemy:GetOrigin() + RandomVector(enemy:GetPaddedCollisionRadius() + 50.0),
								true,
								self:GetCaster(),
								self:GetCaster(),
