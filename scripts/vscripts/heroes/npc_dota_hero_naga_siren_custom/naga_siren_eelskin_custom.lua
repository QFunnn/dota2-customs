...
LinkLuaModifier(
	"modifier_naga_siren_eelskin_custom",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_eelskin_custom",
...
function modifier_naga_siren_eelskin_custom:IsHidden()
	return true
end
function modifier_naga_siren_eelskin_custom:IsPurgable()
	return false
end
function modifier_naga_siren_eelskin_custom:IsPurgeException()
	return false
end
function modifier_naga_siren_eelskin_custom:RemoveOnDeath()
	return false
end
...
function modifier_naga_siren_eelskin_custom:GetModifierEvasion_Constant()
	if self:GetParent():HasModifier("modifier_naga_siren_14") then
		return
	end
	return self.evasion
end
...
function modifier_naga_siren_eelskin_custom:GetModifierTotal_ConstantBlock(kv)
	if IsServer() then
		if not self:GetParent():HasModifier("modifier_naga_siren_14") then
			return
		end
