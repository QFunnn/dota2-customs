...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_mirror_image_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_mirror_image_custom_barrier",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_mirror_image_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function naga_siren_mirror_image_custom:SetBarrier()
	Timers:CreateTimer(0.25, function()
		local units = FindUnitsInRadius(
			...
			nil,
			self.modifier_naga_siren_10_radius,
		...
		for _, unit in pairs(units) do
			...
			unit:AddNewModifier(
				...
				self,
				"modifier_naga_siren_mirror_image_custom_barrier",
				{ duration = self.modifier_naga_siren_10_duration }
			)
		end
	end)
end
...
function modifier_naga_siren_mirror_image_custom:IsHidden()
	return true
end
function modifier_naga_siren_mirror_image_custom:IsPurgable()
	return false
end
...
function modifier_naga_siren_mirror_image_custom:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_naga_siren_17") then
		self.incoming = self.incoming
			+ self:GetAbility().modifier_naga_siren_17[self:GetCaster():GetTalentLevel("modifier_naga_siren_17")]
	end
	self.distance = 72
end
