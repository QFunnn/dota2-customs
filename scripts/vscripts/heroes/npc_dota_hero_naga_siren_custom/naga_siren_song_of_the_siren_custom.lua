...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_song_of_the_siren_custom",
...
LinkLuaModifier(
	"modifier_naga_siren_song_of_the_siren_custom_buff",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_song_of_the_siren_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function naga_siren_song_of_the_siren_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_naga_siren_song_of_the_siren_custom", { duration = duration })
	local naga_siren_song_of_the_siren_cancel_custom = self:GetCaster()
		:FindAbilityByName("naga_siren_song_of_the_siren_cancel_custom")
...
function modifier_naga_siren_song_of_the_siren_custom:IsPurgable()
	return false
end
...
function modifier_naga_siren_song_of_the_siren_custom:OnCreated(kv)
	...
	self:GetCaster()
		:SwapAbilities("naga_siren_song_of_the_siren_custom", "naga_siren_song_of_the_siren_cancel_custom", false, true)
...
function modifier_naga_siren_song_of_the_siren_custom:OnDestroy()
	...
	self:GetCaster()
		:SwapAbilities("naga_siren_song_of_the_siren_cancel_custom", "naga_siren_song_of_the_siren_custom", false, true)
end
...
function modifier_naga_siren_song_of_the_siren_custom:OnIntervalThink()
	...
	local friendly_heroes = FindUnitsInRadius(
	...
	for _, hero in pairs(friendly_heroes) do
		...
		if self:GetParent():HasModifier("modifier_naga_siren_12") then
			local mana = (
				hero:GetMaxMana()
				/ 100
				* self:GetAbility().modifier_naga_siren_12[self:GetCaster():GetTalentLevel("modifier_naga_siren_12")]
			) * 0.1
			hero:GiveMana(mana)
			heal_pct = heal_pct
				+ self:GetAbility().modifier_naga_siren_12[self:GetCaster():GetTalentLevel("modifier_naga_siren_12")]
...
function modifier_naga_siren_song_of_the_siren_custom:GetAuraEntityReject(target)
	if not self:GetCaster():HasModifier("modifier_naga_siren_21") then
		return false
	end
...
function naga_siren_song_of_the_siren_cancel_custom:OnSpellStart()
	...
	local modifier_naga_siren_song_of_the_siren_custom =
		self:GetCaster():FindModifierByName("modifier_naga_siren_song_of_the_siren_custom")
