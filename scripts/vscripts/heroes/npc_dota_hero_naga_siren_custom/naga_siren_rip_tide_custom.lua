...
LinkLuaModifier(
	"modifier_naga_siren_rip_tide_custom",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_rip_tide_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_rip_tide_custom_debuff",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_rip_tide_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_rip_tide_custom_bonus_damage",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_rip_tide_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_naga_siren_rip_tide_custom:IsHidden()
	return self:GetStackCount() == 0
end
function modifier_naga_siren_rip_tide_custom:IsDebuff()
	return false
end
function modifier_naga_siren_rip_tide_custom:IsPurgable()
	return false
end
...
function modifier_naga_siren_rip_tide_custom:GetModifierProcAttack_Feedback(params)
	...
	if not modifier_naga_siren_rip_tide_custom then
		return
	end
		...
		for __, enemy in pairs(enemies) do
			if targets[enemy:entindex()] == nil then
				...
				if self:GetCaster():HasModifier("modifier_naga_siren_15") then
					damage = damage
						+ self:GetAbility().modifier_naga_siren_15[self:GetCaster()
							:GetTalentLevel("modifier_naga_siren_15")]
				end
				local modifier_naga_siren_rip_tide_custom_bonus_damage =
					enemy:FindModifierByName("modifier_naga_siren_rip_tide_custom_bonus_damage")
				if modifier_naga_siren_rip_tide_custom_bonus_damage then
					damage = damage
						+ (
							...
							/ 100
							* (
								modifier_naga_siren_rip_tide_custom_bonus_damage:GetStackCount()
								* self:GetAbility().modifier_naga_siren_19[self:GetCaster()
									:GetTalentLevel("modifier_naga_siren_19")]
							)
						)
				end
				...
				if self:GetCaster():HasModifier("modifier_naga_siren_19") then
					enemy:AddNewModifier(
						...
						self:GetAbility(),
						"modifier_naga_siren_rip_tide_custom_bonus_damage",
						{ duration = self:GetAbility().modifier_naga_siren_19_max_effects_duration }
					)
				end
			end
		end
...
function modifier_naga_siren_rip_tide_custom_debuff:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_naga_siren_6") then
		self.armor = self.armor
			+ self:GetAbility().modifier_naga_siren_6[self:GetCaster():GetTalentLevel("modifier_naga_siren_6")]
	end
end
...
function modifier_naga_siren_rip_tide_custom_bonus_damage:GetTexture()
	return "naga_siren_19"
end
function modifier_naga_siren_rip_tide_custom_bonus_damage:OnCreated()
	...
	if self:GetParent():IsHero() then
		self.particle = ParticleManager:CreateParticle(
			"particles/naga_siren_riptide_counterstack.vpcf",
