...
LinkLuaModifier(
	"modifier_naga_siren_ensnare_custom",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_ensnare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_ensnare_custom_handler",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_ensnare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_ensnare_custom_handler_cooldown",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_ensnare_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function naga_siren_ensnare_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_naga_siren.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_naga_siren.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_naga_siren.vpcf",
		context
	)
end
...
function naga_siren_ensnare_custom:CastNetAoe(target)
	...
	local units = FindUnitsInRadius(
		...
		target:GetOrigin(),
		nil,
		self.modifier_naga_siren_3_radius,
...
function naga_siren_ensnare_custom:OnProjectileHit_ExtraData(target, location, data)
	...
	if
		modifier_naga_siren_ensnare_custom
		and modifier_naga_siren_ensnare_custom:GetRemainingTime() > (duration * (1 - target:GetStatusResistance()))
	then
		return
	end
...
function modifier_naga_siren_ensnare_custom:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_naga_siren_7")
end
...
function modifier_naga_siren_ensnare_custom:OnIntervalThink()
	...
	if not self:GetCaster():HasModifier("modifier_naga_siren_2") then
		return
	end
	local damage = self:GetCaster():GetAverageTrueAttackDamage(nil)
		/ 100
		* self:GetAbility().modifier_naga_siren_2[self:GetCaster():GetTalentLevel("modifier_naga_siren_2")]
...
function modifier_naga_siren_ensnare_custom_handler:IsHidden()
	return true
end
function modifier_naga_siren_ensnare_custom_handler:IsPurgeException()
	return false
end
function modifier_naga_siren_ensnare_custom_handler:IsPurgable()
	return false
end
function modifier_naga_siren_ensnare_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_naga_siren_ensnare_custom_handler:OnAttackLanded(params)
	...
	if not self:GetCaster():HasModifier("modifier_naga_siren_3") then
		return
	end
	if self:GetCaster():HasModifier("modifier_naga_siren_ensnare_custom_handler_cooldown") then
		return
	end
	if
		RollPercentage(
			self:GetAbility().modifier_naga_siren_3[self:GetCaster():GetTalentLevel("modifier_naga_siren_3")]
		)
	then
		...
		self:GetCaster():AddNewModifier(
			...
			"modifier_naga_siren_ensnare_custom_handler_cooldown",
			{
				duration = self:GetAbility().modifier_naga_siren_3_cooldown[self:GetCaster()
					:GetTalentLevel("modifier_naga_siren_3")],
			}
		)
	end
end
...
function modifier_naga_siren_ensnare_custom_handler_cooldown:IsDebuff()
	return true
end
function modifier_naga_siren_ensnare_custom_handler_cooldown:IsPurgeException()
	return false
end
function modifier_naga_siren_ensnare_custom_handler_cooldown:IsPurgable()
	return false
end
function modifier_naga_siren_ensnare_custom_handler_cooldown:RemoveOnDeath()
	return false
end
function modifier_naga_siren_ensnare_custom_handler_cooldown:GetTexture()
	return "naga_siren_3"
end

LinkLuaModifier(
	"modifier_naga_siren_reel_in_custom_buff",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_ensnare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_naga_siren_reel_in_custom_pull",
	"heroes/npc_dota_hero_naga_siren_custom/naga_siren_ensnare_custom",
...
function modifier_naga_siren_reel_in_custom_buff:IsHidden()
	return false
end
function modifier_naga_siren_reel_in_custom_buff:IsPurgeException()
	return false
end
function modifier_naga_siren_reel_in_custom_buff:IsPurgable()
	return false
end
...
function modifier_naga_siren_reel_in_custom_buff:OnIntervalThink()
	...
	for i = #self.units, 1, -1 do
		if
			not self.units[i]
			or self.units[i]:IsNull()
			or not self.units[i]:HasModifier("modifier_naga_siren_ensnare_custom")
		then
	...
	for _, unit in pairs(self.units) do
		...
		dir = dir:Normalized()
		local new_point = unit:GetAbsOrigin()
			+ dir * (self:GetAbility():GetSpecialValueFor("pull_strength") * FrameTime())
...
function modifier_naga_siren_reel_in_custom_pull:IsPurgable()
	return false
end
function modifier_naga_siren_reel_in_custom_pull:IsPurgeException()
	return false
end
