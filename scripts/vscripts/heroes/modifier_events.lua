...
function public:OnAbilityToggled(params)
	if not IsValid(params.unit) then
		return
	end
	if IsValid(params.ability) and params.ability:GetAbilityName() == "bane_nightmare_end" then
		local bane_nightmare = params.unit:FindAbilityByName("bane_nightmare")
		if IsValid(bane_nightmare) then
			bane_nightmare:SetHidden(false)
		end
	end
end
function public:OnSpellTargetReady(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_SPELL_TARGET_READY]
	then
	...
	if
		...
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_SPELL_TARGET_READY]
	then
...
function public:OnAttackRecord(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_RECORD]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_RECORD]
	then
...
function public:OnAttackStart(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_START]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_START]
	then
...
function public:OnAttack(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK]
	then
...
function public:OnAttackLanded(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_LANDED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_LANDED]
	then
...
function public:OnAttackFail(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_FAIL]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_FAIL]
	then
...
function public:OnAttackAllied(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_ALLIED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_ALLIED]
	then
...
function public:OnOrder(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_ORDER]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ORDER]
	then
...
function public:OnUnitMoved(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_UNIT_MOVED]
	then
...
function public:OnAbilityStart(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_ABILITY_START]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ABILITY_START]
	then
...
function public:OnAbilityExecuted(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_ABILITY_EXECUTED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ABILITY_EXECUTED]
	then
...
function public:OnAbilityFullyCast(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_ABILITY_FULLY_CAST]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ABILITY_FULLY_CAST]
	then
	...
	-- 处理有子技能的技能释放后子技能会被隐藏的问题
	if IsValid(params.ability) then
		local switchAbilityList = {
		...
		local linkAbilityList = HeroBuilder.linkedAbilities[abilityName]
		local buffAbilityList = {
		...
		if buffAbilityList[abilityName] then
			local hModifier = params.unit:FindModifierByName(buffAbilityList[abilityName])
			if IsValid(hModifier) then
				Timers:CreateTimer(hModifier:GetDuration() + FrameTime(), function()
					if IsValid(params.ability) then
						params.ability:SetHidden(false)
					end
				end)
			end
		elseif abilityName == "nyx_assassin_vendetta" then
			local nyx_assassin_burrow = params.unit:FindAbilityByName("nyx_assassin_burrow")
			if IsValid(nyx_assassin_burrow) then
				nyx_assassin_burrow:SetHidden(false)
			end
		elseif switchAbilityList[abilityName] then
			params.ability:SetHidden(true)
			-- local hModifier = params.unit:FindModifierByName("modifier_alchemist_unstable_concoction")
		elseif abilityName == "shredder_chakram_lua" or abilityName == "shredder_chakram_lua_return" then
			return
		end
		if linkAbilityList then
			for _, linkAbilityName in pairs(linkAbilityList) do
				local linkAbility = params.unit:FindAbilityByName(linkAbilityName)
				if IsValid(linkAbility) then
					linkAbility:SetHidden(false)
				end
			end
		end
		local originalAbilityName = HeroBuilder.linkedAbilitiesReverse[abilityName]
		if originalAbilityName then
			local originalAbility = params.unit:FindAbilityByName(originalAbilityName)
			if IsValid(originalAbility) then
				originalAbility:SetHidden(false)
			end
		end
	end
end
...
function public:OnAbilityEndChannel(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_ABILITY_END_CHANNEL]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ABILITY_END_CHANNEL]
	then
...
function public:OnTakeDamage(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_TAKEDAMAGE]
	then
	...
	if
		IsValid(params.unit)
		and params.unit.tTargetModifierEvents
		and params.unit.tTargetModifierEvents[MODIFIER_EVENT_ON_TAKEDAMAGE]
	then
...
function public:OnStateChanged(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_STATE_CHANGED]
	then
...
function public:OnDamageCalculated(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_DAMAGE_CALCULATED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_DAMAGE_CALCULATED]
	then
...
function public:OnAttacked(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACKED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACKED]
	then
...
function public:OnDeath(params)
	-- 处理小强钻地被隐藏
	if IsValid(params.unit) and params.unit:IsRealHero() then
		local hModifier = params.unit:FindModifierByName("modifier_nyx_assassin_burrow")
		if hModifier then
			local ability = params.unit:FindAbilityByName("nyx_assassin_burrow")
	...
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_DEATH]
	then
	...
	if
		IsValid(params.unit)
		and params.unit.tTargetModifierEvents
		and params.unit.tTargetModifierEvents[MODIFIER_EVENT_ON_DEATH]
	then
...
function public:OnRespawn(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_RESPAWN]
	then
...
function public:OnSpentMana(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_SPENT_MANA]
	then
...
function public:OnTeleporting(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_TELEPORTING]
	then
...
function public:OnTeleported(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_TELEPORTED]
	then
...
function public:OnHealthGained(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_HEALTH_GAINED]
	then
...
function public:OnManaGained(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_MANA_GAINED]
	then
...
function public:OnTakeDamageKillCredit(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_TAKEDAMAGE_KILLCREDIT]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_TAKEDAMAGE_KILLCREDIT]
	then
		local tModifiers = params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_TAKEDAMAGE_KILLCREDIT]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
			if IsValid(params.target) and IsValid(hModifier) and hModifier.OnTakeDamageKillCredit then
				hModifier:OnTakeDamageKillCredit(params)
			else
				table.remove(tModifiers, i)
			end
...
function public:OnHeroKilled(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_HERO_KILLED]
	then
		local tModifiers = params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_HERO_KILLED]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_HERO_KILLED]
	then
		local tModifiers = params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_HERO_KILLED]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
			if IsValid(params.target) and IsValid(hModifier) and hModifier.OnHeroKilled then
				hModifier:OnHeroKilled(params)
			else
				table.remove(tModifiers, i)
			end
...
function public:OnHealReceived(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_HEAL_RECEIVED]
	then
...
function public:OnModifierAdded(params)
	if
		IsValid(params.unit)
		and params.unit.tSourceModifierEvents
		and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_MODIFIER_ADDED]
	then
...
function public:OnAttackFinished(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_FINISHED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_FINISHED]
	then
...
function public:OnAttackRecordDestroy(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_RECORD_DESTROY]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_RECORD_DESTROY]
	then
...
function public:OnAttackCancelled(params)
	if
		IsValid(params.attacker)
		and params.attacker.tSourceModifierEvents
		and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ATTACK_CANCELLED]
	then
	...
	if
		IsValid(params.target)
		and params.target.tTargetModifierEvents
		and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_ATTACK_CANCELLED]
	then
