...
LinkLuaModifier(
	"modifier_bounty_hunter_wind_walk_custom",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_wind_walk_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bounty_hunter_wind_walk_custom_passive",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_wind_walk_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_bounty_hunter_wind_walk_custom_passive_active",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_wind_walk_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_bounty_hunter_wind_walk_custom_passive:IsHidden()
	return true
end
function modifier_bounty_hunter_wind_walk_custom_passive:IsPurgable()
	return false
end
...
function modifier_bounty_hunter_wind_walk_custom_passive:GetMinHealth()
	...
	if not self:GetParent():HasModifier("modifier_bounty_hunter_14") then
		return
	end
	if self:GetParent():HasModifier("modifier_bounty_hunter_wind_walk_custom_passive_cooldown") then
		return
	end
	return 1
end
...
function modifier_bounty_hunter_wind_walk_custom_passive:OnTakeDamage(params)
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_bounty_hunter_wind_walk_custom_passive_active",
		{ duration = self:GetAbility().modifier_bounty_hunter_14_duration }
	)
...
function modifier_bounty_hunter_wind_walk_custom:IsPurgable()
	return false
end
...
function modifier_bounty_hunter_wind_walk_custom:GetModifierTotal_ConstantBlock(params)
	if self:GetCaster():HasModifier("modifier_bounty_hunter_12") then
		local bonus = self:GetAbility().modifier_bounty_hunter_12[self:GetCaster()
			:GetTalentLevel("modifier_bounty_hunter_12")]
...
function modifier_bounty_hunter_wind_walk_custom:OnAttackLanded(params)
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_10") then
		stun_duration = stun_duration
			+ self:GetAbility().modifier_bounty_hunter_10[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_10")]
	end
...
function modifier_bounty_hunter_wind_walk_custom_passive_cooldown:IsHidden()
	return false
end
function modifier_bounty_hunter_wind_walk_custom_passive_cooldown:IsPurgable()
	return false
end
function modifier_bounty_hunter_wind_walk_custom_passive_cooldown:IsDebuff()
	return true
end
function modifier_bounty_hunter_wind_walk_custom_passive_cooldown:RemoveOnDeath()
	return false
end

modifier_bounty_hunter_wind_walk_custom_passive_active = class({})
function modifier_bounty_hunter_wind_walk_custom_passive_active:IsHidden()
	return false
end
function modifier_bounty_hunter_wind_walk_custom_passive_active:IsPurgable()
	return false
end
...
function modifier_bounty_hunter_wind_walk_custom_passive_active:OnDestroy()
	...
	self:GetParent()
		:Heal(self:GetParent():GetMaxHealth() * (self:GetAbility().modifier_bounty_hunter_14_restore / 100), nil)
	self:GetParent()
		:GiveMana(self:GetParent():GetMaxMana() * (self:GetAbility().modifier_bounty_hunter_14_restore / 100))
end
