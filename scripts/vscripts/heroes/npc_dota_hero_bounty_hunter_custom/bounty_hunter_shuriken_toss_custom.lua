...
LinkLuaModifier(
	"modifier_bounty_hunter_shuriken_toss_custom_dummy",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_shuriken_toss_custom",
...
LinkLuaModifier(
	"modifier_bounty_hunter_shuriken_toss_debuff",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_shuriken_toss_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_bounty_hunter_shuriken_toss_custom_dummy:IsHidden()
	return true
end
function modifier_bounty_hunter_shuriken_toss_custom_dummy:IsPurgable()
	return false
end
...
function bounty_hunter_shuriken_toss_custom:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_19") then
		slow_duration = (
			slow_duration + self.modifier_bounty_hunter_19[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_19")]
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_20") then
		local gold_cap =
			self.modifier_bounty_hunter_20_max[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_20")]
	...
	if ExtraData.main_shuriken ~= 0 then
		...
		for _, unit in pairs(units) do
			if
				unit:HasModifier("modifier_bounty_hunter_track_custom")
				and self[ExtraData.index][unit:entindex()] == nil
			then
...
function bounty_hunter_shuriken_toss_custom:OnProjectileThink_ExtraData(vLocation, ExtraData)
	if self:GetCaster():HasModifier("modifier_bounty_hunter_15") then
		...
		local units_bonus = FindUnitsInRadius(
			self:GetCaster():GetTeamNumber(),
			vLocation,
			nil,
			radius_bonus,
		...
		for _, unit_bonus in pairs(units_bonus) do
			...
			if self:GetCaster():HasModifier("modifier_bounty_hunter_20") then
				damage = damage
					+ (
						self.modifier_bounty_hunter_20
						/ 100
						* math.min(
							self:GetCaster():GetGold(),
							self.modifier_bounty_hunter_20_max[self:GetCaster()
								:GetTalentLevel("modifier_bounty_hunter_20")]
						)
					)
			end
