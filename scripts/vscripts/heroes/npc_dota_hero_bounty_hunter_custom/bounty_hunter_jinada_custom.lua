...
LinkLuaModifier(
	"modifier_bounty_hunter_jinada_custom_passive",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_jinada_custom",
...
LinkLuaModifier(
	"modifier_bounty_hunter_shuriken_toss_custom_gold",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_jinada_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function bounty_hunter_jinada_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_bounty_hunter.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_bounty_hunter.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_bounty_hunter.vpcf",
		context
	)
end
...
function bounty_hunter_jinada_custom:OnOrbImpact(params)
	...
	if params.target:IsRealHero() or self:GetCaster():HasModifier("modifier_bounty_hunter_17") then
		if params.target:GetUnitName() ~= "npc_dota_bounty_hunter_gold_bag" then
			local money_particle = ParticleManager:CreateParticle(
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_1") then
		DoCleaveAttack(
			...
			params.target,
			self,
			(params.original_damage + self:GetSpecialValueFor("bonus_damage"))
				/ 100
				* self.modifier_bounty_hunter_1[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_1")],
			150,
			360,
			650,
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_5") then
		local gold_bag = CreateUnitByName(
			"npc_dota_bounty_hunter_gold_bag",
		...
		if gold_bag then
			gold_bag:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_bounty_hunter_shuriken_toss_custom_gold",
				{ duration = self.modifier_bounty_hunter_5[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_5")] }
			)
		end
	end
...
function modifier_bounty_hunter_jinada_custom_passive:OnIntervalThink()
	...
	if self:GetAbility():IsFullyCastable() then
		if not self:GetCaster():HasModifier("modifier_bounty_hunter_jinada_custom_crit") then
			self:GetCaster()
				:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_bounty_hunter_jinada_custom_crit", {})
		end
	end
end
...
function modifier_bounty_hunter_jinada_custom_passive:GetModifierPreAttack_BonusDamage(params)
	if IsClient() and self:GetCaster():HasModifier("modifier_bounty_hunter_jinada_custom_crit") then
		...
		if self:GetCaster():HasModifier("modifier_bounty_hunter_17") then
			bonus = self:GetAbility().modifier_bounty_hunter_17_dmg[self:GetCaster()
				:GetTalentLevel("modifier_bounty_hunter_17")]
		end
	...
	if
		(
			(
				self:GetCaster():HasModifier("modifier_bounty_hunter_jinada_custom_crit")
...
function modifier_bounty_hunter_jinada_custom_crit:IsPurgable()
	return false
end
function modifier_bounty_hunter_jinada_custom_crit:IsHidden()
	return true
end
function modifier_bounty_hunter_jinada_custom_crit:RemoveOnDeath()
	return false
end
...
function modifier_bounty_hunter_jinada_custom_crit:OnCreated()
	...
	local particle_glow_fx_2 = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_bounty_hunter/bounty_hunter_hand_l.vpcf",
		PATTACH_ABSORIGIN,
		self:GetParent()
	)
...
function modifier_bounty_hunter_shuriken_toss_custom_gold:IsHidden()
	return true
end
function modifier_bounty_hunter_shuriken_toss_custom_gold:IsPurgable()
	return false
end
function modifier_bounty_hunter_shuriken_toss_custom_gold:IsPurgeException()
	return false
end
