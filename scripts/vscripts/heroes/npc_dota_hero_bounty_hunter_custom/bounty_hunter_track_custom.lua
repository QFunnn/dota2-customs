...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_track_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bounty_hunter_track_custom_purge",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_track_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bounty_hunter_track_custom_buff",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_track_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_bounty_hunter_track_custom_passive",
	"heroes/npc_dota_hero_bounty_hunter_custom/bounty_hunter_track_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function bounty_hunter_track_custom:CastFilterResultTarget(target)
	if self:GetCaster():HasModifier("modifier_bounty_hunter_21") then
		return
	end
...
function bounty_hunter_track_custom:OnSpellStart()
	...
	local particle_projectile_fx = ParticleManager:CreateParticleForTeam(
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_18") then
		local bounty_hunter_shuriken_toss_custom = self:GetCaster()
			:FindAbilityByName("bounty_hunter_shuriken_toss_custom")
...
function modifier_bounty_hunter_track_custom_purge:IsHidden()
	return true
end
function modifier_bounty_hunter_track_custom_purge:IsPurgable()
	return not self:GetCaster():HasModifier("modifier_bounty_hunter_5")
end
function modifier_bounty_hunter_track_custom_purge:IsPurgeException()
	return not self:GetCaster():HasModifier("modifier_bounty_hunter_5")
end
...
function modifier_bounty_hunter_track_custom:IsPurgable()
	return true
end
function modifier_bounty_hunter_track_custom:IsPurgeException()
	return true
end
...
function modifier_bounty_hunter_track_custom:OnCreated()
	...
	local particle_shield_fx = ParticleManager:CreateParticleForTeam(
	...
	local particle_trail_fx = ParticleManager:CreateParticleForTeam(
...
function modifier_bounty_hunter_track_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_bounty_hunter_3") then
		AddFOWViewer(
			self:GetCaster():GetTeamNumber(),
			self:GetParent():GetAbsOrigin(),
			self:GetAbility().modifier_bounty_hunter_3[self:GetCaster():GetTalentLevel("modifier_bounty_hunter_3")],
			FrameTime(),
			false
		)
		return
...
function modifier_bounty_hunter_track_custom:OnDeath(keys)
	...
	for _, ally in pairs(allies) do
		if
			self:GetParent():IsRealHero()
			and ally:IsRealHero()
			and not ally:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier")
		then
			...
			if self:GetCaster():HasModifier("modifier_bounty_hunter_16") then
				gold = gold
					+ self:GetAbility().modifier_bounty_hunter_16_gold[self:GetCaster()
						:GetTalentLevel("modifier_bounty_hunter_16")]
			end
...
function modifier_bounty_hunter_track_custom:GetModifierIncomingDamage_Percentage(params)
	if params.attacker:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		local bonus = 0
		if self:GetCaster():HasModifier("modifier_bounty_hunter_16") then
			bonus = self:GetAbility().modifier_bounty_hunter_16_damage[self:GetCaster()
				:GetTalentLevel("modifier_bounty_hunter_16")]
		end
...
function modifier_bounty_hunter_track_custom_buff:OnCreated()
	...
	local particle_haste_fx = ParticleManager:CreateParticle(
...
function modifier_bounty_hunter_track_custom_buff:GetModifierHealthRegenPercentage()
	if self:GetCaster():HasModifier("modifier_bounty_hunter_6") then
		return self:GetAbility().modifier_bounty_hunter_6_regen[self:GetCaster()
			:GetTalentLevel("modifier_bounty_hunter_6")]
	end
end

modifier_bounty_hunter_track_custom_passive = class({})

function modifier_bounty_hunter_track_custom_passive:IsPurgable()
	return false
end
function modifier_bounty_hunter_track_custom_passive:IsHidden()
	return true
end
			...
			if params.target:HasModifier("modifier_bounty_hunter_track_custom") then
				SendOverheadEventMessage(
					...
					params.original_damage
						+ (params.original_damage / 100 * (self:GetAbility().modifier_bounty_hunter_21 - 100)),
					nil
				)
