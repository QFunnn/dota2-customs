...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_axe_custom/axe_culling_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_culling_blade_custom_handler",
	"heroes/npc_dota_hero_axe_custom/axe_culling_blade_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function axe_culling_blade_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_axe_6") then
		...
		local units = FindUnitsInRadius(
			...
			point,
			nil,
			self:GetCaster():GetAoeBonus(self.modifier_axe_6_radius),
...
function axe_culling_blade_custom:CullingTarget(target)
	...
	if self:GetCaster():HasModifier("modifier_axe_5") then
		damage = damage
			+ (
				self.modifier_axe_5[self:GetCaster():GetTalentLevel("modifier_axe_5")]
		...
		if target and not target:IsNull() then
			...
			if target:IsRealHero() and not target:IsAlive() then
				local modifier_axe_culling_blade_custom_handler =
					self:GetCaster():FindModifierByName("modifier_axe_culling_blade_custom_handler")
...
function modifier_axe_culling_blade_custom_handler:IsPurgable()
	return false
end
function modifier_axe_culling_blade_custom_handler:IsPurgeException()
	return false
end
function modifier_axe_culling_blade_custom_handler:RemoveOnDeath()
	return false
end
