...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_axe_custom/axe_berserkers_call_custom",
...
function axe_berserkers_call_custom:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_axe_berserkers_call_custom_buff", { duration = duration })
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_axe_15") then
			local damage = self.modifier_axe_15
				+ self:GetCaster():GetIntellect(false)
					/ 100
					* self.modifier_axe_15_int[self:GetCaster():GetTalentLevel("modifier_axe_15")]
			ApplyDamage({
				victim = enemy,
		...
		if
			self:GetCaster():HasModifier("modifier_axe_18")
			and axe_battle_hunger_custom
			and axe_battle_hunger_custom:GetLevel() > 0
		then
...
function modifier_axe_berserkers_call_custom_buff:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_axe_17") then
		self.bonus_armor = self.bonus_armor
			+ self:GetAbility().modifier_axe_17_armor[self:GetCaster():GetTalentLevel("modifier_axe_17")]
	end
end
...
function modifier_axe_berserkers_call_custom_fear:GetTexture()
	return "axe_21"
end
function modifier_axe_berserkers_call_custom_fear:IsPurgable()
	return false
end
