...
LinkLuaModifier(
	"modifier_axe_counter_helix_custom",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_counter_helix_custom_cooldown",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_counter_helix_custom_active",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_counter_helix_custom_stack_counter",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_berserkers_call_custom_blade_fury",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_axe_berserkers_call_custom_blade_fury_counter",
	"heroes/npc_dota_hero_axe_custom/axe_counter_helix_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function axe_counter_helix_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/juggernaut/jugg_ti8_sword/juggernaut_blade_fury_abyssal_golden.vpcf",
		context
	)
end
...
function modifier_axe_counter_helix_custom_active:GetTexture()
	return "axe_3"
end
function modifier_axe_counter_helix_custom_active:IsPurgable()
	return false
end
function modifier_axe_counter_helix_custom_active:IsPurgeException()
	return false
end
...
function modifier_axe_counter_helix_custom:IsPurgeException()
	return false
end
function modifier_axe_counter_helix_custom:RemoveOnDeath()
	return false
end
...
function modifier_axe_counter_helix_custom:OnAttackLanded(params)
	...
	local bValidAttack = (
		params.target == self:GetParent() and params.attacker:GetTeamNumber() ~= self:GetParent():GetTeamNumber()
	)
	local bValidCounterAttack = (params.attacker == self:GetParent() and self:GetParent():HasModifier("modifier_axe_7"))
	if not (bValidAttack or bValidCounterAttack) then
		return
	end
	if params.no_attack_cooldown then
		return
	end
	if self:GetCaster():HasModifier("modifier_axe_counter_helix_custom_cooldown") then
		return
	end
	if not self:GetCaster():HasModifier("modifier_axe_3") and not self:GetAbility():IsFullyCastable() then
		return
	end
...
function modifier_axe_counter_helix_custom:StartHelix()
	...
	if self:GetCaster():HasModifier("modifier_axe_16") then
		return
	end
	...
	if self:GetCaster():HasModifier("modifier_axe_13") then
		damage = damage
			+ self:GetCaster():GetAgility()
				/ 100
				* self:GetAbility().modifier_axe_13[self:GetCaster():GetTalentLevel("modifier_axe_13")]
	end
	if self:GetCaster():HasModifier("modifier_axe_9") then
	...
	for _, enemy in pairs(enemies) do
		...
		if self:GetCaster():HasModifier("modifier_axe_14") then
			enemy:AddNewModifier(
				...
				self:GetAbility(),
				"modifier_axe_counter_helix_custom_stack_counter",
				{ duration = self:GetAbility().modifier_axe_14_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
		if self:GetCaster():HasModifier("modifier_axe_9") then
			if
				enemy:GetUnitName() == "npc_woda_pig"
				or enemy:GetUnitName() == "npc_woda_frog"
				or enemy:GetUnitName() == "npc_woda_pig_pve"
				or enemy:GetUnitName() == "npc_woda_frog_pve"
				or enemy:GetUnitName() == "npc_dota_weaver_swarm"
			then
	...
	if self:GetCaster():HasModifier("modifier_axe_10") then
		local modifier_axe_berserkers_call_custom_blade_fury_counter =
			self:GetParent():FindModifierByName("modifier_axe_berserkers_call_custom_blade_fury_counter")
		if modifier_axe_berserkers_call_custom_blade_fury_counter == nil then
			modifier_axe_berserkers_call_custom_blade_fury_counter = self:GetParent():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_axe_berserkers_call_custom_blade_fury_counter",
				{}
		...
		if
			modifier_axe_berserkers_call_custom_blade_fury_counter:GetStackCount() >= self:GetAbility().modifier_axe_10
		then
			self:GetCaster():AddNewModifier(
				self:GetCaster(),
				self:GetAbility(),
				"modifier_axe_berserkers_call_custom_blade_fury",
				{ duration = self:GetAbility().modifier_axe_10_duration }
			)
...
function modifier_axe_counter_helix_custom_cooldown:IsHidden()
	return true
end
function modifier_axe_counter_helix_custom_cooldown:IsPurgable()
	return false
end
function modifier_axe_counter_helix_custom_cooldown:IsPurgeException()
	return false
end
function modifier_axe_counter_helix_custom_cooldown:RemoveOnDeath()
	return false
end

modifier_axe_counter_helix_custom_stack_counter = class({})
function modifier_axe_counter_helix_custom_stack_counter:GetTexture()
	return "axe_14"
end
function modifier_axe_counter_helix_custom_stack_counter:OnCreated()
	...
	if self:GetStackCount() >= self:GetAbility().modifier_axe_14_max then
		return
	end
...
function modifier_axe_berserkers_call_custom_blade_fury_counter:GetTexture()
	return "axe_10"
end
function modifier_axe_berserkers_call_custom_blade_fury_counter:IsPurgable()
	return false
end
function modifier_axe_berserkers_call_custom_blade_fury_counter:IsPurgeException()
	return false
end
function modifier_axe_berserkers_call_custom_blade_fury_counter:RemoveOnDeath()
	return false
end

modifier_axe_berserkers_call_custom_blade_fury = class({})
function modifier_axe_berserkers_call_custom_blade_fury:GetTexture()
	return "axe_10"
end
function modifier_axe_berserkers_call_custom_blade_fury:IsPurgable()
	return false
end
function modifier_axe_berserkers_call_custom_blade_fury:IsPurgeException()
	return false
end
function modifier_axe_berserkers_call_custom_blade_fury:OnCreated(params)
	...
	self.dps = self:GetAbility().modifier_axe_10_damage[self:GetCaster():GetTalentLevel("modifier_axe_10")]
		/ 100
		* self:GetCaster():GetAgility()
