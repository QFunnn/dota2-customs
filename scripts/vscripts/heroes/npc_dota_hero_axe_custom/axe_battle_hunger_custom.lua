...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_axe_custom/axe_battle_hunger_custom",
...
function axe_battle_hunger_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_axe.vpcf",
		context
	)
end
...
function modifier_axe_battle_hunger_custom_debuff:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_axe_19") then
		self.armor_multiplier = self.armor_multiplier
			+ self:GetAbility().modifier_axe_19[self:GetCaster():GetTalentLevel("modifier_axe_19")]
	end
	if not IsServer() then
		return
	end
...
function modifier_axe_battle_hunger_custom_debuff:OnIntervalThink()
	...
	if self.damage_delay >= 0.5 then
		self.damage_delay = 0
		local damage = (
			self.damage_per_second + (self.armor_multiplier * self:GetCaster():GetPhysicalArmorValue(false))
		) * 0.5
		ApplyDamage({
