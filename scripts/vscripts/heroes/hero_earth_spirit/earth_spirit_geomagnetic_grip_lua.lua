...
LinkLuaModifier(
	"modifier_earth_spirit_geomagnetic_grip_lua",
	"heroes/hero_earth_spirit/earth_spirit_geomagnetic_grip_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_earth_spirit_geomagnetic_grip_lua_drag",
	"heroes/hero_earth_spirit/earth_spirit_geomagnetic_grip_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
...
function modifier_earth_spirit_geomagnetic_grip_lua:OnAbilityExecuted(params)
	...
	if IsServer() then
		if
			...
			and (not hInflictor:IsToggle())
			and (not hInflictor:IsItem())
			and hInflictor:GetEffectiveCooldown(-1) > 0
		then
	...
	then
		...
		for _, unit in pairs(units) do
			...
			if IsValid(unit) and unit:IsAlive() then
				...
				local fDamage = self.base_damage
					+ self.bonus_damage_pct * hParent:GetAverageTrueAttackDamage(hTarget) * 0.01
				ApplyDamage({
					victim = unit,
					attacker = hParent,
					ability = hAbility,
				...
				if IsValid(unit) and unit:IsAlive() then
					unit:AddNewModifier(
						...
						hAbility,
						"modifier_earth_spirit_geomagnetic_grip_lua_drag",
						{ duration = self.duration * unit:GetStatusResistanceFactor(hParent) }
					)
				end
			end
		end
