...
LinkLuaModifier(
	"modifier_pudge_flesh_heap_lua_block",
	"heroes/hero_pudge/pudge_flesh_heap_lua",
...
function pudge_flesh_heap_lua:OnHeroLevelUp()
	if self:GetLevel() == 0 or (not IsServer()) then
		return
	end
...
function modifier_pudge_flesh_heap_lua_block:OnCreated(kv) end

function modifier_pudge_flesh_heap_lua_block:IsHidden()
	return false
end

function modifier_pudge_flesh_heap_lua_block:RemoveOnDeath()
	return true
end
function modifier_pudge_flesh_heap_lua_block:IsPurgable()
	return true
end
function modifier_pudge_flesh_heap_lua_block:IsDebuff()
	return false
end
...
function modifier_pudge_flesh_heap_lua:OnCreated(kv)
	...
	self.listener = ListenToGameEvent(
		...
		Dynamic_Wrap(modifier_pudge_flesh_heap_lua, "OnPlayerLearnedAbility"),
		self
	)
	...
	self.stunned_chance = ability:GetSpecialValueFor("stunned_chance") or 0
	self.stunned_duration = ability:GetSpecialValueFor("stunned_duration") or 0
...
function modifier_pudge_flesh_heap_lua:OnAttackLanded(params)
	...
	if RollPercentage(self.stunned_chance) then
		params.attacker:AddNewModifier(
			self:GetParent(),
			self:GetAbility(),
			"modifier_stunned",
			{ duration = self.stunned_duration * (1 - params.attacker:GetStatusResistance()) }
		)
	end
end

function modifier_pudge_flesh_heap_lua:GetModifierBonusStats_Strength()
	return self.strength_bonus_total
end
...
function modifier_pudge_flesh_heap_lua:OnDeathEvent(keys)
	if not IsServer() or not keys.unit or not keys.attacker or keys.unit:IsReincarnating() then
		return
	end
	...
	if not (hVictim and caster and ability) then
		return
	end
	...
	if caster:GetTeamNumber() ~= hVictim:GetTeamNumber() and caster:IsAlive() then
		...
		if
			not hVictim:IsSummoned()
			and hVictim:GetUnitName() ~= "aghsfort_mars_bulwark_soldier"
			and (not hVictim:HasModifier("modifier_monkey_king_fur_army_soldier"))
			and (not hVictim:HasModifier("modifier_monkey_king_fur_army_soldier_hidden"))
		then
			local vToCaster = caster:GetOrigin() - hVictim:GetOrigin()
			local flDistance = vToCaster:Length2D()
			if self.fleshHeapRange >= flDistance then
				...
				if
					hVictim:IsRealHero()
					and not hVictim:IsTempestDouble()
					and not hVictim:HasModifier("modifier_arc_warden_tempest_double_lua")
				then
...
function modifier_pudge_flesh_heap_lua:AddCustomTransmitterData()
	...
	self.TransmitterTable.strength = self.strength_bonus_total
