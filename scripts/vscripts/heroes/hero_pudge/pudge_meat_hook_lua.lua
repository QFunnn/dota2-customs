...
LinkLuaModifier(
	"modifier_meat_hook_followthrough_lua",
	"heroes/hero_pudge/modifiers/modifier_meat_hook_followthrough_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_meat_hook_lua",
	"heroes/hero_pudge/modifiers/modifier_meat_hook_lua.lua",
...
function pudge_meat_hook_lua:OnSpellStart()
	...
	Timers:CreateTimer((self.hook_distance / self.hook_speed * 2 + 0.1), function()
		if
			self
			and self:IsNull() == false
			and self:GetCaster()
			and self:GetCaster():IsNull() == false
...
function pudge_meat_hook_lua:OnHookLanded(hTarget, vLocation)
	...
	if hTarget ~= nil and (not hTarget:IsInvisible()) and bTargetPulled then
		ParticleManager:SetParticleControlEnt(
			...
			PATTACH_POINT_FOLLOW,
			"attach_hitloc",
			hTarget:GetOrigin() + self.vHookOffset,
			true
		)
