...
LinkLuaModifier(
	...
	"heroes/hero_batrider/sticky_napalm",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ability_batrider_sticky_napalm_cast",
	"heroes/hero_batrider/sticky_napalm",
	LUA_MODIFIER_MOTION_NONE
)
...
modifier_ability_batrider_sticky_napalm = class({
	...
	GetModifierMagicalResistanceBonus = function(self)
		return -(self.ReducePerStack or 0) * self:GetStackCount()
	end,
	GetModifierMoveSpeedBonus_Percentage = function(self)
		return -(self.Movespeed or 0) * self:GetStackCount()
	end,
	GetModifierTurnRate_Percentage = function(self)
		return -(self.TurnRate or 0)
	end,
...
function modifier_ability_batrider_sticky_napalm:OnCreated(table)
	...
	if IsServer() then
		self.FxStack = ParticleManager:CreateParticleForTeam(
		...
		self.FxStatus = ParticleManager:CreateParticle(
...
function modifier_ability_batrider_sticky_napalm:TakeDamageScriptModifier(event)
	...
	if
		...
		and attacker == caster
		and unit == parent
		and (not inflictor or not self.non_trigger_inflictors[inflictor:GetName()])
		and bit:_and(damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) ~= DOTA_DAMAGE_FLAG_REFLECTION
	then
		local fx = ParticleManager:CreateParticle(
...
function modifier_ability_batrider_sticky_napalm_cast:OnAttackLanded(event)
	...
	if target:IsBuilding() or target:IsTower() or target:IsOther() then
		return
	end
	...
	target:AddNewModifier(
		...
		ability,
		"modifier_ability_batrider_sticky_napalm",
		{ duration = Duration, count = Stacks }
	)
end
...
function modifier_ability_batrider_sticky_napalm_cast:OnOrderCast(keys)
	...
	if keys.ability == self:GetAbility() then
		if
			...
			and (keys.new_pos - self:GetCaster():GetAbsOrigin()):Length2D()
				<= self:GetAbility():GetEffectiveCastRange(self:GetCaster():GetCursorPosition(), self:GetCaster())
		then
...
function modifier_ability_batrider_sticky_napalm_cast:OnModifierAdded(event)
	...
	if
		...
		and ability
		and unit
		and caster == buffCaster
		and buffAbility
		and buffAbility:GetName() == "batrider_flamebreak"
		and caster:HasAbility("batrider_flamebreak")
		and buff:GetName() == "modifier_flamebreak_damage"
	then
		local Duration = ability:GetSpecialValueFor("duration")
