...
LinkLuaModifier(
	"modifier_enigma_demonic_conversion_lua_strike",
	"heroes/hero_enigma/enigma_demonic_conversion_lua_strike.lua",
...
function modifier_enigma_demonic_conversion_lua_strike:OnIntervalThink()
	...
	if
		...
		and hAbility:GetEffectiveManaCost(-1) <= hParent:GetMana()
	then
		...
		if #tTargets < target_count then
			local units = FindUnitsInRadius(
				...
				nil,
				hAbility:GetCastRange(hParent:GetAbsOrigin(), nil),
