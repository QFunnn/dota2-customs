...
LinkLuaModifier(
	"modifier_enigma_demonic_conversion_lua",
	"heroes/hero_enigma/enigma_demonic_conversion_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_enigma_demonic_conversion_lua_summoned",
	"heroes/hero_enigma/enigma_demonic_conversion_lua.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
	...
	if IsValid(hCaster) then
		...
		if IsValid(self.eidolon) and self.eidolon:IsAlive() then
			...
			if self.eidolon:GetUnitName() ~= NameList[self:GetLevel()] then
				...
				if tSummonBuff then
					self.eidolon:AddNewModifier(
						hCaster,
						self,
						"modifier_enigma_demonic_conversion_lua_summoned",
						{ duration = tSummonBuff:GetRemainingTime() + eidolon_duration }
					)
				end
			end
		else
...
function modifier_enigma_demonic_conversion_lua:OnDeath(params)
	...
	if IsServer() then
		if IsValid(hAbility) then
			...
			elseif
				IsValid(hAbility.eidolon)
				and hAbility.eidolon:IsAlive()
				and hAttacker == hAbility.eidolon
				and hParent:GetTeamNumber() ~= hUnit:GetTeamNumber()
			then
