...
function chen_holy_persuasion_custom:OnSpellStart()
	...
	for i = #self.created_units, 1, -1 do
		if self.created_units[i] ~= nil then
			if
				not self.created_units[i]:IsNull()
				and self.created_units[i]:IsAlive()
				and self.created_units[i]:GetTeamNumber() == self:GetCaster():GetTeamNumber()
			then
		...
		if random_unit ~= nil and count ~= nil and duration ~= nil then
			for i = 1, count do
				local unit = CreateUnitByName(
					random_unit,
					self:GetCaster():GetAbsOrigin() + self:GetCaster():GetForwardVector() * RandomInt(200, 500),
					true,
					self:GetCaster(),
					self:GetCaster(),
				...
				if unit then
					print("[CHEN DEBUG] Spawned: " .. random_unit .. " | Model: " .. unit:GetModelName())
