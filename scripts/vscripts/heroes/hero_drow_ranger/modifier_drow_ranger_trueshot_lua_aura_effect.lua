...
function modifier_drow_ranger_trueshot_lua_aura_effect:GetModifierPreAttack_BonusDamage( params )
	...
	if caster and ability and caster.GetAgility then
	   local bonus = self:GetAbility():GetSpecialValueFor("trueshot_ranged_damage") / 100 * self:GetAbility():GetCaster():GetAgility()
	   if _G.Players and _G.Players.QueueAttackBonus and params and params.attacker and params.target then
	       _G.Players:QueueAttackBonus(params.attacker, params.target, bonus, "drow_ranger_trueshot", DAMAGE_TYPE_PHYSICAL)
	   end
	   return bonus
    end
end
