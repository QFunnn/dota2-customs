...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_fueling_hell",
...
function doom_bringer_fueling_hell:OnSpellStart()
	...
	local pulse = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_doom_bringer_fueling_hell", {
	...
	for _, target in pairs(targets) do
		if not self.targets[target] then
			if (not self.IsCircle) or (target:GetOrigin() - self.origin):Length2D() > (radius - self.width) then
				...
				local doom_bringer_infernal_blade_custom = self:GetCaster()
					:FindAbilityByName("doom_bringer_infernal_blade_custom")
