...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_doom_custom",
...
LinkLuaModifier(
	"modifier_doom_bringer_doom_custom_cooldown",
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_doom_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function doom_bringer_doom_custom:ApplyDoom(target, duration)
	...
	if self:GetCaster():HasModifier("modifier_doom_bringer_3") then
		local modifier_doom_bringer_doom_custom_aura =
			target:FindModifierByName("modifier_doom_bringer_doom_custom_aura")
		if
			modifier_doom_bringer_doom_custom_aura
			and modifier_doom_bringer_doom_custom_aura:GetRemainingTime() > duration
		then
			return
		end
	...
	if modifier_doom_bringer_doom_custom and modifier_doom_bringer_doom_custom:GetRemainingTime() > duration then
		return
	end
...
function modifier_doom_bringer_doom_custom:OnCreated(kv)
	...
	if
		self:GetCaster():HasModifier("modifier_doom_bringer_5")
		and not self:GetParent():HasModifier("modifier_doom_bringer_doom_custom_cooldown")
	then
		local percent_damage = self:GetCaster():GetMaxHealth()
			/ 100
			* self:GetAbility().modifier_doom_bringer_5[self:GetCaster():GetTalentLevel("modifier_doom_bringer_5")]
		ApplyDamage({
...
function modifier_doom_bringer_doom_custom:OnIntervalThink()
	...
	if self:GetCaster():HasModifier("modifier_doom_bringer_4") then
		local bonus_damage = self:GetAbility().modifier_doom_bringer_4[self:GetCaster()
			:GetTalentLevel("modifier_doom_bringer_4")] * (self:GetCaster():GetMaxHealth() / self:GetAbility().modifier_doom_bringer_4_hp)
...
function modifier_doom_bringer_doom_custom_aura:OnCreated()
	...
	ParticleManager:SetParticleControl(
		particle,
		1,
		Vector(self:GetCaster():GetAoeBonus(self:GetAbility().modifier_doom_bringer_3), 0, 0)
	)
...
function modifier_doom_bringer_doom_custom_aura:OnIntervalThink()
	...
	if
		(self:GetParent():PassivesDisabled() or self:GetCaster():HasModifier("modifier_disconnect_player_no_damage"))
		and self:GetCaster() == self:GetParent()
		and self:GetCaster():HasModifier("modifier_doom_bringer_7")
	then
...
function modifier_doom_bringer_doom_custom_aura:IsAura()
	if
		self:GetParent():PassivesDisabled()
		and self:GetCaster() == self:GetParent()
		and self:GetCaster():HasModifier("modifier_doom_bringer_7")
	then
		return false
...
function modifier_doom_bringer_doom_custom_aura:GetAuraEntityReject(target)
	if IsServer() then
		if
			...
			or target:GetTeamNumber() == self:GetCaster():GetTeamNumber()
			or target:HasModifier("modifier_wodawisp")
			or target:HasModifier("modifier_wodarelax")
		then
			return true
		else
			return false
...
LinkLuaModifier(
	"modifier_doom_bringer_creep_doom_custom",
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_doom_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_doom_bringer_creep_doom_custom_aura",
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_doom_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function doom_bringer_creep_doom_custom:OnSpellStart()
	...
	if
		not target:IsHero()
		and target:IsInvulnerable()
		and target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber()
	then
		self:EndCooldown()
		self:RefundManaCost()
		return
...
function modifier_doom_bringer_creep_doom_custom:GetAttributes()
...
function modifier_doom_bringer_creep_doom_custom:OnCreated(kv)
	...
	if
		self:GetCaster():HasModifier("modifier_doom_bringer_5")
		and not self:GetCaster():HasModifier("modifier_disconnect_player_no_damage")
	then
		local percent_damage = self:GetCaster():GetMaxHealth()
			/ 100
			* self:GetAbility().modifier_doom_bringer_5[self:GetCaster():GetTalentLevel("modifier_doom_bringer_5")]
...
function modifier_doom_bringer_creep_doom_custom_aura:GetAttributes()
	return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end
function modifier_doom_bringer_creep_doom_custom_aura:IsPurgable()
	return false
end
...
function modifier_doom_bringer_doom_custom_cooldown:IsHidden()
	return true
end
function modifier_doom_bringer_doom_custom_cooldown:IsPurgable()
	return false
end
function modifier_doom_bringer_doom_custom_cooldown:IsPurgeException()
	return false
end