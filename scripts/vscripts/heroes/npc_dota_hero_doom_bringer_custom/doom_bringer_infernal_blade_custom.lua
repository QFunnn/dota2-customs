...
LinkLuaModifier(
	"modifier_doom_bringer_infernal_blade_custom",
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_infernal_blade_custom",
...
function doom_bringer_infernal_blade_custom:OnOrbImpact(params)
	...
	params.target:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = ministun_duration * (1 - params.target:GetStatusResistance()) }
	)

	if
		doom_bringer_doom_custom
		and doom_bringer_doom_custom:GetLevel() > 0
		and self:GetCaster():HasModifier("modifier_doom_bringer_13")
	then
		doom_bringer_doom_custom:ApplyDoom(
			params.target,
			self.modifier_doom_bringer_13[self:GetCaster():GetTalentLevel("modifier_doom_bringer_13")]
		)
	end
		...
		for _, enemy in pairs(enemies) do
			if enemy ~= params.target then
				...
				enemy:AddNewModifier(
					...
					self,
					"modifier_stunned",
					{ duration = ministun_duration * (1 - enemy:GetStatusResistance()) }
				)
				if
					doom_bringer_doom_custom
					and doom_bringer_doom_custom:GetLevel() > 0
...
function modifier_doom_bringer_infernal_blade_custom_debuff:OnCreated(kv)
	...
	if self:GetCaster():HasModifier("modifier_doom_bringer_10") then
		self.damage = self.damage
			+ self:GetAbility().modifier_doom_bringer_10[self:GetCaster():GetTalentLevel("modifier_doom_bringer_10")]
	end
