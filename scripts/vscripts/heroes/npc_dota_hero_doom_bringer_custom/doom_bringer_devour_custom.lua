...
LinkLuaModifier(
	...
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_devour_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_doom_bringer_devour_custom_handler",
	"heroes/npc_dota_hero_doom_bringer_custom/doom_bringer_devour_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function doom_bringer_devour_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/new_year/anniversary_10th_hat_ambient_npc_dota_hero_doom_bringer.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/new_year_2/anniversary_10th_hat_ambient_npc_dota_hero_doom_bringer.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_npc_dota_hero_doom_bringer.vpcf",
		context
	)
end
...
function doom_bringer_devour_custom:OnSpellStart()
	...
	if self:GetCaster():HasModifier("modifier_doom_bringer_21") and target == nil then
		...
		self:GetCaster()
			:AddNewModifier(self:GetCaster(), self, "modifier_doom_bringer_devour_custom", { duration = duration })
...
function modifier_doom_bringer_devour_custom:OnCreated(kv)
	...
	self.modifier_doom_bringer_devour_custom_handler =
		self:GetParent():FindModifierByName("modifier_doom_bringer_devour_custom_handler")
...
function modifier_doom_bringer_devour_custom_handler:IsHidden()
	return true
end
function modifier_doom_bringer_devour_custom_handler:IsPurgable()
	return false
end
function modifier_doom_bringer_devour_custom_handler:IsPurgeException()
	return false
end
function modifier_doom_bringer_devour_custom_handler:RemoveOnDeath()
	return false
end
...
function modifier_doom_bringer_devour_custom_handler:GetModifierOverrideAbilitySpecial(data)
	if
		data.ability:GetAbilityName() == "doom_bringer_scorched_earth_custom"
		and data.ability_special_value == "radius"
		and self:GetParent():HasModifier("modifier_doom_bringer_19")
	then
		return 1
	end
end
...
function modifier_doom_bringer_devour_custom_handler:GetModifierOverrideAbilitySpecialValue(data)
	if
		...
		and self:GetParent():HasModifier("modifier_doom_bringer_19")
	then
		return data.ability:GetLevelSpecialValueNoOverride(data.ability_special_value, data.ability_special_level)
			+ (
				self:GetAbility().modifier_doom_bringer_19[self:GetCaster():GetTalentLevel("modifier_doom_bringer_19")]
				* self:GetStackCount()
			)
	end
end