...
function Pass:Init()
	ListenToGameEvent("game_rules_state_change", function(_)
		self:OnGameRulesStateChange()
	end, self)
	GameListener:SubscribeProtected("ban_ability", function(event)
		self:OnBanAbility(event)
	end)
	GameListener:SubscribeProtected("ban_hero", function(event)
		self:OnBanHero(event)
	end)
...
function Pass:TestAllBan()
	if IS_TEST_BAN_ACTIVATED == true then
		return
	end
...
function Pass:OnBanHero(event)
	...
	if
		self.banInfosByPlayerId[event.PlayerID].heroes.left <= 0
		and not self.banInfosByPlayerId[event.PlayerID].hasUnlimitedBans == true
	then
		return
	end

	local fullHeroName = "npc_dota_hero_" .. event.heroName
	if self.banHeroMap[fullHeroName] == true then
		return
	end
...
function Pass:OnBanAbility(event)
	...
	if
		self.banInfosByPlayerId[event.PlayerID].abilities.left <= 0
		and not self.banInfosByPlayerId[event.PlayerID].hasUnlimitedBans == true
	then
		return
	end
	...
	if self.banAbilitiesMap[event.abilityName] == true then
		return
	end
...
---@param playerId PlayerID
---@return boolean
function Pass:HasStreamerPlus(playerId)
...
function Pass:HasUnlimitedBans(playerId)
	return self:HasStreamerPlus(playerId)
end
