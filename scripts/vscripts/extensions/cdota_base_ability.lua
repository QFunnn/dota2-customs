...
function CDOTABaseAbility:SetRemovalTimer()
	...
	self.removalTimer = Timers:CreateTimer(flDelay, function()
...
function CDOTABaseAbility:IsNotRearmableAbility()
	local abilityName = self:GetAbilityName() or ""
	if GAMEPLAY_KV.NotRearmableAbilities and GAMEPLAY_KV.NotRearmableAbilities[abilityName] then
		return true
	end
...
function CDOTABaseAbility:IsInnateAbility()
	...
	return kv ~= nil and tonumber(kv.Innate) == 1
end