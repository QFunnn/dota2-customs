...
function CDOTA_Modifier_Lua:GetUpgradeValueFor(value_name)
	if self.upgrade_values and self.upgrade_values[value_name] then
	...
	if not GenericUpgrades.generic_upgrades_data[self.upgrade_name] then
		return
	end
	...
	if self:GetParent() then
		if
			rarity == "common" and self:GetParent():HasModifier("modifier_generic_common_stat_boost_upgrade_handler")
		then
			generic_boost = generic_boost
				+ self:GetParent()
					:GetModifierStackCount("modifier_generic_common_stat_boost_upgrade_handler", self:GetParent())
		end

		if rarity == "rare" and self:GetParent():HasModifier("modifier_generic_rare_stat_boost_upgrade_handler") then
			generic_boost = generic_boost
				+ self:GetParent()
					:GetModifierStackCount("modifier_generic_rare_stat_boost_upgrade_handler", self:GetParent())
		end
	end
