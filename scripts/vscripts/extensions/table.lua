...
function table.rank(t, order)
	...
	if order ~= nil and type(order) ~= "boolean" then
		error("Invalid order parameter - only nil / false / true are supported")
	end
	if order then
	...
	for _, value in ipairs(values) do
		if value and value == value then -- value == value is a NaN check
			if not seen_values[value] then
				seen_values[value] = current_rank
			end
...
function table.to_string(t)
	...
	for k, v in pairs(t) do
		if not done[k] then
			table.insert(result, key_to_string(k) .. " = " .. value_to_string(v))
		end
	end
...
function table.reduce(t, callback, initial)
	local acc = initial
	for k, v in pairs(t) do
		acc = callback(acc, v)
	end
