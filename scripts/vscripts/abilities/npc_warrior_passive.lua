...
function modifier_warrior_passive:IsHidden()
	return true
end
...
function modifier_warrior_passive:GetModifierProcAttack_Feedback(param)
	...
	if self:GetParent() == param.attacker and not param.target:IsMagicImmune() then
		...
		param.target:AddNewModifier(
			param.attacker,
			self:GetAbility(),
			"modifier_warrior_debuff",
			{ duration = duration * (1 - param.target:GetStatusResistance()) }
		)
	end
end
...
function modifier_warrior_debuff:IsHidden()
	return false
end
...
function modifier_warrior_debuff:GetModifierLifestealRegenAmplify_Percentage()
...
function modifier_warrior_debuff:GetEffectAttachType()
