...
function modifier_archer_passive:IsHidden()
	return true
end
...
function modifier_archer_passive:AttackRecordEvent_out(keys)
	if IsServer() then
		...
		if self:GetParent() == attacker then
			...
			if frost_attack then
				...
				self:GetParent()
					:SetRangedProjectileName("particles/econ/items/drow/drow_bow_monarch/drow_frost_arrow_monarch.vpcf")
			else
...
function modifier_archer_passive:AttackStartEvent_out(keys)
	if IsServer() then
		local attacker = keys.attacker
		local target = keys.target
		...
		if self:GetParent() == keys.attacker then
			...
			self:GetParent():AddNewModifier(
				...
				self:GetAbility(),
				"modifier_archer_cd",
				{ duration = self:GetAbility():GetCooldownTimeRemaining() }
			)
		end
	end
end

function modifier_archer_passive:GetModifierProcAttack_Feedback(keys)
	if IsServer() then
		local attacker = keys.attacker
		local target = keys.target
...
function modifier_archer_debuff:IsHidden()
	return false
end
...
function modifier_archer_cd:IsHidden()
	return false
end
function modifier_archer_cd:IsPurgable()
	return false
end