...
function modifier_prawler_clap:IsPurgable()
	return false
end

function modifier_prawler_clap:IsHidden()
	return true
end
...
function modifier_prawler_clap:EndCast()
	...
	for _, target in pairs(self.parent:FindTargets(self.aoe)) do
		target:AddNewModifier(
			...
			{ duration = self.root * (1 - target:GetStatusResistance()) }
		)
...
function modifier_prawler_root_custom:IsHidden()
	return true
end
...
function modifier_prawler_armor_custom:IsHidden()
	return false
end
