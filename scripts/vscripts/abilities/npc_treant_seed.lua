...
function npc_treant_seed:OnSpellStart()
	...
	self:GetCursorTarget():AddNewModifier(
		...
		{ duration = self:GetSpecialValueFor("duration") * (1 - self:GetCursorTarget():GetStatusResistance()) }
	)
end
...
function modifier_treant_seed:IsHidden()
	return false
end
...
function modifier_treant_seed_slow:IsHidden()
	return false
end
...
function modifier_treant_seed_cd:IsHidden()
	return false
end
function modifier_treant_seed_cd:IsPurgable()
	return false
end