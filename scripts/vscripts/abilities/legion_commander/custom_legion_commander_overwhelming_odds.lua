...
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_speed",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_slow",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_tracker",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_legendary",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_proc_charge",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
...
LinkLuaModifier(
	"modifier_overwhelming_odds_custom_shield",
	"abilities/legion_commander/custom_legion_commander_overwhelming_odds",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_legion_commander_overwhelming_odds:OnAbilityPhaseStart()
	...
	self.cast = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_legion_commander/legion_commander_odds_cast.vpcf",
...
function custom_legion_commander_overwhelming_odds:ProcOdds(point, source, buffed)
	...
	local particle_damage = wearables_system:GetParticleReplacementAbility(
		self.caster,
		"particles/units/heroes/hero_legion_commander/legion_commander_odds_dmga.vpcf",
		self
	)
	local sound_cast =
		wearables_system:GetSoundReplacement(self.caster, "Hero_LegionCommander.Overwhelming.Location", self)
		...
		if not source then
			...
			if self.talents.has_h4 == 1 then
				enemy:AddNewModifier(
					...
					{ duration = (1 - enemy:GetStatusResistance()) * self.talents.h4_silence }
				)
			end
		end
...
function modifier_overwhelming_odds_custom_speed:IsPurgable()
	return true
end
...
function modifier_overwhelming_odds_custom_tracker:DamageEvent_out(params)
	...
	if params.custom_flag == "lc_odds_duel" then
		return
	end
	if
		params.unit:HasModifier("modifier_legion_commander_duel_custom_buff")
		or params.unit:HasModifier("modifier_legion_commander_duel_custom_legendary_unit")
	then
		return
	end
	...
	params.unit:AddNewModifier(
		...
		self.ability,
		"modifier_overwhelming_odds_custom_damage",
		{ duration = self.ability.talents.q3_duration, damage = params.original_damage }
	)
end
...
function modifier_overwhelming_odds_custom_legendary:OnCreated(table)
	...
	self.effect = ParticleManager:CreateParticle(
		"particles/legion_commander/odds_legendary_aoe.vpcf",
...
function modifier_overwhelming_odds_custom_legendary:OnIntervalThink(first)
	...
	if
		not IsValid(self.current_target)
		or not self.current_target:IsAlive()
		or not self.current_target:IsRealHero()
	then
	...
	self.caster:UpdateUIshort({
		time = self.max_duration - self.total_count,
		max_time = self.max_duration,
		stack = self.proc_count,
		style = "LegionOdds",
		priority = 0,
		active = self.proc_count >= (self.max - 1),
	})
end
...
function modifier_overwhelming_odds_custom_slow:IsPurgable()
	return self.ability.talents.has_h4 == 0
end
function modifier_overwhelming_odds_custom_slow:GetEffectName()
...
function modifier_overwhelming_odds_custom_proc_charge:StatusEffectPriority()
	...
	if not IsValid(self.ability.shield_mod) then
		...
		if self.ability.shield_mod then
			...
			self.particle = ParticleManager:CreateParticle(
				"particles/bristleback/back_shield.vpcf",
				PATTACH_CUSTOMORIGIN_FOLLOW,
				self.parent
			)
			ParticleManager:SetParticleControlEnt(
