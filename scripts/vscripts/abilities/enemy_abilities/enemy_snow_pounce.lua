...
local a = "abilities/enemy_abilities/enemy_snow_pounce"
...
function n.prototype.OnSpellStart(self)
	...
	o:EmitSound("Hero_Lycan.Howl")
	o:AddNewModifier(
		o,
		self,
		"modifier_enemy_snow_pounce",
		{ direction = VectorToString(self.direction), duration = t / r }
	)
end
n = e({ m(nil) }, n)
local v = c()
v.name = "modifier_enemy_snow_pounce"
d(v, h)
...
function v.prototype.OnCreated(self, w)
	if IsServer() then
		...
		self.parent:AddActivityModifier("forcestaff_friendly")
		self.parent:StartGesture(ACT_DOTA_FLAIL)
		self.parent:RemoveActivityModifier("forcestaff_friendly")
	end
end
