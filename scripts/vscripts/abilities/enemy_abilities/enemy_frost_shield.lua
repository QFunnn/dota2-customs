...
local a = "abilities/enemy_abilities/enemy_frost_shield"
...
function o.prototype.OnSpellStart(self)
	local p = self:GetCursorTarget()
	if IsValid(p) then
		p:AddNewModifier(p, self, "modifier_enemy_frost_shield", {})
	end
end
o = f(
	{ n(nil, {
...
local t = c()
t.name = "modifier_enemy_frost_shield"
d(t, i)
...
function t.prototype.OnCreated(self, u)
	...
	if IsServer() then
		v:AddShield(self.shield, "enemy_frost_shield", "override", "permanent")
		...
		ParticleManager:SetParticleControl(w, 1, Vector(100, 100, 100))
		ParticleManager:SetParticleControlEnt(w, 2, v, PATTACH_ABSORIGIN_FOLLOW, nil, v:GetAbsOrigin(), true)
...
function t.prototype.OnDestroy(self)
	if IsServer() then
		local v = self:GetParent()
		v:RemoveShield("enemy_frost_shield")
...
function t.prototype.EventListener(self)
	return {
		consume_shield = function(q, x)
			if self:GetParent():GetShield("enemy_frost_shield") <= 0 then
				self:Destroy()
			end
		end,
	}
