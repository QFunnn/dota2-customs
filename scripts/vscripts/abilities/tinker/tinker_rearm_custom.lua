...
LinkLuaModifier(
	"modifier_tinker_rearm_custom_tracker",
...
LinkLuaModifier(
	"modifier_tinker_rearm_custom_damage_cd",
	"abilities/tinker/tinker_rearm_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_tinker_rearm_custom_damage_cd_creeps",
	"abilities/tinker/tinker_rearm_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function tinker_rearm_custom:OnSpellStart()
	...
	self.caster:AddNewModifier(
		self.caster,
		self,
		"modifier_tinker_rearm_custom",
		{ duration = self:GetChannelTime(), bkb = bkb }
	)
end
...
function tinker_rearm_custom:OnChannelFinish(interrupted)
	...
	if self.talents.has_r3 == 1 and count > 0 then
		self.caster:AddNewModifier(
			...
			self,
			"modifier_tinker_rearm_custom_spell_damage",
			{ duration = self.talents.r3_duration, stack = count }
		)
	end
...
function modifier_tinker_rearm_custom:OnCreated(table)
	...
	if table.bkb == 1 then
		...
		self.mod =
			self.parent:AddNewModifier(self.parent, self.ability, "modifier_tinker_rearm_custom_invun", { effect = 2 })
...
function modifier_tinker_rearm_custom_tracker:GetModifierPercentageCooldown(params)
	...
	return math.min(self.parent:GetIntellect(false) / self.int_per_cdr, self.max_cdr)
		+ (self.ability.talents.has_r4 == 1 and self.ability.talents.r4_cdr or 0)
end
...
function modifier_tinker_rearm_custom_tracker:DamageEvent_out(params)
	...
	if self.parent:HasModifier(mod_name) then
		return
	end
	if not params.inflictor or params.inflictor:IsItem() then
		return
	end
	if params.custom_flag == CUSTOM_FLAG_TINKER_REARM then
		return
	end
	...
	local damageTable = {
		...
		custom_flag = CUSTOM_FLAG_TINKER_REARM,
	}

	local effect_cast =
		ParticleManager:CreateParticle("particles/tinker/laser_proc_damage.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, unit)
	ParticleManager:SetParticleControlEnt(
		effect_cast,
		0,
		unit,
...
function modifier_tinker_rearm_custom_invun_cd:GetTexture()
	return "buffs/tinker/hero_6"
end

modifier_tinker_rearm_custom_spell_damage = class(mod_visible)
function modifier_tinker_rearm_custom_spell_damage:GetTexture()
	return "buffs/tinker/rearm_3"
end
