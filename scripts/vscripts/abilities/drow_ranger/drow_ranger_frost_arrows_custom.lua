...
LinkLuaModifier(
	"modifier_drow_ranger_frost_arrows_custom_root_cd",
	"abilities/drow_ranger/drow_ranger_frost_arrows_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function drow_ranger_frost_arrows_custom:ApplyHaste()
	...
	if not self:IsTrained() or self.talents.has_haste == 0 then
		return
	end
	local caster = self:GetCaster()
	caster:AddNewModifier(
		...
		self,
		"modifier_drow_ranger_frost_arrows_custom_haste",
		{ duration = self.talents.haste_duration }
	)
end
...
function drow_ranger_frost_arrows_custom:ApplySlow(target, is_illusion)
	...
	target:AddNewModifier(
		caster,
		caster:BkbAbility(self, self.talents.has_root == 1),
		"modifier_drow_ranger_frost_arrows_custom_slow",
...
function modifier_drow_ranger_frost_arrows_custom_root_cd:RemoveOnDeath()
	return false
end
