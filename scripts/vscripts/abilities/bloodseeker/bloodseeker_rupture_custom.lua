...
LinkLuaModifier(
	"modifier_bloodseeker_rupture_custom_tracker",
	"abilities/bloodseeker/bloodseeker_rupture_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_bloodseeker_rupture_custom:OnCreated()
	...
	DoDamage({
		...
		victim = self.parent,
		damage = self.parent:GetMaxHealth() * self.hp_pct,
	...
	self.timer_particle = self.parent:GenericParticle(
...
function modifier_bloodseeker_rupture_custom:OnIntervalThink(first)
	...
	if self.parent:IsCreep() then
		DoDamage({
			victim = self.parent,
			attacker = self.caster,
			damage = self.creeps_damage * bonus_damage,
			damage_type = DAMAGE_TYPE_PURE,
...
function modifier_bloodseeker_rupture_custom:SpellEvent(params)
	...
	if self.scepter_count >= self.ability.scepter_max then
		return
	end
...
function modifier_bloodseeker_rupture_custom_caster:RemoveOnDeath()
	return false
end
...
function modifier_bloodseeker_rupture_custom_legendary:GetStatusEffectName()
	return "particles/status_fx/status_effect_gods_strength.vpcf"
end
function modifier_bloodseeker_rupture_custom_legendary:StatusEffectPriority()
	return MODIFIER_PRIORITY_ULTRA
end
...
function modifier_bloodseeker_rupture_custom_legendary:OnIntervalThink()
	...
	self.target:AddNewModifier(
		...
		self.ability,
		"modifier_bloodseeker_rupture_custom_legendary_knockback",
		{ distance = self.knockback_distance, duration = self.knockback_duration }
	)
end
