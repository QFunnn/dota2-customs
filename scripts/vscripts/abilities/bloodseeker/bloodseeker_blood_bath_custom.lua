...
LinkLuaModifier(
	"modifier_bloodseeker_blood_bath_custom_silence",
	"abilities/bloodseeker/bloodseeker_blood_bath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_bloodseeker_blood_bath_custom_thinker:OnDestroy(kv)
	...
	for _, enemy in pairs(self.caster:FindTargets(self.radius, self.center)) do
		...
		enemy:AddNewModifier(
			...
			self.ability,
			"modifier_bloodseeker_blood_bath_custom_silence",
			{ duration = self.silence_duration * (1 - enemy:GetStatusResistance()) }
		)
	...
	if (self.caster:GetAbsOrigin() - self.center):Length2D() <= self.radius then
		...
		if
			self.ability.talents.has_h2 == 1
			and self.caster.sanguivore_ability
			and IsValid(self.caster.sanguivore_ability.tracker)
		then
...
function modifier_bloodseeker_blood_bath_custom_damage_inc:GetEffectName()
...
function modifier_bloodseeker_blood_bath_custom_legendary_self:GetEffectName()
	return "particles/bloodseeker/bloodrage_shield.vpcf"
end
...
function modifier_bloodseeker_blood_mist_custom_health_reduce:GetTexture()
	return "buffs/bloodseeker/bloodrite_3"
end
...
function modifier_bloodseeker_blood_mist_custom_health_inc:GetTexture()
	return "buffs/bloodseeker/bloodrite_3"
end
function modifier_bloodseeker_blood_mist_custom_health_inc:GetAttributes()
