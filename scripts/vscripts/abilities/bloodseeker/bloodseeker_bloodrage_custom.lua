...
function bloodseeker_bloodrage_custom:UpdateTalents()
	...
	if caster:HasTalent("modifier_bloodseeker_bloodrage_2") then
		...
		self.talents.slow_move = caster:GetTalentValue("modifier_bloodseeker_bloodrage_2", "slow")
			/ self.talents.slow_max
		self.talents.slow_damage = caster:GetTalentValue("modifier_bloodseeker_bloodrage_2", "damage_reduce")
			/ self.talents.slow_max
	...
	if caster:HasTalent("modifier_bloodseeker_bloodrage_3") then
		...
		self.talents.bleed_damage = caster:GetTalentValue("modifier_bloodseeker_bloodrage_3", "damage")
			/ self.talents.bleed_duration
		caster:AddAttackEvent_out(self.tracker, true)
	end
...
function bloodseeker_bloodrage_custom:GetLegendaryBonus(ignore_talent)
	if self.talents.has_legendary == 0 and not ignore_talent then
		return 0
	end
	local caster = self:GetCaster()

	if not caster:HasModifier("modifier_bloodseeker_bloodrage_custom") and not ignore_talent then
		return 0
	end
	...
	local bonus = (ignore_talent and 100 or self.talents.legendary_bonus)
		* (1 - (health_percent - self.talents.legendary_health) / (100 - self.talents.legendary_health))
	return bonus
end
...
function modifier_bloodseeker_bloodrage_custom:GetModifierHealthRegenPercentage()
	if self.ability.talents.has_heal == 0 then
		return
	end
	if self.parent:GetHealthPercent() > self.ability.talents.heal_health then
		return
	end
...
function modifier_bloodseeker_bloodrage_custom_tracker:GetModifierMagicalResistanceBonus()
	return self.parent:GetHealthPercent() <= self.ability.talents.resist_health
			and self.ability.talents.magic_resist * self.ability.talents.resist_bonus
		or self.ability.talents.magic_resist
end

function modifier_bloodseeker_bloodrage_custom_tracker:GetModifierStatusResistanceStacking()
	return self.parent:GetHealthPercent() <= self.ability.talents.resist_health
			and self.ability.talents.status_resist * self.ability.talents.resist_bonus
		or self.ability.talents.status_resist
end
...
function modifier_bloodseeker_bloodrage_custom_tracker:AttackEvent_out(params)
	...
	if self.ability.talents.has_bleed == 1 and mod then
		target:AddNewModifier(
			...
			self.ability,
			"modifier_bloodseeker_bloodrage_custom_blood",
			{ duration = self.ability.talents.bleed_duration }
		)
	end
	...
	if self.ability.talents.has_slow == 1 then
		target:AddNewModifier(
			...
			self.ability,
			"modifier_bloodseeker_bloodrage_custom_slow",
			{ duration = self.ability.talents.slow_duration }
		)
	end
...
function modifier_bloodseeker_bloodrage_custom_blood:OnCreated(table)
	...
	self.table = {
		attacker = self.caster,
		victim = self.parent,
		damage_type = self.ability.talents.bleed_damage_type,
		ability = self.ability,
	}
...
function modifier_bloodseeker_bloodrage_custom_slow:IsHidden()
	return false
end
