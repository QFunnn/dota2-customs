...
function npc_ogre_root:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_ogre_root_cd", { duration = self:GetCooldownTimeRemaining() })
...
function modifier_ogre_root:OnCreated(table)
	...
	self.unit:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_ogre_unit",
		{ target = self:GetParent():entindex() }
	)
end
...
function modifier_ogre_root:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

modifier_ogre_unit = class({})

function modifier_ogre_unit:IsPurgable()
	return false
end
function modifier_ogre_unit:IsHidden()
	return true
end
...
function modifier_ogre_unit:OnIntervalThink()
	if
		...
		and self.target:HasModifier("modifier_ogre_root")
	then
...
function modifier_ogre_unit:AttackEvent_inc(param)
	if not IsServer() then
		return
	end
	...
	if self:GetParent() == param.target then
		...
		if self.hits <= 0 then
			if self.target:HasModifier("modifier_ogre_root") then
...
function modifier_ogre_root_cd:IsHidden()
	return false
end
function modifier_ogre_root_cd:IsPurgable()
	return false
end