...
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_tracker",
	"abilities/skywrath_mage/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_legendary_move",
	"abilities/skywrath_mage/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_miss_chance",
	"abilities/skywrath_mage/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_cd_count",
	"abilities/skywrath_mage/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_mystic_flare_custom_arcana_hit",
	"abilities/skywrath_mage/skywrath_mage_mystic_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skywrath_mage_mystic_flare_custom:OnAbilityPhaseStart()
	...
	if caster:HasTalent("modifier_sky_flare_7") then
		...
		caster:StartGestureWithPlaybackRate(
			ACT_DOTA_TELEPORT,
			math.min(1, 7.85 / self:GetCaster():GetTalentValue("modifier_sky_flare_7", "channel"))
		)
	else
...
function skywrath_mage_mystic_flare_custom:OnSpellStart(new_target)
	...
	if caster:HasTalent("modifier_sky_flare_6") then
		caster:AddNewModifier(
			...
			{ duration = caster:GetTalentValue("modifier_sky_flare_6", "bkb"), effect = 1 }
		)
	end
	...
	if caster:HasTalent("modifier_sky_flare_7") then
		caster:AddNewModifier(
			...
			self,
			"modifier_skywrath_mage_mystic_flare_custom_legendary_caster",
			{ duration = duration, thinker = self.thinker:entindex() }
		)
	end
end

modifier_skywrath_mage_mystic_flare_custom_thinker = class({})
function modifier_skywrath_mage_mystic_flare_custom_thinker:IsHidden()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:IsPurgable()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_thinker:OnCreated()
	...
	self.radius = self.ability:GetSpecialValueFor("radius")
		+ self.caster:GetTalentValue("modifier_sky_flare_1", "radius")
	self.damage = self.ability:GetSpecialValueFor("damage")
		+ self.caster:GetTalentValue("modifier_sky_flare_1", "damage")
	...
	self.cd_mod = self.caster:AddNewModifier(
		self.caster,
		self.ability,
		"modifier_skywrath_mage_mystic_flare_custom_cd_count",
	...
	local particle =
		wearables_system:GetParticleReplacementAbility(self:GetCaster(), "particles/skywrath/flare_base.vpcf", self)
	...
	ParticleManager:SetParticleControl(
		self.core_particle_fx,
		1,
		Vector(self.radius, self.duration, self.damage_interval)
	)
...
function modifier_skywrath_mage_mystic_flare_custom_thinker:OnIntervalThink()
	...
	for _, target in pairs(targets) do
		...
		if mod then
			...
			if
				mod:GetStackCount() >= self.stun_max
				and self.hit_targets[target:entindex()]
				and self.hit_targets[target:entindex()] == 0
			then
				...
				target:AddNewModifier(
					...
					self.ability,
					"modifier_stunned",
					{ duration = (1 - target:GetStatusResistance()) * self.stun_duration }
				)
			end
		end
...
function modifier_skywrath_mage_mystic_flare_custom_tracker:IsHidden()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_tracker:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_mystic_flare_custom_tracker:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_sky_flare_3") then
		return
	end
...
function modifier_skywrath_mage_mystic_flare_custom_tracker:SpellEvent(params)
	...
	if not self.parent:HasTalent("modifier_sky_flare_5") then
		return
	end
	if params.unit ~= self.parent then
		return
	end
	if params.ability:GetName() == "skywrath_mage_mystic_flare_custom_legendary" then
		return
	end
...
function modifier_skywrath_mage_mystic_flare_custom_legendary_caster:IsHidden()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_legendary_caster:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_mystic_flare_custom_legendary_caster:SetPos(x, y)
	...
	self.thinker:AddNewModifier(
		...
		self.ability,
		"modifier_skywrath_mage_mystic_flare_custom_legendary_move",
		{ x = x, y = y, duration = self.move_duration }
	)
end
...
function modifier_skywrath_mage_mystic_flare_custom_legendary_move:IsHidden()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_legendary_move:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_mystic_flare_custom_damage_inc:IsHidden()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_damage_inc:IsPurgable()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_damage_inc:GetTexture()
	return "buffs/flare_damage"
end
...
function modifier_skywrath_mage_mystic_flare_custom_miss_chance:IsPurgable()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_miss_chance:IsHidden()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_miss_chance:GetTexture()
	return "buffs/flare_miss"
end
function modifier_skywrath_mage_mystic_flare_custom_miss_chance:GetEffectName()
...
function modifier_skywrath_mage_mystic_flare_custom_miss_chance:GetModifierMiss_Percentage()
	if self.parent:HasModifier("modifier_tower_incoming_speed") then
		return
	end
...
function modifier_skywrath_mage_mystic_flare_custom_perma:IsHidden()
	return not self:GetCaster():HasTalent("modifier_sky_flare_5")
end
function modifier_skywrath_mage_mystic_flare_custom_perma:IsPurgable()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_perma:RemoveOnDeath()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_perma:GetTexture()
	return "buffs/flare_perma"
end
...
function modifier_skywrath_mage_mystic_flare_custom_cd_count:IsHidden()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_cd_count:IsPurgable()
	return false
end
function modifier_skywrath_mage_mystic_flare_custom_cd_count:RemoveOnDeath()
	return false
end

modifier_skywrath_mage_mystic_flare_custom_arcana_hit = class({})
function modifier_skywrath_mage_mystic_flare_custom_arcana_hit:IsHidden()
	return true
end
function modifier_skywrath_mage_mystic_flare_custom_arcana_hit:IsPurgable()
	return false
end
