...
LinkLuaModifier(
	"modifier_skywrath_mage_innate_custom",
	"abilities/skywrath_mage/skywrath_mage_innate_custom",
...
function modifier_skywrath_mage_innate_custom:IsHidden()
	return true
end
function modifier_skywrath_mage_innate_custom:IsPurgable()
	return false
end
function modifier_skywrath_mage_innate_custom:RemoveOnDeath()
	return false
end
...
function modifier_skywrath_mage_innate_custom:DamageEvent_out(params)
	...
	if not self.parent:CheckLifesteal(params, 1) then
		return
	end
	...
	if
		self.parent:HasTalent("modifier_sky_seal_4")
		and params.unit:HasModifier("modifier_skywrath_mage_ancient_seal_custom_silence")
	then
...
function modifier_skywrath_mage_innate_custom:HealEvent_inc(params)
	...
	if not self.parent:HasTalent("modifier_sky_seal_4") then
		return
	end
	...
	if not self.seal or not self.seal:IsTrained() then
		return
	end
