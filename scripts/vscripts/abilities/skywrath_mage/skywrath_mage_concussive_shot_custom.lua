...
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_slow",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_tracker",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_legendary_stack",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_legendary_effect",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_burn",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_burn_tracker",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_skywrath_mage_concussive_shot_custom_haste",
	"abilities/skywrath_mage/skywrath_mage_concussive_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function skywrath_mage_concussive_shot_custom:ApplyBurn(target)
	...
	if not caster:HasTalent("modifier_sky_concussive_4") then
		return
	end
...
function skywrath_mage_concussive_shot_custom:OnSpellStart()
	...
	if caster:HasTalent("modifier_sky_concussive_5") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_skywrath_mage_concussive_shot_custom_haste",
			{ duration = caster:GetTalentValue("modifier_sky_concussive_5", "duration") }
		)
	end
	...
	local heroes = FindUnitsInRadius(
		...
		DOTA_UNIT_TARGET_FLAG_NO_INVIS + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS,
		FIND_CLOSEST,
		false
	)
	...
	local failure_fx = wearables_system:GetParticleReplacementAbility(
		caster,
		"particles/units/heroes/hero_skywrath_mage/skywrath_mage_concussive_shot_failure.vpcf",
		self
	)
...
function skywrath_mage_concussive_shot_custom:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	local damage = self:GetSpecialValueFor("damage")
		+ caster:GetTalentValue("modifier_sky_concussive_1", "damage")
			* caster:GetAverageTrueAttackDamage(nil)
			/ 100
	local slow_duration = self:GetSpecialValueFor("slow_duration")
		+ caster:GetTalentValue("modifier_sky_concussive_3", "duration")
	local shot_vision = self:GetSpecialValueFor("shot_vision")
	...
	local knock = caster:HasTalent("modifier_sky_concussive_6")
		and vec:Length2D() <= knock_dist
		and self:GetAutoCastState() == true
	...
	for _, enemy in pairs(caster:FindTargets(slow_radius, target:GetAbsOrigin())) do
		...
		enemy:AddNewModifier(
			caster,
			caster:BkbAbility(self, caster:HasShard()),
			"modifier_skywrath_mage_concussive_shot_custom_slow",
			{ duration = (1 - enemy:GetStatusResistance()) * slow_duration }
		)
		...
		if caster:HasShard() and cast_ability == nil then
			enemy:AddNewModifier(
				...
				caster:BkbAbility(self, caster:HasShard()),
				"modifier_stunned",
				{ duration = (1 - enemy:GetStatusResistance()) * shard_stun }
			)
		end

		if
			knock == true
			and not enemy:IsCurrentlyHorizontalMotionControlled()
			and not enemy:IsCurrentlyVerticalMotionControlled()
		then
			local knockback = {
				should_stun = 0,
...
function modifier_skywrath_mage_concussive_shot_custom_slow:IsHidden()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_slow:IsPurgable()
	return not self:GetCaster():HasShard()
end
function modifier_skywrath_mage_concussive_shot_custom_slow:GetTexture()
...
function modifier_skywrath_mage_concussive_shot_custom_slow:OnCreated()
	...
	self.movement_speed_pct = self.ability:GetSpecialValueFor("movement_speed_pct")
		+ self.caster:GetTalentValue("modifier_sky_concussive_2", "slow")

	if not IsServer() then
		return
	end

	local slow_fx = wearables_system:GetParticleReplacementAbility(
		self.caster,
		"particles/units/heroes/hero_skywrath_mage/skywrath_mage_concussive_shot_slow_debuff.vpcf",
		self
	)
	...
	if not self.caster:HasTalent("modifier_sky_concussive_6") then
		return
	end
...
function modifier_skywrath_mage_concussive_shot_custom_slow:GetModifierDamageOutgoing_Percentage()
	if not self.caster:HasTalent("modifier_sky_concussive_2") then
		return
	end
...
function modifier_skywrath_mage_concussive_shot_custom_tracker:IsHidden()
	return true
end
function modifier_skywrath_mage_concussive_shot_custom_tracker:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_concussive_shot_custom_tracker:GetModifierBonusStats_Intellect()
	if not self.parent:HasTalent("modifier_sky_concussive_1") then
		return
	end
	return (self.parent:GetStrength() + self.parent:GetAgility())
		* self.parent:GetTalentValue("modifier_sky_concussive_1", "int")
		/ 100
end
...
function modifier_skywrath_mage_concussive_shot_custom_tracker:AttackStartEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_sky_concussive_7") then
		return
	end
...
function modifier_skywrath_mage_concussive_shot_custom_tracker:AttackEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_sky_concussive_4") then
		return
	end
	if self.parent ~= params.attacker then
		return
...
function modifier_skywrath_mage_concussive_shot_custom_tracker:SpellEvent(params)
	...
	if self.parent:HasModifier("modifier_skywrath_mage_concussive_shot_custom_legendary_effect") then
		return
	end
...
function modifier_skywrath_mage_concussive_shot_custom_legendary_stack:IsHidden()
	return true
end
function modifier_skywrath_mage_concussive_shot_custom_legendary_stack:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_concussive_shot_custom_legendary_stack:OnRefresh(table)
	...
	if self:GetStackCount() >= self.max then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_skywrath_mage_concussive_shot_custom_legendary_effect",
			{ duration = self.parent:GetTalentValue("modifier_sky_concussive_7", "effect_duration") }
		)
		self:Destroy()
	end
end
...
function modifier_skywrath_mage_concussive_shot_custom_legendary_effect:IsHidden()
	return true
end
function modifier_skywrath_mage_concussive_shot_custom_legendary_effect:IsPurgable()
	return false
end
...
function modifier_skywrath_mage_concussive_shot_custom_burn:IsHidden()
	return true
end
function modifier_skywrath_mage_concussive_shot_custom_burn:IsPurgable()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_burn:GetAttributes()
...
function modifier_skywrath_mage_concussive_shot_custom_burn_tracker:IsHidden()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_burn_tracker:IsPurgable()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_burn_tracker:GetTexture()
	return "buffs/shot_burn"
end
...
function modifier_skywrath_mage_concussive_shot_custom_haste:IsHidden()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_haste:IsPurgable()
	return false
end
function modifier_skywrath_mage_concussive_shot_custom_haste:GetTexture()
	return "buffs/shot_haste"
end
