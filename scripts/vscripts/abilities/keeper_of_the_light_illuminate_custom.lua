...
LinkLuaModifier(
	...
	"abilities/keeper_of_the_light_illuminate_custom",
...
LinkLuaModifier(
	"modifier_keeper_of_the_light_illuminate_custom_disarmed",
	"abilities/keeper_of_the_light_illuminate_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function keeper_of_the_light_illuminate_custom:OnSpellStart()
	...
	self:GetCaster():AddNewModifier(
		...
		self,
		"modifier_keeper_of_the_light_illuminate_custom",
		{ duration = self:GetChannelTime(), index = index }
	)
end
...
function modifier_keeper_of_the_light_illuminate_custom:OnDestroy()
	...
	local total_damage = self:GetAbility():GetSpecialValueFor("total_damage")
		* math.max(0.5, self:GetElapsedTime() / self:GetDuration())
