...
LinkLuaModifier(
	"modifier_enigma_innate_custom_debuff",
...
function modifier_enigma_innate_custom_debuff:OnCreated()
	...
	self.damage_reduce = self.ability.damage_reduction
		+ (self.ability.talents.has_e4 == 1 and self.ability.talents.e4_damage_reduce or 0)
...
function modifier_enigma_innate_custom_debuff:OnIntervalThink()
	...
	if self.ability.talents.has_h4 == 1 and not self.parent:HasModifier("modifier_enigma_innate_custom_bonus_cd") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_enigma_innate_custom_bonus_cd",
			{ duration = self.ability.talents.h4_cd }
		)
		self.parent:AddNewModifier(
			self.caster,
			self.ability,
	...
	if
		(
			self.caster:GetHealthPercent() >= self.health
			or self.parent:HasModifier("modifier_enigma_innate_custom_bonus_effect")
		) and not self.effect
	then
	...
	if
		self.caster:GetHealthPercent() < self.health
		and not self.parent:HasModifier("modifier_enigma_innate_custom_bonus_effect")
		and self.effect
	then
...
function modifier_enigma_innate_custom_bonus_effect:DamageEvent_out(params)
	...
	local effect_cast =
		ParticleManager:CreateParticle("particles/enigma/innate_shard_damage.vpcf", PATTACH_POINT_FOLLOW, self.caster)
...
function modifier_enigma_innate_custom_scepter_stats:RemoveOnDeath()
	return false
end
