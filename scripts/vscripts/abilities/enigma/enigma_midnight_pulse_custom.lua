...
LinkLuaModifier(
	"modifier_enigma_midnight_pulse_custom_tracker",
	"abilities/enigma/enigma_midnight_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_enigma_midnight_pulse_custom_channel_pull",
	"abilities/enigma/enigma_midnight_pulse_custom",
...
function modifier_enigma_midnight_pulse_custom:OnCreated(table)
	...
	if self.parent == self.caster then
		self.particle = ParticleManager:CreateParticle(
			"particles/enigma/midnight_pulse.vpcf",
...
function modifier_enigma_midnight_pulse_custom:GetAuraEntityReject(hEntity)
	...
	if hEntity:GetTeamNumber() ~= self.parent:GetTeamNumber() then
		return false
	end
...
function modifier_enigma_midnight_pulse_custom_debuff:OnIntervalThink()
	...
	if self.ability.talents.has_e3 == 1 then
		...
		if RollPseudoRandomPercentage(chance, index, self.parent) then
			...
			self.damageTable.damage = self.ability.talents.e3_base
				+ self.ability.talents.e3_damage * self.caster:GetMaxHealth()
			...
			if result then
				self.caster:GenericHeal(
					self.ability.talents.e3_heal * result * real_damage,
					self.ability,
					true,
...
function modifier_enigma_midnight_pulse_custom_channel:OnCreated()
	...
	self.effect = ParticleManager:CreateParticle(
		"particles/enigma/malefice_pull_field.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent
	)
...
function modifier_enigma_midnight_pulse_custom_channel:OnIntervalThink()
	...
	for _, target in pairs(targets) do
		if
			not target:HasModifier("modifier_enigma_midnight_pulse_custom_channel_pull") and not target:IsDebuffImmune()
		then
...
function modifier_enigma_midnight_pulse_custom_channel_pull:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/enigma/midnight_pull_target.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.caster
	)
...
function modifier_enigma_midnight_pulse_custom_move:IsPurgable()
	return true
end
...
function modifier_enigma_midnight_pulse_custom_silence:IsPurgable()
	return true
end
