...
LinkLuaModifier(
	"modifier_primal_beast_onslaught_custom_knockback",
	"abilities/primal_beast/primal_beast_onslaught_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_primal_beast_onslaught_custom_stacks",
	"abilities/primal_beast/primal_beast_onslaught_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_primal_beast_onslaught_custom_stun_slow",
	"abilities/primal_beast/primal_beast_onslaught_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_primal_beast_onslaught_custom_shield",
	"abilities/primal_beast/primal_beast_onslaught_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function primal_beast_onslaught_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_primal_beast/primal_beast_attack_blur_right_to_left.vpcf",
		context
	)
	PrecacheResource(
		"particle",
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_primal_beast/primal_beast_attack_fist_blur_left_to_right.vpcf",
		context
	)
	PrecacheResource(
		"particle",
...
function primal_beast_onslaught_custom:GetDamage()
	...
	local damage = self:GetSpecialValueFor("knockback_damage")
		+ caster:GetTalentValue("modifier_primal_beast_onslaught_2", "damage")
...
function primal_beast_onslaught_custom:GetChargeTime()
	...
	local duration = self:GetSpecialValueFor("chargeup_time")
		+ self:GetCaster():GetTalentValue("modifier_primal_beast_onslaught_4", "cast")
	return duration
end
...
function primal_beast_onslaught_custom:GetChargeSpeed()
	...
	local speed = self:GetSpecialValueFor("charge_speed")
		* (1 + self:GetCaster():GetTalentValue("modifier_primal_beast_onslaught_6", "speed") / 100)
	return speed
end
...
function primal_beast_onslaught_custom:KnockTarget(enemy, knock)
	...
	local stun = (
		self:GetSpecialValueFor("stun_duration") + caster:GetTalentValue("modifier_primal_beast_onslaught_1", "stun")
		...
		stun:SetEndRule(function()
			if enemy and not enemy:IsNull() then
				enemy:AddNewModifier(
					...
					"modifier_primal_beast_onslaught_custom_stun_slow",
					{
						duration = caster:GetTalentValue("modifier_primal_beast_onslaught_6", "leash")
							* (1 - enemy:GetStatusResistance()),
					}
				)
			end
...
function primal_beast_onslaught_custom:OnSpellStart()
	...
	if caster:HasTalent("modifier_primal_beast_onslaught_5") and self:GetAutoCastState() then
		...
		local nFXIndex =
			ParticleManager:CreateParticle("particles/primal_knockback.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)
		...
		for _, enemy in pairs(caster:FindTargets(radius)) do
			...
			enemy:AddNewModifier(
				...
				self,
				"modifier_primal_beast_onslaught_custom_knockback",
				{ duration = knock_duration, x = caster:GetAbsOrigin().x, y = caster:GetAbsOrigin().y }
			)
		end
	end
end
...
function primal_beast_onslaught_custom:OnChargeFinish(interrupt, max)
	...
	if caster:HasTalent("modifier_primal_beast_onslaught_7") then
		if max == 1 or max == true then
			local particle =
				ParticleManager:CreateParticle("particles/pangolier/buckle_refresh.vpcf", PATTACH_CUSTOMORIGIN, caster)
		...
		caster:CdAbility(
			self,
			self:GetCooldownTimeRemaining() * caster:GetTalentValue("modifier_primal_beast_onslaught_7", "cd") * k / 100
		)
	end
	...
	if caster:HasTalent("modifier_primal_beast_onslaught_2") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_primal_beast_onslaught_custom_damage",
			{ duration = caster:GetTalentValue("modifier_primal_beast_onslaught_2", "duration"), damage = damage }
		)
	end
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_primal_beast_onslaught_custom",
		{ charge = charge, duration = duration, max = max }
	)
...
function modifier_primal_beast_onslaught_custom_cast:IsHidden()
	return false
end
...
function modifier_primal_beast_onslaught_custom_cast:OnCreated(kv)
	...
	if self.parent:HasTalent("modifier_primal_beast_onslaught_3") then
		...
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_primal_beast_onslaught_custom_shield",
			{ duration = self.parent:GetTalentValue("modifier_primal_beast_onslaught_3", "duration") }
		)
	end
	...
	if self.parent:HasTalent("modifier_primal_beast_onslaught_5") then
		...
		self.particle = ParticleManager:CreateParticle(
			"particles/pangolier/linken_active.vpcf",
...
function modifier_primal_beast_onslaught_custom_cast:SpellEvent(params)
	...
	if bit.band(params.ability:GetBehaviorInt(), DOTA_ABILITY_BEHAVIOR_IGNORE_CHANNEL) ~= 0 then
		return
	end
	if params.ability:GetName() == "primal_beast_rock_throw" then
		return
	end
...
function modifier_primal_beast_onslaught_custom_cast:GetAbsorbSpell(params)
	...
	local particle =
		ParticleManager:CreateParticle("particles/pangolier/linken_proc.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
	ParticleManager:SetParticleControlEnt(
		particle,
		0,
		self.parent,
...
function modifier_primal_beast_onslaught_custom_cast:OnIntervalThink()
	...
	if
		...
		or self.parent:IsLeashed() and not self.charge_finish
	then
...
function modifier_primal_beast_onslaught_custom:OnCreated(kv)
	...
	if self.parent:HasTalent("modifier_primal_beast_onslaught_5") then
		self.bkb = self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_generic_debuff_immune",
			{ effect = 1, duration = self:GetRemainingTime() + 0.1 }
		)
	end
	...
	self.damageTable =
		{ attacker = self.parent, damage = self.damage, damage_type = DAMAGE_TYPE_PHYSICAL, ability = self.ability }
end
...
function modifier_primal_beast_onslaught_custom_thinker:IsHidden()
	return true
end
function modifier_primal_beast_onslaught_custom_thinker:IsPurgable()
	return false
end
...
function modifier_primal_beast_onslaught_custom_knockback:IsHidden()
	return true
end
...
function modifier_primal_beast_onslaught_custom_knockback:OnDestroy()
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_primal_beast_onslaught_custom_slow",
		{ duration = (1 - self.parent:GetStatusResistance()) * self.slow_duration }
	)
end

modifier_primal_beast_onslaught_custom_slow = class({})
function modifier_primal_beast_onslaught_custom_slow:IsHidden()
	return true
end
...
function modifier_primal_beast_onslaught_custom_stun_slow:IsHidden()
	return true
end
function modifier_primal_beast_onslaught_custom_stun_slow:IsPurgable()
	return true
end
...
function modifier_primal_beast_onslaught_custom_stacks:IsHidden()
	return not self:GetCaster():HasTalent("modifier_primal_beast_onslaught_4")
end
function modifier_primal_beast_onslaught_custom_stacks:IsPurgable()
	return false
end
function modifier_primal_beast_onslaught_custom_stacks:RemoveOnDeath()
	return false
end
...
function modifier_primal_beast_onslaught_custom_stacks:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_primal_beast_onslaught_4") then
		return
	end
	if self:GetStackCount() < self.max then
		return
...
function modifier_primal_beast_onslaught_custom_damage:IsHidden()
	return false
end
function modifier_primal_beast_onslaught_custom_damage:IsPurgable()
	return false
end
function modifier_primal_beast_onslaught_custom_damage:GetTexture()
	return "buffs/Bloodrite_attack"
end
...
function modifier_primal_beast_onslaught_custom_shield:IsHidden()
	return true
end
function modifier_primal_beast_onslaught_custom_shield:IsPurgable()
	return false
end
