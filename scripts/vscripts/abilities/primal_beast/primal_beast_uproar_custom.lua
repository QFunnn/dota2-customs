...
LinkLuaModifier(
	"modifier_primal_beast_uproar_custom_proc",
	"abilities/primal_beast/primal_beast_uproar_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_primal_beast_uproar_custom_shield",
	"abilities/primal_beast/primal_beast_uproar_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function primal_beast_uproar_custom:OnSpellStart()
	...
	local duration = self:GetSpecialValueFor("roar_duration")
		+ caster:GetTalentValue("modifier_primal_beast_uproar_2", "duration")
	local radius = self:GetSpecialValueFor("radius")
	...
	if caster:HasTalent("modifier_primal_beast_uproar_6") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_primal_beast_uproar_custom_shield",
			{ duration = caster:GetTalentValue("modifier_primal_beast_uproar_6", "duration") }
		)
	end
	...
	local damageTable =
		{ attacker = caster, ability = self, damage = damage * stack, damage_type = DAMAGE_TYPE_MAGICAL }
	...
	for _, enemy in pairs(caster:FindTargets(radius)) do
		...
		if caster:HasTalent("modifier_primal_beast_uproar_5") then
					...
					arc:SetEndCallback(function()
						if enemy and not enemy:IsNull() then
							...
							enemy:AddNewModifier(
								...
								self,
								"modifier_primal_beast_uproar_custom_root",
								{ duration = root_duration * (1 - enemy:GetStatusResistance()), stack = stack }
							)
						end
					end)
				end
			...
			enemy:AddNewModifier(
				...
				self,
				"modifier_primal_beast_uproar_custom_debuff",
				{ duration = slow * (1 - enemy:GetStatusResistance()), stack = stack }
			)
		end
	end
...
function modifier_primal_beast_uproar_custom:SpellEvent(params)
	...
	if not self.parent:HasTalent("modifier_primal_beast_uproar_4") then
		return
	end
...
function modifier_primal_beast_uproar_custom:DamageEvent_out(params)
	...
	if params.record and self.records[params.record] and self.parent:CheckLifesteal(params, 2) then
		...
		params.unit:AddNewModifier(
			...
			"modifier_bashed",
			{ duration = self.speed_stun * (1 - params.unit:GetStatusResistance()) }
		)
	end
end
...
function modifier_primal_beast_uproar_custom:DamageEvent_inc(params)
	...
	if final >= self.damage_limit then
		...
		for i = 1, delta do
			if
				not self.parent:HasModifier("modifier_primal_beast_uproar_custom_buff")
				or self.parent:HasModifier("modifier_primal_beast_uproar_custom_legendary")
			then
...
function modifier_primal_beast_uproar_custom_buff:IsHidden()
	return false
end
...
function modifier_primal_beast_uproar_custom_buff:OnCreated(kv)
	...
	self.damage = self.ability:GetSpecialValueFor("bonus_damage_per_stack")
		+ self.parent:GetTalentValue("modifier_primal_beast_uproar_3", "damage")
...
function modifier_primal_beast_uproar_custom_buff:GetModifierMagicalResistanceBonus()
	if not self.caster:HasTalent("modifier_primal_beast_uproar_1") then
		return
	end
...
function modifier_primal_beast_uproar_custom_debuff:IsHidden()
	return false
end
...
function primal_beast_blood_frenzy_custom:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_primal_beast_uproar_custom_legendary",
		{ duration = caster:GetTalentValue("modifier_primal_beast_uproar_7", "duration") }
	)
end

modifier_primal_beast_uproar_custom_legendary = class({})
function modifier_primal_beast_uproar_custom_legendary:IsHidden()
	return false
end
function modifier_primal_beast_uproar_custom_legendary:IsPurgable()
	return false
end
...
function modifier_primal_beast_uproar_custom_legendary:OnCreated(table)
	...
	local effect_cast =
		ParticleManager:CreateParticle("particles/beast_hands.vpcf", PATTACH_OVERHEAD_FOLLOW, self.parent)
	ParticleManager:SetParticleControlEnt(
		effect_cast,
		2,
		self.parent,
			...
			if self.parent:IsAlive() then
				self.parent:GenericHeal(
					self.parent:GetMaxHealth()
						* self.mod:GetStackCount()
						* self.parent:GetTalentValue("modifier_primal_beast_uproar_7", "heal")
						/ 100,
					self.ability,
					nil,
					nil,
...
function modifier_primal_beast_uproar_custom_root:IsHidden()
	return true
end
...
function modifier_primal_beast_uproar_custom_root:OnDestroy()
	...
	local duration = (
		self.ability:GetSpecialValueFor("slow_duration")
		+ self.caster:GetTalentValue("modifier_primal_beast_uproar_2", "duration")
	) * (1 - self.parent:GetStatusResistance())
	self.parent:AddNewModifier(
		self.caster,
		self.ability,
		"modifier_primal_beast_uproar_custom_debuff",
		{ duration = duration, stack = self.stack }
	)
end
...
function modifier_primal_beast_uproar_custom_proc:IsHidden()
	return false
end
function modifier_primal_beast_uproar_custom_proc:IsPurgable()
	return false
end
function modifier_primal_beast_uproar_custom_proc:GetTexture()
	return "buffs/spray_slow"
end
...
function modifier_primal_beast_uproar_custom_shield:IsHidden()
	return true
end
function modifier_primal_beast_uproar_custom_shield:IsPurgable()
	return false
end
function modifier_primal_beast_uproar_custom_shield:OnCreated(table)
	self.parent = self:GetParent()
	self.shield_talent = "modifier_primal_beast_uproar_6"
	self.max_shield = self.parent:GetTalentValue("modifier_primal_beast_uproar_6", "shield")
