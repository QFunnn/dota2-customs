...
local a = "abilities/card_effect/card_effect_7"
...
f.card_effect_7 = c()
local i = f.card_effect_7
i.name = "card_effect_7"
d(i, h)
function i.prototype.spawn(self)
	local j = self:getPlayerID()
	local k = self:getSpecialValueFor("chance")
	local l = self:getSpecialValueFor("gold")
	if self:PRD(k) then
		local m = l * CardEffect:getPlayerCardCount(j, "card_effect_7")
