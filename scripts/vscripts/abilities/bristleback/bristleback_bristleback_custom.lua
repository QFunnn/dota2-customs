...
LinkLuaModifier(
	"modifier_bristleback_bristleback_custom_tracker",
...
function bristleback_bristleback_custom:OnSpellStart()
	...
	self.caster:AddNewModifier(
		...
		self,
		"modifier_bristleback_bristleback_custom_active",
		{ duration = self.talents.h5_duration + 0.1 }
	)
end
...
function bristleback_bristleback_custom:ProcSlow(target)
	...
	if not self.caster.spray_ability then
		return
	end
	if not RollPseudoRandomPercentage(self.talents.e1_chance, 7954, self.caster) then
		return
	end

	local damage_table = { victim = target, attacker = self.caster, ability = self.caster.spray_ability }
	local damage = self.talents.e1_base
		+ self.talents.e1_damage * (self.caster:GetMaxHealth() - self.caster:GetHealth())
...
function modifier_bristleback_bristleback_custom_tracker:GetModifierSpellAmplify_Percentage()
	return self.ability.talents.e3_spell
		* (
			self.parent:HasModifier("modifier_bristleback_bristleback_custom_buff_active")
				and self.ability.talents.e3_bonus
			or 1
		)
end

function modifier_bristleback_bristleback_custom_tracker:GetModifierExtraHealthPercentage()
	return self.ability.talents.e3_health
		* (
			self.parent:HasModifier("modifier_bristleback_bristleback_custom_buff_active")
				and self.ability.talents.e3_bonus
			or 1
...
function modifier_bristleback_bristleback_custom_tracker:GetModifierIncomingDamage_Percentage(params)
	...
	if
		self.parent:PassivesDisabled() and not self.parent:HasModifier("modifier_bristleback_bristleback_custom_active")
	then
		return
	end
	...
	if facing == 1 then
		local particle = ParticleManager:CreateParticle(
			wearables_system:GetParticleReplacementAbility(self.parent, pfx_names[1], self),
		...
		local particle2 = ParticleManager:CreateParticle(
			wearables_system:GetParticleReplacementAbility(self.parent, pfx_names[2], self),
			PATTACH_ABSORIGIN_FOLLOW,
			self.parent
		)
		ParticleManager:SetParticleControlEnt(
	...
	if self.parent:HasModifier("modifier_bristleback_innate_custom_shield") then
		return
	end
...
function modifier_bristleback_bristleback_custom_tracker:GetReduction(facing)
	...
	local side_damage_reduction = self.ability.side_damage_reduction
		+ (self.ability.talents.has_h5 == 1 and self.ability.talents.h5_damage_reduce or 0)
	local back_damage_reduction = self.ability.back_damage_reduction
		+ (self.ability.talents.has_h5 == 1 and self.ability.talents.h5_damage_reduce or 0)
...
function modifier_bristleback_bristleback_custom_tracker:DamageEvent_inc(params)
	...
	if self.ability:GetFacing(attacker) ~= 1 then
		return
	end
...
function modifier_bristleback_bristleback_custom_tracker:IncStacks(stack, attacker)
	...
	local quill_release_threshold = self.ability.quill_release_threshold
		+ (self.ability.talents.has_e4 == 1 and self.ability.talents.e4_damage or 0)
	...
	if final >= quill_release_threshold then
		...
		if
			(self.ability.talents.has_e3 == 1 or self.ability.talents.has_e4 == 1)
			and (
				not self.parent:HasModifier("modifier_bristleback_bristleback_custom_buff_active")
				and not self.parent:HasModifier("modifier_bristleback_bristleback_custom_taunt_cd")
			)
		then
	...
	if self.stack >= self.max then
		...
		if self.ability.talents.has_e4 == 1 then
			...
			local effect_cast = ParticleManager:CreateParticle(
				"particles/bristleback/back_taunt.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				self.parent
			)
			ParticleManager:SetParticleControlEnt(
			...
			for _, target in pairs(self.parent:FindTargets(radius)) do
				if target:IsHero() then
					target:AddNewModifier(
						...
						{ duration = (1 - target:GetStatusResistance()) * self.ability.talents.e4_taunt }
					)
				end
			end
...
function modifier_bristleback_bristleback_custom_buff_active:GetStatusEffectName()
	return "particles/status_fx/status_effect_legion_commander_duel.vpcf"
end
function modifier_bristleback_bristleback_custom_buff_active:StatusEffectPriority()
...
function modifier_bristleback_bristleback_custom_active:OnCreated(table)
	...
	local particle_1 = ParticleManager:CreateParticle(
	...
	local particle_3 = ParticleManager:CreateParticle(
...
function modifier_bristleback_bristleback_custom_taunt_cd:GetTexture()
	return "buffs/bristleback/back_4"
end
