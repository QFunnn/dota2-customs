...
function modifier_antimage_attack:IsHidden()
	return true
end
function modifier_antimage_attack:IsPurgable()
	return false
end
...
function modifier_antimage_attack:GetModifierProcAttack_Feedback(param)
	...
	if param.target:IsMagicImmune() then
		return
	end
	if param.target:GetMaxMana() == 0 then
		return
	end
