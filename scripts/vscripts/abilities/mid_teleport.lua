...
function mid_teleport:GetChannelTime()
	return 3
end
...
function mid_teleport:OnSpellStart()
	...
	self.teleportToEffect = ParticleManager:CreateParticle(
	...
	ParticleManager:SetParticleControlEnt(
		...
		PATTACH_ABSORIGIN_FOLLOW,
		"attach_hitloc",
		self.teleport_center:GetAbsOrigin(),
		true
	)
	ParticleManager:SetParticleControlEnt(
		self.teleportToEffect,
...
function modifier_mid_teleport_cd:IsDebuff()
	return true
end
function modifier_mid_teleport_cd:IsPurgable()
	return false
end
function modifier_mid_teleport_cd:RemoveOnDeath()
	return false
end
function modifier_mid_teleport_cd:GetTexture()
...
function modifier_mid_teleport_cast:IsHidden()
	return true
end
function modifier_mid_teleport_cast:IsPurgable()
	return false
end
...
function modifier_mid_teleport_cast:OrderEvent(params)
	...
	if
		...
		or params.order_type == DOTA_UNIT_ORDER_MOVE_TO_POSITION
		or params.order_type == DOTA_UNIT_ORDER_MOVE_TO_DIRECTION
		or (params.order_type == DOTA_UNIT_ORDER_ATTACK_TARGET and target and target:GetUnitName() == "npc_teleport")
	then
		return 0
	end
	...
	if
		...
		or params.order_type == DOTA_UNIT_ORDER_STOP
...
function modifier_mid_teleport_cast:DamageEvent_inc(params)
	...
	if (params.attacker:GetAbsOrigin() - self:GetParent():GetAbsOrigin()):Length2D() >= 1000 then
		return
	end
	if not params.attacker:IsAlive() then
		return
	end
	if self.parent:IsFieldInvun(params.attacker) then
		return
	end
...
function modifier_mid_teleport_speed:IsHidden()
	return false
end
function modifier_mid_teleport_speed:IsPurgable()
	return false
end
function modifier_mid_teleport_speed:GetTexture()
	return "rune_haste"
end
...
function modifier_mid_teleport_speed:DamageEvent_inc(params)
	...
	if self:GetParent() == params.atttacker then
		return
	end
...
function modifier_mid_teleport_cast_anim:OnCreated()
	...
	self.blight_spot = ParticleManager:CreateParticle(
