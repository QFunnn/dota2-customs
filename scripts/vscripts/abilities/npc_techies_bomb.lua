...
function modifier_techies_bomb:IsHidden()
	return true
end
function modifier_techies_bomb:IsPurgable()
	return false
end
function modifier_techies_bomb:IsDebuff()
	return true
end
...
function modifier_techies_bomb:OnIntervalThink()
	...
	if number % 1 ~= 0 then
		int = number - 0.5
		caster:EmitSound("Hero_Techies.RemoteMine.Priming")
	end
			...
			for _, i in pairs(enemy_for_ability) do
				if i:GetTeam() ~= DOTA_TEAM_NEUTRALS and not i:IsBuilding() then
					...
					i:AddNewModifier(
						...
						{ duration = self.duration * (1 - i:GetStatusResistance()) }
					)
					DoDamage({
						victim = i,
...
function modifier_techies_bomb:AttackEvent_inc(param)
	if not IsServer() then
		return
	end
...
function modifier_techies_bomb_silence:IsHidden()
	return false
end
...
function modifier_techies_bomb_cd:IsHidden()
	return false
end
function modifier_techies_bomb_cd:IsPurgable()
	return false
end