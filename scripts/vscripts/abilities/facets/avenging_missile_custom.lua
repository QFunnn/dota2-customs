...
LinkLuaModifier(
	"modifier_vengefulspirit_avenging_missile_custom",
	"abilities/facets/avenging_missile_custom",
...
function modifier_vengefulspirit_avenging_missile_custom:IsHidden()
	return true
end
function modifier_vengefulspirit_avenging_missile_custom:IsPurgable()
	return false
end
function modifier_vengefulspirit_avenging_missile_custom:IsDebuff()
	return false
end
function modifier_vengefulspirit_avenging_missile_custom:RemoveOnDeath()
	return false
end
function modifier_vengefulspirit_avenging_missile_custom:GetAttributes()
...
function modifier_vengefulspirit_avenging_missile_custom:GetModifierSpellAmplify_Percentage(event)
	...
	if not IsValidEntity(event.target) then
		return 0
	end
	if event.attacker:GetTeamNumber() == event.target:GetTeamNumber() then
		return 0
	end
	if not event.target:IsRealHero() then
		return 0
	end
	if not IsValidEntity(self.caster) then
		return 0
	...
	if not IsValidEntity(event.inflictor) or event.inflictor:GetAbilityName() ~= "vengefulspirit_magic_missile" then
		return 0
	end
