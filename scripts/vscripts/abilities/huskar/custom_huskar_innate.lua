...
function modifier_custom_huskar_innate:DamageEvent_inc(params)
	...
	if
		self.parent:HasModifier("modifier_custom_huskar_innate_cd")
		or self.parent:HasModifier("modifier_custom_huskar_innate_scepter")
	then
		return
	end
	if self.parent:PassivesDisabled() and self.ability.talents.has_h5 == 0 then
		return
	end
	...
	if self.parent:HasScepter() then
		self.parent:AddNewModifier(
			...
			{ duration = self.ability.scepter_duration + 0.1 }
		)
		self.parent:AddNewModifier(
			self.parent,
			self.ability,
...
function modifier_custom_huskar_innate:ProcHeal()
	...
	self.parent:AddNewModifier(
		...
		{ duration = self.ability.cooldown }
	)
end
...
function modifier_custom_huskar_innate:GetMinHealth()
	...
	if self.parent:HasModifier("modifier_custom_huskar_innate_cd") then
		return
	end
	if self.parent:PassivesDisabled() and self.ability.talents.has_h5 == 0 then
		return
