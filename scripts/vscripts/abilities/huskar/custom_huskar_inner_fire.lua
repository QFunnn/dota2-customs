...
function custom_huskar_inner_fire:OnSpellStart(new_ability, override_point)
	...
	for _, enemy in pairs(caster:FindTargets(radius, point)) do
		...
		if IsValid(self.caster.break_ability) and self.talents.has_q7 == 1 and not new_ability then
			self.caster.break_ability:ApplyDelay(enemy)
		end
		...
		if new_ability == "modifier_huskar_disarm_7" then
			...
			knockback_duration = self.knockback_min
...
function modifier_custom_huskar_inner_fire_root:OnIntervalThink()
	...
	if (self.parent:GetAbsOrigin() - self.center):Length2D() <= self.radius then
		return
	end
...
function modifier_custom_huskar_inner_fire_legendary_magic:GetTexture()
