...
function modifier_feral_passive:IsPurgable()
	return false
end
...
function modifier_feral_passive:DeathEvent(param)
	if not IsServer() then
	end
	if
		param.unit:GetUnitName() == "npc_psi_blades_crystal"
		or param.unit:GetUnitName() == "npc_psi_blades_crystal_mini"
	then
		return
	end
	if
		param.unit:GetTeamNumber() == self:GetParent():GetTeamNumber()
