...
LinkLuaModifier(
	"modifier_warlock_boss_shadow_word_cd",
	"abilities/warlock_boss/warlock_boss_shadow_word.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_boss_shadow_word_heal",
	"abilities/warlock_boss/warlock_boss_shadow_word.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function warlock_boss_shadow_word:Precache(context)
...
function modifier_warlock_boss_shadow_word_cd:IsHidden()
	return false
end
function modifier_warlock_boss_shadow_word_cd:IsPurgable()
	return false
end

modifier_warlock_boss_shadow_word_heal = class({})

function modifier_warlock_boss_shadow_word_heal:IsHidden()
	return self:GetParent():GetTeamNumber() == self:GetCaster():GetTeamNumber()
end

function modifier_warlock_boss_shadow_word_heal:IsPurgable()
	return false
end
...
function modifier_warlock_boss_shadow_word_heal:OnIntervalThink()
	...
	if self:GetParent():GetTeamNumber() == self:GetCaster():GetTeamNumber() then
		dota1x6:GenericHeal(self:GetParent(), self.heal * self:GetParent():GetMaxHealth(), self:GetAbility())
	else
		SendOverheadEventMessage(
			self:GetParent(),
			4,
			self:GetParent(),
			self:GetParent():GetMaxHealth() * self.damage,
			nil
		)
		DoDamage({
			victim = self:GetParent(),
			damage = self.damage * self:GetParent():GetMaxHealth(),
...
function modifier_warlock_boss_shadow_word_heal:GetModifierLifestealRegenAmplify_Percentage()
	return self.healing
end
function modifier_warlock_boss_shadow_word_heal:GetModifierHealChange()
	return self.healing
end
function modifier_warlock_boss_shadow_word_heal:GetModifierHPRegenAmplify_Percentage()
	return self.healing
end