...
LinkLuaModifier(
	"modifier_warlock_boss_rain_of_chaos_cd",
	"abilities/warlock_boss/warlock_boss_rain_of_chaos.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_boss_rain_of_chaos_passive",
	"abilities/warlock_boss/warlock_boss_rain_of_chaos.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_boss_rain_of_chaos_thinker",
	"abilities/warlock_boss/warlock_boss_rain_of_chaos.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_warlock_boss_rain_of_chaos_cd:IsHidden()
	return false
end
function modifier_warlock_boss_rain_of_chaos_cd:IsPurgable()
	return false
end

modifier_warlock_boss_rain_of_chaos_passive = class({})
function modifier_warlock_boss_rain_of_chaos_passive:IsHidden()
	return true
end
function modifier_warlock_boss_rain_of_chaos_passive:IsPurgable()
	return false
end
...
function modifier_warlock_boss_rain_of_chaos_thinker:IsHidden()
	return false
end
function modifier_warlock_boss_rain_of_chaos_thinker:IsPurgable()
	return false
end
...
function modifier_warlock_boss_rain_of_chaos_thinker:OnDestroy(table)
	...
	for i = 1, stack do
		local golem =
			CreateUnitByName("npc_warlock_golem_custom", self.parent:GetAbsOrigin(), true, nil, nil, DOTA_TEAM_CUSTOM_5)
	...
	for _, i in pairs(enemy_for_ability) do
		local damageTable = {
			victim = i,
			damage = self.damage * i:GetMaxHealth(),
		...
		i:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = (1 - i:GetStatusResistance()) * self.stun }
		)
	end
end