...
LinkLuaModifier(
	"modifier_warlock_boss_golem_fists_tracker",
	"abilities/warlock_boss/warlock_boss_golem_fists_passive.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_boss_golem_fists_death",
	"abilities/warlock_boss/warlock_boss_golem_fists_passive.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_warlock_boss_golem_fists_debuff",
	"abilities/warlock_boss/warlock_boss_golem_fists_passive.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_warlock_boss_golem_fists_tracker:IsHidden()
	return true
end
function modifier_warlock_boss_golem_fists_tracker:IsPurgable()
	return false
end
...
function modifier_warlock_boss_golem_fists_tracker:OnCreated()
	self.parent = self:GetParent()
	self.parent:AddDeathEvent(self)
end

function modifier_warlock_boss_golem_fists_tracker:DeathEvent(params)
	if not IsServer() then
		return
	end
	if not self:GetParent().summoner then
		return
	end
	if self:GetParent().summoner:IsNull() then
		return
	end
	if self:GetParent().summoner ~= params.unit then
		return
	end
...
function modifier_warlock_boss_golem_fists_tracker:GetModifierProcAttack_Feedback(params)
	if not IsServer() then
		return
	end
...
function modifier_warlock_boss_golem_fists_death:IsHidden()
	return true
end
function modifier_warlock_boss_golem_fists_death:IsPurgable()
	return false
end
...
function modifier_warlock_boss_golem_fists_debuff:IsHidden()
	return false
end
function modifier_warlock_boss_golem_fists_debuff:IsPurgable()
	return false
end
...
function modifier_warlock_boss_golem_fists_debuff:OnCreated(table)
	...
	if
		self:GetCaster().summoner
		and not self:GetCaster().summoner:IsNull()
		and self:GetCaster().summoner:GetUpgradeStack("modifier_waveupgrade_boss") == 2
	then
