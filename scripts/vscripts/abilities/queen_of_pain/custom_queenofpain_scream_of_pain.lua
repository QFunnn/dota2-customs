...
LinkLuaModifier(
	"modifier_custom_scream_slow",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_scream_tracker",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_scream_armor",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_scream_lowhp",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_scream_auto_cd",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_scream_speed",
	"abilities/queen_of_pain/custom_queenofpain_scream_of_pain",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_queenofpain_scream_of_pain:GetCooldown(iLevel)
	...
	if
		caster:HasTalent("modifier_queen_scream_6")
		and caster:GetHealthPercent() <= caster:GetTalentValue("modifier_queen_scream_6", "health")
	then
...
function custom_queenofpain_scream_of_pain:ProcHeal()
	...
	for _, unit in pairs(caster:FindTargets(radius)) do
		DoDamage(
			{ victim = unit, attacker = caster, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self },
...
function custom_queenofpain_scream_of_pain:OnSpellStart(damage_ability)
	...
	if caster:HasTalent("modifier_queen_scream_5") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_scream_speed",
			{ duration = caster:GetTalentValue("modifier_queen_scream_5", "duration"), stack = stack }
		)
	end
	...
	local effect_cast = wearables_system:GetParticleReplacementAbility(
		caster,
...
function custom_queenofpain_scream_of_pain:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	local damage = self:GetSpecialValueFor("damage")
		+ caster:GetTalentValue("modifier_queen_scream_1", "damage") * caster:GetMaxHealth() / 100
		...
		target:AddNewModifier(
			...
			"modifier_custom_scream_slow",
			{
				duration = (1 - target:GetStatusResistance())
					* caster:GetTalentValue("modifier_queen_scream_2", "duration"),
			}
		)
	end

	DoDamage(
		{ victim = target, attacker = caster, ability = self, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL },
		ExtraData.ability
	)
end

modifier_custom_scream_tracker = class({})
function modifier_custom_scream_tracker:IsHidden()
	return true
end
function modifier_custom_scream_tracker:IsPurgable()
	return false
end
...
function modifier_custom_scream_tracker:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_queen_scream_6") then
		return
	end
...
function modifier_custom_scream_tracker:AttackEvent_inc(params)
	...
	if not self.parent:HasTalent("modifier_queen_scream_1") then
		return
	end
	if self.parent:HasModifier("modifier_custom_scream_auto_cd") then
		return
	end
	...
	if (params.attacker:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() > self.radius then
		return
	end
...
function modifier_custom_scream_armor:IsHidden()
	return false
end
function modifier_custom_scream_armor:IsPurgable()
	return false
end
function modifier_custom_scream_armor:GetTexture()
	return "buffs/blood_defence"
end
...
function modifier_custom_scream_lowhp:IsHidden()
	return false
end
function modifier_custom_scream_lowhp:IsPurgable()
	return false
end
function modifier_custom_scream_lowhp:GetTexture()
	return "buffs/scream_fear"
end
...
function modifier_custom_scream_auto_cd:IsHidden()
	return false
end
function modifier_custom_scream_auto_cd:IsPurgable()
	return false
end
function modifier_custom_scream_auto_cd:RemoveOnDeath()
	return false
end
function modifier_custom_scream_auto_cd:GetTexture()
	return "buffs/scream_slow"
end
function modifier_custom_scream_auto_cd:IsDebuff()
	return true
end
...
function modifier_custom_scream_slow:IsHidden()
	return true
end
function modifier_custom_scream_slow:IsPurgable()
	return true
end
...
function modifier_custom_scream_speed:IsHidden()
	return false
end
function modifier_custom_scream_speed:IsPurgable()
	return true
end
