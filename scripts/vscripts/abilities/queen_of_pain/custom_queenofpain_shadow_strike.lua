...
LinkLuaModifier(
	"modifier_custom_shadowstrike_poison",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_slow",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_legendary_count",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_legendary_damage",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_auto_cd",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_auto_tracker",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_magic_resist",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_heal",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_fear_cd",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowstrike_vision",
	"abilities/queen_of_pain/custom_queenofpain_shadow_strike",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_queenofpain_shadow_strike:ThrowDagger(target)
	...
	local projectile_speed = self:GetSpecialValueFor("projectile_speed")
		* (1 + caster:GetTalentValue("modifier_queen_dagger_5", "speed") / 100)

	local proj_name = wearables_system:GetParticleReplacementAbility(
		caster,
...
function custom_queenofpain_shadow_strike:OnSpellStart(new_target)
	...
	if caster:HasTalent("modifier_queen_dagger_2") then
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_shadowstrike_heal",
			{ duration = caster:GetTalentValue("modifier_queen_dagger_2", "duration") }
		)
	end
...
function custom_queenofpain_shadow_strike:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_custom_shadowstrike_fear_cd",
			{ duration = caster:GetTalentValue("modifier_queen_dagger_5", "cd_duration") }
		)
		target:AddNewModifier(
			caster,
			self,
			"modifier_nevermore_requiem_fear",
			{ duration = caster:GetTalentValue("modifier_queen_dagger_5", "fear") * (1 - target:GetStatusResistance()) }
		)
	end
	...
	if caster:HasTalent("modifier_queen_dagger_6") and target:IsRealHero() then
		target:AddNewModifier(
			...
			self,
			"modifier_custom_shadowstrike_vision",
			{ duration = caster:GetTalentValue("modifier_queen_dagger_6", "vision") }
		)
	end
...
function modifier_custom_shadowstrike_poison:IsHidden()
	return true
end
...
function modifier_custom_shadowstrike_poison:OnCreated(table)
	...
	self.sec_damage_total = self.ability:GetSpecialValueFor("duration_damage")
		+ self.caster:GetTalentValue("modifier_queen_dagger_1", "damage")
...
function modifier_custom_shadowstrike_slow:IsHidden()
	return self:GetCaster():HasTalent("modifier_queen_dagger_7")
end
function modifier_custom_shadowstrike_slow:IsPurgable()
	return true
end
function modifier_custom_shadowstrike_slow:GetTexture()
...
function modifier_custom_shadowstrike_slow:OnCreated(table)
	...
	self.max_slow = self.ability:GetSpecialValueFor("movement_slow")
		+ self.caster:GetTalentValue("modifier_queen_dagger_5", "slow")
...
function modifier_custom_shadowstrike_legendary_count:IsHidden()
	return false
end
...
function modifier_custom_shadowstrike_legendary_damage:IsHidden()
	return true
end
function modifier_custom_shadowstrike_legendary_damage:IsPurgable()
	return false
end
...
function modifier_custom_shadowstrike_auto_tracker:IsHidden()
	return true
end
function modifier_custom_shadowstrike_auto_tracker:IsPurgable()
	return false
end
...
function modifier_custom_shadowstrike_auto_tracker:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_queen_dagger_3") then
		return
	end
...
function modifier_custom_shadowstrike_auto_tracker:AttackStartEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_queen_dagger_7") then
		return
	end
	if self.parent:HasModifier("modifier_custom_shadowstrike_auto_cd") then
		return
	end
...
function modifier_custom_shadowstrike_auto_cd:IsHidden()
	return false
end
function modifier_custom_shadowstrike_auto_cd:IsPurgable()
	return true
end
function modifier_custom_shadowstrike_auto_cd:IsDebuff()
	return true
end
function modifier_custom_shadowstrike_auto_cd:RemoveOnDeath()
	return false
end
function modifier_custom_shadowstrike_auto_cd:GetTexture()
	return "buffs/qop_dagger_auto"
end

modifier_custom_shadowstrike_magic_resist = class({})
function modifier_custom_shadowstrike_magic_resist:IsHidden()
	return false
end
function modifier_custom_shadowstrike_magic_resist:IsPurgable()
	return false
end
function modifier_custom_shadowstrike_magic_resist:GetTexture()
...
function modifier_custom_shadowstrike_magic_resist:OnIntervalThink()
	...
	if not self.parent:HasModifier("modifier_custom_shadowstrike_slow") then
		return
	end
...
function modifier_custom_shadowstrike_heal:IsHidden()
	return false
end
function modifier_custom_shadowstrike_heal:IsPurgable()
	return false
end
function modifier_custom_shadowstrike_heal:GetTexture()
...
function modifier_custom_shadowstrike_fear_cd:IsHidden()
	return true
end
function modifier_custom_shadowstrike_fear_cd:IsPurgable()
	return false
end
function modifier_custom_shadowstrike_fear_cd:RemoveOnDeath()
	return false
end
...
function modifier_custom_shadowstrike_vision:IsHidden()
	return true
end
function modifier_custom_shadowstrike_vision:IsPurgable()
	return false
end
