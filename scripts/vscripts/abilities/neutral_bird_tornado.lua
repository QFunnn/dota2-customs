...
function modifier_bird_tornado:IsPurgable()
	return false
end

function modifier_bird_tornado:IsHidden()
	return true
end
...
function modifier_bird_tornado:StartCast(target)
	...
	self:GetParent():AddNewModifier(
		...
		{ target = array_target, duration = 0.4, anim = ACT_DOTA_CAST_ABILITY_1, parent_mod = self:GetName() }
	)
end
...
function modifier_bird_tornado:EndCast()
	...
	local tornado = CreateUnitByName(
		...
		self:GetParent():GetAbsOrigin() + 250 * self:GetParent():GetForwardVector(),
		true,
		nil,
		nil,
		DOTA_TEAM_NEUTRALS
...
function modifier_tornado_think:IsPurgable()
	return false
end
function modifier_tornado_think:IsHidden()
	return true
end
...
function modifier_tornado_think_slow:IsHidden()
	return false
end
