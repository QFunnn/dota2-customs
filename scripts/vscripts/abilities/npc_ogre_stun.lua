...
function npc_ogre_stun:OnSpellStart()
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_ogre_stun_cd", { duration = self:GetCooldownTimeRemaining() })
	local point = self:GetCaster():GetAbsOrigin()
		+ self:GetCaster():GetForwardVector() * self:GetSpecialValueFor("range")
...
function modifier_strike:IsHidden()
	return false
end

function modifier_strike:GetTexture()
	return "elder_titan_echo_stomp"
end

function modifier_strike:IsPurgable()
	return false
end
...
function modifier_strike:OnDestroy(table)
	...
	local nFXIndex =
		ParticleManager:CreateParticle("particles/act_2/ogre_seal_suprise.vpcf", PATTACH_WORLDORIGIN, self:GetCaster())
...
function modifier_strike_debuff:IsPurgable()
	return false
end

function modifier_strike_debuff:GetTexture()
	return "elder_titan_echo_stomp"
end

function modifier_strike_debuff:IsHidden()
	return true
end
...
function modifier_strike_debuff:OnCreated(table)
	...
	self:GetParent():AddNewModifier(
		...
		self:GetAbility(),
		"modifier_stunned",
		{ duration = duration * (1 - self:GetParent():GetStatusResistance()) }
	)
	DoDamage({
...
function modifier_ogre_stun_cd:IsHidden()
	return false
end
function modifier_ogre_stun_cd:IsPurgable()
	return false
end