...
function zuus_arc_lightning_custom:StartArc(target, bounces, not_cast, shard_cast)
	...
	if not not_cast then
		...
		local head_particle = ParticleManager:CreateParticle(
	...
	caster:AddNewModifier(
		...
		{ shard_cast = shard_cast, not_cast = not_cast, starting_unit_entindex = target:entindex(), bounces = bounces }
	)
end

modifier_zuus_arc_lightning_custom = class({})

function modifier_zuus_arc_lightning_custom:IsHidden()
	return true
end
function modifier_zuus_arc_lightning_custom:IsPurgable()
	return false
end
...
function modifier_zuus_arc_lightning_custom:OnCreated(keys)
	if not IsServer() or not self:GetAbility() then
		return
	end
...
function modifier_zuus_arc_lightning_custom:OnIntervalThink()
	if self.current_unit == nil or self.current_unit:IsNull() then
		self:Destroy()
		return
	end
	self.zapped = false
	...
	do
		if
			(not self.units_affected[enemy] or self.units_affected[enemy] < self.max_per_target)
...
LinkLuaModifier(
	...
	"abilities/zuus_arc_lightning_custom",
...
function modifier_zuus_lightning_hands_custom_tracker:IsHidden()
	return false
end
function modifier_zuus_lightning_hands_custom_tracker:IsPurgable()
	return false
end
