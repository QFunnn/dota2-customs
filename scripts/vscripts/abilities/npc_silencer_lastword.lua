...
function modifier_silencer_lastword_debuff:IsPurgable()
	return false
end
function modifier_silencer_lastword_debuff:IsHidden()
	return false
end
...
function modifier_silencer_lastword_debuff:OnCreated(table)
	...
	self:GetParent():AddSpellEvent(self)
end
...
function modifier_silencer_lastword_debuff:OnDestroy()
	...
	if self.cast == true then
		self:GetParent():AddNewModifier(
			...
			self:GetAbility(),
			"modifier_stunned",
			{ duration = self.stun * (1 - self:GetParent():GetStatusResistance()) }
		)
...
function modifier_silencer_lastword_silence:IsHidden()
	return false
end
function modifier_silencer_lastword_silence:IsPurgable()
	return true
end

function modifier_silencer_lastword_silence:CheckState()
	return { [MODIFIER_STATE_SILENCED] = true }
end
function modifier_silencer_lastword_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf"
end

function modifier_silencer_lastword_silence:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

modifier_silencer_lastword_cd = class({})
function modifier_silencer_lastword_cd:IsHidden()
	return false
end
function modifier_silencer_lastword_cd:IsPurgable()
	return false
end