...
LinkLuaModifier(
	"modifier_custom_puck_waning_rift_legendary_charge",
	"abilities/puck/custom_puck_waning_rift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_waning_rift_legendary_damage",
	"abilities/puck/custom_puck_waning_rift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_waning_rift_tracker",
	"abilities/puck/custom_puck_waning_rift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_waning_rift_break",
	"abilities/puck/custom_puck_waning_rift",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_puck_waning_rift:GetBehavior()
	if
		self:GetCaster():HasTalent("modifier_puck_rift_7")
		and not self:GetCaster():HasModifier("modifier_custom_puck_waning_rift_legendary_charge")
	then
...
function custom_puck_waning_rift:GetMaxRange(bonus)
	...
	if caster:HasModifier("modifier_custom_puck_waning_rift_legendary_charge") then
		range = range
			+ caster:GetTalentValue("modifier_puck_rift_7", "range")
				* caster:GetUpgradeStack("modifier_custom_puck_waning_rift_legendary_charge")
				/ 100
	end
	if bonus then
...
function custom_puck_waning_rift:OnSpellStart(self_cast)
	...
	if caster:HasTalent("modifier_puck_rift_7") and not mod and not self_cast then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_puck_waning_rift_legendary_charge",
			{ duration = caster:GetTalentValue("modifier_puck_rift_7", "duration_max") }
		)
		return
	end
	...
	local damage = self:GetSpecialValueFor("damage")
		+ caster:GetTalentValue("modifier_puck_rift_1", "damage") * caster:GetAverageTrueAttackDamage(nil) / 100
	local silence = self:GetSpecialValueFor("silence_duration")
		+ caster:GetTalentValue("modifier_puck_rift_3", "duration")
	...
	local rift_particle = ParticleManager:CreateParticle(
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			caster,
			self,
			"modifier_custom_puck_waning_rift",
			{ duration = silence * (1 - enemy:GetStatusResistance()) }
		)
		...
		if caster:HasTalent("modifier_puck_rift_5") then
			...
			if enemy:GetHealthPercent() <= fear_health then
				...
				enemy:AddNewModifier(
					...
					{ duration = fear * (1 - enemy:GetStatusResistance()) }
				)
			end
		end
	end
...
function modifier_custom_puck_waning_rift:IsHidden()
	return false
end
function modifier_custom_puck_waning_rift:IsPurgable()
	return true
end
...
function modifier_custom_puck_waning_rift:GetModifierLifestealRegenAmplify_Percentage()
	if not self.caster:HasTalent("modifier_puck_rift_4") then
		return
	end
...
function modifier_custom_puck_waning_rift:GetModifierMoveSpeedBonus_Percentage()
	if not self.caster:HasTalent("modifier_puck_rift_3") then
		return
	end
	return self.slow
end
...
function modifier_custom_puck_waning_rift_tracker:IsHidden()
	return true
end
function modifier_custom_puck_waning_rift_tracker:IsPurgable()
	return false
end
...
function modifier_custom_puck_waning_rift_tracker:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_puck_rift_2") then
		return
	end
...
function modifier_custom_puck_waning_rift_legendary_charge:IsHidden()
	return false
end
function modifier_custom_puck_waning_rift_legendary_charge:IsPurgable()
	return false
end
function modifier_custom_puck_waning_rift_legendary_charge:OnCreated()
	...
	self.max_radius = self.ability:GetSpecialValueFor("max_distance")
		+ self.parent:GetCastRangeBonus()
		+ self.parent:GetTalentValue("modifier_puck_rift_7", "range")
	...
	self.particle = ParticleManager:CreateParticleForPlayer(
		"particles/puck/rift_prepare_radius.vpcf",
...
function modifier_custom_puck_waning_rift_legendary_charge:OnIntervalThink()
	...
	self.parent:UpdateUIshort({
		...
		time = math.min(self.duration, self:GetElapsedTime()),
		active = self:GetElapsedTime() >= self.duration,
		stack = tostring(math.floor(self.stack)) .. "%",
		priority = 1,
		style = "PuckRift",
	})
...
function modifier_custom_puck_waning_rift_legendary_damage:IsHidden()
	return true
end
function modifier_custom_puck_waning_rift_legendary_damage:IsPurgable()
	return false
end
function modifier_custom_puck_waning_rift_legendary_damage:GetTexture()
...
function modifier_custom_puck_waning_rift_break:IsHidden()
	return true
end
function modifier_custom_puck_waning_rift_break:IsPurgable()
	return false
end
function modifier_custom_puck_waning_rift_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_custom_puck_waning_rift_break:GetEffectName()
