...
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_stun_cd",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_tracker",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_slow",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_legendary_damage",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_proc_attack",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_proc",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_shield",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_speed",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_puck_phase_shift_absorb",
	"abilities/puck/custom_puck_phase_shift",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_puck_phase_shift:GetDuration()
	return self:GetSpecialValueFor("duration")
		+ self:GetCaster():GetTalentValue("modifier_puck_shift_2", "duration_inc")
end
...
function custom_puck_phase_shift:OnSpellStart()
	...
	if caster:HasTalent("modifier_puck_shift_6") then
		if self:GetAutoCastState() and not caster:IsRooted() and not caster:IsLeashed() then
			caster:Teleport(
				caster:GetAbsOrigin()
					+ caster:GetForwardVector() * caster:GetTalentValue("modifier_puck_shift_6", "range"),
				false,
				"particles/puck/rift_blink_start.vpcf",
				"particles/puck/rift_blint_end.vpcf",
				"Puck.Shift_blink"
			)
		end
	end
...
function modifier_custom_puck_phase_shift:IsHidden()
	return false
end
function modifier_custom_puck_phase_shift:IsPurgable()
	return false
end
...
function modifier_custom_puck_phase_shift:OnIntervalThink()
	...
	self.parent:UpdateUIshort({
		...
		stack = tostring(stack * self.legendary_spell) .. "%",
		style = "PuckShift",
	})
end
...
function modifier_custom_puck_phase_shift:OnDestroy()
	...
	if self.parent:HasTalent("modifier_puck_shift_3") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_custom_puck_phase_shift_speed",
			{ duration = self.parent:GetTalentValue("modifier_puck_shift_3", "duration") }
		)
	end
	...
	if self.parent:HasTalent("modifier_puck_shift_6") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_custom_puck_phase_shift_absorb",
			{ duration = self.parent:GetTalentValue("modifier_puck_shift_6", "duration") }
		)
	end
end
...
function modifier_custom_puck_phase_shift_tracker:IsHidden()
	return true
end
function modifier_custom_puck_phase_shift_tracker:IsPurgable()
	return false
end
...
function modifier_custom_puck_phase_shift_tracker:GetModifierAttackSpeedBonus_Constant()
	if not self.parent:HasTalent("modifier_puck_shift_1") then
		return
	end
...
function modifier_custom_puck_phase_shift_tracker:GetModifierBonusStats_Intellect()
	...
	return (self.parent:GetStrength() + self.parent:GetAgility())
		* self.parent:GetTalentValue("modifier_puck_shift_1", "int")
		/ 100
end
...
function modifier_custom_puck_phase_shift_tracker:GetPriority()
	if not self.parent:HasModifier("modifier_custom_puck_phase_shift_proc") then
...
function modifier_custom_puck_phase_shift_tracker:AttackRecordEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_puck_shift_4") then
		return
	end
...
function modifier_custom_puck_phase_shift_tracker:PuckAttackProc()
	...
	if self.parent:HasTalent("modifier_puck_shift_7") and mod then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_custom_puck_phase_shift_legendary_damage",
			{ duration = mod:GetRemainingTime() + self.legendary_duration }
		)
	end
end
...
function modifier_custom_puck_phase_shift_tracker:AttackEvent_inc(params)
	...
	if
		self.parent:HasTalent("modifier_puck_shift_5")
		and self.parent ~= params.attacker
		and self.parent == params.target
...
function modifier_custom_puck_phase_shift_tracker:ProcDamage(target)
	...
	local damageTable =
		{ attacker = self.parent, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self.ability }
...
function modifier_custom_puck_phase_shift_tracker:SpellEvent(params)
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_custom_puck_phase_shift_proc_attack",
		{ duration = self.proc_spell }
	)
end

modifier_custom_puck_phase_shift_stun_cd = class({})
function modifier_custom_puck_phase_shift_stun_cd:IsHidden()
	return false
end
function modifier_custom_puck_phase_shift_stun_cd:IsPurgable()
	return false
end
function modifier_custom_puck_phase_shift_stun_cd:IsDebuff()
	return true
end
function modifier_custom_puck_phase_shift_stun_cd:RemoveOnDeath()
	return false
end
function modifier_custom_puck_phase_shift_stun_cd:GetTexture()
	return "buffs/multi_spell"
end
...
function modifier_custom_puck_phase_shift_slow:IsHidden()
	return true
end
function modifier_custom_puck_phase_shift_slow:IsPurgable()
	return true
end
function modifier_custom_puck_phase_shift_slow:GetTexture()
...
function modifier_custom_puck_phase_shift_proc_attack:IsHidden()
	return true
end
function modifier_custom_puck_phase_shift_proc_attack:IsPurgable()
	return false
end

modifier_custom_puck_phase_shift_proc = class({})
function modifier_custom_puck_phase_shift_proc:IsHidden()
	return true
end
function modifier_custom_puck_phase_shift_proc:IsPurgable()
	return false
end

modifier_custom_puck_phase_shift_legendary_damage = class({})
function modifier_custom_puck_phase_shift_legendary_damage:IsHidden()
	return false
end
function modifier_custom_puck_phase_shift_legendary_damage:IsPurgable()
	return false
end
function modifier_custom_puck_phase_shift_legendary_damage:GetTexture()
	return "buffs/eye_damage"
end
...
function modifier_custom_puck_phase_shift_legendary_damage:OnIntervalThink()
	...
	self.parent:UpdateUIshort({
		...
		stack = tostring(self:GetStackCount() * self.damage) .. "%",
		style = "PuckShift",
	})
	self:StartIntervalThink(FrameTime())
end
...
function modifier_custom_puck_phase_shift_shield:IsHidden()
	return false
end
function modifier_custom_puck_phase_shift_shield:IsPurgable()
	return false
end
function modifier_custom_puck_phase_shift_shield:GetTexture()
	return "buffs/shift_shield"
end
function modifier_custom_puck_phase_shift_shield:OnCreated(table)
	...
	self.max_shield = self.caster:GetTalentValue("modifier_puck_shift_2", "shield")
		* self.caster:GetIntellect(false)
		/ 100
	self.interval = 0.1
	self.shield = 0
...
function modifier_custom_puck_phase_shift_shield:GetModifierIncomingDamageConstant(params)
	...
	if
		self.caster:HasModifier("modifier_custom_puck_phase_shift") and self.caster:HasTalent("modifier_puck_shift_7")
	then
...
function modifier_custom_puck_phase_shift_speed:IsHidden()
	return false
end
function modifier_custom_puck_phase_shift_speed:IsPurgable()
	return false
end
function modifier_custom_puck_phase_shift_speed:GetTexture()
...
function modifier_custom_puck_phase_shift_absorb:IsHidden()
	return true
end
function modifier_custom_puck_phase_shift_absorb:IsPurgable()
	return false
end
...
function modifier_custom_puck_phase_shift_absorb:OnCreated()
	...
	self.sphere =
		ParticleManager:CreateParticle("particles/puck/shift_sphere.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
