...
LinkLuaModifier(
	"modifier_muerta_dead_shot_custom_thinker",
	"abilities/muerta/muerta_dead_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_muerta_dead_shot_custom_tracker",
	"abilities/muerta/muerta_dead_shot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function muerta_dead_shot_custom:CastFilterResultTarget(target)
	local flag = target:HasModifier("modifier_item_muerta_quest_item_illusion") and DOTA_UNIT_TARGET_FLAG_INVULNERABLE
		or 0
	local team = target:IsBuilding() and DOTA_UNIT_TARGET_TEAM_BOTH or DOTA_UNIT_TARGET_TEAM_ENEMY
	local nResult = UnitFilter(
		target,
		team,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_TREE + DOTA_UNIT_TARGET_BUILDING,
		flag,
...
function muerta_dead_shot_custom:OnVectorCastStart(vStartLocation, vDirection)
	...
	if not target:IsBaseNPC() then
		local dummy =
			CreateUnitByName("npc_dota_companion", target:GetAbsOrigin(), false, nil, nil, self.caster:GetTeamNumber())
...
function muerta_dead_shot_custom:DealDamage(target, damage_ability)
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_muerta/muerta_deadshot_creep_impact.vpcf",
	...
	local real_damage = DoDamage(
		{ victim = target, attacker = self.caster, ability = self, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL },
		damage_ability
	)
	if damage_ability == "modifier_muerta_dead_7" then
	...
	if damage_ability == "modifier_muerta_dead_7" then
		...
		if result then
			self.caster:GenericHeal(
				result * self.talents.q7_heal * real_damage,
				self.ability,
				false,
...
function muerta_dead_shot_custom:RicochetShot(x, y, target, damage_k)
	...
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_muerta/muerta_deadshot_linear.vpcf",
		PATTACH_WORLDORIGIN,
		nil
	)
...
function muerta_dead_shot_custom:ApplyFear(target, duration, x, y, no_sound)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_muerta_dead_shot_custom_fear",
		{ duration = duration * (1 - target:GetStatusResistance()), x = x, y = y }
	)
end
...
function modifier_muerta_dead_shot_custom_speed:GetTexture()
	return "buffs/muerta/dead_4"
end
...
function modifier_muerta_dead_shot_custom_tracker:SpellEvent(params)
	...
	if self.parent:HasModifier("modifier_muerta_dead_shot_custom_auto_cd") then
		return
	end
...
function modifier_muerta_dead_shot_custom_auto_cd:GetTexture()
	return "buffs/muerta/hero_4"
end

modifier_muerta_dead_shot_custom_burn = class(mod_visible)
function modifier_muerta_dead_shot_custom_burn:GetTexture()
	return "buffs/muerta/dead_3"
end
function modifier_muerta_dead_shot_custom_burn:OnCreated(table)
	...
	self.damageTable =
		{ victim = self.parent, attacker = self.caster, damage_type = DAMAGE_TYPE_MAGICAL, ability = self.ability }
	if not IsServer() then
		return
	end
