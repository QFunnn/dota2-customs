...
LinkLuaModifier(
	"modifier_npc_muerta_centaur_chrarge",
	"abilities/muerta/npc_muerta_centaur_abilities",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_npc_muerta_centaur_stuns_cast",
	"abilities/muerta/npc_muerta_centaur_abilities",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_npc_muerta_centaur_passive",
	"abilities/muerta/npc_muerta_centaur_abilities",
	LUA_MODIFIER_MOTION_NONE
)
...
function npc_muerta_centaur_charge:OnSpellStart()
	...
	self.caster:AddNewModifier(
		...
		self,
		"modifier_npc_muerta_centaur_chrarge",
		{ x = point.x, y = point.y, distance = distance, duration = duration }
	)
end

modifier_npc_muerta_centaur_chrarge = class(mod_hidden)
function modifier_npc_muerta_centaur_chrarge:GetStatusEffectName()
	return "particles/status_fx/status_effect_forcestaff.vpcf"
end
function modifier_npc_muerta_centaur_chrarge:StatusEffectPriority()
...
function modifier_npc_muerta_centaur_chrarge:GetModifierDisableTurning()
	return 1
end
	...
	for _, unit in pairs(self.parent:FindTargets(170)) do
		if not self.targets[unit] then
			...
			unit:AddNewModifier(
				...
				self.ability,
				"modifier_stunned",
				{ duration = self.stun * (1 - unit:GetStatusResistance()) }
			)
