...
local a = "abilities/privilege/privilege_weapon_048"
...
function l.prototype.EventListener(self)
	return {
		dungeon_room_start = function(m, n)
			...
			local q = self:GetSpecialValueFor("extra_gold")
			Player:ModifyGold(self:GetPlayerID(), q)
			self.triggeredCount = self.triggeredCount + 1
			Notification:CombatToPlayer(
				self:GetPlayerID(),
				{ message = "Notify_privilege_weapon_048", int_gold_count = q }
			)
		end,
