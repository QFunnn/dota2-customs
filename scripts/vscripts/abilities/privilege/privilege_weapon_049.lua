...
local a = "abilities/privilege/privilege_weapon_049"
...
function l.prototype.EventListener(self)
	return {
		dungeon_room_start = function(m, n)
			...
			local q = self:GetSpecialValueFor("hp_pct")
			p:AddProperty(PropertyFunction.HEALTH_AMPLIFY, q)
			self.triggeredCount = self.triggeredCount + 1
			Notification:CombatToPlayer(
				self:GetPlayerID(),
				{ message = "Notify_privilege_weapon_049", int_attr_value = q }
			)
		end,
