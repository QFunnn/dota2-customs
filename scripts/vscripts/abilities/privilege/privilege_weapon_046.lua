...
local a = "abilities/privilege/privilege_weapon_046"
...
function l.prototype.EventListener(self)
	return {
		dungeon_room_start = function(m, n)
			...
			local q = self:GetSpecialValueFor("ability_damage_pct")
			p:AddProperty(PropertyFunction.SPELL_DAMAGE_AMPLIFY, q)
			self.triggeredCount = self.triggeredCount + 1
			Notification:CombatToPlayer(
				self:GetPlayerID(),
				{ message = "Notify_privilege_weapon_046", int_attr_value = q }
			)
		end,
