...
local a = "abilities/privilege/privilege_043"
...
function m.prototype.EventListener(self)
	return {
		bless_room_reward_claimed = function(n, o)
			if o.playerID ~= self:GetPlayerID() then
				return
			end
			if o.isDouble or self.triggeredCount >= self.valid_count then
				return
			end
			self.triggeredCount = self.triggeredCount + 1
			Bless:DrawBlessSelection(o.playerID, 3)
		end,
	}
end
