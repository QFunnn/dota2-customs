...
local a = "abilities/privilege/privilege_gem_suit_011"
...
local n = c()
n.name = "privilege_gem_suit_011"
d(n, i)
function n.prototype.EventListener(self)
	return {
		ability_cast_complete = function(o, p)
			local q = self:GetCaster()
			if p.caster ~= q or p.abilityTag ~= AbilityTag.Ultimate or not IsValid(q) then
				return
			end
			q:AddNewModifier(
				q,
...
n = e({ j(nil) }, n)
g = c()
g.name = "modifier_privilege_gem_suit_011"
d(g, l)
...
function g.prototype.StaticProperty(self)
	return { [PropertyFunction.MAGICAL_DAMAGE_BOOST] = self.value, [PropertyFunction.PHYSICAL_DAMAGE_BOOST] = self.value }
end
g = e(
	{
		m(
