...
LinkLuaModifier(
	"modifier_antimage_counterspell_custom_shield",
	"abilities/antimage/antimage_counterspell_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_antimage_counterspell_custom_burn_damage:OnIntervalThink()
	...
	if self.ability.talents.has_e2 == 1 then
		local real_mana =
			self.parent:Script_ReduceMana(self.ability.talents.e2_mana * self.parent:GetMaxMana(), self.ability)
...
function modifier_antimage_counterspell_custom_legendary_damage:OnIntervalThink()
	...
	self.caster:UpdateUIshort({
		...
		stack = "+" .. math.abs(self.ability.talents.e7_magic) .. "%",
		style = "AntimageCounter",
	})
end
...
function modifier_antimage_counterspell_custom_shield:LegendaryHit(target, range_hit)
	...
	if self.ability.talents.has_e7 == 1 then
		...
		target:AddNewModifier(
			...
			self.parent:BkbAbility(self.ability, true),
			"modifier_stunned",
			{ duration = (1 - target:GetStatusResistance()) * self.ability.talents.e7_stun }
		)
	end
...
function modifier_antimage_counterspell_custom_shard_shield:GetModifierIncomingPhysicalDamageConstant(params)
	if self.parent:HasModifier("modifier_antimage_counterspell_custom_shield") then
		return
	end
