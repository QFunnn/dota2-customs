...
function npc_arc_field:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_arc_field_cd", { duration = self:GetCooldownTimeRemaining() })
...
function modifier_arc_field:IsHidden()
	return true
end

function modifier_arc_field:IsPurgable()
	return false
end
...
function modifier_arc_field:OnCreated(table)
	...
	ParticleManager:SetParticleControl(
		self.magnetic_particle,
		1,
		Vector(self:GetAbility():GetSpecialValueFor("radius"), 1, 1)
	)
...
function modifier_arc_field_buf:IsHidden()
	return false
end
function modifier_arc_field_buf:IsPurgable()
	return false
end
...
function modifier_arc_field_buf:GetModifierIncomingDamage_Percentage(keys)
	if
		...
		and self:GetAuraOwner():HasModifier("modifier_arc_field")
		...
		and (keys.attacker:GetAbsOrigin() - self:GetAuraOwner():GetAbsOrigin()):Length2D()
			> self:GetAuraOwner():FindModifierByName("modifier_arc_field").radius
	then
		return -self.evasion
	end
end
...
function npc_arc_field_passive:IsHidden()
	return true
end
...
function npc_arc_field_passive2:IsHidden()
	return true
end
...
function modifier_arc_field_cd:IsHidden()
	return false
end
function modifier_arc_field_cd:IsPurgable()
	return false
end