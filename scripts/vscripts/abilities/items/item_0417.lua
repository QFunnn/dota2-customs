...
____exports.item_0417 = __TS__Class()
local item_0417 = ____exports.item_0417
item_0417.name = "item_0417"
__TS__ClassExtends(item_0417, BaseItem_CS)
function item_0417.prototype.GetIntrinsicModifierName(self)
	return ____exports.modifier_item_0417_helheim_crown.name
end
item_0417 = __TS__DecorateLegacy({ registerAbility(nil) }, item_0417)
____exports.item_0417 = item_0417
____exports.modifier_item_0417_helheim_crown = __TS__Class()
local modifier_item_0417_helheim_crown = ____exports.modifier_item_0417_helheim_crown
modifier_item_0417_helheim_crown.name = "modifier_item_0417_helheim_crown"
__TS__ClassExtends(modifier_item_0417_helheim_crown, BaseModifier_CS)
function modifier_item_0417_helheim_crown.prototype.DeclareEvents(self)
	return { { event = BusinessEvents.ON_UNIT_DEATH, target = { scope = "global" } } }
end
function modifier_item_0417_helheim_crown.prototype.IsHidden(self)
	return true
end
function modifier_item_0417_helheim_crown.prototype.OnUnitDeath_CS(self, event)
	...
	local ability_summon_chance_pct = ability:GetSpecialValueFor("ability_summon_chance_pct")
	local ____opt_2 = victim.IsBoss
	if ____opt_2 and ____opt_2(victim) then
		ability_summon_chance_pct = ability_summon_chance_pct
			+ ability:GetSpecialValueFor("ability_boss_bonus_chance_pct")
	else
		local ____opt_4 = victim.IsElite
		local ____temp_8 = ____opt_4 and ____opt_4(victim)
		if not ____temp_8 then
			local ____opt_6 = victim.IsMiniboss
			____temp_8 = ____opt_6 and ____opt_6(victim)
		end
		if ____temp_8 then
			ability_summon_chance_pct = ability_summon_chance_pct
				+ ability:GetSpecialValueFor("ability_elite_bonus_chance_pct")
		end
	end
	if not RollPercentage(ability_summon_chance_pct) then
		return
	end
	self:SummonSpirit(parent, ability, victim)
end
function modifier_item_0417_helheim_crown.prototype.SummonSpirit(self, parent, ability, victim)
	...
	MyGameUnit:CreateSummonedUnitAsync({
		...
		onSpawn = function(____, unit)
			...
			unit:AddNewModifier(
				parent,
				ability,
				____exports.modifier_item_0417_royal_spirit.name,
...
modifier_item_0417_helheim_crown = __TS__DecorateLegacy({ registerModifier(nil) }, modifier_item_0417_helheim_crown)
____exports.modifier_item_0417_helheim_crown = modifier_item_0417_helheim_crown
____exports.modifier_item_0417_royal_spirit = __TS__Class()
local modifier_item_0417_royal_spirit = ____exports.modifier_item_0417_royal_spirit
modifier_item_0417_royal_spirit.name = "modifier_item_0417_royal_spirit"
__TS__ClassExtends(modifier_item_0417_royal_spirit, BaseModifier_CS)
function modifier_item_0417_royal_spirit.prototype.____constructor(self, ...)
	BaseModifier_CS.prototype.____constructor(self, ...)
	self.nextPatrolAt = 0
	self.inheritedHealth = 0
	self.inheritedAttack = 0
end
function modifier_item_0417_royal_spirit.prototype.OnCreated(self)
	if not IsServer() then
		return
	end
...
function modifier_item_0417_royal_spirit.prototype.GetAttributeBonus(self)
	return { bonus_health = self.inheritedHealth, bonus_attack_damage = self.inheritedAttack }
end
function modifier_item_0417_royal_spirit.prototype.OnIntervalThink(self)
	if not IsServer() then
		return
	end
	self:UpdateSummonCombat()
end
function modifier_item_0417_royal_spirit.prototype.CheckState(self)
	return { [MODIFIER_STATE_UNSELECTABLE] = true, [MODIFIER_STATE_NO_UNIT_COLLISION] = true }
end
function modifier_item_0417_royal_spirit.prototype.IsPurgable(self)
	return false
end
function modifier_item_0417_royal_spirit.prototype.UpdateSummonCombat(self)
...
modifier_item_0417_royal_spirit = __TS__DecorateLegacy({ registerModifier(nil) }, modifier_item_0417_royal_spirit)
____exports.modifier_item_0417_royal_spirit = modifier_item_0417_royal_spirit
return ____exports