...
LinkLuaModifier(
	"modifier_item_muerta_mercy_and_grace_custom_stats",
	"abilities/items/item_muerta_mercy_and_grace_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_muerta_mercy_and_grace_custom_active",
	"abilities/items/item_muerta_mercy_and_grace_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_item_muerta_mercy_and_grace_custom_stats:IsHidden()
	return true
end
function modifier_item_muerta_mercy_and_grace_custom_stats:IsPurgable()
	return false
end
...
function modifier_item_muerta_mercy_and_grace_custom_stats:GetModifierProcAttack_Feedback(params)
	if not IsServer() then
		return
	end
...
function modifier_item_muerta_mercy_and_grace_custom_active:IsHidden()
	return false
end
function modifier_item_muerta_mercy_and_grace_custom_active:IsPurgable()
	return false
end
function modifier_item_muerta_mercy_and_grace_custom_active:GetTexture()
	return "buffs/muerta_full"
end
...
function modifier_item_muerta_mercy_and_grace_custom_active:OnCreated()
	self.parent = self:GetParent()
	self.ability = self:GetAbility()

	self.active_bonus = self.ability:GetSpecialValueFor("active_bonus")
	self.active_max = self.ability:GetSpecialValueFor("active_max")

	self.parent:AddDeathEvent(self, true)
	self.parent:AddAttackStartEvent_out(self, true)
end

function modifier_item_muerta_mercy_and_grace_custom_active:DeathEvent(params)
	if not IsServer() then
		return
	end
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		"muerta_quest_panel",
		{ max = 5, stack = self.ability:GetCurrentCharges(), stage = 3 }
	)
	...
	if self.ability:GetCurrentCharges() >= self.ability:GetSpecialValueFor("active_max") then
		local particle_peffect = ParticleManager:CreateParticle(
			"particles/muerta/muerta_quest_item.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			self:GetCaster()
		)
		...
		local full = CreateItem("item_muerta_mercy_and_grace_full_custom", self.parent, self.parent)
		self.parent:AddItem(full)
	end
end

function modifier_item_muerta_mercy_and_grace_custom_active:AttackStartEvent_out(params)
	if not IsServer() then
		return
	end
	if self.parent ~= params.attacker then
...
function modifier_item_muerta_mercy_and_grace_custom_active:GetPriority()
