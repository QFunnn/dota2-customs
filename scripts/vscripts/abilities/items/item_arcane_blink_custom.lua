...
LinkLuaModifier(
	"modifier_item_arcane_blink_custom",
	"abilities/items/item_arcane_blink_custom",
...
function item_arcane_blink_custom:Spawn()
	...
	self.heal_int = self:GetSpecialValueFor("heal_int") / 100
	self.mana_pct = self:GetSpecialValueFor("mana_pct")
end
...
function item_arcane_blink_custom:OnSpellStart()
	...
	local heal = caster:GetIntellect(false) * self.heal_int + self.heal_base
	local mana = heal / self.mana_pct

	caster:GenericHeal(heal, self, false, false)
	caster:GiveMana(mana)
	caster:SendNumber(OVERHEAD_ALERT_MANA_ADD, mana)
