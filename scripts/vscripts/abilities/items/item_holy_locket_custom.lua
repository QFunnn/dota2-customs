...
function item_holy_locket_custom:OnSpellStart()
	...
	local heal = (self:GetSpecialValueFor("restore_health") / max)
	...
	for _, enemy in pairs(caster:FindTargets(radius, target:GetAbsOrigin())) do
		...
		local particle =
			ParticleManager:CreateParticle("particles/items/holy_locket_beam.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, target)
