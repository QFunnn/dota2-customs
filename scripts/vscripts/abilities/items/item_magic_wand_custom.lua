...
LinkLuaModifier("modifier_item_magic_wand_custom", "abilities/items/item_magic_wand_custom", LUA_MODIFIER_MOTION_NONE)

item_magic_wand_custom = class({})

function item_magic_wand_custom:GetIntrinsicModifierName()
	return "modifier_item_magic_wand_custom"
end

function item_magic_wand_custom:Precache(context)
...
function modifier_item_magic_wand_custom:OnDestroy()
	if not IsServer() then
		return
	end
	...
	Timers:CreateTimer(0.1, function()
		if not self.parent:IsNull() and self.ability:IsNull() then
			local item_wand = self.parent:FindItemInInventory("item_holy_locket_custom")
			if item_wand then
...
function modifier_item_magic_wand_custom:AddStack()
	if not IsServer() then
		return
	end
	if self.parent:HasModifier("modifier_item_holy_locket_custom") then
		return
	end
	if self.ability:GetCurrentCharges() >= self.max then
		return
	end
...
function modifier_item_magic_wand_custom:SpellEvent(params)
	if not IsServer() then
		return
	end
	...
	if (self.parent ~= params.unit) and params.unit:GetTeamNumber() == self.parent:GetTeamNumber() then
		return
	end
	if not self.parent:CanEntityBeSeenByMyTeam(params.unit) then
		return
	end
	...
	if (params.unit:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() > self.radius then
		return
	end
...
function modifier_item_magic_wand_custom:DeclareFunctions()
	return {
...
function modifier_item_magic_wand_custom:GetModifierBonusStats_Strength()
	return self.str
end

function modifier_item_magic_wand_custom:GetModifierBonusStats_Agility()
	return self.agi
end

function modifier_item_magic_wand_custom:GetModifierBonusStats_Intellect()
	return self.int
end