...
LinkLuaModifier(
	"modifier_item_lotus_orb_custom_reflect",
	"abilities/items/item_lotus_orb_custom",
...
function modifier_item_lotus_orb_custom_reflect:IsPurgable()
	return true
end
function modifier_item_lotus_orb_custom_reflect:OnCreated(table)
	...
	self.effect_cast =
		ParticleManager:CreateParticle("particles/items3_fx/lotus_orb_shield.vpcf", PATTACH_POINT_FOLLOW, self.parent)
...
function modifier_item_lotus_orb_custom_reflect:DealDamage(target, is_attack)
	...
	local particle2 = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_vengeful/vengeful_magic_missle_end.vpcf",
		PATTACH_CUSTOMORIGIN_FOLLOW,
		target
	)
	ParticleManager:SetParticleControlEnt(
	...
	target:AddNewModifier(
		...
		self.ability,
		"modifier_stunned",
		{ duration = (1 - target:GetStatusResistance()) * self.stun / k }
	)
...
function modifier_item_lotus_orb_custom_reflect:AttackEvent_inc(params)
	...
	if not attacker:CheckCd("lotus_attack", self.cd) then
		return
	end
