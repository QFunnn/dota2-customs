...
LinkLuaModifier(
	"modifier_item_crimson_guard_custom",
	"abilities/items/item_crimson_guard_custom",
...
function modifier_item_crimson_guard_custom:RemoveOnDeath()
	return false
end
function modifier_item_crimson_guard_custom:GetAttributes()
...
function modifier_item_crimson_guard_custom:GetModifierPhysical_ConstantBlock(params)
	...
	if not RollPseudoRandomPercentage(self.chance, 2622, self.parent) then
		return
	end
...
function modifier_item_crimson_guard_custom_active:GetModifierIncomingDamage_Percentage(params)
	if params.inflictor and not Not_spell_damage[params.inflictor:GetName()] then
		return
	end
	...
	if IsServer() then
		...
		local particle_2 =
			ParticleManager:CreateParticle("particles/items/crimson_guard_hit.vpcf", PATTACH_CUSTOMORIGIN, self.parent)
		ParticleManager:SetParticleControlEnt(
			particle_2,
			0,
			self.parent,
