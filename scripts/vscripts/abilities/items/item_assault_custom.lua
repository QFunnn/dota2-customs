...
LinkLuaModifier(
	"modifier_item_assault_custom_debuff_aura",
	"abilities/items/item_assault_custom",
	...
	for _, target in pairs(targets) do
		if target:IsRealHero() then
			self.parent:AddNewModifier(
				self.parent,
				self.ability,
				"modifier_item_assault_custom_buff",
				{ duration = self.timer }
			)
			break
		end
...
function modifier_item_assault_custom:GetAuraEntityReject(hEntity)
	if hEntity.ignore_assault then
		return true
	end
	return false
end
...
function modifier_item_assault_custom_debuff:GetAuraSearchFlags()
	return DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE
end
function modifier_item_assault_custom_debuff:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_item_assault_custom_debuff:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
...
function modifier_item_assault_custom_aura:GetModifierPhysicalArmorBonus()
	return self.aura_positive_armor
		+ self.caster:GetUpgradeStack("modifier_item_assault_custom_aura") * self.bonus_aura_armor
end

function modifier_item_assault_custom_aura:GetModifierAttackSpeedBonus_Constant()
	return self.aura_attack_speed
		+ self.caster:GetUpgradeStack("modifier_item_assault_custom_aura") * self.bonus_aura_speed
end
