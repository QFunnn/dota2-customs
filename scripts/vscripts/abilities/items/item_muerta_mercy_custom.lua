...
LinkLuaModifier(
	...
	"abilities/items/item_muerta_mercy_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_muerta_quest_item_slow",
	"abilities/items/item_muerta_mercy_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_muerta_quest_item_illusion",
	"abilities/items/item_muerta_mercy_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_muerta_quest_item_leash",
	"abilities/items/item_muerta_mercy_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function item_muerta_quest_item:OnProjectileHit(target, vLocation)
	...
	local illusions = CreateIllusions(
		target,
		target,
		{ duration = self.duration, outgoing_damage = -100, incoming_damage = 0 },
		1,
		1,
		false,
		true
	...
	for _, illusion in pairs(illusions) do
		...
		target:AddNewModifier(
			...
			self,
			"modifier_item_muerta_quest_item_leash",
			{ duration = self.duration, soul = illusion:entindex() }
		)
...
function modifier_item_muerta_quest_item_leash:OnIntervalThink()
	...
	if (self.soul:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.max_range then
		return
	end
	self:Destroy()
end
...
function modifier_item_muerta_quest_item_stats:GetModifierTotalDamageOutgoing_Percentage(params)
	if not params.target or not params.target:HasModifier("modifier_muerta_innate_custom_creep") then
		return
	end
...
function modifier_item_muerta_quest_item_slow:IsPurgable()
	return true
end
...
function modifier_item_muerta_quest_item_illusion:GetStatusEffectName()
	return "particles/status_fx/status_effect_muerta_parting_shot.vpcf"
end
function modifier_item_muerta_quest_item_illusion:StatusEffectPriority()
