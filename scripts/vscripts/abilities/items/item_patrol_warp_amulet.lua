...
function item_patrol_warp_amulet:OnSpellStart()
	...
	self.teleportFromEffect = ParticleManager:CreateParticle(
		"particles/econ/events/ti10/teleport/teleport_start_ti10_lvl1_rewardline.vpcf",
		PATTACH_ABSORIGIN,
		self:GetCaster()
	)
	...
	hero:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_patrol_warp_amulet",
		{ duration = self:GetChannelTime(), unit = self.teleport_center:entindex() }
	)
...
function item_patrol_warp_amulet:OnChannelThink(fInterval)
	if
		...
		or self:GetCaster():HasModifier("modifier_custom_puck_phase_shift")
	then
		self:GetCaster():Stop()
