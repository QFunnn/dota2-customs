...
LinkLuaModifier(
	...
	"abilities/items/item_overwhelming_blink_custom",
...
function item_overwhelming_blink_custom:OnSpellStart()
	...
	local max_range = self:MaxRange()
		+ (not caster:HasModifier("modifier_blink_break_custom") and caster:GetCastRangeBonus() or 0)
	...
	local effect =
		ParticleManager:CreateParticle("particles/items3_fx/blink_overwhelming_burst.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	for _, target in pairs(caster:FindTargets(self.radius)) do
		...
		target:AddNewModifier(
			...
			self,
			"modifier_item_overwhelming_blink_custom_debuff",
			{ damage_k = self.multicast_k or 1 }
		)
	end
...
function modifier_item_overwhelming_blink_custom_debuff:IsPurgable()
	return true
end
function modifier_item_overwhelming_blink_custom_debuff:GetStatusEffectName()
	return "particles/status_fx/status_effect_brewmaster_thunder_clap.vpcf"
end
function modifier_item_overwhelming_blink_custom_debuff:StatusEffectPriority()
...
function modifier_item_overwhelming_blink_custom_debuff:OnCreated(table)
	...
	self.damageTable.damage = (self.ability.damage_base + self.ability.damage_pct_instant * self.caster:GetStrength())
		* table.damage_k
