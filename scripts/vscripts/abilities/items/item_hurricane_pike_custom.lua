...
LinkLuaModifier(
	"modifier_item_hurricane_pike_custom",
	"abilities/items/item_hurricane_pike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_hurricane_pike_custom_active",
	"abilities/items/item_hurricane_pike_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_item_hurricane_pike_custom_range",
	"abilities/items/item_hurricane_pike_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function item_hurricane_pike_custom:CastFilterResultTarget(target)
	...
	if
		...
		and (target:IsRooted() or target:IsLeashed())
	then
...
function item_hurricane_pike_custom:OnSpellStart()
	...
	if caster:GetTeamNumber() ~= target:GetTeamNumber() then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_item_hurricane_pike_custom_active",
			{ duration = self.push_time, is_pike = 1, x = target:GetAbsOrigin().x, y = target:GetAbsOrigin().y }
		)
		caster:AddNewModifier(
			caster,
			self,
			"modifier_item_hurricane_pike_custom_range",
			{ duration = self.range_duration, target = target:entindex() }
		)
	end
	...
	target:AddNewModifier(
		...
		self,
		"modifier_item_hurricane_pike_custom_active",
		{ duration = self.push_time, is_pike = is_pike, x = caster:GetAbsOrigin().x, y = caster:GetAbsOrigin().y }
	)
end
...
function modifier_item_hurricane_pike_custom:GetModifierAttackRangeBonus()
	if self.parent:HasModifier("modifier_item_dragon_lance") then
		return
	end
...
function modifier_item_hurricane_pike_custom_active:GetStatusEffectName()
	return "particles/status_fx/status_effect_forcestaff.vpcf"
end
function modifier_item_hurricane_pike_custom_active:StatusEffectPriority()
...
function modifier_item_hurricane_pike_custom_range:IsPurgable()
	return true
end
...
function modifier_item_hurricane_pike_custom_range:GetModifierAttackSpeedBonus_Constant(params)
	...
	if not params.target or params.target ~= self.target then
		return
	end
