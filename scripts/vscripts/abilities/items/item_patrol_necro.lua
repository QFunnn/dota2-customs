...
function item_patrol_necro:OnSpellStart()
	...
	tower:AddNewModifier(
		caster,
		self,
		"modifier_item_patrol_necro",
		{ duration = delay, damage_out = damage_out, damage_inc = damage_inc }
	)
		...
		if spawner then
			...
			Timers:CreateTimer(0, function()
				GameRules:ExecuteTeamPing(
					...
					spawner:GetAbsOrigin().x,
					spawner:GetAbsOrigin().y,
					player,
					0
				)
				count = count + 1
...
function modifier_item_patrol_necro_caster:RemoveOnDeath()
	return false
end
function modifier_item_patrol_necro_caster:IsDebuff()
	return true
end
function modifier_item_patrol_necro_caster:GetTexture()
	return "item_necronomicon_2"
end

modifier_item_patrol_necro_timer = class(mod_visible)
function modifier_item_patrol_necro_timer:IsDebuff()
	return true
end
function modifier_item_patrol_necro_timer:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_item_patrol_necro_timer:RemoveOnDeath()
	return false
end
function modifier_item_patrol_necro_timer:GetTexture()
