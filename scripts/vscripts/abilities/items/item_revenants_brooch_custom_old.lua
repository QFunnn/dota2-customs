...
LinkLuaModifier(
	"modifier_revenants_brooch_custom",
...
function modifier_revenants_brooch_custom_counter:HasMana()
	...
	if
		self.parent:GetMaxMana() == 0
		or self.parent:GetMana() >= self.ability:GetEffectiveManaCost(self.ability:GetLevel())
	then
		return true
	else
		return false
...
function modifier_revenants_brooch_custom_counter:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if parent:HasModifier("modifier_custom_juggernaut_blade_fury") then
		return 0
	end
	...
	if not params.no_attack_cooldown or parent:HasModifier("modifier_alchemist_chemical_rage_custom_legendary") then
		if parent:GetMaxMana() ~= 0 then
			self.parent:SetMana(
				math.max(1, self.parent:GetMana() - self.ability:GetEffectiveManaCost(self.ability:GetLevel()))
			)
		else
