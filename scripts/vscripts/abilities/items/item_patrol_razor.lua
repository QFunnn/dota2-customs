...
LinkLuaModifier(
	"modifier_razor_tower_custom_friendly_aura",
	"abilities/items/item_patrol_razor",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_razor_tower_custom_friendly", "abilities/items/item_patrol_razor", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_razor_tower_custom_friendly_timer",
	"abilities/items/item_patrol_razor",
	LUA_MODIFIER_MOTION_NONE
)
...
function item_patrol_razor:OnSpellStart()
	...
	local shrines = FindUnitsInRadius(
		tower:GetTeamNumber(),
		tower:GetAbsOrigin(),
		nil,
		1200,
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"mini_alert_event",
		{ hero_1 = caster:GetUnitName(), heroes_2 = heroes, event_type = "razor" }
	)
...
function modifier_razor_tower_custom_aura:GetAuraDuration()
	return 0.1
end
function modifier_razor_tower_custom_aura:GetAuraRadius()
	return 1200
end
function modifier_razor_tower_custom_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_razor_tower_custom_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BUILDING
end
function modifier_razor_tower_custom_aura:GetModifierAura()
	return "modifier_razor_tower_custom"
end
...
function modifier_razor_tower_custom:OnCreated(table)
	...
	if mod then
		...
		self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_razor_tower_custom_timer",
...
function modifier_razor_tower_custom_friendly_aura:GetAuraDuration()
	return 0.1
end
function modifier_razor_tower_custom_friendly_aura:GetAuraRadius()
	return 1200
end
function modifier_razor_tower_custom_friendly_aura:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_razor_tower_custom_friendly_aura:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BUILDING
end
function modifier_razor_tower_custom_friendly_aura:GetModifierAura()
...
function modifier_razor_tower_custom_friendly:OnCreated(table)
	...
	self.heal_mod = self.parent:AddNewModifier(
		self.parent,
		self,
		"modifier_generic_repair",
		{ duration = self.heal_duration, tower_heal = self.tower_heal, heal_shrine = self.shrine_heal }
	)
