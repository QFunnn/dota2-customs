...
function modifier_item_gem_custom:OnCreated(table)
	...
	self.particle =
		ParticleManager:CreateParticle("particles/items/gem_activate.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.parent)
...
function modifier_item_gem_custom:DeathEvent(params)
	...
	if self.parent ~= params.unit or self.parent:IsReincarnating() then
		return
	end
	self:Destroy()
end
...
function modifier_item_gem_custom_cd:GetTexture()
	return "item_gem"
end
function modifier_item_gem_custom_cd:RemoveOnDeath()
	return false
end
function modifier_item_gem_custom_cd:IsDebuff()
	return true
end
