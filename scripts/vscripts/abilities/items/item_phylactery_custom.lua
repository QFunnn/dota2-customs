...
function item_phylactery_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/skywrath_mage/skywrath_arcana/skywrath_arcana_phylactery_v2.vpcf",
		context
	)
...
function modifier_item_phylactery_custom:DamageEvent_out(params)
	...
	if not RollPseudoRandomPercentage(self.crit_chance, index, self.parent) then
		return
	end
