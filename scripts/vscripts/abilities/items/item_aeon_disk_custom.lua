...
function item_aeon_disk_custom:GetIntrinsicModifierName()
	return "item_aeon_disk_custom_passive"
end
...
function item_aeon_disk_custom_passive:DamageEvent_inc(params)
	...
	if self.parent:GetHealthPercent() > self.health_pct then
		return
	end
	if self.parent:HasModifier("item_aeon_disk_custom_cd") then
		return
	end
