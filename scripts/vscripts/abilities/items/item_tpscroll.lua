...
function item_tpscroll_custom:GlobalTeleport()
	...
	return caster:HasModifier("modifier_patrol_reward_2_portal")
		or (caster:HasModifier("modifier_tinker_innate_custom") and caster:HasScepter())
end
...
function item_tpscroll_custom:GetCastRange(vector, target)
	local caster = self:GetCaster()
	if
		(IsClient() or caster:HasModifier("modifier_the_hunt_custom_hero"))
		and caster:HasScepter()
		and self.tinker_range
	then
...
function item_tpscroll_custom:OnSpellStart()
	...
	local tinker_checkout =
		wearables_system:GetParticleReplacementAbility(caster, "particles/items2_fx/teleport_start.vpcf", self)
	...
	if
		effect_end == "particles/econ/events/compendium_2024/compendium_2024_teleport_lvl3_end.vpcf"
		or effect_end == "particles/econ/events/compendium_2023/compendium_2023_teleport_lvl3_end.vpcf"
	then
	...
	caster:AddNewModifier(
		caster,
		self,
		"modifier_custom_ability_teleport",
		{ duration = self:GetChannelTime(), center = self.teleport_center:entindex() }
	)
...
function modifier_custom_ability_teleport_tracker:SpellEvent(params)
	...
	if
		self.parent:HasModifier("modifier_item_travel_boots_custom")
		or self.parent:HasModifier("modifier_item_travel_boots_2_custom")
		or self.parent:HasModifier("modifier_item_travel_boots_2_perma")
	then
	...
	if self.ability.tinker_cast and self.parent:HasScepter() then
		...
		if
			self.ability:GetCursorTarget()
			and self.ability:GetCursorTarget() == self.parent
			and towers[self.parent:GetTeamNumber()]
		then
