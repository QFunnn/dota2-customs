...
function modifier_item_javelin_custom:CheckState()
	...
	if not self.parent:HasModifier("modifier_item_javelin_custom_proc") then
		return
	end
	return {
...
function modifier_item_javelin_custom:RollProc()
	...
	if not RollPseudoRandomPercentage(self.ability.bonus_chance, 4161, self.parent) then
		return
	end
