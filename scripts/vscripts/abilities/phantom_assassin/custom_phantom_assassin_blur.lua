...
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_stunready",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_heal",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_legendary_smoke",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_legendary_caster",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_legendary_target",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_damage_stack",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_invun",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_phantom_assassin_phantom_quest",
	"abilities/phantom_assassin/custom_phantom_assassin_blur",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_phantom_assassin_blur:OnSpellStart()
	...
	if caster:HasTalent("modifier_phantom_assassin_blur_6") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_phantom_assassin_phantom_invun",
			{ duration = caster:GetTalentValue("modifier_phantom_assassin_blur_6", "duration") }
		)
	end
...
function modifier_phantom_assassin_phantom_blur:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_blur:IsPurgable()
	return false
end
...
function modifier_phantom_assassin_phantom_blur:AttackEvent_out(params)
	...
	if
		self.parent:HasTalent("modifier_phantom_assassin_blur_5")
		and (mod or stun_mod)
		and not params.no_attack_cooldown
	then
		for i = 1, 3 do
			local particle = ParticleManager:CreateParticle(
				"particles/phantom_assassin/blur_range_hit.vpcf",
	...
	if stun_mod and not params.no_attack_cooldown then
		...
		local effect_cast =
			ParticleManager:CreateParticle("particles/pa_blur_attack.vpcf", PATTACH_OVERHEAD_FOLLOW, target)
		ParticleManager:SetParticleControl(effect_cast, 0, target:GetOrigin())
		ParticleManager:SetParticleControl(effect_cast, 1, target:GetOrigin())
		ParticleManager:SetParticleControlForward(
			effect_cast,
			1,
			(self.parent:GetAbsOrigin() - target:GetAbsOrigin()):Normalized()
		)
	...
	local trail_pfx = ParticleManager:CreateParticle(
		"particles/phantom_assassin/blur_proc_damage.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		target
	)
	...
	ParticleManager:SetParticleControlForward(
		trail_pfx,
		1,
		(target:GetAbsOrigin() - self.parent:GetAbsOrigin()):Normalized()
	)
...
function modifier_phantom_assassin_phantom_blur:GetModifierAttackRangeBonus()
	if not self.parent:HasTalent("modifier_phantom_assassin_blur_5") then
		return
	end
...
function modifier_phantom_assassin_phantom_blur:GetModifierAttackSpeedBonus_Constant()
	if not self.parent:HasTalent("modifier_phantom_assassin_blur_3") then
		return
	end
	local bonus = self.parent:GetTalentValue("modifier_phantom_assassin_blur_3", "speed")
	if
		self.parent:HasModifier("modifier_phantom_assassin_phantom_smoke")
		or self.parent:HasModifier("modifier_phantom_assassin_phantom_heal")
	then
...
function modifier_phantom_assassin_phantom_smoke:IsHidden()
	return false
end
function modifier_phantom_assassin_phantom_smoke:IsPurgable()
	return false
end
...
function modifier_phantom_assassin_phantom_smoke:OnCreated()
	...
	self.delay = self.ability:GetSpecialValueFor("delay")
		+ self.parent:GetTalentValue("modifier_phantom_assassin_blur_1", "delay")
	self.shard_damage = self.ability:GetSpecialValueFor("shard_damage")

	self.speed_bonus = self.ability:GetSpecialValueFor("active_movespeed_bonus")
		+ self.parent:GetTalentValue("modifier_phantom_assassin_blur_2", "speed")
	...
	if self.parent:HasTalent("modifier_phantom_assassin_blur_5") then
		self.stun_mod = self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_phantom_assassin_phantom_stunready",
...
function modifier_phantom_assassin_phantom_smoke:OnDestroy()
	if self.stun_mod and not self.stun_mod:IsNull() then
		self.stun_mod:SetDuration(
			self.parent:GetTalentValue("modifier_phantom_assassin_blur_5", "effect_duration"),
			true
		)
	end
	...
	if
		self.parent:HasTalent("modifier_phantom_assassin_blur_2")
		or self.parent:HasTalent("modifier_phantom_assassin_blur_3")
	then
		self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_phantom_assassin_phantom_heal",
			{ duration = self.parent:GetTalentValue("modifier_phantom_assassin_blur_2", "duration", true) }
		)
	end
	...
	if
		self.parent:HasTalent("modifier_phantom_assassin_blur_7")
		or self.parent:HasTalent("modifier_phantom_assassin_blur_4")
	then
		CreateModifierThinker(
			self.parent,
			self.ability,
			"modifier_phantom_assassin_phantom_legendary_smoke",
			{ duration = self.parent:GetTalentValue("modifier_phantom_assassin_blur_7", "duration", true) },
			self.parent:GetAbsOrigin(),
			self.parent:GetTeamNumber(),
			false
		)
...
function modifier_phantom_assassin_phantom_stunready:IsPurgable()
	return false
end
function modifier_phantom_assassin_phantom_stunready:IsHidden()
	return false
end
function modifier_phantom_assassin_phantom_stunready:GetTexture()
	return "buffs/Blur_silence"
end
...
function modifier_phantom_assassin_phantom_heal:IsHidden()
	return false
end
function modifier_phantom_assassin_phantom_heal:IsPurgable()
	return false
end
function modifier_phantom_assassin_phantom_heal:GetTexture()
	return "buffs/Blur_heal"
end

function modifier_phantom_assassin_phantom_heal:OnCreated(table)
	self.parent = self:GetParent()
	self.speed = self:GetAbility():GetSpecialValueFor("active_movespeed_bonus")
		+ self.parent:GetTalentValue("modifier_phantom_assassin_blur_2", "speed")
...
function modifier_phantom_assassin_phantom_heal:GetModifierHealthRegenPercentage()
	if not self.parent:HasTalent("modifier_phantom_assassin_blur_2") then
		return
	end
	return self.heal
end
...
function modifier_phantom_assassin_phantom_speed:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_speed:IsPurgable()
	return false
end

modifier_phantom_assassin_phantom_legendary_smoke = class({})
function modifier_phantom_assassin_phantom_legendary_smoke:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_legendary_smoke:IsPurgable()
	return false
end
function modifier_phantom_assassin_phantom_legendary_smoke:OnCreated()
	...
	self.particle =
		ParticleManager:CreateParticle("particles/phantom_assassin/blur_smoke.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	if not self.caster:HasTalent("modifier_phantom_assassin_blur_4") then
		return
	end
...
function modifier_phantom_assassin_phantom_legendary_smoke:IsAura()
	return self.caster:HasTalent("modifier_phantom_assassin_blur_7")
end
function modifier_phantom_assassin_phantom_legendary_smoke:GetAuraDuration()
	return 0
end
function modifier_phantom_assassin_phantom_legendary_smoke:GetAuraRadius()
	return self.radius
end
function modifier_phantom_assassin_phantom_legendary_smoke:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_phantom_assassin_phantom_legendary_smoke:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO
end
function modifier_phantom_assassin_phantom_legendary_smoke:GetModifierAura()
	return "modifier_phantom_assassin_phantom_legendary_caster"
end
...
function modifier_phantom_assassin_phantom_legendary_smoke:OnIntervalThink()
	...
	for _, target in pairs(self.caster:FindTargets(self.radius, self.point)) do
		target:AddNewModifier(
			...
			self.ability,
			"modifier_phantom_assassin_phantom_legendary_target",
			{ duration = self.linger }
		)
	end
end

modifier_phantom_assassin_phantom_legendary_caster = class({})
function modifier_phantom_assassin_phantom_legendary_caster:IsHidden()
	return false
end
function modifier_phantom_assassin_phantom_legendary_caster:IsPurgable()
	return false
end
...
function modifier_phantom_assassin_phantom_legendary_caster:NoDamage(params, type)
	...
	if params.damage_type ~= type then
		return 0
	end
...
function modifier_phantom_assassin_phantom_legendary_target:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_legendary_target:IsPurgable()
	return false
end
...
function modifier_phantom_assassin_phantom_damage_stack:IsHidden()
	return false
end
function modifier_phantom_assassin_phantom_damage_stack:IsPurgable()
	return false
end
function modifier_phantom_assassin_phantom_damage_stack:GetTexture()
	return "buffs/tree_regen"
end
function modifier_phantom_assassin_phantom_damage_stack:OnCreated(table)
	...
	self.effect_cast = ParticleManager:CreateParticle(
		"particles/phantom_assassin/blur_stack.vpcf",
...
function modifier_phantom_assassin_phantom_invun:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_invun:IsPurgable()
	return false
end
...
function modifier_phantom_assassin_phantom_quest:IsHidden()
	return true
end
function modifier_phantom_assassin_phantom_quest:IsPurgable()
	return false
end