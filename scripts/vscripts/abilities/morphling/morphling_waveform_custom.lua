...
LinkLuaModifier(
	"modifier_morphling_waveform_custom_tracker",
	"abilities/morphling/morphling_waveform_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_morphling_waveform_custom_trail_aura",
	"abilities/morphling/morphling_waveform_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function morphling_waveform_custom:DealDamage(target)
	...
	if
		self.talents.has_h4 == 1
		and not target:HasModifier("modifier_morphling_waveform_custom_silence_cd")
...
function modifier_morphling_waveform_custom:OnCreated(table)
	...
	self.has_trail = self.ability.talents.has_q1 == 1
	if not self.has_trail then
		return
	end
...
function modifier_morphling_waveform_custom_tracker:OnIntervalThink()
	...
	if self.parent:HasModifier("modifier_morphling_waveform_custom") then
		return
	end
...
function modifier_morphling_waveform_custom_legendary:OnRefresh(table)
	...
	if self:GetStackCount() >= self.max then
		self.parent:AddNewModifier(
			...
			"modifier_morphling_waveform_custom_legendary_effect",
			{ duration = self.ability.talents.q7_effect_duration }
		)
		self:Destroy()
	end
end
...
function modifier_morphling_waveform_custom_legendary_effect:GetTexture()
...
function modifier_morphling_waveform_custom_legendary_effect:OnCreated(table)
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/morphling/wave_health_reduce.vpcf",
...
function modifier_morphling_waveform_custom_legendary_effect:OnIntervalThink(first)
	...
	self.caster:UpdateUIshort({
		...
		stack = self.ability.talents.q7_health_reduce .. "%",
		style = "MorphWave",
	})
end
...
function modifier_morphling_waveform_custom_trail_thinker:IsAura()
	return true
end
function modifier_morphling_waveform_custom_trail_thinker:GetAuraDuration()
	return self.ability.talents.q1_linger
end
function modifier_morphling_waveform_custom_trail_thinker:GetAuraRadius()
	return self.radius
end
function modifier_morphling_waveform_custom_trail_thinker:GetAuraSearchType()
	return DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO
end
function modifier_morphling_waveform_custom_trail_thinker:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_ENEMY
end
function modifier_morphling_waveform_custom_trail_thinker:GetModifierAura()
	return "modifier_morphling_waveform_custom_trail_aura"
end
function modifier_morphling_waveform_custom_trail_thinker:OnCreated(table)
	...
	self.puddle_particle =
		ParticleManager:CreateParticle("particles/morphling/wave_trail.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_morphling_waveform_custom_trail_aura:GetStatusEffectName()
	return "particles/status_fx/status_effect_naga_riptide.vpcf"
end
function modifier_morphling_waveform_custom_trail_aura:StatusEffectPriority()
...
function modifier_morphling_waveform_custom_trail_aura:OnCreated()
	...
	self.damageTable = {
		...
		damage_type = self.ability.talents.q1_damage_type,
		damage = self.ability.talents.q1_damage * self.interval,
	}
...
function modifier_morphling_waveform_custom_magic_reduce:GetTexture()
	return "buffs/morphling/wave_3"
end
...
function modifier_morphling_waveform_custom_bonus:GetEffectName()
	return "particles/econ/items/slardar/slardar_back_ti9/slardar_back_ti9_sprint.vpcf"
end
function modifier_morphling_waveform_custom_bonus:GetTexture()
	return "buffs/morphling/wave_4"
end
...
function modifier_morphling_waveform_custom_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf"
end
function modifier_morphling_waveform_custom_silence:ShouldUseOverheadOffset()
	return true
end
function modifier_morphling_waveform_custom_silence:GetEffectAttachType()
