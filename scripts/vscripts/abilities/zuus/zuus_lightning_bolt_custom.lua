...
LinkLuaModifier(
	"modifier_zuus_lightning_bolt_custom_tracker",
...
function zuus_lightning_bolt_custom:CastLightningBolt(point, damage_ability)
	...
	local bolt_pfx = wearables_system:GetParticleReplacementAbility(
		self.caster,
	...
	for _, unit in pairs(targets) do
		...
		local damage = base_damage
			* (1 + legendary_damage * math.pow(legendary_stack / self.talents.w7_max_stack, self.talents.w7_damage_k))
		...
		if
			self.talents.has_w4 == 1
			and not unit:HasModifier("modifier_zuus_lightning_bolt_custom_root_cd")
			and not unit:IsDebuffImmune()
		then
			unit:AddNewModifier(
				self.caster,
				self,
				"modifier_zuus_lightning_bolt_custom_root",
				{ duration = stun + (1 - unit:GetStatusResistance()) * self.talents.w4_root }
			)
			unit:AddNewModifier(
				self.caster,
				self,
		...
		local real_damage = DoDamage(
			{ attacker = self.caster, ability = self, damage_type = DAMAGE_TYPE_MAGICAL, damage = damage, victim = unit },
			damage_ability
		)
...
function modifier_zuus_lightning_bolt_custom_tracker:SpellEvent(params)
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_zuus_lightning_bolt_custom_item_stack",
		{ target = index, delay = delay, duration = self.ability.talents.w3_duration }
	)
end

modifier_zuus_lightning_bolt_custom_item_stack = class(mod_visible)
function modifier_zuus_lightning_bolt_custom_item_stack:GetTexture()
	return "buffs/zeus/bolt_3"
end
...
function modifier_zuus_lightning_bolt_custom_legendary:OnCreated(table)
	...
	self.zuus_nimbus_particle =
		ParticleManager:CreateParticle("particles/units/heroes/hero_zeus/zeus_cloud.vpcf", PATTACH_WORLDORIGIN, nil)
