...
function modifier_zuus_innate_custom:RemoveOnDeath()
	return false
end
...
function modifier_zuus_innate_custom:DamageEvent_inc(params)
	...
	if self.parent:HasModifier("modifier_zuus_innate_custom_purge_cd") then
		return
	end
	if self.ability.talents.has_h5 == 0 then
		return
...
function modifier_zuus_innate_custom_purge_cd:GetTexture()
	return "buffs/zeus/hero_5"
end

modifier_zuus_innate_custom_damage_reduce = class(mod_hidden)
function modifier_zuus_innate_custom_damage_reduce:GetStatusEffectName()
	return "particles/status_fx/status_effect_mjollnir_shield.vpcf"
end
function modifier_zuus_innate_custom_damage_reduce:StatusEffectPriority()
