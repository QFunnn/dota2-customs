...
LinkLuaModifier(
	"modifier_custom_reqiuem_debuff",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_tracker",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_cast",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_cast_cd",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_legendary",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_legendary_quest",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_legendary_stack",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_legendary_pull",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_damage_reduce",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_speed_damage",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_spell_absorb",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_reqiuem_scepter_heal",
	"abilities/shadow_fiend/custom_nevermore_requiem",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_nevermore_requiem:LegendaryStack(count)
	...
	if
		not caster:HasTalent("modifier_nevermore_requiem_7") and not caster:HasTalent("modifier_nevermore_requiem_4")
	then
		return
	end
	...
	for i = 1, count do
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_reqiuem_legendary_stack",
			{ duration = caster:GetTalentValue("modifier_nevermore_requiem_7", "duration", true) }
		)
	end
end
...
function custom_nevermore_requiem:GetCastPoint()
	...
	local caster = self:GetCaster()
	if
		caster:HasModifier("modifier_custom_reqiuem_legendary_stack")
		and caster:HasTalent("modifier_nevermore_requiem_7")
	then
		bonus = caster:GetUpgradeStack("modifier_custom_reqiuem_legendary_stack")
			* caster:GetTalentValue("modifier_nevermore_requiem_7", "cast")
	end
...
function custom_nevermore_requiem:OnAbilityPhaseStart()
	...
	if
		not caster:HasModifier("modifier_custom_reqiuem_cast_cd") and caster:HasTalent("modifier_nevermore_requiem_5")
	then
		self.block_mod =
			caster:AddNewModifier(caster, self, "modifier_custom_reqiuem_spell_absorb", { duration = cast_time + 0.1 })
	...
	local particle_wing_name = wearables_system:GetParticleReplacementAbility(
		caster,
...
function custom_nevermore_requiem:OnSpellStart(death_cast)
	...
	if mod then
		if
			mod:GetStackCount() >= caster:GetTalentValue("modifier_nevermore_requiem_7", "max")
			and caster:HasTalent("modifier_nevermore_requiem_7")
		then
			caster:CdAbility(
				self,
			...
			local particle = ParticleManager:CreateParticle(
				"particles/shadow_fiend/requiem_refresh.vpcf",
		...
		if caster:HasTalent("modifier_nevermore_requiem_4") then
			caster:AddNewModifier(
				...
				"modifier_custom_reqiuem_speed_damage",
				{
					duration = caster:GetTalentValue("modifier_nevermore_requiem_4", "duration"),
	...
	local particle_caster_souls = wearables_system:GetParticleReplacementAbility(
		caster,
		"particles/units/heroes/hero_nevermore/nevermore_requiemofsouls_a.vpcf",
		self
	)
	local particle_caster_ground = wearables_system:GetParticleReplacementAbility(
		caster,
...
function custom_nevermore_requiem:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	local damage_dealt = DoDamage(
		{ victim = target, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, attacker = caster, ability = self },
		damage_ability
	)
	...
	if not death_cast and not target:IsDebuffImmune() then
		if not fear_mod then
			fear_mod = target:AddNewModifier(
				caster,
				self,
				"modifier_nevermore_requiem_fear",
				{ duration = slow_duration * target_status }
			)
		else
			fear_mod:SetDuration(
				math.min(fear_mod:GetRemainingTime() + slow_duration, max_duration) * target_status,
				true
			)
		end
	end
...
function modifier_custom_reqiuem_debuff:IsHidden()
	return true
end
function modifier_custom_reqiuem_debuff:IsPurgable()
	return true
end
...
function modifier_custom_reqiuem_tracker:IsHidden()
	return true
end
function modifier_custom_reqiuem_tracker:IsPurgable()
	return false
end
...
function modifier_custom_reqiuem_tracker:UpdateUI()
	...
	if
		not self.parent:HasTalent("modifier_nevermore_requiem_7")
		and not self.parent:HasTalent("modifier_nevermore_requiem_4")
	then
		return
	end
	local stack = 0
...
function modifier_custom_reqiuem_legendary_stack:IsHidden()
	return false
end
function modifier_custom_reqiuem_legendary_stack:IsPurgable()
	return false
end
function modifier_custom_reqiuem_legendary_stack:IsDebuff()
	return true
end
	...
	if self:GetStackCount() >= self.max then
		...
		if self.parent:HasTalent("modifier_nevermore_requiem_7") then
			self.radius_visual = ParticleManager:CreateParticleForPlayer(
				"particles/shadow_fiend/requiem_radius.vpcf",
...
function modifier_custom_reqiuem_cast:IsHidden()
	return true
end
function modifier_custom_reqiuem_cast:IsPurgable()
	return false
end
function modifier_custom_reqiuem_cast:OnCreated()
	...
	if
		mod
		and self.parent:HasTalent("modifier_nevermore_requiem_7")
		and mod:GetStackCount() >= self.max
		and not self.parent:HasModifier("modifier_custom_reqiuem_cast_cd")
	then
		...
		for _, target in pairs(self.parent:FindTargets(self.radius)) do
			...
			local mod =
				target:AddNewModifier(self.parent, self.parent:BkbAbility(self.ability, true), "modifier_generic_arc", {
					target_x = point.x,
					target_y = point.y,
					distance = distance,
			...
			if mod then
				local pfx = ParticleManager:CreateParticle(
					"particles/shadow_fiend/requiem_pull.vpcf",
					PATTACH_ABSORIGIN_FOLLOW,
					target
				)
				ParticleManager:SetParticleControlEnt(
...
function modifier_custom_reqiuem_legendary_pull:IsHidden()
	return true
end
function modifier_custom_reqiuem_legendary_pull:IsPurgable()
	return false
end
...
function modifier_custom_reqiuem_cast_cd:IsHidden()
	return false
end
function modifier_custom_reqiuem_cast_cd:IsPurgable()
	return false
end
function modifier_custom_reqiuem_cast_cd:IsDebuff()
	return true
end
function modifier_custom_reqiuem_cast_cd:GetTexture()
	return "buffs/darklord_damage"
end
...
function modifier_custom_reqiuem_legendary_quest:IsHidden()
	return true
end
function modifier_custom_reqiuem_legendary_quest:IsPurgable()
	return false
end
...
function modifier_custom_reqiuem_damage_reduce:IsHidden()
	return false
end
function modifier_custom_reqiuem_damage_reduce:IsPurgable()
	return false
end
function modifier_custom_reqiuem_damage_reduce:GetTexture()
	return "buffs/counterspell_slow"
end
...
function modifier_custom_reqiuem_speed_damage:IsHidden()
	return false
end
function modifier_custom_reqiuem_speed_damage:IsPurgable()
	return false
end
function modifier_custom_reqiuem_speed_damage:GetTexture()
	return "buffs/fireblast_damage"
end
function modifier_custom_reqiuem_speed_damage:OnCreated(table)
	...
	self.buff_particle = ParticleManager:CreateParticle(
		"particles/shadow_fiend/requiem_speed.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent
	)
...
function modifier_custom_reqiuem_spell_absorb:IsHidden()
	return true
end
function modifier_custom_reqiuem_spell_absorb:IsPurgable()
	return false
end
function modifier_custom_reqiuem_spell_absorb:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/shadow_fiend/requiem_block.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_custom_reqiuem_spell_absorb:GetAbsorbSpell(params)
	...
	if params.ability:GetCaster():GetTeamNumber() == self.parent:GetTeamNumber() then
		return
	end
...
function modifier_custom_reqiuem_scepter_heal:IsHidden()
	return false
end
function modifier_custom_reqiuem_scepter_heal:IsPurgable()
	return false
end
