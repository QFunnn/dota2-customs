...
LinkLuaModifier(
	"modifier_nevermore_frenzy_custom",
	"abilities/shadow_fiend/nevermore_frenzy_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_frenzy_custom_shield",
	"abilities/shadow_fiend/nevermore_frenzy_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function nevermore_frenzy_custom:GetBehavior()
	...
	if
		not caster:HasModifier("modifier_slark_saltwater_shiv_custom_legendary_steal")
		and not caster:HasShard()
		and (
			not caster:HasModifier("modifier_custom_necromastery_souls")
			or caster:GetUpgradeStack("modifier_custom_necromastery_souls") < cost
		)
	then
...
function nevermore_frenzy_custom:OnAbilityPhaseStart()
	...
	CustomGameEventManager:Send_ServerToPlayer(
		PlayerResource:GetPlayer(self:GetCaster():GetPlayerID()),
...
function nevermore_frenzy_custom:OnSpellStart()
	...
	local duration = self:GetSpecialValueFor("duration")
		+ caster:GetTalentValue("modifier_nevermore_souls_4", "duration")
...
function modifier_nevermore_frenzy_custom:IsHidden()
	return false
end
function modifier_nevermore_frenzy_custom:IsPurgable()
	return false
end
function modifier_nevermore_frenzy_custom:OnCreated(table)
	...
	self.speed = self.ability:GetSpecialValueFor("bonus_attack_speed")
		+ self.caster:GetTalentValue("modifier_nevermore_souls_2", "speed")
	...
	self.move = self.ability:GetSpecialValueFor("move_speed")
		+ self.caster:GetTalentValue("modifier_nevermore_souls_3", "move")
...
function modifier_nevermore_frenzy_custom:GetModifierStatusResistanceStacking()
	if not self.caster:HasTalent("modifier_nevermore_souls_3") then
		return
	end
	return self.status
end
...
function modifier_nevermore_frenzy_custom:GetPriority()
	if not self:GetParent():HasTalent("modifier_nevermore_souls_7") then
...
function modifier_nevermore_frenzy_custom_shield:IsHidden()
	return true
end
function modifier_nevermore_frenzy_custom_shield:IsPurgable()
	return false
end
function modifier_nevermore_frenzy_custom_shield:OnCreated(table)
	...
	self.max_shield = self.caster:GetTalentValue("modifier_nevermore_souls_1", "shield")
