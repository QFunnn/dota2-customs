...
LinkLuaModifier(
	"modifier_custom_shadowraze_debuff",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_combo",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_speedmax",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_tracker",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_cd",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_auto",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_combo_heroes",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_shadowraze_slow_stack",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_nevermore_requiem_fear_cd",
	"abilities/shadow_fiend/custom_nevermore_shadowraze",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function custom_nevermore_shadowraze_close:OnSpellStart()
	...
	local ability = self
	local cast_response =
		{ "nevermore_nev_ability_shadow_07", "nevermore_nev_ability_shadow_18", "nevermore_nev_ability_shadow_21" }
...
function custom_nevermore_shadowraze_medium:OnSpellStart()
	...
	local ability = self
	local cast_response =
		{ "nevermore_nev_ability_shadow_08", "nevermore_nev_ability_shadow_20", "nevermore_nev_ability_shadow_22" }
	local sound_raze = wearables_system:GetSoundReplacement(caster, "Hero_Nevermore.Shadowraze", self)
	caster:EmitSound(sound_raze)
...
function custom_nevermore_shadowraze_far:OnSpellStart()
	...
	local ability = self
	local cast_response =
		{ "nevermore_nev_ability_shadow_11", "nevermore_nev_ability_shadow_19", "nevermore_nev_ability_shadow_23" }
	local sound_raze = wearables_system:GetSoundReplacement(caster, "Hero_Nevermore.Shadowraze", self)
	caster:EmitSound(sound_raze)
	local raze_radius = ability:GetSpecialValueFor("shadowraze_radius")
	local raze_distance = ability:GetSpecialValueFor("shadowraze_range")
...
function CastShadowRazeOnPoint(caster, ability, point, radius, auto)
	local particle_raze = wearables_system:GetParticleReplacementAbility(
		caster,
	...
	for _, enemy in pairs(enemies) do
		...
		if
			caster:HasTalent("modifier_nevermore_raze_5")
			and ability:GetAbilityName() == "custom_nevermore_shadowraze_close"
			and not auto
			and not enemy:HasModifier("modifier_nevermore_requiem_fear_cd")
		then
	...
	if #enemies > 0 then
		if caster:HasTalent("modifier_nevermore_raze_3") then
			caster:AddNewModifier(
				...
				ability,
				"modifier_custom_shadowraze_speedmax",
				{ duration = caster:GetTalentValue("modifier_nevermore_raze_3", "duration") }
			)
		end
		...
		if caster:HasTalent("modifier_nevermore_raze_7") then
			caster:AddNewModifier(
				...
				ability,
				"modifier_custom_shadowraze_combo",
				{ duration = caster:GetTalentValue("modifier_nevermore_raze_7", "timer", true) }
			)
		end
...
function ApplyShadowRazeDamage(caster, ability, enemy, damage_ability)
	...
	if enemy:HasModifier("modifier_custom_shadowraze_debuff") then
		damage = damage
			+ enemy:FindModifierByName("modifier_custom_shadowraze_debuff"):GetStackCount() * stack_bonus_damage
	end

	local damageTable =
		{ victim = enemy, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, attacker = caster, ability = ability }
	...
	if caster:HasTalent("modifier_nevermore_raze_5") then
		enemy:AddNewModifier(
			...
			ability,
			"modifier_custom_shadowraze_slow_stack",
			{ duration = caster:GetTalentValue("modifier_nevermore_raze_5", "duration") }
		)
	end
end
...
function modifier_custom_shadowraze_debuff:IsPurgable()
	return not self:GetCaster():HasTalent("modifier_nevermore_raze_7")
end
...
function modifier_custom_shadowraze_combo:IsHidden()
	return false
end
function modifier_custom_shadowraze_combo:IsPurgable()
	return false
end
...
function modifier_custom_shadowraze_combo:OnRefresh(table)
	...
	if self:GetStackCount() >= self.max then
		if self.caster:HasTalent("modifier_nevermore_raze_7") then
			...
			self.particle_head = ParticleManager:CreateParticle(
				"particles/econ/items/shadow_fiend/sf_fire_arcana/sf_fire_arcana_shadowraze_triple.vpcf",
...
function modifier_custom_shadowraze_combo_heroes:IsHidden()
	return true
end
function modifier_custom_shadowraze_combo_heroes:IsPurgable()
	return false
end
...
function modifier_custom_shadowraze_speedmax:IsPurgable()
	return true
end
function modifier_custom_shadowraze_speedmax:IsHidden()
	return false
end
function modifier_custom_shadowraze_speedmax:GetTexture()
	return "buffs/raze_speed"
end
...
function modifier_custom_shadowraze_tracker:IsHidden()
	return true
end
function modifier_custom_shadowraze_tracker:IsPurgable()
	return false
end
...
function modifier_custom_shadowraze_tracker:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_nevermore_raze_4") then
		return
	end
	if self.parent:HasModifier("modifier_custom_shadowraze_cd") then
		return
	end
...
function modifier_custom_shadowraze_cd:IsHidden()
	return false
end
function modifier_custom_shadowraze_cd:IsPurgable()
	return false
end
function modifier_custom_shadowraze_cd:GetTexture()
	return "buffs/raze_burn"
end
function modifier_custom_shadowraze_cd:IsDebuff()
	return true
end
function modifier_custom_shadowraze_cd:RemoveOnDeath()
	return false
end

modifier_custom_shadowraze_auto = class({})

function modifier_custom_shadowraze_auto:IsHidden()
	return true
end
function modifier_custom_shadowraze_auto:IsPurgable()
	return false
end
function modifier_custom_shadowraze_auto:OnCreated(table)
	...
	self.radius = self.ability:GetSpecialValueFor("shadowraze_radius")
		* self.caster:GetTalentValue("modifier_nevermore_raze_4", "aoe")
	...
	ParticleManager:SetParticleControl(
		self.effect_cast,
		1,
		Vector(self.radius, 0, -self.radius / self:GetRemainingTime())
	)
...
function modifier_custom_shadowraze_slow_stack:IsHidden()
	return false
end
function modifier_custom_shadowraze_slow_stack:IsPurgable()
	return true
end
function modifier_custom_shadowraze_slow_stack:GetTexture()
...
function modifier_nevermore_requiem_fear_cd:IsHidden()
	return true
end
function modifier_nevermore_requiem_fear_cd:IsPurgable()
	return false
end
function modifier_nevermore_requiem_fear_cd:RemoveOnDeath()
	return false
end
...
function modifier_custom_shadowraze_perma:IsHidden()
	return not self:GetCaster():HasTalent("modifier_nevermore_raze_6")
end
function modifier_custom_shadowraze_perma:IsPurgable()
	return false
end
function modifier_custom_shadowraze_perma:RemoveOnDeath()
	return false
end
function modifier_custom_shadowraze_perma:GetTexture()
...
function modifier_custom_shadowraze_perma:OnIntervalThink()
	...
	if not self.caster:HasTalent("modifier_nevermore_raze_6") then
		return
	end
