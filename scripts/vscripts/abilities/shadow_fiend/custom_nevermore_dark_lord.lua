...
function custom_nevermore_dark_lord:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/npc_dota_hero_nevermorere/nevermore_requiemofsouls_line.vpcf",
		context
	)
end
...
function custom_nevermore_dark_lord:GetCooldown(iLevel)
	if
		self:GetCaster():HasTalent("modifier_nevermore_darklord_7")
		or self:GetCaster():HasTalent("modifier_nevermore_darklord_5")
	then
...
function custom_nevermore_dark_lord:OnSpellStart()
	...
	if caster:HasTalent("modifier_nevermore_darklord_5") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_dark_lord_speed",
			{ duration = caster:GetTalentValue("modifier_nevermore_darklord_5", "duration") }
		)
	end

	if not caster:HasTalent("modifier_nevermore_darklord_7") then
		return
	end
	...
	if not mod then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_custom_dark_lord_legendary",
			{ duration = caster:GetTalentValue("modifier_nevermore_darklord_7", "duration") }
		)
		return
	end
...
function custom_nevermore_dark_lord:OnProjectileHit(target, vLocation)
	...
	target:AddNewModifier(
		...
		{ duration = (1 - target:GetStatusResistance()) * fear }
	)
end

modifier_custom_dark_lord_aura = class({})
function modifier_custom_dark_lord_aura:IsHidden()
	return true
end
function modifier_custom_dark_lord_aura:IsPurgable()
	return false
end
...
function modifier_custom_dark_lord_aura:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_nevermore_darklord_4") then
		return
	end
	...
	if #targets > 0 then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_custom_dark_lord_damage",
			{ duration = self.damage_duration, interval = self.damage_interval }
		)
	end
...
function modifier_custom_dark_lord_aura:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_nevermore_darklord_3") then
		return
	end
...
function modifier_custom_dark_lord_aura:AttackStartEvent_inc(params)
	...
	if not self.parent:HasTalent("modifier_nevermore_darklord_6") then
		return
	end
...
function modifier_custom_dark_lord_debuff:IsHidden()
	return false
end
function modifier_custom_dark_lord_debuff:IsPurgable()
	return false
end
...
function modifier_custom_dark_lord_debuff:OnCreated(table)
	...
	self.armor = -1 * self.ability:GetSpecialValueFor("presence_armor_reduction")
		+ self.caster:GetTalentValue("modifier_nevermore_darklord_1", "armor")
	...
	if not self.caster:HasTalent("modifier_nevermore_darklord_4") or self.self_k == -1 then
		return
	end
...
function modifier_custom_dark_lord_debuff:GetBonus(bonus)
	...
	if self.caster:HasModifier("modifier_custom_dark_lord_legendary") then
		result = result
			* (1 + self.caster:GetUpgradeStack("modifier_custom_dark_lord_legendary") * self.legendary_bonus)
	end
...
function modifier_custom_dark_lord_debuff:GetModifierLifestealRegenAmplify_Percentage()
	if not self.caster:HasTalent("modifier_nevermore_darklord_3") then
		return
	end
	if self.parent:GetHealthPercent() > self.healing_health then
		return
	end
...
function modifier_custom_dark_lord_debuff:GetModifierMagicalResistanceBonus()
	if not self.caster:HasTalent("modifier_nevermore_darklord_1") then
		return
	end
...
function modifier_custom_dark_lord_debuff:GetModifierMoveSpeedBonus_Percentage()
	if not self.caster:HasTalent("modifier_nevermore_darklord_2") then
		return
	end
...
function modifier_custom_dark_lord_legendary:IsHidden()
	return true
end
function modifier_custom_dark_lord_legendary:IsPurgable()
	return false
end
function modifier_custom_dark_lord_legendary:GetTexture()
	return "buffs/darklord_legen"
end
...
function modifier_custom_dark_lord_legendary_fear:IsHidden()
	return true
end
function modifier_custom_dark_lord_legendary_fear:IsPurgable()
	return false
end
...
function modifier_custom_dark_lord_damage:IsHidden()
	return false
end
function modifier_custom_dark_lord_damage:IsPurgable()
	return false
end
...
function modifier_custom_dark_lord_speed:IsHidden()
	return false
end
function modifier_custom_dark_lord_speed:IsPurgable()
	return false
end
	...
	for _, target in pairs(self.parent:FindTargets(self.radius)) do
		if not self.targets[target] then
			...
			target:AddNewModifier(
				...
				self.ability,
				"modifier_custom_dark_lord_slow",
				{ duration = (1 - target:GetStatusResistance()) * self.duration }
			)
		end
	end
end
...
function modifier_custom_dark_lord_slow:IsHidden()
	return true
end
...
function modifier_custom_dark_lord_fear_cd:IsHidden()
	return true
end
function modifier_custom_dark_lord_fear_cd:IsPurgable()
	return false
end
