...
____exports.boss_beast_3 = __TS__Class()
local boss_beast_3 = ____exports.boss_beast_3
boss_beast_3.name = "boss_beast_3"
__TS__ClassExtends(boss_beast_3, MonsterAbility_CS)
function boss_beast_3.prototype.GetMosnterAbilityConfig(self)
	return {
		castPoint = 0.5,
		castDuration = 5,
		...
		OnStart = function()
			local caster = self:GetCaster()
			____exports.modifier_boss_beast_3_thinker:applys(caster, caster, self, { duration = 5 })
			caster:AddNewModifier(caster, self, "slow_gongsu_modfier", { duration = 5 })
		end,
	}
end
function boss_beast_3.prototype.OnProjectileHit_ExtraData(self, target, location, extraData)
	if target then
		local caster = self:GetCaster()
...
boss_beast_3 = __TS__DecorateLegacy({ registerAbility(nil) }, boss_beast_3)
____exports.boss_beast_3 = boss_beast_3
____exports.modifier_boss_beast_3_thinker = __TS__Class()
local modifier_boss_beast_3_thinker = ____exports.modifier_boss_beast_3_thinker
modifier_boss_beast_3_thinker.name = "modifier_boss_beast_3_thinker"
__TS__ClassExtends(modifier_boss_beast_3_thinker, BaseModifier_CS)
function modifier_boss_beast_3_thinker.prototype.OnCreated(self, params)
...
function modifier_boss_beast_3_thinker.prototype.OnIntervalThink(self)
	self:IncrementStackCount()
	self:StartPro(self:GetStackCount())
end
function modifier_boss_beast_3_thinker.prototype.GetEffectName(self)
	return "particles/mirana_3_light_red.vpcf"
end
function modifier_boss_beast_3_thinker.prototype.OnDestroy(self)
	if not IsServer() then
		return
	end
	self:GetParent():SetRenderColor(255, 255, 255)
end
function modifier_boss_beast_3_thinker.prototype.GetAttributeBonus(self)
	return { bonus_movespeed_pct = -60 }
end
function modifier_boss_beast_3_thinker.prototype.StartPro(self, n)
	local caster = self:GetCaster()
...
function modifier_boss_beast_3_thinker.prototype.PlayEffects(self)
	if not IsServer() then
		return
	end
	local caster = self:GetCaster()
...
modifier_boss_beast_3_thinker = __TS__DecorateLegacy({ registerModifier(nil) }, modifier_boss_beast_3_thinker)
____exports.modifier_boss_beast_3_thinker = modifier_boss_beast_3_thinker
return ____exports