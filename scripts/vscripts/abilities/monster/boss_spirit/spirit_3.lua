...
local function CircleMover(self, center, startPosition, time, callback, options)
	...
	Timers:CreateTimer(interval, function()
		local angleDeg = elapsed * angularSpeed
...
local spirit_3 = __TS__Class()
spirit_3.name = "spirit_3"
__TS__ClassExtends(spirit_3, MonsterAbility_CS)
function spirit_3.prototype.____constructor(self, ...)
	MonsterAbility_CS.prototype.____constructor(self, ...)
	self["end"] = false
	self.target = nil
end
function spirit_3.prototype.GetMosnterAbilityConfig(self)
	return {
		castPoint = 1,
		castDuration = 2.2,
...
function spirit_3.prototype.SpellStart(self)
	local caster = self:GetCaster()
	...
	local pfx = ParticleManager:CreateParticle("particles/boss/hoodwink_boomerang_2.vpcf", PATTACH_WORLDORIGIN, nil)
...
function spirit_3.prototype.ShootProjectiles(self)
	local caster = self:GetCaster()
	...
	for ____, dir in ipairs(arr) do
		local projectile =
			ParticleManager:CreateParticle("particles/boss/hoodwink_boomerang_2.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		Timers:CreateTimer(0.05, function()
			...
			self:DamageArea(origin_pos:__add(dir:__mul(1600 / 20)), 200, 10)
			n = n + 1
			if n > 20 then
				ParticleManager:DestroyParticle(projectile, false)
				ParticleManager:ReleaseParticleIndex(projectile)
...
function spirit_3.prototype.DamageArea(self, origin, radius, damage)
	local caster = self:GetCaster()
...
spirit_3 = __TS__DecorateLegacy({ registerAbility(nil) }, spirit_3)
