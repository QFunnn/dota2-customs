...
local CAST_POINT = 1
local CAST_COOLDOWN = 3
local BARRAGE_ROUND_INTERVAL = 0.8
local BARRAGE_ROUND_PROJECTILE_COUNTS = { 1, 3, 5 }
...
____exports.elite_327 = __TS__Class()
local elite_327 = ____exports.elite_327
elite_327.name = "elite_327"
__TS__ClassExtends(elite_327, MonsterAbility_CS)
function elite_327.prototype.____constructor(self, ...)
	MonsterAbility_CS.prototype.____constructor(self, ...)
	self.castToken = 0
end
function elite_327.prototype.Precache(self, context)
...
function elite_327.prototype.GetMosnterAbilityConfig(self)
	return {
...
function elite_327.prototype.PrepareBarrage(self)
...
function elite_327.prototype.StartBarrage(self)
	local caster = self:GetCaster()
	if not IsValidAlive(nil, caster) then
		return
	end
...
function elite_327.prototype.StopBarrage(self)
	self.castToken = self.castToken + 1
end
function elite_327.prototype.PlayBarrageGesture(self, caster)
	local target = self:FindTarget()
	if IsValidAlive(nil, target) then
...
function elite_327.prototype.FireBarrageRound(self, caster, projectileCount)
...
function elite_327.prototype.FindTarget(self)
	local caster = self:GetCaster()
	if not IsValidAlive(nil, caster) then
		return nil
	end
...
function elite_327.prototype.PlayPunchCastEffect(self, origin, forward, caster)
...
function elite_327.prototype.PlayPunchScreenShake(self, point)
	ScreenShake(
		point,
...
function elite_327.prototype.FlatDirection(self, direction)
...
elite_327 = __TS__DecorateLegacy({ registerAbility(nil) }, elite_327)
____exports.elite_327 = elite_327
return ____exports