...
--- 精英技能122 - 短暂蓄力后释放扩散冲击波，击退波及到的周围单位，随后随机释放一个其他可用技能。
____exports.elite_122 = __TS__Class()
local elite_122 = ____exports.elite_122
elite_122.name = "elite_122"
__TS__ClassExtends(elite_122, MonsterAbility_CS)
function elite_122.prototype.Precache(self, context)
	PrecacheResource("particle", PARTICLE, context)
end
function elite_122.prototype.GetMosnterAbilityConfig(self)
	return {
...
function elite_122.prototype.PlayPushEffect(self, caster)
...
function elite_122.prototype.StartExpandingPush(self, caster)
	local origin = caster:GetAbsOrigin()
	local hitRecord = {}
...
function elite_122.prototype.GetPushRadiusByElapsed(self, elapsed)
	local progress = math.min(math.max(elapsed / PUSH_EXPAND_DURATION, 0), 1)
	return PUSH_RADIUS * progress
end
function elite_122.prototype.PushEnemiesOnWave(self, caster, origin, previousRadius, currentRadius, hitRecord)
...
function elite_122.prototype.CastRandomFollowUp(self)
	local caster = self:GetCaster()
	if not IsValidAlive(nil, caster) then
		return
	end
...
function elite_122.prototype.GetReadyFollowUpAbilities(self, caster, target)
	local abilities = {}
...
function elite_122.prototype.CanUseAsFollowUp(self, caster, ability, target)
...
function elite_122.prototype.IsTargetInCastRange(self, caster, ability, target, behavior)
	if
...
function elite_122.prototype.CastFollowUpAbility(self, caster, ability, target)
	local behavior = ability:GetBehavior()
...
elite_122 = __TS__DecorateLegacy({ registerAbility(nil) }, elite_122)
____exports.elite_122 = elite_122
return ____exports