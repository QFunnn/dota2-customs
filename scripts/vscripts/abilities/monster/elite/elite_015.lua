...
local SPIKE_RADIUS = 1300
local SPIKE_COUNT = 18
local SPIKE_DAMAGE_RATE = 15
local SPIKE_AOE_RADIUS = 160
local SPIKE_INTERVAL = 0.8
local GLACIER_SPIKE_PARTICLE = "particles/monster/invoker_glacierwall_ground_spikes.vpcf"
--- 精英技能15 - 冰刺节奏更快，伤害略高
____exports.elite_015 = __TS__Class()
local elite_015 = ____exports.elite_015
elite_015.name = "elite_015"
__TS__ClassExtends(elite_015, MonsterAbility_CS)
function elite_015.prototype.Precache(self, context)
	PrecacheResource("particle", GLACIER_SPIKE_PARTICLE, context)
end
function elite_015.prototype.GetMosnterAbilityConfig(self)
	return {
		...
		OnStart = function()
			...
			caster:AddNewModifier(caster, self, "modifier_elite_015_freeze", { duration = FREEZE_DURATION })
		end,
	}
end
elite_015 = __TS__DecorateLegacy({ registerAbility(nil) }, elite_015)
____exports.elite_015 = elite_015
local modifier_elite_015_freeze = __TS__Class()
modifier_elite_015_freeze.name = "modifier_elite_015_freeze"
__TS__ClassExtends(modifier_elite_015_freeze, MonsterModifier_CS)
function modifier_elite_015_freeze.prototype.____constructor(self, ...)
	MonsterModifier_CS.prototype.____constructor(self, ...)
	self.elapsed = 0
end
function modifier_elite_015_freeze.prototype.OnCreated(self)
...
function modifier_elite_015_freeze.prototype.OnIntervalThink(self)
	if not IsServer() then
		return
	end
...
function modifier_elite_015_freeze.prototype.CheckState(self)
	return { [MODIFIER_STATE_FROZEN] = true }
end
function modifier_elite_015_freeze.prototype.GetEffectName(self)
	return "particles/events/crownfall/shmup/shmup_colde_embracebuff.vpcf"
end
function modifier_elite_015_freeze.prototype.GetEffectAttachType(self)
	return PATTACH_ABSORIGIN_FOLLOW
end
function modifier_elite_015_freeze.prototype.SetupSpikeCycles(self)
	local parent = self:GetParent()
	if not IsValidAlive(nil, parent) then
		return
	end
...
function modifier_elite_015_freeze.prototype.ScheduleSpikeCycle(self, origin, offset)
	local caster = self:GetCaster()
	local parent = self:GetParent()
...
function modifier_elite_015_freeze.prototype.PlayEffect(self, pos)
	local parent = self:GetParent()
	if not IsValidAlive(nil, parent) then
		return
	end
	...
	self:Timer(1, function()
		...
		ParticleManager:SetParticleControl(effectId, 0, pos + Vector(0, 0, 3200))
		ParticleManager:SetParticleControl(effectId, 1, pos)
		ParticleManager:SetParticleControl(effectId, 2, Vector(3200, 0, 0))
		self:Timer(1, function()
...
modifier_elite_015_freeze =
	__TS__DecorateLegacy({ registerModifier(nil, "modifier_elite_015_freeze") }, modifier_elite_015_freeze)
return ____exports