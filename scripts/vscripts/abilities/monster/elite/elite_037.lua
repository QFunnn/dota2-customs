...
local AOE_RADIUS = 325
local DAMAGE_RATE = 20
local SLOW_PCT = 30
local SLOW_DURATION = 2.5
local THUNDERCLAP_PARTICLE = "particles/neutral_fx/ursa_thunderclap.vpcf"
...
____exports.elite_037 = __TS__Class()
local elite_037 = ____exports.elite_037
elite_037.name = "elite_037"
__TS__ClassExtends(elite_037, MonsterAbility_CS)
function elite_037.prototype.Precache(self, context)
	PrecacheResource("particle", THUNDERCLAP_PARTICLE, context)
	PrecacheResource("particle", SLOW_DEBUFF_PARTICLE, context)
end
function elite_037.prototype.GetMosnterAbilityConfig(self)
	return {
		castRange = 400,
		...
		OnPhaseStart = function()
			...
			EmitSoundOnLocationWithCaster(origin, "Hero_Invoker.DeafeningBlast", caster)
		end,
		OnStart = function()
			local caster = self:GetCaster()
			if not IsValidAlive(nil, caster) then
			...
			local origin = caster:GetAbsOrigin() + caster:GetForwardVector():__mul(120)
			local cp0 = origin
			...
			ScreenShake(origin, 20, 20, 0.3, 2000, 0, true)
			...
			for ____, enemy in ipairs(enemies) do
				do
					...
					AddDeBuffStatus(
						...
						{ stack = 10, duration = SLOW_DURATION, effect_name = SLOW_DEBUFF_PARTICLE }
					)
				end
				::__continue8::
			end
...
elite_037 = __TS__DecorateLegacy({ registerAbility(nil) }, elite_037)
____exports.elite_037 = elite_037
return ____exports