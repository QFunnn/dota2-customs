...
local START_KNOCKBACK_RADIUS = 600
local START_KNOCKBACK_DISTANCE = 720
...
____exports.elite_127 = __TS__Class()
local elite_127 = ____exports.elite_127
elite_127.name = "elite_127"
__TS__ClassExtends(elite_127, MonsterAbility_CS)
function elite_127.prototype.Precache(self, context)
...
function elite_127.prototype.GetMosnterAbilityConfig(self)
	return {
...
function elite_127.prototype.FindLaserTarget(self, caster)
	local origin = caster:GetAbsOrigin()
...
function elite_127.prototype.PlayStartKnockbackEffect(self, origin)
...
function elite_127.prototype.KnockBackAround(self, caster, origin)
	local enemies = FindUnitsInRadius(
		caster:GetTeamNumber(),
		origin,
		nil,
...
function elite_127.prototype.PlayCastChargedEffect(self, caster)
	local origin = self:GetAttachmentPosition(caster, "attach_attack1")
	local forward = caster:GetForwardVector()
	local pfx = ParticleManager:CreateParticle(CAST_CHARGED_PARTICLE, PATTACH_POINT_FOLLOW, caster)
	ParticleManager:SetParticleControlEnt(pfx, 40, caster, PATTACH_POINT_FOLLOW, "attach_attack1", origin, true)
...
function elite_127.prototype.GetAttachmentPosition(self, caster, attachmentName)
...
function elite_127.prototype.PlayLaserEffect(self, origin, target)
	local ____target_0
	if target then
...
elite_127 = __TS__DecorateLegacy({ registerAbility(nil) }, elite_127)
____exports.elite_127 = elite_127
return ____exports