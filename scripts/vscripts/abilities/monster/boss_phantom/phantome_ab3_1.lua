...
local phantome_ab3_1 = __TS__Class()
phantome_ab3_1.name = "phantome_ab3_1"
__TS__ClassExtends(phantome_ab3_1, MonsterAbility_CS)
function phantome_ab3_1.prototype.GetMosnterAbilityConfig(self)
	return {
		castPoint = 1,
		castDuration = 1.6,
...
function phantome_ab3_1.prototype.Start(self)
	local unit = self:GetCaster()
	unit:AddNewModifier(unit, self, "phantome_ab3_1_pre", { duration = 0.45 })
	self:GetCaster():SetCustomValue("绝影斩击", 2)
end
phantome_ab3_1 = __TS__DecorateLegacy({ registerAbility(nil) }, phantome_ab3_1)
local phantome_ab3_1_pre = __TS__Class()
phantome_ab3_1_pre.name = "phantome_ab3_1_pre"
__TS__ClassExtends(phantome_ab3_1_pre, BaseModifier)
function phantome_ab3_1_pre.prototype.____constructor(self, ...)
	BaseModifier.prototype.____constructor(self, ...)
	self.max = 1200
	self.min = 800
end
function phantome_ab3_1_pre.prototype.OnCreated(self, params)
...
function phantome_ab3_1_pre.prototype.OnIntervalThink(self)
	local caster = self:GetCaster()
...
function phantome_ab3_1_pre.prototype.GetEffectName(self)
	return "particles/econ/items/underlord/underlord_2021_immortal/underlord_2021_immortal_portal_buildup.vpcf"
end
function phantome_ab3_1_pre.prototype.GetStatusEffectName(self)
	return "particles/status_fx/status_effect_wraithking_ghosts.vpcf"
end
function phantome_ab3_1_pre.prototype.OnDestroy(self)
	if not IsServer() then
		return
	end
	local unit = self:GetCaster()
	...
	unit:AddNewModifier(unit, ability, "phantome_ab3_1_slash", { duration = 0.65 })
end
phantome_ab3_1_pre = __TS__DecorateLegacy({ registerModifier(nil) }, phantome_ab3_1_pre)
local phantome_ab3_1_slash = __TS__Class()
phantome_ab3_1_slash.name = "phantome_ab3_1_slash"
__TS__ClassExtends(phantome_ab3_1_slash, BaseModifier)
function phantome_ab3_1_slash.prototype.OnCreated(self, params)
	if not IsServer() then
		return
	end
	local caster = self:GetCaster()
...
function phantome_ab3_1_slash.prototype.OnIntervalThink(self)
	local unit = self:GetCaster()
	if not IsValidAlive(nil, unit) then
		return
	end
...
function phantome_ab3_1_slash.prototype.OnDestroy(self)
	if not IsServer() then
		return
	end
	local unit = self:GetCaster()
	...
	if slashCount < 3 then
		unit:AddNewModifier(unit, self:GetAbility(), "phantome_ab3_1_pre2", { duration = 0.16 })
...
phantome_ab3_1_slash = __TS__DecorateLegacy({ registerModifier(nil) }, phantome_ab3_1_slash)
local phantome_ab3_1_pre2 = __TS__Class()
phantome_ab3_1_pre2.name = "phantome_ab3_1_pre2"
__TS__ClassExtends(phantome_ab3_1_pre2, BaseModifier)
function phantome_ab3_1_pre2.prototype.OnDestroy(self)
	if not IsServer() then
		return
	end
	local unit = self:GetCaster()
	unit:AddNewModifier(unit, self:GetAbility(), "phantome_ab3_1_pre", { duration = 0.45 })
end
phantome_ab3_1_pre2 = __TS__DecorateLegacy({ registerModifier(nil) }, phantome_ab3_1_pre2)
return ____exports