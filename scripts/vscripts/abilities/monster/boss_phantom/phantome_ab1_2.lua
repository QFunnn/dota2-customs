...
local phantome_ab1_2 = __TS__Class()
phantome_ab1_2.name = "phantome_ab1_2"
__TS__ClassExtends(phantome_ab1_2, MonsterAbility_CS)
function phantome_ab1_2.prototype.GetMosnterAbilityConfig(self)
	return {
		castPoint = PHANTOME_AB1_CHARGE_DURATION,
		counterBreakWindowDuration = PHANTOME_AB1_CHARGE_DURATION,
		castDuration = 6.2,
		...
		OnStart = function()
			local caster = self:GetCaster()
			caster:AddNewModifier(caster, self, "phantome_ab1_2_modifier_start", { duration = 4.5 })
...
function phantome_ab1_2.prototype.StartPrePhaseMove(self, done)
...
function phantome_ab1_2.prototype.GetRepositionMoveDuration(self, caster, distance)
	local speed = math.max(caster:GetIdealSpeed(), PHANTOME_AB1_REPOSITION_MIN_SPEED)
	return math.max(distance / speed, FrameTime())
end
function phantome_ab1_2.prototype.ResolveRepositionPoint(self, caster, target)
...
function phantome_ab1_2.prototype.ResolveRepositionPointByMoveDistanceRange(self, caster, point)
...
function phantome_ab1_2.prototype.ResolveAwayFromCenterDirection(self, caster, targetPoint, centerPoint)
...
function phantome_ab1_2.prototype.IsValidRepositionPoint(self, caster, targetPoint, point)
...
function phantome_ab1_2.prototype.IsMoveDistanceInRange(self, caster, point)
...
function phantome_ab1_2.prototype.IsValidMoveTarget(self, caster, point)
...
phantome_ab1_2 = __TS__DecorateLegacy({ registerAbility(nil) }, phantome_ab1_2)
local phantome_ab1_2_modifier_pre = __TS__Class()
phantome_ab1_2_modifier_pre.name = "phantome_ab1_2_modifier_pre"
__TS__ClassExtends(phantome_ab1_2_modifier_pre, BaseModifier_CS)
function phantome_ab1_2_modifier_pre.prototype.____constructor(self, ...)
	BaseModifier_CS.prototype.____constructor(self, ...)
	self.num = 0
	self.chargeStarted = false
end
function phantome_ab1_2_modifier_pre.prototype.OnCreated(self)
	if not IsServer() then
		return
	end
	local caster = self:GetCaster()
...
function phantome_ab1_2_modifier_pre.prototype.StartChargeWarning(self, caster, lockedTarget)
...
function phantome_ab1_2_modifier_pre.prototype.OnIntervalThink(self)
...
function phantome_ab1_2_modifier_pre.prototype.OnDestroy(self)
	if not IsServer() then
		return
	end
	local caster = self:GetCaster()
...
phantome_ab1_2_modifier_pre = __TS__DecorateLegacy({ registerModifier(nil) }, phantome_ab1_2_modifier_pre)
local phantome_ab1_2_modifier_start = __TS__Class()
phantome_ab1_2_modifier_start.name = "phantome_ab1_2_modifier_start"
__TS__ClassExtends(phantome_ab1_2_modifier_start, BaseModifier_CS)
function phantome_ab1_2_modifier_start.prototype.____constructor(self, ...)
	BaseModifier_CS.prototype.____constructor(self, ...)
	self.num = 0
end
function phantome_ab1_2_modifier_start.prototype.createDaggerProjectiles(self, dirs, getPosition, effectName)
	local caster = self:GetCaster()
	...
	__TS__ArrayForEach(dirs, function(____, dir)
		CreateProjectile(nil, {
			...
			start_point = caster:GetAbsOrigin():__add(Vector(0, 0, 100)):__add(dir:__mul(100)),
...
function phantome_ab1_2_modifier_start.prototype.OnCreated(self, params)
	local caster = self:GetCaster()
	if not IsServer() then
		return
	end
...
phantome_ab1_2_modifier_start = __TS__DecorateLegacy({ registerModifier(nil) }, phantome_ab1_2_modifier_start)
return ____exports