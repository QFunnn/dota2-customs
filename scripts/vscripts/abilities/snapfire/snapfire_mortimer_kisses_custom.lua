...
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_thinker",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_damage_reduce",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_debuff",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_tracker",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_napalm",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_legendary",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_legendary_count",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_stun_check",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_scatter",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_heal_reduce",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_immune",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_blink",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_snapfire_mortimer_kisses_custom_root_count",
	"abilities/snapfire/snapfire_mortimer_kisses_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function snapfire_mortimer_kisses_custom:MaxShots()
	return self:GetSpecialValueFor("projectile_count")
		+ self:GetCaster():GetTalentValue("modifier_snapfire_kisses_2", "max")
end
...
function snapfire_mortimer_kisses_custom:OnSpellStart()
	...
	if caster:HasTalent("modifier_snapfire_kisses_5") then
		...
		caster:AddNewModifier(
			...
			{ effect = 1, duration = caster:GetTalentValue("modifier_snapfire_kisses_5", "bkb") }
		)
		caster:AddNewModifier(
			caster,
			self,
			"modifier_snapfire_mortimer_kisses_custom_immune",
			{ duration = caster:GetTalentValue("modifier_snapfire_kisses_5", "bkb") }
		)
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_snapfire_mortimer_kisses_custom",
		{ duration = duration, pos_x = point.x, pos_y = point.y }
	)
end
...
function snapfire_mortimer_kisses_custom:LauchBall(location, damage_ability, new_caster)
	...
	local source = caster
	local source_attach = caster:GetAttachmentOrigin(caster:ScriptLookupAttachment("attach_mount_head"))
		+ Vector(0, 0, 50)
	...
	if ability == "modifier_snapfire_kisses_7" and new_caster then
		...
		source = new_caster
		source_attach = new_caster:GetAttachmentOrigin(new_caster:ScriptLookupAttachment("attach_mount_head"))
			+ Vector(0, 0, 100)
	end
...
function snapfire_mortimer_kisses_custom:OnProjectileHit_ExtraData(target, location, extraData)
	...
	local duration = self:GetSpecialValueFor("burn_ground_duration")
		+ caster:GetTalentValue("modifier_snapfire_kisses_4", "fire_duration")
	...
	local damageTable =
		{ attacker = caster, damage = damage, damage_type = self:GetAbilityDamageType(), ability = self }
	for _, enemy in pairs(caster:FindTargets(impact_radius, location)) do
		...
		if
			caster:HasTalent("modifier_snapfire_kisses_6")
			and not damage_ability
			and root_mod
			and not root_mod.targets[enemy:entindex()]
		then
			...
			enemy:AddNewModifier(
				...
				self,
				"modifier_snapfire_mortimer_kisses_custom_root",
				{ duration = (1 - enemy:GetStatusResistance()) * root_duration }
			)
		end
...
function snapfire_mortimer_kisses_custom:NapalmStack(point)
	...
	local napalm_impact_particle = ParticleManager:CreateParticle(
...
function modifier_snapfire_mortimer_kisses_custom:OnCreated(kv)
	...
	self.radius_visual = ParticleManager:CreateParticleForPlayer(
		"particles/snapfire/kisses_legendary_radius.vpcf",
	...
	if self.parent:HasTalent("modifier_snapfire_kisses_1") then
		self.reduce_mod = self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_snapfire_mortimer_kisses_custom_damage_reduce",
			{ duration = self:GetRemainingTime() + 1 }
		)
	end
...
function modifier_snapfire_mortimer_kisses_custom:OrderEvent(params)
	...
	elseif
		...
		or (params.order_type == DOTA_UNIT_ORDER_CAST_NO_TARGET and params.ability ~= self.ability)
	then
		self:Destroy()
	end
end
...
function modifier_snapfire_mortimer_kisses_custom:OnIntervalThink()
	...
	if self.count > self.max_real then
		return
	end
...
function modifier_snapfire_mortimer_kisses_custom_debuff:OnCreated(kv)
	...
	self.slow = -self.ability:GetSpecialValueFor("move_slow_pct")
		+ self.caster:GetTalentValue("modifier_snapfire_kisses_3", "slow")
...
function modifier_snapfire_mortimer_kisses_custom_thinker:OnCreated(kv)
	...
	ParticleManager:SetParticleControl(
		self.effect_cast,
		1,
		Vector(self.radius, 0, -self.radius * (self.max_travel / kv.travel_time))
	)
...
function modifier_snapfire_mortimer_kisses_custom_tracker:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_tracker:IsPurgable()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_tracker:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_snapfire_kisses_3") then
		return
	end
...
function modifier_snapfire_mortimer_kisses_custom_tracker:AttackStartEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_snapfire_kisses_4") then
		return
	end
...
function modifier_snapfire_mortimer_kisses_custom_napalm:IsHidden()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_napalm:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_napalm:GetTexture()
...
function modifier_snapfire_mortimer_kisses_custom_napalm:OnCreated()
	...
	self.effect_cast =
		self.parent:GenericParticle("particles/units/heroes/hero_batrider/batrider_stickynapalm_stack.vpcf", self, true)
...
function modifier_snapfire_mortimer_kisses_custom_stun_check:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_stun_check:IsPurgable()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_stun_check:OnIntervalThink()
	...
	if self.parent:HasModifier("modifier_snapfire_mortimer_kisses_custom_immune") then
		return
	end
	...
	if
		...
		or self.parent:GetForceAttackTarget() ~= nil
	then
...
function modifier_snapfire_mortimer_kisses_custom_scatter:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_scatter:IsPurgable()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_heal_reduce:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_heal_reduce:IsPurgable()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_immune:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_immune:IsPurgable()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_blink:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_blink:IsPurgable()
	return false
end

modifier_snapfire_mortimer_kisses_custom_cdr = class({})
function modifier_snapfire_mortimer_kisses_custom_cdr:IsHidden()
	return not self:GetCaster():HasTalent("modifier_snapfire_kisses_6")
end
function modifier_snapfire_mortimer_kisses_custom_cdr:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_cdr:RemoveOnDeath()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_cdr:GetTexture()
...
function modifier_snapfire_mortimer_kisses_custom_cdr:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_snapfire_kisses_6") then
		return
	end
...
function modifier_snapfire_mortimer_kisses_custom_root:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_root:IsPurgable()
	return true
end
...
function modifier_snapfire_mortimer_kisses_custom_root_count:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_root_count:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_root_count:RemoveOnDeath()
	return false
end
...
function modifier_snapfire_mortimer_kisses_custom_legendary:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_legendary:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_legendary:OnCreated(table)
	...
	self.change_health = self.caster:GetMaxHealth()
		* self.caster:GetTalentValue("modifier_snapfire_kisses_7", "health")
		/ 100
...
function modifier_snapfire_mortimer_kisses_custom_damage_reduce:IsHidden()
	return true
end
function modifier_snapfire_mortimer_kisses_custom_damage_reduce:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_damage_reduce:GetEffectName()
...
function modifier_snapfire_mortimer_kisses_custom_legendary_count:IsHidden()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_legendary_count:IsPurgable()
	return false
end
function modifier_snapfire_mortimer_kisses_custom_legendary_count:RemoveOnDeath()
	return false
end
