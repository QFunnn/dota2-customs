...
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_speed",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_tracker",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_cookie",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_bomb_cookie",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_bomb_slow",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_resist",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_legendary_cd",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_legendary_tracker",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_legendary_speed",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_unslow",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_shield",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_snapfire_firesnap_cookie_custom_shield_cd",
	"abilities/snapfire/snapfire_firesnap_cookie_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function snapfire_firesnap_cookie_custom:GetRange()
	return self:GetSpecialValueFor("jump_horizontal_distance")
		+ self:GetCaster():GetTalentValue("modifier_snapfire_cookie_2", "range")
end
...
function snapfire_firesnap_cookie_custom_2:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		ability,
		"modifier_snapfire_firesnap_cookie_custom",
		{ duration = self:GetSpecialValueFor("jump_duration"), x = point.x, y = point.y }
	)
end
...
function modifier_snapfire_firesnap_cookie_custom:OnCreated(table)
	...
	self.impact_stun_duration = self.ability:GetSpecialValueFor("impact_stun_duration")
		+ self.parent:GetTalentValue("modifier_snapfire_cookie_3", "stun")
	self.impact_damage = self.ability:GetSpecialValueFor("impact_damage")
		+ self.parent:GetTalentValue("modifier_snapfire_cookie_1", "damage")
	self.impact_radius = self.ability:GetSpecialValueFor("impact_radius")

	if self.parent:HasTalent("modifier_snapfire_cookie_6") then
		self.impact_stun_duration = self.impact_stun_duration
			* (1 + self.parent:GetTalentValue("modifier_snapfire_cookie_6", "stun") / 100)
		self.impact_damage = self.impact_damage
			* (1 + self.parent:GetTalentValue("modifier_snapfire_cookie_6", "damage") / 100)
	end
	...
	if self.parent:HasShard() then
		...
		self.parent:GenericHeal(
			self.parent:GetMaxHealth() * self.ability:GetSpecialValueFor("shard_heal") / 100,
			self.ability,
			nil,
			nil,
			"shard"
...
function modifier_snapfire_firesnap_cookie_custom:OnDestroy()
	...
	if self.parent:HasTalent("modifier_snapfire_cookie_5") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_snapfire_firesnap_cookie_custom_unslow",
			{ duration = self.parent:GetTalentValue("modifier_snapfire_cookie_5", "unslow") }
		)
		if not self.parent:HasModifier("modifier_snapfire_firesnap_cookie_custom_shield_cd") then
			self.parent:AddNewModifier(
				...
				self.ability,
				"modifier_snapfire_firesnap_cookie_custom_shield_cd",
				{ duration = self.parent:GetTalentValue("modifier_snapfire_cookie_5", "cd") }
			)
			self.parent:AddNewModifier(
				self.parent,
				self.ability,
				"modifier_snapfire_firesnap_cookie_custom_shield",
				{ duration = self.parent:GetTalentValue("modifier_snapfire_cookie_5", "duration") }
			)
		end
	end
	...
	if self.parent:HasTalent("modifier_snapfire_cookie_2") then
		...
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_snapfire_firesnap_cookie_custom_speed",
			{ duration = self.parent:GetTalentValue("modifier_snapfire_cookie_2", "duration") }
		)
	end
	...
	for _, enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = (1 - enemy:GetStatusResistance()) * self.impact_stun_duration }
		)
	end

	GridNav:DestroyTreesAroundPoint(self.parent:GetOrigin(), self.impact_radius, true)

	local land_particle = ParticleManager:CreateParticle(
...
function modifier_snapfire_firesnap_cookie_custom_tracker:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_tracker:IsPurgable()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_tracker:GetModifierMoveSpeedBonus_Constant()
	if not self.parent:HasTalent("modifier_snapfire_cookie_2") then
		return
	end
...
function modifier_snapfire_firesnap_cookie_custom_tracker:UpdateJs()
	...
	if not self.parent:HasTalent("modifier_snapfire_cookie_7") then
		return
	end
...
function modifier_snapfire_firesnap_cookie_custom_tracker:AttackEvent_out(params)
	...
	if
		self.parent:HasTalent("modifier_snapfire_cookie_7")
		and not self.parent:HasModifier("modifier_snapfire_firesnap_cookie_custom_legendary_cd")
	then
	...
	if
		self.parent:HasTalent("modifier_snapfire_cookie_4")
		and RollPseudoRandomPercentage(self.bomb_chance, 1886, self.parent)
	then
		local vec = target:GetAbsOrigin()
			+ self.bomb_distance * (self.parent:GetAbsOrigin() - target:GetAbsOrigin()):Normalized()
...
function modifier_snapfire_firesnap_cookie_custom_tracker:ProcLegendary(target)
	...
	cookie:AddNewModifier(
		...
		self.ability,
		"modifier_snapfire_firesnap_cookie_custom_cookie",
		{ duration = 12, is_hero = is_hero, interval = distance / speed }
	)
end

modifier_snapfire_firesnap_cookie_custom_legendary_tracker = class({})
function modifier_snapfire_firesnap_cookie_custom_legendary_tracker:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_legendary_tracker:IsPurgable()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_legendary_tracker:RemoveOnDeath()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_cookie:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_cookie:IsPurgable()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_cookie:OnIntervalThink()
	...
	if
		(self.parent:GetAbsOrigin() - self.caster:GetAbsOrigin()):Length2D() <= self.radius
...
function modifier_snapfire_firesnap_cookie_custom_legendary_speed:IsHidden()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_legendary_speed:IsPurgable()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_legendary_speed:RemoveOnDeath()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_bomb_cookie:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_bomb_cookie:IsPurgable()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_bomb_cookie:OnIntervalThink()
	...
	self.parent:SetRenderColor(
		255,
		255 * (self.bomb_timer - self:GetElapsedTime() / self.bomb_timer),
		255 * (self.bomb_timer - self:GetElapsedTime() / self.bomb_timer)
	)
...
function modifier_snapfire_firesnap_cookie_custom_bomb_cookie:OnDestroy()
	...
	for _, enemy in pairs(self.caster:FindTargets(self.aoe, self.parent:GetAbsOrigin())) do
		...
		enemy:AddNewModifier(
			...
			self.ability,
			"modifier_snapfire_firesnap_cookie_custom_bomb_slow",
			{ duration = (1 - enemy:GetStatusResistance()) * self.duration }
		)
	end
...
function modifier_snapfire_firesnap_cookie_custom_bomb_slow:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_bomb_slow:IsPurgable()
	return true
end
...
function modifier_snapfire_firesnap_cookie_custom_resist:IsHidden()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_resist:IsPurgable()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_resist:GetTexture()
	return "buffs/cookie_damage"
end
...
function modifier_snapfire_firesnap_cookie_custom_legendary_cd:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_legendary_cd:IsPurgable()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_unslow:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_unslow:IsPurgable()
	return false
end
...
function modifier_snapfire_firesnap_cookie_custom_shield_cd:IsHidden()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_shield_cd:IsPurgable()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_shield_cd:RemoveOnDeath()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_shield_cd:IsDebuff()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_shield_cd:GetTexture()
	return "buffs/berserker_armor"
end
...
function modifier_snapfire_firesnap_cookie_custom_shield:IsHidden()
	return true
end
function modifier_snapfire_firesnap_cookie_custom_shield:IsPurgable()
	return false
end
function modifier_snapfire_firesnap_cookie_custom_shield:OnCreated(table)
	self.parent = self:GetParent()
	self.shield_talent = "modifier_snapfire_cookie_5"
	self.max_shield = self.parent:GetTalentValue("modifier_snapfire_cookie_5", "shield")
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/snapfire/cookie_shield.vpcf",
