...
function npc_treant_passive:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_treant_cd", { duration = self:GetCooldownTimeRemaining() })
end

modifier_treant_cd = class({})

function modifier_treant_cd:IsHidden()
	return false
end
function modifier_treant_cd:IsPurgable()
	return false
end