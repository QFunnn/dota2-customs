...
function alchemist_acid_spray_custom:DoDamage(target)
	...
	if caster:HasTalent("modifier_alchemist_spray_1") then
		...
		bonus = target:IsCreep() and math.min(bonus, caster:GetTalentValue("modifier_alchemist_spray_1", "creeps_max"))
			or bonus
		damage = damage + bonus
	end
	...
	if
		caster:HasTalent("modifier_alchemist_spray_4")
	...
	then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_alchemist_acid_spray_custom_root_cd",
			{ duration = caster:GetTalentValue("modifier_alchemist_spray_4", "cd") }
		)
		target:AddNewModifier(
			caster,
			self,
			"modifier_alchemist_acid_spray_custom_root",
			{ duration = caster:GetTalentValue("modifier_alchemist_spray_4", "root")
...
function acid_spray_aura_class:IsPurgable()
	return false
end
...
function alchemist_acid_spray_mixing:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_alchemist_acid_spray_custom_mixing",
		{ duration = caster:GetTalentValue("modifier_alchemist_spray_legendary", "delay") }
	)
end
...
function modifier_alchemist_acid_spray_custom_tracker:GetModifierStatusResistanceStacking()
	return (
		...
		or self.parent:HasModifier("modifier_alchemist_unstable_concoction_custom")
			...
			and self.ability.talents.status_inc
		or 0
end
...
function modifier_alchemist_acid_spray_custom_tracker:GetModifierHealthRegenPercentage()
	local regen = (
		...
		or self.parent:HasModifier("modifier_alchemist_acid_spray_custom_aura_purple")
			...
			and self.ability.talents.regen_inc
		or 0
...
function modifier_alchemist_acid_spray_custom_armor:OnCreated()
	...
	self.armor = self.caster:HasTalent("modifier_alchemist_unstable_legendary") and 0
		or self.caster:GetTalentValue("modifier_alchemist_spray_3", "armor") / self.max
	self.magic = self.caster:HasTalent("modifier_alchemist_unstable_legendary")
			and self.caster:GetTalentValue("modifier_alchemist_spray_3", "magic") / self.max
		or 0
...
function modifier_alchemist_acid_spray_custom_root:IsHidden()
	return true
end
