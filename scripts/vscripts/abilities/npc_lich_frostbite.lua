...
function modifier_lich_frostbite:IsHidden()
	return true
end
...
function modifier_lich_frostbite:DamageEvent_out(params)
	...
	if
		params.inflictor == blast
		or params.inflictor == ulti and params.unit:GetTeamNumber() ~= params.attacker:GetTeamNumber()
	then
		local duration = self:GetAbility():GetSpecialValueFor("duration")
			+ (self:GetCaster():GetUpgradeStack("modifier_waveupgrade_boss") - 1)
				* self:GetAbility():GetSpecialValueFor("duration_inc")
		params.unit:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_lich_blast", { duration = duration })
	end
end

function modifier_lich_frostbite:DeathEvent(params)
	if not IsServer() then
		return
	end
...
function modifier_lich_blast:IsHidden()
	return false
end
function modifier_lich_blast:IsPurgable()
	return false
end
...
function modifier_lich_blast:GetModifierLifestealRegenAmplify_Percentage()
	return self.heal * self:GetStackCount()
end
