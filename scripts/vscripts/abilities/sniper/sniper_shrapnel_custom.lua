...
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_damage",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_damage_stack",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_mine",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_tracker",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_move_speed",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_shrapnel_custom_silence",
	"abilities/sniper/sniper_shrapnel_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_sniper_shrapnel_custom_thinker:OnCreated(kv)
	...
	self.delay = self.ability:GetSpecialValueFor("damage_delay")
		* (1 + self.caster:GetTalentValue("modifier_sniper_shrapnel_6", "delay") / 100)
	self.radius = self.ability:GetSpecialValueFor("radius")
		+ self.caster:GetTalentValue("modifier_sniper_shrapnel_5", "radius")
...
function modifier_sniper_shrapnel_custom_thinker:OnIntervalThink()
	...
	if
		(self.caster:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.radius
		and self.caster:HasTalent("modifier_sniper_shrapnel_6")
	then
...
function modifier_sniper_shrapnel_custom_tracker:IsHidden()
	return true
end
function modifier_sniper_shrapnel_custom_tracker:IsPurgable()
	return false
end
...
function modifier_sniper_shrapnel_custom_tracker:AttackEvent_out(params)
	...
	if target:HasModifier("modifier_sniper_shrapnel_custom_damage") then
		if self.parent:HasTalent("modifier_sniper_shrapnel_3") then
			self.parent:GenericHeal(
				self.parent:GetMaxHealth() * self.parent:GetTalentValue("modifier_sniper_shrapnel_3", "heal") / 100,
				self.ability,
				nil,
				nil,
	...
	if not self.parent:HasTalent("modifier_sniper_shrapnel_7") then
		return
	end
	...
	if not RollPseudoRandomPercentage(chance, 276, self.parent) then
		return
	end
...
function modifier_sniper_shrapnel_custom:OnCreated(table)
	...
	self.ms_slow = self.ability:GetSpecialValueFor("slow_movement_speed")
		+ self.caster:GetTalentValue("modifier_sniper_shrapnel_1", "slow")
	...
	if not self.caster:HasTalent("modifier_sniper_shrapnel_7") then
		return
	end
...
function modifier_sniper_shrapnel_custom:GetModifierDamageOutgoing_Percentage()
	if not self.caster:HasTalent("modifier_sniper_shrapnel_3") then
		return
	end
	return self.slow
end
...
function modifier_sniper_shrapnel_custom:OnIntervalThink()
	...
	if self.interval_timer >= 0.95 then
		...
		if
			self.caster:HasTalent("modifier_sniper_shrapnel_5")
			and not self.parent:HasModifier("modifier_sniper_shrapnel_custom_silence")
		then
				...
				self.parent:AddNewModifier(
					...
					"modifier_sniper_shrapnel_custom_silence",
					{
						duration = (1 - self.parent:GetStatusResistance())
							* self.caster:GetTalentValue("modifier_sniper_shrapnel_5", "silence"),
					}
				)
			end
		end
...
function modifier_sniper_shrapnel_custom_damage:GetAttributes()
	if not self:GetCaster():HasTalent("modifier_sniper_shrapnel_7") then
		return
	end
...
function modifier_sniper_shrapnel_custom_mine:IsHidden()
	return true
end
function modifier_sniper_shrapnel_custom_mine:IsPurgable()
	return false
end
...
function modifier_sniper_shrapnel_custom_mine:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		enemy:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = (1 - enemy:GetStatusResistance()) * self.stun }
		)
...
function modifier_sniper_shrapnel_custom_move_speed:IsHidden()
	return false
end
function modifier_sniper_shrapnel_custom_move_speed:IsPurgable()
	return false
end
function modifier_sniper_shrapnel_custom_move_speed:GetTexture()
	return "buffs/aim_evasion"
end
...
function modifier_sniper_shrapnel_custom_damage_stack:IsHidden()
	return false
end
function modifier_sniper_shrapnel_custom_damage_stack:GetTexture()
	return "buffs/acorn_range"
end
function modifier_sniper_shrapnel_custom_damage_stack:IsPurgable()
	return false
end
...
function modifier_sniper_shrapnel_custom_silence:IsHidden()
	return true
end
function modifier_sniper_shrapnel_custom_silence:IsPurgable()
	return true
end
function modifier_sniper_shrapnel_custom_silence:CheckState()
	return { [MODIFIER_STATE_SILENCED] = true }
end
function modifier_sniper_shrapnel_custom_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf"
end
function modifier_sniper_shrapnel_custom_silence:GetEffectAttachType()
