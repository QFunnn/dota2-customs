...
LinkLuaModifier(
	"modifier_sniper_headshot_custom_slow",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_headshot_custom_shield",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_headshot_custom_shield_cd",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_headshot_custom_vision",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_sniper_headshot_custom_legendary_effect",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_headshot_custom_legendary_agility",
	"abilities/sniper/sniper_headshot_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sniper_headshot_custom:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_sniper_headshot_custom_invis",
		{ duration = caster:GetTalentValue("modifier_sniper_headshot_7", "invis") }
	)
...
function modifier_sniper_headshot_custom:IsHidden()
	return self:GetStackCount() == 1 or not self:GetCaster():HasTalent("modifier_sniper_headshot_3")
end
function modifier_sniper_headshot_custom:IsPurgable()
	return false
end
...
function modifier_sniper_headshot_custom:GetModifierHealthBonus()
	if not self.parent:HasTalent("modifier_sniper_headshot_5") then
		return
	end
...
function modifier_sniper_headshot_custom:GetModifierAttackRangeBonus()
	if not self.parent:HasTalent("modifier_sniper_headshot_4") then
		return
	end
...
function modifier_sniper_headshot_custom:GetModifierMoveSpeedBonus_Constant()
	if not self.parent:HasTalent("modifier_sniper_headshot_3") then
		return
	end
...
function modifier_sniper_headshot_custom:GetMinHealth()
	...
	if self.parent:HasModifier("modifier_sniper_headshot_custom_shield_cd") then
		return
	end
...
function modifier_sniper_headshot_custom:DamageEvent_out(params)
	...
	if
		self.parent:HasTalent("modifier_sniper_headshot_1")
		and self.parent == params.attacker
		and params.unit:IsUnit()
		and self.record
		and params.record == self.record
	then
		...
		local particle =
			ParticleManager:CreateParticle("particles/sniper/headshot_cleave.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_sniper_headshot_custom:DamageEvent_inc(params)
	...
	if self.parent:GetHealthPercent() > self.health_thresh then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_sniper_headshot_custom_shield",
		{ duration = self.health_duration }
	)
	self.parent:AddNewModifier(
		self.parent,
		self:GetAbility(),
		"modifier_sniper_headshot_custom_shield_cd",
		{ duration = self.health_cd }
	)
end
...
function modifier_sniper_headshot_custom:AttackEvent_out(params)
	...
	if
		self.parent:HasModifier("modifier_sniper_headshot_custom_legendary_effect")
		and (self.parent:GetAbsOrigin() - params.target:GetAbsOrigin()):Length2D() >= self.max_range
	then
		self.parent:AddNewModifier(
			self.parent,
			self.ability,
			"modifier_sniper_headshot_custom_legendary_agility",
			{ duration = self.agi_duration }
		)
	end
end
...
function modifier_sniper_headshot_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if target:IsDebuffImmune() and not self.parent:HasTalent("modifier_sniper_headshot_6") then
		return
	end
	...
	if
		not self.parent:HasModifier("modifier_sniper_take_aim_custom_active")
		or self.parent:HasTalent("modifier_sniper_aim_7")
	then
	...
	local damage = self.damage
		+ self.caster:GetTalentValue("modifier_sniper_headshot_1", "damage") * self.caster:GetAgility() / 100
	...
	target:AddNewModifier(
		self.parent,
		self.parent:BkbAbility(self.ability, self.parent:HasTalent("modifier_sniper_headshot_6")),
		"modifier_sniper_headshot_custom_slow",
	...
	if
		not target:IsCurrentlyHorizontalMotionControlled()
		and not target:IsCurrentlyVerticalMotionControlled()
	...
	if
		self.parent:HasTalent("modifier_sniper_aim_4")
		and (
			self.parent:HasModifier("modifier_sniper_take_aim_custom_active")
			or self.parent:HasModifier("modifier_sniper_take_aim_custom_legendary_damage")
		)
	then
		params.target:AddNewModifier(
			self.parent,
			self.parent:BkbAbility(nil, true),
			"modifier_sniper_take_aim_custom_armor",
...
function modifier_sniper_headshot_custom_shield:IsHidden()
	return false
end
function modifier_sniper_headshot_custom_shield:IsPurgable()
	return false
end
function modifier_sniper_headshot_custom_shield:GetTexture()
	return "buffs/back_ground"
end
function modifier_sniper_headshot_custom_shield:OnCreated()
	...
	self.max_shield = self.parent:GetMaxHealth()
		* self.parent:GetTalentValue("modifier_sniper_headshot_5", "shield")
		/ 100
...
function modifier_sniper_headshot_custom_shield_cd:IsHidden()
	return false
end
function modifier_sniper_headshot_custom_shield_cd:IsPurgable()
	return false
end
function modifier_sniper_headshot_custom_shield_cd:GetTexture()
	return "buffs/back_ground"
end
function modifier_sniper_headshot_custom_shield_cd:IsDebuff()
	return true
end
function modifier_sniper_headshot_custom_shield_cd:RemoveOnDeath()
	return false
end
...
function modifier_sniper_headshot_custom_vision:IsHidden()
	return true
end
function modifier_sniper_headshot_custom_vision:IsPurgable()
	return false
end
...
function modifier_sniper_headshot_custom_invis:IsPurgable()
	return false
end
function modifier_sniper_headshot_custom_invis:IsHidden()
	return false
end
function modifier_sniper_headshot_custom_invis:GetTexture()
	return "buffs/reflection_speed"
end
...
function modifier_sniper_headshot_custom_invis:OnDestroy()
	...
	local mod = self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_sniper_headshot_custom_legendary_effect",
		{ duration = self.parent:GetTalentValue("modifier_sniper_headshot_7", "duration") }
	)
...
function modifier_sniper_headshot_custom_legendary_effect:IsHidden()
	return true
end
function modifier_sniper_headshot_custom_legendary_effect:IsPurgable()
	return false
end
function modifier_sniper_headshot_custom_legendary_effect:OnCreated()
	...
	self.radius_visual = ParticleManager:CreateParticleForPlayer(
		"particles/sniper/headshot_legendary_radius.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.parent,
...
function modifier_sniper_headshot_custom_legendary_agility:IsHidden()
	return true
end
function modifier_sniper_headshot_custom_legendary_agility:IsPurgable()
	return false
end
...
function modifier_sniper_headshot_custom_legendary_agility:OnIntervalThink()
	...
	if not self.parent:HasModifier("modifier_sniper_headshot_custom_legendary_effect") then
		self.parent:UpdateUIshort({
			...
			max_time = self.max_time,
			stack = "+" .. tostring(math.floor(self:GetStackCount() * self.agi * 100)) .. "%",
			style = "SniperHeadshot",
		})
	end
