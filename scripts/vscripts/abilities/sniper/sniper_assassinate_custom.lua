...
LinkLuaModifier(
	"modifier_sniper_assassinate_custom",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_legendary_unit",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_legendary_slow",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_legendary_stack",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_kill_stack",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_kill_tracker",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_heal_reduce",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_damage_reduction",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_slow",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_mark",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_attack",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_sniper_assassinate_custom_tracker",
	"abilities/sniper/sniper_assassinate_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sniper_assassinate_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/sniper/sniper_fall20_immortal/sniper_legendary_assassinate.vpcf",
		context
	)
...
function sniper_assassinate_custom:OnAbilityPhaseInterrupted()
	...
	if not self.storedTargets then
		return
	end
...
function sniper_assassinate_custom:OnSpellStart(new_target)
	...
	for _, target in pairs(targets_array) do
		local sound_thinker = CreateModifierThinker(
			...
			self,
			"modifier_sniper_assassinate_custom_legendary_unit",
			{ duration = self:GetSpecialValueFor("distance") / speed },
			caster:GetAbsOrigin(),
			caster:GetTeamNumber(),
			false
		)
...
function sniper_assassinate_custom:OnProjectileHit_ExtraData(hTarget, vLocation, data)
	...
	if caster:HasTalent("modifier_sniper_assassinate_6") and not target:IsDebuffImmune() then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_sniper_assassinate_custom_slow",
			{ duration = caster:GetTalentValue("modifier_sniper_assassinate_6", "duration", true) }
		)
	end
	...
	local damage = self:GetSpecialValueFor("damage")
		+ caster:GetTalentValue("modifier_sniper_assassinate_2", "damage")
	...
	if caster:HasTalent("modifier_sniper_assassinate_2") then
		target:AddNewModifier(
			...
			self,
			"modifier_sniper_assassinate_custom_heal_reduce",
			{ duration = caster:GetTalentValue("modifier_sniper_assassinate_2", "duration") }
		)
	end
	...
	if caster:HasTalent("modifier_sniper_assassinate_5") then
		...
		target:AddNewModifier(
			...
			"modifier_sniper_assassinate_custom_damage_reduction",
			{ duration = caster:GetTalentValue("modifier_sniper_assassinate_5", "duration", true) }
		)
	...
	if caster:HasTalent("modifier_sniper_assassinate_4") then
		target:AddNewModifier(
			...
			self,
			"modifier_sniper_assassinate_custom_mark",
			{ duration = caster:GetTalentValue("modifier_sniper_assassinate_4", "duration") }
		)
	end
...
function modifier_sniper_assassinate_custom:IsAura()
	return true
end
...
function sniper_assassinate_custom_legendary:OnProjectileHit_ExtraData(hTarget, vLocation, data)
	...
	if not data.index or not self.projectiles or not self.projectiles[data.index] then
		return
	end
	...
	local damageTable =
		{ victim = enemy, attacker = caster, damage = damage, ability = self, damage_type = DAMAGE_TYPE_MAGICAL }
	DoDamage(damageTable)
end
...
function sniper_assassinate_custom_legendary:OnSpellStart()
	...
	local sound_thinker = CreateModifierThinker(
		...
		self,
		"modifier_sniper_assassinate_custom_legendary_unit",
		{ duration = self:GetSpecialValueFor("distance") / self:GetSpecialValueFor("projectile_speed") },
		caster:GetAbsOrigin(),
		caster:GetTeamNumber(),
		false
	)
...
function modifier_sniper_assassinate_custom_legendary_unit:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_legendary_unit:IsPurgable()
	return false
end

modifier_sniper_assassinate_custom_legendary_slow = class({})
function modifier_sniper_assassinate_custom_legendary_slow:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_legendary_slow:IsPurgable()
	return true
end
function modifier_sniper_assassinate_custom_legendary_slow:GetEffectName()
...
function modifier_sniper_assassinate_custom_legendary_stack:IsHidden()
	return false
end
function modifier_sniper_assassinate_custom_legendary_stack:IsPurgable()
	return false
end
...
function modifier_sniper_assassinate_custom_kill_tracker:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_kill_tracker:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_kill_tracker:RemoveOnDeath()
	return false
end
...
function modifier_sniper_assassinate_custom_kill_stack:IsHidden()
	return false
end
function modifier_sniper_assassinate_custom_kill_stack:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_kill_stack:RemoveOnDeath()
	return false
end
...
function modifier_sniper_assassinate_custom_heal_reduce:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_heal_reduce:IsPurgable()
	return false
end
...
function modifier_sniper_assassinate_custom_slow:IsHidden()
	return false
end
function modifier_sniper_assassinate_custom_slow:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_slow:GetTexture()
	return "buffs/assassinate_slow"
end
...
function modifier_sniper_assassinate_custom_damage_reduction:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_damage_reduction:IsPurgable()
	return false
end
...
function modifier_sniper_assassinate_custom_mark:IsHidden()
	return false
end
function modifier_sniper_assassinate_custom_mark:IsPurgable()
	return false
end
function modifier_sniper_assassinate_custom_mark:GetTexture()
...
function modifier_sniper_assassinate_custom_attack:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_attack:IsPurgable()
	return false
end
...
function modifier_sniper_assassinate_custom_tracker:IsHidden()
	return true
end
function modifier_sniper_assassinate_custom_tracker:IsPurgable()
	return false
end
...
function modifier_sniper_assassinate_custom_tracker:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_sniper_assassinate_1") then
		return
	end
...
function modifier_sniper_assassinate_custom_tracker:GetModifierPercentageManacostStacking()
	if not self.parent:HasTalent("modifier_sniper_assassinate_3") then
		return
	end
...
function modifier_sniper_assassinate_custom_tracker:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_sniper_assassinate_4") then
		return
	end
