...
LinkLuaModifier(
	"modifier_furion_blooming_flare_custom",
	"abilities/furion/furion_blooming_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_blooming_flare_custom_tracker",
	"abilities/furion/furion_blooming_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_furion_blooming_flare_custom_root",
	"abilities/furion/furion_blooming_flare_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function furion_blooming_flare_custom:OnProjectileHit(target, vLocation)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = (1 - target:GetStatusResistance()) * self.stun }
	)
...
function modifier_furion_blooming_flare_custom:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_furion_blooming_flare_custom:RemoveOnDeath()
	return false
end
function modifier_furion_blooming_flare_custom:OnCreated(table)
	...
	self.jump_delay = self.ability.jump_delay
		* (1 - (self.ability.talents.has_r4 == 1 and self.ability.talents.r4_speed or 0))
	self.target_max = self.ability.max
	self.radius = self.ability.radius

	self.root = self.ability.root
	self.damage = self.ability.damage
	self.root_max = (self.ability.root_max + (self.caster:HasShard() and self.ability.shard_root or 0))
		* (1 + (self.ability.talents.has_r7 == 1 and self.ability.talents.r7_root or 0))
	...
	self.particle_peffect = ParticleManager:CreateParticle(
		"particles/nature_prophet/wrath_legendary_aoe.vpcf",
...
function modifier_furion_blooming_flare_custom:OnIntervalThink()
	...
	if not IsValid(self.new_target) or self.hit_count >= self.target_max then
		...
		if self.stage == 2 then
			hit_effect = ParticleManager:CreateParticle(
				"particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_hit.vpcf",
		...
		if self.stage == 2 then
			...
			if self.ability.talents.has_r7 == 1 and not self.tree_init and enemy:IsHero() then
				...
				for i = 1, count do
					...
					local unit =
						CreateUnitByName("npc_dota_companion", abs, false, nil, nil, self.caster:GetTeamNumber())
					unit:AddNewModifier(
						self.caster,
						self.ability,
						"modifier_furion_blooming_flare_custom_legendary_tree",
						{ duration = self.ability.talents.r7_duration, tree = tree:entindex() }
					)
				end
			end
			...
			if
				enemy:IsRealHero()
				and self.caster:GetQuest() == "Furion.Quest_8"
				and not self.caster:QuestCompleted()
				and self.tree_count >= self.caster.quest.number
			then
			...
			enemy:AddNewModifier(
				self.caster,
				self.caster:BkbAbility(self.ability, self.caster:HasShard()),
				"modifier_furion_blooming_flare_custom_root",
				{ duration = (1 - enemy:GetStatusResistance()) * self.root }
			)
		end
	end
...
function modifier_furion_blooming_flare_custom_root:OnCreated()
	...
	local nfx = ParticleManager:CreateParticle(
		"particles/nature_prophet/call_root.vpcf",
		PATTACH_CUSTOMORIGIN_FOLLOW,
		self.parent
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_furion_blooming_flare_custom_root:OnDestroy()
	...
	local duration = self.ability.shard_duration
		* (1 + (self.ability.talents.has_r7 == 1 and self.ability.talents.r7_root or 0))
...
function modifier_furion_blooming_flare_custom_tracker:SpellEvent(params)
	...
	if
		not self.parent:CheckCd("furion_r3", self.ability.talents.r3_talent_cd, self.ability.talents.r3_chance, 8991)
	then
		return
	end
...
function modifier_furion_blooming_flare_custom_slow:IsPurgable()
	return true
end
function modifier_furion_blooming_flare_custom_slow:GetStatusEffectName()
	return "particles/status_fx/status_effect_natures_prophet_curse.vpcf"
end
function modifier_furion_blooming_flare_custom_slow:StatusEffectPriority()
...
function modifier_furion_blooming_flare_custom_proc:GetTexture()
	return "buffs/furion/nature_wrath_3"
end
function modifier_furion_blooming_flare_custom_proc:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_furion_blooming_flare_custom_proc:GetEffectName()
...
function modifier_furion_blooming_flare_custom_proc:OnIntervalThink()
	...
	if not self.init then
		local nFXIndex =
			ParticleManager:CreateParticle("particles/nature_prophet/blooming_proc_aoe.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		local damage = (self.ability.talents.r3_base + self.ability.talents.r3_damage * self.caster:GetIntellect(false))
			* count
			* self.interval
			/ self.max
...
function modifier_furion_blooming_flare_custom_legendary_tree:OnIntervalThink()
	...
	for _, player in pairs(players) do
		if
			player:IsAlive()
			and (player == self.caster or (player:GetAbsOrigin() - self.pos):Length2D() <= (self.vision_radius + 200))
		then
