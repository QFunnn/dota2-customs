...
LinkLuaModifier(
	"modifier_storm_spirit_overload_custom",
	"abilities/storm_spirit_overload_custom",
...
function storm_spirit_overload_custom:OnSpellStart()
	...
	if storm_spirit_overload_custom and storm_spirit_overload_custom:GetLevel() > 0 then
		for _, hero in pairs(heroes) do
			local modifier_storm_spirit_overload = hero:AddNewModifier(
				self:GetCaster(),
...
function modifier_storm_spirit_overload_custom:IsPurgable()
	return false
end
function modifier_storm_spirit_overload_custom:IsHidden()
	return true
end
function modifier_storm_spirit_overload_custom:IsPurgeException()
	return false
end
...
function modifier_storm_spirit_overload_custom:OnIntervalThink()
	...
	if not self.parent:HasModifier("modifier_storm_spirit_overload_custom_buff") and self.ability:IsFullyCastable() then
		local modifier_storm_spirit_overload_custom_buff =
			self.parent:AddNewModifier(self.parent, self.ability, "modifier_storm_spirit_overload_custom_buff", {})
		if self:GetCaster():HasShard() then
			modifier_storm_spirit_overload_custom_buff:SetStackCount(
				self:GetAbility():GetSpecialValueFor("shard_activation_charges")
			)
		end
	end
end
...
function modifier_storm_spirit_overload_custom_buff:IsPurgable()
	return false
end
function modifier_storm_spirit_overload_custom_buff:IsPurgeException()
	return false
end
...
function modifier_storm_spirit_overload_custom_buff:GetModifierProcAttack_Feedback(params)
	...
	if self:GetCaster():HasTalent("special_bonus_unique_storm_spirit_7") then
		local enemies = FindUnitsInRadius(
			...
			params.target:GetOrigin(),
			nil,
			self:GetAbility():GetSpecialValueFor("overload_aoe"),
			DOTA_UNIT_TARGET_TEAM_ENEMY,
			DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
			0,
			0,
