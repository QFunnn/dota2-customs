...
function modifier_tower_purge_aura:IsHidden()
	return true
end
...
function modifier_tower_purge_aura:GetModifierAttackRangeBonus()
	return (self.attack_range_override or 850) - 850
end
function modifier_tower_purge_aura:GetModifierModelScale()
...
modifier_tower_purge_aura.purge_exceptions = {
...
function modifier_tower_purge_aura:OnIntervalThink()
	...
	for _, enemy in pairs(enemies or {}) do
		if IsValidEntity(enemy) and enemy:IsAlive() and not self.purge_exceptions[enemy:GetUnitName()] then
