...
function modifier_frostbitten_spam:IsHidden()
	return true
end

function modifier_frostbitten_spam:IsPurgable()
	return false
end
...
function modifier_frostbitten_spam:OnIntervalThink()
	...
	if
		(self:GetCaster():GetAbsOrigin() - self.target:GetAbsOrigin()):Length2D()
...
function modifier_frostbitten_thinker:IsHidden()
	return true
end

function modifier_frostbitten_thinker:IsPurgable()
	return false
end
	...
	for _, i in pairs(enemy_for_ability) do
		if not i:IsMagicImmune() then
			i:AddNewModifier(
				...
				{ duration = self:GetAbility():GetSpecialValueFor("duration_slow") * (1 - i:GetStatusResistance()) }
			)
			DoDamage({
				victim = i,
...
function modifier_frostbitten_spam_buff:IsHidden()
	return false
end
...
function modifier_frostbitten_spam_cd:IsHidden()
	return false
end
function modifier_frostbitten_spam_cd:IsPurgable()
	return false
end