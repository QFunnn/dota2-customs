...
LinkLuaModifier(
	"modifier_bane_brain_sap_custom_tracker",
...
function bane_brain_sap_custom:GetManaCost(level)
	return self.talents.has_w7 == 1 and self.caster:GetMaxMana() * self.talents.w7_mana
		or self.BaseClass.GetManaCost(self, level)
end
...
function bane_brain_sap_custom:DealDamage(target, is_auto)
	...
	if self.talents.has_w4 == 1 and self.caster:GetHealthPercent() < target:GetHealthPercent() and not is_auto then
		if
			...
			and not target:HasModifier("modifier_bane_brain_sap_custom_fear_cd")
			and not target:HasModifier("modifier_bane_nightmare_custom")
		then
			...
			target:AddNewModifier(
				...
				{ duration = self.talents.w4_fear * (1 - target:GetStatusResistance()) }
			)
		end
	end
...
function modifier_bane_brain_sap_custom_tracker:OnIntervalThink()
	...
	if self.parent:HasModifier("modifier_bane_brain_sap_custom_auto_cd") then
		return
	end
	if not self.parent:IsAlive() or self.parent:IsInvisible() then
		return
	end

	local target = self.parent:RandomTarget(self.ability.talents.w3_radius)
	if not target or target:HasModifier("modifier_bane_nightmare_custom") then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_bane_brain_sap_custom_auto_cd",
		{ duration = self.ability.talents.w3_cd }
	)
...
function modifier_bane_brain_sap_custom_tracker:DamageEvent_inc(params)
	...
	if self.parent:HasModifier("modifier_bane_brain_sap_custom_shield_auto_cd") then
		return
	end
	...
	if self.shield_mod then
		...
		self.cast_effect =
			ParticleManager:CreateParticle("particles/bane/brain_shield.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, self.parent)
		ParticleManager:SetParticleControlEnt(
			self.cast_effect,
			0,
			self.parent,
	...
	if not IsValid(self.shield_mod) then
		...
		if self.shield_mod then
			self.cast_effect = ParticleManager:CreateParticle(
				"particles/bane/brain_shield.vpcf",
...
function modifier_bane_brain_sap_custom_shield_auto_cd:GetTexture()
	return "buffs/bane/hero_6"
end

modifier_bane_brain_sap_custom_fear_cd = class(mod_hidden)
function modifier_bane_brain_sap_custom_fear_cd:RemoveOnDeath()
	return false
end
...
function modifier_bane_brain_sap_custom_spells:GetTexture()
	return "buffs/bane/hero_4"
end
