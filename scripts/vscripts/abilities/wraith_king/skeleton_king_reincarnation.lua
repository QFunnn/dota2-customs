...
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom",
...
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom_aura_damage",
	"abilities/wraith_king/skeleton_king_reincarnation",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_skeleton_king_reincarnation_custom_legendary_target",
	"abilities/wraith_king/skeleton_king_reincarnation",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function skeleton_king_reincarnation_custom:UpdateTalents()
	...
	if not self.init then
		...
		self.talents = {
			...
			h6_fear_move = caster:GetTalentValue("modifier_skeleton_hero_6", "fear_move", true),
...
function skeleton_king_reincarnation_custom:ReincarnationStart(params, modifier)
	...
	if self.talents.has_h6 == 1 then
		...
		local effect_cast =
			ParticleManager:CreateParticle("particles/sand_king/sand_pull.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_skeleton_king_reincarnation_custom:OnIntervalThink()
	...
	if self.ability.talents.has_r2 == 1 then
		...
		if
			self.parent:IsAlive()
			and not IsValid(self.ability.shield_mod)
			and not self.parent:HasModifier("modifier_skeleton_king_reincarnation_custom_shield_cd")
		then
			...
			if self.ability.shield_mod then
				...
				self.particle = ParticleManager:CreateParticle(
					wearables_system:GetParticleReplacementAbility(
						self.parent,
						"particles/wraith_king/reinc_shield_base.vpcf",
						self.ability
					),
				...
				self.ability.shield_mod:SetEndFunction(function()
					self.parent:AddNewModifier(
						...
						self.ability,
						"modifier_skeleton_king_reincarnation_custom_shield_cd",
						{ duration = self.ability.talents.r2_talent_cd }
					)
				end)
			end
		end
	...
	if not self.parent:IsAlive() and not self.parent:IsReincarnating() then
		return
	end
	...
	if has_mod then
		...
		if self.count >= 1 then
			...
			self.parent:AddNewModifier(
				...
				self.ability,
				"modifier_skeleton_king_reincarnation_custom_aura_str",
				{ duration = self.ability.talents.r3_duration, stack = 1 }
			)
		end
	end
end
...
function modifier_skeleton_king_reincarnation_custom:RespawnEvent(params)
	...
	if self.parent.reincarnate_res == false then
		return
	end
	self.parent.reincarnate_res = false
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_skeleton_king_reincarnation_custom_move",
		{ duration = self.ability.talents.h6_move_duration }
	)
	...
	for _, target in pairs(self.parent:FindTargets(self.ability.talents.h6_distance)) do
		...
		local mod = target:AddNewModifier(
			...
			{ duration = (1 - target:GetStatusResistance()) * self.ability.talents.h6_fear }
		)
		target:AddNewModifier(
			self.parent,
			self.parent:BkbAbility(self.ability, true),
...
function modifier_skeleton_king_reincarnation_custom_aura_damage:OnIntervalThink()
	...
	local damage = (self.ability.talents.r1_base + self.caster:GetHealth() * self.ability.talents.r1_damage)
		* self.interval
...
function modifier_skeleton_king_reincarnation_custom_slow:IsPurgable()
	return not self.caster:HasShard()
end
function modifier_skeleton_king_reincarnation_custom_slow:GetTexture()
	return self.ability:GetAbilityTextureName()
end
...
function modifier_skeleton_king_reincarnation_custom_aura_str:RemoveOnDeath()
	return false
end
...
function modifier_skeleton_king_reincarnation_custom_perma:IsHidden()
	return self.ability.talents.has_r4 == 0 or self:GetStackCount() == 0 or self:GetStackCount() >= self.max
end
function modifier_skeleton_king_reincarnation_custom_perma:RemoveOnDeath()
	return false
end
function modifier_skeleton_king_reincarnation_custom_perma:GetTexture()
	return "buffs/wraith_king/reincarnation_4"
end
...
function modifier_skeleton_king_reincarnation_custom_magic:GetTexture()
	return "buffs/wraith_king/reincarnation_3"
end
...
modifier_skeleton_king_reincarnation_custom_fear_slow = class(mod_hidden)
function modifier_skeleton_king_reincarnation_custom_fear_slow:OnCreated()
	self.parent = self:GetParent()
	self.caster = self:GetCaster()
	self.ability = self.caster.reincarnate_ability
	if not self.ability then
	...
	self.move = self.ability.talents.h6_fear_move
end

function modifier_skeleton_king_reincarnation_custom_fear_slow:DeclareFunctions()
	return {
