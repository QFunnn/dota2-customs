...
function npc_centaur_stun:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_centaur_cd", { duration = self:GetCooldownTimeRemaining() })
	...
	for _, target in pairs(targets) do
		if not target:IsMagicImmune() then
			target:AddNewModifier(
				...
				self,
				"modifier_stunned",
				{ duration = self:GetSpecialValueFor("stun") * (1 - target:GetStatusResistance()) }
			)
			DoDamage({
				victim = target,
...
function modifier_centaur_cd:IsHidden()
	return false
end
function modifier_centaur_cd:IsPurgable()
	return false
end