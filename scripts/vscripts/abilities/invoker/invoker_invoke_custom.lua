...
LinkLuaModifier(
	"modifier_invoker_invoke_custom_spell_count",
	"abilities/invoker/invoker_invoke_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_invoker_invoke_custom_scepter_1",
	"abilities/invoker/invoker_invoke_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invoker_invoke_custom_scepter_2",
	"abilities/invoker/invoker_invoke_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_invoker_invoke_custom_scepter_3",
	"abilities/invoker/invoker_invoke_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_invoke_custom_tracker:GetModifierSpellAmplify_Percentage()
	return self.ability.talents.r1_spell
		+ ((self.ability.talents.has_s4 == 1 and self:GetCaster():HasScepter()) and self.ability.talents.s4_spell or 0)
end

function modifier_invoker_invoke_custom_tracker:GetModifierPercentageCooldown()
	return self.ability.talents.r2_cdr
		+ ((self.ability.talents.has_s4 == 1 and self:GetCaster():HasScepter()) and self.ability.talents.s4_cdr or 0)
end
...
function invoker_deafening_blast_custom:GetCooldown(level)
	return self.BaseClass.GetCooldown(self, level)
		+ ((self.talents.has_s4 == 1 and self:GetCaster():HasScepter()) and self.talents.s4_cd or 0)
end
