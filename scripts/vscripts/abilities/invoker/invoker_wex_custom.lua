...
LinkLuaModifier(
	"modifier_invoker_emp_custom_legendary_damage",
	"abilities/invoker/invoker_wex_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
LinkLuaModifier(
	"modifier_invoker_emp_custom_teleport",
	"abilities/invoker/invoker_wex_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_invoker_tornado_custom_scepter_count",
	"abilities/invoker/invoker_wex_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_invoker_wex_custom_passive:SpellEvent(params)
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_invoker_alacrity_custom_damage",
		{ duration = self.ability.talents.w3_effect_duration }
	)
end
...
function modifier_invoker_wex_custom_passive:AttackStartEvent_out(params)
	...
	if not IsValid(self.parent.emp_ability) then
		return
	end
...
function modifier_invoker_wex_custom_passive:GetModifierBonusStats_Agility()
	return self.agi
		+ self.ability.talents.w1_stats
		+ self.parent:GetUpgradeStack("modifier_invoker_alacrity_custom_stats") * self.ability.talents.w1_bonus
end

function modifier_invoker_wex_custom_passive:GetModifierBonusStats_Strength()
	return self.ability.talents.w1_stats
		+ self.parent:GetUpgradeStack("modifier_invoker_alacrity_custom_stats") * self.ability.talents.w1_bonus
end
...
function modifier_invoker_emp_custom:OnDestroy()
	...
	local effect_cast =
		ParticleManager:CreateParticle("particles/invoker/emp_legendarya.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	then
		...
		local particle =
			ParticleManager:CreateParticle("particles/invoker/emp_blink_start.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		local particle4 = ParticleManager:CreateParticle(
		...
		self.caster:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_invoker_emp_custom_invun",
			{ duration = self.ability.talents.w4_invun }
		)
...
function modifier_invoker_tornado_custom:OnDestroy()
	...
	if not self.caster or not self.ability then
		return
	end
	...
	if self.ability.talents.has_s3 == 1 and self.caster:HasScepter() then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_invoker_tornado_custom_purge",
			{ duration = self.ability.talents.s3_duration }
		)
	end
	...
	if self.parent:IsRealHero() and self.ability.talents.has_s3 == 0 then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_invoker_tornado_custom_scepter_count",
			{ duration = self.ability.talents.s3_timer }
		)
	end
...
function modifier_invoker_tornado_custom_silence:IsPurgable()
	return true
end
function modifier_invoker_tornado_custom_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf"
end
function modifier_invoker_tornado_custom_silence:ShouldUseOverheadOffset()
	return true
end
...
function modifier_invoker_tornado_custom_scepter_count:DamageEvent_inc(params)
	...
	if not IsValid(self.caster.invoke_ability) then
		return
	end
