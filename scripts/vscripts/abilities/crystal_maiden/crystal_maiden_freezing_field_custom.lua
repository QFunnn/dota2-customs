...
function crystal_maiden_freezing_field_custom:GetCastRange(vLocation, hTarget)
	if self.talents.has_blink == 0 then
		return
	end
...
function crystal_maiden_freezing_field_custom:GetManaCost(level)
	...
	if
		self.talents.has_legendary == 1
		or (caster:HasScepter() and caster:HasModifier("modifier_crystal_maiden_freezing_field_custom"))
	then
		return 0
	end
...
function crystal_maiden_freezing_field_custom:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_crystal_maiden_freezing_field_custom",
		{ duration = self.duration + self.talents.linger_inc }
	)
...
function modifier_crystal_maiden_freezing_field_custom:GetModifierIncomingDamage_Percentage()
	if self.ability.talents.has_blink == 0 then
		return
	end
...
function modifier_crystal_maiden_freezing_field_custom:OnIntervalThink()
	...
	for _, enemy in pairs(self.parent:FindTargets(self.slow_radius)) do
		...
		then
			if self.ability:GetAutoCastState() then
				...
				if (target_point - origin):Length2D() > self.ability.talents.silence_distance_min then
					local pull_point = origin
						+ (target_point - origin):Normalized() * self.ability.talents.silence_distance_min
			...
			enemy:AddNewModifier(
				...
				{ duration = (1 - self.parent:GetStatusResistance()) * self.ability.talents.silence_duration }
			)
			enemy:AddNewModifier(
				self.parent,
				self.ability,
	...
	if
		...
		and self:GetRemainingTime() <= self.ability.talents.linger_inc
		and self.ability.talents.has_linger == 1
	then
		self:EndEffect()
	end
...
function modifier_crystal_maiden_freezing_field_custom_debuff:IsHidden()
	return false
end
