...
function modifier_patrol_armor:IsPurgable()
	return false
end
function modifier_patrol_armor:IsHidden()
	return true
end
...
function modifier_patrol_armor:OnIntervalThink(first)
	...
	for _, player in pairs(players) do
		...
		if
			(player:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.radius
			and not team_register[player:GetTeamNumber()]
			and player:IsAlive()
		then
...
function modifier_patrol_armor:DeathEvent(params)
	...
	self.parent:AddNewModifier(
		...
		{ duration = self:GetAbility():GetSpecialValueFor("death_duration") }
	)
end
...
function modifier_patrol_armor_death:IsHidden()
	return true
end
function modifier_patrol_armor_death:IsPurgable()
	return false
end
function modifier_patrol_armor_death:OnCreated(table)
	...
	self.particle =
		ParticleManager:CreateParticle("particles/glyph_damage.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_patrol_armor_death:GetMinHealth()
	if self:GetParent():HasModifier("modifier_death") then
		return
	end
	return 1
end
...
function modifier_patrol_armor_buff:IsHidden()
	return false
end
function modifier_patrol_armor_buff:IsPurgable()
	return false
end
