...
function modifier_largo_encore:IsHidden()
	return true
end
function modifier_largo_encore:IsPurgable()
	return false
end
function modifier_largo_encore:RemoveOnDeath()
	return false
end
...
if IsServer() then
	local function OnAbilityUsed(event)
		...
		if not largo_ability or largo_ability:GetLevel() == 0 then
			return
		end
	...
	function ProcessBuffDuration(hero, ability_name, largo_ability)
		...
		local ability = hero:FindAbilityByName(ability_name) or hero:FindItemInInventory(ability_name)
		if not ability then
			return
		end
