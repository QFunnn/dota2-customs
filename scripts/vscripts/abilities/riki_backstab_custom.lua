...
function modifier_riki_backstab_custom:OnIntervalThink()
	if self:GetAbility():IsFullyCastable() then
		self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_riki_backstab_custom_invis",
			{ duration = duration }
		)
	else
...
function modifier_riki_backstab_custom:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if self:GetParent():HasModifier("modifier_riki_backstab_custom_cd") then
		return
	end
...
function modifier_riki_backstab_custom_cd:IsHidden()
	return true
end
function modifier_riki_backstab_custom_cd:IsPurgable()
	return false
end
function modifier_riki_backstab_custom_cd:IsPurgeException()
	return false
end
function modifier_riki_backstab_custom_cd:RemoveOnDeath()
	return false
end