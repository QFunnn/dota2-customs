...
function npc_lich_blast:GetCooldown(iLevel)
	return self.BaseClass.GetCooldown(self, iLevel)
		- (self:GetCaster():GetUpgradeStack("modifier_waveupgrade_boss") - 1) * self:GetSpecialValueFor("cd_inc")
end
...
function npc_lich_blast:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(
			self:GetCaster(),
			self,
			"modifier_lich_blast_cd",
			{ duration = self:GetCooldownTimeRemaining() }
		)
	self.target = self:GetCursorTarget()
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_lich_blast_targets", { target = self.target:entindex() })
end

modifier_lich_blast_targets = class({})
function modifier_lich_blast_targets:IsHidden()
	return true
end
function modifier_lich_blast_targets:IsPurgable()
	return false
end
function modifier_lich_blast_targets:OnCreated(table)
	...
	local line_position = self:GetCaster():GetAbsOrigin()
		+ Vector(1) * (self:GetCaster():GetAbsOrigin() - self.target:GetAbsOrigin()):Length2D()
	...
	for i = 1, self.number do
		if random then
			...
			pos = self:GetCaster():GetAbsOrigin()
				+ self:GetCaster():GetForwardVector() * n * self:GetAbility():GetSpecialValueFor("radius")
		end
...
function modifier_lich_blast_targets:OnIntervalThink()
	...
	local delay = self:GetAbility():GetSpecialValueFor("delay_blast")

	if self:GetCaster():GetUpgradeStack("modifier_waveupgrade_boss") then
		delay = delay - self:GetAbility():GetSpecialValueFor("delay_inc")
	end
...
function modifier_lich_blast_thinker:IsHidden()
	return false
end

function modifier_lich_blast_thinker:IsPurgable()
	return false
end
...
function modifier_lich_blast_thinker:OnDestroy(table)
	...
	self:GetParent():EmitSound("UI.Ability_frost")
	...
	for _, i in pairs(enemy_for_ability) do
		local damage = self.damage * i:GetMaxHealth()
...
function modifier_lich_blast_cd:IsHidden()
	return false
end
function modifier_lich_blast_cd:IsPurgable()
	return false
end

modifier_lich_blast_b = class({})

function modifier_lich_blast_b:IsHidden()
	return true
end
function modifier_lich_blast_b:IsPurgable()
	return false
end