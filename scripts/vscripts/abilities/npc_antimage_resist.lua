...
function modifier_antimage_resist:IsHidden()
	return true
end
function modifier_antimage_resist:IsPurgable()
	return false
end
...
function modifier_antimage_resist:DamageEvent_inc(params)
	...
	if self.parent:HasModifier("modifier_antimage_bkb_cd") then
		return
	end
	if self.parent:GetHealthPercent() > self:GetAbility():GetSpecialValueFor("health") then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		{ magic = -50, effect = 1, duration = self:GetAbility():GetSpecialValueFor("bkb") }
	)
	self.parent:AddNewModifier(self.parent, self:GetAbility(), "modifier_antimage_bkb_cd", {})
end

modifier_antimage_bkb_cd = class({})
function modifier_antimage_bkb_cd:IsHidden()
	return true
end
function modifier_antimage_bkb_cd:IsPurgable()
	return false
end

modifier_antimage_bkb = class({})
function modifier_antimage_bkb:IsHidden()
	return false
end
function modifier_antimage_bkb:IsPurgable()
	return false
end
function modifier_antimage_bkb:CheckState()
	return { [MODIFIER_STATE_MAGIC_IMMUNE] = true }
end
function modifier_antimage_bkb:GetEffectName()
