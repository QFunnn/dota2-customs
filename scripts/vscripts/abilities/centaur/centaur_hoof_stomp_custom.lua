...
LinkLuaModifier(
	"modifier_centaur_hoof_stomp_custom_charge",
	"abilities/centaur/centaur_hoof_stomp_custom",
...
function modifier_centaur_hoof_stomp_custom_prepair:OrderEvent(params)
	...
	if
		...
		or params.order_type == DOTA_UNIT_ORDER_CAST_NO_TARGET
		or params.order_type == DOTA_UNIT_ORDER_CAST_TOGGLE
...
function modifier_centaur_hoof_stomp_custom_prepair:CheckState()
	if self.distance <= 0 or self.ability.talents.has_q7 == 0 then
		return
	end
	return {
...
function modifier_centaur_hoof_stomp_custom_tracker:OnIntervalThink()
	...
	if self.parent:HasModifier("modifier_centaur_hoof_stomp_custom_charge") then
		return
	end
...
function modifier_centaur_hoof_stomp_custom_move:GetTexture()
	return "buffs/centaur/stomp_3"
end
	...
	if self.dist_count >= self.distance then
		...
		for _, target in pairs(self.parent:FindTargets(self.radius)) do
			...
			target:AddNewModifier(
				...
				self.ability,
				"modifier_centaur_hoof_stomp_custom_slow",
				{ duration = self.ability.talents.q3_slow_duration }
			)
		end
...
function modifier_centaur_hoof_stomp_custom_slow:IsPurgable()
	return true
end
...
function modifier_centaur_hoof_stomp_custom_legendary_damage:GetTexture()
...
function modifier_centaur_hoof_stomp_custom_damage_reduce:GetTexture()
	return "buffs/centaur/hero_1"
end
...
function modifier_centaur_hoof_stomp_custom_magic_reduce:GetTexture()
	return "buffs/centaur/stomp_3"
end
