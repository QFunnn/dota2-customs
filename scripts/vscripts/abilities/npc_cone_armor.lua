...
function npc_cone_armor:OnSpellStart()
	...
	self:GetCaster():EmitSound("UI.Generic_shield")
	self:GetCaster()
		:AddNewModifier(
			self:GetCaster(),
			self,
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_cone_cd", { duration = self:GetCooldownTimeRemaining() })
end

modifier_cone_armor = class({})

function modifier_cone_armor:IsHidden()
	return false
end
...
function modifier_cone_cd:IsHidden()
	return false
end
function modifier_cone_cd:IsPurgable()
	return false
end