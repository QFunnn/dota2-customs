...
LinkLuaModifier(
	"modifier_wisp_overcharge_custom_cooldown",
	"abilities/wisp_overcharge_custom",
...
function modifier_wisp_overcharge_custom:IsHidden()
	return true
end
function modifier_wisp_overcharge_custom:IsPurgable()
	return false
end
function modifier_wisp_overcharge_custom:IsPurgeException()
	return false
end
function modifier_wisp_overcharge_custom:RemoveOnDeath()
	return false
end
...
function modifier_wisp_overcharge_custom:GetModifierPreAttack_BonusDamage(params)
	...
	if not self.parent:HasModifier("modifier_wisp_overcharge") then
		return
	end
	...
	if self.parent:HasModifier("modifier_wisp_overcharge_custom_cooldown") then
		return
	end
...
function modifier_wisp_overcharge_custom:AttackLandedModifier(params)
	if IsServer() then
		...
		if params.attacker == self.parent then
			...
			if params.target:IsWard() or params.target:IsHeroWard() then
				return
			end
...
modifier_wisp_overcharge_custom_cooldown = class({})
function modifier_wisp_overcharge_custom_cooldown:IsHidden()
	return true
end
function modifier_wisp_overcharge_custom_cooldown:IsPurgeException()
	return false
end
function modifier_wisp_overcharge_custom_cooldown:IsPurgable()
	return false
end