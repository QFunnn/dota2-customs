...
function npc_golem_decay:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_decay_cd", { duration = self:GetCooldownTimeRemaining() })
...
function modifier_decay:IsHidden()
	return false
end

function modifier_decay:GetTexture()
	return "elder_titan_echo_stomp"
end

function modifier_decay:IsPurgable()
	return false
end
...
function modifier_decay:OnDestroy(table)
	...
	for i = 1, self.number do
		local new_skelet = CreateUnitByName(
			"npc_golem_zombie",
			self:GetAbility().point + RandomVector(RandomInt(-150, 150)),
			true,
			nil,
			nil,
			DOTA_TEAM_CUSTOM_5
		...
		new_skelet.host_team = self:GetCaster().host_team
...
function modifier_decay_debuff:GetTexture()
...
function modifier_decay_cd:IsHidden()
	return false
end
function modifier_decay_cd:IsPurgable()
	return false
end