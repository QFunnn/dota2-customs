...
function s.prototype.OnAbilityLearn(self, t)
	...
	local B = AbilityShop:getRandomAbility(u, self.abilityCount, { isAbilityShop = false })
	do
		local C = 0
		while C < #B do
			local D = B[C + 1]
			...
			PlayerData:getplayerData(u):addArtifactAbilities(self:GetAbility():entindex(), D.aid, C == #B - 1)
			C = C + 1
		end
	end
end
