...
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_tracker",
	"abilities/jakiro/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_fire_debuff",
	"abilities/jakiro/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_jakiro_dual_breath_custom_ice_debuff",
	"abilities/jakiro/jakiro_dual_breath_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
		...
		if IsServer() then
			...
			if name == "modifier_jakiro_dual_7" and dota1x6.current_wave >= upgrade_orange then
				caster:AddNewModifier(
					...
					self,
					"modifier_jakiro_dual_breath_custom_legendary_damage",
					{ stack = self.talents.q7_stack_max }
				)
			end
		end
	end
...
function jakiro_dual_breath_custom:OnSpellStart(new_point)
	...
	elseif new_point or self.caster:HasModifier("modifier_jakiro_innate_custom_active_frost") then
		proj_particle = wearables_system:GetParticleReplacementAbility(
			self.caster,
			"particles/jakiro/dual_breath_ice/jakiro_dual_breath_ice.vpcf",
			self.ability,
			"jakiro_dual_breath_custom"
		)
		is_ice = 1
...
function jakiro_dual_breath_custom:OnProjectileHit_ExtraData(target, vLocation, table)
	...
	if legendary_mod and legendary_mod:GetStackCount() >= self.talents.q7_max then
		return
	end
	...
	if mod then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_jakiro_dual_breath_custom_legendary_mark",
			{ duration = self.talents.q7_timer, is_ice = table.is_ice }
		)
	end
end
...
function modifier_jakiro_dual_breath_custom_fire_debuff:GetTexture()
	return "jakiro_dual_breath_fire"
end
function modifier_jakiro_dual_breath_custom_fire_debuff:IsPurgable()
	return self.ability.talents.has_q4 == 0
end
function modifier_jakiro_dual_breath_custom_fire_debuff:GetAttributes()
...
function modifier_jakiro_dual_breath_custom_ice_debuff:GetTexture()
	return "jakiro_dual_breath_ice"
end
...
function modifier_jakiro_dual_breath_custom_magic_reduce:GetTexture()
	return "buffs/jakiro/dual_3"
end
...
function modifier_jakiro_dual_breath_custom_legendary_damage:RemoveOnDeath()
	return false
end
