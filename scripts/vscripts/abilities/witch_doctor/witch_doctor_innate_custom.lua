...
LinkLuaModifier(
	"modifier_witch_doctor_innate_custom",
	"abilities/witch_doctor/witch_doctor_innate_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_witch_doctor_innate_custom_grisgris",
	"abilities/witch_doctor/witch_doctor_innate_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_witch_doctor_innate_custom_grisgris:RemoveOnDeath()
	return false
end
...
function modifier_witch_doctor_innate_custom_grisgris:DeathEvent(params)
	...
	if not unit:IsValidKill(self.parent) and not is_caster then
		return
	end

	if
		(
			unit:GetTeamNumber() ~= self.parent:GetTeamNumber()
			and (
				(params.attacker and params.attacker:FindOwner() == self.parent)
...
function modifier_witch_doctor_innate_custom_grisgris:ConsumeGold()
	...
	local Particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_witchdoctor/witchdoctor_neutral_consume.vpcf",
