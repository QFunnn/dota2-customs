...
function neutral_golem_stun:OnProjectileHit(hTarget, vLocation)
	...
	hTarget:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = self.stun * (1 - hTarget:GetStatusResistance()) }
	)
...
function modifier_golem_stun:IsPurgable()
	return false
end

function modifier_golem_stun:IsHidden()
	return true
end
...
function modifier_golem_stun:StartCast(target)
	...
	self:GetParent():AddNewModifier(
		...
		{ target = array_target, duration = 0.1, anim = ACT_DOTA_CAST_ABILITY_1, parent_mod = self:GetName() }
	)
end
