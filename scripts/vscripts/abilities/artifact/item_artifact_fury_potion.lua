...
local a = "abilities/artifact/item_artifact_fury_potion"
...
local k = c()
k.name = "item_artifact_fury_potion"
d(k, i)
...
function k.prototype.EventListener(self)
	return {
		...
		ability_cast_complete = function(l, n)
			if self:GetStackCount() > 0 and n.caster == self:GetCaster() and n.abilityTag == AbilityTag.Ultimate then
				self:DecrementStackCount()
				self:GetCaster():GiveMana(self:GetSpecialValueFor("fury"))
			end
		end,
	}
end
