...
LinkLuaModifier(
	"modifier_night_stalker_crippling_fear_custom_move",
	"abilities/night_stalker/night_stalker_crippling_fear_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function night_stalker_crippling_fear_custom:OnSpellStart()
	...
	if self.talents.has_w4 == 1 then
		...
		local effect_cast = ParticleManager:CreateParticle(
			"particles/night_stalker/fear_pull.vpcf",
	...
	if self.ability.talents.has_w7 == 1 and IsValid(self.mod) then
		...
		if self.legendary_stack >= 1 then
			...
			self.parent:AddNewModifier(
				...
				self.ability,
				"modifier_night_stalker_crippling_fear_custom_legendary_stack",
				{ duration = self.mod:GetRemainingTime() + self.ability.talents.w7_duration }
			)
		end
	end
...
function modifier_night_stalker_crippling_fear_custom_silence:FearStack()
	...
	if self.mod.fear_targets[self.parent] then
		return
	end
...
function modifier_night_stalker_crippling_fear_custom_pull_leash:OnCreated()
	...
	self.particle = ParticleManager:CreateParticle(
		"particles/night_stalker/fear_pull_leash.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self.caster
	)
	ParticleManager:SetParticleControlEnt(
