...
function night_stalker_void_custom:UpdateTalents(name)
	...
	if not self.init then
		...
		self.talents = {
			...
			q3_max = caster:GetTalentValue("modifier_stalker_void_3", "max", true) / 100,
...
function night_stalker_void_custom:GetCooldown(level)
	...
	if
		...
		and self.caster.dark_ability
		and self.caster:HasModifier("modifier_night_stalker_darkness_custom_active")
	then
...
function night_stalker_void_custom:OnSpellStart()
	...
	for _, aoe_target in pairs(self.caster:FindTargets(self.cast_radius, target:GetAbsOrigin())) do
		aoe_target:AddNewModifier(
			...
			self,
			"modifier_night_stalker_void_custom_slow",
			{ duration = duration * (1 - aoe_target:GetStatusResistance()) }
		)
...
function modifier_night_stalker_void_custom:DamageEvent_out(params)
	...
	if params.inflictor and params.inflictor:GetName() ~= "night_stalker_crippling_fear_custom" then
		return
	end
	if not self.parent:CheckCd("night_stalker_q7", self.ability.talents.q7_cd) then
		return
	end
...
function modifier_night_stalker_void_custom:UpdateUI()
	...
	self.parent:UpdateUIlong({
		stack = stack,
		override_stack = math.floor(self.ability:LegendaryDamage(stack) * 100) .. "%",
		max = self.ability.talents.q7_max,
		style = "StalkerVoid",
	})
end
...
function modifier_night_stalker_void_custom_slow:IsPurgable()
	return true
end
function modifier_night_stalker_void_custom_slow:RemoveOnDeath()
	return false
end
...
function modifier_night_stalker_void_custom_slow:OnDestroy()
	...
	if not IsValid(self.caster.fear_ability) then
		return
	end
...
function modifier_night_stalker_void_custom_legendary_thinker:OnCreated(table)
	...
	self.effect = ParticleManager:CreateParticle(
		"particles/night_stalker/void_legendary_orb.vpcf",
...
function modifier_night_stalker_void_custom_legendary_thinker:OnDestroy()
	...
	local particle =
		ParticleManager:CreateParticle("particles/night_stalker/void_legendary_projf.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_night_stalker_void_custom_legendary_thinker:OnIntervalThink()
	...
	if not self.start_init then
		...
		self.particle =
			ParticleManager:CreateParticle("particles/night_stalker/void_legendary_area.vpcf", PATTACH_WORLDORIGIN, nil)
	...
	if
		((self.caster:GetAbsOrigin() - self.point):Length2D() <= self.radius * 1.2 or self.force_ended)
		and not self.ended
	then
		self.ended = true
...
function modifier_night_stalker_void_custom_legendary_buff:OnIntervalThink()
	...
	for _, player in pairs(players) do
		if
			...
			and (player:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D()
				<= self.ability.talents.q7_check_radius
		then
...
function modifier_night_stalker_void_custom_damage_stack:OnIntervalThink()
	...
	if self.parent:GetHealthPercent() > self.ability.talents.q3_health then
		return
	end
	if not self.parent:CheckCd("night_stalker_q3", self.ability.talents.q3_cd) then
		return
	end

	self.damageTable.damage = math.min(self.ability.talents.q3_max * self.parent:GetMaxHealth(), self.stack)
...
function modifier_night_stalker_void_custom_magic:GetTexture()
	return "buffs/night_stalker/void_3"
end
