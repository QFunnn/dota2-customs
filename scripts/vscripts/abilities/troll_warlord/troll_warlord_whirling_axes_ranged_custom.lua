...
LinkLuaModifier(
	"modifier_troll_warlord_whirling_axes_ranged_custom",
	"abilities/troll_warlord/troll_warlord_whirling_axes_ranged_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_troll_warlord_whirling_axes_ranged_quest",
	"abilities/troll_warlord/troll_warlord_whirling_axes_ranged_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_troll_warlord_whirling_axes_silence_cd",
	"abilities/troll_warlord/troll_warlord_whirling_axes_ranged_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function troll_warlord_whirling_axes_ranged_custom:OnSpellStart(new_target)
	...
	if
		caster:HasModifier("modifier_troll_warlord_berserkers_rage_custom")
...
function troll_warlord_whirling_axes_ranged_custom:OnProjectileHit_ExtraData(target, location, ExtraData)
	...
	if not self.axes[index] or not self.axes[index].targets then
		return
	end

	if target then
		if self.axes[index].targets[target] then
			return
		end
		...
		if
			caster:HasTalent("modifier_troll_axes_5")
			and not target:HasModifier("modifier_troll_warlord_whirling_axes_silence_cd")
		then
			target:AddNewModifier(
				caster,
				self,
				"modifier_troll_warlord_whirling_axes_silence_cd",
				{ duration = caster:GetTalentValue("modifier_troll_axes_5", "cd") }
			)

			local vec = (target:GetAbsOrigin() - caster:GetAbsOrigin())
			local dist =
				math.max(0, math.min(vec:Length2D() - 120, caster:GetTalentValue("modifier_troll_axes_5", "distance")))
			...
			target:AddNewModifier(
				...
				{ sound = "Sf.Raze_Silence", duration = (1 - target:GetStatusResistance()) * silence }
			)
		end
...
function modifier_troll_warlord_whirling_axes_ranged_custom:IsHidden()
	return false
end
function modifier_troll_warlord_whirling_axes_ranged_custom:IsPurgable()
	return not self:GetCaster():HasScepter()
end
function modifier_troll_warlord_whirling_axes_ranged_custom:GetTexture()
...
function modifier_troll_warlord_whirling_axes_ranged_custom:OnCreated()
	...
	self.slow = self.ability:GetSpecialValueFor("movement_speed") * -1
		+ self.caster:GetTalentValue("modifier_troll_axes_5", "slow")
end
...
function modifier_troll_warlord_whirling_axes_ranged_quest:IsHidden()
	return true
end
function modifier_troll_warlord_whirling_axes_ranged_quest:IsPurgable()
	return false
end

modifier_troll_warlord_whirling_axes_silence_cd = class({})
function modifier_troll_warlord_whirling_axes_silence_cd:IsHidden()
	return true
end
function modifier_troll_warlord_whirling_axes_silence_cd:IsPurgable()
	return false
end
function modifier_troll_warlord_whirling_axes_silence_cd:RemoveOnDeath()
	return false
end