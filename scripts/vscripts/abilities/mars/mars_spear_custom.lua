...
function mars_spear_custom:UpdateTalents(name)
	...
	if not self.init then
		...
		self.talents = {
			...
			q7_distance = caster:GetTalentValue("modifier_mars_spear_7", "distance", true),
...
function mars_spear_custom:GetRange()
	return self.spear_range
		+ self.caster:GetCastRangeBonus()
		+ (self.talents.has_q7 == 1 and self.talents.q7_distance or 0)
end
...
function mars_spear_custom:OnSpellStart()
	...
	if self.talents.has_q7 == 1 then
		...
		local nFXIndex = ParticleManager:CreateParticle(
			"particles/mars/spear_legendary_start.vpcf",
		...
		for _, target in pairs(self.caster:FindTargets(self.talents.q7_radius)) do
			...
			target:AddNewModifier(
				...
				{ duration = (1 - target:GetStatusResistance()) * self.talents.q7_slow_duration }
			)
		end
		...
		self.caster:AddNewModifier(
			self.caster,
			self,
			"modifier_mars_spear_custom_legendary",
			{ duration = self.talents.q7_cast }
		)
...
function mars_spear_custom:OnProjectileHit_ExtraData(target, location, table)
	...
	if self.talents.has_q3 == 1 then
		target:AddNewModifier(
			...
			{ duration = self.talents.q3_timer }
		)
	end
...
function mars_spear_custom:OnProjectileThink_ExtraData(vLocation, table)
	...
	if not self.projectiles[index].entindex then
		return
	end
...
function mars_spear_custom:Pinned(index, is_arena)
	...
	if not self.projectiles[index] then
		return
	end
	if not self.projectiles[index].entindex then
		return
	...
	local unit = EntIndexToHScript(self.projectiles[index].entindex)
	local direction = self.projectiles[index].direction
	local duration = (self.stun_duration + (self.talents.has_h4 == 1 and self.talents.h4_stun or 0))
...
function mars_spear_custom:EndSpear(index)
	...
	if self.projectiles[index].entindex then
		...
		spawn_point = unit:GetAbsOrigin() - dir * 200
	...
	if IsValid(self.caster.arena_ability) then
		self.caster.arena_ability:SpawnSoldier(spawn_point)
	end
	...
	CreateModifierThinker(
		...
		"modifier_mars_spear_custom_trail_thinker",
		{
			...
			x = start_point.x,
			y = start_point.y,
			legendary_k = self.projectiles[index].legendary_k,
		},
		end_point,
		self.caster:GetTeamNumber(),
		false
...
function modifier_mars_spear_custom_debuff:StatusEffectPriority()
...
function modifier_mars_spear_custom_debuff:OnDestroy()
	...
	if self.parent:IsAlive() and self.ability.talents.has_h4 == 1 then
		...
		self.parent:AddNewModifier(
			...
			{ duration = (1 - self.parent:GetStatusResistance()) * self.ability.talents.h4_silence }
		)
	end
...
function modifier_mars_spear_custom_legendary:OnCreated(kv)
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		"ability_mars_spear",
		{ state = 1, range = self.distance, width = self.width }
	)
...
function modifier_mars_spear_custom_legendary:OnDestroy()
	...
	self.ability:LaunchSpear(
		self.parent:GetAbsOrigin(),
		self.parent:GetForwardVector() * 10 + self.parent:GetAbsOrigin(),
		self.stack
	)
end
...
function modifier_mars_spear_custom_trail_thinker:OnIntervalThink()
	...
	for _, target in pairs(targets) do
		...
		target:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_mars_spear_custom_trail_burn",
...
function modifier_mars_spear_custom_delay:GetTexture()
	return "buffs/mars/spear_3"
end
...
function modifier_mars_spear_custom_delay:OnDestroy()
	...
	self.caster:GenericHeal(
		result * self.ability.talents.q3_heal * real_damage,
		self.ability,
		false,
...
function modifier_mars_spear_custom_slow:IsPurgable()
	return true
end
...
function modifier_mars_spear_custom_hit_speed:GetTexture()
	return "buffs/mars/spear_4"
end
