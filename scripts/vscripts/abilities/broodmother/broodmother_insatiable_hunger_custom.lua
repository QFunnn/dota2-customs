...
function modifier_broodmother_insatiable_hunger_custom:OnIntervalThink()
	...
	if
		target
		and not self.parent:GetAttackTarget()
		and (target:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= max_range
	then
		if
			not self.parent:HasModifier("modifier_broodmother_insatiable_hunger_custom_rush_speed")
			and (target:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() >= 600
		then
