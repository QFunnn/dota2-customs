...
LinkLuaModifier(
	"modifier_broodmother_incapacitating_bite_custom_tracker",
...
function modifier_broodmother_incapacitating_bite_custom_tracker:UpdateUI()
	...
	self.parent:UpdateUIshort({
		...
		time = time_active,
		stack = "+" .. self.ability.talents.e7_damage .. "%",
		hide = hide_active,
		hide_full = hide_active,
		style = "BroodBite",
	})
end
...
function modifier_broodmother_incapacitating_bite_custom_tracker:GetModifierPhysicalArmorBonus()
	return self.ability.talents.h2_armor
		* (self.parent:GetHealthPercent() <= self.ability.talents.h2_health and self.ability.talents.h2_bonus or 1)
end
...
function modifier_broodmother_incapacitating_bite_custom:GetTexture()
...
function modifier_broodmother_incapacitating_bite_custom:OnCreated()
	...
	if self.ability.talents.has_e3 == 0 and self.ability.talents.has_e7 == 0 and not self.quest then
		return
	end
...
function modifier_broodmother_incapacitating_bite_custom:OnIntervalThink(first)
	...
	if self.ability.talents.has_e3 == 1 then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_broodmother_incapacitating_bite_custom_armor",
			{ duration = self.ability.talents.e3_duration, interval = self.interval }
		)
	end
	...
	if
		...
		and not self.parent:HasModifier("modifier_broodmother_incapacitating_bite_custom_legendary_active")
	then
		self.parent:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_broodmother_incapacitating_bite_custom_legendary_stack",
			{ duration = self.ability.talents.e7_linger, interval = self.interval }
		)
	end
end
...
function modifier_broodmother_incapacitating_bite_custom_speed:GetTexture()
	return "buffs/broodmother/bite_1"
end
...
function modifier_broodmother_incapacitating_bite_custom_armor:GetTexture()
	return "buffs/broodmother/bite_3"
end
...
function modifier_broodmother_incapacitating_bite_custom_armor:AddStack()
	...
	if self.count < self.max_timer then
		return
	end
	self.count = 0
...
function modifier_broodmother_incapacitating_bite_custom_armor:GetModifierPreAttack_Target_CriticalStrike(params)
	...
	if not RollPseudoRandomPercentage(self.ability.talents.e3_chance, 5121, params.attacker) then
		return
	end
