...
function modifier_npc_necro_range_passive:IsHidden()
	return true
end
function modifier_npc_necro_range_passive:IsPurgable()
	return false
end
...
function modifier_npc_necro_range_passive:GetModifierProcAttack_Feedback(params)
	...
	if params.target:IsBuilding() or params.target:IsMagicImmune() then
		return
	end
	...
	params.target:AddNewModifier(
		...
		{ duration = self.slow_duration * (1 - params.target:GetStatusResistance()) }
	)
...
function modifier_npc_necro_range_slow:IsHidden()
	return false
end
