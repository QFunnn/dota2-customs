...
LinkLuaModifier(
	"modifier_puck_dream_coil_custom_thinker_scepter",
	"abilities/puck_dream_coil_custom",
...
function puck_dream_coil_custom:OnSpellStart()
	...
	local coil_thinker = CreateModifierThinker(
		self:GetCaster(),
		self,
		"modifier_puck_dream_coil_custom_thinker",
	...
	if self:GetCaster():HasScepter() then
		CreateModifierThinker(
			...
			self,
			"modifier_puck_dream_coil_custom_thinker_scepter",
			{ duration = latch_duration + coil_stun_duration + 1 },
			self:GetCursorPosition(),
			self:GetCaster():GetTeamNumber(),
			false
		)
	...
	local enemies = FindUnitsInRadius(
		...
		self:GetCursorPosition(),
		nil,
		self:GetSpecialValueFor("coil_radius"),
	...
	for _, enemy in pairs(enemies) do
		...
		if not enemy:HasModifier("modifier_puck_dream_coil_custom") then
			enemy:AddNewModifier(
				self:GetCaster(),
				self,
				"modifier_puck_dream_coil_custom",
...
function modifier_puck_dream_coil_custom:IsPurgable()
	return false
end
function modifier_puck_dream_coil_custom:GetAttributes()
...
function modifier_puck_dream_coil_custom:OnCreated(params)
	...
	self.leash = self:GetParent():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_puck_dream_coil_custom_tether",
...
function modifier_puck_dream_coil_custom:OnIntervalThink()
	...
	if (self:GetParent():GetAbsOrigin() - self.coil_thinker_location):Length2D() >= self.coil_break_radius then
		...
		self:GetParent()
			:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_stunned", { duration = stun_duration })
		self:GetParent():AddNewModifier(
			self:GetCaster(),
			self:GetAbility(),
			"modifier_puck_dream_coil_custom_stun",
			{ duration = (1 - self:GetParent():GetStatusResistance()) * stun_duration }
		)
...
function modifier_puck_dream_coil_custom_tether:IsHidden()
	return true
end
function modifier_puck_dream_coil_custom_tether:IsPurgable()
	return false
end
function modifier_puck_dream_coil_custom_tether:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_puck_dream_coil_custom_thinker_scepter:IsHidden()
	return true
end
function modifier_puck_dream_coil_custom_thinker_scepter:IsPurgable()
	return false
end
...
function modifier_puck_dream_coil_custom_thinker_scepter:OnIntervalThink()
	...
	for _, target in pairs(self.targets) do
		if
			...
			and (target:HasModifier("modifier_puck_dream_coil_custom") or target:HasModifier(
				"modifier_puck_dream_coil_custom_stun"
			))
...
function modifier_puck_dream_coil_custom_stun:IsHidden()
	return true
end
function modifier_puck_dream_coil_custom_stun:IsPurgable()
	return false
end
function modifier_puck_dream_coil_custom_stun:IsPurgeException()
	return true
end
function modifier_puck_dream_coil_custom_stun:IsStunDebuff()
	return true
end