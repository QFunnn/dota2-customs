...
function sniper_assassinate_custom:OnProjectileHit_ExtraData(target, location, extradata)
	...
	if self:GetCaster():HasScepter() then
		damage = damage
			+ (self:GetCaster():GetAverageTrueAttackDamage(nil) / 100 * self:GetSpecialValueFor("scepter_crit"))
	...
	target:AddNewModifier(
		...
		self,
		"modifier_stunned",
		{ duration = self:GetSpecialValueFor("scepter_stun_duration") * (1 - target:GetStatusResistance()) }
	)
