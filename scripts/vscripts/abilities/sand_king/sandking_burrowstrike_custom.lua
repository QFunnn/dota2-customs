...
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_stun",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_stun_second",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_stun_attacks",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_tracker",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_legendary_cd",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_second",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_speed",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_reverse",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_immune",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_quest",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_burrowstrike_custom_leash",
	"abilities/sand_king/sandking_burrowstrike_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sandking_burrowstrike_custom:GetCooldown(level)
	local bonus = 0
	if self:GetCaster():HasTalent("modifier_sand_king_burrow_6") then
		bonus = self:GetCaster():GetTalentValue("modifier_sand_king_burrow_6", "cd")
	end
...
function sandking_burrowstrike_custom:OnSpellStart(new_target)
	...
	local max_distance = self:GetSpecialValueFor("AbilityCastRange") + self:GetCaster():GetCastRangeBonus()
	...
	if self:GetCaster():HasTalent("modifier_sand_king_burrow_5") then
		self.anim_time = self.anim_time
			* (1 - self:GetCaster():GetTalentValue("modifier_sand_king_burrow_5", "speed") / 100)
	...
	if caster:HasTalent("modifier_sand_king_burrow_5") then
		ProjectileManager:ProjectileDodge(self:GetCaster())
		caster:AddNewModifier(
			caster,
			self,
			"modifier_sandking_burrowstrike_custom_immune",
			{ duration = caster:GetTalentValue("modifier_sand_king_burrow_5", "duration") + delay }
		)
	end
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_sandking_burrowstrike_custom",
		{ duration = duration, delay = delay, pos_x = point.x, pos_y = point.y, pos_z = point.z, reverse = 0 }
	)

	if caster:HasTalent("modifier_sand_king_burrow_6") then
		caster:AddNewModifier(
			caster,
			self,
			"modifier_sandking_burrowstrike_custom_reverse",
			{ duration = caster:GetTalentValue("modifier_sand_king_burrow_6", "duration") }
		)
	end
...
function sandking_burrowstrike_custom:OnProjectileHit(target, location)
	...
	local duration = self:GetSpecialValueFor("burrow_duration")
		+ caster:GetTalentValue("modifier_sand_king_burrow_1", "stun")
...
function modifier_sandking_burrowstrike_custom:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom:IsPurgable()
	return false
end
function modifier_sandking_burrowstrike_custom:OnCreated(kv)
	...
	self.caster:StartGesture(ACT_DOTA_SAND_KING_BURROW_IN)
...
function modifier_sandking_burrowstrike_custom:Teleport()
	...
	if self.reverse == 0 then
		if self.caster:HasTalent("modifier_sand_king_burrow_4") then
			self.caster:RemoveModifierByName("modifier_sandking_burrowstrike_custom_second")
			self.caster:AddNewModifier(
				self.caster,
				self:GetAbility(),
				"modifier_sandking_burrowstrike_custom_second",
				{ duration = self.caster:GetTalentValue("modifier_sand_king_burrow_4", "delay") }
			)
		end

		if self.caster:HasTalent("modifier_sand_king_burrow_3") then
			self.caster:AddNewModifier(
				self.caster,
				self:GetAbility(),
				"modifier_sandking_burrowstrike_custom_speed",
				{ duration = self.caster:GetTalentValue("modifier_sand_king_burrow_3", "duration") }
			)
		end
	end

	self.caster:StartGesture(ACT_DOTA_OVERRIDE_ABILITY_1)
	self.caster:FadeGesture(ACT_DOTA_SAND_KING_BURROW_IN)
	FindClearSpaceForUnit(self.caster, self.point, true)

	local sand = self.caster:FindAbilityByName("sandking_sand_storm_custom")
	if sand and sand:IsTrained() and self.caster:HasTalent("modifier_sand_king_sand_7") then
		sand:ProcLegendary(self.caster:GetAbsOrigin())
	end

	local face_dir = self.caster:GetForwardVector()
	face_dir.z = 0
	self.caster:FaceTowards(self.caster:GetAbsOrigin() + face_dir * 10)
	self.caster:SetForwardVector(face_dir)
...
function modifier_sandking_burrowstrike_custom_stun:IsPurgable()
	return false
end
function modifier_sandking_burrowstrike_custom_stun:IsPurgeException()
	return true
end
function modifier_sandking_burrowstrike_custom_stun:IsStunDebuff()
	return true
end
...
function modifier_sandking_burrowstrike_custom_stun:OnCreated(table)
	...
	self.damage = self:GetAbility():GetAbilityDamage()
		+ self.caster:GetTalentValue("modifier_sand_king_burrow_2", "damage")
	...
	self.mod = self.parent:AddNewModifier(self.caster, self:GetAbility(), "modifier_knockback", {
...
function modifier_sandking_burrowstrike_custom_stun:OnDestroy()
	...
	if self.caster:HasTalent("modifier_sand_king_burrow_5") then
		self.parent:AddNewModifier(
			...
			"modifier_sandking_burrowstrike_custom_leash",
			{
				duration = (1 - self.parent:GetStatusResistance())
					* self.caster:GetTalentValue("modifier_sand_king_burrow_5", "leash"),
			}
		)
	end
...
function sandking_burrowstrike_custom_legendary:CreateTalent()
...
function sandking_burrowstrike_custom_legendary:OnAbilityPhaseStart()
	self.particle_sandblast_fx = ParticleManager:CreateParticle(
		"particles/sand_king/burrow_legendary_start.vpcf",
...
function sandking_burrowstrike_custom_legendary:OnSpellStart()
	...
	local ability = caster:FindAbilityByName("sandking_burrowstrike_custom_legendary_exit")

	if ability:IsHidden() then
		caster:SwapAbilities(
			"sandking_burrowstrike_custom_legendary_exit",
			"sandking_burrowstrike_custom_legendary",
			true,
			false
		)
	end

	caster:AddNewModifier(caster, self, "modifier_sandking_burrowstrike_custom_legendary", {})
end

modifier_sandking_burrowstrike_custom_legendary = class({})
function modifier_sandking_burrowstrike_custom_legendary:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_legendary:IsPurgable()
	return false
end
function modifier_sandking_burrowstrike_custom_legendary:OnCreated()
	...
	if not self.caster:HasTalent("modifier_sand_king_burrow_7") then
		self:Destroy()
		return
	end
	...
	self.shield_talent = "modifier_sand_king_burrow_7"
	self.max_shield = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "shield")
		* self.caster:GetMaxHealth()
		/ 100
	self.restore = (self.max_shield / self.caster:GetTalentValue("modifier_sand_king_burrow_7", "restore_timer"))
		* self.interval
	self.speed = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "speed")
	self.attack_cd = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "attack_cd")
	self.radius = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "radius")
	self.attack_stun = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "attack_stun")
	self.damage = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "damage")

	self.speed_bonus = self.caster:GetTalentValue("modifier_sand_king_burrow_3", "speed")
	self.speed_active = self.caster:GetTalentValue("modifier_sand_king_burrow_3", "bonus")

	self.damage_reduce = self:GetAbility():GetSpecialValueFor("damage") / 100
	...
	self.active_effect = ParticleManager:CreateParticle(
		"particles/sand_king/burrow_legendary_active.vpcf",
		PATTACH_ABSORIGIN_FOLLOW,
		self:GetParent()
	)
	self:AddParticle(self.active_effect, false, false, -1, false, false)

	self.ability = self.caster:FindAbilityByName("sandking_burrowstrike_custom")

	self.caster:EmitSound("SandKing.Burrow_loop")

	self.caster:EmitSound("SandKing.Burrow_start")
	...
	self.attack_max = self.caster:GetTalentValue("modifier_sand_king_burrow_7", "attack_cd")
...
function modifier_sandking_burrowstrike_custom_legendary:OnIntervalThink()
	...
	if self.effect_count >= self.effect_max and self.caster:IsMoving() then
		...
		local effect_cast = ParticleManager:CreateParticle(
			"particles/sand_king/sand_king_wave.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			self.caster
		)
	...
	if self.attack_count >= self.attack_max then
		...
		for _, target in pairs(targets) do
			...
			if
				not target:IsCurrentlyHorizontalMotionControlled() and not target:IsCurrentlyVerticalMotionControlled()
			then
				target:AddNewModifier(self.caster, self:GetAbility(), "modifier_knockback", {
...
function modifier_sandking_burrowstrike_custom_legendary:OnDestroy()
	...
	self.caster:StopSound("SandKing.Burrow_loop")

	self.caster:EmitSound("SandKing.Burrow_end")
	...
	caster:SetAbsOrigin(self.caster:GetAbsOrigin() + Vector(0, 0, -300))
	...
	local ability = self.caster:FindAbilityByName("sandking_burrowstrike_custom_legendary")
...
function modifier_sandking_burrowstrike_custom_legendary:GetModifierDamageOutgoing_Percentage()
	return self.damage
end
...
function modifier_sandking_burrowstrike_custom_legendary:GetModifierIncomingDamageConstant(params)
	...
	local damage = math.min(params.damage * self.damage_reduce, self:GetStackCount())
...
function modifier_sandking_burrowstrike_custom_second:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_second:IsPurgable()
	return false
end
...
function modifier_sandking_burrowstrike_custom_stun_second:IsPurgable()
	return false
end
function modifier_sandking_burrowstrike_custom_stun_second:IsPurgeException()
	return true
end
function modifier_sandking_burrowstrike_custom_stun_second:IsStunDebuff()
	return true
end
...
function modifier_sandking_burrowstrike_custom_stun_second:OnCreated(table)
	...
	self.mod = self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_knockback", {
...
function modifier_sandking_burrowstrike_custom_stun_attacks:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_stun_attacks:IsPurgable()
	return false
end
...
function modifier_sandking_burrowstrike_custom_speed:IsHidden()
	return false
end
function modifier_sandking_burrowstrike_custom_speed:IsPurgable()
	return true
end
function modifier_sandking_burrowstrike_custom_speed:GetTexture()
	return "buffs/burrow_speed"
end
...
function modifier_sandking_burrowstrike_custom_reverse:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_reverse:IsPurgable()
	return false
end
...
function sandking_burrowstrike_custom_reverse:OnSpellStart()
	...
	local max_distance = self:GetCaster():GetTalentValue("modifier_sand_king_burrow_6", "range")
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_sandking_burrowstrike_custom",
		{ duration = duration, delay = delay, pos_x = point.x, pos_y = point.y, pos_z = point.z, reverse = 1 }
	)
...
function modifier_sandking_burrowstrike_custom_tracker:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_tracker:IsPurgable()
	return false
end
...
function modifier_sandking_burrowstrike_custom_tracker:GetModifierDamageOutgoing_Percentage()
	if not self.caster:HasTalent("modifier_sand_king_burrow_2") then
		return
	end
	return self.caster:GetTalentValue("modifier_sand_king_burrow_2", "damage_auto")
end

function modifier_sandking_burrowstrike_custom_tracker:GetModifierCastRangeBonusStacking()
	if not self.caster:HasTalent("modifier_sand_king_burrow_1") then
		return
	end
	return self.caster:GetTalentValue("modifier_sand_king_burrow_1", "range")
end

function modifier_sandking_burrowstrike_custom_tracker:GetModifierMoveSpeedBonus_Constant()
	if not self.caster:HasTalent("modifier_sand_king_burrow_3") then
		return
	end
...
function modifier_sandking_burrowstrike_custom_immune:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_immune:IsPurgable()
	return false
end
...
function modifier_sandking_burrowstrike_custom_quest:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_quest:IsPurgable()
	return false
end

modifier_sandking_burrowstrike_custom_leash = class({})
function modifier_sandking_burrowstrike_custom_leash:IsHidden()
	return true
end
function modifier_sandking_burrowstrike_custom_leash:IsPurgable()
	return false
end
function modifier_sandking_burrowstrike_custom_leash:CheckState()
	return {
		[MODIFIER_STATE_TETHERED] = true,
	}
end

function modifier_sandking_burrowstrike_custom_leash:GetStatusEffectName()
	return "particles/status_fx/status_effect_mars_spear.vpcf"
end

function modifier_sandking_burrowstrike_custom_leash:StatusEffectPriority()
	return MODIFIER_PRIORITY_HIGH
end

function modifier_sandking_burrowstrike_custom_leash:OnCreated()
	self.speed = self:GetCaster():GetTalentValue("modifier_sand_king_burrow_5", "slow")
end

function modifier_sandking_burrowstrike_custom_leash:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
	}
end

function modifier_sandking_burrowstrike_custom_leash:GetModifierMoveSpeedBonus_Percentage()
	return self.speed
end

function modifier_sandking_burrowstrike_custom_leash:GetEffectName()
	return "particles/units/heroes/hero_sniper/sniper_headshot_slow.vpcf"
end

function modifier_sandking_burrowstrike_custom_leash:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end