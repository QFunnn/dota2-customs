...
LinkLuaModifier(
	"modifier_sandking_caustic_finale_custom",
	"abilities/sand_king/sandking_caustic_finale_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_caustic_finale_custom_slow",
	"abilities/sand_king/sandking_caustic_finale_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_caustic_finale_custom_legendary",
	"abilities/sand_king/sandking_caustic_finale_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sandking_caustic_finale_custom_lowhp",
	"abilities/sand_king/sandking_caustic_finale_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sandking_caustic_finale_custom:OnSpellStart()
	...
	for i = 1, 3 do
		local particle = ParticleManager:CreateParticle(
			"particles/sand_king/finale_legendary_hit.vpcf",
	...
	target:AddNewModifier(
		...
		self,
		"modifier_sandking_caustic_finale_custom_legendary",
		{ duration = self:GetCaster():GetTalentValue("modifier_sand_king_finale_7", "duration") }
	)
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_sandking_caustic_finale_custom_legendary_caster",
		{ duration = self:GetCaster():GetTalentValue("modifier_sand_king_finale_7", "duration") }
	)
end
...
function sandking_caustic_finale_custom:ApplyEffect(target)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_sandking_caustic_finale_custom_slow",
		{ duration = self:GetSpecialValueFor("caustic_finale_duration") }
	)
end
...
function modifier_sandking_caustic_finale_custom:IsHidden()
	return not self:GetParent():HasTalent("modifier_sand_king_finale_5")
		or self:GetParent():GetHealthPercent() > self.low_health
end

function modifier_sandking_caustic_finale_custom:GetTexture()
	return "buffs/finale_lowhp"
end
function modifier_sandking_caustic_finale_custom:IsPurgable()
	return false
end
...
function modifier_sandking_caustic_finale_custom:GetModifierIncomingDamage_Percentage()
	if not self.caster:HasTalent("modifier_sand_king_finale_5") then
		return
	end
...
function modifier_sandking_caustic_finale_custom_slow:IsHidden()
	return true
end
function modifier_sandking_caustic_finale_custom_slow:IsPurgable()
	return true
end
function modifier_sandking_caustic_finale_custom_slow:GetTexture()
...
function modifier_sandking_caustic_finale_custom_slow:OnStackCountChanged(iStackCount)
	...
	if self.parent:HasModifier("modifier_sandking_caustic_finale_custom_legendary") then
		return
	end

	if
		self:GetStackCount() > 0
		and not self.parent:HasModifier("modifier_sandking_scorpion_strike_custom_legendary_stack")
	then
		self:ShowParticle()
	else
		self:HideParticle()
