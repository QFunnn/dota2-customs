...
LinkLuaModifier(
	...
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_debuff",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_debuff_count",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_quest",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_tracker",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_toggle",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_heal",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_damage_reduce",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_agility",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_agility_cd",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_range",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_break",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_templar_assassin_meld_custom_legendary",
	"abilities/templar_assasssin/templar_assassin_meld_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function templar_assassin_meld_custom:GetCastRange(vLocation, hTarget)
	local caster = self:GetCaster()
	if
		caster:HasTalent("modifier_templar_assassin_meld_5")
		and caster:HasModifier("modifier_templar_assassin_meld_custom_toggle")
	then
		if IsClient() then
...
function templar_assassin_meld_custom:OnSpellStart()
	...
	if caster:HasTalent("modifier_templar_assassin_meld_5") then
		...
		if
			caster:HasModifier("modifier_templar_assassin_meld_custom_toggle")
...
function modifier_templar_assassin_meld_custom_buff:IsPurgable()
	return false
end
...
function modifier_templar_assassin_meld_custom_buff:OnDestroy()
	...
	if self.parent:GetQuest() == "Templar.Quest_6" then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_templar_assassin_meld_custom_quest",
			{ duration = self.parent.quest.number }
		)
	end
	...
	if self.parent:HasTalent("modifier_templar_assassin_meld_5") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_templar_assassin_meld_custom_range",
			{ duration = self.parent:GetTalentValue("modifier_templar_assassin_meld_5", "duration") }
		)
	end

	if self.parent:HasTalent("modifier_templar_assassin_meld_7") then
		local duration = self.legendary_min
			+ (self.legendary_max - self.legendary_min) * (self:GetElapsedTime() / self.max_time)
...
function modifier_templar_assassin_meld_custom_buff:GetModifierMoveSpeedBonus_Percentage()
	if not self.parent:HasTalent("modifier_templar_assassin_meld_7") then
		return
	end
...
function modifier_templar_assassin_meld_custom_buff:ProcAttack(params)
	...
	if self.parent:HasTalent("modifier_templar_assassin_meld_2") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_templar_assassin_meld_custom_heal",
			{ duration = self.parent:GetTalentValue("modifier_templar_assassin_meld_2", "duration") }
		)
	end
...
function modifier_templar_assassin_meld_custom_buff:OrderEvent(params)
	...
	if params.ability and params.ability:GetAbilityName() == "templar_assassin_trap_teleport_custom" then
		return
	end
	if not self.cancel_commands[params.order_type] then
		return
	end
...
function modifier_templar_assassin_meld_custom_buff:OnIntervalThink()
	...
	if self.parent:GetAbsOrigin() == self.abs then
		return
	end
	if self.attacked then
		return
...
function modifier_templar_assassin_meld_custom_tracker:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_tracker:IsPurgable()
	return false
end
...
function modifier_templar_assassin_meld_custom_tracker:GetModifierBonusStats_Agility()
	if not self.parent:HasTalent("modifier_templar_assassin_meld_1") then
		return
	end
...
function modifier_templar_assassin_meld_custom_tracker:GetModifierHealthBonus()
	if not self.parent:HasTalent("modifier_templar_assassin_meld_2") then
		return
	end
...
function modifier_templar_assassin_meld_custom_tracker:GetModifierAttackRangeBonus()
	if not self.parent:HasTalent("modifier_templar_assassin_meld_5") then
		return
	end
	if
		not self.parent:HasModifier("modifier_templar_assassin_meld_custom_range")
		and not self.parent:HasModifier("modifier_templar_assassin_meld_custom_buff")
	then
		return
	end
...
function modifier_templar_assassin_meld_custom_tracker:OnIntervalThink()
	...
	if not self.parent:HasTalent("modifier_templar_assassin_meld_4") then
		return
	end
	if self.parent:HasModifier("modifier_templar_assassin_meld_custom_agility_cd") then
		return
	end
	if self.parent:HasModifier("modifier_templar_assassin_meld_custom_agility") then
		return
	end
...
function modifier_templar_assassin_meld_custom_tracker:AttackEvent_out(params)
	...
	local damage = self.ability:GetSpecialValueFor("bonus_damage")
		+ self.parent:GetTalentValue("modifier_templar_assassin_meld_4", "damage") * self.parent:GetAgility() / 100
	...
	if self.parent:HasTalent("modifier_templar_assassin_meld_3") then
		target:AddNewModifier(
			...
			self.ability,
			"modifier_templar_assassin_meld_custom_damage_reduce",
			{ duration = self.damage_reduce_duration }
		)
	end
	...
	if self.parent:HasTalent("modifier_templar_assassin_meld_6") and type == 0 then
		...
		target:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = (1 - target:GetStatusResistance()) * self.attack_stun }
		)
		...
		if target:GetHealthPercent() <= self.attack_health then
			target:AddNewModifier(
				...
				self.ability,
				"modifier_templar_assassin_meld_custom_break",
				{ duration = (1 - target:GetStatusResistance()) * self.attack_break }
			)
		end
	end
...
function modifier_templar_assassin_meld_custom_debuff:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_debuff:OnCreated(table)
	...
	self.armor = self.ability:GetSpecialValueFor("bonus_armor")
		+ self.caster:GetTalentValue("modifier_templar_assassin_meld_1", "armor")
...
function modifier_templar_assassin_meld_custom_debuff_count:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_templar_assassin_meld_custom_debuff_count:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_debuff_count:IsHidden()
	return true
end
...
function modifier_templar_assassin_meld_custom_quest:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_quest:IsPurgable()
	return false
end

modifier_templar_assassin_meld_custom_toggle = class({})
function modifier_templar_assassin_meld_custom_toggle:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_toggle:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_toggle:RemoveOnDeath()
	return false
end

modifier_templar_assassin_meld_custom_heal = class({})

function modifier_templar_assassin_meld_custom_heal:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_heal:GetTexture()
	return "buffs/Crit_blood"
end
...
function modifier_templar_assassin_meld_custom_damage_reduce:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_damage_reduce:IsPurgable()
	return false
end
...
function modifier_templar_assassin_meld_custom_agility:IsHidden()
	return false
end
function modifier_templar_assassin_meld_custom_agility:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_agility:GetTexture()
...
function modifier_templar_assassin_meld_custom_agility_cd:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_agility_cd:IsPurgable()
	return false
end

modifier_templar_assassin_meld_custom_range = class({})
function modifier_templar_assassin_meld_custom_range:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_range:IsPurgable()
	return false
end

modifier_templar_assassin_meld_custom_break = class({})
function modifier_templar_assassin_meld_custom_break:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_break:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_templar_assassin_meld_custom_break:GetEffectName()
...
function modifier_templar_assassin_meld_custom_legendary:IsHidden()
	return true
end
function modifier_templar_assassin_meld_custom_legendary:IsPurgable()
	return false
end
function modifier_templar_assassin_meld_custom_legendary:GetEffectName()
	return "particles/templar_assassin/meld_buff.vpcf"
end
