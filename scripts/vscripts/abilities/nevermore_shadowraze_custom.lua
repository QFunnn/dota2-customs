...
LinkLuaModifier(
	"modifier_nevermore_shadowraze_custom_debuff",
	"abilities/nevermore_shadowraze_custom",
...
function CastShadowRazeOnPoint(caster, ability, point, radius, multicast)
	if multicast == nil then
		...
		if
			...
			and ability ~= nevermore_shadowraze1_custom
			and nevermore_shadowraze1_custom:GetCurrentAbilityCharges() > 0
		then
			nevermore_shadowraze1_custom:SetCurrentAbilityCharges(
				nevermore_shadowraze1_custom:GetCurrentAbilityCharges() - 1
			)
		end
		local nevermore_shadowraze2_custom = caster:FindAbilityByName("nevermore_shadowraze2_custom")
		if
			nevermore_shadowraze2_custom
			and ability ~= nevermore_shadowraze2_custom
			and nevermore_shadowraze2_custom:GetCurrentAbilityCharges() > 0
		then
			nevermore_shadowraze2_custom:SetCurrentAbilityCharges(
				nevermore_shadowraze2_custom:GetCurrentAbilityCharges() - 1
			)
		end
		local nevermore_shadowraze3_custom = caster:FindAbilityByName("nevermore_shadowraze3_custom")
		if
			nevermore_shadowraze3_custom
			and ability ~= nevermore_shadowraze3_custom
			and nevermore_shadowraze3_custom:GetCurrentAbilityCharges() > 0
		then
			nevermore_shadowraze3_custom:SetCurrentAbilityCharges(
				nevermore_shadowraze3_custom:GetCurrentAbilityCharges() - 1
			)
		end
	end
...
function modifier_nevermore_shadowraze_custom_debuff:IsDebuff()
	return true
end
