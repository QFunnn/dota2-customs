...
LinkLuaModifier(
	...
	"abilities/skeleton_king_vampiric_aura_custom.lua",
...
function skeleton_king_vampiric_aura_custom:OnSpellStart()
	...
	for i = 1, self:GetSpecialValueFor("min_skeleton_spawn") do
		Timers:CreateTimer(delay * i, function()
			self:CreateSkeleton(
				self:GetCaster():GetOrigin() + RandomVector(300),
				nil,
				nil,
				true,
...
function skeleton_king_vampiric_aura_custom:CreateSkeleton(origin, target, duration_custom, reincarnation, name_unit)
	...
	local skelet =
		CreateUnitByName(name_unit, origin, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
	ParticleManager:ReleaseParticleIndex(
		ParticleManager:CreateParticle("particles/neutral_fx/skeleton_spawn.vpcf", PATTACH_ABSORIGIN, skelet)
	)
	...
	local modifier = skelet:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_skeleton_king_vampiric_aura_custom_skeleton_ai",
		{ target = target_enemy, duration = duration }
	)
...
function modifier_skeleton_king_vampiric_aura_custom:TakeDamageScriptModifier(params)
	if not IsServer() then
		return
	end
...
function modifier_skeleton_king_vampiric_aura_custom_skeleton_ai:RemoveOnDeath()
	return false
end
function modifier_skeleton_king_vampiric_aura_custom_skeleton_ai:IsHidden()
	return true
end
function modifier_skeleton_king_vampiric_aura_custom_skeleton_ai:IsPurgable()
	return false
end
...
function modifier_skelet_reincarnation:OnDeathEvent(params)
	if not IsServer() then
		return
	end
