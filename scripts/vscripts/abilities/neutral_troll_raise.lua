...
function modifier_troll_raise:IsPurgable()
	return false
end
function modifier_troll_raise:IsHidden()
	return true
end
...
function modifier_troll_raise:StartCast(target)
	...
	self:GetParent():AddNewModifier(
		...
		{ target = array_target, duration = 0.4, anim = ACT_DOTA_CAST_ABILITY_2, parent_mod = self:GetName() }
	)
end
