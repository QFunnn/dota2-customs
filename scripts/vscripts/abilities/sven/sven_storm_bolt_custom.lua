...
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_legendary",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_legendary_caster",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_tracker",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_scepter",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_stun",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_silence",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_proc",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_proc_slow",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_legendary_stack",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_legendary_cd",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_damage_reduce",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_sven_storm_bolt_custom_shield",
	"abilities/sven/sven_storm_bolt_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function sven_storm_bolt_custom:GetBoltDamage(target)
	...
	local damage = self:GetSpecialValueFor("bolt_damage")
		+ caster:GetStrength() * caster:GetTalentValue("modifier_sven_hammer_1", "damage") / 100

	local mod = target:FindModifierByName("modifier_sven_storm_bolt_custom_legendary_stack")
	if mod then
		damage = damage
			* (1 + mod:GetStackCount() * caster:GetTalentValue("modifier_sven_hammer_7", "damage_inc") / 100)
	end
	return damage
end
...
function sven_storm_bolt_custom:OnSpellStart(new_target)
	...
	local bolt_speed = self:GetSpecialValueFor("bolt_speed")
		* (1 + caster:GetTalentValue("modifier_sven_hammer_6", "speed") / 100)
	...
	if caster:HasTalent("modifier_sven_hammer_4") or caster:HasTalent("modifier_sven_hammer_1") then
		caster:AddNewModifier(
			...
			self,
			"modifier_sven_storm_bolt_custom_proc",
			{ duration = caster:GetTalentValue("modifier_sven_hammer_4", "duration", true) }
		)
	end
...
function sven_storm_bolt_custom:OnProjectileHit_ExtraData(hTarget, vLocation, table)
	...
	local bolt_stun_duration = self:GetSpecialValueFor("bolt_stun_duration")
		+ caster:GetTalentValue("modifier_sven_hammer_2", "stun")
		...
		if not enemy:IsInvulnerable() then
			...
			if caster:HasTalent("modifier_sven_hammer_3") then
				enemy:AddNewModifier(
					...
					self,
					"modifier_sven_storm_bolt_custom_damage_reduce",
					{ duration = duration + reduction_duration }
				)
			end
			...
			DoDamage({
				victim = enemy,
				attacker = caster,
				damage = self:GetBoltDamage(enemy),
				ability = self,
...
function modifier_sven_storm_bolt_custom_tracker:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_tracker:IsPurgable()
	return false
end
...
function modifier_sven_storm_bolt_custom_tracker:GetModifierPercentageCooldown()
	if not self.parent:HasTalent("modifier_sven_hammer_6") then
		return
	end
	return self.cdr
end
...
function modifier_sven_storm_bolt_custom_tracker:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_sven_hammer_2") then
		return
	end
...
function modifier_sven_storm_bolt_custom_tracker:DamageEvent_out(params)
	...
	if not mod or not mod.main or mod.main == 0 then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_sven_storm_bolt_custom_shield",
		{ duration = self.shield_duration, shield = shield }
	)
end
...
function modifier_sven_storm_bolt_custom_tracker:AttackEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_sven_hammer_4") then
		return
	end
	...
	if not RollPseudoRandomPercentage(chance, 4254, self.parent) then
		return
	end
	...
	local real_damage = DoDamage(
		{
			...
			damage = self.parent:GetStrength() * self.proc_damage,
...
function modifier_sven_storm_bolt_custom_scepter:IsPurgable()
	return false
end
...
function modifier_sven_storm_bolt_custom_scepter:GetStatusEffectName()
	return "particles/status_fx/status_effect_forcestaff.vpcf"
end
function modifier_sven_storm_bolt_custom_scepter:StatusEffectPriority()
	return MODIFIER_PRIORITY_HIGH
end

modifier_sven_storm_bolt_custom_stun = class({})

function modifier_sven_storm_bolt_custom_stun:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_stun:IsStunDebuff()
	return true
end
function modifier_sven_storm_bolt_custom_stun:IsPurgeException()
	return true
end
function modifier_sven_storm_bolt_custom_stun:GetEffectName()
...
function modifier_sven_storm_bolt_custom_stun:OnCreated(table)
	...
	if self.caster:GetQuest() ~= "Sven.Quest_5" or self.caster:QuestCompleted() then
		return
	end
...
function modifier_sven_storm_bolt_custom_stun:OnDestroy()
	...
	if not self.caster:HasTalent("modifier_sven_hammer_5") then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		"modifier_sven_storm_bolt_custom_silence",
		{
			duration = (1 - self.parent:GetStatusResistance())
				* self.caster:GetTalentValue("modifier_sven_hammer_5", "silence"),
		}
	)
end

modifier_sven_storm_bolt_custom_silence = class({})
function modifier_sven_storm_bolt_custom_silence:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_silence:IsPurgable()
	return true
end
function modifier_sven_storm_bolt_custom_silence:GetTexture()
...
function modifier_sven_storm_bolt_custom_silence:GetEffectName()
	return "particles/generic_gameplay/generic_silenced.vpcf"
end
function modifier_sven_storm_bolt_custom_silence:ShouldUseOverheadOffset()
	return true
end
function modifier_sven_storm_bolt_custom_silence:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

modifier_sven_storm_bolt_custom_proc = class({})
function modifier_sven_storm_bolt_custom_proc:IsHidden()
	return false
end
function modifier_sven_storm_bolt_custom_proc:IsPurgable()
	return false
end
function modifier_sven_storm_bolt_custom_proc:GetTexture()
...
function modifier_sven_storm_bolt_custom_proc:OnCreated(table)
	...
	if not self.parent:HasTalent("modifier_sven_hammer_1") then
		return
	end
...
function modifier_sven_storm_bolt_custom_proc_slow:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_proc_slow:IsPurgable()
	return true
end
...
function modifier_sven_storm_bolt_custom_legendary_stack:IsHidden()
	return false
end
function modifier_sven_storm_bolt_custom_legendary_stack:IsPurgable()
	return false
end
...
function modifier_sven_storm_bolt_custom_damage_reduce:IsHidden()
	return false
end
function modifier_sven_storm_bolt_custom_damage_reduce:IsPurgable()
	return false
end
function modifier_sven_storm_bolt_custom_damage_reduce:GetTexture()
	return "buffs/spear_heal"
end
...
function sven_storm_bolt_custom_legendary:OnSpellStart()
	...
	self.thinkers[1] = CreateModifierThinker(
		caster,
		self,
		"modifier_sven_storm_bolt_custom_legendary",
		{},
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_sven_storm_bolt_custom_legendary_caster",
		{ thinker = self.thinkers[1]:entindex() }
	)
...
function modifier_sven_storm_bolt_custom_legendary:IsHidden()
	return false
end
function modifier_sven_storm_bolt_custom_legendary:IsPurgable()
	return false
end
...
function modifier_sven_storm_bolt_custom_legendary:OnIntervalThink()
	...
	for _, enemy in pairs(enemies) do
		...
		if
			not enemy:IsInvulnerable()
			and enemy:IsUnit()
			and not enemy:HasModifier("modifier_sven_storm_bolt_custom_legendary_cd")
		then
			enemy:AddNewModifier(
				self.caster,
				self.ability,
				"modifier_sven_storm_bolt_custom_legendary_cd",
				{ duration = self.damage_interval }
			)
			...
			local real_damage = DoDamage({
				...
				damage = self.main:GetBoltDamage(enemy) * self.damage_init,
				ability = self.ability,
				damage_type = DAMAGE_TYPE_MAGICAL,
			})
			enemy:SendNumber(4, real_damage)
...
function modifier_sven_storm_bolt_custom_legendary_cd:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_legendary_cd:IsPurgable()
	return false
end

modifier_sven_storm_bolt_custom_legendary_caster = class({})
function modifier_sven_storm_bolt_custom_legendary_caster:IsHidden()
	return true
end
function modifier_sven_storm_bolt_custom_legendary_caster:IsPurgable()
	return false
end
...
function modifier_sven_storm_bolt_custom_shield:IsHidden()
	return false
end
function modifier_sven_storm_bolt_custom_shield:IsPurgable()
	return false
end
function modifier_sven_storm_bolt_custom_shield:GetTexture()
...
function modifier_sven_storm_bolt_custom_shield:OnCreated(table)
	...
	self.max_shield = self.parent:GetMaxHealth()
		* self.parent:GetTalentValue("modifier_sven_hammer_6", "shield_max")
		/ 100
