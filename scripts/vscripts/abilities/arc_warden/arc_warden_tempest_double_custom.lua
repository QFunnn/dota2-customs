...
LinkLuaModifier(
	"modifier_arc_warden_tempest_double_custom_auto_cd",
	"abilities/arc_warden/arc_warden_tempest_double_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function modifier_arc_warden_tempest_double_custom_tracker:OnIntervalThink()
	...
	if not self.parent:IsTempestDouble() then
		...
		if
			...
			and not self.parent:HasModifier("modifier_arc_warden_tempest_double_custom_auto_cd")
			and self.parent:IsAlive()
			and self.tempest_ability
			and not self.parent:IsInvisible()
			and not self.parent:HasModifier("modifier_arc_warden_tempest_double_custom_far")
		then
			...
			if target then
				...
				self.parent:AddNewModifier(
					...
					self.ability,
					"modifier_arc_warden_tempest_double_custom_auto_cd",
					{ duration = self.ability.talents.r3_cd }
				)
			end
		end
	end
