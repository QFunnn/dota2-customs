...
LinkLuaModifier(
	"modifier_arc_warden_flux_custom_legendary_tempest_cd",
	"abilities/arc_warden/arc_warden_flux_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_arc_warden_flux_custom_count",
	"abilities/arc_warden/arc_warden_flux_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function arc_warden_flux_custom:OnSpellStart()
	...
	if self.talents.has_q7 == 1 then
		...
		target_mod = target:AddNewModifier(
			...
			self,
			"modifier_arc_warden_flux_custom",
			{ duration = duration, is_legendary = 0 }
		)
		...
		for _, aoe_target in pairs(self.caster:FindTargets(self.aoe_radius, target:GetAbsOrigin())) do
			if aoe_target ~= target and aoe_target:IsCreep() then
				aoe_target:AddNewModifier(
					...
					self,
					"modifier_arc_warden_flux_custom",
					{ duration = duration, is_legendary = 0, is_aoe = 1 }
				)
			end
		end
...
function arc_warden_flux_custom:ApplyResist(target, stack)
	...
	target:AddNewModifier(
		...
		self,
		"modifier_arc_warden_flux_custom_resist",
		{ duration = self.talents.q3_duration, stack = stack }
	)
end
	...
	for _, unit in pairs(self.caster:FindTargets(self.radius, self.parent:GetAbsOrigin())) do
		if not unit:HasModifier(self.cd_mod) then
			...
			unit:AddNewModifier(
				...
				self.ability,
				"modifier_arc_warden_flux_custom",
				{ duration = self.duration, is_legendary = 1 }
			)
			...
			if self.ability.talents.has_h4 == 1 and first then
				...
				unit:AddNewModifier(
					...
					self.ability,
					"modifier_arc_warden_flux_custom_silence",
					{ duration = (1 - unit:GetStatusResistance()) * self.ability.talents.h4_silence }
				)
			end
		end
	end
...
function modifier_arc_warden_flux_custom_tracker:RemoveOnDeath()
	return false
end
...
function modifier_arc_warden_flux_custom_resist:GetTexture()
	return "buffs/arc_warden/flux_3"
end
