...
function axe_counter_helix_custom:GetBehavior()
	return self.talents.has_e7 == 1 and (DOTA_ABILITY_BEHAVIOR_NO_TARGET + DOTA_ABILITY_BEHAVIOR_IMMEDIATE)
		or DOTA_ABILITY_BEHAVIOR_PASSIVE
end
...
function axe_counter_helix_custom:Spin(use_cd, ability)
	...
	if self.talents.has_e2 == 1 then
		self.caster:GenericHeal(
			self.caster:GetMaxHealth() * self.talents.e2_heal + self.talents.e2_base,
			self,
			false,
			nil,
	...
	if
		...
		and (GameRules:GetDOTATime(false, false) - self.attack_time) >= self.scepter_attack_cd
	then
		attack = true
...
function modifier_axe_counter_helix_custom:OnIntervalThink()
	...
	if allow then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_axe_counter_helix_custom_scepter",
			{ duration = self.ability.scepter_linger }
		)
	end
end
...
function modifier_axe_counter_helix_custom:DamageEvent_inc(params)
	...
	if self.parent:HasModifier("modifier_axe_counter_helix_custom_shield_cd") then
		return
	end
...
function modifier_axe_counter_helix_custom:CheckStack()
	...
	if self.parent:HasModifier("modifier_axe_counter_helix_custom_legendary_active") then
		return
	end

	if self.ability.talents.has_e7 == 1 then
		if self.parent:HasModifier("modifier_axe_counter_helix_custom_cd") then
			return
		end
	else
...
function modifier_axe_counter_helix_custom_armor:GetTexture()
	return "buffs/axe/helix_1"
end
...
function modifier_axe_counter_helix_custom_shield_cd:GetTexture()
	return "buffs/axe/hero_7"
end
...
function modifier_axe_counter_helix_custom_scepter:IsDebuff()
	return true
end
function modifier_axe_counter_helix_custom_scepter:GetTexture()
