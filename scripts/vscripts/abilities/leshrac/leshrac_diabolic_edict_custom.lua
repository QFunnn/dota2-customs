...
LinkLuaModifier(
	"modifier_leshrac_diabolic_edict_custom",
...
LinkLuaModifier(
	"modifier_leshrac_diabolic_edict_custom_tracker",
	"abilities/leshrac/leshrac_diabolic_edict_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function leshrac_diabolic_edict_custom:OnSpellStart()
	...
	self.caster:AddNewModifier(
		...
		self,
		"modifier_leshrac_diabolic_edict_custom",
		{ duration = self.duration * (1 + self.talents.w2_duration) + 0.1 }
	)
end
	...
	if unit then
		if legendary_effect then
			EmitSoundOnLocationWithCaster(
				unit:GetAbsOrigin(),
				"Leshrac.Edict_legendary_break_active" .. RandomInt(1, 4),
				self.caster
			)
	...
	if self.ability.talents.has_r4 == 1 then
		...
		if self.ability.shield_mod then
			...
			local effect_cast = ParticleManager:CreateParticle(
				"particles/leshrac/edict_shield.vpcf",
...
function modifier_leshrac_diabolic_edict_custom:OnIntervalThink()
	...
	if self.count >= self.explosion then
		return
	end
...
function modifier_leshrac_diabolic_edict_custom:CheckSpeed()
	...
	if not self.parent:CheckCd("leshrac_h5", self.ability.talents.h5_cd) then
		return
	end
...
function modifier_leshrac_diabolic_edict_custom_speed:GetTexture()
	return "buffs/leshrac/hero_5"
end
...
function modifier_leshrac_diabolic_edict_custom_legendary:OnCreated(table)
	...
	self.mod = self.parent:AddNewModifier(
		self.caster,
		self.ability,
		"modifier_leshrac_diabolic_edict_custom_legendary_damage",
...
function modifier_leshrac_diabolic_edict_custom_legendary:OnIntervalThink()
	...
	if
		...
		or not self.caster:HasModifier("modifier_leshrac_diabolic_edict_custom")
	then
		self:Destroy()
	end
end
...
function modifier_leshrac_diabolic_edict_custom_legendary:OnDestroy()
	...
	if IsValid(self.mod) then
		if mod then
			...
			self.mod:SetDuration(self.ability.talents.w7_effect_duration, true)
		else
			self.mod:Destroy()
		end
	end
	...
	self.parent:AddNewModifier(
		...
		"modifier_stunned",
		{ duration = self.stun * (1 - self.parent:GetStatusResistance()) }
	)
end
...
function modifier_leshrac_diabolic_edict_custom_legendary_damage:OnIntervalThink()
	...
	self.caster:UpdateUIshort({
		...
		stack = math.floor(self:GetStackCount() * self.magic) .. "%",
		style = "LeshracEdict",
	})
end
...
function modifier_leshrac_diabolic_edict_custom_legendary_damage:OnDestroy()
	...
	if self ~= self.ability.active_mod then
		return
	end
...
function modifier_leshrac_diabolic_edict_custom_tracker:SpellEvent(params)
	...
	if not target or target:IsCreep() then
		return
	end
	...
	if self.parent:HasModifier("modifier_leshrac_diabolic_edict_custom_root_cd") then
		return
	end
	if target:HasModifier("modifier_leshrac_diabolic_edict_custom_legendary") then
		return
	end
	...
	target:AddNewModifier(
		...
		self.ability,
		"modifier_leshrac_diabolic_edict_custom_root",
		{ duration = (1 - target:GetStatusResistance()) * self.ability.talents.w4_root }
	)
	self.parent:AddNewModifier(
		self.parent,
		self.ability,
...
function modifier_leshrac_diabolic_edict_custom_damage_stack:IsHidden()
	return self.ability.talents.has_w3 == 0 or self:GetStackCount() >= self.max
end
function modifier_leshrac_diabolic_edict_custom_damage_stack:RemoveOnDeath()
	return false
end
function modifier_leshrac_diabolic_edict_custom_damage_stack:GetTexture()
	return "buffs/leshrac/edict_3"
end
...
function modifier_leshrac_diabolic_edict_custom_root_cd:GetTexture()
	return "buffs/leshrac/edict_4"
end

modifier_leshrac_diabolic_edict_custom_root = class(mod_hidden)
function modifier_leshrac_diabolic_edict_custom_root:IsPurgable()
	return true
end
