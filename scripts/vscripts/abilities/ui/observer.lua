...
function custom_ability_observer:OnAbilityPhaseStart()
	...
	if dota1x6:IsCustomRules("no_wards") then
		return false
	end
		...
		if item then
			if item:GetName() == "item_ward_observer" and not item.use_ui then
				if self:GetCaster():IsRealHero() then
					local modifier_observer = self:GetCaster()
						:FindModifierByName("modifier_item_custom_observer_ward_charges")
...
function modifier_item_custom_observer_ward_charges:IsHidden()
	return true
end
function modifier_item_custom_observer_ward_charges:DestroyOnExpire()
	return false
end
function modifier_item_custom_observer_ward_charges:IsPurgable()
	return false
end
...
function modifier_item_custom_observer_ward:IsHidden()
	return true
end
...
function modifier_item_custom_observer_ward:CanParentBeAutoAttacked()
	return false
end
...
function modifier_item_custom_observer_ward:GetModifierHealthBarPips()
	return self.destroy_attacks / self.hero_attack_multiplier
end
