		...
		if item then
			if item:GetName() == "item_ward_sentry" and not item.use_ui then
				if self:GetCaster():IsRealHero() then
					local modifier_sentry = self:GetCaster()
						:FindModifierByName("modifier_item_custom_sentry_ward_charges")
...
function custom_ability_sentry:OnAbilityPhaseStart()
	local mod_stacks = self:GetCaster()
		:GetModifierStackCount("modifier_item_custom_sentry_ward_charges", self:GetCaster())
...
function modifier_item_custom_sentry_ward:RemoveOnDeath()
	return true
end
...
function modifier_item_custom_sentry_ward:GetModifierHealthBarPips()
...
function modifier_item_custom_sentry_ward_charges:IsHidden()
	return true
end
function modifier_item_custom_sentry_ward_charges:DestroyOnExpire()
	return false
end
function modifier_item_custom_sentry_ward_charges:IsPurgable()
	return false
end
