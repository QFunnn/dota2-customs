...
function modifier_satyr_manaburn_npc:IsHidden()
	return true
end
function modifier_satyr_manaburn_npc:IsPurgable()
	return false
end
	...
	if #enemy_for_ability > 0 then
		for _, i in pairs(enemy_for_ability) do
			if not i:IsMagicImmune() then
				...
				if i:IsHero() and not i:IsRealHero() then
					damage = self.illusion
				end
				DoDamage({
					victim = i,
...
function modifier_satyr_manaburn_cd:IsHidden()
	return false
end
function modifier_satyr_manaburn_cd:IsPurgable()
	return false
end