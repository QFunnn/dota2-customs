...
LinkLuaModifier(
	...
	"abilities/monkey_king/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_BOTH
)
LinkLuaModifier(
	"modifier_monkey_king_tree_dance_custom_tracker",
	"abilities/monkey_king/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_tree_dance_vision",
	"abilities/monkey_king/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_tree_dance_custom_activity",
	"abilities/monkey_king/monkey_king_tree_dance_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_monkey_king_tree_dance_custom_tracker:DamageEvent_inc(params)
	...
	local cd = self.ability.jump_damage_cooldown
		+ (self.ability.talents.has_h4 == 1 and self.ability.talents.h4_tree_cd or 0)
...
function modifier_monkey_king_tree_dance_custom:OnIntervalThink()
	...
	if (self.tree.IsStanding and not self.tree:IsStanding()) or self.tree:IsNull() then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = self.unperched_stunned_duration * (1 - self.parent:GetStatusResistance()) }
		)
		self.unperched = true
		self:Destroy()
	end
...
function modifier_monkey_king_tree_dance_custom:OrderEvent(params)
	...
	if
		...
		and params.order_type ~= DOTA_UNIT_ORDER_MOVE_TO_TARGET
