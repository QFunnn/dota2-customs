...
LinkLuaModifier(
	"modifier_monkey_king_innate_custom_tracker",
	"abilities/monkey_king/monkey_king_innate_custom.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_monkey_king_mischief_invun",
	"abilities/monkey_king/monkey_king_innate_custom.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function monkey_king_innate_custom:GetBehavior()
	...
	return DOTA_ABILITY_BEHAVIOR_POINT + DOTA_ABILITY_BEHAVIOR_NOT_LEARNABLE + DOTA_ABILITY_BEHAVIOR_ROOT_DISABLES
end
...
function monkey_king_innate_custom:OnSpellStart()
	...
	if not IsValid(self.caster.command_ability) then
		return
	end
	...
	if dir:Length2D() > self.AbilityCastRange then
		point = self.caster:GetAbsOrigin() + dir:Normalized() * self.AbilityCastRange
	end
	...
	self.caster:AddNewModifier(
		...
		self,
		"modifier_monkey_king_innate_custom",
		{ duration = self.duration, monkey = clone:entindex() }
	)
...
function modifier_monkey_king_innate_custom:OnIntervalThink()
	...
	if
		not IsValid(self.monkey)
		or not self.monkey:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier_active")
	then
		self:Destroy()
		return
	end
...
function modifier_monkey_king_innate_custom:SpellEvent(params)
	...
	if params.ability:GetName() == "monkey_king_primal_spring_custom" or self.ability == params.ability then
		return
	end
...
function modifier_monkey_king_innate_custom:AttackEvent_inc(params)
	...
	if not IsValid(self.monkey) then
		return
	end
	if self.monkey ~= params.target then
		return
	end
...
function modifier_monkey_king_innate_custom_clone:OnCreated()
	...
	local effect = ParticleManager:CreateParticleForTeam(
		"particles/monkey_king/innate_marker.vpcf",
