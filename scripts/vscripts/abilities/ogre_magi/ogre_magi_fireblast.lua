...
LinkLuaModifier(
	"modifier_ogre_magi_fireblast_custom_tracker",
...
function ogre_magi_fireblast_custom_class:GetDamage(target)
	...
	local result = (
		self.fireblast_damage
		+ self.caster:GetMana() * (self.is_scepter and self.mana_damage or self.talents.q1_damage)
	) * k
...
function ogre_magi_fireblast_custom_class:OnSpellStart()
	...
	if self.is_scepter and self.caster:GetUpgradeStack("modifier_ogre_magi_fireblast_custom_tracker") == 0 then
		local mod = self.caster:FindModifierByName("modifier_ogre_magi_fireblast_custom_tracker")
		if mod then
			mod:SetStackCount(1)
		end
	end
...
function ogre_magi_fireblast_custom:GetCooldown(iLevel)
	...
	if
		...
		and self.caster:GetUpgradeStack("modifier_ogre_magi_fireblast_custom_tracker") == 0
	then
		result = result + self.talents.q7_cd_base
	end
	return result
end
...
function modifier_ogre_magi_fireblast_custom_tracker:SpellEvent(params)
	...
	if params.ability == self.ability or params.ability == self.parent.fireblast_scepter_ability then
		return
	end
...
function modifier_ogre_magi_fireblast_custom_tracker:GetModifierTotalDamageOutgoing_Percentage(params)
	if not self.parent.ogre_scepter_damage then
		return
	end
...
function modifier_ogre_magi_fireblast_custom_speed:GetTexture()
	return "buffs/ogre_magi/fireblast_4"
end
...
function modifier_ogre_magi_fireblast_custom_slow:IsPurgable()
	return true
end
...
function modifier_ogre_magi_fireblast_custom_magic:GetTexture()
	return "buffs/ogre_magi/fireblast_3"
end
