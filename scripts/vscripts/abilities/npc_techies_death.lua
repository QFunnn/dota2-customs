...
function modifier_techies_death_passive:IsHidden()
	return true
end

function modifier_techies_death_passive:OnCreated(table)
	self.parent = self:GetParent()
	self.parent:AddDeathEvent(self)
end

function modifier_techies_death_passive:DeathEvent(param)
	...
	if param.unit == self:GetParent() then
		...
		local particle_explosion_fx =
			ParticleManager:CreateParticle(particle_explosion, PATTACH_WORLDORIGIN, self.parent)
...
function modifier_techies_death_debuf:IsHidden()
	return false
end
