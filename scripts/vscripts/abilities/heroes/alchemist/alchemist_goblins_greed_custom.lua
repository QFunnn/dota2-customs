...
LinkLuaModifier(
	"modifier_alchemist_scepter_bonus_damage_lua",
	"abilities/heroes/alchemist/alchemist_goblins_greed_custom",
...
function alchemist_goblins_greed_custom:Spawn()
	...
	Timers:CreateTimer(0.1, function()
		...
		if GameLoop.hero_by_player_id[player_id] ~= caster then
			return
		end
...
function alchemist_goblins_greed_custom:OnOrbCaptured(event)
	...
	if event.team ~= self:GetTeam() then
		return
	end
...
function alchemist_goblins_greed_custom:OnAbilityUsed(event)
	...
	if ability_name ~= "item_ultimate_scepter" then
		return
	end
...
function modifier_alchemist_scepter_bonus_damage_lua:IsHidden()
	return true
end
function modifier_alchemist_scepter_bonus_damage_lua:IsPurgable()
	return false
end
function modifier_alchemist_scepter_bonus_damage_lua:IsDebuff()
	return false
end
function modifier_alchemist_scepter_bonus_damage_lua:RemoveOnDeath()
	return false
end
function modifier_alchemist_scepter_bonus_damage_lua:GetAttributes()
...
function modifier_alchemist_scepter_bonus_damage_lua:OnCreated()
	...
	Timers:CreateTimer(0.1, function()
		...
		if not vanilla_modifier or vanilla_modifier:IsNull() then
			return
		end
...
function modifier_alchemist_scepter_bonus_damage_lua:GetModifierSpellAmplify_Percentage()
	return self:GetStackCount() * self.scepter_spell_amp
end