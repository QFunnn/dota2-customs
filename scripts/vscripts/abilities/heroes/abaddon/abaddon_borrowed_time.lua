...
LinkLuaModifier(
	"modifier_ot3_borrowed_time_buff_hot_caster",
	"abilities/heroes/abaddon/abaddon_borrowed_time.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ot3_borrowed_time_handler",
	"abilities/heroes/abaddon/abaddon_borrowed_time.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_ot3_borrowed_time_coil_counter",
	"abilities/heroes/abaddon/abaddon_borrowed_time.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_ot3_borrowed_time_handler:IsHidden()
	return true
end
function modifier_ot3_borrowed_time_handler:IsPurgable()
	return false
end
function modifier_ot3_borrowed_time_handler:AllowIllusionDuplicate()
	return false
end

function modifier_ot3_borrowed_time_handler:DeclareFunctions()
	return {
...
function modifier_ot3_borrowed_time_buff_hot_caster:IsPurgable()
	return false
end
function modifier_ot3_borrowed_time_buff_hot_caster:IsAura()
	return true
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetModifierAura()
	return "modifier_ot3_borrowed_time_buff_hot_ally"
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetEffectName()
	return "particles/units/heroes/hero_abaddon/abaddon_borrowed_time.vpcf"
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end
function modifier_ot3_borrowed_time_buff_hot_caster:GetStatusEffectName()
	return "particles/status_fx/status_effect_abaddon_borrowed_time.vpcf"
end
function modifier_ot3_borrowed_time_buff_hot_caster:StatusEffectPriority()
	return 15
end
...
function modifier_ot3_borrowed_time_buff_hot_caster:DeclareFunctions()
	return {
...
function modifier_ot3_borrowed_time_buff_hot_caster:OnTakeDamage(kv)
	...
	if (unit:GetAbsOrigin() - caster:GetAbsOrigin()):Length2D() > self.redirect_range_scepter then
		return
	end

	local unit_damage_taken = unit.borrowed_time_damage_taken
	if not unit_damage_taken then
		unit.borrowed_time_damage_taken = 0
	end

	unit.borrowed_time_damage_taken = unit.borrowed_time_damage_taken + kv.damage
	if unit.borrowed_time_damage_taken / self.ally_threshold_scepter < 1 then
		return
	end
	...
	for i = 1, unit.borrowed_time_damage_taken / self.ally_threshold_scepter do
		...
		if coil_counter_modifier and not coil_counter_modifier:IsNull() then
			...
			coil_counter_modifier =
				unit:AddNewModifier(caster, self:GetAbility(), "modifier_ot3_borrowed_time_coil_counter", {
					duration = self:GetRemainingTime(),
				})
		end

		if not coil_counter_modifier or coil_counter_modifier:IsNull() then
			return
		end
...
function modifier_ot3_borrowed_time_coil_counter:IsPurgable()
	return false
end
