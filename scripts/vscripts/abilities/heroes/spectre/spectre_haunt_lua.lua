...
LinkLuaModifier(
	"modifier_spectre_haunt_lua",
	"abilities/heroes/spectre/modifier_spectre_haunt_lua",
	...
	for _, hero in pairs(GameLoop.hero_by_player_id or {}) do
		if hero:GetTeam() ~= caster_team then
			...
			Timers:CreateTimer(delay, function()
				if not IsValidEntity(hero) or not IsValidEntity(self) then
					return
				end
