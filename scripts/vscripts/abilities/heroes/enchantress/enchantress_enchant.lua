...
LinkLuaModifier(
	"modifier_enchantress_rabblerouser_lua",
	"abilities/heroes/enchantress/enchantress_enchant",
...
function enchantress_enchant_custom:RemoveExcessiveUnits(current_unit_count, max_count)
	if current_unit_count < max_count then
		return
	end
...
function enchantress_enchant_custom:OnSpellStart()
	...
	if
		(not target:IsConsideredHero() or target:IsIllusion())
		and not target:HasModifier("modifier_vengefulspirit_hybrid_special")
	then
...
function modifier_enchantress_rabblerouser_lua:IsHidden()
	return false
end
function modifier_enchantress_rabblerouser_lua:IsDebuff()
	return false
end
function modifier_enchantress_rabblerouser_lua:IsPurgable()
	return false
end
...
function modifier_enchantress_rabblerouser_lua:OnAbilityLevelled(event)
	...
	if not self.talent_names[ability_name] then
		return
	end
