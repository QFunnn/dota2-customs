...
function modifier_undying_tombstone_lua:OnCreated()
	...
	EventDriver:Listen("EventProxy:OnModifierAdded", self.OnModifierAdded, self)
end

function modifier_undying_tombstone_lua:OnModifierAdded(event)
	if event.added_buff:GetName() == "modifier_undying_tombstone_bunker_friendly" then
		Timers:CreateTimer(0, function()
			if not IsValidEntity(event.unit) then
				return
			end

			if event.added_buff:IsNull() then
				ResolveNPCPositions(event.unit:GetAbsOrigin(), 64)
			end
...
function modifier_undying_tombstone_lua:OnIntervalThink()
	if not self.ability then
		return
	end -- https://discord.com/channels/501306949434867713/997015568492281956/997015687501467738
	...
	local particle = ParticleManager:CreateParticle(
		"particles/econ/items/undying/undying_pale_augur/undying_pale_augur_decay_smoke_swirl.vpcf",
