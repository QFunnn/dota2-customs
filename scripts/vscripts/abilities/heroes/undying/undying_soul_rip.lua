...
LinkLuaModifier(
	"modifier_undying_soul_rip_active_lua",
	"abilities/heroes/undying/modifier_undying_soul_rip_active_lua",
...
function undying_soul_rip_custom:OnSpellStart()
	...
	if not self:IsTargetTombstone(target) then
		return
	end
...
function undying_soul_rip_custom:PerformSoulRip(caster, target)
	...
	local hp_removal_damage_table = {
		...
		damage_flags = DOTA_DAMAGE_FLAG_HPLOSS
			...
			+ DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION, -- Putting reflection flag here in case of unwanted interactions
		attacker = caster,
		ability = self,
	}
	...
	if is_ally and strength_share_duration > 0 and target ~= caster and not self:IsTargetTombstone(target) then
		target:AddNewModifier(
			...
			self,
			"modifier_undying_soul_rip_active_lua",
			{ duration = strength_share_duration }
		)
	end
