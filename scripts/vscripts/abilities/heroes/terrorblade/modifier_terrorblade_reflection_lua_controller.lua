...
function modifier_terrorblade_reflection_lua_controller:IsPurgable()
	return true
end
function modifier_terrorblade_reflection_lua_controller:IsHidden()
	return true
end
-- to handle rubick spell steal
function modifier_terrorblade_reflection_lua_controller:GetAttributes()
...
function modifier_terrorblade_reflection_lua_controller:OnIntervalThink()
	if
		not IsValidEntity(self.controlled_illusion)
		or not IsValidEntity(self.parent)
		or not IsValidEntity(self.caster)
	then
		self:Destroy()
		return
	end
