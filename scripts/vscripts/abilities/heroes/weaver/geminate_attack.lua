...
LinkLuaModifier(
	"modifier_weaver_geminate_attack_lua",
	"abilities/heroes/weaver/geminate_attack.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_weaver_geminate_attack_delay_lua",
	"abilities/heroes/weaver/geminate_attack.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function weaver_geminate_attack_lua:DoSecondaryAttacks(target)
	...
	for geminate_attacks = 1, self:GetSpecialValueFor("tooltip_attack") do
		caster:AddNewModifier(
			...
			self,
			"modifier_weaver_geminate_attack_delay_lua",
			{ delay = delay_per_attack * geminate_attacks }
		)
	end
end
...
function modifier_weaver_geminate_attack_lua:IsHidden()
	return true
end
...
function modifier_weaver_geminate_attack_lua:OnCreated()
	...
	if IsServer() and not self.ability:GetAutoCastState() then
...
function modifier_weaver_geminate_attack_lua:OnAttack(params)
	...
	if not IsValidEntity(self.parent) or not IsValidEntity(self.ability) then
		return
	end
	if not self.ability:IsFullyCastable() or self.parent:IsIllusion() or self.parent:PassivesDisabled() then
		return
	end
...
function modifier_weaver_geminate_attack_delay_lua:IsHidden()
	return true
end
function modifier_weaver_geminate_attack_delay_lua:IsPurgable()
	return false
end
function modifier_weaver_geminate_attack_delay_lua:GetAttributes()
