...
LinkLuaModifier(
	"modifier_broodmother_spawn_spiderlings_lua",
	"abilities/heroes/broodmother/modifier_broodmother_spawn_spiderlings",
...
function broodmother_spawn_spiderlings:OnProjectileHit(target, location)
	...
	if target:IsInvulnerable() or target:IsOutOfGame() or target:TriggerSpellAbsorb(self) then
		return
	end
...
function broodmother_spawn_spiderlings:SpawnSpiderlings(target)
	...
	for _, spiderling in ipairs(self.summon_list) do
		...
		if not has_spiderling_debuff then -- spiderling debuff also extends duration on target death, so omit of present
			...
			if buff_modifier and not buff_modifier:IsNull() then
				buff_modifier:ExtendLifetime()
			end
		end
	end
end
