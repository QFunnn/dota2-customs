...
LinkLuaModifier(
	"modifier_broodmother_spiderling_lua",
	"abilities/heroes/broodmother/broodmother_buff_spiderling",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_broodmother_spiderling_debuff_lua",
	"abilities/heroes/broodmother/broodmother_buff_spiderling",
	LUA_MODIFIER_MOTION_NONE
)
...
function modifier_broodmother_spiderling_lua:IsHidden()
	return false
end
function modifier_broodmother_spiderling_lua:GetAttributes()
...
function modifier_broodmother_spiderling_lua:GetModifierProcAttack_Feedback(kv)
	...
	if kv.target:IsOther() then
		return
	end
	if kv.target:IsIllusion() then
		return
	end
	if self.target_exceptions[kv.target:GetUnitName()] then
		return
	end
	...
	kv.target:AddNewModifier(
		...
		self.ability,
		"modifier_broodmother_spiderling_debuff_lua",
		{ duration = self.debuff_duration * (1 - kv.target:GetStatusResistance()) }
	)
end
...
function modifier_broodmother_spiderling_debuff_lua:IsHidden()
	return false
end
function modifier_broodmother_spiderling_debuff_lua:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT
end
...
function modifier_broodmother_spiderling_debuff_lua:OnDeath(keys)
	...
	if not self.caster:GetOwner() or self.caster:GetOwner():IsNull() then
		return
	end
