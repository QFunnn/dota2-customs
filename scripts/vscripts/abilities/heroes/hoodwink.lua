...
function ab.prototype.EDeclareEvents(self)
	return {
		[EOMModifierEvents.MODIFIER_EVENT_ON_BATTLE_START_BEFORE] = { -1, -1 },
		[EOMModifierEvents.MODIFIER_EVENT_ON_PREDAMAGE] = { -1, self:GetParent() },
	}
end
function ab.prototype.OnBattleStartBefore(self, y)
	self:GetMaxSectLv()
end
function ab.prototype.OnPreDamage(self, H)
	if self.g_chance <= 0 then
		return
	end
	if H.damage_type ~= EOM_DAMAGE_TYPES.DAMAGE_TYPE_PHYSICAL then
		return
	end
	if H.is_evasion == true then
		return
	end
	if not self:PRD(self.g_chance, "g_greevil") then
		return
	end
	H.damage = 0
	self:GetAbility():GreevilShot()
end
