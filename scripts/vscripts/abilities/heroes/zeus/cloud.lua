...
function zuus_cloud_custom:OnSpellStart()
	if IsServer() then
		...
		self.zuus_nimbus_unit:AddNewModifier(
			...
			{
				duration = self:GetSpecialValueFor("cloud_duration"),
				cloud_bolt_interval = self:GetSpecialValueFor("cloud_bolt_interval"),
...
function modifier_zuus_cloud_custom:IsHidden()
	return true
end
function modifier_zuus_cloud_custom:OnCreated(keys)
	if IsServer() then
		...
		ParticleManager:SetParticleControl(
			...
			2,
			Vector(target_point.x, target_point.y, target_point.z + 450)
		)
...
function modifier_zuus_cloud_custom:LightningBolt(target)
	...
	local bolt_damage = lightning_bolt and lightning_bolt:GetAbilityDamage()
		or self:GetAbility():GetSpecialValueFor("fallback_lightning_damage")

	local bolt_damage_table = self.caster.upgrades["zuus_lightning_bolt"]
		and self.caster.upgrades["zuus_lightning_bolt"]["damage"]
	...
	local glow = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_zeus/zeus_cloud_strike_modglow.vpcf",
		PATTACH_WORLDORIGIN,
		target
	)
	...
	CreateModifierThinker(
		...
		{ duration = lightning_bolt and lightning_bolt:GetSpecialValueFor("sight_duration") or 5 },
		target_point,
		self.caster:GetTeam(),
		false
	)
	target:AddNewModifier(
		...
		self:GetAbility(),
		"modifier_stunned",
		{ duration = 0.3 + (ministun_talent and ministun_talent:GetSpecialValueFor("value") or 0) }
	)
