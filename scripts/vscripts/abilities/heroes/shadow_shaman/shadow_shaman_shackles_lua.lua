...
LinkLuaModifier(
	"modifier_shadow_shaman_shackles_dummy_duration",
	"abilities/heroes/shadow_shaman/modifier_shadow_shaman_shackles_dummy_duration",
	LUA_MODIFIER_MOTION_NONE
)

function shadow_shaman_shackles_lua:GetChannelTime()
	if self.channel_duration then
...
function shadow_shaman_shackles_lua:OnSpellStart()
	...
	local modifier = target:AddNewModifierSR(caster, self, "modifier_shadow_shaman_shackles", { duration = duration })
	...
	self.shackles_pfx = ParticleManager:CreateParticle(
