...
LinkLuaModifier(
	"modifier_shadow_shaman_serpent_ward_chc",
	"abilities/heroes/shadow_shaman/modifier_shadow_shaman_serpent_ward_chc",
...
function shadow_shaman_mass_serpent_ward_lua:IsHiddenAsSecondaryAbility()
	return true
end
function shadow_shaman_mass_serpent_ward_lua:IsHiddenWhenStolen()
	return true
end
...
function shadow_shaman_mass_serpent_ward_lua:SummonWards(
	position,
	ward_count,
	ward_duration,
	spawn_radius,
	...
	for i = 1, ward_count do
		...
		if is_mega_ward then
			ward:SetMinimumGoldBounty(
				ward:GetMinimumGoldBounty() * (self:GetSpecialValueFor("mega_ward_multiplier_tooltip") or 1)
			)
			ward:SetMaximumGoldBounty(
				ward:GetMaximumGoldBounty() * (self:GetSpecialValueFor("mega_ward_multiplier_tooltip") or 1)
			)
		end
