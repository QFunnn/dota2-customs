...
LinkLuaModifier(
	"treant_generating_tree_auto_cast",
	"abilities/heroes/treant/treant_generating_tree",
...
function treant_generating_tree:CreateTree(target_loc)
	...
	local unitsInRadius = FindUnitsInRadius(
	...
	local spawnParticle = ParticleManager:CreateParticle(
...
function treant_generating_tree_auto_cast:IsDebuff()
	return false
end
function treant_generating_tree_auto_cast:IsPurgable()
	return false
end
...
function treant_generating_tree_auto_cast:OnIntervalThink()
	...
	if self.ability:GetCurrentAbilityCharges() ~= self.ability:GetMaxAbilityCharges(self.ability:GetLevel() - 1) then
		return
	end
