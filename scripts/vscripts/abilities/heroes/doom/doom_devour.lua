...
function doom_devour_custom:OnModifierAdded(event)
	if not IsValidEntity(event.unit) or event.unit ~= self:GetCaster() then
		return
	end
	if event.modifier:GetName() ~= "modifier_item_aghanims_shard" then
		return
	end
...
function doom_devour_custom:OnSpellStart()
	...
	local devour_duration =
		math.max(self:GetAbilityChargeRestoreTime(self:GetLevel()), self:GetCooldown(self:GetLevel()))
	...
	if caster:GetUnitName() ~= "npc_dota_hero_doom_bringer" or self:GetAutoCastState() then
		return
	end
...
modifier_doom_devour_custom = {
	...
	GetModifierMagicalResistanceBonus = function(self)
		return self.ability:GetSpecialValueFor("magic_resist")
	end,
...
function modifier_doom_devour_custom:GetModifierAoEBonusPercentage(event)
	if not event or not event.inflictor then
		return
	end
	if not event.inflictor._neutral_creep_ability then
		return
	end
...
function modifier_doom_devour_custom_timer:IsHidden()
	return true
end
function modifier_doom_devour_custom_timer:IsPurgable()
	return false
end
function modifier_doom_devour_custom_timer:RemoveOnDeath()
	return false
end
...
function modifier_doom_devour_custom_timer:OnDestroy()
	if not IsServer() or not self.listener then
		return
	end
...
function modifier_doom_devour_custom_timer:OnUpgradeSelected(event)
	local parent = self:GetParent()
	if event.hero ~= parent then
		return
	end

	if event.upgrade_data.upgrade_name ~= "bonus_creep_level" then
		return
	end
