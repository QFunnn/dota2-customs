...
LinkLuaModifier(
	"modifer_alpha_wolf_critical_strike_lua",
	"abilities/heroes/doom/alpha_wolf_critical_strike",
...
function modifer_alpha_wolf_critical_strike_lua:IsHidden()
	return true
end
function modifer_alpha_wolf_critical_strike_lua:IsPurgable()
	return false
end
function modifer_alpha_wolf_critical_strike_lua:GetAttributes()
...
function modifer_alpha_wolf_critical_strike_lua:GetModifierPreAttack_CriticalStrike(event)
	if event.attacker:PassivesDisabled() then
		return
	end
	if event.target:GetTeamNumber() == event.attacker:GetTeamNumber() then
		return
	end
	if event.target:IsOther() or event.target:IsBuilding() then
		return
	end
