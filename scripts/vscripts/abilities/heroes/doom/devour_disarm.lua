...
function modifier_devour_disarm:IsHidden()
...
function modifier_devour_disarm:GetModifierProcAttack_Feedback(kv)
	...
	if kv.attacker ~= self.caster then
		return
	end
	if kv.target:GetTeamNumber() == kv.attacker:GetTeamNumber() then
		return
	end
	...
	if self:GetStackCount() == self.stack_count_proc then
		kv.target:AddNewModifier(
			...
			self.ability,
			"modifier_disarmed",
			{ duration = self.duration * (1 - kv.target:GetStatusResistance()) }
		)
