...
function modifier_vengefulspirit_command_aura_scepter_debuff_lua:IsHidden()
	return false
end
function modifier_vengefulspirit_command_aura_scepter_debuff_lua:IsDebuff()
	return true
end
function modifier_vengefulspirit_command_aura_scepter_debuff_lua:IsPurgable()
	return false
end
...
function modifier_vengefulspirit_command_aura_scepter_debuff_lua:OnDeath(event)
	...
	Timers:CreateTimer(0.1, function()
		...
		for _, illusion in pairs(illusions) do
			if
				IsValidEntity(illusion)
				and illusion:IsIllusion()
				and illusion:GetPlayerOwnerID() == self.parent:GetPlayerOwnerID()
				and illusion:HasModifier("modifier_vengefulspirit_hybrid_special")
			then
				illusion:RemoveSelf()
			end
		end
