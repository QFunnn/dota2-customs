...
LinkLuaModifier(
	"modifier_lycan_apex_predator_custom",
	"abilities/heroes/lycan/lycan_apex_predator",
...
function modifier_lycan_apex_predator_custom:IsHidden()
	return true
end
function modifier_lycan_apex_predator_custom:IsPurgable()
	return false
end
function modifier_lycan_apex_predator_custom:RemoveOnDeath()
	return false
end
...
function modifier_lycan_apex_predator_custom:GetModifierTotalDamageOutgoing_Percentage(params)
	if
		IsValidEntity(self.parent)
		and IsValidEntity(params.target)
		and params.target:HasModifier("modifier_capturing_orb_custom")
	then
