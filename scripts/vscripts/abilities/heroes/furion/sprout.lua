...
function furion_sprout_custom:OnSpellStart()
	...
	if caster:HasTalent("special_bonus_unique_furion_4") or caster:HasTalent("special_bonus_unique_furion_7") then
		local thinker = CreateModifierThinker(
			caster,
			self,
			"modifier_furion_sprout_talent_thinker",
...
function furion_sprout_custom:KillFirstLargeTreant()
	...
	if
		first_large_treant
		and not first_large_treant:IsNull()
		and IsValidEntity(first_large_treant)
		and first_large_treant:IsAlive()
	then
...
function modifier_furion_sprout_talent_thinker:IsHidden()
	return true
end
function modifier_furion_sprout_talent_thinker:IsPurgable()
	return false
end
...
function modifier_furion_sprout_talent_thinker:OnCreated(kv) end

function modifier_furion_sprout_talent_thinker:OnRefresh(kv) end
