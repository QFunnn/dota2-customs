...
LinkLuaModifier(
	"modifier_demonic_conversion_custom_split",
	"abilities/heroes/enigma/enigma_demonic_conversion.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_demonic_conversion_magic_resist",
	"abilities/heroes/enigma/enigma_demonic_conversion.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function enigma_demonic_conversion_custom:SpawnEidolons(
	unit_name,
	unit_origin,
	caster,
	spawn_count,
...
function modifier_demonic_conversion_custom_split:IsHidden()
	return true
end
function modifier_demonic_conversion_custom_split:IsPurgable()
	return false
end
function modifier_demonic_conversion_custom_split:IsDebuff()
	return true
end
...
function modifier_demonic_conversion_custom_split:GetModifierProcAttack_Feedback(event)
	...
	if self.attack_counter >= self.split_attack_count and self.parent.can_refresh_duration then
		...
		if self.parent.can_produce_eidolon then
			...
			-- spawn a new eidolon
			self.ability:SpawnEidolons(
				self.parent:GetUnitName(),
...
function modifier_demonic_conversion_magic_resist:IsHidden()
	return true
end
function modifier_demonic_conversion_magic_resist:IsPurgable()
	return false
end
