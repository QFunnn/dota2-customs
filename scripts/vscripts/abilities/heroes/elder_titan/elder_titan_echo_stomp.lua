...
function elder_titan_echo_stomp_lua:OnAbilityPhaseStart()
	...
	if self.caster == self.ancestral_spirit and not self.ancestral_spirit.is_hidden then
	...
	if ancestral_spirit_ability and not ancestral_spirit_ability:IsNull() then
		...
		if
			...
			and not self.ancestral_spirit.is_hidden
		then
			...
			then
				...
				Timers:CreateTimer(0.01, function()
					if
						self
						and self.ancestral_spirit
						and IsValidEntity(self.ancestral_spirit)
						and ancestral_spirit_echo_stomp
					then
						self.ancestral_spirit:RemoveGesture(ACT_DOTA_FLAIL)
						self.ancestral_spirit:CastAbilityNoTarget(
							ancestral_spirit_echo_stomp,
							self.ancestral_spirit:GetPlayerOwnerID()
						)
					end
				end)
			end
	...
	self.particle_echo_stomp_cast_combined =
		ParticleManager:CreateParticle(particle_cast, PATTACH_ABSORIGIN_FOLLOW, self.caster)
	return true
end
...
function elder_titan_echo_stomp_lua:OnChannelFinish(interrupted)
	...
	if self.caster and self and IsValidEntity(self.caster) and IsValidEntity(self) then
		...
		for _, enemy in pairs(enemies) do
			...
			enemy:AddNewModifier(
				...
				{ duration = self.sleep_duration * (1 - enemy:GetStatusResistance()) }
			)
		end
		...
		if
			(self.caster == self.ancestral_spirit)
			or (self.caster ~= self.ancestral_spirit and not self.ancestral_spirit)
		then
			local particle_cast = ParticleManager:GetParticleReplacement(
				"particles/units/heroes/hero_elder_titan/elder_titan_echo_stomp_magical.vpcf",
				self.caster
			)
			local particle_magical_stomp_fx =
				ParticleManager:CreateParticle(particle_cast, PATTACH_ABSORIGIN, self.caster)
		...
		if self.caster ~= self.ancestral_spirit then
			...
			local particle_physical_stomp_fx =
				ParticleManager:CreateParticle(particle_cast, PATTACH_ABSORIGIN, self.caster)
	...
	if self.caster == self.owner then
		...
		if not self.ancestral_spirit or self.ancestral_spirit:IsNull() or not IsValidEntity(self.ancestral_spirit) then
			return
		end

		local ancestral_spirit_echo_stomp = self.ancestral_spirit:FindAbilityByName("elder_titan_echo_stomp_lua")
		if not ancestral_spirit_echo_stomp or ancestral_spirit_echo_stomp:IsNull() then
			return
		end
		if ancestral_spirit_echo_stomp:IsInAbilityPhase() or ancestral_spirit_echo_stomp:IsChanneling() then
			return
		end
	...
	if self.caster == self.ancestral_spirit then
		if not self.owner:HasShard() then
			return
		end

		local echo_stomp = self.owner:FindAbilityByName("elder_titan_echo_stomp_lua")
		if not echo_stomp or echo_stomp:IsNull() then
			return
		end
		if not echo_stomp:GetAutoCastState() then
			return
		end
		if echo_stomp:IsInAbilityPhase() or echo_stomp:IsChanneling() then
			return
		end
