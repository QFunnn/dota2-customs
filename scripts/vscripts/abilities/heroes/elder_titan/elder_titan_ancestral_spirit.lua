...
LinkLuaModifier(
	"modifier_elder_titan_ancestral_spirit_lua",
	"abilities/heroes/elder_titan/elder_titan_ancestral_spirit",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_elder_titan_ancestral_spirit_buff_lua",
	"abilities/heroes/elder_titan/elder_titan_ancestral_spirit",
	LUA_MODIFIER_MOTION_NONE
)
...
function elder_titan_ancestral_spirit_lua:GetAssociatedSecondaryAbilities()
	return "elder_titan_return_spirit_lua"
end

function elder_titan_ancestral_spirit_lua:Spawn()
...
function elder_titan_ancestral_spirit_lua:OnSpellStart()
	...
	if not IsValidEntity(self.ancestral_spirit) then
		self:InitSpirit()
	end

	self.ancestral_spirit:SetOrigin(position)
	...
	self:UpdateSpiritAbilities()

	EmitSoundOn("Hero_ElderTitan.AncestralSpirit.Spawn", self.ancestral_spirit)

	-- reset modifier from sleeping state
	self.ancestral_spirit.modifier:SetHidden(false)
	self.ancestral_spirit.modifier:OnCreated()

	self.caster:SwapAbilities(self:GetAbilityName(), "elder_titan_return_spirit_lua", false, true)
end

function elder_titan_ancestral_spirit_lua:OnUnStolen()
	if IsValidEntity(self.ancestral_spirit) then
		self.ancestral_spirit:Destroy()
	end
end
...
function elder_titan_ancestral_spirit_lua:InitSpirit()
	self.caster = self:GetCaster()
	local fv = self.caster:GetForwardVector()

	self.ancestral_spirit = CreateUnitByName(
	...
	local m = self.ancestral_spirit:AddNewModifier(
		self.caster,
		self,
		"modifier_elder_titan_ancestral_spirit_lua",
		{ duration = -1 }
	...
	m:SetHidden(true)
	self.ancestral_spirit.modifier = m

	self:UpdateSpiritAbilities()
end

function elder_titan_ancestral_spirit_lua:UpdateSpiritAbilities()
	if not IsValidEntity(self.ancestral_spirit) then
		return
	end
	...
	for ability_name, _ in pairs(self.elder_titan_abilities) do
		...
		if caster_ability and not caster_ability:IsNull() then
			...
			if ancestral_spirit_ability and not ancestral_spirit_ability:IsNull() then
				...
				ancestral_spirit_ability:SetLevel(math.max(caster_ability:GetLevel(), 1))
			end
		end
	end
end
...
function elder_titan_return_spirit_lua:IsStealable()
	return false
end
function elder_titan_return_spirit_lua:IsHiddenWhenStolen()
	return true
end
function elder_titan_return_spirit_lua:IsHiddenAsSecondaryAbility()
	return true
end
function elder_titan_return_spirit_lua:ProcsMagicStick()
	return false
end
...
function elder_titan_return_spirit_lua:OnSpellStart()
	...
	if self.caster:GetUnitName() == "npc_dota_elder_titan_ancestral_spirit" then
		...
		if not self.owner or self.owner:IsNull() then
			return
		end
	else
		-- Fetch the spirit
	...
	if
		self.ancestral_spirit
		and not self.ancestral_spirit:IsNull()
		and IsValidEntity(self.ancestral_spirit)
		and not self.ancestral_spirit.is_returning
	then
		...
		if
			(echo_stomp and not echo_stomp:IsNull() and (echo_stomp:IsInAbilityPhase() or echo_stomp:IsChanneling()))
			or (
				ancestral_spirit_echo_stomp
				and not ancestral_spirit_echo_stomp:IsNull()
				and (ancestral_spirit_echo_stomp:IsInAbilityPhase() or ancestral_spirit_echo_stomp:IsChanneling())
			)
		then
			return
		end
		...
		self.ancestral_spirit:StartGesture(ACT_DOTA_FLAIL)
...
function elder_titan_move_spirit_lua:IsStealable()
	return false
end
function elder_titan_move_spirit_lua:ProcsMagicStick()
	return false
end
...
function modifier_elder_titan_ancestral_spirit_lua:IsHidden()
	return false
end
function modifier_elder_titan_ancestral_spirit_lua:IsPurgable()
	return false
end
...
function modifier_elder_titan_ancestral_spirit_lua:OnCreated(kv)
	...
	self.parent.is_returning = false

	self:StartIntervalThink(0.1)
	self:SendBuffRefreshToClients()
end

function modifier_elder_titan_ancestral_spirit_lua:SetHidden(state)
	self.is_hidden = state
	self.parent.is_hidden = state
...
function modifier_elder_titan_ancestral_spirit_lua:HandleCustomTransmitterData(kv)
	...
	self.speed = kv.speed
	self.return_speed = kv.return_speed
end
...
function modifier_elder_titan_ancestral_spirit_lua:GetModifierMoveSpeed_Absolute()
	if (IsServer() and self:GetParent().is_returning) or (IsClient() and self.is_returning == 1) then
...
function modifier_elder_titan_ancestral_spirit_lua:OnIntervalThink()
	...
	if self.is_hidden then
		self.parent:SetOrigin(self.owner:GetAbsOrigin())
		return
	end
	...
	if self.parent.is_returning then
		if
			echo_stomp
			and not echo_stomp:IsNull()
			and not echo_stomp:IsInAbilityPhase()
			and not echo_stomp:IsChanneling()
			and ancestral_spirit_echo_stomp
			and not ancestral_spirit_echo_stomp:IsNull()
			and not ancestral_spirit_echo_stomp:IsInAbilityPhase()
			and not ancestral_spirit_echo_stomp:IsChanneling()
		then
			self.parent:MoveToNPC(self.owner)
		end
	else
		if
			(GameRules:GetGameTime() >= self.game_time_start + self.spirit_duration)
			and ancestral_spirit_echo_stomp
			and not ancestral_spirit_echo_stomp:IsNull()
			and not ancestral_spirit_echo_stomp:IsInAbilityPhase()
			and not ancestral_spirit_echo_stomp:IsChanneling()
	...
	if (self.owner:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.hull_distance then
		if ancestral_spirit_echo_stomp and not ancestral_spirit_echo_stomp:IsNull() then
			if
				not ancestral_spirit_echo_stomp:IsInAbilityPhase() and not ancestral_spirit_echo_stomp:IsChanneling()
			then
				if self.bonus_damage > 0 then
					self.owner:AddNewModifier(
						self.owner,
						self.ability,
						"modifier_elder_titan_ancestral_spirit_buff_lua",
						{
...
function modifier_elder_titan_ancestral_spirit_buff_lua:IsHidden()
	return false
end
function modifier_elder_titan_ancestral_spirit_buff_lua:IsPurgable()
	return true
end
function modifier_elder_titan_ancestral_spirit_buff_lua:IsBuff()
	return true
end
function modifier_elder_titan_ancestral_spirit_buff_lua:IsDebuff()
	return false
end
