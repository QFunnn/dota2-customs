...
LinkLuaModifier(
	"modifier_razor_static_link_custom",
...
LinkLuaModifier(
	"modifier_razor_static_link_custom_legendary",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_legendary_no_damage",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_legendary_swap",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_legendary_target",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_spell",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_heal",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_static_link_custom_armor",
	"abilities/razor/razor_static_link_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function razor_static_link_custom:OnSpellStart(new_target)
	...
	local duration = self:GetSpecialValueFor("drain_length")
		+ caster:GetTalentValue("modifier_razor_link_6", "link_duration")
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_razor_static_link_custom",
		{ target = target:entindex(), duration = duration }
	)
end

modifier_razor_static_link_custom = class({})
function modifier_razor_static_link_custom:IsHidden()
	return true
end
function modifier_razor_static_link_custom:IsPurgable()
	return false
end
function modifier_razor_static_link_custom:OnCreated(table)
	...
	if mod and self.caster:HasTalent("modifier_razor_link_4") then
		self.total_damage = self.total_damage
			+ mod:GetStackCount() * self.caster:GetTalentValue("modifier_razor_link_4", "damage")
	end
	...
	if self.caster:HasTalent("modifier_razor_link_5") then
		self.sphere = ParticleManager:CreateParticle(
	...
	self.max_range = self.ability:GetSpecialValueFor("AbilityCastRange")
		+ self.caster:GetTalentValue("modifier_razor_link_2", "range")
		+ self.caster:GetCastRangeBonus()
		+ self.ability:GetSpecialValueFor("drain_range_buffer")
	...
	self.target_mod =
		self.target:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_target", {})
	self.caster_mod =
		self.caster:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_caster", {})
	...
	if self.caster:HasTalent("modifier_razor_link_1") then
		...
		self.caster_mod_armor =
			self.caster:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_armor", {})
		self.target_mod_armor =
			self.target:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_armor", {})
	end
	...
	if self.caster:HasTalent("modifier_razor_link_3") then
		...
		self.target_mod_spell =
			self.target:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_spell", {})
	end

	if self.caster:HasTalent("modifier_razor_link_4") then
		self.heal_mod =
			self.caster:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_heal", {})
	end

	if self.caster:HasShard() then
		self.leash_mod = self.target:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_razor_static_link_custom_leash",
			{ duration = (1 - self.target:GetStatusResistance()) * self.ability:GetSpecialValueFor("shard_leash") }
		)
	end

	local particleFile = wearables_system:GetParticleReplacementAbility(
		self.caster,
	...
	if self.caster:HasTalent("modifier_razor_link_6") and self.ability:GetAutoCastState() == true then
		...
		local distance = math.min(
			self.caster:GetTalentValue("modifier_razor_link_6", "knock_distance_max"),
			dir:Length2D() * self.caster:GetTalentValue("modifier_razor_link_6", "knock_distance") / 100
		)
...
function modifier_razor_static_link_custom:OnIntervalThink()
	...
	if
		...
		or (self.target:GetAbsOrigin() - self.caster:GetAbsOrigin()):Length2D() > self.max_range
	then
		self:Destroy()
		return
	end
...
function modifier_razor_static_link_custom:OnDestroy()
	...
	if self:GetRemainingTime() <= 0.1 or not self.target or self.target:IsNull() or not self.target:IsAlive() then
		...
		if self.caster:HasTalent("modifier_razor_link_6") then
			local effect_cast =
				ParticleManager:CreateParticle("particles/razor/link_stun.vpcf", PATTACH_OVERHEAD_FOLLOW, self.target)
			...
			self.target:AddNewModifier(
				...
				{
					duration = self.caster:GetTalentValue("modifier_razor_link_6", "stun")
	...
	if self.caster:HasTalent("modifier_razor_link_7") then
		local mod = self.caster:AddNewModifier(
			...
			self.ability,
			"modifier_razor_static_link_custom_legendary_swap",
			{ duration = self.caster:GetTalentValue("modifier_razor_link_7", "swap") }
		)
...
function modifier_razor_static_link_custom:GetModifierStatusResistanceStacking()
	if not self.caster:HasTalent("modifier_razor_link_5") then
		return
	end
	return self.status
end
...
function modifier_razor_static_link_custom:GetAbsorbSpell(params)
	if params.ability:GetCaster():GetTeamNumber() == self.caster:GetTeamNumber() then
		return
	end
	...
	local particle =
		ParticleManager:CreateParticle("particles/items_fx/immunity_sphere.vpcf", PATTACH_ABSORIGIN_FOLLOW, self.caster)
	ParticleManager:SetParticleControlEnt(
		particle,
		0,
		self.caster,
...
function modifier_razor_static_link_custom_attacking:IsHidden()
	return true
end
function modifier_razor_static_link_custom_attacking:IsPurgable()
	return false
end
...
function modifier_razor_static_link_custom_attacking:OnIntervalThink()
	...
	if
		self.attack_count >= self.attack_max --and dir:Length2D() <= self.caster:Script_GetAttackRange()
		and not self.caster:IsStunned()
		and not self.caster:IsHexed()
		and not dota1x6:CheckDisarm(self.caster)
		and not self.caster:IsChanneling()
	then
		self.attack_count = 0
	...
	if
		not self.caster:IsCurrentlyHorizontalMotionControlled()
		and not self.caster:IsCurrentlyVerticalMotionControlled()
	then
...
function modifier_razor_static_link_custom_caster:IsHidden()
	return false
end
function modifier_razor_static_link_custom_caster:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_caster:GetAttributes()
...
function modifier_razor_static_link_custom_target:IsHidden()
	return false
end
function modifier_razor_static_link_custom_target:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_target:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_razor_static_link_custom_legendary:IsHidden()
	return true
end
function modifier_razor_static_link_custom_legendary:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_legendary:IsDebuff()
	return false
end
...
function modifier_razor_static_link_custom_legendary:OnCreated(table)
	...
	self.target_mod =
		self.target:AddNewModifier(self.caster, self.ability, "modifier_razor_static_link_custom_legendary_target", {})

	self.particle = ParticleManager:CreateParticle(
		"particles/razor/static_link_beam_custombeam.vpcf",
		PATTACH_POINT_FOLLOW,
		self.target
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_razor_static_link_custom_legendary:OnDestroy()
	...
	self.caster:AddNewModifier(
		...
		self.ability,
		"modifier_razor_static_link_custom_legendary_no_damage",
		{ duration = self.caster:GetTalentValue("modifier_razor_link_7", "lose_duration") }
	)
end
...
function modifier_razor_static_link_custom_legendary_no_damage:IsDebuff()
	return true
end
function modifier_razor_static_link_custom_legendary_no_damage:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_legendary_no_damage:IsHidden()
	return false
end
...
function razor_static_link_custom_legendary:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self.ability,
		"modifier_razor_static_link_custom_legendary",
		{ target = target:entindex(), duration = caster:GetTalentValue("modifier_razor_link_7", "duration") }
	)
end

modifier_razor_static_link_custom_legendary_swap = class({})
function modifier_razor_static_link_custom_legendary_swap:IsHidden()
	return true
end
function modifier_razor_static_link_custom_legendary_swap:IsPurgable()
	return false
end
...
function modifier_razor_static_link_custom_legendary_target:IsHidden()
	return true
end
function modifier_razor_static_link_custom_legendary_target:IsPurgable()
	return false
end
...
function modifier_razor_static_link_custom_spell:IsHidden()
	return false
end
function modifier_razor_static_link_custom_spell:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_spell:GetTexture()
	return "buffs/link_spell"
end
...
function modifier_razor_static_link_custom_armor:IsHidden()
	return false
end
function modifier_razor_static_link_custom_armor:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_armor:GetTexture()
	return "buffs/moment_defence"
end
...
function modifier_razor_static_link_custom_heal:IsHidden()
	return false
end
function modifier_razor_static_link_custom_heal:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_heal:GetTexture()
	return "buffs/link_heal"
end
...
function modifier_razor_static_link_custom_perma:IsHidden()
	return not self:GetParent():HasTalent("modifier_razor_link_4")
end
function modifier_razor_static_link_custom_perma:IsPurgable()
	return false
end
function modifier_razor_static_link_custom_perma:RemoveOnDeath()
	return false
end
function modifier_razor_static_link_custom_perma:GetTexture()
	return "buffs/link_damage"
end
...
function modifier_razor_static_link_custom_perma:OnTooltip()
	if not self.caster:HasTalent("modifier_razor_link_4") then
		return
	end
...
function modifier_razor_static_link_custom_leash:IsHidden()
	return true
end
function modifier_razor_static_link_custom_leash:IsPurgable()
	return false
end
