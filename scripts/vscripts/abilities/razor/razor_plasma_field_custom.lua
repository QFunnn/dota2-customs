...
LinkLuaModifier(
	"modifier_razor_plasma_field_custom",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_slow",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_slow_speed",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_damage_cd",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_stop",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_legendary",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_legendary_anim",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_knock",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_damage",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_speed",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_razor_plasma_field_custom_shield",
	"abilities/razor/razor_plasma_field_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function razor_plasma_field_custom:OnSpellStart()
	...
	if caster:HasTalent("modifier_razor_plasma_4") then
		if
			caster:HasAbility("razor_plasma_field_custom_stop")
			and caster:FindAbilityByName("razor_plasma_field_custom_stop"):IsHidden()
		then
	...
	if caster:HasTalent("modifier_razor_plasma_5") then
		caster:AddNewModifier(
			...
			self,
			"modifier_razor_plasma_field_custom_speed",
			{ duration = caster:GetTalentValue("modifier_razor_plasma_5", "duration") }
		)
	end
...
function modifier_razor_plasma_field_custom:OnCreated(kv)
	...
	self.slow_duration = self.ability:GetSpecialValueFor("slow_duration")
		+ self.caster:GetTalentValue("modifier_razor_plasma_2", "duration")
	self.slow_max = self.ability:GetSpecialValueFor("slow_max")
	self.slow_min = self.ability:GetSpecialValueFor("slow_min")
		+ self.caster:GetTalentValue("modifier_razor_plasma_6", "slow")
...
function modifier_razor_plasma_field_custom:OnDestroy()
	...
	if
		...
		and self.caster:HasAbility("razor_plasma_field_custom_stop")
		and not self.caster:FindAbilityByName("razor_plasma_field_custom_stop"):IsHidden()
	then
...
function modifier_razor_plasma_field_custom:OnIntervalThink()
	...
	local damage_max = (
		...
		+ self.caster:GetAverageTrueAttackDamage(nil)
			* self.caster:GetTalentValue("modifier_razor_plasma_1", "damage")
			/ 100
	) * self.more_damage
	...
	for _, target in pairs(self.caster:FindTargets((self.current_radius + self.width), self.parent:GetOrigin())) do
		...
		if
			(
				not self.targets[target]
				or (self.speed == 0 and not target:HasModifier("modifier_razor_plasma_field_custom_damage_cd"))
			)
			and distance > (real_radius - self.width * 2)
			and (not target:HasModifier("modifier_razor_plasma_field_custom_knock") or self.is_thinker == false)
		then
			dealt_damage = true
			...
			self.damageTable.damage =
				math.max(damage_min, math.min(damage_max, damage_min + (damage_max - damage_min) * k))

			local slow =
				math.max(self.slow_min, math.min(self.slow_max, self.slow_min + (self.slow_max - self.slow_min) * k))

			target:AddNewModifier(
				self.caster,
				self.ability,
				"modifier_razor_plasma_field_custom_slow",
				{ slow = slow, duration = self.slow_duration * (1 - target:GetStatusResistance()) }
			)
			...
			if self.caster:HasTalent("modifier_razor_plasma_2") then
				target:AddNewModifier(
					...
					self.ability,
					"modifier_razor_plasma_field_custom_slow_speed",
					{ duration = self.slow_duration * (1 - target:GetStatusResistance()) }
				)
			end
			...
			if
				self.caster:GetQuest() == "Razor.Quest_5"
				and target:IsRealHero()
				and self.caster.quest.number
				and distance >= self.caster.quest.number
			then
			...
			if self.caster:HasTalent("modifier_razor_plasma_6") and not self.silence_targets[target] then
				...
				target:AddNewModifier(
					...
					{ duration = (1 - target:GetStatusResistance()) * self.silence_duration }
				)
			end
	...
	if
		self.caster:HasTalent("modifier_razor_plasma_4")
		and dealt_damage == true
		and not self.caster:HasModifier("modifier_razor_plasma_field_custom_damage_cd")
	then
...
function modifier_razor_plasma_field_custom_slow:IsHidden()
	return false
end
function modifier_razor_plasma_field_custom_slow:IsPurgable()
	return true
end
function modifier_razor_plasma_field_custom_slow:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
...
function modifier_razor_plasma_field_custom_slow_speed:IsHidden()
	return true
end
function modifier_razor_plasma_field_custom_slow_speed:IsPurgable()
	return true
end
...
function modifier_razor_plasma_field_custom_damage_cd:IsHidden()
	return true
end
function modifier_razor_plasma_field_custom_damage_cd:IsPurgable()
	return false
end
...
function modifier_razor_plasma_field_custom_stop:IsHidden()
	return false
end
function modifier_razor_plasma_field_custom_stop:IsPurgable()
	return false
end
...
function razor_plasma_field_custom_clone:GetBehavior()
	local bonus = 0
	if
		self:GetCaster():HasModifier("modifier_razor_static_link_custom")
		or self:GetCaster():HasModifier("modifier_razor_static_link_custom_legendary")
	then
...
function razor_plasma_field_custom_clone:OnSpellStart()
	...
	for _, illusion in pairs(illusion_self) do
		...
		cast_particle = ParticleManager:CreateParticle(
...
function modifier_razor_plasma_field_custom_legendary:IsHidden()
	return false
end
function modifier_razor_plasma_field_custom_legendary:IsPurgable()
	return false
end
function modifier_razor_plasma_field_custom_legendary:GetStatusEffectName()
...
function modifier_razor_plasma_field_custom_legendary:OnDestroy()
	...
	local particle_explode_fx = ParticleManager:CreateParticle(
	...
	if self.plasma_ability and self.parent:GetHealth() > 1 then
		...
		for _, target in pairs(targets) do
			if target:IsHero() then
				...
				local knock = target:AddNewModifier(caster, self, "modifier_generic_knockback", {
...
function modifier_razor_plasma_field_custom_legendary:OnCreated(table)
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_razor_plasma_field_custom_legendary_anim",
		{ duration = self.caster:GetTalentValue("modifier_razor_plasma_7", "delay") }
	)
	self.count = 0
...
function modifier_razor_plasma_field_custom_legendary_anim:IsHidden()
	return true
end
function modifier_razor_plasma_field_custom_legendary_anim:IsPurgable()
	return false
end
...
function modifier_razor_plasma_field_custom_damage:IsHidden()
	return false
end
function modifier_razor_plasma_field_custom_damage:IsPurgable()
	return false
end
function modifier_razor_plasma_field_custom_damage:GetTexture()
	return "buffs/plasma_damage"
end
...
function modifier_razor_plasma_field_custom_speed:IsHidden()
	return true
end
function modifier_razor_plasma_field_custom_speed:IsPurgable()
	return false
end
...
function modifier_razor_plasma_field_custom_shield:IsHidden()
	return false
end
function modifier_razor_plasma_field_custom_shield:IsPurgable()
	return false
end
function modifier_razor_plasma_field_custom_shield:GetTexture()
	return "buffs/laguna_shield"
end
function modifier_razor_plasma_field_custom_shield:GetStatusEffectName()
	return "particles/status_fx/status_effect_mjollnir_shield.vpcf"
end
function modifier_razor_plasma_field_custom_shield:StatusEffectPriority()
...
function modifier_razor_plasma_field_custom_shield:OnCreated(table)
	...
	self.add_shiled = self.max_shield
		/ (
			self.parent:GetTalentValue("modifier_razor_plasma_5", "duration")
			/ self.parent:GetTalentValue("modifier_razor_plasma_5", "interval")
		)
...
function modifier_razor_plasma_field_custom_knock:IsHidden()
	return true
end
function modifier_razor_plasma_field_custom_knock:IsPurgable()
	return false
end