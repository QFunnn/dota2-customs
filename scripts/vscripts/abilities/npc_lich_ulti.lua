...
function npc_lich_ulti:OnSpellStart()
	...
	self:GetCaster()
		:AddNewModifier(self:GetCaster(), self, "modifier_lich_ulti_cd", { duration = self:GetCooldownTimeRemaining() })
...
function npc_lich_ulti:OnProjectileHit(hTarget, vLocation)
	...
	for _, i in pairs(enemy_for_ability) do
		if
			(
				(i:GetTeamNumber() == self:GetCaster():GetTeamNumber() and i:GetUnitName() == "npc_lich_ice_unit")
				or (i:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and (i:IsCreep() or i:IsHero()))
			) and (i ~= hTarget)
		then
			j = j + 1
			target[j] = i
...
function modifier_lich_ulti_cd:IsHidden()
	return false
end
function modifier_lich_ulti_cd:IsPurgable()
	return false
end