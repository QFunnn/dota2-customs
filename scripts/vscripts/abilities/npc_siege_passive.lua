...
function modifier_npc_siege_dmg:IsHidden()
	return true
end
function modifier_npc_siege_dmg:IsPurgable()
	return false
end
...
function modifier_npc_siege_dmg:DeathEvent(params)
	...
	if params.unit == self:GetParent() then
		...
		if self:GetParent():GetUnitName() == "npc_goodsiege_a" then
			name = "particles/siege_fx/siege_good_death_01.vpcf"
		else
			name = "particles/siege_fx/siege_bad_death_01.vpcf"
		end
