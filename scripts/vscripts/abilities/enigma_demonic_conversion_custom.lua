...
LinkLuaModifier(
	"modifier_enigma_demonic_conversion_custom_creep",
	"abilities/enigma_demonic_conversion_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_enigma_eidolon_overseer",
	"abilities/enigma_demonic_conversion_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function enigma_demonic_conversion_custom:GetCustomCastErrorTarget(target)
	...
	if
		...
		and target:GetPlayerOwnerID() ~= -1
		and target:GetPlayerOwnerID() ~= self:GetCaster():GetPlayerOwnerID()
	then
		return "Нельзя использовать на крипов других игроков"
	end
...
function enigma_demonic_conversion_custom:CreateEidolon(hParent, vLocation)
	...
	local eidolon =
		CreateUnitByName(sType, vLocation, true, self:GetCaster(), self:GetCaster(), self:GetCaster():GetTeamNumber())
...
function modifier_enigma_demonic_conversion_custom_creep:IsHidden()
	return true
end
function modifier_enigma_demonic_conversion_custom_creep:IsPurgable()
	return false
end
function modifier_enigma_demonic_conversion_custom_creep:IsPurgeException()
	return false
end
...
function modifier_enigma_demonic_conversion_custom_creep:OnCreated()
	...
	self.dmg = self.ability:GetSpecialValueFor("eidelon_base_damage")
		+ self.ability:GetSpecialValueFor("eidolon_bonus_damage")
...
function modifier_enigma_eidolon_overseer:IsHidden()
	return true
end
function modifier_enigma_eidolon_overseer:IsPurgable()
	return false
end
function modifier_enigma_eidolon_overseer:RemoveOnDeath()
	return false
end
...
function modifier_enigma_eidolon_overseer:BelongsToMe(u)
	if u:GetPlayerOwnerID() == self.pid then
		return true
	end
