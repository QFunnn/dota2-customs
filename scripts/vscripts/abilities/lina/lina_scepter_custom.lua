	...
	if mod then
		if IsValid(self.lina_clone) then
			...
			self.caster:AddNewModifier(
				self.caster,
				self,
				"modifier_lina_scepter_custom_blink",
				{ duration = 0.1, x = pos.x, y = pos.y }
			)
...
function modifier_lina_scepter_custom:GetStatusEffectName()
	return "particles/status_fx/status_effect_qop_tgt_arcana.vpcf"
end
function modifier_lina_scepter_custom:StatusEffectPriority()
	return MODIFIER_PRIORITY_ILLUSION
end
function modifier_lina_scepter_custom:GetEffectName()
...
function modifier_lina_scepter_custom:OnDestroy()
	...
	for _, target in pairs(self.caster:FindTargets(self.ability.radius, self.parent:GetAbsOrigin())) do
		...
		target:AddNewModifier(
			...
			self.ability,
			"modifier_stunned",
			{ duration = (1 - target:GetStatusResistance()) * self.ability.stun }
		)
	end
