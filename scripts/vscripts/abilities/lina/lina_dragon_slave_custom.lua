...
LinkLuaModifier(
	"modifier_lina_dragon_slave_custom_slow",
	"abilities/lina/lina_dragon_slave_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
function lina_dragon_slave_custom:GetManaCost(level)
	return self.BaseClass.GetManaCost(self, level)
		* (1 + (self.caster:HasModifier("modifier_lina_dragon_slave_custom_legendary") and self.talents.q7_mana or 0))
end
...
function lina_dragon_slave_custom:OnProjectileHit_ExtraData(target, vLocation, table)
	...
	if
		target:IsRealHero()
		and self.caster:GetQuest() == "Lina.Quest_5"
	...
	local particle_impact = wearables_system:GetParticleReplacementAbility(
		self.caster,
		"particles/units/heroes/hero_lina/lina_spell_dragon_slave_impact.vpcf",
		self
	)
	if particle_impact == "particles/units/heroes/hero_lina/lina_spell_dragon_slave_impact.vpcf" then
		return
	end
	if not table.x or not table.y then
		return
	end
...
function modifier_lina_dragon_slave_custom_tracker:SpellEvent(params)
	...
	if self.parent:HasModifier("modifier_lina_dragon_slave_custom_legendary") then
		return
	end
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_lina_dragon_slave_custom_legendary_stack",
		{ duration = self.ability.talents.q7_linger }
	)
end
...
function modifier_lina_dragon_slave_custom_legendary:StatusEffectPriority()
...
function modifier_lina_dragon_slave_custom_slow:IsPurgable()
	return true
end
...
function modifier_lina_dragon_slave_custom_proc:GetTexture()
	return "buffs/lina/hero_3"
end
