...
function npc_lich_ice:OnSpellStart()
	...
	local pilar = CreateUnitByName(
		"npc_lich_ice_unit",
		self:GetAbsOrigin() + RandomVector(RandomInt(-1, 1) + self.radius),
		true,
		nil,
		nil,
		DOTA_TEAM_CUSTOM_5
...
function modifier_lich_ice_resist:IsHidden()
	return true
end
function modifier_lich_ice_resist:IsPurgable()
	return false
end
...
function modifier_lich_ice_resist:OnCreated(table)
	...
	self.particle =
		ParticleManager:CreateParticle("particles/items_fx/glyph.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_lich_ice_resist:AttackEvent_inc(params)
	...
	CustomGameEventManager:Send_ServerToPlayer(
		PlayerResource:GetPlayer(params.attacker:GetPlayerID()),
...
function modifier_lich_ice:IsHidden()
	return true
end
function modifier_lich_ice:IsPurgable()
	return false
end
...
function modifier_lich_ice:DeathEvent(params)
	...
	self:GetCaster():AddNewModifier(
		self:GetCaster(),
		self:GetAbility(),
		"modifier_lich_ice_cd",
		{
			duration = self:GetAbility():GetSpecialValueFor("cd") - (self:GetCaster()
				:GetUpgradeStack("modifier_waveupgrade_boss") - 1) * self:GetAbility():GetSpecialValueFor("cd_inc"),
		}
	)
end
...
function modifier_lich_ice_cd:IsHidden()
	return false
end
function modifier_lich_ice_cd:IsPurgable()
	return false
end