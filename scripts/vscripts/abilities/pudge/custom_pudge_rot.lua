...
LinkLuaModifier(
	"modifier_custom_pudge_rot_legendary_aura",
	"abilities/pudge/custom_pudge_rot",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_rot_legendary_aura_buff",
	"abilities/pudge/custom_pudge_rot",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_pudge_rot:ResetToggleOnRespawn()
	return true
end
...
function modifier_custom_pudge_rot:IsPurgable()
	return false
end
function modifier_custom_pudge_rot:IsHidden()
	return false
end
...
function modifier_custom_pudge_rot:OnCreated()
	...
	self.rot_radius = self.ability:GetSpecialValueFor("rot_radius")
		+ self.parent:GetTalentValue("modifier_pudge_rot_1", "radius")
	...
	if self.parent:HasTalent("modifier_pudge_rot_legendary") then
		...
		self.count_effect = self.parent:GenericParticle(
			"particles/units/heroes/hero_centaur/centaur_shard_buff_strength_counter_stack.vpcf",
			self,
			true
		)
...
function modifier_custom_pudge_rot:OnIntervalThink()
	...
	if self.stack_count >= 0.99 then
		self.stack_count = 0
		if
			(self.parent:HasTalent("modifier_pudge_rot_3") or self.parent:HasTalent("modifier_pudge_rot_6"))
			and self:GetStackCount() < self.stack_max
		then
			self:IncrementStackCount()
		end

		if
			self.parent:HasTalent("modifier_pudge_rot_6")
			and not self.parent:HasModifier("modifier_custom_pudge_rot_shield")
		then
			...
			if self.shield_count >= self.shield_max then
				...
				self.parent:AddNewModifier(
					self.parent,
					self.ability,
					"modifier_custom_pudge_rot_shield",
	...
	if self.parent:HasTalent("modifier_pudge_rot_legendary") then
		...
		self.parent:UpdateUIshort({
			max_time = self.legendary_max,
			time = self.legendary_stack,
...
function modifier_custom_pudge_rot:GetModifierPhysicalArmorBonus()
	if not self.parent:HasTalent("modifier_pudge_rot_3") then
		return
	end
...
function modifier_custom_pudge_rot_legendary_aura:IsHidden()
	return true
end
function modifier_custom_pudge_rot_legendary_aura:IsPurgable()
	return false
end
...
function modifier_custom_pudge_rot_legendary_aura_buff:IsHidden()
	return true
end
function modifier_custom_pudge_rot_legendary_aura_buff:IsPurgable()
	return false
end
function modifier_custom_pudge_rot_legendary_aura_buff:OnCreated(table)
	...
	self.pfx = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_enigma/pudge_pull.vpcf",
...
function modifier_custom_pudge_rot_slow:IsHidden()
	return false
end
function modifier_custom_pudge_rot_slow:IsPurgable()
	return false
end
...
function modifier_custom_pudge_rot_slow:OnCreated(table)
	...
	if not self.caster:HasTalent("modifier_pudge_rot_4") and not self.caster:HasTalent("modifier_pudge_rot_5") then
		return
	end
	self.interval = 0.5
	self.count = 0
...
function modifier_custom_pudge_rot_slow:OnIntervalThink()
	...
	if self.parent:HasModifier("modifier_custom_pudge_rot_poison_cd") then
		return
	end
	...
	if self:GetStackCount() < self.resist_interval then
		return
	end
	...
	self.parent:AddNewModifier(
		self.caster,
		self.ability,
		"modifier_custom_pudge_rot_poison_cd",
	...
	if self.caster:HasTalent("modifier_pudge_rot_4") then
		...
		self.parent:AddNewModifier(
			self.caster,
			self.ability,
			"modifier_custom_pudge_rot_poison",
			{ duration = self.resist_duration }
		)
	end
...
function modifier_custom_pudge_rot_slow:GetModifierSpellAmplify_Percentage()
	if not self.caster:HasTalent("modifier_pudge_rot_6") then
		return
	end
...
function modifier_custom_pudge_rot_slow:OnStackCountChanged(iStackCount)
	...
	if self:GetStackCount() > 0 and not self.parent:HasModifier("modifier_custom_pudge_meat_hook_stack") then
		if not self.particle then
			self.particle =
				self.parent:GenericParticle("particles/units/heroes/hero_drow/rot_silence_stack.vpcf", self, true)
		end
...
function modifier_custom_pudge_rot_tracker:IsHidden()
	return true
end
function modifier_custom_pudge_rot_tracker:IsPurgable()
	return false
end
...
function modifier_custom_pudge_rot_tracker:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_pudge_rot_4") then
		return
	end
	if not params.unit:HasModifier("modifier_custom_pudge_rot_poison") then
		return
	end
...
function modifier_custom_pudge_rot_poison:IsHidden()
	return false
end
function modifier_custom_pudge_rot_poison:GetTexture()
	return "buffs/goo_ground"
end
function modifier_custom_pudge_rot_poison:IsPurgable()
	return false
end
...
function modifier_custom_pudge_rot_poison_cd:IsHidden()
	return true
end
function modifier_custom_pudge_rot_poison_cd:IsPurgable()
	return false
end

modifier_custom_pudge_rot_shield = class({})
function modifier_custom_pudge_rot_shield:IsHidden()
	return true
end
function modifier_custom_pudge_rot_shield:IsPurgable()
	return false
end
