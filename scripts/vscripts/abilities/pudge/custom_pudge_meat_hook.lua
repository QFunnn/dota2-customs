...
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_debuff",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_blood",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_root",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_speed",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_tracker",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_hidden",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_move_speed",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_move_slow",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_meat_hook_move_vision",
	"abilities/pudge/custom_pudge_meat_hook",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_pudge_meat_hook:OnSpellStart()
	...
	self.hook_sound_retract =
		wearables_system:GetSoundReplacement(self:GetCaster(), "Hero_Pudge.AttackHookRetract", self)
...
function custom_pudge_meat_hook:UseHook(direction, distance)
	...
	self.hook_speed = self:GetSpecialValueFor("hook_speed")
		* (1 + caster:GetTalentValue("modifier_pudge_hook_2", "speed") / 100)
	...
	local pudge_meat_hook_particle = wearables_system:GetParticleReplacementAbility(
		self:GetCaster(),
...
function custom_pudge_meat_hook:OnProjectileHitHandle(target, position, projectileIndex)
	...
	if self.hooks[projectileIndex].bRetracting == false then
		...
		if target ~= nil then
			if
				target:HasModifier("modifier_custom_pudge_meat_hook_debuff")
				or target:GetUnitName() == "npc_teleport"
				or target:GetUnitName() == "npc_psi_blades_crystal"
				or target:GetUnitName() == "npc_psi_blades_crystal_mini"
			then
				return false
			end
			...
			target:EmitSound(
				wearables_system:GetSoundReplacement(self:GetCaster(), "Hero_Pudge.AttackHookImpact", self)
			)
			...
			local damage = self:GetSpecialValueFor("damage")
				+ caster:GetAverageTrueAttackDamage(nil)
					* caster:GetTalentValue("modifier_pudge_hook_1", "damage")
					/ 100
			...
			if target:GetTeamNumber() ~= caster:GetTeamNumber() then
				...
				if
					caster:GetQuest() == "Pudge.Quest_5"
					and target:IsRealHero()
					and (caster:GetAbsOrigin() - target:GetAbsOrigin()):Length2D() >= caster.quest.number
				then
				...
				if caster:HasTalent("modifier_pudge_hook_legendary") then
					target:AddNewModifier(
						...
						self,
						"modifier_custom_pudge_meat_hook_stack",
						{ duration = caster:GetTalentValue("modifier_pudge_hook_legendary", "duration") }
					)
				end
				...
				if caster:HasTalent("modifier_pudge_hook_1") or caster:HasTalent("modifier_pudge_hook_3") then
					caster:AddNewModifier(
						...
						self,
						"modifier_custom_pudge_meat_hook_speed",
						{ duration = caster:GetTalentValue("modifier_pudge_hook_1", "duration", true) }
					)
				end
				...
				local impact_particle_d = wearables_system:GetParticleReplacementAbility(
					self:GetCaster(),
		...
		if self.hooks[projectileIndex].bRetracting then
			...
			if self.hooks[projectileIndex].hVictim then
				if not self.hooks[projectileIndex].hVictim:HasModifier("modifier_custom_pudge_meat_hook_debuff") then
					...
					if self.hooks[projectileIndex].start_position then
						dist = (
							self.hooks[projectileIndex].start_position
							- self.hooks[projectileIndex].hVictim:GetAbsOrigin()
						):Length2D()
					end
					self.hooks[projectileIndex].hVictim:AddNewModifier(
						caster,
						self,
						"modifier_custom_pudge_meat_hook_debuff",
						{ dist = dist }
...
function modifier_custom_pudge_meat_hook_debuff:IsHidden()
	return true
end
...
function modifier_custom_pudge_meat_hook_debuff:OnDestroy()
	...
	if
		self.parent:GetUnitName() ~= "npc_dota_companion"
		and self.caster:HasTalent("modifier_pudge_hook_6")
	...
	then
		...
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_custom_pudge_meat_hook_root",
			{ duration = (1 - self.parent:GetStatusResistance()) * duration }
		)
	end
end
...
function modifier_custom_pudge_meat_hook_speed:IsHidden()
	return false
end
function modifier_custom_pudge_meat_hook_speed:IsPurgable()
	return false
end
function modifier_custom_pudge_meat_hook_speed:GetTexture()
	return "buffs/hook_speed"
end
...
function modifier_custom_pudge_meat_hook_speed:GetModifierAttackSpeedBonus_Constant()
	if not self.parent:HasTalent("modifier_pudge_hook_1") then
		return
	end
	return self.speed
end
...
function modifier_custom_pudge_meat_hook_speed:GetModifierHealthRegenPercentage()
	if not self.parent:HasTalent("modifier_pudge_hook_3") then
		return
	end
	return self.heal
end
...
function modifier_custom_pudge_meat_hook_root:IsHidden()
	return true
end
function modifier_custom_pudge_meat_hook_root:IsPurgable()
	return true
end
function modifier_custom_pudge_meat_hook_root:GetTexture()
...
function modifier_custom_pudge_meat_hook_tracker:IsHidden()
	return true
end
function modifier_custom_pudge_meat_hook_tracker:IsPurgable()
	return false
end
...
function modifier_custom_pudge_meat_hook_tracker:GetModifierAttackRangeBonus()
	if not self.parent:HasTalent("modifier_pudge_hook_5") then
		return
	end
	return self.range
end
...
function modifier_custom_pudge_meat_hook_move_speed:IsHidden()
	return false
end
function modifier_custom_pudge_meat_hook_move_speed:IsPurgable()
	return false
end
function modifier_custom_pudge_meat_hook_move_speed:GetTexture()
	return "buffs/edict_speed"
end
...
function modifier_custom_pudge_meat_hook_move_slow:IsHidden()
	return true
end
function modifier_custom_pudge_meat_hook_move_slow:IsPurgable()
	return true
end
...
function modifier_custom_pudge_meat_hook_move_vision:IsHidden()
	return true
end
function modifier_custom_pudge_meat_hook_move_vision:IsPurgable()
	return false
end
...
function modifier_custom_pudge_meat_hook_perma:IsHidden()
	return not self:GetCaster():HasTalent("modifier_pudge_hook_4")
end
function modifier_custom_pudge_meat_hook_perma:IsPurgable()
	return false
end
function modifier_custom_pudge_meat_hook_perma:RemoveOnDeath()
	return false
end
...
function modifier_custom_pudge_meat_hook_perma:GetModifierSpellAmplify_Percentage()
	if not self.parent:HasTalent("modifier_pudge_hook_4") then
		return 0
	end
...
function modifier_custom_pudge_meat_hook_stack:IsHidden()
	return false
end
function modifier_custom_pudge_meat_hook_stack:IsPurgable()
	return false
end
...
function modifier_custom_pudge_meat_hook_hidden:IsHidden()
	return true
end
function modifier_custom_pudge_meat_hook_hidden:IsPurgable()
	return false
end
function modifier_custom_pudge_meat_hook_hidden:IsPurgeException()
	return false
end