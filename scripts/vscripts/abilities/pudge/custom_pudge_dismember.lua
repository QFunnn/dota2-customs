...
LinkLuaModifier(
	"modifier_custom_pudge_dismember",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_HORIZONTAL
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_visual",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_illusion",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_outgoing",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
...
LinkLuaModifier(
	"modifier_custom_pudge_dismember_devour_caster",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_blood_str_buff",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_stack_effect",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_break",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_custom_pudge_dismember_bkb",
	"abilities/pudge/custom_pudge_dismember",
	LUA_MODIFIER_MOTION_NONE
)
...
function custom_pudge_dismember:GetFullDuration()
	...
	if caster:HasModifier("modifier_custom_pudge_dismember_stack_effect") then
		local duration = caster:GetTalentValue("modifier_pudge_dismember_4", "duration")
			/ caster:GetTalentValue("modifier_pudge_dismember_4", "max")
...
function custom_pudge_dismember:OnSpellStart(new_target)
	...
	if stack_mod then
		...
		caster:AddNewModifier(
			...
			"modifier_custom_pudge_dismember_blood_str_buff",
			{
				stack = stack_mod:GetStackCount(),
				duration = caster:GetTalentValue("modifier_pudge_dismember_4", "effect_duration"),
			}
		)
	end
	...
	if caster:HasTalent("modifier_pudge_dismember_5") then
		self.targets = caster:FindTargets(
			self:GetSpecialValueFor("cast_range")
				+ caster:GetCastRangeBonus()
				+ caster:GetTalentValue("modifier_pudge_dismember_5", "radius")
		)
	end

	if caster:HasTalent("modifier_pudge_dismember_6") then
		self.bkb_mod = caster:AddNewModifier(
			caster,
			self,
			"modifier_custom_pudge_dismember_bkb",
	...
	for _, unit in pairs(self.targets) do
		...
		unit:AddNewModifier(
			...
			self,
			"modifier_custom_pudge_dismember",
			{ main = main, duration = self:GetChannelTime() }
		)
	end
end
...
function custom_pudge_dismember:GetDamage()
	...
	local strength_damage = self:GetSpecialValueFor("strength_damage")
		+ caster:GetTalentValue("modifier_pudge_dismember_1", "str") / 100
...
function custom_pudge_dismember:DealDamage(target, damage_amount, no_heal, damage_ability)
	...
	if caster:HasTalent("modifier_pudge_dismember_2") then
		target:AddNewModifier(
			...
			self,
			"modifier_custom_pudge_dismember_outgoing",
			{ duration = caster:GetTalentValue("modifier_pudge_dismember_2", "duration") }
		)
	end
	...
	if caster:HasTalent("modifier_pudge_dismember_5") then
		target:AddNewModifier(
			...
			self,
			"modifier_custom_pudge_dismember_break",
			{ duration = caster:GetTalentValue("modifier_pudge_dismember_5", "duration") }
		)
	end

	if no_heal == true or no_heal == 0 then
		return
	end
...
function modifier_custom_pudge_dismember:IsPurgeException()
	return true
end
function modifier_custom_pudge_dismember:IsPurgable()
	return false
end
function modifier_custom_pudge_dismember:IsStunDebuff()
	return true
end
...
function modifier_custom_pudge_dismember:OnCreated(table)
	...
	if
		self.caster.current_model == "models/items/pudge/arcana/pudge_arcana_base.vmdl"
		or pfx == "particles/econ/items/pudge/pudge_arcana/pudge_arcana_dismember_default.vpcf"
	then
		...
		if self.ability.arcana_style[self.parent:GetUnitName()] then
			if type(self.ability.arcana_style[self.parent:GetUnitName()]) == "string" then
				part = "particles/econ/items/pudge/pudge_arcana/pudge_arcana_dismember_"
					.. self.ability.arcana_style[self.parent:GetUnitName()]
					.. ".vpcf"
			end

			if type(self.ability.arcana_style[self.parent:GetUnitName()]) == "table" then
				part = "particles/econ/items/pudge/pudge_arcana/pudge_arcana_dismember_"
					.. self.ability.arcana_style[self.parent:GetUnitName()][1]
					.. ".vpcf"
		...
		local bonus_checkout_green = wearables_system:GetParticleReplacementAbility(
			self:GetCaster(),
			"particles/econ/items/pudge/pudge_arcana/pudge_arcana_red_back_ambient.vpcf",
			self
		)
		if
			self.caster:HasUnequipItem(77561)
			or bonus_checkout_green ~= "particles/econ/items/pudge/pudge_arcana/pudge_arcana_red_back_ambient.vpcf"
		then
...
function modifier_custom_pudge_dismember_visual:IsHidden()
	return true
end
function modifier_custom_pudge_dismember_visual:IsPurgable()
	return false
end
...
function modifier_custom_pudge_dismember_visual:GetModifierPercentageCooldown()
	if not self.parent:HasTalent("modifier_pudge_dismember_6") then
		return
	end
	return self.cdr
end
...
function modifier_custom_pudge_dismember_visual:GetModifierCastRangeBonusStacking()
	if not self.parent:HasTalent("modifier_pudge_dismember_3") then
		return
	end
...
function modifier_custom_pudge_dismember_outgoing:IsHidden()
	return false
end
function modifier_custom_pudge_dismember_outgoing:IsPurgable()
	return true
end
function modifier_custom_pudge_dismember_outgoing:GetTexture()
	return "buffs/dismember_damage"
end
...
function modifier_custom_pudge_dismember_devour:IsHidden()
	return true
end
function modifier_custom_pudge_dismember_devour:IsPurgable()
	return false
end
...
function modifier_custom_pudge_dismember_devour:OnIntervalThink()
	...
	if
		...
		or not self.caster_mod
		or self.caster_mod:IsNull()
	then
		self:Destroy()
		return
	end
	...
	self.caster:UpdateUIshort({
		...
		stack = self.caster_mod:GetStackCount(),
		priority = 1,
		style = "PudgeDevour",
	})
...
function modifier_custom_pudge_dismember_devour_caster:IsHidden()
	return true
end
function modifier_custom_pudge_dismember_devour_caster:IsPurgable()
	return false
end
function modifier_custom_pudge_dismember_devour_caster:OnCreated(table)
	...
	self.max_shield = self.parent:GetTalentValue("modifier_pudge_dismember_legendary", "shield")
...
function modifier_custom_pudge_dismember_blood_str_buff:IsHidden()
	return false
end
function modifier_custom_pudge_dismember_blood_str_buff:GetTexture()
	return "buffs/hunger_str"
end
function modifier_custom_pudge_dismember_blood_str_buff:IsPurgable()
	return false
end
...
function modifier_custom_pudge_dismember_stack_effect:IsHidden()
	return false
end
function modifier_custom_pudge_dismember_stack_effect:IsPurgable()
	return false
end
function modifier_custom_pudge_dismember_stack_effect:IsDebuff()
	return true
end
...
function modifier_custom_pudge_dismember_stack_effect:OnIntervalThink()
	...
	local heroes = FindUnitsInRadius(
		...
		DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE,
		FIND_CLOSEST,
		false
	)
...
function modifier_custom_pudge_dismember_break:IsHidden()
	return true
end
function modifier_custom_pudge_dismember_break:IsPurgable()
	return false
end
function modifier_custom_pudge_dismember_break:CheckState()
	return { [MODIFIER_STATE_PASSIVES_DISABLED] = true }
end
function modifier_custom_pudge_dismember_break:GetEffectAttachType()
...
function modifier_custom_pudge_dismember_bkb:IsHidden()
	return true
end
function modifier_custom_pudge_dismember_bkb:IsPurgable()
	return false
end
function modifier_custom_pudge_dismember_bkb:OnCreated()
	...
	for i = 0, 8 do
		...
		if current_item and not NoCdItems[current_item:GetName()] then
			local cooldown_mod = self.parent:AddNewModifier(
				self.parent,
				self.ability,
	...
	self.bkb_mod =
		self.parent:AddNewModifier(self.parent, self.ability, "modifier_generic_debuff_immune", { effect = 1 })
end
