...
LinkLuaModifier(
	"modifier_void_spirit_resonant_pulse",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier("modifier_generic_ring_lua", "util/modifier_generic_ring_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_void_spirit_resonant_slow",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_resonant_resist",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_resonant_tracker",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_resonant_stats",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_resonant_invun",
	"abilities/void_spirit/void_spirit_resonant_pulse_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function void_spirit_resonant_pulse_custom:LegendaryStack()
	...
	if not mod or not mod.legendary_max then
		return
	end
	...
	if caster:HasTalent("modifier_void_pulse_4") then
		caster:AddNewModifier(
			...
			self,
			"modifier_void_spirit_resonant_stats",
			{ duration = caster:GetTalentValue("modifier_void_pulse_4", "duration") }
		)
	end
end
...
function void_spirit_resonant_pulse_custom:DealDamage(enemy, legendary_damage, first_hit)
	...
	local damageTable =
		{ attacker = caster, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self, victim = enemy }
	...
	if caster:HasTalent("modifier_void_pulse_6") then
		...
		local dist =
			math.max(0, math.min(vec:Length2D() - 120, caster:GetTalentValue("modifier_void_pulse_6", "distance")))
		...
		enemy:AddNewModifier(
			...
			self,
			"modifier_void_spirit_resonant_slow",
			{ duration = (1 - enemy:GetStatusResistance()) * caster:GetTalentValue("modifier_void_pulse_6", "silence") }
		)
	end
...
function modifier_void_spirit_resonant_pulse:OnCreated(kv)
	...
	self.max_shield = self.ability:GetSpecialValueFor("base_absorb_amount")
		+ self.parent:GetMaxHealth() * self.parent:GetTalentValue("modifier_void_pulse_1", "shield") / 100
	...
	if self.parent:HasTalent("modifier_void_pulse_5") then
		self.parent:AddNewModifier(
			...
			self.ability,
			"modifier_void_spirit_resonant_invun",
			{ duration = self.parent:GetTalentValue("modifier_void_pulse_5", "duration") }
		)
	end
...
function modifier_void_spirit_resonant_pulse:CreateWave(new_damage)
	...
	local pulse = self.parent:AddNewModifier(
		self.parent,
		self.ability,
...
function modifier_void_spirit_resonant_pulse:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_void_pulse_1") then
		return
	end
...
function modifier_void_spirit_resonant_pulse:OnDestroy()
	...
	if self.parent:HasTalent("modifier_void_pulse_5") then
		...
		local heal = (self.parent:GetMaxHealth() - self.parent:GetHealth())
			* self.parent:GetTalentValue("modifier_void_pulse_5", "heal")
			/ 100
	...
	if self.parent:HasTalent("modifier_void_pulse_legendary") then
		legendary_damage = self.legendary_stack
			* self.parent:GetTalentValue("modifier_void_pulse_legendary", "damage")
			* self.parent:GetAverageTrueAttackDamage(nil)
			/ 100
		if self.legendary_stack >= self.legendary_max then
			local effect = ParticleManager:CreateParticle(
				"particles/void_spirit/shield_refresh.vpcf",
...
function modifier_void_spirit_resonant_tracker:IsHidden()
	return true
end
function modifier_void_spirit_resonant_tracker:IsPurgable()
	return false
end
...
function modifier_void_spirit_resonant_tracker:AttackEvent_out(params)
	...
	if not self.parent:HasModifier("modifier_void_spirit_resonant_pulse") then
		return
	end
...
function modifier_void_spirit_resonant_slow:IsHidden()
	return true
end
function modifier_void_spirit_resonant_slow:IsPurgable()
	return true
end
function modifier_void_spirit_resonant_slow:GetTexture()
...
function modifier_void_spirit_resonant_resist:IsHidden()
	return false
end
function modifier_void_spirit_resonant_resist:IsPurgable()
	return false
end
function modifier_void_spirit_resonant_resist:GetTexture()
	return "buffs/Pulse_reduce"
end
...
function modifier_void_spirit_resonant_stats:IsHidden()
	return false
end
function modifier_void_spirit_resonant_stats:IsPurgable()
	return false
end
function modifier_void_spirit_resonant_stats:GetTexture()
...
function modifier_void_spirit_resonant_stats:OnRefresh()
	...
	if self:GetStackCount() >= self.max then
		...
		self.particle = ParticleManager:CreateParticle(
			"particles/void_spirit/shield_buff.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			self.parent
		)
...
function modifier_void_spirit_resonant_invun:IsHidden()
	return true
end
function modifier_void_spirit_resonant_invun:IsPurgable()
	return false
end
function modifier_void_spirit_resonant_invun:OnCreated()
	...
	local effect_cast = ParticleManager:CreateParticle(
		"particles/void_spirit/pulse_shield_invun.vpcf",
		PATTACH_POINT_FOLLOW,
		self.parent
	)
	ParticleManager:SetParticleControlEnt(
