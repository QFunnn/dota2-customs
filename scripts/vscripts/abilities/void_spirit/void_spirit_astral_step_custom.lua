...
LinkLuaModifier(
	"modifier_void_spirit_astral_step",
	"abilities/void_spirit/void_spirit_astral_step_custom",
...
LinkLuaModifier(
	"modifier_void_spirit_astral_step_heal_reduce",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_step_status",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_replicant",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_strikes",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_legendary_illusion",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_void_spirit_astral_stun_cd",
	"abilities/void_spirit/void_spirit_astral_step_custom",
	LUA_MODIFIER_MOTION_NONE
)
...
function void_spirit_astral_step_custom:Precache(context)
	...
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_void_spirit/void_spirit_attack_travel_strike_blur_custom.vpcf",
		context
	)
...
function void_spirit_astral_step_custom:AutoStack(target, is_attack)
	local caster = self:GetCaster()
	if not caster:HasTalent("modifier_void_step_4") then
		return
	end
	...
	target:AddNewModifier(
		...
		self,
		"modifier_void_spirit_astral_step_spells",
		{ duration = caster:GetTalentValue("modifier_void_step_4", "duration"), is_attack = attack }
	)
end
...
function void_spirit_astral_step_custom:DealDamage(target, damage_k)
	...
	if target and caster:HasTalent("modifier_void_step_4") then
		local bonus = (target:GetMaxHealth() - target:GetHealth())
			* caster:GetTalentValue("modifier_void_step_4", "damage")
			/ 100
...
function void_spirit_astral_step_custom:CastSpell(point, new_origin)
	...
	if caster:HasTalent("modifier_void_step_6") then
		...
		caster:AddNewModifier(
			...
			self,
			"modifier_void_spirit_astral_step_status",
			{ duration = caster:GetTalentValue("modifier_void_step_6", "duration") }
		)
	end
	...
	for _, enemy in pairs(enemies) do
		if enemy:IsUnit() and not enemy:HasModifier("modifier_monkey_king_wukongs_command_custom_soldier") then
			enemy:AddNewModifier(
				...
				self,
				"modifier_void_spirit_astral_step",
				{ duration = delay, no_damage = no_damage }
			)
			...
			if no_damage == 0 then
				...
				if
					caster:HasTalent("modifier_void_step_5")
					and not enemy:HasModifier("modifier_void_spirit_astral_stun_cd")
					and enemy:GetHealthPercent() <= health_thresh
				then
					...
					local mod = enemy:AddNewModifier(
						...
						{ duration = (1 - enemy:GetStatusResistance()) * break_duration }
					)
					if mod then
...
function modifier_void_spirit_astral_step:OnCreated(kv)
	...
	local debuff_particle = wearables_system:GetParticleReplacementAbility(
		self.caster,
		"particles/units/heroes/hero_void_spirit/astral_step/void_spirit_astral_step_debuff.vpcf",
		self
	)
...
function modifier_void_spirit_astral_step_attack:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_attack:IsPurgable()
	return false
end
...
function modifier_void_spirit_astral_step_attack:GetCritDamage()
	if not self.parent:HasTalent("modifier_void_step_1") then
		return
	end
...
function modifier_void_spirit_astral_step_tracker:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_tracker:IsPurgable()
	return false
end
...
function modifier_void_spirit_astral_step_tracker:DamageEvent_out(params)
	...
	if not self.parent:HasTalent("modifier_void_step_3") then
		return
	end
	...
	local hide_number = ""
	if
		(params.inflictor and params.inflictor == self.ability)
		or self.parent:HasModifier("modifier_void_spirit_astral_step_attack")
	then
...
function modifier_void_spirit_astral_step_spells:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_spells:IsPurgable()
	return false
end
function modifier_void_spirit_astral_step_spells:GetTexture()
	return "buffs/step_spells"
end
	...
	if self.attack_count >= self.attack_max or is_attack == 0 then
		...
		if self:GetStackCount() >= self.max then
			...
			self.ability:DealDamage(
				self.parent,
				self.caster:GetTalentValue("modifier_void_step_4", "damage_auto") / 100
			)
			self:Destroy()
		end
	end
...
function modifier_void_spirit_astral_step_heal_reduce:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_heal_reduce:IsPurgable()
	return false
end
function modifier_void_spirit_astral_step_heal_reduce:GetEffectName()
...
function modifier_void_spirit_astral_step_status:IsHidden()
	return true
end
function modifier_void_spirit_astral_step_status:IsPurgable()
	return false
end
function modifier_void_spirit_astral_step_status:GetEffectName()
	return "particles/void_spirit/step_status.vpcf"
end
...
function void_spirit_astral_replicant:OnSpellStart()
	...
	caster:AddNewModifier(
		...
		self,
		"modifier_void_spirit_astral_replicant",
		{ duration = caster:GetTalentValue("modifier_void_step_legendary", "duration") }
	)
end

modifier_void_spirit_astral_replicant = class({})
function modifier_void_spirit_astral_replicant:IsHidden()
	return true
end
function modifier_void_spirit_astral_replicant:IsPurgable()
	return false
end
...
function modifier_void_spirit_astral_replicant:OnCreated(table)
	...
	self.particle = ParticleManager:CreateParticleForPlayer(
		"particles/void_spirit/step_legendary_vector.vpcf",
	...
	local illusion_self = CreateIllusions(
		self.parent,
		self.parent,
		{ outgoing_damage = 0, duration = self:GetRemainingTime() + 0.2 },
		1,
		0,
		false,
		false,
...
function modifier_void_spirit_astral_replicant:OnDestroy()
	...
	self.parent:AddNewModifier(
		...
		ability,
		"modifier_void_spirit_astral_strikes",
		{ max = self:GetStackCount(), x = self.point.x, y = self.point.y }
	)
end

modifier_void_spirit_astral_strikes = class({})
function modifier_void_spirit_astral_strikes:IsHidden()
	return true
end
function modifier_void_spirit_astral_strikes:IsPurgable()
	return false
end
...
function modifier_void_spirit_legendary_illusion:IsHidden()
	return true
end
function modifier_void_spirit_legendary_illusion:IsPurgable()
	return false
end
...
function modifier_void_spirit_astral_stun_cd:IsHidden()
	return true
end
function modifier_void_spirit_astral_stun_cd:IsPurgable()
	return false
end
function modifier_void_spirit_astral_stun_cd:RemoveOnDeath()
	return false
end
