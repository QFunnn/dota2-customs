...
local a = "abilities/attack/skeleton_hammer_attack"
...
function j.prototype.OnSpellStart(self)
	...
	m:SimulateCast({ castAnimation = ACT_DOTA_GENERIC_CHANNEL_1, duration = 0.6 })
	local r = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_centaur/centaur_warstomp.vpcf",
		PATTACH_CUSTOMORIGIN,
		nil
	)
	ParticleManager:SetParticleControl(r, 0, n)
	ParticleManager:SetParticleControl(r, 1, Vector(self.melee_radius, self.melee_radius, self.melee_radius))
	m:EmitSound("Hero_Centaur.HoofStomp")
