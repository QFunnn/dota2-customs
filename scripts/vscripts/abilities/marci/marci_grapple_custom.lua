...
LinkLuaModifier(
	"modifier_marci_dispose_custom_knockback",
...
function marci_grapple_custom:UpdateTalents(name)
	...
	if not self.init then
		...
		self.talents = {
			...
			has_h4 = 0,
			h4_slow = caster:GetTalentValue("modifier_marci_hero_4", "slow", true),
			h4_cdr = caster:GetTalentValue("modifier_marci_hero_4", "cdr", true),
			h4_silence = caster:GetTalentValue("modifier_marci_hero_4", "silence", true),
		}
	end
...
function marci_grapple_custom:OnSpellStart()
	...
	if mod then
		...
		target:AddNewModifier(
			...
			self,
			"modifier_marci_dispose_custom_knockback",
			{ dist = dist, duration = knock_duration }
		)
		mod:Destroy()
		return
	end
	...
	if self.talents.has_q4 == 1 then
		self.caster:AddNewModifier(
			self.caster,
			self,
			"modifier_marci_dispose_custom_swap",
...
function marci_grapple_custom:PlayEffects1(point)
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_marci/marci_dispose_land.vpcf",
		PATTACH_WORLDORIGIN,
		nil
	)
...
function marci_grapple_custom:PlayEffects4()
	local particle = ParticleManager:CreateParticle(
		"particles/units/heroes/hero_marci/marci_grapple.vpcf",
		PATTACH_POINT_FOLLOW,
		self.caster
	)
	ParticleManager:SetParticleControlEnt(
...
function modifier_marci_dispose_custom_slow:OnCreated()
	...
	self.slow = self.ability.slow_move + (self.ability.talents.has_h4 == 1 and self.ability.talents.h4_slow or 0)
...
function modifier_marci_dispose_custom_knockback:OnDestroy()
	...
	self.parent:AddNewModifier(
		...
		self.ability,
		"modifier_stunned",
		{ duration = self.ability.talents.q4_stun * (1 - self.parent:GetStatusResistance()) }
	)
end
...
function modifier_marci_dispose_custom_tracker:SpellEvent(params)
	...
	if not self.parent.dispose_ability_legendary then
		return
	end
	if self.parent.dispose_ability_legendary:GetCooldownTimeRemaining() > 0 then
		return
	end
	if params.ability == self.parent.dispose_ability_legendary then
		return
	end
...
function modifier_marci_dispose_custom_tracker:GetModifierPercentageCooldown()
	if self.ability.talents.has_h4 == 0 then
		return
	end
	return self.ability.talents.h4_cdr
end
...
function modifier_marci_dispose_custom_health_reduce:GetTexture()
	return "buffs/marci/dispose_3"
end
...
function modifier_marci_dispose_custom_health_inc:GetTexture()
	return "buffs/marci/dispose_3"
end
function modifier_marci_dispose_custom_health_inc:GetAttributes()
...
function marci_dispose_hits:OnSpellStart()
	...
	self.caster:AddNewModifier(
		self.caster,
		self,
		"modifier_marci_dispose_custom_hits",
...
function modifier_marci_dispose_custom_hits:OnCreated(table)
	...
	self.damageTable = {
		...
		damage = self.parent.dispose_ability:GetDamage() * self.ability.talents.q7_damage / self.max,
	}
