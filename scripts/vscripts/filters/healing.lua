...
function Filters:HealingFilter(event)
	...
	local inflictor = (event.entindex_inflictor_const and event.entindex_inflictor_const ~= 0)
			and EntIndexToHScript(event.entindex_inflictor_const)
		or nil
	local healer = (event.entindex_healer_const and event.entindex_healer_const ~= 0)
			and EntIndexToHScript(event.entindex_healer_const)
		or nil
	...
	if not IsValidEntity(healer) or not IsValidEntity(target) then
		return true
	end
	...
	if is_valid_inflictor and HEALING_EXCLUDED_ABILITIES[inflictor:GetAbilityName()] then
		return true
	end
