...
function Filters:ItemAddedToInventoryFilter(event)
	...
	if IsValidEntity(item) and IsValidEntity(inventory_parent) then
		...
		if purchaser then
			...
			if string.match(item_name, "_ffa") and not string.match(GetMapName(), "_ffa") then
				return
			end
			if string.match(item_name, "_duo") and not string.match(GetMapName(), "_duo") then
				return
			end
			if string.match(item_name, "_quintet") and not string.match(GetMapName(), "_quintet") then
				return
			end
			if string.match(item_name, "_octet") and not string.match(GetMapName(), "_octet") then
				return
			end
			if self:OrbAddedToInventoryFilter(item, inventory_parent) then
				return true
			end
...
function Filters:OrbAddedToInventoryFilter(item, hInventoryParent)
	...
	if orb_rarity then
		...
		CustomChat:MessageToTeam(
			...
			"orb_purchased_chat_message_" .. orb_rarity
		)
