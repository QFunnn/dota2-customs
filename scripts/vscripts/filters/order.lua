...
function Filters:ExecuteOrderFilter(event)
	...
	if order_type == DOTA_UNIT_ORDER_PICKUP_ITEM and player_id ~= -1 then
		if not target or target:IsNull() or not target.GetContainedItem then
			return true
		end
	...
	if order_type == DOTA_UNIT_ORDER_PURCHASE_ITEM and player_id ~= -1 and unit then
		if string.match(event.shop_item_name, "_ffa") and not string.match(GetMapName(), "_ffa") then
			return
		end
		if string.match(event.shop_item_name, "_duo") and not string.match(GetMapName(), "_duo") then
			return
		end
		if string.match(event.shop_item_name, "_quintet") and not string.match(GetMapName(), "_quintet") then
			return
		end
		if string.match(event.shop_item_name, "_octet") and not string.match(GetMapName(), "_octet") then
			return
		end
	end
	...
	if
		...
		and ability
		and ability_name == "abyssal_underlord_dark_portal"
	...
	then
		...
		if IsValidEntity(nearest_epic_spawn_point) then
			...
			local new_target_vector = spawn_point_origin
				+ (order_vector - spawn_point_origin):Normalized() * radius_buffered
	...
	then
		...
		print(
			...
			(not IsValidEntity(target) or target:GetTeamNumber() ~= unit:GetTeamNumber())
		)
	...
	if
		order_type == DOTA_UNIT_ORDER_CAST_POSITION
		and ability
		and ability_name == "muerta_ofrenda"
		and IsValidEntity(unit)
	then
