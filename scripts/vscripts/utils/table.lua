...
function table.deepcopy(orig, seen)
    ...
    for orig_key, orig_value in next, orig, nil do
        copy[table.deepcopy(orig_key, seen)] = table.deepcopy(orig_value, seen)
    end
    setmetatable(copy, table.deepcopy(getmetatable(orig), seen))
    return copy
end
...
function table.random_with_weight(t)
    ...
    for i = 1, # weight_table do
        if weight_table[i].total_weight >= randomValue then
            print("Значение редкости", randomValue, weight_table[i].total_weight)
...
function table.deep_equal(t1, t2)
    if t1 == t2 and type(t1) ~= "table" then
        return true
    end
    ...
    for k, v in pairs(t1) do
        if not table.deep_equal(v, t2[k]) then
            return false
        end
    end
