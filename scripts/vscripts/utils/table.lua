...
-- Чтобы удобно было собрать любую инфу не дублируя код
function table.filter_list(t, bList, filterFunc, bIsArray)
    local isArray = bIsArray
    if isArray == nil then
        isArray = table.is_array(t)
    end
    local result = {}
    ...
    for k, v in pairs(t) do
        if not filterFunc or filterFunc(k, v) then
            if bList then
                table.insert(result, isArray and v or k)
            else
                result[k] = v
            end
        end
...
function table.map(t, mapFunc, bValue)
    local result = {}
    for k, v in pairs(t) do
        local key = bValue and v or k
        result[key] = mapFunc(k, v)
    end
    return result
end
