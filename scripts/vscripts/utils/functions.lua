...
function IsWardUnit(unit)
	...
	if unit.IsWard and unit:IsWard() then
		return true
	end
...
function GameMode:HeroIconClicked(params)
	...
	if
		(not hTargetHero or hTargetHero:IsNull())
		and Players
		and Players.Players
		and Players.Players[nTargetPlayerID]
	then
		local fb = Players.Players[nTargetPlayerID].hero
		if fb and not fb:IsNull() then
			hTargetHero = fb
		end
	end
	if hTargetHero then
			...
			if hHero then
				if (nPlayerID ~= nTargetPlayerID) or IsInToolsMode() then
					CustomGameEventManager:Send_ServerToPlayer(
						PlayerResource:GetPlayer(nPlayerID),
						"ShowActorPanel",
						{ target_player_id = nTargetPlayerID }
					)
				end
			end
		end
...
function GameMode:OnActorPanelOpen(params)
	...
	if nPlayerID == nil or not PlayerResource:IsValidPlayerID(nPlayerID) then
		return
	end
	local nTargetPlayerID = params.target_player_id
	if nTargetPlayerID == nil or not PlayerResource:IsValidPlayerID(nTargetPlayerID) then
		return
	end
	if nTargetPlayerID == nPlayerID and not IsInToolsMode() then
		return
	end -- не на себя
	if PlayerResource:GetPlayer(nPlayerID) then
		CustomGameEventManager:Send_ServerToPlayer(
