...
function SendErrorToPlayer(PID, errorText, errorSound)
	...
	if player then
		CustomGameEventManager:Send_ServerToPlayer(
			...
			{ errorText = errorText, errorSound = errorSound }
		)
	end
end

function PlayerSelectUnit(PlayerID, Entity, bReplace)
	if PlayerID == nil or Entity == nil or Entity:IsNull() then
		return
	end
	...
	if Player then
		CustomGameEventManager:Send_ServerToPlayer(
			Player,
			"select_unit_custom",
			{ entity = Entity:entindex(), replace = bReplace }
		)
	end
end
...
function IsRealHero(Unit)
	...
	if
		not Unit:IsRealHero()
		or Unit:IsIllusion()
		or Unit:IsStrongIllusion()
		or Unit:IsTempestDouble()
		or Unit:IsClone()
		or Unit:GetClassname() == "npc_dota_lone_druid_bear"
	then
		return false
	end
...
function IsModifierChangesAttackCap(ModifierName)
	if MODIFIERS_SETTINGS[ModifierName] == nil then
		return false
	end
...
function IsUnitBerserked(hUnit)
	if not hUnit or hUnit:IsNull() or not hUnit:IsAlive() then
		return false
	end
...
function RemoveAbilitySpawnGroupIfNoUsed(hAbility)
	...
	if AbilityHeroName == nil or hCaster == nil or hCaster:IsNull() then
		return
	end

	local SpawnGroupID = HeroBuilder:GetSpawnGroup(AbilityHeroName)
	if SpawnGroupID == nil then
		return
	end
