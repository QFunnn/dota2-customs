...
function CustomTables:Init()
	CustomGameEventManager:RegisterListener(
		"event_game_request_custom_tables",
		Dynamic_Wrap(self, "OnRequestCustomTables")
	)
	...
	RegisterLoadListener(function(player, playerID)
		CustomTables:OnRequestCustomTables({ PlayerID = playerID })
	end)
end

function CustomTables:SendTableValueToAllClients(stringTableName, stringKeyName, script_tableValue)
	if CustomTables.DebugDisableEvents then
		return
	end
	if CustomTables.DebugDisableEventsList[stringTableName] then
		return
	end
	...
	if GameRules:State_Get() == DOTA_GAMERULES_STATE_INIT then
		Timers:CreateTimer(0.1, function()
			...
			local delayed_last_value = self.LastSentTableData[stringTableName]
					and self.LastSentTableData[stringTableName][stringKeyName]
				or nil
			...
			if is_for_player then
				CustomGameEventManager:Send_ServerToPlayer(
					is_for_player,
					"game_event_update_custom_tables",
					{ table_name = stringTableName, key = stringKeyName, data = delayed_payload_patch }
				)
			else
		...
		if is_for_player then
			CustomGameEventManager:Send_ServerToPlayer(
				is_for_player,
				"game_event_update_custom_tables",
				{ table_name = stringTableName, key = stringKeyName, data = payload_patch }
			)
		else
			CustomGameEventManager:Send_ServerToAllClients(
				"game_event_update_custom_tables",
...
function CustomTables:EnqueueAutoUpdateValue(stringTableName, stringKeyName)
	...
	if self.AutoUpdateQueuedKeys[stringTableName][stringKeyName] then
		return false
	end
...
function CustomTables:StartAutoUpdateQueueProcessor()
	if self._auto_update_queue_processor_initialized then
		return
	end
	...
	Timers:CreateTimer(send_delay, function()
		...
		if item then
			...
			local pending_patch = self.PendingPatchData
					and self.PendingPatchData[item.table_name]
					and self.PendingPatchData[item.table_name][item.key_name]
				or nil
...
function CustomTables:FlushAutoUpdateTable(stringTableName)
	...
	local table_data = self.TableData and self.TableData[stringTableName] or nil
	if not dirty_table or not table_data then
		return
	end
	local dirty_keys = {}
...
function CustomTables:InitAutoUpdater()
	if self._auto_updater_initialized then
		return
	end
...
function CustomTables:SetTablePatchValue(stringTableName, stringKeyName, patch_data, fast_update)
	...
	CustomTables.TableData[stringTableName][stringKeyName] = self:ApplyTablePatch(
		CustomTables.TableData[stringTableName][stringKeyName],
		patch_data
	) or {}
	if self.TablesAutoUpdate and self.TablesAutoUpdate[stringTableName] and not fast_update then
	...
	return self:SendTablePatchToAllClients(
		stringTableName,
		stringKeyName,
		patch_data,
		CustomTables.TableData[stringTableName][stringKeyName]
...
function CustomTables:GetTableValue(stringTableName, stringKeyName)
	return (CustomTables.TableData[stringTableName] and CustomTables.TableData[stringTableName][stringKeyName])
			and CustomTables.TableData[stringTableName][stringKeyName]
		or {}
end
		...
		for key_name, value in pairs(key) do
			if not CustomTables.TablesForPlayer[table_name] or tostring(key_name) == tostring(player_id) then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"game_event_update_custom_tables",
					{ table_name = table_name, key = key_name, data = value, full = true }
				)
			end
		end
	end
	CustomGameEventManager:Send_ServerToPlayer(player, "event_game_custom_tables_loaded", {})
end
