...
function ProtectedCustomEvents:Init()
	if ProtectedCustomEvents.loaded then
		return
	end
	...
	CustomGameEventManager.Send_ServerToTeam_old = CustomGameEventManager.Send_ServerToTeam
	...
	CustomGameEventManager.Send_ServerToPlayer = function(self, player, name, data)
		...
		if
			server_key ~= nil
			or (
				GameRules:State_Get() == DOTA_GAMERULES_STATE_INIT
				or GameRules:State_Get() == DOTA_GAMERULES_STATE_WAIT_FOR_PLAYERS_TO_LOAD
			)
		then
			data.n = server_key
	...
	CustomGameEventManager.Send_ServerToAllClients = function(self, name, data)
		...
		self:Send_ServerToTeam_old(1, name, data)
		data.n = nil
	end
...
function UpdateDisconnect(id)
	ProtectedCustomEvents.client_keys[id] = nil
	ProtectedCustomEvents.server_keys[id] = nil
end
...
CustomGameEventManager:RegisterListener("ok", function(_, data)
	...
	if trigger_load then
		for _, listener in ipairs(ProtectedCustomEvents.load_listeners) do
			listener(player, playerID)
		end
	end
end)
