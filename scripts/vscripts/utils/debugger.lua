	...
	if Admins:IsAdmin(nSteamID) or (GameRules:IsCheatMode()) then
		...
		if szText == "-modifiers" then
			if hero then
				GameRules:SendCustomMessage(
					"=== Модификаторы героя " .. hero:GetUnitName() .. " ===",
					0,
					0
				)
		...
		if szText == "-pall" then
			for i = 0, 24 do
				if PlayerResource:IsValidPlayerID(i) then
					...
					if PlayerInfo then
						GameRules:SendCustomMessage(
							"Players Lib: bIsBot = " .. tostring(PlayerInfo.is_bot) .. " Team = " .. PlayerInfo.team,
							0,
							0
						)
					end
		...
		if forceMatch then
			...
			if roundNum and roundNum >= 1 and roundNum <= GAME_MAX_ROUNDS and GAME_ROUNDS_LIST[roundNum] then
				GameRules:SendCustomMessage(
					"[FORCEROUND] Принудительно запускается раунд "
						.. roundNum
						.. " ("
						.. (GAME_ROUNDS_LIST[roundNum].name or "Unknown")
						.. ")",
					0,
					0
				)
				...
				GameRules:SendCustomMessage(
					"[ERROR] Неверный номер раунда: "
						.. (forceMatch or "nil")
						.. ". Используйте: -forceround 1-"
						.. GAME_MAX_ROUNDS,
					0,
					0
				)
		...
		if szText == "-duelinfo" then
			...
			GameRules:SendCustomMessage(
				"[DUELINFO] Команд: "
					.. #allActiveTeams
					.. " | Последняя дуэль: "
					.. Rounds.LastDuelRound
					.. " | Разница: "
					.. (nextRoundNum - Rounds.LastDuelRound),
				0,
				0
			)
			GameRules:SendCustomMessage(
				"[DUELINFO] Команды: "
					.. table.concat(allActiveTeams, ", ")
					.. " | PVP пар: "
					.. #Rounds.PVPPairs,
				0,
				0
			...
			GameRules:SendCustomMessage(
				"[DUELINFO] Может дуэль: "
					.. (canHaveDuel and "Да" or "Нет")
					.. " | Будет дуэль: "
					.. (shouldHaveDuel and "Да" or "Нет")
					.. " | has_duel="
					.. tostring(nextRoundInfo and nextRoundInfo.has_duel),
				0,
				0
			)
			...
			for playerID, playerInfo in pairs(allActivePlayers) do
				GameRules:SendCustomMessage(
					"[DUELINFO] Игрок " .. playerID .. " в команде " .. playerInfo.team,
					0,
					0
				)
			end
		...
		if szText == "-tgn" then
			...
			NeutralItems:OnPlayerCraftNeutralItem({
				PlayerID = 1,
				item = "item_trusty_shovel",
				enchant = "item_enhancement_mystical",
			})
		...
		if szText == "-tpvp" then
			CustomGameEventManager:Send_ServerToPlayer(
				hPlayer,
				"ShowPvpBet",
				{ players = { { playerID = 0, teamID = 2 }, { playerID = 1, teamID = 3 } }, firstTeamId = 2, secondTeamId = 3 }
			)
		end
		...
		if szText == "-panel" then
			CustomUI:DynamicHud_Create(
				nPlayerId,
				"Hero_Demo",
				"file://{resources}/layout/custom_game/hud_hero_demo/hud_hero_demo.xml",
				nil
			)
		end
			...
			if self.allStopActive then
				...
				if table.count(self.playerArenas) == 0 then
					...
					if mainArenaInfo then
						...
						GameRules:SendCustomMessage(
							"Ошибка: не удалось найти информацию об аренах.",
							0,
							0
						)
						return
				...
				GameRules:SendCustomMessage(
					"Команда -allstop АКТИВИРОВАНА. Крипы ("
						.. creepCount
						.. ") направляются в центры арен.",
					0,
					0
				)
			else
				...
				GameRules:SendCustomMessage(
					"Команда -allstop ДЕАКТИВИРОВАНА. Крипы ("
						.. creepCount
		...
		if szText == "-allstart" then
			...
			if self.allStopActive then
				...
				GameRules:SendCustomMessage(
					"Команда -allstart АКТИВИРОВАНА. Крипы ("
						.. creepCount
						.. ") вернулись к обычному поведению.",
					0,
					0
				...
				GameRules:SendCustomMessage(
					"Режим -allstop не активен. Используйте сначала команду -allstop.",
					0,
					0
				)
			end
	...
	if GameRules:IsCheatMode() and (szText == "-itemslots" or szText == "-slots") then
		...
		for i = DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_6 do
			...
			if item then
				print(
					string.format(
						"Slot %d (MAIN): %s | State: %d | HasModifier: %s",
						i,
						item:GetAbilityName(),
						item:GetItemState(),
		...
		for i = DOTA_ITEM_SLOT_7, DOTA_ITEM_SLOT_9 do
			...
			if item then
				print(
					string.format(
						"Slot %d (BACKPACK/TECHIES): %s | State: %d | HasModifier: %s",
						i,
						item:GetAbilityName(),
						item:GetItemState(),
						hero:HasModifier("modifier_" .. item:GetAbilityName()) and "YES" or "NO"
		...
		if item_neutral then
			print(
				string.format(
					"Slot %d (NEUTRAL): %s | State: %d | HasModifier: %s",
					16,
		...
		for i = DOTA_STASH_SLOT_1, DOTA_STASH_SLOT_6 do
			...
			if item then
				print(
					string.format(
						"Slot %d (STASH): %s | State: %d | HasModifier: %s",
						i,
						item:GetAbilityName(),
						item:GetItemState(),
						hero:HasModifier("modifier_" .. item:GetAbilityName()) and "YES" or "NO"
