...
function Util:MoveHeroToLocation(nPlayerID, vLocation)
	...
	if hHero then
		...
		for _, unit in ipairs(AllControllable) do
			if
				unit:GetUnitName() ~= "npc_dota_hero_target_dummy"
				and Map:GetPositionArena(vLocation) ~= "MINIGAMES"
			then
				Util:RemoveMovemenModifier(unit)
				local particle_start_summon = ParticleManager:CreateParticle(
				...
				local particle_end_summon = ParticleManager:CreateParticle(
...
function Util:RefreshAbilityAndItem(hHero, exceptions)
	...
	for i = 0, hHero:GetAbilityCount() - 1 do
		...
		if
			hAbility
			and hAbility:GetAbilityType() ~= ABILITY_TYPE_ATTRIBUTES
			and not exceptions[hAbility:GetAbilityName()]
		then
	...
	if
		hHero.tempest_double_hClone ~= nil
		and not hHero.tempest_double_hClone:IsNull()
		and hHero.tempest_double_hClone:IsAlive()
	then
...
function Util:RemoveAbilityClean(hHero, sAbilityName)
	...
	if
		sAbilityName == "pudge_meat_hook"
		or sAbilityName == "marci_grapple"
		or sAbilityName == "rattletrap_hookshot"
		or sAbilityName == "bane_nightmare"
	then
...
function Util:CleanBugAbilities(hHero)
	...
	for _, thinker in pairs(thinkers) do
		for id, mod_name in pairs(modifiers_table) do
			if
				thinker
				and not thinker:IsNull()
				and thinker:GetTeamNumber() == hHero:GetTeamNumber()
				and thinker:HasModifier(mod_name)
			then
...
function Util:CleanFurion(hHero)
	...
	for _, thinker in pairs(thinkers) do
		if
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_blind")
		then
		...
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_blind_aura")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_entangle")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_marker")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_shard")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_tether")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		elseif
			...
			and not thinker:IsNull()
			and thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:HasModifier("modifier_furion_sprout_tether_aura")
		then
			GridNav:DestroyTreesAroundPoint(thinker:GetAbsOrigin(), 320, true)
			UTIL_Remove(thinker)
		end
...
function Util:CleanSlark(hHero)
	...
	for _, thinker in pairs(thinkers) do
		if
			thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:GetOwner() == hHero
			and thinker:HasModifier("modifier_slark_depth_shroud")
		then
			UTIL_Remove(thinker)
		elseif
			thinker:GetTeamNumber() == hHero:GetTeamNumber()
			and thinker:GetOwner() == hHero
			and thinker:HasModifier("modifier_slark_depth_shroud_thinker")
		then
			UTIL_Remove(thinker)
		end
	end
