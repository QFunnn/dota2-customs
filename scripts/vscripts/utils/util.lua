...
function Util:ChooseObservingTarget(playerId)
	if
		PvpModule.homeTeamId
		and Settings:Get(playerId, SettingsKey.AUTO_VIEW_PVP) == true
		and not PvpModule.IsPvpEnd
	then
	...
	if
		Settings:Get(playerId, SettingsKey.AUTO_VIEW_PVE) == true
		and GameMode.currentRound
		and not GameMode.currentRound.isEnd
	then
		...
		for teamNumber, isAlive in pairs(GameMode.aliveTeamMap) do
			if
				isAlive
				and GameMode.currentRound.spawners[teamNumber]
				and false == GameMode.currentRound.spawners[teamNumber].isProgressFinished
			then
			...
			local nGold = math.ceil(
				PlayerResource:GetGoldPerMin(nPlayerID)
					* (GameRulesCustom:GetGameTime() - GameRulesCustom.gameStartTime)
					/ 60
			) + 600 - PvpModule.TotalBetSum[nPlayerID]
			return nGold
		end
	end
	return 600
	...
	if IsValid(hHero) then ---@cast hHero CDOTA_BaseNPC_Hero
		...
		for i = 0, hHero:GetAbilityCount() - 1 do
			...
			if
				...
				and Util:IsValidAbility(ability:GetAbilityName())
...
function ForEachPlayer(func, skipFake, allowInvalid)
	for playerId = 0, CHC_MAX_PLAYER_COUNT - 1 do
		if
			(allowInvalid or PlayerResource:IsValidPlayer(playerId))
