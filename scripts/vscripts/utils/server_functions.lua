...
function SendGameHudNotification(player_id, payload)
	...
	if numeric_player_id == nil or numeric_player_id < 0 then
		return false
	end
...
function ConsumeLevelUpItemCharge(caster, item)
	if not IsValid(caster, item) then
		return false
	end
...
function EnsureTwinGateWarpAbility(hero)
	if not IsValid(hero) or not hero:IsRealHero() then
		return nil
	end
...
function GetLevelUpPlayerSettings(player_id)
	...
	server_data.player_settings.camera_distance =
		NormalizeLevelUpCameraDistance(server_data.player_settings.camera_distance)
...
function LevelUpParticleManager:ForEachParticle(handle, callback)
	if not self:IsParticleHandle(handle) then
		return
	end
...
function LevelUpParticleManager:SetParticleControlEnt(
	handle,
	control_point,
	entity,
	attach_type,
...
function LevelUpParticleManager:DestroyParticle(handle, immediate)
	if not self:IsParticleHandle(handle) or handle.destroyed then
		return
	end
...
function LevelUpParticleManager:ReleaseParticleIndex(handle)
	if not self:IsParticleHandle(handle) or handle.released then
		return
	end
...
function GetRealHero(hAttacker)
	...
	if
		hAttacker.IsRealHero
		and hAttacker.IsIllusion
		and hAttacker:IsRealHero()
		and not hAttacker:IsIllusion()
		and hAttacker.IS_CUSTOM_ILLUSION_RD
	then
...
function IsLevelUpSummon(unit)
	...
	if unit_name == "npc_levelup_main_building" then
		return false
	end
	if unit_name == "npc_dota_thinker" or unit_name == "npc_dota_companion" then
		return false
	end
...
function CanUnitTriggerHeroOnlyProc(unit)
	...
	if IsLevelUpAssistant(unit) then
		return false
	end
	return true
end
...
function IsOrdinaryWaveCreep(target)
	...
	return unit_name == "npc_levelup_wave_melee"
		or unit_name == "npc_levelup_wave_melee_crystal"
		or unit_name == "npc_levelup_wave_melee_mega"
		or unit_name == "npc_levelup_wave_melee_mega_crystal"
		or unit_name == "npc_levelup_wave_melee_mega_insect"
		or unit_name == "npc_levelup_wave_melee_mega_bird"
		or unit_name == "npc_levelup_wave_melee_mega_clown"
		or unit_name == "npc_levelup_wave_melee_mega_crocodile"
end
...
function IsLevelUpBossOrMiniBossUnit(unit)
	...
	if LEVELUP_BOSS_UNIT_NAMES[unit_name] then
		return true
	end
	if BOSS_UNIT_ABILITIES and BOSS_UNIT_ABILITIES[unit_name] then
		return true
	end
	if unit:FindAbilityByName("levelup_boss_ability_handler") then
		return true
	end
	if unit:HasModifier("modifier_levelup_boss_ability_handler") then
		return true
	end
...
function IsLevelUpControlImmuneUnit(unit)
	...
	if IsLevelUpBossOrMiniBossUnit(unit) then
		return true
	end
		...
		for player_id, data in pairs(player_data.players_list) do
			...
			if player and IsLevelUpPlayerDamageNumbersEnabled(player_id) then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"event_create_particle_damage_for_player",
					{
						x = point.x,
						y = point.y,
						value = value,
						is_phys_crit = is_phys_crit,
						is_magic_crit = is_magic_crit,
					}
				)
			end
		end
