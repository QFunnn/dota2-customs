...
---comment
---@param player integer | CDOTAPlayerController | nil
---@param table {text: string?, duration: float?, class: string?, style: any, continue: boolean}
function Notifications:Top(player, table)
	if type(player) == "number" then
		player = PlayerResource:GetPlayer(player) ---@cast player CDOTAPlayerController
	end
	...
	if table.text ~= nil then
		CustomGameEventManager:Send_ServerToPlayer(
			player,
			"top_notification",
			{ text = table.text, duration = table.duration, class = table.class, style = table.style, continue = table.continue }
		)
	...
	elseif table.image ~= nil then
		CustomGameEventManager:Send_ServerToPlayer(
			player,
			"top_notification",
			{ image = table.image, duration = table.duration, class = table.class, style = table.style, continue = table.continue }
		)
	...
	elseif table.item ~= nil then
		CustomGameEventManager:Send_ServerToPlayer(
			player,
			"top_notification",
			{ item = table.item, duration = table.duration, class = table.class, style = table.style, continue = table.continue }
		)
	else
		CustomGameEventManager:Send_ServerToPlayer(
			player,
...
---comment
---@param table {text: string?, duration: float?, imagestyle: any?, class: string?, style: any, continue: boolean?}
