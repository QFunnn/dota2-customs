...
function damage_filter_util:GetNewDamageFromAbilities(params, attacker, target)
	...
	if
		target
		and (target:GetUnitName() == "npc_dota_roshan" or target:GetUnitName() == "npc_dota_nian")
		and ability
		and ability_name == "mirana_arrow"
	then
		return 0
	end
	...
	if
		...
		and (target:GetUnitName() == "npc_dota_roshan" or target:GetUnitName() == "npc_dota_nian")
		and ability
		and string.find(ability_name, "item_hydras_breath")
	then
		return 0
	end
	...
	if
		attacker
		and not attacker:IsNull()
		and (
			attacker:HasModifier("modifier_report_curse")
			or (not attacker:IsUnitInPVP() and attacker:HasModifier("modifier_loser_curse"))
		)
	then
		...
		if not attacker:IsUnitInPVP() then
			...
			if lc then
				curseStacks = curseStacks + (lc:GetStackCount() or 0)
			end
		end
	end
	...
	if
		ability
		and damage_filter_util.abilities_max_health_damage[ability:GetAbilityName()]
		and target
	...
	elseif curseStacks > 0 then
		if
			ability == nil
			or (ability ~= nil and not damage_filter_util.abilities_oneshot_curse[ability:GetAbilityName()])
		then
	...
	if ability ~= nil and damage_filter_util.abilities_oneshot[ability:GetAbilityName()] ~= nil then
		if GameMode.currentRound and GameMode.currentRound.nRoundNumber > 70 then
			if
				target
				and RollPercentage(30)
				and target:GetHealth() > target:GetMaxHealth() * 0.51
				and not target:IsHero()
			then
