...
function SetTimer(cmdName, time)
	logger:Log("Set the timer to: " .. time)
	...
	for k, v in ipairs(l) do
		...
		if v ~= "FDesc" then
			...
			elseif type(value) == "userdata" and not done[value] then
				...
				result = result ..
					PrintTableToString((getmetatable(value) and getmetatable(value).__index) or getmetatable(value),
						indent + 2, done) .. "\n"
			else
				if t.FDesc and t.FDesc[v] then
...
function GetRandomValidPosition(vTopLeft, vDownRight)
	...
	while not GridNav:CanFindPath(GameRulesCustom.vWorldCenterPos, randomPos) do
	...
	if IsValidEntity(hHero) then
		for i = 1, hHero:GetAbilityCount() - 1 do
			...
			if hAbility and string.sub(hAbility:GetAbilityName(), 1, 14) ~= "special_bonus_" then
				...
				logger:Log(hAbility:GetAbilityIndex() ..
					":" ..
					hAbility:GetAbilityName() ..
					" nPlaceholder:" .. nPlaceholder .. " Hidden:" .. sHidden .. " Activated:" .. sActivated)
			end
		end
	end
end
...
function TableOverride(mainTable, table)
	if mainTable == nil then return table end
	if table == nil or type(table) ~= "table" then return mainTable end
...
function CreateHashtable(table)
	local new_hastable = {}
	local index = 1
	while Hashtables[index] ~= nil do index = index + 1 end
	if table ~= nil then
...
function GetHashtableIndex(hastable)
	...
	for index, h in pairs(Hashtables) do
		if h == hastable then return index end
	end
	return nil
end

function GetHashtableByIndex(index) return Hashtables[index] end

function HashtableCount()
	local n = 0
	for index, h in pairs(Hashtables) do n = n + 1 end
	return n
end
...
function AddModifierEvents(iModifierEvent, hModifier, hSource, hTarget)
	if IsValid(hTarget) or IsValid(hSource) then
		if IsValid(hSource) then
			...
			table.insert(hSource.tSourceModifierEvents[iModifierEvent],
				hModifier)
		end
		if IsValid(hTarget) then
			...
			table.insert(hTarget.tTargetModifierEvents[iModifierEvent],
				hModifier)
		end
	else
		if _G.tModifierEvents == nil then _G.tModifierEvents = {} end
...
function IsVector(v)
	if v.x == nil then return false end
	if v.y == nil then return false end
	if v.z == nil then return false end
	if type(v.x) ~= "number" then return false end
	if type(v.y) ~= "number" then return false end
	if type(v.z) ~= "number" then return false end
	return true
end
...
function DEVRandomChoice()
	local t = { "lion", "obsidian", "invoker", "vs" }
	local count = {}
	for i = 1, 100, 1 do
		local r = t[RandomInt(1, #t)]
		if count[r] == nil then count[r] = 0 end
		count[r] = count[r] + 1
	end
	for k, v in pairs(count) do logger:Log("N2O", k, v) end
end
...
function Rotation2D(vVector, radian)
	...
	return Vector(vUnitVector2D.x * fCos - vUnitVector2D.y * fSin,
		vUnitVector2D.x * fSin + vUnitVector2D.y * fCos,
		vUnitVector2D.z) * fLength2D
end

function GetHeroFacetsKv(sHeroName)
	local heroKv = TableOverride(LoadKeyValues("scripts/npc/npc_heroes.txt"),
		LoadKeyValues(
			"scripts/npc/npc_heroes_custom.txt"))[sHeroName]
	return heroKv["Facets"]
end

function GetHeroFacetsAbilities(sHeroName)
	local abilities = {}
	for k, v in pairs(GetHeroFacetsKv(sHeroName)) do
		if v["Abilities"] then
			for _, ability in pairs(v["Abilities"]) do
				if ability.AbilityName then
					table.insert(abilities, ability.AbilityName)
				end
			end
		end
	end
...
function GetHeroFacetsAbilitiesKv(sHeroName)
	local abilities = {}
	for k, v in pairs(GetHeroFacetsKv(sHeroName)) do
		if v["Abilities"] then
			for _, ability in pairs(v["Abilities"]) do
