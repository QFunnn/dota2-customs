...
function LoadGameKeyValues()
	...
	for name, paths in pairs(files) do
		local baseFile = {}
		if LOAD_BASE_FILES or name == "ItemKV" or name == "HeroKV" then
			baseFile = LoadKeyValues(scriptPath .. paths.base .. ".txt")
		end
		local file = LOAD_BASE_FILES and baseFile or {}

		-- Bots use base + custom items without the ability override rules.
		local botItems = name == "ItemKV" and copyTable(baseFile) or nil
		local heroWearables
		if name == "HeroKV" then
			-- Preserve only the original fields needed by the wearable system.
			heroWearables = {}
			for heroName, heroData in pairs(baseFile) do
				if type(heroData) == "table" then
					heroWearables[heroName] = {
						ModelScale = heroData.ModelScale,
		...
		local custom_file = LoadKeyValues(scriptPath .. paths.custom .. ".txt")
		if custom_file then
			for key, value in pairs(custom_file) do
				file[key] = value
				if botItems then
					botItems[key] = value
				end
			...
			print("[KeyValues] Critical Error on " .. paths.custom .. ".txt")
			return
		end

		if botItems then
			GameRules.allItemKV = botItems
		elseif name == "UnitKV" then
			-- Keep the custom-only lookup while sharing its records with UnitKV.
			KeyValues.UnitsKv = custom_file
		elseif heroWearables then
			KeyValues.HeroWearableKV = heroWearables
		end

		GameRules[name] = file --backwards compatibility
		KeyValues[name] = file
	end
	...
	-- Release obsolete roots when updating an already initialized Lua state.
	GameRules.itemKV = nil
	GameRules.customItemKV = nil
	_G.npc_heroes = nil
end

-- Keep the existing initialization entry point for the custom shop module.
function KeyValues:InitializeCustomShop(reloadKeyValues)
	return require("utils/custom_shop"):Initialize(reloadKeyValues)
end
...
if not KeyValues.All or not KeyValues.HeroWearableKV or not KeyValues.UnitsKv or not GameRules.allItemKV then
	LoadGameKeyValues()
end