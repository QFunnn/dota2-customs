...
function KeyValues:Init()
	...
	CustomGameEventManager:RegisterListener("key_values_get_ability_info", function(_, event)
		self:OnGetAbilityInfo(event)
	end)
end

function KeyValues:GetHeroMinimapIconID(HeroName)
	if self.MinimapIconsKV["TextureData"] == nil then
		return
	end
...
function KeyValues:GetHeroInnatesAndFacetsAbilities(HeroName)
	if not self.HeroesList[HeroName] then
		return {}
	end
...
function KeyValues:LoadModifiedAbilities()
	...
	local function isAllowed(name, info)
		...
		if name:find("special_bonus") then
			return false
		end
		if name:find("_innate") then
			return false
		end
		if MODIFIED_ABILITIES_EXCLUDE[name] then
			return false
		end
	...
	if not self.RegisteredNotes[AbilityName] then
		...
		if KV then
			...
			CustomGameEventManager:Send_ServerToPlayer(
				Player,
				"key_values_send_ability_info",
				{ ability_name = AbilityName, values = AbilityValues }
			)
		end
	else
		CustomGameEventManager:Send_ServerToPlayer(
			Player,
			"key_values_send_ability_info",
			{ ability_name = AbilityName, values = self.RegisteredNotes[AbilityName] }
		)
	end
end
...
function KeyValues:IsBannedAbility(AbilityName)
	if self.AbilitiesList[AbilityName] == nil then
		return true
	end
...
function KeyValues:IsBannedHero(HeroName)
	...
	if self.HeroesList[HeroName] == nil then
		return true
	end
...
if not KeyValues.bStarted then
	KeyValues:Init()
end