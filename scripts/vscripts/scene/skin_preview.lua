...
function Spawn(params)
	...
	-- 光源
	local vSpecularDirection = StringToVector(
		tPortraitData.PortraitSpecularDirection or defaultData.PortraitSpecularDirection
	) or vec3_zero
	local aSpecularAngles = VectorToAngles(vSpecularDirection)
	local vAmbientDirection = StringToVector(
		tPortraitData.PortraitAmbientDirection or defaultData.PortraitAmbientDirection
	) or vec3_zero
