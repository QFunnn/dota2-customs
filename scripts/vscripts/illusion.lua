...
function Illusion:InitIllusion(hIllusion, hCaster)
		...
		if hAbility ~= nil then
			...
			if not string.find(AbilityName, "special_bonus") then
				...
				if IsValid(hIllusionAbility) then
					...
					if
						IsKvFlagEnabled(kv.IsGrantedByShard)
						and hOriginalHero:HasShard()
	...
	if not hIllusion:IsIllusion() then
		...
		if
			neutralItem ~= nil
			and neutralItem:GetAbilityName() ~= "item_philosophers_stone"
			and not IsKvFlagDisabled(neutralItem:GetAbilityKeyValues().IsTempestDoubleClonable)
		then
		...
		for i = DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_6 do
			...
			if item ~= nil then
				...
				local clonable = not IsKvFlagDisabled(item:GetAbilityKeyValues().IsTempestDoubleClonable)
					and (item_name ~= "item_philosophers_stone")
				if clonable then
