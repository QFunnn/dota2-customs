...
function Tips:Tip(event)
	...
	if not IsValidPlayerID(source_player_id) then
		return
	end

	local target_player_id = event.target_player_id
	if not IsValidPlayerID(target_player_id) then
		return
	end
...
function Tips:GetMaxDailyTips(player_id)
	if GameMode:IsDeveloper(player_id) then
		return 999999
	end
