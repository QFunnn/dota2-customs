...
function WebInventory:Init()
	...
	WebInventory.once_purchased_items = {
		reset_mmr = {},
	}
...
function WebInventory:AddBackendItem(player_id, item_name, count)
	...
	WebApi:Send("api/lua/battlepass/inventory/v2/add_item", {
		steam_id = steam_id,
		item_name = item_name,
...
function WebInventory:BuildClientData()
	...
	for item_name, item_definition in pairs(ITEM_DEFINITIONS or {}) do
		...
		WebInventory.definitions_client_data[item_name] = {
			...
			is_hidden_owned = item_definition.is_hidden_owned,
			chat_wheel_details = item_definition.chat_wheel_details,
		}
	end
...
function WebInventory:AddItem(player_id, item_data)
	if not WebInventory.players_items[player_id] then
		WebInventory.players_items[player_id] = {}
	end
...
function WebInventory:_HasItem(player_id, item_name)
	...
	if not WebInventory.players_items[player_id][item_name] then
		return false
	end
	return true
end
...
function WebInventory:HasItem(player_id, item_name)
	...
	if not WebInventory:_HasItem(player_id, item_name) then
		return false
	end
	return true
end
...
function WebInventory:ConsumeItem(player_id, item_name, item_count, on_item_used)
	local steam_id = tostring(PlayerResource:GetSteamID(player_id))
	if not WebInventory.players_items[player_id] or not WebInventory.players_items[player_id][item_name] then
		return
	end
	...
	WebApi:Send("api/lua/battlepass/inventory/v2/use_item", {
		steam_id = steam_id,
		item_name = item_name,
...
function WebInventory:_FilterSinglePurchasedItem(player_id, item_name)
	if not WebInventory.once_purchased_items[item_name] then
		return
	end

	local player = PlayerResource:GetPlayer(player_id)
	if player and WebInventory.once_purchased_items[item_name][player_id] then
		DisplayErrorWithValues(player_id, "#collection_one_item_purchase_error", { item_name = item_name })
		return true
	end

	WebInventory.once_purchased_items[item_name][player_id] = true
end
...
function WebInventory:PurchaseItem(player_id, item_name, total_cost, count)
	...
	if WebInventory:_FilterSinglePurchasedItem(player_id, item_name) then
		return
	end

	if item_name == "reset_mmr" then
		WebPayments:PurchaseWithCurrencyEvent({
	...
	-- adapting to old webserver
	WebApi:Send("api/lua/battlepass/inventory/v2/buy_items", {
		steam_id = steam_id,
		item_name = item_name,
		...
		print("[WebInventory] successfully purchased item", item_name)
		WebPlayer:SetCurrency(player_id, data.glory)
		WebPlayer:UpdateClient(player_id)
...
function WebInventory:ModifyBackendItemCount(player_id, item_name, item_count_change)
	...
	WebApi:Send("api/lua/inventory/modify_item_count", {
		steam_id = steam_id,
		item_name = item_name,
...
function WebInventory:GetItemCost(item_name)
	local definition = WebInventory:GetItemDefinition(item_name)
	if not definition.unlocked_with then
		return 0
	end
	...
	WebInventory:ConsumeItem(player_id, item_name, consumed_count, function()
		if definition.on_consume then
			...
			if definition.on_consume_context then
				ErrorTracking.Try(
					definition.on_consume,
					definition.on_consume_context,
					player_id,
					item_name,
					item_data,
					definition
