...
function Equipment:Equip(player_id, item_name)
	...
	if not item_definition then
		print("[Equipment] no definition found for", item_name)
		return
	end
	...
	PrecacheManager:PrecacheResourceListAsync(resource_list, function()
		print("Resource list precached")
		Equipment:_EquipOnPrecache(player_id, item_name, item_definition)
	end)

	return true
end

function Equipment:_EquipOnPrecache(player_id, item_name, item_definition)
	local slot = item_definition.slot
	if not slot then
		print("[WebInventory] no slot found for item", item_name)
		return
	end
	...
	if Equipment.slot_callbacks[slot] then
		local _, modifiers, particles, units =
			Equipment:SpecialSlotEquipped(player_id, hero, item_name, item_definition)
...
function Equipment:ParticleFromData(player_id, particle_data, unit, target_table)
	...
	local bound_unit = unit
	if particle_data.unbind_from_target then
		bound_unit = nil
	end
	print(
		"[Equipment] ParticleFromData",
		particle_data.path,
		particle_data.attach_type,
		bound_unit,
		particle_data.unbind_from_target
	)
	local p_id = ParticleManager:CreateParticle(particle_data.path, particle_data.attach_type, bound_unit)

	for cp_id, cp_data in pairs(particle_data.control_points or {}) do
		if cp_data.frozen_on_target then
			ParticleManager:SetParticleControl(p_id, cp_id, unit:GetAbsOrigin())
		end
		...
		if cp_data.orientation then
			ParticleManager:SetParticleControlTransform(p_id, cp_id, unit:GetOrigin(), unit:GetAngles())
		end
	end
...
function Equipment:PlayItemEffects(player_id, item_name, target_override, particle_variant)
	-- DebugMessage("[Equipment] PlayItemEffects", player_id, item_name)
	...
	if definition.particle_variants and particle_variant then
		...
		local p_id = Equipment:ParticleFromData(player_id, particle_data, unit, particles)
...
function Equipment:_RemoveBoundAssets(unit)
	if not unit._equipment_bound_assets then
		return
	end
...
function Equipment:DestroyItemEffects(player_id, item_name)
	...
	local current_equipped_item = Equipment.equipped_items[player_id][slot]
	if not current_equipped_item then
		return
	end
...
function Equipment:GetEquippedItemsForServer(player_id)
	local result = {}
	for slot_id, slot_name in pairs(OLD_SLOT_NAMES) do
		result[slot_name] = result[slot_name] or {}
	end
	for slot, item in pairs(Equipment.equipped_items[player_id] or {}) do
		local slot_name = OLD_SLOT_NAMES[slot]
		print("slot name", slot, type(slot), slot_name)
		if slot_name then
			result[slot_name] = { item.name }
		else
			DebugMessage("[Equipment] failed to parse slot name for item", item.name, "in slot", slot)
		end
	end
...
function Equipment:GetItemInSlot(player_id, slot)
	if not Equipment.equipped_items[player_id] then
		return
	end
	if not Equipment.equipped_items[player_id][slot] then
		return
	end
...
function Equipment:GetParticleVariant(item_name, variant)
	local definition = ITEM_DEFINITIONS[item_name]
	if not definition or not definition.particle_variants then
		print("[Equipment] no definition or particle variants defined for", item_name)
		return
	end
...
function Equipment:OnEntityKilled(event)
	...
	if not killer_player_owner_id or killer_player_owner_id == -1 then
		return
	end
...
function Equipment:OnNpcSpawned(event)
	if not event.unit:IsIllusion() and not event.unit:IsClone() and not event.unit:IsTempestDouble() then
		return
	end
	...
	-- this applies particles and shit to illusions, clones and whatnot
	-- which we will disable for now
...
function Equipment:OnHeroPicked(event)
	...
	Timers:CreateTimer(1, function()
		if not IsValidEntity(event.hero) then
			return
		end
...
function Equipment:StartBackendUpdateTimer()
	...
	Equipment._backend_request_timer = Timers.CreateTimer(EQUIPMENT_UPDATE_DELAY, function()
		if next(Equipment._scheduled_updates) == nil then
		...
		for player_id, _ in pairs(Equipment._scheduled_updates or {}) do
			...
			equipped_items[steam_id] = Equipment:GetEquippedItemsForServer(player_id)
		end
		DeepPrintTable(equipped_items)
		WebApi:Send("api/lua/battlepass/inventory/set_player_equipped_items", {
...
function Equipment:OnPetEquipped(player_id, hero, item_name, item_definition)
	...
	local pet =
		CreateUnitByName("npc_cosmetic_pet", hero:GetAbsOrigin() + RandomVector(300), true, hero, hero, hero:GetTeam())
	-- pet:SetOwner(hero)
...
function Equipment:GetItemResourceList(item_definition)
	local resource_list = {}
	...
	if item_definition.blink_particles then
		if item_definition.blink_particles.start_name then
			-- PrecacheResource("particle", item_definition.blink_particles.start_name, context)
			resource_list[item_definition.blink_particles.start_name] = "particle"
		end
		if item_definition.blink_particles.end_name then
			-- PrecacheResource("particle", item_definition.blink_particles.end_name, context)
			resource_list[item_definition.blink_particles.end_name] = "particle"
		end
	end
