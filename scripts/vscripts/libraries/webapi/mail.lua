...
function WebMail:AddMail(player_id, mail_data)
	if not WebMail.mails[player_id] then
		WebMail.mails[player_id] = {}
	end
...
function WebMail:DeleteMail(event)
	...
	WebApi:Send("api/lua/mail/delete", {
		steam_id = steam_id,
...
function WebMail:ClaimMail(event)
	...
	if not mail_id or not WebMail.mails[player_id] or not WebMail.mails[player_id][mail_id] then
		print(
			"[WebMail] no mail found",
			player_id,
			mail_id,
			WebMail.mails[player_id],
			WebMail.mails[player_id][mail_id]
		)
	...
	WebApi:Send("api/lua/mail/claim", {
		steam_id = steam_id,
		mail_id = event.mail_id,
	}, function(response)
