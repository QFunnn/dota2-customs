...
function WebPromoEvents:Init()
	...
	EventDriver:Listen("Events:state_changed", function(event)
		...
		if not WebApi.__before_match_loaded then
			print("[WebPromoEvents] skipping init - BeforeMatch failed!")
		end

		if WebPromoEvents.__requested_event_data then
			return
		end
...
function WebPromoEvents:IndexEvents()
	for event_id, event_data in pairs(PROMO_EVENTS_DATA) do
		...
		local has_start_date = event_data.started_at
			and event_data.started_at ~= ""
			and DateOlder(event_data.started_at, GetSystemDate())
		...
		if is_active and is_ongoing and has_start_date and has_tasks_defined then
			...
			print(
				"[WebPromoEvents] event is deemed inactive: ",
				event_id,
				is_ongoing,
				is_active,
				has_start_date,
...
function WebPromoEvents:IsClaimed(player_id, event_id, task_id)
	return WebPromoEvents.claimed_tasks[player_id]
		and WebPromoEvents.claimed_tasks[player_id][event_id]
		and WebPromoEvents.claimed_tasks[player_id][event_id][task_id]
end
...
function WebPromoEvents:ClaimRewardEvent(event)
	...
	if not event_id or not task_id then
		return
	end
	if not WebPromoEvents.active_events[event_id] then
		return
	end
	if not WebPromoEvents.fetched_data[event_id] then
		return
	end
	...
	if reported_progress < target then
		return
	end

	if WebPromoEvents:IsClaimed(player_id, event_id, task_id) then
		DebugMessage("Player", player_id, "already redeemed event", event_id, task_id)
		return
	end

	WebApi:Send("api/lua/promo/events/claim", {
		event_id = event_id,
		steam_id = steam_id,
		claim_id = task_id,
...
function WebPromoEvents:RequestEventData()
	...
	for event_id, event_data in pairs(WebPromoEvents.active_events) do
		...
		WebApi:Send("api/lua/promo/events/fetch_data", {
			event_id = event_id,
			...
			DebugMessage(
				"[WebPromoEvents] fetched data for event: ",
				event_id,
				"; took: ",
				GetSystemTimeMS() - WebPromoEvents.__fetch_timing[event_id],
				"ms"
			)
			DeepPrintTable(response)
...
function WebPromoEvents:UpdateUnclamedStatus()
	...
	if fetched_count < expected_count then
		DebugMessage(
			"[WebPromoEvents] received unclaimed update, waiting till all events are fetched:",
			fetched_count,
			"/",
			expected_count
		)
