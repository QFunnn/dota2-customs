...
function WebPayments:Init()
	...
	EventStream:Listen(
		...
		WebPayments.RequestSubscriptionUpgradeUrl,
		WebPayments
	)
...
function WebPayments:RequestCustomerPortalUrl(event)
	...
	WebApi:Send("api/lua/payments/create_customer_portal_url", {
		steam_id = steam_id,
...
function WebPayments:RequestSubscriptionUpgradeUrl(event)
	...
	WebApi:Send("api/lua/payments/create_upgrade_url", {
		steam_id = steam_id,
		subscription_source = source,
	}, function(data)
		local player = PlayerResource:GetPlayer(player_id)
...
function WebPayments:SetPaymentStatus(player_id, status)
	if status then
		...
		WebPayments.pending_requests[player_id] = Timers:CreateTimer(600, function()
...
function WebPayments:ValidatePaymentRequest(event)
	if not WebPayments.valid_payment_methods[event.payment_system] then
		return false
	end
	if not WebPayments.valid_payment_methods[event.payment_system][event.payment_method] then
		return false
	end

	if not WebPayments.valid_modes[event.payment_mode] then
		return false
	end
	if not WebPayments.valid_modes[event.payment_mode][event.payment_method] then
		return false
	end
...
function WebPayments:RequestPaymentUrl(event)
	...
	WebApi:Send("api/lua/payment/get_payment_url", {
		steam_id = steam_id,
...
function WebPayments:CancelSubscription(event)
	...
	WebApi:Send("api/lua/payments/cancel_subscription", {
		steam_id = steam_id,
...
function WebPayments:GetSubscriptionGloryPrice(base_price, duration)
	...
	local scaled_price = price_per_day
		* (
			...
			+ (SUBSCRIPTION_DURATION_MAX - SUBSCRIPTION_MAX_MULTIPLIER)
				* math.pow(
					(duration - SUBSCRIPTION_DURATION_MIN) / (SUBSCRIPTION_DURATION_MAX - SUBSCRIPTION_DURATION_MIN),
					SUBSCRIPTION_STEP
				)
		)
...
function WebPayments:PurchaseWithCurrencyEvent(event)
	...
	if not currency_price then
		return
	end

	local duration =
		math.min(math.max(event.duration_override or SUBSCRIPTION_DURATION_MAX, 1), SUBSCRIPTION_DURATION_MAX)
	print(event.duration_override, SUBSCRIPTION_DURATION_MAX, duration)
	local price_from_duration = VARIABLE_DURATION_PRODUCTS[event.product_name]
			and WebPayments:GetSubscriptionGloryPrice(currency_price, duration)
		or currency_price

	DebugMessage(
		"[WebPayments] buying",
		event.product_name,
		"with currency, duration:",
		duration,
		"price:",
		price_from_duration
	)

	WebApi:Send("api/lua/payment/buy_subscription_with_currency", {
		steam_id = steam_id,
		match_id = WebApi:GetMatchID(),
...
MatchEvents.event_handlers.payment_success = function(data)
	...
	local player_id = WebApi:GetPlayerIdBySteamId(data.steamId)
	if not player_id or not PlayerResource:IsValidPlayerID(player_id) then
		return
	end
