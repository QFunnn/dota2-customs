...
---@return number
--- =====================
--- DEPRECATED. Current method to use rating - WebPlayer:GetRating(player_id)
--- =====================
...
function WebLeaderboards:SendLeaderboard(event)
	...
	if WebLeaderboards.fetched_leaderboards[map_name] then
		...
		CustomGameEventManager:Send_ServerToPlayer(player, "WebLeaderboards:set_leaderboard", {
			...
			requester_rating = WebPlayer:GetRating(player_id),
			map_name = map_name,
		})
		return
	end

	WebApi:Send("api/lua/match/get_leaderboard", {
		...
		WebLeaderboards.fetched_leaderboards[map_name] = response or {}
