...
function GiftCodes:FetchGiftCodes(player_id)
	...
	WebApi:Send("api/lua/gift_codes/get", {
		steam_id = steam_id,
	}, function(response)
...
function GiftCodes:SetPrefetched(player_id, data)
	GiftCodes.gift_codes[player_id] = data
	GiftCodes._fetched[player_id] = true
end

function GiftCodes:SendGiftCode(event)
	if not event.target_id then
		return
	end
...
function GiftCodes:GetRedeemResult(player_id, response)
	...
	if response.items then
		...
		for _, item_data in pairs(response.items) do
			table.insert(products.items, {
				name = item_data.itemName or item_data.name,
	...
	if response.glory then
		products.currency = response.glory - WebPlayer:GetCurrency(player_id)
	end

	if response.supporterState then
		products.subscription = response.supporterState.level
	end
...
function GiftCodes:RedeemGiftCode(event)
	...
	if not event.gift_code then
		return
	end

	local steam_id = tostring(PlayerResource:GetSteamID(player_id))

	WebApi:Send("api/lua/match/redeem_gift_code", {
		steamId = steam_id,
		code = event.gift_code,
	}, function(response)
