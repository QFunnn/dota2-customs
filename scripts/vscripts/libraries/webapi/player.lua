...
function WebPlayer:Init()
	...
	EventStream:Listen("Punishments:fetch_muted_players", WebPlayer.SendMutedPlayersEvent, WebPlayer)
	EventStream:Listen("Punishments:get_punishment", WebPlayer.SendPunishment, WebPlayer)
...
function WebPlayer:SetPlayerData(player_id, player_data)
	...
	WebPlayer.players_data[player_id] = {
		...
		punishment_end_date = player_data.punishment_end_date or "",
	}

	if WebPlayer:GetPunishmentLevel(player_id) > 0 then
		print("[WebPlayer] player", player_id, "banned from kick voting due to punishment level")
...
function WebPlayer:SetSubscriptionStatus(player_id, subscription)
	if not WebPlayer.players_data[player_id] then
		WebPlayer.players_data[player_id] = {}
	end
...
function WebPlayer:AddBackendCurrency(player_id, value)
	...
	WebApi:Send("api/lua/battlepass/add_glory", {
		steamId = steam_id,
		glory = value,
	}, function(response)
		WebPlayer:SetCurrency(player_id, WebPlayer:GetCurrency(player_id) + value)
		WebPlayer:UpdateClient(player_id)

		print(
			"[WebPlayer] successfully added backend currency to player",
			player_id,
			" | new currency: ",
			response.currency
		)
...
function WebPlayer:SetPunishmentLevel(player_id, punishment_level, punishment_reason, submit_to_backend)
	...
	if submit_to_backend then
		WebApi:Send("api/lua/match/set_punishment_level", {
...
function WebPlayer:UseCurrency(player_id, amount_to_spend, on_spent_callback, on_fail_callback)
	...
	WebApi:Send("api/lua/match/spend_currency", {
		steam_id = steam_id,
...
function WebPlayer:SendPunishment(event)
	local player_id = event.PlayerID
	...
	if not punishment_level or punishment_level < 1 then
		return
	end

	CustomGameEventManager:Send_ServerToPlayer(player, "Punishments:set_punishment", {
		punishment_level = punishment_level,
	})
end

function WebPlayer:SendMutedPlayersEvent(event)
	local player_id = event.PlayerID
	if not IsValidPlayerID(player_id) then
		return
	end

	-- collect all players that have punishment level of 4 (global muted)
	-- and send them to requestor player to mute them there, since mute is a client concept

	local muted_players = {}

	for player_id, data in pairs(WebPlayer.players_data or {}) do
		if data.punishment_level and data.punishment_level >= 4 then
			table.insert(muted_players, player_id)
		end
	end

	local player = PlayerResource:GetPlayer(player_id)
	if IsValidEntity(player) then
		CustomGameEventManager:Send_ServerToPlayer(player, "Punishments:set_muted_players", {
