...
function WebPlayerStats:GetMapStatsEvent(event)
	...
	WebApi:Send("api/player/get_map_stats_with_recent_matches", {
		steam_id = tostring(PlayerResource:GetSteamID(player_id)),
		target_map_name = map_name,
	}, function(responce)
		if not responce.matches or not responce.stats then
			return
		end
...
function WebPlayerStats:GetMatchDataEvent(event)
	...
	WebApi:Send("api/lua/match/get_stats", {
		target_match_id = match_id,
	}, function(responce)
		if not responce.match then
			return
		end
