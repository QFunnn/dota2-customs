...
WebSettings.known_defaults = {
	...
	ui_show_game_time = false,
	chatWheelFavourites = {},
	dps_auto_opening = true,
}
	...
	for _, settings in pairs(WebSettings._prepared_manifest or {}) do
		for setting_name, config in pairs(settings or {}) do
			if not config.type then
				error(setting_name .. " has no type specified! Every setting in settings manifest must have a type.")
			end
			if WebSettings.known_defaults[setting_name] then
				config.default = WebSettings.known_defaults[setting_name]
			else
				if config.type == SETTING_TYPE.CHECKBOX then
					config.default = false
				end
				if config.type == SETTING_TYPE.SLIDER then
					config.default = config.min or 0
				end
			end
		end
	end
...
function WebSettings:SetSettingValueEvent(event)
	...
	if not setting_name or not setting_value then
		return
	end
	...
	if default ~= nil and type(default) ~= type(setting_value) then
		if type(default) == "boolean" then
			setting_value = toboolean(setting_value)
		end
	end
...
function WebSettings:ScheduleUpdate(player_id)
	...
	if WebSettings._scheduled_timer then
		Timers:RemoveTimer(WebSettings._scheduled_timer)
	end
...
function WebSettings:CommitChanges()
	...
	for player_id, _ in pairs(WebSettings._scheduled_players) do
		...
		local settings = WebSettings:ExcludeDefaults(WebSettings:GetSettings(player_id))
	...
	WebApi:Send("api/lua/match/update-settings", {
		players = players,
	}, function()
