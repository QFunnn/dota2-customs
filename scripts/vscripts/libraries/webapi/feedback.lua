...
function WebFeedback:Send(event)
	...
	if used_time and (GameRules:GetGameTime() - used_time) < WebFeedback.cooldown then
		return
	end
	...
	WebApi:Send("api/lua/match/suggestion", {
		steamId = steam_id,
		content = event.text,
		supporterLevel = WebPlayer:GetSubscriptionTier(player_id),
	}, function(data)
