...
function WebApi:_HandleBeforeMatchResponse(data)
	...
	for steam_id, player_data in pairs(data.players or {}) do
		...
		Upgrades:LoadFavoriteBuilds(player_id, player_data.favorite_builds)
...
function WebApi:RequestAfterMatch(winner_team, teams_places)
	if not IsInToolsMode() and (GameRules:IsCheatMode() or GameRules:GetDOTATime(false, true) < 60) then
		return
	end

	if winner_team < DOTA_TEAM_FIRST or winner_team > DOTA_TEAM_CUSTOM_MAX then
		return
	end
	if winner_team == DOTA_TEAM_NEUTRALS or winner_team == DOTA_TEAM_NOTEAM then
		return
	end
	...
	for player_id = 0, DOTA_MAX_PLAYERS do
		if PlayerResource:IsValidPlayerID(player_id) and not PlayerResource:IsFakeClient(player_id) then
			...
			if base_place ~= place then
				...
				DebugMessage(
					"[WebApi] place override active for ",
					player_id,
					"recalculated mmr: ",
					endgame_stats.rating_change
				)
			end
			...
			table.insert(players, {
				...
				used_rerolls_count = UpgradeRerolls.used_rerolls[player_id] or 0,
				capture_time_total = endgame_stats.capture_orbs_time or 0,
			})
	...
	if not IsInToolsMode() and #players < 5 then
		return
	end
	...
	DebugMessage("[After-match] time dilation:", time_dilation)

	WebApi:Send("api/lua/match/after", {
		-- map_name \
