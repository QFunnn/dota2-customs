...
function ExtenderStash:Init()
	...
	GameListener:SubscribeProtected("items_move_to_custom_stash", function(event)
		self:OnPlayerWantMoveToCustomStash(event)
	end)
	GameListener:SubscribeProtected("items_move_from_custom_stash", function(event)
		self:OnPlayerWantMoveFromCustomStash(event)
	end)
	GameListener:SubscribeProtected("items_sell_from_custom_stash", function(event)
	...
	for PlayerID = 0, MAX_PLAYER_COUNT - 1 do
		if PlayerResource:IsValidPlayerID(PlayerID) then
			...
			CustomNetTables:SetTableValue(
				"players",
				"player_" .. PlayerID .. "_bonus_stash_items",
...
function ExtenderStash:PlaceInStorage(playerId, item)
	...
	item:SetCombineLocked(true) -- Временное решение, предмет собирается в юните, ломается отображение
end
...
function ExtenderStash:IsRestrictedSourceUnitForCustomStash(unit)
	...
	if
		unit:HasModifier("modifier_arc_warden_tempest_double_lua")
		or unit:HasModifier("modifier_arc_warden_tempest_double_lua_illusion")
	then
		return true
	end
...
function ExtenderStash:MoveCustomItemToDotaSlot(playerId, hero, customItemEntry, dotaSlot, customSlot)
	...
	if customItemItem then
		if
			(dotaSlot == 16 and not customItemItem:IsActiveNeutral())
			or (customItemItem:IsActiveNeutral() and dotaSlot <= 5)
		then
...
function ExtenderStash:HandleDotaToCustomSwap(playerId, hero, slot1Info, slot2Info)
	...
	if dotaItem ~= nil then
		...
		if bestSlot == nil then
			...
			if
				dotaSlot == 16 and bestSlotItem and not bestSlotItem:IsActiveNeutral()
				or (bestSlotItem:IsActiveNeutral() and dotaSlot <= 5)
			then
				self:Log("Item is not for this slot")
				return false
			end
...
function ExtenderStash:SwapItems(playerId, slot1, slot2, sourceUnit)
	...
	elseif
		(slot1Info.type == "DOTA" and slot2Info.type == "CUSTOM")
		or (slot1Info.type == "CUSTOM" and slot2Info.type == "DOTA")
	then
...
function ExtenderStash:RefreshItemsCooldown(playerId, exceptions)
	...
	local playerData = self.Players[playerId]
	if playerData == nil or playerData.items == nil then
		return
	end
			...
			if item and not item:IsNull() and item.EndCooldown then
				...
				if exceptions[itemName] == nil then
					...
					if itemName == "item_hand_of_midas_lua" then
						item:SetCurrentCharges(2) --todo поправить костыль, синхронно с HeroRefreshService:RefreshAbilityAndItem
					end
				end
			end
		end
