...
function PingModifiers:OnModifierAdded(event)
	...
	if not event.modifier or not event.modifier.GetName or not event.modifier.GetCaster then
		return
	end
	if event.modifier:GetName() ~= "modifier_spirit_breaker_charge_of_darkness_vision" then
		return
	end
