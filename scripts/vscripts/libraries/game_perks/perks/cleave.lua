--[[
  ~ dumper · customs · dota2
  ~ credits: rou (a.k.a internetenemy), qfun(a.k.a qfun_g9s)
  ~ special for t.me/wildguild

  ~ build b9dc48c · 2026-08-02 17:42:46 UTC
  ~ auto-generated — do not edit
]]


require("libraries/game_perks/perks/base_game_perk")

cleave = class(base_game_perk)

function cleave:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_ON_DEALDAMAGE_CUSTOM,
	}
end

cleave.vectors = {
	[0] = Vector(1.0, 0.0, 0.0),
	[1] = Vector(-0.8733046400935156, -0.4871745124605095, 0.0),
	[2] = Vector(0.5253219888177297, 0.8509035245341184, 0.0),
	[3] = Vector(-0.04422762066183892, -0.999021480034635, 0.0),
	[4] = Vector(-0.4480736161291701, 0.8939966636005579, 0.0),
	[5] = Vector(0.8268371568000089, -0.562441389066343, 0.0),
	[6] = Vector(-0.9960878351411849, 0.08836868610400143, 0.0),
	[7] = Vector(0.9129390999389944, 0.4080958218391593, 0.0),
	[8] = Vector(-0.5984600690578581, -0.8011526357338304, 0.0),
	[9] = Vector(0.13233681049883225, 0.9912048065798491, 0.0),
	[10] = Vector(0.36731936773024515, -0.9300948780045254, 0.0),
	[11] = Vector(-0.7739002269689113, 0.6333075387972795, 0.0),
	[12] = Vector(0.9843819506325049, -0.1760459464712114, 0.0),
	[13] = Vector(-0.9454304232544339, -0.32582405495135236, 0.0),
	[14] = Vector(0.6669156003948422, 0.7451332645574127, 0.0),
	[15] = Vector(-0.21941055349670321, -0.9756326199006828, 0.0),
}

function cleave:CreateParticleForCleave(particle_name, radius, target, attacker)
	local multiplier = attacker:IsConsideredRangedAttacker() and 1 or -500
	local particle = ParticleManager:CreateParticle(particle_name, PATTACH_ABSORIGIN_FOLLOW, target)
	for i, vector in ipairs(self.vectors) do
		local total_point = multiplier * (target:GetAbsOrigin() - attacker:GetAbsOrigin()):Normalized()
		ParticleManager:SetParticleControl(particle, i, target:GetAbsOrigin() + vector * radius / 2 - total_point)
	end
	ParticleManager:ReleaseParticleIndex(particle)
end

cleave.forbidden_modifiers = {
	"modifier_pangolier_swashbuckle_stunned",
	"modifier_pangolier_swashbuckle",
	"modifier_pangolier_swashbuckle_attack",
	"modifier_monkey_king_boundless_strike_crit",
	"modifier_tidehunter_anchor_smash_caster",
	"modifier_mars_gods_rebuke_crit",
	"modifier_void_spirit_astral_step_caster",
}

function cleave:OnDealDamage(params)
	if not IsServer() then
		return
	end
	if not IsValidEntity(self.parent) then
		return
	end
	if not IsValidEntity(params.target) then
		return
	end
	if not self.parent:IsRealHero() and not self.parent:IsClone() then
		return
	end
	if self.parent:GetTeamNumber() == params.target:GetTeamNumber() then
		return
	end
	if params.target:IsBuilding() then
		return
	end
	if params.target:IsOther() then
		return
	end
	if params.damage_category ~= DOTA_DAMAGE_CATEGORY_ATTACK then
		return
	end
	if not params.process_procs then
		return
	end

	for _, modifier_name in pairs(self.forbidden_modifiers) do
		if self.parent:HasModifier(modifier_name) then
			return
		end
	end

	local is_ranged_hero = self.parent:IsConsideredRangedAttacker()
	local target_loc = params.target:GetAbsOrigin()

	local cleave_percent = (is_ranged_hero and self.range_pct or self.melee_pct) / 100
	local cleave_percent_creep = (is_ranged_hero and self.range_pct_creep or self.melee_pct_creep) / 100

	local damage = params.original_damage * cleave_percent
	local damage_creep = params.original_damage * cleave_percent_creep

	local enemies = FindUnitsInCone(
		self.parent:GetTeamNumber(),
		CalculateDirection(params.target, self.parent),
		target_loc,
		150,
		360,
		self.radius,
		nil,
		DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC,
		DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE,
		FIND_ANY_ORDER,
		false,
		self.parent:IsConsideredRangedAttacker()
	)

	self:CreateParticleForCleave("particles/custom_cleave.vpcf", 100, params.target, self.parent)

	for _, enemy in pairs(enemies) do
		if IsValidEntity(enemy) and enemy ~= params.target then
			ApplyDamage({
				victim = enemy,
				attacker = self.parent,
				damage = enemy:IsCreep() and damage_creep or damage,
				damage_type = DAMAGE_TYPE_PHYSICAL,
				damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL,
			})
		end
	end
end