...
mp_per_kill = class(base_game_perk)

function mp_per_kill:__OnCreated()
...
function mp_per_kill:OnDestroy()
	if not IsServer() then
		return
	end
...
function mp_per_kill:OnUnitKilled(event)
	...
	local mp_restore = self.mana_restore
	if event.killed:IsCreep() then
		mp_restore = self.mana_restore_creep
	end

	self.parent:GiveMana(mp_restore)

	SendOverheadEventMessage(nil, OVERHEAD_ALERT_MANA_ADD, self.parent, mp_restore, nil)
end