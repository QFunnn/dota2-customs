...
function GamePerks:Init()
	...
	EventStream:Listen("game_perks:set_perk", GamePerks.SetGamePerkFromClient, GamePerks)
...
function GamePerks:SetGamePerkFromClient(event)
	event.resetter = false
	GamePerks:SetGamePerk(event)
end
function GamePerks:SetGamePerk(event)
	...
	if IsValidEntity(hero) and hero:IsAlive() then
		self.perks_applied[player_id] = true
		GamePerks:ApplyPerkModifier(player_id, event.resetter)
	end
end

function GamePerks:ApplyPerkModifier(player_id, resetter)
	local perk_name = self.chosen_perks[player_id]
	local perk_tier = self.perk_tiers[player_id]
	local hero = GameLoop.hero_by_player_id[player_id]
	local player = PlayerResource:GetPlayer(player_id)
...
function GamePerks:OnNpcSpawned(event)
	...
	if
		not self.perks_applied[player_id]
		and self.chosen_perks[player_id]
		and GameLoop.hero_by_player_id[player_id] == unit
	then
		GamePerks:ApplyPerkModifier(player_id, false)
	end
