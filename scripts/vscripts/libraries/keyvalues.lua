...
function GetAbilitySpecial(name, key, level, t)
	...
	if t then
		...
		if AbilityValues then
			if key then
				local v = AbilityValues[key]
				if type(v) == "table" then
					return GetValueInStringForLevel(v.value, level)
				elseif type(v) == "string" then
					return GetValueInStringForLevel(v, level)
				end
			else
				local o = {}
				for k, v in pairs(AbilityValues) do
					if type(v) == "table" then
						o[k] = GetValueInStringForLevel(v.value, level)
					elseif type(v) == "string" then
						o[k] = GetValueInStringForLevel(v, level)
					end
				end
				return o
			end
