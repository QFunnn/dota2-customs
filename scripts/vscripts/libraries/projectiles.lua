...
function Projectiles:CreateProjectile(projectile)
	...
	if projectile.bIgnoreSource == nil then
		projectile.bIgnoreSource = true
	end
	if projectile.bMultipleHits == nil then
		projectile.bMultipleHits = false
	end
	if projectile.bRecreateOnChange == nil then
		projectile.bRecreateOnChange = true
	end
	if projectile.bZCheck == nil then
		projectile.bZCheck = true
	end
	projectile.fRehitDelay = projectile.fRehitDelay or 1
	projectile.TreeBehavior = projectile.TreeBehavior or PROJECTILES_DESTROY
	if projectile.bCutTrees == nil then
		projectile.bCutTrees = false
	end
	...
	projectile.fChangeDelay = projectile.fChangeDelay or 0.1
	projectile.UnitTest = projectile.UnitTest or function()
		return false
	end
	projectile.OnUnitHit = projectile.OnUnitHit or function()
		return
	end
	projectile.OnTreeHit = projectile.OnTreeHit or function()
		return
	end
	projectile.OnWallHit = projectile.OnWallHit or function()
		return
	end
	projectile.OnGroundHit = projectile.OnGroundHit or function()
		return
	end
	...
	if projectile.bTreeFullCollision == nil then
		projectile.bTreeFullCollision = false
	end
	projectile.bProvidesVision = projectile.bProvidesVision or false
	if projectile.bFlyingVision == nil then
		projectile.bFlyingVision = true
	end
	...
	projectile.fVisionTickTime = projectile.fVisionTickTime or 0.1
  ...
  elseif projectile.bDynamic == nil then
	...
	if projectile.fRadiusStep then
		...
		projectile.radiusStep = (projectile.fEndRadius - projectile.fStartRadius)
			/ (projectile.fDistance / projectile.vel:Length())
	end
	...
	if projectile.GroundBehavior == PROJECTILES_FOLLOW then
		...
		if ground.z > future.z then
			...
			ParticleManager:SetParticleControl(
				projectile.id,
				projectile.iVelocityCP,
				projectile.vel:Length() * slope * 30
			)
		else
	...
	function projectile:SetVelocity(newVel, newPos)
		if projectile.changes > 0 then
			...
			if projectile.bRecreateOnChange then
				...
				ParticleManager:SetParticleControl(
					projectile.id,
					projectile.iPositionCP,
					newPos or projectile.pos + projectile.vel
				)
				if projectile.ControlPointForwards[1] == nil and projectile.ControlPointOrientations[1] == nil then
					ParticleManager:SetParticleControlForward(
						projectile.id,
						projectile.iPositionCP,
						projectile.vel:Normalized()
					)
				end
	...
	Projectiles:CreateTimer(projectile.ProjectileTimerName, {
		...
		callback = function()
			...
			-- checks
			if
				curTime > projectile.spawnTime + projectile.fExpireTime
				or projectile.distanceTraveled > projectile.fDistance
			then
				ParticleManager:DestroyParticle(projectile.id, false)
			...
			if projectile.draw then
				...
				DebugDrawSphere(pos, color, alpha, radius, true, 0.01)
			end
			...
			for index = 1, tot do
				...
				for i = 1, #ents do
					...
					if
						...
						and v.GetUnitName
						and v:IsAlive()
						and (not projectile.bIgnoreSource or (projectile.bIgnoreSource and v ~= projectile.Source))
						and nozCheck
						and zCheck
					then
				...
				if navConnect then
					if projectile.bCutTrees or projectile.TreeBehavior ~= PROJECTILES_NOTHING then
						local ents =
							GridNav:GetAllTreesAroundPoint(subpos, projectile.radius, projectile.bTreeFullCollision)
						--print('tree hit')
						...
						for i = 1, #ents do
							local tree = ents[i]
							if
								not projectile.bZCheck
								or (
									pos.z < ground.z + 280 + radius - projectile.fGroundOffset
									and pos.z + radius + projectile.fGroundOffset > ground.z
								)
							then
								...
								if projectile.TreeBehavior == PROJECTILES_DESTROY then
									...
									return
								elseif
									projectile.TreeBehavior == PROJECTILES_BOUNCE
									and projectile.changes > 0
									and curTime >= projectile.changeTime
								then
									-- bounce calculation
								end
							end
				...
				if projectile.WallBehavior ~= PROJECTILES_NOTHING and groundConnect then --ground.z > projectile.prevPos.z then
					...
					if normal.z < 0.6 then
						local vec = Vector(
							GridNav:GridPosToWorldCenterX(GridNav:WorldToGridPosX(subpos.x)),
							GridNav:GridPosToWorldCenterY(GridNav:WorldToGridPosY(subpos.y)),
							ground.z
						)
						...
						if projectile.WallBehavior == PROJECTILES_DESTROY then
							...
							return
						elseif
							projectile.WallBehavior == PROJECTILES_BOUNCE
							and projectile.changes > 0
							and curTime >= projectile.changeTime
						then
							-- bounce calculation
				...
				if projectile.GroundBehavior ~= PROJECTILES_NOTHING and groundConnect then
					...
					if projectile.GroundBehavior == PROJECTILES_DESTROY then
						...
						return
					elseif
						projectile.GroundBehavior == PROJECTILES_BOUNCE
						and projectile.changes > 0
						and curTime >= projectile.changeTime
					then
						-- bounce calculation
						...
						break
					elseif
						projectile.GroundBehavior == PROJECTILES_FOLLOW
						and projectile.changes > 0
						and curTime >= projectile.changeTime
					then
						-- follow calculation
			...
			if projectile.bProvidesVision then
				if projectile.currentFrame == projectile.visionTick then
					AddFOWViewer(
						projectile.iVisionTeamNumber,
						projectile.pos,
						projectile.iVisionRadius,
						projectile.fVisionLingerDuration,
						not projectile.bFlyingVision
					)
...
if not Projectiles.timers then
	Projectiles:start()
end