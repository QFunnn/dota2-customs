...
function EarlyConsumables:Init()
	...
	EventStream:Listen("early_consumables:get_state", function(event)
		EarlyConsumables:SendEarlyConsumablesState(event.PlayerID)
	end)
	EventStream:Listen("early_consumables:vote_additional_kl_goal", function(event)
		EarlyConsumables:PlayerVoteAdditionalGoal(event.PlayerID)
	end)
	EventStream:Listen("early_consumables:close_ui", function(event)
		EarlyConsumables:CloseEarlyConsumablesMenu(event.PlayerID)
	end)
end
...
function EarlyConsumables:SendEarlyConsumablesState(player_id)
	...
	if self.early_consumables_closed_ui[player_id] then
		return
	end
...
function EarlyConsumables:PlayerVoteAdditionalGoal(player_id)
	...
	if GameRules:GetDOTATime(false, false) < GAME_DURATION_OPTIONAL_EARLY_CONSUMABLES_TIME then
		if EarlyConsumables:IsPlayerVotedForExtraGoal(player_id) then
			return
		end
