...
function containers_lua_targeting_tree:CastFilterResultTarget(hTarget)
	...
	if
		...
		and (hTarget:GetUnitName() == "npc_dota_sentry_wards" or hTarget:GetUnitName() == "npc_dota_observer_wards")
	then
		return UF_SUCCESS
	end

	local nResult =
		UnitFilter(hTarget, result.targetTeam, result.targetType, result.targetFlags, self:GetCaster():GetTeamNumber())
