...
function DisableHelp.ModifierGainedFilter(filterTable)
	...
	if disabledModifiersNotInParty[filterTable.name_const] then
		...
		if
			...
			and PlayerResource:GetPartyID(parent:GetPlayerOwnerID())
				~= PlayerResource:GetPartyID(caster:GetPlayerOwnerID())
		then
			ability:EndCooldown()
			ability:RefundManaCost()
			DisplayError(caster:GetPlayerOwnerID(), "dota_hud_error_target_has_disable_help")
...
function DisableHelp.ExecuteOrderFilter(orderType, ability, target, unit, orderVector, units)
	...
	if ability:GetAbilityName() == "furion_sprout_custom" then
		if
			...
			and target ~= unit
			and target:GetTeam() == unit:GetTeam()
		then
	...
	elseif
		orderType == DOTA_UNIT_ORDER_CAST_TARGET
		and target
		and disabledAbilities[ability:GetAbilityName()]
		and target:GetTeam() == unit:GetTeam()
		and PlayerResource:IsDisableHelpSetForPlayerID(target:GetPlayerOwnerID(), unit:GetPlayerOwnerID())
	then
