...
function DisplayErrorWithValues(player_id, base_message, values)
	local player = PlayerResource:GetPlayer(player_id)
	if not IsValidEntity(player) then
		return
	end
...
function GetCreepAbilityLevel()
	...
	for _, level_data in ipairs_rev(creep_ability_level_from_time) do
		if current_time >= level_data[1] then
			return level_data[2]
		end
	end
...
function GetIllusionSource(illusion)
	...
	local _, hero = table.find_element(GameLoop.hero_by_player_id, function(t, k, v)
		return v:GetUnitName() == unit_name
	end)
	return hero
end

--- Checks if date_2 is after date_1
---@param date_1 any
---@param date_2 any
function DateOlder(date_1, date_2)
	local m1, d1, y1 = unpack(string.split(date_1, "/"))
	local m2, d2, y2 = unpack(string.split(date_2, "/"))
...
function IsValidTeamNumber(team)
...
function GetActivePlayerCountForTeam(team)
	...
	for x = 0, DOTA_MAX_TEAM do
		local player_id = PlayerResource:GetNthPlayerIDOnTeam(team, x)
		if
			IsValidPlayerID(player_id)
			and (
				PlayerResource:GetConnectionState(player_id) == DOTA_CONNECTION_STATE_NOT_YET_CONNECTED
				or PlayerResource:GetConnectionState(player_id) == DOTA_CONNECTION_STATE_CONNECTED
			)
		then
			number = number + 1
		end
