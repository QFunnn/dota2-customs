...
function CustomPings:Init()
	...
	EventStream:Listen("custom_ping:ping", self.Ping, self)
end
...
function CustomPings:Ping(data)
	...
	if PING_PARTICLES[ping_type] then
		local ping_particle =
			ParticleManager:CreateParticleForTeam(PING_PARTICLES[ping_type], PATTACH_CUSTOMORIGIN, nil, team)
...
function CustomPings:SetColor(player_id, table_rgb_color)
	if not player_id or not table_rgb_color then
		return
	end
