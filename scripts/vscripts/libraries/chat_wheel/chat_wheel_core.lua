...
function ChatWheel:UpdateCollectionPhrases(event)
	...
	local non_filtered_favs = WebSettings:GetSettingValue(player_id, "chatWheelFavourites", {})
	...
	if not table.deep_compare(non_filtered_favs, filtered_favs) then
		WebSettings:SetSettingValue(player_id, "chatWheelFavourites", filtered_favs)
	else
	...
	CustomGameEventManager:Send_ServerToPlayer(
		player,
		"chat_wheel:update_phrases_for_collection",
...
function ChatWheel:UpdateClientRings(event)
	...
	if not hero_name or not CHAT_WHEEL_HEROES[hero_name] then
		return
	end
	...
	for _p_id = 0, 24 do
		...
		if _hero_name then
			selected_heroes[_hero_name] = true
		end
	end
...
function ChatWheel:IsCooldownForPlayer(player, player_id)
	...
	if GameRules:GetGameTime() - self.timer[player_id] > 5 + self.counter[player_id] then
		...
		local remaining_cd =
			string.format("%.1f", 5 + self.counter[player_id] - (GameRules:GetGameTime() - self.timer[player_id]))
		DisplayErrorWithValues(player_id, "#wheel_cooldown", { sec = remaining_cd })
		return true
	end
end

function ChatWheel:TriggerDefaultPhraseLine(player_id, phrase_key)
	if not phrase_key or not CHAT_WHEEL_SOUNDS_DICTIONARY[phrase_key] then
		return
	end
...
function ChatWheel:Say(event)
	...
	if ChatWheel:IsCooldownForPlayer(player, player_id) then
		return
	end
...
function ChatWheel:UpdateFavorites(event)
	...
	WebSettings:SetSettingValue(player_id, "chatWheelFavourites", valided_favorites)
end
...
function ChatWheel:SetMutedPlayers(event)
	...
	if not event.players or not event.type then
		return
	end
	if not self.mute_state or not self.mute_state[event.type] then
		return
	end

	for target_player_id, is_voice_muted in pairs(event.players or {}) do
		self.mute_state[event.type][player_id][target_player_id] = is_voice_muted == 1
	end
end
