...
function GameMode:SpawnOrbDrop(spawn_point, orb_type, should_launch, on_destroyed_callback)
	...
	capture_point:AddNewModifier(
		capture_point,
		nil,
		"capture_point_area",
		{ orb_type = orb_type, should_launch = should_launch }
	)
	capture_point.on_destroyed_callback = on_destroyed_callback

	local particle_name = ORB_PARTICLE_NAME[orb_type]

	if SeasonalEvents:IsChristmas() then
		particle_name = ORB_PARTICLE_NAME_CHRISTMAS[orb_type]
	end
