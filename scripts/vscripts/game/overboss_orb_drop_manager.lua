...
function OrbDropManager:Init()
	GameMode.do_double_orb_drops = GetMapName() == "ot3_necropolis_ffa"
...
function OrbDropManager:OnGameStart()
	...
	for _, building in pairs(units) do
		if building.GetUnitName and building:GetUnitName() == "npc_dota_goodguys_tower1_bot" then
			...
			OrbDropManager.team_ring_origins[team_number] = OrbDropManager.overboss_position
				+ building:GetAbsOrigin():Normalized() * OrbDropManager.ring_radius * 3
	...
	Timers:CreateTimer(10, function()
		if not OrbDropManager.overboss then
			OrbDropManager.overboss = Entities:FindByName(nil, "@overboss")
		end
		OrbDropManager:TryOrbThrow(OrbDropManager.overboss)
		return 1
	end)
end
...
function OrbDropManager:TryOrbThrow(overboss)
	...
	local raw_multiplier = OVERBOSS_ORB_THROW_MULTIPLIER_PCT
		+ OVERBOSS_ORB_THROW_MULTIPLIER_PCT_CHANGE_PER_MINUTE * minutes

	local chance_multiplier = math.max(
		math.min(raw_multiplier, OVERBOSS_ORB_THROW_MULTIPLIER_MAX_CAP),
		OVERBOSS_ORB_THROW_MULTIPLIER_MIN_CAP
	) / 100
...
function OrbDropManager:OverbossSelectRarity()
	...
	local epic_threshold = math.max(
		math.min(
			OVERBOSS_ORB_THROW_EPIC_PCT + OVERBOSS_ORB_THROW_EPIC_PCT_CHANGE_PER_MINUTE * minutes,
			OVERBOSS_ORB_THROW_EPIC_PCT_MAX_CAP
		),
		0
	)
	local rare_threshold = epic_threshold
		+ math.max(
			math.min(
				OVERBOSS_ORB_THROW_RARE_PCT + OVERBOSS_ORB_THROW_RARE_PCT_CHANGE_PER_MINUTE * minutes,
				OVERBOSS_ORB_THROW_RARE_PCT_MAX_CAP
			),
			0
		)
...
function OrbDropManager:GenerateSectors()
	...
	for i = 1, OrbDropManager.sector_count do
		OrbDropManager.sector_weights[i] = GameLoop.current_layout.starting_drop_weights[i] or 0
...
function OrbDropManager:GenerateRandomInSector(sector_id, rarity)
	...
	local random_radius = math.max(
		OrbDropManager.current_radius - OrbDropManager.capture_point_radius - RING_RADIUS_MINIMUM,
		RING_RADIUS_MINIMUM * 0.5
	)
...
function OrbDropManager:SelectRingOrbDropLocation(rarity)
	...
	OrbDropManager.current_radius =
		math.min(OrbDropManager.ring_radius, RING_RADIUS_MINIMUM + OrbDropManager.orbs_dropped * RING_RADIUS_PER_ORB)
	...
	local applicable_sectors = table.filter(OrbDropManager.sector_weights, function(k, v, t)
		return v == min_value
	end)
...
function OrbDropManager:SelectEpicOrbDropLocation(orb_spawn_entities, last_entities)
	...
	if table.count(OrbDropManager.epic_orb_weights) <= 0 then
		...
		OrbDropManager.epic_orb_weights[location_index] = OrbDropManager.epic_orb_weights[location_index]
			+ EPIC_ORB_WEIGHT
