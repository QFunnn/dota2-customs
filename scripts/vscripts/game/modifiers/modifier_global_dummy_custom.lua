...
function modifier_global_dummy_custom:OnTakeDamage(event)
	...
	if not target or target:IsNull() or not target:IsRealHero() or target:IsIllusion() then
		return
	end

	local target_id = target.GetPlayerOwnerID and target:GetPlayerOwnerID()
	if not target_id or not EndGameStats.stats[target_id] then
		return
	end

	local attacker = event.attacker

	if attacker and not attacker:IsNull() and attacker.IsTower and attacker:IsTower() then
		return
	end
