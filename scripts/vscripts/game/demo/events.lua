...
function OT3Demo:OnEntityKilled(event)
	...
	if not killed or not killed:IsRealHero() or killed:IsClone() then
		return
	end
...
function OT3Demo:OnAbilityUsed(event)
	if not self.free_spells_enabled then
		return
	end
...
function OT3Demo:OnFreeSpellsButtonPressed()
	...
	if self.free_spells_enabled then
		self:RefreshPlayers()
	end
end
...
function OT3Demo:OnSpawnEnemyButtonPressed(event)
	...
	local facet_id = event.facet_id or 1

	local function on_hero_precached()
		DebugCreateHeroWithVariant(
			player,
			OT3Demo.hero_to_spawn,
			facet_id,
			event.enemy_team_id,
			true,
			function(new_hero)
				new_hero:SetControllableByPlayer(event.PlayerID, false)
				new_hero:SetRespawnPosition(hero:GetAbsOrigin())
				FindClearSpaceForUnit(new_hero, hero:GetAbsOrigin(), false)
				new_hero:Hold()
				new_hero:SetIdleAcquire(false)
				new_hero:SetAcquisitionRange(0)
			end
		)
	end
	-- will this help? idfk
	PrecacheUnitByNameAsync(OT3Demo.hero_to_spawn, on_hero_precached, event.PlayerID)
end
...
function OT3Demo:OnRemoveSelectedPressed(event)
	...
	if
		IsValidEntity(unit)
		and (not IsValidPlayerID(owner_player_id) or unit ~= PlayerResource:GetSelectedHeroEntity(event.PlayerID))
	then
		unit:Destroy()
	end
end
...
function OT3Demo:OnSpawnAllyButtonPressed(event)
	...
	DebugCreateHeroWithVariant(player, OT3Demo.hero_to_spawn, facet_id, hero:GetTeam(), false, function(new_hero)
		new_hero:SetControllableByPlayer(event.PlayerID, false)
		new_hero:SetRespawnPosition(hero:GetAbsOrigin())
		FindClearSpaceForUnit(new_hero, hero:GetAbsOrigin(), false)
		new_hero:Hold()
...
function OT3Demo:OnSpawnRune(event)
	...
	if not IsValidEntity(hero) then
		print("invalid hero in SpawnRuneInFrontOfUnit", player_id, hero)
		return
	end
...
function OT3Demo:OnLeaveButtonPressed(event)
	local player_id = event.PlayerID
...
function OT3Demo:OnCreepsToggled(event)
	...
	if not self.creeps_enabled then
		local flags = DOTA_UNIT_TARGET_FLAG_DEAD
		...
		for _, creep in pairs(creeps or {}) do
			if
				IsValidEntity(creep)
				and creep:IsCreep()
				and not creep:IsControllableByAnyPlayer()
				and string.starts(creep:GetUnitName(), "npc_dota_creep_")
			then
				creep:Destroy()
			end
		end
