...
function CDOTA_BaseNPC:CheckPersonalCooldown(item)
	...
	if not itemsCooldownForPlayer[itemName] or item.isTransfer or not item:CheckMaxItemsForPlayer(unique_key) then
		return true
	end

	local playerCanBuyItem = lastTimeBuyItemWithCooldown[unique_key] == nil
		or ((GameRules:GetGameTime() - lastTimeBuyItemWithCooldown[unique_key]) >= itemsCooldownForPlayer[itemName])
...
function CDOTA_BaseNPC:IsMaxItemsForPlayer(item)
	...
	local unique_key = itemName .. "_" .. buyerEntIndex
	local playerID = self:GetPlayerID()
	if not maxItemsForPlayers[itemName] or item.isTransfer then
		return true
	end
...
function CDOTA_Item:TransferToBuyer(unit)
	...
	if self:IsTrueNeutralDrop() then
		return true
	end
	...
	if not buyer:DoesHeroHasFreeSlot() then
		CustomGameEventManager:Send_ServerToPlayer(
			PlayerResource:GetPlayer(buyer:GetPlayerID()),
			"display_custom_error",
			{ message = "#dota_hud_error_cant_purchase_inventory_full" }
		)
		return false
	end
...
function CDOTA_Item:CheckMaxItemsForPlayer(unique_key)
	if not maxItemsForPlayers[self:GetAbilityName()] then
		return true
	end
...
function CreateDummyInventoryForPlayer(playerId, unit)
	...
	local dInventory =
		CreateUnitByName("npc_dummy_inventory", startPointSpawn, true, unit, unit, PlayerResource:GetTeam(playerId))
...
-------------------------------------------------------------------------
function CDOTA_Item:IsTrueNeutralDrop()
	return self:IsActiveNeutral() or string.starts(self:GetAbilityName(), "item_enhancement_")
end