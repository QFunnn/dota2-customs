...
function SimulatedEndGame:SendStateEvent(event)
	if not GameLoop.game_over then
		return
	end
...
function SimulatedEndGame:GetErrors()
	...
	if table.reduce(GameLoop.current_kills_count, function(acc, kill_count)
		return acc + kill_count
	end, 0) <= 0 then
...
function SimulatedEndGame:_EndWithWinner(team_id)
	...
	for _, entity in pairs(entities or {}) do
		if
			...
			and entity.AddNewModifier
			and entity.GetUnitName
			and entity:GetUnitName() ~= "npc_dota_thinker"
		then
