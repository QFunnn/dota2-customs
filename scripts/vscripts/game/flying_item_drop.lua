...
function FlyingTreasureDrop:InitOrbDropLocationList()
	...
	for i = 1, 20 do
		local drop_location = Entities:FindByName(nil, "item_spawn_" .. i)
		if drop_location then
			table.insert(self.orb_entities, drop_location)
		end
	end
end
...
function FlyingTreasureDrop:ThinkSpecialItemDrop()
	...
	if GetMapName() == "ot3_desert_octet" then
		return
	end
...
function FlyingTreasureDrop:RollOrbLocations()
	...
	-- print("requested orbs count:", orbs_per_launch)
	...
	for i = 1, orbs_per_launch do
		...
		if i % 2 == 0 then
			...
			if GetMapName() == "ot3_necropolis_ffa" then
			...
			-- for other maps with uneven spawn count, just select any other spawn point
				...
				local _, randomed_index = table.random_with_condition(self.orb_entities, function(t, k, v)
					return k ~= previous_index
				end)
			...
			-- print("selected expected index: ", expected_index)
		end
...
function FlyingTreasureDrop:StageOrbLaunches()
	...
	for i, target_index in pairs(self.staged_orbs) do
		...
		local vision_revealer =
			CreateUnitByName("npc_vision_revealer", spawn_location, false, nil, nil, DOTA_TEAM_GOODGUYS)
		vision_revealer:AddNewModifier(vision_revealer, nil, "modifier_no_collision", { duration = -1 })

		local true_sight_particle = ParticleManager:CreateParticle(
...
function FlyingTreasureDrop:LaunchStagedOrbs()
	...
	for i, target_index in pairs(self.staged_orbs) do
		...
		local treasure_courier =
			CreateUnitByName("npc_dota_treasure_courier", Vector(0, 0, 700), true, nil, nil, DOTA_TEAM_NEUTRALS)
...
function FlyingTreasureDrop:LaunchStagedOctet()
	...
	for i, target_index in pairs(self.staged_orbs) do
		...
		if table.count(self.current_vision_revealers[spawn_id]) == 0 then
			self.current_vision_revealers[spawn_id] = nil
		end
...
function FlyingTreasureDrop:TreasureDrop(treasure_courier)
	...
	local death_particle =
		ParticleManager:CreateParticle("particles/treasure_courier_death.vpcf", PATTACH_CUSTOMORIGIN, nil)
	ParticleManager:SetParticleControl(death_particle, 0, spawn_point)
	ParticleManager:SetParticleControlOrientation(
		death_particle,
		0,
		treasure_courier:GetForwardVector(),
		treasure_courier:GetRightVector(),
		treasure_courier:GetUpVector()
	)
	...
	if table.count(self.current_vision_revealers[treasure_courier.spawn_id]) == 0 then
		self.current_vision_revealers[treasure_courier.spawn_id] = nil
	end
	...
	capture_point:SetOrigin(GetGroundPosition(spawn_point, capture_point))
...
function FlyingTreasureDrop:KnockBackFromTreasure(
	center,
	radius,
	knockback_duration,
	knockback_distance,
...
function FlyingTreasureDrop:CreatePathParticles(spawn_id, target_index, point)
	...
	for player_id, _ in pairs(GameLoop.hero_by_player_id) do
		...
		if IsValidEntity(player) then
			local p_id = ParticleManager:CreateParticleForPlayer(
			...
			ParticleManager:SetParticleControl(
				p_id,
				1,
				IsValidEntity(attachment_entity) and attachment_entity:GetAbsOrigin() or Vector(0, 0, 0)
			)
...
function FlyingTreasureDrop:DestroyPathParticles(spawn_id, target_index)
	if not self.active_path_particles[spawn_id] or not self.active_path_particles[spawn_id][target_index] then
		return
	end
	...
	if table.count(self.active_path_particles) <= 0 then
		self.active_path_particles[spawn_id] = nil
	end
end

function FlyingTreasureDrop:OnPlayerSettingsChanged(event)
	-- toggle active path particles
	if event.setting_name ~= "disable_epic_path" then
		print("discarded changed settings", event.setting_name)
		return
	end
