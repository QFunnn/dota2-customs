...
function HeroChallenges:Init()
	...
	EventStream:Listen("HeroChallenges:ping_progress", HeroChallenges.PingProgress, HeroChallenges)
...
function HeroChallenges:PreparePoolFromWinrates(winrates)
	...
	table.sort(winrates, function(a, b)
		return a[2] > b[2]
	end)
...
function HeroChallenges:ProcessPlayerChallenges(player_id, challenges)
	...
	for index, challenge in pairs(challenges or {}) do
		...
		if not validate_status then
			DebugMessage("INVALID CHALLENGE", challenge.hero_id, challenge.challenge_type)
		end
	end
...
function HeroChallenges:_RollChallengeTypeForHero(hero_name, present_challenges)
	...
	if not challenges_definitions then
		error("[Hero Challenges] missing challenge definition for " .. hero_name)
	end
	...
	if not unrolled_challenge_types or #unrolled_challenge_types <= 0 then
		unrolled_challenge_types = challenges_definitions.types
	end
...
function HeroChallenges:GetTarget(hero_name, challenge_type, difficulty)
	...
	-- override difficulty multiplier for challenges relating to hero kills
	if challenge_type == CHALLENGE_TYPE.KILL or challenge_type == CHALLENGE_TYPE.ASSIST then
		difficulty_multiplier = CHALLENGE_DIFFICULTY_MULTIPLIER_KILLS_OVERRIDE[difficulty] or difficulty_multiplier
	end
	...
	if challenges_definitions.types_override then
		...
		if challenge_override then
			if challenge_override.target then
				base_target = challenge_override.target
			end
			if challenge_override.target_bias then
				target_bias = challenge_override.target_bias
			end
		end
	end
...
function HeroChallenges:GetReward(hero_name, difficulty)
	...
	base_reward.currency = math.ceil(base_reward.currency * multiplier / CURRENCY_REWARD_BOUNDARY)
		* CURRENCY_REWARD_BOUNDARY
...
function HeroChallenges:ValidateChallenge(challenge)
	if not challenge.challenge_type or not challenge.hero_id then
		return false
	end
	...
	if not CHALLENGE_TYPE_LOOKUP[challenge.challenge_type] then
		return false
	end

	-- validate difficulty
	if not CHALLENGE_DIFFICULTY_LOOKUP[challenge.difficulty] then
		return false
	end

	-- validate if challenge type compatible with hero
	local challenges_definition = HERO_CHALLENGE_TYPES[hero_name]
	if not challenges_definition or not table.contains(challenges_definition.types, challenge.challenge_type) then
		return false
	end
...
function HeroChallenges:BuildHeroChallenge(challenge)
	...
	new_challenge.target =
		HeroChallenges:GetTarget(new_challenge.hero_name, challenge.challenge_type, challenge.difficulty)
...
function HeroChallenges:SubmitChallengesToBackend()
	...
	if table.count(new_challenges) <= 0 then
		return
	end
	...
	WebApi:Send("api/lua/match/add_hero_challenges", {
	...
	for player_id, pending_player_challenges in pairs(HeroChallenges.pending_challenges) do
		...
		for index, challenge in pairs(pending_player_challenges) do
			for _, backend_challenge in pairs(data.new_challenges or {}) do
				if
					steam_id == backend_challenge.steam_id
					and challenge.hero_name == GetHeroNameByID(backend_challenge.hero_id)
					and challenge.challenge_type == backend_challenge.challenge_type
					and challenge.difficulty == backend_challenge.difficulty
				then
...
function HeroChallenges:OnHeroInitFinished(event)
	...
	if not IsValidEntity(hero) or not IsValidPlayerID(player_id) then
		return
	end
	...
	print(
		"[Hero Challenges] selected hero is not present in challenges or challenge is already completed",
		player_id,
		selected_hero_name
	)
end
...
function HeroChallenges:PingProgress(event)
	local player_id = event.PlayerID
	if not IsValidPlayerID(player_id) then
		return
	end
	...
	local to_target = active_challenge.target - math.floor(active_challenge.progress)

	if to_target > 0 then
		CustomChat:MessageToTeam(player_id, PlayerResource:GetTeam(player_id), "hero_challenges_ping_progress", {
			not_localize = {
				value = to_target,
			},
		}, nil, { challenge_type = active_challenge.challenge_type })
	else
		CustomChat:MessageToTeam(
			player_id,
			PlayerResource:GetTeam(player_id),
			"hero_challenges_ping_progress_completed"
		)
	end
end
