...
function GameLoop:OnUnitKilled(event)
	...
	if not killed or not event.killer then
		return
	end
	if not killed:IsRealHero() then
		return
	end
	if killed:IsReincarnating() then
		return
	end
	if not GameLoop.heroes_by_team[event.killer:GetTeam()] then
		return
	end
	...
	if killer_team == killed:GetTeam() then
		return
	end
	...
	local team_gold_reward = LEADER_KILL_GOLD_REWARD_PER_DIFFERENCE
		* math.floor(kill_difference / LEADER_KILLS_TO_DIFFERENCE)
	...
	if current_kill_leader and GameLoop.overtime then
...
function GameLoop:UpdateLeaderOverthrowStatus(kill_leader_team)
	...
	if not kill_leader_team or not runner_up_team then
		return
	end
...
function GameLoop:InitHero(hero)
	...
	GameLoop.rare_upgrades_requirement[team] = GameLoop.rare_upgrades_requirement[team]
		or GameLoop.current_layout.rare_upgrade_basic_requirement
	...
	if
		not IsInToolsMode()
		and GameRules:State_Get() < DOTA_GAMERULES_STATE_GAME_IN_PROGRESS
		and GetMapName() ~= "ot3_demo"
	then
...
function GameLoop:InitFOWRevealers()
	...
	for _, team in pairs(GameLoop.current_layout.teamlist) do
		AddFOWViewer(
			team,
			overboss:GetAbsOrigin() + Vector(0, 0, 1000),
			GameLoop.current_layout.center_vision_reveal_radius,
			99999,
			false
		)
	end
...
function GameLoop:DecreaseScoreByPlayerDisconnect(player_id)
	...
	local reduction =
		math.ceil(self.target_kill_goal / (self.current_layout.player_count * #self.current_layout.teamlist))
...
function GameLoop:TransferLeadership(new_kill_leader_team)
	if GameLoop.kill_leader_team == new_kill_leader_team then
		return
	end
	...
	if not new_kill_leader_team then
		return
	end
	...
	if GameLoop._leader_overthrow_candidate and GameLoop:IsOrbSpreeAllowed() then
		...
		-- particle
		local particle =
			ParticleManager:CreateParticle("particles/orb_spree/orb_spree_shockwave.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		if IsValidEntity(hero) then
			...
			if not added_modifier or added_modifier:IsNull() then
				Timers:CreateTimer(0.5, function()
					...
					if GameLoop.kill_leader_team ~= hero:GetTeam() then
						return
					end

					local reapplied_timer = hero:AddNewModifier(hero, nil, "modifier_kill_leader", { duration = -1 })
					if not reapplied_timer or reapplied_timer:IsNull() then
						return 0.5
					end
				end)
			end
...
function GameLoop:UpdateTeamsOrder()
	...
	table.sort(sorted_team_ids, function(a, b)
		return a[2] > b[2]
	end)
...
function GameLoop:GetCommonUpgradesRate(team, place)
	...
	local modifier_names = {
		"modifier_elder_titan_tip_the_scales_lua",
	}

	for _, hero in pairs(GameLoop.heroes_by_team[team] or {}) do
		if IsValidEntity(hero) then
			teamwork_enhancers = teamwork_enhancers
				+ WebInventory:GetItemCount(hero:GetPlayerID(), "bp_teamwork_enhancer")
			...
			if hero:IsAlive() and not hero:PassivesDisabled() then
				for _, modifier_name in ipairs(modifier_names) do
					...
					if modifier and modifier.common_orb_generation_rate_pct then
						modifier_enhancers = modifier_enhancers + modifier.common_orb_generation_rate_pct
					end
				end
			end
		end
...
function GameLoop:CommonUpgradesTick()
	...
	for team, place in pairs(GameLoop.current_kill_order) do
		...
		if GameLoop.common_upgrades_progress[team] >= COMMON_UPGRADES_REQUIREMENT then
			GameLoop.common_upgrades_progress[team] = GameLoop.common_upgrades_progress[team]
				- COMMON_UPGRADES_REQUIREMENT
...
function GameLoop:ProcessRareUpgradeProgress(team)
	...
	if GameLoop.rare_upgrades_progress[team] >= GameLoop.rare_upgrades_requirement[team] then
		...
		if GameLoop.rare_upgrades_filled_times[team] >= GameLoop.current_layout.rare_upgrade_requirement_step then
			GameLoop.rare_upgrades_requirement[team] = GameLoop.rare_upgrades_requirement[team]
				+ GameLoop.current_layout.rare_upgrade_requirement_increment
		end
	end
