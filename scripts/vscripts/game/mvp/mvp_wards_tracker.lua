		...
		for delay, hero in pairs(GameLoop.hero_by_player_id) do
			...
			if
				...
				and not (
					...
					or hero:HasModifier("modifier_smoke_of_deceit")				-- or hero:HasModifier("modifier_phantom_assassin_blur_active")

				)
			then
				for _, sentry in pairs(wards.sentries) do
					MVPController:AddWardsRevealTimeScore(
						sentry:GetPlayerOwnerID(),
						MVP_SENTRY_WARD_MULTIPLIER * timer_delay
					)
				end
			end
		end
