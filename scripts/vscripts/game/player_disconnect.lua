...
function PlayerDC:ProcessPlayer(player_id)
	...
	if not PlayerResource:IsBotOrPlayerConnected(player_id) then
		...
		if
			PlayerDC.disconnect_timer[player_id] >= PlayerDC.abandon_time
			or PlayerResource:HasPlayerAbandoned(player_id)
		then
			if not PlayerDC.has_abandoned[player_id] then
				...
				if not PlayerDC.has_decreased_score[player_id] then
					...
					GameLoop:DecreaseScoreByPlayerDisconnect(
						player_id,
						GameLoop.current_layout.abandon_kill_goal_reduction
					)
				end
...
function PlayerDC:GetTeamConnectedPlayerCount(team_number)
	...
	for i = 0, 23 do
		if
			...
			and PlayerResource:IsBotOrPlayerConnected(i)
			and not PlayerDC.has_abandoned[i]
			and PlayerResource:GetTeam(i) == team_number
		then
			counter = counter + 1
		end
	end
