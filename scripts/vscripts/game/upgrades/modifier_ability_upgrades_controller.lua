...
function modifier_ability_upgrades_controller:IsHidden()
	return true
end
function modifier_ability_upgrades_controller:IsPurgable()
	return false
end
function modifier_ability_upgrades_controller:RemoveOnDeath()
	return false
end
function modifier_ability_upgrades_controller:IsPermanent()
	return true
end
...
function modifier_ability_upgrades_controller:OnRefresh()
	...
	if not IsValidEntity(owner_hero) then
		return
	end
...
function modifier_ability_upgrades_controller:GetModifierOverrideAbilitySpecial(params)
	...
	if not self.parent.upgrades[ability_name] or not self.parent.upgrades[ability_name][special_value] then
		return 0
	end
...
function modifier_ability_upgrades_controller:GetModifierOverrideAbilitySpecialValue(params)
	...
	if string.find(params.ability_special_value, "scepter") and not self:GetParent():HasScepter() then
		return
	end
	...
	if not self.parent.upgrades[ability_name] or not self.parent.upgrades[ability_name][special_value_name] then
		return base_value
	end

	if
		self.cache[ability_name]
		and self.cache[ability_name][special_value_name]
		and self.cache[ability_name][special_value_name][special_value_level]
	then
	...
	if upgrade_data.requires_scepter and not self.parent:HasScepter() then
		return base_value
	end
	if upgrade_data.requires_shard and not self.parent:HasShard() then
		return base_value
	end
...
function modifier_ability_upgrades_controller:GetModifierPercentageCooldown(params)
	...
	if not self.parent.upgrades[ability_name] or not self.parent.upgrades[ability_name].cooldown_and_manacost then
		return 0
	end
	...
	local added_value =
		UpgradesUtilities:CalculateUpgradeValue(self.parent, upgrade_data.value, upgrade_data.count, upgrade_data)
...
function modifier_ability_upgrades_controller:GetModifierSpellAmplify_Percentage(params)
	...
	if self.spell_amp_lock then
		return 0
	end

	local attacker = self:GetParent()
	local ability = params.inflictor
	if not IsValidEntity(ability) or not IsValidEntity(attacker) then
		return 0
	end
	...
	if not ability_upgrades or not ability_upgrades.damage then
		return 0
	end

	-- discard damage instances that come not from AbilityDamage
	if
		params.original_damage ~= ability:GetAbilityDamage()
		and not SPELL_AMP_FILTER_EXCEPTIONS[ability_name]
		and not SPELL_AMP_FILTER_USE_PASSED_DAMAGE[ability_name]
	then
		return 0
	end
	...
	if ability_damage_kv and (not ability_damage_special or ability_damage_special == 0) then
		...
		if base_damage and upgrade_count then
			...
			local upgrade_value = UpgradesUtilities:CalculateUpgradeValue(
				attacker,
				base_damage,
				upgrade_count,
				damage_upgrade,
