...
function Upgrades:Init()
	...
	Upgrades.favourite_builds = {}

	Upgrades.disabled_upgrades_per_player = {}

	Upgrades.lucky_trinket_proc = {}

	EventStream:Listen("Upgrades:dev:load_upgrades", function(event)
		Upgrades:LoadUpgradesData(event.hero_name)
	end)
	EventStream:Listen("Upgrades:dev:request_upgrades", function(event)
		Upgrades:SendUpgradesData(event.PlayerID)
	end)
	EventStream:Listen("Upgrades:dev:add_upgrade", function(event)
		Upgrades:AddToolsUpgrade(event)
	end)
	EventStream:Listen("Upgrades:dev:add_generic_upgrade", function(event)
		Upgrades:AddToolsGenericUpgrade(event)
	end)
	EventStream:Listen("Upgrades:get_debug_localization_check", function(event)
		Upgrades:SendDebugUpgrades(event.PlayerID)
	end)
	...
	EventStream:Listen("Upgrades:get_available_upgrades", function(event)
		Upgrades:SendUpgradesData(event.PlayerID)
	end)
	...
	EventStream:Listen("FavoriteBuilds:apply_build", Upgrades.ApplySavedBuildEvent, Upgrades)
	EventStream:Listen("FavoriteBuilds:save_build", Upgrades.SaveBuildEvent, Upgrades)
	EventStream:Listen("FavoriteBuilds:clear_build", Upgrades.ClearBuildEvent, Upgrades)
	EventStream:Listen("FavoriteBuilds:set_build_name", Upgrades.RenameBuildEvent, Upgrades)
...
function Upgrades:GetPendingUpgradesCount(player_id)
	if not Upgrades.queued_selection or not Upgrades.queued_selection[player_id] then
		return 0
	end
...
function Upgrades:SendPendingFavorites(event)
	-- print("sending pending favorites and builds")
	...
	-- DeepPrintTable(Upgrades.favorites_upgrades[player_id] or {})
	...
	CustomGameEventManager:Send_ServerToPlayer(player, "Upgrades:set_client_favorites", {
		current_favorites = Upgrades.favorites_upgrades[player_id] or {},
		builds = Upgrades.favourite_builds[player_id],
	})
end

function Upgrades:SetFavorites(event)
	if not event.favorites_upgrades then
		return
	end
...
function Upgrades:SendDebugUpgrades(player_id)
	if not player_id or not IsInToolsMode() or GetMapName() ~= "ot3_demo" then
		return
	end
...
function Upgrades:QueueSelection(hero, rarity)
	...
	if Upgrades.lucky_trinket_proc[player_id] then
		return
	end
...
function Upgrades:ShowSelection(hero, rarity, player_id, is_reroll, is_lucky_trinket_proc)
	...
	for _, upgrade_type in pairs({ UPGRADE_TYPE.ABILITY, UPGRADE_TYPE.GENERIC }) do
		...
		if use_generic_from_subscription then
			count_per_selection = upgrade_type == UPGRADE_TYPE.GENERIC and count_per_selection + 1
				or count_per_selection - 1
		end
...
function Upgrades:RollUpgradesOfType(upgrade_type, player_id, rarity, previous_choices, count)
	...
	local upgrades = table.random_some_with_condition(pool, count, function(t, index, upgrade_data)
		...
		if previous_choices and excluded_by_name[upgrade_name .. "_" .. upgrade_data.ability_name] then
			return false
		end
		-- min rarity for ability upgrades
		if upgrade_data.min_rarity and upgrade_data.min_rarity > rarity then
			return false
		end
		-- strict rarity for generics
		if upgrade_data.rarity and upgrade_data.rarity ~= rarity then
			return false
		end
		if upgrade_data.disabled and upgrade_data.disabled == 1 then
			return false
		end
		if upgrade_data.disabled_for_universal and upgrade_data.disabled_for_universal == 1 and is_hero_universal then
			return false
		end
		if disabled_upgrades[upgrade_name] then
			return false
		end

		-- melee/ranged-only upgrades
		if upgrade_data.attack_capability and upgrade_data.attack_capability ~= hero_attack_capability then
			return false
		end
		...
		if upgrade_data.max_count and not upgrade_data.rarity and upgrade_data.max_count < rarity then
			return
		end
		...
		if upgrade_data.max_count and ability_upgrades[upgrade_data.upgrade_name] then
			...
			if current_count + (rarity / (upgrade_data.rarity or 1)) > upgrade_data.max_count then
				return false
			end
		end
	...
	if upgrade_type == UPGRADE_TYPE.GENERIC then
		for index, upgrade in pairs(upgrades or {}) do
			...
			new_upgrade.count = hero.upgrades.generic
					and hero.upgrades.generic[upgrade.upgrade_name]
					and hero.upgrades.generic[upgrade.upgrade_name].count
				or 0
...
function Upgrades:UpgradeSelected(event)
	...
	if not index or not upgrade_data then
		print("failed to find selected upgrade")
		return
	end
	...
	if
		index == 3
		and (
			(rarity == UPGRADE_RARITY_COMMON and subscription_tier < 1)
			or (rarity > UPGRADE_RARITY_COMMON and subscription_tier < 2)
		)
	then
	...
	if upgrade_data.type == UPGRADE_TYPE.ABILITY then
		Upgrades:AddAbilityUpgrade(hero, upgrade_data.ability_name, upgrade_data.upgrade_name, rarity)
	else
	...
	if #Upgrades.queued_selection[player_id] > 0 then
		...
		Upgrades:ShowSelection(
			hero,
			selection_data.rarity,
			player_id,
			false,
			selection_data.is_lucky_trinket_proc or false
		)
	end
end
...
function Upgrades:DisableUpgrade(player_id, ability_name, upgrade_name)
	...
	Upgrades.disabled_upgrades_per_player[player_id][ability_name] = Upgrades.disabled_upgrades_per_player[player_id][ability_name]
		or {}
...
function Upgrades:AddOrIncrementUpgrade(hero, ability_name, upgrade_name, value, rarity)
	...
	if not hero.upgrades then
		hero.upgrades = {}
	end
	if not hero.upgrades[ability_name] then
		hero.upgrades[ability_name] = {}
	end

	local upgrade_data = hero.upgrades[ability_name][upgrade_name]

	if not upgrade_data then
		local upgrade_config = self.upgrades_kv[hero:GetUnitName()][ability_name]
				and self.upgrades_kv[hero:GetUnitName()][ability_name][upgrade_name]
			or nil
...
function Upgrades:ApplyLinkedUpgrades(hero, hero_name, ability_name, special_value_name, rarity)
	...
	for linked_name, value in pairs(linked_special_values) do
		...
		if not upgrade_data.operator then
...
function Upgrades:AddGenericUpgradeModifier(unit, upgrade_name, upgrade_count)
	...
	if unit:IsSpiritBear() and (upgrade_definition.ignore_bear and upgrade_definition.ignore_bear == 1) then
		-- print("AddGenericUpgradeModifier discarded", upgrade_name, "for", unit:GetUnitName(), "- can't be applied to bear", upgrade_definition.ignore_bear)
		return
	end

	if unit:IsClone() and (upgrade_definition.ignore_clones and upgrade_definition.ignore_clones == 1) then
...
function Upgrades:AddGenericUpgrade(hero, upgrade_name, count)
	...
	local current_count = hero.upgrades.generic
			and hero.upgrades.generic[upgrade_name]
			and hero.upgrades.generic[upgrade_name].count
		or 0
	local new_count = current_count + count

	local applied_upgrade = Upgrades:SetGenericUpgrade(hero, upgrade_name, new_count)
	if not applied_upgrade then
		return
	end
	...
	if applied_upgrade.max_count and applied_upgrade.count >= applied_upgrade.max_count then
		...
		print(
			"Disabled",
			upgrade_name,
			"from rolling - max count reached",
			applied_upgrade.count,
			applied_upgrade.max_count
		)
	end
...
function Upgrades:ProcessClone(clone, hero, skip_generics)
	if not clone or not IsValidEntity(clone) or not clone:IsAlive() then
		return
	end
	if not IsValidEntity(hero) then
		hero = clone:GetCloneSource()
	end
	if not IsValidEntity(hero) then
		return
	end
	...
	if not clone:HasModifier("modifier_primary_attribute_reader") then
		clone:AddNewModifier(clone, nil, "modifier_primary_attribute_reader", { duration = -1 })
	end
	...
	if not hero.upgrades.generic or skip_generics then
		return
	end
...
function Upgrades:ApplySummonUpgrades(summon, summon_name, owner)
	if not summon or not summon_name then
		return
	end
		...
		if summon:GetBaseMaxHealth() < required_health then
			...
			if summon_params.health_bonus_as_modifier then
				...
				required_health = required_health
					- ability:GetLevelSpecialValueNoOverride(summon_params.health, ability:GetLevel() - 1)

				local new_modifier =
					summon:AddNewModifier(summon, nil, "modifier_summon_bonus_health", { duration = -1 })
	...
	if
		summon_params.retroactive
		and not (self.summon_list[summon_owner_id] and self.summon_list[summon_owner_id][summon_entity_index])
	then
		if not self.summon_list[summon_owner_id] then
			self.summon_list[summon_owner_id] = {}
		end
...
function Upgrades:OnNpcSpawned(event)
	if not event.owner_player_id then
		return
	end
...
function Upgrades:OnModifierAdded(event)
	...
	if
		modifier_name == "modifier_invoker_ice_wall_slow_aura"
		or modifier_name == "modifier_invoker_ice_wall_thinker"
	then
		local parent = modifier:GetParent()
...
function Upgrades:GetPlayerUpgrades(player_id)
	...
	for ability_name, upgrades in pairs(hero_upgrades) do
		for upgrade_name, _ in pairs(upgrades) do
			if
				hero.upgrades
				and hero.upgrades[ability_name]
				and hero.upgrades[ability_name][upgrade_name]
				and hero.upgrades[ability_name][upgrade_name].count > 0
			then
				table.insert(
					player_upgrades,
					{ ability_name, upgrade_name, hero.upgrades[ability_name][upgrade_name].count }
				)
			end
		end
	end
	...
	for upgrade_name, _ in pairs(Upgrades.generic_upgrades_kv or {}) do
		if
			hero.upgrades
			and hero.upgrades.generic
			and hero.upgrades.generic[upgrade_name]
			and hero.upgrades.generic[upgrade_name].count > 0
		then
			table.insert(
				player_upgrades,
				{ "generic", string.gsub(upgrade_name, "generic_", ""), hero.upgrades.generic[upgrade_name].count }
			)
		end
	end
...
function Upgrades:ApplySavedBuildEvent(event)
	local player_id = event.PlayerID
	local build_id = event.build_id
	if not IsValidPlayerID(player_id) or not build_id then
		return
	end
	print("applying saved build", build_id)

	local player_builds = Upgrades.favourite_builds[player_id]
	if not player_builds then
		return
	end
	local desired_build = player_builds[build_id]
	if not desired_build then
		print("no saved build under index", build_id)
		return
	end

	local current_hero = GameLoop.hero_by_player_id[player_id]
	local current_hero_name = IsValidEntity(current_hero) and current_hero:GetUnitName() or ""

	local build_data = table.deepcopy(desired_build.build_data or {})

	if desired_build.hero_name ~= current_hero_name then
		build_data = {}
		build_data.generic = table.deepcopy((desired_build.build_data or {}).generic or {})

		print("current hero is different from build hero, applying only generic upgrades")
	end

	Upgrades.favorites_upgrades[player_id] = build_data
...
function Upgrades:SaveBuildEvent(event)
	...
	local hero_name = "<no hero>"
	local current_hero = GameLoop.hero_by_player_id[player_id]
	if IsValidEntity(current_hero) then
		hero_name = current_hero:GetUnitName()
	end

	if build_id > BUILD_MAX_PER_SUB_TIER[WebPlayer:GetSubscriptionTier(player_id)] then
		DisplayError(player_id, "#dota_hud_error_cannot_save_into_locked_build_tier")
		return
	end
	...
	if not applying_same_build then
		WebApi:Send("api/lua/match/set_favorite_build_data", build_data, function(response)
...
function Upgrades:ClearBuildEvent(event)
	local player_id = event.PlayerID
	local build_id = event.build_id
	if not IsValidPlayerID(player_id) or not build_id then
		return
	...
	WebApi:Send("api/lua/match/clear_build", {
		steam_id = tostring(PlayerResource:GetSteamID(player_id)),
		build_id = build_id,
	}, function(response)
		print("[Upgrades] successfully saved build", build_id, "for player", player_id)
...
function Upgrades:RenameBuildEvent(event)
	local player_id = event.PlayerID
	local build_id = event.build_id
	if not IsValidPlayerID(player_id) or not build_id or not event.build_name then
		return
	end
	...
	WebApi:Send("api/lua/match/set_favorite_build_name", {
		steam_id = tostring(PlayerResource:GetSteamID(player_id)),
		build_id = build_id,
		build_name = event.build_name,
	}, function(response)
...
function Upgrades:_SerializeFavorites(favorites_data)
	...
	for ability_name, ability_favorites in pairs(favorites_data or {}) do
		...
		for special_name, _ in pairs(ability_favorites or {}) do
			if ability_name == "generic" then
				special_name = special_name:gsub("generic_", "")
			end
...
function Upgrades:_DeserializeFavorites(remote_favorites_data)
	...
	for ability_name, ability_favorites in pairs(remote_favorites_data or {}) do
		...
		for _, special_name in pairs(ability_favorites or {}) do
			if ability_name == "generic" then
				special_name = "generic_" .. special_name
			end
