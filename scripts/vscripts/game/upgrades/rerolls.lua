...
function UpgradeRerolls:_ConsumeRerolls(player_id, rarity)
	...
	if current_free_rerolls >= rarity then
		...
		print(
			"[UpgradeRerolls] reroll funded by free rerolls: "
				.. current_free_rerolls
				.. " => "
				.. UpgradeRerolls.current_free_rerolls[player_id]
		)
		return true
	end
	...
	if current_free_rerolls > 0 and (current_free_rerolls + current_consumable_rerolls) >= rarity then
		...
		print(
			"[UpgradeRerolls] reroll partially compensated by free rerolls: "
				.. rarity + current_free_rerolls
				.. " => "
				.. rarity
		)
	end
...
function UpgradeRerolls:_UpdateRerollCount(event)
	if not IsValidPlayerID(event.player_id) then
		return
	end
