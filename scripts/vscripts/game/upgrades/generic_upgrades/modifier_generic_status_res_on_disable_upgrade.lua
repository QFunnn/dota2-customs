...
modifier_generic_status_res_on_disable_upgrade = modifier_generic_status_res_on_disable_upgrade
	or class(modifier_base_generic_upgrade)

LinkLuaModifier(
	"modifier_generic_status_res_on_disable_bonus",
	"game/upgrades/generic_upgrades/modifier_generic_status_res_on_disable_upgrade",
...
function modifier_generic_status_res_on_disable_upgrade:OnModifierAdded(event)
	if not IsValidEntity(event.unit) or event.unit ~= self.parent then
		return
	end
	...
	if is_stun or state[self.state_hexed] or state[self.state_feared] or state[self.state_rooted] then
		...
		if not modifier or modifier:IsNull() then
			modifier = self.parent:AddNewModifier(
				self.parent,
				nil,
				"modifier_generic_status_res_on_disable_bonus",
				{ duration = -1 }
...
function modifier_generic_status_res_on_disable_bonus:GetTexture()
	return "item_titan_sliver"
end
function modifier_generic_status_res_on_disable_bonus:IsPurgable()
	return false
end
