...
function modifier_base_generic_upgrade:IsHidden()
	return true
end
function modifier_base_generic_upgrade:IsPurgable()
	return false
end
function modifier_base_generic_upgrade:RemoveOnDeath()
	return false
end
...
function modifier_base_generic_upgrade:RefreshOnLevelGained()
	...
	if
		not self.parent:IsRealHero()
		or self.parent:IsIllusion()
		or self.parent:IsClone()
		or self.parent:GetUnitLabel() == "spirit_bear"
	then
		return
	end
...
function modifier_base_generic_upgrade:CalculateBonusPerUpgrade(name, multiplier)
	...
	local upgrade_data = IsServer() and GenericUpgrades.generic_upgrades_data[self.upgrade_name]
		or GENERIC_UPGRADES_DATA[self.upgrade_name]

	local upgrade_value =
		UpgradesUtilities:CalculateUpgradeValue(self.parent, self.bonus_per_upgrade, self:GetStackCount(), upgrade_data)
