...
function modifier_base_shield_handler:IsHidden()
	return false
end
function modifier_base_shield_handler:DestroyOnExpire()
	return false
end
...
function modifier_base_shield_handler:OnIntervalThink()
	...
	if
		self.parent:HasModifier("modifier_fountain_rejuvenation_effect_lua")
		and self.current_shield < self.shield_capacity
	then
...
function modifier_base_shield_handler:HandleShieldDamage(event)
	if not IsServer() then
		if event.report_max then
			return self.shield_capacity
		end
		return self.current_shield
	end
	if event.damage <= 0.5 then
		return
	end
