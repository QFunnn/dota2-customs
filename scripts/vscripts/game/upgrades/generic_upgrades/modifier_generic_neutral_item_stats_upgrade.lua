...
modifier_generic_neutral_item_stats_upgrade = modifier_generic_neutral_item_stats_upgrade
...
function modifier_generic_neutral_item_stats_upgrade:OnAttackLanded(keys)
	...
	if not parent:HasModifier("modifier_item_heavy_blade") then
		return
	end
	...
	if
		parent:IsRealHero()
		and parent:GetTeam() ~= target:GetTeam()
