...
LinkLuaModifier(
	"modifier_generic_magic_resistance_reduction_target",
	"game/upgrades/generic_upgrades/modifier_generic_magic_resistance_reduction_upgrade",
...
if IsServer() then
	...
	function modifier_generic_magic_resistance_reduction_upgrade:OnSpellTargetReady(event)
		...
		if target:GetTeam() == self.parent:GetTeam() then
			return
		end
		...
		if self.parent:IsIllusion() or self.parent:IsMonkeyKingSoldier() or self.parent:IsClone() then
			...
			if not IsValidEntity(modifier_owner) then
				return
			end
		end

		local existing_modifier =
			target:FindModifierByNameAndCaster("modifier_generic_magic_resistance_reduction_target", modifier_owner)

		if not existing_modifier or existing_modifier:IsNull() then
			existing_modifier = target:AddNewModifier(
				modifier_owner,
				nil,
				"modifier_generic_magic_resistance_reduction_target",
...
function modifier_generic_magic_resistance_reduction_target:GetTexture()
	return "../items/veil_of_discord"
end
function modifier_generic_magic_resistance_reduction_target:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
function modifier_generic_magic_resistance_reduction_target:IsDebuff()
	return true
end
