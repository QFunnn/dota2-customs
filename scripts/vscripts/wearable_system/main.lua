...
LinkLuaModifier(
	"modifier_warlock_golem_burning_puppet",
	"Wearable_System/modifier_wearable_model",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_wearable_material_effect",
	"Wearable_System/modifier_wearable_material_effect",
	LUA_MODIFIER_MOTION_NONE
)
	...
	function Wearable_System:PlayItemEffect(iPlayerID, hCaster, sAbilityName, event, tData)
		if self.ItemEffect[iPlayerID] ~= nil then
			...
			if ItemEffectKV_original[sAbilityName] then
				local tInfo = ItemEffectKV_original[sAbilityName][EffectName]
					or ItemEffectKV_original[sAbilityName]["Default"]
				if tInfo then
			...
			elseif ItemEffectKV_custom[sAbilityName] and ItemEffectKV_custom[sAbilityName][EffectName] then
				local tInfo = ItemEffectKV_custom[sAbilityName][EffectName]
					or ItemEffectKV_custom[sAbilityName]["Default"]
				if tInfo then
					for _, EffectConfig in pairs(tInfo) do
						if EffectConfig.event == event then
							EffectConfig["callback"](hCaster, sAbilityName, tData)
