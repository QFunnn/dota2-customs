...
function boss_4_1.prototype.Precache(self, context)
	...
	PrecacheResource(
		"particle",
		"particles/econ/items/primal_beast/primal_beast_2022_prestige/primal_beast_2022_prestige_onslaught_chargeup.vpcf",
		context
	)
...
function boss_4_1.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		self.iParticleID =
			ParticleManager_s2c:CreateParticle("particles/warning/linear.vpcf", PATTACH_CUSTOMORIGIN, nil)
		...
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 2, Vector(fWidth, fCastPoint, 1 / fCastPoint))
	end, { weight = 0 })
...
function boss_4_1.prototype.OnSpellStart(self)
	...
	hCaster:AddNewModifier(hCaster, self, "modifier_boss_4_1_move", {
		duration = fDuration,
		vStartPos = VectorToString(_G, hCaster:GetAbsOrigin()),
...
function boss_4_1.prototype.OnProjectileHit_ExtraData(self, target, location, extraData)
	...
	if IsValid(hCaster) and IsValid(target) then
		...
		target:AddNewModifier(hCaster, self, "modifier_boss_4_1_knockback", {
			duration = 0.2,
			vDirection = VectorToString(_G, vDirection),
		})
		ApplyDamage({
...
boss_4_1 = __TS__DecorateLegacy({ register(_G) }, boss_4_1)
...
modifier_boss_4_1_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_4_1_buff)
...
function modifier_boss_4_1_move.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		local iParticleID = ParticleManager:CreateParticle(
			"particles/boss/boss_4/boss_4_1/prestige/prestige.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			hParent
		)
		self:AddParticle(iParticleID, false, false, -1, false, false)
...
function modifier_boss_4_1_move.prototype.UpdateHorizontalMotion(self, me, dt)
	if IsServer() then
		...
		me:SetAbsOrigin(VectorLerp(self:GetElapsedTime() / self:GetDuration(), self.vStartPos, self.vEndPos))
	end
end
...
modifier_boss_4_1_move = __TS__DecorateLegacy({ register(_G) }, modifier_boss_4_1_move)
...
function modifier_boss_4_1_knockback.prototype.UpdateHorizontalMotion(self, me, dt)
	if IsServer() then
		local fTime = math.min(GameRules:GetGameTime() - self.fStartTime, self:GetDuration())
...
modifier_boss_4_1_knockback = __TS__DecorateLegacy({ register(_G) }, modifier_boss_4_1_knockback)