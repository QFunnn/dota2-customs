...
boss_4_2 = __TS__DecorateLegacy({ register(_G) }, boss_4_2)
...
function modifier_boss_4_2_buff.prototype.OnUnitMoved(self, event)
	...
	if IsValid(hAbility) and fDistance >= self.distance then
		ParticleManager_s2c:ToClient(function()
			...
			EmitSoundOnLocationWithCaster(hParent:GetAbsOrigin(), "Hero_PrimalBeast.Trample", hParent)
		end, { weight = 0 })
		self.vStartPos = vPos
...
modifier_boss_4_2_buff = __TS__DecorateLegacy({ diy(_G) }, modifier_boss_4_2_buff)