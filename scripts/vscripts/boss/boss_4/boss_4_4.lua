...
function boss_4_4.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		self.iParticleID =
			ParticleManager_s2c:CreateParticle("particles/warning/circular.vpcf", PATTACH_CUSTOMORIGIN, nil)
	...
	hCaster:AddNewModifier(hCaster, self, "modifier_boss_4_4_jump", {
		duration = duration,
...
boss_4_4 = __TS__DecorateLegacy({ register(_G) }, boss_4_4)
...
function modifier_boss_4_4_jump.prototype.OnCreated(self, params)
	if IsServer() then
		...
		if self:ApplyVerticalMotionController() and self:ApplyHorizontalMotionController() then
			...
			self.vStartVerticalVelocity = Vector(0, 0, fHeightOffset) / self:GetDuration()
				- self.vAcceleration * self:GetDuration() / 2
		else
			self:Destroy()
		end
	end
	...
	if IsServer() then
		...
		ParticleManager_s2c:ToClient(function()
			local iParticleID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_3/boss_3_1/boom/boom.vpcf",
...
function modifier_boss_4_4_jump.prototype.UpdateHorizontalMotion(self, me, dt)
	if IsServer() then
		me:SetAbsOrigin(VectorLerp(self:GetElapsedTime() / self:GetDuration(), self.vStartPosition, self.vEndPosition))
	end
end
...
function modifier_boss_4_4_jump.prototype.UpdateVerticalMotion(self, me, dt)
	if IsServer() then
		me:SetAbsOrigin(
			self.vStartPosition
				+ self.vStartVerticalVelocity * self:GetElapsedTime()
				+ 0.5 * self.vAcceleration * (self:GetElapsedTime() * self:GetElapsedTime())
		)
	end
end
...
modifier_boss_4_4_jump = __TS__DecorateLegacy({ register(_G) }, modifier_boss_4_4_jump)
...
function modifier_boss_4_4_buff.prototype.OnIntervalThink(self)
	...
	ParticleManager_s2c:ToClient(function()
		local iParticleID =
			ParticleManager_s2c:CreateParticle("particles/boss/boss_4/boss_4_4/hit/hit.vpcf", PATTACH_CUSTOMORIGIN, nil)
		ParticleManager_s2c:SetParticleControl(iParticleID, 0, vPosition)
		ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(1000, 1000, 1000))
		ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
	end, { weight = 0 })
	for _, hTarget in ipairs(tTargets) do
		ApplyDamage({
...
modifier_boss_4_4_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_4_4_buff)