...
function boss_2_3.prototype.OnSpellStart(self)
	...
	hCaster:AddNewModifier(hCaster, self, "modifier_boss_2_3_buff", { duration = self.Values:duration() })
end
boss_2_3 = __TS__DecorateLegacy({ register(_G) }, boss_2_3)
	...
	if self:GetRemainingTime() > 1 then
		for _, hTarget in ipairs(tTargets) do
			...
			ParticleManager_s2c:ToClient(function()
				...
				ParticleManager_s2c:SetParticleControl(iParticleID, 2, Vector(self.radius, 1, 1 / 1))
			end, { weight = 0 })
...
modifier_boss_2_3_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_2_3_buff)
...
modifier_boss_2_3_thinker = __TS__DecorateLegacy({ register(_G) }, modifier_boss_2_3_thinker)