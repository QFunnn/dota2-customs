...
function boss_6_6.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		local iPtclID = ParticleManager_s2c:CreateParticle(
			"particles/warning/circular_2/circular_2.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			hTarget
		)
		ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(hTarget:GetModelRadius(), fCastPoint, 0))
...
function boss_6_6.prototype.OnSpellStart(self)
	...
	local hCyclone = CreateUnitByName("npc_dummy_unit", vPos, false, hCaster, hCaster, hCaster:GetTeamNumber())
...
function boss_6_6.prototype.FindTarget(self, tIgnore)
	...
	EachPlayer(_G, function(____, iPlayerID)
		...
		if hHero ~= nil and hHero:IsAlive() then
			if
				tIgnore == nil
				or __TS__ArrayFindIndex(tIgnore, function(____, v)
						return v == hHero
					end)
					== -1
			then
...
boss_6_6 = __TS__DecorateLegacy({ register(_G) }, boss_6_6)
...
function modifier_boss_6_6_target_debuff.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		local iPtclID = ParticleManager:CreateParticle(
			"particles/boss/boss_6/boss_6_6/target_mark/target_mark.vpcf",
...
modifier_boss_6_6_target_debuff = __TS__DecorateLegacy({ diy_polymer(_G) }, modifier_boss_6_6_target_debuff)
...
function modifier_boss_6_6_cyclone.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		local iPtclID = ParticleManager:CreateParticle(
			"particles/boss/boss_6/boss_6_6/cyclone/cyclone.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			hParent
		)
		ParticleManager:SetParticleControl(iPtclID, 2, Vector(self.Values:range(), 0, 0))
...
function modifier_boss_6_6_cyclone.prototype.OnIntervalThink(self)
	...
	if not IsValid(self.hTarget) or not self.hTarget:IsAlive() then
		...
		if self.hTarget then
			self.hTarget:AddPolymer(hAblt:GetCaster(), hAblt, "modifier_boss_6_6_target_debuff", nil)
	...
	do
		if IsValid(hUnit) and hUnit ~= hParent then
			...
			then
				if
					self.tDamageTargetTime[hUnit:entindex()] == nil
					or self.tDamageTargetTime[hUnit:entindex()] <= GameRules:GetGameTime()
				then
	...
	if self.fCleaveTime ~= nil then
		if self.fCleaveTime <= 0.1 then
			...
			local hMdf = hCyclone:AddNewModifier(
				hTarget or hCyclone,
				hAblt,
				"modifier_boss_6_6_cyclone",
				{ is_cleave = true, cleave_pos = vPos }
			)
...
modifier_boss_6_6_cyclone = __TS__DecorateLegacy({ diy(_G) }, modifier_boss_6_6_cyclone)