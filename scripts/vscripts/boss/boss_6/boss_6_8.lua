...
function boss_6_8.prototype.OnAbilityPhaseStart(self)
				...
				if RollPercentage(50) then
					...
					if iPlayerID ~= nil then
						...
						if IsValid(hHero) then
							self:GameTimer((i - 1) * fCastPoint * RandomFloat(0, 0.2), function()
							...
							end, "little_meteor_" .. tostring(i))
							goto __continue5
						end
					end
				end
				self:GameTimer((i - 1) * fCastPoint * RandomFloat(0.2, 0.4), function()
	...
	ParticleManager_s2c:ToClient(function()
		local iPtclID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_6/boss_6_8/meteor_fly/meteor_fly.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
		ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget + Vector(0, 1500, 1000))
		ParticleManager_s2c:SetParticleControl(iPtclID, 1, self.vTarget + Vector(0, 0, 200))
		ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(fCastPoint, 0, 0))
		local ____self_tPtclIDs_2 = self.tPtclIDs
		____self_tPtclIDs_2[#____self_tPtclIDs_2 + 1] = iPtclID
		iPtclID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_6/boss_6_8/meteor_ground/meteor_ground.vpcf",
			PATTACH_WORLDORIGIN,
			nil
		)
		ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget)
		ParticleManager_s2c:SetParticleControl(iPtclID, 1, Vector(750, 1, 1))
		local ____self_tPtclIDs_3 = self.tPtclIDs
		____self_tPtclIDs_3[#____self_tPtclIDs_3 + 1] = iPtclID
		iPtclID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_6/boss_6_8/caster/caster.vpcf",
		...
		iPtclID =
			ParticleManager_s2c:CreateParticle("particles/warning/circular_3/circular_3.vpcf", PATTACH_WORLDORIGIN, nil)
		ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget)
		ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(10000, fCastPoint, 1 / fCastPoint))
	...
	self:GameTimer(fCastPoint - 1.1, function()
		ParticleManager_s2c:ToClient(function()
			local iPtclID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_6/boss_6_8/meteor_ground2/meteor_ground2.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget)
	...
	self:GameTimer(0, function()
		ScreenShake(self.vTarget, 5, 50, 2, 0, 0, true)
		local tIgnoreUnit = {}
		if IsValid(hAblt4) then
			for _, hMeteor in pairs(hAblt4.tMeteors) do
				...
				do
					if hTarget:IsPositionInRange(hMeteor:GetAbsOrigin(), meteor_block_range) then
				...
				if hMeteor.__boss_6_8__flag == nil then
					...
					ParticleManager_s2c:ToClient(function()
						...
						local vEnd = hMeteor:GetAbsOrigin() + vDir * meteor_block_range
						local iPtclID = ParticleManager_s2c:CreateParticle(
							"particles/boss/boss_6/boss_6_8/wraing_safe/wraing_safe.vpcf",
							PATTACH_WORLDORIGIN,
							nil
						)
						ParticleManager_s2c:SetParticleControl(iPtclID, 0, hMeteor:GetAbsOrigin())
						ParticleManager_s2c:SetParticleControl(iPtclID, 1, vEnd)
						ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(0, fCastPoint, 0))
						hBuff:AddParticle_s2c(iPtclID, true, false, -1, false, false)
					end, { weight = 0 })
					ParticleManager_s2c:ToClient(function()
						local iPtclID = ParticleManager_s2c:CreateParticle(
		...
		do
			if tIgnoreUnit[hTarget:entindex()] == nil then
				hTarget:AddPolymer(hCaster, self, "modifier_boss_6_8_fire_debuff", {
					duration = 0.5,
...
function boss_6_8.prototype.OnSpellStart(self)
	...
	self.tPtclIDs = nil
	local hCaster = self:GetCaster()
	ScreenShake(self.vTarget, 5, 50, 2, 0, 1, true)
	self:GameTimer(0, function()
		ScreenShake(self.vTarget, 500, 1, 2, 0, 0, true)
		ParticleManager_s2c:ToClient(function()
			local iPtclID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_6/boss_6_8/meteor_collapse/meteor_collapse.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget + Vector(0, 0, 150))
			ParticleManager_s2c:SetParticleControl(iPtclID, 1, Vector(1000, 0, 0))
			ParticleManager_s2c:ReleaseParticleIndex(iPtclID)
		end, { weight = 0 })
		self:GameTimer(0.25, function()
			ParticleManager_s2c:ToClient(function()
				local iPtclID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_6/boss_6_8/bomb/bomb.vpcf",
					PATTACH_WORLDORIGIN,
					nil
				)
				ParticleManager_s2c:SetParticleControl(iPtclID, 0, self.vTarget + Vector(0, 0, 150))
				ParticleManager_s2c:SetParticleControl(iPtclID, 1, Vector(10000, 0, 0))
				ParticleManager_s2c:ReleaseParticleIndex(iPtclID)
...
boss_6_8 = __TS__DecorateLegacy({ diy(_G) }, boss_6_8)
...
modifier_boss_6_8_fire_debuff = __TS__DecorateLegacy({ diy_polymer(_G) }, modifier_boss_6_8_fire_debuff)
...
modifier_boss_6_8_buff = __TS__DecorateLegacy({ diy_polymer(_G) }, modifier_boss_6_8_buff)
...
modifier_boss_6_8_ai = __TS__DecorateLegacy({ diy(_G) }, modifier_boss_6_8_ai)
...
modifier_boss_6_8_casting = __TS__DecorateLegacy({ register(_G) }, modifier_boss_6_8_casting)