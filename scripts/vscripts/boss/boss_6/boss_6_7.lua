...
function boss_6_7.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		ParticleManager_s2c:SetParticleControl(iPtclID, 0, hCaster:GetAbsOrigin())
		ParticleManager_s2c:SetParticleControl(iPtclID, 1, vEnd)
		ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(self.Values:width(), fCastPoint, 1 / fCastPoint))
	end, { weight = 0 })
	ParticleManager_s2c:ToClient(function()
		self.iPtclID_Cast = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_6/boss_6_7/cast_mouth/cast_mouth.vpcf",
	...
	self:GameTimer(fCastPoint - 0.3, function()
		ParticleManager_s2c:ToClient(function()
...
function boss_6_7.prototype.OnSpellStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		local iPtclID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_6/boss_6_7/cast_mouth_big_fire/cast_mouth_big_fire.vpcf",
			PATTACH_CUSTOMORIGIN,
			hCaster
		)
		ParticleManager_s2c:SetParticleControlEnt(
	...
	local vOffset = RotatePosition(Vector(0, 0, 0), QAngle(0, 90, 0), (vEnd - vStart):Normalized() * fWidth)
	...
	do
		if IsPointInPolygon(hTarget:GetAbsOrigin(), tBounds) then
			hTarget:AddNewModifier(hCaster, self, "modifier_stunned", { duration = self.Values:stun_duration() })
		end
	end
	for ____, hTarget in
		ipairs(
	...
	local hFire = hCaster:AddPolymer(hCaster, self, "boss_6_7_fire", {
...
boss_6_7 = __TS__DecorateLegacy({ register(_G) }, boss_6_7)
...
function boss_6_7_fire.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		local vOffset =
			RotatePosition(Vector(0, 0, 0), QAngle(0, 90, 0), (self.vEnd - self.vStart):Normalized() * self.fWidth)
		self.tBounds = { self.vStart - vOffset, self.vStart + vOffset, self.vEnd - vOffset, self.vEnd + vOffset }
		self:OnIntervalThink()
		self:StartIntervalThink(0.1)
	else
		local iPtclID =
			ParticleManager:CreateParticle("particles/boss/boss_6/boss_6_7/fire/fire.vpcf", PATTACH_WORLDORIGIN, nil)
		ParticleManager:SetParticleControl(iPtclID, 0, self.vStart)
		ParticleManager:SetParticleControl(iPtclID, 1, self.vEnd)
		ParticleManager:SetParticleControl(iPtclID, 2, Vector(self:GetRemainingTime(), self.fWidth, 0))
...
function boss_6_7_fire.prototype.OnIntervalThink(self)
	...
	do
		if IsPointInPolygon(hTarget:GetAbsOrigin(), self.tBounds) then
			if
				self.tDamageTargetTime[hTarget:entindex()] == nil
				or self.tDamageTargetTime[hTarget:entindex()] <= GameRules:GetGameTime()
			then
				self.tDamageTargetTime[hTarget:entindex()] = GameRules:GetGameTime() + self.fDamageInterval
				ParticleManager_s2c:ToClient(function()
					local iPtclID = ParticleManager_s2c:CreateParticle(
						"particles/boss/boss_6/boss_6_7/fireing_target/fireing_target.vpcf",
...
boss_6_7_fire = __TS__DecorateLegacy({ diy_polymer(_G) }, boss_6_7_fire)