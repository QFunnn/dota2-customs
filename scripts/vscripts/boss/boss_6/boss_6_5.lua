...
function boss_6_5.prototype.OnSpellStart(self)
	...
	hCaster:AddNewModifier(hCaster, self, "modifier_boss_6_5_move", {
		duration = fDuration,
		vStartPosition = VectorToString(_G, vStartPosition),
		vEndPosition = VectorToString(_G, vEndPosition),
	})
end
boss_6_5 = __TS__DecorateLegacy({ register(_G) }, boss_6_5)
...
function modifier_boss_6_5_move.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		iPtclID = ParticleManager:CreateParticle(
			"particles/boss/boss_6/boss_6_5/caster_cyclone/caster_cyclone.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			hParent
		)
...
function modifier_boss_6_5_move.prototype.OnIntervalThink(self)
	...
	do
		...
		elseif hUnit:IsRealHero() then
			...
			hUnit:AddNewModifier(hParent, hAblt, "modifier_knockback", {
				duration = self.Values:stun_duration(),
			...
			hUnit:AddNewModifier(hParent, hAblt, "modifier_stunned", { duration = self.Values:stun_duration() })
			ApplyDamage({
				ability = hAblt,
				attacker = hParent,
...
modifier_boss_6_5_move = __TS__DecorateLegacy({ register(_G) }, modifier_boss_6_5_move)
...
function modifier_boss_6_5_stun.prototype.OnCreated(self, params)
	if IsServer() then
		self:GetParent():StartGestureWithPlaybackRate(ACT_DOTA_DISABLED, 1 / self:GetDuration() * 1.5)
	end
end
...
modifier_boss_6_5_stun = __TS__DecorateLegacy({ register(_G) }, modifier_boss_6_5_stun)