...
function boss_6_4.prototype.FireMeteor(self, params)
	...
	if params.random_offset_max ~= nil then
		tFireData.pos = tFireData.pos
			+ RandomVector(RandomFloat(params.random_offset_min or 0, params.random_offset_max))
	end
	local iPtclID_Warning
	ParticleManager_s2c:ToClient(function()
		iPtclID_Warning =
			ParticleManager_s2c:CreateParticle("particles/warning/circular_1/circular_1.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		ParticleManager_s2c:SetParticleControl(
			iPtclID_Warning,
			2,
			Vector(self.Values:range(), params.duration, 1 / params.duration)
		)
		ParticleManager_s2c:SetParticleControl(
			iPtclID_Warning,
			3,
	...
	if hTarget ~= nil then
		...
		____tFireData_timers_3[#____tFireData_timers_3 + 1] = self:GameTimer(0, function()
	...
	local function createMeteor()
		local hMeteor =
			CreateUnitByName("summon_boss_6_4", tFireData.pos, false, hCaster, hCaster, hCaster:GetTeamNumber())
...
boss_6_4 = __TS__DecorateLegacy({ register(_G) }, boss_6_4)
	...
	if IsServer() then
		...
		if ParticleManager_s2c:CheckClientBlocked() then
			local iPtclID = ParticleManager:CreateParticle(
				"particles/boss/boss_6/boss_6_4/meteor/meteor.vpcf",
	...
	if IsServer() then
		...
		ParticleManager_s2c:ToClient(function()
			local iPtclID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_6/boss_6_4/meteor_crumble/meteor_crumble.vpcf",
				PATTACH_CUSTOMORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iPtclID, 3, hParent:GetAbsOrigin())
			ParticleManager_s2c:SetParticleControlForward(iPtclID, 3, hParent:GetForwardVector())
...
modifier_boss_6_4_meteor = __TS__DecorateLegacy({ register(_G) }, modifier_boss_6_4_meteor)
...
function modifier_boss_6_4_meteor_fly.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		if ParticleManager_s2c:CheckClientBlocked() then
			local iPtclID = ParticleManager:CreateParticle(
				"particles/boss/boss_6/boss_6_4/meteor_fly/meteor_fly.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				hParent
			)
			ParticleManager:SetParticleControlEnt(
...
function modifier_boss_6_4_meteor_fly.prototype.OnDestroy(self)
	if IsServer() then
		...
		ParticleManager_s2c:ToClient(function()
			local iPtclID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_6/boss_6_1/dust_bomb_warp/dust_bomb_warp.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iPtclID, 0, vTarget)
			ParticleManager_s2c:SetParticleControl(iPtclID, 1, Vector(range, 0, 0))
			ParticleManager_s2c:ReleaseParticleIndex(iPtclID)
			local iPtclID2 = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_6/boss_6_4/meteor_bomb/meteor_bomb.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iPtclID2, 0, vTarget)
			ParticleManager_s2c:SetParticleControl(iPtclID2, 1, Vector(range, 0, 0))
		...
		do
			...
			if fDis <= fMeteorReadius then
				local tKnockback = {
					duration = RemapValClamped(fDis, 0, fMeteorReadius, 0.1, 0.3),
		...
		if hMdf ~= nil then
			ParticleManager_s2c:ToClient(function()
				local iPtclID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_6/boss_6_4/meteor_ground/meteor_ground.vpcf",
					PATTACH_WORLDORIGIN,
					nil
				)
				ParticleManager_s2c:SetParticleControl(iPtclID, 0, vTarget)
...
modifier_boss_6_4_meteor_fly = __TS__DecorateLegacy({ register(_G) }, modifier_boss_6_4_meteor_fly)