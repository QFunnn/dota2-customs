...
function boss_5_5.prototype.OnSpellStart(self)
	...
	EachPlayer(_G, function(____, iPlayerID)
		...
		local hPhoenix = CreateUnitByName("boss_5_phoenix", vPosition, true, hCaster, hCaster, hCaster:GetTeamNumber())
		if IsValid(hPhoenix) then
			hPhoenix:AddAbility("attribute")
			AttributeKind.Atk.BONUS:Set(hPhoenix, AttributeKind.Atk:Get(hCaster) * atk * 0.01, "BASE")
...
boss_5_5 = __TS__DecorateLegacy({ register(_G) }, boss_5_5)
		...
		if IsValid(self.hHero) and self.hHero:IsAlive() then
			...
			ParticleManager_s2c:ToClient(function()
				self.iParticleID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_5/boss_5_5/link/link.vpcf",
				...
				self:AddParticle_s2c(self.iParticleID, false, false, -1, false, false)
			end)
		end
...
modifier_boss_5_5 = __TS__DecorateLegacy({ diy(_G) }, modifier_boss_5_5)