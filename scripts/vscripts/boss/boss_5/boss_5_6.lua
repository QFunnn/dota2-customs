...
function boss_5_6.prototype.OnAbilityPhaseStart(self)
	...
	do
		...
		while i <= 4 do
			local index = i
			self:GameTimer(2.33 / 4 * i, function()
				self:_A(index)
				local vDir = Rotation2D(Vector(1, 0, 0), 90 * index)
				...
				ParticleManager_s2c:ToClient(function()
					...
					ParticleManager_s2c:SetParticleControl(
						iParticleID,
						2,
						Vector((fDis - 600) / math.sin(math.rad(45)), 0, 0)
					)
...
function boss_5_6.prototype._A(self, index)
	...
	DebugDrawLine(vStartPosition, vStartPosition + vRightDir * fDis, 255, 0, 0, true, 1)
	DebugDrawLine(vStartPosition, vStartPosition + vLeftDir * fDis, 255, 255, 255, true, 1)
	local iCount = 1
	local iMaxCount = 5
				...
				while i <= 2 * iCount - 1 do
					...
					ParticleManager_s2c:ToClient(function()
						local iParticleID = ParticleManager_s2c:CreateParticle(
							"particles/boss/boss_5/boss_5_6/fire/fire.vpcf",
...
function boss_5_6.prototype.OnSpellStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		local iParticleID =
			ParticleManager_s2c:CreateParticle("particles/boss/boss_5/boss_5_6/aoe/aoe.vpcf", PATTACH_CUSTOMORIGIN, nil)
		ParticleManager_s2c:SetParticleControl(iParticleID, 0, hCaster:GetAbsOrigin())
		ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(distance, 0, 0))
...
boss_5_6 = __TS__DecorateLegacy({ register(_G) }, boss_5_6)
...
modifier_boss_5_6_invulnerable = __TS__DecorateLegacy({ register(_G) }, modifier_boss_5_6_invulnerable)
...
modifier_boss_5_6 = __TS__DecorateLegacy({ register(_G) }, modifier_boss_5_6)
...
function modifier_boss_5_6_buff.prototype.BoomDamage(self, index)
	if IsServer() then
		...
		DebugDrawLine(vStartPosition, vStartPosition + vRighDir * self.distance, 255, 255, 255, true, 1)
		DebugDrawLine(vStartPosition, vStartPosition + vLeftDir * self.distance, 255, 255, 255, true, 1)
		local vEndPosition = vStartPosition + Rotation2D(vRighDir, -45) * 1100
		ParticleManager_s2c:ToClient(function()
			local iParticleID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_5/boss_5_6/light/light.vpcf",
				PATTACH_CUSTOMORIGIN,
				nil
			)
			ParticleManager_s2c:SetParticleControl(iParticleID, 0, vEndPosition)
			ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(500, 1, 1))
			ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
		end, { weight = 0 })
		local fDamage = AttributeKind.Atk:Get(hParent) * self.dmg_factor
		hAbility:GameTimer(1, function()
			ParticleManager_s2c:ToClient(function()
				local iParticleID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_5/b_5_6/boom/boom.vpcf",
					PATTACH_CUSTOMORIGIN,
					nil
				)
				ParticleManager_s2c:SetParticleControl(iParticleID, 0, vEndPosition)
				ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(1000, 0, 0))
				ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
				hParent:EmitSound("Ability.LightStrikeArray")
			end, { weight = 0 })
			local tTargets = FindUnitsInRadiusWithAbility(_G, hParent, hAbility, vStartPosition, self.distance)
			for _, hTarget in pairs(tTargets) do
				...
				if
					index == 4 and fAngle > fMinAngle
					or fAngle >= 0 and fAngle <= fMaxAngle - 360
					or fAngle > fMinAngle and fAngle <= fMaxAngle
				then
					ApplyDamage({
						ability = hAbility,
						attacker = hParent,
...
modifier_boss_5_6_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_5_6_buff)
...
modifier_boss_5_6_ai = __TS__DecorateLegacy({ diy(_G) }, modifier_boss_5_6_ai)