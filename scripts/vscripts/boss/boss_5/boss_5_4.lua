...
function boss_5_4.prototype.OnAbilityPhaseStart(self)
		...
		while i < iCount do
			...
			ParticleManager_s2c:ToClient(function()
				local iParticleID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_5/boss_5_4/sunray/sunray.vpcf",
					PATTACH_ABSORIGIN,
					hCaster
				)
				ParticleManager_s2c:SetParticleControl(iParticleID, 0, vStartPosition + Vector(0, 0, 256))
				ParticleManager_s2c:SetParticleControl(iParticleID, 1, vStartPosition + Vector(0, 0, dis))
				...
				self:GameTimer(1, function()
					do
						...
						while j <= 90 do
							local count = j
							self:GameTimer(self:GetCastPoint() / 2 / 90 * count, function()
	...
	ParticleManager_s2c:ToClient(function()
		self.iParticleID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_5/boss_5_4/a/a.vpcf",
			PATTACH_ABSORIGIN_FOLLOW,
			hCaster
		)
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 1, Vector(500, 0, 0))
	end, { weight = 0 })
...
boss_5_4 = __TS__DecorateLegacy({ register(_G) }, boss_5_4)
	...
	if IsServer() then
		...
		do
			...
			while i < self.fCount do
				...
				local tTargets =
					FindUnitsInLineWithAbility(_G, hParent, hAbility, vStartPosition, vEndPosition, self.iWidth)
				...
				for _, hTarget in pairs(tTargets) do
					...
					ParticleManager_s2c:ToClient(function()
						local iParticleID = ParticleManager_s2c:CreateParticle(
							"particles/boss/boss_5/boss_5_4/hit/hit.vpcf",
							PATTACH_ABSORIGIN_FOLLOW,
							hTarget
						)
						ParticleManager_s2c:SetParticleControl(iParticleID, 1, vStartPosition + Vector(0, 0, 256))
...
modifier_boss_5_4_turn = __TS__DecorateLegacy({ register(_G) }, modifier_boss_5_4_turn)