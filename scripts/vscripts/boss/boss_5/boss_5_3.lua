...
function boss_5_3.prototype.OnSpellStart(self)
	...
	do
		...
		while i < iCount do
			...
			local hEgg = CreateUnitByName("boss_5_egg", vEndPosition, true, hCaster, hCaster, hCaster:GetTeamNumber())
			if IsValid(hEgg) then
				hEgg:AddAbility("attribute")
				AttributeKind.HpLimit.BONUS:Set(hEgg, AttributeKind.HpLimit:Get(hCaster) * hp * 0.01, "BASE1")
				AttributeKind.Armor.BONUS:Set(hEgg, AttributeKind.Armor:Get(hCaster) * armor * 0.01, "BASE")
...
boss_5_3 = __TS__DecorateLegacy({ register(_G) }, boss_5_3)
...
function modifier_boss_5_3_buff.prototype.OnCreated(self, params)
	...
	if IsServer() then
		...
		local iParticleID = ParticleManager:CreateParticle(
			"particles/boss/boss/boss_5_3/circle/circle.vpcf",
...
function modifier_boss_5_3_buff.prototype.OnDestroy(self)
	if IsServer() then
		...
		if self:GetRemainingTime() <= 0 then
			local tTargets = FindUnitsInRadiusWithAbility(_G, hCaster, hAbility, hParent:GetAbsOrigin(), self.radius)
			...
			ParticleManager_s2c:ToClient(function()
				local iParticleID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_5/boss_5_3/aoe/aoe.vpcf",
					PATTACH_CUSTOMORIGIN,
					nil
				)
...
modifier_boss_5_3_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_5_3_buff)