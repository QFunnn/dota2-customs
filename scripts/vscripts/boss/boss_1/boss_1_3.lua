...
boss_1_3 = __TS__DecorateLegacy({ register(_G) }, boss_1_3)
...
function modifier_boss_1_3_buff.prototype.OnFire(self)
	...
	hParent:GameTimer(1.6, function()
		if IsValid(hParent) and hParent:IsAlive() and IsValid(self) and IsValid(hAbility) then
			...
			local vStartPosition =
				GetGroundPosition(hParent:GetAttachmentOrigin(hParent:ScriptLookupAttachment("attach_mouth")), hParent)
			...
			do
				...
				while i < count - 1 do
					...
					ParticleManager_s2c:ToClient(function()
						...
						ParticleManager_s2c:SetParticleControl(iParticleID, 3, Vector(0, self.crack_time, 0))
					...
					hParent:GameTimer(self.crack_time, function()
						if IsValid(hParent) and hParent:IsAlive() and IsValid(hAbility) and IsValid(self) then
							...
							for _, hTarget in ipairs(tTargets) do
								...
								local vCenterPosition =
									self:GetCenterPos(vStartPosition, vPostion, hTarget:GetAbsOrigin())
								ParticleManager_s2c:ToClient(function()
									local iParticleID = ParticleManager_s2c:CreateParticle(
										"particles/units/heroes/hero_elder_titan/elder_titan_earth_splitter_move.vpcf",
									...
									ParticleManager_s2c:SetParticleControl(iParticleID, 1, hTarget:GetAbsOrigin())
									ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
								end, { weight = 0 })
...
modifier_boss_1_3_buff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_1_3_buff)