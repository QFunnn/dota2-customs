...
function boss_1_2.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		self.iParticleID =
			ParticleManager_s2c:CreateParticle("particles/warning/trapezium/trapezium.vpcf", PATTACH_WORLDORIGIN, nil)
		...
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 2, Vector(0, end_width, fCastPoint))
	end, { weight = 0 })
	return true
end
...
function boss_1_2.prototype.OnSpellStart(self)
	...
	self:GameTimer(distance / speed, function()
		CreateModifierThinker(hCaster, self, "modifier_boss_1_2_thinker", {
			duration = self.Values:burn_duration(),
			vStartPos = VectorToString(_G, vStartPos),
			vEndPos = VectorToString(_G, vPostion),
		}, vStartPos, hCaster:GetTeamNumber(), false)
	end)
end
function boss_1_2.prototype.OnProjectileHit_ExtraData(self, target, location, extraData)
	if IsValid(target) then
		...
		target:AddNewModifier(hCaster, self, "modifier_knockback", {
			center_x = vCenter.x,
			center_y = vCenter.y,
			center_z = vCenter.z,
			should_stun = 0,
...
boss_1_2 = __TS__DecorateLegacy({ register(_G) }, boss_1_2)
...
function modifier_boss_1_2_thinker.prototype.OnCreated(self, params)
	if IsServer() then
		...
		DebugDrawLine(self.vStartPos, self.vEndPos, 255, 0, 0, false, self:GetDuration())
	end
end
...
modifier_boss_1_2_thinker = __TS__DecorateLegacy({ register(_G) }, modifier_boss_1_2_thinker)