...
function boss_8_2.prototype.Precache(self, context) end
...
function boss_8_2.prototype.OnAbilityPhaseStart(self)
	...
	if self.IsFalse then
		...
		ParticleManager_s2c:ToClient(function()
			local iParticleID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_8/boss_8_1/msg/msg.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				hCaster
			)
			ParticleManager_s2c:SetParticleControl(iParticleID, 4, Vector(self:GetCastPoint(), 0, 0))
	...
	ParticleManager_s2c:ToClient(function()
		...
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 0, hCaster:GetAbsOrigin())
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 2, Vector(radius, self:GetCastPoint(), 0))
	end, { weight = 0 })
	return true
end
...
function boss_8_2.prototype.OnSpellStart(self)
	...
	if self.IsFalse then
		...
		local tTargets = FindUnitsInRadiusWithAbility(_G, hCaster, self, vPosition, false_radius)
		...
		local tTargets = FindUnitsInRadiusWithAbility(_G, hCaster, self, vPos, radius)
		for _, hTarget in pairs(tTargets) do
			ApplyDamage({
				ability = self,
				attacker = hCaster,
...
boss_8_2 = __TS__DecorateLegacy({ register(_G) }, boss_8_2)