...
function boss_8_4.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		local iParticleID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_8/boss_8_4/fire_cast/fire_cast.vpcf",
...
function boss_8_4.prototype.OnSpellStart(self)
	...
	EachPlayer(_G, function(____, iPlayerID)
		...
		if IsValid(hHero) then
			...
			local hSummon =
				CreateUnitByName("boss_8_3_summon", vPosition, true, hCaster, hCaster, hCaster:GetTeamNumber())
			if IsValid(hSummon) then
				hSummon:AddAbility("attribute")
				AttributeKind.Atk.BONUS:Set(hSummon, AttributeKind.Atk:Get(hCaster) * atk * 0.01, "BASE")
				AttributeKind.MoveSpd.BONUS:Set(hSummon, AttributeKind.MoveSpd:Get(hCaster) * move_speed * 0.01, "BASE")
...
boss_8_4 = __TS__DecorateLegacy({ register(_G) }, boss_8_4)
	...
	if IsServer() then
		...
		if ParticleManager_s2c:CheckClientBlocked() then
			local iParticleID = ParticleManager:CreateParticle(
				"particles/boss/boss_8/boss_8_4/fire/fire.vpcf",
				PATTACH_ABSORIGIN_FOLLOW,
				hParent
			)
...
function modifier_boss_8_4.prototype.OnIntervalThink(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		ParticleManager_s2c:SetParticleControl(iParticleID, 2, Vector(self.radius, self.delay_time, 0))
	end, { weight = 0 })
	hParent:GameTimer(self.delay_time, function()
		...
		if hAbility.IsFalse then
			...
			local tTargets = FindUnitsInRadiusWithAbility(_G, hCaster, hAbility, vPosition, false_radius)
			...
			ParticleManager_s2c:ToClient(function()
				...
				ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(false_radius, false_radius, false_radius))
				ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
			end, { weight = 0 })
			ParticleManager_s2c:ToClient(function()
				local iParticleID = ParticleManager_s2c:CreateParticle(
			...
			ParticleManager_s2c:ToClient(function()
				...
				ParticleManager_s2c:SetParticleControl(iParticleID, 0, vPosition)
				ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(false_radius, 0, 0))
				ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
			end, { weight = 0 })
		else
			ParticleManager_s2c:ToClient(function()
...
function modifier_boss_8_4.prototype.IsPositionIntPos(self, hTarget)
	...
	for _, iSummonIndex in pairs(hAbility.tSummonIndex) do
		...
		if IsValid(hSummon) and hTarget:IsPositionInRange(hSummon:GetAbsOrigin(), self.radius) then
			b = true
			break
		end
	end
...
modifier_boss_8_4 = __TS__DecorateLegacy({ register(_G) }, modifier_boss_8_4)