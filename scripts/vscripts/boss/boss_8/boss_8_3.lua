...
function boss_8_3.prototype.OnAbilityPhaseStart(self)
		...
		do
			...
			while i < iCount do
				...
				local iParticleID = ParticleManager_s2c:CreateParticle(
					"particles/boss/boss_8/boss_8_3/warning/linear.vpcf",
					PATTACH_WORLDORIGIN,
					nil
				)
				...
				ParticleManager_s2c:SetParticleControl(
					iParticleID,
					2,
					Vector(self.Values:width(), fCastPoint, 1 / fCastPoint)
				)
	...
	ParticleManager_s2c:ToClient(function()
		self.iParticleID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_8/boss_8_3/wings/arcana_wings.vpcf",
...
function boss_8_3.prototype.OnSpellStart(self)
	...
	do
		...
		while i < iCount do
			...
			tHashtable.iParticleID = ParticleManager:CreateParticle(
				"particles/boss/boss_8/boss_8_3/meteor/meteor.vpcf",
				PATTACH_CUSTOMORIGIN,
				nil
			)
			ParticleManager:SetParticleControl(tHashtable.iParticleID, 0, vStartPosition)
			ParticleManager:SetParticleControl(tHashtable.iParticleID, 1, vTempDiretion:Normalized() * speed)
			vTempDiretion.z = 0
			local tInfo = {
				Ability = self,
				Source = hCaster,
...
boss_8_3 = __TS__DecorateLegacy({ register(_G) }, boss_8_3)
...
modifier_boss_8_3 = __TS__DecorateLegacy({ register(_G) }, modifier_boss_8_3)