	...
	do
		...
		while i < count do
			...
			ParticleManager_s2c:ToClient(function()
				...
				ParticleManager_s2c:SetParticleControl(iParticleID, 2, Vector(radius, self:GetCastPoint(), 0))
...
function boss_8_1.prototype.IsPositionIntPos(self, hTarget)
	local b = false
	for _, vPos in pairs(self.tPos) do
		if hTarget:IsPositionInRange(vPos, self.Values:radius()) then
			b = true
			break
		end
	end
...
boss_8_1 = __TS__DecorateLegacy({ register(_G) }, boss_8_1)