...
function boss_7_4.prototype.Precache(self, context)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_broodmother/broodmother_scepter_sticky_snare_telegraph.vpcf",
		context
	)
	PrecacheResource(
		"particle",
		"particles/units/heroes/hero_broodmother/broodmother_scepter_sticky_snare.vpcf",
		context
	)
	PrecacheResource(
		"particle",
...
function boss_7_4.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 2, Vector(width, fCastPoint, 1 / fCastPoint))
		local tSound = {
			"Boss_7.Start_10",
			"Boss_7.Start_11",
			"Boss_7.Start_12",
...
function boss_7_4.prototype.OnSpellStart(self)
	...
	hCaster:GameTimer(self.Values:dealy(), function()
		if iParticleID then
			ParticleManager_s2c:ToClient(function()
		...
		if IsValid(hCaster) then
			CreateModifierThinker(hCaster, self, "modifier_boss_7_4_thinker", {
...
boss_7_4 = __TS__DecorateLegacy({ register(_G) }, boss_7_4)
		...
		for _, hTarget in pairs(tTargets) do
			if entity == hTarget then
				...
				if IsValid(hAbility5) and hAbility5.CreateSpiderEgg then
					hAbility5.CreateSpiderEgg(hAbility5, entity:GetAbsOrigin(), self.count)
				end
				self:Destroy()
				return true
			end
...
modifier_boss_7_4_thinker = __TS__DecorateLegacy({ register(_G) }, modifier_boss_7_4_thinker)
...
modifier_boss_7_4_debuff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_7_4_debuff)