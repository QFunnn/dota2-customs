...
function boss_7_1.prototype.OnAbilityPhaseStart(self)
	...
	ParticleManager_s2c:ToClient(function()
		...
		ParticleManager_s2c:SetParticleControl(self.iParticleID, 2, Vector(fRange, fCastPoint, 1 / fCastPoint))
		local tSound = {
	...
	self.sGameTimerID = hCaster:GameTimer(fCastPoint - 0.3, function()
		ParticleManager_s2c:ToClient(function()
			local iParticleID = ParticleManager_s2c:CreateParticle(
				"particles/boss/boss_7/boss_7_1/spray_cast/spray_cast.vpcf",
				PATTACH_WORLDORIGIN,
				nil
			)
...
function boss_7_1.prototype.OnSpellStart(self)
	...
	for _, hTarget in pairs(tTargets) do
		...
		self:CreateSpiderling(hTarget:GetAbsOrigin(), count)
	end
	ParticleManager_s2c:ToClient(function()
		local iParticleID = ParticleManager_s2c:CreateParticle(
			"particles/boss/boss_7/boss_7_1/spiderlings_spawn/spiderlings_spawn.vpcf",
			PATTACH_CUSTOMORIGIN,
			nil
		)
		ParticleManager_s2c:SetParticleControl(iParticleID, 0, vPostion)
		ParticleManager_s2c:SetParticleControl(iParticleID, 1, Vector(fRadius, fRadius, fRadius))
		ParticleManager_s2c:ReleaseParticleIndex(iParticleID)
	...
	do
		...
		while i < count do
			if #self.tSpiderling < max_count then
				...
				AttributeKind.Atk.BONUS:Set(hSpiderling, AttributeKind.Atk:Get(hCaster) * atk * 0.01, "BASE")
				AttributeKind.HpLimit.BONUS:Set(hSpiderling, AttributeKind.HpLimit:Get(hCaster) * hp * 0.01, "BASE1")
				AttributeKind.Armor.BONUS:Set(hSpiderling, AttributeKind.Armor:Get(hCaster) * armor * 0.01, "BASE")
				AttributeKind.AtkSpd.BONUS:Set(hSpiderling, AttributeKind.AtkSpd:Get(hCaster) * atk_spd * 0.01, "BASE")
...
function boss_7_1.prototype.OnDeath(self, event)
	if __TS__ArrayIncludes(self.tSpiderling, event.unit:entindex()) then
		__TS__ArraySplice(self.tSpiderling, __TS__ArrayIndexOf(self.tSpiderling, event.unit:entindex()))
	end
end
boss_7_1 = __TS__DecorateLegacy({ diy(_G) }, boss_7_1)
...
modifier_boss_7_1_debuff = __TS__DecorateLegacy({ register(_G) }, modifier_boss_7_1_debuff)
...
modifier_summon_boss_7_1 = __TS__DecorateLegacy({ register(_G) }, modifier_summon_boss_7_1)