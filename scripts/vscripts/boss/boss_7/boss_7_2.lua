	...
	do
		...
		while i < count do
			...
			ParticleManager_s2c:ToClient(function()
				local iPtclID =
					ParticleManager_s2c:CreateParticle("particles/warning/linear.vpcf", PATTACH_WORLDORIGIN, nil)
				...
				ParticleManager_s2c:SetParticleControl(iPtclID, 2, Vector(width, fCastPoint, 1 / fCastPoint))
...
function boss_7_2.prototype.OnProjectileHit_ExtraData(self, target, location, extraData)
	if IsValid(target) then
		...
		if IsValid(hAbility) and hAbility.CreateSpiderling then
			hAbility.CreateSpiderling(hAbility, target:GetAbsOrigin(), self.Values:count())
		end
		ApplyDamage({
			ability = self,
			attacker = hCaster,
...
boss_7_2 = __TS__DecorateLegacy({ register(_G) }, boss_7_2)