...
function Util:ChooseObservingTarget(nPlayerID)
	...
	if GameMode.autoCreepMap[nPlayerID] and GameMode.currentRound and not GameMode.currentRound.bEnd then
		...
		for nTeamNumber, bAlive in pairs(GameMode.vAliveTeam) do
			if
				bAlive
				and GameMode.currentRound.spanwers[nTeamNumber]
				and false == GameMode.currentRound.spanwers[nTeamNumber].bProgressFinished
			then
...
function Util:MoveHeroToLocation(nPlayerID, vLocation, hObservingTarget, sRoomName, bPVPend)
	...
	if hHero then
		...
		if Econ.vPlayerData[nPlayerID] and Econ.vPlayerData[nPlayerID].sCurrentStartBlinkEffect then
			iStartPtx = ParticleManager:CreateParticle(
				Econ.vPlayerData[nPlayerID].sCurrentStartBlinkEffect,
				PATTACH_ABSORIGIN,
				hHero
			)
		...
		units = FindUnitsInRadius(
			PlayerResource:GetTeam(nPlayerID),
			hHero:GetAbsOrigin(),
			nil,
			4000,
		...
		if Econ.vPlayerData[nPlayerID] and Econ.vPlayerData[nPlayerID].sCurrentEndBlinkEffect then
			iEndPtx = ParticleManager:CreateParticle(
				Econ.vPlayerData[nPlayerID].sCurrentEndBlinkEffect,
	...
	for _, unit in pairs(units) do
		if
			...
			and unit:IsAlive()
			and unit ~= hHero
			and unit:GetPlayerOwnerID() == nPlayerID
	...
	if hHero then
		if GameRules.nGameStartTime then
			local nGold = math.ceil(
				PlayerResource:GetGoldPerMin(nPlayerID) * (GameRules:GetGameTime() - GameRules.nGameStartTime) / 60
			) + 600 - PvpModule.betValueSum[nPlayerID]
			return nGold
		end
	end
	return 600
...
function Util:IsPointInsideRectangle(p, lu, ld, ru, rd)
	return (Util:GetCross(ru - lu, p - lu) * Util:GetCross(ld - rd, p - rd) >= 0)
		and (Util:GetCross(lu - ld, p - ld) * Util:GetCross(rd - ru, p - ru) >= 0)
end
...
function Util:IsEscaping(hUnit, vPos)
	...
	if hUnit:IsRealHero() then
		...
		return not (
			Util:IsInRoom(hUnit, "prepare", vPos)
			or Util:IsInRoom(hUnit, "center_" .. hUnit:GetTeamNumber(), vPos)
			or Util:IsInRoom(hUnit, nil, vPos)
		)
	end
...
function Util:GetActivePlayerCount()
	...
	for i = 0, DOTA_MAX_TEAM_PLAYERS - 1 do
		if
			PlayerResource:IsValidPlayer(i)
	...
	if IsValid(hHero) then
		...
		for i = 0, hHero:GetAbilityCount() - 1 do
			...
			if
				...
				and Util:IsValidAbility(ability:GetAbilityName(), false)
				and string.find(ability:GetAbilityName(), "special_bonus_") == nil
			then
