...
local function PublishExclusionTables()
	...
	local mc_count = 0
	for _ in pairs(MULTICAST_EXEMPT) do
		mc_count = mc_count + 1
	end
	local rm_count = 0
	for _ in pairs(REARM_EXEMPT) do
		rm_count = rm_count + 1
	end
	local rd_count = 0
	for _ in pairs(REARM_DISCOUNT) do
		rd_count = rd_count + 1
	end
	local ik_count = 0
	for _ in pairs(INSTA_KILL_ABILITIES) do
		ik_count = ik_count + 1
	end
	print(
		string.format(
			"[ABILITY_EXCLUSIONS] NetTables published — multicast=%d entries, rearm=%d entries, rearm_discount=%d entries, insta_kill=%d entries",
			mc_count,
			rm_count,
			rd_count,
			ik_count
