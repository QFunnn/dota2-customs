...
local f = {}
local g = require("tstl_plugins.@bigciba.typescript-to-lua")
local h = require("tstl_plugins.@bigciba.typescript-to-lua.dist.transformation.visitors.call")
local i = h.transformCallAndArguments
local j = require("tstl_plugins.path")
local k = require("tstl_plugins.typescript")
...
local function p(m, q)
	...
	return {
		visitors = {
			[k.SyntaxKind.CallExpression] = function(m, t, u)
				if k:isIdentifier(t.expression) and c(s, t.expression.text) then
					local v = u.checker:getResolvedSignature(t)
					local w = k.factory:createIdentifier(r)
					local x, y = unpack(i(nil, u, t.expression, t.arguments, v, w), 1, 2)
					return g:createCallExpression(x, y, t)
				end
