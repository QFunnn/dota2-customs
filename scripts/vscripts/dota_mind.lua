...
function DotaMind:GameStart()
	...
	local body =
		{ message = { appId = DotaMind.sAppId, signature = sSignature, data = data }, route = "wisp.game.start" }
	local sSignature =
		md5.sumhexa(tostring(JSON:encode({ body = tostring(JSON:encode(body)), secret = DotaMind.sSecret })))
	...
	request:Send(function(result)
		...
		if result.StatusCode == 200 and result.Body ~= nil then
			...
			DotaMind.sLog = DotaMind.sLog
				.. "GameStart Fail: result.StatusCode"
...
function DotaMind:GamePost()
	...
	local body =
		{ message = { appId = DotaMind.sAppId, signature = sSignature, data = data }, route = "wisp.game.post" }
	local sSignature =
		md5.sumhexa(tostring(JSON:encode({ body = tostring(JSON:encode(body)), secret = DotaMind.sSecret })))
	...
	request:Send(function(result)
		...
		if result.StatusCode == 200 and result.Body ~= nil then
			...
			DotaMind.sLog = DotaMind.sLog
				.. "GamePost Fail: result.StatusCode"
				.. tostring(result.StatusCode)
				.. " result.Body"
				.. tostring(result.Body)
		end
...
function DotaMind:UpdateScore(bFinalScoreUpdate)
	...
	for nTeamNumber, _ in pairs(GameMode.vAliveTeam) do
		...
		for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS - 1 do
			if
				...
				and PlayerResource:GetSelectedHeroEntity(nPlayerID)
				and PlayerResource:GetSelectedHeroEntity(nPlayerID):GetTeamNumber() == nTeamNumber
			then
				local sPlayerSteamId = DotaMind:GetSteamID(nPlayerID)
	...
	local body =
		{ message = { appId = DotaMind.sAppId, signature = sSignature, data = data }, route = "wisp.score.calc" }
	local sSignature =
		md5.sumhexa(tostring(JSON:encode({ body = tostring(JSON:encode(body)), secret = DotaMind.sSecret })))
	...
	request:Send(function(result)
		...
		if result.StatusCode == 200 and result.Body ~= nil then
			...
			DotaMind.sLog = DotaMind.sLog
				.. "UpdateScore Fail: result.StatusCode"
				.. tostring(result.StatusCode)
				.. " result.Body"
				.. tostring(result.Body)
		end
