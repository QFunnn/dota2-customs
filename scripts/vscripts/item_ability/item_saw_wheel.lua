...
function item_saw_wheel:OnSpellStart()
	if IsServer() then
		...
		do
			...
			--если есть дерево — урон и оглушение
			if
				nTreeNumber > 0
				and not self:GetCaster():HasFlyMovementCapability()
				and DOTA_UNIT_CAP_MELEE_ATTACK == self:GetCaster():GetAttackCapability()
			then
		...
		if
			not self:GetCaster():HasFlyMovementCapability()
			and DOTA_UNIT_CAP_MELEE_ATTACK == self:GetCaster():GetAttackCapability()
		then
			GridNav:DestroyTreesAroundPoint(
				self:GetCaster():GetAbsOrigin(),
				self:GetSpecialValueFor("whirling_radius"),
				false
			)
		end
	end
...
function modifier_item_saw_wheel:IsDebuff()
	return false
end
function modifier_item_saw_wheel:IsHidden()
	return true
end
function modifier_item_saw_wheel:IsPurgable()
	return false
end
function modifier_item_saw_wheel:GetAttributes()
