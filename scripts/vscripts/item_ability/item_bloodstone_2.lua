...
function modifier_item_bloodstone_2:OnDeath(keys)
	if
		keys.unit:IsRealHero()
		and self.parent:IsRealHero()
		and not keys.unit:IsTempestDouble()
	...
	then
		...
		if
			self.parent:GetTeam() ~= keys.unit:GetTeam()
			and ((keys.unit:GetAbsOrigin() - self.parent:GetAbsOrigin()):Length2D() <= self.charge_range or self.parent == keys.attacker)
		...
		elseif
			self.parent == keys.unit
			and (not keys.unit.IsReincarnating or (keys.unit.IsReincarnating and not keys.unit:IsReincarnating()))
		then
...
function modifier_item_bloodstone_2:OnTakeDamage(keys)
	...
	then
		if
			...
			and keys.inflictor
			and bit:_and(keys.damage_flags, DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL)
			...
			if keys.unit:IsIllusion() then
				...
				then
					keys.damage = keys.original_damage
						* (1 - GetReductionFromArmor(keys.unit:GetPhysicalArmorValue(false)))
