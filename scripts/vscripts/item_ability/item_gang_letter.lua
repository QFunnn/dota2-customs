...
function modifier_item_gang_letter:GetModifierIncomingDamage_Percentage(params)
	...
	if IsServer() then
		if self.link_damage > 0 and fDamage > 0 then
			if IsValid(hAbility) and ---@cast hAbility CDOTABaseAbility
				hAbility:IsCooldownReady() and IsValid(hAttacker) and IsValid(hParent) then 
				local hp_max = 0
				local hUnit
	...
	if IsServer() then
		if IsValid(hParent) and IsValid(hAbility) and IsValid(hCaster) then 
			...
			if IsValid(hTarget) and hTarget:IsAlive() then
				...
				if hAbility:IsCooldownReady() and self.aura_bash_chance > 0 and RollPseudoRandomPercentage(self.aura_bash_chance, DOTA_PSEUDO_RANDOM_GENERIC_BASHER, hCaster) then
...
function modifier_item_gang_letter_effect:GetModifierPreAttack_CriticalStrike(params)
	if IsServer() then
		...
		if IsValid(hParent) and IsValid(hTarget) and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber() then
			if hTarget:IsFeared() or RollPseudoRandomPercentage(self.aura_crit_chance, DOTA_PSEUDO_RANDOM_ITEM_GREATERCRIT, hParent) then
