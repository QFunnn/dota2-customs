...
if item_gang_letter == nil then
	item_gang_letter = class({}) ---@class CDOTA_Item_Lua
end
...
if modifier_item_gang_letter == nil then
	modifier_item_gang_letter = class({}) ---@class CDOTA_Modifier_Lua
end
...
function modifier_item_gang_letter:GetAuraEntityReject(hEntity)
	...
	if
		(not hEntity:HasAttackCapability())
		and (not hEntity:HasMovementCapability())
		and (hEntity:GetUnitName() ~= "npc_dota_zeus_cloud")
	then
		return true
	end
				...
				if #units > 0 then
					for _, unit in pairs(units) do
						if
							IsValid(unit)
							and unit:IsAlive()
							and unit:HasModifier("modifier_item_gang_letter_effect")
						then
