...
if item_gang_letter == nil then
	item_gang_letter = class({}) ---@class item_gang_letter : CDOTA_Item_Lua
end
...
if modifier_item_gang_letter == nil then
	modifier_item_gang_letter = class({}) ---@class modifier_item_gang_letter : CDOTA_Modifier_Lua
end
...
function modifier_item_gang_letter:GetAuraEntityReject(entity)
	local parent = self:GetParent()
	if entity:GetUnitLabel() == "spirit_bear" then
		return false
	end
	if entity:IsHero() then
		return true
	end
	if parent:GetPlayerOwnerID() ~= entity:GetPlayerOwnerID() then
		return true
	end
	if (not entity:HasAttackCapability()) and (not entity:HasMovementCapability()) then
		return true
	end
	if entity:GetUnitName() == "npc_dota_zeus_cloud" then
		return true
	end
	if entity.by_chen_holy_persuasion then
		return true
	end
	return false
end
...
function modifier_item_gang_letter:OnCreated(params)
	...
	self.bonus_hp_regen_pct = ability:GetSpecialValueFor("bonus_hp_regen_pct")
	self.aura_radius = ability:GetSpecialValueFor("aura_radius")
	self.bonus_armor = ability:GetSpecialValueFor("bonus_armor")
	self.link_damage = ability:GetSpecialValueFor("link_damage")
end
...
function modifier_item_gang_letter:GetModifierIncomingDamage_Percentage(params)
	...
	if self.link_damage > 0 and fDamage > 0 then
		if
			IsValid(hAbility) ---@cast hAbility CDOTABaseAbility
			and hAbility:IsCooldownReady()
			and IsValid(hAttacker)
			and IsValid(hParent)
		then
			local hp_max = 0
			local hUnit
...
if modifier_item_gang_letter_effect == nil then
	modifier_item_gang_letter_effect = class({}) ---@class CDOTA_Modifier_Lua
end
...
function modifier_item_gang_letter_effect:OnIntervalThink()
	...
	local hParent = self:GetParent()
	if IsValid(hCaster) and IsValid(hParent) then ---@cast hCaster CDOTA_BaseNPC
...
function modifier_item_gang_letter_effect:GetModifierBaseDamageOutgoing_Percentage(params)
	local hCaster = self:GetCaster()
	if IsValid(hCaster) then ---@cast hCaster CDOTA_BaseNPC
		return hCaster:GetStrength() * self.aura_bonus_damage_str
			+ hCaster:GetIntellect(true) * self.aura_bonus_damage_int
			+ hCaster:GetAgility() * self.aura_bonus_damage_agi
			+ self.aura_bonus_damage_pct
	end
end
...
function modifier_item_gang_letter_effect:GetModifierSpellAmplify_Percentage(params)
	...
	if IsValid(hCaster) then ---@cast hCaster CDOTA_BaseNPC
		if IsServer() then
			if IsValid(hInflictor) and IsValid(hParent) then ---@cast hInflictor CDOTABaseAbility
				local sAbilityName = hInflictor:GetAbilityName()
				if GAMEPLAY_KV.NoSpellAmpAbilities and GAMEPLAY_KV.NoSpellAmpAbilities[sAbilityName] then
					return 0
				end
			end
		end
	...
	if IsValid(hParent) and IsValid(hAbility) and IsValid(hCaster) then
		...
		if IsValid(hTarget) and hTarget:IsAlive() then
			...
			if
				...
				and self.aura_bash_chance > 0
				and RollPseudoRandomPercentage(self.aura_bash_chance, DOTA_PSEUDO_RANDOM_GENERIC_BASHER, hCaster)
			then
				hTarget:AddNewModifier(hTarget, hAbility, "modifier_bashed", {
					duration = self.aura_bash_duration * hTarget:GetStatusResistanceFactor(hParent),
				})
		...
		if fDamage > 0 and self.aura_lifesteal > 0 then
			...
			local iParticleID2 = ParticleManager:CreateParticle(
...
function modifier_item_gang_letter_effect:GetModifierIncomingDamage_Percentage(params)
	...
	if GameRulesCustom:GetGameTime() > self.LastTime + self.aura_bonus_return_str_cd then
		if
			...
			and hAttacker:GetTeamNumber() ~= hParent:GetTeamNumber()
		then
			---@cast hCaster CDOTA_BaseNPC
			...
			self.LastTime = GameRulesCustom:GetGameTime()
		end
	end
end
...
function modifier_item_gang_letter_effect:GetModifierPreAttack(params)
	...
	local hParent = self:GetParent()
	self.accuracy = RollPseudoRandomPercentage(self.aura_accuracy_chance, DOTA_PSEUDO_RANDOM_ITEM_MKB, hParent)
end
...
function modifier_item_gang_letter_effect:GetModifierPreAttack_CriticalStrike(params)
	...
	if IsValid(hParent) and IsValid(hTarget) and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber() then
		if
			hTarget:IsFeared()
			or RollPseudoRandomPercentage(self.aura_crit_chance, DOTA_PSEUDO_RANDOM_ITEM_GREATERCRIT, hParent)
		then
