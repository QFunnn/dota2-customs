...
function modifier_item_cloud_razor:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if
		...
		and hTarget:IsAlive()
		and iDamageType == DAMAGE_TYPE_PHYSICAL
		and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber()
	then
		hTarget:AddNewModifier(
			hParent,
			hAbility,
			"modifier_item_cloud_razor_corruption",
			{ duration = self.corruption_duration * hTarget:GetStatusResistanceFactor(hParent) }
		)
	end
end
...
function modifier_item_cloud_razor_corruption:OnCreated(params)
	...
	if IsServer() then
		...
		if IsValid(hParent) and IsValid(hAbility) then
			...
			if IsValid(hAbility) then
				fArmor_reduction = fArmor_reduction
					+ hParent:GetPhysicalArmorValue(false)
						* self.corruption_armor_pct_charge
						* hAbility:GetCurrentCharges()
						* 0.01
			end
...
function modifier_item_cloud_razor_corruption:OnRefresh(params)
	...
	if IsServer() then
		...
		if IsValid(hParent) and IsValid(hAbility) then
			...
			if IsValid(hAbility) then
				fArmor_reduction = fArmor_reduction
					+ (hParent:GetPhysicalArmorValue(false) + self.armor_reduction)
						* self.corruption_armor_pct_charge
						* hAbility:GetCurrentCharges()
						* 0.01
			end
