...
function modifier_item_azure_song:OnCreated(params)
	...
	if IsServer() then
		...
		if self.total_spell_amp_pct_day > 0 then
			...
			local iParticleID =
				ParticleManager:CreateParticle("particles/items/star_design.vpcf", PATTACH_CUSTOMORIGIN_FOLLOW, hParent)
...
function modifier_item_azure_song:OnIntervalThink()
	...
	if IsValid(hParent) and IsValid(hAbility) then
		local iCharge = math.floor(
			hParent:GetIntellect(true) * self.spell_amp_per_int
				+ (hParent:GetAgility() + hParent:GetStrength()) * self.spell_amp_per_other
		)
		...
		if self.total_spell_amp_threshold > 0 then
			...
			self.total_bonus_spell_amp = math.floor(
				hParent:GetSpellAmplification(false) * 100 - (self.total_bonus_spell_amp or 0)
			) * iTotalBonusPct * 0.01
		end
	end
end
...
function modifier_item_azure_song:GetModifierSpellAmplify_Percentage(params)
	...
	if IsServer() then
		if IsValid(hInflictor) and IsValid(hParent) then
			...
			-- 巫医25级天赋特殊处理
			-- if sAbilityName == "witch_doctor_voodoo_restoration" then
			-- 	local doctorTalent = hParent:FindAbilityByName("special_bonus_unique_witch_doctor_2")
			-- 	if IsValid(doctorTalent) and doctorTalent:GetLevel() > 0 then

			-- 	end
			-- end
		end
		if IsValid(hTarget) and hTarget:IsRealHero() then
			fTotalBonusSpellAmp = fTotalBonusSpellAmp
				- fSpellAmp * (100 - self.hero_pct) * 0.01 * self:GetStackCount() * 0.01
...
function modifier_item_azure_song:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if
		...
		and not hInflictor:IsPassive()
		and IsValid(hAbility)
		and hTarget:IsAlive()
		and hTarget:GetTeamNumber() ~= hParent:GetTeamNumber()
	then
			...
			for _, unit in pairs(units) do
				if IsValid(unit) and unit:IsAlive() and not unit:IsDebuffImmune() then
					unit:AddNewModifier(
						hParent,
						hAbility,
						"modifier_item_azure_song_slow",
						{ duration = self.slow_duration * unit:GetStatusResistanceFactor(hParent) }
					)
