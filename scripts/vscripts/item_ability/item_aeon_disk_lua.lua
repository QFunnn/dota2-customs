...
function modifier_item_aeon_disk_lua:IsDebuff()
	return false
end
function modifier_item_aeon_disk_lua:IsPurgable()
	return false
end
...
function modifier_item_aeon_disk_lua:GetModifierTotal_ConstantBlock(keys)
	...
	if
		...
		and hParent:GetHealth() - keys.damage <= (health_threshold + 1)
...
function modifier_item_aeon_disk_lua_buff:OnDestroy() end
