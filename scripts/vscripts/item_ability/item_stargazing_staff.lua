...
LinkLuaModifier(
	"modifier_item_stargazing_staff_debuff",
	"item_ability/item_stargazing_staff.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_stargazing_staff_buff",
	"item_ability/item_stargazing_staff.lua",
	LUA_MODIFIER_MOTION_NONE
)

if item_stargazing_staff == nil then
	item_stargazing_staff = class({}) ---@class item_stargazing_staff : CDOTA_Item_Lua
end
...
function item_stargazing_staff:OnSpellStart()
	...
	if IsValid(hCaster) then
		local particleID =
			ParticleManager:CreateParticle("particles/items2_fx/veil_of_discord.vpcf", PATTACH_WORLDORIGIN, hCaster)
		...
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				unit:AddNewModifier(hCaster, self, "modifier_item_stargazing_staff_debuff", {
					duration = debuff_duration * unit:GetStatusResistanceFactor(hCaster),
				})
			end
		end
...
if modifier_item_stargazing_staff == nil then
	modifier_item_stargazing_staff = class({}) ---@class modifier_item_stargazing_staff : CDOTA_Modifier_Lua
end
	...
	if IsValid(self) then
		...
		if self == buffs[1] then
			if
				...
				and IsValid(hTarget)
				and hAttacker == hParent
				and (not hParent:HasModifier("modifier_item_stargazing_staff_cd"))
			then
...
function modifier_item_stargazing_staff:OnMagicalCrit(params)
	...
	if self == buffs[1] then
		if IsValid(hParent) and IsValid(hAttacker) and IsValid(hTarget) and hAttacker == hParent then
			--уходит во встроенный кулдаун
			if hAbility and params.crit_buff == self then
				if
					not (hTarget:HasModifier("modifier_item_stargazing_staff_debuff") and hTarget:IsNeutralUnitType())
				then
					hParent:AddNewModifier(hParent, hAbility, "modifier_item_stargazing_staff_cd", {
						duration = self.crit_cd * hParent:GetCooldownReduction(),
					})
				end
			end
...
function modifier_item_stargazing_staff_debuff:GetModifierIncomingDamage_Percentage(params)
	local hParent = self:GetParent()
	if
		params.damage_type == DAMAGE_TYPE_MAGICAL and not hParent:HasModifier("modifier_item_veil_of_discord_debuff")
	then
