...
LinkLuaModifier("modifier_item_stargazing_staff_debuff", "item_ability/item_stargazing_staff.lua",
...
function item_stargazing_staff:OnSpellStart()
	...
	if IsValid(hCaster) then
		local particleID = ParticleManager:CreateParticle("particles/items2_fx/veil_of_discord.vpcf", PATTACH_WORLDORIGIN,
			hCaster)
		...
		local units = FindUnitsInRadius(hCaster:GetTeamNumber(), vPos, nil, debuff_radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
...
function modifier_item_stargazing_staff:OnMagicalCrit(params)
	...
	if self == buffs[1] then
		if IsValid(hParent) and IsValid(hAttacker) and IsValid(hTarget) and hAttacker == hParent then
			...
			hParent:AddNewModifier(hParent, hAbility, "modifier_item_stargazing_staff_buff",
