...
if item_felling_axe_2_lua == nil then
	item_felling_axe_2_lua = class({}) ---@class item_felling_axe_2_lua : CDOTA_Item_Lua
end
...
if modifier_item_felling_axe_lua == nil then
	modifier_item_felling_axe_lua = class({}) ---@class modifier_item_felling_axe_lua : CDOTA_Modifier_Lua
end
...
function modifier_item_felling_axe_lua:OnCreated(params)
	...
	self.neutral_bonus_dmg_pct = ability:GetSpecialValueFor("neutral_bonus_dmg_pct")
end
...
function modifier_item_felling_axe_lua:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if self == buffs[1] then
		if
			not params.target:IsConsideredHero()
...
function modifier_item_felling_axe_lua:OnOrder(params)
	...
	local is_attack_order = params.order_type == DOTA_UNIT_ORDER_ATTACK_TARGET
		or params.order_type == DOTA_UNIT_ORDER_ATTACK_MOVE

	self.bonus_range_active = self == buffs[1]
		and is_attack_order
		and (
			params.order_type == DOTA_UNIT_ORDER_ATTACK_MOVE
			or (IsValid(params.target) and not params.target:IsConsideredHero())
		)
end
...
function modifier_item_felling_axe_lua:OnAttackStart(params)
	...
	if params.attacker == hParent then
		...
		if
			...
			and params.target.IsConsideredHero ~= nil
