...
function item_wild_moon:OnProjectileHit(hTarget, vLocation)
	...
	if IsValid(hTarget) and hTarget:IsAlive() then
		...
		hTarget:AddNewModifier(
			hCaster,
			self,
			"modifier_item_wild_moon_knockback",
			{ duration = knockback_duration * hTarget:GetStatusResistanceFactor(hCaster) }
		)
		ApplyDamage({
			victim = hTarget,
			ability = self,
...
function modifier_item_wild_moon:OnAttack(params)
	...
	if IsValid(hParent) and IsValid(hTarget) and IsValid(hAbility) and hAttacker == hParent then
		...
		local units = FindUnitsInRadius(
			...
			nil,
			hParent:Script_GetAttackRange() + hParent:GetHullRadius(),
		...
		table.sort(units, function(unitA, unitB)
			return (unitA:GetAbsOrigin() - hParent:GetAbsOrigin()):Length2D()
				< (unitB:GetAbsOrigin() - hParent:GetAbsOrigin()):Length2D()
		end)
...
function modifier_item_wild_moon_knockback:OnIntervalThink()
	...
	vParentPos.z = 0
	if
		(vCasterPos - vParentPos):Length2D() < math.min(self.max_knockback_distance, hCaster:Script_GetAttackRange())
	then
