...
LinkLuaModifier(
	"modifier_item_manifold_paradox_dagger_caster",
	"item_ability/item_manifold_paradox.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_manifold_paradox_dagger_debuff",
	"item_ability/item_manifold_paradox.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_manifold_paradox_strike_buff",
	"item_ability/item_manifold_paradox.lua",
...
function item_manifold_paradox:OnProjectileHit(hTarget, vLocation)
	...
	if IsValid(hCaster) and IsValid(hTarget) and hCaster:IsAlive() and hTarget:IsAlive() then
		hTarget:AddNewModifier(
			...
			self,
			"modifier_item_manifold_paradox_dagger_debuff",
			{ duration = dagger_duration }
		)
		local iAttackState = ATTACK_STATE_SKIPCOOLDOWN
			+ ATTACK_STATE_IGNOREINVIS
			+ ATTACK_STATE_NOT_USEPROJECTILE
			+ ATTACK_STATE_NEVERMISS
			+ ATTACK_STATE_NOT_USECASTATTACKORB
...
function item_manifold_paradox:OnSpellStart()
	...
	hCaster:AddNewModifier(
		...
		self,
		"modifier_item_manifold_paradox_strike_buff",
		{ duration = strike_duration, index = hTarget:entindex() }
	)
end
function item_manifold_paradox:LaunchDagger(hTarget)
	...
	if IsValid(hCaster) and IsValid(hTarget) then
		local tInfo = {
			Ability = self,
			EffectName = ParticleManager:GetParticleReplacement(
...
function modifier_item_manifold_paradox:GetModifierPreAttack_CriticalStrike(params)
	...
	if IsValid(hTarget) and IsValid(hParent) and hTarget:IsAlive() then
		if
			RandomFloat(0, 100) < self.crit_chance or hParent:HasModifier("modifier_item_manifold_paradox_strike_buff")
		then
			if IsServer() then
		...
		if IsServer() then
			...
			if self.tRecords[record] ~= nil then
				...
				ParticleManager:SetParticleControlForward(
					particleID,
					1,
					(hParent:GetAbsOrigin() - hTarget:GetAbsOrigin()):Normalized()
				)
