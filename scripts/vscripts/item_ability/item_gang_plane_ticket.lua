...
function item_gang_plane_ticket:OnSpellStart()
	...
	if IsValid(hCaster) and vLocation then
		...
		local fRange = math.min(self:GetCastRange(vLocation, nil), (vLocation - hCaster:GetAbsOrigin()):Length2D())
		...
		local iParticleID =
			ParticleManager:CreateParticle("particles/items_fx/blink_dagger_start.vpcf", PATTACH_CUSTOMORIGIN, hCaster)
		...
		for _, unit in pairs(units) do
			if IsValid(unit) and unit:IsAlive() then
				...
				unit:AddNewModifier(
					hCaster,
					self,
					"modifier_item_gang_plane_ticket",
		...
		EmitSoundOnLocationWithCaster(vLocation, "Hero_DarkWillow.Fear.Location", hCaster)
	end
end
...
if modifier_item_gang_plane_ticket == nil then
	modifier_item_gang_plane_ticket = class({})
end
