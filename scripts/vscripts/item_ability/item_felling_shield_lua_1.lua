...
function modifier_item_felling_shield_lua_shield:GetModifierIncomingPhysicalDamageConstant(params)
	...
	if IsServer() then
		if IsValid(hParent) and IsValid(hAttacker) then
			if hParent:GetTeamNumber() ~= hAttacker:GetTeamNumber() and hAttacker:IsNeutralUnitType() and bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
				local damage = params.damage or 0
