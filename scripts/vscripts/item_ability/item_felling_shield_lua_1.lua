...
function modifier_item_felling_shield_lua_1:GetModifierProcAttack_BonusDamage_Physical(params)
	...
	if self == buffs[1] then
		if
			...
			and not hTarget:HasState(MODIFIER_STATE_DEBUFF_IMMUNE)
		then
			if self.feedback_mana_burn > 0 then
				...
				if mana_burned > 0 then
					...
					Wearable_System:PlayItemEffect(
		...
		then
			...
			if damage > 0 then
				...
				hParent:AddNewModifier(
					...
					self:GetAbility(),
					"modifier_item_felling_shield_lua_shield",
					{ duration = self.shield_duration, stack = stack }
				)
			end
...
function modifier_item_felling_shield_lua_shield:GetModifierIncomingPhysicalDamageConstant(params)
	...
	local hAttacker = params.attacker
	if IsServer() then
		if IsValid(hParent) and IsValid(hAttacker) then
			if
				hParent:GetTeamNumber() ~= hAttacker:GetTeamNumber()
				and hAttacker:IsNeutralUnitType()
