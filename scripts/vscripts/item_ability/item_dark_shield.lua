...
function modifier_item_dark_shield:GetModifierPhysicalArmorBonus(params)
	...
	return self.bonus_armor
		+ hParent:GetModifierStackCount("modifier_item_dark_shield_stack", hParent) * self.stack_armor
end
...
function modifier_item_dark_shield:GetModifierIncomingPhysicalDamage_Percentage(params)
	...
	if
		...
		and hCaster:GetTeamNumber() ~= hAttacker:GetTeamNumber()
	then
		...
		if iStack > 0 then
			hCaster:AddNewModifier(
				hCaster,
				hAbility,
				"modifier_item_dark_shield_stack",
				{ duration = self.stack_duration, stack = iStack }
			)
		...
		then
			if iDamageCategory == DOTA_DAMAGE_CATEGORY_ATTACK then
				if self.lifesteal_percent > 0 then
					...
					if flAmount >= 1 then
						...
						local fOverHeal =
							math.floor(self.heal_to_shield * 0.01 * (flAmount - hParent:GetHealthDeficit()))
						if fOverHeal >= 1 then
							hParent:AddNewModifier(
								hParent,
								hAbility,
								"modifier_item_foulfell_power_shield",
								{ duration = self.shield_duration, stack = fOverHeal }
							)
						end
					end
				end
			elseif iDamageCategory == DOTA_DAMAGE_CATEGORY_SPELL then
				if self.spell_lifesteal > 0 then
					-- 技能吸血
					if
						bit.band(iDamageFlags, DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION)
							~= DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION
						and bit.band(iDamageFlags, DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL)
					...
					then
						...
						if not hTarget:IsHero() then
							fLifestealPercent = fLifestealPercent
								* (100 - self.creep_spell_lifesteal_reduction_pct)
								* 0.01
						end
						local flAmount = fDamage * fLifestealPercent * 0.01
						...
						if flAmount >= 1 then
							...
							if fOverHeal >= 1 then
								hParent:AddNewModifier(
									hParent,
									hAbility,
									"modifier_item_bloodstone_3_shield",
									{ duration = self.shield_duration, stack = fOverHeal }
								)
							end
						end
