			...
			if IsValid(unit) and unit:IsAlive() then
				if not unit:TriggerSpellAbsorb(self) then
					if IsValid(unit) then
						local iParticleID = ParticleManager:CreateParticle("particles/generic_gameplay/generic_manaburn.vpcf", PATTACH_ABSORIGIN_FOLLOW, unit)
...
function modifier_item_felling_shield_lua_3_slow:OnCreated(params)
	...
	if IsServer() then
		...
		if IsValid(hParent) and self then
			local iParticleID = ParticleManager:CreateParticle("particles/items_fx/diffusal_slow.vpcf", PATTACH_ABSORIGIN_FOLLOW, hParent)
