...
LinkLuaModifier(
	"modifier_item_felling_shield_lua_3_slow",
	"item_ability/item_felling_shield_lua_3.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_felling_shield_lua_3_buff",
	"item_ability/item_felling_shield_lua_3.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_item_felling_shield_lua_3_root",
	"item_ability/item_felling_shield_lua_3.lua",
	LUA_MODIFIER_MOTION_NONE
)
--Abilities
...
function item_felling_shield_lua_3:OnSpellStart()
	...
	if IsValid(hCaster) then
		...
		local units = FindUnitsInRadius(
			...
			nil,
			self:GetCastRange(hCaster:GetAbsOrigin(), nil),
				...
				if not unit:TriggerSpellAbsorb(self) then
					...
					if not unit:IsConsideredHero() then
						unit:AddNewModifier(
							...
							self,
							"modifier_item_felling_shield_lua_3_root",
							{ duration = purge_root_duration * unit:GetStatusResistanceFactor(hCaster) }
						)
					end
					unit:AddNewModifier(
						hCaster,
						self,
						"modifier_item_felling_shield_lua_3_slow",
						{ duration = purge_slow_duration * unit:GetStatusResistanceFactor(hCaster) }
					)
				end
			end
		end
