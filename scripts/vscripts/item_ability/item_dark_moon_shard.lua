...
function item_dark_moon_shard:OnSpellStart()
	...
	if
		(hCaster:IsRealHero() or string.find(hCaster:GetUnitName(), "npc_dota_lone_druid_bear") ~= nil)
...
function modifier_item_dark_moon_shard:OnCreated(kv)
	self.base_attack_time_reduction = tonumber(kv.base_attack_time_reduction)
		or self:GetAbilitySpecialValueFor("base_attack_time_reduction")
		or 30
	self.base_attack_time_min = tonumber(kv.base_attack_time_min)
		or self:GetAbilitySpecialValueFor("base_attack_time_min")
		or 1.1
	if IsServer() then
		...
		if tKv.AttackRate then
			...
			self.AttackRate =
				math.max(self.base_attack_time_min, AttackRate * (100 - self.base_attack_time_reduction) * 0.01)
		else
			self:Destroy()
		end
