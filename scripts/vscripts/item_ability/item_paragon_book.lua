		...
		if hCaster and hCaster:IsRealHero() and not hCaster:IsTempestDouble() and not hCaster:HasModifier("modifier_arc_warden_tempest_double_lua") and hCaster:GetUnitLabel() ~= "spirit_bear" then
			...
			if hCaster.bTakenOverByBot then
				...
				if hPlayer then
					...
					HeroBuilder.BonusAbilityNumber[hPlayer:GetPlayerID()] = HeroBuilder.BonusAbilityNumber[hPlayer:GetPlayerID()] + 1
					HeroBuilder.totalAbilityNumber[nPlayerID] = HeroBuilder.BasicAbilityNumber[nPlayerID] + HeroBuilder.BonusAbilityNumber[nPlayerID]
