	...
	if IsValid(hParent) and IsValid(hAbility) and IsValid(hTarget) and hParent == hAttacker then
		if hTarget:GetTeamNumber() ~= DOTA_TEAM_NEUTRALS then
			hParent:AddNewModifier(
				hParent,
				hAbility,
				"modifier_item_last_laugh_disable",
				{ duration = self.disable_duration }
			)
		end
	end
end
function modifier_item_last_laugh:GetModifierProcAttack_Feedback(params)
	...
	then
		...
		if self.crit_chance > 0 then
			local iParticleID =
				ParticleManager:CreateParticle("particles/items/flame_reaper.vpcf", PATTACH_CUSTOMORIGIN, hParent)
...
function modifier_item_last_laugh:GetModifierDamageOutgoing_Percentage(params)
	...
	if IsServer() then
		...
		if IsValid(hTarget) and IsValid(hParent) then
			if
				hTarget:GetTeamNumber() ~= DOTA_TEAM_NEUTRALS
				and not hParent:HasModifier("modifier_item_last_laugh_disable")
			then
				hParent:AddNewModifier(
					hParent,
					hAbility,
