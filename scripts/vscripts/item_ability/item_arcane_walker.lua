...
LinkLuaModifier(
	"modifier_item_arcane_walker_magical_armor_pen",
	"item_ability/item_arcane_walker.lua",
...
function item_arcane_walker:OnSpellStart()
	...
	if IsValid(hCaster) then
		...
		local iParticleID2 =
			ParticleManager:CreateParticle("particles/items_fx/arcane_boots.vpcf", PATTACH_ABSORIGIN_FOLLOW, hCaster)
...
function modifier_item_arcane_walker:GetModifierTotalDamageOutgoing_Percentage(params)
	...
	if
		IsValid(hParent)
		and IsValid(hTarget)
		and IsValid(hInflictor)
		and IsValid(hAbility)
		and not hParent:IsIllusion()
		and hInflictor ~= hAbility
		and hTarget:IsAlive()
		and iDamageType == DAMAGE_TYPE_MAGICAL
...
function modifier_item_arcane_walker:OnDeath(params)
	...
	if
		...
		and IsValid(hParent)
		and IsValid(hUnit)
		and hParent:IsRealHero()
		and hUnit:IsRealHero()
		and hAttacker:GetTeamNumber() ~= hUnit:GetTeamNumber()
	then
...
function modifier_item_arcane_walker:OnTakeDamage(params)
	...
	if IsServer() then
		if
			IsValid(hAbility)
			and IsValid(hInflictor)
			and self.bonus_spell_lifesteal > 0
			and fDamage > 0
			and IsValid(hParent)
			and IsValid(hTarget)
			and hParent == hAttacker
			and hParent:GetTeamNumber() ~= hTarget:GetTeamNumber()
		then
			local sAbilityName = hInflictor:GetAbilityName()
			local fSpellLifestealPct = self.bonus_spell_lifesteal
			if
				PERCENTAGE_ABILITIES and PERCENTAGE_ABILITIES[sAbilityName]
				or (sAbilityName == "enigma_black_hole" and hParent:HasScepter())
				or (
					sAbilityName == "witch_doctor_voodoo_restoration"
					and hParent:HasTalent("special_bonus_unique_witch_doctor_2")
				)
			then
			else
...
function modifier_item_arcane_walker_magical_armor_pen:GetModifierMagicalResistanceBonus(params)
	...
	if IsServer() then
		if
			IsValid(hInflictor)
			and IsValid(hAbility)
			and hAbility ~= hInflictor
			and hInflictor:GetCaster() == hCaster
		then
			local sAbilityName = hInflictor:GetAbilityName()
			if
				PERCENTAGE_ABILITIES and PERCENTAGE_ABILITIES[sAbilityName]
				or (sAbilityName == "enigma_black_hole" and hCaster:HasScepter())
				or (
					sAbilityName == "witch_doctor_voodoo_restoration"
					and hCaster:HasTalent("special_bonus_unique_witch_doctor_2")
				)
			then
				return 0
			else
