...
function modifier_item_ranged_cleave:GetEnemiesCached(attacker, center, radius)
	...
	if
		...
		and moved_sq <= shift_threshold_sq
	then
...
function modifier_item_ranged_cleave:GetModifierProcAttack_Feedback(keys)
	...
	if not attacker:IsRealHero() or not attacker:IsRangedAttacker() then
		return
	end
	if target:IsBuilding() or attacker:GetTeam() == target:GetTeam() then
		return
	end
