...
local a = "units/full_body/portrait_courier_base"
...
function Spawn(self, d)
	local e = thisEntity:GetUnitName()
	...
	do
		...
		while u <= 9 do
			...
			if B ~= nil then
				...
				local C = KeyValues.ItemsGame[tostring(A)]
				if C ~= nil then
	...
	local D = SpawnEntityListFromTableSynchronous(j)
	if f.ambient ~= nil then
		ParticleManager:CreateParticle(f.ambient, PATTACH_ABSORIGIN_FOLLOW, D[1])
	end
end