...
local a = "units/portraits/abyssal_underlord_v2"
...
function Spawn(self, e)
	...
	do
		...
		while v <= 9 do
			...
			if C ~= nil then
				...
				local D = KeyValues.ItemsGame[tostring(B)]
				if D ~= nil then
	...
	if l.PortraitParticle ~= nil then
		local E = ParticleManager:CreateParticle(l.PortraitParticle, PATTACH_ABSORIGIN_FOLLOW, thisEntity)
	end
end