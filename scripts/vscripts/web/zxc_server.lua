...
function Server:Init()
	...
	CustomGameEventManager:RegisterListener("PlayerSettings", function(source, event)
		self:OnPlayerChangeSettings(event)
	end)

	CustomGameEventManager:RegisterListener("custom_keybind_changed", function(source, event)
		self:OnPlayerChangeKeyBind(event)
	end)
	CustomGameEventManager:RegisterListener("custom_keybind_quickcast_changed", function(source, event)
		self:OnPlayerChangeKeyBindQuickcast(event)
	end)

	CustomGameEventManager:RegisterListener("server_update_notification_filter_settings", function(source, event)
		self:OnPlayerChangeNotificationFilter(event)
	end)

	CustomGameEventManager:RegisterListener("server_get_rating_season_info", function(source, event)
		self:OnPlayerWantGetRatingSeasonInfo(event)
	end)

	CustomGameEventManager:RegisterListener("server_on_player_use_promocode", function(source, event)
		self:OnPlayerAttemptUsePromocode(event)
	end)

	CustomGameEventManager:RegisterListener("server_on_player_add_to_favorite_ban", function(source, event)
		self:OnPlayerAddFavoriteBan(event)
	end)

	CustomGameEventManager:RegisterListener("server_buy_item", function(source, event)
		self:OnPlayerWantBuyItem(event)
	end)

	CustomGameEventManager:RegisterListener("server_wear_item", function(source, event)
		self:OnPlayerWantWearItem(event)
	end)

	CustomGameEventManager:RegisterListener("BattlePassBuy", function(source, event)
		self:BattlePassBuy(event)
	end)

	CustomGameEventManager:RegisterListener("bug_report_submit", function(source, event)
		self:OnBugReportSubmit(event)
	end)

	CustomGameEventManager:RegisterListener("player_report_submit", function(source, event)
		self:OnPlayerReportSubmit(event)
	end)
	CustomGameEventManager:RegisterListener("player_report_open", function(source, event)
		self:OnPlayerReportOpen(event)
	end)

	CustomGameEventManager:RegisterListener("mute_prefs_changed", function(source, event)
		self:OnPlayerChangeMutePrefs(event)
	end)
	CustomGameEventManager:RegisterListener("mute_prefs_all_changed", function(source, event)
		self:OnPlayerChangeMutePrefsAll(event)
	end)
end
...
function Server:CreatePlayerProfile(data, PlayerID, SteamID)
	...
	if data.settings then
		local settings_table = {
			...
			-- Маркер «это настройки ИЗ БД, а не стартовые дефолты». Панорама (settings.js)
			-- по нему понимает, что меню можно заполнять и что отправка изменений разрешена:
			-- до ответа сервера в setting_data лежат дефолты, и отправка их обратно затирала
			-- бы сохранённое игроком. Сервер лишнего поля не ждёт и игнорирует его.
			profile_loaded = 1,
		}
	...
	print(
		string.format(
			"[CREEP-SETTING] PID=%s from_api=%s (%s) stored=%s",
			tostring(PlayerID),
			tostring(_dbgApi),
			type(_dbgApi),
			tostring(_dbgStored)
...
function Server:OnPlayerChangeMutePrefs(event)
	...
	if PlayerID == nil or not self.Players[PlayerID] then
		return
	end

	local TargetPlayerID = event.target_player_id
	if TargetPlayerID == nil or not PlayerResource:IsValidPlayerID(TargetPlayerID) then
		return
	end
	...
	if target_steamid == nil or target_steamid <= 0 then
		return
	end
...
function Server:CreatePlayerData(PlayerID, Place)
	...
	if PlayerBuilderInfo == nil or PlayerMainInfo == nil then
		return
	end
	...
	if Rounds:IsLastDuelActive() then
		...
		CustomNetTables:SetTableValue(
			...
			tostring(PlayerID),
			{
				original_rating = self.Players[PlayerID].profile.rating,
				new_rating = self.Players[PlayerID].profile.rating + PlayerPreRating,
			}
		)
	end
...
function Server:SendDataToServer(TeamID, bWin)
	...
	if Players:GetTeam(TeamID) == nil then
		return
	end
	...
	if SERVER_CHECK_CONDITIONS then
		if self:GetPlayersCount() <= SERVER_CONDITIONS_TO_COUNT_GAME.PLAYERS then
			return
		end

		local GameMinutes = (GameRules:GetDOTATime(false, false) / 60)
		if GameMinutes < SERVER_CONDITIONS_TO_COUNT_GAME.MINUTES then
			return
		end
	end
	...
	for PID, PlayerInfo in pairs(self.Players) do
		if PlayerInfo.end_data.SteamID ~= nil then
			...
			elseif
				PlayerInfo.end_data.player_data
				and PlayerInfo.end_data.player_data.rating_change ~= nil
				and Rounds:IsLastDuelActive()
				and PlayerInfo.end_data.player_data.place <= 2
			then
				PlayerInfo.end_data.player_data.rating_change = self:AbsRating(
					self:CalculateRatingByGame(
						PID,
						PlayerInfo.end_data.player_data.rating_change,
				...
				CustomNetTables:SetTableValue(
					...
					tostring(PID),
					{
						original_rating = PlayerInfo.profile.rating,
						new_rating = PlayerInfo.profile.rating + PlayerInfo.end_data.player_data.rating_change,
					}
				)
			end
...
function Server:GetRatingTop(SeasonID, bSendToPanorama)
		...
		SendRequest(SERVER_URL .. "get_rating_top", { SeasonID = SeasonID }, function(Res)
			...
			if bSendToPanorama then
				CustomGameEventManager:Send_ServerToAllClients(
					"server_send_rating_season_info",
					{ season = SeasonID, top = Res }
				)
			end
		end, true)
	elseif bSendToPanorama then
		CustomGameEventManager:Send_ServerToAllClients(
			"server_send_rating_season_info",
			{ season = SeasonID, top = self.RatingTops[SeasonID] }
		)
	end
end
...
function Server:OnPlayerWantBuyItem(event)
	...
	if ItemInfo == nil or ItemInfo.buyable == false or self:PlayerHasItem(PlayerID, ItemName) then
		return
	end
	...
	if not self.Players[PlayerID] or self.Players[PlayerID].server_connected == false then
		if Player then
			CustomGameEventManager:Send_ServerToPlayer(
				Player,
				"menu_special_notification",
				{ text = "#bp_error_server_disconnect", class = "Error", type = 1 }
			)
		end
		return
	end
	...
	if self.Players[PlayerID].profile.coins >= Price then
		...
		SendRequest(SERVER_URL .. "on_buy_item", Data, function(Res)
			...
			if Player then
				CustomGameEventManager:Send_ServerToPlayer(
					Player,
					"menu_special_notification",
					{ text = "#bp_accept", class = "Success", type = 2, duration = 1 }
				)
			end
		end, true)
	else
		if Player then
			CustomGameEventManager:Send_ServerToPlayer(
				Player,
				"menu_special_notification",
				{ text = "#bp_error_no_arena_coin", class = "Error", type = 1 }
			)
		end
	end
end
...
function Server:OnPlayerWantWearItem(event)
	...
	if ItemInfo == nil or not self:PlayerHasItem(PlayerID, ItemName) then
		return
	end
	...
	if SlotName ~= "NONE" and self.Players[PlayerID] and self.Players[PlayerID].player_items_slots then
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"PLAYER_INVENTORY_ITEM_WEAR_UPDATE",
			{ PlayerID = PlayerID, SlotName = SlotName }
		)
...
function Server:UpdatePlayerCosmetics(PlayerID, SlotName, ItemName)
	...
	for _, Unit in ipairs(Units) do
		if
			Owner
			and not Owner:IsNull()
			and Unit
			and not Unit:IsNull()
			and table.contains(TypesToUpdate, ItemSlotType)
		then
	...
	SendRequest(SERVER_URL .. "on_use_promocode", Data, function(Res)
		if Res.status == 1 then
			...
			CustomGameEventManager:Send_ServerToPlayer(
				Player,
				"menu_special_notification",
				{ text = "#promocode_error", class = "Error", type = 1 }
			)
		end
	end, true)
end
...
function Server:OnPlayerAddFavoriteBan(event)
	...
	if self.Players[PlayerID][BanCategory] == nil then
		return
	end
...
function Server:BattlePassBuy(event)
	...
	self:OnAttemptBuyBattlePass(
		PlayerID,
		store_system.BattlePassDuration[duration_type],
		store_system.BattlePassPrice[duration_type]
	)
end
...
function Server:OnPlayerChangeSettings(event)
	...
	if settings then
		...
		local highlight_fields = {
			"highlight_master_enabled",
			"highlight_sss_enabled",
			"highlight_modified_enabled",
			"highlight_use_categories",
			"color_default_sss",
			"color_modified",
			"color_forms",
			"color_splashes",
...
function Server:GetPlayerSettingValue(PlayerID, SettingName)
	if not self.Players[PlayerID] or not self.Players[PlayerID].settings then
		return 0
	end
...
function Server:GetPlayerRating(PlayerID)
	if not self.Players[PlayerID] or not self.Players[PlayerID].profile then
		return 3000
	end
...
function Server:IsPlayerBattlePassSubscribed(PlayerID)
	...
	if
		self.Players[PlayerID].profile.battle_pass ~= nil
		and self.Players[PlayerID].profile.battle_pass.status == true
	then
		return true
	end
...
function Server:IsPlayerPenguinSoundEnabled(PlayerID)
	...
	if self.Players[PlayerID].settings == nil then
		return true
	end
...
function Server:OnBugReportSubmit(event)
	...
	print(
		"[BugReport] SERVER RECEIVED: PlayerID="
			.. tostring(PlayerID)
			.. ", Type="
			.. bugType
			.. ", Desc length="
	...
	-- Отправляем на наш сервер, он положит сообщение в Discord (вебхук хранится там).
...
function Server:SendBugReportToServer(
	playerName,
	steamID,
	bugType,
	description,
	...
	-- Discord-вебхук здесь держать НЕЛЬЗЯ: vscripts мода едут игрокам внутри VPK кастомки,
	-- то есть любой URL в этом файле публичен. Шлём данные на наш сервер (аутентификация —
	-- GameKey внутри SendRequest), а embed собирает и отправляет D2_Server (server/discord.js).
	SendRequest(SERVER_URL .. "on_bug_report", {
...
--------------------------------------------------------------------------------
-- РЕПОРТ НА ИГРОКА (жалоба) → наш сервер (он пишет в БД и шлёт в Discord).
--------------------------------------------------------------------------------
...
function Server:OnPlayerReportOpen(event)
	local PlayerID = event.PlayerID
	print(
		"[PlayerReport] OnPlayerReportOpen: pid="
			.. tostring(PlayerID)
			.. " target="
			.. tostring(event.target_player_id)
	)
	if PlayerID == nil then
		return
	end
	...
	if
		TargetPlayerID == nil
		or not PlayerResource:IsValidPlayerID(TargetPlayerID)
		or TargetPlayerID == PlayerID
		or PlayerResource:GetTeam(TargetPlayerID) == 1
	then
		print(
			"[PlayerReport] OnPlayerReportOpen REJECT: target="
				.. tostring(TargetPlayerID)
				.. " valid="
				.. tostring(TargetPlayerID ~= nil and PlayerResource:IsValidPlayerID(TargetPlayerID))
				.. " team="
				.. tostring(TargetPlayerID ~= nil and PlayerResource:GetTeam(TargetPlayerID))
		)
		return
	end
...
function Server:OnPlayerReportSubmit(event)
	...
	if targetHero then
		targetHeroName = targetHero:GetUnitName()
	end
	...
	-- Репорт уходит одним запросом на наш сервер: он пишет копию в БД и сам постит
	-- в Discord (вебхук хранится на сервере — в моде его держать нельзя, VPK публичен).
	-- Ники и текст причины знает только мод, поэтому шлём их вместе с данными.
	SendRequest(SERVER_URL .. "on_player_report", {
		...
		reason = reason,
		reason_text = (PLAYER_REPORT_REASONS and PLAYER_REPORT_REASONS[reason]) or reason,
		description = description,
		hero = targetHeroName,
		round = currentRound,
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		"player_report_response",
		{ success = true }
	)
end

if not Server.bStarted then
	Server:Init()
end