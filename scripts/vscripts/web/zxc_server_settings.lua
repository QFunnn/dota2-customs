...
-- Discord-вебхуков здесь БОЛЬШЕ НЕТ и быть не должно: vscripts мода уезжают игрокам
-- внутри VPK кастомки, поэтому любой URL в этих файлах публичен — кто угодно распакует
-- VPK и получит право писать в наш Discord. Багрепорты и жалобы уходят обычным
-- SendRequest на наш сервер (GameKey), а он уже постит в Discord: D2_Server/server/discord.js,
-- ключи config.json → discord_bug_report_webhook / discord_player_report_webhook.
