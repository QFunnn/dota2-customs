		...
		if player.high_five_cooldown and player.high_five_cooldown > 0 then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"PauseNotification",
					{ message = "#cha_highfive_cooldown", time = player.high_five_cooldown }
				)
			end
			return
		end
		player.high_five_cooldown = 15
		local modifier_cha_high_cooldown =
			hero:AddNewModifier(hero, nil, "modifier_cha_high_cooldown", { duration = player.high_five_cooldown })
...
function store_system:PauseDisconnectPlayer(id)
	...
	if hero and not hero:IsNull() and hero:IsAlive() then
		if
			store_system.PlayerLeavePauseCooldown[id]
			and store_system.PlayerLeavePauseCooldown[id] <= 0
		...
		then
			...
			Timers:CreateTimer({
				...
				endTime = 1,
				callback = function()
					if store_system.PauseCooldown <= 0 then
						return
					end
					if
						PlayerResource:GetConnectionState(id) ~= nil
						and (
							PlayerResource:GetConnectionState(id) == DOTA_CONNECTION_STATE_ABANDONED
							or PlayerResource:GetConnectionState(id) == DOTA_CONNECTION_STATE_UNKNOWN
							or PlayerResource:GetConnectionState(id) == DOTA_CONNECTION_STATE_CONNECTED
						)
					then
						PauseGame(false)
			...
			Timers:CreateTimer({
				...
				endTime = 1,
				callback = function()
					if store_system.PlayerLeavePauseCooldown[id] <= 0 then
						return nil
					end
