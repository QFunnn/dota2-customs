...
function arena_system:StartGameOverthrow()
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"set_arena_timer",
		{ time = time, full_time = STARTGAME_DURATION_OVERTHROW, relax = true, duel = false }
	)
...
function arena_system:StartEndTimeOverthrow()
	...
	CustomGameEventManager:Send_ServerToAllClients(
		...
		{ time = time, full_time = FULL_TIME_OVERHROW }
	)
...
function arena_system:SpawnSphereBlue(point)
	...
	capture_point.orb_fx =
		ParticleManager:CreateParticle("particles/orb_rare.vpcf", PATTACH_ABSORIGIN_FOLLOW, capture_point)
end
...
function arena_system:SpawnSphereRed(point)
	...
	capture_point.orb_fx =
		ParticleManager:CreateParticle("particles/orb_rare_red.vpcf", PATTACH_ABSORIGIN_FOLLOW, capture_point)
end
...
function arena_system:SpawnDummyOverthrow()
	Timers:CreateTimer(0.03, function()
		...
		local npc_dota_hero_target_dummy = CreateUnitByName(
			"npc_dota_hero_target_dummy",
			Vector(-265.921, -712.218, 43.3432),
			true,
			nil,
			nil,
			hHero:GetTeamNumber() == DOTA_TEAM_GOODGUYS and DOTA_TEAM_BADGUYS or DOTA_TEAM_GOODGUYS
		)
		...
		DebugCreateUnit(hPlayer, "npc_dota_hero_pudge", DOTA_TEAM_BADGUYS, false, function(hEnemy)
		...
		DebugCreateUnit(hPlayer, "npc_dota_hero_axe", DOTA_TEAM_CUSTOM_2, false, function(hEnemy)
			hEnemy:SetControllableByPlayer(0, false)
			hEnemy:SetRespawnPosition(pos2)
			FindClearSpaceForUnit(hEnemy, pos2, false)
			hEnemy:Hold()
