...
function player_system:RegisterPlayerInfo(id, hero)
	if not IsInToolsMode() then
		if not PlayerResource:IsValidPlayerID(id) then
			return
		end
		if tostring(PlayerResource:GetSteamAccountID(id)) == nil then
			return
		end
		if PlayerResource:GetSteamAccountID(id) == 0 then
			return
		end
		if PlayerResource:GetSteamAccountID(id) == "0" then
			return
...
function player_system:DisconnectPlayerChecked(team_check)
	...
	for player_id, player_info in pairs(PLAYERS) do
		if
			PlayerResource:GetConnectionState(player_id) ~= nil
			and (
				PlayerResource:GetConnectionState(player_id) == DOTA_CONNECTION_STATE_ABANDONED
				or PlayerResource:GetConnectionState(player_id) == DOTA_CONNECTION_STATE_UNKNOWN
			)
		then
...
function player_system:NetWorthUpdate()
	...
	for id, info in pairs(PLAYERS) do
		if info.hero ~= nil then
			...
			if get_enchantment then
				local tier_counter =
					neutrals_reward:GetNeutralTier(get_enchantment:GetAbilityName(), get_enchantment:GetLevel())
				networths = networths + (neutrals_reward.NEUTRALS_COST[tier_counter] or 0)
			end

			CustomGameEventManager:Send_ServerToAllClients(
				"event_update_scoreboard_data",
				{ table_name = "playernetworths_end", player_id = id, data = { networths = networths } }
			)
			...
			CustomGameEventManager:Send_ServerToAllClients(
				"event_update_scoreboard_data",
				{ table_name = "playernetworths", player_id = id, data = { networths = networths } }
			)
		end
	end
	...
	for team, netw in pairs(networth_team) do
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"event_update_scoreboard_data",
			{ table_name = "playernetworths_team", player_id = team, data = { networths = netw } }
		)
	end
...
function player_system:GetPlayerLastNetworths()
	...
	for count = #playerstable, 1, -1 do
		if
			playerstable[count]
			and (
				self:IsLose(playerstable[count].id)
				or PlayerResource:GetConnectionState(playerstable[count].id) == DOTA_CONNECTION_STATE_ABANDONED
			)
		then
	...
	table.sort(playerstable, function(x, y)
		return y.networths < x.networths
	end)
	...
	if not arena_system:DuelActive() then
		CustomGameEventManager:Send_ServerToAllClients(
			"event_update_scoreboard_data",
			{ table_name = "duelplayers", data = result }
		)
	end
	...
	for count = #playerstable, 1, -1 do
		if playerstable[count] then
			...
			for _, team_player in pairs(playerstable[count].players) do
				if
					not self:IsLose(team_player)
					and PlayerResource:GetConnectionState(team_player) ~= DOTA_CONNECTION_STATE_ABANDONED
				then
					team_is_lose = false
				end
			end
...
function player_system:GetPlayerPositionLevels(player_id)
	...
	table.sort(playerstable, function(x, y)
		return y.hero:GetLevel() < x.hero:GetLevel()
	end)
...
function player_system:SetLose(id)
	if (8 - player_system:GetPlayerCount()) < arena_system:GetArena() then
		if
			self:GetPlayerCountAll() >= 7
			and not GameRules:IsCheatMode()
			and (
				player_system.PLAYERS_GLOBAL_INFORMATION[id]
				and player_system.PLAYERS_GLOBAL_INFORMATION[id].last_leave == 1
			)
		then
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"notification_duel_lose",
			{ id = id, heroname = PLAYERS[id].hero:GetUnitName() }
		)
	else
			...
			Timers:CreateTimer({
				...
				callback = function()
					...
					if PLAYERS[id].hero and PLAYERS[id].hero.fake_dazzle then
						local wardmodifier_dazzle =
							PLAYERS[id].hero.fake_dazzle:FindModifierByName("modifier_item_ward_dispenser_custom_ward")
...
function player_system:RegisterWebData(player_id)
	...
	if tostring(PlayerResource:GetSteamAccountID(player_id)) == nil then
		return
	end
	if PlayerResource:GetSteamAccountID(player_id) == 0 then
		return
	end
	if PlayerResource:GetSteamAccountID(player_id) == "0" then
		return
	...
	local set_data = function(data, id)
		...
		CustomGameEventManager:Send_ServerToAllClients(
			"event_update_scoreboard_data",
			{
				table_name = "reported_info",
				player_id = id,
				data = { reported_info = new_player_information.players_repoted },
			}
		)
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "tip_cooldown", player_id = player_id, data = { cooldown = 0 } }
	)

	RequestData(
		"https://"
			.. player_system.site_url
			.. "/data/get_player_data.php?steamid="
			.. PlayerResource:GetSteamAccountID(player_id)
			.. "&token="
			.. GetWodaServerKey(),
		function(data)
			set_data(data, player_id)
		end
	)
end

function player_system:RegisterSeasonInfo()
	RequestData("https://" .. player_system.site_url .. "/data/get_free_donate_heroes.php", function(data)
		player_system.SetFreeDonateHeroes(data)
	end)
	RequestData("https://" .. player_system.site_url .. "/data/get_top_rating_150.php", function(data)
		player_system.SetTopMmr(data)
	end)
	RequestData("https://" .. player_system.site_url .. "/data/get_top_rating_pve_arena.php", function(data)
		player_system.SetTopMmrPVE(data)
	end)
	RequestData("https://" .. player_system.site_url .. "/data/get_top_donaters.php", function(data)
		player_system.SetTopMmrOverthrow(data)
	end)
	RequestData("https://" .. player_system.site_url .. "/data/get_heroes_votes.php", function(data)
		player_system.SetVotesHeroesInfo(data)
	end)
end
...
function player_system:BuyVotes(data)
		...
		if data ~= nil then
			...
			if current_hero_votes == -1 then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"shop_error_notification",
					{ error_name = "shop_error_no_hero_data" }
				)
				player_system.HEROES_VOTES_DATA = data
				return
			end
			...
			if current_hero_votes + votes > 450000 then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"shop_error_notification",
					{ error_name = "shop_error_more_votes" }
				)
				player_system.HEROES_VOTES_DATA = data
				return
			end
			...
			if tonumber(player_donate_table.coins) < tonumber(votes) then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"shop_error_notification",
					{ error_name = "shop_error_no_coins" }
				)
				player_system.HEROES_VOTES_DATA = data
				return
			else
			...
			CustomGameEventManager:Send_ServerToPlayer(
				...
				"shop_set_currency",
				{
					crystall = player_donate_table.crystals,
					coin = player_donate_table.coins,
					plus_days = player_donate_table.plus_days,
				}
			)
	...
	RequestData("https://" .. player_system.site_url .. "/data/get_heroes_votes.php", function(data)
		UpdateHeroesVotes(data)
	end)
end

player_system.XSOLLA_ORDER_SKU_LIST = {
	["2"] = true,
	["3"] = true,
	["4"] = true,
	["5"] = true,
...
player_system.XSOLLA_ORDER_COOLDOWN = {}

function player_system:XsollaCreateOrder(data)
	if data.PlayerID == nil then
		return
	end
	local id = data.PlayerID
	local sku = tostring(data.sku or "")
	if not player_system.XSOLLA_ORDER_SKU_LIST[sku] then
		return
	end
	local player = PlayerResource:GetPlayer(id)
	if player == nil then
	...
	local last_time = player_system.XSOLLA_ORDER_COOLDOWN[id]
	if last_time ~= nil and now - last_time < 3 then
		return
	end
	player_system.XSOLLA_ORDER_COOLDOWN[id] = now
	...
	SendData("https://" .. player_system.site_url .. "/data/xsolla_create_order.php", post_data, function(res)
		local target_player = PlayerResource:GetPlayer(id)
		if target_player == nil then
			return
		end
		if res and res.ok and res.url then
			CustomGameEventManager:Send_ServerToPlayer(target_player, "xsolla_order_created", { url = res.url })
		else
			CustomGameEventManager:Send_ServerToPlayer(target_player, "shop_error_notification", {})
		end
	end)
end
...
function player_system:UpdateReportsInfo()
	for id, player_info in pairs(player_system.PLAYERS_GLOBAL_INFORMATION) do
		player_system.PLAYERS_GLOBAL_INFORMATION[id].has_report_random =
			player_system:GetReportSystemRandom(player_info.reports, id)
		player_system.PLAYERS_GLOBAL_INFORMATION[id].has_report_kill =
			player_system:GetReportSystemRandom(player_info.reports, id)
...
function player_system:GetCoinsPlaceOverthrow(id)
	...
	if PlayerResource:GetConnectionState(id) == DOTA_CONNECTION_STATE_ABANDONED then
		CustomGameEventManager:Send_ServerToAllClients(
			"event_update_scoreboard_data",
			{ table_name = "coins_end", player_id = id, data = { coin = 0 } }
		)
		return 0
	end
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "coins_end", player_id = id, data = { coin = coin } }
	)
	return coin
end
...
function player_system:GetRatingPlaceOverthrow(id)
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "rating_end", player_id = id, data = { rating = 0 } }
	)
	return 0
end
...
function player_system:GetRatingPlacePve(id)
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "rating_end", player_id = id, data = { rating = rating } }
	)
	return rating
end
	...
	if player_system:GetPlayerCountAll() >= 7 or IsInToolsMode() or GetMapName() == "arena" then
		...
		for id, player_info in pairs(player_system.PLAYERS_GLOBAL_INFORMATION) do
			...
			if PLAYERS[id] then
				...
				if
					PlayerResource:GetConnectionState(id)
					and PlayerResource:GetConnectionState(id) == DOTA_CONNECTION_STATE_ABANDONED
				then
...
function player_system:GetCoinsPlace(id)
	...
	if GetMapName() == "rating_300" or GetMapName() == "rating_duo_300" then
		...
		if
			player_system.PLAYERS_GLOBAL_INFORMATION[id]
			and player_system.PLAYERS_GLOBAL_INFORMATION[id].rating >= 1000
		then
...
function player_system:GetRatingPlace(id)
	...
	if player_system:IsDisconnectFree() then
		if PLAYERS[id].first_leave then
			CustomGameEventManager:Send_ServerToAllClients(
				"event_update_scoreboard_data",
				{ table_name = "rating_end", player_id = id, data = { rating = -60 } }
			)
			return -60
		end
		return 0
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "rating_end", player_id = id, data = { rating = rating, is_double = is_double } }
	)
	return rating
end
...
function player_system:LoseTestForBB(player_id) end
...
function player_system:GetPlayerRuneLevel(player_info, rune_id)
	if not player_info or not player_info.runes then
		return 0
	end
...
function player_system:RuneSpinRequest(data)
	...
	CustomGameEventManager:Send_ServerToPlayer(
		player,
		"shop_set_currency",
		{ crystall = player_info.crystals, coin = player_info.coins, plus_days = player_info.plus_days }
	)
	...
	Timers:CreateTimer(4.8, function()
		...
		CustomGameEventManager:Send_ServerToPlayer(
			player,
			"woda_rune_spin_complete",
			{ rune_id = rune_id, level = new_level }
		)
...
function player_system:RuneToggleEquip(data)
	...
	if player_system:GetPlayerRuneLevel(player_info, rune_id) <= 0 then
		return
	end
...
function player_system:ApplyRunesFromStart(id, hero)
	...
	local player_info = player_system.PLAYERS_GLOBAL_INFORMATION[id]
	if not player_info or not player_info.equipped_runes then
		return
	end
		...
		if player_info and player_info.equipped_runes then
			for _, rune_id in pairs(player_info.equipped_runes) do
				if tostring(rune_id) == "rune_time" then
					local level =
						math.max(1, math.min(player_system:GetPlayerRuneLevel(player_info, "rune_time"), #values))
		...
		if player_info and player_info.equipped_runes then
			for _, rune_id in pairs(player_info.equipped_runes) do
				if tostring(rune_id) == "rune_creator" then
					local level =
						math.max(1, math.min(player_system:GetPlayerRuneLevel(player_info, "rune_creator"), #values))
					bonus = bonus + (values[level] or 0)
					break
				end
			end
		...
		if player_info and player_info.equipped_runes then
			for _, rune_id in pairs(player_info.equipped_runes) do
				if tostring(rune_id) == "rune_rebirth" then
					local level =
						math.max(1, math.min(player_system:GetPlayerRuneLevel(player_info, "rune_rebirth"), #values))
					bonus = bonus + (values[level] or 0)
					break
				end
			end
...
function player_system:HasDonateItem(player_donate_table, item_id)
	if not player_donate_table or not player_donate_table.donate_items then
		return false
	end
...
function player_system:BuyItem(data)
	...
	local player = PlayerResource:GetPlayer(id)
	local player_donate_table = player_system.PLAYERS_GLOBAL_INFORMATION[id]
	if not player_donate_table then
		return
	end
	local get_item_data_cost = _G.ALL_DONATES_ITEMS_DATA[tostring(item_id)]
	if not get_item_data_cost then
		return
	end
	if
		player_system.ONE_TIME_STORE_ITEMS[tostring(item_id)]
		and not IsInToolsMode()
		and player_system:HasDonateItem(player_donate_table, item_id)
	then
		CustomGameEventManager:Send_ServerToPlayer(
			player,
			"shop_error_notification",
			{ error_name = "shop_error_you_get_reward" }
		)
		return
	end
	local change_valute_crystall = 0
	...
	if currency == "crystal" then
		if tonumber(player_donate_table.crystals) < price then
			CustomGameEventManager:Send_ServerToPlayer(
				player,
				"shop_error_notification",
				{ error_name = "shop_error_no_crystals" }
			)
			return
		else
			if player_donate_table then
...
function player_system:BuyHero(data)
	...
	if type(hero_name) ~= "string" or _G.HEROES_LIST_CUSTOM[hero_name] == nil then
		return
	end
	if price == nil or price <= 0 or price % 1 ~= 0 then
		return
	end
	if days == nil or days <= 0 or days % 1 ~= 0 then
		return
	end
...
function player_system:ChangePetPremium(data)
	...
	local player = PlayerResource:GetPlayer(id)
	if player then
		if pet_id == nil or PETS_DATA_LIST[pet_id] == nil then
			return
		end
		local current_pet = player_system.PLAYERS_GLOBAL_INFORMATION[id]
			and player_system.PLAYERS_GLOBAL_INFORMATION[id].pet_id
		if
			pet_id ~= current_pet
			and not IsInToolsMode()
			and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[id], pet_id)
		then
			return
		end
		...
		if hero then
			...
			if
				player_system.PLAYERS_GLOBAL_INFORMATION[id]
				and player_system.PLAYERS_GLOBAL_INFORMATION[id].pet_id == pet_id
			then
				delete_pet = true
			end
			if player.pet and player.pet ~= nil and not delete_pet then
				...
				if PETS_DATA_LIST[pet_id].particle ~= nil then
					player.pet.particle = ParticleManager:CreateParticle(
						PETS_DATA_LIST[pet_id].particle,
			...
			elseif player.pet and player.pet ~= nil and delete_pet then
				...
				player_system.PLAYERS_GLOBAL_INFORMATION[id].pet_id = pet_id
				player.pet = CreateUnitByName(
					"npc_dota_donate_pet",
					hero:GetAbsOrigin() + RandomVector(RandomFloat(0, 100)),
					true,
					hero,
					nil,
					hero:GetTeamNumber()
...
function player_system:ChangeEffectPremium(data)
	...
	if player then
		...
		if effect ~= 0 and EMBLEMS_LIST[effect] == nil then
			return
		end
		local current_effect = player_system.PLAYERS_GLOBAL_INFORMATION[id]
			and player_system.PLAYERS_GLOBAL_INFORMATION[id].effect_id
		if
			effect ~= 0
			and effect ~= current_effect
			and not IsInToolsMode()
			and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[id], effect)
		then
			return
		end
		local hero = player:GetAssignedHero()
		if hero then
			...
			if
				player_system.PLAYERS_GLOBAL_INFORMATION[id]
				and player_system.PLAYERS_GLOBAL_INFORMATION[id].effect_id == effect
			then
				delete_effect = true
			end
...
function player_system:ChangeFivePremium(data)
	...
	if player then
		...
		local current_five = player_system.PLAYERS_GLOBAL_INFORMATION[id]
			and player_system.PLAYERS_GLOBAL_INFORMATION[id].five_id
		if
			five_id ~= 0
			and five_id ~= current_five
			and not IsInToolsMode()
			and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[id], five_id)
		then
			return
		end
		local delete_effect = false
		if
			player_system.PLAYERS_GLOBAL_INFORMATION[id]
			and player_system.PLAYERS_GLOBAL_INFORMATION[id].five_id == five_id
		then
			delete_effect = true
		end
		if not delete_effect then
...
function player_system:AddUpgradeFromHeroLevel(hero, id, unit)
	if player_system.PLAYERS_GLOBAL_INFORMATION[id] then
		...
		if player then
			...
			if GetMapName() == "arena" then
				local modifier_hero_level_upgrade_damage =
					unit:AddNewModifier(unit, nil, "modifier_hero_level_upgrade_damage", {})
			...
			if tier > 0 then
				local particle = ParticleManager:CreateParticle(
					"particles/hero_spawn_hero_level_" .. tier .. ".vpcf",
					PATTACH_ABSORIGIN_FOLLOW,
					unit
				)
...
function player_system:donate_promocode(data)
			...
			if data.promo_code ~= nil then
				if data.promo_use == "0" then
					...
					if player_donate_table then
						...
						if tonumber(data.battlepass) > 0 then
							if player_donate_table.has_battlepass_2026 > 0 then
								player_donate_table.battlepass_level_2026 = player_donate_table.battlepass_level_2026
									+ 49
							else
						...
						SendData(
							"https://" .. player_system.site_url .. "/data/use_promocode_server.php",
							post_data,
							nil
						)
						return
					...
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"shop_error_notification",
						{ error_name = "shop_error_nothing_player" }
					)
					return
				end
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"shop_error_notification",
					{ error_name = "shop_error_promo_used" }
				)
				return
			end
			CustomGameEventManager:Send_ServerToPlayer(
				player,
				"shop_error_notification",
				{ error_name = "shop_error_promo_null" }
			)
			return
		end
		CustomGameEventManager:Send_ServerToPlayer(
	...
	RequestData(
		"https://"
			.. player_system.site_url
			.. "/data/get_promo_code.php?promo="
			.. promo
			.. "&token="
			.. GetWodaServerKey()
			.. "&id="
			.. PlayerResource:GetSteamAccountID(id),
		function(data)
			promo_check(data, id, promo)
		end
	)
end
...
function player_system:PlayerTip(keys)
	...
	if player_system.PLAYERS_GLOBAL_INFORMATION[id_caster] then
		...
		then
			...
			CustomGameEventManager:Send_ServerToAllClients(
				...
				{ player_id_1 = id_caster, player_id_2 = id_target, tip = tips[RandomInt(1, #tips)] }
			)
		else
			CustomGameEventManager:Send_ServerToAllClients(
				"TipPlayerNotification",
				{ player_id_1 = id_caster, player_id_2 = id_target, tip = 0 }
			)
		end
	end
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"event_update_scoreboard_data",
		{ table_name = "tip_cooldown", player_id = id_caster, data = { cooldown = cooldown } }
	)
...
function player_system:PlayerBanLeaved(id)
	if player_system.PLAYERS_GLOBAL_INFORMATION[id] == nil then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "883703644"
	then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "122413750"
	then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "181517303"
	then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "1442313314"
	then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "1602096619"
	then
		return
	end
	if
		player_system.PLAYERS_GLOBAL_INFORMATION[id]
		and tostring(player_system.PLAYERS_GLOBAL_INFORMATION[id].steamid) == "1899469460"
	then
		return
	end
	if player_system.PLAYERS_GLOBAL_INFORMATION[id].last_leave == 1 then
		return
	end
	if GetMapName() == "rating_duo" or GetMapName() == "rating_duo_300" then
		return
	...
	if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID] then
		if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].players_repoted then
			...
			for count = #player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].players_repoted, 1, -1 do
				if
					player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].players_repoted[count]
					and (player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].players_repoted[count] == report_id)
				then
			...
			CustomGameEventManager:Send_ServerToAllClients(
				"event_update_scoreboard_data",
				{
					...
					data = { reported_info = player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].players_repoted },
				}
			)
		end
	end
...
function player_system:change_premium_tip(data)
	...
	if
		not IsInToolsMode()
		and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID], tip)
	then
		return
	end
	if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID] then
		if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].tips then
			...
			for count = #player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].tips, 1, -1 do
				if
					player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].tips[count]
					and (player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].tips[count] == tip)
				then
...
function player_system:change_premium_bg(data)
	...
	local current_bg = player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID]
		and player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].background_id
	if
		background_id ~= 0
		and background_id ~= current_bg
		and not IsInToolsMode()
		and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID], background_id)
	then
		return
	end
	if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID] then
		if
			player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].background_id
			and player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].background_id == background_id
		then
...
function player_system:change_premium_tp(data)
	...
	if teleport_id == nil then
		return
	end
	local current_tp = player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID]
		and player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].teleport_id
	if
		teleport_id ~= 0
		and teleport_id ~= current_tp
		and not IsInToolsMode()
		and not player_system:HasDonateItem(player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID], teleport_id)
	then
		return
	end
	if player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID] then
		if
			player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].teleport_id
			and player_system.PLAYERS_GLOBAL_INFORMATION[data.PlayerID].teleport_id == teleport_id
		then
...
function player_system:web_update_player_coin_base(data)
	...
	if player_donate_table then
		...
		SendData("https://" .. player_system.site_url .. "/data/web_update_player_coin.php", post_data, function(data)
			player_system:UpdatePlayerCoinWeb(data, id)
		end)
	end
end
...
function player_system:UpdatePlayerCoinWeb(data, id)
	...
	if data.update_data == nil or tonumber(data.update_data) == 0 then
		return
	end
	local player_object = PlayerResource:GetPlayer(id)
	local player_donate_table = player_system.PLAYERS_GLOBAL_INFORMATION[id]
...
function player_system:AddPlayerEmblem(effect_id, hero)
	...
	local modifier_woda_emblem = hero:AddNewModifier(
		hero,
		nil,
		"modifier_woda_emblem",
		{ status_effect = effect_info["status_effect"], effect_id = effect_id }
	)
	if modifier_woda_emblem then
		if effect_info["particle_name"] then
			...
			if effect_real_name == "particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient" then
				effect_real_name = "particles/econ/events/anniversary_10th/anniversary_10th_hat_ambient_"
					.. hero:GetUnitName()
					.. ".vpcf"
			end
			...
			if
				effect_real_name == "particles/woda_unique/pumpkin_head.vpcf"
				or effect_real_name == "particles/woda_unique/tiny_head.vpcf"
				or effect_real_name == "effect/crown/2_shield.vpcf"
				or effect_real_name == "particles/woda_unique/skull_head.vpcf"
			then
			...
			if
				effect_real_name == "particles/econ/items/ember_spirit/ember_ti9/ember_ti9_flameguard_shield_core.vpcf"
			then
				ParticleManager:SetParticleControlEnt(
					particle_fx_1,
					1,
			...
			if
				effect_real_name == "particles/econ/events/fall_2022/mjollnir/mjollnir_shield_fall2022_beam_zap.vpcf"
			then
				ParticleManager:SetParticleControlEnt(
					particle_fx_1,
					1,
		...
		if effect_info["particle_name_2"] then
			local particle_fx_2 =
				ParticleManager:CreateParticle(effect_info["particle_name_2"], PATTACH_ABSORIGIN_FOLLOW, hero)
			ParticleManager:SetParticleControl(particle_fx_2, 0, hero:GetAbsOrigin())
			if
				effect_info["particle_name_2"] == "particles/econ/items/pugna/pugna_ward_ti5/pugna_ambient_ti_5.vpcf"
			then
...
function player_system:woda_plus_get_player_reward(data)
	...
	if _G.QUESTS_LIST[quest_id] == nil then
		return
	end
	...
	if player_data.plus_days <= 0 then
		return
	end
	if player_data.quest_data[quest_id] == nil then
		return
	end
	if player_data.quest_data[quest_id] < _G.QUESTS_LIST[quest_id][1] then
		return
	end
	if _G.QUESTS_LIST[quest_id][2] ~= reward_id then
		return
	end
	if player_system:HasDonateItem(player_data, reward_id) then
		return
	end
...
function player_system:PlayerQuestProgress(player_id, quest_id, progress, not_save)
	if player_system.PLAYERS_GLOBAL_INFORMATION[player_id] == nil then
		return
	end
	if not not_save then
		...
		if GetMapName() == "overthrow" and not IsInToolsMode() then
			return
		end
	end
...
function player_system:UpdateQuestsEndGame(player_id)
	...
	if player_hero == nil then
		return
	end
	if player_hero:IsNull() then
		return
	end
	local player_data = player_system.PLAYERS_GLOBAL_INFORMATION[player_id]
	if player_data == nil then
	...
	if player_hero.IsMainAtturibute == nil then
		return
	end
...
function player_system:UpdatePlayerBPLevelsHero(id)
	...
	for _, hero_info in pairs(player_system.PLAYERS_GLOBAL_INFORMATION[id]["heroes_level"]) do
		...
		if hero_level >= 30 then
			if
				_G.HEROES_LIST_CUSTOM[hero_name]
				and _G.HEROES_LIST_CUSTOM[hero_name].AttributePrimary == "DOTA_ATTRIBUTE_STRENGTH"
			then
				player_system:PlayerQuestProgress(id, 22, 1, true)
			elseif
				_G.HEROES_LIST_CUSTOM[hero_name]
				and _G.HEROES_LIST_CUSTOM[hero_name].AttributePrimary == "DOTA_ATTRIBUTE_AGILITY"
			then
				player_system:PlayerQuestProgress(id, 44, 1, true)
			elseif
				_G.HEROES_LIST_CUSTOM[hero_name]
				and _G.HEROES_LIST_CUSTOM[hero_name].AttributePrimary == "DOTA_ATTRIBUTE_INTELLECT"
			then
				player_system:PlayerQuestProgress(id, 66, 1, true)
			elseif
				_G.HEROES_LIST_CUSTOM[hero_name]
				and _G.HEROES_LIST_CUSTOM[hero_name].AttributePrimary == "DOTA_ATTRIBUTE_ALL"
			then
...
function player_system:SelectHeroVO(data)
	...
	if (player.sound_cooldown[1] > 0) and (player.sound_cooldown[2] > 0) then
		...
		if player then
			...
			CustomGameEventManager:Send_ServerToPlayer(
				player,
				"panorama_cooldown_error",
				{ message = "#dota_sound_error", time = cooldown_sound }
			)
		end
		return
	end
	if player.sound_cooldown[1] > 0 then
		...
		Timers:CreateTimer({
			...
			endTime = 1,
			callback = function()
				if player.sound_cooldown[2] <= 0 then
					return nil
				end
	...
	CustomGameEventManager:Send_ServerToAllClients(
		"chat_dota_sound",
		{
			...
			sound_name_global = data.message,
			level = data.level,
		}
	)
end
...
function player_system:battlepass_get_player_reward(data)
	...
	if level == nil or reward_id == nil then
		return
	end
	if _G.BATTLEPASS_REWARDS[reward_id] == nil then
		return
	end
	if player_system:HasDonateItem(player_data, reward_id) then
		return
	...
	if data.battlepass_2026 then
		if player_data.battlepass_level_2026 <= 0 then
			return
		end
		if player_data.has_battlepass_2026 <= 0 then
			return
		end
		if player_data.battlepass_level_2026 < level then
			return
		end
		if player_data.battlepass_level_2026 < _G.BATTLEPASS_REWARDS[reward_id] then
			return
		end
	else
		if player_data.battlepass_level <= 0 then
			return
		end
		if player_data.has_battlepass <= 0 then
			return
		end
		if player_data.battlepass_level < level then
			return
		end
		if player_data.battlepass_level < _G.BATTLEPASS_REWARDS[reward_id] then
			return
		end
	end
	...
	CustomGameEventManager:Send_ServerToPlayer(
		...
		"shop_set_currency",
		{ crystall = player_data.crystals, coin = player_data.coins, plus_days = player_data.plus_days }
	)
end
...
function player_system:win_condition_predict(data)
	...
	if player_data.double_tokens <= 0 then
		return
	end
