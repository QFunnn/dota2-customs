...
LinkLuaModifier("modifier_animation", "libs/modifiers/modifier_animation.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier(
	"modifier_animation_translate",
	"libs/modifiers/modifier_animation_translate.lua",
	LUA_MODIFIER_MOTION_NONE
)
LinkLuaModifier(
	"modifier_animation_translate_permanent",
	"libs/modifiers/modifier_animation_translate_permanent.lua",
	LUA_MODIFIER_MOTION_NONE
)
...
function StartAnimation(unit, table)
	...
	rate = math.floor(math.max(0, math.min(255 / 20, rate)) * 20 + 0.5)
	...
	if translate ~= nil then
		if _ANIMATION_TRANSLATE_TO_CODE[translate] == nil then
			print(
				"[ANIMATIONS.lua] ERROR, no translate-code found for '"
					.. translate
					.. "'.  This translate may be misspelled or need to be added to the enum manually."
			)
			return
		end
	...
	if
		unit:HasModifier("modifier_animation")
		or (unit._animationEnd ~= nil and unit._animationEnd + 0.067 > GameRules:GetGameTime())
	then
		EndAnimation(unit)
		Timer(0.066, function()
			if translate2 ~= nil then
				unit:AddNewModifier(
					unit,
					nil,
					"modifier_animation_translate",
					{ duration = duration, translate = translate2 }
				)
				unit:SetModifierStackCount(
					"modifier_animation_translate",
					unit,
					_ANIMATION_TRANSLATE_TO_CODE[translate2]
				)
			end
...
function AddAnimationTranslate(unit, translate)
	...
	unit:AddNewModifier(
		...
		nil,
		"modifier_animation_translate_permanent",
		{ duration = duration, translate = translate }
	)
