					...
					if heroSlot then
						...
						if item.visuals and item.visuals.styles then
							...
							for style, style_table in pairs(item.visuals.styles) do
								...
								if
									style_table.alternate_icon
									and item.visuals.alternate_icons
									and item.visuals.alternate_icons[tostring(style_table.alternate_icon)]
								then
...
function Wearable:RequestParticles(response)
	Http:RequestParticles(function(sBody)
		if not sBody then
			response(false)
			return
		end
			...
			if asset_modifiers then
				for _, am_table in pairs(asset_modifiers) do
					...
					elseif type(am_table) == "table" and am_table.type == "particle_projectile" then
						...
						if
							((not am_table.style) or tostring(am_table.style) == sStyle)
							and not am_table.spawn_in_loadout_only
						then
...
function Wearable:_WearItemsRespawn(hUnitOrigin, hNewWears)
	...
	CreateUnitByNameAsync(sUnitNameWithWear, position, true, nil, nil, hUnitOrigin:GetTeam(), function(hUnitNew)
		...
		CustomGameEventManager:Send_ServerToPlayer(hPlayer, "RespawnWear", {
...
function Wearable:_WearProp(hUnit, sItemDef, sSlotName, sStyle)
	...
	if sModel_player then
		...
		if sDefaultAnim then
			hModel = SpawnEntityFromTableSynchronous(sPropClass, {
				model = sModel_player,
			...
			if type(am_table) == "table" and am_table.type == "particle_create" then
				...
				then
					...
					if not Wearable:IsDisplayInLoadout(sHeroName, sSlotName) then
						...
						for sSubSlotName, hSubWear in pairs(hUnit.Slots) do
							if
								hSubWear["replace_particle_names"] and hSubWear["replace_particle_names"][particle_name]
							then
								bReplaced = true
...
function Wearable:WearCourier(hUnit, sItemDef, sStyle, bFlying, bDire)
	...
	if asset_modifiers then
		...
		for am_name, am_table in pairs(asset_modifiers) do
			...
			elseif
				type(am_table) == "table"
				and ((bDire and am_table.asset == "dire") or ((not bDire) and am_table.asset == "radiant"))
			then
				if
					((not bFlying) and type(am_table) == "table" and am_table.type == "courier")
					or (bFlying and type(am_table) == "table" and am_table.type == "courier_flying")
				then
					if (not am_table.style) or tostring(am_table.style) == sStyle then
						local sNewModel = am_table.modifier
						print(sNewModel)
...
function Wearable:WearWard(hUnit, sItemDef, sStyle)
	...
	if asset_modifiers then
		for am_name, am_table in pairs(asset_modifiers) do
			...
			elseif
				type(am_table) == "table"
				and am_table.type == "entity_model"
				and hUnit:GetUnitName() == am_table.asset
			then
				if (not am_table.style) or tostring(am_table.style) == sStyle then
					local sNewModel = am_table.modifier
					print(sNewModel)
...
function Wearable:CanPrismatic(sParticle, sPrismatic)
	if
		sParticle == "particles/econ/courier/courier_trail_lava/courier_trail_lava.vpcf"
		and sPrismatic == "unusual_red"
	then
		return false
	end
	return true
...
function Wearable:SwitchPrismatic(hUnit, sPrismaticName)
	...
	if
		not hUnit["prismatic_particles"]
		and not PrismaticParticles[projectile]
		and not (hUnit.Slots and hUnit.Slots["summon"] and hUnit.Slots["summon"]["itemDef"] == "7380")
	then
	...
	CustomNetTables:SetTableValue(
		"hero_prismatic",
		tostring(hUnit:GetEntityIndex()),
		{ prismatic_name = sPrismaticName }
	)
end
