...
function Illusion:InitIllusion(hIllusion)
	...
	for i = 0, hOriginalHero:GetAbilityCount() - 1 do
		...
		if hOriginalHeroAbility and hOriginalHeroAbility.GetAbilityName then
			...
			then
				if
					not (
						hIllusion:HasModifier("modifier_phantom_lancer_juxtapose_illusion")
						and Illusion.juxtaposeException[sOriginalAbilityName]
					)
				then
	...
	if
		not hIllusion:HasModifier("modifier_player_cosmetics")
		and hOriginalHero:HasModifier("modifier_player_cosmetics")
	then
