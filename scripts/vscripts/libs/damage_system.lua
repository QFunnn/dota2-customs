...
function damage_system:StartUpdateTimer()
	if damage_system.update_timer_started then
		return
	end
...
function damage_system:MarkPlayerDirty(player_id)
	...
	if damage_system.damage_table[player_id] == nil then
		return
	end
	...
	for player_id, _ in pairs(damage_system.damage_dirty_players) do
		...
		if player and damage_system.damage_table[player_id] then
			CustomGameEventManager:Send_ServerToPlayer(
				player,
				"WODA_Player_update_damage_filter",
				damage_system.damage_table[player_id]
			)
		end
...
function damage_system:UpdatePlayerDamage(attacker_id, victim_id, damage, damage_type)
	if damage_system.damage_table[attacker_id] then
		damage_system.damage_table[attacker_id][2][damage_type] = (
			damage_system.damage_table[attacker_id][2][damage_type] or 0
		) + damage
		damage_system.damage_table_server[attacker_id][2][damage_type] = (
			damage_system.damage_table_server[attacker_id][2][damage_type] or 0
		) + damage
		damage_system:MarkPlayerDirty(attacker_id)
	end
	if damage_system.damage_table[victim_id] then
		damage_system.damage_table[victim_id][1][damage_type] = (
			damage_system.damage_table[victim_id][1][damage_type] or 0
		) + damage
		damage_system.damage_table_server[victim_id][1][damage_type] = (
			damage_system.damage_table_server[victim_id][1][damage_type] or 0
		) + damage
	...
	for id, info in pairs(PLAYERS) do
		if info.hero ~= nil and damage_system.damage_table_server[id] then
			CustomGameEventManager:Send_ServerToAllClients(
				...
				{ table_name = "end_game_damage", player_id = id, data = damage_system.damage_table_server[id] }
			)
		end
	end
end
