...
function neutrals_reward:SpawnRandomNeutrals(arena)
	...
	if not neutrals_reward.timer then
		...
		neutrals_reward.timer = Timers:CreateTimer(1, function()
			if neutrals_reward.timer == nil then
				return
			end
			time = time - 1
			CustomGameEventManager:Send_ServerToAllClients(event_names[2], { time = time, max = 60 })
	...
	for id, player_info in pairs(PLAYERS) do
		if neutrals_reward.RewardForPlayer[id][5] ~= nil and not player_system:IsLose(id) then
			Timers:CreateTimer(FrameTime(), function()
				local out_gold = PlayerResource:ModifyGold(
					id,
					neutrals_reward.RewardForPlayer[id][5],
					true,
...
function neutrals_reward:RerollNeutralReward(data)
	...
	if neutrals_reward.RewardCountForPlayer[id] <= 0 then
		return
	end
	...
	for i = #random_items_list, 1, -1 do
		if random_items_list[i] then
			if
				random_items_list[i].item_name == old_table[1].item_name
				or random_items_list[i].item_name == old_table[2].item_name
				or random_items_list[i].item_name == old_table[3].item_name
				or random_items_list[i].item_name == old_table[4].item_name
			then
...
function neutrals_reward:SelectNeutralReward(data)
	...
	if neutrals_reward.RewardForPlayer[id][1] == nil then
		return
	end
	if neutrals_reward.RewardForPlayer[id][2] == nil then
		return
	end
	if neutrals_reward.RewardForPlayer[id][3] == nil then
		return
	end
	if neutrals_reward.RewardForPlayer[id][4] == nil then
		return
	end
	if neutrals_reward.RewardForPlayer[id][5] == nil then
		return
	end
	if choose <= 4 then
		...
		if hero and item_info then
			if item_info.item_enchant then
				...
				if
					check_item_divine_regalia_broken
					and check_item_divine_regalia_broken:GetAbilityName() == "item_divine_regalia_broken"
				then
				...
				if hero.neutral_item_save and not hero.neutral_item_save:IsNull() then
					local item_tier = neutrals_reward:GetNeutralTier(
						hero.neutral_item_save:GetAbilityName(),
						hero.neutral_item_save:GetLevel()
					)
					if item_tier then
						local items_cost = neutrals_reward.NEUTRALS_COST[tonumber(item_tier)]
						local out_gold = hero:ModifyGold(items_cost / 2, false, DOTA_ModifyGold_SellItem)
	...
	elseif choose == 5 then
		if hero then
			local out_gold = PlayerResource:ModifyGold(
				data.PlayerID,
				neutrals_reward.RewardForPlayer[id][choose],
				true,
				DOTA_ModifyGold_GameTick
			)
			CreateEffectGold(hero, out_gold, hero)
