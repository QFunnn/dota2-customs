...
function WODAGameMode:ExecuteOrderFilter(filterTable)
	...
	local target = (filterTable.entindex_target and filterTable.entindex_target ~= 0)
			and EntIndexToHScript(filterTable.entindex_target)
		or nil
	...
	if unit then
		if
			(order == DOTA_UNIT_ORDER_PICKUP_ITEM or order == DOTA_UNIT_ORDER_ATTACK_TARGET)
			and filterTable.queue == 0
		then
			local item = target
	...
	if unit and unit:HasModifier("modifier_pangolier_gyroshell_custom") then
		if unit:HasModifier("modifier_wind_waker") then
			return true
		end
		...
		if validMoveOrders[order] then
			...
			if mod then
				mod:OnOrderCustom(
					Vector(filterTable.position_x, filterTable.position_y, filterTable.position_z),
					target
				)
			end
	...
	for _, mod_order in pairs(orders_list_func) do
		...
		if mod_order_handle and mod_order_handle.OnOrder then
			mod_order_handle:OnOrder({
				unit = unit,
				new_pos = Vector(filterTable.position_x, filterTable.position_y, filterTable.position_z),
				target = target,
				order_type = order,
			})
		end
	...
	if isCastOrder(order) and unit and target and target.GetUnitName then
		if
			target:HasModifier("modifier_wodacreepchampion")
	...
	if
		unit
		and unit:HasModifier("modifier_medusa_20")
		and unit:HasModifier("modifier_medusa_mana_shield_custom_active")
	then
	...
	if unit and unit:HasModifier("modifier_legion_commander_duel_custom") then
		if
			...
			or order == DOTA_UNIT_ORDER_CONTINUE
	...
	if
		unit
		and target
		and isCastOrder(order)
		and target.GetUnitName
		and target:GetUnitName() == "npc_dota_bounty_hunter_gold_bag"
	then
		if not ability then
			return true
		end -- ранее было просто return; корректнее пропустить
	...
	if unit and target and isCastOrder(order) and target.GetUnitName then
		...
		if
			name == "npc_dota_creature_barrel"
			or name == "small_barrel"
			or name == "small_barrel_side"
			or name == "big_barrel"
		then
			return false
		end
	...
	if
		...
		or abilityName(ability) == "marci_companion_run_custom"
	then
		-- вторая точка
		...
		if order == DOTA_UNIT_ORDER_CAST_POSITION and abilityName(ability) ~= "muerta_dead_shot_custom" then
			...
			ability.OnSpellStart = function(self)
				return OverrideSpellStart(self, p1, dir)
			end
		end

		-- клик по цели/дереву для Muerta/Marci
		if
			(order == DOTA_UNIT_ORDER_CAST_TARGET or order == DOTA_UNIT_ORDER_CAST_TARGET_TREE)
			and (
				abilityName(ability) == "muerta_dead_shot_custom"
				or abilityName(ability) == "marci_companion_run_custom"
			)
		then
			ability.vectorTargetPosition = Vector(filterTable.position_x, filterTable.position_y, 0)
...
function WODAGameMode:DamageFilter(damageTable)
	...
	local hAbility = damageTable.entindex_inflictor_const and EntIndexToHScript(damageTable.entindex_inflictor_const)
		or nil
	if damageTable.entindex_inflictor_const ~= nil then
		...
		if
			hAbility
			and hAbility.GetAbilityName
			and (
				"item_dagon" == hAbility:GetAbilityName()
				or "item_dagon_2" == hAbility:GetAbilityName()
				or "item_dagon_3" == hAbility:GetAbilityName()
				or "item_dagon_4" == hAbility:GetAbilityName()
				or "item_dagon_5" == hAbility:GetAbilityName()
			)
		then
			if not IsInToolsMode() then
				if
					GetMapName() == "overthrow"
					and (math.floor(math.floor(GameRules:GetDOTATime(false, false)) / 60)) < 10
				then
	...
	if
		hAttacker
		and (
			(hAttacker:IsHero() or hAttacker:GetUnitName() == "npc_dota_lone_druid_bear_custom")
			or GetMapName() == "arena"
		)
	then
		if
			hVictim
			and (
				(hVictim:IsHero() or hVictim:GetUnitName() == "npc_dota_lone_druid_bear_custom")
				or GetMapName() == "arena"
			)
		then
			if hVictim ~= hAttacker then
				damage_system:UpdatePlayerDamage(
					hAttacker:GetPlayerOwnerID(),
					hVictim:GetPlayerOwnerID(),
					damageTable.damage,
...
function WODAGameMode:OnPlayerChat(data)
	...
	if text == "die" then
		ApplyDamage({
			attacker = PlayerResource:GetSelectedHeroEntity(0),
			victim = PlayerResource:GetSelectedHeroEntity(0),
			damage = 999999,
			damage_type = DAMAGE_TYPE_PURE,
		})
		ApplyDamage({
			attacker = PlayerResource:GetSelectedHeroEntity(1),
			victim = PlayerResource:GetSelectedHeroEntity(1),
			damage = 999999,
			damage_type = DAMAGE_TYPE_PURE,
		})
	end
