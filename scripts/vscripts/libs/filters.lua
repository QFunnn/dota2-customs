...
function GameMode:ModifyGoldFilter(event)
	...
	local ok, err = xpcall(function()
		...
		-- [A83] Запоминаем СПИСАНИЕ за покупку: возврат за отклонённый гем (libs/item.lua)
		-- разрешён только против реального списания, иначе окно быстрой покупки
		-- давало чистые +5000 (предмет доезжал до инвентаря, а золото не снималось).
		if
			event.gold < 0
			and (
				event.reason_const == DOTA_ModifyGold_PurchaseItem
				or event.reason_const == DOTA_ModifyGold_PurchaseConsumable
			)
		then
			GEM_PURCHASE_SPEND = GEM_PURCHASE_SPEND or {}
			GEM_PURCHASE_SPEND[event.player_id_const] = {
				amount = -event.gold,
		...
		if
			event.reason_const == DOTA_ModifyGold_PurchaseConsumable
			or event.reason_const == DOTA_ModifyGold_PurchaseItem
			or event.reason_const == DOTA_ModifyGold_Death
			or event.reason_const == DOTA_ModifyGold_SellItem
		then
			return true
		end
	...
	if not ok then
		return true
	end -- безопасный fallback: пропускаем gold change
	return true
end
...
function GameMode:OnPlayerUsedAbility(params)
	...
	Timers:CreateTimer(0, function()
		...
		if
			MainAbility
			and LinkedAbility
			and MainAbility:IsHidden()
			and LinkedAbility:IsHidden()
			and MainAbility._bCustomDisabled == nil
		then
...
function GameMode:ModifierGainedFilter(params)
	...
	if iGlobalCaster ~= nil and iGlobalParent ~= nil then
		...
		if
			hGlobalCaster
			and hGlobalParent
			and not Players:IsUnitCanAttackOrCastOnThis(hGlobalCaster, hGlobalParent)
		then
			local Block = true
			if iGlobalAbility ~= nil then
				...
				if
					hGlobalAbility
					and not hGlobalAbility:IsNull()
					and GetAbilitySetting(hGlobalAbility:GetName(), "bWorkOnOtherArena")
				then
					Block = false
				end
			end
		...
		if iGlobalAbility ~= nil then
			...
			if hGlobalCaster and hGlobalParent and hGlobalAbility then
				...
				if
					AbilityInfo
					and hGlobalAbility
					and AbilityInfo.modifier ~= nil
					and AbilityInfo.modifier == modifier
				then
	...
	elseif
		(
			...
			or modifier == "modifier_spiritbreaker_greater_bash_knockback"
			or modifier == "modifier_spiritbreaker_greater_bash_stun"
			or modifier == "modifier_spirit_breaker_greater_bash_stun"
		) and params.entindex_ability_const ~= nil
	then
		local ability = EntIndexToHScript(params.entindex_ability_const)
		if ability then
			...
			if is_bara_bash then
				if
					hero:HasModifier("modifier_minigames_pudge_shift")
					or hero:HasModifier("modifier_riki_tricks_of_the_trade_custom")
				then
					return false
				end
				if
		...
		if target_modifier and not hero:IsHero() then
			...
			if hCaster and hAbility then
				hero:AddNewModifier(
					...
					{ duration = flDuration, primary = true }
				)
	...
	elseif modifier == "modifier_lycan_summon_wolves_crit_maim" then
		local target = EntIndexToHScript(params.entindex_parent_const)
		if target then
			local modifier_lycan_summon_wolves_crit_maim_modifiers =
				target:FindAllModifiersByName("modifier_lycan_summon_wolves_crit_maim")
			local last_modifier =
				modifier_lycan_summon_wolves_crit_maim_modifiers[#modifier_lycan_summon_wolves_crit_maim_modifiers]
...
function GameMode:DamageFilter(event)
	if event.entindex_attacker_const == nil then
		return true
	end
	...
	if
		event.damage > 0
		and target:IsAlive()
		and not target._deferred_berserk_kill
		and target:HasModifier("modifier_creep_controll")
	then
		local modif = target:FindModifierByName("modifier_creep_controll")
		if
			modif
			and modif:GetStackCount() > 0
			and event.damage >= target:GetHealth()
			and target:GetAttackTarget() == attacker
		then
	...
	if
		target
		and (
			target:HasModifier("modifier_hero_refreshing")
			or (IsRealHero(target) and Players:PlayerArenaIs(target:GetPlayerOwnerID(), "MAIN"))
		)
	then
...
function GameMode:OrderFilter(params)
	...
	if not ok then
		return true
	end -- safe fallback при ошибке: разрешаем order, не блокируем игру
...
function GameMode:_OrderFilterImpl(params)
	...
	then
		...
		if
			td
			and td.begined == true
			and td.start_time ~= nil
			and (GameRules:GetGameTime() - td.start_time) >= 0.25
			and Map:GetPositionArena(unit) ~= "MAIN"
		then
			allowEul = true
		end
		if not allowEul then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"SendHudError",
					{ message = "dota_hud_error_eul_disabled_on_arena" }
				)
			end
			return false
		end
			...
			if PID ~= nil then
				...
				if player then
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"SendHudError",
						{ message = "dota_hud_error_cant_attack_on_other_arena" }
					)
				end
			end
			return false
	...
	if
		(orderType == DOTA_UNIT_ORDER_CAST_POSITION or orderType == DOTA_UNIT_ORDER_VECTOR_TARGET_POSITION)
		and item
		and not GetAbilitySetting(item:GetName(), "bCanCastOnOtherArena")
	then
		...
		if not bResult then
			...
			if string.find(AbilityName, "blink") ~= nil then
				...
				if ArenaInfo ~= nil then
					...
					if
						not unit:HasFlyMovementCapability()
						and ArenaInfo.ground_mins ~= nil
						and ArenaInfo.ground_maxs ~= nil
					then
					...
					if NewPos.x >= Maxs.x then
						NewPos.x = Maxs.x
					end
					if NewPos.x <= Mins.x then
						NewPos.x = Mins.x
					end
					if NewPos.y >= Maxs.y then
						NewPos.y = Maxs.y
					end
					if NewPos.y <= Mins.y then
						NewPos.y = Mins.y
					end
					...
					if PID ~= nil then
						...
						if player then
							CustomGameEventManager:Send_ServerToPlayer(
								player,
								"SendHudError",
								{ message = "dota_hud_error_cant_cast_on_other_arena" }
							)
						end
					end
					return false
	...
	if orderType == DOTA_UNIT_ORDER_MOVE_ITEM and item then
		...
		if item:GetName() == "item_smoke_of_deceit_custom" then
			...
			if currentSlot >= DOTA_STASH_SLOT_1 and currentSlot <= DOTA_STASH_SLOT_6 and targetSlot ~= 15 then
				...
				unit:SetContextThink(DoUniqueString("SmokeStashFix"), function()
					...
					if not hSmoke or hSmoke:IsNull() then
						return
					end
					...
					if
						...
						and existing ~= hSmoke
						and existing:GetAbilityName() == "item_smoke_of_deceit_custom"
					then
		...
		item:SetContextThink(DoUniqueString("Swap"), function(ent)
			...
			local IsNeutral = (fSlot and table.contains(NeutralItemsList, fSlot:GetName()))
				or (sSlot and table.contains(NeutralItemsList, sSlot:GetName()))
					...
					if NeutralItem:GetItemSlot() < SlotsToCheck then
						...
						if player then
							CustomGameEventManager:Send_ServerToPlayer(
								player,
								"SendHudError",
								{ message = "dota_neutral_item_slot_swap" }
							)
						end
					elseif
						currentSlot == DOTA_ITEM_NEUTRAL_ACTIVE_SLOT
						or ent:GetItemSlot() == DOTA_ITEM_NEUTRAL_ACTIVE_SLOT
					then
	...
	if
		(
			...
			or orderType == DOTA_UNIT_ORDER_GIVE_ITEM
			or orderType == DOTA_UNIT_ORDER_EJECT_ITEM_FROM_STASH
		) and item
	then
		...
		if table.contains(NeutralItems, item:GetName()) then
			...
			if player then
				CustomGameEventManager:Send_ServerToPlayer(
					player,
					"SendHudError",
					{ message = "dota_hud_error_cant_drop" }
				)
			end
			...
			if PlayerInfo and PlayerInfo.player_die_round ~= nil then
				...
				if player then
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"SendHudError",
						{ message = "dota_hud_error_you_died" }
					)
				end
				return false
			end
	...
	if
		(orderType == DOTA_UNIT_ORDER_PICKUP_ITEM or orderType == DOTA_UNIT_ORDER_ATTACK_TARGET)
		and params.queue == 0
	then
				...
				if RealPurchaser and RealPurchaser ~= unit then
					...
					if player then
						CustomGameEventManager:Send_ServerToPlayer(
							player,
							"SendHudError",
							{ message = "dota_hud_error_cant_destroy_other_items" }
						)
					end
					return false
				end
	...
	if orderType == DOTA_UNIT_ORDER_PURCHASE_ITEM then
			...
			if unit:HasModifier("modifier_item_gem_shard") then
				...
				if player then
					CustomGameEventManager:Send_ServerToPlayer(
						player,
						"SendHudError",
						{ message = "dota_hud_error_item_already_purchased" }
					)
				end
				return false
			end
		...
		if params.shop_item_name == "item_gem_shard_2" then
			if not unit:HasModifier("modifier_item_gem_shard") or unit:HasModifier("modifier_item_gem_shard_2") then
				local player = PlayerResource:GetPlayer(unit:GetPlayerOwnerID())
				if player then
					local Error = not unit:HasModifier("modifier_item_gem_shard")
							and "dota_hud_error_not_buyed_1_version"
						or "dota_hud_error_item_already_purchased"
	...
	if (orderType == DOTA_UNIT_ORDER_CAST_TARGET) and params.queue == 0 then
		if target then
					...
					if unit:IsTempestDouble() then
						...
						if hPlayer then
							CustomGameEventManager:Send_ServerToPlayer(
								hPlayer,
								"SendHudError",
								{ message = "dota_hud_error_ability_inactive" }
							)
						end
						return false
					end
			...
			if
				unit:GetTeamNumber() ~= target:GetTeamNumber()
				and target:HasModifier("modifier_hero_refreshing")
				and not IsCheatsEnabled()
			then
				local hAbility = EntIndexToHScript(params.entindex_ability)
				if hAbility and hAbility.GetAbilityName then
					if
						"item_nullifier" ~= hAbility:GetAbilityName()
						and "rubick_spell_steal_custom" ~= hAbility:GetAbilityName()
					then
						return false
					end
				end
	...
	if (orderType == DOTA_UNIT_ORDER_CAST_TARGET) and params.queue == 0 then
		...
		if hAbility and hAbility.GetAbilityName then
			...
			if
				"life_stealer_infest" == ability_name
				or "doom_bringer_devour_custom" == ability_name
				or "night_stalker_hunter_in_the_night_custom" == ability_name
				or "item_iron_talon" == ability_name
				or "snapfire_gobble_up_custom" == ability_name
				or "item_hand_of_midas_custom" == ability_name
			then
				if target then
					if
						target
						and target.GetUnitName
						and (
							target:GetUnitName() == "npc_dota_roshan"
							or target:GetUnitName() == "npc_dota_nian"
							or target:HasModifier("modifier_skill_call_of_the_ancient_buff")
						)
					then
						if nPlayerID then
							local hPlayer = PlayerResource:GetPlayer(nPlayerID)
							if hPlayer then
								CustomGameEventManager:Send_ServerToPlayer(
									hPlayer,
									"SendHudError",
									{ message = "dota_hud_error_cant_cast_on_roshan" }
								)
							end
						end
						return false
						...
						if nPlayerID then
							...
							if hPlayer then
								CustomGameEventManager:Send_ServerToPlayer(
									hPlayer,
									"SendHudError",
									{ message = "dota_hud_error_cant_cast_on_ally" }
								)
							end
						end
						return false
	...
	if
		unit
		and item
		and item.GetAbilityName
		and item:GetAbilityName() == "tusk_snowball"
		and (orderType == DOTA_UNIT_ORDER_CAST_NO_TARGET or orderType == DOTA_UNIT_ORDER_CAST_POSITION)
	then
		if
			unit:HasModifier("modifier_ember_spirit_sleight_of_fist_caster")
			or unit:HasModifier("modifier_ember_spirit_sleight_of_fist_in_progress")
		then
			return false
		end
	end
	...
	if
		...
		and item
		and item.GetAbilityName
		and item:GetAbilityName() == "dark_willow_shadow_realm"
		and (
	...
	then
		...
		if player then
			CustomGameEventManager:Send_ServerToPlayer(
				player,
				"SendHudError",
				{ message = "dota_hud_error_cant_cast_on_ward" }
			)
		end
		return false
	end
	...
	if
		unit
		and unit:HasModifier("modifier_duel_buff")
		and item
		and (
