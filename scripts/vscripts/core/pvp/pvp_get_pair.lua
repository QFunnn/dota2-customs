...
function PvpModule:GetOnePair()
	...
	for i = 1, #self.PvpPairs do
		if self.PvpPairs[i] then
			if
				self.PvpPairs[i].nFirstTeamId == result.nFirstTeamId
				or self.PvpPairs[i].nSecondeTeamId == result.nFirstTeamId
			then
				self.PvpPairs[i].nTeamJoinTimes = self.PvpPairs[i].nTeamJoinTimes + 1
			end
			if
				self.PvpPairs[i].nFirstTeamId == result.nSecondeTeamId
				or self.PvpPairs[i].nSecondeTeamId == result.nSecondeTeamId
			then
				self.PvpPairs[i].nTeamJoinTimes = self.PvpPairs[i].nTeamJoinTimes + 1
			end
		end
