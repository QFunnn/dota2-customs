...
function HeroBuilder:AddAbility(playerId, abilityName, levelNumber, flCoolDown, unit)
	...
	if
		Util:GetPlayerAbilityCount(playerId) < AbilityQuota:GetTotal(playerId)
		and Util:GetPlayerAbilityCount(playerId) < 6
	then
		...
		for i, abilityName in ipairs(abilityList) do
			...
			if not ability then
				...
				if IsValid(scepterAbility) then
					...
					elseif
						string.find(
							GetAbilityKeyValuesByName(abilityName).AbilityBehavior,
...
function HeroBuilder:AddScepterShardAbility(heroIndex)
			...
			if not ability then
				...
				if scepterOrShardAbility then
					...
					if i == 1 then
						...
						Timers:CreateTimer(0.5, function()
							if
								scepterOrShardAbility
								and (not scepterOrShardAbility:IsNull())
								and scepterOrShardAbility:IsHidden()
							then
	...
	if
		hero
		and hero:IsRealHero()
		and hero:GetUnitName() == "npc_dota_hero_lycan"
