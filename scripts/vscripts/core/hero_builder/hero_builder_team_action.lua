...
function HeroBuilder:ProposeTeammateSwap(event)
	...
	if not hFirstAbility then
		return
	end
	if not hFirstAbility:GetCaster() then
		return
	end
	...
	if abilityExclusion[hSecondAbility:GetAbilityName()] then
		for _, sExclusion in ipairs(abilityExclusion[hSecondAbility:GetAbilityName()]) do
			if
				table.contains(hFirstHero.abilitiesList, sExclusion)
				and sExclusion ~= hFirstAbility:GetAbilityName()
			then
	...
	if abilityExclusion[hFirstAbility:GetAbilityName()] then
		for _, sExclusion in ipairs(abilityExclusion[hFirstAbility:GetAbilityName()]) do
			if
				table.contains(hSecondHero.abilitiesList, sExclusion)
				and sExclusion ~= hSecondAbility:GetAbilityName()
			then
				if hFirstPlayer then
...
function HeroBuilder:AcceptTeammateSwap(event)
	xpcall(function()
		...
		local sVerification = tostring(event.own) .. "_" .. tostring(event.other)
		if not self.pendingSwaps[sVerification] then
			return
		end
		...
		if
			event.PlayerID ~= secondPlayerId
			and not (PlayerResource:IsFakeClient(secondPlayerId) or secondHero.bTakenOverByBot)
		then
			return
		end
