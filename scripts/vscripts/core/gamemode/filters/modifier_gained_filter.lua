...
function GameMode:ModifierGainedFilter(keys)
	...
	if
		...
		and keys.name_const
		and (
			keys.name_const == "modifier_item_ultimate_scepter"
			or keys.name_const == "modifier_item_ultimate_scepter_consumed"
		)
	then
		...
		if hHero:IsRealHero() and not hHero:HasModifier("modifier_illusion") then
			HeroBuilderService:AddScepterAbility(hHero)
			HeroBuilderService:AddScepterLinkAbilities(hHero)
			HeroBuilderService:RegisterScepterOwner(hHero)
	...
	if keys and keys.name_const and keys.name_const == "modifier_item_aghanims_shard" then
		...
		if hHero:IsRealHero() and not hHero:HasModifier("modifier_illusion") then
			HeroBuilderService:AddShardLinkAbilities(hHero)
			HeroBuilderService:AddScepterShardAbility(keys.entindex_parent_const)
		end
	end
	...
	if
		keys.name_const
		and HeroBuilderService.attackCapabilityModifiers[keys.name_const]
		and keys.entindex_parent_const
	then
		local hParent = EntIndexToHScript(keys.entindex_parent_const)
		if hParent then
			HeroBuilderService:RegisterAttackCapabilityChanged(hParent)
		end
	end
	...
	if
		keys
		and keys.name_const
		and keys.name_const == "modifier_life_stealer_infest_creep"
		and IsValid(hParent)
		and hParent ~= nil
	then
