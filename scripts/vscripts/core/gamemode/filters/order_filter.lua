...
function GameMode:OrderFilter(orderTable)
	...
	if
		(orderTable.order_type == DOTA_UNIT_ORDER_PICKUP_ITEM or orderTable.order_type == DOTA_UNIT_ORDER_ATTACK_TARGET)
		and orderTable.queue == 0
	then
						...
						if nPlayerID then
							...
							if hPlayer then
								CustomGameEventManager:Send_ServerToPlayer(
									hPlayer,
									"SendHudError",
									{ message = "dota_hud_error_target_already_infected" }
								)
							end
						end
						return false
	...
	if (orderTable.order_type == DOTA_UNIT_ORDER_CAST_TARGET) and orderTable.queue == 0 then
		...
		if hAbility and hAbility.GetAbilityName then
			if
				"life_stealer_infest" == hAbility:GetAbilityName()
				or "doom_bringer_devour" == hAbility:GetAbilityName()
				or "night_stalker_hunter_in_the_night" == hAbility:GetAbilityName()
			then
				if orderTable.entindex_target then
					local hTarget = EntIndexToHScript(orderTable.entindex_target)
