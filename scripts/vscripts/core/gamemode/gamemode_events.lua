...
function GameMode:RegisterEventListeners()
	...
	ListenToGameEvent("player_connect_full", function(event)
		PlayerConnectionService:HandleConnect(event.PlayerID)
	end, nil)
	ListenToGameEvent("player_reconnected", function(event)
		PlayerConnectionService:HandleReconnect(event.PlayerID)
	end, nil)
...
function GameMode:OnNPCSpawned(event)
	...
	if
		spawnedUnit:GetOwner()
		and spawnedUnit:GetOwner().IsTempestDouble
		and spawnedUnit:GetOwner():IsTempestDouble()
	then
	...
	if
		spawnedUnit:GetName() == "npc_dota_companion"
		and spawnedUnit:GetOwner() ~= PlayerResource:GetSelectedHeroEntity(spawnedUnit:GetPlayerOwnerID())
	then
...
function GameMode:OnPlayerUsedAbility(event)
	...
	HeroBuilderService:RefreshAbilityOrder(event.PlayerID)
end
