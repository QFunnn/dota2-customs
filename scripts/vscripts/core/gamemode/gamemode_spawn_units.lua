...
function GameMode:SpawnNeutralUnits()
	...
	_G.FountainThinker =
		CreateModifierThinker(nil, nil, "modifier_fountain_thinker", {}, Vector(0, 0, 128), DOTA_TEAM_NEUTRALS, false)

	if not IsValid(_G.MODIFIER_EVENTS_DUMMY) then
		_G.MODIFIER_EVENTS_DUMMY =
			CreateModifierThinker(nil, nil, "modifier_events", {}, Vector(0, 0, 0), DOTA_TEAM_NOTEAM, false)
	end

	if not IsValid(_G.ABILITIES_FIX_DUMMY) then
		_G.ABILITIES_FIX_DUMMY =
			CreateModifierThinker(nil, nil, "modifier_abilities_fix", {}, Vector(0, 0, 0), DOTA_TEAM_NOTEAM, false)
	end
end