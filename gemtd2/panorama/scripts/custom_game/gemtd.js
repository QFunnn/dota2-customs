...
FindDotaHudElement('CommonItems').style['opacity'] = '0';
...
var yyyymm = new Date().getFullYear() + '' + (new Date().getMonth() >= 9 ? new Date().getMonth() + 1 : '0' + (new Date().getMonth() + 1));
$('#text_leaderboard').text = $.Localize('#'+'text_leaderboard') + ' ' + yyyymm + $.Localize('#'+'text_leaderboard2');
...
// 世界排行榜
RequestProxy('http://gemtd.ppbizon.com/gemtd/20181201/ranking/getall?hehe=' + Math.random(), function(aa){
    if (!aa || !aa['data']) {
        return;
    }
...
function OnGameStateChanged(table, key, data) {
    ...
    if (key == 'gem_life') {
        ...
        for (var p = 0; p < data.p; p++) {
            ...
            $('#player_hp' + (p + 1)).style['width'] = (table_value.gem_life / 100) * 245 + 'px';
        }
    ...
    if (key == 'gem_life_race') {
        ...
        $('#player_hp' + (data.player + 1)).style['width'] = (data.gem_life / 100) * 245 + 'px';
...
function fill_quest_board(q) {
    ...
    for (var j in q) {
        if (j == "quest_expire") {
            ...
            if (q[j] > 0) {
                var days = parseInt(q[j] / 86400);
                if (days >= 1) {
                    var hours = parseInt((q[j] - days * 86400) / 3600);
                    $("#quest_time_random").text = days + 'd ' + hours + 'h';
                    ...
                    var hours = parseInt((q[j] - days * 86400) / 3600);
                    $("#quest_time_random").text = hours + 'h';
                    $("#quest_time_random").style['color'] = '#666';
                    $("#quest_text_random").style['color'] = '#666';
                    $("#quest_image_random").SetImage('file://{images}/custom_game/quest.png');
                }
        ...
        if (q[j]) {
            if (q[j] == -999) {
                ...
                if (days >= 1) {
                    var hours = parseInt((q[j] - days * 86400) / 3600);
                    $("#quest_time_" + j).text = days + 'd ' + hours + 'h';
                    $("#quest_time_" + j).style['color'] = '#666';
                    $("#quest_text_" + j).style['color'] = '#666';
                    $("#quest_image_" + j).SetImage('file://{images}/custom_game/quest.png');
                }
